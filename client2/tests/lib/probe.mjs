...
// Which names the subject imports from a given specifier, as LOCAL -> SOURCE.
//
// 🔴 THE TWO ARE NOT ALWAYS THE SAME, and getting it wrong is silent. map_editor.js does
//    `import { snapshot as effortSnapshot } from './effort_meter.js'`: the harness knows it as
//    `effortSnapshot`, but a stub module has to EXPORT `snapshot`, because that is the name
//    the import statement asks for. Exporting the local name instead produces a module the
//    subject never reads -- no error, no warning, and every assertion that depended on the
//    stub quietly measures the real function. Measured 2026-09-03 on effort_instrument: 35
//    failures with nothing in the diff to see.
//
// A stub for a name the subject never imported is refused outright (condition 4).
function importedNames(subjectText, specifier) {
  ...
  while ((m = re.exec(subjectText))) {
    for (const part of m[1].split(',')) {
      ...
      if (!piece) continue;
      const bits = piece.split(/\s+as\s+/);
      const source = bits[0].trim();
      const local = bits[bits.length - 1].trim();
      if (local) out.set(local, source);
    }
  }
  return out;
}
...
function stubSource(specifier, names, tag, values, sourceOf) {
  ...
  // The EXPORTED name is the dependency's; the SINK is keyed by the name the harness used.
  const lines = names.map((n) => {
    const out = sourceOf.get(n) || n;
    return typeof values[n] === 'function'
      ? `export function ${out}(...args) { return SINK[${JSON.stringify(n)}](...args); }`
      : `export const ${out} = SINK[${JSON.stringify(n)}];`;
  });
