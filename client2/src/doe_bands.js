...
//
// ── 2026-07-29 추가분 ────────────────────────────────────────────────────────
//   COLOR* — `칠함*`과 **같은 줄, 같은 커밋, 같은 별표**다. 폐기된 머리줄(`tp-ch-row l1`,
//           b35bc9f)이 `<span class="drv">COLOR*</span>`와 `<span class="r drv">칠함*</span>`을
//           나란히 그렸고 7694b42에서 그 줄이 통째로 사라졌다. `칠함*`을 남겨 두는 유일한
//           근거(옛 내보내기·손으로 만든 시트가 아직 그 단어를 담고 있다)가 `COLOR*`에도
//           **똑같이** 성립하므로, 하나만 로스터에 있는 것은 근거 없는 비대칭이었다.
//           ⚠️ 앱은 이제 `*`를 어디에서도 렌더하지 않는다(측정 2026-07-29: client2/src 전체에
//           별표를 붙인 머리글 문자열이 0건). 두 항목은 **하위호환 전용**이다.
//
//   MAT · BIN · MAP · 가용 · 사용 · 사용≈ · 잔여 · 잔여≈
//         — 자재 집계표 ②의 머리줄. `ROLLUP_COLUMNS`(이 파일 아래)와 transfer_plan의
//           `tp-ch-row`가 그리는 그 단어들이고, `rollupToGrid`의 주석이 **②→① 왕복을 설계
//           의도로 선언**한다("a copied ② can be pasted into ①"). 의도가 그런데 그 머리줄
//           단어를 모르면, 사용자가 ②를 긁어 ①에 붙이는 순간 머리줄이 값 행이 된다 —
//           `COUNT`와 정확히 같은 결함이다.
//           `≈` 붙은 형태를 따로 싣는 이유: 화면이 `사용<span class="ap">≈</span>`로 그리므로
//           평문으로는 `사용≈` 한 덩어리로 도착한다. 대소문자·공백만 접는 판정기는 그 기호를
//           벗기지 않고, 여기서 벗기기 시작하면 정규화 규칙이 하나 더 생긴다.
//
// ⚠️ 이 배열은 **평평한 리터럴로 유지한다.** 계약(`contracts/doe_band_rules`)이 이 대괄호를
//    소스에서 통째로 떼어내 집합으로 대조한다 — 다른 상수를 참조하게 만들면 계약이 "집합이
//    달라졌다"가 아니라 "추출 실패"로 죽어서 무엇이 바뀌었는지 읽을 수 없게 된다.
const IGNORED_HEADERS = [
  'COLOR', 'COLOR*', '칠함', '칠함*', 'COUNT',
  'MAT', 'BIN', 'MAP', '가용', '사용', '사용≈', '잔여', '잔여≈',
];
