...
async function resolveValidDie(meta, targetTable, homeMapKey) {
  ...
  try {
    ...
    //   ③ [F8] — 아무것도 채택하지 않았다. 프레임을 그대로 두고 밀림을 «말만» 했다.
    //   ④ [규칙 ①-b, 지금] — **참조의 «규격»을 채택하고 격자를 그 규격에서 «파생»한다.**
    //      사용자 판정 2026-09-03: 「규격 격자가 당연히 맞지」. 구현은 위 `set` 의 (a) —
    //      `applyPresetObject` 가 여섯 물리값을 갈아끼우고 `applyPhysicalGeometry` 가 cols/rows 를
    //      다시 유도한다. 참조가 «선언한» 치수를 베끼는 것이 아니다 — 그것이 ②였고 거절됐다.
    ...
    // ═══ ⚠️ 이 절은 이제 **사실이 아니라 이력**이다 — 남기는 이유는 아래에 있다 ═══════
    // 「그리드 크기가 달라도 좌표는 db값 그대로 보존하고 화면 표기 밀리게 그냥 보여주기」
    // (사용자 2026-07-30). 그 지시의 «뒷줄»은 지금도 산다 — 저장 좌표는 그대로고, 마스크가
    // 밀려 보이는 것은 말로 한다. 그러나 «앞줄»—「아무것도 채택하지 않는다」—는 **거짓이 됐다**:
    // 사용자 판정 2026-09-03 「규격 격자가 당연히 맞지」 이후, 위 `set` 의 (a) 가 참조의
    // «규격»을 채택하고 격자를 그 규격에서 다시 유도한다.
    //
    // 🔴 그래도 이 문단을 안 지우는 이유: 아래 근거가 **지금도 참**이고, 그것이 ④가 무엇을
    //    감당해야 하는지를 정한다. 셀에 저장된 좌표라는 것은 **없다** — `gridData` 는
    //    `물리 키 → 값` 뿐이고, DB 의 x/y 는 Push 시점에 `cellObj.x/.y` (= 현재 프레임으로
    //    렌더가 유도한 좌표)에서 만들어진다(`pushMapData` 의 직렬화 루프). 그래서 치수를
    //    바꾸면 같은 칸이 다른 좌표를 낳는다 — 그것이 F6 이 재배치 기계장치를 만들어
    //    막으려던 것이다. ③ 는 그것을 «채택을 안 하는 것»으로 해결했고, ④ 은 다시
    //    채택하므로 **그 빚을 다시 진다** — 위 (2) 의 `reseatCellsToStoredCoords` 가 같은 호출
    //    안에서 반드시 따라붙는 이유가 그것이다.
    // ⚠️ 미판정: 새 프레임이 만들지 못하는 저장 좌표를 어떻게 할 것인가. ③ 아래에서는
    //    그런 좌표가 «생기지 않았고», ④ 아래에선 생길 수 있다. 그 판정은 아직 안 났다.
    //
    ...
    //
    // ⚠️ 그래서 이 분기는 **말도 한다** (③ 아래에선 «말만» 했다). 밀림은 원인이 화면에 남지 않는 종류의 차이라
