...
`;function o(e){if(!e||typeof e.createElement!=`function`||e.querySelector&&e.querySelector(`style[${i}]`))return!1;let t=e.createElement(`style`);return t.setAttribute(i,``),t.textContent=a,(e.head||e.documentElement).appendChild(t),!0}var s=[`both`,`outgoing`,`incoming`],c=`서버 기본`,l=(e,t,n,r)=>{let i=e.createElement(t);return n&&(i.className=n),r!==void 0&&(i.textContent=String(r)),i};function u(n,i,a){let u=a||{},d=u.apiBase||``;o(n);let f=e({apiBase:d,fetchImpl:u.fetchImpl}),p={decl:null,declState:`loading`,declReason:``,type:``,keys:{},follow:new Set,collect:new Set,direction:``,hops:``,nodeLimit:``,run:`idle`,result:null,reason:``},m=e=>String(e||``).split(`@`)[0],h=()=>p.decl&&p.decl.entities||[],g=e=>{let t=h().find(t=>t.type===e);return t&&t.keys||[]},_=()=>{let e=p.decl&&p.decl.predicates||[];return p.type?e.filter(e=>(e.subjects||[]).includes(p.type)).map(e=>e.name):e.map(e=>e.name)};function v(){if(!p.decl||!p.type||!p.collect.size)return[];let e=[];for(let t of p.collect)for(let n of r(p.decl,m(p.type),m(t)))e.push({...n,to:m(t)});return e.sort((e,t)=>e.hops-t.hops||e.follow.length-t.follow.length)}function y(){let e={type:p.type,keys:p.keys};p.follow.size&&(e.follow=[...p.follow]),p.collect.size&&(e.collect=[...p.collect]),p.direction&&(e.direction=p.direction);let t=parseInt(p.hops,10);Number.isFinite(t)&&(e.hops=t);let n=parseInt(p.nodeLimit,10);return Number.isFinite(n)&&(e.node_limit=n),e}async function b(){if(!p.type)return;p.run=`running`,p.result=null,p.reason=``,w();let e=await f(y());e&&e.ok?(p.run=`done`,p.result=e):(p.run=`failed`,p.reason=e&&e.message||`알 수 없음`),w()}function x(e){let t=l(n,`div`,`wk-field`);return t.append(l(n,`div`,`wk-label`,e)),t}function S(e){let t=x(`노드 타입`),r=l(n,`select`,`wk-select`);r.append(l(n,`option`,``,`— 고르십시오 —`));for(let e of h()){let t=l(n,`option`,``,e.type);t.value=e.type,e.type===p.type&&(t.selected=!0),r.append(t)}r.addEventListener(`change`,()=>{p.type=r.value;let e=new Set(g(p.type));p.keys=Object.fromEntries(Object.entries(p.keys).filter(([t])=>e.has(t)));let t=new Set(_());p.follow=new Set([...p.follow].filter(e=>t.has(e))),p.result=null,p.run=`idle`,w()}),t.append(r),e.append(t);let i=x(`키`),a=g(p.type);p.type?a.length||i.append(l(n,`div`,`wk-note`,`이 타입은 키가 없습니다`)):i.append(l(n,`div`,`wk-note`,`타입을 고르면 키가 나옵니다`));for(let e of a){let t=l(n,`label`,`wk-keyrow`);t.append(l(n,`span`,`wk-keyname`,e));let r=l(n,`input`,`wk-input`);r.type=`text`,r.value=p.keys[e]===void 0?``:p.keys[e],r.addEventListener(`input`,()=>{p.keys[e]=r.value}),t.append(r),i.append(t)}e.append(i);let o=x(`collect · 무엇을 가져오나`),u=h().map(e=>e.type);u.length||o.append(l(n,`div`,`wk-note`,`선언에 엔터티 없음`));for(let e of u){let t=l(n,`label`,`wk-check`+(p.collect.has(e)?` is-on`:``));t.setAttribute(`data-collect`,e);let r=l(n,`input`);r.type=`checkbox`,r.checked=p.collect.has(e),r.addEventListener(`change`,()=>{p.collect.has(e)?p.collect.delete(e):p.collect.add(e),w()}),t.append(r,l(n,`span`,``,e)),o.append(t)}if(u.length&&o.append(l(n,`div`,`wk-note`,`안 고르면 ${c} · 전부`)),e.append(o),p.type&&p.collect.size){let t=x(`경로 · 선언이 아는 길`),r=v();r.length||t.append(l(n,`div`,`wk-note`,`${m(p.type)} 에서 ${[...p.collect].map(m).join(` · `)} 로 가는 길 없음`));for(let e of r){let r=l(n,`button`,`wk-path`);r.type=`button`,r.append(l(n,`span`,`wk-pathto`,`→ ${e.to}`)),r.append(l(n,`span`,`wk-pathchain`,e.chain.join(` → `))),r.append(l(n,`span`,`wk-pathmeta`,`${e.hops}홉 · ${e.follow.join(`, `)}`)),r.addEventListener(`click`,()=>{p.follow=new Set(e.follow),p.hops=String(e.hops),w()}),t.append(r)}e.append(t)}let d=x(`follow · 어느 길로`),f=_();f.length||d.append(l(n,`div`,`wk-note`,p.type?`${p.type} 에서 나가는 술어 없음`:`선언에 술어 없음`));for(let e of f){let t=l(n,`label`,`wk-check`+(p.follow.has(e)?` is-on`:``));t.setAttribute(`data-follow`,e);let r=l(n,`input`);r.type=`checkbox`,r.checked=p.follow.has(e),r.addEventListener(`change`,()=>{p.follow.has(e)?p.follow.delete(e):p.follow.add(e),w()}),t.append(r,l(n,`span`,``,e)),d.append(t)}f.length&&d.append(l(n,`div`,`wk-note`,`안 고르면 ${c}`)),e.append(d);let y=x(`걸음`),S=l(n,`label`,`wk-keyrow`);S.append(l(n,`span`,`wk-keyname`,`direction`));let C=l(n,`select`,`wk-select`);C.append(l(n,`option`,``,c));for(let e of s){let t=l(n,`option`,``,e);t.value=e,e===p.direction&&(t.selected=!0),C.append(t)}C.addEventListener(`change`,()=>{p.direction=C.value}),S.append(C),y.append(S);for(let[e,t,r,i]of[[`hops`,`hops`,1,40],[`node_limit`,`nodeLimit`,10,5e3]]){let a=l(n,`label`,`wk-keyrow`);a.append(l(n,`span`,`wk-keyname`,e));let o=l(n,`input`,`wk-input`);o.type=`number`,o.min=String(r),o.max=String(i),o.placeholder=c,o.value=p[t],o.addEventListener(`input`,()=>{p[t]=o.value}),a.append(o),y.append(a)}e.append(y);let T=l(n,`button`,`wk-go`,p.run===`running`?`걷는 중`:`날리기`);T.type=`button`,T.disabled=!p.type||p.run===`running`,T.addEventListener(`click`,b),e.append(T)}function C(e){if(p.run===`idle`)return;let t=l(n,`div`,`wk-result`);if(p.run===`running`)t.append(l(n,`div`,`wk-note`,`걷는 중`));else if(p.run===`failed`){let e=l(n,`div`,`wk-fail`);e.append(l(n,`b`,``,`실패`),l(n,`span`,``,` · `+p.reason)),t.append(e)}else if(p.result){let e=p.result;if(t.append(l(n,`div`,`wk-counts`,`노드 ${e.nodes.length} · 엣지 ${e.edges.length}`)),e.walk&&t.append(l(n,`div`,`wk-walk`,`요청 ${e.walk.hops_requested}홉 · 도달 ${e.walk.hops_reached}홉 · ${e.walk.direction}`)),e.cut&&t.append(l(n,`div`,`wk-trunc`,`절단됨 · ${e.truncated.reason}`)),e.nodes.length){let r=new Map;for(let t of e.nodes){let e=t.type||`—`;r.set(e,(r.get(e)||0)+1)}let i=l(n,`div`,`wk-dist`);i.append(l(n,`span`,`wk-distlabel`,`타입`));for(let[e,t]of[...r.entries()].sort((e,t)=>t[1]-e[1])){let r=l(n,`span`,`wk-distchip`+(p.collect.has(e)||p.collect.has(`${e}@1`)?` is-asked`:``));r.append(l(n,`b`,``,e),l(n,`span`,``,` ${t}`)),i.append(r)}t.append(i)}e.nodes.length||t.append(l(n,`div`,`wk-note`,e.message||`닿은 노드 없음`));for(let r of e.nodes.slice(0,200)){let e=l(n,`div`,`wk-row`);e.append(l(n,`span`,`wk-rowtype`,r.type||`—`)),e.append(l(n,`span`,`wk-rowlabel`,r.label||r.id||``)),t.append(e)}e.nodes.length>200&&t.append(l(n,`div`,`wk-note`,`이 아래 ${e.nodes.length-200} 개 안 그림`))}e.append(t)}function w(){i.textContent=``;let e=l(n,`div`,`wk-form`);if(p.declState===`loading`)e.append(l(n,`div`,`wk-note`,`선언 · 읽는 중`));else if(p.declState===`failed`){let t=l(n,`div`,`wk-fail`);t.append(l(n,`b`,``,`선언 못 읽음`),l(n,`span`,``,` · `+p.declReason));let r=l(n,`button`,`wk-go`,`다시`);r.type=`button`,r.addEventListener(`click`,T),e.append(t,r)}else S(e),C(e);i.append(e)}async function T(){p.declState=`loading`,w();let e=await t({apiBase:d,fetchImpl:u.fetchImpl});e&&e.ok?(p.decl=e,p.declState=`ready`):(p.declState=`failed`,p.declReason=e&&e.message||`알 수 없음`),w()}return T(),{state:p,spec:y,fire:b,render:w}}if(typeof document<`u`){let e=document.getElementById(`wk-host`);e&&n(async()=>{let{API_BASE:e}=await import(`./config-C6wMXuF6.js`);return{API_BASE:e}},__vite__mapDeps([0,1])).then(({API_BASE:t})=>{u(document,e,{apiBase:t})})}