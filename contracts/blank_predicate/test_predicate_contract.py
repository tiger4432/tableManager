...
# ---------------------------------------------------------------------------
# ⚰️ [S-283] THE RESOLUTION SEAM IS GONE, AND SO ARE ITS THREE VECTORS
# ---------------------------------------------------------------------------
# This block held the seam between `_resolve_one` (which decided a joined cell in Python,
# for the payload the grid painted) and `resolved_expression` (which decided the SAME cell
# in SQL, so it could go in a WHERE clause). Two implementations of one answer is exactly
# what a contract vector is for, and this one ran the whole blank corpus through both.
#
# 🔴 BOTH HALVES WERE DELETED WITH THE READ-TIME JOIN (ruling 461). A contract needs TWO
# implementations; there is no second one left to disagree. Keeping the vectors would have
# meant keeping the engine alive to be measured, which is the tail wagging the dog.
#
# ⚠️ WHAT IS NOT LOST. The predicate itself - `is_blank_value` / `clean_str_value` and the
# SQL/Python agreement about what counts as blank - is still pinned by every other section
# of this file; only the JOIN's use of it went away. If a read-time computed column is ever
# declared again, this seam comes back with it, and this comment says what it has to prove.
