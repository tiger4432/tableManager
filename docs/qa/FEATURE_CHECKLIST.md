...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-13 (2차 배치 — **§1.13 신설: 정준 원장 + void 스키마 인벤토리 4행**. 🔴 **원장 L2는 「빈 원장에 물어도 홉이 하나 나오는가」가 점검의 핵심**이고, **L3는 「관례 홉과 실측 홉이 반대로 표시되지 않는가」**다(경합 홉을 **실제로 만들어** 볼 것 — 추론으로는 안 나타난다). 🔴 **void는 「깨끗한 스캔이 런 행을 만드는가」** — 그 행이 이 설계의 존재 이유이고 그것을 잃는 결함이 실제로 있었으며 **무음이었다.** 직전 **§P2 인제션에 tier-1 끌어올리기 5항** — `831ab68`. 🔴 **콜드 스윕 대조군 없이 초록으로 읽지 말 것** · 🔴 **걸러진 파일도 «이동»은 갚아야 한다**(회귀가 난 자리). **드리프트 부팅 점검에 자기 수리 5항** — `eb700e5`. 🔴 **시스템 테이블은 강등되지 않는다**가 대조 항목이고, **타입 불일치가 안 잡히는 것은 정상**이다) | **Owner:** Integrity/QA | 갱신 **doc-keeper 전담** · 정합 감사 **doc-auditor**
>
