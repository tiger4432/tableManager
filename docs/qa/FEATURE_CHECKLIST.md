...
>
> **유지 규율:** 새 기능이 병합되면 코드맵 갱신과 같은 사이클로 §1 기능 행과 §2 점검 항목을 넣습니다.
> **사용법:** §1은 「무엇이 있는가」(기능 지도), §2는 「어떻게 확인하는가」(릴리스 전·회귀 수동 점검).
> 체크박스는 점검 회차마다 복사해 씁니다.
>
> ⚠️ **이 문서가 재는 것은 «지금 있는 것»입니다.** 은퇴한 것을 재는 항목은 점검자에게
> 「제품이 고장 났다」고 말합니다 — 가장 비싼 종류의 거짓말입니다.

---

## 1. 기능 인벤토리 (서브시스템별)

> 진입 경로의 페이지: 메인 그리드 `/`(index.html) · 어드민 `/admin.html` · 맵 에디터 `/map_editor.html` · 맵 정렬 `/map_editor2.html` · R&D 보드 `/rnd-board.html`(nav 링크 «없음» — 주소로 연다). ⚠️ `graph.html`·`trace.html` 은 «묘비» 페이지이고 점검 대상은 §1.9 한 줄뿐이다.
...
| ~~**맵 메타 자동 등록 (M3, 2026-07-29 `ab6ac02`)**~~ | ⚰️ **2026-09-07 은퇴**(S-38, 소유자 지시). 인제션은 `wafer_map_metadata` 행을 만들지 않습니다. 관문은 `tests/test_map_meta_registrar.py` 의 「두 배선 자리에서 메타 행 0」 둘입니다 |
...
### 1.8 어드민 대시보드 (`/admin.html`) — 파이프라인 생애주기 탭

탭 축은 **파이프라인 생애주기 탭**(`#overview/#file/#chain/#autoupdate/#enrichment/#ontology`) + 코드 에디터 **공용 뷰**(`#editor=<path>` 딥링크). 구 해시 별칭(`#outbox→#chain` 등) 호환 유지.
...
| **소급 적용(backfill) 어드민 표면 — ✅ [2026-08-31] 화면이 있다** | CLI에만 있던 소급 연산들에 **인벤토리·건수·실행** 라우트가 생겼고(2026-07-31 `fbc1053`), **2026-08-31에 화면과 실행 등록부가 착지했다**: 어드민 **Overview** 탭의 소급 블록(연산 선택 · 파라미터 폼 · 건수 · 실행)과 **도는 실행 목록 + 취소**. 🔴 **종전 이 행의 「화면은 아직 없다」·「소급 5종」은 둘 다 거짓이 됐다** — 등재 연산에 원장 쪽 둘(`ledger_backfill`·`ledger_rescope`)이 들어왔고 **수는 여기 적지 않는다**(정본은 `GET /admin/retroactive/operations`). 🔴 **취소는 «협조적»이다** — 프로세스를 죽이지 않고 `retroactive_runs` 행에 값을 세우고 도는 쪽이 배치 사이에서 묻는다. **`cancellable` 이 연산마다 선언되고 거짓이면 버튼이 «안 그려진다»**. 🔴 **`cancel_requested` 와 `cancelled` 는 다른 상태다.** 🔴 **인벤토리·건수 응답이 `deletes`·`restartable`·`commit_granularity`를 실어 나른다**: 확인 문구 하나로 다섯 버튼을 덮으면 그 하나가 틀리기 때문이다(넷은 청크 커밋이라 이어서 재실행되지만 **고아 스윕만 삭제 루프가 끝난 뒤 한 번 커밋**해 중단 시 전부 롤백). 🔴 **모든 건수가 `count_kind`를 함께 답한다**(`exact`/`sample`/`upper_bound`) — 다섯 중 넷은 요청 경로에서 정확할 수 없고 **어느 것도 정확한 척하지 않는다**. 실행은 **아웃박스 한 줄 + 즉시 반환**이고 실제 실행은 스케줄러 전용 스레드(동시 1건). **새 연산은 하나도 구현하지 않았다** — 카운트는 각 연산 자신의 dry-run, 실행은 같은 함수의 `apply=True` | 진입: `curl -H "X-Admin-Token: …"` → `GET /admin/retroactive/operations` · `.../{op}/count` · `POST .../{op}/run`(**strict 토큰**) — 코드: `server/admin/retroactive.py`(등록부) · `run_auto_update.start_retroactive_run` · 점검 §2.8-quinquies · [BACKFILL_GUIDE §7](../guide/BACKFILL_GUIDE.md) · [backend §2](../architecture/backend.md) · [AUTO_UPDATE_GUIDE §4-quater](../guide/AUTO_UPDATE_GUIDE.md) |
...
>
> 🔴 **[2026-09-05] 이 자리에 있던 「대신 점검할 것 하나 — 은퇴가 정직하게 보이는가」 다섯 항목은 «돌리지 마십시오» — 점검 대상이 사라졌습니다.**
> 소유자가 「갈래를 통째로」라 판정해 묘비까지 삭제됐습니다(`8ffe23d7` · 화면은 `45d8b66f`).
>
> ```
> 🗄️ 믿지 말 것   410 · reason "old_graph_branch_retired" · successor 필드 · Cache-Control: no-store
>                graph.html 묘비 · 「🕸️ 추적」 버튼 · test_graph_branch_retired.py
> ✅ 오늘 실제 모습   GET /graph/stats -> index.html + 200 (SPA catch-all)
>                POST /api/graph/sync -> 404 (/api 가드)
>                graph.html · trace.html -> 없음
> ```
>
> ✅ **남은 점검은 «하나»입니다** — **재기동 후에도 저장소 표 셋이 다시 생기지 않는다.** ⚠️ 기제가 바뀌었습니다 — 종전엔 「부활 경로 셋을 봉인다」였고, 지금은 **모델 클래스와 `ensure_graph_tables` 가 함께 제거**돼 요구 자체가 없습니다([data_model §1.1](../architecture/data_model.md)). 그래도 돌리는 이유는 이 점검이 «은퇴가 「아직 안 됨」의 옷을 입는 것»을 막기 때문입니다.
> ✅ **살아남는 생각** — 「은퇴는 상태코드가 아니라 구조화된 `reason` 으로 말한다」·「`successor` 를 «그대로 호출»해 봐야 한다 — 이름 대조로는 «이름은 그럴듯한데 없는» 주소를 못 잡는다(2026-08-27 실측: 그 상태로 스위트 열둘을 초록 통과)」. **다음에 묘비를 세우는 자리에서 다시 씁니다** — [PRIMITIVES §6](../architecture/PRIMITIVES.md).
...
> 🔴 **2026-08-27에 전수로 다시 썼습니다.** 종전 행 가운데 여섯이 «없는 것»을 재고 있었습니다 —
> 라우트 다섯(`/trace` · `/coverage` · `/journey` · `/compare` · `POST /actions`)과 삭제된 페이지
> (`/ledger.html`) 하나. 아래는 라우트 표에 대고 센 것입니다.

**기둥 둘** — 이 절의 모든 행이 그 위에 섭니다:
① 원장은 «선언» 위에 서 있다 (선언 파일 하나가 entities · vocabulary · sources 를 정한다)
② 답은 «walk» 이 한다 (마킹에서 걸어 서브그래프를 만든다)
...
| **L4 선언 조회** | `GET /api/ledger/declaration` — 원장을 한 줄도 안 읽는다. 점검: ① 원장이 비어 있어도 답하는가 ② 🆕 **[2026-08-31] `sources[]` 절**이 `{source, relation, emits[], scope_columns[]}` 로 오는가 ③ 🔴 **선언을 못 읽는 상황을 만들면 `sources` 가 «빈 배열»이 아니라 «키째 없어야»** 한다(부재 = 「모른다」, 빈 배열 = 「없다」 — 접으면 그리드가 「이 표는 소스가 아니다」라는 거짓을 말한다) | `/api/ledger/declaration` |
| 🆕 **L4-bis 결측 카탈로그** | `GET /api/ledger/gaps` — 「무엇이 아직 없나」. 점검: ① **인자 없이 부르면 DB 에 안 닿는다**(DB 를 내려도 답하면 정상) ② `?name=<이름>` 은 «잰다» — `count_kind`(`exact`/`sample`)와 부재 어휘가 함께 오는가 🔴 **수만 있고 종류가 없으면 결함**(「정말 없음」·「표본이 다 못 봄」·「해당 없음」이 같은 0 으로 보인다) ③ 없는 이름은 **404** ④ 🔴 **이름 표와 선언을 어긋나게 만들면 «양방향으로» 503 `gap_table_mismatch`** — 술어를 선언에서 지웠는데 표에 남아 있어도, 표에 없는 술어가 선언에 있어도 거절해야 한다. ⚠️ **질문 «개수»를 단언하지 마십시오** — 하루 저녁에 열하나에서 열다섯으로 자랐고 시험도 개수를 안 셉니다 | `/api/ledger/gaps` |
| ⚰️ ~~**L5 구조 뷰**~~ · ~~**L6 발견 종류**~~ · ~~**L7 또래 대조**~~ · ~~**L8 시계열·맵·구성**~~ | 🔴 **[2026-08-28] 네 항목이 대던 라우트가 «전부» 없다** — `structure` · `kinds` · `siblings` · `trends` · `lot_map` · `composition`. **이 행들을 점검하면 없는 것을 「고장」으로 신고하게 된다** (이 표 §1.13 배너가 경고하는 바로 그 모양이다). 이 주소들은 410 이 아니라 SPA 폴백의 HTML 200 을 답하므로 «성공처럼» 보인다. 오늘의 원장 읽기 점검은 **L3 · L4 · L4-bis 셋**이다(2026-08-31 재실측 — `gaps` 가 붙었다. **수의 정본은 [backend §2](../architecture/backend.md)**) | — |
...
⚠️ **지어내지 마십시오 — 이 다섯은 «없습니다»** (실측 2026-08-27, 라우트 표 대조):
`/api/ledger/trace` · `/api/ledger/coverage` · `/api/ledger/journey` · `/api/ledger/compare` ·
`POST /api/ledger/actions`. 그것들을 재는 항목을 만들면 점검자가 «없는 고장»을 올립니다.

#### A1 선언 작성 표면

- [ ] `GET /admin/ledger/sources` 가 `kinds`·`sources`·`config_path` 를 낸다. 🔴 `config_path` 가 있어야 운영자가 라이브 선언과 샘플을 가린다
- [ ] 선언을 읽을 수 없을 때 `error` 가 채워지고 **500 이 아니다** — 선언이 깨졌을 때 여는 화면이 그것 때문에 안 뜨면 안 된다
- [ ] `POST /admin/ledger/dry-run` 이 잘못된 선언을 «이름 붙은» 거절로 돌려준다 (`unknown_relation`·`unknown_column`·`invalid_identifier` …)
- [ ] 그다음 단계는 `declaration_rejected` 로 거절된다 — **문장이지 500 이 아니다**
- [ ] 선언 편집은 `/admin.html` 의 **ontology 탭**에서 초안 → 검토 → 활성화로 돈다. 🔴 `/admin/ledger/` 아래에 쓰는 라우트는 없다
- [ ] `pytest server/tests/test_ledger_source_contract.py` 가 통과한다

### 1.15 R&D 진단 보드 (`/rnd-board.html`)

> ⚠️ **nav 링크가 없습니다** — 어느 화면에서도 안 걸리고 주소로 직접 엽니다.
...
| **mapper가 선언 이름을 모른다 — 문장 «별명»** (2026-08-19 `71865b7`·`77cf39a` · 🔴 **2026-08-21 `e795c706`으로 «구조 탐색»이 «별명 조회»가 됐다**) | `SentenceShape`/`ProfileSentences`. mapping은 이제 **mapper가 선언한 문장 별명으로 키가 매겨지고**(`bind.mappings.<별명>`) 해석은 dict 조회 한 번이다. 점검: ① 🔴 **predicate·entity type 철자가 mapper 파일에 «없는가»**(`grep`) — 하나라도 있으면 「다른 스키마 환경에서 코드 0줄」이 깨진다(별명은 mapper의 낱말이라 **있어야 맞다**) ② 🔴 **config의 별명 키를 하나 오타 내면 `unknown_mapping`으로 이름을 대며 거절되는가** — 조용히 다른 문장으로 흐르면 안 된다 ③ 한 `SentenceShape` 인스턴스를 두 속성에 바인딩하면 클래스 생성 시점에 `ambiguous_sentence_shape`인가 ④ 별명을 안 준 shape(클래스 속성에 안 묶인 것)를 쓰면 `unnamed_sentence`인가 ⑤ 🔴 **`validate_role_frame`의 `claim_ref`·`emits` 비교 둘을 «지우지 않았는가»** — 지금은 양쪽이 같은 `use`에서 나와 항등식이지만, 그 함수는 자기가 만들지 않은 프레임을 받는 **경계**다. ⚰️ **은퇴한 점검**: 「모양이 같은 mapping 둘」·선택 필드 `sentence`·`ambiguous_sentence` — 별명이 키라서 **중복을 적을 수가 없다**(`_sentence_signature`·`_ambiguous_sentences`·`has_object`가 함께 삭제됐다) | `pytest server/tests/test_ledger_setup_*.py` | `server/ledger/roleframe.py`(`ProfileSentences._resolve`·`validate_role_frame`) · `server/mappers/ledger_v2_lot_event_role_mapper.py` · [ONTOLOGY_LEDGER_SETUP §7.6](../guide/ONTOLOGY_LEDGER_SETUP.md) |
...
| 헬스 엔드포인트 | **항상 JSON**, 정상 200 / `unhealthy` 503. `checks{database, workers, outbox, supervisor}` + 사람이 읽는 `problems[]`. DB 프로브 2초 타임아웃·중복 프로브 차단 | `GET /health` | `server/runtime/health.py` · `main.py` |
...
| 기능 | 설명 | 진입 경로 | 코드 |
|---|---|---|---|
| 어드민 공유 토큰 게이트 | `/admin/*` **API 라우트 전부**(2026-07-31 실측 **22개** — `@app.<verb>("/admin` 세기, 페이지 서빙 2개 제외)가 `ASSY_ADMIN_TOKEN` 환경변수 + `X-Admin-Token` 헤더 필요(비교 `secrets.compare_digest`). ⚠️ **이 수는 admin 라우트가 추가되는 커밋마다 낡는다** — 커버리지 판정의 정본은 수가 아니라 **`test_admin_auth.py`가 FastAPI 라우트 테이블을 열거하는 단언**이다. **조회도 포함** — 소스 코드 반환·파이프라인 열거도 유출이다. 미제시 **401**, 불일치 **403**. 예외는 페이지 서빙 `GET /admin`·`/admin.html` 2개(브라우저 내비게이션이라 헤더를 붙일 수 없고, 표시 데이터는 전부 게이트된 JSON에서 온다) | 서버 환경변수 | `server/admin/auth.py` · [backend §API](../architecture/backend.md) · [DEPLOY_SETUP §1-4](../guide/DEPLOY_SETUP.md) |
...
- [x] ~~**맵 메타 자동 등록 — 양방향 토글 / absent-only 불변식(M3 `ab6ac02`)**~~ ⚰️ **2026-09-07 은퇴**(S-38): 켤 손잡이가 없고 쓰는 자리도 없습니다. 회귀는 자동 시험이 잡습니다(`tests/test_map_meta_registrar.py`).
...
- [ ] ⚰️ **[2026-09-09 S-82] 이 항목은 «점검 대상이 아닙니다»** — 제품 문(`PUT /tables/{t}/data/updates`)이 인제션·체인과 «같은 상수·같은 함수»로 축약을 켭니다. 🔴 **다만 이 항목이 검증하던 «진짜 성질»은 남습니다** — 바로 아래로 옮겼습니다. ~~그리드 셀 편집·맵 Push → 그 tx의 outbox 행이 **행마다 1건**이고 `data`를 나른다~~
- [ ] 🎯 **사람 교정이 «화면에» 닿는다**: 클라이언트 둘을 띄우고 한쪽에서 그리드 셀 편집·맵 Push → **다른 쪽 화면**에 그 행이 반영된다. 🔴 **안 닿으면 즉시 NO-GO다**(핵심가치 #3). ⚠️ 아웃박스의 «모양»은 이 성질의 대리가 아닙니다 — 실측 2026-09-09: 이 문의 브로드캐스트는 `crud.apply_batch_updates` 의 «반환값»으로 짓고, 복구 스윕은 `table_name` 으로 표 단위 새로고침만 쏩니다. 대리를 재던 것이 위 되물린 항목입니다.
...
- [ ] **워크리스트 정상**: 원본 인제션(결손 target 포함) → 어드민의 **Enrichment 탭** → 규칙 선택 → 결손 판단키만 목록에 표시(dedup — 원본 5행 → 유니크 3행 등 압축 확인).
...
### 2.8-quinquies 소급 적용 어드민 표면 🎯 (2026-07-31 `fbc1053` · **2026-08-31 화면 + 실행 등록부 착지**)

> ✅ **[2026-08-31] 화면이 있습니다 — 종전 서두의 「화면이 없습니다, 이 절은 전부 `curl`」은 거짓이 됐습니다.** 어드민 **Overview** 탭의 소급 블록입니다. `curl` 항목도 그대로 유효하니 **둘 다** 점검하십시오.
...
- [ ] 🎯 **[신설] 못 멈추는 연산은 버튼이 «없다»**: 인벤토리에서 `cancellable: false`인 항목(`ledger_rescope`·`enrichment_confirm`)은 화면에 **취소 버튼이 그려지지 않는다**. 🔴 **눌러도 아무 일 없는 버튼이 있으면 결함**이다 — 그것이 이 선언이 존재하는 이유다.
- [ ] **[신설] 그리드에서 넘긴 범위가 «한 번만» 먹힌다**: 그리드에서 행을 골라 범위를 어드민으로 넘긴 뒤, 어드민을 **다시 열면** 그 범위가 **비어 있다**. 🔴 남아 있으면 결함이다 — 운영자가 「지금 고른 적 없는」 범위를 자기가 고른 것으로 읽는다. 그리고 **주소창에 범위 값이 안 보인다.**
...
- [ ] **탭 전환**: Overview/File/Chain/AutoUpdate/Enrichment/Ontology 여섯 탭 모두 렌더 + 콘솔 에러 없음. 해시 라우팅(`#file` 등) 직접 진입 동작, 구 별칭(`#outbox`)이 Chain 탭으로 리다이렉트.
...
> 🔴 **아래 절차는 전부 실패합니다** — 저장소가 DROP되고 라우트도 화면도 «없습니다»(⚠️ **[2026-09-05] 종전 이 자리의 「라우트가 410입니다」는 거짓입니다** — 거절 스텁까지 삭제돼 `/graph/*` 는 index.html + 200 입니다). 이 절 대신 **§1.9의 「은퇴가 정직하게 보이는가」 다섯 항목**을 돌리십시오.
...
**스키마 드리프트 부팅 점검 (2026-08-05 `f6406b1` · `server/admin/schema_drift.py`)** — 「마이그레이션이 돌아야 한다」는 배포 사실을 **아무도 안 나르고 있었다.** 개발·테스트 박스에서는 절대 안 보이고 **오직 운영 화면에서만** 드러난다.
...
**인자 관문 (2026-08-04 `63b17f7` · `server/runtime/launcher_args.py`)** — 🔴 **종전 이 자리는 *「`--help`가 없고 오타는 조용히 무시되므로 철자를 확인할 것」*이라고 적고 있었고, 그 문장은 이제 거짓이다.** 그대로 두면 점검자에게 **고쳐진 결함을 통과시키라고** 지시하는 셈이다.
...
- [ ] 🎯 **traversal은 404다**(토큰 설정 여부와 무관 — **인증 없이** 확인할 것):
  ...
  curl -si --path-as-is "$H/../../server/admin/auth.py"             | head -1   # 404
  curl -si --path-as-is "$H/%2e%2e%2f%2e%2e%2fserver/admin/auth.py" | head -1   # 404 (인코딩 변형)
  curl -si "$H/index.html"                                          | head -1   # 200 (정상 서빙은 살아 있어야 한다)
  ```
  ⚠️ **상태코드만 보지 말고 본문을 볼 것.** 200에 SPA HTML이 오는 것은 catch-all의 정상 동작이고, **파일 내용이 오면 실패**다. 위 4종 외에 `..%5c`(백슬래시)·절대경로 `/C:/Windows/win.ini`·드라이브 상대 `C:server/admin/auth.py`도 던진다 — **문자 denylist로 막은 구현이라면 바로 여기서 갈린다.**
...
- [ ] **나머지는 열려 있다**: 탭 전부 정상 렌더 + 토큰 프롬프트가 **뜨지 않음**. (첫 재기동에 운영자가 어드민 전체에서 잠기지 않게 한 의도된 상태다.)
