...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-11 | **Owner:** Integrity/QA | 갱신 **doc-keeper 전담** · 정합 감사 **doc-auditor**
>
...
- [ ] 🎯 **셀 이력이 비는 행 (2026-08-11 — 서버는 착지, 화면은 미착지)**: 인제션·체인으로만 채워진 행(예: 파일로 방금 적재한 행)의 아무 셀이나 골라 **Cell History** 탭 → **비어 있다.** 같은 행 **Row History** 탭 → **`ROW_UPDATE` 기록이 보인다.** 이것은 이력 유실이 아니라 **기계 쓰기가 셀 단위로 기록되지 않기 때문**이다(감사 94.08%가 `ROW_UPDATE` — [backend §이력/감사](../architecture/backend.md)). ✅ **서버 확인**: `GET /tables/{t}/rows/{id}/cells/{col}/history` 응답에 `row_history_total`이 실리고 그 값이 행 탭 건수와 같다. ❌ **화면은 아직** `No change history recorded.`로 그린다 — 두 상태(진짜 이력 없음 / 이 화면이 못 보여줌)를 구별해 그리는 것은 **클라 후속 작업**이며, 그때 이 항목의 ❌를 걷는다.
