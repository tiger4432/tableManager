...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-15 3차 (**§1.13에 A2 추가 — 「코드 0줄로 테이블 하나 붙이기」 + 뿌리 키 롤업.** 🔴 **A2의 두 축은 「정말 코드가 0줄인가」와 「오타 난 `when` 연산자가 조건을 «항상 참»으로 만들지 않는가」다** — 뒤엣것의 실패는 거절이 아니라 **조용한 초과 발화**라 초록으로 위장한다. 함께 **A1 ⑫의 전제를 정정**했다: 「등재한 낱말을 발화할 번역기가 없다」는 더 이상 참이 아니고, 이제 **양팔**(선언 전엔 부재를 말하는가 / 선언 후엔 그 문장이 스스로 사라지는가)로 채점한다) · 직전 2026-08-14 밤 2차 (**§1.13에 J1 추가 — 여정 대조(주어 «둘» 전용 읽기)**. ⚠️ **서버만 착지했고 화면이 없어 REST로만 채점한다.** 🔴 **J1의 핵심은 「무엇이 «없는가»를 확인하는 것」**이다 — 집단 통계는 `null`이 아니라 **키 부재**여야 하므로 최상위만 보지 말고 **응답 전체를 재귀로** 훑을 것, 그리고 **정상 쌍만 태우면 arity 거절 축이 채점되지 않는다**(오타·셋을 섞어라). 🔴 **빈칸 셋을 «따로» 만들어라** — 「구간을 안 걸음」·「걸었는데 기록 없음」·「측정된 `0`」이 한 낱말로 오면 결함이다. ⚠️ **`recorded_null`은 이 박스에 표본이 0이니 안 나오는 것을 결함으로 읽지 말 것.** 직전 — **§1.13에 C1 추가 — 마킹 대조 + 놀라움 표**(`66e2925`·`5ea29b6`·`f21a916`·`60c7c93`). 🔴 **C1의 핵심은 「scope가 못 찾은 값을 흡수하지 않는가」와 「마킹 집합이 곧 비교 모집단인가」**다 — 실재+오타 랏을 물어 오타가 이름 불리는지, 웨이퍼 축 마킹이 랏으로 불리지 않는지, 수치 후보가 주어당 한 값으로 접히는지. 그리고 **잘림은 서버 플래그가 없어도 `shown < scored`에서 울려야 한다.** 직전 4차 — **§1.13에 L6 추가 — 3축 맵의 코어축이 처음 그려진다**(`50a21c7` · **라우트 코드 0줄**). 🔴 **이 항목의 핵심은 「초록이 아니라 «음성 케이스»를 확인하는 것」**이다: 코어 컬럼을 가진 것은 본딩 랏 **108개 중 24개**뿐이고 안 심은 랏은 여전히 `unreachable`/`no_live_bridge`여야 한다 — 축이 항상 `ready`가 되면 「없으면 없다고」 말하는 능력을 잃는다. 🔴 **그리고 「다대다를 지원한다」고 쓰면 «거짓»이다** — 본딩→DT는 25 고정(변이 0)이라 「many」를 하드코딩한 소비자와 세는 소비자가 구별되지 않는다. 🔴 **`dt_map.core_lot`은 여전히 전 행 NULL**(선언만 고쳐졌고 다음 체인 패스가 채운다). 직전 2026-08-14 3차 — **§1.13에 L5 추가 — 결함 관측의 원장 번역 + 걷기 선언**(R-2026-08-14-D·E). 🔴 **L5의 핵심은 「빈 원장에서 태우면 아무것도 증명 못 한다」**이다: `observed`가 걷기에서 빠졌는지는 **관측 원자를 채운 상태에서** 추적을 태워야 판별되고, 인출 대상이 없을 때는 두 철자가 같은 답을 낸다. 🔴 **그리고 「도착 시각과 세상 시각을 섞지 않았는가」** — 관측 행의 `updated_at`은 도착이고 발견의 순간은 런에 있다. 그 둘이 섞이면 원자는 여전히 well-formed하고 **아무것도 항의하지 않는다**(시간대 사고와 같은 계급). ⚠️ **첫 호출 225 ms는 워밍업이라 회귀로 읽지 말 것.** 직전 2026-08-14 — **§1.13에 두 행 추가 — L4 공정·레시피 어휘 · F1 결함 종류 레지스트리**. 🔴 **L4의 핵심은 「초록만으로는 아무것도 증명 못 한다」**이다: 「실측이 설정값을 이긴다」는 랭킹 코드 0줄로 성립하므로, **계급 경계를 뺀 뮤턴트가 «반대» 답을 내는지**까지 봐야 그 초록이 계급에 대한 것이 된다(같은 답이면 산술에 대한 단언이다). 그리고 **`0`·`False`를 따로 태워라** — 존재 검사를 진리값 검사로 잘못 쓰면 **그 둘만** 거절되어 온전한 payload 하나로는 두 철자가 구별되지 않는다. 🔴 **F1의 핵심은 `kind_clean`이 「스캔됨 MINUS 발견됨」인가** — `NOT EXISTS`로 쓰면 **한 번도 안 본 280,001개**가 깨끗한 쪽으로 간다. 그리고 **종류를 바꿨을 때 분모가 실제로 움직이는지**를 봐야 한다(두 종류가 method를 공유하면 초록이 일반화의 증거가 못 된다). 직전 2026-08-13 5차 배치 — **§1.13에 원장 두 행 추가**: **L1-bis 「거절이 반쪽을 남기지 않는다」**(`f313279`+`eb1ae8b` — 🔴 **거절 팔에서 «셋을 따로»** 단언할 것(원자 0 · 세어짐 · 이름 대어짐, 어느 하나는 **틀린 이유로도 참일 수 있다**) · 🔴 **대조군 런이 먼저**여야 「0개」가 뜻을 갖는다 · ⚠️ **공유 하네스 둘이 `AssertionError`를 성공으로 쳐서 가드가 꺼진 채 초록이던 항목이 있었다**)와 **L2-bis 「`/coverage`와 거절 내역」**(`0198e7e` — 🔴 **`refusals_unaccounted`는 «부호»로 읽고 `1`은 정상이다** · 🔴 **마이그레이션 안 돌린 상태에서 500이 아니어야** 하고 그때 키가 **아예 없어야** 한다 · ⚠️ **`atoms.exact`는 언제나 false**). 직전 2차 배치 — **§1.13 신설: 정준 원장 + void 스키마 인벤토리 4행**. 🔴 **원장 L2는 「빈 원장에 물어도 홉이 하나 나오는가」가 점검의 핵심**이고, **L3는 「관례 홉과 실측 홉이 반대로 표시되지 않는가」**다(경합 홉을 **실제로 만들어** 볼 것 — 추론으로는 안 나타난다). 🔴 **void는 「깨끗한 스캔이 런 행을 만드는가」** — 그 행이 이 설계의 존재 이유이고 그것을 잃는 결함이 실제로 있었으며 **무음이었다.** 직전 **§P2 인제션에 tier-1 끌어올리기 5항** — `831ab68`. 🔴 **콜드 스윕 대조군 없이 초록으로 읽지 말 것** · 🔴 **걸러진 파일도 «이동»은 갚아야 한다**(회귀가 난 자리). **드리프트 부팅 점검에 자기 수리 5항** — `eb700e5`. 🔴 **시스템 테이블은 강등되지 않는다**가 대조 항목이고, **타입 불일치가 안 잡히는 것은 정상**이다) | **Owner:** Integrity/QA | 갱신 **doc-keeper 전담** · 정합 감사 **doc-auditor**
>
...
- [ ] `GET /admin/ledger/sources`의 각 kind가 실제 translator profile을 함께 제공한다.
- [ ] 표본에 나오지 않은 가능한 Claim 충돌도 source 행을 읽기 전에 `translator_vocabulary_mismatch`로 거절된다.
- [ ] dry-run 응답에 정적 `source_contract.emissions[]`와 실제 `atoms_rendered[]`가 함께 있고 쓰기는 0이다.
- [ ] Template Method의 늦은 거절이 해당 분자의 first-sight register 메모를 남기지 않는다.
