...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-23 (**§1.13에 L8 신설 — 「소유자가 폼만으로 소스를 만들어 원자를 얻는다」.** 기능 하나가 아니라 **작성 여정 하나**를 채점하는 행이고, 셋업 관문 ①의 수락 기준이다). 직전 2026-08-21 (원장 config 모양 **v5** — 문장 «별명» 행 재작성: 구조 탐색 점검 셋이 은퇴하고 별명 조회 점검 다섯이 들어왔다. 🔴 `packs`/`claims` 은퇴 뒤 문장은 `predicate`로 술어를 직접 대고 Role은 도출된다) | **Owner:** Integrity/QA | 갱신 **doc-keeper 전담** · 정합 감사 **doc-auditor**
>
...
| **L8 소유자가 «폼만으로» 소스를 하나 만들어 원자를 얻는다 — 셋업 관문 ①의 수락** (2026-08-23 · `3c6a854d`~`189193a4` · 소유자 판정 여럿) | 이 행이 채점하는 것은 기능 하나가 아니라 **작성 여정 하나**다 — 「+ New」에서 시작해 코드를 한 줄도 안 쓰고 백필까지. 실제로 그렇게 착지한 것이 `transfer_event`(1,405행 → 1,405 분자 → 1,405 원자, 거절·중복·미완 0, 재실행 0행). 점검: ① 🔴 **범용 매퍼가 «시각 Role»을 채우는가** — 전용 매퍼 없는 소스로 확인할 것. 전용 매퍼 둘이 각자 값을 변환하며 이 칸을 **몇 달간 가리고** 있었고, `invalid_time_role`은 그것이 처음 드러난 형태다 ② 🔴 **빨강 0이 «컴파일된다»와 같은 말인가** — 한때 아니었다: 화면이 제안한 후보를 눌렀더니 컬럼만 들어가고 timezone이 비어, 앉을 칸이 없는 거절 셋이 남았다. **후보를 전부 눌러 본 뒤 시험 실행이 통과하는지**로 채점한다 ③ 🔴 **저장이 파생 칸을 «문서에» 쓰는가** — 「id를 고르면 버전이 따라온다」가 빈 상자 옆 문장으로만 있으면 파일에는 없다. 우회 대조: 채움을 건너뛰면 정확히 그 칸들이 빨개져야 한다 ④ 🔴 **`input_columns` 둘이 전체 선택으로 도착하고 강제 컬럼이 «눌린 채 안 눌리는가»** — `disabled`가 아니라 `data-action` 부재로. 마우스·키보드·합성 클릭 셋 다 시도할 것. 그리고 **화면이 잠긴 컬럼을 문서에 넣지도 빼지도 않는가** ⑤ 🔴 **지운 mapping이 진짜로 사라지는가** — 계획과 문서의 합집합 때문에 행이 자기 삭제에서 살아남고 마이너스 버튼만 사라졌었다 ⑥ ⚠️ **「+ New」 직후 첫 저장 «전»에는 계획 행이 0개라 잠금·전체선택이 안 보인다** — 알려진 구멍이고, 점검할 때 이것을 결함으로 다시 올리지 말 것 ⑦ 🔴 **착지 = 끝이 아니다. 서버 재기동까지가 한 걸음** — 시각 Role 수리가 08:59에 착지했는데 돌던 서버가 08:31이라 **한 시간 동안 화면에서 계속 거절**이었다 | `http://127.0.0.1:8080/admin.html#ontology` → 「+ New」 → 시험 실행 → `cd server && conda run -n assy_manager python -m ledger.backfill --source <id>` | `server/ledger/config_authoring.py`(`filled_declaration`·`_locked_read_columns`·계획 행) · `server/ledger/config_drafts.py` · `server/ledger/roleframe.py`(`DeclarativeRoleMapper`의 time Role) · `server/ledger/source_preparation.py`(`locked_select_columns`) · `client2/src/ontology_explorer_view.js` · 하네스 `client2/tests/ontology_authoring_panel_harness.mjs` · [ONTOLOGY_LEDGER_SETUP §13.3-quater](../guide/ONTOLOGY_LEDGER_SETUP.md) · [frontend §5](../architecture/frontend.md) |
