...
>
> **이번 라운드 (2026-08-11)**: §1.2에 **히스토리 페이징(더 보기)** 신규 행 + 글로벌 타임라인 행에 bounded-scan 캐비앗(`fde424c` — `/audit_logs/recent`도 바디 엔벨로프가 됐다, 리스트 키 `groups`). §1.6 Enrichment Queue — **컨베이어 입력·결손 배지 취소선**(둘 다 UI가 삭제됐다, `ab36fab`+`5116f67`) + **그리드 사이드바 참조뷰** 신규 행(`1e29078`). §2.8-ter에 세 번째 config 도메인 `binding`(`68db020`) 점검 5건 신설.
>
