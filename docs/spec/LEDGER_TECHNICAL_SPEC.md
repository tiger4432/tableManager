...
> 🔴 **[2026-08-31] 종전 이 자리는 「살아 있는 읽기 라우트는 «둘»」이라 적고 있었고 그것이 거짓이 됐습니다** — `GET /api/ledger/gaps` 가 붙어 **셋**입니다(`subgraph` · `declaration` · `gaps`). **이 문서는 이제 그 수를 적지 않습니다** — 정본은 [backend §2](../architecture/backend.md) 입니다. 같은 문장의 사본이 이 파일 안에 «다섯» 더 있었고 한꺼번에 낡았습니다.
>
...
> ```
> 🗄️ 믿지 말 것    entities.<type>.references[] 문법 · from/edge/to 낱말 · 합성 엣지 · 원자 없음
...
> 그것은 «몇 분짜리 참»이었다.** 배포 사실은 커밋 하나 뒤에 뒤집힌다 — 이 문단을 근거로
> 「설치가 깨졌다」고 진단하기 «전»에 두 파일을 직접 대조하라.
...
### 4.5 ⚰️ 보행 알고리즘 — **라우트는 은퇴했다.** 아래는 «기록»이다

🔴 **`GET /api/ledger/trace` 는 «없다»** — 실측 2026-08-31에도 그대로다(살아 있는 라우트 목록의
정본은 [backend §2](../architecture/backend.md) 이고 **여기에 수를 적지 않는다**).
「이 랏의 부모는?」은 오늘 `GET /api/ledger/subgraph` 가
`follow` 로 술어를 좁혀서 답한다. 아래는 그 라우트가 답하던 방식의 «기록»이다.
...
⚰️ **[2026-08-14 3차] 인출 집합을 «선언»에서 파생시키던 층은 v1 어휘와 함께 은퇴했다.**
그날의 요점(「인출 집합이 코드가 아니라 선언에서 나온다」)은 **오늘도 참이다** — 다만 그것을 싣던
선언 키가 v5 선언에 «0»회이고, `ledger_trace` 에 그 파생 함수 둘도 «없다». 오늘 같은 일을 하는 것은
`subgraph` 의 `follow` 이고, 선언에 없는 술어는 빈 그래프가 아니라 **422** 로 거절된다.
...
### 4.6 ⚰️ 응답 형태 — **라우트는 은퇴했다.** 형태가 어디 남았는지까지 적는다

🔴 **은퇴한 것은 «라우트»이고, 이 형태를 오늘 «만드는» 코드는 없다.** 실측 2026-08-28:
`server/ledger/trace.py` 에 `def trace` 가 없고 `hops`·`terminal_reason`·`generated_at` 을 만드는
자리도 없다 — 그 이름은 118줄 «주석»에만 남아 있다. 세 키를 아직 «이름 대어» 들고 있는 곳은
`server/tests/test_ledger_trace_pg.py:637` 하나이고, 그 시험은 은퇴한 `/api/ledger/trace` 를 부르며
`ASSY_PG_TEST_DATABASE_URL` 없이는 «시끄럽게 건너뛴다».
⚠️ 그러니 아래 동결은 «지켜지는 계약»이 아니라 **그때의 계약**이다. 되살리려면 형태를 «만드는»
코드가 먼저 있어야 한다.
...
> ⚰️ **[2026-08-28 은퇴] 이 절이 서술하는 라우트도 그 모듈(`ledger_kinds.py`)도 종류 카탈로그(`finding_kinds.py`)도가 «없습니다» (실측 2026-08-29). 아래는 «무엇이 있었나»의 기록이며 현재 계약이 아닙니다.** 오늘 살아 있는 읽기 라우트의 정본은 이 문서가 아니라 [backend §2](../architecture/backend.md) 입니다 — **여기에 수를 다시 적지 않습니다**(2026-08-31 에 「둘」이 「셋」이 되면서 이 문장의 사본 다섯이 한꺼번에 낡았습니다). 🔴 **종류는 이제 «노드»입니다** — `defect_kind@1` 엔터티에 `of_kind@1` 술어로 닿습니다. 파이썬 dict 카탈로그는 하드코딩으로 판정돼 삭제됐습니다.
...
`rollup_subject_types(<주어 타입>)` 전부를 읽는다. 🔴 **그런데 v5 에는 파생 주어 타입이 «없다»** — 그 함수(`ledger/trace.py`)는 주어 타입 «자기 자신»만 돌려주고 자기 주석에 그렇게 적어 두었다. 실측 2026-08-27: `WaferLeg` 원자 «0» · 선언의 개체 여섯에도 «없음». 이 자리는 «파생 타입이 선언되는 날»을 위한 이음새이지 오늘 무엇을 데려오는 자리가 아니다.
...
> ⚰️ **[2026-08-28 은퇴] 이 절이 서술하는 라우트 넷(`/trends` · `/composition` · `/selection/resolve` · `/kinds`)가 «없습니다» (실측 2026-08-29). 아래는 «무엇이 있었나»의 기록이며 현재 계약이 아닙니다.** 오늘 살아 있는 읽기 라우트의 정본은 이 문서가 아니라 [backend §2](../architecture/backend.md) 입니다 — **여기에 수를 다시 적지 않습니다**(2026-08-31 에 「둘」이 「셋」이 되면서 이 문장의 사본 다섯이 한꺼번에 낡았습니다). 🔴 **이 절이 대는 주어와 술어도 선언에 없습니다** — `WaferLeg` · `FinalChip` 은 엔터티가 아니고, `transferred` · `measured` 는 술어가 아닙니다(살아 있는 철자는 `transfer@1` · `measures@1`). 마킹 단위는 «웨이퍼»이고 실험 구간은 수식어입니다.
...
🔴 이것이 `ledger/trace.py`의 3분할(해결기/조회기/보행)이 **구조 요구**인 이유다 — 주 2의 슬롯 작업은
...
> ⚰️ **[2026-08-28 은퇴] 이 절이 서술하는 라우트(§4.7 `/structure`)가 «없습니다» (실측 2026-08-29). 아래는 «무엇이 있었나»의 기록이며 현재 계약이 아닙니다.** 오늘 살아 있는 읽기 라우트의 정본은 이 문서가 아니라 [backend §2](../architecture/backend.md) 입니다 — **여기에 수를 다시 적지 않습니다**(2026-08-31 에 「둘」이 「셋」이 되면서 이 문장의 사본 다섯이 한꺼번에 낡았습니다). 🔴 **「O(원자)인 읽기는 캐시가 아니라 «선언된 크기 게이트»로 막는다」는 원리는 살아 있습니다** — 사라진 것은 그것을 적용하던 라우트입니다. 여기 적힌 수(노드 6 · 엣지 54)는 엔터티 여섯 시절의 것이고 오늘 선언은 아홉입니다.
...
> ⚰️ **[2026-08-28 은퇴] 이 절이 서술하는 라우트가 «없습니다» (실측 2026-08-29). 아래는 «무엇이 있었나»의 기록이며 현재 계약이 아닙니다.** 오늘 살아 있는 읽기 라우트의 정본은 이 문서가 아니라 [backend §2](../architecture/backend.md) 입니다 — **여기에 수를 다시 적지 않습니다**(2026-08-31 에 「둘」이 「셋」이 되면서 이 문장의 사본 다섯이 한꺼번에 낡았습니다). 🔴 **「부호가 클라이언트 계약이다」(`refusals_unaccounted`)를 지금 지키는 소비자는 «없습니다»** — 라우트가 없으므로 그 필드를 내보내는 곳도, 부호로 분기하는 화면도 없습니다. **원리(집계 옆 내역을 같은 트랜잭션에 쓰고 차이를 부호로 말하게 한다)는 [PRIMITIVES §7](../architecture/PRIMITIVES.md)이 소유합니다.**
