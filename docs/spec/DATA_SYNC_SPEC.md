...
> **Status:** ⚪ 스냅샷 — **폐기된 PySide6 데스크톱 클라이언트 기준** | **Last-verified:** 2026-07-27
>
> 🛑 **읽기 전에: 본문의 구현 서술은 현행 클라이언트의 것이 아닙니다.** 2026-07-27 전수 대조 결과 `WsListenerThread` · `ApiLazyTableModel` · `_build_row_id_map` · `_search_session_id` · `_chunk_size` · `beginMoveRows`/`beginResetModel`/`canFetchMore`(Qt 모델 API)는 **`server/`·`client2/`·`client/` 어디에도 없습니다.** 전부 제거된 데스크톱 클라이언트의 심볼입니다.
>
...
### 3.3 Search Session Guard (v1.4)
- **문제**: 고속 타이핑 시 이전 검색 결과가 늦게 도착하여 현재 데이터를 오염시키는 경합 현상.
- **해결**: `_search_session_id` (UUID)를 도입하여 모든 비동기 워커가 세션 ID를 공유합니다. 모델은 현재 세션과 일치하지 않는 모든 응답(Stale Response)을 전수 폐기하여 정합성을 유지합니다.

### 3.4 Server-Driven Total Sync (v1.4)
- **문제**: 외부 행 삭제 시 검색 필터 매칭 여부와 무관하게 카운트가 오작동하는 현상.
- **해결**: 외부 변경 이벤트 발생 시 클라이언트가 직접 수동 계산하는 대신, 서버에 현재 검색 필터를 포함하여 `total`을 재요청하여 UI 정합성을 100% 보장합니다.
...
---
---
*AssyManager Data Integrity Specification v2.1 | 2026.04.20 (Search Session Guard Revision)*
