...
### 5.7-bis 프레임 채택 — 참조 맵 크기로 격자를 여는 것과, 그것이 거절되는 자리 (F6 · `73b5925` → 거절 규율 `ae2811c` · 2026-07-30)
...
| `wafer_map_metadata` | ❌ **쓰지 않습니다**(INV-F6-2) | 채택은 **화면 컨트롤만** 건드립니다. 메타만 바뀌고 저장된 셀은 그대로인 상태가 위험한 형태입니다 |

> 채택 뒤 프레임을 **다시 읽어** 치수가 맞았는지 확인하고, 못 맞췄으면 사유를 대고 거절합니다 — 다른 인덱스 공간의 키 집합으로 마스크를 앉히면 **화면은 멀쩡하고 값만 틀립니다.**

🔴 **거절 규율 — 이미 값이 있는 셀의 저장 좌표를 움직이는 채택은 하지 않습니다 (`ae2811c`)**
...
- **판정은 치수 비교가 아니라 좌표 비교입니다.** `adoptionCoordinateCost`가 채택 전/후 프레임에서 `물리 키 → 저장 좌표` 사상을 각각 만들어(`dbCoordsByPhysKey`, 기존 `withPhysFrame` 창 안에서 — 새 변환을 만들지 않습니다) 값이 있는 셀을 `moved`/`lost`/`kept`로 셉니다. `moved > 0 || lost > 0`이면 거절입니다.
- **그래서 "치수가 다르면 거절"이 아닙니다.** 좌표가 하나도 움직이지 않는 치수 변경(bbox 항이 같은 경우)은 **통과합니다** — 45×45→47×47에 직경이 다른 케이스가 `allowed`인 것이 그 증거로 고정돼 있습니다. 반대로 같은 규격의 45×45→46×46은 **1272셀을 움직이고 하나도 잃지 않아** `lost`만 보던 가드로는 잡히지 않았습니다.
- **거절은 수·실제 표본·복구 경로를 함께 말합니다** — 몇 개가 움직이는지, `(24, 24) → (22, 22)` 같은 실제 before/after 하나, 그리고 **격자를 그 치수로 맞춘 뒤 📂 Load로 다시 불러오면** 셀이 그 좌표계로 읽혀 채택 없이 지정된다는 안내. **빈 맵·새 맵에는 이 제한이 없습니다**(움직일 좌표가 없음).
- ⏳ **재키(re-key)는 의도적으로 이 커밋에 없습니다.** 값이 있는 맵에서도 채택이 되게 하려면 `gridData`·`serverCellKeys`·`loadedFCells`가 **함께** 이주해야 하고(하나만 남으면 조용히 낡습니다), 그것을 같은 커밋에 넣으면 위 거절 가지가 **도달 불가**가 됩니다 — 죽은 것으로 증명된 가드는 없는 가드보다 나쁩니다.
