...
- **바운딩 박스는 이 기능의 영향권 밖입니다.** `get_wafer_bounding_box`/`getWaferBoundingBox`는 계속 원으로 계산합니다 — 저장 좌표의 기준(§5.1 "저장 규약")이 바뀌면 같은 맵의 좌표가 조용히 다른 수로 재해석되기 때문입니다. 원을 `inside`에서 빼는 것은 phase 3입니다.
