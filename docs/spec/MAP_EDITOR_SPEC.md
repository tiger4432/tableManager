...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-06 | **Owner:** UI/Map | **Source-of-truth:** `client2/src/map_editor.js`, **`client2/src/map_key.js`**, **`client2/src/split_registry_row.js`**, `client2/src/transfer_plan.js`, `server/map_overlay.py`, `server/bonding_plan.py`, `server/transfer_plan.py`, `server/utils/coordinate_transformer.py` · 상위 [SYSTEM_OVERVIEW](../overview/SYSTEM_OVERVIEW.md)
>
> ### 🔴 읽기 전에 — 라운드가 지나도 이 자리에서 내리지 않는 네 줄
...
> 4. **클립보드 파서는 사소한 차이로 거절하지 않습니다 — 「파서를 더 관대하게」를 다시 스펙하지 마십시오**(실측 정정 2026-08-06 · 전문은 **§7.3**). 그 반대의 믿음이 **이미 기능 요청 하나를 움직였습니다.** 실제 결함은 **다섯 테이블에서 죽어 있는 것**과 **조용히 오파싱하고 성공이라 보고하는 것**이고, 수리 방향이 다릅니다. *(이 줄이 여기 있는 자격은 「잃으면 하루를 다시 쓰는 것」이 아니라 「잃으면 엉뚱한 것을 만든다」이고, 그것은 이미 한 번 일어났습니다.)*
>
> ### 이번 라운드 (2026-08-06 · 실주행 사용성 이관 `372c538` + 일곱째 토큰 정정)
> - **§7 신설 — 조작 가능성 요구사항.** 브라우저 실주행 18건이 **요구사항 + 측정된 격차**로 들어왔고 **수리는 하나도 예정돼 있지 않습니다**(제품 소유자 판정). 🔴 **두 부류를 한 호흡에 고치지 마십시오** — §7.1 **고장**(Push가 7개 중 6개 테이블에서 거절 · 붙여넣기가 5개에서 침묵 · 병합 제목 행이 **성공이라고 보고**)과 §7.2 **사용성**(주 버튼 713px 이동 · 대비 1.18 · 레이어 5·6 도달 불가)은 **완료 판정의 기준부터 다릅니다.** 실측의 정본은 [process/EDITOR_USABILITY_FINDINGS](../process/EDITOR_USABILITY_FINDINGS.md)이고 §7은 수를 다시 적지 않습니다.
> - 🔴 **§7.5에 「이 문서가 스스로와 어긋나는 자리」 다섯을 적었고 판정하지 않았습니다** — 특히 **§4-bis.3의 「📐 표준 = 전 셀 pushable」이 넘어온 유효 다이 지정 앞에서 성립하지 않습니다**(§3 #39가 그 메커니즘을 이미 적고 있습니다). **어느 쪽으로도 구현하지 마십시오.**
> - 🔴 **§1-ter.1의 「어휘는 여섯입니다」가 거짓이 됐고, 여섯을 일곱으로 고치지 않고 *기수를 지웠습니다*** — `confirmed`가 [D7]로 착지했습니다. **목록 옆의 수는 목록의 두 번째 사본**이고, 같은 날 그 결함이 `PRIMITIVES §7`과 클라 하네스(`TOKENS.length === 6`)까지 셋을 한꺼번에 낡게 했습니다. §4-bis.3-ter의 반증 표에 `confirmed` 칸을 더했습니다.
>
> ### 직전 라운드 (2026-08-05 · 2차 — `d4e0fed`+`04ed01b`+`b9a0ab1`+`b445c2e`, **설계 협의 기록**)
> - **§5.9 신설 — 「규격을 모르는 맵을 정렬한다」 사슬 하나.** 전제는 **규격 부재가 정상이라는 것**(조작자가 정렬을 도는 이유가 그 규격을 모르기 때문)이고 나머지는 전부 거기서 따라 나옵니다: **빌린다**(웨이퍼 규격 여섯 + 격자 치수 + `grid_start_x/y`) → ~~담기는지 본다~~(**[D16] 2026-08-06 은퇴** — `cells_outside_grid`는 날 저장 좌표를 남의 원점 상자에 견줘 조작자의 정상 맵을 지웠다) → **바닥이 선언이 아니면 요청 단위로 한 번 거절**(`basis_undeclared`). 🔴 **판정·실측의 정본은 [MAP_ALIGNMENT_SPEC §9.1·§9.4·§9.5·§9.6](./MAP_ALIGNMENT_SPEC.md)이고 §5.9는 수를 다시 적지 않습니다.**
> - 🔴 **§1-ter.1의 「빌리는 것은 `PHYS_KEYS` 여섯뿐」이 뒤집혔습니다** — 고쳐 쓰지 않고 **뒤집혔다고 표시**했습니다. 옛 규칙은 **메커니즘에 대해 옳았고 어느 경우가 전형인가에서 틀렸습니다.**
> - **§4-bis.3-ter 신설 — 고른 프레임은 골랐다고 기록됩니다**(`frame_chosen_from: "data"|"panel"`). 🔴 **provenance 토큰이 아닙니다**(⚠️ **[2026-08-06] 종전 이 자리는 「*일곱째* provenance 토큰이 아닙니다 — *여섯* 토큰은…」이었습니다.** 그 두 기수는 **여섯 줄 위 §의 「여섯을 일곱으로 고치지 않고 기수를 지웠습니다」와 같은 헤더 블록 안에서 정면으로 어긋나 있었습니다** — 기수를 지운 라운드가 자기 헤더의 다른 문장은 못 본 것입니다) — 토큰들은 「이 **축의 값**이 어떤 증거인가」에 답하고, 「사람이 모달에 답했다」는 **행에 대한 사실**입니다. 선례는 토큰이 아니라 `phys_assumed_from`. 그리고 **⚙️ 현재 패널 분기는 빈 칸을 0으로 지어내지 않고 이름을 대며 거절합니다**(§4-bis.3-ter.1).
> - **§5.9 ④에 미판정 항목 하나를 OPEN으로 적었습니다** — 「가정 수락이 소스 맵의 **선언된** 격자까지 덮어야 하는가」. **오늘 코드는 덮지 않습니다.** 양쪽 근거를 함께 적었으니 **어느 쪽으로도 구현하지 마십시오.**
>
> ### 그 앞 라운드 (2026-08-05 · 1차 — `98b48e9`)
...
> - **§1-ter.1의 어휘에 `assumed`가 들어왔습니다**(`aa24bfd` — 이 라운드의 다른 레인이 작성). **개수를 핀으로 박지 마십시오**: 「정확히 다섯」이라는 핀이 정당한 서버 측 추가를 클라 결함으로 읽게 만들었습니다. *(그 경고를 적은 줄이 스스로 「여섯」을 박았고, 2026-08-06에 그대로 낡았습니다.)*
>
> ### 그 앞 라운드 (2026-08-04)
...
- 🔴 **2026-08-05 — 정렬 경로에는 토큰이 하나 더 있고(`assumed`), 질문도 둘로 갈렸습니다** (맵 정렬 스펙 [§9.1](./MAP_ALIGNMENT_SPEC.md)). 규격 선언이 없는 소스 맵을 **기준 맵의 웨이퍼 치수를 빌려** 채점할 수 있게 되었고(순환을 끊는 자리: 규격이 선언돼야 채점하는데 조작자가 정렬을 도는 이유가 그 규격을 모르기 때문입니다), 빌린 사본은 `map_overlay.GEOMETRY_ASSUMED`(`"assumed"`)로 답합니다.
  - **어휘의 구성원은 이것들입니다**: `declared` · `auto_registered` · `absent` · `unparsable` · `indeterminate`(방위 축 전용) · `assumed` · **`confirmed`**(2026-08-06 [D7] — *확정된 정렬 아래에서 기준 맵으로부터 파생됐다*는 뜻이고 **선언은 아닙니다**). 클라 거울 `client2/src/map2/declaration.js`의 `DECLARATION_TOKENS`가 **같은 구성원**을 갖고, 양쪽 채점은 `contracts/map2_seam/`의 `geometry_declaration_cases`입니다.
  - 🔴 **개수를 적지 마십시오 — 목록 옆의 수는 목록의 두 번째 사본이고 반드시 갈립니다.** 이 자리는 「정확히 다섯」으로 한 번, **「여섯」으로 다시 한 번** 낡았고, 뒤엣것은 *「개수를 핀으로 박지 마십시오」*를 **같은 줄에 적어 놓고** 그 규칙을 자기가 어겼습니다. 같은 날 클라 하네스 하나가 `TOKENS.length === 6` 핀 때문에 빨개졌는데, 그 핀은 **길이 핀이 왜 틀렸는지 설명하는 주석 아홉 줄 아래**에 있었습니다. **구성원을 이름으로 부르고 기수는 부르지 마십시오.**
  - 🔴 **`assumed`와 `confirmed`를 「둘 다 근거이고 선언이 아니다」로 접지 마십시오 — 클라에 대해 서로 다릅니다.** `assumed`는 **클라가 생성할 수 없습니다**(빌린 사본이 저장되지 않으므로 클라가 읽는 메타에는 표지가 없습니다). 그래도 거울에 있어야 하는 이유는 **토큰이 문자열로 도착**하기 때문이고, 빠지면 클라는 침묵하는 게 아니라 **다른 통으로 분류**합니다. 반면 **`confirmed`는 `wafer_map_metadata`에 저장됩니다**(`frame_confirmation.record_confirmation` → `map_alignment.confirmed_meta_for`, 표지 `phys_confirmed_from`/`frame_confirmed_from` — [MAP_ALIGNMENT_SPEC §9.7](./MAP_ALIGNMENT_SPEC.md)) — 즉 **클라는 그것을 DB에서 곧바로 읽습니다.** 🔴 **그래서 이식이 토큰 목록만 늘리고 `geometryDeclaration`에 `phys_confirmed_from` 분기를 만들지 않으면, 아무도 잰 적 없는 맵에 클라가 `declared`라고 답합니다** — 목록보다 그 분기가 **더 큰 절반**이었습니다.
  - 불변식은 개수가 아니라 **「빌려 오고, 짓지 않는다」**입니다.
...
| 파일 | 무엇이 사나 |
|---|---|
| `client2/src/map_key.js` | **맵 키의 정준형**(이음새 작업 **7b** — 이 문서의 §7이 아닙니다. 채점은 `contracts/map_seam/` + `client2/tests/seam_7b_oracle.py`) — `canonicalKeyValue` · `composeMapId` · `decomposeMapKey` · `canonicalMapKey` · `getMapIdFromMeta`. R1 `689ebb9` |
...
> ⏸️ **「마스크가 사실상 없다 · 전 셀이 pushable」은 넘어온 유효 다이 지정 앞에서 성립하지 않습니다.** 이 분기가 세우는 것은 **원 프리셋**(`applyPresetObject`)인데, §3 #39대로 **유효 다이 맵이 지정돼 있으면 원이 아니라 그 맵이 이깁니다** — 실측: 📐 표준을 누른 뒤 캔버스의 **26%**가 마스크였습니다(직전 맵에서 넘어온 `CORE_1X`). → §7.2 R-U5/R-U8 · **§7.5 ②③(총괄 판정 대기 — 어느 쪽으로도 구현 금지)**.
...
🔴 **provenance 토큰이 아니고, 그 구분이 이 판정의 전부입니다.** §1-ter.1의 토큰들은 전부 **「이 *축의 값*이 어떤 종류의 증거인가」** 하나에 답합니다. 「사람이 모달에 답했다」는 그 질문의 답이 아니라 **행에 대한 사실**입니다. 🔴 **이 표는 토큰마다 한 칸이고, 어휘가 자라면 함께 자랍니다** — *「N개를 다 확인했다」*로 요약하지 마십시오(그 요약이 이 문서에서 이미 두 번 낡았습니다 — §1-ter.1). 하나씩 확인한 결과:
...
| **`confirmed`** *(2026-08-06 추가)* | **확정된 정렬 아래 파생됐다**는 뜻이고 `geometry_computable`이 그것을 **근거로 받습니다** — 모달의 답에 붙이면 **아무도 잰 적 없는 프레임이 정렬 채점의 근거**가 됩니다. 게다가 이 토큰은 **저장되므로**(`phys_confirmed_from`/`frame_confirmed_from`) 그 거짓이 메모리가 아니라 **행에 남습니다** |
...
> ⏸️ **로스터는 「그 줄이 머리줄로 판정될 때만」 작동합니다 — 위에 병합 제목 행이 한 줄 있으면 판정 자체가 일어나지 않고, 실측은 그 상태가 *성공으로 보고*된다고 말합니다(§7.1 R-B3). 단어를 더 넣어서는 닫히지 않습니다. 수리는 예정돼 있지 않습니다.**
>
...
> ⏸️ **아래의 *「레이어를 잃는 것도 아닙니다」*는 「선언되지 않은 *값*」에 대한 문장이지 레이어 수에 대한 보증이 아닙니다.** 실측은 레이어가 실제로 사라지는 경로 둘을 보고합니다(칸당 점 상한 4 · `align_unavailable`인데 뱃지는 셈) — §7.2 R-U6 · §7.5 ④. **레이어 보증으로 인용하지 마십시오.** 수리는 예정돼 있지 않습니다.
...
- 🔴 **바운딩 박스는 이제 이 기능의 영향권 *안*입니다** (**정정 2026-07-30 `da8f390`** — 이 줄은 종전에 *"영향권 밖이고 계속 원으로 계산한다"*고 적었고 그 서술은 **거짓이 됐습니다**). `getWaferBoundingBox`는 근거가 `ref`로 해석됐을 때 **유효 다이 마스크의 최소 사각형**을 원점 상자로 쓰고, 그 외(원·`refused`·저작 캔버스·프레임 창 안·`opts.circleOnly`)에서만 원으로 계산합니다. 판정식은 `maskDeclaresTheFrame = !circleOnly && !physFrameOverride && validDieBasis() === 'ref'`이고, 캐시 태그가 `'C'`(원) / `V<validDieResolveSeq>`(마스크)로 갈립니다.
  ...
  - 🔴 **정정 2026-08-06 — 서버에도 갈래가 생겼습니다(단, `inside` 판정이 아니라 원점 상자에만).** 확정이 소스 맵마다 `valid_die_ref`를 적기 시작하면서 이 마스크 갈래가 확정된 모든 맵에서 켜졌고, 서버가 원점을 **원 상자로만** 계산하던 것이 그대로 어긋남이 됐습니다(실측: 16 프레임 중 12, 3,840셀 중 2,880). 그래서 `map_overlay.origin_box(meta, die_mask)`가 생겼고 **같은 판정식으로** 갈립니다 — 클라 `validDieBasis() === 'ref'` ↔ 서버 `resolve_valid_die_basis(...)["source"] == SOURCE_REF`. **`is_inside_wafer`/`circle_die_mask`는 한 글자도 바뀌지 않았습니다** — 여기서 갈리는 것은 「어느 다이가 유효한가」가 아니라 **「저장 좌표가 어느 사각형에 상대적인가」** 하나뿐이고, 원의 은퇴는 여전히 phase 3입니다. 계약과 실측은 [spec/MAP_ALIGNMENT_SPEC §9.7-ter](./MAP_ALIGNMENT_SPEC.md), 오라클은 `server/tests/oracle/editor_origin_box_oracle.mjs`(이 파일의 `getWaferBoundingBox`를 잘라 실행합니다).
...
| ~~**② 담는지 본다**~~ | **은퇴** ([D16] 2026-08-06) — 이 자리에 `cells_outside_grid`가 있었다 | **날 저장 좌표를 남의 원점 상자에 견주지 않는다.** 판정·실측은 [MAP_ALIGNMENT_SPEC §9.4](./MAP_ALIGNMENT_SPEC.md) |
...
**~~② 담김이 유일하게 남은 관문~~ → 은퇴했습니다** ([D16] 2026-08-06)

- 🔴 **여기 있던 세 문단은 삭제됐습니다.** 관문은 소스의 **날 저장 좌표 bbox**를 바닥의 **인덱스 상자**에 견줬는데, 저장 원점은 맵마다 「랜덤이야」(§9.1)라 두 상자가 서로 다른 공간에 삽니다 — 포함 관계가 아무것도 재지 못합니다. 게다가 **배치보다 먼저** 물어서, 아직 놓지 않은 자리에 안 들어간다는 이유로 맵을 지웠습니다.
- 조작자 신고 그대로: 「셀 범위 2~28이 빌린 격자 −4~26을 벗어나서 점수 못 낸다는데, **당연히 shift가 있으니 범위가 다를 수 있는 거 아니야?**」. 소스 폭 **27** < 격자 폭 **31** — **담깁니다.** 어긋난 것은 원점이고, 그것을 맞추는 것이 정렬입니다.
- **판정·실측의 정본은 [MAP_ALIGNMENT_SPEC §9.4](./MAP_ALIGNMENT_SPEC.md)**입니다(관문 유지 시 8후보 전부 `not_scorable` / 제거 시 706/706 · margin 157, 그리고 관문이 「다른 맵」을 못 걸렀다는 대조 실측). 여기서는 수를 다시 적지 않습니다.
...
> ⏸️ **게이트 4의 *판정*은 유효하고, 실측이 다투는 것은 그 판정이 아니라 시점과 조치 가능성입니다** — 거절이 **7개 중 6개 테이블**에서 나고(**기본 진입 테이블 `dt_log` 포함**) **마지막 클릭에** 도착하며, 조작자는 테이블에서 컬럼을 뺄 수 없습니다. **§6의 초기 테이블 선택과 이 게이트가 같은 테이블에 반대 답을 냅니다** → §7.1 R-B1 · §7.5 ①. 수리는 예정돼 있지 않습니다.
...
## 7. 조작 가능성 요구사항 (Operability Requirements) — 실주행에서 옮겨 온 것 (2026-08-06 · ⏸️ **수리 미착수**)

**이 절이 담는 것은 규칙과, 그 규칙에 대해 측정된 격차입니다.** 근거 — 브라우저 실주행(1920×1080 · light · 시드 데이터)의 수치·재현 절차 — 의 **정본은 [process/EDITOR_USABILITY_FINDINGS](../process/EDITOR_USABILITY_FINDINGS.md)**(`372c538`)이고, **이 절은 그 실측을 다시 적지 않습니다.** 행마다 격차가 읽히는 수 **하나**만 들고 옵니다. 수를 인용해야 하면 실측 문서를 읽으십시오.

> ⏸️ **수리는 하나도 예정돼 있지 않습니다** — 제품 소유자 판정 2026-08-06(*「맵 스펙 파일에 적어두고 나중에」*). **일정·담당·우선순위를 이 절에서 지어내지 마십시오.** 실측 문서의 순위는 *맵 여러 장 흐름에서 조작자 시간을 얼마나 먹는가*(지수 = 마우스 클릭 수)이고, 이 절은 그 순위를 재배열하지도 승격하지도 않습니다.
>
> 🔴 **두 부류를 한 호흡에 고치지 마십시오.** §7.1 **고장**은 「이 화면이 자기 일을 못 한다」이고 §7.2 **사용성**은 「되지만 너무 비싸다」입니다. **완료 판정의 기준부터 다릅니다** — 고장은 「경로가 생겼는가」, 사용성은 「클릭·픽셀이 줄었는가」. 섞으면 레이아웃 손질이 *저장 불가를 고쳤다*고 보고되고, 반대로 저장 경로 수리가 대비·스크롤까지 인질로 잡습니다.
>
> 🟩 **잘 되는 것도 실측 문서가 그대로 적습니다.** 로드는 저장된 프레임(`13×13, start(0,0), rot 0, front, chip 1×1`)을 정확히 재현하고, **칩 피치를 지어내길 거부하는 것은 §1-ter.1이 요구하는 옳은 동작**이며, 규격 조회의 엉뚱한 테이블 회귀는 고쳐져 있고(`(target_table, map_id)` 쌍), Push 확인창은 델타인 척하지 않습니다. **아래 어느 요구사항도 그 넷을 되돌리라고 말하지 않습니다.**

### 7.1 고장 — 화면이 자기 일을 못 하는 자리

각 항목은 **요구 · 측정 · 이 문서의 자리** 세 줄입니다.

#### R-B1 — 화면이 여는 테이블은 저장할 수 있어야 하고, 없으면 **여는 시점에** 말해야 한다

- **요구**: 에디터가 어떤 테이블을 고르게 한다면 `⚡ Push`의 거절은 **첫 클릭 전에** 알려져야 합니다. **조작자가 되돌릴 수 없는 거절**(테이블에서 컬럼을 뺄 수 없습니다 — 테이블의 영구 속성이지 실수가 아닙니다)을 **마지막 클릭**에 놓지 않습니다.
- **측정**: 화면이 제공하는 **7개 중 6개**가 거절되고 `bonding_map`만 받습니다. 거절되는 쪽에 **페이지 로드 시 기본 테이블 `dt_log`**가 들어 있습니다. 거절은 테이블 선택·키 입력·로드·좌표계 선택·칠하기·DOE 편집을 **전부 마친 뒤** 네이티브 `alert()`로 옵니다(화면에 남지 않습니다).
- **이 문서의 자리**: §6.0-ter의 **게이트 4(로그형 대상)**. 🔴 **이 요구사항은 그 게이트의 *판정*을 건드리지 않습니다** — 판정은 니어미스 하나(dt_log 256행이 confirm 한 번 거리)에서 나온 데이터 보호이고 유효합니다. 다투는 것은 **시점**과 **조치 가능성**입니다.
- ⚠️ **§6과 §6.0-ter가 같은 테이블에 반대 답을 냅니다.** §6은 초기 테이블을 *「선언된 stage 타깃 중 첫 맵 테이블」*(`stageTargetTables`)로 정하고, `transfer_plan_config.json`은 `dt_log`를 stage 타깃으로 선언합니다. 게이트 4는 같은 테이블을 거절합니다 — **화면이 자기가 저장할 수 없는 테이블에서 엽니다.** → §7.5 ①

#### R-B2 — 광고된 붙여넣기는 **침묵**해서는 안 된다

- **요구**: DOE 패널이 `⇄ 엑셀`과 *「엑셀 표를 그대로 붙여넣습니다」*를 띄우는 동안 그 경로가 무동작이면, 무동작이 아니라 **이름 댄 거절**이어야 합니다. 능력이 없으면 광고를 내립니다.
- **측정**: **7개 중 5개**에서 `onPlanPaste`가 첫 줄에서 반환합니다 — **거절도 없이 침묵**이고, 진단이 가장 어려운 실패 모양입니다. 원인은 `transfer_plan_config.json`이 `dt_log`·`bonding_map`에만 stage를 선언하는 것이고, 패널의 안내는 그대로 서 있습니다.
- **이 문서의 자리**: §6 첫머리. **강등은 이미 계약돼 있습니다** — stage 없는 테이블은 *「일반 맵 (legend)」* 배지로 내려갑니다. 강등이 **배지에는 도달했고 붙여넣기 안내에는 도달하지 않았습니다.**
- 🔴 **이것은 파서 문제가 아닙니다** — §7.3.

#### R-B3 — 머리줄을 데이터로 읽은 결과를 **성공이라고 보고해서는 안 된다**

- **요구**: 「값 N개를 붙여넣었습니다」는 그 N개가 **값**일 때만 참입니다. 머리줄이 데이터 행으로 들어갔으면 그것은 성공이 아닙니다.
- **측정**: 헤더 위에 제목 한 줄(`DOE 계획표`)이 있는 표를 붙여넣으면 범례 3행이 생기고(**하나는 이름이 `DOE 계획표`**, 하나는 `VALUE`에 STACK `"STACK"`) 토스트는 **「값 3개를 붙여넣었습니다」**입니다. 뒤따르는 빨강 블록은 **엉뚱한 문제를 지목**합니다. 한글 헤더 `값/층수/설명`도 인식되지 않습니다.
- **이 문서의 자리**: §4-ter.7. 그 절은 **정확히 이 실패 모양**(*「값 이름이 `VALUE`이고 STACK이 `COUNT`인 행이 생깁니다」*)을 막으려고 `IGNORED_HEADERS`를 둔다고 적습니다. 🔴 **로스터는 그 줄이 머리줄로 *판정될 때만* 작동하고, 위에 제목 한 줄이 있으면 판정 자체가 일어나지 않습니다 — 단어를 더 넣어서는 이 결함이 닫히지 않습니다.**
- 🔴 **병합 제목 행은 이 앱 자신이 씁니다** — §4-ter.2/§4-ter.2-bis의 COPY HEADER MODE TITLE 줄이 그것이고, 회사 양식의 정상 모양입니다. **양식은 하나인데 리더가 둘이고 서로 다른 것을 압니다**: 격자 경로(#48 `readCompanyMapBlock`)는 TITLE을 알고 정체 확인에까지 쓰는데, DOE 패널 경로(`onPlanPaste`)는 모릅니다.

#### R-B4 — 맵 테이블에는 쓰기 경로가 **최소 하나** 있어야 한다

- **요구**: 어떤 맵 테이블도 조작자가 도달할 수 있는 쓰기 경로가 **0개**여서는 안 됩니다.
- **측정**: `core_wafer_map`(**가장 큰 맵 테이블, 200맵**)은 R-B1으로 저장이 막히고 R-B2로 붙여넣기가 무반응이라 **경로가 0개**입니다. 같은 테이블은 규격 **200/200이 `auto_registered`**라 §1-ter.1대로 오버레이 정렬도 거절됩니다 — 세 번째 경로도 닫혀 있습니다.
- **이 문서의 자리**: R-B1(§6.0-ter) + R-B2(§6) + §1-ter.1. **세 계약이 각각 옳고, 합성이 한 테이블을 읽기 전용으로 만듭니다.** 어느 하나를 고쳐도 나머지 둘은 그대로입니다.

### 7.2 사용성 — 되지만 너무 비싼 자리

#### R-U1 — 맵을 바꿔도 주 버튼은 같은 자리에, 그리고 화면 **안에** 있어야 한다

- **측정**: 연속된 두 맵(`CL-2601-001_05` → `_06`)에서 `⚡ Push`가 `(1420,85) → (2133,85)`로 **713px 이동**하고 **x=2133은 오른쪽 끝에서 213px 밖**입니다. 로드 다음 클릭이 전 맵과 다른 자리에 있고, 화면에도 없습니다.
- **원인**: 툴바 안 알림 `#cell-aspect-note`가 `nowrap`에 `min-width:0` 없이 **698px를 주장**합니다. 같은 맵에서 알림 숨김↔표시 실측 — 사이드바 x **1490 → 1929**(뷰포트 1920) · 툴바 높이 **68 → 164px** · Push 버튼 **158×37 → 74×133**(글자가 한 줄에 한 자).
- 🟩 **알림 자체는 옳습니다.** 발화 조건은 §1-ter.1의 합성 피치이고, **재지 않은 피치를 지어내지 않는 것은 그 절이 요구하는 동작**입니다. 요구사항은 *알림을 없애라*가 아니라 **알림이 레이아웃을 파괴하지 않아야 한다**입니다.
- ⚠️ **드문 상태가 아닙니다** — 실측 규격 672행 중 320행이 합성이고, `core_wafer_map`·`bonding_log`는 **모든 맵에서** 발화합니다. (§1-ter.1은 2026-08-04에 668행 중 320행으로 셌습니다. **분모는 움직이므로 어느 쪽도 핀으로 박지 마십시오.**)

#### R-U2 — 맵당 클릭의 절반이 **재수립**이어서는 안 된다

- **요구**: 앞 맵이 이미 세운 것을 다음 맵에서 다시 세우게 하지 않습니다. 확인창은 **실제로 잃을 것이 있을 때만** 뜹니다.
- **측정**: `core_wafer_map` 최소 **3클릭**(슬롯·Load·확인) 중 **확인은 순수 재수립**입니다. 프레임 컨트롤을 건드리기만 해도 dirty가 되어 다음 Load에서 네이티브 `confirm()`이 뜨고, 그 문구가 자기모순입니다 — *「편집이 사라집니다 · **셀은 하나도 바뀌지 않았습니다**」*. 규격 없는 맵이면 +1(선택 모달), 회전 재설정 +1.
- **이 문서의 자리**: §5.7-b-2. 그 두 번째 줄은 `unsavedWorkNotice()`가 만드는 **정직한** 줄입니다(싼 쪽을 이름으로 부릅니다). 🔴 **문제는 그 정직함이 확인창을 *띄우는 조건*에 반영되지 않는 것**입니다 — §6.4-bis가 `frameTouched`로 「보기만 해도 뜨던 확인창」을 한 번 좁힌 것과 **같은 계급**의 판정입니다.
- ⚠️ 네이티브 대화상자 실측: `map_editor.js`에 `confirm` **13** · `alert` **15** · `prompt` **2** — 유리 모달 시스템이 따로 있는 화면에서.

#### R-U3 — 「이 다이가 진짜인가」는 **기본 테마에서** 읽혀야 한다

- **측정**(실제 배경 위에 합성해 잰 대비비): 유효 다이 vs 웨이퍼 밖 **light 1.18** / dark 1.81 · 칠해진 셀 vs 웨이퍼 밖 light 2.02 / dark 6.18 · 칠해진 셀 vs 유효-빈칸 light 1.71 / dark 3.41.
- **왜 워시가 일하지 않는가**: 마스크 워시가 `rgba(23,114,69,0.06)`이라 **아무 일도 하지 않고 1.2px 선 하나가 전부 지고 있습니다**(유효 다이 내부 선 vs 밖은 light에서도 4.75).
- 🔴 **light가 모든 행에서 더 나쁘고, light가 기본값입니다.**

#### R-U4 — 화면을 다시 쓰는 조작은 **화면이** 말한다 (콘솔이 아니라)

- **측정 셋**:
  - **유효 다이 지정 적용** — `CORE_1X` 선택 하나로 격자 13×13 → **45×39**, chip 1,1 → **7,8**. **토스트 없음.** 콘솔에는 7개 항목 진단이 다 찍힙니다(기준 프레임·마스크 중심·거울축·유효 다이 범위·원점 셀·「121칸 재배치, 격자 밖 0칸」). **계산도 완결도 다 돼 있는데 조작자 대신 콘솔로 갑니다.**
  - **맵 로드** — 모든 성공 로드에서 **토스트 0건**. 키도 셀 수도 채택된 프레임도 뜨지 않고, 셀 수는 `console.debug`에만 있습니다.
  - **붙여넣기 기준 행** — 모델은 새 값인데 `VALUE` 박스만 **옛 이름**을 표시합니다(하류의 개수·검증·저장은 전부 새 이름을 씁니다).
- **이 문서의 자리**: §5.7-b-3은 **해제** 토스트의 정확한 철자를 적고 있습니다(두 저장 버튼을 이름으로 부르는 그 문장). **적용 쪽에는 대응하는 문장이 없고, 파괴적인 방향은 적용 쪽입니다.**
- 🔴 **§4-bis.4가 이 요구를 이미 문장으로 갖고 있습니다** — *「사후 통지가 정당한 이유는 정확히 '아무것도 안 바뀌었기 때문'이고, 이 성질이 깨지면(= 무언가를 적용하게 되면) 토스트로는 부족합니다」*. 프레임 전체를 다시 쓰면서 토스트조차 없는 것은 그 규율의 **반대편**입니다. 「읽기 무마찰」은 **말하지 않는 것과 다릅니다.**

#### R-U5 — 화면이 이름을 대는 기준은 **그림을 지배하는 기준**이어야 한다

- **측정**: 맵 전환에서 회전 180 → **0**(리셋)인데 유효 다이 필드 `CORE_1X`는 **유지**됩니다. 그런데 그 기준에서 나온 프레임은 맵 자신의 13×13/1×1로 돌아가 있습니다 — **화면이 그림을 지배하지 않는 기준의 이름을 대고 있습니다.**
- **그리고 실효인 경우가 더 나쁩니다**: 코어용 `CORE_1X`가 **테이블 전환을 넘어 `dt_log` 맵에 조용히 실렸고** 실효였습니다(해제하니 마스크 픽셀 **147,413 → 198,142**).
- **이 문서의 자리**: §5.4는 오버레이를 **맵 로드 · 테이블 전환 · 프레임 진입** 셋에서 해제한다고 계약하고, 그 계약이 생긴 이유는 *「이전 테이블의 값을 새 테이블에 써 넣을 수 있었다」*입니다. **유효 다이 참조에는 대응하는 규칙이 이 문서에 없습니다.** → §7.5 ②

#### R-U6 — 오버레이 정체는 **색 하나**에 실리지 않고, 선언된 레이어는 **도달 가능**해야 한다

- **측정**: 소스마다 우상단에 작은 타원 하나, **모양도 크기도 같고 0.8px 링 색만 다릅니다**(자리는 명시적으로 비식별). 점은 `cx < x0`까지 놓고 **조용히 `break`** 하므로 상한이 **어떤 셀 크기에서도 4개**(cellW≈35px에서 3개)인데 `OVERLAY_COLORS`는 **6개**입니다 — **5·6번째 레이어는 도달 불가이고 그 사실을 알리는 표시도 없습니다.**
- **부수**: `core_wafer_map` 5장 추가 → 뱃지 **5**, 다섯 줄 전부 `align_unavailable`, **아무것도 안 그려짐** — `#overlay-count`가 **거절된 레이어를 셉니다.** 그리고 칩에 내부 열거값 `align_unavailable`이 번역 없이 노출됩니다(마크업 주석은 「정렬됨/무보정/화면기준」이어야 한다고 적고 있습니다). §1-ter.1이 정렬 거절문에 대해 고친 것과 **같은 계급의 누출이 다른 표면에 남아 있습니다.**
- **이 문서의 자리**: §5.4-bis. 🔴 **그 절의 *「…레이어를 잃는 것도 아닙니다」*는 「선언되지 않은 *값*」에 대한 문장이지 레이어 수에 대한 보증이 아닙니다.** 실측은 레이어가 실제로 사라지는 경로 **둘**을 보입니다(정원 초과 · 정렬 거절). **그 문장을 레이어 보증으로 인용하지 마십시오.** → §7.5 ③

#### R-U7 — 작업 맥락에서 **맵 ID로 가는 길**이 있어야 한다

- **측정**: lot/slot 박스는 「검색어 입력」이라 적혀 있지만 필터가 **`type: 'equals'`**라 부분 입력은 0건입니다. 데이터리스트는 로드 직후·2자 입력 후·전체 입력 후 **전부 n=0**이고, 두 컬럼이 **독립**이라 어느 조합이 실재 맵인지 아무것도 알려주지 않습니다. **로드 경로에 그 테이블의 맵을 나열하는 것이 없습니다.** `map_editor.js`에 `URLSearchParams`가 **0건**이라 그리드에서 lot을 들고 와도 다시 타이핑합니다.
- ⚠️ **코드는 안전하고 사람이 못 봅니다** — 규격 조회는 `(target_table, map_id)` 쌍으로 걸리고 중복 감지도 있습니다(엉뚱한 테이블 회귀는 고쳐졌습니다). 다만 **`map_id` 84개가 둘 이상의 테이블에 살고**(예: `aa123_a`가 `sample_map`과 `bonding_map`) 조작자가 고를 때 **어느 테이블인지 화면에 없습니다.**

#### R-U8 — 고르라고 물으면 **고를 근거**를 같은 화면에 둔다

- **측정**: 좌표계 선택 모달에서 「📐 표준」은 결과를 다 적어 주는데 **「⚙️ 현재 좌측 패널 설정 사용」은 그 설정이 무엇인지 말하지 않습니다.** 그 순간 패널은 `START 0,0 · rot 90 · front · CORE_1X`였고 모달에는 하나도 나오지 않았습니다. 버튼 셋 중 둘은 아직 영어이고, **어느 맵인지도 왜 묻는지도** 말하지 않습니다.
- **이 문서의 자리**: §4-bis.3(진입 시 `warning` 토스트 1회로 *왜 묻는지* 알린다) · §4-bis.3-ter.1(⚙️ 분기는 빈 칸을 **이름 대며** 거절한다). **두 계약 다 「말한다」 쪽이고, 정작 고르는 순간의 화면이 침묵합니다.**
- 🔴 **그리고 「표준」의 `마스크 없음`이 참이 아니었습니다** — 누른 뒤 캔버스의 **26%**를 덮는 마스크가 실효였습니다(넘어온 `CORE_1X`). → §7.5 ④

#### R-U9 — 컨트롤과 캔버스는 **1080p 화면 안**에 있어야 한다

- **측정(도달)**: 사이드바 `scrollHeight` **1945** vs `clientHeight` **916** — **1029px가 접힘 아래**입니다. 맵마다 유효 다이까지 **567px**, Apply Physical Geometry까지 **925px** 스크롤이고, **1080p에서 3·4절은 화면에 나오지 않습니다.**
- **측정(캔버스)**: 작업영역 1242×831에 캔버스는 **751×751**(`min(availW, availH)` 고정)이라 **가로 491px이 미사용**입니다. 다이당 픽셀 = `751/N`이라 N=100 → 7.5px, N=120 → 6.3px이고 **최소 셀 크기 클램프가 어디에도 없습니다**(저장소에 1.17px·3.89px 실측 기록이 있습니다).
- **이 문서의 자리**: §1-ter가 *「격자 래퍼를 정사각으로 유지해 타원 왜곡을 막는다」*를 **원인 오진으로 기각**하고 왜곡의 원인은 **셀**이라고 판정했습니다. 그 판정이 옳다면 **캔버스를 정사각으로 유지할 근거는 이 문서 안에 남아 있지 않습니다.** ⚠️ 다만 **다른 근거가 있는지를 이 라운드는 확인하지 않았습니다** — 없다고 단정하고 고치지 마십시오.

#### R-U10 — 선언되지 않은 칠 값이 **선언된 값과 같은 색**으로 그려져서는 안 된다

- **측정**: `UNLISTED_VALUE_FILL`이 `LEGEND_PALETTE[0]`과 **바이트 동일**(둘 다 `#10b981`)이라, 선언 안 된 값이 범례 값 `1`과 **정확히 같은 초록**으로 그려집니다. 그 값의 칠해진 개수는 **계산됐다가 버려집니다** — 보여줄 행이 없어서.
- **이 문서의 자리**: §5.4-bis. 🔴 **같은 화면 안에서 같은 질문에 두 답이 있습니다** — 오버레이 점은 *「색을 지어내지 않고 속이 빈 링으로 그린다」*를 계약하고 하네스가 그것을 채점하는데(`pickUnusedColor|LEGEND_PALETTE|OVERLAY_COLORS` 0건), **메인 격자의 칠은 정반대로 선언된 값과 구별 불가능한 색을 줍니다.** → §7.5 ⑤

#### R-U11 — 화면은 조작자가 **뭘 하기도 전에** 에러 상태로 열리지 않는다

- **측정**: config 기본 범례가 STACK 없는 한 행이라 DOE 패널이 즉시 *「값 '1'의 STACK (비어 있음)을(를) …」*을 빨강으로 그립니다. **모든 맵이 조작자 탓이 아닌 에러 상태로 열립니다.**
- **이 문서의 자리**: **두 계약이 각각 옳고 합성이 이 상태를 만듭니다.** §5.6은 빈 맵의 legend 시드를 *「`null`이면 VALUE 1 하나짜리 **빈** 행(`EMPTY_DOE_SEED`)」*으로 정하고, §6.0-bis의 **V5**는 *「공백=미기입=차단」*이며 **가장 먼저** 판정합니다. 즉 시드 행은 **정의상** STACK이 비어 있고 V5는 **정의상** 그것을 첫 번째로 빨갛게 칠합니다.
- ⚠️ **V5를 완화하는 방향으로 읽지 마십시오** — §6.0-bis가 V5를 가장 먼저 두는 이유(16층 스택이 조용히 15층이 되던 결함)는 그대로 유효합니다. 다투는 것은 **아무도 아직 아무 말도 하지 않은 행을 위반으로 그리는 것**입니다.

#### R-U12 — 쓰기를 유도하는 안내가 **되돌릴 길 없는 쓰기**를 시켜서는 안 된다

- **측정**: 캔버스 좌클릭 한 번이 **즉시** 다이를 칠합니다(모드 구분 없음, 화면상 되돌리기 안내 없음). 그리고 `📋 Copy to Excel` 실패 문구가 *「표를 클릭한 뒤 다시 시도」*라고 시키는데, **그 클릭이 다이를 칠합니다.**
- 🟩 **클릭 즉시 칠하는 것 자체는 결함이 아닙니다** — §6이 *「값 페인팅이 정본」*(rect 영역 선택 모드는 폐기)이라고 계약합니다. 요구사항은 그 상호작용을 되돌리라는 것이 **아니고**, ① 화면에 되돌리기 안내가 없다는 것과 ② **에러 문구가 그 쓰기를 유도한다**는 것 둘입니다.
- ⚠️ 복사 실패 자체는 **확인되지 않았습니다** — §7.4.

### 7.3 🔴 정정 — 클립보드 파서는 **사소한 차이로 거절하지 않는다**

**제품 소유자는 파서가 사소한 차이에 거절한다고 믿었고, 그 믿음 위에서 좌표형 붙여넣기 기능 요청이 하나 나갔습니다. 실측은 정반대입니다.**

- **수용이 실측된 것**: CRLF · 따옴표 씌운 숫자 · 뒤쪽 빈 컬럼 · 앞뒤 공백 · 소문자 헤더 · 회사 `VALUE|COUNT|STACK|DESC` 양식(`COUNT`는 §4-ter.6대로 **올바르게 무시**) · 단일 컬럼 · **화면 자신의 Ctrl+C 왕복**.
- **거절할 때는 조치 가능합니다** — **정확한 셀과 정확한 텍스트**를 댑니다(`값 'G1'의 STACK "3층"`, `"'2"`, `"２"`).

🔴 **왜 이 정정이 스펙에 있어야 하는가**: **왜 실패하는지에 대한 틀린 믿음은 엉뚱한 곳을 겨눈 수리를 낳고, 이 믿음은 이미 기능 요청 하나를 움직였습니다.** 「파서를 더 관대하게」는 이 화면의 결함이 아닙니다 — 실제 결함은 **다섯 테이블에서 죽어 있는 것**(R-B2)과 **조용히 오파싱하고 성공이라 보고하는 것**(R-B3)이고, **수리 방향이 다릅니다.** 관대함을 한 눈금 더 넣는 라운드는 이 표면에서 살 것이 없습니다(관대함은 이미 계약입니다 — §4-ter.7의 13단어 로스터 · §4-ter.6의 `COUNT` 인식 후 폐기 · §4-ter.1의 `parseTsv`가 엑셀 인용 규칙을 아는 유일한 리더라는 사실).

⚠️ **엄격함이 정당한 자리를 이 정정으로 덮지 마십시오 — 측정된 파서와 §4-ter.5의 거부는 서로 다른 것입니다.** 실측된 관대함은 **DOE 값 파서**의 것이고, §4-ter.5의 다섯 갈래 거부는 **격자 프레임 대조**의 것입니다(지문 부재 거부는 rot 0↔180 오적재를 막는 유일한 신호입니다). **둘을 한 문장으로 접으면** 「파서가 관대하다」가 프레임 대조를 완화하는 근거로 읽힙니다.

🔴 **그리고 「붙여넣기가 대부분의 맵에서 안 된다」는 서로 독립한 경로 둘에서 각각 참입니다** — **DOE 패널**은 테이블 **5/7**에서 무반응이고(R-B2), **격자 왕복**은 §4-ter.5가 이미 적는 대로 선언된 맵 **179개 중 152개**에서 지문 부재로 거부됩니다. **한쪽을 고쳐도 다른 쪽은 그대로 남습니다.** 어느 하나를 고치고 「붙여넣기를 고쳤다」고 보고하지 마십시오.

### 7.4 이관하지 않은 것 — 관찰이지 요구사항이 아니다

- ❓ **`📋 Copy to Excel` 복사 실패**(*「복사 실패 — 표를 클릭한 뒤 다시 시도」*). `writeClipboardRich`가 **신뢰된 사용자 제스처**를 요구할 가능성이 커서 **자동화가 원인일 수 있고 배제되지 않았습니다.** 🔴 **관찰이 자동화 산물일 수 있는 동안 규칙을 세우지 않습니다** — 없는 결함에 수리를 태우게 됩니다. 사람 손으로 60초면 갈립니다. (그 문구가 시키는 클릭이 다이를 칠하는 것은 **별개로 참**이고 R-U12에 있습니다.)
- **키잉이 갈리는 것 · Push 게이트와 `dt_index` 선언** — 실측 문서의 「표현이 아니라 로직」 목록에 있는 이 둘은 **어느 쪽이 옳은지 이 라운드가 판정하지 않았습니다.** 요구사항이 아니라 미해결 질문이므로 §7.5로 보냅니다.
- **`[Legend & DOE] stage 'dt' … 하드코딩 폴백 'core_defect_map'`** — 매 로드 콘솔 경고. §6의 U6 규율(*클라 builtin stage 목록 `BUILTIN_STAGES`는 삭제됐습니다*)은 **그 심볼에 대해 참이고**, 실측된 폴백은 **다른 파일의 다른 철자**입니다(`client2/src/transfer_plan.js`의 `SOURCE_TABLE_FALLBACK`). **「삭제됐다」를 「폴백이 하나도 없다」로 읽지 마십시오** — 새 요구사항을 세우는 대신 **그 규율의 적용 범위를 총괄이 정할 일**로 남깁니다.

### 7.5 🕐 이 절이 기존 절과 어긋나는 자리 — **총괄 판정 대기, 어느 쪽으로도 구현 금지**

**스스로와 어긋나는 스펙은 불완전한 스펙보다 나쁩니다.** 아래 다섯은 이 라운드가 **발견했고 판정하지 않은** 것입니다. 한쪽을 골라 문장을 고쳐 쓰지 마십시오.

| # | 어긋남 | 양쪽 |
|---|---|---|
| **①** | **화면이 저장할 수 없는 테이블에서 연다** | §6의 초기 테이블 선택(`stageTargetTables` — 선언된 stage 타깃 중 첫 맵 테이블)은 `dt_log`를 고르고, §6.0-ter 게이트 4는 같은 테이블의 Push를 거절합니다. **둘 다 자기 근거가 있습니다**(계획 stage 선언 ↔ 데이터 파괴 방지). 판정되어야 하는 것은 「stage 타깃이면서 push 불가한 테이블에서 여는 것이 옳은가」입니다 |
| **②** | **유효 다이 지정이 테이블 전환을 넘어 산다** | §5.4는 오버레이를 **테이블 전환에서 해제**한다고 계약하고 그 이유는 *「이전 테이블의 값을 새 테이블에 써 넣을 수 있었다」*입니다. 유효 다이 참조에는 대응 규칙이 이 문서에 **없고**, 실측은 `CORE_1X`가 `dt_log` 맵에서 **실효**였다고 보고합니다(R-U5) |
| **③** | **📐 표준의 「마스크 없음」이 참이 아니었다** | §4-bis.3은 📐 표준을 *「마스크가 사실상 없는 물리 기하 · 전 셀이 pushable」*로 정의하고 설계 원칙을 *「어떤 기본 선택도 Push 불가능한 맵을 만들면 안 된다」*로 적습니다. 그런데 그 분기가 세우는 것은 **원 프리셋**(`applyPresetObject`)이고, §3 #39가 이미 적는 대로 **유효 다이 맵이 지정돼 있으면 원이 아니라 그 맵이 이깁니다.** 실측(R-U8): 표준을 누른 뒤 캔버스의 26%가 마스크였습니다. **넘어온 지정이 있는 한 §4-bis.3의 보증은 성립하지 않습니다** — ②와 같은 뿌리입니다 |
| **④** | **레이어를 잃지 않는다는 문장이 레이어 보증으로 읽힌다** | §5.4-bis의 *「…레이어를 잃는 것도 아닙니다」*는 **선언되지 않은 값**에 대한 문장입니다. 실측은 레이어가 실제로 사라지는 경로 둘을 보입니다(정원 4개 초과 · `align_unavailable` 거절인데 뱃지는 셈). **문장의 범위를 좁힐 것인지, 보증을 실제로 세울 것인지**가 판정입니다 |
| **⑤** | **같은 화면이 「선언 안 된 값의 색」에 두 답을 낸다** | §5.4-bis는 오버레이 점에 대해 *「색을 지어내지 않는다 — 속이 빈 링」*을 계약하고 하네스가 소스 대조로 채점합니다. 메인 격자의 칠은 `UNLISTED_VALUE_FILL === LEGEND_PALETTE[0]`으로 **선언된 값 1과 구별 불가능**합니다(R-U10). **어느 쪽이 이 저장소의 규칙인가**가 판정입니다 |

⚠️ **표 밖의 하나 — 어휘가 갈린 것**: R-B1의 거절이 `dt_map`에서 `dt_index`를 지목하는데 `map_overlay_config.json`은 그 컬럼을 **dt_map 바인딩의 일부로 선언**합니다. §6.0-ter는 게이트 4의 커버를 *「맵 키 · 현재 바인딩된 X/Y/값 · 시스템 컬럼 · 합성 bk」*로 정의하고, §5.6-bis는 **바인딩의 단독 해석자를 서버**로 못박습니다(`resolve_binding_info`). **두 절이 「바인딩」의 외연을 다르게 쓰고 있을 가능성이 있고, 이 라운드는 코드를 읽어 확인하지 않았습니다.** 판정 전에 **양쪽을 실측하십시오** — 한쪽만의 근거로 차이를 주장하면 없는 결함이 세워집니다.
