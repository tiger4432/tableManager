...
> **Status:** 🟢 Living | **Last-verified:** 2026-07-30 (🔴 **§5.7-bis 두 번째 전면 개정 — 채택 자체가 폐기됐다(F8 `61440e6`+`94b9baa`)** · 사용자 지시 「그리드 크기가 달라도 좌표는 db값 그대로 보존하고 화면 표기 밀리게 그냥 보여주기」. **같은 날 아침 쓴 §5.7-bis가 같은 날 오후에 통째로 거짓이 됐다.** 소스 실측: `adoptFrameSpec`·`storedCoordRepositionPlan`·`applyStoredCoordReposition`·`repositionRefusalReason`·`adoptionCoordinateCost`·`adoptedFrameOf`·`dbCoordsByPhysKey`·`announceFrameAdoption`이 `client2/src/`에 **0건**. 지금 계약은 **어떤 축도 채택하지 않는다** — 치수가 다르면 마스크가 밀려 보이고, 거절도 확인창도 없이 **info 토스트 1회**만 낸다. 저장 좌표가 안 움직이는 이유는 **셀에 저장된 좌표라는 것이 없기 때문**이다(`gridData`는 물리 키→값, DB x/y는 Push 시점에 `pushMapData`가 유도) — 그래서 "보존"의 유일한 구현이 **칸을 그대로 두는 것**이다. 살아남은 거절은 **치수 정의역 `1~100` 정수**(`frameDimError`) 하나이고 근거가 바뀌었다(채택 4회 순회 → `projectCellsToPhys` 안 `getWaferBoundingBox`의 참조 치수 전수 순회 + `gridDimNum`/`parseInt` 오독). 하네스 실행 실측 **192 단언 · 변이 16/16 명명 포착**(직전 206은 F6 하네스의 수). F6 서술은 삭제하지 않고 `<details>` 아카이브로 접어 뒀다 — **"참조 규격이 메타에 있으니 그대로 열면 되지 않나"는 재유도하기 쉽고 실제로 그렇게 만들어졌다가 되돌려졌다.** 직전 **§5.7-bis 전면 개정 — 거절 규율이 재배치 규율로 바뀌었다** · `7873070`+`d4b9660` 소스 대조(`storedCoordRepositionPlan`/`applyStoredCoordReposition`/`repositionRefusalReason`/`frameDimBounds`/`frameDimError`/`announceFrameAdoption`) + `client2/tests/valid_die_frame_adoption_harness.mjs` 실행(206 단언, 0 실패)으로 수를 실측. 종전 §5.7-bis의 **네 서술이 거짓이 됐다**: ① 판정자가 `adoptionCoordinateCost`의 `moved > 0 || lost > 0`이 아니라 **`storedCoordRepositionPlan`**(표현 가능성 — `unrepresentable`/`stranded`/`collision`)이고 `adoptionCoordinateCost`는 **독립 대조 오라클**로 강등됐다(항등 `rekeyedWithValue == moved + lost`, 갈리면 console.error) ② *"⏳ 재키는 이 커밋에 없다"* → **`7873070`에 착지**했고 세 캐시가 같은 plan 하나로 이주하며 **서빙 키도 거절 사유**다(불변식 ④) ③ 45×45→46×46은 잡히는 케이스가 아니라 **채택·재배치되는 케이스**다(실측: 재배치 없으면 1272 이동 + 41 소실 / 있으면 2025 좌표 바이트 보존) ④ `wafer_map_metadata` ❌ 행의 근거에 **다음 Push가 메타와 셀을 함께 다시 쓴다**는 방향이 빠져 있었다(요청 2건·비트랜잭션). 신설: **H5 격자 치수 정의역 `1~100` 정수 거절**(clamp 아님 — 참조 셀은 `OVERLAY_CELL_LIMIT`이 막았는데 참조 치수는 아무도 막지 않았다) · H2 **한 수량에 한 수**(`plan.rekeyedWithValue` 하나, 0이면 문장 삭제) · H4 모집단별 거절 문구(collision 단독 = 프로그램 결함) · 🔴 **저장 좌표 = 오리진 기준 칸수이고 mm 주소가 아니다**(사용자 판정 2026-07-30 — 칸수에 피치를 곱해 읽으면 없는 결함이 만들어진다). 직전: 🟠 **§1에 미해결 불일치 1건 기록(총괄 판단 대기)** — `getVisualCoords`(:1892-1905)는 X에 **미러 분기가 없고**(`box.maxC`를 **참조하지 않음**) Y는 `invertY`만 보는데, §1의 표는 back 미러 갈래와 `isYMirrored` 두 행을 문서화한다. 역변환도 같은 무-미러 식이라 왕복은 성립하므로 어긋나는 것은 **문서와 코드** 사이다. 같은 파일 안 `c_zero`도 :1157(미러 없음)과 :2924(미러 있음)로 갈린다. **표는 원문 보존하고 사실만 기록** — 판정은 저장 좌표 규약의 문제라 총괄·QA 소관. 그 외 **정합 정정 4건 + 신설 3절** — 소스 대조: `client2/src/map_editor.js`(`dbCoordsByPhysKey`/`adoptionCoordinateCost`/`adoptFrameSpec`/`computeNotchCell`/`notchMarkCell`/`checkPasteAgainstFrame`/`applyRoutedPreset`, `clearOverlayLayers` 호출부 2곳), `client2/src/transfer_plan.js`(`matMapCacheHit`/`notifyMapContext`/`stackCrossesMarkerBoundary`). ① **§5.7의 "§5.1 저장 규약" 참조 정정** — §5.1은 클라 파이프라인이고 그 규약을 담지 않는다. 저장 좌표 규약의 정본은 **§1의 1)**(bbox 항이 든 변환 수식)이며 개념 서술은 [philosophy §2.3](../map_editor/philosophy.md). 앵커 드리프트. ② **§5.4 해제 알림 정정** — 앞의 둘이 `c24d47b`에서 **`console.debug`로 내려갔다**(해제 동작은 불변, 해제 지점은 여전히 셋). ③ **§5.8-bis 신설** — F5 클라 절반이 `73b5925`에 착지(`applyRoutedPreset`, `loadExistingMap`의 `!loadedGridMeta` 아래·모달보다 앞), 종전의 "HEAD `c9bf2c7` 기준 클라 소비자 없음"과 그 후속 의무를 이행: **§4-bis.3 모달은 폐기되지 않고 ⚙️의 뜻만 라우팅 규격으로 바뀐다**. ④ **§5.7-bis 프레임 채택 신설** — 채택하는 축/않는 축 + `ae2811c`의 거절 규율(**치수 변경은 물리 키 불변이 아니다** — `getVisualCoords`의 bbox 항이 저장 좌표를 재좌표화, 실측 DB(24,24)→(22,22)에 대조 게이트는 0 보고. 판정은 치수 비교가 아니라 **좌표 비교**, 재키는 의도적 미포함). ⑤ **§4-ter.4/.5 정정** — 지문 부재는 경고가 아니라 **거부**(다섯째 갈래 신설, 거부 4갈래→5갈래), 술어를 복사·붙여넣기가 공유(`notchMarkCell` — 칠해진 노치는 지문 없음, 값 `D`는 데이터), 경계는 **인자 rotation**으로 잰다. ⚠️ **선언 맵 179개 중 노치 on-grid 27개 = 나머지 152개에서 왕복 소실**, 양식 후속은 대기열. ⑥ **§6.1-ter 신설** — F4 `c24d47b`: 트리거 `changed || serverRead`(정체 변화와 서버 재독은 다른 질문) + 존재 캐시는 **긍정 답만**(`matMapCacheHit`, `has()`는 `false`/`null`도 확정 취급) + `ae2811c`의 STACK 마커 경계 재조회. 직전 **§4-ter 회사 양식 왕복 계약 신설 + §5.8 로드 시 프리셋 라우팅 신설** — 코드 대조: `client2/src/map_editor.js`(`copyTitleText`/`computeNotchCell`/`HDR_GAP_COLS`/`readCompanyMapBlock`/`checkPasteAgainstFrame`/`applyPastedGridRows`/`applyPastedAuxRows`/`onMapGridPaste`, paste 리스너 `document` 1개), `client2/src/doe_bands.js`(`IGNORED_HEADERS` **13개 실측**, `rollupToGrid` **importer 0건 실측**), `server/map_preset_routing.py`+`server/main.py`(엔드포인트·status 6종). §4-ter = **왕복 항등(INV-F1ⓑ-1)** · 평문을 읽는 근거 셋 · **병합 관례 INV-F1ⓑ-3** · 그룹 띠 **의도적 미판독**(자재는 왕복하지 않음) · `HDR_GAP_COLS` 단일 상수 · **노치 `D`의 세 역할** · **거부 네 갈래** · 적용 규율(**붙여넣기는 값을 지우지 않는다**·COUNT 폐기·COLOR 미왕복) · **로스터 4→13과 롤업 8단어의 PREPARATORY 지위**. §5.8 = **절대 우선순위 `wafer_map_metadata` > 라우팅 > 패널**(서버 강제)·**조회 miss는 정상 경로**·환경 분기 없음·`business_key_val` 지름길 기각. §3에 함수 #48 추가(47→48). 직전 **F2 `064550f` — 적재 대조 게이트의 대조축 정정(긴급)**: §6.0-ter·§2 #40의 "직렬화 수 < **화면의** non-empty 수" 서술은 F2 이후 **양변이 같은 술어**(`eachSavableCell`)라 "N < N"으로 읽혀 게이트를 죽은 코드로 오독시켰다 — 실제 분모는 **원시 `gridData`의 non-empty 키 수**라는 세 번째 값이므로 그 이름을 명시. 함께 §2 #39 `fillGrid` 용도를 유효 다이 집합 필터 + 건너뛴 칸 토스트로 갱신(종전 "원 영역 내부" 서술은 실제 동작이 아니었고, 그 격차가 Fill All 영구 거절의 원인). 직전 **M4 phase 1 — §5.7 유효 다이의 근거(`valid_die_ref`) 신설**: 원 기하가 판정자에서 생성기로 강등되는 첫 단계, **가산적 공존**(선언 없는 맵은 `2a9f6c4`와 동일)·참조 실패 시 **원으로 조용히 되돌아가지 않음**·참조 키는 7b 캐노니컬화 경유. 필드 규격 정본은 [architecture_and_management §2.3-bis](../map_editor/architecture_and_management.md), 양측 채점은 `contracts/map_seam/`. 직전 **7b/7c/M3 `ab6ac02` + 미반영 F1 `17f65bd`**: **§5.6-bis 서빙되는 좌표 바인딩 신설**(paint-rules `binding` = `{x,y,val,key_columns,source}`, 선언>유도, 클라 유도 ~40줄 삭제 — `fallback_guess`는 데이터 경로 거부·로드 경로 경고·오버레이 경로 **거부**), §5.1 `binding_unavailable` 뜻 정정(클라 유도 실패 → 서버 미해석/추측 거부), §5.2 소비자 표에 `binding` 추가 · **§5.0에 7b 맵 정체성 캐노니컬화 규율 신설**(선언 타입 기준 — `number`면 `'01'`=`'1'`, 구현은 `map_overlay.canonical_key_value` 하나, **등록 측도 경유**) + M3 착지로 열린 격차 🔴→🟠 재기술(인제션 경로는 닫힘·기존 39만 건 백필은 M4) · **§6.2-bis 선언된 미추적 소비 신설**(`transfer_log: "none"` → `connected(untracked)`, transferred null·remaining null + 진짜 상한, `"none"` 문자열만 유효). 직전 **Gate4/U6 `deed6d2`**: §6.0-ter 게이트 3종→**4종** — **로그형 대상 게이트** 신설(맵 계약 밖 데이터 컬럼 보유 테이블로의 Push를 모든 다이얼로그 이전에 거부, 판별은 구조적 — "페이로드/서버가 재구성하는 컬럼만 살아남는다", 합성 bk는 소스 전부 커버일 때만 생존, `map_push_ok: true` 사이트 선언 시 차단→소실 confirm 1회, JSON boolean만 유효) + **replace_map 응답 `scope: {filters, deleted, inserted}` 정직성 계약**(범위 파생 불가 = 무음 200 no-op 대신 400, 명시적 `scope` + 빈 updates = 합법적 전량 소거), §2 #40에 게이트 항목 추가. 직전 같은 날 **5b `0052d76`**: §4-bis.1 초안 보존 약속의 실체화(불일치 유지 시 로드 경로 영속 게이트) + legendDirty 로드 성공 리셋·프레임 왕복 스냅샷, **§4-bis.3 메타 없는 맵의 기본 프레임 신설**(데이터 bbox + 마스크 중립 기하 — 기본 선택이 Push 불가 맵을 만들지 않음), §6.4-bis 뒤로가기 가드 `frameTouched`·프레임 모달 취소 롤백, §6.0-ter 대조 게이트 행에 기본 프레임 변경 반영, 값 컬럼 후보 매칭 exact 통일(§5.6 잔여 이슈 해소). 직전 같은 날: **U6 `95bf072`**: **§5.6 서버 선언 맵 기본값 신설** — paint-rules가 `value_column_candidates`(RESOLVED)·`default_legend`(선언 그대로/null)를 서빙, 클라 하드코딩 6종 삭제(후보 2사본·builtin stage·팔레트 3사본·E1/E2 색), §6에 stage 선언 단일 소스(`/api/transfer-plan/stages`, builtin 폴백 삭제)·§4-bis.1에 U6-1 시드 갈래(0셀+행 0개 = 시드, 읽기 실패 = 행 보존). 직전 같은 날: **H1/H2 `6db517d`**: §4-bis.1 로드 경로 단일 영속·「복구」= 화면 변화 기준·복구는 미저장 표시, **§6.0-ter 데이터 보호 게이트 3종 신설**(적재 대조 게이트 추가). 직전 같은 날: **U9 STACK 0 마커**: §6 층 구조 행에 마커 의미론, §6.0-bis **V1~V6**(V6 = 마커 모순, 마커 행 유일 규칙), §6.1-bis에 **M1 위임 stage의 bin_map 무효** 명기 — 서버·클라 공히 vectors v3 채점. 직전 같은 날: **§4-bis 새로고침 생존 계약 신설**(지문 게이트 초안 + `map_editor_last_open` 재연 복원) · **§6.4 자재 이동 LOAD 동등성**(분해 불가 ID → 첫 키 컬럼 폴백) — `280ebf0`. 직전 같은 날: §6 DOE zone 모델 — STACK + 1H/MID/TOP이 band 모델을 대체, §6.0-bis 차단 규칙 V1~V5 신설, 폐기 `bands` 마이그레이션·거부 규칙) | **Owner:** UI/Map | **Source-of-truth:** `client2/src/map_editor.js`, `client2/src/transfer_plan.js`, `server/map_overlay.py`, `server/bonding_plan.py`, `server/transfer_plan.py`, `server/utils/coordinate_transformer.py` · 상위 [SYSTEM_OVERVIEW](../overview/SYSTEM_OVERVIEW.md)
>
...
> - **같은 파일 안에서도 두 식이 갈립니다**: (0,0)이 격자 위에 있는지 판정하는 `c_zero`/`r_zero`가 `getGridCellObject`(:1157-1158)에서는 **미러 없이**, `renderGridCanvas`(:2921-2930)에서는 **`isXMirrored`/`isYMirrored`를 적용해** 계산됩니다 — `side === 'back'`에서 두 값이 달라집니다.
...
### 4-bis.3 메타 없는 맵의 기본 프레임 — 데이터 bbox + 마스크 중립 기하 (5b `0052d76` · 총괄 설계)

`wafer_map_metadata`가 없는 맵을 로드하면 좌표계 선택 모달이 뜹니다(📐 표준 / ⚙️ 현재 좌측 패널 설정 / ❌ 취소). 설계 원칙은 하나입니다 — **어떤 기본 선택도 Push 불가능한 맵을 만들면 안 됩니다.**
...
- **바운딩 박스는 이 기능의 영향권 밖입니다.** `get_wafer_bounding_box`/`getWaferBoundingBox`는 계속 원으로 계산합니다 — 저장 좌표의 기준(**[§1의 1) 기하 변환 수식](#1-기하-변환-수식-geometric-transformation-formulas)** — `xv = c − box.minC + startX`)이 바뀌면 같은 맵의 좌표가 조용히 다른 수로 재해석되기 때문입니다. 원을 `inside`에서 빼는 것은 phase 3입니다.
...
### 5.7-bis 격자 크기가 다를 때 — **아무것도 채택하지 않는다** (F6 `73b5925`→`ae2811c`→`7873070`→`d4b9660` → **F8 전량 철회 `61440e6`+`94b9baa`** · 2026-07-30)

지정하려는 참조 맵의 격자가 이 맵의 격자와 다를 수 있습니다. 답은 **아무것도 채택하지 않는 것**입니다. 사용자 지시(2026-07-30): 「그리드 크기가 달라도 좌표는 db값 그대로 보존하고 화면 표기 밀리게 그냥 보여주기」.

- **유효 다이 지정은 어떤 축도 채택하지 않습니다** — 격자 치수도, 물리 규격(직경·칩 피치·offset·margin)도, 회전·면·원점도. 그 지정은 마스크 하나만 만들고 **화면 컨트롤을 한 글자도 건드리지 않습니다**.
- **참조 격자가 다른 크기면 마스크가 화면에서 밀려 보입니다.** 마스크 키는 `projectCellsToPhys(cells, refFrame)`가 **참조 자신의 프레임**으로 만듭니다(화면 컨트롤을 읽지 않습니다) — 채택이 하던 일은 타깃의 인덱스 공간을 참조의 것과 **맞춰** 겹쳐 보이게 하는 것뿐이었습니다. 맞추지 않으면 밀립니다. **요구된 화면이 정확히 그것입니다.**
- **저장 좌표는 하나도 움직이지 않습니다.** 셀에 저장된 좌표라는 것은 **없습니다** — `gridData`는 `물리 키 → 값`뿐이고, DB의 x/y는 Push 시점에 `pushMapData`의 직렬화 루프가 `cellObj.x/.y`(= 현재 프레임으로 **렌더가 유도한** 좌표)에서 만듭니다. 그래서 "좌표를 보존한다"의 유일한 구현은 **칸을 그대로 두는 것**입니다. 치수를 채택하면 같은 칸이 다른 좌표를 낳습니다 — F6이 재배치 기계장치로 막으려던 것이고, 그 기계장치는 새 프레임이 만들지 못하는 좌표 앞에서 셀을 버리거나(삭제) 번호를 다시 매기는(재좌표화) 수밖에 없었습니다. **둘 다 금지입니다.**
- **치수가 다르다는 이유로 거절하지 않습니다.** 거절도 확인창도 없습니다. 밀림은 원인이 화면에 남지 않는 차이라(격자는 멀쩡해 보이고 마스크만 어긋납니다) 조용하면 운영자가 데이터를 의심합니다. 그래서 **info 토스트 1회**로만 말합니다(`dedupeKey: 'valid_die_dims_differ'` · 낡은 세대의 해석은 토스트도 내지 않습니다).
...
- **호출 조건과 위치가 계약입니다.** `if (!loadedGridMeta)` 아래, 그리고 **좌표계 선택 모달(§4-bis.3)과 `'standard'`/`'current'` 분기보다 앞**입니다. 그 분기가 좌측 패널을 읽으므로, **라우팅이 패널을 먼저 정해야 "라우팅 > 패널"이 성립**합니다. 로드당 정확히 1회이고, 메타가 있는 맵에서는 애초에 호출되지 않습니다(§5.8의 절대 우선순위가 클라 쪽에서도 구조로 성립).
- **§4-bis.3은 폐기되지 않았습니다.** 라우팅이 답해도 메타 없는 맵의 **좌표계 선택 모달은 그대로 뜹니다.** 바뀐 것은 모달의 ⚙️ *"현재 좌측 패널 설정"*이 이제 **라우팅이 채워 넣은 규격**을 뜻한다는 것이고, 📐 표준(데이터 bbox + 마스크 중립)은 종전 그대로입니다. 즉 **라우팅은 패널의 기본값을 갈아끼우고, 모달은 여전히 사람에게 묻습니다** — 복원 경로에서 물어야 하는 것을 묻는다는 §4-bis 규율과 같은 형태입니다.
