...
>
> ### 직전 라운드 (2026-07-31 · `35e84c3`/`1e4f23c` · `4761a3a`/`39eebe1`)
> - **§1의 0-요약 신설 — 맵 기준 가치 6줄**(사용자 확정). 좌표 규약의 **상위**이고 정본은 [보드](../process/PROJECT_STATUS.md)입니다. 여기 있는 것은 사본이며 고칠 권한은 총괄에게 있습니다.
> - **§1-bis 신설 — 좌표 공간 개명**(`35e84c3`). `getVisualCoords`→**`getDbCoords`** · `getPhysicalCoords`→**`getDieIndex`**(mm가 아니라 **칸 번호**) · `getCellFrom*`→`getCanvasCellFrom*` · `xv`/`yv`→`dbX`/`dbY`. 🔴 **`mm`은 일부러 비워 뒀습니다.**
