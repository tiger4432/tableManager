...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-17 Profile 2단계 승인 · LedgerFrame Chain mapper 3단계 `AWAITING_REVIEW` / `NOT_APPROVED` — 코드 대조
> 
...
| **정준 원장(ledger) — 개체·주장·원천 사건의 정본** | 작성 순서 [ONTOLOGY_LEDGER_SETUP](../guide/ONTOLOGY_LEDGER_SETUP.md) · 실행 경계 [LedgerFrame Chain Mapper](../architecture/LEDGER_FRAME_CHAIN_MAPPER.md) · 계약 [LEDGER_TECHNICAL_SPEC](../spec/LEDGER_TECHNICAL_SPEC.md) · [Evidence Subgraph](../spec/LEDGER_EVIDENCE_SUBGRAPH_SPEC.md) · 판정 [LEDGER_RULINGS](../process/LEDGER_RULINGS.md) | `server/ledger/*` — `source_profile.py`(승인된 2단계 canonical Profile + readiness), `ledger_frame.py`(pandas Claim 경계), `chain_mapper.py`(trusted registry/호출), `profile_chain_mapper.py`(column·constant·declared_lookup), `backfill.py`/`dry_run.py`(기존 cursor가 동일 mapper 호출), `server/mappers/ledger_lot_event_mapper.py`(첫 실제 source 전환), 기존 gate/store/trace/structure/trends/composition. 3단계는 `AWAITING_REVIEW`/`NOT_APPROVED` |
