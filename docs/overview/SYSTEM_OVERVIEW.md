...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-23 원장 셋업 완주 · 은퇴 울타리 — **사실 동기화만 받았고 아키텍처 «결정»은 하나도 건드리지 않았습니다**(⚠️ 총괄 검수 대상). 직전 2026-08-18 Ontology Config Explorer `COMPLETE / APPROVED` (`2d1ad863`)
> 
...
| 경로 | 무엇 |
|---|---|
| `server/config/ontology/ledger_config.json` | **Ledger v2 셋업 정본 — «의미»만.** 🔴 **[2026-08-21] `setup_version: 5`, 필수 section «셋»**(`entities`·`sources`·`vocabulary`). 소스 하나가 실행 순서대로 다섯 절을 **직접** 든다 — `relation`·`read`·`prepare`·`map`·`bind`. 🔴 **[2026-08-21] `packs`·`claims`도 은퇴했다** — 문장이 술어를 «직접» 든다(`bind.mappings.<문장>.predicate`), Role은 `vocabulary`가 강제한다(`9b6c5da0`). `source_preparers`·`mappers`·`profiles` 세 section은 은퇴했고 **본문이 그것을 쓰는 소스 안으로 들어갔다**(`087e7d8`·`d64f047e`·`a55f3059`); `bind.mappings`는 **mapper가 선언한 문장 별명으로 키가 매겨진다**(`mapping_id` 은퇴, `e795c706`). 옛 세대는 `server/scripts/migrate_ledger_config_to_v4.py`가 올린다. **이 root에는 다른 `.json`을 두지 못한다.** 🔴 **[2026-08-18 판정] 물리 스키마는 여기 없다** — `table_config.json` 하나가 정본이고, 옛 `tables` section을 다시 적으면 `unknown_field`로 거절된다(사본 둘을 대조하는 코드가 없어 조용히 갈라졌다). 원장은 그 파일에서 `column_types`·`composite_key_source`·`business_key`를 읽고, **실물 DB 대조는 `schema_drift`가 이미 한다**. 🔴 **[2026-08-22~23 · `setup_version`은 5 그대로]** 선언에서 낱말 넷이 더 없어졌다 — vocabulary의 **`layer`**(이제 `unknown_field`)와 binding의 **`binding_origin`·`approval_status`·`suggestion_reason`**(검증기가 **받아서 버린다** — 옛 파일은 그대로 열린다). **「binding 승인」이라는 실행 관문은 없다.** 그리고 **`read.cursor`는 묻지 않고 `read.order_by`에서 덮어써 파생**된다. 작성법은 [ONTOLOGY_LEDGER_SETUP §5·§7·§13.3-quater](../guide/ONTOLOGY_LEDGER_SETUP.md) |
...
| **정준 원장(ledger) — 개체·주장·원천 사건의 정본** | 작성 순서 [ONTOLOGY_LEDGER_SETUP](../guide/ONTOLOGY_LEDGER_SETUP.md) · 실행 경계 [LedgerFrame Chain Mapper](../architecture/LEDGER_FRAME_CHAIN_MAPPER.md) · 🔴 **설정 선언의 정본은 위 [ONTOLOGY_LEDGER_SETUP](../guide/ONTOLOGY_LEDGER_SETUP.md) 하나다** · 🗄️ **[2026-08-21] 재설계 계획(1~7단계 완료·닫힘)은 [_archive/ledger_v2_redesign_plan_20260817](../_archive/ledger_v2_redesign_plan_20260817/README.md)로 이관** — 1단계 실측·단계별 수락 근거·목표 구조·[Mapper 패턴](../_archive/ledger_v2_redesign_plan_20260817/MAPPER_DESIGN_PATTERN.md) 26편이 그 폴더 안에 있고, **가르치는 config 문법은 은퇴했다**(`setup_version: 2` · `packs`·`profiles`를 낀 일곱 절 · `profiles.use → packs/claim`). **세팅 지침으로 읽지 말 것** — 「왜 그렇게 됐는가」의 역사로만 연다 · 계약 [LEDGER_TECHNICAL_SPEC](../spec/LEDGER_TECHNICAL_SPEC.md) · [Evidence Subgraph](../spec/LEDGER_EVIDENCE_SUBGRAPH_SPEC.md) · 판정 [LEDGER_RULINGS](../process/LEDGER_RULINGS.md) | `server/ledger/*` — gate/store/cursor/LedgerFrame/read API를 유지한 v2 1~7단계가 승인·main 병합됐다. 🔴 **[2026-08-18] 셋업은 파일 하나(`server/config/ontology/ledger_config.json`)이고 «선언이 곧 활성화»다** — `manifest.json` 다섯 파일 모양과 `mode`/`parity_status` selector, 문법 드라이버 넷이 함께 은퇴했다. 로드 경계는 `server/ledger/setup.py`의 `load_setup`이고 백필 드라이버는 하나다. 🔴 **[2026-08-21] `setup_version: 5` — 소스 하나가 `relation`·`read`·`prepare`·`map`·`bind`를 직접 든다**(위 config 표 참조). 기존 cursor shape/version은 자동 변환하지 않으며 운영 reset·legacy 삭제는 별도 승인 전까지 잠겨 있다.<br>🔴 **[2026-08-23 · 사실 동기화] 이 서브시스템은 «두 반쪽»이다 — 쓰기·셋업은 살아 자라고 읽기 면의 절반은 «얼어 있다».** 소유자 순서가 ① 셋업 완주 → ② 응용 → ③ 은퇴이고, ③이 데려갈 모듈이 파일 «이름» 글롭이 아니라 **파일 단위 실측 목록**으로 그어졌다(이름으로 그었을 때 넷이 빠져나갔고 그 넷이 resolver·어휘·계보 술어를 데려간다). ⛔ 얼음: `ledger_trace.py`·`ledger_trace_router.py`·`ledger_admin.py`·`ledger/config.py`·`ledger_explorer.py`·`ledger_structure.py`·`ledger_journey.py`·`ledger_lots.py`. ✔ 안 얼음: `ledger_subgraph`·`ledger_catalog`·`ledger_composition`·`ledger_selection`·`ledger_siblings`·`ledger_trends`. **얼어 있는 모듈 위에 새 일을 얹지 않는다.** 목록의 정본은 [PROJECT_STATUS](../process/PROJECT_STATUS.md) 2026-08-23 10:3x. ✅ **셋업 완주의 수락 사건은 소유자가 «폼만으로» 소스 하나(`transfer_event`)를 만들어 원자를 얻은 것**이다 — 절차는 [ONTOLOGY_LEDGER_SETUP §13.3-quater](../guide/ONTOLOGY_LEDGER_SETUP.md). |
