...
| `chain_rules.json` | 체인 인제션 규칙(trigger→target, mapper). 🔴 **[2026-09-05] 「규칙 하나 = 엣지 하나」가 아니다** — `allow_map_metadata_upsert` 를 선언한 규칙은 `target_table` 말고 **맵 메타데이터 표에도** 쓰고 그 쓰기가 자기 체인 이벤트를 낸다. 로드 시점 순환 검사가 **엣지를 둘 다** 세우며, 종전에 앞의 하나만 보아 «살아 있는 순환»이 통과했다. ⚠️ 그래프의 출발점은 `trigger_table` 이고 `source_table` 은 맵퍼가 «읽는» 곳이라 엣지가 아니다. 계약 [chain_ingestion_guide](../guide/chain_ingestion_guide.md) · 🆕 **[2026-09-15] 통합 선언도 이 파일에 적는다** — `derive: {kind: join \| decide}` + `on` + `into`(S-237 join `builtin:join_into` · S-239 decide). `enabled: false` 는 규칙이 «안 선다»(판정 399); `ASSY_CHAIN_SYNTHESIZE` 는 옛 파일 파생분만 끈다. 키는 [config/chain_rules §5-B-bis](../guide/config/chain_rules.md) |
