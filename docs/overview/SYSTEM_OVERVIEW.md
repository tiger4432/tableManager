...
| 경로 | 무엇 |
|---|---|
| `server/config/ontology/ledger_config.json` | **Ledger v2 셋업 정본 — «의미»만.** 🔴 **[2026-08-21] `setup_version: 4`, 필수 section «넷»**(`vocabulary`·`entities`·`packs`·`sources`) + 선택 `virtual_joins`. 소스 하나가 실행 순서대로 다섯 절을 **직접** 든다 — `relation`·`read`·`prepare`·`map`·`bind`. `source_preparers`·`mappers`·`profiles` 세 section은 은퇴했고 **본문이 그것을 쓰는 소스 안으로 들어갔다**(`087e7d8`·`d64f047e`·`a55f3059`); `bind.mappings`는 **mapper가 선언한 문장 별명으로 키가 매겨진다**(`mapping_id` 은퇴, `e795c706`). 옛 세대는 `server/scripts/migrate_ledger_config_to_v4.py`가 올린다. **이 root에는 다른 `.json`을 두지 못한다.** 🔴 **[2026-08-18 판정] 물리 스키마는 여기 없다** — `table_config.json` 하나가 정본이고, 옛 `tables` section을 다시 적으면 `unknown_field`로 거절된다(사본 둘을 대조하는 코드가 없어 조용히 갈라졌다). 원장은 그 파일에서 `column_types`·`composite_key_source`·`business_key`를 읽고, **실물 DB 대조는 `schema_drift`가 이미 한다**. 작성법은 [ONTOLOGY_LEDGER_SETUP §5](../guide/ONTOLOGY_LEDGER_SETUP.md) |
