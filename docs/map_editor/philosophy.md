...
> **Status:** 🟢 Living | **Last-verified:** 2026-07-31 | **Owner:** UI/Map
> 
> ### 이번 라운드 (2026-07-31 · `9d7d9a4`)
> - **§2.3 — 반응의 네 번째 호출 자리(격자 `COLS`/`ROWS`)** 등재. 성질(치수 변경이 저장 좌표를 옮긴다)은 그대로이고 그 이동을 상쇄하는 반응이 이제 그 입력에도 걸립니다. ⚠️ **드리프트는 이동이 아니라 *차이*** — `box.minC`가 그대로여도 전량이 재번호될 수 있습니다.
>
> ### 직전 라운드 (2026-07-31 · `35e84c3` + `4761a3a`)
> - **§2.1/§2.2 개명 반영** — 「물리 좌표계」 → **다이 인덱스**(`getDieIndex`) · 「시각 좌표계」 → **저장 좌표계**(`getDbCoords`). 두 옛 이름은 자기가 돌려주는 것의 **반대**를 말하고 있었습니다. 대응표는 [MAP_EDITOR_SPEC §1-bis](../spec/MAP_EDITOR_SPEC.md).
> - **§2.3의 결론이 네 번째로 갱신됐습니다** — 기하가 참조를 따라가고(가치 1), 반응이 근거를 가리지 않습니다(선언 없는 맵의 기하 편집도 같은 재배치).
> 
...
### 2.1 다이 인덱스 (Die Index — 구 "물리 좌표계")
* **정의**: 기판(Wafer)의 물리적 기준면에 고정된 절대 좌표계입니다. (Rotation 0, Side FRONT 상태의 좌표와 1:1 대응)
* **용도**: 자바스크립트 내부 캐시인 `gridData`의 Key(`"${x}_${y}"`)로 활용됩니다.
  > 🔴 **밀리미터가 아니라 칸 번호입니다.** 2026-07-31(`35e84c3`)에 `getPhysicalCoords` → **`getDieIndex`**로 개명한 이유가 그것입니다 — "physical"이 mm처럼 읽혀 칸수에 피치를 곱한 오독이 반복됐습니다. 대응표는 [MAP_EDITOR_SPEC §1-bis](../spec/MAP_EDITOR_SPEC.md).
* **변환 함수**: `getDieIndex(c, r, cols, rows, rotation, side)` *(구 `getPhysicalCoords`)*
  * 화면 상의 셀 인덱스 `(c, r)`를 현재 회전각과 앞뒷면 상태를 반영하여 Wafer 기준의 고정 다이 인덱스 `(x, y)`로 변환합니다.
...
### 2.2 저장 좌표계 (DB Coordinates — 구 "시각 좌표계")
...
* **변환 함수**: `getDbCoords(c, r, cols, rows, rotation, side, invertY, startX, startY)` *(구 `getVisualCoords` — 이름이 "visual"이었지만 돌려주는 것은 **DB에 들어가는 수**였습니다)*
...
### 2.3 저장 좌표계가 화면에 고정하는 것과 하지 않는 것 (2026-07-30 정정 · 2026-07-31 개명 반영)
...
```
dbX = colVisual − box.minC + startX
box = getWaferBoundingBox(rotation, side)
```

> 🟠 위 식은 `getDbCoords`(`client2/src/map_editor.js:2033`, 구 `getVisualCoords`) **2026-07-31 실측 그대로**입니다 — X에 **미러 분기가 없고** Y는 `invertY`만 봅니다. [MAP_EDITOR_SPEC §1](../spec/MAP_EDITOR_SPEC.md)은 back 면의 미러 갈래(`box.maxC − c + startX`)와 `isYMirrored` 행을 문서화하고 있으나 **코드에 대응물이 없습니다**(총괄 판단 대기 — 그 절의 🟠 블록 참조). 아래 결론(**원점은 `box`의 함수이고 `box`는 회전·면·치수의 함수**)은 어느 쪽으로 판정되어도 유지됩니다.

이고, `box`는 **회전·면의 함수**입니다. 즉 `dbX = 0`이 화면의 몇 번째 칸인지는 `box.minC`가 정하며, 그 값은 회전에 따라 달라질 수 있습니다.
...
> 🔴 **그래서 저장 좌표는 격자 치수에도, 유효 다이 근거에도 민감합니다.** `box`는 격자 치수(`cols`/`rows`)로 **유효 셀**을 훑어 만들므로, ① **치수를 바꾸면** 물리 키가 그대로여도 저장될 `dbX/dbY`가 함께 움직이고, ② **유효 다이 근거가 `ref`로 해석되면 `box` 자체가 마스크의 최소 사각형이 됩니다**(`da8f390` — 종전 이 문단은 `box`가 언제나 원이라고 전제했습니다). 회전은 물리 키에 불변이지만 **치수 변경도 근거 변경도 불변이 아닙니다.**
>
> ⚠️ **여기서 나오는 결론이 네 번 뒤집혔습니다.** ① 「참조 크기로 격자를 열고(채택) 좌표를 재배치한다」(F6) → ② 「아무것도 채택하지 않고 **칸을 그대로 둔다**」(F8) → ③ 「근거가 바뀌면 `box`가 움직이므로 **각 셀을 자기 저장 좌표가 가리키는 칸으로 다시 앉힌다**」(`da8f390`) → ④ **지금**(2026-07-31 `35e84c3`+`4761a3a`).
>
> **④가 ③에 더한 것 둘:**
> - **기하는 참조를 따라갑니다.** 유효 다이를 지정하면 물리 규격을 갈아끼우고 격자 치수를 **그 규격에서 파생**시킵니다(사용자 기준 가치 1). 폐기된 것은 참조가 **선언한** 치수를 **베끼는** 동작이지 기하를 따라가는 것이 아닙니다.
> - **반응은 근거를 가리지 않습니다.** 유효 다이 선언이 없는 맵에서 유효 영역은 곧 웨이퍼 원이므로, **기하 프리셋·피치 편집도 같은 재배치를 탑니다** — 그래서 구현이 `reseatCellsToStoredCoords` **하나**로 수렴했습니다. **2026-07-31 `9d7d9a4`에 격자 `COLS`/`ROWS` 직접 입력이 네 번째 호출 자리로 합류**했고, 함수는 여전히 하나입니다. ⚠️ 그 경로에서 배운 것 하나: **드리프트는 이동이 아니라 *차이***여서 `box.minC`가 그대로여도 전량이 재번호될 수 있습니다(`QERWER` 23→22열 · 261/261) — 「기준이 안 움직였으니 반응할 것도 없다」는 술어를 쓰지 마십시오([PRIMITIVES §4](../architecture/PRIMITIVES.md)).
>
> 붙드는 것은 번호이고 칸은 파생입니다. 계약은 [MAP_EDITOR_SPEC §5.7/§5.7-bis/§5.7-ter](../spec/MAP_EDITOR_SPEC.md), 좌표 규약 자체는 같은 문서 **§1의 0)**, 그 상위의 기준 가치는 **§1의 0-요약**입니다. 🔴 F6의 프레임 채택 기계장치는 **삭제됐습니다**(`61440e6`+`94b9baa`) — 이름을 찾아도 소스에 없습니다.
