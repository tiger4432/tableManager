...
> **Status:** 🟢 Living | **작성:** 2026-07-28 · Server PM | **Last-verified:** 2026-09-05 (정적 페이지 목록이 «두 번째로» 낡아 — `graph.html`·`trace.html` 제거 — **목록을 지우고 `vite.config.js` 를 가리키게 바꿨다**) · 직전 2026-08-05
>
...
- `admin.html`·`map_editor.html`은 `no-store`로 서빙되므로 새로고침 한 번이면 새 해시를 집는다. **루트 `index.html`에는 그 헤더가 없다** — 강제 새로고침이 필요할 수 있다. (⚠️ **[2026-08-11] `enrichment.html`은 삭제됐다.** 🔴 **[2026-09-05] 그리고 이 목록이 대던 `graph.html`·`trace.html` 도 «없다»** — `45d8b66f` 에서 제거됐다. **이 자리에 목록을 다시 적지 않는다** — 두 번 낡았고, 정본은 `client2/vite.config.js` 의 `rollupOptions.input` 이다.)

---

## 8. 드릴 — 이 절차는 실제로 실행됐다 (2026-07-28)

운영이 아니라 **격리 스택**에서 돌렸다: 전용 데이터 루트(`ASSY_DATA_ROOT`), `assy_qa` 스냅샷 DB, API :8082, 그래프 :8092, 런처(`run_decoupled_app.py --server-only`)로 **당시 5개 프로세스 전부** 기동. ⚠️ **[2026-08-14] 이 드릴은 2026-07-28 기록이고 그때의 구성입니다** — 그래프 워커가 은퇴해 오늘 같은 드릴은 프로세스가 하나 적습니다(:8092도 없습니다).
