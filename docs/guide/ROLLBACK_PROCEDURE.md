...
| **`ASSY_ADMIN_TOKEN`이 설정된 환경** | 워커는 런처 기동 시점의 환경을 복사해 물려받는다. 부분 재기동으로 워커가 옛 토큰을 든 채 남으면 `/internal/events/*`가 **401**을 돌려주고 **실시간 동기화가 조용히 멈춘다** — 화면은 멀쩡하고 데이터만 안 들어온다. 증상은 워커 로그의 `API notification failed: ... -> 401`뿐이다 |
