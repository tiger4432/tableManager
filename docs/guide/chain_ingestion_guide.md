...
📎 심볼과 줄 번호는 [CODE_MAP §🆕㉗](../architecture/CODE_MAP.md) 가 정본입니다.
📎 `server/parsers/notebooks/parser_workbench.ipynb` 는 이것의 «전신»입니다 — ⚠️ **그리고 «아직 있습니다».**
실측(2026-09-12): 그 파일은 «미추적이 아니라» `2a6adb50`(08-19)부터 추적돼 왔고, 새 노트북과 «다른 파일»입니다
(diff +166 / −376 — 새것이 훨씬 작습니다). 그러므로 「옮겨졌다」가 아니라 «둘이 나란히 있다»가 오늘의 사실이고,
옛것을 어떻게 할지는 소유자 몫입니다. 이 문서가 «가리키는 정본»은 `server/notebooks/` 의 둘입니다.
