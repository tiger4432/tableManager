...
return {
  "updates": [{
    ...
    "origin_row_id": "<이 값을 읽어 온 «입력 행»의 row_id>"   # 🆕 선택 — 아래를 읽을 것
  }]
}
```
...
> ⚠️ **「말 안 함」이지 「못 함」이 아닙니다.** 이 칸은 **모든 맵퍼가 이미 짓는 `GeneralUpdateItem` 에 있으므로** 파일 맵퍼도 «적을 수 있습니다». 다만 제품은 «라이브 맵퍼가 적는지»를 셀 수 없어서(`server/mappers/` 는 소유자의 것) 삭제 때마다 그 규칙 이름을 대며 한 줄 남깁니다(`chain/rule_run.py::retraction_refusal`).
...
config load and **names every cycle it finds** (`_validate_chain_cascade_graph` — one
INFO line per trail per process, and the trails come back as a value the admin graph
draws). ⚰️ **It used to REFUSE the load (판정 402, 2026-09-15 `2504fbde`).** A loop such
as `dt_log → dt_inventory` by mapper and back by join is an intended shape; what keeps it
finite is `max_chain_depth`, and nothing refuses a cycle at load, at save
(`save_chain_rule_raw`) or at replay. Three field names decide what that graph sees, and
two of them are easy to confuse:

| 필드 | 그래프에서 하는 일 |
|---|---|
| `enabled` | 켜는 키. `false` 인 규칙은 그래프에 **안 들어간다**(생략하면 켜짐). 실행 순서 유도(`chain/rule_order`)도 같은 답이다 — 꺼진 규칙과 `follow_up` 규칙은 «돌 수 없는 엣지»라 이웃을 정렬하지 않는다(2026-09-15 `ea8f91d2`, 운영이 만난 가짜 고리). 통합 선언(`derive`)에서는 로더가 «규칙 자체를 안 세운다»(판정 399) |
...
(`chain/ingestion_worker.py`, the `execute_custom_mapper` call sites). Returning a
...
[`server/chain/key_gate.py`](file:///c:/Users/kk980/Developments/assyManager/server/chain/key_gate.py),
...
⚰️ **[2026-09-15 판정 402] The guard still SEES this loop; it no longer refuses it.** It
returns the trail (the admin graph shows it) and says it once; `max_chain_depth` is what
bounds the loop at run time.
...
>
> 🔴 **`.sample`은 씨앗이지 거울이 아니다 — 그리고 룰이 가리키는 모듈이 없어도 아무도 말해 주지 않는다.** `chain_rules.json`의 `mapper_module`은 **로드 시점에 검증되지 않는다**. 유일하게 import를 시도하는 곳은 워커 웜업(`chain/ingestion_worker.py:630-639`)인데 ⓐ `enabled:false` 룰은 **건너뛰고** ⓑ 실패해도 `[Warmup] Mapper pre-import failed` **경고 한 줄 뒤 계속 기동**한다. 그래서 룰이 존재하지 않는 모듈을 가리켜도 활성화 전까지는 완전히 조용하고, 활성화하면 그때부터 해당 트랜잭션 그룹이 통째로 실패한다. 2026-08-02에 `mappers.dt_map_mapper`가 실제로 그 상태였다(`.sample`만 있고 `.py`가 없었다). 그리고 라이브와 `.sample`을 동기화하는 장치는 **없다** — `production_mapper.py`와 그 `.sample`은 이미 서로 다른 파일이다(함수 구성도 다름). 라이브를 고칠 때 샘플을 따라 고칠지는 **사람이 결정**해야 합니다.
...
## 📓 1-ter. 워크벤치 — 노트북 «둘», 고치는 셀은 «하나» (S-197, 2026-09-12 신설)

```
server/notebooks/mapper_workbench.ipynb   (셀 13)
server/notebooks/parser_workbench.ipynb   (셀 15)
```
위 §1-bis 의 `dev_bench` «그대로» 씁니다 — 노트북은 «부품을 새로 만들지 않습니다».
`server/tests/test_the_workbench_notebooks_call_production.py` 가 그것을 채점합니다(셀이 컴파일되나 ·
벤치 «말고» 다른 것을 import 하지 않나 · 벤치가 하는 일을 다시 구현하지 않나 · 이 박스의 이름이 실려 있지 않나).

### 🔴 고치는 셀은 «하나»입니다 — 나머지는 그대로 실행합니다
```
맵퍼   0 부트스트랩 → 1 «내가 고를 것» → 2 입력 → 3 만들기(자유폼) → 4 발행 → 5 시험으로 남기기(선택)
파서   0 부트스트랩 → 1 «내가 고를 것» → 2 파일에 무엇이 있나 → 3 읽기(자유폼) → 4 가공(자유폼)
      → 5 «누가» 집나 → 6 발행
```
🔵 **입력은 «운영이 넘기는 모양 그대로»입니다** — `dev_bench.input_for_mapper(SOURCE, rows=ROWS)`.
표 이름이든 샘플 파일이든 같은 함수이고, 그래서 노트북에서 본 프레임과 워커가 주는 프레임이
«같은 것»입니다(§1-bis 의 「CLI 와 시험이 같은 함수를 지난다」와 같은 이유).
⚠️ `autoreload` 가 걸려 있어 «편집이 즉시 반영»됩니다 — 그리고 그것도 시험이 단언합니다.

### 🔴 파서는 「읽기」부터 시작하지 「파싱」부터가 아닙니다
```
2 셀   `dev_bench.raw_for_parser(...)` — 원문 몇 줄과 «인코딩 사다리»의 결과
       🔴 `UNREADABLE`(「DataFrame 으로 못 읽음」)이 «값»입니다. 파서 작업은 「이 파일이 무엇인가」를
          모르는 데서 시작하므로 «못 읽었다»가 예외가 아니라 «답»이어야 합니다
5 셀   `dev_bench.claimers(...)` — 이 파일을 «무는» 파서가 누구인가를 제품과 «같은 방식»(각 파서의 `match()`)으로
```

### 🔴 발행은 파일을 쓰는 일이 아니라 «대조»입니다
```
publish_mapper(name, params, body, frame=DF, expected=…)   -> 파일을 쓰고, 다시 import 해서
publish_parser(name, read_body, process_body, file=…, …)      «같은 프레임/파일»에 돌려 «같은 결과»인지 본다
어긋나면                                                    지우고 `PublishRefused` 로 «이름 대어» 거절
```
🔴 **왜 대조가 필요한가:** `inspect.getsource` 도 복붙도 「마지막으로 «실행된» 셀」을 내보내지
「화면 위의 셀」을 내보내지 «않습니다». 발행한 것이 방금 본 것과 같다는 보장은 «다시 돌려 보는 것»뿐입니다.
⚠️ **프레임은 «넘깁니다», 다시 유도하지 않습니다** — 다시 유도하면 창(`rows=`/`where=`)이 다시 돌아
«다른 창 = 다른 답»이 되고, 대조가 «본문과 무관한 이유»로 실패합니다.
🔵 마지막 셀이 그 결과를 §1-bis 의 «샘플 폴더»로 남깁니다 — 그러면 시험이 «하나» 늘어납니다.

📎 심볼과 줄 번호는 [CODE_MAP §🆕㉗](../architecture/CODE_MAP.md) 가 정본입니다.
⚰️ **`server/parsers/notebooks/` 는 은퇴했습니다**(S-197-d `4519a2df`) — 디렉터리째 없습니다.
이 문서가 가리키는 정본은 `server/notebooks/` 의 «둘»이고, 이제 «하나»의 자리입니다.
📌 **이 한 줄이 하루에 두 번 뒤집힌 이유를 적어 둡니다 — 갈랐던 것은 «도구»입니다.**
`git check-ignore` 는 「ignore 규칙에 걸리나」를 답하지 「추적되나」를 «답하지 않습니다»(추적 파일도 패턴에
걸립니다). 추적 여부의 철자는 `git ls-files --error-unmatch` 이고, 그 차이가 옛 노트북을 「미추적」에서
「추적·중복」으로, 그리고 「소유자 몫」에서 「은퇴 지시」로 바꿨습니다.
...
> 기준 스펙: [ENRICHMENT_QUEUE_SPEC.md](../spec/ENRICHMENT_QUEUE_SPEC.md) · 구현: `server/enrichment/config.py`, `server/enrichment/mapper.py`
...
#### R1 은 «얼마나 세게 밀지»를 받는다 (`--pace`, 2026-09-02)

```bash
conda run -n assy_manager python server/scripts/chain_replay_cli.py replay <룰> --pace slow --apply
```

- **이름을 고르지 숫자를 고르지 않습니다.** `fast`(기본 — **여태까지와 정확히 같다**: 상한 없음·쉼 0) · `slow` · `trickle`. 각 이름이 무엇을 뜻하는지와 「언제 쓰나」는 선언이 든다.
- 🔴 **표는 `server/pacing.json` «하나»이고 여기 사본을 만들지 않았습니다.** 원장 백필·파일 인제션도 **같은 표**를 읽습니다(계약은 [backend §4](../architecture/backend.md), 재사용 관점은 [PRIMITIVES §6](../architecture/PRIMITIVES.md)). 노브를 작업마다 만들면 운영자가 작업마다 다른 숫자를 외워야 합니다.
- 🔴 **여기서 한 «단위»는 «페이지»입니다.** 같은 `slow` 가 파일 인제션에서는 «청크»를 뜻합니다 — 표가 선언하는 것은 «리듬»이지 «양»이 아닙니다.
- 🔴 **모르는 이름은 «첫 페이지 전에» 거절됩니다**(`resolve_pace` → `ReplayRefused`). 반쯤 돌고 나서 거절하면 그 실행은 되돌릴 것과 이어 갈 것을 둘 다 갖습니다. 오타를 안은 채 전속력으로 도는 것도 막습니다.
- ⚠️ **쉼은 «자기 페이지의 SELECT 가 이미 끝난 뒤»에 옵니다.** `iter_pages` 는 질의하고 나서 내주므로, 쉬는 동안 열린 커서를 붙들고 있지 않습니다.
- ⚠️ 어드민에서 같은 노브를 씁니다 — 소급 등록부의 `chain_replay` 연산이 `pace` 를 파라미터로 선언하고, 선택지는 요청 시점에 같은 파일에서 조립됩니다. **CLI 와 버튼이 같은 일을 해야** 운영자가 둘 중 하나를 골라도 결과가 같습니다.

#### 멈춤은 «죽이는» 것이 아니라 «페이지 경계에서 묻는» 것이다

`replay_rule(..., checkpoint=…)` 이 **배치 경계마다 한 번** 불리고, `True` 를 돌려주면 루프가 끊깁니다. 그 자리가 유일하게 안전한 자리입니다 — **앞 페이지는 커밋됐고 다음 페이지는 시작 전**입니다. 🔴 **멈춘 실행도 통계를 그대로 답하고 `stopped` 를 세웁니다** — 취소된 실행도 «진짜 일»을 했고 그것을 보고해야 하기 때문입니다. 등록부·취소 규약은 [BACKFILL_GUIDE §7.5](./BACKFILL_GUIDE.md).
...
**재적용 순서**는 계약입니다: 생산자(`→ inventory_master`)가 소비자보다 먼저 갑니다. 자기 간선은 순서에서 제외하고 ①로 다룹니다. **서로 다른 테이블 사이의 순환**은 전순서가 없으므로 ⚰️ ~~순환 경로를 이름으로 밝히며 **거부**합니다~~ → **[2026-09-15 판정 402] 이름을 대되 «선언 순»으로 돌립니다** — 부분 하강이 풀리며 나오는 순서는 아무도 안 쓴 «셋째 순서»라 쓰지 않습니다(`rule_order.order_rules`, `RuleCycleRefused` 삭제).
...
- **신규 이벤트 타입도 신규 화면도 만들지 않습니다** — R2와 같은 자세이고, 설명은 기존 셀 이력 타임라인이 합니다. **CLI 전용 연산**이며 어드민 API 등록부(`server/admin/retroactive.py`의 `OPERATIONS`)에도 **없습니다**(2026-08-11 실측 — ⓐ~ⓔ 다섯만 등재).
...
- 🔴 **이것은 억제가 아니라 옵트인입니다.** `chain_ingestion` 라벨은 R1이 이미 지나는 **룰별 옵트인**(`_rule_accepts_event`, `chain/ingestion_worker.py:328`)에 R3를 함께 세웁니다 — `allow_chain_trigger: true`를 선언한 룰은 **여전히 이 이벤트를 받습니다.** 「이벤트를 안 내보낸다」로 고쳤다면 그 선언이 조용히 무의미해졌을 것이고, **침묵과 동의는 다른 물건**입니다(뮤테이션 채점에서 억제형 「수리」가 첫 가드를 통과하고 둘째에서 죽습니다 — 가드가 둘인 이유가 그것입니다).
...
체인은 **옵트인(`allow_replace_map: true`)을 선언한 룰에 한해** `replace_map` 을 실을 수 있습니다(§1) — 워커가 `write_batches` 에서 그것을 풀어 배치에 넘깁니다(`chain/ingestion_worker.py:1626`·`:1631`).
> 🔴 **[2026-09-17 정정] 이 자리는 「`chain_ingestion_worker` 가 `replace_map` 을 «절대 설정하지 않는다»(`:437-441`)」라고 적고 있었고 «둘 다» 거짓이었습니다** — 워커는 설정하고(위 두 앵커), `:437-441` 은 오늘 `NO_GROUP_MERGE` 상수 블록이라 배치와 무관합니다. 아래 문단의 결론(**「`replace_map` 은 답이 아니다」**)은 그대로 유효하고, 그 근거는 「워커가 못 싣는다」가 **아니라** 「맵 키 계약이 퍼지를 맵 전체 단위로만 잡는다」입니다.
