...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-15 2차 (`ledger_vocabulary.json` 행 신설 — 로더·라우트 실측) | **Owner:** Lead / Backend
>
> ### 이번 라운드 (2026-08-15 2차 · 어휘의 «선언 절반» · R-2026-08-15-M)
...
| **`ledger_config.json`** (2026-08-13 신설 · `f896020`) | **정준 원장 번역기 선언 — 소스 하나당 항목 하나.** 🔴 **키 표(문법 셋 `lineage`·`observation`·`transfer`의 필수/선택 키와 각 부재의 거절 이름)는 이 표가 소유하지 않는다 — [guide/ONTOLOGY_LEDGER_SETUP](./ONTOLOGY_LEDGER_SETUP.md) **§3이 정본**이다**(2026-08-15 통폐합: 종전 이 칸이 두 문법의 키 집합을 거의 필드 단위로 다시 적고 있었고, 그런 사본은 낡는다). 여기서 알아야 할 것 셋: **`occurred_at_*`가 없으면 그 소스는 통째로 거절**(도착 시각이 대신 서지 않는다) · **선언을 바꾸면 `source_translator_ver` 해시가 움직여 재실행이 dedupe가 아니라 「같은 주장, 새 provenance」의 중복 적재가 된다**([LEDGER_GUIDE §4.3](./LEDGER_GUIDE.md)) · **단수 `subject_type`은 은퇴했고 남아 있으면 로드 에러**다 | 사용자 | ignored (`.sample` 有) | 번역기 실행 시 재독 | `server/ledger/*`(백필·번역기). ⚠️ **웹서버·워커는 부팅 시 이것을 읽지 않습니다** |
