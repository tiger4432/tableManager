...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-15 3차 (`ledger_config.json`의 넷째 `kind` — 코드 대조) | **Owner:** Lead / Backend
>
> ### 이번 라운드 (2026-08-15 3차 · 소스 선언에 «넷째 문법» · 브리핑 §6-2)
> - 🔴 **§1의 `ledger_config.json` 행에 네 번째 `kind`가 생겼습니다 — `declared`.** 앞의 셋과 성질이 다릅니다: **베낄 파이썬 번역기가 «없습니다».** 행→원자 사상이 `emit` 선언 그 자체라, **새 «모양»의 테이블에 코드가 0줄**입니다. 이 표의 관점에서 중요한 것은 하나입니다 — 🔴 **이 선언은 「어느 코드가 무엇을 읽을지」를 고르는 것이 아니라 «코드가 할 일 자체»입니다.** 오타가 잘못된 화면이 아니라 **잘못된 원자**를 낳으므로, 검증이 다른 어느 선언보다 촘촘하고 **거절이 저장 시점에 옵니다**(백필 시점이 아니라).
> - 🔴 **`occurred_at_basis`가 필수이고 기본값이 없습니다**(R-2026-08-15-N ②) — `claim_time` \| `row_created`. 대장의 세계 시각이 «주장이 성립한 때»인지 «행이 생긴 때»인지는 **둘 다 합법이고 뜻이 다릅니다.** 기본값을 두면 「언제 참이 됐나」가 조용히 「언제 적재됐나」가 됩니다.
> - 🔴 **`when`은 닫힌 연산자 집합에서 «정확히 하나»**이고 0개·2개·오타가 전부 거절입니다 — **무시된 연산자는 조건을 「항상 참」으로 만들어 아무도 요청하지 않은 원자를 낳습니다.** 값은 `"$컬럼"`이면 그 행의 컬럼, 아니면 리터럴(`"$$"`는 «$» 자체)이고 **없는 컬럼을 `$`로 부르면 거절**입니다(빈 값으로 풀지 않습니다).
> - 키 표 정본은 [ONTOLOGY_LEDGER_SETUP §3.4](./ONTOLOGY_LEDGER_SETUP.md), 계약은 [spec §3.8](../spec/LEDGER_TECHNICAL_SPEC.md)입니다. **이 표는 다시 적지 않습니다.**
>
> ### 직전 라운드 (2026-08-15 2차 · 어휘의 «선언 절반» · R-2026-08-15-M)
> - 🔴 **§1에 `ledger_vocabulary.json` 행 신설 — 이 표에서 «아무것도 좁히지 않는» 첫 선언입니다.** 다른 선언 파일은 전부 무언가를 «고르거나 거른다»(축·종류·구간·모델)는데, 이 파일은 **닫힌 어휘에 낱말을 «더합니다».** 지웠을 때 잃는 것도 그래서 다릅니다 — 화면이 좁아지는 것이 아니라 **그 낱말이 어휘에서 사라집니다**(그 낱말로 이미 누운 원자는 남습니다. 읽기는 안 막힙니다).
> - 🔴 **이 파일에는 `.sample` 폴백이 «없고 그것이 설계입니다».** 이 표의 다른 거의 모든 행이 「라이브 없으면 `.sample`」인데 여기만 아닙니다 — **샘플이 로드되면 아무도 선언한 적 없는 낱말이 닫힌 어휘에 들어갑니다.** `.sample` 파일 자체는 **모양 설명용으로만** 있습니다(로더가 절대 안 읽습니다).
> - ⚠️ **깨진 파일은 «통째로» 무시되고 코드 집합으로 내려갑니다**(절반만 실린 어휘는 프로세스마다 다른 낱말을 인정합니다). 그 강등은 조용하지 않습니다 — `GET /admin/config/resolve?domain=ledger`가 사유와 함께 말합니다.
>
> ### 직전 라운드 (2026-08-15 · 🧱 원장·온톨로지 셋업 통폐합 — 소유자 지시)
> - 🔴 **§1의 원장·온톨로지 다섯 행이 「필드 계약을 다시 적는」 자리에서 「가리키는」 자리로 바뀌었습니다** — `ledger_config.json` · `finding_kinds.json` · `siblings_axes.json` · `mechanism_models.json` · `ledger_journey.json`. **정본은 [guide/ONTOLOGY_LEDGER_SETUP](./ONTOLOGY_LEDGER_SETUP.md)** 하나입니다. 종전에는 `ledger_config.json` 한 칸이 **두 문법의 키 집합을 거의 필드 단위로** 다시 적고 있었고, 그런 사본은 낡습니다(같은 날 운영자 가이드 셋이 **이미 삭제된 것을 돌리라고** 말하고 있었습니다).
> - 🔴 **`mechanism_models.json`에 `models`라는 블록은 «없습니다»** — 최상위 예약 키는 `__doc`와 `bindings`뿐이고 나머지 키 하나하나가 모델이며 `signatures`는 **모델 안의 키**이고 로더가 무시합니다. 이 표를 포함해 **세 문서가 같은 오기**를 나르고 있었습니다(2026-08-15 실측 정정).
> - ⚰️ **§3 S4(그래프 온보딩)에 「실행하지 마십시오」 배너** — R-2026-08-14-H로 은퇴했고, 오늘 그 선언을 읽는 살아 있는 소비자는 **아무도 안 읽는 표지 하나**뿐입니다. **§7 부트스트랩에 원장 단계(8번)를 링크로 추가**했습니다(순서는 여기 다시 적지 않습니다).
>
> ### 직전 라운드 (2026-08-14 밤 2차 · 여정 대조의 이름 선언)
...
| **`ledger_config.json`** (2026-08-13 신설 · `f896020`) | **정준 원장 번역기 선언 — 소스 하나당 항목 하나.** 🔴 **키 표(문법 «넷» `lineage`·`observation`·`transfer`·`declared`의 필수/선택 키와 각 부재의 거절 이름)는 이 표가 소유하지 않는다 — [guide/ONTOLOGY_LEDGER_SETUP](./ONTOLOGY_LEDGER_SETUP.md) **§3이 정본**이다**(2026-08-15 통폐합: 종전 이 칸이 두 문법의 키 집합을 거의 필드 단위로 다시 적고 있었고, 그런 사본은 낡는다). 여기서 알아야 할 것 셋: **`occurred_at_*`가 없으면 그 소스는 통째로 거절**(도착 시각이 대신 서지 않는다) · **선언을 바꾸면 `source_translator_ver` 해시가 움직여 재실행이 dedupe가 아니라 「같은 주장, 새 provenance」의 중복 적재가 된다**([LEDGER_GUIDE §4.3](./LEDGER_GUIDE.md)) · **단수 `subject_type`은 은퇴했고 남아 있으면 로드 에러**다 · 🔴 **[2026-08-15 3차] 넷째 `kind` `declared`는 이 표의 다른 어느 선언과도 성질이 다르다 — 「무엇을 읽을지」가 아니라 «코드가 할 일 자체»를 적는다**(베낄 파이썬 번역기가 없다). 그래서 오타가 **잘못된 화면이 아니라 잘못된 원자**를 낳고, 검증이 **저장 시점에** 촘촘하다: `occurred_at_basis` 필수·기본값 없음 · `when`은 닫힌 집합에서 **정확히 하나** · **없는 컬럼을 `"$col"`로 부르면 거절**([spec §3.8](../spec/LEDGER_TECHNICAL_SPEC.md)) | 사용자 | ignored (`.sample` 有) | 번역기 실행 시 재독 | `server/ledger/*`(백필·번역기). ⚠️ **웹서버·워커는 부팅 시 이것을 읽지 않습니다** |
| **`ledger_vocabulary.json`** (2026-08-15 신설 · R-2026-08-15-M) | **닫힌 어휘의 «선언 절반» — ontology 층 술어의 append-only 확장.** 🔴 **이 표에서 «아무것도 좁히지 않는» 유일한 선언입니다** — 축·종류·구간을 고르는 다른 파일과 달리 이것은 **낱말을 더합니다.** 지우면 화면이 좁아지는 것이 아니라 **그 낱말이 어휘에서 사라지고**(이미 그 낱말로 누운 원자는 남습니다 — 은퇴는 발화를 막지 읽기를 막지 않습니다), 정준 층(`register`·`pin`·`same_as`)과 **개체 타입은 여기서 못 늘립니다**(코드 + 판정 전용). 🔴 **`.sample` 폴백이 «없고 그것이 설계입니다»** — 샘플이 로드되면 **아무도 선언한 적 없는 낱말이 닫힌 어휘에 들어갑니다.** 저장소의 `.sample`은 **모양 설명용이고 로더가 절대 안 읽습니다.** 🔴 **서명이 완결돼야 저장됩니다**(`label_ko`·`subject`·`object`·`traversable`·`direction`·`since`·`layer`·`status`) — 절차와 삼상태·은퇴 규칙은 [guide/ONTOLOGY_LEDGER_SETUP](./ONTOLOGY_LEDGER_SETUP.md) **§4**, 계약은 [spec §3.7-sexies](../spec/LEDGER_TECHNICAL_SPEC.md). ⚠️ **깨지면 통째로 무시하고 코드 집합으로 내려가되 «조용하지 않습니다»**(`/admin/config/resolve?domain=ledger`가 사유를 답합니다) | 사용자(**어드민 화면에서 — `POST /admin/ledger/dry-run` → `/save`**) | ignored (**`.sample` 有 — 그러나 폴백 아님**) | **재기동 0회** — 저장이 `/admin/reload-configs`와 **같은 함수**(`reload_local_process_cache`)를 부르고 `SYSTEM_RELOAD`가 나머지 프로세스에 같은 교체를 퍼뜨립니다. 그 훅이 `vocabulary.reset_cache()`와 **`ledger_trace.reset_walk_cache()`를 «둘 다»** 부릅니다 — 걷기의 인출 집합이 어휘의 파생물이라 하나만 비우면 **낡은 걷기 집합이 새 낱말 위에서 돕니다** | web(`/structure`·`/journey`·게이트·걷기) + 체인 워커 |
| **`finding_kinds.json`** (2026-08-14 신설) | **결함 «종류»의 정의 — 종류마다 항목 하나.** 🔴 **선택 파일이고 «코드 기본값 위에 종류 단위로 병합»된다**(`server/finding_kinds.py`의 `DEFAULT_FINDING_KINDS`) — 없으면 기본값 둘(`void`·`delam`)로 돕니다. ⚠️ **이 저장소에 `.sample`이 없습니다.** 필드별 뜻과 「`observed_by` 부재 ≠ 빈 목록」·「등급은 저장하지 않는다」·「새 종류의 method는 기존과 달라야 한다」는 [guide/ONTOLOGY_LEDGER_SETUP](./ONTOLOGY_LEDGER_SETUP.md) **§5**가 소유합니다 | 사용자 | ignored (**`.sample` 無**) | 프로세스 수명 캐시(`load(force_reload=True)`) | web(콘솔의 종류 선택자·분모·class 축) |
| **`siblings_axes.json`** (2026-08-14 밤 등재) | **원장 콘솔의 요인 기하** — `defaults` · `geometry`(모집단 단위와 런 컬럼) · `attribution[]`(관계마다 `axes[]`) · `kinds`(종류별 덮어쓰기). 🔴 **축을 더하는 것은 항목 하나이고 파이썬 0줄**이며, 고카디널리티 방어가 **두 반쪽**(선언된 `rank: false` + 측정된 `defaults.factors.high_cardinality_at`)인 것과 **모든 이름이 로드 시점에 SQL 식별자 검사를 받는다**는 것까지 [guide/ONTOLOGY_LEDGER_SETUP](./ONTOLOGY_LEDGER_SETUP.md) **§6.2**가 소유합니다. 라이브가 없으면 **`.sample`로 폴백**하고 둘 다 없으면 이름 붙은 거절입니다 | 사용자 | ignored (`.sample` 有) | 프로세스 수명 캐시(`load_axes_config(force_reload=True)`) | web(`/siblings`·`/lots`·`/structure`) |
| **`mechanism_models.json`** (2026-08-14 등재 · `.sample`은 `f52628f`) | **M4 기전 그래프 — 방향만 있는 인과 선언**이고 「**물리 경로 있음**」 관문이 읽는 것. 🔴 **[2026-08-15 정정] `models`라는 블록은 «없습니다»** — 최상위 예약 키는 `__doc`와 `bindings`뿐이고 **나머지 키 하나하나가 모델**이며 `signatures`는 **모델 «안»의 키**이고 로더가 무시합니다(종전 이 칸·[LEDGER_GUIDE §4.6-bis](./LEDGER_GUIDE.md)·[spec §4.7](../spec/LEDGER_TECHNICAL_SPEC.md) 셋이 같은 오기를 나르고 있었습니다). 모델의 필드와 **`bindings`가 항목 목록이 아닌 이유**, **바인딩은 데이터가 실재하는 날 켠다**는 규율은 [guide/ONTOLOGY_LEDGER_SETUP](./ONTOLOGY_LEDGER_SETUP.md) **§6.1** | 사용자(물리 모델러) | ignored (`.sample` 有) | **프로세스 수명 캐시**(부재·깨짐은 예외가 아니라 **상태**) | web(3관문 랭킹의 기전 관문 + `/structure` 기전 층) |
| **`ledger_journey.json`** (2026-08-14 밤 신설) | **여정 대조의 선언 층.** 🔴 **한 파일 안의 두 반쪽이 «정반대로» 실패합니다** — `segments`는 **구조적**(어느 술어가 여정을 나르고 step 이름이 payload 어디에 있는지. 비면 `state: "absent"`·구간 0개)이고, `step_labels`/`family_labels`/`field_labels`는 **아무것도 안 좁힙니다**(지워도 값은 그대로, 한국어만 잃습니다). 쓰는 절차는 [guide/ONTOLOGY_LEDGER_SETUP](./ONTOLOGY_LEDGER_SETUP.md) **§6.3**, 응답 읽는 법은 [LEDGER_GUIDE §4.6-quater](./LEDGER_GUIDE.md) | 사용자 | ignored (`.sample` 有) | 프로세스 수명 캐시(`load_config(force_reload=True)`) | web(`/journey`) |
...
### S4. ⚰️ ~~그래프에 올릴 때~~ — **은퇴 (R-2026-08-14-H). 이 절차를 실행하지 마십시오**

> 🔴 **이 체크리스트를 따라가지 마십시오.** 옛 그래프 갈래는 은퇴했습니다 — 라우트들은 **410**으로 답하고(404가 아닙니다 — 「있었고 의도적으로 뺐다」),
> 머티리얼라이저 워커는 프로세스 목록에서 빠졌으며 저장소는 DROP됐습니다. **`ontology_mapping.json`을 새로 쓰면 아무도 안 읽는 표지 하나를 흔들 뿐입니다.**
> **오늘의 온톨로지는 그래프 테이블이 아니라 원장의 어휘이고**, 무엇을 어디에 선언하는지는
> [guide/ONTOLOGY_LEDGER_SETUP](./ONTOLOGY_LEDGER_SETUP.md)이 답합니다(§7이 이 은퇴를 다룹니다).
> 아래는 **이미 그렇게 선언된 환경을 읽기 위한 기록**으로만 남깁니다.
...
6. 필요에 따라 ~~S4~~(⚰️ 은퇴)·S6·S7·S8.
7. `GET /health`가 **JSON 200**인지 확인한다(워커 4종이 `ok`인지 포함) → [backend §1.3](../architecture/backend.md).
8. **원장·온톨로지를 올릴 것이면** → [guide/ONTOLOGY_LEDGER_SETUP §2](./ONTOLOGY_LEDGER_SETUP.md).
   🔴 **여기 순서를 다시 적지 않는다** — 시간대 판정·`tzdata`·마이그레이션 둘·소스 선언·백필·화면 층 선언이 그 문서 하나에 있다.
