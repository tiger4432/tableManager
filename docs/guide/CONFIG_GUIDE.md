...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-14 밤 2차 | **Owner:** Lead / Backend
>
> ### 이번 라운드 (2026-08-14 밤 2차 · 여정 대조의 이름 선언)
> - **§1 표에 `ledger_journey.json` 행 신설** — `GET /api/ledger/journey`(주어 둘 전용)의 **step·항목 한국어 이름** 층입니다. 🔴 **항목 목록이 아니고 «아무것도 좁히지 않습니다»** — 이름 안 붙은 잎도 똑같이 비교되고 **원시 경로로** 화면에 나오며, 파일을 통째로 지워도 구간·항목·값은 하나도 안 줄어듭니다(사라지는 것은 한국어뿐). 🔴 **블록 넷 중 «구조적인 것»은 `segments` 하나**입니다 — payload의 어디에 step 이름이 있는지는 어휘가 «말하지 않으므로»(요구 필드 넷 중 무엇이 step인지는 위치 관례일 뿐) 여기서 선언하고, 새 여정 술어를 붙이는 것이 항목 하나·파이썬 0줄입니다. ⚠️ **`field_labels`는 조회 철자가 셋**(`<술어>:<경로>` → `<경로>` → **맨 잎 이름**)이고 맨 잎이 기본인 것은 한 항목이 `params_actual.*`와 `params_setpoint.*`를 동시에 이름 짓게 하기 위함입니다(설정/실측 구분은 라벨이 아니라 **해결 등급**이 나릅니다).
>
> ### 직전 라운드 (2026-08-14 밤 · 리빙 문서 동기화 · 원장 콘솔 선언 둘 등재)
> - 🔴 **§1 표에 `siblings_axes.json`·`mechanism_models.json` 행 신설 — 둘 다 «있는데 행이 없던» 파일입니다.** 앞엣것은 원장 콘솔의 요인 기하(축 추가 = 항목 하나·파이썬 0줄 — `by=core_lot`이 `60c7c93`으로, `wafer` 마킹 축이 `5ea29b6`으로 **선언만으로** 살아난 것이 그 성질의 실증)이고 라이브가 없으면 **`.sample`로 폴백**하는 첫 부류의 config입니다. 뒤엣것은 M4 기전 그래프(방향만 · `bindings`는 항목 목록이 아니라 **바인딩 없는 후보가 `unknown`을 달고 나오는** 구조 · 🔴 **바인딩은 데이터가 실재하는 날 켭니다** — `87374a5`).
>
> ### 직전 라운드 (2026-08-14 4차 · 리빙 문서 동기화 · 픽스처 세트 정비 — `50a21c7`)
...
>
> ### 직전 라운드 (2026-08-14 · 리빙 문서 동기화 · 결함 종류 레지스트리 + 두 번째 관측 테이블)
> - **§1 표에 `finding_kinds.json` 신설** — 🔴 **이 파일은 «없는 것이 정상»입니다**(코드의 `DEFAULT_FINDING_KINDS`가 기본이고 `.sample`도 없습니다). 있으면 종류별로 **덮어씁니다**. 🔴 **`observed_by`가 분모의 정의**이고 **키 «부재»는 로드 시점 거절**입니다 — **부재와 빈 목록은 다른 뜻**입니다(빈 것 = 「체계적 스캔이 없으니 분모가 없다」는 결정, 없는 것 = 그 결정을 아무도 안 함). 🔴 **파일이 깨졌으면 조용히 기본값으로 안 떨어지고 `FindingKindError`로 거절**합니다.
> - **§5.8-ter의 void 행이 「결함 관측」 행으로 넓어졌습니다** — 두 번째 종류 **`delam_obs`**(SCAT)가 도착했습니다. 🔴 **테이블 생성은 `create_missing_dynamic_tables`이지 `sync_dynamic_tables_schema`가 «아닙니다»**(후자는 `ADD COLUMN`만 — 없는 테이블에는 아무 일도 안 하고 200처럼 조용합니다). 🔴 **두 종류의 `method`가 «달라야» 합니다**(`sat` vs `scat`) — 공유하면 종류를 바꿔도 같은 런을 세어 일반화가 도는지 판별할 수 없습니다.
>
> ### 직전 라운드 (2026-08-13 3차 · 리빙 문서 동기화 · `eb1ae8b`)
> - 🔴 **§1의 `ledger_config.json` 행: `subject_type`(단수) → `subject_types`(복수 허용목록)이고 이제 «뭅니다».** 밖에 있는 원자는 `undeclared_subject_type`으로 이름 대고 거절·계수됩니다. ⚠️ **옛 단수 키가 남아 있으면 «로드 에러»입니다**(무시도 승계도 아닙니다 — 메시지가 새 이름과 예시를 답니다). 그 필드는 검증만 되고 **어느 원자에도 도달하지 않았고**, 애초에 **한 소스가 개체 타입 둘에 대한 원자를 만들어서** 단수가 틀린 모양이었습니다. 🔴 **이 선언 변경이 `source_translator_ver` 해시를 움직입니다**(`d8d1c9e0` → `34311f15`) — `--reset-cursor` 재실행은 dedupe가 아니라 **중복 적재**입니다.
>
> ### 직전 라운드 (2026-08-13 2차 · 리빙 문서 동기화 · `c0fb735`+`346aa88`+`f896020`+`272da5b`)
...
| ⚰️ **`ontology_mapping.json`** | ~~그래프 노드/엣지 매핑 v2~~ — **[2026-08-14 `2ec78b9` · R-2026-08-14-H] 소비자를 잃었습니다.** 파일은 디스크에 살아 있지만 **읽는 워커도, 이 선언으로 답하는 라우트도 없습니다**(`graph_sync_worker` 스택 이탈 · `/graph/*` 전부 410 · 저장소 DROP). 🔴 **고쳐도 아무 일도 일어나지 않습니다** — 이 표에서 가장 위험한 상태이므로 별표로 남깁니다. 은퇴 자체는 판정 R-2026-08-14-H의 후속 항목 | 사용자 | ignored (`.sample` 有) | ⚰️ 반영 경로 없음 | ⚰️ **소비자 0** |
...
| **`ledger_config.json`** (2026-08-13 신설 · `f896020`) | **정준 원장 번역기 선언 — 소스 하나당 항목 하나.** 🔴 **`occurred_at_*` 셋은 필수**이고 하나라도 빠지면 번역기가 **그 소스를 통째로 거절**합니다(도착 시각으로 대신하지 않습니다). 🔴 **`occurred_at_timezone`은 「이 소스의 naive 텍스트가 무슨 뜻인가」의 선언**입니다 — 2026-08-13 제품 소유자 판정으로 `Asia/Seoul`(fab는 현지시간, ISO 8601 `T`). **자기 오프셋을 가진 문자열은 이미 자기 순간을 말했으므로 이 존을 덧씌우지 않습니다.** `vocabulary`는 이벤트 타입별 관례 선언(`lineage`·`slot_pairing`·`emit_*`)이고 **`source_translator_ver`에 해시되어** 원자마다 「어느 관례가 만들었는가」가 남습니다. 🔴 **`subject_types`는 복수 허용목록이고 «뭅니다»**(2026-08-13 `eb1ae8b` · R-2026-08-13-D) — 이 소스의 번역기가 말해도 되는 개체 타입 목록이고, 밖에 있는 원자는 `undeclared_subject_type`으로 **이름 대고 거절되며 세어집니다.** ⚠️ **단수 `subject_type`은 은퇴했고 남아 있으면 «로드 에러»입니다**(메시지가 새 이름과 예시를 답니다) — 그 필드는 검증만 되고 **어느 원자에도 도달하지 않았고**, 애초에 **한 소스가 개체 타입 둘에 대한 원자를 만들기 때문에** 단수가 틀린 모양이었습니다. 🔴 **이 선언을 바꾸면 `source_translator_ver` 해시가 움직입니다** — 재실행이 dedupe가 아니라 「같은 주장, 새 provenance」의 **중복 적재**가 됩니다([LEDGER_GUIDE §4.3](./LEDGER_GUIDE.md)). 그 밖에 `batch.molecules_per_transaction`(**사건 «전체» 단위 — 절대 분수 아님**) · `partitioning.interval`(월) · `lag.probe_interval_seconds`.<br>🔴 **[2026-08-14 3차 · R-2026-08-14-D] 소스가 «어느 문법을 말하는지»를 선언합니다 — `kind`**(`lineage` \| `observation`). **기본값이 `lineage`**라 기존 선언 전부가 정확히 옛 뜻을 유지하고, 검증이 이 낱말로 **분기**합니다(혈통 필수 컬럼으로 관측 소스를 채점하면 `parent_lot`이 없다는 이유로 거절됐을 것입니다). **관측 소스(`kind: "observation"`)의 키** — **`finding_kind`**(종류 하나. 정의 자체는 `finding_kinds.json`이 소유하고 여기는 «어느 소스가 생산하는가»만) · **`run`**`{relation, key_column, method_column}`(🔴 **세상의 시각과 분모가 둘 다 여기서 나옵니다** — `occurred_at`을 발견 행이 아니라 이 관계에서 읽고, **런을 못 풀면 원자 0개**입니다) · **`watermark.columns`**(🔴 **커서가 «키셋»입니다** — `updated_at` 하나가 91,756행에 찍히므로(서로 다른 값 92개) 시각 커서는 한 적재를 쪼갤 수 없습니다. **유니크 + 인덱스 뒷받침** 필수 — ✅ **그 인덱스는 이미 있습니다**: `database/models.py`가 **모든 동적 테이블**에 `idx_<테이블>_updated (updated_at, row_id)`를 만들므로, 키셋을 **그 컬럼 순서 그대로** 고른 것이 재사용입니다) · `synthetic`(참이면 **모든 원자 payload에 `"synthetic": true`** — 실물 피드가 오는 날 **이 줄을 지우는 것**이 그 사실의 철회입니다) · **`columns`**(필수 셋 `row_identity`·`wafer`·`run_key`, 선택 `die_*`·`inchip_*`·`extent_*`·`unit`·`class`. 🔴 **목록 밖 이름은 로드 시점 거절**입니다) · `subject_types: ["Wafer"]`. ⚠️ **`vocabulary` 키를 관측 소스에 두면 «로드 에러»입니다** — 그것은 이벤트 타입 지도이고 관측 소스에는 이벤트 타입이 없습니다(한 행이 한 발화). 실물 둘(`void_obs`·`delam_obs`)이 `.sample`에 실려 있습니다 → [LEDGER_GUIDE §3 ②-bis](./LEDGER_GUIDE.md) | 사용자 | ignored (`.sample` 有) | 번역기 실행 시 재독 | `server/ledger/*`(백필·번역기). ⚠️ **웹서버·워커는 부팅 시 이것을 읽지 않습니다** |
| **`finding_kinds.json`** (2026-08-14 신설) | **결함 «종류»의 정의 — 종류마다 항목 하나.** 정본은 코드의 `DEFAULT_FINDING_KINDS`(`server/finding_kinds.py`)이고 **이 파일은 «없는 것이 정상»입니다**(`.sample`도 없습니다) — 있으면 종류별로 **덮어씁니다**(종류 단위 얕은 병합이라, 선언한 종류의 필드만 바꾸면 됩니다). 종류가 코드의 분기가 아니라 **조회**여야 하는 이유는 두 번째 작성자가 그 분기를 **한 곳 빠뜨리기 때문**입니다(제품 소유자 판정 2026-08-14: 「기본값 아닌 `finding_kind='void'` 하드코딩이 보이면 일반화가 소실된 것」). 필드 **다섯**(2026-08-14 3차에 `classes` 추가) — 🔴 **`observed_by`가 «분모의 정의»**입니다(이 종류를 찾는 `inspection_run.method` 값들). 🔴 **부재와 빈 목록은 다른 뜻**이고 **부재는 로드 시점 거절**입니다: 빈 목록 = 「체계적 스캔이 없으니 분모가 없다」는 **결정**(그때 정직한 답은 「분모 없음 — 대조 불가」이지 근처 행으로 만든 비율이 아닙니다), 부재 = 그 결정을 아무도 안 함. · `observation_table`(SQL에 이름이 끼워지므로 **평범한 식별자인지 검사**합니다 — 운영자 config가 문장이 될 수 없게) · `extent_columns`(**얼마나 큰가**. 🔴 저장된 등급으로 접지 마십시오) · `label` · **`classes`**(2026-08-14 3차 — 이 종류가 발화할 수 있는 **닫힌 값 집합**, 추가 전용. 실측 `delam`: `["die-to-die","die-to-substrate"]` · `void`: **`[]`**. 🔴 **화면용 참고 목록이 아니라 «게이트»입니다** — 원장 번역기가 소스의 class 값을 이 집합에 대고 검사해 **밖의 값은 원자를 이름 대며 거절**합니다. 🔴 **빈 목록은 「이 종류는 class를 발화하지 않는다」는 결정**이고 `observed_by`의 「부재 ≠ 빈 목록」이 그대로 적용됩니다. 🔴 **class는 저장된 속성이 아니라 «주장»**이라 컬럼이 아니라 원장 payload로 갑니다). 🔴 **깨진 파일은 조용히 기본값으로 안 떨어지고 `FindingKindError`로 거절**합니다. ⚠️ **미선언 종류를 물으면 기본값으로 떨어지지 않고 이름을 대며 거절**합니다(오타 난 종류 아래 void의 숫자가 그려지면 화면의 모든 수가 다른 것에 대한 참이 됩니다) → 저장 쪽은 [data_model §1.2-bis.1](../architecture/data_model.md) | 사용자 | ignored (**`.sample` 없음 — 기본값이 코드에 있습니다**) | **프로세스 수명 캐시**(`load(force_reload=True)`로만 다시 읽습니다 — 리로드 훅 없음) | web(케이스-컨트롤 조회) |
| **`siblings_axes.json`** (2026-08-14 밤 등재) | **원장 콘솔의 요인 기하 — 「모집단 하나가 요인들에 어떻게 귀속되는가」.** `geometry`(모집단 단위 = **패키지** — `finding_kinds.population_ctes`와 같은 입도, 일부러 · `unit_columns`·런 컬럼 셋 · **`ledger_subject`** — 걷기 대조가 모집단 단위를 원장 주어로 옮기는 선언. 🔴 **생략하면 걷기가 추측하지 않고 「걷기 불가 — 원장 주어 선언 없음」으로 답합니다** · `universe` — 「스캔될 수 «있었던» 것」, 셋째 버킷의 유일한 근거) · **`attribution[]`**(관계·`join`·`axes[]` — 🔴 **축 추가는 항목 하나에 파이썬 0줄**이고, 선언된 축은 `/siblings`의 요인 · `/lots`의 `by=` · `scope=` 마킹에 **동시에** 제공됩니다) · `defaults`(`contrast` 밴드 — 미끼 필터의 선은 프로그래머가 아니라 **운영자의 판단**이라 여기 있습니다 · **`walk` — 필터가 아니라 «예산»**: CASE는 절대 안 깎고 control만 결정적 표본, `high_cardinality_at`도 행을 지우지 않고 주석만 답니다) · `kinds` 종류별 오버라이드. 🔴 **종류 정의의 둘째 사본이 아닙니다** — 무엇이 종류인가는 `finding_kinds.py` 소유. 🔴 **`wafer` 축(2026-08-14 밤 `5ea29b6`)은 주어 «자체»를 마킹 축으로 선언한 것** — 「이 두 장 vs 나머지」를 표현 가능하게 만들었고, 요인 랭킹에서는 값 2,600개짜리 식별자라 `axes=`로 좁힙니다. ⚠️ **`inspection_run` 귀속의 `filter.values_from: "observed_by"`는 실측 필수입니다** — 없으면 딴 종류 스캐너의 레시피가 이 종류의 요인으로 샙니다(delam 스캐너 레시피가 void 요인으로 보고된 실측) | 사용자 | ignored (`.sample` 有 — ⚠️ **라이브가 없으면 `.sample`로 폴백**합니다. 새 체크아웃이 500 대신 답하게) | **프로세스 수명 캐시**(`ledger_siblings.load_axes_config(force_reload=True)` — 리로드 훅 없음) | web(`/api/ledger/siblings`·`/lots`·`/lot_map`) |
| **`mechanism_models.json`** (2026-08-14 등재 · `.sample`은 `f52628f`) | **M4 기전 그래프 — 방향만 있는 인과 선언**([PHYSICS_ONTOLOGY_SETUP §4](../architecture/PHYSICS_ONTOLOGY_SETUP.md) 제안의 착지형, 소유자 검토 2026-08-14). `models`(엣지 `dir: '+'/'-'/'u'` — `u`는 「모름」이 아니라 **비단조라는 단언**. 🔴 **방정식은 일부러 없습니다** — 설명 관문에는 방향만으로 충분합니다) · `signatures`(원인이 남기는 공간 패턴 — **사람용 문서이고 로더가 무시합니다**) · **`bindings`**(필드→물리량 — `params_actual.pressure_MPa`는 누가 「`bond_pressure`를 잰다」고 말하기 전엔 payload 속 숫자일 뿐. 🔴 **항목 목록이 아닙니다** — 바인딩 없는 후보도 대조에서 안 좁혀지고 기전 칸에 `unknown`을 달고 나오므로, 갓 번역된 술어가 선언 0줄로 화면에 닿습니다. 키 형식 `<술어>:<점 경로>` 또는 맨 경로). 🔴 **바인딩은 «데이터가 실재하는 날» 켭니다** — `post_bond_queue_h`는 측정 필드가 없는 동안 정직한 공백이었고 증강 레인이 필드를 실재로 만든 저녁에 켜졌습니다(`87374a5` · 코드 0줄). **화면을 완성돼 보이게 하려고 공백에 바인딩을 지어내지 마십시오.** ⚠️ **`.sample`과 라이브를 동일하게 편집합니다**(추적은 `.sample`만) | 사용자(물리 모델러) | ignored (`.sample` 有) | **프로세스 수명 캐시**(`mechanism_gate.load(force_reload=True)` — 부재·깨짐은 예외가 아니라 **상태**로 답합니다) | web(3관문 랭킹의 기전 관문 + `/structure` 기전 층) |
| **`ledger_journey.json`** (2026-08-14 밤 신설) | **여정 대조의 «이름» 층 — 원장이 `processed_with·params_actual.pressure_MPa`라 말하는 자리에 화면이 「본딩 · 압력」이라 쓰게 하는 선언**([LEDGER_GUIDE §4.6-quater](./LEDGER_GUIDE.md) · 라우트 [backend §2](../architecture/backend.md)). 🔴 **항목 목록이 «아니고» 무엇이 비교되는지를 하나도 안 좁힙니다** — 두 주어 원자의 **모든 잎**이 이름 유무와 무관하게 비교되고, 이름 없는 잎은 **원시 경로 그대로** 렌더됩니다(정직 우선 — 빠진 선언은 «빠져 보여야» 합니다). **이름 블록 셋(`step_labels`·`family_labels`·`field_labels`)을 통째로 비워도 구간·항목·값은 0개 줄고** 한국어만 사라집니다. ⚠️ **`segments`만은 예외입니다 — 그것은 구조적 선언이라 없으면 그릴 축이 없습니다**(응답은 500이 아니라 200 + `state:"absent"` + `reason:"no_journey_predicate_declared"` + `segments: []`). ✅ **[2026-08-14 심야 재확인] 직전 라운드가 보고한 「코드 산문이 자기 동작과 어긋난다」는 «닫혔습니다»** — `ledger_journey.py`의 `JourneyConfig` docstring과 `.sample`의 `__doc`가 둘 다 고쳐져 **「이 파일의 두 반쪽은 서로 다르게 실패한다」**를 명시하고 `segments`가 구조적임을 스스로 적습니다(양쪽 다 「앞선 판이 정확히 그렇게 적었고 틀렸다」를 남겨 뒀습니다 — **틀린 문장을 지우는 것보다 틀렸다고 적는 것이 낫습니다**). ⚠️ **이 항목을 「미해결 결함」으로 다시 인용하지 마십시오.** 부재·깨짐이 예외가 아니라 **상태**인 것은 맞고(`labels.state`), 응답이 `origin`으로 **라이브를 읽었는지 `.sample`을 읽었는지** 말합니다(`mechanism_gate`와 같은 폴백 규칙). 블록 넷 — 🔴 **`segments`**(`{<술어>: {name_path, family_path}}` · **유일한 «구조적» 선언**입니다: `vocabulary.py`는 `processed_with`의 목적어가 `['step','step_family','eqp','recipe']`를 요구한다고만 말하고 **그중 무엇이 step인지는 말하지 않으므로**, 목록의 «자리»로 읽는 것은 관례이고 관례는 스키마가 다른 날 깨집니다. **새 여정 술어를 붙이는 것 = 항목 하나 · 파이썬 0줄**이고, 선언이 하나도 없으면 응답이 `no_journey_predicate_declared`로 그렇게 말합니다. `family_path`는 생략 가능 — 그때 계열은 **추측되지 않고** 원시 값으로 남습니다) · `step_labels`·`family_labels`(값 → 한국어. 미선언은 원시 값) · **`field_labels`**(payload 잎 → 한국어. 값은 문자열 **또는** `{label, unit}`이고 `unit` 없으면 숫자를 **맨몸으로** 씁니다 — 단위를 추측하지 않습니다. ⚠️ **조회 철자가 셋이고 구체적인 것부터**입니다: `<술어>:<점 경로>` → `<점 경로>` → **맨 잎 이름**. 맨 잎이 기본인 것은 한 항목이 `params_actual.pressure_MPa`와 `params_setpoint.pressure_MPa`를 **동시에** 이름 짓게 하려는 것이고, **설정/실측 구분은 라벨이 아니라 원자의 «해결 등급»이 나릅니다**. 같은 이름의 잎 둘이 **다른 물리량**일 때만 긴 철자를 씁니다. 어느 철자가 답했는지는 응답의 `label_matched_on`에 실립니다) | 사용자 | ignored (`.sample` 有 — ⚠️ **라이브가 없으면 `.sample`로 폴백**하고 그 사실이 응답에 실립니다) | **프로세스 수명 캐시**(`ledger_journey.load_config(force_reload=True)` — 리로드 훅 없음) | web(`/api/ledger/journey`) |
...
> 🔴 **「대부분」이 아닌 예외가 하나 있고, 두 번의 사고가 거기서 났습니다.** `bonding_plan_config.json` / `transfer_plan_config.json`의 역할 선언만은 **검증되지 않습니다** — 틀린 컬럼 이름이 유도(상속)를 조용히 이기고, 나중에 `missing` 한 단어로만 나타납니다. **어느 선언이 무엇에서 상속되는지 · 무엇이 무엇을 이기는지 · 「지우면 상속된다」가 참인 자리가 어디인지**의 정본은 [architecture/CONFIG_INHERITANCE.md](../architecture/CONFIG_INHERITANCE.md)입니다. 계획 config의 역할을 고치기 **전에** 그 문서 §4를 보고, `GET /admin/transfer-plan/dry-run`에 물으십시오.
