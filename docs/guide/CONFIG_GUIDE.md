...
| **`virtual_join_rules.json`** | **저장하지 않는 조인 선언**(`left_table`/`right_table`/`join_key`/`expose`) + **로드 시점 팬아웃 가드** — 조인 키가 오른쪽 행 하나를 지목하지 못하면 목록에 들어가지 않는다(실측: 같은 두 테이블을 `(lot,slot,x,y)`로 이으면 103,040행, `(lot,slot)`로 이으면 132,715,520행). ⏳ **조인 실행 코드는 아직 없다** — 유효 선언도 보고서에서 `ineffective`(2026-07-31) | 사용자 | ignored (`.sample` 有) | 조회 즉시 | web(선언 검증·보고) |
...
서버가 선언을 **세 모집단**으로 나눠 돌려줍니다. 사유는 닫힌 어휘이고, 사람이 읽을 문장(`detail`)은 **서버가 만듭니다** — 화면이 생기면 그것을 **그대로** 보여주게 됩니다(계약).

> ⏳ **지금은 어드민 화면이 아니라 `curl`로 읽습니다** (2026-07-30 실측). 클라 렌더러는 아직 착지하지 않았고, 위 「화면」은 **계약의 서술**입니다 — 클라가 「효과 없음」을 자기 규칙으로 판정하는 것을 미리 금지해 두는 것이 요점입니다(`contracts/config_resolve_report/client_harness.mjs`가 렌더러 없이도 그 금지를 채점합니다).
