...
⏳ **그리고 `data:` URL 사본 자체가 «다리»다.** 대상이 import 되게 만드는 것이 도착지이고(§1),
`data:` 로 싣는 것은 그 대공사를 오늘 안 하기 위한 다리다 — 판정 345 는 그 다리를 **C-91 과 «같이» 은퇴**
시키기로 했다. ⚠️ **[2026-09-13 04:31 정정] 그 「C-91 과 같이」는 오늘 «더 정확한 이름»을 얻었다** — 판정 346 이 C-91 을 셋으로 쪼갰고, 다리를 걷는 것은 ⓑ = **C-94** 다. C-91 ⓐ(`d9e9d704`)는 «착지했고» 다리는 «그대로 선다». 제 실측: `git grep -l "data:text/javascript" -- client2 contracts` 가 «여덟 파일»을 내고 그중 하나가 이 절이 말하는 로더 `lib/board_modules.mjs` 다. 🔴 그러므로 이 ⏳ 는 «걷혔다»가 아니라 «다음 이름으로 넘어갔다»이고, 다리 표기가 해야 하는 일이 그것이다 — 이름이 바뀌면 «바뀐 이름을 적는다», 표기를 지우지 않는다. 🔴 **다리를 놓을 때 «언제 걷을지»를 같이 적어야 영구 건물이 되지 않는다**(같은 규율로
D-19 가 `AGGREGATE` export 를 「⏳ C-90 ② 에서 은퇴 예정」으로 적었고, 하루 만에 그대로 걷혔다).
