...
> 갱신 2026-09-17 — §2 훅 표에 **스타일시트 갈래**(C-95, 2026-09-13 착지 · 이 문서에 «한 번도» 없었다). 그 부재가 「CSS 때문에 import 가 안 된다」를 살려 뒀고 C-117 `d11723b1` 에서 값을 냈다. 앵커 `probe_hooks.mjs:46` → `:73`.
> 직전 2026-09-16 «후속» — §5-nonies 에 둘: 「하니스는 «모양이 딴 데 적힌» 관계를 짓지 않는다」(S-260 — 그 29 건은 «내용»이 아니라 «자리»였다) · 「돌았다 ≠ 단언이 돌았다」(S-263-b).
...
### 훅 — `client2/tests/lib/probe_hooks.mjs:73` `export async function resolve(...)`
```
지정자가 «스타일시트» 이면 (.css/.scss/.sass/.less)
   importer 를 «안 보고» 빈 모듈로                 <- C-95. 갈래 «넷» 중 유일하게 importer 무조건
...
```
🆕 🔴🔴 **[C-95] 「이 파일은 CSS 를 끌어서 import 가 안 된다」는 «오늘 거짓»입니다 — 그 벽은 «해석 한 줄»이었습니다.** 스타일시트 갈래는 위 셋과 달리 **importer 를 안 봅니다**(`:77` — `fromStub` 검사보다 «먼저»). 그래서 사본 자신의 줄만이 아니라 **import 그래프 «전체»**에서 `.css` 가 죽지 않습니다 — 사본이 진짜 이웃을 부르고 그 이웃이 자기 스타일시트를 부르는 «세 모듈 깊이»가 실제로 이 갈래를 요구했습니다. 🔵 **잘라낸 양은 여전히 «0»입니다**: 사본은 그 import 줄을 그대로 들고 있고 «지정자가 무엇으로 풀리는지»만 바뀝니다(스텁 이웃과 같은 방식). `probe.mjs` 의 바이트-접두 단언이 그대로 통과하는 것이 그 증거입니다.
> 🔴 **이 한 줄이 «금지를 정당화하던 문장»을 죽입니다.** 그 문장이 살아 있는 동안 하니스가 잘라쓰기를 «사유와 함께» 유지했고, 그것이 C-117 에서 값을 냈습니다 — `health_card_absence_harness.mjs` 가 「`admin.js` 는 import 할 수 없다」를 머리글에 달고 `refreshFileAndAutoHealth` 의 본문을 `indexOf` 로 잘라 정규식을 먹이고 있었는데, **그 사이 `chain_rule_user_path_harness` 는 같은 파일을 «통째로» import 하고 있었습니다.** 대가는 재현됐습니다(`d11723b1`): `errorText(` 를 «부르면서 답을 버리는» 변이에 옛 단언 넷이 «전부» 초록이었고, 그 변이가 정확히 그 파일이 막으려던 결함입니다. 단언 9 → 15, 그 변이는 이제 잡힙니다.
> ⚠️ **그러므로 「import 가 안 된다」를 사유로 적기 «전»에 «오늘» 시도해 보십시오.** 이 저장소에서 그 사유가 거짓으로 밝혀진 것이 **이번이 셋째**입니다(C-62 의 둘 + C-117).
