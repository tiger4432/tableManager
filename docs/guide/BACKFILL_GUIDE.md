...
> **Status:** 🟢 Living | **작성:** 2026-07-31 · doc-keeper | **Last-verified:** 2026-09-15 (§0·§2.1 — 선언된 join 은 «왼쪽 규칙»에 ⓐ R1, 보고는 «행», 오른쪽 이름은 거절 — S-242 `bd0a3db7`) · 직전 2026-09-02 (§7.6 페이싱 — `pace` 를 든 연산이 **하나에서 둘로**(`chain_replay` 합류), 읽는 쪽은 **둘에서 셋으로**, 모르는 이름의 갈림은 「둘이 다르다」가 아니라 **2 대 1**) · 직전 2026-08-31
>
...
| 🆕 **선언된 join**(`derive: {kind: join}`)을 켰거나 고쳤는데 옛날 행의 `into` 칸이 비어 있다 | **ⓐ R1** — 🔴 **«왼쪽» 규칙 이름으로**(`into` 표를 트리거하는 쪽). 오른쪽(참조 쪽) 규칙 이름은 «이름 대고 거절»된다 — 오른쪽 행마다 같은 일을 다시 하는 것이라. 드라이런·사전 계수는 셀이 아니라 **「다시 계산할 행」**을 말하고, 던진 페이지는 그 페이지만 잃는다(`pages_failed`) — 2026-09-15 S-242 |
...
🆕 **[2026-09-15 S-242] `builtin:` 규칙 — 선언된 join — 도 같은 명령입니다.** 룰 이름은 «왼쪽»(`into` 표를 트리거하는 쪽) 것을 주십시오. 오른쪽(참조 쪽) 이름은 「그 규칙은 선언의 뒤따르기 반쪽이다 — 대상 쪽 규칙을 돌려라」로 거절됩니다. 보고서에 **`rows_written`** 이 따로 섭니다(이 종류는 셀을 «제안하지 않으므로» `cells_proposed` 는 0 이 정상) · 드라이런은 넘겨받을 «행 수» · 던진 페이지는 `pages_failed`·`page_failures` 로 세고 다음 페이지로 갑니다. 어드민 사전 계수 화면도 이 종류에는 「다시 계산할 행」을 보입니다 — 종전엔 「0 셀」이었고 그것은 «아무 일도 없다»로 읽혔습니다.
