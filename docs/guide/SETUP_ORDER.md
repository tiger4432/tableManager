# 셋업 «순서» — 여섯 걸음, 걸음마다 «두 줄» (2026-09-11 신설, D-9)

> 🔴 **이 문서가 있는 이유:** 소유자 「체계적인 셋업이 안 됨」(09-11).
> 선언 문서는 각각 «자기 파일»을 잘 설명하는데, **「무엇을 «먼저» 적어야 하나」를 말하는 자리가 없었다.**
> 📌 그래서 이 장은 «새로운 것을 가르치지 않는다» — 여섯 파일의 순서와 «서로의 전제»만 적는다.
> 각 걸음의 정본은 링크로 간다.

## 읽는 법

```
두 줄     「운영에서는 <이 파일>의 <이 칸>을 적으면 됩니다」  — 스키마 이름은 «빈칸»이다
앞        이 걸음 «전»에 서 있어야 하는 것
거절      틀리면 제품이 «무엇이라고» 거절하나 — 거절문의 «이름»이다(조용한 실패가 아니다)
```
⚠️ **칸 이름은 로더가 «실제로 읽는» 이름이다**(2026-09-11 출하 샘플과 코드에 대고 확인).
이 박스의 표 이름·컬럼 이름은 «한 개도» 안 쓴다 — 쓰면 그 순간 이 문서가 이 설치의 것이 된다.

---

## ① 표 — 무엇이 «있는가»

```
운영에서는 «표 카탈로그»(table_config.json)에 그 표의 `column_types` 와 `business_key` 를 적으면 됩니다.
키가 여러 컬럼이면 `composite_key_source` 와 `composite_key_separator` 를, 판단 단위가 있으면 `decision_key` 를 적습니다.
```
```
앞     없음 — 여기가 뿌리다. 다른 다섯 걸음이 전부 이 표를 «이름으로» 가리킨다
거절   `decision_key` 가 «선언된 컬럼의 비지 않은 목록»이 아니면 `invalid_catalog` 로 경로를 대고 거절
      선언 안 된 컬럼을 쓰면 `unknown_column`
```
🔵 **키 컬럼이 «비어» 있을 수 있으면 한 칸 더**(09-11 S-181):
```
운영에서는 그 표의 `key_null_policy` 에 컬럼별로 `skip` 을 적으면 됩니다 — 그 컬럼이 비면 «그 행을 건너뜁니다».
안 적으면 오늘처럼 «이름 대어 거절하고 셉니다»(`unkeyed_refused`).
```
🔴 그리고 「빈 값」의 뜻은 «제품 전체가 하나»다(상설, 판정 284·285): 길이 0 인 문자열은 — 공백만 있는 것도
strip 뒤 길이가 0 이면 — NULL 이고, 키를 «견주는» 자리에서는 NULL = NULL 이다. 접는 함수는 «하나»다.

🔵 **읽기 전용 관계(뷰)는 «한 줄»입니다**(09-12 S-186·S-187):
```
운영에서는 그 표에 `kind: "view"` 를 적으면 됩니다 — 그러면 쓰기가 «이름을 대어» 거절되고,
모델은 «선언한 컬럼만» 듭니다. 전순서를 줄 컬럼(`row_id` 또는 `business_key`)은 그대로 필요합니다.
```
```
앞     없음 — 이것도 이 표의 성질입니다
거절   `kind` 가 `table`·`view` 밖이면 «로드 때 경로를 대어» 거절 · 뷰에 쓰면 쓰기 문이 «이름을 대어» 거절
      🔴 전순서 컬럼이 «둘 다 없으면» 그 관계는 페이징이 거절됩니다(`422`, SCHEMA_CANON R7) —
        읽히기는 하나 «상한 걸린 읽기»가 매번 다른 페이지를 내므로
```
⚠️ **`kind` 를 «안 적으면» `table` 입니다** — 기본값을 정하는 자리는 제품 안에 «하나»뿐이고,
화면도 그 값을 받아 뷰에는 편집 진입을 «안 내줍니다»(종전에는 운영자가 타이핑한 뒤 400 으로 배웠습니다).

🔵 `search_columns` 는 «찾기»의 축이고 `map_key_columns` 는 맵의 축이다 — 둘 다 «이 표의 성질»이지
그것을 읽는 쪽의 성질이 아니다. 📎 [`config/`](./config/) · [CODE_MAP §crud](../architecture/CODE_MAP.md)

## ② 파생 — 무엇이 «무엇을 깨우는가»

```
운영에서는 «체인 규칙»(chain_rules.json)에 `trigger_table` 과 `target_table` 을, 그리고 그 일을 하는
`mapper_module`·`mapper_function` 을 적으면 됩니다. 특정 컬럼만 깨우려면 `trigger_columns` 를 적습니다.
```
```
앞     ① 두 표가 «카탈로그에» 있어야 한다. 목적지 표의 `business_key` 가 없으면 쓰기가 신원을 못 만든다
거절   `trigger_columns` 의 «값»이 트리거 표에 없는 컬럼이면 로드 때 «이름을 대어» 보고된다(S-140)
      옵트인 엣지만으로 된 «고리»는 로드 때 «이름 대어» 보고된다 — 거절이 아니다(09-15 판정 402). 유한성은 `max_chain_depth`
🔴 그러나 «키» 오타는 조용히 지나간다 — 규칙 하나의 «모양»을 검증하는 자리가 아직 없다(S-152)
```
🔵 **매퍼를 이름 대는 칸은 «하나»이고, 매퍼 «고유»의 값은 `params` 안에 넣습니다**(09-12 S-188):
```
운영에서는 그 규칙에 `mapper` 한 칸으로 매퍼 이름을 적고, 그 매퍼만 쓰는 값들은 `params` 안에 적으면 됩니다.
매퍼 쪽은 `@mapper(params=("<이름>", …))` 로 «자기가 받는 이름»을 선언합니다.
```
```
앞     ② 의 나머지와 같습니다 — 두 표가 카탈로그에 있어야 합니다
거절   `params` 에 있어야 할 이름이 «최상위»에 흘러나오면 로더가 «그 이름을 대고» 거절하며
      「어느 칸을 옮기라」고 말합니다. 규칙이 최상위에서 받는 이름 전부는 «한 목록»에서 나옵니다
      (`chain_bindings.routing_keys()`) — 그래서 문법이 한 칸 늘면 거절도 «같이» 압니다
🔵 두 칸 철자(`mapper_module` + `mapper_function`)도 «그대로 읽힙니다» — 은퇴가 아닙니다
```
📎 규칙 «하나»의 모양은 `server/chain_skeleton.json` 이 들고 있고, 그 파일은 «생성»됩니다 — 평면 모양(`root`)은 위 목록에서, 통합 모양(`unified_root` — `on`/`derive`/`into`, 2026-09-16 S-241)은 `rule_shape`·`join_into` 의 상수에서.
손으로 맞추는 둘째 목록이 아닙니다. 폼이 어느 모양을 그릴지는 raw 응답의 `grammar` 칸이 말합니다. ⚠️ 다만 **모양은 스켈레톤, 「무엇이 좋은가」는 로더**입니다.

⚠️ 🆕 **[2026-09-13 S-221 `fc0914a7`] `max_group_attempts` 는 이제 «규칙 칸이 먼저»이고 문서 칸은 «기본값»이다**(합친 단위는 최소). `max_rows_not_visible_defers`(기본 30)는 문서 «최상단»이고 규칙이 아니라
«문서»의 칸이고, SYSTEM_RELOAD 로 반영된다(재기동 불필요).
📎 [CHAIN_CONTRACT_COMPLETENESS](../architecture/CHAIN_CONTRACT_COMPLETENESS.md) · [chain_ingestion_guide](./chain_ingestion_guide.md)

🔵 **그리고 그 규칙을 «화면에서» 등록할 수 있습니다**(09-13 C-86 `910c18fb`):
```
운영에서는 어드민 chain 탭에서 [규칙 추가] → 칸을 채우고 → 맵퍼를 «고르고» → 저장하면 됩니다.
칸의 «모양»은 서버가 주고(`chain_rules` raw 응답의 `skeleton`), 고를 수 있는 맵퍼는 «등록된 것»뿐입니다.
```
```
🔴 저장은 «장전»까지입니다 — 새 규칙은 `enabled: false` 로 저장됩니다. 표 등록은 저장해도 아무것도
  안 돌지만 «규칙은 다음 리로드에 돕니다», 그래서 저장이 곧 발화이면 안 됩니다.
  켜는 것은 원문 편집기에서 `enabled` 를 고치는 «그 한 자리»입니다(둘째 컨트롤을 만들지 않습니다)
🔴 맵퍼 목록은 «등록부»입니다 — `@mapper` 가 붙은 것만. 파일에 있는 함수 전부가 아닙니다.
  저장이 «받아 줄» 이름과 «고를 수 있는» 이름이 같아야, 고른 뒤 거절받는 길이 없습니다
⚠️ `enabled` 는 «세 상태»입니다: true · false · «키가 없음». 없는 것을 false 로 그리면
  「안 물어봤다」가 「꺼져 있다」가 됩니다
```


## ③ 확정 — 무엇을 «판단 단위»로 모으는가

```
운영에서는 «인리치 규칙»(enrichment_rules.json)에 `source_table`·`derived_table` 과 `decision_key`,
그리고 채울 `target_fields` 를 적으면 됩니다. 후보를 보여 줄 질의는 `reference_views` 에 적습니다.
```
```
앞     ① 의 `decision_key`(판단 단위는 «표»의 성질이다) · 파생 표를 체인이 쓴다면 ②
거절   참조뷰의 바인드 파라미터가 «판단 키 컬럼»이 아니면 그 뷰 «하나»만 떨어진다(나머지는 산다)
      `aggregations` 의 함수가 지원 밖이면 «이름 대어» 거절한다(조용히 드롭하지 않는다, S-102)
```
🔵 `auto_confirm` 은 기본 꺼짐이다 — 자동 확정은 «옵트인»이고, 그 일은 커밋 경로가 아니라
뒤따르기에서 돈다. 📎 [config/enrichment_rules](./config/enrichment_rules.md)

## ④ 가상 조인 — 복사하지 «않고» 옆 표의 컬럼을 보여 주기

```
운영에서는 «가상 조인 규칙»(virtual_join_rules.json)에 `left_table`·`right_table`·`join_key` 와
보여 줄 컬럼 `expose` 를 적으면 됩니다.
```
```
앞     ① 두 표가 «카탈로그에» 있어야 한다 — 규칙이 `left_table`·`right_table` 을 «이름으로» 댄다
      🔴 그리고 오른쪽 표에 그 조인 키를 덮는 «UNIQUE 인덱스»가 «먼저» 있어야 한다.
        ⚠️ **이것은 «걸음»이 아니라 «DB 상태»다** — 걸음으로 적으면 없는 의존이 생긴다.
        없으면 이 걸음이 «자기» 모집단에서 `no_unique_index` 로 거절한다(뒤 걸음을 막는 것이 아니다)
      🔵 그 인덱스는 «식 인덱스»다 — `coalesce(<컬럼>, '')`, 텍스트가 아닌 컬럼이면 `coalesce(<컬럼>::text, '')`(09-11 S-181 ·
        09-15 S-245 — 식의 저자는 `notation_norm.key_expression_text` 하나, 조인이 비교하는 식과 같은 함수). 평범한 UNIQUE 는
        `(L1, NULL)` «둘»을 들여보내서 NULL 키에 유일성이 «안 걸린다»(실측). 제품이 DDL 을 인쇄해 준다
거절   그 인덱스가 없으면 규칙이 `no_unique_index` 로 거절되고, `GET /admin/config/resolve` 가
      «자기를 그렇게» 보고한다 — 조용히 실패하지 않는다(필요한 DDL 도 같이 인쇄된다)
```
⚠️ 이름이 왼쪽 표의 컬럼과 «부딪히면» 안 된다 — 부딪히는 가상 컬럼은 어느 쪽을 읽는지 모르게 만든다.

🔵 **그리고 이제 조인은 «실제로 쓸 수도» 있습니다**(09-12 S-189, 판정 298·302):
```
운영에서는 그 규칙에 `materialize: true` 와 `max_rewrite_rows` «두 칸»을 적으면 됩니다 — 그러면 조인 값이
대상 행에 «실제로 써지고», 그 값의 출처(`source_name`)는 «그 규칙 이름»이라 레이어링이 누가 썼는지 압니다.
```
```
앞     위와 같고, 🔴 **왼쪽 표에도 색인이 하나 더** 필요합니다 — 참조 행이 바뀔 때 «그것을 참조하던 대상 행»을
      찾는 질의가 씁니다(제품이 `required_left_index_ddl` 로 그 DDL 을 인쇄해 줍니다). ⚠️ 오른쪽 유일 색인과
      «다른» 색인입니다: 오른쪽은 «읽기 시점 조인»이 쓰고 왼쪽은 «되쓰기»가 씁니다
거절   `max_rewrite_rows` 를 «안 적으면» `no_rewrite_cap` 으로 거절하고, **그 수를 세는 질의를 같이 인쇄**합니다
      — 기본값이 «없습니다». ⛔ 그 상한을 넘으면 «거절»이지 «거기까지만 쓰기»가 아닙니다
```
🔴 **왜 기본값이 없나 — 그 칸은 «시간»이기 때문입니다.** 참조 행 «하나»가 바뀌면 그것을 참조하던 대상 행을
«전부» 다시 씁니다. 상설 「변경 비용이 핵심 제약」이 이 자리를 고른 것이고, 제품이 «말한 적 없는» 긴 쓰기의
저자가 되지 않도록 그 수를 «운영자가» 적습니다.
🔵 **실체화는 «옵트인»입니다** — 이 칸을 안 적으면 종전대로 «질의 시점»에만 보여 줍니다. 그리고 실체화한 값은
사람이 그 셀을 고치면 «사람이 이깁니다»(`user` 덮어쓰기가 이름 붙은 소스를 이미 이깁니다 — 여기에 두 번째
우선순위 규칙을 두지 «않습니다»).


## ⑤ 원장 — 무엇을 «사실»로 적는가

```
운영에서는 «원장 선언»(ledger_config.json)의 `entities` 에 노드 타입과 그 `keys` 를, `vocabulary` 에 술어와
그 `subjects`·`object` 를 적고, `sources` 에 「어느 표를 읽어 어떤 문장을 만드나」를 적으면 됩니다.
```
```
앞     ① 그 표와 컬럼이 카탈로그에 있어야 한다 · 파생 표를 읽는다면 ②③ 가 먼저 그것을 채운다
거절   `object.value_type` 이 넷 밖이면 `unsupported_value_type`
      `timestamp` 값인데 바인딩에 시간대가 없거나 이상하면 `invalid_timezone`
      선언 안 된 컬럼을 바인딩하면 `unknown_column` · 은퇴한 타입을 내는 문장은 «이름 대어» 거절
      한 문장이 «양 끝을 같은 신원»으로 읽으면 자기 엣지로 거절 · `cardinality: one` 위반은 쓰기에서 분자째 거절
🔵 깨진 선언 하나는 «혼자» 떨어진다(로더가 어느 것인지 말한다) — 나머지 선언은 그대로 돈다
```
📎 [ledger_declaration_by_example](./config/ledger_declaration_by_example.md) — «두 줄»의 정본 ·
[LEDGER_SCHEMA_COMPLETENESS](../architecture/LEDGER_SCHEMA_COMPLETENESS.md) — 칸이 있나/읽는 쪽이 있나

## ⑥ 걷기 좌석 — 무엇을 «묻는가»

```
운영에서는 화면의 좌석에 «시작 마킹»과 `follow`(어느 술어를 밟나) · `collect`(무엇을 실어 오나)를 고르면 됩니다.
방향과 깊이는 `direction`·`hops` 이고, 기간이 필요하면 `since`·`until` 을 줍니다.
```
```
앞     ⑤ — 좌석이 고르는 이름은 «선언된» 엔터티와 술어다. 선언에 없으면 고를 수 없다
거절   선언 안 된 노드 타입이면 `node_type_not_declared` 로 «아는 집합»과 함께 거절
      시각이 ISO 가 아니면 `interval_not_iso8601` · 시작 ≥ 끝이면 `interval_empty`
```
🔵 `follow` 는 «길»이고 `collect` 는 «짐»이다. 그리고 후보 경로는 «선언이 뽑아 준다» — 손으로 적지 않는다.
📎 [WALK.md](../architecture/WALK.md)

---

## ✅ 순서는 이제 «코드»가 안다 — 이 장은 그 설명이다 (S-180 ⓒ, 2026-09-12)

```
정본   server/config_resolve_report.py 의 SETUP_STEPS — 여섯 걸음의 번호·이름·도메인·앞걸음
답     GET /admin/config/resolve 가 도메인마다 `step` 과 `blocked_by` 를 «같이» 낸다
       blocked_by = 앞 걸음이 «안 선» 번호. 「거절이 0」이 아니라 「효과가 있나」가 기준이다 —
       아무것도 선언 안 된 걸음도 뒤가 가리킬 것을 주지 못한다
걸음 밖  `notation` · `binding` 은 여섯 걸음의 «밖»이고 `step: null` 로 «보인다»(숨기지 않는다)
```
⚠️ **그러므로 이 장에서 순서를 «다시 적지 않는다».** 두 목록이 생기면 갈라지고, 갈라진 쪽은
오류를 안 낸다 — 여기 있는 것은 걸음마다의 «두 줄»과 «거절»이고, 순서 자체는 위 리스트다.

## 🔴 아직 «없는» 것

```
S-179  네 언어(카탈로그 · 체인 · 인리치 · 원장)가 «각자» 컬럼을 이름 대고 «각자» 거절한다.
       같은 컬럼을 네 번 적는 자리가 있고, 넷이 어긋나도 «오류가 안 난다»
```

---
📎 이 장은 «옮겨 적기»다 — 각 걸음의 정본은 위 링크이고, 여기서 새로 정한 것은 «없다».
📎 칸 이름은 2026-09-11 에 출하 샘플과 로더 코드에 대고 확인했다.
