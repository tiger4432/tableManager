...
둘 다 **읽는 쪽에서 구분할 수 없다.** 「그때 생겼다」로 읽히고 **walk 이 그 값을 세계 시각으로
취급한다**(⚰️ 종전 이 자리는 여정·대조 두 소비자를 댔고, `/journey` 와 `/siblings` 는 둘 다
2026-08-28 에 은퇴했다 — 소비자가 줄어도 **틀린 시각이 원장에 눕는다는 사실은 그대로다**).
§15의 「가짜 값 생성 금지」와 정면으로 어긋난다.
