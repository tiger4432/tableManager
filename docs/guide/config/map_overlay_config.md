...
| `table_bindings.<table>.columns.key_columns` | 🔴 **[2026-08-14 R-2026-08-14-A F3] 정체성은 `table_config.<t>.map_key_columns` «한 곳»에서만 선언합니다.** 이 키를 생략하면 거기서 상속되므로, **`table_config`와 같은 값을 여기 다시 적지 마십시오** — 아무것도 바꾸지 않으면서 진실의 사본을 둘로 만들고, 유도가 아직 도는지를 가립니다. **다르게** 선언해야 할 때만 적고, 그럴 때는 그 테이블 블록에 **`__reason`을 반드시 함께 적으십시오**(오버레이가 `table_config`가 모르는 무엇을 알기에 다르게 부르는지). 이유 없는 오버라이드는 선언이 아니라 드리프트입니다. **집행 지점**: `GET /admin/config/resolve?domain=binding`이 키마다 「선언이 유도를 이겼다 / 유도와 같아서 지워야 한다 / `__reason` 없는 오버라이드다」를 문장으로 말합니다(`fields.restates_derivation`·`fields.reason_declared`). ⚠️ 실제 사고: `core_wafer_map`이 이 사본을 들고 갈라졌고, 쓰는 쪽은 `core_lot`+`core_slot`으로 맵 id를 짓는데 읽는 쪽은 `wafer_id`로 찾아 **등록된 바닥 204개 중 201개가 조작자에게 아예 제시되지 않았습니다** |
