...
체인 mapper와 백필은 빈 판단키 조합을 애초에 만들지 않지만(스킵·집계), **그리드 빈 행 추가나 판단키 없는 직접 편집**으로 생긴 행은 이제 워크리스트에 **뜹니다** — 판단키 누락은 인제션 단계 결함이고, 숨기면 영영 고쳐지지 않기 때문입니다. 그 행들은 워크리스트 **맨 뒤**로 내려가고 헤더의 **「판단키 없음 N건」** 배지로 집계되며, 선택하면 상세와 참조뷰가 "판단키 없음 — 조회 불가"라고 **이름으로** 답합니다(참조뷰는 판단키 **값으로** 조회되므로 빈 값은 근거를 찾을 수 없습니다).

- **운영자가 할 일**: 이 배지가 0이 아니면 그 수만큼 **인제션 쪽에 판단키가 안 채워지고 있다**는 뜻입니다. 큐에서 지울 수 있는 항목이 아닙니다 — 원본/매핑을 고쳐야 사라집니다. 원인별 내역은 `python server/scripts/enrichment_insights.py classify <규칙>`의 **`blank_decision_key`** 행에서 확인합니다.
- **[2026-08-05 재정] 자동 확정(①)은 판단키가 일부만 있으면 남은 키로 판단합니다.** 쓰기 경로는 이제 `queue_filters`(= 큐 전체)를 걷습니다 - `auto_confirm` 자체가 무인 쓰기에 대한 동의이므로 그 아래에 관문을 하나 더 두지 않습니다. 물어볼 것이 **아무것도** 남지 않았을 때만 거절합니다: 빈 컬럼을 바인드하는 뷰는 `missing_bind`, 판단키가 전부 비었으면 `no_decision_key`.
  - 이렇게 결정된 셀은 `priority_source`가 **`enrichment_auto_confirm_partial_key`**입니다(평범한 자동확정과 **같은 서열 99**, 이름만 다름). 빠졌던 판단키 컬럼이 나중에 채워지면 이 이름으로 "온전한 키 없이 결정된 셀"을 **골라낼 수 있습니다** - 그것이 이름을 나눈 유일한 이유입니다.
  - 그래도 **판단키 누락 자체는 여전히 인제션 결함**입니다. 배지와 `blank_decision_key` 분류는 그대로 남아 있고, 채워야 사라집니다.
- 데이터는 삭제하지 않습니다.
...
- **`limit`을 1처럼 아주 작게 두지 마십시오.** 후보 조회는 `limit + 1`개까지의 **서로 다른 값**만 받아오므로, 값 종류가 그보다 많으면 집계가 잘려 `distinct_truncated`로 거절됩니다(2026-07-30 QA 수리 — 종전에는 거절하지 않았고, 잘려 돌아온 값들이 공백·소수점 차이로 **하나로 접히면** 「후보 1개」로 읽혀 자동 확정될 수 있었습니다).
...
> ✅ **어드민 화면에 나옵니다** (2026-07-31 `93610cb` — **Overview 탭의 세 번째 계기 줄**). 아래 사유는 그 줄에서 바로 읽히고, `curl` 응답의 `reason` 필드에도 같은 단어가 있습니다. 옆에 딸린 `detail`이 **사람이 읽을 문장**이며 **서버가 만들고 화면은 그대로 렌더합니다** — 둘이 다르면 그 자체가 결함입니다.
