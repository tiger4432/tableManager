...
| **`audit_history_config.json`** (2026-08-11 신설) | 행/셀 감사 이력 조회의 페이지 상한(`default_limit`/`max_limit`) + **전역 「최근」 패널 discovery 걸음의 상한**(`recent_*` 넷 — `server/admin/audit_cache.py`가 같은 파일을 재사용, 두 번째 로더 없음) | 요청마다 재읽기(요청당 1회 스냅샷) | 전용 조회 라우트 없음 — `limit`을 크게 요청해 clamp+`truncated`로 확인, 오타는 서버 로그 `[AuditCache] '<key>' must be a positive integer` | [audit_history_config.md](./audit_history_config.md) |
...
| `suggest_config.json` | 입력 제안(고유값 조회) 노브 + **접두 인덱스 대상 선정** | 조회 노브 = 즉시 / `index_*` = **`ops_setup_db_performance.py` 재실행** | `GET /tables/{t}/columns/{c}/values` | [suggest_config.md](./suggest_config.md) |
...
| 🗄️ `virtual_join_rules.json` | 🔴 **[2026-09-17 `306419fd` · 판정 461] 「저장하지 않는 조인」은 은퇴했다** — `materialize: false`(와 «칸 없음»)는 **이름 대어 거절**된다. 살아 있는 것은 `materialize: true`(표에 «쓰는» 조인)뿐이고, **새 조인은 이 파일이 아니라 `chain_rules.json` 의 `derive: {kind: "join"}` + `into: {table: …}`** 로 적는다. ⚰️ ~~읽기 경로가 실제로 실행해 `expose` 컬럼을 붙인다(`d70a33d`)~~ — 엔진이 삭제돼 오늘 «거짓». 여전히 참인 것: **팬아웃 가드**(승인 조건 = 조인 키를 덮는 UNIQUE 인덱스) | 쓸 때(`materialize: true`) | 모양은 `GET /admin/config/resolve?domain=virtual_join` · 승인 여부는 `GET /admin/config/virtual-join/verify` | [virtual_join_rules.md](./virtual_join_rules.md) |
