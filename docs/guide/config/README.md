...
| `notation_rules.json` | **표기 정규화** — 원본 컬럼(`WF.01`/`WF-01`/`WF_01`)을 접어 **파생 컬럼 `<컬럼>_norm`**에 기록. 🔴 **원본은 절대 안 고칩니다**(그래서 규칙이 틀려도 재파생으로 복구). ⚠️ **1단계 — 아직 아무도 그 값을 읽지 않습니다**(맵 키·필터·조인은 원본을 씁니다). 켜는 것은 **층 셋**: `table_config`의 컬럼 → 여기 쌍 선언 → (선택) `display_columns` 가시성 | 저장 후 **TTL 5초** 또는 `POST /admin/reload-configs` | `GET /admin/config/resolve?domain=notation` · 접기가 옳은지는 **false-merge 쿼리** | [notation_rules_config.md](./notation_rules_config.md) |
