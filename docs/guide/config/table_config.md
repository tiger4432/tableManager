...
| `search_columns` | string[], 기본 = `composite_key_source` | 🔴 **`?q=` 검색창이 «어느 컬럼»을 훑나.** 선언 안 하면 기본은 그 표의 «신원»(`composite_key_source`, 없으면 `business_key`, 그것도 없으면 `row_id`·`business_key_val`)이고, 요청의 `?cols=`가 «건별로» 이것을 덮습니다. ⚠️ **선언한 이름이 `column_types`에도 가상조인에도 없으면 설정 재적재 때 «이름 대어» ERROR로 거절**되고 그 표는 기본값으로 검색합니다(조용히 무시하지 않습니다). 🔴 **왜 기본이 «전부»가 아닌가 — 실측 2026-09-10 `dt_log` 200,215행**: 예전 기본(선언 컬럼 전부 ∪ 가상조인 컬럼 전부)은 한 번의 검색에 `ILIKE` 항 **31개**와 `dt_inventory`로 가는 **LEFT JOIN 6개**(같은 키, 가상 컬럼마다 별칭 하나)를 만들어 **2,174 ms**가 걸렸습니다. 신원 기본으로는 항 3개·조인 0개·**117 ms**입니다. 선두 와일드카드 `ILIKE`는 btree로 못 걸리므로 **범위를 좁히는 것이 유일한 구조적 처방**입니다(trigram 색인은 별건) |
...
| `source_priority` | {소스: 정수} | 소스 서열 맵의 테이블별 오버라이드. 🔴 **판정 405(S-243)**: 빈 칸은 서열 싸움에 안 끼어듭니다 — «사람(`user`)»과 «체인(`chain_ingestion`)»의 빈 칸만 NULL 층을 세우고(고의로 비운 것), 파일·스크립트·미등재 소스의 빈 칸은 층을 «안 세웁니다»(「아직 입력하지 않은 것」). ⚠️ 그래서 파일이 어제 값을 주고 오늘 빈 칸을 주면 «어제 값이 남습니다» — 지우는 것은 사람·철회(R2)·`replace_map` 의 일입니다 |
