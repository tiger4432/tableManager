...
- 🔴 **`val`을 생략하지 마십시오.** `bin_map`의 `bin`과 달리 **유도되지 않습니다** — 그리고 `val` 없이 `fail_values`만 있으면 **그 테이블 전 행이 fail로 계산**됩니다(아래 함정 ①).
