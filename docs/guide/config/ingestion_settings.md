...
| `external_sources` | 배열, 기본 `[]` | **외부 읽기 전용 바인딩**. 항목은 `enabled`(기본 true)·절대 `path`·`table_name`·`parser`·`recursive`(기본 true)·`options`입니다. 대상 테이블은 업무 키를 선언해야 하며 관리 워크스페이스와 겹치는 경로는 거절합니다. 같은 루트가 서로 다른 테이블을 먹이는 것은 허용하지만 같은 테이블의 겹치는 루트는 거절합니다. 현재 내장 파서는 `voids_json` 하나이며 대상은 `inspection_run`/`void_obs`입니다. **재기동 반영**. 상세·JSON 형태는 [INGESTION_GUIDE §1.12](../INGESTION_GUIDE.md) |
