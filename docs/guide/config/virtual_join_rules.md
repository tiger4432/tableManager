...
- **읽기 시점 조인은 `chain_rules.json` 에도 산다 — 목록 하나, 검증기 하나**(2026-09-16 S-251 `e175d3f8`).
  `load_virtual_join_rules` 가 이 파일을 읽은 «뒤» 통합 파일의 `derive.kind: join` + `into: {read: true}` 선언을 «같은 목록»에
  «같은 `_validate_join`» 으로 붙인다(`_read_time_joins_from_unified`, `origin: "decl"`) — 그래서 같은 이름공간 · 같은 유일 게이트(S-235) ·
  같은 철회(S-248)를 받는다. 둘째 검증기는 「이 조인이 돌 수 있나」의 둘째 답이라 «없다». ⛔ **같은 이름이 두 파일에 있으면 «이름 대고» 거절**(통합
  쪽이 무시된다 — 골라 주면 다른 파일이 거짓이 된다). ⚠️ **이 파일이 «없어도» 질문은 끝나지 않는다** — 종전엔 파일 부재가 곧 `[]` 였고, 통합 파일에만
  적은 배포는 조인이 «하나도» 없었다. ⚠️ `path` 를 넘긴 «부분» 읽기는 통합 반쪽을 «안» 집는다(부분 목록에서 철회를 안 도는 것과 같은 사유).
  체인 로더 쪽은 그 선언을 «거절이 아니라 노트»로 넘긴다 — 종전엔 맵퍼 칸 없는 규칙으로 읽혀 `unresolvable_mapper` 로 «버려졌고», 옳은 선언이 오타로 보고됐다.
