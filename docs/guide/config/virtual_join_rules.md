...
여전히 열려 있는 것 (**커밋 트리 `77d27d3` 기준 실측** — ⚠️ 두 항목 모두 **작업이 진행 중**이므로 인용 전에 현재 소스를 확인하고, 해소되면 위 세 곳을 함께 걷을 것):

- ⏳ **CSV export에 가상 컬럼이 실리지 않는다.** `GET /tables/{t}/export`는 조인 실행기를
  부르지 않는 별도 경로라, **화면에 보이는 컬럼이 추출물에는 없다.** 운영자가 화면에서 보고
  받아 간 CSV를 열었을 때 컬럼이 사라져 있는 형태이므로, 조인 컬럼을 **보고서 재료로 쓰지
  말라고 안내해야 한다**(고쳐질 때까지).
- ⏳ **`미상` 행을 찾을 방법이 없다.** 가상 컬럼은 `filter: false`다 — 이 그리드의 컬럼 필터는
  **서버측**이고 서버는 그 이름으로 조건을 만들 수 없어서, 필터를 켜면 페이지가 필터 없이
  돌아오고 **화면만 걸러진 것처럼 보인다**(`Matches:`와 페이지 수는 전량 그대로). 반만
  동작하면서 아무 말도 안 하는 필터보다 **필터가 없는 편이 낫다**는 판단이고, 제대로 고치려면
  서버가 그 컬럼을 아는 형태가 필요하다 — **클라만으로는 못 고친다.**
