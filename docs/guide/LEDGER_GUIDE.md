...
> 넷째 문법 **`declared`**로 붙는 소스는 **파이썬을 0줄 쓴다**(§3 머리의 상자) — §3 ③~⑥이 통째로 해당 없음이고
> 절차는 [ONTOLOGY_LEDGER_SETUP §3.4](./ONTOLOGY_LEDGER_SETUP.md), 계약은 [spec §3.8](../spec/LEDGER_TECHNICAL_SPEC.md)이 소유한다.
> ⚠️ **이 문서를 「새 소스에는 이 절차」로 읽던 사람이 가장 손해 보는 자리다** — 그래서 §3의 첫 화면에 상자를 두었다.
...
| `config.py` | `ledger_config.json`을 **로드 시점에** 검증 · 🔴 **문법이 «넷»**(2026-08-15 4차) | **선언 없는 것은 기본값이 아니라 거절이다** — 시각 컬럼·시간대가 없으면 소스 전체를 거절한다. `translator_version()`이 **선언 전체를 해시**해 원자마다 어떤 규칙이 만들었는지 남긴다. **런당 1회 읽는다**(행마다 아님). 🔴 **소스는 이제 `kind`를 선언한다**(`lineage` \| `observation` \| **`transfer`**(2026-08-14 4차), **기본 `lineage`** — 그래서 기존 선언 전부가 정확히 옛 뜻을 유지한다). 검증이 그 낱말로 **분기**하고 필수 컬럼 집합이 갈린다 — 관측 소스를 혈통 규칙으로 채점하면 `parent_lot`이 없다는 이유로 거절됐을 것이다(선언 키는 [ONTOLOGY_LEDGER_SETUP §3.2](./ONTOLOGY_LEDGER_SETUP.md)). 🔴 **`transfer`는 `group`(묶는 컬럼 + 행 정렬 타이브레이크)과 `container`(목적지 신원을 «확정하는» 관계)를 «추가로» 요구한다** — 확정 관계를 선언 못 하게 하면 번역기가 읽을 것은 행에 있는 추론 대상뿐이고, 그것을 읽으면 **틀린 조인이 조용히 성공한다**(선언 키는 [ONTOLOGY_LEDGER_SETUP §3.3](./ONTOLOGY_LEDGER_SETUP.md)). 🔴 **[2026-08-15 4차] 넷째 갈래 `declared`에서는 이 모듈이 «컴파일러»다** — 그 문법엔 산출물을 검토할 파이썬 클래스가 없으므로 `_validate_declared_source`/`_validate_emit_rule`이 유일한 사전 검사다. 그래서 **저장 시점에 알 수 있는 것을 실행 시점으로 미루지 않는다**: `emit[].subject.type`이 그 소스의 `subject_types` 밖이면 **로드 거절**이다(게이트도 거절하지만 그것은 행마다 백필 시점이다). 선언 키는 [ONTOLOGY_LEDGER_SETUP §3.4](./ONTOLOGY_LEDGER_SETUP.md) |
| **`declared_translator.py`** (2026-08-15 4차) | 🔴 **번역기 «클래스»가 아니라 선언 실행기** — 행→원자 사상이 `emit` 선언 그 자체다(브리핑 §6-2) | 🔴 **이 표에서 「이 소스는 이렇게 읽는다」를 말하지 «않는» 첫 행이다.** 새 모양의 대장·참조표가 와도 **파이썬 0줄**이고, 그것이 소유자 완성 조건(「다른 스키마의 운영 환경에서 코드 0줄」)에 실제로 닿는 첫 자리다. 근거: **번역기는 소스가 아니라 «모양»에 속한다**(`void_obs`·`delam_obs`가 이미 하나를 공유한다). 🔴 **`occurred_at_basis`가 필수이고 기본값이 없으며**(R-2026-08-15-N ②) `row_created`면 그 사실을 **value payload에 찍는다**(`entity_ref` payload는 모양이 엄격 검사돼 여분 키가 거절되므로 **거기엔 못 싣는다** — 비대칭이 코드 주석에 이름 붙어 있다). 🔴 **`when`은 닫힌 연산자 집합에서 «정확히 하나»**(0개·2개·오타 전부 로드 거절 — 무시된 연산자는 조건을 «항상 참»으로 만든다) · 🔴 **없는 컬럼을 `"$col"`로 부르면 거절**(빈 값으로 안 푼다) · 규칙 이름이 곧 파생 이름(§3 ④). ⚠️ **규칙에 하나도 안 걸린 행은 거절이 아니고 `rows_matching_nothing`으로 «세어진다»** — 「1,181행이 왜 원자 40개인가」를 드라이런이 그 수로 답한다. ⚠️ **범위 밖: 리스트 분해·위치 짝짓기**(그건 선언이 아니라 작은 프로그래밍 언어가 된다) |
...
**문법 넷(`lineage`·`observation`·`transfer`·`declared`)과 각각의 선언 키 표는
[ONTOLOGY_LEDGER_SETUP §3](./ONTOLOGY_LEDGER_SETUP.md)이 소유한다.** 아래 ①·③·④·⑤·⑥은 **문법을 가리지 않고** 적용된다.

> ### 🔴 **[2026-08-15] 먼저 이것부터 — 「행 하나가 선언한 대로 원자 1~N개」면 이 절을 «건너뛴다».**
>
> 넷째 문법 **`declared`**는 **파이썬을 0줄 쓴다.** 번역기 클래스도, 새 파일도, 픽스처도, `③`의 어느 것도 없다 —
> **행→원자 사상이 `emit` 선언 그 자체**이고 그 선언 한 장이 이 절 전체를 대신한다.
> **가서 할 일**: [ONTOLOGY_LEDGER_SETUP §3.4](./ONTOLOGY_LEDGER_SETUP.md)의 키 표대로 선언 → 어드민 드라이런 → 저장.
> 계약은 [spec §3.8](../spec/LEDGER_TECHNICAL_SPEC.md).
>
> 🔴 **번역기는 «소스»가 아니라 «모양»에 속한다** — 그것이 이 문법의 근거다. `void_obs`와 `delam_obs`가
> 번역기 하나를 공유하는 것이 이미 그 증거였다(둘은 **한 모양**이다). 그래서 새 «모양»이 아니라 **새 테이블**일 뿐이라면
> 새 클래스는 세 번째 사본이지 세 번째 번역기가 아니다.
>
> **이 절로 «돌아와야» 하는 경우**(선언이 코드보다 읽기 쉬운 지점을 지난 자리):
> - **리스트 열을 분해하거나 위치로 짝지어야 한다**(`slot_numbers` ↔ `wafer_ids` → `derived_from` 1 + `has_wafer` 19).
>   그걸 JSON으로 적으면 **디버거도 스택 트레이스도 없는 작은 프로그래밍 언어**가 된다.
> - **여러 행이 모여 한 사건**이거나(`lineage`·`transfer`) **세상의 시각을 다른 관계에서 조인**해 와야 한다(`observation`).
> - 소스가 **테이블이 아니다** → §3-bis. ⚠️ **`declared`는 그 문을 열지 않는다** — 드라이버가 **소스 이름을 관계명으로** 페이징한다.
...
**`server/config/ledger_config.json`의 키 표 넷(문법별 `lineage` · `observation` · `transfer` · `declared`)은 2026-08-15에 그쪽으로 옮겼다.**
...
🔴 **[2026-08-15] `declared` 문법에서는 그 표가 «선언에서 통째로» 나온다** — `emit` 규칙의 `rule` 값 하나하나가 파생 이름이고
그 밖엔 `first_sight`(등록형 타입을 선언했을 때)뿐이다. **그래서 위 표를 손으로 늘릴 일이 없고**,
`WHERE source_translator_ver LIKE '%#<rule>'`이 손으로 쓴 번역기의 파생과 **똑같이** 질의된다.
⚠️ **그 문법의 새 파생은 아래 빨강을 «안» 낸다** — 그 테스트는 `.sample`의 **`lot_event` 소스 하나**를 열거하므로
운영자가 화면에서 켠 규칙은 그 그물에 아예 안 들어온다(실측 2026-08-15).
🔴 **그러므로 그 문법으로 새 규칙을 켤 때 「내 원자가 몇 류인가」를 물어 주는 것은 테스트가 아니라 사람이다**:
소스 «행»을 옮긴 것이면 2류이고, **행에 없는 것을 config 가정으로 채웠으면 3류**라 `inference_derivations`에 적어야 한다.
...
`ledger_config.json`에 항목을 만들려고 하면 **막힌다** — 기본 문법(`lineage`)의 `config.validate()`가 `columns` 일곱(`row_identity`·`lot`·`event_type`·
`parent_lot`·`child_lot`·`slots`·`wafers`)을 요구하고, 생성기에는 그 컬럼이 하나도 없다.

🔴 **[2026-08-15] 넷째 문법 `declared`가 이 벽을 «낮춘 것처럼 보이지만 아니다».** 그 문법의 필수 컬럼은 `row_identity` 하나뿐이라
「이걸로 생성기를 선언하면 되겠다」가 자연스러운 오독인데, **막는 것은 컬럼 목록이 아니라 드라이버다** —
백필이 **소스 «이름»을 그대로 관계명으로** 페이징한다(`SELECT … FROM <소스> ORDER BY <워터마크>`).
**질의할 관계가 없으면 어느 문법으로도 선언이 돌지 않는다.** 그러므로 아래 규칙은 그대로 산다.
