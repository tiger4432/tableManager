...
펼친 **표**를 만들고, 그 표를 읽는 소스를 범용 `declarative-role@1` 로 선언한다.
🔴 그 표는 **체인이 만든다**(소유자 판정 2026-08-31: 「lot event 에서 파생하라」).
스크립트로 «뷰»를 만들지 않는다 — 뷰는 그 스크립트를 돌린 박스에만 있어서, 그것을 가리키는
선언은 «다른 어디에서도 안 뜨는» 소스를 출하한다.
실례: 좌석↔웨이퍼 문장이 그렇게 옮겨 갔다 — 체인 규칙 `lot_event_to_lot_slot_wafer`
(`lot_event` → 표 `lot_slot_wafer`, 매퍼 `mappers/lot_slot_wafer_mapper`), 새 소스 하나 · 술어 하나.
옮기면서 옛 mapper 의 문장 셋이 은퇴했다.
⚠️ 처음엔 뷰로 만들었다가 되돌린 자리다. 원자 수는 양쪽에서 «907 로 같았고», 다른 것은
«다른 환경에서 뜨느냐»뿐이었다 — 이 박스에서 도는 것만으로는 그 차이가 안 보인다.
