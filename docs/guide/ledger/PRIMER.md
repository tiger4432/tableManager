...
📌 **선언된 술어는 «열넷», 그중 목적어가 `entity_ref` 인 것은 «열둘»이다** (실측 2026-08-29 심야,
라이브·`.sample` 동일). 나머지 둘은 `has_netdie@1`(`value`) 과 `register@1`(`none` — 등록 술어라
...
🔴 **[2026-08-29] 걷기는 «거절 규칙 셋»을 진다 — 그리고 그 셋은 손잡이가 아니다.** 요청이
못 고른다: 무엇이 정적인지도, 정적끼리 어느 술어로 갈 수 있는지도 **선언이 정하고 walk 이 매
요청 읽는다.** 규칙의 내용·기전·시험·실측은
