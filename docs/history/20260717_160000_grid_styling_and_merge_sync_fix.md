...
* 대상 행에 이미 동일한 소스명(예: `file.txt`)이 존재하여 DB 유니크 키 제약조건으로 인해 덮어써지는 유실을 막기 위해, **이식 시점에 껍데기 행의 비즈니스 키 명칭(예: `a_b`)을 접미사로 부착하여 `'file.txt (a_b)'` 와 같이 고유화해 개별 소스로 축적 적재** 합니다.
