...
### F. 충돌 병합 시 진짜 데이터 원천 소스명 보존 및 다중 이력 누적 (Append)
* 기존 병합 시점에 `CellSource` 의 `source_name` 을 일괄 `"collision_merge"` 로 하드코딩 교체하던 방식을 완전히 탈피했습니다.
* 이제 복합 비즈니스 키 중복 충돌로 병합(Silent Merge)이 일어날 때, **`_load_metadata_row_cell` 헬퍼 함수를 통해 삭제되는 껍데기 행들이 가졌던 모든 데이터 원천 소스 리스트를 누락 없이 대상 행으로 계승 및 이식(Append)** 합니다.
* 병합으로 유입되는 껍데기 행의 소스 이식 시, **껍데기 행의 비즈니스 키 및 row_id 앞 6글자 고유 슬라이스(예: `a_b_019ee4`)를 접미사로 무조건 강제 부착하여 `'file.txt (a_b_019ee4)'` 와 같이 고유화해 개별 소스로 축적 적재** 함으로써, DB 유니크 충돌(덮어써짐)에 의한 이력 누락을 100% 원천 예방했습니다.
* **사용자 수동 수정값 보호 시에도 소스 강제 이식**: 대상 행에 기존 사용자 수동 수정(`user`) 값이 이미 적재되어 있어 실제 값 덮어쓰기(병합)는 차단되더라도, 껍데기 행들의 소스 정보는 아예 무시되지 않고 **대상 행의 이력(CellSource)으로 무조건 추가 적재(Append)**되도록 보호 정책 가드와의 실행 흐름을 완벽하게 분리 설계했습니다.
...
### H. 연쇄 인제션 워커 및 디렉터리 와처 언패킹 결함 수정 및 실시간 제거 연계
