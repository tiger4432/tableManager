...
- **해결 내역**:
  ...
  - **어드민 대시보드 Auto Updates 상태판 및 강제 기동 연동 (Auto-Update Admin Panel & Run Now Trigger)**:
     ...
     - **스케줄러 기동 블록 해제 및 스케줄링 흐름 교정**:
        ...
        - `discover_and_load_collectors()` 호출 중에 `_write_status_file()`을 동기식 호출하면서 동일 스레드 내에서 Lock이 이중 획득되어 발생하던 **스케줄러 기동 데드락(Deadlock) 결함**을 `threading.RLock()` (재진입 가능 락) 구조로 전환하여 완벽하게 영구 소멸시켰습니다.
     - **GraphSync & Chain 워커 파싱 및 제어 이벤트 무시 가드 연동**:
        - SQLite 환경 및 직렬화 특성에 따라 아웃박스의 `payload`가 JSON 객체가 아닌 문자열(`str`)로 로드될 수 있는 예외 케이스를 안전하게 캐치하여 `json.loads` 디코딩을 수행하도록 타입 방어 로직을 보강했습니다.
        - 수집기 제어용 아웃박스 이벤트인 `SCHEDULER_RUN_NOW` 등의 레코드가 적재될 때, 체인/그래프 동기화 가공 트랜잭션 대상이 아니므로 예외 없이 즉각 건너뛰고 처리 완료 마크를 부여하도록 필터링 가드를 장착했습니다.
