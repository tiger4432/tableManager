...
- 🔴 **[2026-08-21] `setup_version: 5` — 필수 section은 «셋»(`vocabulary`·`entities`·`sources`)이고 소스 하나가 `relation`·`read`·`prepare`·`map`·`bind`를 직접 든다.** `packs`·`source_preparers`·`mappers`·`profiles` 네 section이 은퇴했고(앞의 셋은 본문이 소스 안으로, `packs`는 **술어에서 도출** — `setup_bundle.predicate_claim`), mapping은 mapper가 선언한 **문장 별명**으로 키가 매겨지며 `use` 대신 **`predicate`**로 술어를 직접 댄다(`mapping_id`·`map.emits`·`bind.packs` 은퇴). 🔴 **[2026-08-22~23 · `setup_version`은 5 그대로]** 낱말 넷이 더 없어졌다 — vocabulary의 **`layer`**(`unknown_field`로 거절. 마이그레이션 `migrate_ledger_config_drop_vocabulary_layer.py`)와 binding의 **`binding_origin`·`approval_status`·`suggestion_reason`**(검증기가 **받아서 버린다** — 옛 파일은 그대로 열리고, 떼면 커서 지문이 움직이므로 정규 번들에는 실려 간다). **「binding 승인」이라는 실행 관문은 없다.** 그리고 **`read.cursor`는 이제 묻지 않고 `read.order_by`에서 덮어써 파생**된다. 옛 세대는 `migrate_ledger_config_to_v4.py` → `migrate_ledger_config_to_v5.py` 순. 모양의 정본은 [ONTOLOGY_LEDGER_SETUP §4·§7](../guide/ONTOLOGY_LEDGER_SETUP.md), 작성 화면이 채워 주는 것은 [§13.3-quater](../guide/ONTOLOGY_LEDGER_SETUP.md)
...
- active snapshot 기준: 🔴 **여기 적어 두지 않는다.** 이 자리의 `f6223d6c…`는 2026-08-21의
  config 모양 라운드 넷으로 **낡았다** — `canonical_content_json`이 바뀌면 해시가 움직인다.
  지금 값은 위 dry-run의 `snapshot_sha256`이 답한다
