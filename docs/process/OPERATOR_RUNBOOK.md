...
| **🔴 [2026-08-21] 그 환경의 config가 «옛 세대»면 백필 전에 올려야 합니다** | 위 dry-run이 `unsupported_setup_version`이나 `unknown_field`(`source_preparers`/`mappers`/`profiles`)로 거절하면 그 root는 `setup_version: 4` 이전입니다. 올리는 한 줄은 (`server/`에서)<br>`conda run -n assy_manager python -m scripts.migrate_ledger_config_to_v4 <config 경로> --check`<br>**`--check`는 무엇이 바뀔지만 보고하고 아무것도 쓰지 않습니다** — 먼저 이걸로 보고, 빼면 제자리에 씁니다. **멱등**이라 이미 v4인 파일은 안 바뀝니다. 별명을 못 정하는 mapping이 있으면 **그 자리와 사유를 찍고 파일을 손대지 않습니다.** 설명은 [ONTOLOGY_LEDGER_SETUP §2.4](../guide/ONTOLOGY_LEDGER_SETUP.md) |
