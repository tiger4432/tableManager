...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-21 | **Owner:** Lead / PM
...
| **🔴 이 문서는 소스 «문법»의 개수를 적지 않습니다** | 이 자리엔 오래 「셋」이라 적혀 있었고 틀렸습니다 — 같은 문장이 `LEDGER_GUIDE`·`CONFIG_GUIDE`·`README`에도 복사돼 **넷이 함께 틀렸습니다.** 그래서 개수 대신 **정본과 세는 법**만 둡니다: 레거시 경로의 문법 목록은 `server/ledger/config.py`의 `SOURCE_KINDS`이고, 다시 세는 한 줄은<br>`conda run -n assy_manager python -c "import sys; sys.path.insert(0,'server'); from ledger import config; print(sorted(config.SOURCE_KINDS))"`<br>⚠️ **Ledger v2 선언에는 `kind`가 «아예 없습니다»** — v2 소스 하나가 `relation`·`read`·`prepare`·`map`·`bind` 다섯 절을 직접 들 뿐이라 **v2에서는 「문법 개수」라는 것이 세어지지 않습니다**(🔴 [2026-08-21] `setup_version: 5`. 종전 여기 적혀 있던 `driver`·`preparer_id`·`mapper_id`·`profile_id`는 은퇴한 낱말이고, `packs`·`claims`·`use`도 같은 날 은퇴했습니다). 🔴 **[2026-08-18] 백필이 타는 경로에 `kind`는 이제 관여하지 않습니다** — `SOURCE_KINDS`가 남아 있는 곳은 레거시 선언을 읽는 어드민 dry-run 쪽이고, 백필 드라이버는 하나입니다 |
...
| **🔴 [2026-08-21] 그 환경의 config가 «옛 세대»면 백필 전에 올려야 합니다** | 위 dry-run이 `unsupported_setup_version`이나 `unknown_field`(`packs`/`source_preparers`/`mappers`/`profiles`)로 거절하면 그 root는 `setup_version: 5` 이전입니다. 🔴 **스크립트는 둘이고 순서대로 탑니다** — (`server/`에서)<br>`conda run -n assy_manager python -m scripts.migrate_ledger_config_to_v4 <config 경로> --check`<br>`conda run -n assy_manager python -m scripts.migrate_ledger_config_to_v5 <config 경로> --check`<br>**`--check`는 무엇이 바뀔지만 보고하고 아무것도 쓰지 않습니다** — 먼저 이걸로 보고, 빼면 제자리에 씁니다. **멱등**이라 이미 목표 모양인 파일은 안 바뀝니다. 별명을 못 정하는 mapping이 있으면 **그 자리와 사유를 찍고 파일을 손대지 않습니다.** 🔴 **v5는 Claim을 버리기 전에 그것이 술어의 되풀이였음을 «대조»하고, 어긋나면 차이를 찍고 거절합니다.** 설명은 [ONTOLOGY_LEDGER_SETUP §2.4](../guide/ONTOLOGY_LEDGER_SETUP.md) |
