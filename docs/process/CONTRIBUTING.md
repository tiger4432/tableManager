...
- 🔴🔴 **빌드는 «게이트»이자 «산출물»입니다 — `client2/dist` 가 «같은 커밋»에 들어갑니다** (2026-09-13 C-108-b). `npm run build` 를 돌려 초록을 «보고»하고 `dist` 를 되돌리면, 소스에는 있고 **사용자에게는 없습니다**: 페이지는 `dist/index.html` 이 가리키는 «옛 번들»을 계속 받습니다. 실측 그날 — 새 코드가 담긴 커밋의 dist 델타가 «0» 이었고, 그때 `dist/index.html → main-B6WWaaXd.js` 그 번들에 새 낱말이 «0 건»이었습니다(재빌드하면 1). 🔴 판별식은 「빌드가 초록인가」가 아니라 **「`git show --stat` 에 `client2/dist` 가 있나」**입니다. ⚠️ `git add` 는 언제나 **파일 이름을 나열**해서 — 해시 이름이 바뀌므로 «지워진 셋 + 새로 난 셋»을 같이 적습니다(`-A` 금지는 그대로).
