...
- **진행 중(in-flight)**: ① **온톨로지 G1** — 구현 완료(137 passed, C-7/#8 동승 해소) → qa-reviewer 판정 **GO-WITH-FIXES**: 엣지 provenance 구조 오염 2건 높음(H1 로우 이벤트 source 일괄 날인→user 위조·경로 간 불일치·병렬 증식 / H2 재교정 시 구 RESOLVED_AS 잔존) + 중간 1(워커 이벤트 루프 기아) + 낮음 4 — server-pm이 같은 브랜치에서 수정 중. 수정 후 병합→데모 매핑 반영→라이브 검증(사용자 승인 완료) ② **admin UX 소안** — client-pm worktree 구현 중(차단 3+마이크로 8+헬스 4카드), 이후 중안(파이프라인 축 탭 재편) 연이어 ③ 서브그래프 미니 뷰어 지시서 준비됨(`Ontology_subgraph_viewer_task.md`) — G1 병합 후 서버 API(server-pm)→뷰어 페이지(ui-designer) 순차 착수.
