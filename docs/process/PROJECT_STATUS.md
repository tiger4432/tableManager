...
2-3. **🟡 M2.6 — DOE 입력 단순화** · 삭제 버그 수정 착지 후 착수(같은 파일)
   ...
   - **할 일 ② 구간 = 정수 2칸 `[from] - [to]`**. 자유 텍스트 `stack_band` 폐기. `1, 2-15, 16`은 **구간 3개**가 된다(쉼표 표기는 세 구간을 한 칸에 넣은 것이었다). 층 수 = `to − from + 1` — **파싱이 아니라 산술**이라 서버·클라가 갈릴 여지가 없다. 비수치 라벨 처리 문제도 함께 소멸.
