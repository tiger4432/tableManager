...
>
> **진행 중**: server-pm 폴더 구조 보존 인제션(경로를 파서 인자로, 조건부 아카이브) — `server/parsers/*` 2파일 + 신규 테스트 2개가 미착지 상태.
>
> 🔴 **`client2/dist`는 아직 재빌드/커밋 안 됨.** 지금 트리의 빌드 산출물은 server-pm 미착지 작업까지 번들한다 — **server-pm 라운드 착지 후 한 번에** 재빌드한다.
>
