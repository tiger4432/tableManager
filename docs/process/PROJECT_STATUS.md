...
- **온톨로지 트랙 현재 상태(2026-07-25 저녁)**: **G1 + 뷰어 라이브 가동 완료** — ① G1 QA 수정(H1/H2/M3+4) 병합 후 라이브 검증 통과([GraphLatency] lag 162ms, 백필 청킹 1초/C-7 실증, 교정→RESOLVED_AS(user) 실시간) ② H2-b(교정 비움 시 잔존) 총괄 라이브 실증→수정 커밋 d130c65(146 passed)→잔존 엣지 부분 resync 정리 완료 ③ 조회 API 3종(stats/neighbors/search — c63b881, 158 passed) + 서브그래프 뷰어 graph.html(f41ca3e) 배포, 재기동 후 **라이브 시연 통과**: stats(Chip 1,304·RESOLVED_AS 5), LOT-C|15 중심 20노드 서브그래프에 user provenance 엣지 강조 렌더, 자동완성·재중심 탐색·콘솔 에러 0. 다음: G2(추적 리포트) — 착수 대기.
- **admin 트랙**: 소안+중안 배포 완료 — 파이프라인 5탭 IA(387d987) 라이브 검증 통과. 대안(온보딩 위저드·규칙 CRUD) 이관 목록 대기.
