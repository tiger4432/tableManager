...
| 2 | 문서 정비 — **새 4분할 구조 첫 실행**(historian·mapper·keeper 동시 → auditor 검수) | 대기 |
| 3 | **B4 롤백 절차** (프로덕션 차단) | 대기 |
| 4 | **C1 접근 통제** — 최소 `admin/scripts/code`·`auto-update/run-now` 두 라우트 | 대기 |
| 5 | **C3 백업 절차** — config·workspace·PG. 복원을 실제로 해볼 것 | 대기 |
| 6 | `replace_map` 빈 집합 — 서버 scope 필드 | 대기 |
| 7 | 문서 등급 감사(A/B/C) + C등급 아카이브 — 살아있는 문서 49개 축소 | 대기 |
| 8 | M3 맵 메타 자동 등록 (ingestion 체인) | 대기 |
| 9 | P3 대형 파일 (backpressure·COPY·상한) | 대기 |
| 10 | 온톨로지 — **장기 트랙** | 별도 |
