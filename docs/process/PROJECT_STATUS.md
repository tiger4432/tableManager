...
- **진행 중(in-flight)**: ① **온톨로지 G1 착수(사용자 승인)** — server-pm이 worktree에서 구현 중(지시서 `Server_ontology_g1_task.md`: PG 엣지 스토어 + ontology_mapping v2 + materializer + C-7/#8 동승). 완료 시 qa-reviewer 검수(고위험 도메인 필수) → 병합 → 커밋 ② doc-keeper 코드맵 동기화(#7 배치 반영) 완료 대기 — 도착 시 검수·커밋. 스펙은 `ONTOLOGY_GRAPH_SPEC.md` v0 확정(4기둥: LLM 지식그래프/JOIN 효율화/불량 추론 네트워크/시공간 topology + §7.6 그래프 보조 교정 순환).
