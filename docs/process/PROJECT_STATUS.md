...
>
> 🔴 **`f3fd785` QA 결과 — GO-WITH-FIXES. 확정 결함 1건이 조용한 데이터 소실 경로다(총괄 라이브 재현 완료, 읽기 전용).** `candidate_for`에 오타가 하나 있으면 → 탐침 SQL 실패 → **Postgres 트랜잭션 abort** → `_diagnose_probe_failure`가 **같은 세션에** 진단 질의를 던져 `InternalError` → `resolve_target_candidate`가 그걸 삼키고 정상 반환 → 체인 워커의 `except`는 **발화하지 않는다** → `db.commit()`이 **정상 반환**(Postgres가 COMMIT을 ROLLBACK으로 바꾼다) → **작업 단위가 조용히 사라지고 로그는 「chain write unaffected」라고 말한다.** `apply_batch_updates`에 `db.commit()`은 **0개**이므로 그 주석의 「already committed」는 사실이 아니다. **오늘은 불발**(선언 0건) — 그러나 F9 어드민 표면이 하는 일이 정확히 「선언하라」다. 수리안: 탐침을 `begin_nested()`(SAVEPOINT) 안에서 돌린다 — 한 수로 오염과 `candidate_column_missing` 도달 불가가 같이 닫힌다. **SQLite 테스트는 이걸 못 잡는다**(pysqlite는 SELECT에 트랜잭션을 안 연다 — 초록 테스트가 운영에 없는 동작을 보증하고 있었다)
>
