...
| **O10** | 🔴 **`dt_log` 코어 쪽은 같은 수를 쓰면 안 된다** ― 판정 기준이 나왔다 | > **두 번째 프레임의 키가 선언된 맵 키와 1:1인가?** 그렇다 → x/y 재지정이 두 번째 뷰의 전부. 아니다 → **다른 맵 정체성**이라 x/y만 바꾸면 **조용히 겹쳐 쌓인다.** <br>실측: `bonding_log` bond↔DT는 **1:1**(120↔120, 충돌 0) · `dt_log` dt_job↔core는 **아님**(한 job이 코어 웨이퍼 **2~12개**를 섞는다). 그래서 `dt_log`에 그대로 하면 **77행이 58셀**로 그려지고 **19개가 서로 덮어쓰며 셀 수 말고는 신호가 없다** |
| **O11** | ✅ **사용자 판정 2026-08-02: 코어 쪽 뷰는 「표기 정규화」로 간다** | 후보는 둘이었다 ― `core_wafer`(깨끗, **2,491행**) vs `(core_lot, core_slot)`(**7,440행**, 더러움). 후자를 택하되 **정규화 선행**. 🔴 **더러움의 정체**: `core_lot` **766행에 `_`가 들어 있는데** 맵 키 파서(`map_overlay.map_key_parts`)가 `_`로 쪼갠다 → `CL_2601_001_09`가 `core_lot='CL'`+`core_slot='2601_001_09'`로 분해돼 **셀 0개**. (`dt_job`이 안전했던 건 **키 컬럼이 하나라 문자열 전체를 먹기** 때문) `_`→`-` 접기로 철자 30→15, 쌍 369→151. <br>⚠️ **그래도 남는 것**: 8,700행 중 **1,260행(14.5%)에 `core_lot`/`core_slot`이 아예 없다**(`core_wafer`만). 정규화로도 안 덮인다 |
...
| **N7** | 🔴 **가상 조인이 `number` 컬럼에서 죽는다** (사용자 신고 2026-08-02, 진행 중) | `resolved_expression`이 `COALESCE(..., <unresolved_label>)`을 만드는데 **라벨은 문자열**이라 숫자 식에 못 들어간다. <br>📌 **왜 출하됐나**: server-pm 실측이 당시 **「숫자 expose 컬럼 0개」**였다 ― 축이 존재하지 않아 안 걸렸다. 픽스처에도 숫자 expose가 없으니 **먼저 하나 선언해 빨갛게 만든 뒤** 고쳐야 한다. <br>📌 **설계는 페이로드 쪽에선 이미 알고 있었다** ― `announced_columns`가 *「값의 정의역에 `unresolved_label`(문자열)도 들어간다」*고 적고 그래서 라벨을 스키마에 함께 싣는다. **나중에 붙은 SQL 경로가 그 추론을 물려받지 않았다.** <br>**유력 수리**: COALESCE 전에 양쪽을 텍스트로 캐스팅(필터 경로가 이미 override를 항상 텍스트로 취급한다). <br>🔴 **그러면 SQL은 텍스트, 페이로드(`_resolve_one`)는 원값**이 되어 **검색하는 것과 보이는 것이 갈릴 수 있다** ― `contracts/blank_predicate`가 지키라고 만든 이음새다. **숫자 컬럼으로 두 철자를 서로 채점**해 일치 여부를 답해야 하고, **일치시킬 수 없으면 그게 수리보다 값진 발견**이다 |
