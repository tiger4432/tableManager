...
2-3. **🟡 M2.6 — DOE 입력 단순화** · 삭제 버그 수정 착지 후 착수(같은 파일)
   ...
   - **할 일 ⑤ `map_doe`·`map_doe_source` 폐기 — 3테이블 → 1.** 구간에서 수량·knob이 빠지면 남는 건 `{seq, to}`뿐이고, 자재도 그 구간에 딸린 문자열 배열이라 **값 행의 JSON 하나**로 충분하다. `map_split_registry`가 **값 = DOE 조건 그 자체**를 담는다(색·설명·knobs·`bands[{seq,to,materials[]}]`). 온톨로지가 소비하는 `knobs`·`split_desc`는 **평평한 컬럼으로 남는다** — 중첩되는 건 계획 구조뿐이다. 원래 M2-v2 노트의 "legend/split registry와 DOE를 합친다"가 여기서 완성된다.
     > ⚠️ 이 항목은 한때 "3→2"로 적혀 있었다(`82d4e8e`). 사용자가 이후 `map_doe_source`까지 접기로 정했고 착지분(`cdcddee`)은 **1테이블**이다 — 보드를 그때 갱신하지 않아 doc-keeper가 지시서와 보드의 불일치로 잡아냈다.
