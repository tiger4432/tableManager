...
| 2 | **C1 접근 통제** — 실태는 체크리스트보다 넓다: `main.py`에 **admin 라우트 16개, 인증 전무**(모든 `Depends`가 `get_db`). `POST /admin/scripts/code`(:3896, 임의 py 파일 쓰기) + `POST /admin/auto-update/run-now`(:3586, 실행) = **사내망 임의 코드 실행**. 설계(사용자 확정): **공유 토큰 헤더** · 미설정 시 **위험 2개는 503으로 차단**(fail closed)하고 나머지는 열어둬 잠금 사고 방지 · `/health`는 무인증 유지(B2) · 로그인 화면 없음 | 🔵 착수 (**T1** — 잠긴 척하는 실패가 최악이라 등급 상향) |
