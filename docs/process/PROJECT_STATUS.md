...
>
> ✅ **빌드 해제됨 (`392fca4`)** — 게이트 초록(16개 중 11 강제·5 부채). ⚠️ **`dist` 커밋은 보류 중**: client-pm 레인이 `admin.js`를 고치는 중이라, 끝나고 **한 번만 다시 빌드**해서 올린다. 그 전까지 `:8081`은 여전히 `93610cb` 번들이다.
...
>
> 🔵 **다음**: ① `copy_header_count` 재기준(**계약이 바뀐 게 맞다** — 규약이 바뀌었으므로 재기준이 정당) + 404 메시지 → 빌드·`dist` 해제 ② 문서 정비(12커밋 밀림 — 스펙 §5.1·§5.7, 유효 다이 가이드, 하네스 수 15→16이 낡음)
>
> 🅿️ **파킹**: Task A③(격자 다른 참조의 Push 게이트) · Task B(회사 양식 상단 병합 맵 영역 한정) · 오버레이=규칙 6(**미구현이고 주장하지 않았다**) · enrichment 상한 3개 config화(대기열 20)
>
