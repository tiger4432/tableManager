...
>
> **사용자 확인 대기 3건**: ⓐ 응답 헤더(`Server:`가 프록시를 자칭하는가 · **`WWW-Authenticate: X-Admin-Token` 부재 = 우리 게이트 아님** 증명) ⓑ 같은 포트의 `/health` 상태(200이면 경로별 차단) ⓒ 8080을 소유한 프로세스(python이 아니면 워커의 `API_BASE_URL`이 프록시를 향하고 있다).
>
