...
>
> 🔴 **운영 서버가 `f3fd785` 이전 프로세스다** — `/health` 200인데 `/admin/config/resolve` **404**(8080·8081 둘 다). 그래서 F9 화면이 「조회 실패」만 뜬다. **코드 결함이 아니라 재기동 사안.** 다만 클라가 404를 「조회 실패」로 뭉개는 것은 고칠 값어치가 있다 — 404는 「이 서버엔 그 기능이 없다=재기동하라」로 **행동이 다르다**.
>
