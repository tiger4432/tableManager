...
| 21 | **높음** | 🔴 **`business_key_val`에 UNIQUE 제약이 하나도 없고, dedup 전제가 이미 깨져 있다** (2026-07-31 실측, 가상 조인 가드 라운드의 부산물 — 총괄 재확인). 그 컬럼을 덮는 인덱스 **54개 전부 non-unique**다. 시스템의 dedup 업서트는 「키 하나당 행 하나」를 전제로 하는데 **DB가 그 전제를 강제한 적이 없고**, 라이브에서 이미 위반돼 있다: 중복 그룹 **2,201개 · 초과 행 2,636개** — `bonding_map` 1,878그룹(**최악 437행/키**) · `inventory_master` 163 · `bonding_log` 117 · `wafer_process` 43. <br>⚠️ **가상 조인보다 넓은 문제다** — 시스템이 「행 하나」라고 믿고 읽는 모든 자리가 실제로는 여러 행일 수 있다. 가상 조인 가드가 이걸 **거절 사유로 삼아** 막고 있으나(`bonding_map`은 구조 검사를 통과하고 라이브 프로브에서 거절됨), 그건 조인 경로 한 곳만 막는 것이다. **먼저 물어야 할 것: 이 중복이 정상인가(정본이 여러 행)인가, 아니면 업서트가 놓친 것인가.** 전자면 계약을 고쳐야 하고 후자면 데이터를 고쳐야 한다 — 데이터 정리는 사용자 소관 | Server | 판정 대기 |
