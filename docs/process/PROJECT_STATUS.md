...
- **진행 중(in-flight)**: 온톨로지/그래프 트랙 스펙 논의 중 — 확정된 방향: 자동 대량 노드/엣지 승격이 본체(매핑 config 1급, 처리량 1급 제약, 엣지 provenance 레이어링), **LLM용 백본 지식그래프**가 최종 용도(사용자 지시), 추적 UX는 리포트형, v1 범위는 데모 3종, 저장소는 PG 엣지 스토어 방향(Neo4j 이관 가능성 질의에 답변 중). 다음: `docs/spec/ONTOLOGY_GRAPH_SPEC.md` 초안.
