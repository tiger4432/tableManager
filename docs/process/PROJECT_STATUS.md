...
>
> 🟡 **진행 중 (2026-07-31 저녁)**: map-pm 1레인 — **Copy to Excel 상단 병합을 맵 격자 열 수로 한정**(사용자 판정 ⓐ: 지금은 그보다 넓게 병합된다) + **`COLS`/`ROWS` 재배치**(위 🔴). 문서 2레인 — 코드맵 재측정(개명 반영) · 히스토리. **doc-keeper 몫은 `8a257f0`으로 착지**(13파일, ⏳ 6곳 정리, 헤더 6개 압축).
>
