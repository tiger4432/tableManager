...
>
> 🔴 **`client2/dist`가 두 라운드 밀려 있다** (`93610cb` 시점 빌드). `prebuild`가 `check:harnesses`에서 막히고 그 원인이 `copy_header_count`(102/2)다 — 그 픽스처가 **바뀐 물리 키 규약을 고정**하고 있다. **`:8081`에서 QA하면 두 라운드 수리가 없는 코드를 본다.** dev(`:5173`)만 새 코드다.
>
