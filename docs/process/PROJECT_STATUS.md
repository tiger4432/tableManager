...
| **F2** | 🔴 **화면의 DOE 수량이 저장 불가능한 셀을 포함한다** (M4② 라운드 부수 발견 2026-07-29, **기존 결함**). `🎨 Fill All`이 `inside`와 무관하게 사각 전체를 칠하므로 원 기반 맵에서 **약 21%가 push 불가 셀**로 칠해지고, `updateLegendCounts`가 **그것까지 센다** → 사용자가 보는 DOE 수량이 실제 저장 가능한 양보다 많다. `pushMapData`가 `!inside`를 떨구므로 **화면 숫자와 저장 결과가 갈린다** — 조용히 틀린 숫자 계급. ⚠️ **수정하면 셀 카운트가 바뀌고 셀 카운트는 DOE 산술**이라 영향 범위 확인이 선행. F1ⓐ의 "COUNT는 원 안만"과 **같은 축**이므로 묶어서 처리 검토 | map-pm | T2 | 대기 |
