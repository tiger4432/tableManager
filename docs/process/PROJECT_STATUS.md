...
| **F12** | 🔴 **유효 다이 채택 차단을 없앤다 — 「그냥 틀어지는대로」** (사용자 재확인 2026-07-30: *"유효 다이 그리드 개수 다름 차단 없애라고 했는데 왜 있어? 그냥 틀어지는대로 하라고"*). <br>⚠️ **왜 아직 있었나 — 내가 지시를 내 판단으로 덮었다.** 사용자가 먼저 제안했을 때 측정 후 「지우면 `status: ok`인 틀린 답이 나온다」로 **기각하고 기각 사유를 보드에 적었고**(F6 행의 ❌ 항목), F6 착지 때도 없애는 대신 **기준만 바꿔**(좌표 이동 → 표현 가능성) 살려 뒀다. 커밋 메시지에는 「죽은 가드가 없는 가드보다 나쁘다」고 썼다. 그 추론은 내 것이고 그 결정은 내 것이 아니었다. **두 번 말했으므로 없앤다.** <br>✅ **없애는 것: 차단만.** `storedCoordRepositionPlan`의 `unrepresentable`·`stranded`·`collision`이 **더 이상 채택을 거절하지 않는다** — 채택이 진행되고 표현 가능한 것은 재배치되며 보존 불가한 좌표는 그냥 새 값을 갖는다. 사용자의 다른 판정(저장 좌표 = 오리진 기준 칸수)과 정합한다: 칸수가 바뀌는 것을 받아들인다는 뜻이다. <br>✅ **남기는 것: 계측 전부.** 사용자는 **막히는 것**에 반대했고 **알리는 것**에 반대하지 않았다. plan은 세 모집단을 계속 세고 알림은 「막았다」가 아니라 **「N개가 이동했고 N개는 유지 못 했다」**로 사후 시제로 말한다. `repositionRefusalReason`은 판정이 아니라 서술이 된다. 이 계측이 이제 조용한 오답과의 유일한 경계다. <br>🔴 **한 가지는 침묵시키지 않는다 — 좌표 이동과 다른 것이다.** `serverCellKeys`의 표현 불가 항목은 `replace_map`이 **산 서버 행을 지운다**(불변식 ④) — 변위가 아니라 삭제다. 그 개수를 **이미 있는 Push 확인창**에 넣는다. 새 모달·새 게이트·새 컨트롤 0개 — Push는 이미 그 「쓰기 1회 확인」이다. <br>🔵 **서버는 무변경**: `map_overlay.py:406`은 클라가 채택하지 않은 조합·낡은 저장 조합에 대한 정직한 백스톱으로 그대로 둔다 | map-pm | T2 | 🔴 **진행 중 — 5시 마감 라운드 1순위** (감산적이라 마감 하에 가장 안전) |
| **F10** | 🟠 **세 번째 죽은 하네스 — `copy_header_count_harness.mjs`가 HEAD에서 크래시** (map-pm 발견 2026-07-30). `copyGridToExcel`이 `auxHeadWords()`(`map_editor.js:5846`)를 부르는데 하네스의 `SYMBOLS`에 없어 `ReferenceError`로 죽고 **그 뒤 모든 단언이 미채점**이다. `git log -S"function auxHeadWords"`는 `ae2811c`를 지목하는데 그 커밋은 하네스를 건드리지 않았다. ⚠️ **`check_contracts.mjs`가 죽은 하네스를 잡으려고 만든 가드인데 이걸 못 잡았다** — 대기열 2번(스코프가 `client2/src contracts`만 봐서 `client2/tests`를 안 본다)과 **같은 구멍**일 가능성. 두 건을 한 라운드로 볼 것 | map-pm+client-pm | T2 | 대기 (대기열 2번과 병합 검토) |
