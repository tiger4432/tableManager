...
>
> 🔴 **`COLS`/`ROWS`를 손으로 고치면 재배치가 안 돈다** (2026-07-31 doc-keeper 발견). 물리 규격 6개 입력은 `reseatCellsToStoredCoords`를 타는데 **격자 치수 두 칸은 clamp 후 다시 그리기만 한다**(`map_editor.js:1016-1037`). 같은 결함 계급의 **네 번째 자리**이고 사용자 눈에 보인다 — 오늘 셋(참조 교체·영역 변경·기하 편집)을 닫는 동안 이 경로는 아무도 안 봤다. 스펙 §5.7-ter·가이드 §4-bis.5에 기록됨, **수리 미착수**
>
