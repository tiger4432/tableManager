...
>
> 📌 **좌표 규약 (사용자 판정, 정본)**: ① **화면이 정본** — 저장은 화면을 그대로 따른다, 결함은 「화면을 조용히 움직이는 것」 ② 표시 = **오리진 + DB값** ③ `start_x/y` = **유효 다이 영역의 최소 열/행 값** ④ 오리진 = start를 놓았을 때 (0,0)인 점. <br>실측 근거: `c − c_origin ≡ xv` **항등**(204프레임 315,210셀 불일치 0) — 표시와 저장은 **한 양**이다
>
> 🔵 **다음**: ④ 유효 다이 `start` 정합 — `DTWWER→BASE_4E` 차이 **(4,3)**. ⚠️ **착수 전 계약 판정 필요**: 마스크 기준이 **물리 키**(현행, SPEC §5.0)냐 **DB 라벨**이냐. 양쪽 값 실측됨 — 물리 키면 105셀 Push 통과, 라벨이면 81셀 + 24 거절
>
