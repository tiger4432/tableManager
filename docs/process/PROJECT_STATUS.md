...
>
> ❓ **미해결 — 어드민 토큰 모달이 안 뜬다 (2026-07-31 사용자 실측)**. localStorage(`assy.adminToken`)가 **비어 있는데** 401이 화면에 그대로 흐르고 모달이 안 떴다. **새로고침해도 안 떴다** — 그래서 「`adminTokenDeclined`가 서 있었고 리로드로 풀린다」는 내 설명은 **반증됐다**(그 플래그는 모듈 변수라 리로드에 초기화된다). 사용자는 값을 직접 넣어 통과했으므로 **막혀 있지는 않다.** <br>🔬 **추측하지 말고 잴 것**: `askForAdminToken`의 `tokenPromptInFlight`·`setTimeout` 경로, 그리고 **다른 어드민 호출이 401을 안 냈을 가능성**(이 박스에서 `/admin/chain/rules`는 토큰 없이 **200**이었다 — 게이트가 열려 있으면 F9 라우트만 401이고 모달을 띄울 계기가 다르게 잡힐 수 있다). 재현 조건부터 확정할 것
>
