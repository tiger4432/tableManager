...
| 5b | **맵 에디터 소형 3건** (H1·H2 라운드 인접 발견, 기존 결함): ⓐ 지문 불일치 토스트가 "초안은 지우지 않았습니다"라는데 직후 로드 persist가 덮음 ⓑ `legendDirty`가 맵 전환 시 리셋 안 됨 ⓒ **메타 없는 맵 프레임 사상 근본 수리** — 퀵 QA 실측: 기본 프레임 전 선택지가 원형 마스크로 373/1293만 담아 **모든 기본 프레임에서 Push 불가**(가드가 정상 발화 — 즉 근본 수리 전까지 메타 없는 맵은 기하 조정 없이는 못 올림) + 로드 후 격자 크기 입력이 readOnly라 UI에서 프레임 조정 불가 | map-pm | T2 | 대기 |
