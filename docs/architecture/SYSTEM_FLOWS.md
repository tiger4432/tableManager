...
### 🔴 2차 실측이 «또» 넷을 찾았습니다 — 그리고 **번호가 충돌했습니다** (2026-09-06 저녁)

⚠️ **D 와 F 가 «각자» ㉗㉘ 을 붙였습니다.** 레인이 따로 재니 당연한 일이고, 그대로 두면
「㉗」이 두 가지를 가리킵니다. **여기서 정본 번호를 줍니다 — 그리고 번호는 «읽기 편의»이고
정본은 «이름»입니다.**

| # | 흐름 | 어디 접혀 있었나 | 실측 |
|---|---|---|---|
| ㉗ | **외부 읽기 전용 소스 인제션** | ① 안 | `_D.md` |
| ㉘ | **런타임 워크스페이스 등록** | ⑱ 의 재시도 폴러 안 | `_D.md` |
| ㉙ | **프로세스 로깅 «두 가족»** (`*_stdout.log` tee vs `*.log` FileHandler) | ㉔ 안 | `_F.md` |
| ㉚ | **그리드 페이지의 어드민 토큰 경로** | ㉑ 안 | `_F.md` |

🔴 **넷 다 «같은 모양»입니다 — 같은 이름의 것이 «둘»이고, 둘째가 첫째의 규율을 «안 물려받았습니다».**
   ㉘ 은 특히 중요합니다: **①의 「워처가 아무것도 안 본다」에 대한 «유일한 복구 경로»**인데,
   어느 흐름에도 안 들어가 있으면 그 복구 «가능성»을 아무도 못 잽니다.

⚠️ 그리고 «따로 세지 않은» 것도 실측이 적어 뒀습니다 — 트리 인제션 · 기동/주기 스윕 · heavy 레인은
   전부 «같은 물음에 다른 트리거»이고 한 함수로 합류합니다. **트리거가 다르다고 흐름이 되지 않습니다.**

### 진행 — **전부 쟀습니다**
```
1차 (2026-09-06 낮)   ⑩⑦⑧ · ⑲⑳⑯ · ⑪⑫⑬                     아홉   -> §5
2차 (2026-09-06 저녁)  ①②㉖⑱ · ③④⑤⑥ · ㉑㉓㉔㉕ · ⑨⑭⑮⑰㉒     열일곱  -> task/SYSTEM_FLOWS_D/E/F/G.md
찾은 것               ㉓㉔㉕㉖(1차) · ㉗㉘㉙㉚(2차)              여덟   인벤토리에 «없던» 흐름
=> 목록 22 + 발견 8 = «30». 전부 실측됨
```
🔴 **§5 는 아직 «1차만» 들고 있습니다.** 2차의 표는 `task/` 의 네 파일이 정본이고,
   결함 목록은 [`SERVER_DEFECT_QUEUE.md`](../process/SERVER_DEFECT_QUEUE.md) 가 정본입니다.
   ⚠️ 병합한다면 **번호가 아니라 «이름»으로** 옮기십시오 — 위 충돌이 그래서 났습니다.
...
| L-17 | `ledger_events` + 커서 | 🪦 `ledger_trace.coverage()` — **함수가 «없다»**(2026-09-16 S-261 `8868d1a0`) | — | — | — | — | — | ⚰️ 라우트가 2026-08-28 에 독자를 데려갔고, 껍데기는 2026-09-16 에 `trace.py` 에서 내려갔다(1,721 → 1,050줄). ✅ **그 물음들이 오늘 사는 자리**: 배포 부재/빈 원장 = walk 라우트의 `ledger_relation_absent`·`state: empty` + `admin/schema_drift` · 「어느 소스가 썼나」와 거절 세 상태 = **L-13** (`ledger.admin.ingestion_view`) · `reltuples` 추정 = `backfill` · 파티션 목록 = `ledger.schema`. 🔴 **이름 둘은 남았다** — `COVERAGE_STATES`(`event_constants`·`listing_absence` 가 「absent/empty/ready」의 저자로 댄다)와 `ATOMS_UNKNOWN`(「이것은 추정」의 공용 모양). 투영은 죽고 그것이 저자인 낱말은 안 죽는다 |
...
📎 그리고 **⑩ 은 이 저장소의 「반쪽」 중 «드문 방향»이다** — 보통은 쓰는 쪽이 살고 읽는 쪽이
   ...
   `ledger/admin.py` 가 이미 절반은 자백하고 있다: 「이걸 읽던 유일한 코드는
...
| C-18 | `database_outbox` | 운영자 | **`GET /admin/chain/queue`** (`require_admin_token`) | HTTP 응답 | 대기 깊이·나이를 **소유자별로** 가름(`event_constants.outbox_owner`), 자리표시자 표이름(`__retroactive__`)은 표로 안 셈, tx 로 접은 목록 · 도는 규칙·마지막 결과는 `chain/activity.snapshot()`(S-246 `38b5d8e1`: `builtin:` 종류도 등록 — 두 문이 `activity.running` 하나를 지난다) | 클라 `admin.js` → `chain_queue_panel.js` **1** | 🔊 404 면 화면이 「이 서버 프로세스에 … 없습니다 (404) — 재기동이 필요합니다」 | ✅ |
...
```
⚰️ **[2026-09-15 판정 402 `2504fbde`] 위 「REFUSED」는 그날의 실측이고 오늘은 «안 난다»** — 같은 플립이 이제 로드되고, 검증기는 같은 trail 을 «한 번 말하고 돌려준다»(어드민 그래프의 「고리」 칸). 유한성은 `max_chain_depth` 가 맡는다. 「둘째 엣지가 순환을 만든다」는 그대로 참이다 — 바뀐 것은 순환의 «판정»(오류 → 모양)이지 «검출»이 아니다.
...
> 프로세스는 `runtime/process_supervisor.py` 가 아니라 **`run_decoupled_app.py`** 의
...
| A-14 | `heartbeat.beat("scheduler")` | `run_decoupled_app.py` 명부 → `server/runtime/health.py` | 매 틱(5초) | 심박 파일 | `note` 없음; `DEFAULT_STALE_AFTER_SEC=60.0` | 1 writer · 1 명부 · 1 reader | 🔊 `/health` 가 stale/wedged 로 보고. ⚠️ 다만 §3-⑤ | ⚠️ |
...
| `backfill.run()` 결과 dict | `admin/retroactive.py:387-390 _run_ledger_backfill` 반환 dict | 운영자가 어드민에서 `POST /admin/retroactive/ledger_backfill/run` | 함수 반환값 | **7칸만 통과**: `rows_read`·`batches`·`inserted`·`deduped`·`molecules`·`stopped`·`cursor_after`. 🔴 `refused_total`·`refused_samples`·`refused_samples_capped` 는 **여기서 버려진다** | 1 (`admin/retroactive.py:1477` → `RetroactiveRun.result` JSON) | 🔴 **무음** — 화면은 「분자 N개 만들었다」만 보고, 몇 건이 왜 거절됐는지는 «응답에 아예 없다» | 🔴 끊김 |
| `RetroactiveRun.result` | `client2/src/admin.js:2551` → 소급 실행 화면 | 어드민 소급 탭 열기 / 폴링 | HTTP `GET /admin/retroactive/runs?limit=50` (`admin/retroactive.py:1127 runs()`) | `{run_id, op, label, params, state, processed_rows, total_rows, result, error, …}` — `result` 는 위 7칸 | 1 | 화면이 조용히 「성공」으로 읽는다 — 거절된 행이 있어도 `state` 는 성공이다 | ⚠️ 반쪽 |
...
| 자리 | 적혀 있는 것 | 실측 |
|---|---|---|
| `gate.py:256` docstring | 「…for the report and for **`/health`**」 | `/health` 는 게이트를 **안 읽는다**. `runtime/health.py:432-443` payload 의 `checks` 는 `database·workers·outbox·supervisor·config_backup` 다섯뿐 |
...
| 런처 `run_decoupled_app.py:351` | `heartbeat.py:327 write_roster` → `_roster.json` | 부팅 1회 (`Supervisor` 생성 직전) | 파일 | `{"written_at": t, "processes": {name: t}}` — 실측 이름 **셋**: `watcher`·`chain`·`scheduler`. 🔴 **웹서버는 `heartbeat=` 가 없어 명부에 «없다»**(`:310-314`), 데스크톱도 없다 | **1** — `runtime/health.py:219 _hb.read_roster()` | 시끄럽다 — 실패 시 WARNING 한 줄(`run_decoupled_app.py:355`) 후 «디스크 폴백». 명부가 비면 「존재하는 비트 전부」로 떨어진다(`runtime/health.py:220-221`) | ✅ 이어짐 |
| `process_supervisor.Supervisor.write_status` | `<config>/supervisor_status.json` | 1초 폴 / 5초 강제 갱신 (`STATUS_REFRESH_SEC`) | 파일 | `{supervisor_pid, updated_at, children{…state, restarts, pid, last_exit_code, failure_reason, correlated_with, correlated_retries, **terminal_verdict**}, failed_children, correlated_children}` | 1 (`main.py:262 _supervisor_mod.read_status()`) | 시끄럽다 — `updated_at` 이 감독자 자신의 생존 신호. 낡으면 `runtime/health.py:174-181` 이 UNHEALTHY + 문장 | ✅ 이어짐 |
| `supervisor_status.json` 의 `terminal_verdict` | `/health` 응답 | — | — | `broken_child` / `port_conflict` (`runtime/process_supervisor.py:867`, 스냅샷 `:1069`) | **0** — `runtime/health.py:158-167` 의 `sup_check["children"]` 이 복사하는 칸 7개에 **없다.** `server/*.py`·`client2/src/*.js` 전건에서 이 값으로 분기하는 코드 0 | 🔴 무음 — 「포트를 누가 점유했다」는 판정이 파일에만 앉는다 | 🔴 끊김 |
| `heartbeat.read_all` + `read_status` + `read_roster` + DB/outbox 프로브 | `runtime/health.py:107 compute_health` | `GET /health` 요청마다 | 함수 인자 | 워커 판정 = 감독자 뷰 × 비트 뷰 조인. `expected` 결정 순서 **감독자 → 명부 → 디스크**(`:213-221`), `uptime` 없으면 명부 시작시각에서 채움(`:229`) — 이 한 줄이 재기동 503 이었다 | 1 | — | ✅ 이어짐 |
...
| `event_constants.py:300-302` 미전달 마커 3종 | 쓰는 쪽 / 줍는 쪽 | — | 상수 | `UNDELIVERED_MARKER_STATUS="SUCCESS"` · `_PROCESSED_CHAIN=True` · `_TAG="undelivered_notification"` | STATUS **2**(`internal_event_client.py:145` 쓰기 / `chain/ingestion_worker.py:1383` 줍기) · PROCESSED_CHAIN **2**(`:146` / `:1382`) · **TAG 1 — 쓰기뿐**(`:144`). 스윕 필터(`:1381-1386`)는 `payload["marker"]` 를 «안 본다» | — | ⚠️ 반쪽 — 철자를 묶은 것은 옳고, `TAG` 만 여전히 「쓰고 아무도 안 읽는」 칸이다 |
...
| `total_log_count` | 화면 | 대량 tx | WS | 발신: `chain/ingestion_worker.py:1092·1102` · `main.py:5828`. **`main.py` 의 `batch_row_upsert` 5곳 전부에 없다** | 🔴 **0** (`client2/src` 히트 0) | 🔴 무음 | 🔴 끊김 |
...
| `config_backup.py:366 probe` | `runtime/health.py:80 probe_config_backups` | `GET /health` (60초 캐시, `runtime/health.py:70`) | 함수 반환값 | `ok\|missing\|stale\|unknown` + detail. import 실패·예외는 **`unknown`** — 확인 불가를 이상 없음으로 내지 않는다 | 1 (`runtime/health.py:419`) | 시끄럽다 | ✅ 이어짐 |
| `runtime/health.py:420-430` | `/health` `checks.config_backup` | 위 | HTTP JSON | `missing`/`stale`/`unknown` → `escalate(STATUS_DEGRADED)`(`:426`) + `problems[]` 문장. 🔴 **절대 503 이 아니다**(`:444`) — 백업 부재는 「다음 인시던트가 어려워진다」이지 「지금 장애」가 아니고, 503 이면 모니터가 멀쩡한 스택을 재시작한다 | 위 ⑥ 참조 | 🔴 **무음** — 상태코드로는 안 보이고 200 안의 문자열이다. 그리고 그 문자열을 읽는 화면이 **0** | 🔴 끊김 (마지막 칸) |
...
| `ledger/admin.py:738-744` | 어드민 화면으로 저장되는 config·mapper — 「이것이 유일한 undo 기제」라고 자기 주석이 선언(`:734-736`) | 어드민 저장 |
...
| `COLLECTS.trend_y` → `fetchTrends` → `GET /api/ledger/trends` | 서버 | — | — | 🔴 **그 라우트가 없다.** `ledger/trace_router.py` 의 `@router` 는 «셋»뿐: `/subgraph`(84) · `/gaps`(353) · `/declaration`(400). 그리고 `collect:'trend_y'` 를 대는 좌석이 **0** — `main.js:218` 이 그 줄이 떠난 자리를 적고 있고, `main_trend_panel.js:85` 는 폴백을 `options.collect \|\| null` 로 «닫아 놨다** | **0** | 🔇 조용 | ⚰️ |
...
| `system_reload` | outbox 행 + `NOTIFY outbox_event` | 위 | DB 행 + PG NOTIFY | `DatabaseOutbox(event_type="SYSTEM_RELOAD")` commit 후 `NOTIFY`. 🔴 **NOTIFY 는 bare `except:` 로 통째로 삼킨다** — 실패해도 폴러가 나중에 잡는다 | `chain/ingestion_worker.py:1698`(`SYSTEM_RELOAD` 를 `id desc` 로, 스로틀) · `run_watcher.py` 폴러. ⚠️ **원장 v2 소비자는 이 이벤트를 안 읽는다** | 🔇 NOTIFY 실패는 완전 조용(폴링이 커버) | ✅ |
...
- **어드민 토큰이 두 갈래.** 라우트 15 중 읽기 6은 `require_admin_token`, 쓰기 9는 `require_admin_token_strict`. 클라는 `adminFetch` 하나로 둘 다 태우고 503(토큰 미설정)만 별도 토스트로 가른다(🆕 **[2026-09-17] 몸통은 `client2/src/admin_token.js` — `admin.js:177` 은 `deps` 둘을 주입하는 래퍼이고 그 토스트는 어드민이 «넘긴» `onServiceUnavailable` 이다. 그리드는 안 넘긴다 — 배너 줄이 이미 그 자리를 들고 있어 넘기면 같은 문장이 둘 뜬다**) — **strict 거부와 일반 거부를 화면이 구분하지 않는다**.
...
| `FrameConfirmation.superseded_by` / `supersedes_uid` 사슬 | (없음) | `record_confirmation:622` 이 `prev.superseded_by = uid; header.supersedes_uid = prev.confirmation_uid` | DB 컬럼 | 마이그레이션 `add_frame_confirmation.py` 가 append-only 로 깔고, 재확정마다 version+1 로 사슬을 **쓴다** | 🔴 **0** — 읽는 곳은 `.is_(None)` 필터 «셋»(`maps/frame_confirmation.py:141,174` · `map_alignment.py:6649`)뿐이고 전부 「현행만 고르려고」 읽는다. **이력을 내는 GET 라우트 0건**(`main.py` 의 `/api/maps/*` 전건 열거로 확인) | 🔇 조용 — 아무 화면도 「이 단위가 세 번 확정됐고 두 번 뒤집혔다」를 말하지 못한다 | 🔴 **흐름 ⑨ 의 「확정 이력」 절반이 쓰기만 있고 읽기가 없다** |
...
| `mappers/dt_map_mapper` → `dt_map_derivation.join_rule` | 🆕 **체인 선언**(`chain_rules.json` 의 `derive: {kind: "join"}`) | 체인 적재 | 함수 인자 | `join_rule(db, CONFIRMED_JOIN_RULE)` / `join_rule(db, FRAME_JOIN_RULE)` = `"dt_log_confirmed_attribution"` / `"dt_log_frame_attribution"`(`dt_map_derivation.py:104-105`, 호출 `:664-665`) | 1 | 🔊 `DerivationRefused(REFUSE_JOIN_RULE_MISSING, "join rule %r is absent from the chain declaration; … Next: declare it there as \`derive: {kind: \"join\"}\` with \`on\` … and \`take\` …")` — 로더가 그 이름을 «거절»한 경우는 «다른» 문장(`"… is declared but no rule stands for it … The loader said: %s"`) | ⚠️ **미측정** — 종전의 🔴끊김 판정은 「`virtual_join_rules.json.sample` 에서 그 두 이름이 `_retired_` 접두」를 «기제»로 삼았고, `0a6786f6` 이 **읽는 자리를 통째로 바꿔** 그 기제가 사라졌다. 오늘 끊겼는지는 «배포의 `chain_rules.json`»이 답하고 이 표는 그것을 못 잰다 |
| `POST /confirm` 의 `frames` | `dt_map_derivation.FRAME_COLUMN`(`"dt_frame"`) | — | — | 화면이 `frames:{}` 를 보냄 → `header.dt_frame = frames.get('dt_frame')` = **NULL**. 그리고 `dt_map_derivation` 은 `dt_frame` 을 🆕 **«체인 선언의 조인 규칙»**(`take` 목록)에서 읽지 `FrameConfirmation` 에서 읽지 않는다(`grep frame_confirmation server/dt_map_derivation.py` = **0** — 오늘도 참) | **0** | 🔇 조용 | 🔴 양쪽 다 존재하는데 잇는 것이 없다 |
...
## 6. 체크리스트 — 🔴 **여기 없습니다. 정본은 «대기열»입니다**

> 📌 **[`docs/process/SERVER_DEFECT_QUEUE.md`](../process/SERVER_DEFECT_QUEUE.md)**

이 절은 «1차 실측에서 뽑은 체크리스트»를 들고 있었습니다. 2차 실측이 끝나고 그 추출물이
대기열 문서로 모이면서, **같은 목록을 두 문서가 들게 됐습니다.** 소유자가 잡으셨습니다
(2026-09-06: 「시스템 플로우 문서랑 중복 아니야?」).

```
기준 ④ 판별식   「둘이 있나」가 아니라 「둘이 «갈라질 수» 있나」
              -> 갈라집니다. 항목을 고치고 한쪽만 갱신하면 «다른 쪽이 거짓»이 됩니다
=> 정본을 하나로 둡니다. 여기는 «부릅니다»
```

### 두 문서의 «물음»이 다릅니다 — 그래서 합치지 않고 «가릅니다»
```
이 문서    「무엇이 «어떻게 흐르나»」    지도입니다. §1 칸 · §2 목록 · §5 실측(8칸 표)
대기열     「지금 무엇을 «어느 순서로» 고치나」  작업 순서입니다. 등급 · 담당 · 상태
```
🔴 **§5 의 실측은 «그대로 여기»에 있습니다.** 옮긴 것은 «뽑은 것»(투영)이고,
   근거(표)는 안 옮겼습니다 — 「투영은 지워도 되고 기록은 안 된다」.

### ⚠️ 다만 §4 의 «규칙» 하나는 여기 남습니다 — 목록이 아니라 판별식이라서
```
「소비자 0」은 «두 뜻»이다
   빼기      아무도 안 쓴다        -> 지울 것
   퍼뜨리기   한 곳에서만 쓴다      -> 이을 것
...
```
📎 §4 로 항목을 뽑는 방법은 안 바뀝니다. 뽑은 «결과»가 가는 곳만 바뀝니다.
