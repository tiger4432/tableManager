...
>
> ⚰️ **[2026-08-16 정정 · 2026-08-18 절 축약 완료] `graph_sync_worker`·`run_graph_sync`·`ontology_config`·`graph_materializer`·`graph_orphans`·`graph_stale_edges`는 트리에 없습니다.** 그 상세 절 82줄은 **삭제하고 자리마다 🪦 묘비로 대체**했습니다 — 🔴 **여기 이 한 줄만으로는 부족했습니다: §5 링크로 바로 들어온 사람은 문서 맨 위를 지나오지 않으므로, 살아 있는 API처럼 읽히는 표를 그대로 만납니다.** 부정은 부정되는 것 «옆»에 있어야 합니다. `main.py /graph/*`에는 410 호환 라우트만 남고, 현재 경로는 `server/ledger/*`입니다.
>
...
>
> 🆕⑦ **이 패스가 새로 등재/정정한 것 — 전건**: [§5-H](#5-h-정본-원장-canonical-ledger) 읽기 측 대확장 — `ledger_trace_router.py`(80 → **419줄**, 라우트 1개 → **7개**: `/api/ledger/{trace,siblings,kinds,structure,lots,lot_map,coverage}`) · `ledger_trace.py`(1,179 → **1,937줄** — `STATE_CONTESTED`/`HOP_STATES`/`hop_basis` 구조화 basis, `LINEAGE_PREDICATES`의 PEP 562 전환, `coverage()`/`relation_exists()` 신설) · 신설 서버 모듈 **7종**(`finding_kinds.py`·`ledger_kinds.py`·`ledger_siblings.py`·`ledger_structure.py`·`ledger_lots.py`·`ledger_walk_contrast.py`·`mechanism_gate.py`) · `server/config/sample/mechanism_models.json.sample`(구조만) · 클라 콘솔(`ledger.html` 393 → **2,499** · `ledger_trace.js` 149 → **1,205** · core 286 → **603** · view 173 → **299** · 하니스 674 → **1,556**) · 놀라움 축 4모듈(`surprise_core.js`·`surprise_axis.js`·`surprise_map_core.js`·`surprise_map_view.js`) 실측 등재 · 쓰기 측 `server/ledger/` **11 → 13파일**(신설 `observation_translator.py`·`transfer_translator.py`, 줄 수 전건 재측정) · `vocabulary.PREDICATES` **7 → 11종** 정정 · 「읽기 측은 `server/ledger/`를 import하지 않는다」 정정(이제 **지연 import 한다**).
>
> ⚠️ **이 패스가 확인 못 했거나 범위 밖으로 남긴 것**: 콘솔의 나머지 클라 9모듈(`case_control_core/view`·`lot_reference_core/view`·`ontology_structure_core/view`·`contrast_core/view`·`surprise_view.js`·`ledger_console.css`)은 **줄 수와 최상위 export 존재만** 실측했고 심볼 표는 미등재다. 쓰기 측 `server/ledger/` 기존 11파일의 **심볼 표는 재측정하지 않았다**(줄 수만 갱신 — `vocabulary.py`의 walk 선언 계열과 `PREDICATES` 키 목록만 실측). 이 구간이 건드린 `server/main.py`(라우터 등록 두 줄만 확인)·`map_alignment.py`·`models.py`·`config_resolve_report.py`·`transfer_plan.py`·`retroactive.py`·`run_auto_update.py`·`client2/src/main.js`·`graph_viewer.js`와 씨앗 스크립트 `scripts/seed_syn_*` 8종·신규 테스트들은 **열지 않았다**. `server/config/sample/*.json.sample` 중 이 패스가 연 것은 `mechanism_models.json.sample` 하나다.

> 🆕⑨ **[2026-08-18 앵커·줄 수·미등재 패스의 측정 기준]** 이 패스가 잰 것은 **`5359fdd`(HEAD)의 커밋된 blob**이다(`git show 5359fdd:<path>` — 워킹트리 아님). 🔴 **워킹트리를 근거로 쓸 수 없는 이유가 측정 시각에 실재했다**: `server/ledger/`의 번역기 다섯(`declared_translator.py`·`lot_event_translator.py`·`observation_translator.py`·`transfer_translator.py`·`translator_pattern.py`)이 **워킹트리에서 삭제된 상태**였고 `server/config/ontology/`의 JSON 둘이 modified였다 — 다른 레인이 진행 중이다. 아래 값은 전부 커밋된 상태이고, **그 다섯 파일은 이 지도에서 아직 살아 있는 것으로 적혀 있다**(HEAD 기준으로 참이다. 그 레인이 착지하면 묘비가 필요하다).
>
> 🆕⑨ **이 패스의 범위는 같은 날 앞선 패스가 «구조적으로 볼 수 없었던» 것 넷이다**: ① 라인 앵커(앞 패스는 심볼의 **존재**만 쟀다) ② 제목의 줄 수 주장 ③ **맵에 절이 아예 없는 파일**(맵이 주장하지 않는 것은 맵을 훑어서는 보이지 않는다) ④ 없는 파일을 가리키는 blob 핀.
>
> 🆕⑨ 🔴 **가장 값비싼 발견은 심볼 존재 검사를 «통과한» 절이었다 — [§1.5](#15-그래프-조회-구간은-410으로-은퇴했다)다.** `get_chip_trace`·`get_graph_stats`·`search_graph_nodes` 등 일곱 핸들러(**접두어가 둘이다** — `/graph/`의 여섯 + `POST /api/graph/sync`)는 **이름이 전부 실재한다**. 그래서 파일 단위 감사도 심볼 단위 감사도 이 절을 초록으로 통과시켰다. 그런데 실측하면 **일곱 전부가 본체 한 줄짜리 `raise _graph_branch_retired()`**(410)다 — §1.5의 26줄짜리 캡·어휘·헬퍼 서술은 **한 줄도 남아 있지 않은 동작**을 기술하고 있었다(`_expand_graph_subgraph`·`_serialize_graph_edge`·`_chip_trace_leg`·`GRAPH_NEIGHBOR_NODE_CAP`·`GRAPH_CHIP_TRACE_SEED_LABEL` — grep **전부 0건**). **살아 있는 이름이 죽은 서술을 떠받치는 것**이 이 문서가 반복해 지는 실패의 최악형이다: 확인하러 온 사람이 이름을 grep해 찾고 통과시킨다.
>
> 🆕⑨ ⚠️ **이 패스가 열지 않은 것**: `crud.py`·`directory_watcher.py`·`map_alignment.py`·`map_overlay.py`·`transfer_plan.py`·`bonding_plan.py`·`enrichment_*`·`map_editor.js`·`admin.js`·`timeline.js`의 **심볼 표**는 열지 않았다(줄 수만 쟀다 — 아래 「줄 수 실측표」). 그 절들은 여전히 미검증이다. §5-H 쓰기 측의 심볼 표도 재측정하지 않았다(줄 수와 파일 목록만).

> 🆕⑩ **[2026-08-21 원장 쓰기 측·온톨로지 작성 표면 패스의 측정 기준]** 이 패스가 잰 것은 **`8d1e6c4c`(HEAD)의 커밋된 blob**이다(`git show 8d1e6c4c:<path>` — 워킹트리 아님). 범위는 `5359fdd..8d1e6c4c`(**449커밋**)이고 **지시받은 반경은 `server/ledger/` 패키지 + 온톨로지 config 작성/탐색 표면 하나**다. 🔬 **워킹트리 실측**: 이 패스의 대상 파일은 전부 blob과 동일했다(`git status --porcelain` 기준 dirty였던 것은 `client2/dist/*`·`client2/src/{dom,grid,style}`·`server/{dt_map_derivation,map_alignment,map_overlay}.py`·`server/config/sample/ledger_config.json.sample`뿐 — **전부 이 패스의 반경 밖**이다).
>
> 🆕⑩ 🔴 **삭제부터 훑었고, 이 구간의 삭제가 이 문서에서 가장 비싼 항목이었다.** `--diff-filter=D`의 **소스 8건**: `ledger/declared_translator.py` · `ledger/lot_event_translator.py` · `ledger/observation_translator.py` · `ledger/transfer_translator.py` · `ledger/translator_pattern.py` · `ledger/cutover_v2.py` · `ledger/examples/grouped_translator_template.py` · `mappers/ledger_lot_event_mapper.py`. **🆕⑨가 「그 레인이 착지하면 묘비 다섯이 필요하다」고 적어 둔 바로 그 레인이 착지했다** — 그리고 그중 `lot_event_translator.py`는 §5-H에 **심볼 10행짜리 표를 통째로** 달고 살아 있었다. 그 표를 지우고 묘비로 대체했다(아래 §5-H). config 삭제는 **11건**이고 «세는 방법에 따라 6으로 보인다» — 🔴 **기본 rename 탐지가 그중 다섯을 «삭제가 아니라 이동»으로 짝지어 목록에서 지우기 때문이다**(`server/config/ontology/{manifest,catalog/tables,catalog/virtual_joins,dataflows/chains,dataflows/enrichments}.json` → `task/evidence/ontology_root_before_20260818/…`, R100). **`--no-renames`로 다시 세어 11을 얻었다.** 내역: ① 라이브 root의 다중 파일 6개(위 다섯 + `ledger_config.json`)가 사라지고 **`server/config/ontology/`엔 `README.md`만 tracked로 남았다** — 라이브 `ledger_config.json`은 **gitignored**이고 디스크에는 있다 ② `config/sample/ontology/transfer_explorer/`의 다중 파일 5개가 **단일 `ledger_config.json`으로 접혔다**(`scripts/convert_ontology_to_single_file.py`). ⚠️ **이 지도는 어느 쪽 실값도 옮겨 적지 않는다** — 구조만이다.
>
> 🆕⑩ **이 패스가 새로 등재/정정한 것 — 전건**: §5-H 쓰기 측 표 전면 재작성(파일 목록 19→**33 + `examples/` 1**, 줄 수 전건 재측정, 묘비 7) · `lot_event_translator.py` 심볼 표 삭제→묘비 · `backfill.py` 행(1,574 → **983**, 드라이버 넷 소멸) + **고아 `fetch_*` 아홉의 실측 반박**(아래) · **[§5-H-bis 신설](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재)** — `ontology_config_explorer_router`(**15**라우트 — 전건 계수) · `OntologyExplorerService.test_run` · `config_authoring`(`Ground`/`Field`/4상태/`disposition` 5종) · `implementations.py` · `column_stats.py` · `ledger_skeleton.json` · `source_preparation.SOURCE_ROW_EXCLUDED_COLUMN`/`_assemble_prepared_frame` · 클라 `ontology_explorer{,_store,_view}.js` + 신설 3종(`ontology_path`·`ontology_skeleton`·`dom_patch`) · §5-I의 낡은 워킹트리 경고 철회.
>
...
>
> 🆕⑪ 🔒 **은퇴 울타리 — 아래는 «동결»이고 이 패스가 살아 있는 표면으로 갱신하지 않았다.** 지도에서 지우지 않되 **새 작업의 출발점으로 읽지 마라**: **A군** `ledger_trace.py` · ~~`ledger_trace_router.py`~~(🆕⑫ **파일 통째 동결은 철회됐다 — 라우트별로 갈렸다**, 바로 아래 🆕⑫ 블록) · `ledger_admin.py` · `ledger/config.py` — **C군** `ledger_explorer.py` · `ledger_structure.py` · `ledger_journey.py` · `ledger_lots.py`. **동결이 아닌 것**(계속 사는 읽기 측): `ledger_subgraph` · `ledger_catalog` · `ledger_composition` · `ledger_selection` · `ledger_siblings` · `ledger_trends`.
>
> 🆕⑫ **[2026-08-23 후속 — `ledger_trace_router.py` 울타리 재절단의 측정 기준]** 이 패스가 잰 것은 **`bd0c192e`(HEAD)의 커밋된 blob**이다 — `git show bd0c192e:server/ledger_trace_router.py`(blob `ca088aff`), 워킹트리 아님. 🔴 **반경은 그 한 파일의 라우트 16개와 그것이 부르는 심볼뿐이고, 이 문서의 다른 절은 한 곳도 열지 않았다.** 라우트 계수는 표본이 아니라 전건이다(`grep -c '^@router\.'` = **16**, `git log`상 이 파일을 마지막으로 바꾼 것은 `5aa666d5`).
>
> 🔬 🆕⑫ **측정 중 HEAD가 `bd0c192e` → `093837f2`로 움직였고, 「소스가 안 바뀌었다」를 가정하지 않고 확인했다.** 들어온 일곱은 전부 `task/*`만 건드렸고, 그래도 이 패스가 읽은 **14파일 전건을 `git rev-parse <rev>:<path>`로 두 리비전에서 대조해 바이트 동일을 확인했다**(불일치 **0** — 라우터·`ledger_trace`·동결 넷·열림 여덟). 그래서 아래 판정은 `bd0c192e` 기준이자 `093837f2` 기준이다.
>
> 🆕⑫ 🔴 **파일 통째 동결이 틀렸고, 지도는 그 판정을 내린 HEAD에서 이미 자기모순이었다.** 🆕⑪이 `ledger_trace_router.py`를 A군에 **통째로** 올렸는데, 같은 문단이 `ledger_subgraph`를 「동결이 아니고 이 구간에 가장 크게 자란 읽기 측 모듈」이라 적었고 **그 모듈의 유일한 HTTP 표면인 `@router.get("/subgraph")`가 동결 표지가 붙은 그 파일 안에 있다.** 파일 하나를 얼리면 **살아 있는 읽기 측 여섯 전부의 문이 잠긴다** — 그래서 울타리를 라우트별로 다시 그었다(**8 동결 / 8 열림**, 16행 전건은 §5-H `ledger_trace_router.py` 절).
>
> 🆕⑫ 🔴 **가르는 규칙은 «라우트가 부르는 심볼»이지 «임포트하는 모듈»이 아니다.** 동결 모듈이 얹고 있는 **공용 배관은 아무것도 얼리지 않는다** — `ledger_trace.relation_exists` · **`ledger_trace._fetch`** · `ledger_trace.ResolverConfigError` · `ledger_trace.REASON_RELATION_ABSENT` · `ledger_trace.rollup_subject_types` · `ledger_trace.SqlClaimLookup` · `ledger_explorer`의 엔티티 id 코덱(`entity_id`/`decode_entity_id`/`_entity`). 🔴 **`_fetch`가 배관이라는 것은 취향이 아니라 실측이 강제한다: 열린 여덟 모듈이 «8/8 전부» `from ledger_trace import _fetch, relation_exists`로 시작한다**(`ledger_subgraph`·`ledger_siblings`·`ledger_trends`·`ledger_composition`·`ledger_selection`·`ledger_catalog`·`ledger_kinds`·`ledger_walk_contrast` — `bd0c192e` 실측). 그것을 동결 계산으로 세면 **16라우트 전부가 동결**이 되어 이 절단 자체가 공허해진다. **얼리는 것은 계산 심볼이다** — `ledger_trace.trace`/`coverage` · `ledger_explorer.explore`/`explore_entity` · `ledger_journey.journey` · `ledger_structure.structure` · `ledger_lots.lots`/`lot_map`.

| 절 / 파일 | 상태 | 기준 리비전 | 비고 |
|---|---|---|---|
| 🆕⑫ §5-H `ledger_trace_router.py` — **파일 통째 동결 → 라우트별 울타리** | 🟢 **라우트 16개 전건 + 각 라우트가 부르는 심볼 실측(2026-08-23)** | **`bd0c192e`**(HEAD, blob `ca088aff`) | 779 → **806**(`5aa666d5`). **8 동결 / 8 열림.** 🔴 **동결 표지가 `/subgraph`의 유일한 문 위에 얹혀 있었다** — 같은 문단이 `ledger_subgraph`를 「동결 아님」이라 적으면서. 가르는 규칙은 **호출 심볼**이고 **모듈이 아니다**(`_fetch`·`relation_exists`는 열린 여덟 모듈 **8/8**이 쓰는 배관이라 얼리면 16라우트 전부가 동결이 된다). ⚠️ **이 패스는 라우트 표만 열었다 — 이 절의 나머지 산문·`ledger_trace.py` 심볼 표는 재검증하지 않았다** |
...
| 🆕⑩ 원장 **읽기** 측(§5-H 읽기 절 · §5-I 읽기 표) | 🔴 **이 패스가 한 파일도 열지 않았다** | ~~`8d1e6c4c`~~ → **`5359fdd`/`a729a7f`** | `ledger_trace`·`ledger_trace_router`·`ledger_explorer`·`ledger_catalog`·`ledger_subgraph`·`ledger_journey`·`ledger_trends`·`ledger_composition`·`ledger_selection`·`ledger_identity`·`ledger_walk_contrast`·`mechanism_gate`. **449커밋 구간이 지났으므로 밀렸다고 가정하라** |
...
| `server/ledger_walk_contrast.py` | 1,327 | **1,337** | +10 |
| `server/mechanism_gate.py` | 368 | **370** | +2 |
...
| 🔻 `client2/ledger.html`(§5-H) | 2,499 | **2,151** | **−348** |
...
| 🆕 **[신설] 2026-08-04(2차)** — **`server/notation_norm.py`**(표기 정규화 파생 컬럼) · ~~`server/graph_stale_edges.py`~~ | **542** (🪦 **[2026-08-16] `graph_stale_edges.py` 549줄과 CLI `graph_stale_edge_sweep.py` 193줄이 삭제돼 이 행은 이제 «1종»입니다.** `rederive_notation_norm.py`는 그보다 앞서 `8d306a5`에서 삭제됐습니다) | [§5-E](#5-e-2026-08-042차-신설-서버-모듈-2종--표기-정규화--낡은-엣지-스윕) |
...
**후계는 `server/ledger/*`와 [§5-H](#5-h-정본-원장-canonical-ledger)의 원장 콘솔이다.** 계보 추적은 `/api/ledger/trace`, 구조는 `/api/ledger/structure`([§5-I](#5-i-미등재-모듈-레지스트리-2026-08-18-실측)의 읽기 측 신설 모듈도 참조). 저장소 모델·워커·config의 은퇴는 [§5 🪦 블록](#-serverontology_configpy--graph_materializerpy--graph_sync_workerpy--graph_orphanspy--파일이-없습니다).
...
### 🪦 `server/ontology_config.py` · `graph_materializer.py` · `graph_sync_worker.py` · `graph_orphans.py` — 파일이 없습니다

**[2026-08-16 은퇴 · 2026-08-18 절 축약]** 네 파일 모두 트리에 없습니다. 종전 이 자리에는 **살아 있는 API처럼 읽히는 상세 표 57줄**이 있었고, 문서 맨 위(§0)의 「역사 기록입니다」 한 줄로만 부정돼 있었습니다 — 🔴 **§5 링크로 바로 들어온 사람은 그 한 줄을 지나오지 않습니다.** 부정은 부정되는 것 옆에 있어야 합니다.

같은 라운드에 사라진 것: `server/run_graph_sync.py` · `server/scripts/graph_orphan_sweep.py` · `server/config/sample/ontology_mapping.json.sample`. 부재는 `server/tests/test_graph_branch_retired.py::test_retired_graph_execution_files_stay_removed`가 지킵니다.

🔴 **[2026-08-18] 저장소 «모델»도 함께 제거**(`GraphNode`/`GraphEdge`/`GraphSyncState`/`ensure_graph_tables`). 🆕⑨ **날짜는 셋으로 갈린다 — 표 DROP 마이그레이션 `2ec78b9`(2026-08-14) · 워커·실행 파일 은퇴 `9851374`(2026-08-16) · 모델 클래스 제거 2026-08-18.** 이 절의 「[2026-08-16 은퇴]」는 **파일 삭제**를 가리키는 것이라 옳다. 표만 DROP하고 클래스를 남긴 중간 상태가 **재기동마다 「SCHEMA DRIFT: missing 3 thing(s)」를 찍었습니다** — 부팅 점검은 `Base.metadata`를 「이 빌드가 요구하는 표」로 읽으므로, 없어야 할 표를 결손으로 신고한 것입니다. 은퇴가 «고장»으로 보였습니다.

현재 경로는 `server/ledger/*`이고, 옛 주소 **일곱**은 `main.py`에서 **410 + `Cache-Control: no-store`**로 답합니다. 🆕⑨ 🔴 **`/graph/`로 grep하면 «여섯»만 나오고, 빠지는 하나가 하필 쓰기 진입점입니다** — 접두어가 둘이기 때문입니다: `/graph/`의 여섯(`stats`·`neighbors`·`nodes/search`·`trace`·`chip-trace`·`mapping-summary`) + **`POST /api/graph/sync`** 하나(지우면 SPA catch-all이 HTML 200을 돌려줘 거절이 「알 수 없는 오류」가 됩니다).
...
| 🪦 ~~**`server/graph_orphans.py`**~~ | — | ⚰️ **[2026-08-16 삭제 · 2026-08-18 실측 확인] 파일이 트리에 없다**(`git show HEAD:server/graph_orphans.py` → 실패). 🔴 **종전 이 칸에는 `sweep_enabled`~`run_scheduled` 10심볼 + 상수 6종의 앵커와 「프로덕션 유일의 `graph_nodes` DELETE 경로」라는 살아 있는 서술이 있었다** — 같은 날 앞선 패스가 이 파일의 «상세 절»은 지웠지만 **표 안의 이 행은 남았다.** 부정이 절 단위로만 이루어지면 표 행이 살아남는다. CLI `server/scripts/graph_orphan_sweep.py`도 함께 삭제됐다. 부재는 `server/tests/test_graph_branch_retired.py`가 지킨다. 저장소 표·모델의 은퇴는 [§5 🪦 블록](#-serverontology_configpy--graph_materializerpy--graph_sync_workerpy--graph_orphanspy--파일이-없습니다), 라우트 은퇴는 [§1.5](#15-그래프-조회-구간은-410으로-은퇴했다) |
...
### 🪦 `server/graph_stale_edges.py` — 파일이 없습니다

**[2026-08-16 은퇴 · 2026-08-18 절 축약]** 이 파일과 CLI `server/scripts/graph_stale_edge_sweep.py`가 함께 제거됐습니다. 종전 이 자리의 상세 표 25줄은 지웠습니다 — 위 🪦 절과 같은 이유입니다(부정은 부정되는 것 옆에).

⚠️ **다만 이 절이 적어 둔 «규칙»은 그래프와 무관하게 살아 있습니다**: 삭제만 보고하는 스윕은 「전부 거절됐다」를 「할 일이 없었다」와 똑같이 읽히게 만든다 — 드라이런이 기본이고, 거절과 무작업은 다른 종료코드여야 한다. 이 규칙의 현재 거처는 [PRIMITIVES](./PRIMITIVES.md)입니다.
...
### `server/ledger_trace_router.py` — 🔒 **울타리는 «라우트별»이다 (8 동결 / 8 열림)**

> 🆕⑫ **크기: 806줄 @`bd0c192e`**(blob `ca088aff` — 구 표기 779@`5359fdd` / 419@`a729a7f`. 자란 것은 `5aa666d5`). **제목에 줄 수를 얹지 않는다** — 제목은 이름이지 측정값이 아니고, 파일이 자랄 때마다 앵커가 깨진다.
>
> 🔒 🆕⑫ 🔴 **[2026-08-23] 이 절에 있던 「[동결 — 위 A군]」은 «파일 통째»였고 그것이 틀렸다.** 이 파일은 살아 있는 읽기 측 여섯의 **유일한 HTTP 표면**이라, 파일을 얼리면 그 여섯의 문이 전부 잠긴다. 🔴 **지도는 그 판정을 내린 리비전에서 이미 자기모순이었다** — 같은 문단이 `ledger_subgraph`를 「동결이 아니고 이 구간에 가장 크게 자란 읽기 측 모듈」이라 적었는데, `@router.get("/subgraph")`가 동결 표지 밑에 있었다.
>
> 🆕⑫ 🔴 **가르는 규칙 — 라우트가 «부르는 심볼»이지 «임포트하는 모듈»이 아니다.** 라우터는 15모듈을 import하지만 import는 아무것도 판정하지 않는다.
> - **얼리지 않는 공용 배관**: `ledger_trace.relation_exists` · **`ledger_trace._fetch`** · `ledger_trace.ResolverConfigError` · `ledger_trace.REASON_RELATION_ABSENT` · `ledger_trace.rollup_subject_types` · `ledger_trace.SqlClaimLookup` · `ledger_explorer`의 엔티티 id 코덱(`entity_id`/`decode_entity_id`/`_entity`).
> - **얼리는 계산 심볼**: `ledger_trace.trace` · `ledger_trace.coverage` · `ledger_explorer.explore`/`explore_entity` · `ledger_journey.journey` · `ledger_structure.structure` · `ledger_lots.lots`/`lot_map`.
> - 🔬 **`_fetch`가 배관인 것은 실측이 강제한다**: 열린 여덟 모듈이 **8/8 전부** `from ledger_trace import _fetch, relation_exists`로 시작한다. 동결 계산으로 세면 **16라우트 전부가 동결**이 되어 절단이 공허해진다. 라우터 자신의 `_subgraph_contract_state`도 `ledger_trace._fetch`를 직접 부르며, 그것이 `/subgraph`를 얼리지 «않는» 이유가 이것이다.
> - 🔬 같은 이유로 `ledger_subgraph`·`ledger_catalog`가 `ledger_explorer.entity_id`/`decode_entity_id`/`_entity`를 쓰는 것도 얼리지 않는다 — **id 코덱은 공용 철자이지 걷기 계산이 아니다.** `ledger_walk_contrast`의 `ledger_lots` 언급 2건은 **주석·docstring뿐이고 호출이 아니다**(`bd0c192e` 실측).

**🆕⑫ 라우트 16개 전건 — 동결/열림 (`bd0c192e` 실측, `grep -c '^@router\.'` = 16)**

| 라우트 | 핸들러 | 부르는 계산 | 판정 |
|---|---|---|---|
| `GET /trace` | `trace_lineage` | `ledger_trace.trace` (+`_lookup_for`→`SqlClaimLookup`) | 🔒 **동결** |
| `GET /explore` | `explore_lineage` | `ledger_explorer.explore` | 🔒 **동결** |
| `GET /explore_entity` | `explore_registered_entity` | `ledger_explorer.decode_entity_id` → `explore` \| `explore_entity` | 🔒 **동결** |
| `GET /coverage` | `ledger_coverage` | `ledger_trace.coverage` | 🔒 **동결** |
| `GET /journey` | `ledger_journey_route` | `ledger_journey.journey` | 🔒 **동결** |
| `GET /structure` | `ledger_structure_view` | `ledger_structure.structure` | 🔒 **동결** |
| `GET /lots` | `ledger_lot_grid` | `ledger_lots.lots` | 🔒 **동결** |
| `GET /lot_map` | `ledger_lot_map` | `ledger_lots.lot_map` | 🔒 **동결** |
| `GET /subgraph` | `evidence_subgraph` | `_signed_start` → `_evidence_graph` → `ledger_subgraph.subgraph`/`SqlEvidenceLookup` (+`tabular_projection`, `enrichment_actions.SqlEnrichmentActionLookup`) | 🟢 **열림** — `ledger_trace` 접촉은 `relation_exists`·`_fetch`(배관)뿐 |
| `GET /subgraph/table` | `evidence_subgraph_table` | 위와 같은 `_evidence_graph` + `ledger_subgraph.tabular_projection` + CSV 직렬화 | 🟢 **열림** — 동상 |
| `GET /entities` | `registered_entity_catalog` | `ledger_catalog.entity_catalog` | 🟢 **열림** — 배관 + `ledger_explorer` id 코덱뿐 |
| `GET /siblings` | `ledger_siblings_route` | `ledger_siblings.siblings` \| `scope` 있으면 `ledger_walk_contrast.contrast` | 🟢 **열림** — 엔진 둘 다 |
| `GET /trends` | `ledger_trends_route` | `ledger_trends.trends` | 🟢 **열림** |
| `GET /composition` | `ledger_composition_route` | `ledger_composition.composition` | 🟢 **열림** |
| `POST /selection/resolve` | `ledger_selection_resolve_route` | `ledger_selection.resolve` | 🟢 **열림** |
| `GET /kinds` | `ledger_kind_catalog` | `ledger_kinds.catalog` | 🟢 **열림** — 동결 의존 **0** |

> 🆕⑫ ⚠️ **이 패스가 «잰» 것은 위 표뿐이다** — 라우트 이름·핸들러 이름·부르는 계산 심볼. 🔴 **아래 심볼 표의 산문 서술(거절 어휘·응답 모양·상태 어휘)은 열지 않았고 여전히 `a729a7f` 기준이다.** 그리고 위 표의 16개 중 **아홉**(`/explore`·`/entities`·`/explore_entity`·`/subgraph`·`/subgraph/table`·`/journey`·`/trends`·`/composition`·`POST /selection/resolve`)은 **아래 심볼 표에 자기 행이 없다** — 소비 모듈은 [§5-I](#5-i-미등재-모듈-레지스트리-2026-08-18-실측)에 있고, 이 패스는 그것을 메우지 않았다. 🔴 **동결 여부와 등재 여부는 다른 축이다**: 그 아홉 중 셋(`/explore`·`/explore_entity`·`/journey`)은 **동결이면서 미등재**다.
>
> 🆕⑨ 🔴 **[2026-08-18 실측] 라우트는 7개가 아니라 «16개»다.** `@router.get`/`@router.post` 전건 계수(`5359fdd`, 🆕⑫가 `bd0c192e`에서 **16 재확인**): 아래 표가 서술하는 일곱(`/trace`·`/siblings`·`/kinds`·`/structure`·`/lots`·`/lot_map`·`/coverage`) **외에 아홉이 더 있고 이 지도에 등재된 적이 없다** — `/explore` · `/entities` · `/explore_entity` · `/subgraph` · `/subgraph/table` · `/journey` · `/trends` · `/composition` · **POST `/selection/resolve`**. 🔴 **그 아홉의 소비 모듈이 [§5-I](#5-i-미등재-모듈-레지스트리-2026-08-18-실측)에 올린 `ledger_explorer`·`ledger_catalog`·`ledger_subgraph`·`ledger_journey`·`ledger_trends`·`ledger_composition`·`ledger_selection`이다.** ⚠️ **이 패스는 그 아홉의 시그니처·거절 어휘를 열지 않았다** — 존재와 경로만 실측했다. 아래 일곱 행은 `a729a7f` 기준 서술이고 **재검증하지 않았다.**

| 심볼 | 무엇인가 |
|---|---|
| `router = APIRouter(prefix="/api/ledger", tags=["ledger"])` | 자족적 라우터 — `main.py` 등록이 두 줄. 🆕⑫ **정정 — import하는 모듈은 7종이 아니라 «15종»이다**(`bd0c192e` 실측, `^import ` 전건 계수 18 − 표준 3): `finding_kinds`·`enrichment_actions`·`ledger_catalog`·`ledger_composition`·`ledger_explorer`·`ledger_journey`·`ledger_kinds`·`ledger_lots`·`ledger_selection`·`ledger_siblings`·`ledger_structure`·`ledger_subgraph`·`ledger_trace`·`ledger_trends`·`ledger_walk_contrast`. 🔴 **그 목록은 울타리를 판정하지 않는다** — 판정은 라우트가 «부르는 심볼»이 한다(위 🆕⑫ 표) |
...
### `server/enrichment_actions.py` + Evidence Action 결합 (2026-08-15 가산)

> 이 행은 현재 워킹트리의 심볼 계약만 등재한다. 줄 수·옛 라우트 개수는 다시 세지 않았으므로
> 위 `a729a7f` 측정값을 현행 수치로 인용하지 않는다.

| 심볼/호출 | 무엇인가 |
|---|---|
| `EnrichAction` · `enrich_action_node_id` · `decode_enrich_action_id` · `action_node` | 원장 Entity/Claim이 아닌 재계산 Action projection과 canonical opaque ID. grain은 decision-key `claim_resolution`과 rule-level `source_contract` 둘 |
| `EnrichmentActionLookup` · `InMemoryEnrichmentActionLookup` | Claim anchor 정확 매칭, target 결손, 공급 source 부재를 순수 평가. 같은 rule-level Meta Action은 decision key마다 복제하지 않음 |
| `SqlEnrichmentActionLookup` | 요청 범위 decision key의 materialized derived row만 `business_key_val IN (…)`로 읽는다. reference-view candidate SQL은 실행하지 않는다. table 검증 실패 rule은 row read 없이 `repair_enrichment_contract`로 강등 |
| `ledger_subgraph.subgraph(..., action_lookup=…)` | 새로 읽은 Claim에서 `needs_enrichment` 엣지로 Action에 도달한다. Action은 자동 BFS 말단이고 opaque ID 직접 seed는 유효 |
| `ledger_trace_router`의 `/subgraph`, `/subgraph/table` | `enrich_actions=true` 기본으로 request-scoped SQL lookup을 연결. false면 순수 원장 증거만 반환 |
| `enrichment_config._normalize_claim_contract` | 기존 rule에 optional Claim 계약을 가산 검증. 잘못된 계약은 rejection을 남기되 legacy rule 자체는 유지 |

계약 정본은 [CLAIM_REQUIREMENT_WORKLIST_SPEC](../spec/CLAIM_REQUIREMENT_WORKLIST_SPEC.md)와
[LEDGER_EVIDENCE_SUBGRAPH_SPEC](../spec/LEDGER_EVIDENCE_SUBGRAPH_SPEC.md), 테스트는
`server/tests/test_enrichment_actions.py`다.
...
### 🆕⑦ `server/mechanism_gate.py` (**370줄**(🆕⑨ `5359fdd` 실측 — 구 표기 368)) + `server/config/sample/mechanism_models.json.sample`
...
🔴 **`server/_qa1_head_probe.py`(5,993줄)는 등재하지 않는다 — 트리에 «있어서는 안 될» 파일로 보이기 때문이다.** 실측: docstring이 `map_alignment.py`의 것과 글자 그대로 같고(「맵 정렬 채점 — 후보 8개를 한 번에 채점해…」), 내용은 `map_alignment.py`(**6,965줄**)의 **잘린 사본**이다. 이름의 `_qa1_`·`head_probe`가 말하듯 QA 레인의 진단 산출물이 `f03b165`(`chore: consolidate pending ledger and map artifacts`)에 딸려 들어간 것으로 보인다. **이 지도는 파일을 지우지 않으므로 처분을 판정하지 않는다** — 서버 도메인 소관으로 남긴다. ⚠️ **실질 피해는 grep이다**: `map_alignment.py`의 심볼을 `server/`에서 찾으면 **두 파일에서 나온다**.
...
🔴 **vite 엔트리 둘(`rnd_console` → `rnd-console.html` · `ledger_graph` → `ledger-graph.html`)이 이 지도에 «화면도 모듈도» 없었다.** [§7](#7-client2src--웹-클라이언트) 도입부의 엔트리 개수도 함께 정정했다(7 → **9**).

| 파일 | 줄 | `^export` | 무엇 |
|---|---|---|---|
| `client2/rnd-console.html` | **50** | — | R&D 콘솔 페이지 (vite 엔트리 `rnd_console`) |
| `client2/ledger-graph.html` | **119** | — | 원장 그래프 페이지 (vite 엔트리 `ledger_graph`) |
| `client2/src/rnd_console/investigation_workspace.js` | **1,227** | 13 | 조사 워크스페이스 |
| `client2/src/rnd_console/trend_workbench.js` | **711** | 5 | 추이 워크벤치 (서버 짝 `ledger_trends.py`) |
| `client2/src/rnd_console/composite_lineage_adapter.js` | **198** | 2 | 복합 계보 어댑터 (서버 짝 `ledger_composition.py`) |
| `client2/src/ledger_graph/graph_core.js` | **215** | 6 | 그래프 순수 코어 (서버 짝 `ledger_subgraph.py`) |
| `client2/src/ledger_graph/entity_catalog.js` | **112** | 3 | 엔티티 카탈로그 (서버 짝 `ledger_catalog.py`) |
| `client2/src/journey_core.js` | **731** | **22** | 여정 대조 순수 코어 (서버 짝 `ledger_journey.py`) |
| `client2/src/journey_view.js` | **543** | 1 | 그 뷰 |
...
| 🪦 ~~`server/graph_sync_worker.py` · `graph_materializer.py` · `ontology_config.py`~~ | ⚰️ **[2026-08-16 삭제 · 2026-08-18 실측 확인] 세 파일 모두 트리에 없다.** 🆕⑨ 종전 이 행은 「온톨로지 그래프 트랙 — 함수 앵카는 §5」로 살아 있는 모듈처럼 읽혔고, 그 §5는 이미 묘비다 — 즉 **이 행은 묘비로 가는 링크를 «살아 있는 앵카»로 제시하고 있었다.** 현행 경로는 [§5-H](#5-h-정본-원장-canonical-ledger)와 [§5-I](#5-i-미등재-모듈-레지스트리-2026-08-18-실측) |
...
> 🆕⑨ 🔴 **[2026-08-18 재측정] 엔트리는 7이 아니라 «9»다.** 실측(`5359fdd`의 `build.rollupOptions.input`) — `main`(index.html) · `admin` · `map_editor` · `map_editor2` · `graph` · `trace` · `ledger` · 🆕 **`rnd_console`**(rnd-console.html) · 🆕 **`ledger_graph`**(ledger-graph.html). 🔴 **뒤의 둘은 이 지도에 «화면도 모듈도» 등재된 적이 없다** — 소비 모듈은 [§5-I](#5-i-미등재-모듈-레지스트리-2026-08-18-실측)에 존재·크기만 올렸다. ⚠️ **`graph`와 `trace` 엔트리는 살아 있으나 그 서버 라우트는 410으로 은퇴했다**([§1.5](#15-그래프-조회-구간은-410으로-은퇴했다)) — **이 패스는 두 화면이 의도적 잔존인지 잔재인지 판정하지 않았다**(클라 도메인 소관).
