...
### 4-ter-b. `server/chain/rule_shape.py` (🆕 **384줄** @`b1db471a` — 374 @`dc877746`, 325 @`474f1aa9`, 구 표기 271 · `b7b5010f` 신설 때 109, S-234 2단계)

| | |
|---|---|
| 무엇 | 세 문법을 담는 «내부 규칙» 하나 + 어댑터 둘(`from_chain_rule` :36/`as_chain_rule` :58 · `from_join_rule` :109/`as_join_rule` :128) + 새 문법의 입출력(`to_declaration`/`from_declaration` :147·:167)(전부 @`b1db471a` — 구 표기 :104/:123/:142/:162) |
| 🆕 **`key` 칸이 «번역을 살아남는다»** (`8cab58da` S-240) | `from_declaration` :167 이 최상위 `key` 를 `"key": dict(raw.get("key") or {})` :188 로 나르고, `to_declaration` :147 이 `("on", "derive", "into", "key", "limits")` 순으로 되돌리며, `as_chain_rule` :58 이 join 종류일 때 `out["key"] = dict(internal["key"])` :99 로 «규칙 옆»에 싣는다 — `params` «안»이 아니다(맵퍼 인자가 아니라 «껍데기가 적재 시점에 읽는 칸»이고 `join_into` 는 그것을 «못 본다»). 🔴 종전엔 이 칸이 «바닥에 떨어졌다» — 아무도 나르지 않아 아무도 읽을 수 없었고, 그래서 §4-ter-c 의 옛 「읽는 자가 없다」줄이 참이었다. 읽는 자는 §5-C `builtins.declared_unique_targets` :181 |
| 🆕 **join 종류의 동반자** (S-237, 이 범위 «전»부터 있었으나 미등재) | `companion_rules(internal)` :201 — 조인 선언 «하나»가 규칙 «둘»(왼쪽 트리거 + `:reference` 접미의 오른쪽 트리거, `REFERENCE_SUFFIX` :198)이 되는 자리 · `class JoinTriggerConflict(ValueError)` :234 · `join_trigger_columns` :241 · `refuse_join_trigger_conflict` :250 · `unknown_join_cells` :270(:280 이 `join_into.unknown_cells` 를 부른다)(전부 @`b1db471a` — 구 표기 :191/:188/:224/:231/:240/:260/:270) |
| 🆕 **`decide` 종류** (`dc29056a` S-239 · 판정 399~401) | `DECIDE_CELLS` :286(@`b1db471a` — 이 행의 앵커 구 표기 :276/:280/:283/:307/:316) = `key · fields · list_columns · aggregations · reference_views · auto_confirm · alignment`(🔴 `auto_confirm_declared` 는 칸이 «아니다» — 판정 401: 「적었나」는 키의 «존재»에서 유도된다, `enabled_written` 과 같은 부류) · `_DECIDE_TO_ENRICHMENT` :290(`key→decision_key` · `fields→target_fields` — 새 문법 칸을 인리치 어휘 철자로) · **`decide_rules(internal, known_tables=None) -> (rules, refusal)`** :293 — 🔴 **번역기가 아니라 «문»이다**: 칸 이름만 바꿔 `enrichment.config.chain_rules_from_cells` 로 «내려보낸다». 확장기는 그쪽 «하나»고 이 파일은 둘째 확장기를 «만들지 않는다» · `unknown_decide_cells(internal)` :317(이름 대고 «거절 안 함») · **`is_switched_off(internal) -> bool`** :326 = `enabled_written and enabled is False` — 🔴 **「적힌 `false`」만 OFF 다**. 없으면 ON, truthy 면 ON(옛 껍데기가 읽던 그대로). 워커는 이것을 «맨 먼저» 물어 참이면 «아무것도» 안 한다(§4 `load_chain_rules` 행 — 🆕 이제 아래 `expand_declaration` «안»에서) |
| 🆕 **`expand_declaration(declaration, table_config=None) -> (rules, refusal, notes)`** :338(@`b1db471a` — 구 표기 :328) (`00da7e91` S-244) | 🔴 **저자 «하나», 부르는 자 «둘»** — 규칙 파일 항목 하나 → 「그것이 세우는 체인 규칙들」. 로더(`ingestion_worker.load_chain_rules` :740)와 저장 관문(`ledger/admin.save_chain_rule_raw` :691·:720)이 «같은 함수»를 «같은 카탈로그»로 부른다 — 종전엔 로더는 «번역한 뒤» 심판하고 관문은 «날것»을 심판해, `POST /admin/chain/rules/raw` 가 로더가 «받는» 선언을 세 문장(`trigger_table` 없음 · `derive` 모름 · `unresolvable_mapper`)으로 거절했다(S-204 「저장 관문과 로더가 같은 판정자」가 «입력»에서 조용히 거짓이 됐던 것 — 같은 판정자, 다른 입력은 판정자 둘과 같은 결함). 결과 «셋»: ① `derive` 가 dict 가 아니면 `([declaration], None, [])` — **옛 평면 규칙은 «손 안 대고» 돌아온다**(옛 저장 경로가 바이트 그대로인 이유) ② `is_switched_off` → `([], None, [note])` — 🔴 **OFF 는 거절이 «아니다»**(판정 399) ③ `decide_rules` 거절 / `JoinTriggerConflict` → `([], "<name>: …", [])` · 아니면 `decided` 또는 `[as_chain_rule] + companion_rules` 와 «모르는 칸» 노트. 시험 `test_the_save_gate_and_the_loader_read_one_declaration_alike.py`(230줄 / `def test_` 9) |
...
### 🆕 4-ter-g. `server/chain/rule_order.py` (🆕 **139줄** @`dc877746` — 95 @`474f1aa9`, `c7bc59db` 신설 2026-09-13, S-156 · 판정 385·387·388) — ⚠️ 통합 라운드의 «여섯»에 들지 않는다. 미등재라 이 옆에 둔다

| | |
|---|---|
| 무엇 | 체인 규칙의 «순서» — 선언에서 «유도»된다, 새 칸이 아니다. ⚰️ ~~`class RuleCycleRefused(ValueError)`~~(`2504fbde` 에서 «사라졌다» — §0 ㉘) · **`order_rules(rules, on_cycle=None) -> list`** :55 — `trigger_table` ↔ 다른 규칙의 `target_table` 로 생산자→소비자 엣지를 긋고 후위 DFS 로 «생산자 먼저». 🔴 **재생이 늘 하던 유도인데 «읽는 자가 하나»였다** — 워커는 «파일 순서»로 돌았다(이 모양의 «넷째»: S-152·S-221·S-154) |
| 🆕 **고리는 «모양»이지 오류가 아니다** (`2504fbde` 판정 402) — `_SAID = set()` :23 · `cycle_note(trail, ceiling=None) -> str` :26 · **`say_cycle_once(logger_, trail, ceiling=None) -> bool`** :40 · `forget_cycles()` :50 | `dt_log → dt_inventory`(맵퍼) → `dt_log`(조인)은 «의도된» 고리이고 유한하게 만드는 것은 드레인이 이미 강제하는 `max_chain_depth` 다. 적재 때 그것을 거절하는 것은 «도는 선언»을 거절하는 것이었고, 드레인이 «배치마다» 파일을 다시 읽으니 그 줄이 초당 두 번 흘렀다(운영자 「사이클 오류 계속 뜨네」). 이제 `order_rules` 의 걷기는 고리를 만나면 `on_cycle(path + [name])` 을 부르고 «내려가기를 멈추며», 🔴 **고리가 하나라도 있으면 `list(rules)` — «선언 순»을 «통째로» 돌려준다**(부분 걷기가 되감긴 순서는 선언 순도 유도 순도 아닌 «아무도 안 적은 셋째 순서»라). `say_cycle_once` 는 자취(tuple)당 «프로세스당 한 번» INFO — 문장은 S-247 의 운영자 줄 모양(`[ChainRules] 고리: a -> b -> a (순서는 선언 순 · 홉 상한 max_chain_depth=N 이 막습니다). 다음: 없음 …`). `forget_cycles` 호출자: 시험뿐. 🔵 `_validate_chain_cascade_graph` 도 «같은 입»을 쓴다(:1057) — 다른 엣지 집합, 같은 문장 |
...
| 누가 부르나 | `chain/ingestion_worker.py` :890(`load_chain_rules` 끝 — `on_cycle=lambda trail: say_cycle_once(logger, trail, max_chain_depth(_RULES_DOCUMENT))` :891, 체인을 «안 멈춘다») · `chain/replay.py` :188 `order_rules` 가 «감싸서» 같은 것을 부른다 — ⚰️ 종전의 `RuleCycleRefused → ReplayRefused` 재던지기는 «없다»(번역할 거절이 남지 않았다) → `scripts/chain_replay_cli.py` :185 · `scripts/outbox_triage.py` :155 가 그 래퍼를 지난다 |
| 시험 | `test_the_worker_runs_rules_in_the_order_the_declaration_implies.py`(`ea8f91d2` 가 둘을 뒤집었다 — 조인의 페이싱 참조 규칙은 더는 「먼저」로 단언되지 않는다: 트리거 그룹을 «공유하지 않으므로» 앞뒤가 «뜻이 없다») · 🆕 `2504fbde` 가 `assert not hasattr(rule_order, "RuleCycleRefused")` 를 단언한다 — 고리 픽스처는 «선언 순»을 받는다 |

### 🆕 4-ter-h. `server/operator_line.py` (**102줄**, `3aab7173` 신설 2026-09-15, S-247) — ⚠️ 통합 라운드의 «여섯»에 들지 않는다. 같은 날 신설이라 이 옆에 둔다

| | |
|---|---|
| 무엇 | 운영자가 읽는 «한 줄»의 정본 — 「무엇이 일어났나」에 「다음: 무엇을 하나」가 «반드시» 붙는다. **`line(where, subject, what, action, samples=()) -> str`** :86 = `"[where:subject] what (표본: …) → 다음: action"` — 「다음:」절이 «없는» 줄은 이 함수가 «만들 수 없다». 접두 대괄호가 «검색어»다(자리마다 모양이 다르면 운영자가 로그에서 못 찾는다) |
| 🔴 닫힌 행동 어휘 «다섯» — 함수다, 상수가 아니다 | `fold_the_data(table, columns)` :40(진짜 중복 → «데이터»를 합쳐라; 합쳐도 되는지는 접기 계획이 가른다) · `widen_the_key(where_declared, what_to_add)` :46(키가 신원보다 좁다 → «선언»을 넓혀라, 데이터는 «건드리지 마라») · `fill_or_declare_null(table, columns)` :52(비어 있다 → 채우거나 카탈로그 `null_policy`) · `restart_to_apply()` :58(제품이 «스스로» 세운다 — 손 DDL 없음) · `nothing_to_do()` :63(「없음」도 행동이다 — 그걸 모르면 운영자는 «무엇이든» 한다). 함수인 이유: 행동마다 «채울 이름»이 다르고, 문자열 상수면 부르는 자리가 포맷을 짜서 «저자가 넷»이 된다. 새 행동은 «여기» 함수 하나로 — 그러면 그 행동이 «무엇을 말해야 하는지»가 한 자리에서 정해진다 |
...
| 표본 | `MAX_SAMPLES = 3` :32 · `samples_of(items, cap=MAX_SAMPLES)` :77 — 셋까지 + 「외 N」(셋만 보이고 수를 안 말하면 「셋뿐」으로 읽힌다; 수천 건 나열은 진단이 아니라 덤프) · `_names(seq)` :69(리스트 repr 의 대괄호를 운영자에게 안 보낸다) |
...
| 누가 부르나 (`git grep … HEAD` 전건, 시험 제외 — 파일 «셋», 자리 «다섯») | `chain/join_into._write` :222(팬아웃 → `fold_the_data`, §4-ter-c) · `database/crud.refuse_virtual_join_duplicates` :4044(배치 안 중복 → `fold_the_data`) · :4108(저장된 행과 충돌 → `fold_the_data`) · `database/crud.apply_batch_updates` :4386(BK 충돌 «미해결» → **`widen_the_key`** — 위 둘의 «정반대» 수리이고 로그에선 같은 낱말로 보인다, §2) · `virtual_join/unique_key.describe` :115·:120·:128·:148(`restart_to_apply`·`fill_or_declare_null`·`fold_the_data`, §4-ter-d). ⚠️ **`nothing_to_do` 는 호출자 «0»**(자기 시험뿐) — `rule_order.say_cycle_once`(§4-ter-g)는 「다음: 없음」을 :35 에 «손으로» 적고 이 모듈을 «안 부른다»(같은 모양, 다른 손 — 「같은 기능에 두 경로」의 씨앗이고 여기 적어 둔다). `restart_to_apply` 호출자는 `unique_key.describe` 뿐 |
| 시험 | `test_every_refusal_line_says_what_to_do_next.py`(223줄 / `def test_` 12 — 네 자리의 줄이 «전부» `→ 다음:` 을 들고 «행동 어휘 안»의 문장인지) · `test_business_key_conflict_retry.py`(371줄 / 17) · `test_the_product_sets_up_its_own_unique_key.py`(200줄 / 16) |
