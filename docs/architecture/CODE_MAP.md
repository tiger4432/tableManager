...
| 🆕㉝ `seed_type` 축 :1666·:1701~ · `DEFAULT_SEED_LIMIT = 200` :284 · `limits.seeds`/`truncated.seeds` (:2269·:2318) | **씨앗 집합을 «저장하거나 열거»하지 않고 «타입으로 기술»한다**(S-148-a `3644b8dc`+`9b3b20a9`+`e29f7ab6`, 판정 337) — 대조군이 «요청 하나»에 들어간다. ⛔ 선언에 없는 타입은 `seed_type_not_declared` 로 «아는 집합과 함께» 거절한다(:1711). 🔴 **예산이 «이미 있는 쌍» 위에 붙었다** — `limits`/`truncated` 에 `seeds` 가 한 칸 늘었을 뿐이고, 별도 예산을 만들면 「잘렸나」의 답이 «둘»이 된다(판정 331 의 「예산은 하나」 그대로). ⚠️ **그리고 이 기능은 «세 번째 좌석»에서야 닫혔다** — ① 함수는 초록이었고 ② 라우트가 `id` 를 «필수»로 요구해 «핸들러에 닿기 전» 거절했고(`9b3b20a9`) ③ 질의가 SQLAlchemy 커넥션에 «DBAPI 커서»를 써서 박스에서 500 이었다(`e29f7ab6`). 🔴 **앞 두 겹은 «함수를 직접 부르는» 시험이 영원히 못 본다** — 그래서 처방이 「라우트를 태우는 시험」이다 |
