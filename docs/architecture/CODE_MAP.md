...
> 🆕⑨ 🔴 **[2026-08-18 실측] 이 파일은 `5359fdd`(blob `9cee25d`)에서 **6,057줄**이다.** 이 문서 안에 공존하던 두 값(§0 표의 **6,322** · 여기 있던 **6,128**)은 둘 다 낡았다 — **줄어든 것이 이 파일에서 처음이 아니고**, 이번 감소는 그래프 구간 은퇴([§1.5](#15-그래프-조회-구간은-410으로-은퇴했다))가 만든 것이다. ⚠️ **이 패스는 줄 수만 쟀다** — §1.1~§1.4의 심볼 표는 열지 않았다.
>
...
> 🆕⑭ **크기: 374줄 @`3a8494dc`** (369 @`2a4e51cf`)(🆕⑬ 324 @`3b77fbc3` — **+45**, 신설 `_static_types`/`_static_step_predicates` 둘이 그 대부분이다). **제목에 줄 수를 얹지 않는다** — 제목은 이름이지 측정값이 아니다.
>
