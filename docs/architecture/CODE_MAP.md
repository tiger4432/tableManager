...
| 🆕㉗ 동적 모델의 **`kind: view` 갈래** :1043~ (S-186 `71676c4c`·`2c6d7335`) | **뷰의 모델은 «선언된 컬럼만» 든다** — 레이어링 컬럼 넷(`row_id`·`updated_at` 등)을 붙이면 그 관계에 «없는 컬럼»을 SELECT 해 PostgreSQL 이 `UndefinedColumn` 으로 답하고 라우트가 500 이 된다. 🔴 **`total_order_key` 가 착지한 «뒤에도» 라우트가 깨진 이유가 이 자리다** — 정렬 좌석은 다 접혔는데 «모델»이 여전히 없는 컬럼 넷을 약속하고 있어서, 실패가 ORDER BY 에서 SELECT 로 «옮겨갔을 뿐»이다. 소스가 그렇게 적는다: 「`kind` 를 읽는 자리가 이 앞에 셋 있었고 그중 «여기»는 없었다」. 🔴 **매핑된 클래스에는 PK 가 필요하지만, «신원»은 다른 주장이다** — **출하 샘플**(`table_config.json.sample`, 2026-09-12 실측) `kind: view` «열하나» 중 — ⚠️ 소스 주석의 「ten」은 «라이브 카탈로그»(gitignore)의 수라 저장소 주장이 아니다; **넷**이 `business_key` 를 «아예 선언하지 않는다». 클래스를 «지을 수 있게» 컬럼 하나를 지명할 뿐이고, 그 넷은 읽기 경로에서 `total_order_key` 가 R7 로 422 를 낸다 — 「이 관계는 페이징할 수 없다」를 말하는 자리는 거기다. ⚠️ `row_id` 를 «선언한» 뷰는 그것이 키다(판정 296) — `business_key` 를 대신 주면 모델과 정렬이 신원에 대해 «서로 다른 말»을 한다(`dt_log_transferable` 이 그 실물이었다) |
