...
| 🆕㉝ `rnd_board/api.js` 의 `AGGREGATE` :1345 (export) | ⏳ **«다리»다 — C-90 ② 에서 은퇴 예정**(C-90 ① `705a884a`). 집계 표가 «닿을 수» 있게(=새 이음매를 «채점»이라도 할 수 있게) 내놓은 export 이고, 걷기가 자기 모집단을 접게 된 뒤에는 «클라가 접을 이유»가 없다(S-146/146-c). 🔴 **다리를 영구 건물로 읽지 말 것** — 상설 「덧붙이기는 다리다」와 같은 자세로 적는다 |
