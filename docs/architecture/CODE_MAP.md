...
6. `_required_entity_columns(source_plan)`의 컬럼이 **살아남은 행 전부**에 있고 비지 않았는가 → 아니면 `source_preparation_incomplete`(경로가 `event_frame.rows[N].<col>`)
