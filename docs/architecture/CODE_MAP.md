...
| 🔻 `server/retroactive.py` | 697 | **545** | **−152** |
...
🔴 **줄어든 일곱(🔻)이 늘어난 스물둘보다 위험하다.** 자란 파일은 「이 절에 없는 것이 있다」는 뜻이고 독자는 소스를 열어 찾는다. **줄어든 파일은 「이 절에 있는 것이 소스에 없다」는 뜻**이고, 독자는 지도에 적힌 이름을 그대로 믿는다 — 그것이 [§1.5](#15-그래프-조회-구간은-410으로-은퇴했다--그-일곱은-이제-없다)에서 실제로 일어난 일이다. **`product_tables.py`(−74) · `retroactive.py`(−152) · `ui.js`(−84) · `ledger.html`(−348)의 심볼 표는 이 패스가 열지 않았으므로 «죽은 이름이 있다고 가정하라».**
...
> git show HEAD:server/ledger_trace_router.py | grep -c "^@router\."
...
> git grep -nE '_(count|run)_graph_orphans|"graph_orphans": \{' -- server/retroactive.py
...
> #       enrichment_config.py 1 · run_decoupled_app.py 1.  **줄면 누가 고친 것이고 그건 볼 일이다.**
> #    ⚠️ 이 검사는 «지도»가 아니라 «소스»를 잰다 — 이 지도는 §1.5 에서 이미 정정했다.
> git grep -c "mapping-summary\|_graph_branch_retired" -- server/main.py server/database/models.py server/config_resolve_report.py server/enrichment_config.py run_decoupled_app.py
> ```
>
...
> **서버** — 🔴 **`main.py` = `454b649`** · 🆕 **`column_filter.py` = `7a9cbca`** · 🔴 **`database/crud.py` = `d637d91`** · `database/models.py` = `cd78e43` · `map_overlay.py` = `01bbe65` · 🔴 **`transfer_plan.py` = `faed7e0`** · 🔴 **`bonding_plan.py` = `9e7b326`** · 🔴 **`product_tables.py` = `f132d6c`** · `effort_metric.py` = `9e3ef4e` · 🪦 ~~`ontology_config.py` = `851d0f8`~~(**파일 없음**) · 🪦 ~~`graph_sync_worker.py` = `79f0a51`~~(**파일 없음**) · `value_suggest.py` = `a1d7a5e` · `map_preset_routing.py` = `ce5fee0` · 🔴 **`parsers/directory_watcher.py` = `0609cfa`** · `parsers/advanced_ingester.py` = `fa37bba` · 🆕🔴 **`parsers/html_topology_parser.py` = `61dc149`**
...
> **서버(소급·재생·표기·그래프)** — `retroactive.py` = `d8db501` · `enrichment_backfill.py` = `a2039a3` · `chain_replay.py` = `c824ab7` · 🪦 ~~`graph_orphans.py` = `30517d6`~~(**파일 없음**) · 🆕 **`notation_norm.py` = `358ae68`** · 🪦 ~~`graph_stale_edges.py` = `cdf0108`~~(**파일 없음**)
...
| 🆕⑮ **`server/system_reload.py`** (config 핫리로드 — `main.py`에서 이사. **같은 규칙의 세 번째 사례**) | **151** (신설 @`5a73021a`) | [§1.9](#19-serversystem_reloadpy--리로드가-엔트리포인트-밖으로-나간-자리-5db3b80c-신설) |
...
| **[신설] 2026-08-04 서버 모듈** — **`virtual_join_executor.py`**(가상 조인 **실행**, 535 → **554**) · `retroactive.py`(🆕⑯ 소급 «등록부» — op **6종** + 실행 행 + 취소) · `enrichment_backfill.py` · `trace_fixture/` 패키지 6파일 | **554** + 🆕⑯ **1,139**(구 표기 697/545) + **411** + **1,356** | [§5-D](#5-d-2026-08-04-신설-서버-모듈) |
...
>
> ⚠️ **낡은 소스 주석 «다섯»이 아직 「410 을 답한다」고 적는다**(전건 실측 `2ac37874` — [§0](#0-묘비-목록--소스에-존재하지-않는-이름) 게이트 ㉗-c 가 이것을 센다): `main.py:768` · `database/models.py:940` · `config_resolve_report.py:122` · `enrichment_config.py:708` · **`run_decoupled_app.py:324`**(「진입은 `main.py` 의 `_graph_branch_retired` 가 막고」 — 그 함수도 «없다»). **코드 소관이라 여기서 못 고친다. 그 주석을 동작의 증거로 읽지 마라.**
>
...
> sweep_undelivered_broadcasts             chain_ingestion_worker.py «1357»  (구 781)
...
## 1.9 `server/system_reload.py` — 리로드가 **엔트리포인트 밖으로** 나간 자리 (`5db3b80c` 신설)
...
### `server/runtime_loops.py` (**148줄**, S-176) — 고리 «아홉»이 마지막으로 무엇을 했나
```
...
### `server/chain_graph.py` (**373줄**, S-178) — 한 흐름, 네 선언을 «한 그림»으로
```
...
⚰️ **정정의 정정(09-11 22:0x, 판정 293-b · S-179 ① `20e84d74`)** — 아래 「읽는 쪽이 둘」은
   **21:3x 에 참이었고 지금은 아니다.** 둘이던 «이유»(합성 규칙을 섞으면 두 번 그려짐)가 사라졌다:
   ...
   (`chain_graph.py:89` `rule.get("origin") or "file"`). 그래서 **읽는 쪽은 «하나»이고 출처는 «칸»이 말한다.**
...
### `server/health.py` (🆕⑱ **498줄** @`2ac37874` — 구 표기 384) — `/health` 판정표 (순수 함수 + config 백업 프로브)
...
### `server/enrichment_config.py` (**1,638줄**(🆕⑨ `5359fdd` 실측 — 구 표기 676), `c520012` 585에서 **+63** — `f9289f6`) — 인리치먼트 규칙 로더/검증
...
### `server/retroactive.py` (🆕⑳ **1,502줄** @`328a5c20` — 🆕⑱ 1,441 @`6345ef3d` — 🆕⑰ 1,156 @`64b562b6` — 🆕⑯ 1,139 @`9eb30691` — 🆕⑨ 545 @`5359fdd`, 구 표기 697) — 소급 적용의 **레지스트리**
...
### `server/enrichment_backfill.py` (**458줄**(🆕⑨ `5359fdd` 실측 — 구 표기 411), 신설) — 룰 도입 **이전** 소스 행의 파생 행 생성
...
### 🔒 `server/ledger_trace.py` (🆕⑭ **1,700줄** @`2a4e51cf` — 🆕⑬ 1,688 @`3b77fbc3`, 구 표기 2,039줄(🆕⑦/🆕⑩ `5359fdd` 실측)이고 그 사이에 **장사 다섯이 떠났다**. 구: 2,039줄(🆕⑨ `5359fdd` 실측 — 구 표기 1,937) @`a729a7f`) — 읽기 측 **[동결]**

> 🔒 🆕⑪ **[2026-08-23 은퇴 울타리 A군] 이 절(`ledger_trace.py`)은 «동결»이다** — 지도에서 지우지 않되 **살아 있는 표면으로 갱신하지 않았고, 새 작업의 출발점으로 읽지 마라.** 줄 수는 🆕⑫가 `bd0c192e`에서 재확인했다: **2,039**(무변동).
>
...
> 🆕⑫ 🔴 **[2026-08-23 후속 정정] 「이 절과 아래 `ledger_trace_router.py` 절은 동결」이라 적혀 있었다 — 라우터는 «더 이상 파일 통째로 동결이 아니다».** 그 파일은 살아 있는 읽기 측 여섯(`ledger_subgraph`·`ledger_catalog`·`ledger_composition`·`ledger_selection`·`ledger_siblings`·`ledger_trends`)의 **유일한 HTTP 표면**이라 통째 동결이 그들의 문을 잠갔다. 라우트 16개가 **8 동결 / 8 열림**으로 갈렸다 — 아래 절의 🆕⑫ 표.
>
...
| `server/ledger_explorer.py` | 🆕⑬ **175**(구 표기 445 — **−270**) | 🔴 **이제 «id 코덱»이다.** `entity_id(entity_type, keys)` · `decode_entity_id(value)` · `_entity` · `_edge_rows` · `_depths` · `_target_of`. **`explore`·`explore_entity`는 없다** — 「동결」 딱지가 붙어 있던 그 둘이 동결이 아니라 삭제로 끝났다 |
| `server/ledger_trace.py` | 🆕⑭ **1,700**(🆕⑬ 1,688, 구 표기 2,039) | 해결기·계보 walk. 🔴 **`trace()` 자체가 없다** — `^def trace` 전건 히트 0 (`_map_slot` 도 같이 갔다). `coverage()` 는 살아 있으나 **HTTP 소비자가 «0»이다**(`/trace`·`/coverage` 둘 다 라우트가 없다). 라우터가 실제로 쓰는 것은 배관 셋뿐이고, **심볼 표는 §5-H에 있으며 `a729a7f` 기준이라 재검증되지 않았다** |
...
>
> ⚠️ **[§5-H](#5-h-정본-원장-canonical-ledger) 의 `ledger_trace_router.py` 절이 이미 같은 모양을 적어 두었다** — 그 파일의 모듈 docstring도 「the ledger read routes — ten of them」이라며 열을 나열하는데 `@router.` 는 둘뿐이다. **같은 폴더의 파일 둘이 같은 방식으로 낡았다: 「`docstring` 을 목록의 출처로 쓰지 마라」는 이 폴더의 규칙이지 한 파일의 사고가 아니다.**
...
#### `server/dev_bench.py` (🆕㉙ **824줄** — S-197 에서 385→824) + `server/scripts/try_core.py` (**59줄**) — 샘플 하나로 «돌려 보는» 자리 (S-192 `803a709c`·`12985006`)
...
> 🔴 **값이 상수가 아니라 «파일»에 있는 것이 요점 전부다.** 체인 쪽의 같은 것들(`chain_ingestion_worker.py` 의 `OUTBOX_PURGE_MAX_CHUNKS`·`SWEEP_INTERVAL`)은 **상수**라, 새벽 두 시에 화면이 기어가는 것을 본 운영자가 «코드를 고치고 서버를 재기동»해야 한다 — **그 재기동이 바로 이 손잡이가 없애려는 것**이다.
...
| 🆕㉓ **`server/alignment_batch_counts.py`** (**191줄**, `f6371646` 기준) | **체인 그룹 한 번의 «초»를 층별로 모으는 자리 — 세 dict.** `phase(name)`(뷰 빌드 «안») · `stage(name)`(그룹 «전체»의 기계류) · 🆕 `write_step(name)`(`write:*` «안», 셋째 dict라 «두 번 세지 않는다»). 정렬을 한 번도 안 만진 그룹은 줄을 «안 찍는다**(0 으로 채운 줄이 진짜를 묻는다) | (S-94·S-151, 판정 241·261) |
