...
| `server/ledger/cutover_v2.py` | **285** | Stage 7 **manifest 전용 v2 컷오버 경계**. `DEFAULT_ONTOLOGY_ROOT` 소유 |
...
⚠️ 🔴 **[§5-H](#5-h-정본-원장-canonical-ledger) 쓰기 측 표의 줄 수는 전부 낡았다 — 이 패스가 재측정한 값**: `envelope` 264 → **323** · `vocabulary` 622 → **1,280** · `gate` 468 → **581** · `config` 623 → **1,073** · `schema` 360 → **427** · `store` 442 → **461** · `lot_event_translator` 526 → **535** · `backfill` 1,002 → **1,574** · `observability` 424 → **428** · `declared_translator` 258 → **294** · `dry_run` 416 → **596** · `source_contract` 281 → **319** · `translator_pattern` 164 → **197** · `observation_translator` **362**(무변동) · `transfer_translator` **395**(무변동) · `setup_bundle` **1,610** · `source_profile` **1,512** · `chain_mapper` **359** · `profile_chain_mapper` **469** · `ledger_frame` **278** · `profile_lookup_adapters` **105** · `legacy_import` **31** · `source_profile_builtins` **146**(무변동) · `__init__` **27**(무변동). 🔴 **패키지는 19가 아니라 «34파일 + `examples/` 1»이다.** 그 표의 심볼 서술은 이 패스가 열지 않았다.
