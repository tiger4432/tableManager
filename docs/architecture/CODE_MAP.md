...
| 🆕㉕ ↳ 합성기 :992~ (`AUTO_CONFIRM_MAPPER` · `DEDUP_PREFIX` · `AUTO_CONFIRM_PREFIX` · `synthesized_rule_names`) | **인리치 규칙 하나가 «체인 규칙 둘»을 낸다**(S-179 ①, 판정 292): dedup 과 auto-confirm. 두 이름을 «한 자리»가 짓고 충돌 검사도 그것을 읽는다 — 「합성된 이름이 어떻게 생겼나」에 대해 거절과 합성이 «어긋날 수» 없다. 자동 확정은 `follow_up` 이라 커밋 경로가 아니라 «뒤따르기»에서 돈다 |
| 🆕㉔ `crud.refuse_virtual_join_duplicates(...)` · `crud.source_unchanged(...)` | 전자: 행 «하나»의 중복 조인 키가 표 전체의 체인을 멈추지 않게 «그 행만» 이름 대어 거절(S-174). 후자: 값이 «안 바뀐» 재적재를 걸러 낸다 — 🔵 S-165 는 «지을 것»이 아니라 «이을 것»이었다(DO UPDATE 가 `ingested_at` 을 덮어 재적재마다 죽은 튜플 다섯 → 오토배큠 상주) | |
| 🆕㉔ `ledger/config_authoring.ISOLATION_ROOTS` · `setup_registry.SourcePlan.planned/refusal` | **깨진 선언이 «혼자» 떨어진다**(S-177 ②③, 판정 280·281). `planned=False` + `refusal{code,path,message}` 이고 로더가 «어느 것이» 떨어졌는지 말한다. 교차 검사는 «성한 부분집합»에서 다시 돈다(고정점). ⚠️ 그 고정점이 «가상 조인 규칙까지 닿지 않는다»를 시험이 못 박았고(`f9fa9c24`), 그래서 뿌리가 «자기 상수»를 갖는다 | |
