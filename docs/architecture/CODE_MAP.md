...
> **Status:** 🟢 Living | **Last-verified:** 2026-07-28 (**HEAD `0c6ac1a`의 커밋된 blob 실측** — 워킹트리 아님. [§0 측정 기준](#0-묘비-목록--소스에-존재하지-않는-이름)) | **Owner:** 전 에이전트 공용 | **Source-of-truth:** 각 표의 코드 경로
...
| `server/main.py` | ~4,195 | [§1](#1-servermainpy--api--ws-허브) |
...
| 소형 서버 모듈 (models/std_parser/enrichment_*/ingestion_activity/ingestion_checkpoint/bonding_plan/**map_overlay**/**transfer_plan**) + 그래프 트랙(graph_sync_worker/graph_materializer/ontology_config) + **운영 6종**(paths/process_supervisor/health/heartbeat/product_tables/**config_backup**) | ~9,970 | [§5](#5-소형-서버-모듈) |
| 기타 서버 모듈 (한줄 요약) + 설치·개발환경 스크립트 + **교차 구현 계약 `contracts/`** | — | [§6](#6-기타-서버-모듈-한줄-요약) |
| `client2/src/*` | ~21,800 (js 18,971 + css 2,825) | [§7](#7-client2src--웹-클라이언트) |
...
### `server/map_overlay.py` (~733줄, **U6 `95bf072`로 +38**) — [M2 신규] 범용 맵 오버레이 (계획 전용 아님 — 맵 인프라)
...
| `_SYSTEM_COLUMNS` / `derive_table_binding(table, val_candidates=None)` / `resolve_binding(cfg, table)` | `table_config`에서 x/y/val·key_columns 자동 유도(후보 순, 시스템 컬럼 제외 — **[U6] 후보 목록은 인자로 받는다**: None이면 `DEFAULT_VAL_CANDIDATES`, cfg 보유 호출자는 resolve를 넘겨야 한다) / **선언 우선 + 유도 폴백**(resolve된 후보를 전달) | ~512/518/560 |
| `build_key_filters(model, binding, map_key)` | `_` 조인 복합 map_key를 key_columns로 분해해 ORM equality 필터 생성(마지막 컬럼이 나머지 흡수) | ~572 |
| `get_overlay(db, cfg, target_table, target_key, sources, cell_cap=MAX_OVERLAY_CELLS) -> dict` | **메인 진입점** — 소스별 바인딩·align 해결 → 셀 조회 → 타깃 프레임 좌표 변환 → `{target, overlays[], cell_cap}`. **`eqp` 인자는 `by_eqp` 분기와 함께 삭제됐다**(엔드포인트 쿼리 파라미터만 no-op으로 존치 — 축소는 총괄 승인 사항) | ~597 |
| `get_paint_rules(cfg, table=None) -> dict` | `paint_lock`의 `"*"` 기본 + 테이블별 선언 머지 → `{enabled, blocking_values, from_overlay, message}`. **[U6] main.py `/api/maps/paint-rules`가 이것과 위 U6 2종을 한 응답에 묶는다** | ~714 |
...
### `server/transfer_plan.py` (~3,136줄, **`1fefd12`로 +56**) — [M2] Universal Transfer Plan 엔진 (v2 = 계획 정체성이 곧 맵 정체성, **`b35bc9f` zone 모델 · `2baf9ff` U9 marker · `1fefd12` count_only/column_unresolved 강등**)
...
>
> **수량은 저장되지 않고 유도된다** — `zone_layers`(1H=1층·TOP=1층·MID=`stack−이웃 수`) × `painted(값)` = `zone_demand`, 자재당 = `ceil(total / n)`(**배분이 아니라 충분성 검사**). 그래서 페인팅 분포 읽기(`_painted_values`)가 **load-bearing**이고, 실패 시 모든 required가 0이 되어 부족이 영원히 발화하지 않는다 → `painted_reliable`(~2795)이 유도 전체를 게이트한다.
>
> **[`1fefd12`] 유령 잔여(phantom remaining) 수정 — 강등 어휘 2종 추가**: ① **`connected(count_only)`** — `transfer_log`가 x/y 없이 바인딩되면 카운트는 실값이되 칩 정체 미상이라 집합 감산이 불가능하다(used_set이 비어 `remaining`이 과대). 강등 엔진이 remaining을 null + 상한(upper bound)으로 내리고, `by_core`의 used/remaining도 **log·area_map 양 경로 모두 null**(가짜 0 금지 — `used_count_only` 플래그 ~1187/~1210). ② **`connected(column_unresolved:<roles>)`** — 선언-미해석 컬럼(config 오타)의 침묵 제거, 기계장치는 `bonding_plan._ResolvedColumns` 공유(`_demote_unresolved`/`_unresolved_of` ~271/279가 위임 래퍼). `fail_values` 선언 + `val` 미해석은 필터 없는 카운트를 **거부**(0 + 강등 — 감산항 과대는 상한 불변식 위반).
>
...
| **`_summarize_inline(db, stage_name, stage_cfg, lot, slot, region=None, bins=None)`** | **가용 엔진 정본(tape-kind)** — `origin_log` 연결 시 `remaining = total − \|fail_union ∪ used_set\|`, 미해석 시 M1식 감산 폴백. `by_core` 7키 + `by_core_origin` 마커 `"log"`\|`"area_map"`(`fail=None`으로 0 위장 금지). `bins` 요청 시 `_bins_block` 동봉. **[`1fefd12`] `transfer_log`가 x/y 없이 바인딩되면 `connected(count_only)` 강등**(`used_count_only` ~1187/~1210) — `transferred` 카운트는 유지하되 remaining은 null+상한, `by_core` used/remaining은 **양 경로 모두 null**(가짜 0 금지). fail 소스의 `val` 미해석은 카운트 거부(0 + 강등) | ~1138 |
| `_bin_warnings(bins)` / `get_stage_source_summary(db, cfg, stage_name, lot, slot, bp_config=None, ref_table=None, map_key=None, bins=None)` / **`get_lot_bin_summary(db, cfg, stage_name, lot, bins=None)`** | BIN 블록 경고 승격 / **핸들러 진입점(scope=slot)** — 미선언 stage `KeyError`(→404) / **[`269b39e`] scope=lot 진입점** — 슬롯 전개 후 BIN 합산, **`chips` 없음**(로트 단위 헤드라인 잔여를 지어내지 않는다) | ~1548/1578/1631 |
| `_plan_store_binding(cfg, role, required)` | plan_store 역할 바인딩 (⚠️ `_num`은 삭제된 채 유지 — 저장 수치 파싱 없음) | ~1742 |
| **`_parse_bands(raw) -> (밴드[], 읽었는가, 거부된_원소_수)`** | **[레거시 `bands` 경로]** 3-튜플(호출부 `bands, readable, dropped = …` ~2740). **"못 읽음"과 "구간 없음"을 절대 합치지 않는다.** 객체가 아닌 원소는 거부하되 조용히 버리지 않는다(세어서 `layer_range_invalid reason:"not_a_band"`로 표면화). 크기 검사가 `json.loads`보다 **먼저** | ~1748 |
| `_band_seq(raw)` / `_band_materials(band)` / `_assign_band_seqs(bands)` | [레거시] 선언 `seq` 판독(정수값 float 허용 — `JSON.parse`가 먼저 접으므로 클라는 물리적으로 거부 불가) / 자재 목록 정규화(문자열 아님 거부) / 계획 내 `seq` 유일화 | ~1785/1817/1846 |
| `BAND_TO_OK/BLANK/INVALID` · **`STACK_MARKER`** · **`_int_state(raw)`** · `_band_to(band)` · **`_bin_of(raw)`** · `_prev_to(bands, i)` | 판정 어휘 3종(~1875–1877) + **[`2baf9ff` U9] `STACK_MARKER="marker"`(~1881 — STACK 전용 제4상태.** `_int_state`에서 절대 나오지 않고 **`stack_state`만 명시적 0을 승격**한다 — band `to`·BIN은 여전히 0 거부. 클라 `stackState`의 `'marker'` 미러) · **[`b35bc9f` 신설] 단일 정수 판독기** — `stack`과 `to`가 **같은 판독기**를 쓴다(클라 `bandToState` 미러. `'7.5'`→invalid — 컬럼 타입이 아니라 이것이 가독성을 결정) · `to` 판정(_int_state 위임) · BIN 라벨 판독 · prevTo 걷기(blank·invalid 동일 스킵, invalid는 보고) | ~1875–1881/1886/1928/1938/1957 |
| **[zone 코어 `b35bc9f`·`2baf9ff` — `doe_bands.js` 미러, 정본은 `contracts/doe_band_rules`]** `parse_material_list(raw)` / `_zone_tokens` / `stack_state(row)` / `mid_zone(row)` / `zone_layers(row, zone)` / `zone_demand(row, zone, painted)` / `parse_material_token(raw)` / **`material_pool_key(tok)`** / `validate_zone_plan(rows)` / `material_rollup_rows(rows, painted_of)` / `remaining_state(availability, used)` / **`bands_to_zones(bands)`** | mat_* JSON 배열 판독 / 행별 토큰 수집(비문자열 거부) / **`stack` 4상태(ok·blank·invalid·marker — 명시적 0만 marker, 음수는 invalid로 값 보존)** / MID 구간 유도(marker는 `{size:0, known:true}` — E행의 0층과 같은 "진짜 0") / 구역 층수(1H=1·TOP=1·MID=stack−이웃, **marker는 전 구역 `[]`**) / 구역 수요 = painted×layers / 토큰 → `{lot, slot?, bin?, scope}` 문법 / **풀 정체 키 — 분리자 조인이 아니라 튜플**(U+001F 조인이 디스크에서 분리자를 잃고 두 풀을 합산한 사고의 재발 방지) / **V1–V6 차단 규칙**(+W-DUP-MAT advisory — **marker 행은 V6 하나에만 답한다**: V6 블록 ~2281, V3 풀 스캔 제외 ~2362) / MAT×BIN 롤업 행(**marker 행은 부재** ~2419 — "사용 0" 행이 아니라 아예 없음) / 잔여 판정(신뢰 불가 가용은 잔여를 억제) / **레거시 band 계획 → zone 사상**(불가하면 `not_convertible`로 거부 — 손실 접기 금지) | ~1979/2005/2055/2079/2103/2126/2146/2199/2234/2401/2454/2478 |
| **`_material_identity_rule(cfg)` / `_split_material(material_id, rule)`** | 자재 ID → 소스 `(lot, slot)`. 규칙은 **`plan_store.material_identity` 선언으로만** 성립. 분해는 뒤에서부터(클라 `splitMaterialId`와 방향 동일, `map_overlay.build_key_filters`는 반대). **분리자가 없으면 거부** | ~2542/2561 |
| `_painted_values(db, ref_table, map_key, overlay_cfg)` | 대상 맵 자신의 셀 값 분포 group-by(`ORDER BY` — 절단 재현성). 반환 `({값: 셀수}, 상태, 절단여부)` — 세 번째 값이 [B2]의 핵심 | ~2594 |
| `validate_plan(db, cfg, ref_table, map_key, overlay_cfg=None)` | **핸들러 진입점** — **`plan_store.registry` 미구성은 `LookupError`(→404)**. 레지스트리 1회 조회(`ORDER BY value` ~2687 — 절단 재현성). **[zone] 행별로 `stack`/`mat_*`를 먼저 읽고**(내부 `_reg_get` ~2695, zone_row 조립 ~2713), zone이 비어 있으면 레거시 `bands`를 `bands_to_zones`로 사상(~2740 — `REGISTRY_LEGACY_ROLE` 미선언이면 그마저 없음). **`validate_zone_plan` 호출 ~2821이 V1–V6을 채점**하고 위반은 `zone_rule_violation`으로. marker 행은 수요 유도에서도 자연 소멸(`zone_layers`가 `[]` — 자재 해석·조회·수요 0, 주석 ~2937). **[B2] `painted_reliable`(~2795)이 유도 전체를 게이트.** `remaining_reliable=False`면 부족·fail 판정 전부 생략 + `availability_unreliable`만. 캡 절단은 `result_truncated`를 역할·캡별로 각각 발행. **`structural_refusal`(~2704, 세트 ~2722/2746/2778)이 `availability_checked`에 AND로 물린다**(~3097). 최종 `status`는 `ok`/`warnings`/**`unverified`**(~3119) 3값 — "검사 안 함"과 "이상 없음"을 같은 값으로 내지 않는다 | ~2644 |
...
| `server/event_constants.py` (51줄) | 프로세스 간 내부 이벤트(`/internal/events/*`) 공용 상수 — `MAX_NOTIFY_CREATED_LOGS=500`(~14, 발신측 created_logs 절단 상한: 워처 `directory_watcher:1739` · 체인 워커 `chain_ingestion_worker:474` · 수신 `main.py:3754/3802` 공유 — 2026-07-28 재측정, directory_watcher 앵커는 [Flatten]으로 +312 이동) · [P2] `MAX_AUDIT_VALUE_CHARS=4096`(~22)과 `truncate_audit_value(value, max_chars)`(~25 — 반환 `(값, 절단여부)`, str은 `…[truncated: 총 N자]` 마커, dict/list는 타입·길이 플레이스홀더)를 `crud.create_audit_log`가 소비 |
...
### `map_editor.js` (~5,665줄, **6-fix `0052d76`로 +79**) — 웨이퍼 맵 에디터 (단일 페이지 스크립트, export 없음 — 단 `import`는 있다: `transfer_plan.js`의 `bandToState`·**[U6] `stageTargetTables`**, **`doe_bands.js`의 `parseMaterialList`·`bandsToZones`**)
- **[U6 `95bf072`] 서빙된 맵 기본값 — 클라 사본 0**: `overlayContract`(~91 — paint-rules 응답에 실려 오는 `{valueColumnCandidates, defaultLegend}` 캐시, 첫 성공 fetch까지 null. **"미조회/불가"는 "자동감지 불가·빈 시드 1행"으로 동작**하고 절대 내장 목록이 아니다) · `EMPTY_DOE_SEED`(~188 — "선언 없음" arm의 빈 DOE 1행, 구 ~~`DEFAULT_LEGEND`~~) · `defaultLegendRows()`(~192 — 선언 > 시드) · `declaredLegendRow(value)`(~201 — 값 자동 추가 시 선언 행의 color/desc 우선) · **`LEGEND_PALETTE`(~2360 — 12색 사본 3곳 통합)** · `pickUnusedColor`(~2363) · **`autoAddLegendValue(value, fallbackDesc)`(~2372 — 값이 legend에 자동 추가되는 **유일 경로**: 패널 [+ 값]·E1/E2 autopaint·미지 값)**. 값 컬럼 자동감지 소비 2곳: `fillColumnDropdowns`(~1260 — **[`0052d76` fix D] 후보 매칭이 대소문자 무시에서 `cols.includes(c)` 정확 일치로 통일**(~1290): 이 지점만 느슨해 서버·`deriveMapBinding`과 같은 목록에서 다른 답이 나왔다. 대소문자 어긋난 후보는 정직하게 자동선택 안 함) · `deriveMapBinding`(~5161). `switchTable`(~1125)이 스키마 fetch와 병렬로 paint-rules 왕복을 **await**한다.
- 좌표 변환 코어: `getPhysicalCoords`(~1332) `getCellFromPhysicalCoords`(~1382) `getCellFromVisualCoords`(~1422) `getVisualCoords`(~1491) `getTransformedPhysicalConfig`(~1506) `getWaferBoundingBox`(~1439) `getScreenShift`(~1541) `isCellInsideWaferFast`/`isCellInsideWafer`(~1567/1605) — 회전/면반전 불변식은 [MAP_EDITOR_SPEC](../spec/MAP_EDITOR_SPEC.md).
  - **[7d931dc] 프레임 창(frame window)** — `physFrameOverride`(~1305) + `physNum`(~1308)/`gridDimNum`(~1317)/`withPhysFrame(frame, fn)`(~1326). 변환 함수가 규격을 DOM에서 읽는 지점을 잠깐 갈아끼우는 장치로, **주입 지점은 `getTransformedPhysicalConfig`·`getWaferBoundingBox` 두 곳뿐**이다. `withPhysFrame`은 **동기 전용**(내부 `await` 금지 — `try/finally` 복원이 새면 조용한 오답). 기존 `parseFloat(v) || dflt` 규약(0 → 기본값) 보존.
- 캔버스 렌더: `renderGridCanvas`(~1957, 본체 — 오버레이 마커 호출 ~2119) `scheduleRenderGridCanvas`(~1920) `fitGridToWorkspace`(~1942) `updateNotchPosition`(~2301).
- 데이터 IO(REST — WS 아님): `loadExistingMap(opts={})`(~3444 — **[`280ebf0`] `{quiet}` 옵션**은 last-open 부트 복원용. **[`0052d76` fix G] 사용자 취소(맵 키 공란·선택 모달 dismiss)는 `{count:0, cancelled:true}` 반환**(~3466/~3594) — 호출자가 "미구축 맵"과 구분한다. **[`6db517d` H1] 로드 경로는 legend 채택 시점에 초안을 저장하지 않는다** — 구 `saveLegendToStorage()` 즉시 호출이 방금 로드한 **서버 상태**를 초안 위에 덮어써 리로드마다 칠한 셀 초안을 전멸시켰다(사유 주석 ~3768–3773). 저장은 초안 우선순위 판정이 끝난 뒤 registry 블록 안에서 **1회**만 — **단 [`0052d76` fix A] 낡은 초안(stale draft)을 보존 중이면 그 1회도 생략**: `staleDraftKept`(~3826 선언, ~3847 판정) 상태에서 persist하면 "초안은 지우지 않았습니다" 토스트가 방금 한 약속을 그 초안을 덮어쓰며 깬다 → `if (!staleDraftKept) saveLegendToStorage()`(~3862). **[`0052d76` fix B] 로드 성공은 `legendDirty = false`로 리셋**(~3790 — 맵 A의 미저장 칩이 맵 B로 새던 결함. 초안 복구가 이 리셋 **뒤에** 다시 세우므로 복구 초안은 여전히 미저장으로 표시 — 읽기 실패 분기도 초안 내용이 화면을 바꿨으면 ~3879에서 재마킹). 복구된 초안은 여전히 미저장 편집이므로 `legendDirty = true` ~3837. **[U6-1 `95bf072`] 0셀 로드는 registry 적용 전에 `seedEmptyDoe()`로 리셋**(~3811, `read.ok` 안에서만): `uniqueVals.size > 0` 가드가 페인트 유도 legend 재구성을 건너뛰어 **같은 테이블의 직전 맵 legend가 새 빈 맵에 상속**되던 결함 — 읽기 실패 시엔 화면 유지(미상 서버 상태는 "빈 것"이 아니다), 초안 우선순위 블록은 종전대로 그 뒤에 이긴다. **[`0052d76` fix C] 메타 없는 맵의 기본 선택지 = 데이터 전체 bbox + "마스크 없음"**(~3620–3634): 종전엔 좌패널 물리 규격(웨이퍼 원)이 기본 프레임 밑에 살아 있어 모서리 셀이 `inside:false`로 찍히고 H2 관문이 모든 Push를 거부했다(1293→379). inside 술어에 off 스위치가 없으므로 "마스크 없음"을 그 어휘로 표현한다 — chip 1×1·offset 0 + 격자 반(半)대각선을 외접하는 웨이퍼 지름을 `applyPresetObject`(기존 프리미티브)로 기입. 선택 모달 버튼 라벨도 정직화(~3563)) `pushMapData`(~3945, 저장 본체 — **[`b35bc9f`] legend/DOE 저장의 유일한 트리거이기도 하다**: 내부에서 `saveLegendToServer` 호출 ~4189. **[`6db517d` H2] 대비 관문(contrast guard) ~4050–4077**: 직렬화된 페이로드의 비어 있지 않은 셀 수 < 화면의 비어 있지 않은 셀 수면 **confirm 이전에 적재 거부** — `replace_map`은 맵 행 전체를 지우고 다시 쓰므로 프레임이 덮지 못한 셀은 누락이 아니라 **삭제**된다(QA 실측 1293→379, 메타 없는 맵 + 기본 프레임 — fix C가 1차 방어, 이 관문은 2선 유지). 데이터 보호 관문 3형제의 셋째(zone 컬럼 부재·레거시 판독 불가와 같은 패턴 — 직렬화하지 않은 것을 지우는 쓰기를 막는다). 계산은 루프와 같은 공백 술어라 사용자가 지운 셀은 양쪽에서 상쇄) `fetchGridMetaFor(table, mapId)`(~3411) 프리셋 `fetchAndRenderPresets`/`saveCustomPreset`/`deleteCustomPreset`(~1662/1768/1818) + `applyPresetObject`(~1718, `loadSelectedPreset` ~1751에서 추출한 공용 함수 — fix C의 규격 기입자).
...
- 레전드/브러시: `renderLegendTable`(~3054) `selectBrush`(~3242 — **[`280ebf0` 입력 랙 수정] 선택은 DOE 리스트 재구성·전 그리드 카운트 스캔을 유발하지 않는다**(의도 주석 ~3256): mousedown이 innerHTML 재구성을 유발해 커서 밑 input이 분리되던 결함) + `saveLegendToStorage`(~2403). `getCurrentMapKey`(~2613)는 **로드된 맵이 아니라 현재 메타 입력 필드**를 읽는다(오버레이 관문 F2의 근원). 서버 레지스트리 IO는 아래 전용 블록 참조.
- **Split Registry = DOE의 유일한 기록자**(~216–507 선언 · ~2645–3052 IO). **[`269b39e`+`b35bc9f` ZONE 모델]** legend 행 = DOE 1건 = `{value, desc, color, knobs, stack, mat_1h[], mat_mid[], mat_top[]}` — zone 계약 정본 주석 ~247–267(`bands`는 **폐기됐지만 읽기 전용으로 생존**: 서버에 band 계획이 남아 있고 저장이 `replace_map`이라, 안 읽으면 다음 키 입력이 그 계획을 빈 집합으로 지운다). `SPLIT_REGISTRY_TABLE='map_split_registry'`(~216) · **`SPLIT_KEY_SEP='\|'`(~219)** · `buildSplitKey`(~244).
  - 정규화 순수 함수: `parseJsonCol`(~268) **`normalizeBands`(~274 — 하니스 검증 대상, 레거시 읽기 전용. `to` 해석은 `transfer_plan.js`의 `bandToState` **하나만** 쓴다. 읽을 수 없는 값은 **원문 그대로 보존**)** `normalizeKnobs`(~307) `knobsToObject`(~319) `serializeKnobs`(~328) `normalizeLegendItem`(~359 — zone 필드 포함) **`cloneLegend`(~380 — 깊은 복사. 배열 필드를 얕게 넘기면 프레임 스냅샷과 화면이 같은 배열을 공유한다)**. (~~`serializeBands`~~는 band 편집기와 함께 삭제)
  - 동시성 장치: **`legendReplaceScope`(~232 — `{table, mapKey, fingerprint}`. `replace_map` **권한**이자 동시성 검사의 **기준선**)** · **`legendVocabularySeed`(~236 — [`269b39e` 신설] 플레이스홀더 시드 시점의 행 서명 Map. `reconcileVocabClaims`가 "사용자가 이 행을 바꿨다"를 **유도**하게 한다)** · **`legendConflict`(~240 — 다른 세션이 바꿨다. 리로드 전까지 그 맵의 registry 쓰기 전량 차단. upsert 강등 금지)** · `legendSaveState`(~242). 지문 계산 `LEGEND_PAYLOAD_COLUMNS`(~405)/`canonRegistryRow`(~426)/`FP_UNIT`·`FP_ROW`(~433–434)/`registryFingerprint`(~435) — **`contracts/legend_map_scope` 하니스가 이 상수·함수들을 이름으로 추출**한다.
  - 페이로드 순수 함수: **`buildLegendRegistryUpdates(refTable, mapKey, legendArr, user, nowStr)`(~450)** / **`parseLegendRegistryRows(result, dedupeByValue)`(~499)**.
  - 서버 IO: `REGISTRY_SCOPES=['map']`(~2645 — **테이블 전체 어휘 시드는 삭제됐다**, `269b39e` 결함의 원인) `fetchRegistryRows`(~2646, `limit=500` — **부분 읽기는 읽은 게 아니라 throw**) `readRegistryScope`(~2671) `legendRowSignature`(~2682) `reconcileVocabClaims`(~2703) `applyRegistryRowsToLegend`(~2728 — 빈 legend 폴백도 **[U6] `defaultLegendRows()`** 경유) **`saveLegendToServer(mapKeyOverride)`(~2807 — 쓰기 단위가 **한 맵의 값 전체 집합**이라 `replace_map:true`. 지운 값·구간·자재는 집합에 없다는 것만으로 서버에서 삭제된다. 쓰기 **전에** ① `probeZoneColumns` 게이트(~2925, `ZONE_COLUMNS` ~2924 — 물리 zone 컬럼이 없으면 **거부**: crud가 페이로드에서 zone을 떨어뜨린 채 replace_map이 나가면 계획 전체가 층 구조 없이 대체된다) ② 재조회+지문 비교(불일치면 `legendConflict`로 거부). **참조는 정확히 2곳 — 정의 + `pushMapData`(~4189)**)** `LEGEND_SAVE_MESSAGE`(~2953) `applyLegendSaveResult`(~2966) `getPlanSaveState`(~2993) `persistLegend`(~3006) **`scheduleCellDraft`(~3028 — 400ms 디바운스로 `saveDoeDraft`+`notifyLegendChanged`. 페인팅·드래그·fill·paste·legend 개명까지 **호출 10곳**(1055/2284/3219/3348/3398/3939/4371/4398/4420/5544 — 2026-07-28 `0052d76` 후 재계수) — 모든 편집 경로가 새로고침을 견딘다. **[fix E] `persistLegend`(~3008)·`scheduleCellDraft`(~3030) 두 영속화 관문이 `frameTouched = true`도 세운다** — 뒤로가기 경고의 "실제 편집" 판정 근거)** `renderLegendMetaOnly`(~3037). ⚠️ **~~`scheduleLegendServerSave`~~(자동 저장)는 삭제** — 묘비 주석 ~2980, 서버 쓰기는 ⚡ Push 하나뿐([§0](#0-묘비-목록--소스에-존재하지-않는-이름)).
  - 로컬 초안(**[`b35bc9f`] 셀까지 나른다**): `seedEmptyDoe`(~2389 — 행 없는 맵의 시드, `vocab:true` — **[U6] 시드 원천은 `defaultLegendRows()`**: 서빙된 `default_legend` \| `EMPTY_DOE_SEED` 1행) `doeDraftKey`(~2411, `map_doe_draft::<table>::<mapKey>`) `DRAFT_VERSION=3`(~2433) `cellsDigest`(~2437, FNV-1a) `draftBase`(~2449 — 열었을 때 서버가 갖고 있던 것의 지문) **`saveDoeDraft`(~2451 — DOE(zone·knob 있는 행만)+`cells`+기반 지문을 함께 저장 — 초안 지문 우선순위의 근거)** `readDoeDraft`(~2488, v3 미만은 "기반 미상"으로 수용) `clearDoeDraft`(~2501) `applyDoeDraftRecord`(~2560 — **우선순위 판정은 호출부**, 이 함수는 적용만. **[`6db517d` H1] "applied"의 의미가 "내용이 있었다"→"화면을 바꿨다"로 교체**됐다(~2563–2589, before/after JSON 비교): 초안은 registry 저장 직후에도 재저장되므로 Push+새로고침 뒤 초안 = 서버 행인데, 그것을 "복구"로 보고하면 계획 있는 맵을 리로드할 때마다 유령 미저장 칩·토스트가 부활한다) `applyDraftCells`(~2597).
  - **[`280ebf0`] last-open 복원**: `LAST_OPEN_KEY='map_editor_last_open'`(~2509) `recordLastOpenMap`(~2511 — **depth-0 정체성만**: 자재 프레임은 여정이지 집이 아니다. 기록 지점은 로드 성공 ~3893·Push 성공 ~4174) `restoreLastOpenMap`(~2530 — 부트 1회 호출 ~646. **수동 경로 그대로**(switchTable+메타 입력+`loadExistingMap({quiet})`)를 걷는다 — 초안 우선순위·missing-key 동작이 별도 복원 경로가 아니라 같은 코드. 테이블이 사라졌으면 조용히 초기 화면).
  - 패널 관문(컨트롤러가 부르는 쓰기 3종): `addLegendRowForPanel`(~3277 — **[U6] 색 선정은 `autoAddLegendValue` 경유**) **`updateLegendRowForPanel(value, patch)`(~3288 — DOE 변조의 유일 관문. 배열 필드는 **새 배열로 통째 교체**, 제자리 수정 금지)** `deleteLegendRowForPanel`(~3333). + `fetchMapKeyColumns`(~3354) `probeMapExists`(~3372 — **존재를 추측하지 않는다**) `remapGridValues`(~3389).
- 편집 도구: `fillGrid`(~3915) `getEdgeClassification`(~4229) `selectEdgeCells`(~4309) `autoPaintE1E2`(~4336 — **[U6] E1/E2 고정 색 삭제**: `autoAddLegendValue` 경유, 선언 행 > 팔레트 규칙) `copyGridToExcel`(~4423).
- 프레임 스택: **[`0052d76` fix E] `frameTouched`(~4601 선언)** — "이 프레임을 연 뒤 실제로 편집했는가". `framePushed`만으로는 `!framePushed && cells>0`이 **보기만 한** 자재 맵에서도 뒤로가기 confirm을 띄웠다. 세우는 곳은 영속화 관문 2곳(`persistLegend`/`scheduleCellDraft`), 리셋은 `setLoadedIdentity`(~4842), **초안 복구는 의도적으로 안 세운다**(복구 내용은 초안 슬롯에 살아 있어 되돌아가도 잃는 게 없다). `snapshotEditorState`(~4603 — **[fix B/E] `legendDirty`·`frameTouched`도 스냅샷**에 포함 ~4629–4630: 자재 맵 왕복이 부모 프레임의 미저장 칩·백가드 기준선을 지우던 결함) `restoreEditorState`(~4664, 복원 ~4714–4715) `openMapFrame`(~4891 — `allowEmpty`로 빈 그리드 오픈 허용, 키는 Push에서 생성. **[fix G] `r.cancelled`면 프레임 롤백 + info 토스트 1회**(~4923–4934) — 종전엔 "맵이 아직 없습니다" 거짓 토스트와 함께 빈 격자 프레임이 열리거나 무피드백 롤백) `popMapFrame`(~4957 — **[fix E] dirty 판정에 `frameTouched` AND**(~4961): 편집 없는 프레임은 조용히 복귀). **복원 대상에 `overlayLayers`·캔버스 스크롤이 포함된다**(~4586 주석).
- **[M2] 페인트 잠금**(~41–178, 서버 선언 소비 — 구 `'F'` 하드코딩 대체): `NO_PAINT_LOCK`(~41) `isLockedValue`(~45) `isOverlayLocked`(~55) **`isProtectedFCell`(~67 — 편집 불가 판정의 단일 관문, 전 편집 경로가 여기로 수렴)** `applyPaintLockConfig`(~72) `fetchPaintRules`(~103, GET `/api/maps/paint-rules` — **[U6] 같은 응답에서 `overlayContract`도 갱신**: 후보 목록을 실은 응답만 캐시를 바꾼다, 구버전 서버는 기존 값 유지) `updatePaintLockIndicator`(~147) `recomputeLockedCells`(~164). 404/405만 "선언 없음"(해제)이고 네트워크·5xx는 **직전 잠금 유지** + `source:'stale'` + 툴바 칩. ⚠️ **[QA C4 미해소] 콜드 스타트는 여전히 fail-open** — "직전 값"이 페이지 로드 직후엔 기본값 `NO_PAINT_LOCK{enabled:false}`라 첫 조회가 실패하면 강제 지점이 열린 채 시작한다(칩은 뜨므로 **조용한** fail-open은 아님). 테이블 전환 시 실패하면 **이전 테이블의 잠금 값**을 새 테이블에 계속 적용한다(fail-closed 방향이라 안전하나 의미상 부정확).
- **[7d931dc] 오버레이 레이어**(~5002–5665) — **변환은 클라 단일 구현**이다. 계약은 [MAP_EDITOR_SPEC §5](../spec/MAP_EDITOR_SPEC.md). ⚠️ **[`280ebf0`] `.overlay-box`/`.ov-*` CSS는 `b35bc9f`의 transfer_plan.css 재작성이 떨어뜨렸다가 원문 그대로 복원**됐다(마크업은 계속 그 클래스를 쓰고 있었다):
  - 상태 `OVERLAY_COLORS`(~5002) / `overlayLayers`(~5003) / `activeOverlayLayers`(~5004) / `overlaySeq`(~5005) / `recomputeActiveOverlays`(~5007, 렌더 루프 내 재계산 금지) / `drawOverlayMarkers`(~5012, 렌더 호출 ~2119).
  - **프레임 계산**: `frameFromMeta(meta)`(~5036, `grid_metadata` JSON → 프레임 기술자. **없는 물리 항목은 undefined로 남겨** 현재 화면 값 폴백) / `currentFrame()`(~5063) / `resolveFrame(frame)`(~5076, 축 전부를 실값으로 확정) / `frameAxesKey(rf)`(~5093, 회전·면·y반전·START·치수·물리 6종 = identity/derived 판정의 유일한 근거).
  - **`projectCellsToPhys(cells, frame)`(~5104)** — `getCellFromVisualCoords` → `getPhysicalCoords`를 **소스 프레임을 씌운 채** 호출한다. `loadExistingMap` 셀 루프와 **같은 함수·같은 인자 순서** — **오버레이 전용 기하식은 0줄**이다.
  - `pushFailedOverlay`(~5123) — 실패도 목록 행으로 남긴다(같은 소스 중복은 갱신).
  - 소스 읽기: `OVERLAY_CELL_LIMIT=2000`(~5144, 메인 로드와 동일 상한) `fetchTableSchemaCached`(~5147) `OVERLAY_SYSTEM_COLS`(~5156) `deriveMapBinding(schema)`(~5161 — **[U6] ~~`OVERLAY_VAL_CANDIDATES`~~ 삭제**: 후보는 `overlayContract.valueColumnCandidates`(서빙), 불가면 첫 비제외 컬럼 폴백 — 서버 `derive_table_binding`과 같은 규약) `buildKeyFilters(keyColumns, mapKey)`(~5182, 서버 `build_key_filters`와 동일 — 마지막 컬럼이 나머지 흡수).
  - `addOverlayLayer(sourceTable, sourceKey, targetOverride)`(~5202) — **메인 로드와 코드 경로 완전 분리**. 흐름: ① 바인딩 유도 → ②③ `Promise.allSettled`로 셀 + 소스/타깃 메타 병렬 조회 → ④ 프레임 확정 → ⑤ `cols×rows` 호환성 관문 → ⑥ 정렬 요약 + 격자 밖 셀 카운트. 명명된 실패 status **4종**: `binding_unavailable` `meta_unavailable` `no_data` `align_unavailable` (+ IO 실패는 일반 `error`). **이 함수는 `/api/maps/overlay`를 호출하지 않는다**.
  - `removeOverlayLayer`(~5396) `toggleOverlayLayer`(~5403) `clearOverlayLayers`(~5412).
  - `overlayGeomSig`(~5425) / `currentGeomSignature`(~5427) / `syncOverlayGeometry`(~5444, 서명 변경 시 `rawCells`+`o.frame`에서 재투영. 훅 2곳 — 렌더 ~1996 · 프레임 복원 ~4706). ✅ **[QA C7 해소]** 서명이 화면 7축 + **물리 6종**을 담는다(소스 메타 완비 시 재투영은 항등 — 6종이 실제로 일하는 곳은 물리 규격 미등록 폴백 경로뿐).
  - `overlayAlignChip(o)`(~5467) — 정렬 상태 칩. 판정은 **`align.origin`으로만**.
  - `importOverlayToGrid(id)`(~5498) — 유일한 의도적 교차: 오버레이 셀을 `gridData`로만(**서버 쓰기 없음**, `isProtectedFCell` 존중, 웨이퍼 밖 셀 스킵). `ensureLegendValues`(~5556)는 **로컬 legend 캐시만** 갱신.
  - `renderOverlayList`(~5565) `handleAddOverlayClick`(~5619) `CORE_CANONICAL_TABLE='core_defect_map'`(~5648) `addOverlayForSource(sourceTable, lot, slot)`(~5650) `listOverlayLayers`(~5659) — 뒤 둘은 `transfer_plan.js`에 넘기는 컨트롤러 표면.
  - **오버레이 해제 지점은 2곳이다** — 테이블 전환 `switchTable`(호출 ~1165, 토스트) · **맵 로드 `loadExistingMap`(호출 ~3488, 토스트)**. 둘 다 `overlayLayers.length > 0`일 때만 실행한다. ⚠️ **`openMapFrame`은 오버레이를 해제하지 않는다** — 프레임 스택은 반대로 오버레이를 **보존**한다(~4586 주석). 툴바 버튼(`#btn-clear-overlays`) 배선은 ~802.
- **전사 계획 배선**: `initTransferPlan({...})`(~606, import ~6 — `bandToState`·**[U6] `stageTargetTables`**도 여기서 들여온다. **[U6] `loadTablesList`(~1061)의 초기 테이블 선택이 `stageTargetTables()`를 소비** — 구 `bonding_map` 하드코딩 대체: 선언된 stage 타깃 맵 우선, 없으면 첫 맵 테이블) + `notifyMapContext`(1172/3892/4173/4931/4945/4952/4971 = **7곳** — 2026-07-28 재계수, +1은 [fix G] 취소 롤백 지점) `notifyLegendChanged`(2977/3032/3038/3055 = **4곳** — ⚠️ `selectBrush`엔 의도적으로 없다, 주석 ~3256) `notifyPaintCounts`(~1872). rect 영역 선택 모드는 **전면 폐기**(값 페인팅이 정본 — 코드 부재).
