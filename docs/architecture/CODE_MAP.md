...
🔴 **줄어든 일곱(🔻)이 늘어난 스물둘보다 위험하다.** 자란 파일은 「이 절에 없는 것이 있다」는 뜻이고 독자는 소스를 열어 찾는다. **줄어든 파일은 「이 절에 있는 것이 소스에 없다」는 뜻**이고, 독자는 지도에 적힌 이름을 그대로 믿는다 — 그것이 [§1.5](#15-그래프-조회-구간은-410으로-은퇴했다)에서 실제로 일어난 일이다. **`product_tables.py`(−74) · `retroactive.py`(−152) · `ui.js`(−84) · `ledger.html`(−348)의 심볼 표는 이 패스가 열지 않았으므로 «죽은 이름이 있다고 가정하라».**
...
> #    ⚠️ `server/tests` 를 범위에 넣지 마라 — `test_retroactive_admin.py` 와
> #       `test_graph_branch_retired.py` 가 «은퇴를 채점하려고» 이 이름을 든다(그게 옳다).
> #    2보다 크면 되살아난 것이고, 클라 쪽 1이 0이 되면 그 주석이 고쳐진 것이다(둘 다 볼 일).
> git grep -c "graph_orphans" -- server ':!server/tests' client2/src
> # ㉕-c `MultiDiscoveryScheduler.maybe_sweep_graph_orphans` — 실측 **정확히 1건**이고
> #      그 하나는 `server/tests/test_graph_branch_retired.py` 의 «부재를 단언하는 줄»이다
> #      (`assert "self.maybe_sweep_graph_orphans()" not in src`). 🔴 §6 `run_auto_update.py`
> #      행이 이것을 «살아 있는 메서드»로 싣고 `~668` 앵커까지 달고 있었고, 그 668 은 지금
> #      `retroactive_busy` 다 — 실재하는 «다른» 함수라 도착지가 멀쩡해 보인다. 2건 이상이면 부활.
...
>
> 🆕⑨ 🔴 **[2026-08-18 실측] 이 파일은 `5359fdd`(blob `9cee25d`)에서 **6,057줄**이었다.** 이 문서 안에 공존하던 두 값(§0 표의 **6,322** · 여기 있던 **6,128**)은 둘 다 낡았다 — **줄어든 것이 이 파일에서 처음이 아니고**, 이번 감소는 그래프 구간 은퇴([§1.5](#15-그래프-조회-구간은-410으로-은퇴했다))가 만든 것이다. ⚠️ **이 패스는 줄 수만 쟀다** — §1.1~§1.4의 심볼 표는 열지 않았다.
>
...
| 메서드 경로 | 핸들러 | 역할 |
|---|---|---|
| 🪦 POST `/api/graph/sync` | `manual_graph_sync` | ⚰️ **[2026-08-18 실측] 본체는 `raise _graph_branch_retired()` 한 줄이다 — 410.** 종전 이 칸은 「:8090 프록시 백필/복구 트리거」와 `httpx.AsyncClient(trust_env=False)` 프록시 규율을 서술했는데, **`GraphSyncRequest`도 httpx 호출도 이 파일에 없다**(grep 0건). 프록시 규율 자체는 여전히 유효하고 살아 있는 서술은 [§1.7](#17-serverinternal_event_clientpy--내부-http-호출의-단일-소유자-23a346d-신설)에 있다 — **여기서 읽지 마라**. 아래 [§1.5](#15-그래프-조회-구간은-410으로-은퇴했다) |
...
### 1.5 그래프 조회 구간은 410으로 은퇴했다

⚰️ **[라우트 410 전환 = `2ec78b9`(2026-08-14) · 절 축약 = 2026-08-18 실측]** ⚠️ **날짜를 하나로 적지 마라** — 라우트를 410으로 바꾸고 표 DROP 마이그레이션을 들여온 것은 **`2ec78b9`(2026-08-14)**이고, **워커·실행 파일 삭제는 `9851374`(2026-08-16)**이며, **모델 클래스 제거는 2026-08-18**이다. 🔴 **여기 있던 표 두 개(26줄)는 «이름은 전부 실재하는데 동작은 하나도 남지 않은» 서술이었다.** `main.py`(`5359fdd` blob `9cee25d`) 실측:
...
>
> 🔴 **어휘를 새로 만들지 않았다.** 런타임 열화 어휘(`main.CHIP_TRACE_*`, [§1.5-bis](#15-그래프-조회-구간은-410으로-은퇴했다))를 **그대로 재사용**한다 — 같은 구분이 config 로드 시점으로 한 층 올라온 것뿐이라 어휘가 갈라질 이유가 없다.
>
...
- 🆕⑯ **실행 «행» 계열 — 이 라운드의 신설 절반.** 상태 여섯 `RUN_QUEUED`/`RUN_RUNNING`/`RUN_DONE`/`RUN_CANCEL_REQUESTED`/`RUN_CANCELLED`/`RUN_FAILED`. 🔴 **`cancel_requested` 는 «요청»이지 결과가 아니다** — 세워진 시점에 연산은 아직 돌고 있고, 배치 사이에서 실제로 멈춘 뒤에야 `cancelled` 가 된다. **둘을 접으면 아직 쓰는 중인 실행이 끝난 것처럼 보인다.**
  ...
  - **`_mark_run(run_id, *, state, started=False, finished=False, result=None, error=None)`** — 🔴 **자기 세션.** 「실행이 실패했다」가 바로 그 행이 연산과 함께 롤백되면 안 되는 순간이다. `error` 는 2000자로 자른다.
...
### `server/ledger_api/ontology_config_explorer_router.py` (🆕⑩ **264줄** — 구 표기 129)
...
### `server/ledger/config_explorer_service.py` (🆕⑩ **946줄** — 구 표기 305)
...
### 🆕⑩ `server/ledger/implementations.py` (**172줄**, 신설) — 실행 가능한 구현의 **유일한 발견자**
...
> ⚰️ 🆕⑬ 🔴 **[2026-08-29 재측정] 엔트리는 «7»이고 구성이 바뀜다.** 실측(`3b77fbc3` 의 `build.rollupOptions.input`) — `main`(index.html) · `admin` · `map_editor` · `map_editor2` · `graph` · `trace` · 🆕 **`rnd_board`**(rnd-board.html — [§7-B](#7-b-rd-보드-rnd_board-2026-08-29-신설-등재)). 🔴 **`ledger`·`rnd_console`·`ledger_graph` 셋은 삭제됐다**(은퇴가 아니라 삭제 — `vite.config.js` 주석이 소유자 판정 「ㅇㅇ 버려」를 적고 있다). 아래는 그것들이 살아 있던 때의 서술이다: ~~🆕⑨ **[2026-08-18 재측정] 엔트리는 7이 아니라 «9»다**~~. 실측(`5359fdd`의 `build.rollupOptions.input`) — `main`(index.html) · `admin` · `map_editor` · `map_editor2` · `graph` · `trace` · `ledger` · 🆕 **`rnd_console`**(rnd-console.html) · 🆕 **`ledger_graph`**(ledger-graph.html). 🔴 **뒤의 둘은 이 지도에 «화면도 모듈도» 등재된 적이 없다** — 소비 모듈은 [§5-I](#5-i-미등재-모듈-레지스트리-2026-08-18-실측)에 존재·크기만 올렸다. ⚠️ **`graph`와 `trace` 엔트리는 살아 있으나 그 서버 라우트는 410으로 은퇴했다**([§1.5](#15-그래프-조회-구간은-410으로-은퇴했다)) — **이 패스는 두 화면이 의도적 잔존인지 잔재인지 판정하지 않았다**(클라 도메인 소관).
