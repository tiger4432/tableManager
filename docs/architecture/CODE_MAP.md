...
| 🆕 **`VERDICT_BROKEN_CHILD = "broken_child"` / `VERDICT_PORT_CONFLICT = "port_conflict"`** | **종단 판정 어휘 2종.** 스냅샷의 `"terminal_verdict"` 키(**977**)와 이벤트 스트림(`_record(child, "permanently_failed", verdict=…)` **728**)에 실린다. ⚠️ **현재 `main.py`·`health.py`에 이 값으로 분기하는 독자가 없다** — 소비자는 `tests/test_duplicate_launcher.py`뿐이다(선언은 있고 화면이 아직 안 읽는다) | **184 / 185** |
