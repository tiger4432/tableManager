...
| 🆕⑳ 🔒 GET·POST `/admin/tables/config/raw` · GET·POST `/admin/chain/rules/raw` | `get_table_config_raw`/`post_table_config_raw` · `get_chain_rule_raw`/`post_chain_rule_raw` | 표 «하나»·규칙 «하나» 단위의 원시 선언 편집(원자적 쓰기 + 저장 전 검증 + base 지문). 🆕⑱ 가 계수 블록에서 이름만 세고 이 표에 안 실었던 넷이다. ⚠️ **이 패스는 존재·핸들러명·게이트만 실측했고 본체를 읽지 않았다** — 소비자는 [§5-H-bis 클라 부품](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재)의 `raw_registry_panel.js` 계열이다 |
...
| 🆕㉖ `_declared_entities()` :409 · 같은 `/subgraph` 라우트의 **`format`** 가지 :141·:211·:216 | **새 라우트가 «없다» — 자란 것은 «인자»다**(S-183). `format` 은 `json`·`rows` 둘뿐이고 그 밖은 거절이며, `rows` 면 `PlainTextResponse` 로 나간다. ⚠️ **선언된 컬럼을 채우려면 엔터티 카탈로그가 필요하고** 그것을 이 함수가 준다 — 라이브 거절 둘이 그것을 가르쳐 줬다(`aae64301` 라우트 가지에 시험이 없어 `NameError` 출하 · `f493eb0d` 픽스처가 제품 모양이 아니라 선언 컬럼 0) |
