...
| 🆕㉔ **`refuse_virtual_join_duplicates(db, table_name, ...)`** :3891 | **행 «하나»의 중복 조인 키가 표 전체의 체인을 멈추지 않게 «그 행만» 이름 대어 거절한다**(S-174 `a50d7328`). 📌 부류는 「거절이 «비싸면» 거절이 사고가 된다」 — 같은 밤의 재전개 이분(사건 1,000 → 21)과 같은 설계다 |
...
| 🆕 **`failure_cause(error_reason) -> str`** | **[`90d971ba` S-248] 기록된 실패에서 운영자가 읽을 «한 문장» — «마지막» 비어 있지 않은 줄.** 트레이스백의 첫 줄은 「Traceback (most recent call last):」라 종전 `permanently failed` 로그 한 줄이 «아무것도» 말하지 않았다(박스 실측 09-15: 하루 종일 「매번 다른 행에서 permanently failed」, 원인은 보이지 않는 인덱스). 한 줄짜리 사유는 «그대로». 소비자 `process_pending_groups` :2319(`… -> FAILED. 원인: %s`) | :625 |
...
| 🆕 **후속 랩 «한 원인 한 번»** — `_FOLLOWUP_SERVED = OrderedDict()` :2612 · `MAX_REMEMBERED_CAUSES = 512` :2613 · **`followup_already_served(transaction_id, table, rule_name, row_ids) -> list`** :2616 · `forget_followups()` :2647 | **[`b1e89e36` S-249 ⓒ]** 이 규칙이 이 «원인(origin tx)»에 대해 «아직 못 받은» 행만 돌려주고 나머지를 기억한다 — 키는 `(tx) -> {(table, rule): frozenset(row_ids)}`, 상한 512 원인(가장 오랜 것부터 «퇴거»). ⚠️ **tx 가 없으면 기억 «안 한다»** — 소급·백필은 tx 없이 줄을 서고, 그것을 한 키에 묶으면 둘째 백필이 첫째의 «반복»으로 보인다. ⚠️ «다른 원인»으로 같은 행을 다시 보는 것은 막지 않는다(그것이 정상이고 키가 «행이 아니라 tx»인 이유). `forget_followups` 호출자: 시험뿐 | :2612~:2650 |
| 🆕 **`log_followup_folded(logger_, rule_name, table, rows_in, written, woke_by, hop, max_hop, refusal=None)`** · `_FOLLOWUP_SAID` :2654 · `FOLLOWUP_LOG_EVERY = 500` :2655 · `forget_followup_lines()` :2689 | **[`6edf4db4` S-249 ⓔ-2] 후속 랩의 «한 줄»에 «무엇이 깨웠나»(`woke_by=<table>#<tx>`)와 홉(`hop=n/max`)이 실린다.** 🔴 **쓴 것이 «0»이고 거절도 없으면 DEBUG** — 할 일 없는 후속은 매 랩의 «보통»이라 INFO 면 일한 랩을 가린다. INFO 는 «첫 줄 + 500 마다 한 줄»(`(xN)` 접미), `(rule, table)` 키 2,000 을 넘으면 «비운다»(누수보다 재계수). `log_failure_folded` 와 «같은 손» | :2658 |
| 🆕 **`_run_builtin_followups(db, done)`** — 🔴 **랩이 «홉»이다** | **[`b1e89e36`·`6edf4db4` S-249 ⓒ·ⓔ-1]** `done` 에서 `chain_depth` 를 읽어 **`request_chain_depth.set((incoming or 0) + 1)`** :2730 로 감싼다(`finally` 로 reset) — 종전엔 `request_chain_depth` 가 «그룹 스텝 한 자리»에서만 세팅됐고 이 드레인은 «자기 스레드»라 여기서 쓴 것은 홉이 «없어» `max_chain_depth` 를 «영원히» 못 만났다(한 천장, 두 답). 규칙마다 ① `followup_already_served(done["transaction_id"], table, rule name, row_ids)` :2739 로 «신선한» 행만 — 없으면 «건너뜀» 한 줄 ② **`with outbox_mode(event_constants.OUTBOX_MODE_COLLAPSED)`** :2751 안에서 `builtins.run_builtin(kind, db, rule, row_ids=fresh, done=done)` :2752 — 행별 이벤트 1,000 이 outbox 1,000 · 큐 1,000 · 랩 1,000 · 줄 1,000 이 되던 것(소유자 「한 행당 로그 하나」)을 그룹 경로와 «같은 컨텍스트 매니저»로 접는다 ③ `log_followup_folded(...)` :2757. ⚠️ 여전히 «격리» — 실패는 원장 후속을 «못 깨뜨린다». 시험 `test_the_follow_up_lap_is_a_hop_and_serves_one_cause_once.py`(280줄 / `def test_` 16) | :2693 |
...
### 4-ter-b. `server/chain/rule_shape.py` (🆕 **374줄** @`dc877746` — 325 @`474f1aa9`, 구 표기 271 · `b7b5010f` 신설 때 109, S-234 2단계)

| | |
|---|---|
| 무엇 | 세 문법을 담는 «내부 규칙» 하나 + 어댑터 둘(`from_chain_rule` :36/`as_chain_rule` :58 · `from_join_rule` :104/`as_join_rule` :123) + 새 문법의 입출력(`to_declaration`/`from_declaration` :142·:162) |
| 🆕 **join 종류의 동반자** (S-237, 이 범위 «전»부터 있었으나 미등재) | `companion_rules(internal)` :191 — 조인 선언 «하나»가 규칙 «둘»(왼쪽 트리거 + `:reference` 접미의 오른쪽 트리거, `REFERENCE_SUFFIX` :188)이 되는 자리 · `class JoinTriggerConflict(ValueError)` :224 · `join_trigger_columns` :231 · `refuse_join_trigger_conflict` :240 · `unknown_join_cells` :260(:270 이 `join_into.unknown_cells` 를 부른다) |
| 🆕 **`decide` 종류** (`dc29056a` S-239 · 판정 399~401) | `DECIDE_CELLS` :276 = `key · fields · list_columns · aggregations · reference_views · auto_confirm · alignment`(🔴 `auto_confirm_declared` 는 칸이 «아니다» — 판정 401: 「적었나」는 키의 «존재»에서 유도된다, `enabled_written` 과 같은 부류) · `_DECIDE_TO_ENRICHMENT` :280(`key→decision_key` · `fields→target_fields` — 새 문법 칸을 인리치 어휘 철자로) · **`decide_rules(internal, known_tables=None) -> (rules, refusal)`** :283 — 🔴 **번역기가 아니라 «문»이다**: 칸 이름만 바꿔 `enrichment.config.chain_rules_from_cells` 로 «내려보낸다». 확장기는 그쪽 «하나»고 이 파일은 둘째 확장기를 «만들지 않는다» · `unknown_decide_cells(internal)` :307(이름 대고 «거절 안 함») · **`is_switched_off(internal) -> bool`** :316 = `enabled_written and enabled is False` — 🔴 **「적힌 `false`」만 OFF 다**. 없으면 ON, truthy 면 ON(옛 껍데기가 읽던 그대로). 워커는 이것을 «맨 먼저» 물어 참이면 «아무것도» 안 한다(§4 `load_chain_rules` 행 — 🆕 이제 아래 `expand_declaration` «안»에서) |
| 🆕 **`expand_declaration(declaration, table_config=None) -> (rules, refusal, notes)`** :328 (`00da7e91` S-244) | 🔴 **저자 «하나», 부르는 자 «둘»** — 규칙 파일 항목 하나 → 「그것이 세우는 체인 규칙들」. 로더(`ingestion_worker.load_chain_rules` :740)와 저장 관문(`ledger/admin.save_chain_rule_raw` :691·:720)이 «같은 함수»를 «같은 카탈로그»로 부른다 — 종전엔 로더는 «번역한 뒤» 심판하고 관문은 «날것»을 심판해, `POST /admin/chain/rules/raw` 가 로더가 «받는» 선언을 세 문장(`trigger_table` 없음 · `derive` 모름 · `unresolvable_mapper`)으로 거절했다(S-204 「저장 관문과 로더가 같은 판정자」가 «입력»에서 조용히 거짓이 됐던 것 — 같은 판정자, 다른 입력은 판정자 둘과 같은 결함). 결과 «셋»: ① `derive` 가 dict 가 아니면 `([declaration], None, [])` — **옛 평면 규칙은 «손 안 대고» 돌아온다**(옛 저장 경로가 바이트 그대로인 이유) ② `is_switched_off` → `([], None, [note])` — 🔴 **OFF 는 거절이 «아니다»**(판정 399) ③ `decide_rules` 거절 / `JoinTriggerConflict` → `([], "<name>: …", [])` · 아니면 `decided` 또는 `[as_chain_rule] + companion_rules` 와 «모르는 칸» 노트. 시험 `test_the_save_gate_and_the_loader_read_one_declaration_alike.py`(230줄 / `def test_` 9) |
...
| 시험 | `test_the_internal_rule_gives_back_exactly_what_it_was_given.py` · `test_a_declared_join_writes_what_it_says_into_the_table.py` · `test_enrichment.py` · 🆕 `test_an_enrich_declaration_reads_the_same_in_either_grammar.py`(`dc29056a`, 11건 — 두 문법이 «칸 대 칸» 같은 규칙, OFF 통합 선언은 `ensure_once` «호출 0», `ASSY_CHAIN_SYNTHESIZE=0` 이 통합 선언을 «못 끈다») |

### 4-ter-c. `server/chain/join_into.py` (🆕 **288줄** @`dc877746` — 272 @`474f1aa9`, 구 표기 260(`7f3c3634` 신설 때 259), S-237 · 판정 396~398)
...
| 🆕 **행 단위 팬아웃 그물** (`593aac50` S-237) | `_write(db, left_table, rows, spec, source_name)` :167 안 — LEFT JOIN 결과에서 `row_id` 가 «두 번 이상» 나온 왼쪽 행은 **어느 답도 «그 답»이 아니므로 «어느 것도» 안 쓴다**. 그 행들을 «이름으로» 건너뛰고(배치당 경고 «한 줄», 첫 열 개 id) 나머지는 쓴다 — 나쁜 키 하나가 배치를 «못 죽인다»(지배 원칙 2). ⚠️ **카탈로그 게이트(좁은 키/인덱스 없음)는 시도했다가 뺐다** — `virtual_join` 을 import 해야 했고 그것을 경계 시험(`test_this_module_does_not_borrow_the_read_time_executor`)이 막는다. 그물은 카탈로그가 «필요 없다». 로그 줄 모양은 `RUN.md` 가 든다 |
| 🔴 **S-240 사실 — 오늘 `key.unique` 를 읽는 자가 «없다»** (2026-09-15 실측, 큐) | 통합 join 선언의 `key: {columns, unique: true}` 를 `server/chain` 의 «아무도» 읽지 않는다 — `rule_shape.from_declaration` :162 는 `name/enabled/on/derive/into/limits/extra` 만 나르고 최상위 `key` 는 «떨어진다»(`extra` 에도 안 든다 — `extra` 는 `raw["extra"]` 에서만 읽는다). `unique_key.ensure_once` 의 호출자는 `virtual_join/config.py` :852 «하나»(`git grep ensure_once -- server/chain` = 0). 즉 «쓰기 시점 조인»은 오른쪽 유일성을 «세우지도 검사하지도» 않는다 — 위 그물이 «팬아웃이 났을 때» 잡을 뿐이다. 이 칸을 로더가 읽어 S-235 의 손으로 인덱스를 세우는 것이 S-240 이고 **아직 착지 전**이다 |
| 🆕 **`CHAIN_LAYER = "chain_ingestion"`** :34 — 쓰기의 «층» (`1aa50d3d`) | 🔴 **층은 «체인의 것», 저자는 «규칙의 것».** `_write(db, left_table, rows, spec, source_name)` :178 이 `crud.apply_batch_updates` 에 `source_name=CHAIN_LAYER` · `updated_by=<규칙 이름>` 으로 넘긴다. 첫 판은 운영자가 「누가 이 셀을 썼나」를 보게 하려고 «규칙 이름을 층에» 넣었는데, 그 이름이 워커의 «깨움 필터»가 읽는 것이었다 — `chain_ingestion` 이 아닌 층의 쓰기는 그 표의 «모든» 규칙을 깨우므로 이 조인의 쓰기가 «자기를 먹이는 enrich 를 다시 깨웠다»(핑퐁, 소유자 「핑퐁은 제대로 고쳐」). 체인 쓰기의 층이 «하나»여야 `allow_chain_trigger` 옵트인이 체인 쓰기가 규칙을 깨우는 «유일한» 길로 남는다. `updated_by` 는 규칙당 «상수»라 값이 안 바뀐 쓰기는 여전히 no-op |
| 🆕 **`_folded(column, fold)`** :91 — 숫자 키는 «TEXT 로 접는다» (`fd53b87b`) | `coalesce(col, '')` 는 «텍스트 문장»이라 `number` 컬럼에서 PostgreSQL 이 「invalid input syntax for type double precision: ""」로 답했다 — 운영자가 «고르지 않은» 컬럼 타입 때문에 선언한 조인이 죽었다(소유자 「알아서 접어서 하라고」). `isinstance(column.type, String)` 이 아니면 `cast(column, Text)` 를 «양쪽»에, 텍스트 키는 «바이트 그대로»(어제 맞던 인덱스가 오늘도 맞도록). ⚠️ 접기 식이 유일 인덱스 식과 «일치»해야 한다 — 어긋나면 «실패가 아니라» 순차 스캔이 되고 시험은 초록이다 |
...
### 4-ter-d. `server/virtual_join/unique_key.py` (🆕 **462줄** @`dc877746` — 348 @`474f1aa9`, 구 표기 328(`d3a92648` 신설 때 224), S-235)

| | |
|---|---|
| 무엇 | 선언이 `cardinality: one` 이라 말하면 **«제품이» 그 유일성을 세운다**. `inspect` :85 · `ensure` :134 · `duplicate_keys` :58 · `invalid_leftovers` :37 · `describe` :102 |
| 🆕 **프로세스당 규칙당 «한 번»** (`1497ea3e`, 2026-09-15 장애 다섯째) | `_TRIED = {}` :192 · **`ensure_once(db, rule_name, table, columns, folds=None) -> dict`** :195 — 🔴 **스위치 `ASSY_VJOIN_AUTO_INDEX` 가 OFF(`0/false/off/no`)면 «맨 보고»를 돌려주고 DB 를 «안 건드린다»** — 첫 판은 OFF 에서도 `inspect()` 를 불렀고, 그 GROUP BY 가 숫자 키에서 «읽기 경로»의 트랜잭션을 abort 시켜 «운영자가 잡은 스위치가 아무것도 안 바꿨다». ON 이면 `ensure(apply=True)` 를 `try` 로 감싼다: **프로브가 던지면 `db.rollback()` 하고 `state: "probe_failed"` 로 `_TRIED` 에 «기억»** — 한 번 시도하고 «다시는» 안 한다(이번 실행엔 그냥 «자동 수리 없음», 홍수 없음). `forget(rule_name=None)` :355(@`dc877746` — 구 표기 :241) 이 그 기억을 지운다 — ⚠️ **호출자 «0»**(전건, 시험 포함) |
| 🆕 **인덱스는 «그것을 요구하는 조인만큼» 산다** (`90d971ba` S-248) — `_RETRACTED = {}` :244 · **`product_indexes(db) -> [(table, index)]`** :247 · **`retract_unrequired_once(db, required) -> dict`** :270 · `forget_retractions()` :350 | 박스 실측 09-15: S-235 가 조인이 살아 있을 때 `uq_vjoin_dt_inventory_…` 를 세웠고, 그 조인이 거절·이관·OFF 된 뒤 «인덱스만» 남았다. `dedup` 이 충돌하는 재고 행을 넣자 23505 — 그룹이 «영구 실패», 재시도마다 같은 실패. ⛔ **쓰기 관문은 그것을 «못 본다»** — `crud.refuse_virtual_join_duplicates` 는 «검증된 규칙»의 키만 알므로 규칙이 사라진 인덱스는 «관문 밖에서» 문다. `product_indexes` 는 `pg_index` 를 `config.INDEX_PREFIX`(`uq_vjoin_` :128 — 철자를 «다시 적지 않는다») 접두로 읽는다 — **접두가 안전의 «전부»**: 운영자가 «자기 이름»으로 세운 인덱스는 «절대» 회수되지 않는다. `retract_unrequired_once`: 🔴 `ASSY_VJOIN_AUTO_INDEX` OFF 면 «DB 무접촉»(`ensure_once` 가 배운 문장 그대로) · PostgreSQL 이 아니면 «건너뜀» · 목록 프로브가 던지면 `rollback` + 건너뜀 · 요구 집합에 없는 것을 `DROP INDEX CONCURRENTLY IF EXISTS` 로 «자기 AUTOCOMMIT 커넥션»에서 하나씩(하나의 실패가 나머지를 «못 막는다») · 보고 `{dropped, kept, skipped}` 를 «요구 집합(frozenset)당 한 번» 기억. 호출자 `virtual_join/config.load_verified_rules` :896 «하나»(`path is None` 일 때만 — §5-C). `forget_retractions` 호출자: 시험뿐. 시험 `test_a_product_index_lives_as_long_as_the_join_that_needs_it.py`(218줄 / `def test_` 8) |
| 🆕 이 범위 «전»부터 있었으나 미등재 | `MAX_PLANNED_KEYS = 50` :363 · `fold_plan` :366 · `describe_fold_plan` :420 · **`narrower_than_identity(table, columns, known_tables=None)`** :442(전부 @`dc877746` — 구 표기 :249/:252/:306/:328) — 조인 키가 그 표의 «신원»보다 좁으면 유일 인덱스는 «영원히» 못 선다는 것을 «데이터 한 행 안 읽고» 안다. 호출자 `virtual_join/config.py` :842(`ensure_once` :852 «앞»에서 묻는다) |
...
### `server/virtual_join/config.py` (🆕 **1,021줄** @`dc877746` — 999 @`474f1aa9`, 🆕⑨ 697 @`5359fdd`, 구 표기 687 · `4e06eec` 신설 540줄 → `b6942ec`로 **475**) — 가상 조인 선언 로더/검증
...
🆕 **[`90d971ba` S-248] `load_verified_rules(db, path=None, …)` :796 이 검증을 마친 뒤 «제품 인덱스를 회수»한다** — `path is None` 일 때만 `unique_key.retract_unrequired_once(db, {r["unique_index"] for r in verified if r.get("unique_index")})` :896 을 `try` 로 부른다(§4-ter-d). ⚠️ **기본 선언 파일일 때만**인 이유: `path` 를 준 호출자(시험·보고서)는 «부분 목록»을 읽고 있고, 부분 목록에서 계산한 「요구됨」은 진짜 조인이 «아직 쓰는» 인덱스를 걷어낸다. ⛔ **적재를 «절대» 못 깨뜨린다** — 무엇이 나든 `warning` 한 줄 후 `verified` 를 돌려준다.
...
| **`load_verified_rules(db, path=None, known_tables=None, rejections=None)`** | **이 파일이 보장하는 것**: 여기서 돌려준 선언은 **오른쪽이 조인 키로 유일함이 데이터베이스에 의해 강제된다.** 스냅샷이 아니라 제약이므로 이후의 쓰기가 깰 수 없다.<br>🆕 **[`e88cb2be`, 2026-09-15 장애 다섯째] 선언 «하나»의 실패는 «그 규칙만» 거절한다** — `verify_uniqueness(db, rule)` :771 호출이 `try` «밖»이어서 카탈로그 질의 하나가 던지면 «모든 표»의 조인이 같이 내려갔다(「모든 선언 무조건 다 돌면서 다 막아버렸네」). 이제 규칙마다 `try/except`: 던지면 `db.rollback()` → `_say_once(("verify", name))` → `_record(code=CODE_SHAPE)` → `continue`. 다음 규칙의 질의가 «abort 된 트랜잭션»의 답을 받지 않는다.<br>🆕 **[S-235 · 2026-09-15] 거절하기 «전»에 제품이 «한 번» 세워 본다** — `refused` 면 ① `unique_key.narrower_than_identity(right_table, right_columns, known_tables)` :839 — 키가 신원보다 «좁으면» `_say_once(("narrow", …))` 뒤 `_NarrowKey` 로 «만들어 보지 않는다» ② **`unique_key.ensure_once(db, name, right_table, right_columns, right_folds)` :852**(⚠️ `git grep` 전건에서 «유일한» 호출자 — S-240) ③ `created` 면 `verify_uniqueness` 를 «다시» 묻는다. 그래도 `refused` 면 `_say_once(("unique", …))` :863 + `CODE_NO_UNIQUE_INDEX`. ⚠️ **어느 세션이 오나** — `executor._verified_by_left_table` 은 이제 «자기 `SessionLocal()`» 을 넘긴다(§5-D). 읽는 쪽의 세션은 여기까지 «안 온다» | ~~**~401**~~ 🆕 **:796** @`474f1aa9` |
...
### 🆕㉘ `server/chain/builtins.py` (**177줄** @`dc877746` — 구 표기 171, S-189 ⓒ `1964c65a` 신설 · S-195 에서 120→171) — 합성 «한 자리» + `builtin:` 종류 표
...
### `server/virtual_join/executor.py` (🆕 **889줄** @`474f1aa9` — 🆕⑨ 631 @`5359fdd`, 구 표기 584 · `ed9cfdb` 535에서 **+19**) — 가상 조인의 **실행** 절반
