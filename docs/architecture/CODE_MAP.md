...
> # ㉓-d 원장 라우트는 **정확히 2**여야 한다(`/subgraph` · `/declaration`).
> #    🔴 이 수가 늘면 «키를 받는 데이터 라우트가 다시 생겼다»는 뜻일 수 있고, 그것이
> #    개정 6 이 없앱 모양이다. 실측 `3b77fbc3`: **2** (`bd0c192e` 에서는 16).
...
> 🆕⑮ 🔴 **[2026-08-30 실측] 이 파일은 `5a73021a`(HEAD)에서 **5,860줄**이다** — 직전 `2a4e51cf` 의 5,962 에서 **−102**이고, 그 감소는 전부 `5db3b80c` 가 리로드 두 함수의 «몸»을 [`server/system_reload.py`](#19-serversystem_reloadpy--리로드가-엔트리포인트-밖으로-나간-자리-5db3b80c-신설) 로 옮긴 것이다. ⚠️ **이 패스가 잰 것은 줄 수와 리로드 블록·`startup_event` 뿐이다** — §1.2~§1.7 의 라우트 표·심볼 표는 열지 않았다.
>
...
### `server/database/models.py` (🆕⑨ **1,079줄** @`5359fdd` blob `fbe4bde` — 제목의 636줄은 세 세대 낡은 값이었다) — ORM + 동적 모델/런타임 DDL
정적 ORM 클래스 **9종**과 config 주도 동적 테이블 관리 함수. 🆕⑨ **[2026-08-18 실측] 클래스 목록이 종전 등재보다 둘 많다** — `AuditLog` · `InteractionEffortLog` · `DatabaseOutbox` · `FileIngestionLog` · `FileIngestionCheckpoint` · `CellOverwrite` · `CellSource` · **`FrameConfirmation`** · **`FrameConfirmationSource`**(뒤의 둘은 이 절에 등재된 적이 없다 — 확정 스키마 자체의 서술은 [§5 `frame_confirmation.py`](#-serverframe_confirmationpy--확정의-기록자)와 `migrations/add_frame_confirmation.py`에 있다). **그래프 3모델은 2026-08-18에 제거됐습니다**(아래 취소선 행). 모듈 레벨에 `mapper_registry` · `_KEY_TIERS` · **`declared_key_columns(table_cfg)`**(미등재)도 있다.
...
### `server/retroactive.py` (**545줄**(🆕⑨ `5359fdd` 실측 — 구 표기 697), 신설) — 소급 적용의 **레지스트리**
...
**무엇인가.** `lot_event` 같은 기존 소스 테이블을 **읽어** 봉투 7필드짜리 원자(atom)로 번역해 `ledger_events`에 **추가만** 하는 계보 원장. 쓰기 경로(`crud.apply_batch_updates`·`cell_sources`·체인·그래프)는 **한 줄도 바뀌지 않았다** — 원장은 기존 시스템의 **소비자**로 태어난다. ⚰️ 🆕⑬ **[2026-08-29] 그 콘솔은 없다.** 읽기에 답하는 것은 **`GET /api/ledger/subgraph` 하나**이고 그 옆에 선언 카탈로그 `GET /api/ledger/declaration` 이 서 있다(라우트 전건 계수 = **2**, `3b77fbc3` 실측). ~~🆕⑦ 읽기 측은 이제 라우트 하나가 아니라 **원장 콘솔**이다~~ — 계보 추적(`/trace`) 옆에 관측 종류 카탈로그(`/kinds`) · 형제 대조(`/siblings`) · 온톨로지 구조(`/structure`) · 랏 격자(`/lots`) · 축별 맵 투영(`/lot_map`) · 커버리지(`/coverage`)가 섰다.
...
| `envelope.py` | 🆕⑩ **329** | 봉투 7필드의 파이썬 표현 + 타입 보존 |
...
| `profile_lookup_adapters.py` | **105** | (🆕⑩ 무변동) |
...
**🆕⑭ 라우트 전건 (`2a4e51cf` 실측 — `grep -c '^@router\.'` = **2**, 무변동)**
...
| `GET /declaration` | `ledger_declaration_catalog` | **원장을 한 줄도 읽지 않는다** — `ledger.config.load()`의 `entities`/`vocabulary`를 읽어 그대로 낸다. **DB 세션 인자 자체가 없다** |
...
> 🆕⑭ **[2026-08-29 심야 재측정 — `2a4e51cf`]** 줄 수는 `git show HEAD:<path> | wc -l`, 개수는 `grep -c "def test_"`. 🆕⑬ 이 `3b77fbc3` 에서 잰 아홉 중 **셋이 그 사이에 자랐다**(`l1_unit` +10 · `trace_contract` +19 · `subgraph` +76/+2). 나머지 여섯은 무변동이다.

| 파일 | 줄 | `def test_` | 구 표기 |
|---|---|---|---|
| `test_ledger_l1_unit.py` | 🆕⑭ **879** | **34** | ~~869~~ / ~~1,530 / 61~~ |
| `test_ledger_l1_pg.py` | **1,606** | **32** | ~~1,126 / 22~~ |
| `test_ledger_trace.py` | **431** | **11** | ~~898 / 38~~ |
| `test_ledger_trace_contract.py` | 🆕⑭ **422** | **8** | ~~403~~ / ~~476 / 14~~ |
| `test_ledger_trace_pg.py` | **1,630** | **47** | ~~1,658 / 48~~ |
| `test_finding_kinds.py` | **166** | **7** | 165 / 7 |
| `test_ledger_observed_unit.py` | **91** | **2** | ~~330 / 14~~ |
| `test_ledger_transfer_unit.py` | **209** | **10** | ~~500 / 26~~ |
| 🆕⑭ `test_ledger_subgraph.py` | **786** @`3a8494dc` | **19** | ~~576 / 14~~ @`2a4e51cf` · ~~500 / 12~~ |
...
### `map_editor.js` (**11,060줄**(🆕⑨ `5359fdd` 실측 — 구 표기 11,031) — `d3ed167` 10,866에서 **+165**. 🔴 **코드는 6,391줄이다** — 주석 3,858 + 공백 782) — 웨이퍼 맵 에디터 (단일 페이지 스크립트, export 없음)
...
### `admin.js` (**3,814줄**(🆕⑨ `5359fdd` 실측 — 구 표기 3,704) — `1dc761b` 3,202에서 **+502**, 소급 적용 패널) — 어드민 페이지 (2026-07-25 전면 재작성 — 파이프라인 5탭, export 없음)
