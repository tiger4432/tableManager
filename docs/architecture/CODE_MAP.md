...
| 🆕⑭ `@router.get("/subgraph") evidence_subgraph(node_id=Query(..., alias="id"), hops=12, direction="both", node_limit=400, edge_limit=1200, positive=None, negative=None, follow=None, backbone_hops=ledger_subgraph.DEFAULT_BACKBONE_HOPS, db)` | **인자 아홉**(수는 그대로, **아홉째의 이름이 바뀌었다**). `hops` `ge=1 le=40` · `direction` `^(outgoing\|incoming\|both)$` · `node_limit` `ge=10 le=1000` · `edge_limit` **`ge=20, le=ledger_subgraph.MAX_EDGE_LIMIT`**(= **6000**, 구 표기 3000 — 라우터가 수를 옮겨 적는 대신 **상수를 참조**하게 바뀌었다) · `backbone_hops` `ge=0 le=40`. `positive`/`negative`/`follow`는 **반복 쿼리 파라미터**다. 🔴 **`continues_hops` 는 없다** — 2026-08-29 저녁에 `backbone_hops` 로 개명됐고 **옛 철자의 별칭을 받지 않는다**(소스 주석: 소비자가 둘 다 우리 것이라 호환층은 걷을 사람이 없어진다). 🔴 **`enrich_actions`·`collect`·`observations`·`include_values`·`shape`·`property_limit`은 없다** |
