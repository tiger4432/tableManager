...
| 🆕㉖ `_declared_entities()` :409 · 같은 `/subgraph` 라우트의 **`format`** 가지 :141·:211·:216 | **새 라우트가 «없다» — 자란 것은 «인자»다**(S-183). `format` 은 `json`·`rows` 둘뿐이고 그 밖은 거절이며, `rows` 면 `PlainTextResponse` 로 나간다. ⚠️ **선언된 컬럼을 채우려면 엔터티 카탈로그가 필요하고** 그것을 이 함수가 준다 — 라이브 거절 둘이 그것을 가르쳐 줬다(`aae64301` 라우트 가지에 시험이 없어 `NameError` 출하 · `f493eb0d` 픽스처가 제품 모양이 아니라 선언 컬럼 0)<br>🆕㉜ **[S-206 `df7de5c6`] 발효가 걷기로 하여금 선언을 «다시 읽게» 한다 — 재기동이 아니라.** 🔴 그 캐시가 «리로드 좌석에 안 앉아» 있었고, 부류는 D-15 에 적힌 그대로다: **리셋 없는 캐시는 「리로드가 아무 뜻도 없게 되는」 이유**다. 이 프로세스엔 그 좌석이 이미 있었다 |
...
| 🆕㉜ `AGGREGATE_MEASURES` :59 (`count`·`distinct`·`sum`·`mean`·`min`·`max`·`median`) · `NUMERIC_MEASURES` :63 · `_fold(measure, values)` :95 · `group_by` 가 이름 댈 수 있는 것 :65 | **걷기가 «자기 모집단»을 접는다**(S-146 `a4d80302`, 판정 331) — 그래서 **수가 예산을 «살아남는다»**. 🔴 그것이 이 인자의 존재 이유다: 클라가 세려면 걷기가 «전부» 실어 와야 하고, 큰 규모에서 그것은 「세려다 잘린다」가 된다. 접는 자리를 옮기면 «잘린 표»가 아니라 «옳은 수»가 온다. 🔵 측정자는 «일곱이고 그 밖은 없다»(소스가 그렇게 적는다) — 열린 목록이면 화면마다 다른 것을 세기 시작한다. ⛔ **못 받는 이름은 «이름 대어» 거절한다** — 철자 틀린 measure 가 조용히 떨어지면 그 수는 «다른 수»이고 아무도 모른다. 숫자를 접는 측정자에 숫자 아닌 값이 오면 `measure_needs_numbers` 로 이름 댄다. 📎 계약 `contracts/walk_aggregate/` |
