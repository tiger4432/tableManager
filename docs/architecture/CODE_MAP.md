...
> 🆕 **[2026-09-15 실측 · `474f1aa9`] 이 파일은 **3,669줄**이다** — 아래 1,886 은 09-05 값이고 S-214 행의 3,166 은 09-12 값이다. `load_chain_rules` 는 **:682** 에 있고 그 «안»의 자리는 아래 표의 그 행에 적었다. 라인이 아니라 «이름»으로 찾아라.
>
...
| `load_chain_rules()` | chain_rules 설정 로드(+enrichment 룰 병합). **`chain_replay.load_rules`가 이 함수를 그대로 부른다** — 재생이 라이브 룰 집합을 정확히 그대로 보게 하려는 것 | ~292<br>🆕㉛ **[S-201 `789e9ee2`·`174346f2`, 판정 316] `read_rules_document(path=None)` — 「그 파일을 읽는」 «무상태 독자» 하나.** `/admin/chain/rules`(`main.py`)가 자기 `open` 을 버리고 이것을 지난다. 🔴 **리스트 형태 파일은 «로더와 같은 방식»으로 거절한다** — 새 독자가 자기 거절을 쓰면 운영자가 같은 실수에 «두 문장»을 받는다<br>🆕 **[2026-09-15 실측 `474f1aa9` · `dc29056a` S-239] `def load_chain_rules()` :682 — 그 «안»의 순서, 이름으로:** ① `read_rules_document()` → ② **통합 문법 번역 블록** — `derive` «한 칸»이 있는 규칙만 `rule_shape.from_declaration` :722 를 지난다. 🔴 **`is_switched_off(internal)` :723 이 «맨 먼저»다** — 참이면 `continue`: **규칙이 «하나도» 서지 않고 그 아래 줄은 «안 돈다»**(DB 무접촉 — 거절 목록에도 «안 센다», 운영자가 «일부러» 끈 것이라). 그다음 `decide_rules(internal, crud.TABLE_CONFIG)` :731 — `decide` 종류면 여기서 규칙 «둘»(dedup·auto-confirm)이 나와 `translated` 에 들고, 모르는 하위 칸은 `unknown_decide_cells` :737 로 «이름 대고 돈다»; 거절문이 오면 `continue`. `join` 종류는 `refuse_join_trigger_conflict` :744 → `unknown_join_cells` → `as_chain_rule` + `companion_rules` :762(규칙 «둘»). ⚠️ **블록 전체가 `try` 안이다** — 새 문법을 «못 읽으면» 옛 모양 그대로 간다 → ③ `chain_bindings.rule_refusals` 문법 심판(파일 규칙만) → ④ **합성** `builtins.synthesize_chain_rules` — 🔴 **`ASSY_CHAIN_SYNTHESIZE` 스위치는 «이 블록만» 끈다**(주어: 운영자가 «안 쓴» 파생 규칙). 통합 선언에는 «닿지 않는다» — 그쪽 스위치는 위 ②의 `enabled: false` 다(판정 399, ③′) → ⑤ **`rule_order.order_rules(rules)` :907**(고리는 `RuleCycleRefused` 를 «로그하고 순서는 그대로» — §4-ter-g) → ⑥ `rule_census` 사진 + 지난 적재와의 diff → ⑦ `_validate_chain_cascade_graph` · `_report_unwatchable_trigger_columns` · `activity.registry.seed_rules` |
...
### `server/chain/graph.py` (**373줄**, S-178) — 한 흐름, 네 선언을 «한 그림»으로
```
...
### 4-ter-b. `server/chain/rule_shape.py` (🆕 **325줄** @`474f1aa9` — 구 표기 271 · `b7b5010f` 신설 때 109, S-234 2단계)
...
| 누가 부르나 | `chain/ingestion_worker.py` · `scripts/preview_unified_declarations.py` (+시험). 🆕 이 파일이 «부르는» 쪽: `enrichment.config`(함수 안 지연 import — 순환을 피하려고) · `chain.join_into` |
...
### 4-ter-c. `server/chain/join_into.py` (🆕 **272줄** @`474f1aa9` — 구 표기 260(`7f3c3634` 신설 때 259), S-237 · 판정 396~398)
...
### 4-ter-d. `server/virtual_join/unique_key.py` (🆕 **348줄** @`474f1aa9` — 구 표기 328(`d3a92648` 신설 때 224), S-235)

| | |
|---|---|
| 무엇 | 선언이 `cardinality: one` 이라 말하면 **«제품이» 그 유일성을 세운다**. `inspect` :85 · `ensure` :134 · `duplicate_keys` :58 · `invalid_leftovers` :37 · `describe` :102 |
| 🆕 **프로세스당 규칙당 «한 번»** (`1497ea3e`, 2026-09-15 장애 다섯째) | `_TRIED = {}` :192 · **`ensure_once(db, rule_name, table, columns, folds=None) -> dict`** :195 — 🔴 **스위치 `ASSY_VJOIN_AUTO_INDEX` 가 OFF(`0/false/off/no`)면 «맨 보고»를 돌려주고 DB 를 «안 건드린다»** — 첫 판은 OFF 에서도 `inspect()` 를 불렀고, 그 GROUP BY 가 숫자 키에서 «읽기 경로»의 트랜잭션을 abort 시켜 «운영자가 잡은 스위치가 아무것도 안 바꿨다». ON 이면 `ensure(apply=True)` 를 `try` 로 감싼다: **프로브가 던지면 `db.rollback()` 하고 `state: "probe_failed"` 로 `_TRIED` 에 «기억»** — 한 번 시도하고 «다시는» 안 한다(이번 실행엔 그냥 «자동 수리 없음», 홍수 없음). `forget(rule_name=None)` :241 이 그 기억을 지운다 — ⚠️ **호출자 «0»**(전건, 시험 포함) |
| 🆕 이 범위 «전»부터 있었으나 미등재 | `MAX_PLANNED_KEYS = 50` :249 · `fold_plan` :252 · `describe_fold_plan` :306 · **`narrower_than_identity(table, columns, known_tables=None)`** :328 — 조인 키가 그 표의 «신원»보다 좁으면 유일 인덱스는 «영원히» 못 선다는 것을 «데이터 한 행 안 읽고» 안다. 호출자 `virtual_join/config.py` :839(`ensure_once` «앞»에서 묻는다) |
...
### 🆕 4-ter-g. `server/chain/rule_order.py` (**95줄** @`474f1aa9`, `c7bc59db` 신설 2026-09-13, S-156 · 판정 385·387·388) — ⚠️ 통합 라운드의 «여섯»에 들지 않는다. 미등재라 이 옆에 둔다

| | |
|---|---|
| 무엇 | 체인 규칙의 «순서» — 선언에서 «유도»된다, 새 칸이 아니다. `class RuleCycleRefused(ValueError)` :20 · **`order_rules(rules) -> list`** :25 — `trigger_table` ↔ 다른 규칙의 `target_table` 로 생산자→소비자 엣지를 긋고 후위 DFS 로 «생산자 먼저». 🔴 **재생이 늘 하던 유도인데 «읽는 자가 하나»였다** — 워커는 «파일 순서»로 돌았다(이 모양의 «넷째»: S-152·S-221·S-154) |
...
| 누가 부르나 | `chain/ingestion_worker.py` :907(`load_chain_rules` 끝 — 고리는 `RuleCycleRefused` 를 «로그하고 순서는 그대로», 체인을 «안 멈춘다») · `chain/replay.py` :159 `order_rules` 가 «감싸서» 같은 것을 부른다(:166) → `scripts/chain_replay_cli.py` :185 · `scripts/outbox_triage.py` :155 가 그 래퍼를 지난다 |
| 시험 | `test_the_worker_runs_rules_in_the_order_the_declaration_implies.py`(`ea8f91d2` 가 둘을 뒤집었다 — 조인의 페이싱 참조 규칙은 더는 「먼저」로 단언되지 않는다: 트리거 그룹을 «공유하지 않으므로» 앞뒤가 «뜻이 없다») |
...
### `server/virtual_join/config.py` (🆕 **999줄** @`474f1aa9` — 🆕⑨ 697 @`5359fdd`, 구 표기 687 · `4e06eec` 신설 540줄 → `b6942ec`로 **475**) — 가상 조인 선언 로더/검증
...
### 🆕㉘ `server/chain/builtins.py` (**171줄**, S-189 ⓒ `1964c65a` 신설 · S-195 에서 120→171) — 합성 «한 자리» + `builtin:` 종류 표
...
📐 **[S-195 ⓐ `c4aeb56c`] 디스패처가 «드레인 배치마다 규칙 파일을 다시 읽는» 것을 그만뒀다** — `chain_ingestion_worker._followup_builtin_rules()` :1682 가 «리로드당 한 번» 싣고 `_FOLLOWUP_BUILTIN_RULES` :1679 에 든다. 🔴 **무효화는 «다른 워커 캐시와 같은 자리»다**(`reload_worker_process_cache` :1694) — **리셋 없는 캐시는 「리로드가 아무 뜻도 없게 되는」 이유**이고, 이 프로세스에는 그 자리가 이미 하나 있다.
...
### `server/admin/retroactive.py` (🆕⑳ **1,502줄** @`328a5c20` — 🆕⑱ 1,441 @`6345ef3d` — 🆕⑰ 1,156 @`64b562b6` — 🆕⑯ 1,139 @`9eb30691` — 🆕⑨ 545 @`5359fdd`, 구 표기 697) — 소급 적용의 **레지스트리**
...
| 🔒 `server/ledger/admin.py` | 🆕⑪ **848**(구 표기 844) | **[동결 — A군]** admin으로 소스를 원장에 잇고 어휘를 늘린다 — **문법 검증과 저장(1단·3단)**. 판정 정본은 `docs/process/LEDGER_RULINGS.md` **R-2026-08-15-M** |
