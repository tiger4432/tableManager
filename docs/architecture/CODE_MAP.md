...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-04 (**소스 앵커는 `41b17ee`의 커밋된 blob 실측** — 워킹트리 아님. HEAD는 측정 중 `35b03cc`로 이동했고 그 두 커밋의 차이는 `docs/process/PROJECT_STATUS.md`와 `client2/scripts/check_harnesses.mjs`뿐이라 **소스 blob은 하나도 다르지 않다**) | **Owner:** 전 에이전트 공용 | **Source-of-truth:** 각 표의 코드 경로
>
...
>
> 🔴 **이번 범위는 `ed9cfdb`→`35b03cc`, 37커밋이다.** 성격은 **깔때기화(funnelling)**다 — 세 번의 서로 다른 라운드가 전부 **같은 형태의 결함**을 고쳤다: *두 곳이 같은 답을 내야 하는데 각자 철자하고 있었다.*
> - `main.get_column_filter_condition` → **`column_filter.py`**(워커에서 `import main`이 엉뚱한 파일에 바인딩됐다)
> - `virtual_join_executor`의 숫자 전용 캐스팅 → **`crud.column_text_sql`**(「어떤 타입이 캐스팅이 필요한가」를 「어떤 타입이 *이미* 텍스트인가」로 뒤집었다)
> - `transfer_plan`/`bonding_plan`의 역할 바인딩 인라인 철자 → **역할 상수 + `resolve_effective_columns` + `explain_binding_refusal`**
>
> ✅ **이번 범위에서 소스가 바뀐 파일은 20개, 신설 5개(소스만 — 테스트 15종·config 샘플 제외), 🔴 삭제 0개다.** `D` 라인은 전부 `client2/dist` 번들이다(**`D` 라인부터 훑는 규율은 이번에 값을 가르지 않았지만, 그것을 확인한 것 자체가 결과다**).
> - **서버 신설 3(모듈)**: 🆕 **`server/column_filter.py`(181줄)** — [§1.8](#1-8-servercolumn_filterpy--필터-dsl-번역기가-엔트리포인트-밖으로-나간-자리) · 🆕 **`server/notation_norm.py`(542줄)** · 🆕 **`server/graph_stale_edges.py`(549줄)** — [§5-E](#5-e-2026-08-042차-신설-서버-모듈-2종--표기-정규화--낡은-엣지-스윕) (+ CLI 2종)
> - **서버 변경(큰 것)**: `crud.py` **2,612 → 3,209**(+597) · `transfer_plan.py` **3,450 → 3,826**(+376) · `bonding_plan.py` **562 → 932**(+370) · `process_supervisor.py` **718 → 1,024**(+306) · `internal_event_client.py` **235 → 359** · `html_topology_parser.py` **638 → 768** · `config_resolve_report.py` **556 → 668** · `run_decoupled_app.py` **132 → 228** · `directory_watcher.py` **2,180 → 2,274** · 🔴 **`main.py` 5,595 → 5,488(−107 — 이 파일이 줄어든 첫 범위다)**
> - **클라 변경 3뿐**: `map_editor.js` **9,163 → 9,683**(+520) · `api.js` **456 → 485** · `state.js` **127 → 130**. **그 밖의 `client2/src/*`는 전부 blob 동일**이다.
>
> 🔴 **`map_editor.js`의 분해는 이번에 *파일 안으로* 향했다.** 직전 라운드는 두 모듈을 파일 **밖으로** 잘라냈고(`map_key.js`·`split_registry_row.js`), 이번 라운드는 세 거대 함수(`loadExistingMap` R4 · `resolveValidDie` R5 · `pushMapData` R6)를 **이름 붙은 단계 함수 17개**로 쪼갰다 — [§7 분해 표](#7-client2src--웹-클라이언트). 단계 함수는 파일 뒤쪽에 앉으므로 **§7의 후반 앵커가 최대 +495 밀렸고**, 그것이 이 패스에서 고친 가장 큰 덩어리다.
>
> 🔬 **「커밋된 blob 기준」 규율이 이번에 세 번 필요했다.** ① 서버 레인이 `server/database/crud.py`·`server/notation_norm.py`를 **측정 중에 워킹트리에서 편집**하고 있었다 — 그래서 그 둘은 **마지막에** 재고 **어느 blob을 쟀는지 §2에 못 박아 뒀다**(`crud.py` = blob `d637d91`). ② 측정 도중 HEAD가 `41b17ee` → `35b03cc`로 움직였다 — 조용히 화해시키지 않고 확인했더니 문서 1개와 빌드 스크립트 1개뿐이라 **소스 blob은 하나도 다르지 않았다.**
>
> 🔴 **③ 그리고 이 문서를 쓰는 동안 맵 레인이 10커밋을 더 착지시켰다(`35b03cc`→`c97b319`).** 그중 둘이 **`client2/src/map_editor.js`를 9,683 → 10,109줄(+426)**로 바꿨다(좌표표 붙여넣기 254줄 + 유효 다이 읽기 고정). **§7의 앵커는 `41b17ee` 기준 그대로 두고, 그 위에 [§7 머리의 오프셋 표](#7-client2src--웹-클라이언트)를 실측해 붙였다** — 절반만 옮긴 앵커 표는 어느 리비전에도 맞지 않으므로 **한 리비전에 정직하게 고정하고 차이를 옆에 적는 것**이 이 문서가 반복해 배운 답이다. 🔴 **다음 라운드의 첫 작업은 §7 `map_editor.js`를 `c97b319` 이후로 재측정하는 것이다.** 그 밖의 소스 파일은 `41b17ee`↔`c97b319`에서 **blob 동일**이다(`contracts/map_seam/*`와 `client2/map_editor.html` 제외).
...
> #       `resolveValidDie` 안 **8167–8168**)과 이 맵이 선언한 START를 🆕 **`diagnoseDesignationAlignment`(8429) 안 8459**에서 비교한다.
> #       구 헬퍼가 쓰던 "DB (0,0)을 두 프레임으로 투영해 비교"는 마스크 재중심화 **이전**의
> #       인덱스 공간에서 재는 값이라 화면과 무관한 수를 냈다(실측: 화면은 맞아 있는데
> #       「31칸·8행 어긋남」). 사유 주석은 `map_editor.js` **~8455–8458**(`diagnoseDesignationAlignment` 안)에 있다.
...
> #    `server/column_filter.py`([§1.8](#1-8-servercolumn_filterpy--필터-dsl-번역기가-엔트리포인트-밖으로-나간-자리)).
...
> - 🔴 **`crud.py`** 2,612 → **3,209**(+597). 신설 블록 **둘 다 파일 앞쪽**이라 뒤로 갈수록 크게 밀렸다: **+292**(버전 게이트 99–390) → **+500**(텍스트 렌더러 829–1036) → **+547** → **+584** → **+597**. `set_cell_manual_priority_batch` 2,165 → **2,762**.
...
| `server/chain_ingestion_worker.py` | **1,086** (1,065 → **+21**: 미배달 브로드캐스트 스윕이 **원천 테이블까지** 복구한다) | [§4](#4-serverchain_ingestion_workerpy--체인-워커) |
| 소형 서버 모듈 (models/std_parser/**enrichment_config**/enrichment_mapper/ingestion_activity/ingestion_checkpoint/**bonding_plan**/map_overlay/**transfer_plan**/map_meta_registrar/effort_metric) + 그래프 트랙(graph_sync_worker/graph_materializer/ontology_config) + **운영 6종**(paths/**process_supervisor**/health/heartbeat/**product_tables**/config_backup) | **13,499** (2026-08-04 20파일 전건 재측정 합 — 12,416에서 **+1,083**: `bonding_plan.py` **+370** · `transfer_plan.py` **+376** · `process_supervisor.py` **+306** · `product_tables.py` **+31**. 나머지 16개는 blob 동일) | [§5](#5-소형-서버-모듈) |
...
| **`server/config_resolve_report.py`** (「내 config가 먹었는가」의 답) | **668** (556 → **+112**: 세 번째 도메인 `notation`) | [§5-B](#5-b-serverconfig_resolve_reportpy--내-config가-먹었는가의-답-f3fd785-신설) |
| **`server/db_safety.py`**(#16a 테스트 프로세스 DB 가드) + **`server/virtual_join_config.py`**(가상 조인 **선언** 로더/검증) | **215** + **512** (둘 다 무변동) | [§5-C](#5-c-2026-07-31-신설-서버-모듈-2종) |
...
| 🆕 **[신설] 2026-08-04(2차)** — **`server/notation_norm.py`**(표기 정규화 파생 컬럼) · **`server/graph_stale_edges.py`**(낡은 엣지 스윕) + CLI 2종 | **542** + **549** (+ `rederive_notation_norm.py` 118 · `graph_stale_edge_sweep.py` 193) | [§5-E](#5-e-2026-08-042차-신설-서버-모듈-2종--표기-정규화--낡은-엣지-스윕) |
...
> | 라우트 | 핸들러 | 게이트 | 라인 |
> |---|---|---|---|
> | GET `/admin/transfer-plan/dry-run` | `get_transfer_plan_dry_run()` → `transfer_plan.dry_run(config)` | `require_admin_token` (**strict 아님**) | **4241/4242** |
>
...
>
> | 구 앵커 범위(`ed9cfdb`) | 이동 | 무슨 일이 있었는가 |
> |---|---|---|
> | 1–1175 | **0** | 무접촉 |
> | 🔴 **1175–1319** | **삭제** | `get_column_filter_condition` 본체 **146줄이 통째로 나갔다.** 그 자리에 재export 4줄(**1176–1179**). **오프셋 금지 — 이 구간의 옛 앵커는 밀린 것이 아니라 없다** |
> | 1320–4053 | **−141** | `VirtualColumnBinder`(신 **1193**) · `get_table_data`(신 **1337**) · `export_table_csv`(신 **1688**) · `get_table_schema`(신 **1924**) · `apply_batch_updates_endpoint`(신 **2287**) · `_chip_trace_declaration`(신 **2962**) · `reload_local_process_cache`(신 **3883**) |
> | 4054–4372 | **−132** | 🆕 `reload_local_process_cache` 안 **+9** — `notation_norm.reset_cache()` 핫리로드(**3914–3922**) |
> | 4373– | **−107** | 🆕 dry-run 라우트 **25줄** |
>
> ⚠️ **계단표는 "줄이 밀렸다"만 말하고 "그 자리에 다른 것이 들어앉았다"는 말하지 못한다.** 이 문서가 반복해 실패한 자리가 정확히 그것이다 — **함수명으로 Grep해 확인하고 나서 읽어라.**
>
> ✅ **주요 앵커 실측 (전건 `41b17ee`)**: `bootstrap_database_schema` **70** · `db_context_middleware` **121** · `health_check` **204** · `startup_event` **253** · `fetch_and_merge_metadata` **651** · `list_tables` **790** · `VirtualColumnBinder` **1193** · `apply_column_filters` **1233** · `apply_search_filter` **1271** · `get_table_data` **1337** · `export_table_csv` **1688** · `get_table_schema` **1924** · `get_column_unique_values` **2041** · `_validate_effort` **2212** · `apply_batch_updates_endpoint` **2287** · `_chip_trace_declaration` **2962** · `upload_file` **3307** · `reload_local_process_cache` **3883** · `get_transfer_plan_dry_run` **4242** · `list_retroactive_operations` **4723** · `serve_static_or_index` **5444**.
...
| `load_maps_config() / save_maps_config(data)` | 맵 프리셋 JSON 파일 IO (`MAPS_CONFIG_PATH = paths.config_path("maps.json")` **3984**) | **3986/3995** |
...
**등록 위치가 계약이다.** FastAPI는 등록 순서로 매칭하므로 이 블록은 파일 맨 아래 SPA catch-all `@app.get("/{file_name:path}")`(**5443**)보다 **위에** 있어야 한다. 이 라우트가 없던 시절 `/health`는 catch-all로 떨어져 **HTML을 200으로** 반환했다 — 외부 모니터가 죽은 서버를 살아 있다고 불렀다. `tests/test_health_endpoint.py`가 양쪽(‌`/health`는 JSON · 엉뚱한 경로는 여전히 HTML)을 단언하므로, 재배치로 라우트가 다시 가려지면 조용히 죽지 않고 테스트가 깨진다.
...
| 🔒 GET `/admin/scripts/list` · GET `/admin/scripts/code` | `list_admin_scripts` / `get_admin_script_code` | Monaco 에디터용 스크립트 조회 (경로 검사 `_resolve_admin_script_path` **5233** — 격리 서버가 라이브 트리에 쓰려 하면 **403**. ⚠️ 이 403은 **게이트가 낸 것이 아니라 핸들러가 낸 것**이라 `WWW-Authenticate`가 없다. 클라가 둘을 구분하는 근거 → [§7 `admin.js` `isGateRejection`](#7-client2src--웹-클라이언트)) | **5173/5276** |
| **🔒! POST `/admin/scripts/code`** | `save_admin_script_code` | 스크립트 저장. **strict인 이유: `mappers/`·`ingestion_workspace/`에 임의 파이썬 파일을 쓴다** | **5313/5314** |
| GET/POST/DELETE `/map-presets` (+`/api/` 별칭) | `get_map_presets`(**4019**) / `_save_map_preset_impl`(**4024**) / `_delete_map_preset_impl`(**4057**) + 4개 얇은 래퍼 | 맵 프리셋 CRUD (`class MapPresetItem` **4005**, `MAPS_CONFIG_PATH` **3984**) | **4017–4076** |
...
| 🔒 POST `/internal/events/batch-refresh` · `/broadcast` · `/file-processed` | `internal_event_batch_refresh`(**5039**) / `internal_event_broadcast`(**5076**) / `internal_event_file_processed`(**5118**) | **워커/워처 → 웹서버 브로드캐스트 위임 (경계 계약)** — 수신부는 `total_log_count`(실건수) 우선 + `MAX_NOTIFY_CREATED_LOGS` 방어 절단(인시던트 `cc57b64`, 절단 지점 **~5058·~5106**). [P1] batch-refresh는 msg 재구성 시 `total_log_count` 동봉, broadcast는 `file_ingestion_progress`를 레지스트리에 인터셉트, file-processed는 레지스트리 제거 인터셉트. **[`90e284f`] 게이트 추가 — `/internal`이 `/admin`과 같이 묶인 이유는 `broadcast`가 임의 dict를 전 WS 클라이언트에 중계하고 audit_cache에 주입하기 때문**(읽기 전용 admin은 잠그고 이건 열어 두는 것이 거꾸로였다) | **5038/5039 · 5075/5076 · 5117/5118** |
...
| **격리 검사** | **5475–5479** | `dist_base = os.path.abspath(client2_dist_path)` → `target_path = os.path.abspath(os.path.join(dist_base, file_name))` → **`target_path`가 `dist_base` 자신이거나 `dist_base + os.sep`로 시작하지 않으면 거부.** `_resolve_admin_script_path`(**5233**)와 **같은 모양**이다(사유 주석 **5461–5096**) |
...
>
> ⚠️ **직전 패스가 이 절에서 112줄을 놓쳤던 실패는 반복되지 않게 이번엔 끝·경계·최근 변경 구역에서 표본을 뽑았다** — `set_cell_manual_priority_batch`(**2762**) · `check_needs_rollback`(**3172**) · `get_row_cell`(**2594**) 셋이 파일 뒤쪽 표본이고 전부 위 계단표와 일치한다. 🔴 **핀은 "파일이 안 바뀌었다"만 말하지 "지도가 이 파일을 옳게 적었다"를 말하지 않는다.** 앞쪽 절반이 전건 일치하는 표는 뒤쪽 절반도 맞아 보인다.
>
> 아래 표에 **없는** 실재 심볼: `SOURCE_PRIORITY`(**414**) · `USER_SOURCE`(**431**) · `CONFIG_PATH`(**442**) · `TABLE_CONFIG` 싱글턴(**599**) · 재수정률 블록 `RECORRECTION_WINDOW_DAYS`(**1181**)/`_is_json_null`(**1188**)/**`get_recorrection_stats`(**1199**)** · `EFFORT_WINDOW_DAYS`(**1270**) · `create_empty_row`(**2484**) · `get_row_cell`(**2594**).
...
🔴 **게이트는 거부권(veto)이지 승격이 아니다.** 통과한 행도 셀마다 `compute_priority_value`를 그대로 지나므로 `user`(0)가 모든 기계 소스를 이긴다 — **버전은 같은 우선순위 계층 *안에서만* 순서를 매기고 계층을 건너뛰지 못한다.** 통과했다고 페이로드를 행에 그냥 써 버리면(「버전이 권위다」의 그럴듯한 오독) 다음 인제션이 사람의 교정을 조용히 되돌린다. `server/tests/test_version_gated_overwrite.py::test_human_correction_survives_a_newer_version`이 그 순간 빨개진다.
...
| 🔴 **`version_gate_verdict(table_name, config, row, is_new, update_item) -> (applied: bool, reason: str\|None)`** | **행 단위 판정.** 셀 단위로 재지 않는다 — 셀별 검사는 한 행의 일부만 받아들여 **어느 버전에도 존재한 적 없는 행**을 만든다. 호출부는 `apply_row_update_internal` 안 **1630** 한 곳이고, 직접 호출자 6개 + HTTP가 전부 그 관문으로 수렴한다 | **297** |
| **`log_version_gate_summary(table_name, version_col, source_name, stats)`** | **개별은 침묵, 집계는 이름으로** — 인제션 드롭 리포트와 같은 자세. 행마다 한 줄이면 1000만 행에서 진짜 사건이 전부 묻힌다. 호출부는 `apply_batch_updates` 안 **2447**(버전 컬럼 조회는 **2419**) | **346** |
...
| `apply_batch_updates(db, table_name, batch, replace_report=None)` | **배치 진입점** — tx 컨텍스트, 캐시 프리로드, 행별 코어 호출, 벌크 flush, outbox 발화. 반환 `(results, changed_cells, created_logs, deleted_row_ids)`(4-튜플 불변 — 워커/파서/테스트 호출부 보호). [P2] 워처가 이 함수의 commit에 오프셋 갱신을 동승시킨다. **`batch.replace_map`(purge 블록 **2301–2350**)** — `derive_replace_map_scope`(호출 **2307**)가 정한 스코프의 기존 행을 클린 삭제 후 재기록(**차집합 계산 없는 집합 교체** 연산. 소비자: 맵 Push·DOE 저장). **[gate4] 스코프 유도 불가면 `ValueError`로 거부**(**2309–2310**, 침묵 noop 폐지), **`replace_report` out-param**(선언 **2273**, 채움 **2344–2346**)에 `{filters, deleted}`를 채워 API 계층이 정직하게 echo. **명시 `scope` + 빈 `updates` = 그 스코프의 의도적 전량 삭제**(erase-all). 🆕 **쓰기 가드 둘이 맨 앞이다**(**2285/2286**) 🆕 **버전 게이트 요약 로깅은 맨 끝**(`version_col` 조회 **2419**, `log_version_gate_summary` **2447**) | **2272** |
...
| `set_cell_manual_priority_batch(db, table_name, updates, source_name, updated_by)` | 수동 Pin 일괄(§크고 복잡 — 표시값 재계산·감사 포함) | **2762** |
...
>
> ✅ **앵커 재측정 (2026-08-04, `41b17ee`)** — 2,180 → **2,274줄**(+94). 이동은 단순하다: **구 앵커 66 이전은 무이동 · 67–1861은 +62 · `WorkspaceWatcher`(신 **1956**)는 +94**(클래스 안 추가분 때문).
>
...
> - **`MAX_DROPPED_COLUMNS_REPORTED = 64`(**70**)** — `crud._MAX_UNDECLARED_WARNED_PER_TABLE`을 그대로 비춘다. **`_dropped_column_announced`(**76**)** — 🔴 **crud의 레지스트리를 재사용하지 않은 것이 의도다**: 관문이 다르고 config 키가 다르다(**파일당 `display_columns`** vs **셀당 `column_types`**).
> - **`_announce_dropped_columns(t_name, dropped_value_counts, defined_cols, filename, row_count)`(**79**)** — 크기 배분이 계약이다: **행·셀 단위로는 아무것도 찍지 않고**, (테이블, 컬럼)당 **프로세스 1회** 첫 목격에 WARNING, **파일당 1회** 이름과 건수를 INFO. 누적은 `_send_to_upsert` 안(**~1852–1861**).
> - 🆕 **`IngestionHandler._compose_detail(skipped_no_key, plan, has_rows=True)`(**1213**, 호출 **1177·1431**)** — 세 번째 인자가 **0행 파싱의 같은 침묵**을 닫는다: 「한 셀도 저장되지 않음」과 「정상 처리」가 화면에서 구별되지 않았다.
> - 테스트: 🆕 **`tests/test_ingestion_drop_visibility.py`(257줄)**.
>
> 📐 **주요 앵커(`41b17ee`)**: `AUTO_PROVISION_EXCLUDED_TABLES` **129** · `load_global_table_config` **141** · `load_ingestion_settings` **214** · `get_heavy_threshold_bytes` **240** · `nested_dirs_enabled` **319** · `get_workspace_serial_lock` **336** · `class HeavyIngestionLane` **346** · `find_workspace_alias` **400** · `resolve_workspace_root` **441** · `resolve_workspace_table` **474** · **`class IngestionHandler` 562** · **`class WorkspaceWatcher` 1956**.
>
...
| `start_chain_ingestion_worker(db_session_factory)` | **메인 루프** — LISTEN 대기, 리로드 체크(1s 간격), 스윕, purge 스케줄. SYSTEM_RELOAD 블록에서 `models.refresh_dynamic_models(engine)`(지연 import) 호출 — 신규 테이블 CREATE 보충 안전망(이슈 #7). **[`8117456`] 루프 안에서 `heartbeat.beat("chain")`(**931**)** — `/health`가 "살아 있음"이 아니라 **"진척이 있음"**으로 판정하는 근거(`server/utils/heartbeat.py`, [§5](#5-소형-서버-모듈)). 🆕 **[`23a346d`] 함수 첫머리(**888**)에서 `internal_event_client.startup_lines("Chain Worker")`를 로그한다** — 데이터가 흐르기 전에 토큰 지문·프록시 설정·`/health` 도달성을 말한다. **2026-07-30 403을 받은 것이 이 워커이고, 그때 이 프로세스가 기동에 남긴 어떤 줄도 이유를 말할 수 없었다.** 🆕 **제어 이벤트 필터(**1026**)가 `event_constants.CONTROL_EVENT_TYPES` 멤버십인데, 그 frozenset에 `EVENT_BROADCAST_RECOVERY`가 더해진 것이 미배달 마커를 맵퍼 경로 밖으로 지키는 장치다** | **880** |
...
### `server/process_supervisor.py` (**1,024줄** — `ed9cfdb` 718에서 **+306**) — 자식 프로세스 감독
...
>
> 📐 **앵커 이동 — `_database_endpoint`(**195**)·`shared_dependency_down`(**224**)까지 **+52**, `psutil_status` 이후는 **+158**, `Supervisor` 내부는 최대 **+306**.**

| 시그니처 | 역할 | 라인 |
|---|---|---|
| `BACKOFF_BASE_SEC=2.0` / `BACKOFF_MAX_SEC=60.0` / `MAX_CONSECUTIVE_FAILURES=5` | **재시도 예산** — 연속 n번째 실패는 `min(base·2^(n-1), max)` 대기, 예산 초과 시 `FAILED`로 **영구 정지**(배너 로그 + `/health` 비-200). 즉사하는 자식을 무한 재시작하면 CPU를 태우고 로그를 덮고 **무엇보다 감독이 동작하는 것처럼 보인다** | **131–133** |
| `HEALTHY_UPTIME_SEC=60.0` / `POLL_INTERVAL_SEC=1.0` / `STATUS_REFRESH_SEC=5.0` / `MAX_EVENTS=100` | 이만큼 살아 있었다면 크래시 루프가 아니므로 **연속 카운터를 리셋**한다 — 이게 없으면 한 달에 한 번 재시작하는 시스템이 결국 아무것도 재시작하지 않게 된다 / 폴 주기·상태파일 갱신 주기·이벤트 링버퍼 상한 | **135/136/138/140** |
| **`CORRELATION_WINDOW_SEC=120.0` / `CORRELATED_MIN_CHILDREN=2` / `CORRELATED_BACKOFF_SEC=60.0`** | **공유원인 실패 판정.** 상관은 **exit code가 아니라 시간으로** 정의한다 — 윈도우에선 미처리 파이썬 예외가 전부 exit 1이라 코드 시그니처는 아무 쌍이나 상관으로 부르고 아무것도 증명하지 못한다. 규칙은 **창 안에서 서로 다른 자식 2개 이상이 죽었는가**. 창이 120초인 것은 **예산 1사이클보다 길어야** 하기 때문(2+4+8+16+32초 ≈ 첫 죽음 후 80초에 판정이 나고, 그 시점에 동료들의 마지막 실패는 20–40초 전이다) | **153/157/160** |
| 🆕 **`PORT_PROBE_TIMEOUT_SEC = 0.5`** / 🆕 **`CHILD_LOG_MAX_BYTES = 20MiB`** | 포트 프로브 상한 / 자식 stdout 로그 회전 임계(백업 `.1` 하나) | **166 / 172** |
| `STATE_RUNNING\|BACKOFF\|FAILED\|STOPPED` / **`STATE_RETRYING_CORRELATED`** / `_WAITING_STATES` | 자식 상태 어휘. **`retrying_correlated` = 예산은 소진했지만 혼자가 아니다 → 영구 실패시키지 않고 계속 재시도** | **174–177 / 179 / 188** |
| 🆕 **`VERDICT_BROKEN_CHILD = "broken_child"` / `VERDICT_PORT_CONFLICT = "port_conflict"`** | **종단 판정 어휘 2종.** 스냅샷의 `"terminal_verdict"` 키(**977**)와 이벤트 스트림(`_record(child, "permanently_failed", verdict=…)` **812**)에 실린다. ⚠️ **현재 `main.py`·`health.py`에 이 값으로 분기하는 독자가 없다** — 소비자는 `tests/test_duplicate_launcher.py`뿐이다(선언은 있고 화면이 아직 안 읽는다) | **184 / 185** |
| `status_path()` | `paths.config_path("supervisor_status.json")` | **191** |
| `_database_endpoint(url=None)` / **`shared_dependency_down(url=None, timeout=2.0) -> (down, detail)`** | **[신설] 환경이 깨졌다는 직접 증거.** 왜 동료실패 규칙만으론 부족한가 — PostgreSQL 불통 콜드스타트 실측에서 **죽는 자식은 정확히 하나**, 웹서버다. 워커 4개는 자기 루프에서 에러를 삼키고 살아남는다. 즉 "재부팅 후 DB가 늦게 떴다"의 가장 흔한 실제 형태가 **고립된 자식 1개**이고, 동료만 세는 규칙은 94초 뒤 웹서버를 영구 실패시킨 뒤 DB가 돌아와도 안 살린다.<br>⚠️ **이 규칙의 근거를 `create_all`이 어느 줄에서 도는지에 묶지 마라.** 소스 docstring(~176–180)과 이 지도의 종전 서술은 근거를 "**import**가 `Base.metadata.create_all`을 도는 웹서버"로 적었고 HEAD `b8307c2`에서는 여전히 그렇다(`main.py` **~53**, 모듈 최상위 — [§1.1](#11-기동미들웨어공용-헬퍼)). 그런데 **그 줄은 이동 중이다**(미착지 라운드가 기동 함수 안으로 옮긴다). 판정이 실제로 딛고 있는 사실은 두 가지이고 **둘 다 위치와 무관**하다: ① 웹서버는 부팅 경로에서 DB에 무조건 닿고 워커는 안 닿는다 → 불통 시 죽는 자식은 하나 ② **감독자는 종료 코드를 아예 보지 않는다** — `poll_once`가 `code is None`인지만 보고(~569–570) 값은 이벤트 기록용일 뿐, 파일 전체에 exit code 비교가 **0건**이다(`grep -n "exit_code ==\|code ==" = 0`). QA가 죽은 포트에 uvicorn을 물려 실측한 **exit 3**도 이 경로에서는 다른 코드와 구분되지 않는다. 결론은 유효하다 — 바뀔 수 있는 것은 근거 문장의 **메커니즘 절**뿐이다. **TCP 도달성만** 본다(인증·스키마 결손은 재시도가 못 고치는 설정 결함). **모르면 healthy** — 이 프로브는 증거를 **더하기만** 할 뿐 실패시킬 능력을 빼앗지 않는다. **[`2728bd9`] `url=None`이면 `paths.resolve_database_url()`로 해석**(database.py와 같은 우선순위 env > `config/database.json` — 예외 시 env 폴백, 기본값 없음: "아무것도 설정 안 됨"은 종전대로 프로브 대상 없음) | **195 / 224** |
| 🆕 **`port_owner(port, log=None)` / `port_is_taken(port, host="0.0.0.0", timeout=…)` / `describe_port_conflict(port, host, timeout)` / `port_conflict(spec, timeout)` / `preflight_port_check(ports, host, timeout)`** | **[신설] 포트 점유 판정 5종.** `port_owner`는 점유 PID를 이름으로 대고, `port_conflict`는 `ChildSpec.ports`를 받아 자식 단위로 답하며, **`preflight_port_check`는 런처가 *아무것도 띄우기 전에* 부르는 것**([§6 `run_decoupled_app.py`](#6-기타-서버-모듈-한줄-요약)) | **256 / 289 / 318 / 331 / 347** |
| `psutil_status()` / `_psutil_or_warn(log=None)` | 손자 정리 무장 여부를 **기동 시 1회 announce**. 정리 경로가 조용히 퇴화하는 것이 고아 수집기 프로세스가 몇 주씩 쌓이는 경위라, 종료 때 발견하지 않고 부팅 때 말한다 | **362 / 383** |
| `_descendant_pids(pid, log=None)` / `_kill_pids(pids, log=None)` | 종료 시 손자 프로세스까지 수거 | **400 / 418** |
| 🔴 **`class ChildSpec(name, cmd, cwd, env=None, restartable=True, heartbeat=None, start_delay=0.0, ports=(), port_host=None, log_file=None)`** | 자식 1개의 기동법 + 죽었을 때의 처분. **`restartable=False`는 "이게 죽으면 전체를 멈춘다"**(데스크톱 창 닫기), **`heartbeat=`는 그 자식이 발행하는 비트 이름** — `/health`가 프로세스 관점(감독자)과 진척 관점(비트)을 조인하는 열쇠. 🆕 **키워드 3종이 이번에 추가됐다**: `ports`/`port_host`(포트 충돌 판정의 입력) · **`log_file`**(설정되면 spawn이 `stdout=PIPE, stderr=STDOUT` + `PYTHONUNBUFFERED=1`로 바뀐다) | **436 / `__init__` 443** |
...
**668줄**(`ed9cfdb` 556에서 **+112**). import는 `json`·`logging`·`os` **셋뿐**이고 나머지(`enrichment_config`·`enrichment_candidates`·**`virtual_join_config`**·**`notation_norm`**·`database.crud`)는 **전부 함수 안 지연 import**다 — 이 모듈을 import하는 것만으로는 아무 애플리케이션 모듈도 끌려오지 않는다.

> 📐 **앵커는 `_resolve_virtual_join`(**445**)까지 전부 무이동이고, 그 뒤에 세 번째 도메인이 통째로 붙었다.** 신설 블록은 **531–638**(`DOMAIN_NOTATION`부터 `_resolve_notation` 끝까지)이고, 그 아래 `_RESOLVERS`·`resolve_report`만 밀렸다(**528/534 → 639/646**).
>
> 🔴 **도메인이 2개에서 3개가 됐다** — `enrichment` · `virtual_join` · 🆕 **`notation`**. 구 지도의 "현재 2개"는 낡았다.
...
| 🆕 **`DOMAIN_NOTATION`** / **`_NOTATION_CODE_TO_REASON`** / **`_NOTATION_CODE_LEAD`** | **[신설] 세 번째 도메인.** 🔴 **여기서도 새 사유 단어를 만들지 않았다** — `notation_norm`의 내부 코드를 기존 닫힌 어휘로 사상한다: `zero_pad_unimplemented` → **`not_reached`**(선언은 읽혔는데 그 규칙까지 실행이 도달하지 않는다) · `unknown_rule`/`would_rewrite_raw`/`key_column`/`shape` → **`mapping_unavailable`** · `undeclared` → **`not_declared`**. `_NOTATION_CODE_LEAD`는 코드별 한국어 앞머리(`_VJ_CODE_LEAD`와 같은 자세) | **531** / **533** / **544** |
| 🆕 **`_resolve_notation() -> dict`** | 세 번째 도메인 등록기. 🔴 **DB를 건드리지 않는다** — config만 읽는다. 앞의 둘과 **다른 점 하나**: 유효한 표기 정규화 선언은 곧 `effective`다(파생이 쓰기 경로에서 실제로 돌아 값을 남긴다). 그러나 **Phase 1이라 아직 아무도 그 값을 읽지 않는다**는 사실을 **선언마다 문장으로 달아** 보고한다 — 🔴 **「먹었다」와 「쓰이고 있다」는 다른 질문이고, 둘을 붙여 두지 않으면 다음 사람이 맵 키가 이미 정규화된 값으로 조회된다고 읽는다.** `zero_pad: true`가 조용히 무시되는 노브가 아니라 **이름 붙은 거절**로 화면에 뜨는 것도 이 등록기 때문이다 | **561** |
| `_RESOLVERS` / **`resolve_report(domains=None) -> dict`** | 도메인 등록기 dict — **현재 3개**(`enrichment` · `virtual_join` · `notation`, 나머지 config는 여기에 한 줄씩 붙는다) / 진입점. 🔴 **한 도메인의 실패가 나머지를 삼키지 않는다** — 예외는 잡아 그 도메인만 `rejected` 1건(`mapping_unavailable`)으로 강등한다. 응답에 **`vocabulary`를 함께 싣는 것이 계약**이다: 클라가 라벨·필터를 하드코딩하지 않게 | **639** / **646** |
...
### `server/virtual_join_executor.py` (**554줄**, `ed9cfdb` 535에서 **+19**) — 가상 조인의 **실행** 절반
...
| **`resolved_expression(db, left_model, left_table, col_name, query)`** | 기여 규칙마다 `outerjoin` 1개(오른쪽은 `aliased`) + `func.coalesce(*parts, label)`. 반환 `(query, expr, label)` \| `(query, None, None)`.<br>🔴 **[N8 정정] 중첩 헬퍼 `_text_part(col)`(**351**)는 더는 숫자만 분기하지 않는다** — 본문이 **`crud.column_text_sql(col)`(**356**) 한 줄**이 됐다. 구 코드는 `isinstance(col.type, (Numeric, Float, Integer))`로 **「어떤 타입이 캐스팅이 필요한가」**를 물었고, **같은 문으로 들어온 다음 타입이 똑같이 죽었다**: `datetime`은 `InvalidDatetimeFormat`, `boolean`은 `InvalidTextRepresentation`(둘 다 프로덕션 방언 실측), **SQLite에서는 죽지도 않고 매칭 안 된 행마다 `True`를 답한다.** 새 관문은 **「이 타입이 *이미* 텍스트인가」**를 묻고 미지 타입은 500이 아니라 CAST로 떨어진다([§2 신설 블록 ②](#2-serverdatabasecrudpy--레이어링-코어)) | **300** |
...
### 🆕 `server/notation_norm.py` (**542줄**, 신설) — WF/lot/slot 표기 정규화, **파생 컬럼이지 덮어쓰기가 아니다**
...
>
> **도달**: 쓰기 경로(`crud.apply_row_update_internal`) · 쓰기 거부(`crud.refuse_notation_derived_columns` **2235**) · 보고서(`config_resolve_report._resolve_notation` **561**) · CLI **`server/scripts/rederive_notation_norm.py`(118줄** — `main` **63**, `format_report` **45**. 기본 드라이런, exit `0`=보고/기록 · `2`=거부).
>
...
### `state.js` (**130줄**, `ed9cfdb` 127에서 **+3** — 주석만) — 전역 싱글턴
...
### `api.js` (**485줄**, `ed9cfdb` 456에서 **+29**) — REST 소비 계층 (경계 계약의 클라이언트측)
...
### `map_editor.js` (**9,683줄** — `ed9cfdb` 9,163에서 **+520**) — 웨이퍼 맵 에디터 (단일 페이지 스크립트, export 없음)
...
>
> **재측정 트리거**: `git rev-parse HEAD:client2/src/map_editor.js`가 **`72f2fb7`**(= `41b17ee`의 blob)이 아니면 이 절 전체가 재측정 대상이다. **직전 지도의 트리거 값 `f31e080`은 이번에 만료됐고, 아래 앵커는 전건 `41b17ee` 실측이다.**
>
> 🔴🔴 **이 트리거는 *이 문서가 커밋되는 시점에 이미 빨간불이다* — 그리고 그것을 숨기지 않는 것이 이 절이 지금 할 수 있는 최선이다.** 이 패스를 쓰는 동안 맵 레인이 **10커밋**을 착지시켰고(`35b03cc`→`c97b319`) 그중 둘이 이 파일을 **9,683 → 10,109줄**(+426)로 바꿨다. 아래 앵커는 여전히 **`41b17ee`(blob `72f2fb7`) 실측**이고, `c97b319`(blob **`964018e`**)에 대해서는 **아래 오프셋을 더해서 읽어라.** 🔴 **이것은 계단표이고, 계단표는 「그 자리에 다른 것이 들어앉았다」를 말하지 못한다 — 반드시 함수명으로 Grep해 확인하고 나서 읽어라.**
>
> | 아래 표의 앵커 | `c97b319`에서 더할 값 | 근거 |
> |---|---|---|
> | ~460 이전 | **0** | 무접촉 |
> | ~463–2050 | **+16** | `initDOMElements` 안 리스너 3블록 |
> | ~2051–2090 | **+22 ~ +30** | `validDie` 상태 블록 재작성 — 🔴 **오프셋 금지, 함수명으로 찾아라** |
> | ~2091–6958 | **+35** | `parseValidDieRef` 확장 |
> | ~6959–8500 | **+343** | 🆕 **좌표표 붙여넣기 블록 254줄 삽입**(아래) |
> | ~8501– | **+352 ~ +426** | 유효 다이 저작·표시 블록 재작성 — 🔴 **오프셋 금지** |
>
> 🆕 **아직 이 절에 항목이 없는 신설 5종**(`c97b319` 실측): **`coordRulerTicks(line)`(7053)** · **`readCoordTableBlock(text)`(7088)** · **`currentCoordFrame()`(7189)** · **`planCoordPaste(coord, cf)`(7213)** — 엑셀 모양의 **2D 좌표표를 화면 위치가 아니라 DB 좌표로** 읽는 붙여넣기 경로(축은 표의 헤더에서 읽는다) — 그리고 **`renderValidDieKeyControl()`(8895)** — 유효 다이 **읽기**를 `valid_die_ref`에 고정하고 없으면 **이름으로 거절**하는 라운드의 컨트롤. **삭제·개명은 0건**이다.
...
| 부모 | 단계 함수 | 무엇인가 | 라인 |
|---|---|---|---|
| **R4 `loadExistingMap`**(**4521**) | `collectMapKeyFilterModel()` | ① 메타 입력칸 → 조회 필터. 값이 하나도 없으면 `hasFilter === false`이고 **판정은 호출부가 한다** | **4890** |
| | `scanCoordinateBounds(result, xCol, yCol)` | ② 응답 행 → 저장 좌표 bbox. 🔴 **센티넬(`minX 9999`)이 계약의 일부다** — 파싱된 셀이 0개면 그대로 돌아가고 호출부가 그 값으로 「해석된 셀 0개」를 가른다 | **4913** |
| | `resolveDeclaredGridMeta(selectedTable, tableSchema, filterModel, result)` | ③ `wafer_map_metadata` 해석 → `{ok:true, gridMeta, mapKey}` \| `{ok:false, refusal}`. 🔴 **「확인 못 했다」는 「선언이 없다」가 아니다** — `fetchGridMetaFor`의 404/405-만-null 규율이 여기서 끝나지 않도록 한 자리에 모았다 | **4943** |
| | `promptCoordinateChoice(el)` | ④ 선언 없는 맵의 좌표계 선택 모달 → `'standard'\|'current'\|'cancel'`. ⚠️ **`el`을 인자로 받는 것이 시그니처의 말이다** — 이 단계가 손대는 유일한 바깥 것 | **5039** |
| | **`resolveGridFrame(userChoice, loadedGridMeta, minX, minY, maxX, maxY, el, currentRotation, currentSide)`** | ⑤ 선택 → 격자·원점·회전·면. 🔴 **셋 중 정확히 하나가 프레임을 정한다**가 이 함수의 전부. `boundingBoxCache` 무효화는 **호출부**가 한다 | **5098** |
| | `deriveLegendFromCellValues(uniqueVals, legend, predefinedColors)` | ⑥ 셀 값 집합 → legend 배열을 **돌려줄 뿐 대입은 호출부**가 한다(「어느 시점에 화면의 legend가 바뀌는가」가 한 줄로 읽혀야 한다) | **5177** |
| | **`restoreDoeDraftWithPrecedence(selectedTable, loadedMapKey, serverFp, serverCellsFp)`** | ⑦ 초안 우선순위. 기반 지문이 같으면 초안이 더 새 것 · 어긋나면 **누가 썼다** → 적용하면 남의 저장을 지운다. 🔴 **적용하지 않고, 버리지도 않고, 사실을 드러낸다**(`restoredUnsavedEdits` → `legendDirty` · `staleDraftKept` → persist 보류) | **5231** |
| **R6 `pushMapData`**(**5394**) | `confirmLogShapedPushTarget(tableSchema, el, selectedTable)` | 로그형 타깃의 손실 인지 확인창. **어느 모드가 거절하고 무엇을 말하는가는 `logShapedPushDecision`의 답 그대로** | **5774** |
| | `collectMetaFieldValues(tableSchema)` | 메타 패널을 있는 그대로 읽는다. `ok:false`는 **선언된 메타 필드가 있는데 하나도 안 채웠을 때**뿐 — 필드가 아예 없는 테이블은 막지 않는다(공백 검사에 둘째 항이 있는 이유) | **5813** |
| | `buildPushGridMetadata(cols, rows, startX, startY, invertY, el, currentRotation, currentSide, validDie)` | 저장될 `grid_metadata` 조립. 🔴 **무엇을 쓸지는 여기서 정하지 않는다** — `validDieRefForPush()`(바꿨는가) + `applyValidDieRef()`(무엇을 쓰는가) + `validDieRefPayload()`(페이로드로), 셋 다 이음매 벡터가 채점한다. 선언 없고 사용자가 안 건드린 맵의 페이로드는 `2a9f6c4`와 **바이트 단위로 같다**(INV-1) | **5847** |
| | `confirmMissingSplitDescriptions(updates, valCol, legend)` | split 서술이 빈 값에 대한 확인. **판정이 아니라 보고**다 — 예라고 하면 계획은 불완전한 채 저장된다 | **5874** |
| | `outsideCircleNoteForPush(cols, rows, currentRotation, gridCells2D, gridData)` | [M4②] 원 밖으로 나가는 셀을 **이미 있는 확인문의 한 줄로** 말한다. 🔴 **말할 것이 없으면 `''`를 돌려주므로 확인문이 글자 하나 안 바뀐다**(INV-1) | **5893** |
| **R5 `resolveValidDie`**(**7867**) | `fitGridToMask(keys, el, currentRotation, currentSide)` | ① 파생 격자가 마스크를 온전히 담는가. `set` 클로저에서 **글자 그대로** 들어냈고 진단 로그 문구를 돌려준다(대입은 호출부) | **8283** |
| | `summariseReseat(seatsBefore, placed, nc, nr, gridData, loadedFCells, serverCellKeys)` | ② **넷 이동량을 물리 좌석 키의 집합 차이로** 잰다. `netMoved > 0` 분기는 호출부가 그대로 갖고 있어 제어 흐름 무변경 | **8343** |
| | `resolveReferenceSpec(ref)` | ③ 참조 맵의 키 스펙·좌표 바인딩·메타·프레임. await 사슬 하나, 거절 셋, **모듈 상태 0**. `ref.mapKey`는 종전대로 **제자리에서** 캐노니컬화된다. 🔴 **거절은 취하지 않고 사유로 돌려준다** — `refuse`는 모듈 상태를 쓰므로 오케스트레이터에 남는다 | **8367** |
| | `deriveMaskKeys(rawKeys)` | ④ 마스크 키 집합과 그 중심. 🔴 **0으로 둔 평행이동 항과 「왜 0이어야 하는가」의 경고가 함께 옮겨졌다** — 수와 사유를 떼어 놓은 것이 그 항을 한 번 되살렸던 경위다 | **8399** |
| | `diagnoseDesignationAlignment(refResolved, hereResolved, refMinX, refMinY, hereInvertY, el, currentRotation, currentSide)` | ⑤ 지정 **뒤** 참조 맵과 이 맵이 무엇이 다른가. **두 축(원점·크기)은 독립**이라 각각 재고 성립할 때만 보고한다. 🔴 **지정이 끝난 뒤의 격자로 푼다** — 옛 치수로 푼 진단은 사용자가 보고 있지 않은 화면을 설명한다 | **8429** |
...
| **`let isMouseDown = false`(**62**)** | 🪦 **쓰기 전용.** 대입 2곳(**961** true · **1031** false)뿐이고 **읽는 곳이 0건** | 조건식·반환·전달 인자 어디에도 등장하지 않는다 |
| ~~`validDieRefTableTouched`~~ | 🪦 **[신설 묘비] 삭제됐다.** 종전 지도가 모듈 상태로 등재하던 이름이고, 지금 이 파일에 남은 것은 **~2060의 묘비 주석 1건**뿐이다 | `git grep -n "validDieRefTableTouched" -- client2/src` → **주석 히트 1건, 선언 0건** |
...
- **`getDieIndex(colVisual, rowVisual, cols, rows, rotation, side)`(**1442**)** / **`getCanvasCellFromDieIndex(xp, yp, cols, rows, rotation, side)`(**1501**)** — 서로의 **정확한 역함수**. 물리 키의 원점은 **웨이퍼 중심**이지 격자 중심이 아니다. 되기준 항에서 남긴 것은 크기가 아니라 **패리티**다(짝수 격자면 웨이퍼 중심이 칸 경계에, 홀수면 칸 한가운데에 앉는다 — 운영 데이터 214개 맵 중 **163개가 짝수 치수**). ⚠️ **헬퍼로 빼지 않는다** — 이 둘을 슬라이스해 실행하는 하니스가 넷이라 모듈 전역 의존이 하나 늘 때마다 넷이 전부 `ReferenceError`로 죽는다.
- **`getCanvasCellFromDb(dbX, dbY, cols, rows, rotation, side, invertY, startX, startY)`(**1608**)** / **`getDbCoords(…같은 인자…)`(**1775**)** — 저장 좌표 ↔ 캔버스 칸. **같은 상자에서만** 역함수다.
- **`getWaferBoundingBox(rotation, side, opts)`(**1663**)** — **좌표계의 원점 상자.** 기준은 **유효 다이 영역**이다(원 bbox는 회전·반전에 거의 불변인데 유효 다이 영역은 아니다 — 「회전할 때마다 origin이 틀어지네」의 정체). 판정을 새로 쓰지 않는다: 원은 `isCellInsideWaferFast`(**1975**), 마스크는 `isValidDieAt`(**2140**), 렌더 루프가 부르는 그 둘을 **같은 순회 안에서** 누적한다(두 번째 전수 순회를 만들지 않기 위해). 캐시 `boundingBoxCache`(**1623**), 캐시 키 첫 항은 **근거 태그**(`'C'` = 원 기하 / `V<validDieResolveSeq>` = 해석된 마스크 — 셀 **개수**로 태그를 만들면 같은 크기의 다른 참조가 서로의 상자를 덮어쓴다). `opts.circleOnly`의 **유일한 소비자는 `computeNotchCell`**(**6348**, 호출 **6354**). ⚠️ **마스크가 격자 안에 한 칸도 없으면 원 상자로 돌아간다** — 빈 상자는 `{0,0,0,0}`으로 무너져 좌표계 전체를 조용히 옮긴다. **미상은 0이 아니다.**
- 🆕 🔴 **[`cd3e0f4`] 진짜 밀리미터 공간이 들어왔다 — 「규칙 6」 오버레이의 좌표계다.** 아래 셋이 mm 변환의 전부이고 **회전·반전·오프셋은 한 줄도 없다**(전부 `getDieIndex` 안에서 끝난다 — 여기서 더하는 것은 **단위 환산 하나**뿐):
  - **`frameDieLattice(frame)`(**1570**)** — 격자 기준점을 `getDieIndex`**에게 묻는다**(캔버스 칸 (0,0) 하나를 그 함수에 넣어 얻은 답). 다이 격자는 간격 1의 정수 격자이므로 **기준점 하나가 격자 전체를 정한다** — 패리티·회전 부호표·오프셋 부호표가 여기 한 줄도 없는 이유가 그것이다. `ix0`는 반올림된 인덱스, **`ux0`는 반올림 전 연속값**.
  - **`dieIndexToWaferMm(ix, iy, L)`(**1583**)** — 다이 인덱스 → 그 다이 **중심**의 절대 웨이퍼 mm.
  - **`waferMmToDieCell(mmX, mmY, L)`(**1596**)** — 위의 역함수, **단 나머지를 버리지 않는다**: 몫 = 그 점을 담는 다이 인덱스, 나머지(`rx`/`ry`) = 그 다이 **안에서의** `[0, 피치)` 밀리미터. 🔴 **나머지는 절대 길이라 피치에 의존한다** — 7mm 칩 안의 3mm와 15mm 칩 안의 3mm는 다른 자리다. 그래서 칩 내 좌표는 맵 사이를 그대로 건널 수 없고 **반드시 절대 mm를 거쳐 다시 나눠야** 한다.
...
- `getTransformedPhysicalConfig`(**1914**) · `getScreenShift`(**1949**) · `isCellInsideWafer`(**2013**).
- **프레임 창(frame window)** — `physFrameOverride`(**1365**) + `physNum`(**1368**)/`gridDimNum`(**1377**)/`physDeclaration`(**1401**)/`withPhysFrame(frame, fn)`(**1414**). 변환 함수가 규격을 DOM에서 읽는 지점을 잠깐 갈아끼우는 장치로, **주입 지점은 `getTransformedPhysicalConfig`·`getWaferBoundingBox` 두 곳뿐**이다. `withPhysFrame`은 **동기 전용**(내부 `await` 금지 — `try/finally` 복원이 새면 조용한 오답).
...
- 🔴 **`isValidDieAt` 호출은 여전히 6곳이고, 앵커는 전부 이동했다**(2026-08-04 전건 재계수. `grep -c "isValidDieAt"` = **17** = 정의 1(**2140**) + 호출 6 + **주석 10** — 구 지도의 총계 8은 낡았다): 셀 객체 조립 **825**(`getGridCellObject` 안) · `getWaferBoundingBox`의 마스크 누적 루프 **1742** · 캔버스 **3073** · `fillGrid` **5311** · 엣지 분류 **5930** · `canvasSeatKeys` **7846**. **두 번째가 요점이다** — 마스크 판정이 **좌표계 자신**의 입력이다. ⚠️ **개수(6)만 확인하고 통과시키면 여섯 앵커가 전부 틀린 채 남는다.**
...
- **`cellsSeatedUnder`(**1806**, `let … = null`)** — 셀이 **마지막으로 앉은** 좌표계 기록. 🔴 **상자에 대한 두 번째 진실이 아니다**: 상자가 무엇인가는 `getWaferBoundingBox` 하나만 답하고, 여기 담기는 것은 **그 함수가 이미 내놓은 답**이다. 뜻은 "무엇이 옳은가"가 아니라 **"셀이 마지막으로 어디에 앉았는가"**다. 둘을 대조해 화해시켜야 하는 상황이 생기면 **설계가 틀린 것이므로 화해시키지 말고 보고할 것.**
- **`seatingSnapshot()`(**1808**)** — 그 기록을 만드는 순수 수집기. ⚠️ **키 이름이 `currentFrame`·`frameFromMeta`가 쓰는 그 이름이어야 한다** — 이 객체를 그대로 `withPhysFrame`에 넘겨 **옛 좌표계를 다시 열기** 때문이다. 🔴 **프레임 창 안에서는 `null`을 돌려준다**(소스 맵의 좌석을 이 화면의 좌석으로 기록하지 않기 위해).
- **`reseatCellsToStoredCoords(was)`(**1861**)** — **반응 그 자체.** 반환 `null`(반응 없음) | `{moved, offGrid, visC, visR, held}`.
...
- 🔴 **호출 지점은 넷 그대로이고 앵커는 전부 이동했다**(2026-08-04 전건 재계수. `grep -c "reseatCellsToStoredCoords"` = **10** = 정의 1 + 호출 4 + **주석 5** — 구 총계 9는 낡았다):
  1. **격자 치수(cols/rows) 편집 리스너**(**588**, `initDOMElements` 안) — 「격자 치수도 기하 편집이다」. 「격자 치수도 기하 편집이다 — 물리 규격 한 칸을 고치는 것과 같은 연산」. 치수가 바뀌면 ① 원점 상자(원의 반지름은 칸 수로 고정인데 중심이 `visualCols / 2`라 격자가 넓어지면 원 전체가 미끄러진다)와 ② 다이 인덱스 자체가 **함께** 움직이고, 저장 좌표가 갈리는 것은 그 **차이**다. 실측(생산 프레임 3개 × 각 축 ±1~±3): **36건 중 16건이 어긋났고, 어긋난 16건은 예외 없이 셀의 100%였다**(261/261 · 273/273 · 461/461). ⚠️ **"상자가 안 움직였으면 반응할 것도 없다"는 참이 아니다** — QERWER 23→22열은 `box.minC`가 2에서 그대로인데 261칸 전부가 다시 번호를 받았다.
  2. **`onPhysicalGeometryEdit`**(**730** 선언, 호출 **742**, 리스너 배선 **747–748**) — ⚠️ **`initDOMElements` 안의 지역 화살표 함수이지 모듈 레벨 함수가 아니다.** 모듈 레벨에서 이 이름을 Grep하면 없다. `input`/`change`는 DOM 값이 **이미 바뀐 뒤** 뜨므로 이 리스너는 변경 전 상태를 스스로 잡을 수 없다 — 그래서 직전 렌더가 남긴 `cellsSeatedUnder`가 옛 좌표계다. **이 기록이 필요한 이유가 정확히 이것이다.**
  3. **`applyPhysicalGeometry`**(**2362** 선언, 호출 **2394**) — 파생 치수가 자리를 잡은 **뒤**, 렌더보다 **앞**. 순서가 전부다: 옛 치수로 앉히면 렌더가 새 치수로 좌표를 되만들어 저장 좌표가 옮겨가고, 렌더 뒤에 물으면 비교할 옛 좌표계가 이미 갱신돼 있다.
  4. **`resolveValidDie`의 `set()` 안**(**7982**).
...
- 모듈 상태 `validDie`(**2047**) · **`validDieResolveSeq`(**2072** — 경쟁 해소 세대 카운터이자 `getWaferBoundingBox`의 캐시 태그)**. 🪦 **`validDieRefTableTouched`는 삭제됐다**(위 묘비 표) — 구 지도가 `~2005`에 등재하던 이름이고 **지금 이 파일에는 선언이 없다.**
- `parseValidDieRef(meta, currentTable)`(**2080**) · `validDieBasis(state)`(**2121**) · **`isValidDieAt(physX, physY, circleInside, state)`(**2140** — 판정이 갈리는 유일한 지점. 참조가 없으면 인자로 받은 원 판정을 그대로 돌려준다)** · `buildValidDieTemplate(shape)`(**2172**) · `validDieRefDisplay`(**2205**) · `applyValidDieRef`(**2234**) · `validDieRefFromControls`(**2279**) · `validDieRefForPush`(**2286**) · `validDieRefPayload`(**2316**) · `validDieChainError`(**2342**). 상수 `VALID_DIE_TEMPLATE_PREFIX`(**2406**) · `VALID_DIE_TEMPLATE_OPTIONS`(**2407**) · `VALID_DIE_LIST_LIMIT=500`(**2413**).
- 🔴 **`resolveValidDie(meta, targetTable, homeMapKey)`(**7867** — 구 지도의 `~7568`은 **+299** 틀려 있었다)** — ⚠️ **시그니처는 3인자 그대로.** 세 번째는 선언 맵 자신의 키이고 **자기참조 체인 관문** 전용이다. 지역 클로저 `set(basis, keys, reason, ref, physPreset)`의 **순서가 계약**이다:
  1. **좌석 기록 확보** — 한 번도 그리지 않았으면 지금 잡는다. ⚠️ **여기서 지역 변수에 캐시하면 안 된다**(아래 `applyPresetObject`가 같은 반응을 먼저 한 걸음 돌린다).
  2. **`validDie` 대입.**
  3. **[규칙 ①] 기하 재로드** — `applyPresetObject(physPreset)`(호출 **7945**, 선언 **2508**)으로 참조의 **규격**을 갈아끼우고, 그 안의 `applyPhysicalGeometry`가 **새 규격에서 cols/rows를 다시 파생**한다. 🔴 **치수는 인자로 넘기지 않는다 — 규격에서 파생시킨다.** 삭제된 프레임 채택 함수가 거절당한 것은 참조가 **선언한** cols/rows를 베끼면서 셀은 칸에 그대로 둬 **273칸 전부의 좌표를 움직였기** 때문이다. **파생과 재배치는 한 쌍이고 한쪽만 하면 그것이 거절당한 그 동작이다.**
  4. **마스크 적합 격자 확장** — 파생 격자가 마스크를 온전히 담지 못할 때만, 담을 때까지만 **한 번에 한 칸**. 상한은 편집기의 치수 정의역(`frameDimBounds` **7673**)에서 온다. 상한에 닿으면 **더 넓히지 않고 멈추고 사유를 남긴다.** 🆕 ⚠️ **이 블록은 더는 인라인이 아니다 — `fitGridToMask`(**8283**)로 뽑혀 나왔다.** 슬라이스 하니스가 죽지 않는 이유는 그 함수가 **읽는 모든 바인딩을 인자로 받아 모듈 상태를 0으로** 유지하기 때문이고(본문은 인라인 시절과 바이트 동일), 그것이 이 추출의 성립 조건이었다.
  5. 🔴 **`boundingBoxCache = {}`** — 세대 카운터는 **진입 시** 오르는데 `validDie` 대입은 그 뒤다. **그 사이에 상자를 한 번이라도 물으면 옛 마스크로 만든 상자가 새 번호의 키에 실린다.**
  6. **`reseatCellsToStoredCoords(cellsSeatedUnder)`(호출 **7982**)** — 기하 프리셋 편집이 타는 것과 **같은 함수**. 🔴 **인자는 지금 읽는다.**
  7. **넷 이동량은 좌석 집합의 차이로 잰다**(🆕 `summariseReseat` **8343**) — 걸음마다 세면 서로 상쇄되는 두 걸음이 「N칸 이동 후 N칸 이동」으로 읽혀 **사용자에게 거짓 수를 준다.**
  ...
  - `frameDimError`(**7681**) · 저작·표시: `renderValidDieChip`(**8471**) · `syncValidDieRefControls`(**8510**) · `onValidDieRefChanged`(**8529**) · `populateValidDieRefList`(**8774**) · `enterValidDieAuthoring(shape)`(**8898**).
  - 🆕 **`saveValidDieRefDeclaration()`(**8560**) — 유효 다이 지정을 `wafer_map_metadata`에 되쓰는 유일 경로.** 🔴 **무엇을 쓸지에 두 번째 구현을 만들지 않는다** — 결정은 `validDieRefForPush()`(**2286**), 쓰기는 `applyValidDieRef()`(**2234**)이고 **⚡ Push가 쓰는 바로 그 두 함수**라 이음매 벡터가 채점한다. 여기서 `meta.valid_die_ref = …`를 직접 쓰면 저장 경로가 둘이 된다. ⚠️ **서버본에서 유래한 화면만 저장 권한을 가진다**(`loadedIdentity` 없으면 거절 — 어느 행을 고칠지 추측해서 남의 행을 고치지 않는다). 그리고 **`fetchGridMetaFor`의 「선언 없음(null)」/「확인 못 함(throw)」 구분을 그대로 쓴다**: 확인 못 한 규격 위에 되쓰면 cols/rows/START/회전/물리가 **한 번의 저장으로 사라진다**. **UI 규율 — 읽기는 무마찰, 쓰기는 확인창 정확히 1회.**
...
- 프레임: `editorFrames`(**7052**) · `snapshotEditorState`(**7084**)/`restoreEditorState`(**7149**) · `openMapFrame(spec)`(**7381**) · `popMapFrame`(**7454**) · `frameFromMeta(meta)`(**7628**) · `frameDimBounds`(**7673**) · `currentFrame`(**7689**) · `resolveFrame`(**7702**) · `frameAxesKey`(**7719**).
- 🔴 **[`cd3e0f4`] 오버레이 투영이 mm를 경유한다** — **`projectCellsToWaferMm(cells, frame)`(**7734**)**가 실제 계산이고, **`projectCellsToPhys(cells, frame)`(**7764**)는 그 결과에서 키만 옮기는 얇은 래퍼**다(시그니처·의미는 종전 그대로 — 유효 다이 해석의 입력이다). 🔴 **같은 수를 두 번 계산하지 않는다.**
...
- **`seatWaferMmInFrame(items, frame)`(**7779**)** — [규칙 6] 물리 mm 항목을 **타깃 프레임의 칸에 앉힌다.** 반환 `Map(다이키 → 항목 **배열**)`. 🔴 **대표값을 고르지 않는다**(사용자 확정 「전부 나열」): 타깃 피치가 소스보다 굵으면 한 칸이 소스 여러 칸을 받는데(실측 최대 6) 대표를 고르면 나머지를 조용히 버리면서 **한 값을 자신 있게** 보여 준다. 단 **같은 물리 위치의 같은 값은 접어 넣는다**(정보가 아니라 소스에 같은 행이 두 번 있을 뿐 — 접지 않으면 피치가 **같은** 맵도 fanout 2가 되어 「여러 값」 가져오기에서 제외된다). 위치가 같은데 값이 다르면 그것은 중복이 아니라 **충돌**이라 둘 다 남긴다. ⚠️ **앉히는 기준은 지금 화면의 프레임이다.**
- `canvasSeatKeys()`(**7828**, 캐시 `seatKeyCache` **7827**) · `reseatOverlayLayer(o)`(**9306**) · `overlayGeomSig`(**9368**)/`currentGeomSignature()`(**9370**)/**`syncOverlayGeometry()`(**9387**)** — ⚠️ **mm 좌석은 피치의 함수라 화면 규격이 바뀌면 다시 앉혀야 한다**(종전 다이 인덱스 키는 화면에 불변이었다). `overlayAlignChip`(**9411**)/`overlayFanChip`(**9437**).
- 오버레이 본체: `OVERLAY_COLORS`(**7503**) · `overlayLayers`(**7509**)/`activeOverlayLayers`(**7510**, 렌더 루프에서 재계산 금지용 캐시)/`overlaySeq`(**7511**)/`recomputeActiveOverlays()`(**7513**) · `drawOverlayMarkers`(**7580**) · `pushFailedOverlay`(**8966**) · `OVERLAY_CELL_LIMIT=2000`(**8991**) · `buildKeyFilters`(**8998**) · `addOverlayLayer`(**9012**) · `removeOverlayLayer`(**9276**)/`toggleOverlayLayer`(**9283**)/`clearOverlayLayers`(**9292**).
...
- **[U6] 맵 기본값**: `overlayContract`(**135**) · `EMPTY_DOE_SEED`(**276**) · `defaultLegendRows()`(**280**) · `declaredLegendRow(value)`(**289**) · **`LEGEND_PALETTE`(**3389**)** · `pickUnusedColor`(**3392**) · **`autoAddLegendValue(value, fallbackDesc)`(**3401** — 값이 legend에 자동 추가되는 유일 경로)**. ⚠️ **`pickUnusedColor`는 색을 *지어내는* 함수라 오버레이 점 경로에서 부르면 안 된다**(위 `legendColorForValue` 참조).
- **[F1/F3] 좌표 바인딩도 서빙받는다 — 클라 매칭기 0**: `servedBindingCache`(**146**) · `normalizeServedBinding`(**150**) · `fetchServedBinding(table)`(**166**) · `fetchPaintRules(table)`(**186**). `fillColumnDropdowns`(**1304**)의 드롭다운 preselect가 소비 지점. **`source:'fallback_guess'`는 경고**. ⚠️ ~~`deriveMapBinding`~~은 묘비이고 이 파일에 **주석 2건**(**142** · **8988**)으로만 남아 있다.
...
- 서버 IO: `REGISTRY_SCOPES=['map']`(**3697**) · `fetchRegistryRows`(**3698**) · `readRegistryScope`(**3723**) · `reconcileVocabClaims`(**3745**) · `applyRegistryRowsToLegend`(**3770**) · **`saveLegendToServer(mapKeyOverride)`(**3849** — 호출자는 `pushMapData` 하나)** · `zoneColumnsPresent`(**3964**)/`ZONE_COLUMNS`(**3971**)/`probeZoneColumns()`(**3972**) · `LEGEND_SAVE_MESSAGE`(**4000**) · `applyLegendSaveResult`(**4013**) · `legendDirty`(**4037**) · `getPlanSaveState`(**4040**) · `persistLegend`(**4053**) · **`scheduleCellDraft`(**4075**)** · `renderLegendMetaOnly`(**4084**).
  - 🔢 **`scheduleCellDraft` 호출은 14곳 그대로이고 앵커는 전부 이동했다**(2026-08-04 전건 재계수, `grep -c "scheduleCellDraft"` = **15** = 정의 1 + 호출 14): **1074 · 3313 · 4270 · 4399 · 4461 · 5272 · 5320 · 5530 · 6061 · 6095 · 6117 · 7014 · 8958 · 9532.** ⚠️ **개수만 확인하고 통과시키면 14개가 전부 틀린 채로 남는다.**
- 로컬 초안: `seedEmptyDoe`(**3418**) · `saveLegendToStorage`(**3432**) · `doeDraftKey`(**3440**) · 🔴 **`DRAFT_VERSION = 4`(**3466**)** · `cellsDigest`(**3470**) · `draftBase`(**3482**) · **`serverCellKeys`(**3492**)/`serverCellKeySet()`(**3496**)** · **`saveDoeDraft`(**3503**)** · `readDoeDraft`(**3540**) · `clearDoeDraft`(**3553**) · `applyDoeDraftRecord`(**3612**) · `applyDraftCells`(**3649**).
- last-open 복원: `LAST_OPEN_KEY`(**3561**) · `recordLastOpenMap`(**3563**) · `restoreLastOpenMap`(**3582**).
- 패널 관문: `addLegendRowForPanel`(**4328**) · **`updateLegendRowForPanel(value, patch)`(**4339**)** · `deleteLegendRowForPanel`(**4384**). + `mapKeyColumnCache`(**4404**)/`fetchMapKeySpec`(**4409**)/`fetchMapKeyColumns`(**4430**) · `probeMapExists`(**4435**) · `remapGridValues`(**4452**).
- 레전드/브러시: `renderLegendTable`(**4101**) · `selectBrush`(**4293**) · `getCurrentMapKey`(**3665** — **로드된 맵이 아니라 현재 메타 입력 필드**를 읽는다) · `computeLegendCounts`(**2841**)/`updateLegendCounts`(**2848**).

#### 데이터 IO · 렌더 · 편집 도구

- **`loadExistingMap(opts={})`(**4521**)** — 🔴 **「유효 다이맵 → 오리진 → 셀 위치」가 이 함수의 순서 계약이다**(단계 7종은 [분해 표](#7-client2src--웹-클라이언트) 참조): 회전·면·격자 컨트롤 동기화 → **`await resolveValidDie(…)`** → `boundingBoxCache = {}` → **[규칙 ①-b] 치수를 되읽는다**(유효 다이 해석이 참조 규격에서 치수를 다시 파생시켰을 수 있다) → **그다음에** 셀 루프. ⚠️ **여기서 START X,Y는 되읽지 않는다.** 🔴 **로드 경로에서 재배치는 무비용이다** — `gridData`·`loadedFCells`·`serverCellKeys`를 `resolveValidDie`보다 **먼저** 비우므로 순회할 것이 없다. 사용자 취소는 `{count:0, cancelled:true}`.
  ...
  - `fetchGridMetaFor(table, mapId)`(**4474**)는 **404/405만 "규격 미등록"(null)**로 읽고 그 외 실패는 **throw**한다.
- **`pushMapData()`(**5394**)** — 저장 본체이자 **legend/DOE 저장의 유일한 트리거**(단계 5종은 [분해 표](#7-client2src--웹-클라이언트) 참조). 관문 본체 `logShapedPushDecision`(**5388**, 모든 다이얼로그보다 앞) · 장치 `PUSH_SYSTEM_COLUMNS`(**5347**)/`getUnprotectedPushColumns`(**5369**). 저장 가능성 술어 3종 `eachSavableCell(fn)`(**2770**)/`classifyUnsavableCells()`(**2803**)/**`pushBlockingCount(u)`(**2836**)** — 🔴 **소비자는 `pushMapData`의 관문 하나다.**
- 렌더: **`getGridCellObject`(**795**)** · `getGridCellFromMouseEvent`(**834**) · `scheduleRenderGridCanvas`(**2950**) · `updateSideIndicator`(**2961**) · `fitGridToWorkspace`(**2972**) · `renderGridCanvas`(**2987**) · `handleCellClick`(**3262**) · `updateCellStyles`(**3316**) · `updateNotchPosition`(**3330**). 테마·색: `rebuildThemeColorCache`(**2874**)/`getThemeColors`(**2900**)/`UNLISTED_VALUE_FILL`(**2907**)/`cellFillColor`(**2912**)/`parseCssColor`(**2921**)/`toExcelHex`(**2932**).
  - 🔴 **`getGridCellObject`는 `getCanvasCellFromDb`를 부르지 않는다 — 식을 인라인한다**(`isOriginCell` 판정 **819**). 사유: **이 함수를 슬라이스해 실행하는 하니스가 둘**(`company_roundtrip`·`copy_header_count`)이고 모듈 전역 의존이 하나 늘 때마다 그 둘이 `ReferenceError`로 죽는다. 상자도 같은 `getWaferBoundingBox` 하나이므로 **새 유도가 아니라 같은 식의 특수값**이고 두 갈래로 갈릴 수 없다.
  - `renderGridCanvas` 쪽은 **호출을 쓴다**(`isOriginCell` **3090**) — 이 함수는 슬라이스 대상이 아니기 때문이다. 🆕 **그리고 이 함수가 좌석 기록의 원천이다**: `cellsSeatedUnder = seatingSnapshot() || cellsSeatedUnder`.
- 프리셋: **`applyPresetObject(preset)`(**2508**)** — 🔴 **호출자 전부의 관문이고 이번에도 5곳 그대로다**: **2632 · 2661 · 5139 · 7430 · 7945**. `declaredRot`/`declaredSide`가 화면과 다르면 **info 토스트 1회**로 "규격만 적용했습니다"라고 말하고 **일치하면 침묵**한다. · `applyRoutedPreset(table, mapKey)`(**2601**) · `fetchAndRenderPresets`(**2435**)/`renderPresetDropdown`(**2452**)/`loadSelectedPreset`(**2643**)/`saveCustomPreset`(**2668**)/`deleteCustomPreset`(**2720**) · `applyPhysicalGeometry`(**2362**) · `updateOrientationUI`(**2415**) · `serverPresets`(**2402**).
- 편집 도구: `clearGrid`(**5261**) · `fillGrid`(**5275**) · `getEdgeClassification`(**5916**) · `getVisualGridDimensions`(**5989**) · `selectEdgeCells`(**5999**) · `autoPaintE1E2`(**6026**) · **`fillSelectedCells`(**6072**)/`clearSelectedCells`(**6098**)** · `writeClipboardRich`(**6133**) · `copyGridToExcel`(**6406**).
- **[F1ⓑ] COPY HEADER MODE**: `COPY_HEADER_KEY`(**6200**) · `colHeaderWord`(**6204**) · **`auxHeadWords()`(**6211**)** · `copyHeaderEnabled`(**6215**) · `mapKeyGroupLabel`(**6227**) · `copyHeaderGroups`(**6240**) · 픽셀 상수 `HDR_COL_PX`/`HDR_PAD_PX`/`HDR_CHAR_PX`/`HDR_MIN_SPAN`/`HDR_MAX_SPAN`(**6269–6273**) · **`HDR_GAP_COLS=1`(**6277**)** · `headerSpanFor`(**6280**) · `distributeSpans`(**6290**) · **`auxColumnSpans`(**6306**)** · `copyHeaderAuxRows`(**6314**) · **`copyTitleText()`(**6330**)**.
- **[MEDIUM-3] 노치 지문 술어 2종**: `computeNotchCell(rotation, side)`(**6348** — bbox를 `circleOnly: true`로 묻는다(**6354**), **이 옵션의 유일한 소비자**) · **`notchMarkCell(rotation, side)`(**6397**)**.
- **[F1ⓑ] 회사 본딩맵 시트 왕복 붙여넣기**: `pasteBlank`/`pasteAt`(**6696/6697**) · **`auxHeaderInLine(line)`(**6720**)** · **`readCompanyMapBlock(text)`(**6755**)** · **`checkPasteAgainstFrame(parsed, frame)`(**6825**)** · `applyPastedGridRows`(**6891**) · `pastedCellCount`(**6914**) · `applyPastedAuxRows`(**6931**) · **`onMapGridPaste(e)`(**6962**)**. ⚠️ **이것은 `transfer_plan.js`의 패널 붙여넣기와 다른 경로다** — 이쪽은 `e.defaultPrevented`를 먼저 보고 **패널이 이미 처리했으면 양보한다**. 그리고 회사 시트 쪽은 **VALUE로 주소를 매기고**(개명 없음) 패널 쪽은 **위치 기반**이다.
- **메타 입력칸의 값 제안(datalist) 블록** — `KEY_SUGGEST_DEBOUNCE_MS=120`(**8681**)/`COLUMN_VALUE_LIST_LIMIT=50`(**8684**) · `markSuggestState`(**8688**) · `listFillSeq`(**8704**)/`claimListFill`(**8705**)/`fillDatalist`(**8711**) · `mapKeyListCache`(**8723**)/`populateMapKeyDatalist`(**8725**) · `populateOverlayKeyList`(**8779**) · 캐시 3종 `columnValueComplete`/`columnValueRefused`/`columnValueTruncated`(**8788–8790**) · `colValueKey`(**8792**)/`dropColumnValueCache`(**8796**)/`canReuseComplete`(**8811**)/`populateColumnValueDatalist`(**8816**) · `onMetaInputSuggest`(**8885**, 배선 **625**). 하니스: `client2/tests/map_key_datalist_harness.mjs`. ⚠️ **그리드 셀의 값 제안(`value_suggest.js`)과 별개 구현이다** — 이쪽은 네이티브 `<datalist>`이고 저쪽은 AG-Grid 커스텀 에디터다.
- 프레임·정체성 보조: `loadedIdentity`(**7073**)/`framePushed`(**7074**)/`frameTouched`(**7082**) · `applyGridMetaObject`(**7226**) · `findPresetByKind`(**7246**) · `applyCellsToGrid`(**7266**) · `collectPlanCells`(**7289**) · `currentIdentityMismatch`(**7308**)/`setLoadedIdentity`(**7315**) · `frameTitle`(**7331**)/`currentFrameTitle`(**7336**)/`renderBreadcrumb`(**7340**) · `switchTableQuiet`(**7357**).
- 진입·테이블: `debounce`(**329**) · `el`(**408**) · `initDOMElements`(**409**) · `initPlanSidebarResizer`(**902**) · `initMouseDragEvents`(**959**) · `loadTablesList`(**1080**) · `switchTable`(**1148**) · `renderMetadataInputs`(**1219**) · `getBaseColumnName`(**1296**) · `fillColumnDropdowns`(**1304**). 공수 계기: `ROUTE_MAIN`(**7068**)/`ROUTE_MATERIAL`(**7069**)/`effortRoute()`(**7070**).
