...
### `server/enrichment/config.py` (**1,638줄**(🆕⑨ `5359fdd` 실측 — 구 표기 676), `c520012` 585에서 **+63** — `f9289f6`) — 인리치먼트 규칙 로더/검증
...
| 🆕㉕ ↳ 합성기 :878~:945 — `AUTO_CONFIRM_MAPPER` :878 · `DEDUP_PREFIX` :883 · `AUTO_CONFIRM_PREFIX` :884 · `synthesized_rule_names(rule_name)` :887 | **인리치 규칙 하나가 «체인 규칙 둘»을 낸다**(S-179 ①, 판정 292): dedup 과 auto-confirm. 두 이름을 «한 자리»가 짓고 충돌 검사도 그것을 읽는다 — 「합성된 이름이 어떻게 생겼나」에 대해 거절과 합성이 «어긋날 수» 없다. 자동 확정은 `follow_up` 이라 커밋 경로가 아니라 «뒤따르기»에서 돈다. ⚠️ **[2026-09-12 재측] 줄 번호가 밀렸다** — D-11 은 :992~ 라 적었고, 오늘 `git grep` 은 위 넷이다 | — |
...
### `server/virtual_join/config.py` (**697줄**(🆕⑨ `5359fdd` 실측 — 구 표기 687), `4e06eec` 신설 540줄 → `b6942ec`로 **475**) — 가상 조인 선언 로더/검증
...
### `server/virtual_join/executor.py` (**631줄**(🆕⑨ `5359fdd` 실측 — 구 표기 584), `ed9cfdb` 535에서 **+19**) — 가상 조인의 **실행** 절반
