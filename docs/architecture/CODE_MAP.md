...
| `bulk_upsert_cell_sources(db, mappings)` / `bulk_upsert_cell_overwrites(db, mappings)` | 메타 테이블 벌크 업서트(ON CONFLICT) |
| `bulk_delete_cell_overwrites(db, delete_keys)` | overwrite 벌크 삭제 |
| `_get_or_create_row(db, table_model, update_item, row_cache, table_name) -> (row, is_new)` | row_id/비즈니스키로 행 확보(캐시 활용). ⚠️ **HEAD(`e943e46`)에서 이 함수는 프리페치의 부재 증명을 읽지 않는다** — 그래서 신규 행 삽입은 여전히 행당 `SELECT`를 낸다(아래 예산 테스트가 **일부러 고정한 미수리 항목**). 🔴 **이 행은 미착지 변경이 곧 뒤집는다 — 아래 [P3] 블록을 먼저 읽어라** |
...
| `_update_row_business_key(row, key_col, update_item, row_cache)` | 비즈니스 키 갱신 |
...
**어디서**: `apply_batch_updates`가 조립하고 → `apply_row_update_internal`(인자로 받아 전달)이 실어 나르고 → `_load_metadata_row_cell`이 소비한다. 적중의 전제는 `assemble_composite_business_key`다.
...
#### 🔴 [P3 · 2026-08-07] **이 절을 다시 바꾸는 미착지 변경이 워킹트리에 있다 — 커밋 전이므로 아직 지도가 아니다**

> 🔬 **판정 기준**: 이 지도의 모든 서술과 마찬가지로 위 표는 **커밋된 상태**(`e943e46`)를 적는다. 아래는 그 상태가 **아직 아닌 것**이고, 착지하면 위 `_get_or_create_row` 행과 [P6] 블록 ③이 **동시에** 틀린다. 미리 고쳐 두지 않는 이유는 이 문서가 반복해서 지불한 값 때문이다 — **구현 24시간 전의 동작을 문서화하면 그 서술은 앵커 두 개가 실재하는 함수를 가리키는 동안 계속 참처럼 읽힌다.**

**HEAD(`e943e46`)에서 참인 것 — 이것이 지금 지도다:**
- `_get_or_create_row(db, table_model, update_item, row_cache, table_name)` — 인자 **다섯**, 프리페치 관련 인자 **없음**.
- 그래서 신규 행 삽입은 행당 `SELECT`를 낸다. [P6] 프리페치의 부재 증명은 `_load_metadata_row_cell`만 읽는다.

**워킹트리에서 준비 중인 것(미커밋, `server/database/crud.py` +264/−33 실측):**
- `_get_or_create_row(..., table_name, probed_identity: "ProbedIdentity" = None)` — **여섯 번째 인자가 뒤에 붙는다**(앞이 아니라 뒤이고 기본값이 있으므로, 이 시그니처 변경은 기존 호출을 조용히 틀리게 만들지 않는다 — `map_editor.js`에서 `frame`이 **선두에** 끼었던 것과 반대 성질이다).
- 새 헬퍼 `_absence_is_proven(value, probed) -> bool` — 「배치 프리페치가 이 값을 물었고 아무것도 못 가져왔다」.
- 유일 호출부 `apply_row_update_internal`이 `probed_identity=probed_identity`로 넘긴다. **다른 모든 호출자는 `None`을 넘겨 오늘의 두 질의를 그대로 유지한다**(소스 주석이 그렇게 못박는다).

🔴 **착지 전까지 이 블록을 위 표로 옮기지 마라.** 착지하면 ① 위 `_get_or_create_row` 행의 시그니처와 ⚠️ 문장 ② [P6] 채점자 항목 ③ — 셋을 **같은 커밋 해시 기준으로** 다시 적어야 한다.
...
### 🆕 `server/map_alignment.py` (**5,961줄**) — 프레임 정렬의 **채점자**
...
>
> ⚠️ **`069b4e9`가 이 파일에 +283/−8을 넣었다.** 이 절은 이미 라인을 걷어낸 뒤라 표에서는 아무것도 깨지지 않았지만, **문서 밖에서 라인으로 인용하던 값은 전부 밀렸다.** 실측 예 — `direction_violations`의 **호출부**가 `3079` → **`3167`**, 그러나 **정의는 `1513`**이다.
...
| 🆕 **`serpentine_index`** | **`serpentine_index(cells, top_is_min_y: bool = True) -> dict`** |
...
- ⚠️ **의도된 비대칭**(소스 주석 **621–625** @`e943e46`가 명시 — 🔴 **구 지도의 `559–565` @`5609ff0`는 낡았다**): `confirmed_meta_for`는 이 술어를 **재사용하지 않고** 더 엄격한 `!= GEOMETRY_DECLARED`를 쓴다. **채점은 「빌려야 하나」를 묻고 쓰기는 「여기 써도 되나」를 묻는다.**
...
| 심볼 | 무엇인가 |
|---|---|
| **`serpentine_index(cells, top_is_min_y=True) -> dict`** | 유효 다이 집합 → `{순번: (x, y)}`, 1부터. **phys도 메타도 안 읽는다.** 규칙 셋을 **명시적으로** 고정한다: ① 행 순서(맨 위 행부터) ② 방향 교대(**셀이 있는 행에서만** — 통째로 빈 행은 방향을 뒤집지 않는다) ③ **행 안의 빈칸은 번호를 먹지 않는다** |
| **`serpentine_rank(cells, top_is_min_y=True) -> dict`** | 같은 훑기의 **역**: `{(x, y): 순번}`. 🔴 **훑기를 두 번 구현하지 않는다 — `serpentine_index`를 뒤집는다.** 좌표 중복 시 **먼저 훑힌 번호가 남는다** |
...
| **`view_model.js`** | **1,197**<br>(구 1,073) | 세션+페이로드+판정 → **화면이 보여 줄 문자열과 플래그 정확히 그것**. 순수, DOM 무지 | `VIEW_STATE` · `UNKNOWN`(`'미상'`) · `WORDS`/`ATTRIBUTION`/`EVIDENCE`/`REFERENCE_KIND_WORD` · `referenceOptionLabel(item)` · **`buildViewModel(input)`** · `agreementText(agree, discriminating)` · `marginText(marginDies)` · `HEADLINE`/`CAUSE` · `selectAlignmentRules(rules)` · `CROSS_SOURCE_ROW_ID` · `countCoordinatePairs(columnNames)` · `assertNoRatio(vm)` |
...
| **`main.js`** | **2,437**<br>(구 1,975) | **합성 루트 — DOM이 존재한다는 것을 아는 유일한 모듈** | `ELEMENT_IDS` · `normaliseWorklist(res)` · **`bootstrap(deps)`** · `adaptPayload(raw)` · `framesFor(payload, candidateId)` · 🆕 **`seatingFor(payload, candidateId, cells)`** · 🆕 **`floorSeating(payload)`** · `paintCandidateThumbs(surfaceFor, payload, source, viewport, palette)` · 재수출 `{ VIEW_STATE, createApiClient, spellFrame }`, `spellFrame` 정의) |
...
### `enrichment.js` (**905줄**) — 인리치먼트 컨베이어 페이지 (export 없음)
