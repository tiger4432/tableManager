...
> **Status:** 🟢 Living | **Last-verified:** 2026-07-28 (**HEAD `a98dc72`의 커밋된 blob 실측** — 워킹트리 아님. [§0 측정 기준](#0-묘비-목록--소스에-존재하지-않는-이름)) | **Owner:** 전 에이전트 공용 | **Source-of-truth:** 각 표의 코드 경로
...
> ```
>
> **측정 기준 (라인 앵커가 가리키는 상태)**: **전 절이 HEAD `a98dc72`의 커밋된 blob 실측**이다(`git show a98dc72:<path>` 기준, 2026-07-28). 검증용 blob 해시(`git rev-parse a98dc72:<path>` 선두 7자):
> `server/transfer_plan.py` = `6740e30` · `contracts/band_arithmetic/vectors.json` = `861a031` · `client_harness.mjs` = `ebd25cc` · `client2/src/map_editor.js` = `833e6b4` · `client2/src/transfer_plan.js` = `e72e358` · `client2/src/doe_bands.js` = `3da00fb` (`git hash-object <path>`로 대조). **이 값이 다르면 해당 절의 라인 앵커는 재측정 대상**이고, 함수명으로 Grep해서 쓰라.
>
> 💡 **이 해시가 실제로 일한 기록**: 한 패스에서는 `vectors.json`이 도중에 바뀌어(`17698dd`→`8696ea7`) 막 적은 서술이 즉시 낡은 것을 잡았고, 한 패스(2026-07-28 오전)에서는 다섯 해시 전부가 불일치로 나와 **다섯 파일 전체 재측정**의 근거가 됐다 — zone 모델 착지(`b35bc9f`)로 `transfer_plan.py`가 1,815→3,019줄, `map_editor.js`가 4,873→5,466줄이 됐고, 같은 날 후속 패스(`2baf9ff` U9 marker + `6db517d` H1/H2)에서는 계약 파일 둘만 일치하고 소스 셋이 전부 불일치라 재측정 범위를 정확히 그 셋(+`doe_bands.js`)으로 좁혀 줬다. 검사는 통과할 때도 실패할 때도 일을 한다. 같은 날 U6 패스(`95bf072`)에서는 소스 셋(`map_editor.js` +81 · `transfer_plan.js` +27 · `doe_bands.js`)이 불일치, 계약 둘이 일치로 나왔다 — 단 `doe_bands.js`의 불일치는 **주석 한 줄 리포인트**였다: 해시는 "변했다"만 말하고 "앵커가 밀렸다"는 재측정이 판정한다.
>
...
| 소형 서버 모듈 (models/std_parser/enrichment_*/ingestion_activity/ingestion_checkpoint/bonding_plan/**map_overlay**/**transfer_plan**) + 그래프 트랙(graph_sync_worker/graph_materializer/ontology_config) + **운영 6종**(paths/process_supervisor/health/heartbeat/product_tables/**config_backup**) | ~9,850 | [§5](#5-소형-서버-모듈) |
| 기타 서버 모듈 (한줄 요약) + 설치·개발환경 스크립트 + **교차 구현 계약 `contracts/`** | — | [§6](#6-기타-서버-모듈-한줄-요약) |
| `client2/src/*` | ~21,700 (js 18,884 + css 2,810) | [§7](#7-client2src--웹-클라이언트) |
...
### `map_editor.js` (~5,586줄, **U6 `95bf072`로 +81**) — 웨이퍼 맵 에디터 (단일 페이지 스크립트, export 없음 — 단 `import`는 있다: `transfer_plan.js`의 `bandToState`·**[U6] `stageTargetTables`**, **`doe_bands.js`의 `parseMaterialList`·`bandsToZones`**)
- **[U6 `95bf072`] 서빙된 맵 기본값 — 클라 사본 0**: `overlayContract`(~91 — paint-rules 응답에 실려 오는 `{valueColumnCandidates, defaultLegend}` 캐시, 첫 성공 fetch까지 null. **"미조회/불가"는 "자동감지 불가·빈 시드 1행"으로 동작**하고 절대 내장 목록이 아니다) · `EMPTY_DOE_SEED`(~188 — "선언 없음" arm의 빈 DOE 1행, 구 ~~`DEFAULT_LEGEND`~~) · `defaultLegendRows()`(~192 — 선언 > 시드) · `declaredLegendRow(value)`(~201 — 값 자동 추가 시 선언 행의 color/desc 우선) · **`LEGEND_PALETTE`(~2356 — 12색 사본 3곳 통합)** · `pickUnusedColor`(~2359) · **`autoAddLegendValue(value, fallbackDesc)`(~2368 — 값이 legend에 자동 추가되는 **유일 경로**: 패널 [+ 값]·E1/E2 autopaint·미지 값)**. 값 컬럼 자동감지 소비 2곳: `fillColumnDropdowns`(~1260, 서빙 목록 없으면 자동감지 없음) · `deriveMapBinding`(~5082). `switchTable`(~1125)이 스키마 fetch와 병렬로 paint-rules 왕복을 **await**한다.
- 좌표 변환 코어: `getPhysicalCoords`(~1328) `getCellFromPhysicalCoords`(~1378) `getCellFromVisualCoords`(~1418) `getVisualCoords`(~1487) `getTransformedPhysicalConfig`(~1502) `getWaferBoundingBox`(~1435) `getScreenShift`(~1537) `isCellInsideWaferFast`/`isCellInsideWafer`(~1563/1601) — 회전/면반전 불변식은 [MAP_EDITOR_SPEC](../spec/MAP_EDITOR_SPEC.md).
  - **[7d931dc] 프레임 창(frame window)** — `physFrameOverride`(~1301) + `physNum`(~1304)/`gridDimNum`(~1313)/`withPhysFrame(frame, fn)`(~1322). 변환 함수가 규격을 DOM에서 읽는 지점을 잠깐 갈아끼우는 장치로, **주입 지점은 `getTransformedPhysicalConfig`·`getWaferBoundingBox` 두 곳뿐**이다. `withPhysFrame`은 **동기 전용**(내부 `await` 금지 — `try/finally` 복원이 새면 조용한 오답). 기존 `parseFloat(v) || dflt` 규약(0 → 기본값) 보존.
- 캔버스 렌더: `renderGridCanvas`(~1953, 본체 — 오버레이 마커 호출 ~2115) `scheduleRenderGridCanvas`(~1916) `fitGridToWorkspace`(~1938) `updateNotchPosition`(~2297).
- 데이터 IO(REST — WS 아님): `loadExistingMap(opts={})`(~3438 — **[`280ebf0`] `{quiet}` 옵션**은 last-open 부트 복원용. **[`6db517d` H1] 로드 경로는 legend 채택 시점에 초안을 저장하지 않는다** — 구 `saveLegendToStorage()` 즉시 호출이 방금 로드한 **서버 상태**를 초안 위에 덮어써 리로드마다 칠한 셀 초안을 전멸시켰다(사유 주석 ~3739–3742). 저장은 초안 우선순위 판정이 끝난 뒤 registry 블록 안에서 **1회**만. 복구된 초안은 여전히 미저장 편집이므로 `legendDirty = true` ~3800. **[U6-1 `95bf072`] 0셀 로드는 registry 적용 전에 `seedEmptyDoe()`로 리셋**(~3775, `read.ok` 안에서만): `uniqueVals.size > 0` 가드가 페인트 유도 legend 재구성을 건너뛰어 **같은 테이블의 직전 맵 legend가 새 빈 맵에 상속**되던 결함 — 읽기 실패 시엔 화면 유지(미상 서버 상태는 "빈 것"이 아니다), 초안 우선순위 블록은 종전대로 그 뒤에 이긴다) `pushMapData`(~3896, 저장 본체 — **[`b35bc9f`] legend/DOE 저장의 유일한 트리거이기도 하다**: 내부에서 `saveLegendToServer` 호출 ~4140. **[`6db517d` H2] 대비 관문(contrast guard) ~4001–4028**: 직렬화된 페이로드의 비어 있지 않은 셀 수 < 화면의 비어 있지 않은 셀 수면 **confirm 이전에 적재 거부** — `replace_map`은 맵 행 전체를 지우고 다시 쓰므로 프레임이 덮지 못한 셀은 누락이 아니라 **삭제**된다(QA 실측 1293→379, 메타 없는 맵 + 기본 프레임). 데이터 보호 관문 3형제의 셋째(zone 컬럼 부재·레거시 판독 불가와 같은 패턴 — 직렬화하지 않은 것을 지우는 쓰기를 막는다). 계산은 루프와 같은 공백 술어라 사용자가 지운 셀은 양쪽에서 상쇄) `fetchGridMetaFor(table, mapId)`(~3405) 프리셋 `fetchAndRenderPresets`/`saveCustomPreset`/`deleteCustomPreset`(~1658/1764/1814) + `applyPresetObject`(~1714, `loadSelectedPreset` ~1747에서 추출한 공용 함수).
...
- 레전드/브러시: `renderLegendTable`(~3048) `selectBrush`(~3236 — **[`280ebf0` 입력 랙 수정] 선택은 DOE 리스트 재구성·전 그리드 카운트 스캔을 유발하지 않는다**(의도 주석 ~3250): mousedown이 innerHTML 재구성을 유발해 커서 밑 input이 분리되던 결함) + `saveLegendToStorage`(~2399). `getCurrentMapKey`(~2609)는 **로드된 맵이 아니라 현재 메타 입력 필드**를 읽는다(오버레이 관문 F2의 근원). 서버 레지스트리 IO는 아래 전용 블록 참조.
- **Split Registry = DOE의 유일한 기록자**(~216–507 선언 · ~2641–3046 IO). **[`269b39e`+`b35bc9f` ZONE 모델]** legend 행 = DOE 1건 = `{value, desc, color, knobs, stack, mat_1h[], mat_mid[], mat_top[]}` — zone 계약 정본 주석 ~247–267(`bands`는 **폐기됐지만 읽기 전용으로 생존**: 서버에 band 계획이 남아 있고 저장이 `replace_map`이라, 안 읽으면 다음 키 입력이 그 계획을 빈 집합으로 지운다). `SPLIT_REGISTRY_TABLE='map_split_registry'`(~216) · **`SPLIT_KEY_SEP='\|'`(~219)** · `buildSplitKey`(~244).
  ...
  - 서버 IO: `REGISTRY_SCOPES=['map']`(~2641 — **테이블 전체 어휘 시드는 삭제됐다**, `269b39e` 결함의 원인) `fetchRegistryRows`(~2642, `limit=500` — **부분 읽기는 읽은 게 아니라 throw**) `readRegistryScope`(~2667) `legendRowSignature`(~2678) `reconcileVocabClaims`(~2699) `applyRegistryRowsToLegend`(~2724 — 빈 legend 폴백도 **[U6] `defaultLegendRows()`** 경유) **`saveLegendToServer(mapKeyOverride)`(~2803 — 쓰기 단위가 **한 맵의 값 전체 집합**이라 `replace_map:true`. 지운 값·구간·자재는 집합에 없다는 것만으로 서버에서 삭제된다. 쓰기 **전에** ① `probeZoneColumns` 게이트(~2921, `ZONE_COLUMNS` ~2920 — 물리 zone 컬럼이 없으면 **거부**: crud가 페이로드에서 zone을 떨어뜨린 채 replace_map이 나가면 계획 전체가 층 구조 없이 대체된다) ② 재조회+지문 비교(불일치면 `legendConflict`로 거부). **참조는 정확히 2곳 — 정의 + `pushMapData`(~4140)**)** `LEGEND_SAVE_MESSAGE`(~2949) `applyLegendSaveResult`(~2962) `getPlanSaveState`(~2989) `persistLegend`(~3002) **`scheduleCellDraft`(~3023 — 400ms 디바운스로 `saveDoeDraft`+`notifyLegendChanged`. 페인팅·드래그·fill·paste·legend 개명까지 **호출 10곳**(1055/2280/3213/3342/3392/3890/4322/4349/4371/5465 — 2026-07-28 U6 후 재계수) — 모든 편집 경로가 새로고침을 견딘다)** `renderLegendMetaOnly`(~3031). ⚠️ **~~`scheduleLegendServerSave`~~(자동 저장)는 삭제** — 묘비 주석 ~2976, 서버 쓰기는 ⚡ Push 하나뿐([§0](#0-묘비-목록--소스에-존재하지-않는-이름)).
  - 로컬 초안(**[`b35bc9f`] 셀까지 나른다**): `seedEmptyDoe`(~2385 — 행 없는 맵의 시드, `vocab:true` — **[U6] 시드 원천은 `defaultLegendRows()`**: 서빙된 `default_legend` \| `EMPTY_DOE_SEED` 1행) `doeDraftKey`(~2407, `map_doe_draft::<table>::<mapKey>`) `DRAFT_VERSION=3`(~2429) `cellsDigest`(~2433, FNV-1a) `draftBase`(~2445 — 열었을 때 서버가 갖고 있던 것의 지문) **`saveDoeDraft`(~2447 — DOE(zone·knob 있는 행만)+`cells`+기반 지문을 함께 저장 — 초안 지문 우선순위의 근거)** `readDoeDraft`(~2484, v3 미만은 "기반 미상"으로 수용) `clearDoeDraft`(~2497) `applyDoeDraftRecord`(~2556 — **우선순위 판정은 호출부**, 이 함수는 적용만. **[`6db517d` H1] "applied"의 의미가 "내용이 있었다"→"화면을 바꿨다"로 교체**됐다(~2559–2585, before/after JSON 비교): 초안은 registry 저장 직후에도 재저장되므로 Push+새로고침 뒤 초안 = 서버 행인데, 그것을 "복구"로 보고하면 계획 있는 맵을 리로드할 때마다 유령 미저장 칩·토스트가 부활한다) `applyDraftCells`(~2593).
  - **[`280ebf0`] last-open 복원**: `LAST_OPEN_KEY='map_editor_last_open'`(~2505) `recordLastOpenMap`(~2507 — **depth-0 정체성만**: 자재 프레임은 여정이지 집이 아니다. 기록 지점은 로드 성공 ~3844·Push 성공 ~4125) `restoreLastOpenMap`(~2526 — 부트 1회 호출 ~646. **수동 경로 그대로**(switchTable+메타 입력+`loadExistingMap({quiet})`)를 걷는다 — 초안 우선순위·missing-key 동작이 별도 복원 경로가 아니라 같은 코드. 테이블이 사라졌으면 조용히 초기 화면).
  - 패널 관문(컨트롤러가 부르는 쓰기 3종): `addLegendRowForPanel`(~3271 — **[U6] 색 선정은 `autoAddLegendValue` 경유**) **`updateLegendRowForPanel(value, patch)`(~3282 — DOE 변조의 유일 관문. 배열 필드는 **새 배열로 통째 교체**, 제자리 수정 금지)** `deleteLegendRowForPanel`(~3327). + `fetchMapKeyColumns`(~3348) `probeMapExists`(~3366 — **존재를 추측하지 않는다**) `remapGridValues`(~3383).
- 편집 도구: `fillGrid`(~3866) `getEdgeClassification`(~4180) `selectEdgeCells`(~4260) `autoPaintE1E2`(~4287 — **[U6] E1/E2 고정 색 삭제**: `autoAddLegendValue` 경유, 선언 행 > 팔레트 규칙) `copyGridToExcel`(~4374).
- 프레임 스택: `snapshotEditorState`(~4546) `restoreEditorState`(~4602) `openMapFrame`(~4826 — `allowEmpty`로 빈 그리드 오픈 허용, 키는 Push에서 생성) `popMapFrame`(~4880). **복원 대상에 `overlayLayers`·캔버스 스크롤이 포함된다**(~4538 주석).
...
- **[7d931dc] 오버레이 레이어**(~4923–5586) — **변환은 클라 단일 구현**이다. 계약은 [MAP_EDITOR_SPEC §5](../spec/MAP_EDITOR_SPEC.md). ⚠️ **[`280ebf0`] `.overlay-box`/`.ov-*` CSS는 `b35bc9f`의 transfer_plan.css 재작성이 떨어뜨렸다가 원문 그대로 복원**됐다(마크업은 계속 그 클래스를 쓰고 있었다):
  - 상태 `OVERLAY_COLORS`(~4923) / `overlayLayers`(~4924) / `activeOverlayLayers`(~4925) / `overlaySeq`(~4926) / `recomputeActiveOverlays`(~4928, 렌더 루프 내 재계산 금지) / `drawOverlayMarkers`(~4933, 렌더 호출 ~2115).
  - **프레임 계산**: `frameFromMeta(meta)`(~4957, `grid_metadata` JSON → 프레임 기술자. **없는 물리 항목은 undefined로 남겨** 현재 화면 값 폴백) / `currentFrame()`(~4984) / `resolveFrame(frame)`(~4997, 축 전부를 실값으로 확정) / `frameAxesKey(rf)`(~5014, 회전·면·y반전·START·치수·물리 6종 = identity/derived 판정의 유일한 근거).
  - **`projectCellsToPhys(cells, frame)`(~5025)** — `getCellFromVisualCoords` → `getPhysicalCoords`를 **소스 프레임을 씌운 채** 호출한다. `loadExistingMap` 셀 루프와 **같은 함수·같은 인자 순서** — **오버레이 전용 기하식은 0줄**이다.
  - `pushFailedOverlay`(~5044) — 실패도 목록 행으로 남긴다(같은 소스 중복은 갱신).
  - 소스 읽기: `OVERLAY_CELL_LIMIT=2000`(~5065, 메인 로드와 동일 상한) `fetchTableSchemaCached`(~5068) `OVERLAY_SYSTEM_COLS`(~5077) `deriveMapBinding(schema)`(~5082 — **[U6] ~~`OVERLAY_VAL_CANDIDATES`~~ 삭제**: 후보는 `overlayContract.valueColumnCandidates`(서빙), 불가면 첫 비제외 컬럼 폴백 — 서버 `derive_table_binding`과 같은 규약) `buildKeyFilters(keyColumns, mapKey)`(~5103, 서버 `build_key_filters`와 동일 — 마지막 컬럼이 나머지 흡수).
  - `addOverlayLayer(sourceTable, sourceKey, targetOverride)`(~5123) — **메인 로드와 코드 경로 완전 분리**. 흐름: ① 바인딩 유도 → ②③ `Promise.allSettled`로 셀 + 소스/타깃 메타 병렬 조회 → ④ 프레임 확정 → ⑤ `cols×rows` 호환성 관문 → ⑥ 정렬 요약 + 격자 밖 셀 카운트. 명명된 실패 status **4종**: `binding_unavailable` `meta_unavailable` `no_data` `align_unavailable` (+ IO 실패는 일반 `error`). **이 함수는 `/api/maps/overlay`를 호출하지 않는다**.
  - `removeOverlayLayer`(~5317) `toggleOverlayLayer`(~5324) `clearOverlayLayers`(~5333).
  - `overlayGeomSig`(~5346) / `currentGeomSignature`(~5348) / `syncOverlayGeometry`(~5365, 서명 변경 시 `rawCells`+`o.frame`에서 재투영. 훅 2곳 — 렌더 ~1992 · 프레임 복원 ~4644). ✅ **[QA C7 해소]** 서명이 화면 7축 + **물리 6종**을 담는다(소스 메타 완비 시 재투영은 항등 — 6종이 실제로 일하는 곳은 물리 규격 미등록 폴백 경로뿐).
  - `overlayAlignChip(o)`(~5388) — 정렬 상태 칩. 판정은 **`align.origin`으로만**.
  - `importOverlayToGrid(id)`(~5419) — 유일한 의도적 교차: 오버레이 셀을 `gridData`로만(**서버 쓰기 없음**, `isProtectedFCell` 존중, 웨이퍼 밖 셀 스킵). `ensureLegendValues`(~5477)는 **로컬 legend 캐시만** 갱신.
  - `renderOverlayList`(~5486) `handleAddOverlayClick`(~5540) `CORE_CANONICAL_TABLE='core_defect_map'`(~5569) `addOverlayForSource(sourceTable, lot, slot)`(~5571) `listOverlayLayers`(~5580) — 뒤 둘은 `transfer_plan.js`에 넘기는 컨트롤러 표면.
  - **오버레이 해제 지점은 2곳이다** — 테이블 전환 `switchTable`(호출 ~1165, 토스트) · **맵 로드 `loadExistingMap`(호출 ~3482, 토스트)**. 둘 다 `overlayLayers.length > 0`일 때만 실행한다. ⚠️ **`openMapFrame`은 오버레이를 해제하지 않는다** — 프레임 스택은 반대로 오버레이를 **보존**한다(~4538 주석). 툴바 버튼(`#btn-clear-overlays`) 배선은 ~802.
- **전사 계획 배선**: `initTransferPlan({...})`(~606, import ~6 — `bandToState`·**[U6] `stageTargetTables`**도 여기서 들여온다. **[U6] `loadTablesList`(~1061)의 초기 테이블 선택이 `stageTargetTables()`를 소비** — 구 `bonding_map` 하드코딩 대체: 선언된 stage 타깃 맵 우선, 없으면 첫 맵 테이블) + `notifyMapContext`(1172/3843/4124/4868/4875/4892 = **6곳**) `notifyLegendChanged`(2973/3026/3032/3049 = **4곳** — ⚠️ `selectBrush`엔 의도적으로 없다, 주석 ~3250) `notifyPaintCounts`(~1868). rect 영역 선택 모드는 **전면 폐기**(값 페인팅이 정본 — 코드 부재).
...
### `transfer_plan.js` (~1,395줄) — 「2. Legend & DOE」 패널 (map_editor.html에서 소비, **`b35bc9f` zone 편집기로 재작성 · `2baf9ff` U9 marker/U8 피드백 · `95bf072` U6 stage 선언 단일화**)
...
- 소스 요약(**풀 단위**): `poolCacheId`(~338) `summaryKeyFor`(~342) `getPoolSummary(pool, force)`(~347, GET `/api/transfer-plan/source-summary` — BIN·scope 파라미터 포함) `availabilityOfPool`(~392) `isPlainNotFound`(~432) `materialMetaValues`(~439) `probeMaterialMap`(~458) `refreshMaterials(force=false)`(~1149 — **[`2baf9ff` U8] `force=true`는 [↻ 가용] 버튼 단 하나의 경로**이고, 그때만 완료 토스트를 낸다: 전 풀 정상이면 info, 미상이 있으면 **최빈 사유 1줄** warning — 종전엔 서버가 BIN 축을 거부해 전부 미상이면 같은 미상 셀만 다시 칠해져 버튼이 죽은 것으로 보였다. 셀별 사유는 종전대로 미상 툴팁, 클라측 폴백 계산 없음) `rewardAfterReturn`(~1122 — 복귀 시 그 자재만 재조회).
- 렌더(zone 그리드): `renderPlanHead`(~472) `zoneCellHtml`(~561) `ZONE_PLACEHOLDER`(~593) `zoneIsInapplicable`(~605 — **[`2baf9ff` U9] marker 행(STACK 0)은 전 구역 `inapplicable`**: `해당 없음` 렌더 + fix 문구 `'STACK 0 = 상태 표시 값 (층 없음)'` — 구조적 부재 셀과 같은 취급) `materialChipHtml`(~633) `planOf`(~647) `renderDoeList`(~651 — STACK 입력의 `bad` 클래스는 `ok`·`marker` 둘 다 아님일 때만, 푸터 노트에 marker 안내 1줄) `refreshRowZones`(~736 — **행 단위 갱신**: 전체 리스트 재구성이 입력 랙의 원인이었다, `280ebf0`) `focusedRowValue`/`focusedColumnId`(~766/772) `bindDoeList`(~781 — keystroke 배지도 `marker`를 오류로 칠하지 않는다) `rollupRows`(~862) `renderMaterialPane`(~874) `knobChipsFor`(~983) `renderAll`(~1187) `buildWorkspace`(~1193). **[`280ebf0`] DOE 입력·라벨 한 단계 확대**(.68→.82rem — Excel 붙여넣기 계약은 index 기반이라 무영향).
...
- 이동 허브: `openMaterial(id)`(~1090) — **맵 간 이동의 유일 지점**(프레임 스택). **[`280ebf0` LOAD-parity]** `(lot, slot)`으로 분해되지 않는 id도 **라우팅한다**(~1097–1110): 첫 키 컬럼 필터 `{firstKeyColumn: rawId}`로 폴백 — 「1. Map Search & Load」에서 그 필드 하나만 치는 것과 같은 경로. 행 없는 키는 빈 그리드로 열리고 ⚡ Push에서 생성된다. **`probeMaterialMap`은 이런 id에 계속 null(미상)을 반환한다** — 내비게이션 추측은 되지만 존재 주장은 안 된다.
- 진입/통지 export: `notifyMapContext(info={})`(~1231 — 여기서 서버 조회를 하지 않는다: legend 로드·채택·가드는 전부 `map_editor`의 registry 경로. **[U6] `stagesStatus === 'error'`면 여기서 fetchStages 재시도** ~1247) `notifyLegendChanged`(~1254) `notifyPaintCounts(counts)`(~1266, **`textContent`만 패치**) `initTransferPlan(paintController)`(~1292 — **[U6] `ensureStages()` 호출**: `stageTargetTables()`가 먼저 불렀어도 fetch는 1회). (+ 순수 함수 `bandToState`·`prevTo` export ~256, `stageTargetTables` export ~196)
- ⚠️ **`__held_*` 함수군(~1314–1395)은 명시적 보류 구역** — 호출자 없음. 검증/경고 UI는 사용자 지시로 미구현. ⚠️ `__HELD_WARN_SEVERITY`(~1364)에 **`layer_coverage_gap` 키가 남아 있으나 서버에서 삭제된 경고 타입이라 사문**이다.
...
| `transfer_plan.css` (~672) | 전사 계획 사이드바 스타일 — tokens.css 시맨틱 토큰만 사용(듀얼 테마 자동 대응). **[`b35bc9f`] zone 편집기용으로 전면 재작성**(826→593줄) — 그 재작성이 룰셋들을 떨어뜨렸고 두 번에 걸쳐 복원됐다: **[`280ebf0`]** `.overlay-box`/`.ov-*`, **[`a98dc72`]** `.map-breadcrumb`/`.bc-*`(프레임 브레드크럼)·`.plock-chip`(페인트 잠금 상태 칩 — "확인 불가"는 보여야 한다) 원문 복원 + 50/50 패널 분할 주석 정정(`flex-basis: 0`이 근거). 마크업은 내내 그 클래스들을 쓰고 있었다 — **재작성이 CSS만 떨어뜨리는 패턴 2회째** |
