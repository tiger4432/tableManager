...
### `server/retroactive.py` (🆕⑱ **1,441줄** @`6345ef3d` — 🆕⑰ 1,156 @`64b562b6` — 🆕⑯ 1,139 @`9eb30691` — 🆕⑨ 545 @`5359fdd`, 구 표기 697) — 소급 적용의 **레지스트리**
...
| `gate.py` | 🆕⑱ **601** @`6345ef3d`(🆕⑯ 580 · 구 표기 581) | 문 앞에서 거절하고 **센다**. 단위는 행이 아니라 **분자(molecule)** |
...
| 🆕⑩ `ledger_skeleton.json` | 🆕⑱ **797** @`6345ef3d`(🆕⑪ 710 · 구 표기 763 — ⚠️ 🆕⑮ 가 §5-H-bis 에는 797 로 적고 **이 표는 안 고쳐서** 한 문서가 두 수를 들고 있었다) | config가 **무엇으로 이루어지는지**를 말하는 문서. **줄어들었다** — `vocabulary.*.layer`가 빠지고(`ddc93f5b`) 마지막 네 칸이 「계획 행」에서 「문장」으로 바뀌었다(`81382191`). 상세는 [§5-H-bis](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재) |
...
### 🆕⑩ `server/ledger/ledger_skeleton.json` (🆕⑮ **797줄** @`5a73021a` — 🆕⑪ 710, 구 표기 763) — config가 «무엇으로 이루어지는가»
...
### 🆕⑩ `server/ledger/implementations.py` (🆕⑱ **228줄** @`6345ef3d` — 구 표기 172) — 실행 가능한 구현의 **유일한 발견자**
...
심볼: `_IMPLEMENTATION_PACKAGE = "mappers"` · `_IMPLEMENTATION_MODULE_PREFIX = "ledger_v2_"`(🔴 접두어가 있는 이유 — 같은 디렉터리의 **체인 인제션 맵퍼**는 DB 기계장치를 import하므로, 여기서 함께 import하면 원장 셋업 경로가 무관한 의존성 뒤에 선다) · `ImplementationDeclarationError` · `source_preparer_declarations()` · `mapper_declarations()` · `trusted_implementations()` · `source_preparer_registry()` · `role_mapper_registry()`.
...
| `client2/src/join_verification.js` | 82 | `JOIN_UNREAD` · `joinVerificationView` | 조인 선언이 «승인»됐나 + «만들 DDL» — `GET /admin/config/virtual-join/verify` 의 첫 소비자 |

| `client2/src/gap_catalogue.js` | 76 | `GAPS_UNREAD` · `SIDE_LABELS` · `gapCatalogueView` | 선언이 「있어야 한다」 한 자리 중 «비어 있는» 곳 — «공허»(있을 수 없음)를 «없음»과 가른다. `GET /api/ledger/gaps` 의 첫 소비자 |

| `client2/src/form_demand.js` | 55 | `PLAN_UNREAD` · `SHAPE_MISSING` · `demandState` | «빈 선언»의 칸을 누가 설명하나 — 계획이 비면 «스켈레톤»이 받는다 (「문」) |
...
| 파일 | 줄 | 무엇인가 (🆕⑱ 실측) |
|---|---|---|
| **`server/chain_activity.py`** | **82**(신설 `2574e251`) | 「지금 어느 맵퍼 «안»에 무엇이 들어 있나」의 프로세스 내 레지스트리. `class ChainActivityRegistry` — `attach()` · `attached`(property) · `start(rule, mapper, target_table, rows_in)` · `finish(token)` · `snapshot()` · `clear()`, 그리고 모듈 말미의 싱글턴 **`registry = ChainActivityRegistry()`**. **소비자 «2»**: `chain_ingestion_worker.py`(채우는 쪽) · `main.py`(어드민 큐 라우트가 `chain_activity.registry.snapshot()` 과 `.attached` 를 읽는다). 🔴 **`attached` 가 이 모듈의 요점이다** — 「아무것도 안 돈다」와 「이 프로세스에는 루프가 없어서 «볼 수 없다»」는 다른 사실이고, 레지스트리가 프로세스 안에 사는 한 웹 프로세스는 후자다 |
| 🔴 **`server/mapper_sdk.py`** | **266**(신설 `f74f09c9`) | 체인 맵퍼 저자가 `df -> df` 만 쓰고 비즈니스 키는 프레임워크가 정하게 하는 SDK. `_python_scalar` · `class MapperContractError` · `payloads_to_df(payloads)` · `class BaseMapper` · `sql(db, query, params=None)` · `df_to_updates(df, table_name, *, source_name, updated_by)` · `mapper(target_table=None, *, source_name="chain_ingestion", …)` 데코레이터.<br>🔴 **운영 소비자 «0»** — `server/` 전건에서 이 모듈을 import 하는 것은 `server/tests/test_mapper_sdk.py` 뿐이다(나머지 히트는 `docs/guide/chain_ingestion_guide.md` 와 `task/**` 의 산문). **가이드가 이미 그것을 가르치는데 도는 맵퍼는 아직 하나도 안 쓴다** — [§5 `health.roster_states`](#5-소형-서버-모듈)·[§5-H-bis `implementation_choices`](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재) 와 같은 부류다 |
