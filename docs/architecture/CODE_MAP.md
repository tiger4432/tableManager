...
> **무엇인가** — 파일을 전량 읽지 않고도 어디에 무엇이 있는지 찾는 «구조 지도»입니다.
> 구현 설명은 리빙 문서([backend](./backend.md) · [data_model](./data_model.md))가 임자입니다.
>
> **기준 둘** — 이 지도가 서 있는 곳은 그 둘뿐입니다.
> ① 원장은 **v5 선언** 위에 섭니다 (`server/config/ontology/ledger_config.json`).
> ② 읽기는 **walk 이 답합니다** (`GET /api/ledger/subgraph`).
>
> 🔴 **이 문서에 문서 전체의 `Last-verified` 는 없습니다 — 없앤 것이 아니라 가질 자격이 없습니다.**
> 아래 표에 적힌 절만 실측됐고 나머지는 미검증입니다. 검증 주장이 검증 범위보다 넓으면
> 그 주장은 다음 독자가 확인하는 것을 막고, 그래서 아무 주장도 없느니만 못합니다.
>
> 📎 **어떻게 여기까지 왔는지는 여기 없습니다.** 측정 패스마다의 리비전·범위·그날 무엇을 열었나는
> `docs/history/` 가 임자이고 그쪽은 append-only 입니다. 지도는 «지금 무엇이 있나»만 말합니다.

**📍 절별 검증 상태 — 이 표가 이 문서의 유일한 신뢰 기준입니다.**

| 절 / 파일 | 상태 | 기준 리비전 | 비고 |
|---|---|---|---|
| 🆕 **S-243 · S-243-b · S-247-b · S-250 · C-109 · `b1db471a`..`c904b389` — §1.4 `GET /admin/chain/rules/replayable` 신설 · §2 `crud`(`DROP_ABSENT_NOT_WRITTEN` · `CHAIN_SOURCE` · `can_mean_emptied` · `apply_row_update_internal` 의 「부재는 층을 안 만든다」 관문) · §4-ter-b `rule_shape`(`DECLARED_KINDS`·`declared_kind` + 앵커 전건 재측정) · §4-ter-g `rule_order.cycle_note` 가 `operator_line` 을 지난다 · §4-ter-h `operator_line.nothing_to_do(unless)` · §5-A `chain/replay.replayable_rules_for` · 🆕 §6 **`server/scripts/count_absent_null_layers.py` 신설 등재** · 🆕 §7 **`client2/src/replayable_rules.js` 신설 등재** · §7 `redo_banner`(줄이 객체 + 종류 배지) · `main.js::redoBannerFollows`** — 「파일의 빈 칸이 조인의 값을 더는 안 가리고, 그리드의 재생 목록이 «이 표에서 실제로 도는 집합»이 된 날」 | 🟢 **지목 심볼·시그니처·줄 수·호출자 전건 `git show HEAD:<path>` / `git grep … HEAD` 실측(2026-09-15) — 전부 `c904b389` 커밋 blob, 워킹트리 아님(`ingestion_worker.py` 에 미커밋 편집이 있어 트리를 «열지 않았다»). 삭제 파일 «하나»(`git diff --name-status b1db471a c904b389`: D = `server/M1_SOURCE_CONFIG_REF.RETIRED.md` — 이 문서에 그 이름의 앵커는 «0건»이었다, 지울 것 없음). `dist` 번들 R 셋은 «이름을 적지 않는다»** | **`c904b389`**(HEAD) | 🔴 **실측으로 바로잡은 앵커**: ① §4-ter-h 「`nothing_to_do` 는 호출자 «0» … `say_cycle_once` 가 :35 에 «손으로» 적는다」 — `d469bacf` 가 그 씨앗을 «접었다»: `cycle_note` :42 가 `operator_line.line(…, nothing_to_do(unless=…))` 를 부른다(호출자 «1») ② §4-ter-b 의 앵커 «스물»이 `declared_kind` 블록(:60~:93) 삽입으로 «+37» 밀렸다 — 전건 «따로» 재서 적었다(가설 오프셋 아님) ③ §5-A `replay.py` 의 `replay_rule` :261→:296 · `order_rules` :188→:223 등 다섯 ④ §4-ter-h 의 `crud` 호출자 셋 :4044/:4108/:4386 → :4095/:4159/:4437 ⑤ §1.4 `sorted(mapper_sdk.MAPPER_REGISTRY)` :5508 → :5663 ⑥ 줄 수 — `crud` 5,715 · `main.py` 6,989(라우트 정본 철자 «90») · `replay` 1,042 · `rule_shape` 421 · `rule_order` 149 · `operator_line` 108 · `redo_banner.js` 375 · `main.js` 2,144(지도의 2,333 은 이 범위 «전»에 이미 낡아 있었다) · `test_chain_replay.py` 854/33(구 897/35). ⚠️ **확인 못 한 것**: `agent_workspace/memory/code-mapper.md` 가 main 트리에 «없다»(워크트리 사본만) — 교훈 파일은 그 사본으로 읽었다 |
| 🆕 **S-240 · S-245 · S-246 · S-247 · `dc877746`..`b1db471a` — §4 `warmup_worker` 0-bis · `mapper_call.py` · §4-ter-b `rule_shape` `key` 칸 · §4-ter-c `join_into.right_key`/`_folded`/`_write` 거절 줄 · §4-ter-d `unique_key._expressions`/`describe` · 🆕 §4-ter-h **`server/operator_line.py` 신설 등재** · §5-C `virtual_join/config.py`(`column_is_text`·`index_key_expression(…, table)`·회수 요구 집합) · §5-C `builtins.py`(S-240 셋 + `run_builtin` 등록) · §5-D `executor.join_onclause` · §5-E `notation_norm` 키 식 «한 저자» · §5-I `activity.running` · §2 `crud` 거절 줄 셋 · §5 마이그레이션 이름 하나** — 「선언한 유일 키를 제품이 세우고, 키 식의 저자가 하나가 되고, 어느 문으로 돌든 큐에 보이고, 거절 줄마다 다음 행동이 실린 날」 | 🟢 **지목 심볼·시그니처·줄 수·호출자 전건 `git show HEAD:<path>` / `git grep … HEAD` 실측(2026-09-15) — 전부 `b1db471a` 커밋 blob, 워킹트리 아님. 삭제 파일 «0»(`git diff --name-status dc877746 b1db471a` 전체에 D 없음 — 서버는 M 13 · A 5, A 는 `operator_line.py` 하나 + 시험 넷)** | **`b1db471a`**(HEAD) | 🔴 **낡은 문장 둘이 «착지로» 거짓이 됐다**: ① §4-ter-c 「오늘 `key.unique` 를 읽는 자가 «없다» … S-240 은 아직 착지 전」 — `8cab58da` 가 `rule_shape` 에 `key` 칸을 나르고 `builtins.declared_unique_targets` 가 읽는다 ② §4-ter-c·§4-ter-d·§5-C 의 「`unique_key.ensure_once` 의 «유일한» 호출자는 `virtual_join/config.py`」 — 둘째 호출자 `chain/builtins.py` :177. 🔴 **줄 수 열하나**: `builtins.py` 177 → **332** · `join_into` 288 → **316** · `rule_shape` 374 → **384** · `unique_key` 462 → **480** · `virtual_join/config` 1,021 → **1,054** · `notation_norm` 808 → **910** · `activity.py` 120 → **264** · `mapper_call.py` 253 → **248**(S-246 이 `import event_constants` 와 접힌 try 를 뺐다) · `executor.py` 889 → **891** · `ingestion_worker` 3,790 → **3,812**(1,990 «뒤»가 전부 +22 — 그래도 행마다 «실측»으로 적었다, 오프셋 가산 아님) · `crud.py` **5,664**. 🔴 **앵커 낡음(실측값)**: `mapper_call` :88/:48/:59/:63/:69/:195/:220/:46 → **:87/:47/:58/:62/:68/:190/:215/:45** · `crud.refuse_virtual_join_duplicates` :3891 → **:3978** · `executor.join_onclause` :165 → **:181** · `execute_rule` :487 → **:505** · `materialize_rows`/`on_target_rows_changed`/`on_reference_rows_changed`/`retract_rows` :754/:795/:805/:852 → **:772/:821/:831/:878** · `config.load_verified_rules` :796 → **:821**(안의 `verify_uniqueness` :771 → :841 · `narrower_than_identity` :839 → :867 · `ensure_once` :852 → :877 · `_say_once(("unique"…))` :863 → :888 · 회수 :896 → :930) · `JOIN_MAPPER` :864 → **:996** · `verification_report` ~432 → **:937** · `required_index_name`/`required_index_ddl` ~136/~146 → **:198/:359** · `rule_shape` 열여섯 자리(+5~+10, 행마다 실측) · `join_into` `JOIN_CELLS`/`unknown_cells`/`_folded`/`_write` :71/:74/:91/:167·:178 → **:73/:76/:110/:195** · `unique_key` 열여섯 자리(+5~+18, 행마다 실측) · `ingestion_worker` 후속 랩 블록 :2612~:2757 → **:2634~:2779** · `warmup_worker` ~615 → **:1965**. 🔵 **이 구간의 다섯 커밋**: `8cab58da`+`07a568ad`(선언한 유일 키는 제품이 세운다 · `key.columns` 는 검사다) · `ddd5b3ba`(키 식의 저자가 하나 — 숫자 키가 타입 오류이기를 그만둔다) · `38b5d8e1`(어느 문으로 돌든 큐 화면에 있다) · `3aab7173`(거절 줄마다 다음 행동 — 아무도 물을 수 없으므로). ⚠️ **못 잰 것**: `virtual_join/config.py` 의 `CODE_NO_REWRITE_CAP = "no_rewrite_cap"` :124 는 이 범위 «밖»인데 HEAD 에서 `^CODE_NO_REWRITE_CAP *=` 히트 «0» — 다음 패스가 그 절을 열어야 한다 |
| 🆕 **S-242~S-249 · `474f1aa9`..`dc877746` — §0 ㉘ `RuleCycleRefused` 묘비 · §4 `load_chain_rules`·`_validate_chain_cascade_graph`·후속 랩(`_run_builtin_followups`)·`failure_cause`·`_resolvable_mapper` · §4 `graph.py` · §4-ter-b `expand_declaration` · §4-ter-c `CHAIN_LAYER`/`_folded` · §4-ter-d `retract_unrequired_once` · §4-ter-g `rule_order` 고리 «모양» · §5 `replay.py` builtin 갈래 · §5-C `virtual_join/config.py` · §5-D `retroactive._count_chain_replay` · §5-H `followup.py`·`ledger/admin.py`** — 「고리가 오류이기를 그만두고, 저장 관문이 로더와 «같은 것»을 읽고, 후속 랩이 홉이 된 날」 | 🟢 **지목 심볼·시그니처·줄 수·호출자 전건 `git show HEAD:<path>` 실측(2026-09-15) — 전부 `dc877746` 커밋 blob, 워킹트리 아님. 삭제 파일 «0»(`git diff --name-status 474f1aa9 dc877746 -- server` 에 D 없음)** | **`dc877746`**(HEAD) | 🔴 **묘비 하나**: `class RuleCycleRefused` 가 `2504fbde` 에서 사라졌는데 지도 «세 자리»(§4 `load_chain_rules` 행 ⑤ · §4-ter-g 「무엇」·「누가 부르나」)가 현행 앵커로 들고 있었다 — 시험이 `not hasattr(rule_order, "RuleCycleRefused")` 를 단언한다. 🔴 **줄 수 열하나가 낡았다**: `ingestion_worker` 3,669 → **3,790** · `rule_shape` 325 → **374** · `join_into` 272 → **288** · `unique_key` 348 → **462** · `rule_order` 95 → **139** · `virtual_join/config` 999 → **1,021** · `graph.py` 373 → **407** · `replay.py` 1,200 → **1,007**(S-214 가 맵퍼 호출자를 떼어 «줄었다» — `474f1aa9` 에서 이미 922) · `followup.py` 234 → **659** · `ledger/admin.py` 848 → **1,284** · `retroactive.py` 1,502 → **1,609**. 🔴 **앵커 낡음**: `load_chain_rules` :682 → **:698** · `_followup_builtin_rules` :1682 → **:1935**(`_FOLLOWUP_BUILTIN_RULES` :1679 → :1932 · `reload_worker_process_cache` :1694 → :1947) · `unique_key.forget` :241 → **:355** · `MAX_PLANNED_KEYS`/`fold_plan`/`describe_fold_plan`/`narrower_than_identity` :249/:252/:306/:328 → **:363/:366/:420/:442** · `builtins.py` 171 → **177**(이 범위 «밖»에서 이미). 🔵 **이 구간의 여덟 커밋**: `fd53b87b`(숫자 키는 TEXT 로 접는다) · `00da7e91`(저장 관문이 «로더가 세울 것»을 심판) · `1aa50d3d`(조인의 쓰기는 체인의 층) · `90d971ba`(인덱스는 그것을 요구하는 조인만큼 산다) · `2504fbde`(고리는 모양이지 오류가 아니다) · `b1e89e36`(후속 랩은 홉 · 한 원인 한 번) · `6edf4db4`(후속 랩이 이벤트를 접고 «무엇이 깨웠나»를 말한다) · `bd0a3db7`(선언된 조인도 소급된다 · 나쁜 페이지는 그 페이지만) |
| 🆕 **D-38 후속 · `af3c2901`..`474f1aa9` — §4 `load_chain_rules` 번역 블록 · §4-ter-b/c/d(`rule_shape`·`join_into`·`unique_key`) · 🆕 §4-ter-g `rule_order.py` **미등재 → 등재** · §5 `enrichment/config.py` · §5-C `virtual_join/config.py` · §5-D `virtual_join/executor.py`** — 「스위치가 «주어»를 되찾고, 한 선언의 실패가 «그 선언만» 거절하게 된 날」 | 🟢 **지목 심볼·시그니처·줄 수·호출자 전건 `git grep` 실측(2026-09-15) — 전부 `474f1aa9` 커밋 blob, 워킹트리 아님** | **`474f1aa9`** (server/ 트리 해시는 HEAD `82590983` 과 동일 — 그 뒤 커밋은 `task/` 만) | 🔴 **`server/chain/rule_order.py`(`c7bc59db`, 09-13, S-156)가 지도에 «한 줄도» 없었다** — 워커가 `load_chain_rules` 끝에서 `rule_order.order_rules(rules)` :907 로 매 적재마다 지나는 파일이다. §4-ter-g 로 등재. 🔴 **줄 수 여섯이 낡았다**: `rule_shape` 271 → **325** · `join_into` 260 → **272** · `unique_key` 328 → **348** · `enrichment/config` 1,638 → **1,958** · `virtual_join/config` 697 → **999** · `virtual_join/executor` 631 → **889**(앞 셋은 D-38 등재 «당일» 값이라 하루 만에 낡았고, 뒤 셋은 🆕⑨ 이후 «한 번도» 안 쟀다). 🔴 **앵커 셋이 낡았다**: `load_verified_rules` ~401 → **:796** · `_verified_by_left_table` ~97 → **:116** · `reset_cache`/`rules_for` ~91/126 → **:109/:162**. 🔵 **이 구간의 다섯 커밋**: `ea8f91d2`(꺼진 규칙은 순서를 못 정한다) · `1497ea3e`(OFF 는 DB 무접촉 · 프로브 실패는 롤백+기억) · `e88cb2be`(선언 하나의 실패는 그 규칙만 · 검증은 «자기 세션») · `593aac50`(`join_into._write` 팬아웃 그물) · `dc29056a`(S-239 — `decide` 종류, 확장기 «하나»에 문 «둘»). ⚠️ **확인 못 한 것**: `1497ea3e` 는 시험을 «안 실었다»(stat: `unique_key.py` 하나 — 「박스에서 검증」만) — OFF 경로의 «호출 0» 단언은 S-240 의 일이다. ⚠️ **S-240 사실(큐)**: 오늘 `server/chain` 에서 통합 join 선언의 `key.unique` 를 읽는 자가 «없다» — `rule_shape.from_declaration` :162 가 `name/enabled/on/derive/into/limits/extra` 만 나르고 최상위 `key` 는 «떨어진다»(`extra` 에도 안 든다). `unique_key.ensure_once` 의 호출자는 `virtual_join/config.py` :852 «하나»(전건 `git grep`). 🔴 **그 밖의 절은 열지 않았다** |
| 🆕㉗ **서버 레인 2026-09-10 착지 — 스무 건** (S-86·84-b·92·117~119·122/122-b·123·124·125·127·128·129·130·131·132·133·134·135·136·139·140·141) | 🟢 **각 항목이 자기 시험을 들고 착지, 그 시험이 심볼을 «이름으로» 단언** | `6aa21c25` | **선언 칸이 늘었다**: `search_columns`(S-125, 표마다 `?q=` 범위 — 기본은 그 표의 «신원») · `analyze_after_rows`(S-124②·S-130, 적재 뒤 + 기동 때 `n_mod_since_analyze`) · `max_group_attempts`(S-139, 기본 **1**) · `trigger_columns`(S-140, 규칙을 «컬럼까지» 좁힘) · `aggregations` 가 `{fn, column}`(S-129, count\|min\|max) · `cardinality`(S-133, 어휘) · 값 바인딩의 `timezone`(S-84-b, `ledger_skeleton.json`) · `object.value_type` 넷(S-84).<br>**읽는 쪽이 생긴 것**: `cardinality` 는 S-133 «전»에 컴파일에서 떨어졌고(`predicate_claim` 이 `{emit, roles}` 만 냄) 이제 `PredicateDescriptor.cardinality` 로 실린다 · `supersedes` 는 읽는 쪽(`ledger_trace.live_claims`)만 있고 **쓰는 자가 0** 이었다 — S-133 ① 이 첫 writer, S-141 이 걷기를 그 필터에 «지나게» 했다(그 전까지 걷기는 대체된 엣지와 대체한 엣지를 «둘 다» 그렸다).<br>**성능 셋, 전부 «선언/기본값»이 원인이었다**: `.nullslast()` 가 `DESC NULLS LAST` 를 만들어 인덱스를 죽였다(S-131 — 211 ms + Sort → 0.257 ms) · 범위 없는 `?q=` 가 ILIKE 항 31 + `dt_inventory` 조인 «여섯»을 만들었다(S-125 — 2,174 → 117 ms) · 큰 적재 뒤 통계가 낡아 플래너가 인덱스를 안 썼다(S-124/130).<br>⚠️ **경로**: `ledger_subgraph.py` 는 `server/ledger_api/` 아래다(`75c32b50`). `ledger/trace_router.py` 는 «안 옮겨졌다» — server/ 루트 그대로 |
| 🆕㉖ **§5-H 원장 파일 표·schema·`setup_registry` — 오후 착지분** 「원장이 «삭제까지» 따라가게 된 오후」 | 🟢 **지목 심볼 전건 grep + 줄 수 실측(2026-09-08 오후) — 문장마다 커밋 해시, HEAD 대조** | **`67416e45`**(HEAD) | 🆕 **`server/ledger/followup.py` 신설 251줄**(S-54 `1d4bb79d`) — 워커 «그룹 층»의 한 걸음이고 새 구독이 아니다. `FOLLOWUP_JOB = "chain_followup"` :46. 🔴 **`backfill.rescope` 가 «한 트랜잭션»이 됐다**(S-60 `ddf3fb10` 「a failed remake no longer leaves the withdrawal standing, because they are one transaction」) — ⚠️ **이 문서와 그 함수의 docstring 이 오전까지 「THE WITHDRAWAL AND THE REMAKE ARE TWO TRANSACTIONS, NAMED RATHER THAN HIDDEN」이라 적고 있었고 그 문장은 «오후에 거짓»이 됐다**(응용 S-54-b 명세가 그 줄을 인용했다 — 인용도 함께 낡았다). 🆕 **S-54-b 심볼 셋이 `backfill.py` 에 들어왔다**(`d8b58f2d`·`005b53ba`·`f13b93cf`·`88eb8d93`·`cc2ea78e`): `sources_without_row_index(setup, *, relation=None, source=None)` :915 · `index_existing_refs(engine, source, setup=None, apply=False, pace=None, …)` :936 · `withdraw_deleted_rows(engine, setup, relation, row_ids, apply=False)` :1056. 🆕 **`schema.py` 에 표 하나와 제약 하나**: `ROW_REF_TABLE = "ledger_source_row_ref"` :69(원장 소유 row_id 색인 — 판정 132 ㉡ 의 자리) · `OBJECTLESS_PAYLOAD_CONSTRAINT = "ck_ledger_objectless_carries_only_qualifiers"` :99(S-52-h `28b298d6`). 🆕 **`setup_bundle` 카탈로그가 `kind` 를 받는다** — `("table", "view")` 만, 그 밖은 거절 :251~255, 그리고 `kind != "view"` 일 때만 심는다(`e32c0ce1` · 판정 137·138 의 착지). 🆕 **`setup_registry._NOT_ATOM_MATERIAL = frozenset({"config_path", "frame_row_id"})`** :84, 지문에서 제외되는 자리 :98(`27a5f4f5`). 🆕 **`ledger_subgraph`: 노드의 자기 컬럼이 «`follow` 를 통해 오지 않는다»**(`4097b2a4` 「a registration is not an edge」 · S-52-i). 🔵 **줄 수 실측**: `followup.py` **251** · `backfill.py` **1,621** · `schema.py` **586** · `setup_bundle.py` **2,224**(오전 2,203 에서 또 움직였다 — 하루에 두 번). ⚠️ **§5-J 는 «대조만» 했다(지시)** — `pacing.json` 이 «최상위 `jobs` 맵»을 얻었고 오늘 그 안은 `chain_followup` 하나다. §5-J 본문은 «프로파일과 소비자 셋»을 말하고 `jobs` 를 «한 번도 안 부른다** — 낡았다고 «표시만» 하고 고치지 않았다. 🔴 **그 밖의 절·보드·히스토리는 열지 않았다.** |
| 🆕㉕ **§5-H `setup_bundle`·`ledger_skeleton.json` 줄 수 · 노드 `attributes` 계열 여섯** — 「노드가 자기 속성을 들게 된 날」 | 🟢 **지목 심볼·줄 수 실측(2026-09-08) — 문장마다 커밋 해시, 오늘 HEAD 에서 grep 대조** | **`%s`**(HEAD) | 🔴 **`attributes` 가 «여섯 자리»에 동시에 들어왔다** — 하나가 아니라 계열이다: ① `setup_bundle.py` 가 문법을 든다(`4b3ac8ac`·`aa1f6204`·`ca211dcc` — `_column_values(descriptor["attributes"])` :497 · 엔티티 서술자 exact 키에 `attributes` :1099 · `_nonblank_list` 검사 :1123~1125, 그 옆 주석이 「`attributes` are the values a node carries that are NOT its identity」로 «신원 아님»을 적는다 :1087) · ② `ledger_skeleton.json` 이 «세 자리»에 `attributes` 좌석을 둔다(`851075e5`·`5b28f832` — key :9 · :168 · :660, 그리고 그 좌석이 «엔티티 바인딩»에만 열리도록 잠긴다) · ③ `ledger_subgraph.py` 가 노드에 실어 낸다(`8f46b3a3` — `node["attributes"]` :1293 · **`node["attribute_conflicts"]` :1294**, `_expand_atom` :996) · ④ `ledger/trace_router.py` 의 `/declaration` 이 «타입마다» 이름을 내려보낸다(`a25cdc42` — :640~642, 그 위 :630 주석이 「so no screen has to name one (S-52)」로 «화면이 이름을 안 짓는다»를 사유로 적는다) · ⑤ `runtime_v2.py` 의 등록 필터가 «두 함수»로 갈린다(`1e24cef2`·`f6b839c7` — :421 「Two functions, not one: `registration_token` answers existence and is the only …」 · `_registration_slot` 이 `registration_token` + `registration_fingerprint` 를 «합쳐» 슬롯을 만든다 :425, `_filtered_event_atoms` :429) · ⑥ 출하 샘플에 «실물 하나»(`b871226a` — `ledger_config.json.sample` 의 엔티티 `attributes` 목록 :266 와 소스 bind 쪽 :767). 🔵 **줄 수 둘이 낡아 고쳤다**(취소선 보존): `setup_bundle.py` 2,004 → **2,203** · `ledger_skeleton.json` 761 → **823**(`wc -l` 실측). ⚠️ **이 패스가 연 소스**: 위 여섯 파일의 «지목 줄만» + `wc -l` 넷(`setup_bundle` · `ledger_skeleton.json` · `ledger_subgraph` 1,360 · `ledger_trace_router` 709). 🔴 **그 밖의 절은 열지 않았다** — 히스토리·보드는 총괄 전담이라 «건드리지 않았다**. ⚠️ **안 고친 것 하나**: `ledger_subgraph`·`ledger_trace_router` 의 «현재 줄 수»를 싣는 살아 있는 행을 이 문서에서 «못 찾았다** — 위 두 수는 여기 기록만 하고 그 행은 다음 패스로 넘긴다 |
| 🆕㉔ **§5-I 이력 행(34행) `collect` · §5-H 라우트 표·심볼 표 `GET /key-values` · §5 `listing_absence.py` 신설 등재** | 🟢 **지목 심볼 넷 실측(2026-09-07)** | **`70ec9248`**(HEAD) | 🔴 **같은 사실이 지도의 «세 자리»에 있었고 수리가 «둘»에만 갔다** — 34행이 아직 「`collect` 와 `NODE_KINDS` 는 함께 은퇴」라 적고 있었다(묘비 표와 라우트 인자 행은 `e025b39b` 가 고쳤다). 🔴 **지우지 않고 «고쳤다»** — 「한때 은퇴했고 «다른 축»으로 돌아왔다」가 사실이고, 지우면 다음 사람이 「왜 `NODE_KINDS` 는 없지」를 다시 유도한다. 실측: `subgraph(..., collect=None)` **있음** · `NODE_KINDS`·`RETIRED_NODE_KINDS`·`FOLDED_KINDS` 는 `server/` 전건에서 **정의 0**(남은 둘은 «퇴장을 적은» 주석과 시험 주석) · `absent_listing` 호출자 **넷**(전부 `main.py` 의 어드민 라우트) · `ledger/trace_router.py` 라우트 **4**(`^@router\.` 계수 — 지도가 싣던 3에서 늘었다). ⚠️ **이 패스가 연 소스**: `server/listing_absence.py`(전량) · `ledger/trace_router.py`(`/key-values` 핸들러와 상수 셋 · 라우트 계수) · `ledger_api/ledger_subgraph.py`(`subgraph` 시그니처 · 상단 `NODE_KINDS` 주석) · `main.py`(호출자 넷의 데코레이터만). 🔴 **그 밖의 절은 열지 않았다.** ⚠️ **발견 하나 — 이 라운드가 «고치지 않았다»**: `/key-values` 의 `key` 인자 설명이 아직 응답 필드를 `seedable` 이라 부르는데 그 필드는 `covers_declared_keys` 로 개명됐다 — «같은 부류»다(수리가 «응답»에만 갔다) |
| 🆕㉒ **§2 `purge_map_rows` · §6 `event_constants` 깊이 절 · §6 런처 행 `reclaim_stranded_claims`** | 🟢 **지목 심볼 셋 · 시그니처 · 줄 수 실측(2026-09-06)** | **`df483dcd`(HEAD)** 커밋 blob — 워킹트리 아님. 직전 정비 `bee55a3a` | 🔴 **㉑ 이 「이름으로 적어 둔」 자리 중 «셋»을 열어 등재했다** — 그 행은 파일 이름만 들고 있었고 심볼은 «하나도» 없었다(전건 grep: `reclaim_stranded_claims` 0 · `purge_map_rows` 0 · `chain_depth_of` 0). ⚠️ **㉑ 은 걷지 «않는다»** — 그 행이 이름을 댄 나머지(`main`·`run_auto_update`·`database/database`·`database/context`·클라 전부)는 이 패스가 «열지 않았다». 걷으면 안 연 것까지 검증된 것으로 읽힌다. ⚠️ **이 패스가 열어 본 소스**(전부 `df483dcd` 커밋 blob): `server/run_watcher.py`·`server/database/crud.py`·`server/event_constants.py`(시그니처·상수 전건) · `server/ingestion/checkpoint.py`·`server/database/context.py`(이름 확인만) · 다섯 파일 `wc -l`. 🔴 **그 밖의 절은 열지 않았다.** ⚰️ **[2026-09-08 · `fe2d9c6a`] 그 유보는 «닫혔다».** 「§2 의 `test_composite_key_prefetch_budget` 채점자 블록은 일부러 손대지 않았다 — 두 독법이 같은 수를 낸다」가 이 자리에 있었다. 두 독법이 같은 수를 내는 것은 «여전히 참»이고, 그래서 그 시험이 이제 «수 말고 둘을 더» 잰다: 그 한 문장이 «배치 전체»를 실었나(bind == ROWS) · 행이 «정확히 ROWS 개» 한 번씩 착지했나. 프로브를 «건너뛴» 쪽은 행을 잃으므로 갈린다 |
| ⚠️㉑ **2026-09-06 변경분 — «미검증»** | 🔴 **낡음** (이 패스는 «측정이 아니라 표시»입니다) | `5cdb92de`..`498b9a3e` — 이 구간에 이 지도를 «다시 연 적이 없습니다» | 🔴 **오늘 코드가 바뀐 자리를 «이름으로» 적어 둡니다. 열기 전에 이 줄을 보십시오.** 서버: `chain_ingestion_worker`(깊이 — 홉 계수·거절·정적 검사 «무변경») · `database/context`·`database/database`·`event_constants`(깊이 표지가 봉투 빌더 «하나»를 지남) · `run_watcher`(재시도 선점 뒤 «설정»이 try 안으로) · `main`·`run_auto_update`. 클라: `admin.js`(fetchData 가 탭 «일곱»을 알게 됨 + else 거절) · `effort_meter` · `map2/authoring` · `rnd_board/api` · `walk/*` · 하니스 다수. ⚰️ **`client2/src/enrichment.js` 는 «삭제»됐습니다**(잔해, 3,233줄) — 이 지도가 그것을 이름으로 들고 있으면 그 줄은 «오늘부로 거짓»입니다. 📎 그날의 판정·근거는 [`SERVER_DEFECT_QUEUE`](../process/SERVER_DEFECT_QUEUE.md) 와 [`PROJECT_STATUS`](../process/PROJECT_STATUS.md) 의 저녁 절 |
| 🆕⑳ **`5cdb92de`..HEAD 구간 패스** — §1.4 라우트 다섯 · §4 연쇄 그래프 검증기 · §5-D `retroactive` 상수 · §5-H `gate`/`backfill`/`setup_bundle`/파일 표 줄 수 · §5-H-bis `config_authoring`/`implementations`/`source_preparation`/`ledger_skeleton`/클라 부품 표 · §5-I `chain_activity` · §6 `event_constants` · §6-2 계약 재계수 + `temporal_text` 신설 + 덧붙이기 다리 | 🟢 **지목 심볼·시그니처·소비자 전건 grep · 줄 수 전건 · 키 계수 전건 실측(2026-09-05)** | **`328a5c20`(HEAD)** 커밋 blob — 워킹트리 아님(측정 시각에 `server/dt_map_derivation.py`·`map_alignment.py`·`map_overlay.py` 가 트리에서 수정 중이었고 **셋 다 이 패스의 대상이 아니다**). 직전 정비 `5cdb92de` | 🔴 **§6-2 의 「6계약」이 세 번째로 낡았고, «수 하나로는 셀 수 없는» 디렉터리였다** — 계약 디렉터리 **9** · `client_harness.mjs` **7** · 파이썬 절반 **6**. 철자를 함께 적었다. 🔴 **§5-H-bis 클라 부품 절이 제목 「일곱」과 본문 「이 열넷이」로 «한 절에 두 수»를 들고 있었고**, 뒤 여섯 행이 빈 줄로 끊겨 **마크다운이 표로 렌더하지 않았다**. 🔴 **그 표의 줄 수 «넷»이 틀렸고**(`join_verification` 82→83 · `gap_catalogue` 76→74 · `form_demand` 55→51 · `pickup_state` 84→80) **넷 다 `5cdb92de` 와 HEAD 에서 바이트 동일**이다 — 즉 **등재되던 날부터 틀렸다**(워킹트리를 쟀거나 편집 전에 쟀다). 🔴 **`ledger_skeleton.json` 은 「797 → 721 → 761」로 «줄었다 늘었다»** — 한 방향 가정으로 고치면 못 맞춘다. 🔴 **§5-H-bis 가 `closed_lists()` 를 무인자로 싣고 있었다**(인자는 `b72843cc` 에 붙었고 §5-H-bis 의 «다른 블록»은 그것을 이미 적고 있었다 — 한 절 안의 두 자리가 갈려 있었다). ⚠️ **이 패스가 열어 본 소스**(전부 `328a5c20` 커밋 blob): `server/ledger/config_authoring.py`(`closed_lists` 전량 + `_implementation_fields` diff + 최상위 심볼) · `server/ledger/setup_bundle.py`(`public_bundle_schema` 전량) · `server/ledger/implementations.py`(전량) · `server/ledger/source_preparation.py`·`server/mappers/ledger_v2_lot_event_role_mapper.py`(diff) · `server/ledger/gate.py`·`backfill.py`·`runtime_v2.py`(diff) · `server/chain/ingestion_worker.py`(`_validate_chain_cascade_graph` 전량 + `enabled` 전건 grep) · `server/event_constants.py`·`server/main.py`·`server/chain/activity.py`·`server/admin/retroactive.py`(diff + 최상위 심볼) · `server/runtime/health.py`(최상위 심볼 + `roster_states` 전건 grep — **무변동 확인**) · `contracts/**` 전건 `ls-tree` + `temporal_text` 둘 · `client2/tests/lib/appended_module.mjs`(전량) · `client2/scripts/check_harnesses.mjs`(diff) · `client2/src/` 부품 열넷 `wc -l` + `^export` · 범위 전건 `git diff --name-status`. 🔴 **그 밖의 절은 열지 않았다** — §0·§1.1~§1.3·§1.5~§1.9·§2·§3·§5·§5-A~§5-G·§5-J·§7·§7-A·§7-B 는 앞 패스 기준 그대로다. ⛔ **§6-2 의 하니스 «전수 채점» 블록(2026-07-31 실행값)은 다시 돌리지 않았다** — 파일 수부터 16 → **82** 로 낡았다고 «표시만» 했다 |
| 🆕⑱ **서버 전용 패스** — §1.1-bis·§1.1-ter·§1.5·§1.7 · §5 `runtime/health.py`/`heartbeat.py`/`models.py` · §5-D `admin/retroactive.py` · §5-H 파일 표 줄 수 · §5-H-bis `implementations.py` · §5-I 신설 둘 · §6 `event_constants.py` · §0 게이트 ㉕-c — 「명부가 «발행»됐는데 «읽는 쪽»이 아직 없는 날」 | 🟢 **지목 심볼·시그니처·소비자 계수·삭제 전건 실측 + 신설 주장 전건 심볼 게이트(2026-09-05)** | **측정 `6345ef3d` → 재확인 `2ac37874`(HEAD)** — ⚠️ **측정 «중»에 HEAD 가 움직였고 «`server/runtime/health.py` 하나»가 달라졌다**(`git diff 6345ef3d..2ac37874 -- server/ *.py` = `runtime/health.py` M + 시험 A/D 둘). 그 하나는 아래대로 «재측정해 되물렸고», 나머지 절의 앵커는 **두 리비전 모두에서 참**이다. 직전 정비 `4b6014b6`(측정 리비전 `64b562b6`) | 🔴 **§1.5 가 통째로 낡았다 — 그래프 라우트 «일곱»은 410 이 아니라 «없다»**(`8ffe23d7`, 2026-09-04). 그 절이 「7개 전부 살아 있다 · 본체는 `raise _graph_branch_retired()`」라 적고 핸들러 이름 일곱을 「grep 1건씩」으로 싣고 있었는데 **전건 0**이다. 🔴 **그리고 §1.4-bis 가 경고한 바로 그 결과가 «실제 동작»이 됐다** — `/graph/*` 여섯은 SPA catch-all 로 떨어져 **index.html 200**, `/api/graph/sync` 만 404(접두어 가드에 `api` 가 있고 `graph` 는 없다). 🔴 **§1.7 의 「공유 상수가 없다」 블록쿼트가 «반증됐다»** — `event_constants.UNDELIVERED_MARKER_{STATUS,PROCESSED_CHAIN,TAG}` 가 생겼고 양쪽이 그것을 읽는다(`443c1181`). 그 블록쿼트의 앵커 셋(`sweep_undelivered_broadcasts` **781** · 쿼리 **802–807** · `chain_ingestion_worker:838`)은 전부 낡았다. 🔴 **「서 있는데 «부르는 곳이 없는» 것」 넷을 등재했다** — `heartbeat.read_roster`(§5) · `ledger.implementations.implementation_choices`(§5-H-bis) · `mapper_sdk` 전체(§5-I) · `main.RETIRED_GRAPH_TABLES`(§1.5). **이 부류가 이 지도에서 제일 일을 많이 한다 — 다음 패스의 작업 지시가 그 행이다.** 🔴 **그리고 그중 하나가 «이 패스 안에서» 배선됐다**: `read_roster` 소비자가 «0 → 1»(`f00ac31d` 이 `compute_health` 의 워커 루프에 먹였다). 🪦 **같은 커밋이 `health.roster_states` 와 `ROSTER_*` 넷을 «지웠다»** — 판정이 둘이 되지 않게. ⚠️ **이 지도가 그 다섯을 «살아 있는 것으로» 한 번 적었다가 같은 패스의 «심볼 게이트»가 잡아 되물렸다** — 라인이 아니라 심볼로 인용하는 이유가 정확히 그것이다(낡은 심볼은 grep 0 으로 자기가 낡았다고 말한다). 🔴 **§0 게이트 ㉕-c 의 기대값이 «1 → 0» 이다** — 그 1건이던 `server/tests/test_graph_branch_retired.py` 가 같은 커밋에 삭제됐다(실행에서 뽑은 값). ⚠️ **이 패스가 열어 본 소스**(전부 `6345ef3d` 커밋 blob): `server/runtime/health.py`(전량 — **두 리비전 «다»**) · `server/utils/heartbeat.py`(최상위 심볼 + 명부/`own_name` 블록) · `run_decoupled_app.py`(명부 발행부) · `server/event_constants.py`(최상위 심볼 + 미배달 마커 절) · `server/internal_event_client.py`(최상위 심볼 + `record_undelivered_notification` 전량) · `server/chain/ingestion_worker.py`(`sweep_undelivered_broadcasts` 머리·마커 필터·줄 수) · `server/admin/retroactive.py`(최상위 심볼 전건 + `runner_identity`·`_runner_state`·`_mark_run`) · `server/database/models.py`(`RetroactiveRun` 전량 · 줄 수) · `migrations/add_retroactive_runs_runner.sql`(전량) · `server/ledger/implementations.py`(최상위 심볼 + `implementation_choices` 전량) · `server/main.py`(`get_dashboard_summary` 전량 · `/health` 블록 · catch-all 접두어 가드 · 라우트/게이트 전건 계수 · 줄 수) · `server/database/schemas.py`(`DashboardSummaryResponse` 만) · `server/ledger/envelope.py`(최상위 심볼 + `check_envelope` diff) · `server/ledger/**`·`server/ledger_api/**` 전건 `wc -l` · `ontology_config_explorer_router.py`(라우트 전건) · `server/chain/activity.py`·`server/mapper_sdk.py`(최상위 심볼) · 범위 전건 `git diff --diff-filter=D/A`. 🔴 **그 밖의 절은 열지 않았다** — §1.2·§1.3·§1.4·§1.4-bis·§1.6·§1.8·§1.9·§2·§3·§4·§5-A~§5-G·§5-J·§7·§7-A·§7-B 는 앞 패스 기준 그대로다. ⛔ **§7 클라 부품 표는 «다른 레인 소관»이라 이 패스가 손대지 않았다** |
| 🆕⑰ §7 클라 **신설 넷**(`match_count`·`dropdown`·`admin_token`·`redo_banner`) + **삭제 하나**(`grid_rescope_menu`) · §1 개수 라우트 · §5-I `follow_keys` · §5-G 체인 맵퍼 · §5-J 호출자 · §5-D `_pace_param` · §5 `models.py` — 「개수가 «늦게» 오게 된 날, 그리고 뷰가 체인이 된 날」 | 🟢 **지목 심볼·시그니처·계수 실측 + §0 게이트 전건 재실행(2026-09-02)** | **측정 리비전 `64b562b6`** — ⚠️ 측정 중 HEAD 가 `944941a8` 로 움직였으나 **`server/`·`client2/` 트리 해시가 양쪽에서 동일하다**(`d8f6fe0e1716` · `e493e28bc97a` — 그 여덟 커밋은 `docs/`·`task/` 만 건드렸다). 즉 이 절의 앵커는 **두 리비전 모두에서 참**이다. 직전 정비 `4b6014b6`(측정 리비전 `9eb30691`) 에서 코드 커밋 **22** | 🔴 **지시서의 기준선 `23dfeb2c` 는 «다섯 세대 낡았다»** — 그 뒤 `641dbb4e`·`4248a6fe`·`3636409e`·`d3db636e`·`4b6014b6` 다섯 번의 정비가 더 있었다. 173커밋이 아니라 **22커밋**이 실제 범위이고, 그 차이가 「이미 등재된 것을 신설로 다시 적는」 부류를 만든다. 🔴 **`client2/src/grid_rescope_menu.js` 가 «없는 파일인데 §7 에 자기 절을 통째로 들고 있었다»**(심볼 표·소비자·채점자까지) — `6e060fd7` 이 그 파일과 하니스를 지우고 `redo_banner.js` 로 갈았다. 🔴 **`server/scripts/create_lot_slot_wafer_view.py` 삭제 + 체인 맵퍼 신설** — `lot_slot_wafer` 가 «뷰 → 체인 파생 표»가 됐다(지도에 «한 번도» 없던 계열). 🔴 **§5-J 호출자가 «셋 → 넷»**(`chain/replay.py` 가 같은 표를 읽는다). 🔴 **`gaps.py`·`gap_names.json` 의 거절문이 가리키는 스펙이 `task/` → `docs/spec/` 로 옮겨졌다.** ✅ **§0 게이트 ㉓-c/㉓-d/㉓-e/㉔-a/㉔-b/㉕ 여섯 전부 기대값 그대로**(0 · 3 · 9+1+1 · 0 · 1+2 · 0). ⚠️ **이 패스가 열어 본 소스**: `main.py`(`narrowed_table_query`·`cached_table_count`·`get_table_data_count`·`fetch_and_merge_metadata` + 계수 전건) · `ledger_api/ledger_subgraph.py`(최상위 심볼 + `subgraph` 시그니처·키 가드) · `ledger/trace_router.py`(전량) · `chain/replay.py`(`resolve_pace`·`replay_rule`·페이싱 루프) · `admin/retroactive.py`(`_pace_param`·`OPERATIONS` 키) · `database/models.py`·`crud.py`(해당 diff) · `mappers/lot_slot_wafer_mapper.py.sample`(전량) · `chain_rules.json.sample`(규칙 전건) · 클라 신설 넷(전량) + `grid.js`/`main.js`/`api.js` 지목 심볼. **§1.3·§1.5~§1.8·§2~§4·§5-A~§5-F·§7-A·§7-B 는 열지 않았다.** |
| 🆕⑯ §5-J **신설**(`pacing`) · §5-H `gaps` **신설** · §5-D `retroactive` · §5-H 파일 표·라우터·채점자 · §1 라우트 계수 · §7 클라 신설 셋 — 「긴 작업에 손잡이가 둘 생긴 날: **멈춤**과 **늦춤**」 | 🟢 **지목 심볼·시그니처·파일 전건 실측(2026-08-31)** | **`9eb30691`**(HEAD) 기준 · 직전 정비 `5a73021a` 에서 **232커밋** | 🔴 **`retroactive.OPERATIONS` 가 «5종에서 6종»이 됐고 그 사이에 «구성원이 바뀌었다»** — `graph_orphans` 는 **없는 이름**(서버 전건 grep 0)이고 `ledger_backfill`·`ledger_rescope` 둘이 들어왔다. 지도는 없는 op 를 심볼 표에 싣고 있었다. 🔴 **`server/pacing.py`·`pacing.json` 신설** — 원장 백필·파일 인제션·소급 등록부 «셋»이 같은 표를 읽는다([§5-J](#5-j-serverpacingpy--pacingjson--긴-작업이-얼마나-미는가-표-하나-49988247-신설)). 🔴 **`server/ledger/gaps.py`·`gap_names.json` 신설** + 라우터에 `GET /api/ledger/gaps` — **라우트가 «둘»에서 «셋»이 됐다.** 🔴 **§5-H 파일 표에 «없는 파일 다섯»이 행으로 살아 있었다**(`chain_mapper`·`profile_chain_mapper`·`profile_lookup_adapters`·`legacy_import`·`shadow_parity` — 🆕⑬ 이 산문으로만 적고 표는 안 고쳤다). ⚠️ **이 패스가 열어 본 소스**: `server/pacing.py`(전량) · `pacing.json`(전량) · `ledger/gaps.py`(전량) · `ledger/gap_names.json`(전량) · `admin/retroactive.py`(전량) · `ledger/backfill.py`(`run`·`_run_v2_lineage` 페이싱 루프·`resolve_pace`/`load_paces`/`rows_past_cursor`/`preview_first_batch` · 최상위 심볼) · `ledger/runtime_v2.py`(최상위 심볼 + `execute_scoped_batch`) · `ledger/store.py`(최상위 심볼 + `write_batch` docstring) · `chain/replay.py`(최상위 심볼 + `replay_rule` 시그니처) · `main.py`(소급 라우트 다섯 전량 · 라우트/게이트 전건 계수 · 줄 수) · `ledger/trace_router.py`(라우트 전건 · `/gaps`·`/declaration` 핸들러 · 모듈 docstring · 줄 수) · `database/models.py`(`RetroactiveRun` 전량 · 줄 수) · `parsers/directory_watcher.py`(페이싱 블록과 그 감싸는 메서드 · 쉼 자리) · `config/sample/ingestion_settings.json.sample`(`ingestion_pace` 키) · `client2/src/{grid_source_label,rescope_handoff,grid_rescope_menu}.js`(전량) · `retroactive_view.js`(export 전건) · `admin.js`·`main.js`(배선부만) · `server/ledger/**` 전건 `wc -l` · 원장 계열 시험 열넷의 줄 수·`def test_` 계수. 🔴 **그 밖의 절은 열지 않았다** — §1.1~§1.3·§1.5~§1.9·§2~§4·§5-A~§5-G·§5-H-bis·§5-I·§6·§7-A·§7-B 는 앞 패스 기준 그대로다 |
| 🆕⑮ §1.9 **신설** · §1.1 · §1.4 · §5-H 라우터/`setup_bundle` · §5-I `ledger_subgraph` — 리로드가 엔트리포인트 밖으로 나간 날 | 🟢 **지목 심볼·시그니처 실측(2026-08-30)** | **`5a73021a`**(HEAD) 기준 | 🔴 **`server/runtime/system_reload.py` 가 신설·추적됐고 이 지도에 «절이 아예 없었다»**(`5db3b80c`) — [§1.9](#19-serversystem_reloadpy--리로드가-엔트리포인트-밖으로-나간-자리-5db3b80c-신설) 로 등재. `main.reload_local_process_cache` 는 **`def` 가 아니라 재export 대입**이 됐다(§1.1 행 정정). 🔴 **`ledger_api/ledger_subgraph.py` 의 «모듈 docstring» 이 은퇴한 그래프를 그린다** — 파일 자체는 `2a4e51cf` 와 **blob 동일**이라 🆕⑭ 심볼 표는 그대로 참이고, 낡은 것은 docstring 하나다(§5-I). 🔴 **`continues` 가 검증기에서 «나갔다»** — `setup_bundle._validate_vocabulary` 의 `optional=("continues",)` 가 삭제돼 이제 «거절»이다(`1d17c34a`). 🔴 **`_followable_predicates` 의 docstring 자기모순이 «해소됐다»**(`12a94097`) — 🆕⑬/🆕⑭ 가 두 번 적어 둔 그 반박은 이제 낡았다. 🔴 **`ledger_skeleton.json` 에 `references` 노드가 «생겼다»** — 「문법은 있는데 아무도 안 읽는다」에 **작성 폼이 그린다**가 더해졌다. ⚠️ **이 패스가 열어 본 소스**: `server/runtime/system_reload.py`(전량) · `main.py`(리로드 블록 · `startup_event` 의 `active_watcher` 대입 · 줄 수) · `ledger_api/ontology_config_explorer_router.py`(`system_reload` 호출부만) · `ledger_api/ledger_subgraph.py`(docstring · `decode_node_id` · `_seed_node` · `subgraph` 의 mechanism 주석) · `ledger/trace_router.py`(`_followable_predicates` docstring · 라우트 계수 · 줄 수) · `ledger/setup_bundle.py`(`_validate_vocabulary`·`_validate_entities`·`_validate_references` 만) · `ledger/ledger_skeleton.json`(diff) · `ledger/source_profile_builtins.py`(diff) · `config/sample/ledger_config.json.sample`(구조 프로브 — 실값 미전재) · `tests/test_entrypoint_import_isolation.py`. 🔴 **그 밖의 절은 열지 않았다** — §1.2~§1.7·§2~§4·§5~§7·§7-B 는 앞 패스 기준 그대로다 |
| 🆕⑭ §5-H · §5-I 원장 **읽기 측** — 걷기 가드 둘이 들어온 밤 | 🟢 **지목 심볼·시그니처 실측(2026-08-29 심야)** | **`2a4e51cf`** 기준 · 아래 ⚠️ 재측정 줄 참조 | 🔴 **`continues` 축이 «엔티티 class» 축으로 통째 교체됐다.** `DEFAULT_CONTINUES_HOPS` → **`DEFAULT_BACKBONE_HOPS`** · `subgraph(..., continuing=…)` → **`static_types=` + `static_follow=`** · 라우터 `_continuing_predicates()` → **`_static_types()` + `_static_step_predicates()`** · 쿼리 파라미터 `continues_hops` → **`backbone_hops`**. 🔴 **`_reach` 가 «나누기를 그만뒀다»** — 반환이 2-튜플에서 **3-튜플 `(reach, parents, kinds)`** 이 되고 값이 정수다. 🔴 **`NODE_TABLE_COLUMNS`·`EDGE_TABLE_COLUMNS`·`PROPERTY_TABLE_COLUMNS` 는 이제 «없는 이름»이다**(앞 패스가 「호출자 0」으로 남겨 둔 셋이 삭제됐다). ⚠️ **이 패스가 열어 본 소스**: `ledger_api/ledger_subgraph.py`(전량) · `ledger/trace_router.py`(전량) · `ledger/trace.py`(`DEFAULT_RESOLVER_CONFIG` 만) · `ledger/explorer.py`(최상위 심볼) · `ledger/config.py`(최상위 심볼 + `load`/`_validate_for_version`) · `ledger/setup_bundle.py`(`_validate_entities`·`_validate_references`·`_validate_vocabulary` 만) · `server/config/sample/**` 둘의 «키 존재만» · `server/tests/` 읽기 측 아홉의 줄 수·`def test_` 계수. 🔴 **그 밖의 절은 열지 않았다** — §5-H 쓰기 측 심볼 표·§5-H-bis·§1–§7·§7-B 는 앞 패스 기준 그대로다 |
| 🆕⑬ §5-H · §5-I 원장 **읽기 측** — 개정 6 이후 전면 대조 | 🟢 **라우트 전건 + 파일 존재 전건 + 지목 심볼 실측(2026-08-29)** | **`3b77fbc3`**(HEAD) | 라우트 **16 → «2»** (`GET /subgraph` · `GET /declaration`). 키를 받는 데이터 라우트 여덟이 사라졌고 그 몸이던 모듈도 파일이 없다. 🔴 **이 지도가 살아 있는 앵커로 싣던 서버 파일 «15»가 지금 그 경로에 없다 — 삭제 11 · 이동 4** (`ledger_kinds`·`ledger_siblings`·`ledger_structure`·`ledger_lots`·`ledger_trends`·`ledger_composition`·`ledger_selection`·`ledger_identity`·`ledger_catalog`·`ledger_journey`·`ledger_walk_contrast` 삭제 · `ledger_api/finding_kinds.py` → `scripts/support/` · `ledger_api/mechanism_gate.py`·`enrichment_actions.py`·`audit_changeset.py` → `server/_archive/`). `decode_node_id` 는 접두어 «하나»(`ledger-entity:v1:`)만 푸고 claim seed 를 거절한다. ⚠️ **이 패스가 열어 본 소스**: `ledger/trace_router.py` · `ledger_api/ledger_subgraph.py` · `ledger_api/declared_entities.py` · `ledger_api/__init__.py` · `ledger/explorer.py` · **`ledger/trace.py`(이 지도가 주장하는 심볼 148개 전건 대조)** · `ledger/setup_bundle.py`(`_validate_vocabulary` 만) · `ledger/ledger_skeleton.json` · `client2/vite.config.js` · `client2/src/rnd_board/**`(일곱 본체, 나머지 줄 수만) · 그리고 `git ls-tree -r HEAD` 전건으로 이 문서가 이름 대는 경로 전부의 존재 대조 + `server/tests/` 원장 계열 줄 수·`def test_` 전건 계수. 🔴 **그 밖의 절은 열지 않았다** — 쓰기 측 심볼 표(§5-H)·§5-H-bis·§1–§7은 앞 패스 기준 그대로다 — 쓰기 측(§5-H `server/ledger/`)과 §1–§7 의 나머지는 여전히 앞 패스 기준이다<br>🆕㉕ **[S-224 `b985846c`] `crud.refuse_write_to_view(table_name)` :864 — 「이 관계가 쓰기를 받나」가 «답해지는 한 자리»**, 부르는 곳 «다섯» :4233 · :4851 · :4935 · :5009 · :5175. 🔴 **종전엔 그 문장이 `apply_batch_updates` «안»에 있었고 「모든 쓰기가 거기로 모인다」가 «거짓»이었다** — `create_empty_rows_batch` · `delete_rows_batch` · `delete_cell_source_batch` · `set_cell_manual_priority_batch` 가 관계에 «직접» 닿아서, 소유자의 행 추가 버튼이 **500**(`TypeError: row_id is an invalid keyword argument`)을 냈다. ⚠️ 뷰가 `row_id` 를 선언한 경우엔 «충돌조차 안 난다** — 삭제가 셀 층을 지우고 마지막 문장만 PostgreSQL 이 거절했을 것이다. 🔵 그리고 관문은 «빈 페이로드 지름길 «앞»»에 선다 — 「빈 요청이라 통과」와 「쓸 수 있는 관계라 통과」가 같은 답이 되면 거절이 «보낸 것»에 의존하기 시작한다. ⚠️ 낱말은 `catalog_kind` 가 정한다 — `.get("kind") or "table"` 의 «넷째 철자»를 만들지 않으려고(S-187) |
| 🆕⑫ §5-H `ledger/trace_router.py` — **파일 통째 동결 → 라우트별 울타리** | ⚰️ **[2026-08-29] 폐기 — 라우트는 16이 아니라 «2»다.** 구: 라우트 16개 전건 실측(2026-08-23) | **`bd0c192e`**(HEAD, blob `ca088aff`) | 779 → **806**(`5aa666d5`). **8 동결 / 8 열림.** 🔴 **동결 표지가 `/subgraph`의 유일한 문 위에 얹혀 있었다** — 같은 문단이 `ledger_subgraph`를 「동결 아님」이라 적으면서. 가르는 규칙은 **호출 심볼**이고 **모듈이 아니다**(`_fetch`·`relation_exists`는 열린 여덟 모듈 **8/8**이 쓰는 배관이라 얼리면 16라우트 전부가 동결이 된다). ⚠️ **이 패스는 라우트 표만 열었다 — 이 절의 나머지 산문·`ledger/trace.py` 심볼 표는 재검증하지 않았다** |
...
| 🆕⑪ §5-I `ledger_subgraph.py` — Quantity 투영 + 부호 있는 seed | 🟢 **심볼 실측 신규 등재(2026-08-23)** | **`b3e1ec83`**(HEAD) | 1,254 → **1,625**. `quantity_node_id`·`_quantity_node`·`_bound_quantities`·`NODE_KINDS`(8종)·`_signed_seeds`·`_reach`·`_rank_layers`·`_seed_node`·`subgraph(..., collect=None)` (⚰️ **[2026-08-28] `collect` 와 `NODE_KINDS` 는 함께 은퇴** — 노드 종류가 «하나»가 됐고 좁히는 축은 `follow` 뿐이었다. 🔵 **[2026-09-06] 둘 중 `collect` «만» 돌아왔다 — 그것도 «다른 축»으로.** 구 `collect` 는 «배관 종류»(entity·point·claim·collection)를 골랐고, 새 `collect: list[str]\|None` 은 «도메인 노드 타입»(die·defect…)을 고른다. `subgraph(..., collect=None)` 은 오늘 «있는» 시그니처다. 🔴 **`NODE_KINDS`·`RETIRED_NODE_KINDS`·`FOLDED_KINDS` 는 그대로 없다** — 소스가 `ledger_subgraph.py` 상단에 그 셋의 퇴장을 적어 둔다. 인자 정본은 §5-H 라우트 인자 행, 축 셋(`follow`=길 · `collect`=짐)의 정본은 [`WALK.md`](WALK.md)). 🔴 **`/subgraph`는 엔티티 seed에서 «쓰인 이래 계속» 500이었다**(f-string 안의 JSON 경로 중괄호 → `NameError`) — `c5b13cf7`이 함께 고쳤다 |
...
| 🆕⑩ §5-I 원장 v2 작성/설정 계열 · 클라 온톨로지 3종 — **승격 후 삭제** | 🟢 **승격(2026-08-21)** | **`8d1e6c4c`**(HEAD) | 서버 9행 → §5-H/§5-H-bis, 클라 3행 → §5-H-bis. 🔴 클라 3종은 **셋 다 3배 이상 자랐다**(368/246/375 → **1,323/465/2,199**) — 같은 표의 나머지 클라 행도 밀렸다고 가정하라. 2026-08-18의 「워킹트리에서만 삭제」 경고는 **철회**(HEAD에서도 삭제됐다) |
| 🆕⑩ 원장 **읽기** 측(§5-H 읽기 절 · §5-I 읽기 표) | 🔴 **이 패스가 한 파일도 열지 않았다** | ~~`8d1e6c4c`~~ → **`5359fdd`/`a729a7f`** | `ledger_trace`·`ledger_trace_router`·`ledger_explorer`·`ledger_catalog`·`ledger_subgraph`·`ledger_journey`·`ledger_trends`·`ledger_composition`·`ledger_selection`·`ledger_identity`·`ledger_walk_contrast`·`mechanism_gate`. ⚰️ **[2026-08-28] 이 목록에서 `ledger_trends`·`ledger_composition`·`ledger_selection`·`ledger_walk_contrast` 는 삭제됐고 `mechanism_gate` 는 `_archive/` 로 옮겨졌다.** **449커밋 구간이 지났으므로 밀렸다고 가정하라** |
| 🆕⑨ §1.5 / §1.5-bis 그래프 조회 — **묘비로 대체** | 🔴 **서술 전량 폐기(2026-08-18)** | **`5359fdd`**(HEAD, `main.py` blob `9cee25d`) | 일곱 핸들러는 **이름만** 살아 있고 본체는 410 한 줄. 헬퍼·캡·status 어휘는 grep 0건. 🔴 접두어 둘(`/graph/` 여섯 + `POST /api/graph/sync`) |
| 🆕⑨ §5 `server/database/models.py` — 라인 앵커 전건 재측정 | 🟢 **심볼 실측 + 라인 폐기(2026-08-18)** | **`5359fdd`**(blob `fbe4bde`) | 636(제목) → 실측 **1,079**. 앵커 14개 중 **맞은 것은 `AuditLog` ~11 하나**. 나머지는 라인 열을 걷어내고 심볼만 남겼다 |
| 🆕⑨ §5-A `server/value_suggest.py` — 라인 앵커 40여 개 폐기 | 🟢 **심볼 실측 + 라인 폐기(2026-08-18)** | **`5359fdd`**(blob `ba921bb`) | 1,154 → 실측 **1,166**. 드리프트가 **조각(+1/+2/+8/+12)**이라 일괄 가산이 불가능했다 — 심볼 40건 전건 존재 확인 후 라인만 걷어냈다 |
| 🆕⑨ §1.4-bis `serve_static_or_index` 구간 라인 | 🟢 **재측정(2026-08-18)** | **`5359fdd`**(blob `9cee25d`) | 세 구간 전부 틀렸다(접두어 5449–5458 → **6014–6027** · 격리 검사 6115–6117 → **6044–6046** · 서빙 5481–5487 → **6050–6056**). 이름 없는 블록이라 라인을 남기되 sha를 함께 적었다 |
| 🆕⑨ 제목의 줄 수 주장 55건 전건 대조 | 🟢 **줄 수만 실측(2026-08-18)** | **`5359fdd`**(HEAD) | **29건 불일치.** 줄 수만 고쳤고 **그 절들의 심볼 표는 열지 않았다** — 아래 「줄 수 실측표」에 실측값과 미검증 표기 |
| 🆕⑨ **맵에 절이 없는 소스 파일** — [§5-I](#5-i-미등재-모듈-레지스트리-2026-08-18-실측) 신설 | 🟠 **존재·줄 수만(2026-08-18)** | **`5359fdd`**(HEAD) | 서버 25 · 클라 13. **심볼 표 없음** — 있다고 주장하지 않으려고 존재와 크기만 적었다 |
| 🆕⑨ blob 핀 목록 — 없는 파일 넷 은퇴 표기 | 🔴 **정정(2026-08-18)** | **`5359fdd`**(HEAD) | `ontology_config.py`·`graph_sync_worker.py`·`graph_orphans.py`·`graph_stale_edges.py` 핀은 `git show`가 실패한다. **목록 전체는 여전히 `41b17ee` 기준이고 재생성하지 않았다** |
| 🆕⑦ §5-H 읽기 측 — `ledger/trace_router.py`(7라우트) · `ledger/trace.py` | 🟢 **심볼 실측 재측정(2026-08-14)** | **`a729a7f`**(HEAD) | 80 → **419** · 1,179 → **1,937**. 🆕⑥이 예고한 재측정이 이것이다. `ledger/trace.py`는 **기존 top-level 심볼 소실 0**(이름 수준 전건 대조) + 신설 심볼 실측, 단 **기존 행의 산문 서술은 재검증하지 않았다** |
| 🆕⑦ §5-H 신설 서버 7종 — `finding_kinds`·`ledger_kinds`·`ledger_siblings`·`ledger_structure`·`ledger_lots`·`ledger_walk_contrast`·`mechanism_gate` | 🟢 **심볼 실측 신규 등재(2026-08-14)** | **`a729a7f`**(HEAD) | 종전 등재 **0**. `walk_contrast`·`lots`·`mechanism_gate`는 상세, 나머지는 압축 등재. ⚰️ **[2026-08-28] 이 일곱 중 `ledger_kinds`·`ledger_siblings`·`ledger_structure`·`ledger_walk_contrast` 는 삭제, `mechanism_gate` 는 `_archive/` 로 이동, `finding_kinds` 는 `scripts/support/` 로 이동** |
...
| 🆕⑥ §5-H 읽기 측 — `ledger/trace.py` · `ledger/trace_router.py` | 🟢 **심볼 실측 신규 등재(2026-08-13)** | **`aeddac8`** | ⚠️ 두 파일 다 측정 시각에 워킹트리 modified였고 → 🆕⑦이 `a729a7f`에서 재측정했다(위 행) |
...
| 🆕⑤ §5 `server/ingestion/checkpoint.py` — tier-1 원장 + 배치 조회 | 🟢 **심볼 실측(2026-08-13)** | **`831ab68`**(HEAD) | 258 → **587줄**. 표의 라인 번호를 전부 걷어내고 심볼로 재작성했다 |
...
| 🆕🆕🆕🆕 신설 `server/chain_bindings.py` | 🟢 **실측 신규 등재(2026-08-11 3차 후속)** | **`5b09d69`** | **244줄, 신설.** job-column 이름의 단일 해석기 — `rule[key]` 선언 > `table_config` 유도(`identity_column`: `map_key_columns` 단일 컬럼 우선, 그다음 `business_key`) > **이름을 대고 거절**(`ColumnBindingRefused(ValueError)`). 리터럴 기본값(`"dt_job"`) 없음. `resolve_column(rule, key, table, purpose)` / `resolve_decision_column(rule, key, decision_key, purpose)`(정렬 규칙의 `decision_key` 소스) / `model_column(model, table, column, purpose)`(로드된 모델에 그 속성이 없으면 거절) / `identity_column(table)` / `declared_columns(table)`. 해석은 **체인이 아니라 테이블 단위** — 트리거·소스·타깃이 각자 다른 이름을 선언할 수 있다. `map_overlay.resolve_binding_parts`(좌표 바인딩)와 **의도적으로 별개**(그쪽은 lot/slot 관습 폴백을 갖고 있어 이 모듈이 피하려는 바로 그것을 한다). ⚠️ `ORIGIN_DECLARED`/`ORIGIN_INHERITED` 중 **`ORIGIN_INHERITED`는 선언만 되고 이 파일 안에서 반환되지 않는다**(`identity_column`이 실제로 돌려주는 origin 문자열은 `"table_config.map_key_columns"`/`"table_config.business_key"`) — 죽은 상수인지 예약값인지는 확인 못 함<br>🆕㉕ **[S-152 `dadcdb36`, 판정 371·372] `rule_refusals(rule, path, *, mapper_resolvable, mapper_params=None)` :285** — 🔴 **맵퍼가 «무엇을 읽는지 말했으면» 그 밖의 납작한 칸은 «거절»이다.** `@mapper(params=…)` 가 선언이므로, 선언이 있는 순간 제품이 「오타」와 「낡은 인자」를 «가를 수 있다» — `trigger_colums` 가 `trigger_columns` 자리에서 경고로 굴러가며 표를 통째로 데려가던 자리다. ⚠️ **「가를 수 있다」가 규칙의 전부다** — 선언이 없으면 거절도 «없고» 오늘의 경고 그대로다. ⚠️ 그리고 `mapper_resolvable` 은 «호출 가능한 것»이지 레지스트리가 아니다 — 이 모듈이 `mapper_sdk` 를 import 하면 「규칙 파일이 무엇을 말해도 되나」가 「이 프로세스가 무엇을 import 했나」에 «의존»하게 된다(S-188 이 정한 방향)<br>🆕㉕ **[S-153 `ba4fbb95`, 판정 378] 규칙 칸 `max_group_rows`** — `MAX_GROUP_ROWS_KEY` :401 · 타입 표 :418 · 스켈레톤 :136. 읽는 자리는 `chain/ingestion_worker.max_group_rows(rule)` :441 이고 **«규칙에서» 읽는다** — 병합 단위가 「같은 표, 같은 규칙」이라 상한이 다른 곳에 살면 그것을 묘사할 수 없다. ⚠️ 나쁜 값은 «적재에서 이미» 거절되므로 이 자리는 다시 판정하지 않고 «폴백»한다 — 그룹을 짓는 중에 놀라면 있을 수 없는 칸 때문에 배치가 선다<br>🆕㉕ **[S-221 `fc0914a7`, 판정 378] 형제 칸 `max_group_attempts(rule, document)`** — «규칙 칸이 먼저 · 문서 칸은 기본값 · 없으면 1» — 합친 단위는 «최소». 🔴 그전엔 칸이 규칙에도 «열려 있고 폼이 그렸는데» 워커가 문서만 봐서 **규칙에 적어도 아무 일도 안 일어났다** — 「선언 칸이 있다」와 「읽는 쪽이 그것을 본다」가 «다른 사실»이라는 이 저장소의 상설 부류다. 🔵 그리고 «두 출처가 다 인자»라 그 세 층을 직접 채점할 수 있다 — 모듈 상태를 안 읽는다<br>🆕㉕ **[S-154 `051cf472`, 판정 381·383] 규칙 칸 `group_by`** — `GROUP_BY_KEY` :439 · 문법 :366~378(`bad_group_by` · `unknown_group_column` 을 «이름 대어» 거절). 읽는 자리는 `chain/ingestion_worker.py` 의 셋 `group_key` :487 · `group_id` :531 · `group_events` :559 이고, **스윕과 배치 «두 좌석»이 그 하나를 지난다**(종전엔 각자 `transaction_id or single_<uuid>` 를 손으로 적었다). 🔴 규칙 «둘»이 다른 키를 물으면 **합친 키**가 답이다 — 사건은 정확히 한 그룹에 들어가므로 둘을 따로 들어줄 수 없고, 합친 키가 «더 잔» 분할이라 둘을 동시에 만족시킨다(「가장 엄한 상한이 이긴다」의 한 축 옆 판). ⚠️ 「이번 배치 안」이 경계다 — 늦게 온 행은 «자기 그룹»이 된다(기다리려면 「얼마나」 칸이 필요하고 그건 다른 기능이다). ⛔ **재확장된 사건은 «절대» 다시 안 묶는다** — `outbox_expand` 가 «일부러» 쪼갠 것을 도로 접으면 방금 실패한 그룹을 «조용히» 되살린다<br>🆕㉕ **[S-155 `cce52773`, 판정 384] 규칙 칸 `idempotent`** — `false` = 「첫 실패에 격리」. 상한을 «같은 함수»가 답하고(둘째 좌석 금지, :603), 재생은 `_refuse_unless_idempotent` (`chain/replay.py` :246)로 **`force` 에서만** 넘어간다<br>🆕㉕ **[S-228 `2df0b798`, 판정 385 ㉡] `trigger_tables_in_order(events)` :987 — 트리거 표 순회가 «말해진 순서»가 됐다(정렬).** 🔴 **그전엔 «집합»의 순서였고, 문자열 집합의 순서는 파이썬이 «프로세스마다 무작위로» 섞는 해시가 정한다** — 이 상자 실측(`PYTHONHASHSEED` 미설정): 표 이름 «여섯»을 다섯 프로세스에서 펴니 **순서가 다섯 가지**. 즉 트리거 표 둘을 든 그룹이 «재기동마다 달라질 수 있는» 순서로 규칙을 돌았고, **아무것도 그 사실을 말하지 않았다**. ⚠️ **그 측정을 «두 번» 한 것이 요점이다** — 이름 «넷»을 세 프로세스에서 돌렸을 때는 «전부 같았고», 거기서 멈췄으면 「결정적이다」가 **표본이 답을 정한 결론**으로 나왔을 것이다. ⚠️ 정렬은 «임의»이고 그렇게 적혀 있다 — 「도착 순서」는 아웃박스 id 순이라 «둘째 축»이 되고, 그러면 답이 「쓴 쪽이 커밋을 어떻게 쪼갰나」에 의존한다. 여기서 필요한 것은 «모든 프로세스가 같은 답»뿐이고, 그래야 규칙 순서(S-156)가 설 자리가 생긴다<br>🆕㉕ **[S-227 `36ed2500`] 합친 단위의 시도 수 = «구성원의 최대»** (`max_retry_num = max(max_retry_num, event.retry_count)` :2008). 🔴 그전엔 «격리된 절반»과 «처음인 절반»이 한 그룹에 있으면 한쪽은 FAILED, 다른 쪽은 RETRYING 이 되고 로그가 「(2/2)」를 적었다 — 한 단위가 «두 상태»를 가졌다<br>🆕㉕ **[S-228 `2df0b798`, 판정 385 ㉡] `trigger_tables_in_order(events)` :987 — 트리거 표 순회가 «말해진 순서»가 됐다(정렬).** 🔴 **그전엔 «집합»의 순서였고, 문자열 집합의 순서는 파이썬이 «프로세스마다 무작위로» 섞는 해시가 정한다** — 이 상자 실측(`PYTHONHASHSEED` 미설정): 표 이름 «여섯»을 다섯 프로세스에서 펴니 **순서가 다섯 가지**. 즉 트리거 표 둘을 든 그룹이 «재기동마다 달라질 수 있는» 순서로 규칙을 돌았고, **아무것도 그 사실을 말하지 않았다**. ⚠️ **그 측정을 «두 번» 한 것이 요점이다** — 이름 «넷»을 세 프로세스에서 돌렸을 때는 «전부 같았고», 거기서 멈췄으면 「결정적이다」가 **표본이 답을 정한 결론**으로 나왔을 것이다. ⚠️ 정렬은 «임의»이고 그렇게 적혀 있다 — 「도착 순서」는 아웃박스 id 순이라 «둘째 축»이 되고, 그러면 답이 「쓴 쪽이 커밋을 어떻게 쪼갰나」에 의존한다. 여기서 필요한 것은 «모든 프로세스가 같은 답»뿐이고, 그래야 규칙 순서(S-156)가 설 자리가 생긴다<br>🆕㉕ **[S-227 `36ed2500`] 합친 단위의 시도 수 = «구성원의 최대»** (`max_retry_num = max(max_retry_num, event.retry_count)` :2008). 🔴 그전엔 «격리된 절반»과 «처음인 절반»이 한 그룹에 있으면 한쪽은 FAILED, 다른 쪽은 RETRYING 이 되고 로그가 「(2/2)」를 적었다 — 한 단위가 «두 상태»를 가졌다<br>🆕㉕ **[S-156 `c7bc59db`] 신설 `server/chain/rule_order.py`(84줄) — 규칙 «사이»의 순서가 «선언에서 유도»되어 워커가 그것을 읽는다**(`chain/ingestion_worker.py:67` import · `:786` `rule_order.order_rules(rules)` · `:787` ~~`RuleCycleRefused`~~ — ⚰️ `2504fbde` 에서 «사라졌다», §0 ㉘ · 오늘 `:890` `order_rules(rules, on_cycle=…)`). 🔵 **저자는 한 집이다** — `chain/replay.py:159` 의 같은 이름은 «위임»이고 자기 머리글에 「저자는 `chain.rule_order` 다」라 적으며, 거절만 호출자가 이미 잡는 `ReplayRefused` 로 다시 던진다<br>🆕㉕ 🔴 **[S-232 `c70dc340` 판정 387 · S-232-b `684ffb4d` 판정 388] 그 순서의 예외는 «이름»이 아니라 «성질»이다** — 종전엔 `producer.name == name` 이라 trigger==target 인 «서로 다른» 규칙 셋이 서로의 생산자가 되어 가짜 고리를 만들었고, 그래서 S-156 이 착지하고도 «무동작»이었다. 🔴 **그리고 성질의 «절반»으로는 모자랐다(388)** — 「표 사이를 안 옮긴다」는 「아무도 안 읽는다」와 같은 문장이 아니다. 오늘의 규칙: 엣지 `P → X` 는 **P 와 X 가 «둘 다» 그 표를 읽고 쓸 때만** 건너뛴다 |
...
| 🆕🆕🆕 §6 `server/admin/audit_cache.py` · `admin/audit_history.py`(신설) | 🟢 **심볼 실측(2026-08-11 후속)** | **`2630790`**(HEAD) | `admin/audit_cache.py` **247 → 643줄**(전면 재작성). 「growing OFFSET로 100그룹 찾을 때까지 훑기」가 **config-driven ceiling(`RECENT_DEFAULTS`) + `(timestamp, id)` keyset walk + DB-side aggregate(`_count_by_transaction`) + bounded hydration(`_hydrate`)**로 바뀌었다 — `truncated`/`next_cursor`/**이벤트 크레딧**(`_claim`/`_absorb_one`, `add_logs_batch`가 워터마크를 못 넘기는 경로의 이중 카운트 수정) 신설. `admin/audit_history.py`(신설, 242줄)는 `fetch_page`/`encode_cursor`/`decode_cursor`/`apply_cursor`/`order_desc`/`load_config`/`resolve_settings`/`resolve_limit`을 서버 3곳(`audit_cache`·`main.get_row_history`·`main.get_cell_history`)이 공유 |
...
| 🆕🆕🆕 §5 `server/map_alignment.py` · `maps/frame_confirmation.py` · §5-G 맵퍼 | 🟢 **심볼 실측(2026-08-11 후속)** | **`68db020`** | `_basis_cells_for`의 **세 번째 손제작 사본**이 하나로 합쳐졌다 — `frame_confirmation._basis_cells_for`(private, **삭제**)와 `mappers/dt_alignment_metadata_mapper._basis_cells_for`(private, **삭제**)가 없어지고 **`map_alignment.basis_cells_for(db, reference, cfg=None)`**(public, 신설)로 대체됐다. `maps/frame_confirmation.py` 837→**798**(-39, 삭제분). `map_alignment.py`는 6,395→**6,468** |
...
| 🆕🆕 §5 `server/maps/frame_confirmation.py` | 🟢 **심볼 실측(2026-08-11)** | **`7097a67`**(HEAD) | 815 → **837**. **top-level 심볼·시그니처는 `34d2518` 대비 무변동**(전건 대조) — 바뀐 것은 `_placement_of` **본문**이다(`97b29da`: `ruling["by_frame"]` 우선 조회) |
...
| 🆕🆕 §5/§6 신설 서버 모듈 · 스크립트 · 맵퍼 | 🟢 **실측 신규 등재(2026-08-11)** | **`7097a67`**(HEAD) | `alignment_view_service.py`(**85**) · `dt_frame_transform.py`(**96**) · `scripts/` 3종 · `mappers/*.py.sample` **5종**(DT/core 체인). `chain/replay.py`·`chain/ingestion_worker.py`·`admin/audit_cache.py`·`main.py`의 신설 심볼도 실측 |
...
| 🆕 §6 `utils/logger.py` · `event_constants.py` · `database/database.py` · 신설 3모듈 | 🟢 **실측(2026-08-08)** | **`34d2518`**(HEAD) | `logger.py` 145 → **221**(등재 앵커 셋 전부 낡았다) · `event_constants.py` 86 → **223**(outbox **접기** 절 신설) · `database.py`에 `stage_collapsed_event` · 신설 등재 **`outbox_expand.py`** · **`migrations/add_business_key_unique_index.py`** · **`server/scripts/check_missing_business_key.py`** |
| 🆕 §4 `server/chain/ingestion_worker.py` | 🟠 **라인 앵커만 재측정(2026-08-08)** | **`34d2518`**(HEAD) | 1,086 → **1,198**. 🔴 **밀림이 조각이다 — +4 / +55 / +96 세 구간.** 표의 「라인」 열은 절 머리의 재측정 블록이 대체한다. **산문 서술 자체는 재검증하지 않았다** |
...
| §5 `server/map_alignment.py` · `maps/frame_confirmation.py` | 🟢 **심볼 실측(2026-08-07 전면 재측정)** | **`e943e46`**(HEAD) | 🔴 **직전 등재값은 `87a944e` 기준이었고 그 사이에 파일이 배로 늘었다** — `map_alignment.py` **3,272 → 5,961**, `maps/frame_confirmation.py` **688 → 815**. 정렬 **채점 계열**(순번 훑기·방향·그룹·bin 지문·앵커/잔차 배치·진단 로그)이 통째로 미등재였다 → [§5-F](#5-f--정렬-채점-계열-index-scoring-family--servermap_alignmentpy-2026-08-07-등재)에 신설 등재 |
...
| §1.6 `server/admin/auth.py` | 🟠 **부분** | — | **EOF 밖 앵커 1쌍**(`GATE_CHALLENGE_HEADER`/`_GATE_HEADERS`가 463줄 파일에 **5846/5847**로 적혀 있었다 → **100/101**)과 지문 상수 행만 고쳤다. **나머지 미검증** |
| §5 `server/admin/retroactive.py` · `runtime/process_supervisor.py` · §3 `parsers/directory_watcher.py` | 🟢 **심볼별 실측** | `5609ff0` | ⚠️ **오프셋은 균일하지 않았다** — 「전건 +23 / +92 / +62」는 틀린 모델이다. 각 절은 **앞부분이 이미 맞고 어느 지점부터 밀린 조각(piecewise)** 이었다(예: `admin/retroactive.py` 16개 중 6개는 이미 정확). 그래서 오프셋을 더하지 않고 **심볼마다 실측값으로 다시 적었다**(54개 정정). 남은 미검증: 표 밖 산문 앵커 |
| 🔴 **그 밖 전부** — `admin.js` · `enrichment/config.py` · `enrichment/candidates.py` · `transfer_plan.py` · `graph_*` · `contracts/*` · 나머지 클라 모듈 | 🔴 **미검증** | — | 감사 실측: `admin.js` 앵커 61개 중 **정확 0** · `enrichment/candidates.py` 36개 중 **0** · `enrichment/config.py` 37개 중 **0** · `directory_watcher.py` 142개 중 **16**. **이 절들의 라인 번호를 믿지 말고 함수명으로 Grep하라** |


**📏 🆕⑨ 줄 수 실측표 (2026-08-18, `5359fdd` 커밋된 blob)** — **절 제목**이 줄 수를 주장하는 **55곳**을 전건 대조해 **29곳**이 틀렸고, 거기에 **§6 표 안의 줄 수 주장 8건**과 §1·§5-H의 인라인 주장 2건을 더해 아래가 된다. ⚠️ **§6 표의 나머지 행과 §5-H 쓰기 측 표는 이 표가 아니라 [§5-I](#5-i-미등재-모듈-레지스트리-2026-08-18-실측)에 있다.** 🔴 **이 표가 말하는 것은 「크기」뿐이다 — 해당 절의 심볼·앵커·산문은 이 패스가 «열지 않았다».** 파일이 자랐다는 것은 **그 절에 없는 심볼이 있다**는 뜻이고, 줄었다는 것은 **그 절이 없는 것을 서술한다**는 뜻이다. 어느 쪽도 이 표는 판정하지 않는다.

| 파일 | 구 표기 | 실측 | 차 |
|---|---|---|---|
| `server/enrichment/config.py` | 676 | **1,638** | **+962** |
| `server/database/models.py` | 636 | **1,079** | +443 |
| `server/ledger/trace_router.py` | 419 | **779** | +360 |
| `server/admin/audit_cache.py`(§6) | 643 | **645** | +2 |
| `run_decoupled_app.py`(저장소 루트, §6) | 228 | **389** | +161 |
...
| 🔻 `server/product_tables.py` | 232 | **158** | **−74** |
| 🔻 `server/transfer_plan.py` | 3,826 | **3,731** | −95 |
| 🔻 `server/admin/retroactive.py` | 697 | **545** | **−152** |
| 🔻 `server/run_auto_update.py`(§6) | 882 | **840** | −42 |
| 🔻 `server/outbox_expand.py`(§6) | 303 | **302** | −1 |
| `client2/src/timeline.js` | 899 | **1,008** | +109 |
| `client2/src/admin.js` | 3,704 | **3,814** | +110 |
| `client2/src/map_editor.js` | 11,031 | **11,060** | +29 |
| `client2/src/map_editor2.js` | 470 | **496** | +26 |
| `client2/src/graph_viewer.js` | 1,254 | **1,274** | +20 |
| `client2/src/state.js` | 187 | **203** | +16 |
| `client2/src/clipboard.js` | 858 | **865** | +7 |
| `client2/src/grid.js` | 869 | **871** | +2 |
| `client2/src/api.js` | 533 | **534** | +1 |
| `client2/src/main.js` | 2,047 | **2,048** | +1 |
| 🔻 `client2/src/ui.js` | 426 | **342** | **−84** |
| `server/main.py`(§1) | 6,128 / 6,322(두 값 공존) | **6,057** | — |

🔴 **줄어든 일곱(🔻)이 늘어난 스물둘보다 위험하다.** 자란 파일은 「이 절에 없는 것이 있다」는 뜻이고 독자는 소스를 열어 찾는다. **줄어든 파일은 「이 절에 있는 것이 소스에 없다」는 뜻**이고, 독자는 지도에 적힌 이름을 그대로 믿는다 — 그것이 [§1.5](#15-그래프-조회-구간은-410으로-은퇴했다--그-일곱은-이제-없다)에서 실제로 일어난 일이다. **`product_tables.py`(−74) · `admin/retroactive.py`(−152) · `ui.js`(−84) · `ledger.html`(−348)의 심볼 표는 이 패스가 열지 않았으므로 «죽은 이름이 있다고 가정하라».**

⚠️ **일치한 26곳도 「그 절이 옳다」는 뜻이 아니다** — 크기가 같아도 내용이 바뀔 수 있고, 이 문서는 그 값을 이미 지불했다(§0 표지의 「바이트 동일한데 드리프트가 가득했다」).

> 🔴 **[2026-08-07 이 패스가 실제로 고친 것 — 범위는 위 표의 세 행뿐이고 나머지는 열지 않았다]**
> - 🔴 **경로 하나가 문서 전체에서 벗겨져 있었다.** 이 저장소에 **`server/database/crud.py`는 없다** — 실제 경로는 **`server/database/crud.py`** 하나다. 그런데 산문은 `crud.py:1623`·`crud.py **1969–1974**`처럼 파일명만 적어 왔고, 그 표기를 그대로 따라간 독자는 **존재하지 않는 파일**을 찾게 된다(2026-08-07에 실제로 그럴 뻔했다). 이번 패스에서 벗은 표기를 전부 온전한 경로로 바꿨다. **모듈 이름으로서의 `crud.xxx`**(파이썬 심볼 참조)는 그대로 둔다 — 그것은 파일명이 아니다.
...
> - 🆕 **신규 등재**: [§7-A Map Editor 2](#7-a--map-editor-2--map_editor2html--client2srcmap2-2026-08-0506-신설) · `push_columns.js` · `enrichment_queue.js` · [`map_alignment.py`](#-servermap_alignmentpy--프레임-정렬의-채점자) · [`maps/frame_confirmation.py`](#-serverframe_confirmationpy--확정의-기록자) · `migrations/add_frame_confirmation.py`.
>
...
> #    (`parsers/directory_watcher.OPERATOR_IMPORT_NAMES` :966, 판정 364). 그중 «최상위 평면 모듈»은 열 — `database` 는 이미 패키지,
> #    `pipeline_base`·`html_topology_parser`·`void_sat_format` 은 `parsers/` 밑이다
> #    (제 실측 — «열하나»와 «열넷»은 다른 모집단이다)
>
> # ⚠️ 그리고 이 문서의 경로는 «오늘의 자리»를 가리킨다 — «그때의 자리»가 아니다.
> #    경로 치환이 «기계적»이었으므로, 옆에 낡은 리비전 해시가 적힌 줄은 이제
> #    `git show <그 해시>:<새 경로>` 로 «안 열린다». 그럴 땐 `git log --follow -- <새 경로>` 를 쓴다 —
> #    해시는 «언제»를, 경로는 «어디»를 말하고 그 둘은 이제 같은 날을 가리키지 않는다
>
> # 🆕㈟ [2026-09-13 신설 · S-210 `f74bbd98` · S-211 ③ `e3a21b31`]
> #    사라진 «디렉토리 셋» — 경로로 드는 히트가 0건이어야 한다
> #    ⚠️ `server/_archive/` 는 «이동된 곳»이었고 이제 «삭제»되었다 —
> #       그래서 이 문서의 다른 절에 남은 `server/_archive/` 문장들은
> #       «묘비 문맥»이어야 한다(가리키는 장소가 아니라 지나간 사실)
> git ls-tree -r --name-only HEAD -- server/_archive server/scratch client
> #    🔴 `client/` 는 «없어진» 것이 아니라 `desktop/` 로 «이름이 바뀐» 것이다 — 그 둘은 다른 사실이고,
> #       이 검사가 섮이면 그것은 «리버트된 것»이지 «복구된 것»이 아닐 수 있다
>
> # 🆕 [2026-09-15 · C-109 `c87080e3`] `main.js::loadChainRuleNames` — «선언·호출» 히트 0건이어야 한다
> #    (후계는 `client2/src/replayable_rules.js::loadReplayableRules(table)` → `GET /admin/chain/rules/replayable?table=`.
> #     주석 히트 1건은 남는다 — 그 파일 머리글 :4, 묘비 문맥. 아래 철자는 그 주석을 «안 잡는다»)
> git grep -nE "function loadChainRuleNames|loadChainRuleNames\(" -- client2/src
>
...
> #    🔴 실측 히트 **정확히 1건**: `chain/ingestion_worker.py:133`(`ed9cfdb` 실측) — 삭제된 코드를 설명하는
> #       **묘비 주석**이다(문자열이 백틱 안에 있다). **2건 이상이면 새 발신자가 생긴 것**이고,
> #       그것이 이 저장소가 같은 엔드포인트에서 이미 세 번 고친 결함이다.
> git grep -nE "requests\.(post|Session)\(" -- server/run_watcher.py server/chain/ingestion_worker.py server/graph_sync_worker.py
...
> # ⑲ [🆕⑩ 2026-08-21 신설] **원장 v1 번역기 계열 — 파일도 심볼도 0건**이어야 한다.
> #    🔴 이것은 이 문서가 «심볼 표를 통째로» 달고 살려 두던 이름들이다(§5-H의
> #       `lot_event_translator.py` 10행). 소유자 판정 「remove legacy」로 v1 문법 넷이
> #       은퇴하면서 파일 일곱이 함께 사라졌다.
> #    ⚠️ 범위에 `docs/`·`agent_workspace/`·`task/`를 넣지 마라 — 그쪽 히트는 전부
> #       묘비·이력 문맥이고, 넣으면 이 검사가 영원히 빨개진다.
> git ls-tree -r --name-only HEAD -- server/ledger/ | grep -E "(declared|lot_event|observation|transfer)_translator|translator_pattern|cutover_v2|grouped_translator_template"
> git grep -nE "class (LotEvent|Observation|Transfer|Declared)Translator|def group_molecules|def molecule_key" -- server/
> # ⑲-b 그리고 삭제된 체인 맵퍼 하나 — `server/mappers/` 히트 0건이어야 한다.
> #     (살아 있는 v2 맵퍼 둘은 `ledger_v2_` 접두어를 «반드시» 갖는다 — `implementations.py`가
> #      그 접두어로만 import하므로 접두어 없는 이름은 config에서 닿지 못한다.)
...
> # ㉓ [🆕⑬ 2026-08-29 신설] **개정 6 — 은퇴한 노드 id 바더 여섯, 선언 0건**이어야 한다.
> #    노드 id 를 만드는 함수가 일곱이라 `decode_node_id` 가 접두어로 갈래를 틀었고, 그 갈래 수가
> #    그대로 walk 루프의 갈래 수였다. 오늘 남은 바더는 `ledger_explorer.entity_id` **하나**다.
> #    🔴 실측 `3b77fbc3`: 히트 **0**. 고장 주입 — `b3e1ec83:server/ledger_subgraph.py` 에는 여섯 전부 있다.
> git grep -nE "^def (event_node_id|claim_node_id|value_node_id|finding_point_node_id|finding_collection_node_id|quantity_node_id)" -- server
> # ㉓-b 그리고 그 바더들이 찍던 **노드 id 접두어 여섯** — 히트 0건이어야 한다.
> #    🛑 **`evidence-edge` 를 이 정규식에 넣지 마라** — `ledger-evidence-edge:v1:` 는 «살아 있다»
> #    (`ledger_subgraph._edge()` 안, 정확히 1건). 「접두어 하나」는 **노드** id 의 이야기이고,
> #    엣지까지 세면 이 검사가 정상 코드에서 빨개진다. 🔴 실측 `3b77fbc3`: 히트 **0**.
> git grep -nE "ledger-(event|claim-atom|value|finding-point|finding-collection|quantity):v1:" -- server client2/src
> # ㉓-c 은퇴한 읽기 측 모듈 열둘 — `server/_archive/` 밖에서 히트 0건이어야 한다.
> #    ⚠️ 범위에서 `_archive` 를 빼는 것이 요점이다(⑦·⑩와 같은 이유) — 아카이브는
> #    그 이름들을 사본으로 들고 있고 그게 옳다. 🔴 실측 `3b77fbc3`: **0**.
> git ls-tree -r --name-only HEAD -- server | grep -v "^server/_archive/" | grep -cE "/(ledger_kinds|ledger_siblings|ledger_structure|ledger_lots|ledger_trends|ledger_composition|ledger_selection|ledger_identity|ledger_catalog|ledger_journey|ledger_walk_contrast|mechanism_gate)\.py$"
> # ㉓-d 원장 라우트는 🆕⑯ **정확히 3**이어야 한다(`/subgraph` · `/gaps` · `/declaration`).
> #    🔴 이 수가 늘면 «키를 받는 데이터 라우트가 다시 생겼다»는 뜻일 수 «있고», 그것이
> #    개정 6 이 없앤 모양이다. 실측 `3b77fbc3`: **2** (`bd0c192e` 에서는 16).
> #    🆕⑯ **실측 `9eb30691`: 3.** 늘어난 하나는 `GET /gaps` 이고 **키를 받지 않는다** —
> #       인자는 «질문 이름»(`name=`)이고 없으면 선언만 읽어 목록을 낸다. 즉 이 검사가
> #       잡으려는 그 부류가 «아니다». 🔴 **그래서 수를 3으로 «올려 적는다»** — 빨간 채로
> #       두면 다음 사람이 「원래 빨갛다」로 배우고, 그때부터 진짜 증가를 못 본다.
> #    ⚠️ 이 수를 다시 올릴 때는 «무엇이 늘었고 왜 그 부류가 아닌지»를 여기 적어라.
> git show HEAD:server/ledger/trace_router.py | grep -c "^@router\."
> # ㉓-e ⚠️ **이 검사는 지금 «빨간상태»이고 그게 기록이다.** 클라가 서버가 지운 원장
> #    라우트를 여전히 이름으로 들고 있다. 🔴 실측 `3b77fbc3`: **11건 / 3파일**
> #    (`rnd_board/api.js` **9** · `rnd_board/composition_panel.js` **1** · `rnd_board/main.js` **1**).
> #    `ROUTES` 의 `lot_map`·`composition`·`trends`·`siblings` 네과 `COLLECTS` 의 다섯 줄이 그것이다
> #    ([§7-B](#7-b-rd-보드-rnd_board-2026-08-29-신설-등재)). **클라 도메인 소관이라 여기서 고치지 않았다** —
> #    수가 줄면 그 라운드가 착지한 것이고, 늘어나면 새로 생긴 것이다.
> #    🆕⑭ **`2a4e51cf` 재실행 — ㉓-c/㉓-d/㉓-e 셋 다 위 기대값 그대로다**(0 · 2 · 9+1+1).
> git grep -c -E "api/ledger/(lot_map|trends|siblings|composition|kinds|structure|trace|coverage|explore|entities|journey)" -- client2/src
> # ㉔ [2026-08-29 심야 신설] 걷기 «예산 축»의 옛 철자 — 개정된 이름 일곱.
> #    `continues` 축이 «엔티티 class» 축으로 교체되면서 이 일곱이 죽었다.
> #    ㉔-a **선언** 히트 0건이어야 한다. 🔴 실측 `2a4e51cf`: **0**.
> git grep -nE "^\s*(def _continuing_predicates|def _link_containers|def tabular_projection|DEFAULT_CONTINUES_HOPS *=|NODE_TABLE_COLUMNS *=|EDGE_TABLE_COLUMNS *=|PROPERTY_TABLE_COLUMNS *=)" -- server client2 contracts
> #    ㉔-b **언급**은 0이 아니다 — 🔴 실측 `2a4e51cf`: **정확히 3건, 셋 다 묘비 주석**이다.
> #       server/ledger_api/ledger_subgraph.py **~54**  `continues_hops` 개명 사유
> #       server/ledger_api/ledger_subgraph.py **~1015** `_link_containers` 삭제 사유
> #       client2/src/rnd_board/api.js         **~379**  같은 개명의 클라 쪽 사유
> #    3보다 크면 옛 철자가 «되살아난» 것이고, 0이면 사유 주석이 지워진 것이다(둘 다 볼 일).
> #    ⚠️ 기대값을 «추론»하지 않고 실행에서 뽑았다 — 0이라 적었으면 첫날부터 빨간 검사였다.
> git grep -cE "DEFAULT_CONTINUES_HOPS|_continuing_predicates|NODE_TABLE_COLUMNS|EDGE_TABLE_COLUMNS|PROPERTY_TABLE_COLUMNS|tabular_projection|_link_containers|continues_hops" -- server client2 contracts
> # ㉕ [🆕⑯ 2026-08-31 신설] **소급 등록부의 `graph_orphans` — 선언 0건**이어야 한다.
> #    🔴 이 문서는 이것을 «OPERATIONS 다섯 중 하나»로, `_count_graph_orphans`/`_run_graph_orphans`
> #       를 «함수»로 싣고 있었다(§5-D). 실제로는 `2ec78b99`(2026-08-14, 판정 R-2026-08-14-H)가
> #       등록을 뺐고 그 모듈(`server/graph_orphans.py`)도 2026-08-16 에 삭제됐다.
> #       **지도가 «두 주» 동안 없는 버튼을 다섯 중 하나로 세고 있었다.**
> #    ✅ **이 판정은 «강제되는 자리»에 산다** — `server/tests/test_retroactive_admin.py::
> #       test_the_retired_graph_sweep_is_not_offered_as_a_button` 이 `"graph_orphans" not in
> #       retroactive.OPERATIONS` 를 단언하고, 라우트가 400 으로 거절하는 것까지 잰다.
> #       그래서 이 검사는 시험의 «값싼 미리보기»다(정본은 그 시험).
> #    🔬 고장 주입 완료: `2ec78b99^` 에 대고 돌리면 **5건**, HEAD 는 **0**.
> git grep -nE '_(count|run)_graph_orphans|"graph_orphans": \{' -- server/admin/retroactive.py
> # ㉕-b **언급**은 0이 «아니다» — 실행에서 뽑은 값이다(2026-08-31, `9eb30691`):
> #    server (tests 제외)  **1건**  migrations/drop_graph_storage.py  — 은퇴를 «설명하는» 산문
> #    client2/src          **1건**  retroactive_view.js:177          — 🔴 «낡은» 주석이다
> #                                  (「five buttons … graph_orphans deletes」. 오늘 op 는 여섯)
> #    ⚠️ `server/tests` 를 범위에 넣지 마라 — `test_retroactive_admin.py` 가
> #       «은퇴를 채점하려고» 이 이름을 든다(그게 옳다).
> #    2보다 크면 되살아난 것이고, 클라 쪽 1이 0이 되면 그 주석이 고쳐진 것이다(둘 다 볼 일).
> #    🔬 🆕⑱ 재실행 `6345ef3d`: **2건 그대로**(`drop_graph_storage.py` 1 · `retroactive_view.js` 1).
> git grep -c "graph_orphans" -- server ':!server/tests' client2/src
> # ㉕-c `MultiDiscoveryScheduler.maybe_sweep_graph_orphans` — 🆕⑱ 기대값 **0건**이다.
> #      🔴 [2026-09-05 실행에서 뽑은 값] 종전 이 칸은 «정확히 1건»이라 적고 그 하나를
> #      `server/tests/test_graph_branch_retired.py` 의 부재 단언 줄로 지목했는데,
> #      **그 시험 파일이 `8ffe23d7` 에서 삭제됐다** — 그래프 지선(枝線)을 통째로 지우면서
> #      「그 코드를 재던 시험이 부재로 빨개진다」는 이유로 같은 커밋에 나갔다.
> #      그래서 이제 1건이면 «부활»이고, 종전 기대값을 그대로 두면 «정상이 빨간 체크»가 된다.
> #      🔴 §6 `run_auto_update.py` 행은 여전히 이것을 «살아 있는 메서드»로 싣고 `~668` 앵커까지
> #      달고 있고, 그 668 은 `retroactive_busy` 다 — 실재하는 «다른» 함수라 도착지가 멀쩡해 보인다.
> git grep -c "maybe_sweep_graph_orphans" -- server client2
> # ㉕-d `server/ledger/` 에 **없어야 하는 파일 다섯** — 0건이어야 한다.
> #    🔴 이것은 §5-H 파일 «표»가 행으로 들고 있던 것들이다. 🆕⑬(2026-08-29)이 산문으로
> #       「넷은 _archive/ 로, 셋은 삭제」라 적으면서 **표는 안 고쳤고**, 그래서 표를 읽는
> #       독자에게는 두 패스 동안 살아 있었다(`shadow_parity.py` 는 역할 서술까지 달고).
> #    ⚠️ 범위가 `server/ledger/` 인 것이 요점 — 셋은 `server/_archive/` 에 «사본으로» 있고
...
> # ㉗ [🆕⑱ 2026-09-05 신설] **은퇴한 그래프 지선의 «핸들러 일곱» — 0건**이어야 한다.
> #    🔴 이 문서는 [§1.5](#15-그래프-조회-구간은-410으로-은퇴했다--그-일곱은-이제-없다) 에서 이 일곱을 «살아 있는 410 스텁»으로 싣고
> #       **「핸들러 이름 grep 1건씩 — 살아 있다」**고 명시적으로 적고 있었다. `8ffe23d7`(2026-09-04)
> #       이 소유자 판정으로 스텁과 헬퍼를 전부 지웠다. 🔴 **그 절이 스스로 「살아 있는 이름이 죽은
> #       서술의 알리바이가 됐다」고 경고해 놓고, 이름이 죽은 뒤엔 그 경고가 낡음을 못 막았다.**
> #    🔬 고장 주입 완료: `8ffe23d7^` 에 대고 돌리면 **7건**(핸들러 정의 일곱), HEAD 는 **0**.
> git grep -cE "def (get_graph_stats|get_graph_neighbors|search_graph_nodes|post_graph_trace|get_chip_trace|get_graph_mapping_summary|manual_graph_sync)" -- server
> # ㉗-b `_graph_branch_retired` — 실측 **정확히 1건**이고 그 하나는 «산문»이다:
> #      `server/migrations/drop_graph_storage.py:64` 가 무엇이 은퇴했는지 «설명»한다.
> #    ⚠️ 0 이 아니다 — 0 으로 적으면 정당한 묘비 주석이 이 검사를 빨갛게 만든다.
> #       2건 이상이면 스텁이 되살아난 것이다.
> #    ⚠️ 범위에 `run_decoupled_app.py` 를 넣지 마라 — 거기 «낡은 주석»이 하나 더 있다(아래 ㉗-c).
> git grep -c "_graph_branch_retired" -- server
> # ㉗-c **「410 을 답한다」고 적는 «낡은 소스 주석» — 실측 5건**이고 전부 코드 소관이다.
> #    🔴 라우트가 없으므로 `/graph/*` 는 SPA catch-all 로 떨어져 **index.html 200** 이다(404 도 410 도 아니다).
> #       `/api/graph/sync` 만 404 — catch-all 접두어 가드에 `api` 가 있고 `graph` 는 없기 때문이다.
> #    🔬 실측 `2ac37874`: main.py 1 · database/models.py 1 · config_resolve_report.py 1 ·
> #       enrichment/config.py 1 · run_decoupled_app.py 1.  **줄면 누가 고친 것이고 그건 볼 일이다.**
> #    ⚠️ 이 검사는 «지도»가 아니라 «소스»를 잰다 — 이 지도는 §1.5 에서 이미 정정했다.
> git grep -c "mapping-summary\|_graph_branch_retired" -- server/main.py server/database/models.py server/config_resolve_report.py server/enrichment/config.py run_decoupled_app.py
> # 🆕㉘ [2026-09-15 · `2504fbde` 판정 402] `RuleCycleRefused` — 고리는 «모양»이지 오류가 아니다(§4-ter-g).
>  #   소스 히트는 «시험의 부재 단언» 1건뿐이어야 한다
> git grep -n "RuleCycleRefused" -- server
> #    기대: server/tests/test_the_worker_runs_rules_in_the_order_the_declaration_implies.py 의
> #    `assert not hasattr(rule_order, "RuleCycleRefused")` «한 줄». 그 밖의 히트는 되살아난 것이다
> # 🆕㉘-b 그리고 이 문서에서 그 이름은 «묘비 문맥»뿐이어야 한다
> #    (`~~`·⚰️·「없다」 없이 `:숫자` 앵커가 붙어 있으면 §4-ter-g 의 그 실패다)
> grep -n "RuleCycleRefused" docs/architecture/CODE_MAP.md
> ```
>
...
>
> 🆕⑨ 🪦 **[2026-08-18] 이 목록의 핀 넷은 «없는 파일»을 가리킨다 — 쓰기 전에 읽어라.** `ontology_config.py` · `graph_sync_worker.py` · `graph_orphans.py` · `graph_stale_edges.py`는 2026-08-16 그래프 은퇴에서 삭제됐고, 그 핀으로 `git show <hash>:<path>`를 돌리면 **파일이 없어서** 실패한다. 아래에서 ~~취소선~~으로 은퇴 표기했다. 🔴 **그리고 그 넷은 이 목록의 실패 방식을 보여 준다**: 핀 검사는 「해시가 다르다」와 「파일이 없다」를 같은 빨강으로 보고하므로, **은퇴를 드리프트로 읽게 만든다.** 나머지 핀은 여전히 `41b17ee` 기준이고 **이 패스도 목록 전체를 재생성하지 않았다**(범위 밖) — 세 항목만 고치면 목록이 자기 기준을 배신하므로 은퇴 표기만 했다.
>
> 검증용 blob 해시(`git rev-parse 41b17ee:<path>` 선두 7자). 🔴 = 이번 범위에서 바뀐 핀:
> **서버** — 🔴 **`main.py` = `454b649`** · 🆕 **`column_filter.py` = `7a9cbca`** · 🔴 **`database/crud.py` = `d637d91`** · `database/models.py` = `cd78e43` · `map_overlay.py` = `01bbe65` · 🔴 **`transfer_plan.py` = `faed7e0`** · 🔴 **`bonding_plan.py` = `9e7b326`** · 🔴 **`product_tables.py` = `f132d6c`** · `effort_metric.py` = `9e3ef4e` · 🪦 ~~`ontology_config.py` = `851d0f8`~~(**파일 없음**) · 🪦 ~~`graph_sync_worker.py` = `79f0a51`~~(**파일 없음**) · `value_suggest.py` = `a1d7a5e` · `maps/preset_routing.py` = `ce5fee0` · 🔴 **`parsers/directory_watcher.py` = `0609cfa`** · `parsers/advanced_ingester.py` = `fa37bba` · 🆕🔴 **`parsers/html_topology_parser.py` = `61dc149`**
> **서버(인리치먼트 F9)** — `enrichment/config.py` = `ff98b8b` · `enrichment/candidates.py` = `956fb02` · 🆕🔴 **`enrichment/analysis.py` = `82df7ea`** · 🔴 **`config_resolve_report.py` = `8d811f5`**
> **서버(내부 이벤트 경계 · 런처)** — `admin/auth.py` = `16e565b` · 🔴 **`chain/ingestion_worker.py` = `175b37f`** · 🔴 **`internal_event_client.py` = `d898971`** · 🔴 **`run_watcher.py` = `4d722d1`** · `run_auto_update.py` = `2fdd00a` · 🔴 **`event_constants.py` = `2fc2679`** · 🆕🔴 **`runtime/process_supervisor.py` = `52ab177`** · 🆕🔴 **`run_decoupled_app.py` = `cdf5121`**
> **서버(DB 안전 · 가상 조인)** — `database/database.py` = `cb0fe7d` · `db_safety.py` = `8ee2eb8` · `virtual_join/config.py` = `b58efc8` · 🔴 **`virtual_join/executor.py` = `1c8c8cf`**
> **서버(소급·재생·표기·그래프)** — `admin/retroactive.py` = `d8db501` · `enrichment/backfill.py` = `a2039a3` · `chain/replay.py` = `c824ab7` · 🪦 ~~`graph_orphans.py` = `30517d6`~~(**파일 없음**) · 🆕 **`notation_norm.py` = `358ae68`** · 🪦 ~~`graph_stale_edges.py` = `cdf0108`~~(**파일 없음**)
...
>
> 💡 **핀이 50 → 56으로 늘었다.** 새로 들어온 여섯은 전부 **핀 밖에서 밀리고 있던 것 또는 이번 신설**이다: `column_filter.py`·`notation_norm.py`·`graph_stale_edges.py`(신설) · `runtime/process_supervisor.py`·`run_decoupled_app.py`·`html_topology_parser.py`·`enrichment/analysis.py`·`product_tables.py`(핀 밖에서 변경됨).
>
> ✅ **이 패스(`ed9cfdb`→`35b03cc`, 37커밋)에서 핀이 일한 기록.** 구 50핀 중 불일치가 **18개**다. 바이트 동일이라 **끝 쪽 표본만 뽑고 넘긴 것**: `map_overlay.py`(1,448줄 무변동, 4패스 연속) · `value_suggest.py`(끝 표본 `classify_seek_plan` **1055** · `index_targets` **1093**) · `enrichment/candidates.py`(끝 표본 `AutoConfirmCollector` **487** · `log_stats` **475**) · `admin/retroactive.py`(`publish` **611** · `OPERATIONS` **425**) · `models.py` · `effort_metric.py` · `ontology_config.py` · `graph_sync_worker.py` · `maps/preset_routing.py` · `advanced_ingester.py` · `enrichment/config.py` · `admin/auth.py` · `run_auto_update.py` · `database.py` · `db_safety.py` · `virtual_join/config.py` · `enrichment/backfill.py` · `chain/replay.py` · `graph_orphans.py` · 클라 10종 · 계약 3종.
>
...
> - **`runtime/process_supervisor.py`** 718 → **1,024**(+306). 포트 충돌 판정 + 자식 로그 펌프. `read_status` 711 → **1017**.
> - **`internal_event_client.py`** 235 → **359**(+124) · **`run_watcher.py`** 341 → **367**(+26) · **`chain/ingestion_worker.py`** 1,065 → **1,086**(+21). **셋이 한 복구 경로의 세 조각**이다(미배달 통지 마커).
>
...
| 🆕 **`_get_http_session()`** · 모듈 로컬 `_http_local` | `server/chain/ingestion_worker.py` (구 [Warmup #3] 스레드별 세션 저장소, `a82aa47:133`) | **[`23a346d`] `internal_event_client.internal_event_session()`으로 이관**([§1.7](#17-serverinternal_event_clientpy--내부-http-호출의-단일-소유자-23a346d-신설)). keep-alive·스레드 로컬 성질은 그대로이고 **`trust_env = False`가 추가**됐다 — 이 파일이 자기 세션을 만들던 것이 2026-07-30 프로덕션 403의 경위다. 개명이 아니라 **소유권 이전**이고, 그래서 세 발신자(`chain_ingestion_worker`·`graph_sync_worker`·`run_watcher`) 전부가 같은 객체를 쓴다. 소스에 묘비 주석이 있다(`chain/ingestion_worker.py` ~131–136 — [§0 ⑧](#0-묘비-목록--소스에-존재하지-않는-이름)의 실측 히트 1건이 그것이다) |
...
| `server/database/crud.py` | 🆕 **5,715** @`c904b389`(HEAD — `b1db471a` 5,664 에서 **+51**: S-243 `DROP_ABSENT_NOT_WRITTEN` · `CHAIN_SOURCE` · `can_mean_emptied` · `apply_row_update_internal` 의 부재 관문, 아래 §2) · 🆕🆕🆕🆕 4,172 @`347de78`(3,980 → **+192**: `compute_priority_value`의 3단 tie-break 신설 + `resolution_ingested_at` 신설 + `_warn_undeclared_column_once` 재작성 + `undeclared_column_drops()` 신설 — 아래 §2) · 그전 3,980 @`7097a67`(top-level 심볼 집합 `34d2518` 대비 **무변동**) · 종전 등재 3,526 (@`e943e46` 재확인 — `d3ed167` 3,209 → **+317**: 🆕 **[P6] 복합 키 프리페치** · 🆕 **[`87a944e`] `replace_map` 차집합 경로**). ⚠️ **`server/database/crud.py`는 없다** | [§2](#2-serverdatabasecrudpy--레이어링-코어) |
...
| `server/chain/ingestion_worker.py` | 🆕🆕🆕🆕 **1,328** @`347de78`(종전 등재 1,086 → `34d2518` 1,198 → `7097a67` 1,294 → **+34**: 신설 `_undeclared_drop_note()`/`_DROP_NOTE_TOP_N=5` — `crud.undeclared_column_drops()`의 다이제스트를 하트비트 `note=`로 얹는다) | [§4](#4-serverchain_ingestion_workerpy--체인-워커) |
| 소형 서버 모듈 (models/std_parser/**enrichment_config**/enrichment_mapper/ingestion_activity/ingestion_checkpoint/**bonding_plan**/map_overlay/**transfer_plan**/map_meta_registrar/effort_metric) + 그래프 트랙(graph_sync_worker/graph_materializer/ontology_config) + **운영 6종**(paths/**process_supervisor**/health/heartbeat/**product_tables**/config_backup) | 🔴 **13,499은 낡았다** (2026-08-04 20파일 전건 재측정 합). 🆕⑤ **[2026-08-13] 이 합은 재계산하지 않았다 — 20파일 전건을 다시 재야 유효한 수인데 이 패스는 그중 `ingestion/checkpoint.py` 하나만 열었다**(258 → **587**, 즉 이 합은 최소 **+329** 밀렸고 그 밖의 파일이 얼마나 움직였는지는 **모른다**). 합계를 「+329 했다」고 적으면 안 잰 19파일을 잰 척하게 된다 | [§5](#5-소형-서버-모듈) |
| **[신설] 2026-07-30 서버 모듈 8종** (value_suggest/map_preset_routing/~~graph_orphans~~/chain_replay/**enrichment_analysis**/enrichment_candidates/keyset_scan/utils.time_format) | **4,094** (2026-08-04 8파일 전건 재합산, 4,078에서 **+16**). 🪦 **[2026-08-16] `graph_orphans.py`가 삭제돼 이제 «7종»이고, 위 합계는 지운 파일을 포함한 낡은 수입니다 — 재합산해야 유효합니다**(안 잰 일곱을 잰 척하지 않으려고 빼기만 하지 않았습니다) | [§5-A](#5-a-2026-07-30-신설-서버-모듈-8종) |
...
| **`server/db_safety.py`**(#16a 테스트 프로세스 DB 가드 🆕⑤ **+ 운영자 패스 읽기 전용 가드**) + **`server/virtual_join/config.py`**(가상 조인 **선언** 로더/검증) | 🆕⑤ **453** @`831ab68`(215에서 **+238**, `1260c9b`) + **512** (`virtual_join/config.py`는 이번 패스가 재측정하지 않았다 — ⚠️ 이 값은 아래 §5-C 제목의 **687**과도 어긋나 있고, 그 불일치는 이번 구간이 만든 것이 아니다) | [§5-C](#5-c-2026-07-31-신설-서버-모듈-2종) |
| **[신설] 2026-08-04 서버 모듈** — **`virtual_join/executor.py`**(가상 조인 **실행**, 535 → **554**) · `admin/retroactive.py`(🆕⑯ 소급 «등록부» — op **6종** + 실행 행 + 취소) · `enrichment/backfill.py` · `trace_fixture/` 패키지 6파일 | **554** + 🆕⑯ **1,139**(구 표기 697/545) + **411** + **1,356** | [§5-D](#5-d-2026-08-04-신설-서버-모듈) |
| 🆕⑯ **[신설] `server/pacing.py` + `server/pacing.json`** — 긴 작업 «셋»(원장 백필·파일 인제션·소급 등록부)이 **한 표**를 읽는다 | **56** + **26** (신설 `49988247`) | [§5-J](#5-j-serverpacingpy--pacingjson--긴-작업이-얼마나-미는가-표-하나-49988247-신설) |
| 🆕⑯ **[신설] `server/ledger/gaps.py` + `gap_names.json`** — 「선언은 있어야 한다는데 원장이 비어 있는 곳」. 라우트 `GET /api/ledger/gaps` | **346** + **72** (신설 `b18b22a6`) | [§5-H](#5-h-정본-원장-canonical-ledger) |
...
| 🆕⑩ **[신설] 온톨로지 config 작성·탐색 표면** — `ontology_config_explorer_router.py`(**15라우트**) + `ledger/config_{authoring,explorer,explorer_service,drafts}.py` + `implementations.py` · `column_stats.py` · `ledger_skeleton.json` + 클라 `ontology_explorer{,_store,_view}.js` · `ontology_path.js` · `ontology_skeleton.js` · `dom_patch.js` | 서버 **264**+**1,548**+**1,380**+**946**+**806**+**172**+**287**+**763** / 클라 **1,323**+**465**+**2,199**+**92**+**153**+**221** (@`8d1e6c4c`) | [§5-H-bis](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재) |
| 🆕⑨ **[신설] 미등재 모듈 레지스트리** — 맵에 **절이 아예 없던** 서버 25 · 클라 13 + 페이지 2(존재·크기만, 심볼 표 없음). 🆕⑩ **원장 v2 작성 계열 9 + 클라 온톨로지 3은 위 두 절로 승격했다** | — | [§5-I](#5-i-미등재-모듈-레지스트리-2026-08-18-실측) |
| 기타 서버 모듈 (한줄 요약) + 설치·개발환경 스크립트 + **교차 구현 계약 `contracts/` (6계약)** + **빌드 게이트 3종** + **런처 `run_decoupled_app.py`(🆕⑨ **389** @`5359fdd` — 구 표기 228)** | — | [§6](#6-기타-서버-모듈-한줄-요약) |
| 🆕 **`server/map_alignment.py`**(프레임 정렬 채점자) + **`server/maps/frame_confirmation.py`**(확정 기록자) + `migrations/add_frame_confirmation.py` | 🆕🆕🆕 **6,468** + **798** (@`68db020` 실측 — 등재 당시 3,272 + 688. 🔴 **`frame_confirmation`이 처음으로 줄었다** — private `_basis_cells_for`가 `map_alignment.basis_cells_for`로 이사·공개됐다) | [§5](#-servermap_alignmentpy--프레임-정렬의-채점자) · [§5-F](#5-f--정렬-채점-계열-index-scoring-family--servermap_alignmentpy-2026-08-07-등재) |
...
> 🆕 🔴 **[2026-09-15 실측] 이 파일은 `c904b389`(HEAD)에서 **6,989줄**이고 정본 철자(`^@app\.(get|post|put|delete)\(`)의 라우트는 **90**이다** — `b1db471a` 의 6,956/89 에서 **+33 / +1**: 그 하나가 `GET /admin/chain/rules/replayable`([§1.4](#14-api-라우트-표--어드민운영그래프맵인리치먼트), `df824a9a` S-250). ⚠️ 아래 🆕⑱ 블록의 6,282/87 과 `b1db471a` 사이의 커밋들은 이 패스가 «열지 않았다» — 그 구간의 라우트 증분(+2)은 이 표에 미등재일 수 있다.
>
> 🆕⑱ 🔴 **[2026-09-05 실측] 이 파일은 `6345ef3d`(HEAD)에서 **6,282줄**이다** — 🆕⑰ 의 5,987 에서 **+295**. ⚠️ **그런데 라우트는 «줄었다»**(89 → 87) — 아래 계수 블록의 −7/+5 참조. 이 구간에서 이 파일이 답한 것 중 이 지도가 재측정한 것은 **`/dashboard/summary`**(응답 필드 셋 삭제, [§1.1-ter](#11-ter-핵심가치-1-계측--재수정률--상호작용-공수-점수-2a9f6c4-v1-신설))·**`/health` 블록**([§1.1-bis](#11-bis-헬스-블록-8117456-신설--파일-상단))·**그래프 지선 삭제**([§1.5](#15-그래프-조회-구간은-410으로-은퇴했다--그-일곱은-이제-없다)) 셋이다. **그 밖의 라우트 표(§1.2·§1.3·§1.4)는 이 패스가 열지 않았다.**
>
> 🆕⑰ 🔴 **[2026-09-02 실측] 이 파일은 `64b562b6`에서 **5,987줄**이었다** — 🆕⑯ 의 5,912 에서 **+75**. 증분의 머리기사는 **개수 라우트 하나**(`GET /tables/{table_name}/data/count`, [§1.2](#12-api-라우트-표--데이터-조회편집))와 그 라우트가 «그리드 조회와 공유하는 조립» 둘이다.
>
> 🔢 **계수 재측정 — 철자를 «명시»한다. 종전 표기의 `@app.<verb>(` 는 `@app.websocket` 을 «안 세는» 철자였고, 그 사실이 안 적혀 있어서 다시 재는 사람은 하나 큰 수를 얻는다.**
> ```
> 철자                                                        9eb30691   64b562b6   🆕⑱ 6345ef3d(HEAD)
> ^@app\.(get|post|put|delete)\(          <- 이 표의 «정본»       88          89          «87»
>   + websocket 까지                                            89          90          «88»
> ^\s*@app\.(get|post|put|delete)\(  (들여쓰기 포함)              97          98          «96»
>   + websocket 까지                                            98          99          «97»
> @app.…(dependencies=[Depends(require_admin_token…)])  «한 줄»   33          33          «38»
>   + 여러 줄 데코레이터의 이어지는 줄까지                          34          34          «39»
> ```
> 🔴 🆕⑱ **[2026-09-05] 계수가 «줄었는데 파일은 자랐다» — 그것이 이 줄에서 제일 중요한 사실이다.** 파일은 5,987 → **6,282줄**(+295)인데 라우트는 89 → **87**(−2)이다. 갈라 보면 **−7 / +5**:
> ```
> 나갔다 «7»   `8ffe23d7` — 은퇴한 그래프 지선의 410 스텁 일곱 (아래 §1.5. 지금 «없다»)
>              POST /api/graph/sync · GET /graph/{stats,neighbors,nodes/search,chip-trace,mapping-summary} · POST /graph/trace
> 들어왔다 «5»  GET /admin/chain/queue
>              GET·POST /admin/tables/config/raw   ·   GET·POST /admin/chain/rules/raw   (넷 다 require_admin_token)
> ```
> ⚠️ **「87 < 89 니까 후퇴」로 읽으면 두 사건이 «한 수»에 접혀 보이지 않는다** — 계수 하나는 구성원을 못 센다. 🔴 **어드민 게이트 계수가 33 → 38 로 «다섯» 는 것이 그 +5 와 정확히 짝을 이룬다.**
>
> ✅ 🆕⑱ **strict 게이트는 여전히 «셋»이고 이름도 그대로다**(`POST /admin/retroactive/{op}/run` · `POST /admin/auto-update/run-now` · `POST /admin/scripts/code` — `require_admin_token_strict`. 전건 grep 4건 중 하나는 import 줄이다). 🔴 **개수 라우트는 게이트가 «없다»** — 그리드 조회(`/data`)와 «같은 노출 면»이고, 개수만 따로 잠그면 그 잠금이 아무것도 안 지킨다(같은 필터로 행 자체를 이미 내주므로).
>
...
>
> 🆕⑮ 🔴 **[2026-08-30 실측] 이 파일은 `5a73021a`(HEAD)에서 **5,860줄**이었다** — 직전 `2a4e51cf` 의 5,962 에서 **−102**이고, 그 감소는 전부 `5db3b80c` 가 리로드 두 함수의 «몸»을 [`server/runtime/system_reload.py`](#19-serversystem_reloadpy--리로드가-엔트리포인트-밖으로-나간-자리-5db3b80c-신설) 로 옮긴 것이다. ⚠️ **이 패스가 잰 것은 줄 수와 리로드 블록·`startup_event` 뿐이다** — §1.2~§1.7 의 라우트 표·심볼 표는 열지 않았다.
>
> 🆕⑨ 🔴 **[2026-08-18 실측] 이 파일은 `5359fdd`(blob `9cee25d`)에서 **6,057줄**이었다.** 이 문서 안에 공존하던 두 값(§0 표의 **6,322** · 여기 있던 **6,128**)은 둘 다 낡았다 — **줄어든 것이 이 파일에서 처음이 아니고**, 이번 감소는 그래프 구간 은퇴([§1.5](#15-그래프-조회-구간은-410으로-은퇴했다--그-일곱은-이제-없다))가 만든 것이다. ⚠️ **이 패스는 줄 수만 쟀다** — §1.1~§1.4의 심볼 표는 열지 않았다.
>
> 🔴 **[2026-08-06] (당시 표기) 이 파일은 5,488 → **6,128줄**(+640)이었다.** 이 범위의 머리기사는 **프레임 정렬·확정 라우트 4종**(`/api/maps/alignment/{view,confirm,worklist,references}`)과 **큐 술어**(`apply_enrichment_queue_predicate`)의 착지다.
>
...
| **`class VirtualColumnBinder`** | **[`cd3e0f4`] 가상 조인 컬럼을 쿼리에 물리는 자리.** `__init__`에서 `virtual_join_executor.exposed_columns(db, table_name)`을 한 번 물어 들고 있다가, 노출 컬럼이 필터·검색·🆕**정렬**(A-6 · 2026-09-07)에 등장하면 `resolved_expression`으로 `outerjoin`+`COALESCE` 식을 만들어 준다. 정렬은 `main.resolve_sort`(`apply_column_filters` 바로 위)가 이 객체를 지난다 — 「모르는 이름은 이름 대어 422」이고 조용한 `row_id` 기본값은 없다. 🔴 **`/schema`가 알리는 목록과 검색이 받아들이는 목록이 같은 함수에서 나온다** — 갈리면 화면에 보이는 컬럼으로 검색이 안 되는데 아무 에러도 안 난다 |
...
| 🆕㉖ 🔴 **`total_order_key(table_model, table_name)`** :1635 | **그 관계에 «전순서»를 주는 컬럼** — `row_id`, 없으면 카탈로그의 `business_key`(S-186 토대 `d0972e91`, 판정 296). 🔴 **함수 하나, 부르는 자리 «다섯»** — `fetch_and_merge_metadata` :839(`.key` — 행 봉투의 신원 이름) · `_named_sort` :1671(동점 깨기) · `resolve_sort` :1738(기본 정렬) · `get_table_data` :2152(목표 행 점프) · `get_table_data` :2258(페이지 경계 — 2단계 페이징의 첫 단계). ⚠️ **독스트링은 「ONE FUNCTION, THREE CALLERS」라 적고 있고 그것은 오늘 거짓이다** — 소스 자신이 :2258 에서 「A FOURTH SEAT, FOUND BY GREP RATHER THAN NAMED BY THE ORDER」라고 적어 둔다. 이 줄의 다섯은 `git grep` 으로 센 수다. 🔴 `row_id` 가 있으면 «언제나» 그것이 답이다(S-131: 그리드의 전순서를 옮기면 `idx_<t>_updated` 가 못 쓰이고 한 페이지가 44만 행을 0.7 s 동안 정렬한다). 🔴 **`None` 을 돌려줄 수 «없다»** — SCHEMA_CANON R7(「상한 걸린 읽기에는 전순서가 있어야 한다」)이라 둘 다 없는 관계는 422 로 «이름을 대어» 거절된다<br>🆕㉕ 🔴 **[2026-09-13 S-229 `a847cff1` · S-229-b `c6c26b89`] 이름도 답도 바뀌었다 — 오늘은 `total_order_keys(table_model, table_name)` :1702 «복수»이고, 답이 «세 단»이다: `row_id` → `business_key` → 🆕 `composite_key_source`.** 🔴 그래서 **위 줄의 「둘 다 없는 관계는 422」가 오늘 거짓이다** — 카탈로그가 «컴포짓»을 선언하면 그 관계는 페이징된다. 부르는 자리는 `main.py` 안 «여섯»(git grep 으로 센 수). 🔵 그리드의 행 신원은 `crud.compose_business_key`(22 파일)가 조각을 잇는다 — 컴포짓 키 뷰에는 그것을 «한 컬럼»으로 나르는 자리가 없기 때문 |
| 🔴 🆕⑮ **`reload_local_process_cache = system_reload.reload_local_process_cache`** | 🔴 **[2026-08-30 `5db3b80c`] 이 파일에 함수 선언이 없다 — *의도적 재export*다.** 위 `get_column_filter_condition` 행과 **글자 그대로 같은 모양**이고 같은 규칙이 낳았다([§1.9](#19-serversystem_reloadpy--리로드가-엔트리포인트-밖으로-나간-자리-5db3b80c-신설)). **`main.py`에서 `def reload_local_process_cache`를 찾으면 없다** — `git grep -n "^def reload_local_process_cache" -- server/main.py` 는 0건이고, 몸은 `server/runtime/system_reload.py` 에 있다. ✅ **`main.reload_local_process_cache` 를 이름으로 부르던 코드는 계속 해석된다** — 대입이라 **같은 함수 객체**다. 종전 서술(아래)은 이제 `system_reload` 쪽 함수의 서술로 읽어라: 웹서버 config 핫리로드 — `models.refresh_dynamic_models(engine)` 위임(싱글턴·ORM·신규 테이블 물리 CREATE, 이슈 #7) + `virtual_join_executor.reset_cache()` · **`notation_norm.reset_cache()`**(파생 스펙은 워커에 TTL이 있지만 어드민에서 편집된 선언은 **여기서 다음 쓰기부터** 먹어야 한다) · `ledger_trace.reset_walk_cache()` + `load_resolver_config(force_reload=True)` · `config_authoring.skeleton.cache_clear()` · `sys.modules` 에서 `mappers.*`/`pipeline_plugin_*` 퇴거. 🪦 **구 서술의 「`crud._ontology_cache` 무효화」는 없는 이름이다** — 이 함수 몸에 없을 뿐 아니라 **`server/` 전체(`_archive/` 제외)에 `_ontology_cache` 가 0건**이다(`5a73021a` 전건 grep). 이 낡음은 이번 이사가 만든 것이 아니다 — 옮겨 온 몸이 바이트 그대로이므로 **그 전부터 틀려 있었다** |
| 🆕⑮ `startup_event()` 안 **`system_reload.active_watcher = global_watcher`** | 🔴 **대입 «위치»가 계약이다.** `WorkspaceWatcher(...)` 생성자가 반환하는 **그 줄**에서 대입하고, `discover_and_watch()`·`start()` **뒤가 아니다** — 그 둘은 핸들러의 `except` 로 raise 할 수 있고, 구 코드는 그 경로에서도 다음 리로드에 `global_watcher`(이미 바인딩됨)를 읽었다. 늦게 대입하면 **정확히 그 경로에서 워크스페이스 동기화가 조용히 사라진다**. `global_watcher` 자체는 `main` 의 모듈 전역이라 **옮겨 갈 수 없었던 유일한 것**이다 |
...
| GET `/health` → `health_check()` | `heartbeat.read_all()` + `process_supervisor.read_status()` + DB/outbox 프로브를 `health.compute_health`에 넘겨 **JSONResponse + 실제 HTTP 상태**(unhealthy면 503)로 반환. ⚠️ **게이트 없음** — 외부 모니터가 토큰 없이 폴링해야 하므로 의도적으로 열려 있다(`/admin/*`이 아니라 `/health`다).<br>🔴 🆕⑱ **[2026-09-05 실측 · `2ac37874`] 명부(roster)는 «이 핸들러가 아니라 `compute_health` 안»에서 읽힌다.** `main.health_check` 는 종전 그대로 `heartbeat.read_all()` + `process_supervisor.read_status()` + 프로브만 넘기고, **`heartbeat.read_roster()` 는 `health.compute_health` 가 «감독자 목록이 비었을 때» 부른다** — 그러니 `main.py` 에서 `read_roster` 를 찾으면 **0건**이고 그것이 정상이다. 🔴 **그 배선이 «재기동마다 503」을 없앴다** — 상세와 사유는 [§5 `runtime/health.py`](#5-소형-서버-모듈) 의 명부 절. ⚠️ **같은 날 아침엔 «안 읽었다»** — `6345ef3d` 에서는 명부 판정이 별도 함수(`roster_states`)로 서 있고 소비자가 0이었다. |
...
| GET `/dashboard/summary` → `get_dashboard_summary` | `recorrection` + **`effort`**를 한 응답에(`schemas.DashboardSummaryResponse.effort`).<br>🔴 🆕⑱ **[2026-09-05 실측 · `471f66f7`] 응답 필드 «셋»이 나갔고 그것을 만들던 «루프 하나»도 같이 나갔다** — `total_tables` · `total_rows` · `table_stats`. `schemas.DashboardSummaryResponse` 에서도 삭제됐다(소스 주석이 그 자리에 묘비를 남긴다). **남는 필드 다섯**: `today_updates` · `system_health` · `recorrection` · `effort` · `uncounted_tables`.<br>⛔ **`uncounted_tables` 는 «남았고», 그것을 남기려고 루프도 «모양만» 남았다** — 컬럼 셋(`row_id`·`updated_at`·`created_at`)을 여전히 해소하되 **`.limit(0)`** 이라 계획만 서고 행은 안 온다. 🔴 **그 이유가 이 행에서 제일 중요하다**: 못 세는 표 목록은 count 가 «실패해서» 나오던 **부산물**이라, 질의를 그냥 지웠으면 그 목록이 **조용히 비었을** 것이다 — 「어느 표도 안 깨졌다」와 「내가 안 봤다」는 다른 사실이다.<br>⚠️ **표마다 자기 `try` + `db.rollback()`** — PostgreSQL 은 실패 문장 하나로 트랜잭션 전체를 중단하므로 롤백이 없으면 격리가 «주석»이 되고 뒤의 표가 전부 따라 죽는다 |
...
| GET `/api/download/client` :642 · GET `/api/desktop/download` :664 | `download_desktop_client` | 데스크톱 셸 배포. 🆕㈟ **[S-211 ③ `e3a21b31`] 둘 다 `main.DESKTOP_DIR` :639 밑의 `dist/` 를 봅니다** — 종전에는 `client/` 이 여러 줄에 «손으로» 적혀 있었고, 이제 이 프로세스에 쳊자가 «하나»입니다. ⚠️ 그 상수는 `run_decoupled_app.DESKTOP_DIR` :36 과 «짝»이고, 소스 주석(:635)이 그 둘을 「한 것의 양끝」으로 적습니다 |
| GET `/tables` | `list_tables` :1001 | 테이블 목록. 🆕㉗ **[S-187 `8d4f2ceb`] 응답이 `kinds` 를 든다** :1034 — `{표: kind}`, 값은 `setup_bundle.catalog_kind(entry)` :217 «하나»가 정한다. 🔴 **모든 표가 «반드시» 들어 있다**(`map_key_columns` 와 다르다 — 거기서는 부재가 「맵 키 없음」이다). 모든 관계는 kind 를 «가지»므로, 빠진 항목은 읽는 쪽에게 «추측»을 시키고 그 추측은 이 응답이 명시적으로 넘겨주는 바로 그 기본값이 된다. 🔵 **왜 클라에 주나** — 그리드가 «쓰기 문이 어차피 거절할 것»에 편집 진입을 내주지 않게 하려는 것이다(종전: 운영자가 «타이핑한 뒤» 400 으로 배웠다) |
...
| 메서드 경로 | 핸들러 | 역할 |
|---|---|---|
| 🪦 POST `/api/graph/sync` | 🆕⑱ **핸들러가 «없다»**(구: `manual_graph_sync`) | 🔴 🆕⑱ **[2026-09-05 실측 · `6345ef3d`] 이 라우트는 등록되지 않는다** — `8ffe23d7` 이 스텁 일곱을 지웠고 `manual_graph_sync` 는 `server/` 전건 grep **0**이다. 실제 응답은 **404**(`/api` 접두어가 catch-all 가드에 있다). 아래 [§1.5](#15-그래프-조회-구간은-410으로-은퇴했다--그-일곱은-이제-없다). **아래는 410 이던 시절의 기록이다.** ⚰️ ~~**[2026-08-18 실측] 본체는 `raise _graph_branch_retired()` 한 줄이다 — 410.**~~ 종전 이 칸은 「:8090 프록시 백필/복구 트리거」와 `httpx.AsyncClient(trust_env=False)` 프록시 규율을 서술했는데, **`GraphSyncRequest`도 httpx 호출도 이 파일에 없다**(grep 0건). 프록시 규율 자체는 여전히 유효하고 살아 있는 서술은 [§1.7](#17-serverinternal_event_clientpy--내부-http-호출의-단일-소유자-23a346d-신설)에 있다 — **여기서 읽지 마라**. 아래 [§1.5](#15-그래프-조회-구간은-410으로-은퇴했다--그-일곱은-이제-없다) |
...
| 🔒 POST `/admin/reload-configs` | `reload_system_configs` → 🆕⑮ **`system_reload.reload_system_configs(db)`** | config 핫리로드 — 동기 CREATE(1차 DDL 소유자)가 outbox 발화보다 선행 (+SYSTEM_RELOAD outbox 발화). **[2026-07-30] 그래프 resync도 이제 같은 레버를 쓴다** — `graph_sync_worker.publish_system_reload`([§5](#5-소형-서버-모듈)). 🆕⑮ **[`5db3b80c`] 라우트는 `main.py` 에 남고 «몸만» 나갔다** — 같은 경로 · 같은 `require_admin_token` 의존성 · 같은 응답이고 핸들러 한 줄이 위임한다([§1.9](#19-serversystem_reloadpy--리로드가-엔트리포인트-밖으로-나간-자리-5db3b80c-신설)). 🔴 **`main.reload_system_configs` 는 라우트 핸들러이지 재export가 «아니다»** — `system_reload` 쪽 동명 함수와 **다른 객체**이고 시그니처도 다르다(`db: Session = Depends(get_db)` vs `db: Session`). 재export인 것은 `reload_local_process_cache` **하나**뿐이다 |
| 🔒 GET `/admin/chain/rules` · `/admin/mappers/list` | `get_chain_rules` / `get_mappers` | 체인 룰·맵퍼 목록. 🆕㉜ **[S-207 `2ec11afc`] `/admin/mappers/list` 가 `registered` 를 «더» 낸다** — `sorted(mapper_sdk.MAPPER_REGISTRY)`(`main.py` :5663 @`c904b389` — 구 표기 :5508), 즉 **「저장이 «받아 줄» 이름」**이다. ⚠️ **`data` 는 «그대로 AST»다** — 파일 안의 함수 전부이고, 등록되지 않은 것도 든다. 즉 이 응답은 «두 목록»이고 그것이 의도다: 화면이 «고르게» 하는 것은 `registered` 이고, `data` 는 「파일에 무엇이 있나」다. 🔴 그 둘이 갈리는 것이 «발견»이다 — `@mapper` 가 안 붙은 함수는 저장에서 거절되고, 종전에는 화면이 그것을 «고를 수 있게» 내놓고 있었다<br>🆕㈥ **[S-223 `db282163`, 판정 379] 칸 «셋»이 더 붙었다 — `candidates` · `other` · `refused`, 서로 «배타».** 🔴 **드롭다운은 이제 `candidates` 에서 온다** — `registered` 하나로 채우니 «도는 맵퍼로 가득한 디렉터리»를 두고 「선택지 없음」이었기 때문이다(운영 맵퍼는 대개 규칙이 `mapper_function` 으로 이름 대는 «모듈 수준 함수»다). ⚠️ 셋을 합치면 화면에 «거짓 문장»이 선다 — 「여기 없다」·「고장났다」·「다른 계열이다」는 세 답이다. `data`·`registered` 는 «한 글자도» 안 바뀜다 |
| 🆕 🔒 GET `/admin/chain/rules/replayable?table=` | `get_replayable_chain_rules(table: str)` :5598(@`c904b389`, `df824a9a` S-250) | **「이 표를 «트리거»로 하는, 다시 돌릴 수 있는 규칙」만** — 그리드 배너([§7 `redo_banner.js`](#7-client2src--웹-클라이언트))가 고르는 목록. 🔴 **목록과 실행이 «갈릴 수 없다»**: 몸은 `chain/replay.replayable_rules_for(table)`(§5-A) «하나»이고 그것은 소급이 실제로 도는 집합(`load_rules`)을 «같은 함수»로 거르며 참조 쪽 거절도 `find_rule` 이 쓰는 `is_reference_side` 를 지난다 — 종전 화면은 위 `/admin/chain/rules`(파일 «원문»)를 읽어 통합 join(`on.table` 을 쓰지 `trigger_table` 이 없다)이 «안 보였고» 합성 규칙(인리치·가상 조인 — 로더가 세운다)은 «아예 없었다». ⚠️ **모르는 표는 «빈 목록»이 아니라 404** — `wanted not in crud.TABLE_CONFIG` → 「규칙이 없다」와 「그런 표가 없다」를 «같은 픽셀»로 접지 않는다. 새 SQL «0». 응답 원소 `{name, trigger_table, target_table, kind}` — `kind` 는 §4-ter-b `rule_shape.declared_kind` 의 세 낱말(`join`·`decide`·`mapper`). 시험 `test_the_replay_list_is_the_set_that_actually_runs.py`(186줄 / `def test_` 11) |
| 🆕⑳ 🔒 GET `/admin/chain/queue` | `get_chain_queue_depth` | **「체인 요청이 씹히는 것 같다」를 수로 바꾼다 — 읽기 전용.** 🆕⑱ 가 계수 블록에서 «들어왔다 5» 로 이름만 세고 이 표에는 안 실었던 라우트다.<br>🔴 🆕⑳ **`table_name` 이 `event_constants.PLACEHOLDER_TABLE_NAMES` 안이면 `tables` 목록에 «안 싣는다»** — `database_outbox.table_name` 은 관례상 비지 않아서 표가 없는 행이 자리를 채우려고 이름을 «지어내고»(`RETROACTIVE_RUN_TABLE = "__retroactive__"`), 그 이름이 화면에 뜨면 운영자가 없는 표를 «찾으러 간다». 신원은 안 지운다 — 같은 그룹의 `event_types` 가 이미 말한다.<br>🔴 **부류는 「통제 이벤트인가」가 «아니다».** 그 규칙을 먼저 썼다가 한 명이 너무 넓어 되물렸다(`e7a6fe33`→`2742446f`) — `SCHEDULER_RUN_NOW` 는 호출자가 준 **실재하는** 표 이름을 싣고, `event_types` 는 그 답을 들고 있지 않다. ⛔ **리터럴로 맞추지 않는다** — 자리표시자는 [§6 `event_constants.py`](#6-기타-서버-모듈-한줄-요약)에 «선언»돼 있고 그 선언이 곧 「이 이름들은 표가 아니다」라는 문서다.<br>🆕⑳ **응답에 `**chain_activity.registry.ages()` 가 펼쳐진다** — `loop_uptime_seconds` · `mapper_reload_age_seconds`([§5-I `chain/activity.py`](#5-i-미등재-모듈-레지스트리-2026-08-18-실측)). ⚠️ **재적재가 없었으면 `null` 이고, `0` 은 「방금」이라는 «반대» 사실이다**<br>🆕㉕ **[2026-09-07] 응답에 «둘» 더** — `generated_at`(S-20: 이 응답의 «기준 시각». 🔴 그 «수를 만든 시각»이지 「지금」이 아니다 — 이 핸들러는 시계를 «한 번»만 읽고 나이들과 이 칸이 «같은 순간»을 쓴다. 그 「한 번」을 시험이 «모양»으로 못 박는다) · `rule_outcomes`(C-1: 규칙별 «마지막 결과» — `{last_outcome, last_reason, last_age_seconds}`. 어휘는 `event_constants.RULE_OUTCOMES` «여섯»이고, 선언된 규칙은 로더에서 `never_evaluated` 로 «씨가 뿌려져» 부재가 「옛 서버」 하나만 뜻한다. 🔵 `running` 을 내는 «같은 경로»이고 새 표는 «0» — 자리는 `chain_activity.registry` 다) |
| 🆕⑳ 🔒 GET·POST `/admin/tables/config/raw` · GET·POST `/admin/chain/rules/raw` | `get_table_config_raw`/`post_table_config_raw` · `get_chain_rule_raw`/`post_chain_rule_raw` | 표 «하나»·규칙 «하나» 단위의 원시 선언 편집(원자적 쓰기 + 저장 전 검증 + base 지문). 🆕⑱ 가 계수 블록에서 이름만 세고 이 표에 안 실었던 넷이다. ⚠️ **이 패스는 존재·핸들러명·게이트만 실측했고 본체를 읽지 않았다** — 소비자는 [§5-H-bis 클라 부품](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재)의 `raw_registry_panel.js` 계열이다<br>🆕㉜ **[S-205 `45f0da47`] `chain_rule_raw_view` 가 `skeleton` 을 «같이» 낸다** — `chain_bindings.skeleton()` :382 이고, **폼이 규칙의 «모양»을 알 필요가 없게** 하는 것이 요점이다(클라 절반은 탐색기의 `renderSkeletonForm` 을 그대로 쓴다 — §7).<br>🆕㉜ **[S-204 `5422b308`] 저장이 «로더 자신의 판정자»에게 묻는다** — `rule_refusals`(§`chain_bindings`), 그리고 **이 프로세스가 그 답을 할 수 있게** API 기동·리로드 뒤 `mapper_sdk.discover()` 를 부른다(판정 326). 🔴 **접는 것과 «같은 것을 보게 하는 것»은 별개였다** — 판정자를 하나로 접어도 등록부가 워커 프로세스에만 차 있으면 API 는 모든 매퍼를 「못 푼다」로 읽는다<br>🆕㈟ **[S-212 `3bdab11c`] 체인 규칙 GET 의 인자가 `rule` → **`name`** 입니다.** 화면은 쓸 때부터 `?name=` 을 보냈고(`client2/src/admin.js` :1120, 패널 선언 `nameKey: 'name'`, POST 페이로드도 `name`), 럼드는 그 문자를 모르는 GET 라우트만 `rule` 을 선언했습니다. 🔴 **FastAPI 는 모르는 쿼리를 «떨괍니다»** — 뷰가 `chain_rule_raw_view(None)` 을 받아 체인 탭에서 고른 규칙이 폼을 «항상 빈 채» 남겨 두었고, **아무것도 던지지 않았습니다**. 표 절반이 돌았던 것은 양쪽이 둘 다 `table` 을 말했기 때문입니다. 🔵 수리는 «둘째 받는 키»를 더하는 것이 아니라 «라우트 개명»입니다 — 한 낙말, 한 철자. ⚠️ **종전 라우트 시험이 이걸 못 본 이유**: 그 시험들이 라우트를 태우되 «서버의 낙말»(`params={"rule": …}`)로 불렀고, **핸들러가 치는 것을 그대로 치는 시험은 이음새를 못 찾습니다** |
...
| 🆕 🔒 **POST `/admin/retroactive/{op}/run`** | `trigger_retroactive_run` | 🔴 **`require_admin_token_strict`.** 그리고 **여기서 아무것도 실행하지 않는다** — `retroactive.publish`가 outbox 행 1개 + **`retroactive_runs` 행 1개**(🆕⑯ — **같은 커밋**) + `NOTIFY`만 하고, 실행은 스케줄러 프로세스의 데몬 스레드가 한다([§5-D](#5-d-2026-08-04-신설-서버-모듈)) |
| 🆕⑯ 🔒 GET `/admin/retroactive/runs` | `list_retroactive_runs(limit=50, db)` | **[2026-08-31 신설] 요청형 연산의 실행 행 — 최근 것부터.** `retroactive.runs(db, limit)` 하나를 부른다. 🔴 **파일 인제션은 여기 «없다»** — 파일마다 `file_ingestion_checkpoints` 에 total/processed/chunk 가 이미 있고, 그 화면은 그 표를 «그대로» 읽는다(핸들러 docstring 이 그렇게 못 박는다). 🔴 **`total_rows` 가 `null` 로 나오고 그것은 «0 이 아니라 모름»이다** — 행이 떨어질 때까지 걷는 연산은 시작 시점에 전체를 모른다. 0 으로 채우면 화면이 「할 일 없음」과 0% 를 그린다 |
| 🆕⑯ 🔒 **POST `/admin/retroactive/runs/{run_id}/cancel`** | `cancel_retroactive_run(run_id, db)` | **[2026-08-31 신설] 실행에 «멈춰 달라»고 적는다 — 프로세스를 죽이지 않는다.** `retroactive.request_cancel`. 🔴 **strict 가 «아니다»** — 값 하나를 세울 뿐이고 멈추는 것은 «연산 자신»이다(배치 사이에서 그 값을 보고 스스로 멈춘다). 🔴 **이미 끝난 실행은 «이름으로 거절»한다**(400) — 끝난 것에 「취소됨」을 돌려주면 운영자는 데이터가 반만 처리됐다고 읽는데 사실은 전부 처리됐다. ⚠️ **`cancellable: false` 인 연산에는 이 버튼을 내지 않는다** — `/operations` 가 그 값을 든다 |
...
> ⚠️ **위 정적 라우트는 조건부 등록이다** — `serve_admin_page`·`serve_map_editor_page`·🆕 **`serve_map_editor2_page`**·`serve_enrichment_page`·`serve_static_or_index` 전부 **`if os.path.exists(client2_dist_path):` 블록 안에 들여쓰기**돼 있다(🆕⑨ `/assets` mount **5928** @`5359fdd` — 구 표기 6004). ⚠️ **구 지도가 `client2_dist_path` 조립 지점으로 적던 `503–505`는 이 블록이 쓰는 그 변수가 아니다** — 🆕⑨ **[2026-08-18 재측정] 정적 블록이 읽는 모듈 레벨 조립은 `5921–5923`이고, 함수 지역의 다른 사본은 `564–566`이다**(구 표기 5997–5999 / 503–505는 둘 다 낡았다). 🔴 **두 사본이 «같은 이름, 다른 스코프»라는 사실이 요점이고 그것은 여전히 참이다** — 라인만 밀렸다. 즉 **빌드 산출물이 없으면 이 네 라우트는 앱에 존재하지 않고** `/admin`은 404가 아니라 **catch-all도 없는 상태**가 된다. 게이트 감사(`test_admin_auth.py`)가 앱의 라우트 목록을 훑는 방식이라, dist 없이 돌린 테스트는 이 넷을 **아예 보지 못한다**.

> ✅ 🔴 **게이트 개수 재측정 (2026-08-04 여덟 번째 패스) — 구 표기 "24 + 4 = 28"은 틀렸다.** 실측(`87a944e`): 게이트 데코레이터 **28개** = `/admin` + `/internal`. 게이트 없는 `/admin` 라우트는 **0개**다. ⚠️ **구 표기 27 = 23 + 4는 낡았고, 같은 문서의 다른 자리가 「24 + 4 = 28」이라 적어 두 값이 공존하고 있었다** — 지금은 실측이 후자와 일치한다. **strict — 전건 열거**(이름으로 고정, 개수에 기대지 않는다): **`POST /admin/retroactive/{op}/run` · `POST /admin/auto-update/run-now` · `POST /admin/scripts/code`**. 🔴 **셋의 공통점이 strict의 정의다 — 서버가 코드를 실행하게 만드는 요청**이다. 소급 실행이 여기 들어온 것은 그것이 데이터를 지우거나(`withdraw`·~~`graph_orphans`~~ 🆕⑯ **`ledger_rescope`** — 앞의 것은 **없는 op 다**, [§5-D](#5-d-2026-08-04-신설-서버-모듈)) 되쓰기(`chain_replay`) 때문이고, **조회 라우트는 평범한 `require_admin_token`이다.**
>
> 🆕⑯ ✅ **[2026-08-31 재측정 — `9eb30691`] 그 「28」은 이제 «33»이다.** 같은 철자(`^\s*@app\.…require_admin_token…`)로 다시 세었고, 늘어난 다섯 중 둘이 이 구간의 소급 라우트 둘이다. 🔴 **strict 는 «셋 그대로»이고 이름도 그대로다** — 개수가 늘어도 strict 가 안 늘었다는 것이 이 문단이 이름으로 고정한 이유다. ⚠️ **소급 조회 라우트는 이제 «둘»이 아니라 «셋»이다**(`operations`·`{op}/count`·`runs`) + 취소 하나. 「조회 둘」로 읽지 마라.

### 1.4-bis `serve_static_or_index` — SPA catch-all이자 **파일시스템 격리 경계** (`90e284f`)

⛔ **이것은 "정적 파일 핸들러"가 아니다.** 인증 없이 도달 가능한 이 함수가 곧 **프로세스가 읽을 수 있는 모든 파일과 외부 사이의 유일한 경계**다. 격리 검사가 없던 시절 `os.path.join(client2_dist_path, file_name)`이 그대로 서빙돼 `/../../server/config/table_config.json` · `/../../../../../../Windows/win.ini` · `/../../server/admin/auth.py`가 **전부 200을 반환**했다. 그 상태에서는 `GET /admin/scripts/code`·`/admin/chain/rules`·`/admin/file-ingestion/workspaces`에 건 게이트가 **장식**이다 — 지키려던 바이트를 옆문으로 읽을 수 있었고, 읽히는 파일 어딘가에 토큰이 있었다면 그것까지 함께 나갔다.

| 구간 | 라인 | 내용 |
|---|---|---|
| 접두어 목록 (`tables`/`ws`/`audit_logs`/`dashboard`/`admin`/`map-editor`/`map_editor`/`map-presets`/`enrichment/`/`api`) | 🆕⑨ **6014–6027** @`5359fdd`(blob `9cee25d`) — 구 표기 5449–5458 | **API 섀도잉 방지장치이지 보안 경계가 아니다** — 소스 주석이 그렇게 명시한다. 경로 **시작**만 보므로 `../../server/config/table_config.json`은 `admin`과 조금도 닮지 않아 그대로 통과한다 |
| **격리 검사** | 🆕⑨ **6044–6046** @`5359fdd` — 구 표기 6115–6117 | `dist_base = os.path.abspath(client2_dist_path)` → `target_path = os.path.abspath(os.path.join(dist_base, file_name))` → **`target_path`가 `dist_base` 자신이거나 `dist_base + os.sep`로 시작하지 않으면 거부.** `_resolve_admin_script_path`와 **같은 모양**이다(사유 주석은 접두어 목록 바로 위 **6014–6017**) |
| 서빙/폴백 | 🆕⑨ **6050–6056** @`5359fdd` — 구 표기 5481–5487 | 통과한 실파일만 `FileResponse`, 그 외 `index.html` |
...
- **거부는 403이 아니라 404다**(🆕⑨ **6047** 주석 @`5359fdd` — 구 표기 5478) — 정적 라우트가 "그 탈출 경로는 파싱됐다"고 확인해 주면 안 된다.
...
### 1.5 그래프 조회 구간은 410으로 은퇴했다 → 🆕⑱ **그 일곱은 이제 «없다»**

> 🔴 🆕⑱ **[2026-09-05 실측 · `6345ef3d`] 아래 표 전체가 낡았다 — 라우트 일곱은 «410 을 답하지 않는다. 존재하지 않는다».**
> `8ffe23d7`(2026-09-04, 소유자 판정 「지선을 통째로」)이 `main.py` 에서 **일곱 스텁 + `_graph_branch_retired()` + `GRAPH_BRANCH_RETIRED_REASON` + `GRAPH_BRANCH_SUCCESSOR`** 를 지웠다.
> ```
> 실측한 것                                            결과 (`6345ef3d`)
> 핸들러 이름 일곱                                      server/ 전건 grep  «0»
>   get_graph_stats · get_graph_neighbors · search_graph_nodes
>   post_graph_trace · get_chip_trace · get_graph_mapping_summary · manual_graph_sync
> _graph_branch_retired                                 «1» — migrations/drop_graph_storage.py 의 «산문»뿐
> @app 라우트 계수 (^@app\.(get|post|put|delete)\()      89 → «87»  (−7 그래프, +5 어드민 신설)
> ```
> 🔴 **그래서 「살아 있는 이름이 죽은 서술의 알리바이가 됐다」는 아래 문단의 교훈이 «한 세대 더» 나아갔다** — 이제 그 이름들은 grep 0 으로 자기가 낡았다고 말한다. **낡은 것은 이 절의 서술이지 이름이 아니다.**
>
> 🔴 **그리고 [§1.4-bis](#14-bis-serve_static_or_index--spa-catch-all이자-파일시스템-격리-경계-90e284f) 가 경고한 결과가 «지금 도는 동작»이다 — 커밋이 그것을 재고 적어 두었다.**
> ```
> GET /graph/stats · /graph/neighbors · /graph/nodes/search
>     /graph/trace · /graph/chip-trace · /graph/mapping-summary
>   -> SPA catch-all `@app.get("/{file_name:path}")` 이 «index.html 을 200» 으로 답한다.  404 가 «아니다»
> POST /api/graph/sync
>   -> «404».  catch-all 의 접두어 가드 목록에 `api` 가 «있고» `graph` 는 «없기» 때문이다
> ```
> ⚠️ **즉 아래 문단의 「410 은 catch-all 에 대한 방어이지 하위 호환이 아니다」는 «옳았고, 그 방어가 걷혔다».** 클라가 그 응답을 읽는다면 「알 수 없는 오류」가 아니라 **HTML 을 JSON 으로 파싱하려는 실패**를 본다.
>
> 🪦 **`server/tests/test_graph_branch_retired.py` 도 같은 커밋에 삭제됐다** — 그 시험이 잰 것이 정확히 위 코드였다. 🔴 그 삭제가 [§0](#0-묘비-목록--소스에-존재하지-않는-이름) 게이트 ㉕-c 의 기대값을 **1 → 0** 으로 바꿨다(그쪽에 반영했다).
>
> ⚠️ 🔴 **`RETIRED_GRAPH_TABLES = ("graph_nodes", "graph_edges", "graph_sync_state")` 는 «남아 있고 소비자가 0» 이다**(`main.py`, 지시 목록에 없어서 남겼다고 커밋이 적는다). `server/` 안의 나머지 히트 둘은 `main.py:121` 의 부팅 주석과 `migrations/drop_graph_storage.py` 의 산문이다 — **읽는 코드가 없다.** 이 지도가 그것을 적는 이유는 이 지도의 규율 그대로다: 「이 심볼이 살아 있는 이유」가 거짓이면 지도에 적고, 다음 패스가 그것이 착지했는지 본다.
>
> ⚠️ **낡은 소스 주석 «다섯»이 아직 「410 을 답한다」고 적는다**(전건 실측 `2ac37874` — [§0](#0-묘비-목록--소스에-존재하지-않는-이름) 게이트 ㉗-c 가 이것을 센다): `main.py:768` · `database/models.py:940` · `config_resolve_report.py:122` · `enrichment/config.py:708` · **`run_decoupled_app.py:324`**(「진입은 `main.py` 의 `_graph_branch_retired` 가 막고」 — 그 함수도 «없다»). **코드 소관이라 여기서 못 고친다. 그 주석을 동작의 증거로 읽지 마라.**
>
> ⚠️ **그리고 이 지도 «밖»의 문서 셋이 같은 낡음을 들고 있다** — `docs/architecture/backend.md`(410 라우트 표 + 「왜 지우지 않는가」) · `docs/architecture/PRIMITIVES.md`(`_graph_branch_retired` 를 정본 자리로 지목) · `docs/qa/FEATURE_CHECKLIST.md`(「`GET /graph/stats` → 410 이고 본문에 `reason` 이」를 점검 항목으로). 🔴 **셋 다 이 지도의 소관이 아니다**(리빙 문서·체크리스트는 doc-keeper). **여기 적는 이유는 그 낡음이 «같은 커밋 하나»에서 왔고, 한 자리만 고치면 나머지가 그 수정의 알리바이가 되기 때문이다.**
>
> **아래는 그것이 참이었을 때(2026-08-18 실측)의 기록이다.**

⚰️ **[라우트 410 전환 = `2ec78b9`(2026-08-14) · 절 축약 = 2026-08-18 실측 · 🪦 라우트 삭제 = `8ffe23d7`(2026-09-04)]** ⚠️ **날짜를 하나로 적지 마라** — 라우트를 410으로 바꾸고 표 DROP 마이그레이션을 들여온 것은 **`2ec78b9`(2026-08-14)**이고, **워커·실행 파일 삭제는 `9851374`(2026-08-16)**이며, **모델 클래스 제거는 2026-08-18**이다. 🔴 **여기 있던 표 두 개(26줄)는 «이름은 전부 실재하는데 동작은 하나도 남지 않은» 서술이었다.** `main.py`(`5359fdd` blob `9cee25d`) 실측:

| 실측한 것 | 결과 |
|---|---|
| 라우트 데코레이터 — 🔴 **접두어가 «둘»이다** | **7개 전부 살아 있다.** `/graph/`의 여섯(`stats` · `neighbors` · `nodes/search` · `trace` · `chip-trace` · `mapping-summary`) + **`POST /api/graph/sync`** 하나. 🔴 **`/graph/`로만 grep하면 여섯만 잡히고, 빠지는 하나가 «쓰기 진입점»이다** — 은퇴 여부를 확인하러 온 사람이 정확히 그 하나를 놓친다 |
| 그 일곱의 **본체** | 전부 **`raise _graph_branch_retired()` 한 줄** — HTTP **410** + `Cache-Control: no-store`, `detail={reason, state:"retired", successor, ruling:"R-2026-08-14-H", message}` |
| 핸들러 이름 (`get_graph_stats`·`get_graph_neighbors`·`search_graph_nodes`·`post_graph_trace`·`get_chip_trace`·`get_graph_mapping_summary`·`manual_graph_sync`) | **grep 1건씩 — 살아 있다.** 파라미터는 전부 사라졌다(`get_chip_trace(identity, db)` → `get_chip_trace()`) |
| 헬퍼·상수·어휘 (`_expand_graph_subgraph`·`_serialize_graph_edge`·`_serialize_graph_nodes`·`_chip_trace_declaration`·`_chip_trace_leg`·`_chip_trace_sort_key`·`GRAPH_NEIGHBOR_NODE_CAP`·`GRAPH_TRACE_DEPTH_CAP`·`GRAPH_CHIP_TRACE_SEED_LABEL`·`CHIP_TRACE_RECORDED` 계열) | 🔴 **grep 전부 0건** |

🔴 **이것이 이 문서가 지는 최악형 실패다 — 그리고 같은 날 앞선 감사 둘이 이 절을 통과시켰다.** 파일 단위 감사는 `main.py`가 있으니 통과시켰고, 심볼 존재 감사는 핸들러 이름 일곱이 전부 grep에 잡히니 통과시켰다. **살아 있는 이름이 죽은 서술의 알리바이가 됐다** — 확인하러 온 사람은 이름을 찾고 만족한다. 낡은 심볼은 「0건」으로 자기가 낡았다고 말하지만, **은퇴한 핸들러의 이름은 그 말을 하지 않는다.**

**지우지 않고 410으로 남긴 이유는 [§1.4-bis](#14-bis-serve_static_or_index--spa-catch-all이자-파일시스템-격리-경계-90e284f)에 있다** — 라우트를 지우면 SPA catch-all이 `index.html`을 **200**으로 돌려주고, 거절이 클라에서 「알 수 없는 오류」가 된다. 즉 **410은 catch-all에 대한 방어이지 하위 호환이 아니다.**

**후계는 `server/ledger/*`와 [§5-H](#5-h-정본-원장-canonical-ledger)의 원장 콘솔이다.** 🔴 **[2026-08-28] 이 두 라우트도 은퇴했다** — `/api/ledger` 아래 사는 것은 `subgraph` 와 `declaration` «둘»이고, 계보도 구조도 `GET /api/ledger/subgraph` 를 `follow` 로 좁혀 답한다. ~~계보 추적은 `/api/ledger/trace`, 구조는 `/api/ledger/structure`~~([§5-I](#5-i-미등재-모듈-레지스트리-2026-08-18-실측)의 읽기 측 신설 모듈도 참조). 저장소 모델·워커·config의 은퇴는 [§5 🪦 블록](#-serverontology_configpy--graph_materializerpy--graph_sync_workerpy--graph_orphanspy--파일이-없습니다).

⚠️ **클라 절반은 아직 이 지도에 살아 있는 것으로 적혀 있다** — [§7 `graph_viewer.js`](#7-client2src--웹-클라이언트)(**1,274줄**, 실측)와 `trace.js`·`trace_core.js`·`trace_launch.js`, 그리고 vite 엔트리 `graph`/`trace`가 그대로다. **이 패스는 그 넷을 열지 않았다** — 서버가 410을 주는 화면이 클라에 남아 있는 것이 의도인지(410 문구를 보여 주려고) 잔재인지는 **판정하지 않았고, 클라 도메인 소관이다.**

---

## 1.6 `server/admin/auth.py` — 어드민/내부 토큰 게이트 (`90e284f` 신설)
...
| 🆕 🔴 **`record_undelivered_notification(session_factory, table_name, endpoint, reason, logger=None)`** | **[신설] 배달 못 한 내부 통지의 durable 마커.** 종전 `run_watcher.post_event`는 실패를 **로그하고 버렸다** — 마커도, 재시도도, 복구 경로가 찾을 수 있는 기록도 없었다. 그래서 허브가 불통인 동안 나간 통지는 **영구 소실**됐다: **행은 DB에 앉았고 화면은 끝내 몰랐다.**<br>🔴 **새 큐를 만들지 않고 `database_outbox` 행 하나를 쓴다** — "자기 재시도 정책을 가진 두 번째 큐는 틀릴 곳이 하나 더 생기는 것"이다. 행 모양은 `event_type=event_constants.EVENT_BROADCAST_RECOVERY` + `status`/`processed_chain` + `broadcast_at=NULL` 이고, **그것이 체인 워커의 미배달 스윕이 이미 걷어 가는 정확한 모양**이다.<br>🔴 🆕⑱ **[2026-09-05 `443c1181`] 그 세 값을 «손으로 철자하지 않는다»** — `event_constants.UNDELIVERED_MARKER_STATUS` / `..._PROCESSED_CHAIN` / `..._TAG`(→ `payload["marker"]`)를 읽는다. 걷는 쪽이 «같은 상수»를 읽으므로 한쪽만 바꾸는 것이 이제 불가능하다. 아래 블록쿼트가 정본. ⚠️ **구 앵커 `137`·`135–142` 는 낡았다 — 행 조립은 **140–147**이다.<br>🔴 **실패한 통지의 페이로드를 복사해 넣지 않는다** — batch-refresh 페이로드는 감사 로그 **최대 500건**을 나르고, **그만큼 큰 마커는 복구 경로가 다음 인시던트가 되는 방법**이다(2026-07-25, ~50MB 페이로드). **절대 raise하지 않는다** | **95** |
...
> ✅ 🔴 🆕⑱ **[2026-09-05 실측 · `6345ef3d`] 이 블록쿼트가 「가장 깨지기 쉬운 항목」이라 부르던 그 결함이 «닫혔다» — 공유 상수가 «생겼다»**(`443c1181`). **아래 취소선 문단은 이제 거짓이므로 인용하지 마라.**
> ```
> 철자의 «집»          server/event_constants.py  (파일 «말미»)
>   UNDELIVERED_MARKER_STATUS           = "SUCCESS"
>   UNDELIVERED_MARKER_PROCESSED_CHAIN  = True
>   UNDELIVERED_MARKER_TAG              = "undelivered_notification"   # payload["marker"]
> 쓰는 쪽              internal_event_client.record_undelivered_notification  — 셋 «다» 읽는다
> 걷는 쪽              chain_ingestion_worker.sweep_undelivered_broadcasts    — 앞의 «둘»을 읽는다
> ```
> ⚠️ **셋 중 «컬럼 값»은 둘뿐이다** — `..._TAG` 는 컬럼이 아니라 `payload["marker"]` 에 찍는 귀속 표지다. 「컬럼 값 셋」으로 세지 마라.
> 🔴 **`broadcast_at` 은 «일부러» 그 집에 없다** — NULL 은 쓰는 쪽이 «세우는 값»이 아니라 걷는 쪽이 «찾는 부재»다. 이름을 지어 주면 누군가 그것을 쓰게 된다(소스 주석).
> 🔴 **그리고 시험도 이음매를 «건넌다»** — `server/tests/test_undelivered_marker_seam.py` 가 (ⓐ 쓰는 쪽이 실제로 만든 행을 걷는 쪽 필터로 집고 (ⓑ 두 파일의 «소스 텍스트»에 상수 이름이 있는지 단언한다. 종전엔 스윕을 덮는 시험 다섯이 **자기가 만든 행**을 넣어서, 쓰는 쪽이 무엇을 하든 초록이었다.
>
> 🔬 **실측 앵커(`6345ef3d`)** — 종전 이 문단의 수는 **전부 낡아 있었다**:
> ```
> record_undelivered_notification          internal_event_client.py  «95»    (행 조립 «140–147»,  구 135–142)
> sweep_undelivered_broadcasts             chain/ingestion_worker.py «1357»  (구 781)
>   그 안의 마커 필터                                                «1382–1385» (구 802–807)
> run_watcher._record_undelivered                                    «54»    (구 ~71)
> ```
>
> ~~🔴 **미배달 마커의 계약은 심볼이 아니라 *컬럼 값 세 개*라서 이 지도에서 가장 깨지기 쉬운 항목이다.** 쓰는 쪽은 `record_undelivered_notification`(**95**, 행 조립 **135–142**), 걷는 쪽은 `chain_ingestion_worker.sweep_undelivered_broadcasts`(**781**)의 쿼리(**802–807**)이고 그 필터가 **정확히** `processed_chain == True` · `status == "SUCCESS"` · `broadcast_at IS NULL` · `created_at < now() − 5초`다. **공유 상수가 없으므로 한쪽만 바꾸면 아무 테스트도 안 깨진 채 마커가 영원히 안 걷힌다.**~~ ✅ **필터 «네 조건»의 내용 자체는 여전히 참이다** — 바뀐 것은 「공유 상수가 없다」와 그 앵커 넷이다.
>
> 🔴 **`table_name`이 이 복구 경로 전체의 조인 키다.** `run_watcher._record_undelivered`(🆕⑱ **54**)가 `payload["table_name"]`을 꺼내고 **없으면 마커를 쓰지 않는다** → `internal_event_client`가 `DatabaseOutbox.table_name`으로 저장 → `chain_ingestion_worker.sweep_undelivered_broadcasts`(🆕⑱ **1357**)가 `source_tables`로 되읽어 그 테이블에 `batch_refresh_required`를 쏜다. **고리 하나만 끊어도 마커는 복구 불능이다.** ⚠️ 🆕⑱ **구 표기 `chain_ingestion_worker:838` 은 낡았다 — 그 되읽기는 위 함수 «안»이므로 라인이 아니라 «함수 이름»으로 찾아라.**
...
## 1.9 `server/runtime/system_reload.py` — 리로드가 **엔트리포인트 밖으로** 나간 자리 (`5db3b80c` 신설)

> 🟢 🆕⑮ **`5a73021a` 커밋 blob 전량 실측(2026-08-30).** **151줄.** import는 모듈 스코프에 `logging` · `sqlalchemy.orm.Session` · `database.models` · `database.database.engine` 넷뿐이고 **나머지는 전부 함수 안**이다.

**[§1.8](#18-servercolumn_filterpy--필터-dsl-번역기가-엔트리포인트-밖으로-나간-자리-신설)과 같은 규칙이 낳은 «세 번째» 파일이다.** 앞의 둘은 `utils/time_format.py`(체인 워커의 `from main import to_local_str`)와 `column_filter.py`(`enrichment_analysis`의 `main.get_column_filter_condition`)였다. 이번 위반자는 **`ledger_api/ontology_config_explorer_router.py`** 로, `delete_declaration`·`activate_draft` **안에서** `import main as app_main` 을 하고 `main.reload_system_configs` 를 불렀다.

| 심볼 | 무엇인가 (🆕⑮ `5a73021a` 실측) |
|---|---|
| `logger = logging.getLogger("Server")` | **`crud` 가 로그하는 그 이름 그대로.** `get_process_logger` 가 루트에 붙인 핸들러가 이 줄들을 같은 파일·같은 origin 으로 나른다 |
| 🔴 **`active_watcher = None`** | 🔴 **이사에서 «옮겨 갈 수 없었던» 유일한 것의 대체 자리다.** 원본 `global_watcher` 는 `main` 의 모듈 전역이라 여기 올 수 없었다. 이 프로세스가 임베디드 워처를 돌릴 때만 채워지고(대입은 [§1.1](#11-기동미들웨어공용-헬퍼) `startup_event` 행), **그 밖의 모든 프로세스에서는 `None` 인 채로 남아 아래 워크스페이스 동기화가 워처가 없던 시절과 «글자 그대로 같이» 건너뛰어진다** |
| **`reload_local_process_cache()`** | 웹 서버 프로세스의 캐시 무효화 **전부**. `models.refresh_dynamic_models(engine)`(싱글턴·ORM·신규 테이블 물리 CREATE — 이슈 #7) → `virtual_join_executor.reset_cache()` → `notation_norm.reset_cache()` → `ledger_trace.reset_walk_cache()` + **`ledger_trace.load_resolver_config(force_reload=True)`** → `config_authoring.skeleton.cache_clear()` → `sys.modules` 에서 `mappers.*`·`pipeline_plugin_*` 퇴거. 🔴 **첫 블록 말고는 전부 `try/except Exception: pass` 다** — 캐시 하나를 못 버리는 것이 리로드 전체를 죽이지 않는다. ⚠️ **그래서 「리로드했다」는 응답이 「전부 갱신됐다」를 뜻하지 않는다** |
| **`reload_system_configs(db: Session)`** | ① `reload_local_process_cache()` ② `active_watcher` 가 `None` 이 아니면 `sync_new_workspaces()`(실패는 `logger.error` 로 삼킨다) ③ `DatabaseOutbox` 에 **`event_type="SYSTEM_RELOAD"` 1행** + `db.commit()` ④ `NOTIFY outbox_event`(**bare `except:`** 로 삼킨다) ⑤ `{"status": "success", "message": …}`. 🔴 **시그니처가 `main` 쪽과 «다르다»** — 여기는 `db: Session` 을 «받고», `main.reload_system_configs` 는 `Depends(get_db)` 를 붙인 라우트 핸들러다. **같은 이름이지 같은 객체가 아니다** |

> 🔴 **`main.py` 에 남은 것과 나간 것**(`5db3b80c` diff 실측 — `main.py` **5,962 → 5,860줄**, −102):
> ```
> 남았다   @app.post("/admin/reload-configs")  같은 경로 · 같은 require_admin_token · 같은 응답
>          reload_local_process_cache = system_reload.reload_local_process_cache   ← 재export «대입»
>          global_watcher (모듈 전역, 그대로)
> 나갔다   두 함수의 «몸» — 커밋 메시지의 표현으로 byte-for-byte
> ```
>
> ✅ **재export는 유지된다** — 대입이므로 `main.reload_local_process_cache` 는 `system_reload.reload_local_process_cache` 와 **동일 객체**다. 이름으로 그것에 닿던 코드는 계속 해석되고, 이 지도의 종전 행들(§2 `refresh_dynamic_models` 호출처 · §5-C `virtual_join_config.reset_cache` 소비자 · §8 흐름 6)이 **그 이름을 쓰는 한 여전히 참이다.** 🔴 **틀려지는 것은 「`main.py` 를 열면 그 `def` 가 보인다」뿐이다.**
>
> 🔴 **왜 「지연 import 라서 안전」이 아니었나 — [§1.8](#18-servercolumn_filterpy--필터-dsl-번역기가-엔트리포인트-밖으로-나간-자리-신설)이 이미 적어 둔 것과 같은 문장이다.** `run_auto_update.py` 는 컬렉터마다 그 디렉터리를 `sys.path[0]` 에 꽂고 빼지 않으며 `parsers/directory_watcher.py` 는 테이블마다 `scripts/` 에 같은 짓을 한다. 둘 다 **사용자 소유 디렉터리**다. 첫 삽입 이후 그 프로세스에서 `main` 은 안정된 이름이 아니고, 사용자가 놓아둔 `main.py` 가 먼저 바인딩된다. **지연 import 는 `main` 을 안전하게 만든 것이 아니라 늦게 만들었을 뿐이다.**
>
> 🔴 **이 판정이 «강제되는 자리»**: `server/tests/test_entrypoint_import_isolation.py`(**184줄 · `def test_` 2개**). `test_no_server_module_imports_the_web_entrypoint` 가 `os.walk(server/)` 로 트리 전체를 훑어 `^import main\b` / `^from main import\b` 를 **줄 첫머리 앵커 정규식**으로 잡는다(면제는 `server/main.py` 하나, 제외 디렉터리는 `__pycache__`·`tests`·`ingestion_workspace`·`config_backup_20260801_reset`). **주석 줄은 건너뛴다** — 규칙을 «설명하는 산문»이 규칙에 걸리지 않도록 일부러 그렇게 돼 있다. 둘째 시험은 소스 규칙이 아니라 **프로세스 시험**이다(디코이 `main.py` 를 `sys.path` 맨 앞에 두고 서브프로세스에서 큐 술어를 돌린다). 🔴 **이 시험은 `system_reload` 를 이름으로 «알지 못한다»** — 실패 메시지가 「공용 심볼을 엔트리포인트 아닌 모듈로 옮기고 재export하라」는 처방만 적고, `utils/time_format.py`·`column_filter.py` 를 예로 든다. 즉 **이 파일은 규칙의 산물이지 규칙이 이름 대는 대상이 아니다.**
>
> **소비자 2곳(전건, `5a73021a`)**: `ledger_api/ontology_config_explorer_router.py` 의 **두 자리**(`import system_reload` 를 함수 «안»에서 하고 `reload_callback=lambda: system_reload.reload_system_configs(db)` 로 넘긴다) · `main.py`(모듈 스코프 `import system_reload` — 재export 대입 · 라우트 위임 · `startup_event` 의 `active_watcher` 대입). 🔴 **`reload_local_process_cache` 를 «`system_reload` 이름으로» 부르는 곳은 아직 없다** — 바깥 소비자는 전부 `reload_system_configs` 를 부르거나 `main` 의 재export를 쓴다.

---

## 2. `server/database/crud.py` — 레이어링 코어

> 🔴 **경로 주의 — 이 문서의 나머지가 이 파일을 `crud.py`라고만 부르는 곳이 있다.** 실제 경로는 **`server/database/crud.py`** 하나이고 **`server/database/crud.py`는 존재하지 않는다.** 위 표지 블록쿼트와 「핀 해시」 계열 산문에는 `main.py`·`models.py`와 나란히 짧은 이름으로 적힌 **역사 서술**이 남아 있다 — 그 문단들은 과거 패스의 기록이라 손대지 않았으나, **거기서 파일을 찾아 나설 때는 이 줄의 경로를 쓰라.** (파이썬 심볼 참조인 `crud.apply_batch_updates`·`crud.SOURCE_PRIORITY` 꼴은 파일명이 아니라 **모듈명**이므로 그대로다.)
...
| 🆕🆕🆕🆕 **`undeclared_column_drops() -> dict`** | **[`347de78` 신설, 공개] `{(table, col): count}` 스냅샷** — `(table, None)`은 그 테이블의 예산 초과분. **드롭은 체인 워커에서 일어나고 "이 배포가 아직 컬럼을 잃고 있나"는 다른 프로세스(웹서버)에서 물어야 하므로 공개다.** `chain_ingestion_worker`가 이 스냅샷의 다이제스트를 하트비트 `note=`로 실어 나른다(§4 `chain/ingestion_worker.py`) — 로그를 grep하지 않아도 `/health`가 이미 읽는 채널로 답이 나온다 |
| 🆕 **드롭 사유 어휘 «넷»** — `DROP_UNDECLARED_COLUMN = "undeclared_column"` :195 · `DROP_UNIQUE_VIOLATED = "unique_violated"` :197 · `DROP_SYSTEM_COLUMN = "system_column"` :200 · 🆕 **`DROP_ABSENT_NOT_WRITTEN = "absent_not_written"`** :205(전부 @`c904b389`) | `drop_report` 의 사유 낱말. `_record_dropped_cell(drop_stats, row_id, business_key_val, col_name, reason)` :240 이 적는다. 🆕 **[`fac454af` S-243, 판정 405] 넷째는 «실패가 아니라 부재»다** — 「비울 수 없는 쓴 이」가 보낸 빈 칸은 층을 안 만들고 «세어진다»(경고 아님). 이름이 없으면 「컬럼이 비어서 온 파일」과 「그 컬럼을 «안 부른» 파일」이 보고서에서 같은 모양이라 넷째 이름이 생겼다 |
| 🆕 **`CHAIN_SOURCE = "chain_ingestion"`** :675 · **`can_mean_emptied(source_name: str) -> bool`** :697 | **「이 쓴 이가 «내가 비웠다»를 뜻할 수 있나」의 «한 철자»** = `source_name in (USER_SOURCE, CHAIN_SOURCE)` :713 — 사람(서열 0)과, 맞춘 오른쪽 행이 «비어 있다»고 단언하는 체인(판정 f3c04dee) «둘뿐». 🔴 **양성 선택이지 블랙리스트가 아니다** — 파일 파서는 «인제스트한 파일명»으로 쓰므로 자동 소스 이름의 집합은 «열려» 있어 열거할 수 없다(`USER_SOURCE` 주석의 사유 그대로). 함수인 이유: 읽는 자가 «둘»이다 — 쓰기 관문(아래 `apply_row_update_internal` :3157)과 [§6 `scripts/count_absent_null_layers.py`](#6-기타-서버-모듈-한줄-요약) :86(어제까지 «저장된» NULL 층을 세는 쪽). 두 철자면 세는 수가 «다른 것의 수»가 된다(`478fcf32` 가 관문의 조건식을 이 함수로 «뽑았다»). `CHAIN_SOURCE` 는 `SOURCE_PRIORITY` :684 의 키이기도 하다(값 4 그대로) — ⚠️ 같은 문자열이 다른 모듈 «여섯»에 아직 리터럴로 있다(소스 주석, 이 라운드 밖) |
...
| `_update_row_business_key(row, key_col, update_item, row_cache)` | 비즈니스 키 갱신. 🆕 🔴 **[`818c9c0`] 빈 키 컬럼은 `''`를 절대 만들지 않고, 아무것도 쓰지 않는다 — 기존 키를 지우지도 않는다.** 두 철자는 교환 가능하지 않다: `''`는 **값**이라 한 테이블의 모든 무키 행이 **같은 정체성**을 갖고, D3 유일 인덱스 아래서 한 배치의 그런 행들이 **서로** 충돌한다. 그리고 `apply_batch_updates`의 회복은 그것을 못 고친다 — 회복이 「롤백하고 다시 읽어 승자의 행 위에 앉는다」인데 **충돌한 행 중 아무것도 커밋된 적이 없기** 때문이다 → 재생이 같은 충돌을 재현하고 배치가 **거절**된다(실측: 키 컬럼이 빈 5행 페이로드를 세 번 밀어 **매번 0행**). 🔴 **NULL로 지우는 것도 틀리다** — 맵 테이블의 키 컬럼은 **파생**이라 「비었다」는 「파일이 안 줬다」이지 「이 행은 키가 없다」가 아니다. 지우면 재푸시가 복합 키 재계산을 건너뛰고(행이 신규 아님·소스 무변경) 아무것도 키를 되돌려 놓지 않는다. 🆕 🔴 **[2026-09-08 · `96ca4d95`] 항목이 `business_key_val` 을 들고 오면 «그것이 이긴다» — key_col 유무와 무관하다.** 첫 갈래가 `not key_col` 로 막혀 있어서 plain 키 표는 그 갈래를 «못 타고» 페이로드에서 키를 «다시 유도»했고, 그 유도가 `str().strip()` 이라 float `1234.0` 을 «`'1234.0'`»으로 적었다 — 조회는 SDK·조립기의 `clean_str_value` 철자 «`'1234'`»로 하므로 «찾은 손잡이와 저장한 손잡이»가 갈렸다. 오류는 «안 난다**(문자열이 다르니 `uq_bk_` 도 안 부딪힌다) — 다음 푸시가 못 찾고 사본을 만든다. 🔴 **철자는 이제 «한 함수»다**: `_apply` 와 `get_row_by_business_key` 의 target_val 이 `clean_str_value`, SDK(`df_to_updates`)·`compose_business_key` 와 같은 것. `str().strip()` 이 남은 자리 «0** |
...
| `apply_row_update_internal(db, table_name, update_item, row_cache, sources_cache, overwrites_cache, transaction_id, logs_to_cache, cell_sources_to_upsert, cell_overwrites_to_upsert, cell_overwrites_to_delete, deleted_row_ids) -> (row, is_new, changed_cols)` | **[통합 코어]** 단일 행 업데이트 + 레이어링 재계산. 모든 쓰기 경로가 여기로 수렴. 🆕 **버전 게이트 `version_gate_verdict` 호출(값 루프보다 앞 — 거부되면 셀 레이어링에 아예 들어가지 않는다)** · 미선언 컬럼 경고 `_warn_undeclared_column_once` · 🆕 🔴 **[`fac454af`+`478fcf32` S-243, 판정 405] 부재는 층을 «안 만든다», 고의의 빈 칸만 만든다** — 값 루프 안 :3157(@`c904b389`): `if is_blank_value(clean_val) and not can_mean_emptied(update_item.source_name)` → `_record_dropped_cell(…, DROP_ABSENT_NOT_WRITTEN)` :3160 하고 `continue`. 종전엔 파일의 빈 칸이 NULL 로 캐스팅돼 «그 소스의 층»으로 저장됐고, 파일 소스가 체인을 앞서므로(2 또는 99 대 4) 조인이 «올바르게 쓴» 값을 «영원히» 가렸다. ⚠️ **기존 층은 «그대로 둔다», 지우지 않는다** — 「안 적었다」는 「어제 말한 것을 물린다」가 아니다(지우는 것은 사람·`withdraw_source`·`replace_map` 의 일). 읽기 쪽 `compute_priority_value` 는 «무변동» — 맨 위 층이 NULL 이어도 그것으로 답한다(고의의 빈 칸이 «이길 수» 있어야 한다). 이미 저장된 NULL 층은 §6 `count_absent_null_layers.py` 가 «센다»(S-243-b, 지우기는 S-243-c 미착지). 시험 `test_absence_makes_no_layer_only_a_deliberate_blank_does.py`(182줄 / `def test_` 9) · 🪦 🔴 **표기 정규화 파생은 이 함수에서 사라졌다 — 종전 지도가 등재하던 `notation_norm.apply_derivations` 호출은 없다.** `8d306a5`가 저장되는 파생 컬럼을 철회하면서 **쓰기 경로는 원본만 저장하고**(사유 주석 — `server/database/crud.py` **1969–1974** @`e943e46` 재확인 — 이 범위는 밀리지 않았다), 접기는 **질의 시점 SQL**로 옮겨갔다(`notation_norm.fold_notation_sql` — §5-E) |
...
| 🆕㉒ 🔴 **`purge_map_rows(db, table_model, table_name: str, row_ids)`** | **[2026-09-06 신설] 이 시스템에서 outbox 가 «볼 수 없던» 쓰기 «둘» 중 하나.** ⚰️ **[2026-09-08 S-74 정정] 「유일한」은 거짓이었다** — 같은 부류의 «둘째»가 `delete_rows_batch`(제품의 삭제 문)였고, 그것도 벌크 `.delete(synchronize_session=False)` 라 `before_flush` 가 못 봤다. 실측: 이 박스 아웃박스 전 역사에 CREATE 1,618 · RETROACTIVE_RUN 1 · **DELETE 0**. 이제 그쪽도 «같은 헬퍼»를 부른다. 다른 모든 쓰기는 SQLAlchemy `before_flush` 가 `session.deleted` 를 훑어 `stage_event` 에 닿는데, 대량 `.delete(synchronize_session=False)` 는 행을 세션에 «싣지 않으므로» 그 목록이 비고 `replace_map` purge 가 이벤트를 **하나도** 남기지 않았다 — 그래프도 미배달 스윕도 그 삭제를 끝내 몰랐다. 이제 세 삭제(대상 표 · `CellSource` · `CellOverwrite`) **앞**에서 `stage_collapsed_event(db, "DELETE", table_name, row_ids)` 를 «같은 flush 안에» 세운다. 빈 목록/`None` 은 «아무것도 세우지 않는다» (행 0을 지웠다는 통지는 통지가 아니다). 반환은 받은 `row_ids`.<br>🔴 **purge 자리가 «둘»이었다** — 차집합 갈래(`removed_row_ids`)와 비-차집합 갈래(`purged_row_ids`)가 «각자» 같은 세 삭제를 손으로 철자하고 있었다. 읽던 한쪽만 고치면 다른 쪽은 그대로 안 보인다 — 둘 다 이 헬퍼를 부르는 것이 「한쪽에만 이벤트를 붙이고 잊는 것」을 불가능하게 만드는 부분이다.<br>⚠️ **이것은 삭제를 outbox 에 «올릴» 뿐, 감사 이력을 쓰지 «않는다»** — 다른 두 가지고 닫힌 것은 앞의 하나다(docstring 이 그 문장을 들고 있고, 시험이 그 문장의 존재를 단언한다). ⚠️ **DELETE 이벤트는 체인을 깨우지 못한다** — 워커가 트리거를 고르는 세 자리 전부가 `CREATE`/`EDIT` 만 받는다(추측이 아니라 재고 넣은 문장이다). 시험: **`server/tests/test_a_map_purge_is_visible_to_the_outbox.py`** |
| 🆕㉔ **`refuse_virtual_join_duplicates(db, table_name, ...)`** :3978(@`b1db471a` — 구 표기 :3891) | **행 «하나»의 중복 조인 키가 표 전체의 체인을 멈추지 않게 «그 행만» 이름 대어 거절한다**(S-174 `a50d7328`). 📌 부류는 「거절이 «비싸면» 거절이 사고가 된다」 — 같은 밤의 재전개 이분(사건 1,000 → 21)과 같은 설계다.<br>🆕 **[`3aab7173` S-247] 그 거절 «두 줄»(배치 안 중복 :4044 · 저장된 행과 충돌 :4108)이 `operator_line.line("VirtualJoinUnique", rule_name, …, operator_line.fold_the_data(table_name, columns), <표본>)` 이 됐다** — 「다음: 그 키의 행들을 하나로 합치십시오」가 줄 안에 실린다. 그리고 🔴 **같은 낱말(「중복」)의 «정반대» 수리**가 `apply_batch_updates` :4294 의 BK 충돌 «미해결» 줄 :4386 — `operator_line.line("BKConflict", table_name, …, operator_line.widen_the_key("table_config 의 '<표>' 의 composite_key_source", "두 행을 가르는 컬럼"))`: 여기의 중복은 표의 «자기 신원»이 두 행을 못 가르는 것이라 데이터를 접으면 «사실이 사라진다» — 지난번 통한 수리를 집는 운영자가 «거꾸로» 간다(§4-ter-h `operator_line.py`). 🆕 **[`ddd5b3ba` S-245] `_stored_join_key_owners(db, table_name, columns, folds, …)` :3929 가 `vjc.index_key_expression(column, fold, table_name)` :3953 으로 «표»를 넘긴다** — 저장된 절반의 프로브가 인덱스와 «타입까지» 같은 식으로 묻는다(§5-C) |
...
- **[P2] 체크포인트 재개 + 해시 dedup**(`f78ab0a`) — 파일 전체 sha256 시그니처(`sha256:<size>:<digest>`)를 계산해 ① 동일 시그니처 `DONE`이면 skip ② 미완이면 오프셋 재개. 저장소는 신규 테이블 `file_ingestion_checkpoints`([`ingestion/checkpoint.py` §5](#5-소형-서버-모듈)). **오프셋 갱신은 청크 upsert와 같은 트랜잭션** — "커밋된 행 수 == 기록된 오프셋"이 원자적으로 성립. 재개는 시그니처+`total_rows`+`source_kind`+오프셋 범위가 **전부** 일치할 때만. heavy/normal·스윕·관리자 재시도 4경로 동일 동작.
...
- ⚰️ **[M3 은퇴 · 2026-09-07] 맵 메타 자동 등록 배선은 «없다»**(S-38). 아래는 `b697d34`~`S-38` 사이의 기록이다 — `import map_meta_registrar`. 파일 1건 = 하나의 작업 단위이므로 컬렉터를 `_send_to_upsert` 진입부에서 **파일당 1회 생성**(~1746, 토글 스냅샷이 D1 config 스냅샷과 같은 경계를 공유한다), 청크마다 정규화된 `items`로 bbox만 누적(~1784 — DB 작업 0), **데이터가 커밋된 뒤** 별도 세션으로 부재분만 등록(~1846). 실패는 인제션을 절대 실패시키지 않는다(이미 들어간 데이터). 모듈 계약은 [§5 `map_meta_registrar.py`](#5-소형-서버-모듈).
- 통지 로그 상한 `MAX_NOTIFY_CREATED_LOGS`는 `event_constants.py` 공용 상수 import(~51).
- 테스트: `tests/test_workspace_config_deprecation.py`(21개) · `tests/test_heavy_lane.py`(27개, `hvy_test_*`) · `tests/test_ingestion_checkpoint.py` · **`tests/test_nested_dir_ingestion.py`(22개, `600b49d` 신설 — 구 ~~`tests/test_flatten_nested_dirs.py`~~(15개)를 **삭제하고 교체**했다)** · **`tests/test_filename_rules_declaration.py`(`600b49d` 신설 — `advanced_ingester` 선언 검증·병합 서열)** · **`tests/test_map_meta_registrar.py`(⚰️ 12 → **4** @S-38 — 은퇴 관문 둘 + 남은 둘)** · 🆕⑤ **`tests/test_sweep_tier1_hoist.py`(신설 693줄, `grep -c "def test_" = 18` @`831ab68`)** · 🆕⑤ **`tests/test_ingestion_ledger_tier1.py`(신설 428줄, 17개)**.
...
## 4. `server/chain/ingestion_worker.py` — 체인 워커
...
> 🆕 **[2026-09-15 실측 · `dc877746`] 이 파일은 **3,790줄**이다** — `474f1aa9` 의 3,669 · 아래 1,886 은 09-05 값이고 S-214 행의 3,166 은 09-12 값이다. `load_chain_rules` 는 **:698** 에 있고 그 «안»의 자리는 아래 표의 그 행에 적었다. 라인이 아니라 «이름»으로 찾아라.
>
> 🔴 🆕⑳ **[2026-09-05 실측 · `328a5c20`] 이 파일은 **1,886줄**이다 — 아래 두 블록의 값(1,294 · 1,198)은 전부 낡았고, 그 사이 구간을 이 패스가 열지 않았다. 라인이 아니라 «이름»으로 찾아라.**
>
> 🆕 **[`2504fbde` 판정 402 · `dc877746` 실측] `_validate_chain_cascade_graph(rules) -> list` :1005 — «던지지 않는다».** 옵트인(`allow_chain_trigger`) 엣지의 고리를 «자취 문자열 목록»으로 «돌려준다»(`"allow_chain_trigger cycle: a -> b -> a"`), 발견마다 `rule_order.say_cycle_once(logger, found, event_constants.max_chain_depth(_RULES_DOCUMENT))` :1057 로 «프로세스당 한 번» 말한다. ⚰️ 종전엔 `ValueError` 를 던져 «규칙 문서 전체»와 저장 라우트를 같이 죽였다 — `dt_log → dt_inventory`(맵퍼) → `dt_log`(조인)는 «의도된» 고리이고 유한하게 만드는 것은 드레인의 `max_chain_depth` 다. 읽는 쪽 «셋»: `chain/graph.py` :380 `cycles = list(worker._validate_chain_cascade_graph(chain_rules) or ())`(종전 `except` 로 잡던 것을 «값»으로 — 검증기가 던지기를 그만뒀는데 그 자리가 계속 `except` 였다면 화면이 «조용히 비었을» 것이다) · `ledger/admin.py` :723(저장 관문 — «확장된 집합» 위에서 부르고 결과를 «거절에 안 쓴다», 구 `chain_cycle` 거절 «없음») · `load_chain_rules` ⑦.
>
> 🔴 🆕⑳ **`_validate_chain_cascade_graph(rules)` 가 이제 규칙 하나에 «엣지 둘»을 긋는다** (`2742446f` 계열, 소스 주석이 실측 사례를 적는다).
> ```
> 규칙의 «켜짐» 판정   rule.get("enabled", True)      🔴 `is_active` 가 «아니다». 이 파일의 다른 다섯 자리도 전부 `enabled`
> 엣지 ①              rule["target_table"]           맵퍼의 행이 가는 곳
> 엣지 ②              map_meta_registrar.META_TABLE   🆕 `allow_map_metadata_upsert` 가 켜져 있을 때만
> ```
> 🔴 **②가 없던 것이 결함이었다 — 그 쓰기도 «자기 체인 이벤트를 낸다».** 즉 그 엣지는 «도는 시스템»에 있었고 이 그래프에만 없었다. 소스가 2026-09-04 실측을 적는다: 규칙 #3(`dt_inventory → dt_map`)이 실행마다 메타 5행을 쓰고 → 그것이 규칙 #2(`wafer_map_metadata → dt_inventory`)를 깨우고 → #3 이 `allow_chain_trigger` 로 `dt_inventory` 를 먹는다. **순환이고, 살아 있었고, 이 검증기가 통과시켰다.** ⚠️ **메타 표 이름은 «규칙»이 아니라 «등록기»에서 온다** — 규칙 #3 은 메타 표를 아예 선언하지 않고, `metadata_target_table` 은 빌려 쓸 수 없다(`dt_metadata_to_dt_inventory` 에서 그 키는 맵퍼의 **소스**를 가리킨다).
>
...
> 함수 **안**의 호출부 — ~~`MapMetaCollector(`~~(⚰️ S-38 에서 사라졌다) · `AutoConfirmCollector(` **529** · broadcast 구성부 **594** · `startup_lines("Chain Worker")` **984** · `heartbeat.beat("chain")` **1027** · `CONTROL_EVENT_TYPES` 멤버십 **1122**.
>
...
| `load_chain_rules()` | chain_rules 설정 로드(+enrichment 룰 병합). **`chain_replay.load_rules`가 이 함수를 그대로 부른다** — 재생이 라이브 룰 집합을 정확히 그대로 보게 하려는 것 | ~292<br>🆕㉛ **[S-201 `789e9ee2`·`174346f2`, 판정 316] `read_rules_document(path=None)` — 「그 파일을 읽는」 «무상태 독자» 하나.** `/admin/chain/rules`(`main.py`)가 자기 `open` 을 버리고 이것을 지난다. 🔴 **리스트 형태 파일은 «로더와 같은 방식»으로 거절한다** — 새 독자가 자기 거절을 쓰면 운영자가 같은 실수에 «두 문장»을 받는다<br>🆕 **[2026-09-15 실측 `474f1aa9` · `dc29056a` S-239 · 🆕 `dc877746` 재측정] `def load_chain_rules()` :698(구 표기 :682) — 그 «안»의 순서, 이름으로:** ① `read_rules_document()` → ② **통합 문법 번역 블록** — 🆕 **[`00da7e91` S-244] 규칙마다 `rule_shape.expand_declaration(rule, _catalogue.TABLE_CONFIG)` :740 «하나»를 부른다** → `(stood, refusal, notes)`: `notes` 는 `warning` 으로 «이름 대고» 찍고, `refusal` 이 있으면 `error` 후 `continue`(⛔ 규칙 «하나»만 떨어진다, 파일이 아니라), `stood` 를 `translated` 에 «편다». 🔴 **OFF(`is_switched_off` — 규칙 «하나도» 안 서고 DB 무접촉, 거절 목록에도 «안 센다») · `decide` 규칙 «둘» · `join` 의 `companion_rules` · 옛 평면 규칙 «그대로 통과» — 그 순서는 이제 «전부 §4-ter-b `expand_declaration` :338(@`b1db471a` — 구 표기 :328) 안»에 있다**(종전 이 함수 «안»의 :722~:762 블록이 통째로 그리 «옮겨졌다» — 저장 관문 `ledger/admin.save_chain_rule_raw` 가 «같은 함수»를 부르므로 「이 선언이 무엇을 세우나」의 답이 «문에 무관»하다). ⚠️ **블록 전체가 `try` 안이다** — 새 문법을 «못 읽으면» 옛 모양 그대로 간다 → ③ `chain_bindings.rule_refusals` 문법 심판(파일 규칙만) → ④ **합성** `builtins.synthesize_chain_rules` — 🔴 **`ASSY_CHAIN_SYNTHESIZE` 스위치는 «이 블록만» 끈다**(주어: 운영자가 «안 쓴» 파생 규칙). 통합 선언에는 «닿지 않는다» — 그쪽 스위치는 위 ②의 `enabled: false` 다(판정 399, ③′) → ⑤ **`rule_order.order_rules(rules, on_cycle=…)` :890** — 🆕 **[`2504fbde` 판정 402]** `on_cycle=lambda trail: rule_order.say_cycle_once(logger, trail, event_constants.max_chain_depth(_RULES_DOCUMENT))` :891. 고리는 «프로세스당 한 번» 말해지고 순서는 «선언 순»; ⚰️ `RuleCycleRefused` 는 «없다»(§4-ter-g · §0 ㉘) → ⑥ `rule_census` 사진 + 지난 적재와의 diff → ⑦ `_validate_chain_cascade_graph` · `_report_unwatchable_trigger_columns` · `activity.registry.seed_rules` |
| `_mapper_accepts_rule(mapper_func) -> bool` | 맵퍼가 rule 인자를 받는지 시그니처 검사 | ~321 |
| ⚰️ ~~`execute_custom_mapper(…)` 가 이 파일에~~ → 🆕㈣ **[S-214 `236b24e9`] `server/chain/mapper_call.py` :87**(@`b1db471a` — 구 표기 :88) | mappers/ 동적 로드·실행 — 이제 «자기 집»에 산다 | 새 모듈 **253**줄(등재 때) · 워커 3,366 → **3,166** |
| 🆕㈣ **`server/chain/mapper_call.py`** (🆕 **248줄** @`b1db471a` — 구 표기 253, `236b24e9`) | **맵퍼를 «부르는 자리» — 워커와 재생이 «같이» 읽는다**(판정 370). `execute_custom_mapper` :87 · `_mapper_accepts_rule` :47 · `without_missing` :58 · `_payload_row_count` :62 · `_result_row_count` :68 · `_is_missing_scalar` :190 · `_missing_as_none` :215 · `MAPPER_LOG_TAG` :45(전부 @`b1db471a` — 구 표기 :88/:48/:59/:63/:69/:195/:220/:46). 🆕 **[`38b5d8e1` S-246] 등록이 «이 문 안»에서 나갔다** — `activity.registry.start/record_outcome/finish` 를 여기서 «손으로» 하던 것을 **`with activity.running(rule_name, who, target_table, rows_in) as run:`** :130 이 대신하고, 이 함수는 `run.produced(rows_out)` 만 말한다. `import event_constants` 가 이 파일에서 «사라졌다»(결과 어휘는 `activity.running` 이 든다 — §5-I). 남은 것은 이 문의 «줄»(`END`/`RAISED` 로그)뿐 — `builtin:` 종류는 맵퍼가 아니라 «자기 문장»을 쓴다(§5-C `builtins.run_builtin`). 🔴 **전제가 바뀜다** — 「한 실행기가 둘로 쪼개졌다」가 아니라 **«공용 프리미티브가 한 호출자의 집에 살았다»**였다. 실행기는 저장소에 «한 벌»이고 그 docstring 이 스스로 「모든 커스텀 맵퍼가 지나는 유일한 자리」라 적었는데, 그 집이 워커여서 재생이 맵퍼를 돌리려면 «워커를 import» 해야 했다. 🔵 S-211 ① 의 `withdraw_source` 와 **같은 기제, 반대 방향**이고 처방도 같다 — 둘보다 «아래»로. ⚠️ **트랜잭션은 안 옴긴다** — 재생의 «청크 커밋»과 워커의 «그룹 트랜잭션»은 그 «호출자»의 성질이고, 옴긴 것은 「맵퍼를 어떻게 부르나」뿐이다. ⚠️ 로그 파일도 안 갈라졌다(`LOG_FILENAME = "chain_worker.log"` :34) — 여기서 둘로 나누면 한 런의 줄이 두 파일로 쪼개진다. 🔴 남은 «재생 → 워커» 엣지는 `load_chain_rules` «하나»다(판정 358) | — |
...
| 🆕 **`_resolvable_mapper(name)`** | 「이 프로세스가 «실제로» 돌릴 수 있는 맵퍼」 = 등록된 것 **또는** `builtin:` 종류. 🔴 **[`00da7e91`] 저장 관문 `ledger/admin.save_chain_rule_raw` :702 가 `MAPPER_REGISTRY.get` 대신 «이것»을 `rule_refusals(mapper_resolvable=…)` 에 넘긴다** — 종전엔 `builtin:join_into` 를 적은 번역 규칙이 «여기서 거절되고 저기서 돌았다»(한 제품의 「돌 수 있나」에 답이 둘) | :641 |
...
| 🆕 **후속 랩 «한 원인 한 번»** — `_FOLLOWUP_SERVED = OrderedDict()` :2634 · `MAX_REMEMBERED_CAUSES = 512` :2635 · **`followup_already_served(transaction_id, table, rule_name, row_ids) -> list`** :2638 · `forget_followups()` :2669(전부 @`b1db471a` — 구 표기 :2612/:2613/:2616/:2647) | **[`b1e89e36` S-249 ⓒ]** 이 규칙이 이 «원인(origin tx)»에 대해 «아직 못 받은» 행만 돌려주고 나머지를 기억한다 — 키는 `(tx) -> {(table, rule): frozenset(row_ids)}`, 상한 512 원인(가장 오랜 것부터 «퇴거»). ⚠️ **tx 가 없으면 기억 «안 한다»** — 소급·백필은 tx 없이 줄을 서고, 그것을 한 키에 묶으면 둘째 백필이 첫째의 «반복»으로 보인다. ⚠️ «다른 원인»으로 같은 행을 다시 보는 것은 막지 않는다(그것이 정상이고 키가 «행이 아니라 tx»인 이유). `forget_followups` 호출자: 시험뿐 | :2634~ (@`b1db471a` — 구 표기 :2612~:2650) |
| 🆕 **`log_followup_folded(logger_, rule_name, table, rows_in, written, woke_by, hop, max_hop, refusal=None)`** · `_FOLLOWUP_SAID` :2676 · `FOLLOWUP_LOG_EVERY = 500` :2677 · `forget_followup_lines()` :2711(전부 @`b1db471a` — 구 표기 :2654/:2655/:2689) | **[`6edf4db4` S-249 ⓔ-2] 후속 랩의 «한 줄»에 «무엇이 깨웠나»(`woke_by=<table>#<tx>`)와 홉(`hop=n/max`)이 실린다.** 🔴 **쓴 것이 «0»이고 거절도 없으면 DEBUG** — 할 일 없는 후속은 매 랩의 «보통»이라 INFO 면 일한 랩을 가린다. INFO 는 «첫 줄 + 500 마다 한 줄»(`(xN)` 접미), `(rule, table)` 키 2,000 을 넘으면 «비운다»(누수보다 재계수). `log_failure_folded` 와 «같은 손» | :2680 (@`b1db471a` — 구 표기 :2658) |
| 🆕 **`_run_builtin_followups(db, done)`** — 🔴 **랩이 «홉»이다** | **[`b1e89e36`·`6edf4db4` S-249 ⓒ·ⓔ-1]** `done` 에서 `chain_depth` 를 읽어 **`request_chain_depth.set((incoming or 0) + 1)`** :2752 로 감싼다(`finally` 로 reset) — 종전엔 `request_chain_depth` 가 «그룹 스텝 한 자리»에서만 세팅됐고 이 드레인은 «자기 스레드»라 여기서 쓴 것은 홉이 «없어» `max_chain_depth` 를 «영원히» 못 만났다(한 천장, 두 답). 규칙마다 ① `followup_already_served(done["transaction_id"], table, rule name, row_ids)` :2761 로 «신선한» 행만 — 없으면 «건너뜀» 한 줄 ② **`with outbox_mode(event_constants.OUTBOX_MODE_COLLAPSED)`** :2773 안에서 `builtins.run_builtin(kind, db, rule, row_ids=fresh, done=done)` :2774(🆕 S-246 부터 이 호출이 «큐 화면에 등록»된다 — §5-C) — 행별 이벤트 1,000 이 outbox 1,000 · 큐 1,000 · 랩 1,000 · 줄 1,000 이 되던 것(소유자 「한 행당 로그 하나」)을 그룹 경로와 «같은 컨텍스트 매니저»로 접는다 ③ `log_followup_folded(...)` :2779. ⚠️ 여전히 «격리» — 실패는 원장 후속을 «못 깨뜨린다». 시험 `test_the_follow_up_lap_is_a_hop_and_serves_one_cause_once.py`(280줄 / `def test_` 16) | :2715 (@`b1db471a` — 구 표기 :2693; 안의 :2730/:2739/:2751/:2752/:2757 → :2752/:2761/:2773/:2774/:2779) |
| 🆕㉓ **`ROWS_NOT_VISIBLE = "rows_not_visible"`** | **「이 그룹의 행을 아직 읽어올 수 없다」의 «한 철자».** 내는 쪽과 알아보는 쪽이 «같은 이름»을 읽어 갈라질 수 없다 (S-160, 판정 267) | ~360 |
| 🆕㉓ **`max_rows_not_visible_defers()`** / `DEFAULT_…= 30` / `_ROWS_NOT_VISIBLE_DEFERS` | 미룸 상한(스윕 ≤2 s → «약 1분») · tx 별 카운터. 🔴 **미룸은 «시도»가 아니다** — `retry_count` 와 별도다. 시도로 세면 `max_group_attempts`(기본 1)에서 첫 번에 격리되고 격리는 «행 단위 재확장»이다 | ~364~394 |
| 🆕㉓ 그룹 줄 «세 층» | `MACHINERY`(stage) · `INSIDE THE VIEW`(phase) · `INSIDE THE WRITE`(write_step) — 층마다 `unnamed` 잔여를 찍어 «합을 검산»할 수 있게. 배치 발사는 «자기 줄»(`batch: broadcast dispatch …`) | ~1473 · ~1772 |
| `warmup_worker(rules, db_session_factory=None)` | 콜드스타트 제거 — 맵퍼·커넥션 프리로드. **[`23a346d`] 3단계의 HTTP 프리워밍이 `internal_event_client.internal_event_session()` 호출로 교체**(같은 목적: 첫 통지에서 `requests` import 비용 제거).<br>🆕 **[`8cab58da` S-240] 0-bis — «선언된 유일 키»를 «적재 시점»에 세운다**: `db_session_factory` 가 있으면 `builtins.ensure_declared_unique_keys(_index_db, rules)` :2006 을 «자기 세션»으로 부르고 `finally` 로 닫는다 — 건너뛴 것(`skipped`)은 이름 대어 INFO, 던지면 `error` 한 줄(「체인은 계속」). 🔴 **왜 여기인가** — 「규칙을 방금 (다시) 읽었다」와 「DB 가 있다」가 만나는 자리가 이것뿐이다: `load_chain_rules()` 는 세션이 없고 읽기 경로는 §0-ter ① 이 새 SQL 을 금한다. 부팅 «과» 리로드마다 돈다(선언이 바뀔 수 있는 때). ⛔ 인덱스를 못 세워도 워커는 «뜬다» — 조인의 행 단위 그물(§4-ter-c)이 팬아웃을 여전히 거절한다. 세우는 셋은 §5-C `builtins.py` | ~~~615~~ 🆕 **:1965** @`b1db471a` |
...
## 4-bis. 🆕㉔ 운영자가 «보는» 것 — 09-11 신설 모듈 넷

> 🔴 넷 다 같은 제약에서 나왔다: **소유자가 운영 DB 에 질의를 못 낸다.** 그래서 「운영에서 재 보라」가
> 답이 될 수 없고, 제품과 «명령»이 값을 말해 줘야 한다(판정 276).

### `server/runtime/loops.py` (**148줄**, S-176) — 고리 «아홉»이 마지막으로 무엇을 했나
```
LOOPS = (web · watcher · chain · outbox_purge · listen · ledger_followup · ledger_census · scheduler · postgres)
        = 보드 번호 ①~⑦ 에 ③ 을 «셋»으로 가른 것(③ · ③-a · ③-b)
```
🔴 **`/health` 와 «다른 일»이다(판정 282)**: `/health` 는 «판정»한다(ok/degraded/unhealthy + HTTP 상태,
감시가 걸 수 있는 «한 수»가 필요하니까). 이쪽은 «값»을 답한다 — 「체인이 도나」를 묻는 사람에게 필요한 것은
남이 계산한 결론이 아니라 «lap 과 큐»다. 한쪽이 다른 쪽을 대신 정하면, 한 시간 안 움직인 큐 옆에 초록 불이 선다.
🔴 **아무것도 «재지 않는다»** — 모든 수는 그 고리가 «이미» 계산한 것이다(lap 은 `heartbeat.record_lap`,
outbox 깊이는 `/admin/chain/queue` 가 이미 돌리는 질의, pace 는 `pacing.json`). 이 파일이 더하는 것은 «조립»뿐이다.
⚠️ **없는 칸은 «생략»이지 0 이 아니다** — 「lap 을 보고한 적 없다」와 「마지막 lap 이 0 초」는 다른 사실이다.

### `server/chain/graph.py` (🆕 **407줄** @`dc877746` — 구 표기 373, S-178) — 한 흐름, 네 선언을 «한 그림»으로

🆕 **[`2504fbde` 판정 402] `chain_graph(db)` :325 안 `cycles = list(worker._validate_chain_cascade_graph(chain_rules) or ())` :380** — 종전 `try/except` 로 «던진 것»을 줍던 자리가 «값»을 읽는다. 화면은 «똑같이» 고리를 보인다(「운영자가 다른 데서 못 보는 한 가지」) — 검증기가 던지기를 그만뒀는데 이 자리가 계속 `except` 였다면 그 화면은 «조용히 비었을» 것이다.
```
소유자   「chain 이 너무 거미줄 같아」 — 거미줄은 «코드»가 아니라, 한 흐름이 네 파일
        (chain_rules · enrichment_rules · virtual_join_rules · ledger_config)에 나눠 적혀 있다는 «사실»이다
🔴 로직 «0»  아무것도 판단하지 않는다. 선언에서 «읽기만» 하고 라이브 파일은 안 쓴다
⚰️ **정정의 정정(09-11 22:0x, 판정 293-b · S-179 ① `20e84d74`)** — 아래 「읽는 쪽이 둘」은
   **21:3x 에 참이었고 지금은 아니다.** 둘이던 «이유»(합성 규칙을 섞으면 두 번 그려짐)가 사라졌다:
   인리치가 «체인 규칙의 종류»가 되면서 합성기가 하나가 되고, 규칙이 `origin` 칸을 든다
   (`chain/graph.py:89` `rule.get("origin") or "file"`). 그래서 **읽는 쪽은 «하나»이고 출처는 «칸»이 말한다.**
   🔵 라이브 실측(21:45, PID 37708): 엣지 40 · mapper 20(파일 12 + 합성 8) · **중복 «0»** · 고리 0
   📌 그러므로 이 절은 «하루에 세 판»이다 — 독스트링을 옮긴 것 → 재서 둘 → 이유가 사라져 하나.
      가운데 판이 틀린 것이 아니라 «그때의 참»이었고, 그 사실이 이 줄에 남아야 다음 사람이 안 헷갈린다
<details><summary>⚰️ 21:3x 의 판 (기록으로만)</summary>
```
`_chain_rule_file()` :46   `chain_rules.json` 이 «자기 파일에 든» 규칙만 — 「운영자가 «적은» 것」
                           🔴 `load_chain_rules()` 는 인리치에서 «합성한» 규칙을 «섞어» 돌려주므로,
                              그것으로 그래프를 지으면 인리치 규칙이 «두 번» 그려지고 파일별 수가 안 맞는다
`worker.load_chain_rules()` :327  워커가 «실제로» 도는 목록 — 합성 규칙은 `wakes` 로 «보인다»
=> 정직한 가름이다: «파일»은 적힌 것을, «wakes»는 도는 것을 말한다
⚠️ 판정 293: 게이트 ⓒ 가 「그래프가 오늘 도는 규칙을 «다» 그린다」를 전제했고 그 전제가 거짓이었다 —
   끝점 집합이 달라지는 것은 «회귀가 아니라 수리»다
```
📌 **부류(제 오류): 독스트링을 «동작»의 증거로 옮겼다.** 주석은 «의도»의 증거다 — 그 문장이 오늘도
   참인지는 코드에서 재야 했고, 재 보니 «둘»이었다.
</details>
```

### `server/scripts/tune_layer_tables.py` (**378줄**, S-170) · `server/scripts/outbox_triage.py` (**200줄**, S-172)
```
tune_layer_tables   「배큠 계속 나오는데 어케해」의 답. 🔴 **아무것도 «정하지 않는다»**(소유자 정정으로
                    권고 설정이 빠졌다) — 상태를 읽어 보여 주고 `--apply` 는 사람이 고른 값만 쓴다
outbox_triage       터진 재전개를 치운다. per-row 사건을 «건너뛰고» 접힌 채 다시 쏜다.
                    🔵 근거가 수다: 운영이 per-row 사건 «약 66 만» — 그룹 배관 ~1.7 s 로 «며칠»인데,
                    접으면 «약 2 시간»이다
```
...
fold_time_value(v, zone)  «값을 답한다». 🔴 오프셋을 «이미 든» 값은 그대로 돌려준다 —
                          소스가 말한 것이 선언을 이긴다
note_naive_time(...)      셈 · `naive_time_counts()` · `naive_time_note()` — 청크 줄과 기동 줄이 읽는다
⚰️ `LOCAL_TIMEZONE` «사라졌다» — 남은 세 히트는 「이제 안 쓴다」는 «주석»과 그 시험이다
   (`schemas.py:41,:288` 은 「더 이상 `astimezone(LOCAL_TIMEZONE)` 이 아니다」를 적어 둔 자리)
```
🔵 **술어와 «폴드»가 다른 함수인 이유**: 폴드는 값을 답하고 모르면 `None` 을 낸다 — 거절해야 하는 자리가
...
### 4-ter-b. `server/chain/rule_shape.py` (🆕 **421줄** @`c904b389` — 384 @`b1db471a`, 374 @`dc877746`, 325 @`474f1aa9`, 구 표기 271 · `b7b5010f` 신설 때 109, S-234 2단계)

| | |
|---|---|
| 무엇 | 세 문법을 담는 «내부 규칙» 하나 + 어댑터 둘(`from_chain_rule` :36/`as_chain_rule` :95 · `from_join_rule` :146/`as_join_rule` :165) + 새 문법의 입출력(`to_declaration`/`from_declaration` :184·:204)(전부 @`c904b389` — 🔴 `df824a9a` 가 :60~:93 에 `declared_kind` 블록을 «끼워» :57 뒤의 앵커가 «+37» 밀렸다, 전건 따로 잼; 구 표기 @`b1db471a` :36/:58/:109/:128 · :147/:167) |
| 🆕 **`DECLARED_KINDS = ("join", "decide", "mapper")`** :60 · **`declared_kind(rule: dict) -> str`** :63(@`c904b389`, `df824a9a` S-250) | **적재된 규칙 → 그 «선언»이 쓸 낱말 하나.** 🔴 **배관의 낱말이 아니라 운영자의 낱말이다** — 화면이 `builtin:join_into` 나 `enrichment_dedup:` 을 내밀면 규칙을 고르려고 제품 내부를 알아야 한다(COLLECT 드롭다운이 `point`·`collection` 을 내밀던 것과 같은 결함, 2026-08-27). 판별: `mapper ∈ {join_into.JOIN_INTO_MAPPER, virtual_join.config.JOIN_MAPPER}` → `join` · `mapper == enrichment_config.AUTO_CONFIRM_MAPPER` 또는 이름이 `DEDUP_PREFIX`/`AUTO_CONFIRM_PREFIX` 로 시작 → `decide`(⚠️ `decide` 선언의 «두 반쪽»이 «둘 다» `decide` 라 답한다 — 한 선언에서 나왔으니) · 그 밖 → `mapper`. ⚠️ **`builtins.synthesized_kind_counts` 와 «다른 질문»이다** — 그쪽은 「제품이 origin 별로 «몇 개» 합성했나」(부팅 줄), 이쪽은 「이 규칙 «뒤의 선언»이 무엇이라 말하나」. 같은 접두, 다른 주어 — 접으면 답 하나가 질문 둘을 섬긴다(소스 주석). 읽는 자: §5-A `replay.replayable_rules_for` :218 «하나»(+시험). 지연 import 셋(`join_into` · `enrichment.config` · `virtual_join.config`) — 함수 안, 순환 회피 |
| 🆕 **`key` 칸이 «번역을 살아남는다»** (`8cab58da` S-240) | `from_declaration` :204 이 최상위 `key` 를 `"key": dict(raw.get("key") or {})` :225 로 나르고, `to_declaration` :184 이 `("on", "derive", "into", "key", "limits")` 순으로 되돌리며, `as_chain_rule` :95 이 join 종류일 때 `out["key"] = dict(internal["key"])` :136 로 «규칙 옆»에 싣는다 — `params` «안»이 아니다(맵퍼 인자가 아니라 «껍데기가 적재 시점에 읽는 칸»이고 `join_into` 는 그것을 «못 본다»). 🔴 종전엔 이 칸이 «바닥에 떨어졌다» — 아무도 나르지 않아 아무도 읽을 수 없었고, 그래서 §4-ter-c 의 옛 「읽는 자가 없다」줄이 참이었다. 읽는 자는 §5-C `builtins.declared_unique_targets` :181 |
| 🆕 **join 종류의 동반자** (S-237, 이 범위 «전»부터 있었으나 미등재) | `companion_rules(internal)` :238 — 조인 선언 «하나»가 규칙 «둘»(왼쪽 트리거 + `:reference` 접미의 오른쪽 트리거, `REFERENCE_SUFFIX` :235)이 되는 자리 · `class JoinTriggerConflict(ValueError)` :271 · `join_trigger_columns` :278 · `refuse_join_trigger_conflict` :287 · `unknown_join_cells` :307(:317 이 `join_into.unknown_cells` 를 부른다)(전부 @`c904b389` — 구 표기 @`b1db471a` :201/:198/:234/:241/:250/:270/:280) |
| 🆕 **`decide` 종류** (`dc29056a` S-239 · 판정 399~401) | `DECIDE_CELLS` :323(@`c904b389` — 이 행의 앵커 구 표기 @`b1db471a` :286/:290/:293/:317/:326) = `key · fields · list_columns · aggregations · reference_views · auto_confirm · alignment`(🔴 `auto_confirm_declared` 는 칸이 «아니다» — 판정 401: 「적었나」는 키의 «존재»에서 유도된다, `enabled_written` 과 같은 부류) · `_DECIDE_TO_ENRICHMENT` :327(`key→decision_key` · `fields→target_fields` — 새 문법 칸을 인리치 어휘 철자로) · **`decide_rules(internal, known_tables=None) -> (rules, refusal)`** :330 — 🔴 **번역기가 아니라 «문»이다**: 칸 이름만 바꿔 `enrichment.config.chain_rules_from_cells` 로 «내려보낸다». 확장기는 그쪽 «하나»고 이 파일은 둘째 확장기를 «만들지 않는다» · `unknown_decide_cells(internal)` :354(이름 대고 «거절 안 함») · **`is_switched_off(internal) -> bool`** :363 = `enabled_written and enabled is False` — 🔴 **「적힌 `false`」만 OFF 다**. 없으면 ON, truthy 면 ON(옛 껍데기가 읽던 그대로). 워커는 이것을 «맨 먼저» 물어 참이면 «아무것도» 안 한다(§4 `load_chain_rules` 행 — 🆕 이제 아래 `expand_declaration` «안»에서) |
| 🆕 **`expand_declaration(declaration, table_config=None) -> (rules, refusal, notes)`** :375(@`c904b389` — 구 표기 :338 @`b1db471a`, :328) (`00da7e91` S-244) | 🔴 **저자 «하나», 부르는 자 «둘»** — 규칙 파일 항목 하나 → 「그것이 세우는 체인 규칙들」. 로더(`ingestion_worker.load_chain_rules` :740)와 저장 관문(`ledger/admin.save_chain_rule_raw` :691·:720)이 «같은 함수»를 «같은 카탈로그»로 부른다 — 종전엔 로더는 «번역한 뒤» 심판하고 관문은 «날것»을 심판해, `POST /admin/chain/rules/raw` 가 로더가 «받는» 선언을 세 문장(`trigger_table` 없음 · `derive` 모름 · `unresolvable_mapper`)으로 거절했다(S-204 「저장 관문과 로더가 같은 판정자」가 «입력»에서 조용히 거짓이 됐던 것 — 같은 판정자, 다른 입력은 판정자 둘과 같은 결함). 결과 «셋»: ① `derive` 가 dict 가 아니면 `([declaration], None, [])` — **옛 평면 규칙은 «손 안 대고» 돌아온다**(옛 저장 경로가 바이트 그대로인 이유) ② `is_switched_off` → `([], None, [note])` — 🔴 **OFF 는 거절이 «아니다»**(판정 399) ③ `decide_rules` 거절 / `JoinTriggerConflict` → `([], "<name>: …", [])` · 아니면 `decided` 또는 `[as_chain_rule] + companion_rules` 와 «모르는 칸» 노트. 시험 `test_the_save_gate_and_the_loader_read_one_declaration_alike.py`(230줄 / `def test_` 9) |
...
| 누가 부르나 | `chain/ingestion_worker.py` · 🆕 `ledger/admin.py`(`save_chain_rule_raw` → `expand_declaration`) · `scripts/preview_unified_declarations.py` (+시험). 🆕 이 파일이 «부르는» 쪽: `enrichment.config`(함수 안 지연 import — 순환을 피하려고) · `chain.join_into` |
| 시험 | `test_the_internal_rule_gives_back_exactly_what_it_was_given.py` · `test_a_declared_join_writes_what_it_says_into_the_table.py`(🆕 `b1db471a` 467줄 / `def test_` 19) · `test_enrichment.py` · 🆕 `test_a_declared_unique_key_is_made_by_the_product.py`(`key` 칸의 왕복, 334줄 / `def test_` 18) · 🆕 `test_an_enrich_declaration_reads_the_same_in_either_grammar.py`(`dc29056a`, 11건 — 두 문법이 «칸 대 칸» 같은 규칙, OFF 통합 선언은 `ensure_once` «호출 0», `ASSY_CHAIN_SYNTHESIZE=0` 이 통합 선언을 «못 끈다») |

### 4-ter-c. `server/chain/join_into.py` (🆕 **316줄** @`b1db471a` — 288 @`dc877746`, 272 @`474f1aa9`, 구 표기 260(`7f3c3634` 신설 때 259), S-237 · 판정 396~398)

| | |
|---|---|
| 무엇 | `builtin:join_into`(`JOIN_INTO_MAPPER` :32) — 통합 선언의 `join` 종류가 «쓰는» 조인. 선언 하나가 규칙 «둘»이 된다. `join_spec` :37 · `_pairs` :42 · `_answer` :173 · **`run(db, rule, row_ids=None, done=None, **_)`** :256 |
...
| 🔴 가상 조인 «무접촉» — 코드로도 | 실행기·캐시·로더를 «import 하지 않는다». 빌려 오는 것은 🆕 `notation_norm.key_expression_sql`(`ddd5b3ba` — 접기 «함수»가 아니라 «키 식»의 저자, §5-E)과 🆕 `operator_line`(§4-ter-h, stdlib 만 든다)뿐이다. 유일 인덱스에 필요한 것은 «돌려주기만» 한다(아래 `right_key`) — 세우는 것은 껍데기(§5-C `builtins.ensure_declared_unique_keys`)의 일이고, 경계 시험 `test_this_module_does_not_borrow_the_read_time_executor` 가 그것을 지킨다 |
| 🔵 **허용 목록이 «있다»** | `JOIN_CELLS = ("right_table","on","take")` :73 + `unknown_cells()` :76(@`b1db471a` — 구 표기 :71/:74) — `rule_shape.py:280`(구 표기 :270) 이 그것을 부른다. ⚠️ **옛 가상 조인 선언에는 그 자리가 «없다»**(D-37 실측: `virtual_join/config.py` 전체에 허용 목록 0 → 큐 S-238). 즉 오타 칸이 **새 문법에선 이름이 대어지고 옛 껍데기에선 조용하다** |
| 🆕 **행 단위 팬아웃 그물** (`593aac50` S-237) | `_write(db, left_table, rows, spec, source_name)` :195(@`b1db471a` — 구 표기 :167) 안 — LEFT JOIN 결과에서 `row_id` 가 «두 번 이상» 나온 왼쪽 행(`fanned` :213)은 **어느 답도 «그 답»이 아니므로 «어느 것도» 안 쓴다**. 그 행들을 «이름으로» 건너뛰고(배치당 경고 «한 줄» — 🆕 **[`3aab7173` S-247] 그 줄이 `operator_line.line("join_into", source_name, "왼쪽 행 N 개가 오른쪽에서 «둘 이상»과 맞아 건너뜁니다 …", operator_line.fold_the_data(right_table, right columns), fanned)` :222 가 됐다** — 「다음: 그 표에서 그 키의 행들을 하나로 합치십시오」가 «줄 안»에 실린다. 이건 «데이터»의 결함이라 선언을 넓혀도 안 바뀐다. 표본은 «셋» + 「외 N」(`operator_line.samples_of`) — 구 표기 「첫 열 개 id」는 낡았다) 나머지는 쓴다 — 나쁜 키 하나가 배치를 «못 죽인다»(지배 원칙 2). ⚠️ **카탈로그 게이트(좁은 키/인덱스 없음)는 시도했다가 뺐다** — `virtual_join` 을 import 해야 했고 그것을 경계 시험(`test_this_module_does_not_borrow_the_read_time_executor`)이 막는다. 그물은 카탈로그가 «필요 없다». 로그 줄 모양은 `RUN.md` 가 든다 |
| 🆕 **`right_key(rule) -> (right table, right columns, folds)`** :80 (`8cab58da` S-240) — ⚰️ 종전 줄 「오늘 `key.unique` 를 읽는 자가 «없다» … S-240 은 **아직 착지 전**」은 «착지로 거짓이 됐다» | 「오른쪽에 유일 인덱스를 세우려면 무엇이 필요한가」의 «셋». 🔴 **여기서 묻는 이유는 접기가 «여기서» 정해지기 때문이다** — 인덱스와 조인이 «같은 식»에서 나와야 PostgreSQL 이 인덱스를 쓴다(S-181). 껍데기가 둘째로 계산하면 판정 397 이 걷어낸 «둘째 저자»다. ⚠️ 그래도 이 모듈은 `virtual_join` 을 «모른다» — 셋을 돌려줄 뿐이고 세우는 자는 §5-C `builtins.declared_unique_targets` :181(:204 가 이것을 부른다) → `ensure_declared_unique_keys` :143(적재 시점 `warmup_worker` 0-bis, §4)이다. 그래서 종전 줄의 「`ensure_once` 호출자 «하나»」도 낡았다 — 이제 «둘»(`virtual_join/config.py` :877 · `chain/builtins.py` :177). `key` 칸이 여기까지 «오는» 길은 §4-ter-b(`from_declaration` :188 → `as_chain_rule` :99). 시험 `test_a_declared_unique_key_is_made_by_the_product.py`(334줄 / `def test_` 18) |
...
| | |
|---|---|
| 무엇 | 선언이 `cardinality: one` 이라 말하면 **«제품이» 그 유일성을 세운다**. `inspect` :90 · `ensure` :152 · `duplicate_keys` :63 · `invalid_leftovers` :42 · `describe` :107(전부 @`b1db471a` — 구 표기 :85/:134/:58/:37/:102) |
| 🆕 **`_expressions(table, columns, folds)`** :30 — **[`ddd5b3ba` S-245] «표»를 받는다** | 프로브가 GROUP BY 하는 식이 인덱스가 «세워지는» 식과 «타입까지» 같아야 한다 — `vjc.index_key_expression(column, fold, table)` :37. 표를 안 받던 때는 컬럼이 텍스트인지 «물을 수 없어» `GROUP BY coalesce(number_col, '')` 가 읽기 경로의 «자기» 트랜잭션을 abort 시켰다(09-15). 호출자 `duplicate_keys` :63 · `fold_plan` :384 |
| 🆕 **`describe` :107 — [`3aab7173` S-247] 네 상태가 «전부 행동으로» 끝난다** | `operator_line.line("VirtualJoinIndex", table, …, <행동>)`: `missing`/`invalid` → `restart_to_apply()` :118·:124(「재기동하면 제품이 스스로 세웁니다」) · `blank_keys` → `fill_or_declare_null(table, columns)` :132(중복이 아니라 «부재») · 중복 목록(값이 «곧» 진단이라 한 줄이 아니라 목록)도 마지막 줄 `→ 다음: fold_the_data(table, columns)` :148 로 끝난다. 부르는 쪽 `virtual_join/config.py` :870·:888 (`_say_once`) · `scripts/check_one_row_one_fact.py` :89. §4-ter-h |
| 🆕 **프로세스당 규칙당 «한 번»** (`1497ea3e`, 2026-09-15 장애 다섯째) | `_TRIED = {}` :210 · **`ensure_once(db, rule_name, table, columns, folds=None) -> dict`** :213(@`b1db471a` — 구 표기 :192/:195) — 🔴 **스위치 `ASSY_VJOIN_AUTO_INDEX` 가 OFF(`0/false/off/no`)면 «맨 보고»를 돌려주고 DB 를 «안 건드린다»** — 첫 판은 OFF 에서도 `inspect()` 를 불렀고, 그 GROUP BY 가 숫자 키에서 «읽기 경로»의 트랜잭션을 abort 시켜 «운영자가 잡은 스위치가 아무것도 안 바꿨다». ON 이면 `ensure(apply=True)` 를 `try` 로 감싼다: **프로브가 던지면 `db.rollback()` 하고 `state: "probe_failed"` 로 `_TRIED` 에 «기억»** — 한 번 시도하고 «다시는» 안 한다(이번 실행엔 그냥 «자동 수리 없음», 홍수 없음). `forget(rule_name=None)` :373(@`b1db471a` — 구 표기 :355 @`dc877746`, :241) 이 그 기억을 지운다 — ⚠️ **호출자 «0»**(전건, 시험 포함). 🆕 **호출자가 «둘»이 됐다**(S-240): `virtual_join/config.load_verified_rules` :877(읽기 시점 선언) · `chain/builtins.ensure_declared_unique_keys` :177(통합 join 선언 — 적재 시점, §5-C) |
| 🆕 **인덱스는 «그것을 요구하는 조인만큼» 산다** (`90d971ba` S-248) — `_RETRACTED = {}` :262 · **`product_indexes(db) -> [(table, index)]`** :265 · **`retract_unrequired_once(db, required) -> dict`** :288 · `forget_retractions()` :368(전부 @`b1db471a` — 구 표기 :244/:247/:270/:350) | 박스 실측 09-15: S-235 가 조인이 살아 있을 때 `uq_vjoin_dt_inventory_…` 를 세웠고, 그 조인이 거절·이관·OFF 된 뒤 «인덱스만» 남았다. `dedup` 이 충돌하는 재고 행을 넣자 23505 — 그룹이 «영구 실패», 재시도마다 같은 실패. ⛔ **쓰기 관문은 그것을 «못 본다»** — `crud.refuse_virtual_join_duplicates` 는 «검증된 규칙»의 키만 알므로 규칙이 사라진 인덱스는 «관문 밖에서» 문다. `product_indexes` 는 `pg_index` 를 `config.INDEX_PREFIX`(`uq_vjoin_` :128 — 철자를 «다시 적지 않는다») 접두로 읽는다 — **접두가 안전의 «전부»**: 운영자가 «자기 이름»으로 세운 인덱스는 «절대» 회수되지 않는다. `retract_unrequired_once`: 🔴 `ASSY_VJOIN_AUTO_INDEX` OFF 면 «DB 무접촉»(`ensure_once` 가 배운 문장 그대로) · PostgreSQL 이 아니면 «건너뜀» · 목록 프로브가 던지면 `rollback` + 건너뜀 · 요구 집합에 없는 것을 `DROP INDEX CONCURRENTLY IF EXISTS` 로 «자기 AUTOCOMMIT 커넥션»에서 하나씩(하나의 실패가 나머지를 «못 막는다») · 보고 `{dropped, kept, skipped}` 를 «요구 집합(frozenset)당 한 번» 기억. 호출자 `virtual_join/config.load_verified_rules` :930 «하나»(`path is None` 일 때만 — §5-C; 🆕 **[`8cab58da` S-240] 요구 집합이 `chain_builtins.declared_unique_index_names(known_tables)` :928 을 «합친다»** — 통합 join 이 요구하는 인덱스도 같은 `uq_vjoin_` 접두라, 읽기 시점 선언만으로 센 요구 집합은 웜업이 세운 것을 «다음 읽기에» 걷어낸다). `forget_retractions` 호출자: 시험뿐. 시험 `test_a_product_index_lives_as_long_as_the_join_that_needs_it.py`(218줄 / `def test_` 8) |
| 🆕 이 범위 «전»부터 있었으나 미등재 | `MAX_PLANNED_KEYS = 50` :381 · `fold_plan` :384 · `describe_fold_plan` :438 · **`narrower_than_identity(table, columns, known_tables=None)`** :460(전부 @`b1db471a` — 구 표기 :363/:366/:420/:442 @`dc877746`) — 조인 키가 그 표의 «신원»보다 좁으면 유일 인덱스는 «영원히» 못 선다는 것을 «데이터 한 행 안 읽고» 안다. 호출자 `virtual_join/config.py` :867(`ensure_once` :877 «앞»에서 묻는다) · `scripts/check_one_row_one_fact.py` :47 |
...
| 누가 부르나 | `virtual_join/config.py`(:867·:877·:930) · 🆕 `chain/builtins.py`(`ensure_once` :177 — S-240) · `scripts/check_one_row_one_fact.py`(:47·:86·:89) (+시험) |
| 시험 | `test_the_product_sets_up_its_own_unique_key.py`(🆕 `b1db471a` 200줄 / `def test_` 16) · 🆕 `test_every_refusal_line_says_what_to_do_next.py`(223줄 / 12) · `test_a_product_index_lives_as_long_as_the_join_that_needs_it.py` (+ 이름을 드는 시험) |
...
### 🆕 4-ter-g. `server/chain/rule_order.py` (🆕 **149줄** @`c904b389` — 139 @`dc877746`, 95 @`474f1aa9`, `c7bc59db` 신설 2026-09-13, S-156 · 판정 385·387·388) — ⚠️ 통합 라운드의 «여섯»에 들지 않는다. 미등재라 이 옆에 둔다

| | |
|---|---|
| 무엇 | 체인 규칙의 «순서» — 선언에서 «유도»된다, 새 칸이 아니다. ⚰️ ~~`class RuleCycleRefused(ValueError)`~~(`2504fbde` 에서 «사라졌다» — §0 ㉘) · **`order_rules(rules, on_cycle=None) -> list`** :65(@`c904b389` — 구 표기 :55) — `trigger_table` ↔ 다른 규칙의 `target_table` 로 생산자→소비자 엣지를 긋고 후위 DFS 로 «생산자 먼저». 🔴 **재생이 늘 하던 유도인데 «읽는 자가 하나»였다** — 워커는 «파일 순서»로 돌았다(이 모양의 «넷째»: S-152·S-221·S-154) |
| 🆕 **고리는 «모양»이지 오류가 아니다** (`2504fbde` 판정 402) — `_SAID = set()` :23 · `cycle_note(trail, ceiling=None) -> str` :26 · **`say_cycle_once(logger_, trail, ceiling=None) -> bool`** :50 · `forget_cycles()` :60(전부 @`c904b389` — 구 표기 :40/:50) | `dt_log → dt_inventory`(맵퍼) → `dt_log`(조인)은 «의도된» 고리이고 유한하게 만드는 것은 드레인이 이미 강제하는 `max_chain_depth` 다. 적재 때 그것을 거절하는 것은 «도는 선언»을 거절하는 것이었고, 드레인이 «배치마다» 파일을 다시 읽으니 그 줄이 초당 두 번 흘렀다(운영자 「사이클 오류 계속 뜨네」). 이제 `order_rules` 의 걷기는 고리를 만나면 `on_cycle(path + [name])` 을 부르고 «내려가기를 멈추며», 🔴 **고리가 하나라도 있으면 `list(rules)` — «선언 순»을 «통째로» 돌려준다**(부분 걷기가 되감긴 순서는 선언 순도 유도 순도 아닌 «아무도 안 적은 셋째 순서»라). `say_cycle_once` 는 자취(tuple)당 «프로세스당 한 번» INFO — 문장은 🆕 **[`d469bacf` S-247-b] «같은 저자»를 지난다** — `cycle_note` :42~:47 이 `operator_line.line("ChainRules", "a -> b -> a", "고리 (순서는 선언 순 · 홉 상한 max_chain_depth=N 이 막습니다)", operator_line.nothing_to_do("더 긴 고리가 필요하면 chain_rules.json 의 max_chain_depth"))` 를 부른다(함수 안 지연 import). 실제 줄: `[ChainRules:a -> b -> a] 고리 (…) → 다음: 없음 — 이 줄은 «알림»이고 지금 조치할 것이 없습니다 (더 긴 고리가 필요하면 …)`. 🔴 종전 `cycle_note` 는 그 모양을 «손으로» 철자하고 docstring 으로 「S-247 이 준 모양」이라 «주장»만 했다 — 저자 하나가 조용히 둘이 되는 길이고, 직전 지도 패스가 §4-ter-h 에 씨앗으로 적어 둔 것이 이 커밋으로 «접혔다»(`cycle_note` 의 docstring :34~:38 이 「Measured by the code-map pass」로 그 출처를 든다). `forget_cycles` 호출자: 시험뿐. 🔵 `_validate_chain_cascade_graph` 도 «같은 입»을 쓴다(:1057) — 다른 엣지 집합, 같은 문장 |
| 왜 «자기 집»인가 | 워커가 `replay` 를 import 하면 DB·셀 층·맵퍼 호출자가 «같이» 열린다 — `chain/mapper_call.py` 와 같은 처방(S-214, 판정 370): 공용 프리미티브는 «가벼운 집»에 살고 양쪽이 import 한다 |
| 🔵 건너뛰는 엣지 «둘» | ① **같은 표 위의 «쌍»**(판정 388): P 와 X 가 «둘 다» `t` 를 읽고 `t` 에 쓰면 그 사이엔 «찾을 순서가 없다» — 이름 비교로 «안» 정한다(구 이름 비교는 `enrichment_auto_confirm:*` 셋(`dt_inventory → dt_inventory`)을 서로의 생산자로 만들어 «매 적재마다» 고리를 찍었고, 그래서 S-156 이 이 박스에서 «죽어 있었다») · ② 🆕 **못 발사하는 생산자**(`ea8f91d2`, 2026-09-15 운영 고리): `not producer.get("enabled", True) or producer.get("follow_up")` 이면 `continue` — 트리거 경로가 규칙을 돌리기 «전»에 묻는 두 질문(`_rule_accepts_event` 가 follow_up 을 거절 · `enabled:false` 는 SKIPPED_DISABLED)을 이 걷기가 «안 물었다». 그래서 통합 join 이 내는 «페이싱된 오른쪽 규칙»(right→left, follow_up — 트리거 경로에 «절대» 안 온다)이 살아 있는 left→right 규칙과 «엣지 하나짜리 고리»를 만들었다. 소유자 「enable false 여도 고리 인식하나?」 — 그랬다. 이제 «발사 못 하는 엣지는 순서를 못 정한다». 서로 «다른» 표 사이의 «살아 있는» 고리는 여전히 «이름 대고» 거절된다 |
| ⚠️ 이것이 «못 보는» 것 | 선언이 «읽는다»고 말하지 않은 표를 읽는 규칙 — 다른 모든 선언 구동 가드와 같은 «공인된 사각»(「선언으로 표현할 수 없는 교차 테이블 의존」) |
| ⚠️ `_validate_chain_cascade_graph` 의 고리와 «다른» 고리다 | 그쪽은 `allow_chain_trigger` 엣지만 걷고 «런타임 루프»를 묻는다. 이쪽은 «모든» 생산자→소비자 엣지를 걷고 «불가능한 순서»를 묻는다. 두 질문, 두 엣지 집합 |
| 누가 부르나 | `chain/ingestion_worker.py` :890(`load_chain_rules` 끝 — `on_cycle=lambda trail: say_cycle_once(logger, trail, max_chain_depth(_RULES_DOCUMENT))` :891, 체인을 «안 멈춘다») · `chain/replay.py` :223(@`c904b389` — 구 표기 :188) `order_rules` 가 «감싸서» 같은 것을 부른다 — ⚰️ 종전의 `RuleCycleRefused → ReplayRefused` 재던지기는 «없다»(번역할 거절이 남지 않았다) → `scripts/chain_replay_cli.py` :185 · `scripts/outbox_triage.py` :155 가 그 래퍼를 지난다 |
| 시험 | 🆕 `test_every_refusal_line_says_what_to_do_next.py`(`d469bacf` 가 고리 줄의 단언을 더했다 — 249줄 / `def test_` 14, 구 223/12) · `test_the_worker_runs_rules_in_the_order_the_declaration_implies.py`(`ea8f91d2` 가 둘을 뒤집었다 — 조인의 페이싱 참조 규칙은 더는 「먼저」로 단언되지 않는다: 트리거 그룹을 «공유하지 않으므로» 앞뒤가 «뜻이 없다») · 🆕 `2504fbde` 가 `assert not hasattr(rule_order, "RuleCycleRefused")` 를 단언한다 — 고리 픽스처는 «선언 순»을 받는다 |

### 🆕 4-ter-h. `server/operator_line.py` (🆕 **108줄** @`c904b389` — 102 @`3aab7173` 신설 2026-09-15, S-247 · `d469bacf` S-247-b) — ⚠️ 통합 라운드의 «여섯»에 들지 않는다. 같은 날 신설이라 이 옆에 둔다

| | |
|---|---|
| 무엇 | 운영자가 읽는 «한 줄»의 정본 — 「무엇이 일어났나」에 「다음: 무엇을 하나」가 «반드시» 붙는다. **`line(where, subject, what, action, samples=()) -> str`** :92(@`c904b389` — 구 표기 :86) = `"[where:subject] what (표본: …) → 다음: action"` — 「다음:」절이 «없는» 줄은 이 함수가 «만들 수 없다». 접두 대괄호가 «검색어»다(자리마다 모양이 다르면 운영자가 로그에서 못 찾는다) |
| 🔴 닫힌 행동 어휘 «다섯» — 함수다, 상수가 아니다 | `fold_the_data(table, columns)` :40(진짜 중복 → «데이터»를 합쳐라; 합쳐도 되는지는 접기 계획이 가른다) · `widen_the_key(where_declared, what_to_add)` :46(키가 신원보다 좁다 → «선언»을 넓혀라, 데이터는 «건드리지 마라») · `fill_or_declare_null(table, columns)` :52(비어 있다 → 채우거나 카탈로그 `null_policy`) · `restart_to_apply()` :58(제품이 «스스로» 세운다 — 손 DDL 없음) · 🆕 **`nothing_to_do(unless: str = "")`** :63(「없음」도 행동이다 — 그걸 모르면 운영자는 «무엇이든» 한다. 🔴 **[`d469bacf` S-247-b] 첫 판은 「건너뛴 것을 셉니다」를 문장에 «박아» 두어 어느 자리에서도 참이 아니었고 그래서 호출자가 «0»이었다** — 지금은 어디서나 참인 것(「이 줄은 «알림»이고 지금 조치할 것이 없습니다」)만 말하고, 그 자리에만 참인 조건은 `unless` 로 받아 괄호에 붙인다). 함수인 이유: 행동마다 «채울 이름»이 다르고, 문자열 상수면 부르는 자리가 포맷을 짜서 «저자가 넷»이 된다. 새 행동은 «여기» 함수 하나로 — 그러면 그 행동이 «무엇을 말해야 하는지»가 한 자리에서 정해진다 |
| 왜 | 소유자 2026-09-15 「난 이 에러를 이해할 수가 없다, 조치를 뭘 해야 하는지 안 알려줌」 + 같은 날 「운영은 보안 땜에 못 붙여」 — 🔴 **같은 요구다.** 운영 로그는 밖으로 못 나오므로(붙여 넣고 물어볼 수 없으므로) 줄이 «혼자» 답해야 한다. ⛔ 그리고 «조치 없는 줄»은 중립이 아니다 — 이 제품엔 **같은 증상(「중복 키」)에 수리가 «정반대»**인 자리가 있다(키가 신원보다 좁다 → «선언» · 진짜 중복 → «데이터». 뒤집으면 사실이 사라지거나 중복이 남는다). 둘이 같은 낱말로 로그에 나오므로 조치 없는 줄은 운영자를 «반반의 확률로» 틀린 수리로 보낸다. 그래서 행동은 부르는 자리가 «짓지» 않고 여기서 «고른다» |
| 표본 | `MAX_SAMPLES = 3` :32 · `samples_of(items, cap=MAX_SAMPLES)` :83(@`c904b389` — 구 :77) — 셋까지 + 「외 N」(셋만 보이고 수를 안 말하면 「셋뿐」으로 읽힌다; 수천 건 나열은 진단이 아니라 덤프) · `_names(seq)` :75(구 :69; 리스트 repr 의 대괄호를 운영자에게 안 보낸다) |
| 🔴 import «0»(stdlib 만) | `virtual_join/refusal.py` 와 같은 자세 — 부르는 자리가 `chain` · `database` · `virtual_join` 세 패키지에 흩어져 있고 어느 쪽도 나머지를 읽으면 안 된다. 📎 **`refusal.py` 와 «다른 것»이다**: 그쪽은 «화면»(설정 보고서·verify 라우트)이 읽는 문장의 정본, 이쪽은 «로그»의 정본. 화면엔 저자가 있었고 로그엔 없었는데 운영에 «실제로 있는» 채널은 로그 쪽이다 |
| 누가 부르나 (`git grep … HEAD` 전건 @`c904b389`, 시험 제외 — 파일 «넷», 자리 «여섯») | `chain/join_into._write` :222(팬아웃 → `fold_the_data`, §4-ter-c) · `database/crud.refuse_virtual_join_duplicates` :4095(@`c904b389` — 구 :4044; 배치 안 중복 → `fold_the_data`) · :4159(구 :4108; 저장된 행과 충돌 → `fold_the_data`) · `database/crud.apply_batch_updates` :4437(구 :4386; BK 충돌 «미해결» → **`widen_the_key`** — 위 둘의 «정반대» 수리이고 로그에선 같은 낱말로 보인다, §2) · `virtual_join/unique_key.describe` :115·:120·:128·:148(`restart_to_apply`·`fill_or_declare_null`·`fold_the_data`, §4-ter-d). 🆕 **`nothing_to_do` 호출자 «1»** — `chain/rule_order.cycle_note` :42~:47(§4-ter-g; `d469bacf` S-247-b). 직전 패스가 여기 적어 둔 «씨앗»(「`say_cycle_once` 가 「다음: 없음」을 :35 에 손으로 적고 이 모듈을 안 부른다」)이 그 커밋으로 «접혔다» — 고리 줄도 이제 이 모듈의 `line()` 을 지나고, 이 파일의 `unless` 인자가 그 접힘의 값이다. `restart_to_apply` 호출자는 `unique_key.describe` 뿐 |
| 시험 | `test_every_refusal_line_says_what_to_do_next.py`(🆕 249줄 / `def test_` 14 @`c904b389` — 구 223/12, `d469bacf` 가 고리 줄을 더했다 — 네 자리 + 고리의 줄이 «전부» `→ 다음:` 을 들고 «행동 어휘 안»의 문장인지) · `test_business_key_conflict_retry.py`(371줄 / 17) · `test_the_product_sets_up_its_own_unique_key.py`(200줄 / 16) |
...
### `server/runtime/process_supervisor.py` (**1,116줄** — `ed9cfdb` 718에서 **+306**) — 자식 프로세스 감독
> 🟢 **심볼 실측 완료** — 이 절의 심볼은 **`5609ff0`의 커밋된 blob에 존재함이 확인됐고, 그 뒤 HEAD가 움직인 다음 재대조에서도 동일했다**(워킹트리 아님). 🔴 **라인 번호는 싣지 않는다** — 위치는 `git grep -n "<심볼>" -- <경로>`로 확정하라. 숫자가 남아 있는 곳은 **파일 줄 수**이거나, 이름이 없는 덩어리를 가리키며 측정 sha가 함께 적힌 자리뿐이다.
...
| 🆕 **`VERDICT_BROKEN_CHILD = "broken_child"` / `VERDICT_PORT_CONFLICT = "port_conflict"`** | **종단 판정 어휘 2종.** 스냅샷의 `"terminal_verdict"` 키와 이벤트 스트림(`_record(child, "permanently_failed", verdict=…)`에 실린다. ⚠️ **현재 `main.py`·`runtime/health.py`에 이 값으로 분기하는 독자가 없다** — 소비자는 `tests/test_duplicate_launcher.py`뿐이다(선언은 있고 화면이 아직 안 읽는다) |
...
### `server/runtime/health.py` (🆕⑱ **498줄** @`2ac37874` — 구 표기 384) — `/health` 판정표 (순수 함수 + config 백업 프로브)
**`8117456` 신설, `b35bc9f`에서 `checks.config_backup` 추가.** 테스트: `tests/test_health_endpoint.py` · 🆕⑱ `tests/test_health_roster_feeds_the_loop.py`.

> ⚠️ 🆕⑱ **이 파일은 «신설이 아니다»** — `8117456f`(2026-07) 부터 있었고 그 뒤 커밋 여덟이 지났다(`d56e7e20`·`b35bc9fc`·`229611a5`·`83627dc5`·`a7f003b3`·`f00ac31d`). 신설이라 적힌 지시서를 받거든 그것이 뜻하는 것은 **아래 명부(roster) 블록**이다.
...
| `probe_outbox(db)` | 백로그 나이 + (상한된) 크기. 둘 다 부분 인덱스 `idx_outbox_unprocessed`를 타고, 나이는 `ORDER BY id ASC LIMIT 1`로 **테이블 크기와 무관한 O(1)**, 카운트는 `LIMIT cap+1`로 감싸 1천만 행 테이블에서도 ~1만 인덱스 엔트리를 넘지 않는다.<br>🆕⑱ **응답에 `undelivered_oldest_age_seconds` 가 붙었다** — 🔴 **「안전망이 자기가 지키는 것 «안»에 산다」가 그 사유다**: `chain_ingestion_worker.sweep_undelivered_broadcasts` 는 체인 워커 루프 «안»에서 도는데, 스윕만 멈추고 워커가 계속 비트를 찍으면 **아무것도 그 사실을 말하지 않았다.** 재는 것은 마지막 실행 시각이 아니라 **백로그**다(이 파일이 아웃박스를 크기 아닌 나이로 재는 것과 같은 사유 — 매번 실패하는 스윕은 «신선한 타임스탬프 + 자라는 백로그»를 남긴다). 행 집합은 스윕 자신이 쓰는 `idx_outbox_undelivered` 와 «같은 정의»다 | 🆕⑱ ~447 @`2ac37874` |

#### 🆕⑱ 명부(roster)는 **`compute_health` 안의 «이미 있던 루프»로 들어간다** (`f00ac31d`)

> 🔴 🪦 **`roster_states` · `ROSTER_RUNNING` · `ROSTER_STARTING` · `ROSTER_DOWN` · `ROSTER_OFF_ROSTER` — 이 다섯은 «없는 이름»이다**(`2ac37874` 전건 grep 0). `a7f003b3` 이 신설했고 `f00ac31d` 이 **같은 날 지웠다**. 🔴 **사유는 「안 쓰여서」가 아니라 「판정이 둘이 됐다」다** — `compute_health` 가 이미 워커마다 상태를 정하는데 두 번째 분류기를 옆에 두면 **한 사실에 집이 둘**이 된다. **앵커로 쓰지 마라.**
>
> ⚠️ **이 지도가 그 다섯을 한 번 «살아 있는 것으로» 적었다가 같은 패스 안에서 되물렸다** — 측정(`6345ef3d`)과 기록 사이에 HEAD 가 움직였고, 이 절의 **심볼 게이트**가 그것을 잡았다. 🔴 **그것이 라인이 아니라 «심볼»로 인용하는 이유다** — 낡은 심볼은 grep 0 으로 «자기가 낡았다고 말한다».
>
> ✅ **`server/tests/test_health_roster_feeds_the_loop.py` 가 그 부활을 «막는다»** — `assert not hasattr(health, "roster_states")`. 판정이 다시 둘이 되면 빨개진다.

**지금 도는 것 — `compute_health` 의 워커 루프 «안»이고 새 함수가 없다.**

| 자리 | 무엇이고 왜 그런가 |
|---|---|
| **`expected` 를 정하는 순서: 감독자 → 명부 → 디스크** | 감독자 자식 목록이 있으면 그것, 없으면 **`heartbeat.read_roster()`**, 그것도 비면 «디스크의 비트 전부». 🔴 **명부가 디스크보다 «먼저»인 것이 요점이다** — 곧장 「존재하는 하트비트 파일 전부」로 떨어지면 **한 번이라도 돈 것이 명부에 영원히 남는다**(은퇴한 그래프 워커·원장 워커·워처가 이 박스를 영구 unhealthy 로 읽히게 한 그 모양이다).<br>⚠️ **빈 명부는 «그래도» 디스크로 떨어진다** — 「아무도 발행 안 했다」는 「아무것도 돌면 안 된다」가 아니고, 런처가 이 기능보다 오래된 박스는 «볼 수 있는 것»을 계속 보고해야 한다 |
| 🔴 **`uptime` 을 명부의 시작 시각에서 «채운다»** | `if uptime is None and roster.get(hb_name): uptime = now − roster[hb_name]`.<br>🔴 **이 «한 줄»이 재기동 503 이었다.** 아래 `starting` 갈래는 자기 유예를 갖고 «이미 있었는데», 그것이 보는 `uptime` 이 **감독자 자식 레코드**에서 온다 — 감독자가 없으면 `cinfo` 가 없고 `uptime` 이 `None` 이라 그 갈래가 **도달 불가**였고, 첫 비트를 아직 안 쓴 프로세스가 `missing` + UNHEALTHY 로 보고됐다.<br>✅ **새 상태도 새 임계도 만들지 않았다** — 명부가 이미 발행하는 «시작 시각»이 그 빠져 있던 재료다 |
| **`off_roster` — 이름을 대되 «등급을 올리지 않는다»** | 명부에 없는데 뛰고 있는 비트는 `{"status": "off_roster", "detail": "beating, but no launcher roster declares it", "age_seconds", "beats", "stale_after_seconds"}` 로 나간다. 🔴 **조용히 잊으면 오경보를 «맹점»과 바꾸는 것이고 후자가 더 나쁘다** — 파일은 여전히 디스크에 있다. **아무것도 그것을 약속하지 않으므로 상태를 악화시키지도 않는다** |

> ✅ 🔴 🆕⑱ **[2026-09-05 재측정 · `2ac37874`] `heartbeat.read_roster()` 의 «운영 소비자가 0 → 1» 이 됐다** — `health.compute_health`(**~219**, 함수 «안»의 지연 import). 🔴 **그래서 명부는 이제 「발행되는데 안 읽히는」 것이 아니다.** 발행 쪽은 `run_decoupled_app.py` 하나 그대로다([§5 `heartbeat.py`](#5-소형-서버-모듈)).

### `server/config_backup.py` (**440줄**(🆕⑨ `5359fdd` 실측 — 구 표기 379)) — 주간 config 스냅샷 + FIFO 보존 (`b35bc9f` 신설)
**C3 — 롤백 절차(B4)의 빠져 있던 의존성.** `server/config/*.json`을 파일별로 `backup/config_<yymmdd>.json.bak`(동일 날짜 2회째부터 `b`,`c`,… 접미)으로 스냅샷한다. 설치·어드민 저장 직전 사본과 복원 직전 증거도 같은 `backup/`에 두고 파일명으로 역할을 구분한다. 추적되는 템플릿은 `sample/`에 분리한다. **신선도는 cron 슬롯이 아니라 디스크의 최신 스냅샷 나이로 판정**한다(놓친 주가 다음 틱에 자가 치유). 전환 중 루트와 옛 `_archive/weekly`에 남은 구형 주간 스냅샷도 읽되 새 파일은 항상 `backup/`에 쓴다. 소비자 3곳: `run_auto_update.MultiDiscoveryScheduler.maybe_backup_configs`(주기 실행) · `health.probe_config_backups`(`/health` 프로브) · `server/scripts/backup_config.py`(CLI). 테스트: `tests/test_config_backup.py`.
...
| `_config_dir(config_dir=None)` / **`backup_dir`** / `backup_dir_for(path)` / `snapshot_path(filename, …)` | 🔴 **[2026-08-18 실측 정정] 종전 지도의 `archive_dir`/`weekly_dir`/`changes_dir`/`restore_dir` 네 이름은 이 파일에 «하나도 없다»**(전건 검색 0). 실제 API는 이 넷이고, 전부 `config_dir` 인자를 받아 기본값만 `paths.CONFIG_DIR`이다 — 「모두 CONFIG_DIR 상대」는 재정의 가능성을 지운 서술이었다. `backup_dir_for`는 `sample/` 아래 경로를 부모로 접어 올린다. `snapshot_path`는 **`backup/` → `_archive/weekly` → 루트** 순으로 찾는다 — 두 번째가 레거시 읽기 폴백이고, 종전 이름들은 그 폴백 시절의 어휘였다 | 142/146/150/158 |
| `snapshot_name` / `source_files` / `list_snapshots` / `newest` | 이름 조립 / 대상 열거 / backup+legacy 스냅샷 열거(오래된 순) / 파일별 최신 | 171/177/207/239 |
...
### `server/utils/heartbeat.py` (🆕⑱ **371줄** @`6345ef3d` — 구 표기 303, 그전 174) — 워커 진척 비트 + **작업 단위 claim** + 🆕⑱ **명부 발행**
...
| 🆕⑱ **`_own_name` / `own_name()`** | **이 프로세스가 «어느 이름으로» 비트를 찍고 있는가.** 첫 `beat(name)` 이 모듈 전역에 세우고, 한 번도 안 찍었으면 `None`. 🔴 **발행하는 이유가 이 행의 요점이다** — 「나는 누구인가」를 말해야 하는 부품이 그것을 «두 번째로 적으면» 두 번째 철자가 두 번째로 틀릴 자리다. **운영 소비자 1**: `retroactive.runner_identity`([§5-D](#5-d-2026-08-04-신설-서버-모듈)) | ~149/152 |
| `beat(name, note=None, force=False)` | **워커의 실제 작업 루프 안에서 반복마다 호출.** 반환값은 테스트용이고 호출자는 무시한다. 고정 temp 파일명 + `os.replace`로 **원자적**(독자가 부분 파일을 보지 않는다). **모니터링 기능이 새 장애 모드가 되면 안 되므로 모든 디스크 오류는 삼키고 카운트만 한다** — 워커 루프로 예외를 올리지 않는다. 🆕⑱ **여기서 `_own_name` 을 세운다** | 🆕⑱ ~157 (구 ~145) |
| `_work_snapshot_locked(name)` | 그 워커의 **가장 오래 진척 없는 claim**. **나이가 아니라 절대 타임스탬프를 publish**한다 — 독자가 비트가 쓰인 시점이 아니라 **지금**에서 stall을 재게 하기 위함 | 🆕⑱ ~213 (구 ~198) |
| **`work_claim(name, what)` (contextmanager)** | **작업 단위 1건 선언.** 파일 1건의 인제션 전체를 감싸고 그 안에서 `beat(name)`을 부르면 같은 스레드의 비트가 claim 진척을 갱신한다. **실패 경로 포함 항상 해제**된다 — 크래시한 잡이 남긴 claim은 영원한 stall과 구분되지 않는다. 진입 시 `force=True` 비트를 한 번 찍어 **다음 폴러 틱을 기다리지 않고** 즉시 보이게 한다. 소비자 **2곳, 둘 다 `with` 진입 줄이 앵커다**: `directory_watcher.process_with_retry`(함수 ~1033, claim **~1044**)·`process_archived_file_sync`(함수 ~1316, claim **~1328**) — 구 표기 1062/1329는 §3의 표(1046/1335)와도 어긋나 있었다([§3](#3-serverparsersdirectory_watcherpy--파일-인제션)) | 🆕⑱ ~232 (구 ~217) |
| `open_claims()` | 이 프로세스의 열린 claim 전량(테스트·진단용) | 🆕⑱ ~257 (구 ~242) |
| `read_all(stale_after=DEFAULT_STALE_AFTER_SEC, now=None, stall_after=DEFAULT_STALL_AFTER_SEC)` | 전 비트 판독(+`age_seconds`/`stale`). **읽을 수 없거나 깨진 파일은 건너뛰지 않고 `error` 필드를 단 stale로 보고한다** — 침묵은 헬스체크가 절대 주면 안 되는 답이다 | 🆕⑱ ~263 (구 ~248) |
| 🆕⑱ **`roster_path()`** | `heartbeat_dir()/_roster.json`. **하트비트 «옆»인 것이 의도다** — 둘은 같이 읽히고, 한쪽에 닿는 독자는 다른 쪽에도 닿는다 | ~321 |
| 🆕⑱ 🔴 **`write_roster(names, now=None) -> bool`** | **런처가 「내가 무엇을 띄웠나」를 내놓는 자리.** `{"written_at": now, "processes": {name: now, …}}`.<br>🔴 **«두 번째 선언»이 아니라 «발행»이다** — 명부는 이미 `run_decoupled_app.py` 의 `ChildSpec(heartbeat=…)` 목록으로 존재하지만, 그 파일은 **저장소 루트의 스크립트**이고 모듈 본문이 import 시점에 돌아서 **어떤 서버 프로세스도 그 자리에서 읽을 수 없다.** 이 함수는 그 목록이 «이미 말한 것»을 쓴다 — 고칠 자리가 하나 더 생기는 것이 아니다.<br>🔴 **시작 시각이 요점이다** — 그것이 없으면 「아직 비트가 없다」와 「비트가 영영 없다」가 «한 사실»이 되고, 그게 재기동마다 503 을 내던 그 모양이다.<br>**절대 raise 하지 않는다** — 명부를 못 쓰는 런처도 자식은 띄워야 한다. `os.replace` 로 원자적.<br>**운영 소비자 «1»**: `run_decoupled_app.py`(`_hb.write_roster([s.heartbeat for s in specs if …])`, 실패는 WARNING 한 줄) | ~327 |
| 🆕⑱ 🔴 **`read_roster() -> {name: started_epoch}`** | 아무도 발행 안 했으면 **`{}`**. ⚠️ **「빈 것」은 「아무도 안 말했다」이지 「아무것도 안 돈다」가 아니다** — 독자가 그것을 「모든 하트비트가 명부 밖」으로 접으면 **정상 가동 중인 시스템을 미선언으로 부른다.** 판단은 호출자 몫이고 이 함수는 «쓰인 것»만 보고한다.<br>✅ 🔴 **운영 소비자 «1»** — `health.compute_health`(`2ac37874` 재측정). ⚠️ **같은 날 «0» 이었다** — `6345ef3d` 에서는 시험만 불렀고, `f00ac31d` 가 그것을 `compute_health` 의 워커 루프에 먹였다. 🪦 **그 커밋이 `health.roster_states` 를 «지웠다»**(판정이 둘이 되지 않게) — [§5 `runtime/health.py`](#5-소형-서버-모듈) 의 명부 절 | ~358<br>🆕㉗ **[S-142 ② `19735ed5`] `ROSTER_FILENAME = "_roster.json"` :384** 이고, 비트 파일을 훑는 자리가 :324 에서 그 이름을 «빼고» 센다 — 명부는 비트가 아니다. 🔴 **낡은 감독자 기록이 «도는 pid» 를 보고하던 것을 멈춘다** — 죽은 감독의 자식 목록이 살아 있으면 `/health` 는 「돌고 있다」를 그 목록에서 읽고, 그것이 「한 시간 안 움직인 큐 옆의 초록 불」이다(판정 282 가 `/health` 와 `/runtime` 을 가른 바로 그 이유) |
...
>
> 이 이름이 `run_decoupled_app.py`의 `ChildSpec(heartbeat=…)`와 짝을 이룬다. 🆕⑱ **그리고 이제 같은 목록이 «명부»로도 나간다** — 런처가 `specs` 를 그대로 훑어 `write_roster(...)` 를 부른다(위 표). ⚠️ **위 네 이름은 이 패스가 재측정하지 않았다** — 앵커 넷은 `8cf9455` 기준 그대로이고 그 사이 커밋이 많으므로 **밀렸다고 가정하라.**
>
...
### 🆕㉔ `server/listing_absence.py` (**35줄**, 2026-09-07 신설) — 「원천이 «없다»」와 「있는데 «비었다»」를 가르는 **한 자리**

| 심볼 | 무엇인가 |
|---|---|
| `LISTING_ABSENT = "absent"` | `ledger_trace.COVERAGE_STATES` 의 낱말을 **그대로 쓴다** — 여기서 다시 정의하지 않는다. 같은 뜻에 두 철자가 생기면 화면이 둘 중 하나만 알게 된다 |
| `absent_listing(path, **extra)` | `{status: "success", data: [], state: "absent", absent_path: <못 찾은 경로>}`. 🔴 **`status` 는 `success` 로 «둔다»** — 실패한 것은 «요청»이 아니다. `data` 만 보는 독자는 **전과 똑같이** 돌고, 「없음/빔」을 가르려는 독자에게만 칸이 생긴다(회귀 반경 «0»). `absent_path` 는 «찾다가 못 찾은 그 경로»다 — 운영자가 고칠 자리를 그 문자열로 찾는다. `extra` 는 «그 라우트만의» 칸이고, 공용 모양에 **안 들어간다** |

🔴 **호출자 넷**(전부 `main.py`, 전부 `require_admin_token`): `GET /admin/file-ingestion/workspaces` · `GET /admin/chain/rules` · `GET /admin/mappers/list` · `GET /admin/auto-update/status`(이 하나만 `last_updated=None` 을 `extra` 로 싣는다). 넷이 각자 손으로 쓰던 줄이었고 **그중 하나가 이미 다른 셋에 없는 필드를 달고 있었다** — 판형에 집이 없으면 그렇게 갈라진다.

⚠️ 클라 쪽 짝은 `client2/src/body_error.js` 다 — 「없음」은 **오류가 아니라서** `status` 가 `success` 라는 것을 그쪽 주석이 적는다.

### 🆕 `server/ingestion/file_ingestion_status.py` (**32줄**, 2026-09-07 신설) — 인제션 실행이 «쓸 수 있는 상태 어휘»의 **정본**
...
> **왜 두 번째 JSON이 아니라 Python 모듈인가**: `server/config/**`는 gitignored(`sample/*.sample`만 tracked)라 그 안의 정본 JSON은 배포되지 않는다. 이 모듈은 코드이고 tracked이며 소비자는 정확히 둘 — ① `server/scripts/install_product_tables.py` ② `config/sample/table_config.json.sample`(같은 설치기가 `--sample --apply`로 **생성**하고 `tests/test_install_product_tables.py`가 둘의 일치를 단언하므로 샘플이 조용히 어긋날 수 없다).
>
> ✅ **ZONE 모델 착지 (`269b39e` 선언 + `b35bc9f` 엔드투엔드)** — 값 하나의 층 구조는 **숫자 하나가 함의하는 고정 3구역**이다: `stack` = 총 층수, `mat_1h` = 1층, `mat_top` = `stack`층, `mat_mid` = 그 사이 전부. FROM도 TO도 구간 행도 없다 — **세 구역이 `1..stack`을 구성적으로 덮으므로 겹침·구멍 검사는 옮겨진 게 아니라 어길 방법이 없어졌다.** 정본 서술은 `map_split_registry.__comment`, 스펙은 [`MAP_EDITOR_SPEC.md` **§6.0-bis**](../spec/MAP_EDITOR_SPEC.md)(⚠️ ~~`docs/spec/DOE_BAND_MODEL.md`~~는 **존재한 적 없는 경로**다 — 코드 주석의 유령 인용도 `95bf072`에서 §6.0-bis로 리포인트됐다), 클라 미러는 `client2/src/map_editor.js` ~301–320 주석.
...
### `server/database/models.py` (🆕⑱ **1,222줄** @`6345ef3d` — 🆕⑰ 1,134 @`64b562b6` — 🆕⑯ 1,127 @`9eb30691` — 🆕⑨ 1,079 @`5359fdd` blob `fbe4bde`, 제목의 636줄은 세 세대 낡은 값이었다) — ORM + 동적 모델/런타임 DDL
정적 ORM 클래스 🆕⑯ **10종**과 config 주도 동적 테이블 관리 함수 — `AuditLog` · `InteractionEffortLog` · `DatabaseOutbox` · `FileIngestionLog` · `FileIngestionCheckpoint` · `CellOverwrite` · `CellSource` · **`FrameConfirmation`** · **`FrameConfirmationSource`**(이 둘의 확정 스키마 서술은 [§5 `maps/frame_confirmation.py`](#-serverframe_confirmationpy--확정의-기록자)와 `migrations/add_frame_confirmation.py`) · 🆕⑯ **`RetroactiveRun`**. **그래프 3모델은 2026-08-18에 제거됐습니다**(아래 취소선 행). 모듈 레벨에 `mapper_registry` · `_KEY_TIERS` · **`declared_key_columns(table_cfg)`**(미등재)도 있다.

> 🆕⑰ 🔴 **[2026-09-02 실측 — `64b562b6` 에서 **1,134줄**] `init_dynamic_models` 가 «새 표»에 그래프 동기화 컬럼 «셋»을 더는 만들지 않는다** — `is_graph_synced` · `needs_graph_rollback` · `graph_synced_at`(`42efb58b`·`21e9df3d`). 앞의 둘은 **인덱스가 걸려 있었으므로**, 모든 insert 가 존재하지 않는 동기화기를 위해 인덱스 둘을 유지하고 있었다(`graph_sync_worker.py` 는 없고 `/graph/mapping-summary` 는 410 — [§1.5](#15-그래프-조회-구간은-410으로-은퇴했다--그-일곱은-이제-없다)).
>
...
>
> 🆕⑯ **[2026-08-31 신설] `class RetroactiveRun(Base)` — `__tablename__ = "retroactive_runs"`.** 요청형 연산 실행 «하나»의 행이고 **등록부 전체가 이 표 하나를 쓴다**([§5-D](#5-d-2026-08-04-신설-서버-모듈)). 컬럼: `run_id`(String(32) PK — `retroactive.publish` 가 만드는 12자리 hex 이고 **큐 이벤트 payload 와 같은 값**이라 아웃박스 행과 서로를 가리킨다) · `op`(indexed) · `params`(JSON 문자열) · `requested_by` · `state`(indexed, 기본 `"queued"`) · `processed_rows` · **`total_rows`(🔴 NULL 가능 — «0 이 아니라 모름»)** · `result`(JSON) · `error` · `queued_at`(`server_default=func.now()`) · `started_at` · **`last_progress_at`**(배치마다 갱신 — 「살아 있나」를 이 값의 «나이»로 본다) · `finished_at` · 🆕⑱ **`runner`(`String(160)`, nullable)**.<br>🆕⑱ 🔴 **`runner` — 「누가 돌리고 있나」.** 실행이 «시작될 때» 찍힌다(`retroactive._mark_run(..., started=True)`). 🔴 **행이 그것을 소유한 프로세스보다 «오래 산다»** — 실행 중에 죽은 스케줄러는 `running` 을 영원히 남기고 `finished_at` 은 안 찍히며 그 행이 잡은 게이트는 다시 안 열린다. 신원이 없으면 **「죽었다」와 「느리다」가 같은 행**이다. ⛔ **그런데 «아무것도 이것으로 판정하지 않는다»** — 자동 회수(reaping)는 «일부러» 안 만들었다: 신원 없이 회수하면 「영원히 안 끝난다」가 「둘이 동시에 돈다」가 되고 후자가 더 나쁘다. **이 컬럼은 그 판정을 «가능하게» 하는 재료이지 판정이 아니다.** ⚠️ **이전 행에는 NULL 이고 NULL 은 «없음»이 아니라 «모름»이다.**<br>🔴 🆕⑱ **[정정] 마이그레이션 파일이 «생겼다»** — **`server/migrations/add_retroactive_runs_runner.sql`**(`ALTER TABLE retroactive_runs ADD COLUMN IF NOT EXISTS runner VARCHAR(160)`). **표 자체는 여전히 `create_all` 이 만들지만 «기존 표에 컬럼을 붙이는 것»은 `create_all` 이 절대 안 한다** — 그래서 이미 있는 DB 는 이 파일을 «한 번» 돌려야 한다. 아래 취소선은 표가 새것이던 때의 서술이다: ~~🔴 **마이그레이션 파일이 «없다»** — 새 표라 `create_all` 이 만든다. `server/migrations/` 에서 `retroactive` 를 찾으면 **0건**이고 그것이 정상이다.~~ 🔴 **이제 `git grep -l retroactive -- server/migrations/` 는 «1건»이다.** 🔴 **연산마다 표를 만들지 않는 것이 설계다** — 그러면 새 op 가 「등록부 항목 하나」가 아니라 「마이그레이션」이 되고 등록부가 범용이라는 성질이 끝난다. ⚠️ **상시형(파일 인제션)은 여기 «들어오지 않는다»** — `FileIngestionCheckpoint` 가 이미 파일마다 total/processed/chunk 를 든다.
...
> 🆕⑨ 🔴 **[2026-08-18] 그리고 세 번째로 밀렸다. 이번엔 라인 열을 통째로 걷어냈다 — 다시 채우지 마라.** 실측(`5359fdd` blob `fbe4bde`) 대 구 앵커 14개: **맞은 것은 `AuditLog` ~11 하나뿐**이다.
>
> | 심볼 | 구 앵커 | 실측 | 차 |
> |---|---|---|---|
> | `AuditLog` | ~11 | **11** | 0 |
> | `InteractionEffortLog` | ~51 | **143** | +92 |
> | `DatabaseOutbox` | ~118 | **210** | +92 |
> | `FileIngestionLog` | ~186 | **278** | +92 |
> | `FileIngestionCheckpoint` | ~200 | **292** | +92 |
> | `CellOverwrite` | ~247 | **364** | +117 |
> | `CellSource` | ~264 | **382** | +118 |
> | `DYNAMIC_TABLES` | ~347 | **708** | +361 |
> | `init_dynamic_models` | ~352 | **786** | +434 |
> | `sync_dynamic_tables_schema` | ~439 | **929** | +490 |
> | `_runtime_ddl_lock` | ~476 | **966** | +490 |
> | `create_missing_dynamic_tables` | ~479 | **969** | +490 |
> | `ensure_ingestion_checkpoint_table` | ~557 | **1016** | +459 |
> | `refresh_dynamic_models` | ~590 | **1049** | +459 |
>
> 🔴 **드리프트가 0 → +92 → +118 → +490 → +459로 «커졌다가 줄어든다» — 일괄 가산이 불가능한 모양이다.** 그리고 `~463`(구 `GraphNode` 행 앵커)은 지금 **`FrameConfirmationSource` 근처**를 가리킨다: **삭제된 클래스의 앵커가 실재하는 다른 클래스에 착지**하는, 이 문서가 반복해 지는 바로 그 실패다. **위치는 `git grep -n "class <이름>" -- server/database/models.py`로 확정하라.**
>
> ⚠️ `init_dynamic_models` 행의 「테이블당 인덱스 2종 부착(~416–417)」도 낡았다 — 실측 위치는 `idx_{table}_updated`가 **882**, `idx_{table}_declared_key`가 **902**다(같은 sha). 이름 없는 줄이라 라인으로만 지목되지만, **두 인덱스가 «같은 자리 두 줄»이 아니라 20줄 떨어져 있다**는 것이 구 표기가 감추던 사실이다.

| 시그니처 | 역할 |
|---|---|
| **`class InteractionEffortLog`** | **[V1 `2a9f6c4` 신설]** 교정 1건의 **원시 상호작용 카운트**(`transaction_id`·`session_id`·`key`·`mouse`·`nav`·`nav_preserved`). **점수 컬럼이 없는 것이 설계다** — 가중치는 config이고 점수는 읽는 시점에 계산되므로, 가중치를 재조정하면 과거 tx가 전부 새 가중치로 재해석된다(선언 절반은 이 절 아래 `server/effort_metric.py`) |
| ~~`class GraphNode` / `GraphEdge` / `GraphSyncState`~~ | ⚰️ **[2026-08-18] 클래스가 트리에서 제거됐습니다.** 🆕⑨ 🔴 **[2026-08-18 날짜 정정] 종전 이 칸은 「물리 표는 2026-08-16에 DROP」이라고 적었는데 틀렸다 — 은퇴는 «세 날짜, 세 가지 일»이다**(실측): **표 DROP 마이그레이션은 `2ec78b9`(2026-08-14)**가 들여왔고(`server/migrations/drop_graph_storage.py`+reverse 신설, 같은 커밋이 410 라우트를 세웠다) · **워커·실행 파일 은퇴는 `9851374`(2026-08-16)**로 표를 건드리지 않았으며 · **모델 클래스 제거가 2026-08-18**이다. ⚠️ 그리고 «마이그레이션이 착지한 날»은 «어느 운영 DB에서 실제로 돌아간 날»이 아니다 — 이 문서는 후자를 주장하지 않는다. 표가 사라진 뒤에도 클래스는 남아 있었고, 그 중간 상태가 **재기동마다 「SCHEMA DRIFT: missing 3 table(s)」를 찍었습니다** — 부팅 점검은 `Base.metadata`를 「이 빌드가 요구하는 것」으로 읽기 때문입니다. 즉 은퇴가 «고장»으로 보였습니다. 자리에 묘비 주석 |
| `DYNAMIC_TABLES` | 동적 테이블 싱글턴(`sys._dynamic_tables_singleton`) |
| 🆕㉗ 동적 모델의 **`kind: view` 갈래** :1043~ (S-186 `71676c4c`·`2c6d7335`) | **뷰의 모델은 «선언된 컬럼만» 든다** — 레이어링 컬럼 넷(`row_id`·`updated_at` 등)을 붙이면 그 관계에 «없는 컬럼»을 SELECT 해 PostgreSQL 이 `UndefinedColumn` 으로 답하고 라우트가 500 이 된다. 🔴 **`total_order_key` 가 착지한 «뒤에도» 라우트가 깨진 이유가 이 자리다** — 정렬 좌석은 다 접혔는데 «모델»이 여전히 없는 컬럼 넷을 약속하고 있어서, 실패가 ORDER BY 에서 SELECT 로 «옮겨갔을 뿐»이다. 소스가 그렇게 적는다: 「`kind` 를 읽는 자리가 이 앞에 셋 있었고 그중 «여기»는 없었다」. 🔴 **매핑된 클래스에는 PK 가 필요하지만, «신원»은 다른 주장이다** — **출하 샘플**(`table_config.json.sample`, 2026-09-12 실측) `kind: view` «열하나» 중 — ⚠️ 소스 주석의 「ten」은 «라이브 카탈로그»(gitignore)의 수라 저장소 주장이 아니다; **넷**이 `business_key` 를 «아예 선언하지 않는다». 클래스를 «지을 수 있게» 컬럼 하나를 지명할 뿐이고, 그 넷은 읽기 경로에서 `total_order_key` 가 R7 로 422 를 낸다 — 「이 관계는 페이징할 수 없다」를 말하는 자리는 거기다. ⚠️ `row_id` 를 «선언한» 뷰는 그것이 키다(판정 296) — `business_key` 를 대신 주면 모델과 정렬이 신원에 대해 «서로 다른 말»을 한다(`dt_log_transferable` 이 그 실물이었다)<br>🆕㉕ 🔴 **[2026-09-13 S-229-b `c6c26b89`] 그 «넷»은 오늘 페이징된다 — 그리고 «틀린 지명»의 값이 측정됐다.** 출하 샘플 실측(오늘 다시 셈): `kind: view` «열하나» 중 `business_key` 를 «안» 선언하는 넷(`bonding_core_lot`·`bonding_core_die`·`lot_slot_move`·`bonding_die_from_core`)은 **넷 다 `composite_key_source` 를 «선언한다»** — 즉 신원이 «없는» 것이 아니라 «여러 칸»이었다. 🔴 **값**: `bonding_core_lot` 은 행 3,658 · `(base_id, core_wafer)` 쌍 3,658 인데 `base_id` 만으로는 «160». 그 한 칸에 매핑하면 SQLAlchemy 의 identity map 이 같은 값을 가진 행들을 «한 객체»로 접어 **1,000 을 물은 페이지가 41 행으로 돌아온다 — 200 으로, 오류 없이, 물은 것의 24분의 1**. 📌 부류: 「같은 수로 돌아온 것이 무엇으로 만들어졌는지는 안 보인다」의 «행» 판 |
| `init_dynamic_models(config_dict)` | config → 동적 ORM 클래스 생성·등록. `column_types`/`business_key`/`composite_key_*`만 읽는다(그 외 키는 무시 — `product_tables.ANNOTATION_KEYS`의 근거). 테이블당 인덱스 2종 부착(`idx_{t}_updated` · `idx_{t}_declared_key` — 후자의 컬럼은 `declared_key_columns`가 정한다) |
| 🆕⑨ **`declared_key_columns(table_cfg)`** + `_KEY_TIERS = ("map_key_columns", "composite_key_source")` | **[2026-08-18 신규 등재 — 이 절에 없던 심볼]** 선언된 키 컬럼을 **티어 순서**로 고른다. 위 `idx_{t}_declared_key` 인덱스가 무엇을 덮는지를 이 함수가 단독으로 정하므로, **인덱스와 키 선언이 갈릴 수 없다.** ⚠️ 이 패스는 존재와 시그니처만 실측했고 **본체 규칙은 읽지 않았다** |
| `sync_dynamic_tables_schema(engine)` | ⚠️ 이름과 달리 **존재하는 테이블의 ALTER 전용**(`has_table` 아니면 skip — 신규 CREATE 안 함). 부팅 경로에서만 호출 |
| `_runtime_ddl_lock` | in-process DDL 직렬화 락(watchdog 스레드 vs reload-configs 요청 스레드) |
| `create_missing_dynamic_tables(engine) -> list[str]` | **신규 테이블 한정 물리 CREATE**(이슈 #7) — information_schema 게이트 + `checkfirst=True` + 테이블별 독립 트랜잭션(실패 자체 rollback). 기존 테이블 런타임 ALTER는 범위 밖(C-8) |
| ~~`ensure_graph_tables(engine) -> list`~~ | ⚰️ **[2026-08-18] 제거.** 마지막까지 남은 이유는 그래프 단위 테스트의 픽스처 생성뿐이었고, 그 테스트들과 함께 걷혔습니다 |
| `ensure_ingestion_checkpoint_table(engine)` | [P2] `file_ingestion_checkpoints` 생성(동일 패턴) |
| `refresh_dynamic_models(engine=None) -> list[str]` | **핫리로드 공용 진입점** — config 디스크 재로드 → `crud.TABLE_CONFIG` 싱글턴 갱신(빈/손상 config 시 기존 보존) → `init_dynamic_models` → engine 지정 시 물리 CREATE(`create_missing_dynamic_tables` + `ensure_ingestion_checkpoint_table`). 🆕⑨ 🔴 **[2026-08-18 실측 정정] 「+그래프 테이블 보장」은 더 이상 일어나지 않는다** — 그 자리에 `⚰️ [R-2026-08-14-H] ensure_graph_tables(engine) 호출이 여기 있었다`는 묘비 주석만 남아 있다. 호출처도 **「graph worker SYSTEM_RELOAD」를 빼라**(그 워커가 없다): main `reload_local_process_cache` / config_watcher(간접) / run_watcher · chain worker SYSTEM_RELOAD |
...
### `server/enrichment/config.py` (🆕 **1,958줄** @`474f1aa9` — 🆕⑨ 1,638 @`5359fdd`, 구 표기 676 · `c520012` 585에서 **+63** — `f9289f6`) — 인리치먼트 규칙 로더/검증
...
| `load_enrichment_rules(path=None, known_tables=None, rejections=None)` / `load_enrichment_chain_rules(path=None, known_tables=None)` | 로드 / **체인 룰 형태로 변환**(rule["enrichment"] 내장). 🔴 **파일 부재는 거부가 아니다** — 빈 목록을 돌려주고 **수집기에 남기지 않는다**(`/graph/mapping-summary`가 `source.exists`로 하는 것과 같은 규율). 읽기 실패·형태 오류만 `_record`로 남는다 | **~373/396** → 🆕 `:839/:895` @`474f1aa9` |
| 🆕 **`chain_rules_for(rule) -> list`** :946 · **`chain_rules_from_cells(name, enabled_written, enabled, source_table, derived_table, cells, known_tables=None) -> (rules, refusal)`** :921 (`dc29056a` S-239) | 🔴 **확장기 «하나», 문 «둘».** `load_enrichment_chain_rules` :895 의 루프 «몸통»이 `chain_rules_for` 로 나왔다 — 정규화된 인리치 규칙 «하나» → 체인 규칙 «둘»(dedup `enrichment.mapper.map_enrichment_dedup` + auto-confirm `builtin:auto_confirm` follow_up, 둘 다 `origin: synthesized:<name>`). **몸통은 «무변»**, 루프만 밖으로 나갔다. 옛 파일(`enrichment_rules.json`)은 `load_enrichment_rules` → `chain_rules_for`, 통합 `derive: {kind: "decide"}` 는 `rule_shape.decide_rules` → **`chain_rules_from_cells`** → `_validate_rule(name, raw, known_tables)` :598 → «같은» `chain_rules_for`. 🔴 **둘째 정규화기가 «없다»는 것이 이 라운드의 주장 전부다** — 둘째 확장기는 쓰인 날엔 같고 어휘에 칸 하나 늘 때 «갈라진다». `chain_rules_from_cells` 가 `enabled_written` 일 때만 `raw["enabled"]` 를 «적고», `auto_confirm_declared` 는 정규화기가 «키의 존재»(`"auto_confirm" in cells`)에서 «유도»한다(판정 401 — 칸이 «아니다»). `_validate_rule` 이 `None` 을 내면 `(…, refusal)` 로 «문장째» 올라간다. ⚠️ 규칙 «수»는 껍데기의 사실이다 — `auto_confirm` 칸(판정 400, boolean)이 있든 없든 껍데기는 «둘» 다 내고, 도는지는 실행 시 컬렉터 `.active` 가 정한다(무변) | **:946 / :921** |
| 🆕 **`_isolated_execute(db, stmt, params) -> (columns, rows)`** | **[`f9289f6` 신설] 사용자가 쓴 참조 문장 1건을 SAVEPOINT 안에서 실행한다** — `db.begin_nested()` → 실패면 `nested.rollback()` 후 re-raise, 성공이면 `nested.commit()`. **호출자 둘뿐이고 그 둘이 이 파일의 SQL 실행 전부다**(`execute_reference_view` 안 **~543** · `execute_candidate_probe` 안 **~597**).<br>🔴 **왜 `try/except`가 아니라 SAVEPOINT인가 — docstring(**~471–507**)에 라이브 읽기 전용 실측이 있다.** PG에서 실패한 문장은 **둘러싼 트랜잭션을 abort**시키고, 그 뒤 같은 커넥션의 모든 문장이 `InFailedSqlTransaction`을 낸다. 조용해지는 지점은 그다음이다: **abort된 트랜잭션에서 `COMMIT`은 정상 반환하고** 서버는 그것을 ROLLBACK으로 바꾼다. 즉 **드라이버 예외를 잡는 것은 봉쇄가 아니다** — 세션은 이미 죽었고 호출자도 로그도 그것을 알 수 없다. 실측 4줄: 나쁜 SELECT → `ProgrammingError` / 다음 SELECT → `InternalError`(세션 오염) / `db.commit()` → **정상 반환**(서버는 롤백) / 같은 것을 SAVEPOINT 안에서 → 다음 SELECT **성공**.<br>🔴 **그 위에 두 가지가 얹혀 있었고 둘 다 깨져 있었다**: ① `enrichment_candidates._diagnose_probe_failure`가 **같은 세션에서** 뷰를 재조회해 `candidate_column_missing`과 `view_error`를 가른다 — abort된 세션에선 그 재조회가 실패밖에 못 하므로 **PG에서 `candidate_column_missing`은 도달 불가**였다(진단이 존재하는 유일한 이유가 그 사유다). ② 체인 워커가 훅의 예외를 삼키므로 **오염된 세션이 `process_pending_groups`로 탈출**해 `processed_chain=True` 커밋이 롤백되고 그룹이 영원히 재생됐다([§4](#4-serverchain_ingestion_workerpy--체인-워커)).<br>⚠️ **pysqlite에는 그 규칙이 없다**(SELECT에 트랜잭션을 열지도 않는다) — 그래서 스위트가 **프로덕션이 도달할 수 없는 거절을 인증**할 수 있었다. 규칙을 테스트에 복원하는 폴트 인젝션은 `server/tests/test_enrichment_candidates.py`의 `pg_abort_semantics`다. 비용은 참조 문장당 왕복 2회(SAVEPOINT+RELEASE)<br>🔴 🆕㉓ **[S-166] SAVEPOINT 에 앉으려면 «앉을 트랜잭션»이 먼저 있어야 한다** — `in_transaction()` 이 거짓이면 열고 들어간다. ⚠️ 그 가드로도 autocommit 커넥션에서는 «안 열린다»(BEGIN 이 안 나감) — 뿌리는 S-167 이다. 📐 **[2026-09-12 재측] 이 행의 `~470` 은 낡았다 — 오늘 `def _isolated_execute` 는 :1359 이고, D-8 이 적은 :1349 도 아니다** | **:1359** |
...
| `to_public_rule(rule) -> dict` | 클라이언트 공개용 필드만 추출. **[F9 `f3fd785`] `reference_views[]`가 `{label}` → `{label, candidate_for}`로 넓어졌다**(가산적, 총괄 승인) — 「어느 뷰가 어느 `target_field`의 후보 원천인가」를 클라가 **스스로 유도하지 않게 하는 유일한 길**이고, 노출되는 컬럼명은 참조뷰 응답 헤더에 이미 나타나므로 **신규 노출 0**이다(쿼리 본문·`limit`은 그대로 비노출). **[`1fefd12`] `queue_filters` 동봉** — "큐 항목"의 **서버 단일 정의**: 전 decision_key `notBlank` AND 전 target_field `blank`(제네릭 `/tables/{t}/data` 필터 DSL 형태). 판단키가 빈 행은 사람이 해소할 수 없으므로 워크리스트에 절대 안 뜬다 — 워크리스트(`enrichment.js`)·어드민 결손 카운트(`admin.js`)·메인 그리드 배지(`ui.js`) 3소비처가 **같은 객체**를 소비해 수치가 어긋날 수 없다. **[2026-07-30] 소비처가 4번째로 늘었다** — `enrichment_analysis._queue_condition`이 이 `queue_filters`를 받아 `main.get_column_filter_condition`으로 번역하므로 **리포트도 같은 정의를 쓴다**([§5-A](#5-a-2026-07-30-신설-서버-모듈-8종)) | **~614**<br>⚰️ 🆕㉗ **[S-184 `4cdede18`, 판정 295] `claim_contract` 은퇴 — «거절이 아니라 삼킴»이고 «이름을 댄다»** (`RETIRED_CLAIM_CONTRACT_NOTE` :590, 삼키는 자리 :743~:747). 🔴 거절했으면 그 칸을 든 «기존 선언»이 로드되지 못해 인리치 전체가 선다. 조용히 지웠으면 운영자가 «왜 안 도는지» 알 길이 없다. 그래서 이름을 대어 «수집기»에 남기고 규칙은 계속 돈다. 📌 뜻: 인리치 결과는 «표»에 쓰고 그 표를 원장이 읽는다 — 상설 「표에 원천 데이터를 넣고 그걸로 원장」 그대로 |
| 🆕㉕ ↳ 합성기 ~~:878~:945~~ 🆕 **:881~:1003** @`474f1aa9` — `AUTO_CONFIRM_MAPPER` :881 · `DEDUP_PREFIX` :886 · `AUTO_CONFIRM_PREFIX` :887 · `synthesized_rule_names(rule_name)` :890 | **인리치 규칙 하나가 «체인 규칙 둘»을 낸다**(S-179 ①, 판정 292): dedup 과 auto-confirm. 두 이름을 «한 자리»가 짓고 충돌 검사도 그것을 읽는다 — 「합성된 이름이 어떻게 생겼나」에 대해 거절과 합성이 «어긋날 수» 없다. 자동 확정은 `follow_up` 이라 커밋 경로가 아니라 «뒤따르기»에서 돈다. ⚠️ **[2026-09-12 재측] 줄 번호가 밀렸다** — D-11 은 :992~ 라 적었고, 오늘 `git grep` 은 위 넷이다 | — |

### `server/enrichment/mapper.py` (~177줄) — 인리치먼트 dedup 맵퍼
| 시그니처 | 역할 | 라인 |
|---|---|---|
...
### `server/ingestion/activity.py` (~149줄) — [P1 신규] 인제션 진행 스냅샷 레지스트리
...
> - ⚠️ **`map_overlay.py:953`과 `maps/preset_routing.py:90`이 각자 `STATUS_NOT_DECLARED = "not_declared"`를 따로 선언한다** — 같은 리터럴이지만 **import한 것이 아니라 우연히 일치하는 별개 어휘**다. 이 파일의 상수를 고쳐도 그 둘은 따라오지 않는다.
...
### `server/ingestion/checkpoint.py` (🆕⑤ **587줄** @`831ab68` — 종전 등재 ~258에서 **+329**) — [P2] 오프셋 체크포인트 + 2-tier 재처리 방지 원장

> 🆕⑤ 🔴 **[2026-08-13 실측] 이 표에서 라인 번호를 전부 걷어냈다** — 위치는 `git grep -n "<심볼>" -- server/ingestion/checkpoint.py`로 확정하라. 낡은 라인은 **실재하는 다른 함수**를 가리키며 멀쩡해 보이고, 낡은 심볼은 Grep 0건으로 자기가 낡았다고 말한다.
...
### `server/map_overlay.py` (**2,744줄**(🆕⑨ `5359fdd` 실측 — 구 표기 2,712) @`68db020` — 직전 등재 2,526에서 **+186**) — [M2 신규] 범용 맵 오버레이 (계획 전용 아님 — 맵 인프라)
...
>
> 🆕 **확정 마커 키**: **`PHYS_CONFIRMED_KEY`, `"phys_confirmed_from"`)** · **`FRAME_CONFIRMED_KEY`, `"frame_confirmed_from"`)**. 둘에 실리는 값은 `{table, map_id, confirmation_uid, confirmed_by, confirmed_at}`이고 **조성 지점은 `maps/frame_confirmation.py`**다. 클라 짝은 [`map2/declaration.js`](#7-a--map-editor-2--map_editor2html--client2srcmap2-2026-08-0506-신설)의 동명 키.
>
...
| `resolve_binding(cfg, table) -> dict\|None` | 🔴 **[`68db020`] `resolve_binding_parts`에 위임하는 얇은 래퍼로 강등** — 반환 계약(바인딩 dict 또는 None) 무변경. ⚰️ [M3] 종전 `MapMetaCollector`의 「이 테이블이 맵인가」 게이트이기도 했다(S-38 에서 그 호출자가 사라졌다) |
...
### 🆕 `server/maps/frame_confirmation.py` — 확정의 **기록자**
...
### `server/map_meta_registrar.py` (**102줄**(🆕 `S-38` 실측 — 구 379), `b697d34` 신설) — `wafer_map_metadata` 신원 + 마스크 중립 합성 프레임
⚰️ **[2026-09-07 · S-38, 소유자 지시 「맵 메타 자동 등록은 없애줘」] 이 모듈의 «쓰는 쪽»이 은퇴했다.**
사라진 것: `MapMetaCollector`(collect/pending/flush/_remember) · `auto_register_enabled` ·
`_load_ingestion_settings` · `reset_known_cache` · `SOURCE_NAME="auto_map_meta"` ·
`SETTINGS_KEY`/`DEFAULT_ENABLED`/`INGESTION_SETTINGS_PATH`/`CHUNK_SIZE`/`_KNOWN_CACHE_MAX` ·
배선 둘(`directory_watcher._send_to_upsert` · `chain_ingestion_worker.process_chain_transaction_group`) ·
`ingestion_settings.json.sample` 의 `auto_register_map_meta` 와 그 `_doc`.
사유는 [INGESTION_GUIDE §1.10](../guide/INGESTION_GUIDE.md) 이 정본이다 — 합성 행이 `grid_start_x/y` 를
배치 min 으로 박아 **원점을 고정**했고 에디터가 되돌리지 못했다.

🔴 **남은 넷은 쓰는 일과 «무관하게» 살아 있다:**

| 시그니처 | 역할 | 라인 |
|---|---|---|
| `META_TABLE = "wafer_map_metadata"` | 메타 업서트 규칙(`allow_map_metadata_upsert`, worker ~426·~821·~913)·`chain_replay`·`frame_confirmation` 이 이 이름을 가리킨다 | ~28 |
| **`compose_map_id(key_columns, row, table_name=None)`** | 행 → 맵 키 문자열. **정규화는 `map_overlay.canonical_bind_value`** — 선언 컬럼 타입이 지배한다. 키 성분이 하나라도 없으면 **None**(정체성을 지어내지 않는다). 소비: `dt_map_derivation` · 씨앗 스크립트 넷 | ~31 |
| `meta_business_key(target_table, map_id)` | 메타 행의 business key — **THE 철자**. `frame_confirmation` 과 매퍼 샘플 셋이 같이 쓴다(신원 두 철자는 「행은 있는데 못 찾는다」가 된다) | ~55 |
| **`synthesize_grid_meta(min_x, min_y, max_x, max_y) -> dict`** | 마스크 중립 합성 프레임. 반환 키 14종(`grid_cols/rows`·`grid_start_x/y`·`grid_y_invert`·`rotation`·`side`·`phys_*`·**`auto_registered: true`**). 지름은 `max(300, ceil(2*(half_diag+4)))`. 🔴 **은퇴 «밖»이다** — `map_alignment.assumed_meta_for_unregistered`(~488)가 부르고, 그 파일 주석(~433)이 「세 번째 프레임 합성기를 만들지 않는다」고 못 박았다. 재는 자리이지 «쓰는» 자리가 아니다 | ~78 |

> ⚠️ **표지 `auto_registered` 와 그 독자는 «전부» 그대로다** — 이미 쓰인 행이 들고 있는 기록
> 어휘이고, 지우면 그 행들이 「declared」로 거짓이 된다. 독자: `map_overlay.geometry_declaration` ·
> `map_alignment`(참조 거절·차용·[D8] 떼기) · `models` 토큰 · `enrichment_candidates` ·
> 클라 `map2/declaration.js` 세 갈래 · `map_editor.js`.
> 테스트: `tests/test_map_meta_registrar.py`(**4건** — 은퇴 관문 둘 + 남은 둘. 은퇴 관문은
> 두 배선 자리에서 「메타 행 0 · 데이터는 들어옴」을 재고, 배선이 돌아오면 빨개진다).
...
### `server/transfer_plan.py` (**3,731줄**(🆕⑨ `5359fdd` 실측 — 구 표기 3,826), `ed9cfdb` 3,450에서 **+376**) — [M2] Universal Transfer Plan 엔진 (v2 = 계획 정체성이 곧 맵 정체성, **`b35bc9f` zone 모델 · `2baf9ff` U9 marker · `1fefd12` count_only/column_unresolved 강등 · `deed6d2` self-frame fail count_only · `b697d34` 키 정규화 위임 + 7c 선언된 untracked**)
...
> - 어휘의 정본은 [`bonding_plan.py`](#5-소형-서버-모듈)이고 이 파일은 **import한다**(**~117**: `from bonding_plan import STATUS_NOT_DECLARED, role_is_declared`). 🔴 **두 번째 철자를 만들지 않은 것이 요점이다** — 같은 리터럴을 각자 선언한 `map_overlay.py`·`maps/preset_routing.py`가 그 반례로 옆에 있다.
...
**한 세션에 서버 파일 8개가 새로 들어왔다.** 🪦 **[2026-08-16] 그중 `graph_orphans.py`가 삭제돼 이제 «7종»이다**(아래 취소선 행).

🆕⑨ **[2026-08-18 전건 재합산 — `5359fdd`]** 살아 있는 일곱의 실측 합은 **4,644줄**이다: `value_suggest` **1,166** + `map_preset_routing` **536** + `chain_replay` **1,111** + `enrichment_analysis` **706** + `enrichment_candidates` **954** + `keyset_scan` **83** + `utils/time_format` **88**. 🔴 **종전 표기 「4,094」는 낡았고 «지워진 파일 481줄을 포함»하고 있었다** — 그래서 이 패스는 빼기만 하지 않고 일곱을 전부 다시 셌다. ⚠️ **이 재합산은 «줄 수»만 잰 것이고, 아래 각 행의 심볼 표·앵커는 `value_suggest.py` 행을 빼면 재측정하지 않았다** — 특히 `chain_replay`(947 → **1,111**) · `enrichment_analysis`(548 → **706**) · `enrichment_candidates`(613 → **954**) 세 파일은 크게 자랐으므로 **그 행의 `~NNN` 앵커는 전부 밀렸다고 가정하라.** 계보는 셋이다 — **인리치먼트/체인 재생**(4종) · ~~온톨로지 고아 스윕(1종)~~ · **입력 제안·프리셋 라우팅·시간 포맷**(3종).
...
>
> ⚠️ **blob 동일은 「지도가 이 파일을 옳게 적었다」가 아니다.** 그래서 그 패스는 무변동 일곱 개에 대해 **파일 끝 쪽에서** 표본을 뽑아 대조했다 — `value_suggest.classify_seek_plan` **1055** · `index_targets` **1093** · `_index_state` **993** / `enrichment_candidates.AutoConfirmCollector` **487** · `log_stats` **475** · `confirm_keys` **396**.
>
> 🆕⑨ 🔴 **[2026-08-18] 위 여섯 표본은 이제 전부 낡았다 — 지우지 않고 남기는 이유는 그것이 이 문서의 규율을 실증하기 때문이다.** 실측: `classify_seek_plan` 1055 → **1067** · `index_targets` 1093 → **1105** · `_index_state` 993 → **1005**. **끝 쪽 표본은 그때 맞았고 지금은 틀렸다** — 표본이 맞는다는 것은 「그 시점에 맞았다」는 뜻이지 「이 절이 앞으로도 맞는다」는 뜻이 아니다. 그래서 `value_suggest.py` 행의 라인 앵커는 **표본을 다시 뽑는 대신 통째로 걷어냈다**(아래).
>
...
>
> 🔴 **어휘를 새로 만들지 않았다.** 런타임 열화 어휘(`main.CHIP_TRACE_*`, [§1.5-bis](#15-그래프-조회-구간은-410으로-은퇴했다--그-일곱은-이제-없다))를 **그대로 재사용**한다 — 같은 구분이 config 로드 시점으로 한 층 올라온 것뿐이라 어휘가 갈라질 이유가 없다.
>
...
### `server/virtual_join/config.py` (🆕 **1,054줄** @`b1db471a` — 1,021 @`dc877746`, 999 @`474f1aa9`, 🆕⑨ 697 @`5359fdd`, 구 표기 687 · `4e06eec` 신설 540줄 → `b6942ec`로 **475**) — 가상 조인 선언 로더/검증

🆕 🔴 **[2026-09-15 D-38] 이 선언은 «그대로 둡니다».** 운영이 그것을 타고 있고, 이 라운드는 선언·실행기·읽기 경로 «무접촉»입니다. 쓰는 쪽은 새 `join` 종류(`chain/join_into.py`, `builtin:join_into` — §4-ter-c)가 맡고, **은퇴는 enrich·mapper 가 통합 선언으로 옮겨진 «뒤»**입니다. 정본 명제: `BASIS.md` §0-bis.

🆕 **[`90d971ba` S-248] `load_verified_rules(db, path=None, …)` :821(@`b1db471a` — 구 표기 :796) 이 검증을 마친 뒤 «제품 인덱스를 회수»한다** — `path is None` 일 때만 `unique_key.retract_unrequired_once(db, required)` :930 을 `try` 로 부른다(§4-ter-d). 🆕 **[`8cab58da` S-240] `required` 는 «두 생산자의 합»이다** — 읽기 시점 선언의 `{r["unique_index"] …}` **|** `chain_builtins.declared_unique_index_names(known_tables=known_tables)` :928(통합 join 선언이 «요구하는» `uq_vjoin_*`, 아래 `builtins.py`). 읽기 시점 선언만으로 계산한 요구 집합은 이 함수가 `path` 를 받았을 때 회수를 «거부하는» 바로 그 «부분 목록»이라 — 반쪽 요구 집합은 조금 덜 회수하는 게 아니라 «틀린 것»을 회수한다(웜업이 세운 인덱스를 다음 읽기가 걷어내고 다음 재기동이 다시 세우는, 「혼자 깜빡이는 스위치」). ⚠️ 그리고 그 반쪽을 «못 읽으면» 회수는 «안 돈다» — `declared_unique_index_names` 가 던지면 같은 `except` 가 «회수 전체»를 건너뛴다. ⚠️ **기본 선언 파일일 때만**인 이유: `path` 를 준 호출자(시험·보고서)는 «부분 목록»을 읽고 있고, 부분 목록에서 계산한 「요구됨」은 진짜 조인이 «아직 쓰는» 인덱스를 걷어낸다. ⛔ **적재를 «절대» 못 깨뜨린다** — 무엇이 나든 `warning` 한 줄 후 `verified` 를 돌려준다.


> 🆕㉘ **[S-189 ⓐ `446a7076`, 판정 302] 실체화가 «자기 예산»을 선언한다 — 기본값이 «없다».** `materialize` 는 true/false 만(:359~:367), true 면 `max_rewrite_rows` 를 «반드시» 적어야 하고 없으면 `CODE_NO_REWRITE_CAP = "no_rewrite_cap"` :124 로 «이름 대어» 거절하면서 **그 수를 세는 질의를 같이 인쇄한다**(일을 떠넘기지 않는다). 상한 검사는 :279. 🔴 **왜 기본값이 없나 — 수 때문이다**: 이 박스 실측으로 `dt_log` 606,215 행 중 한 `dt_job` 이 70,800 행을 덮으므로, 참조 행 «하나»가 바뀌면 70,800 행을 다시 쓴다 ≈ **92 s**(소유자 IO 규격 기준). 기본값을 두면 제품이 «한 번도 말한 적 없는 92 초짜리 쓰기»의 저자가 된다 — `occurred_at_basis` 와 같은 자세다. ⛔ **상한 초과는 «거절»이지 «잘라 쓰기»가 아니다** — 상한까지 쓰면 표가 반은 새 값 반은 옛 값이 되고 «어느 행이 어느 쪽인지 말해 주는 것이 없어» 화면이 조용히 틀린 답을 보인다. 🆕 `JOIN_MAPPER = "builtin:join"` :996(@`b1db471a` — 구 표기 :864) · `JOIN_PREFIX = "virtual_join:"` :997 · `synthesized_join_chain_rules(path=None, known_tables=None)` :1006
...
| **`required_index_name(table, columns, folds=None)` / `required_index_ddl(table, columns, folds=None)`** | **거부가 운영자에게 할 일을 준다.** 「UNIQUE 인덱스가 없다」만 말하고 어느 컬럼인지 말하지 않는 거부는 **행동할 수 없는 거부**다. `b6942ec`의 삭제 후에도 이 둘은 남았다. 🆕 이름은 §5-C `builtins.ensure_declared_unique_keys` 가 「한 선언 = 인덱스 하나」를 접는 «키»이기도 하다(S-240) | ~~**~136** / **~146**~~ 🆕 **:198 / :359** @`b1db471a` |
| 🆕 **`column_is_text(table, column) -> bool`** :220 · **`index_key_expression(column, fold_rules=None, table=None) -> str`** :242 | **[`ddd5b3ba` S-245] 인덱스 키 «한 칸»의 SQL 식 — 이제 «철자»는 `notation_norm.key_expression_text('"col"', fold_rules, text_column=column_is_text(table, column))` :272 «하나»에서 나오고, 이 함수가 «홀로» 정하는 것은 「그 컬럼이 텍스트인가」뿐이다(표를 든 호출자만 답할 수 있는 질문 — §5-E). `column_is_text` 는 `models.DYNAMIC_TABLES[table].<column>.type` 을 `notation_norm.is_text_type` :237 에 묻고, 🔴 **모르면 「텍스트」** — 「텍스트가 아니다」로 답하면 기존 인덱스에 없는 `::text` 가 식에 들어가 그 표의 «모든» 조인이 미승인(장애)이 되고, 「텍스트」로 답하면 숫자 키 «그 자리»만 어제만큼 깨진 채(현상 유지 · 시끄러움) 남는다. 호출자 «일곱», 전부 `table` 을 넘긴다: `rewrite_row_count` :291 · `required_index_ddl` :379 · `unique_index_covering` :786 · `executor._left_row_ids_for_key` :865 · `unique_key._expressions` :37 · `crud._stored_join_key_owners` :3953 · `migrations/add_vjoin_null_safe_indexes._duplicate_census` :57 | **:220 / :242** @`b1db471a` |
...
| **`load_verified_rules(db, path=None, known_tables=None, rejections=None)`** | **이 파일이 보장하는 것**: 여기서 돌려준 선언은 **오른쪽이 조인 키로 유일함이 데이터베이스에 의해 강제된다.** 스냅샷이 아니라 제약이므로 이후의 쓰기가 깰 수 없다.<br>🆕 **[`e88cb2be`, 2026-09-15 장애 다섯째] 선언 «하나»의 실패는 «그 규칙만» 거절한다** — `verify_uniqueness(db, rule)` :841(@`b1db471a` — 구 표기 :771) 호출이 `try` «밖»이어서 카탈로그 질의 하나가 던지면 «모든 표»의 조인이 같이 내려갔다(「모든 선언 무조건 다 돌면서 다 막아버렸네」). 이제 규칙마다 `try/except`: 던지면 `db.rollback()` → `_say_once(("verify", name))` → `_record(code=CODE_SHAPE)` → `continue`. 다음 규칙의 질의가 «abort 된 트랜잭션»의 답을 받지 않는다.<br>🆕 **[S-235 · 2026-09-15] 거절하기 «전»에 제품이 «한 번» 세워 본다** — `refused` 면 ① `unique_key.narrower_than_identity(right_table, right_columns, known_tables)` :867 — 키가 신원보다 «좁으면» `_say_once(("narrow", …))` :870 뒤 `_NarrowKey` 로 «만들어 보지 않는다» ② **`unique_key.ensure_once(db, name, right_table, right_columns, right_folds)` :877**(⚰️ 「`git grep` 전건에서 «유일한» 호출자」는 S-240 착지로 낡았다 — 둘째는 `chain/builtins.ensure_declared_unique_keys` :177, 아래 `builtins.py`) ③ `created` 면 `verify_uniqueness` :881 를 «다시» 묻는다. 그래도 `refused` 면 `_say_once(("unique", …))` :888 + `CODE_NO_UNIQUE_INDEX`(앵커 전부 @`b1db471a` — 구 표기 :839/:852/:863). ⚠️ **어느 세션이 오나** — `executor._verified_by_left_table` 은 이제 «자기 `SessionLocal()`» 을 넘긴다(§5-D). 읽는 쪽의 세션은 여기까지 «안 온다» | ~~**~401**~~ 🆕 **:821** @`b1db471a`(:796 @`474f1aa9`) |
| **`verification_report(db, path=None, known_tables=None) -> dict`** | `GET /admin/config/virtual-join/verify`의 본체([§1.4](#14-api-라우트-표--어드민운영그래프맵인리치먼트)) | ~~**~432**~~ 🆕 **:937** @`b1db471a` |
...
### 🆕㉘ `server/chain/builtins.py` (🆕 **332줄** @`b1db471a` — 177 @`dc877746`, 구 표기 171, S-189 ⓒ `1964c65a` 신설 · S-195 에서 120→171) — 합성 «한 자리» + `builtin:` 종류 표 + 🆕 «선언한 유일 키»를 세우는 껍데기(S-240)
...
| `class UnknownBuiltinKind(ValueError)` :69 · `BUILTIN_KINDS` :256 · `register_builtin(kind, fn)` :259 · `run_builtin(kind, db, rule, **kwargs)` :278 · `_install()` :316(@`b1db471a` — 구 표기 :138/:141/:151/:161) | **이 어휘가 «처음» 갖는 표다.** 🔴 **모르는 종류는 «이름 대어» 거절된다**(아는 목록을 같이 인쇄) — 구현이 «없는» 종류를 적은 규칙은 «켜진 채로 살아 보이면서» 한 번도 안 돌기 때문이다. 🔵 실측이 그것을 부른 자리: `builtin:auto_confirm` 은 그때까지 «정확히 두 번» 나왔고(둘 다 `enrichment_config`) **아무도 안 읽었다** — S-179 가 로더·그래프 층에는 그 종류를 선언해 두고 실행은 스윕에 남겨 뒀다.<br>🆕 **[`38b5d8e1` S-246] `run_builtin` 이 «등록»한다** — `with activity.running(name, kind, target_table, _rows_handed(kwargs), no_rows_reason="the rule wrote no rows") as run:` :302 안에서 `fn` 을 부르고, 결과 dict 의 `written` 이 «있을 때만» `run.produced(int(written))`(「안 셌다」와 「0 이었다」는 다른 사실). 🔴 종전엔 시작·결과·종료가 `mapper_call.execute_custom_mapper`(«파일 맵퍼»의 문) «안에만» 있어 `join_into`·`builtin:join`·`auto_confirm` 은 큐 화면에 «안 뜨고» 돌았고, 로더가 모든 규칙을 `never_evaluated` 로 «씨앗»하므로 천 번 돈 builtin 이 「아직 평가 안 됨」이었다(소유자 2026-09-15 「체인 대기열에서 안 뜨고 돌고 있었네」 — 「같은 기능에 두 경로」, 안 보이던 반쪽이 «돌던» 반쪽). ⚠️ 모르는 종류의 거절은 등록 «밖»이다 — 안 돈 것에 「돌았다」는 항목을 한 raise 만큼도 안 만든다. `_rows_handed(kwargs)` :269 — 두 트리거 팔이 행을 `row_ids` / `key_values` 로 «다르게» 부른다. `_run_auto_confirm` :90 이 `written: confirmed` 를 «덧붙여» 답한다(`join_into.run`·`materialize_rows` 가 이미 그 이름으로 답하고 있었고, S-249 의 후속 줄이 auto-confirm 에서 `written=None` 을 찍던 것). 시험 `test_a_rule_that_runs_is_in_the_queue_view_whichever_door_ran_it.py`(215줄 / `def test_` 12). §5-I `activity.running` |
| `_run_join(db, rule, row_ids=None, key_values=None)` :75 | `builtin:join` 의 실행 — 아래 `virtual_join_executor` 로 내려간다 |
| 🆕 **`ensure_declared_unique_keys(db, rules) -> {ensured, skipped}`** :143 · **`declared_unique_targets(rules)`** :181 · **`declared_unique_index_names(known_tables=None) -> set`** :223 | **[`8cab58da`·`07a568ad` S-240] 통합 join 이 «선언한» 유일 키를 «제품이» 적재 시점에 세운다.** 걷는 자는 `declared_unique_targets` «하나»: `mapper == join_into.JOIN_INTO_MAPPER` 인 규칙 중 `enabled` 가 아니면 `"enabled=false"` 로 건너뜀(⛔ OFF 는 «호출 0» — 판정 399 ③′, 09-14 를 내린 「찔러 보는 스위치」 금지) · `key.unique` 가 없으면 «조용히» 없음(묻지 않은 질문에 「아니오」가 아니다 — 행 단위 그물은 그대로) · `join_into.right_key(rule)` :204 로 (표, 컬럼, 접기) · 🆕 **[`07a568ad`] `key.columns` 는 «선택»이 아니라 «검사»다** — 적혀 있는데 그 조인의 오른쪽 키와 «다르면» 이름 대어 건너뜀(인덱스는 조인의 «자기» 키 위에만 서야 PostgreSQL 이 쓴다(S-181) — 오타가 「아무도 안 보는 칸」이 아니라 «문장»이 되게, 그것이 이 라운드의 결함 «전부»였다). `ensure_declared_unique_keys` 는 대상마다 `vjc.required_index_name(table, columns, folds)` 으로 «한 선언 = 규칙 둘(`:reference` 동반자) = 인덱스 하나»를 접고(`seen`) `unique_key.ensure_once(db, name, table, columns, folds)` :177 을 «한 번» 부른다(두 번 물으면 리로드마다 `pg_index` 를 두 번 찌르고 인덱스 하나를 둘로 센다). `declared_unique_index_names` 는 `ingestion_worker.read_rules_document()` 를 `rule_shape.expand_declaration(raw, catalogue)` :243 — **로더와 «같은 판정자»**(S-244) — 로 펴서 «요구되는» `uq_vjoin_*` 이름 집합을 낸다(거절은 «다시 말하지 않는다» — 로더가 이미 말했고 여기서 또 말하면 읽기 경로에 몇 초마다 거절이 뜬다, 09-14 의 홍수). 🔴 **왜 걷는 자가 하나인가** — 「무엇을 세우나」와 「무엇을 요구하나」가 갈라지는 날 제품은 웜업에 세우고 다음 읽기에 회수한다, 영원히. 호출자: `ingestion_worker.warmup_worker` 0-bis :2006(§4) · `virtual_join/config.load_verified_rules` :928(위). ⚠️ `join_into` 가 «아니라» 이 껍데기가 세우는 이유 — 그 모듈은 `virtual_join` 을 import 못 한다(자기 경계 시험), 그래서 오른쪽 표·컬럼·접기를 «돌려주고» 두 반쪽을 «이미 아는» 이 자리가 세운다. 시험 `test_a_declared_unique_key_is_made_by_the_product.py`(334줄 / `def test_` 18) |
...
📐 **[S-195 ⓐ `c4aeb56c`] 디스패처가 «드레인 배치마다 규칙 파일을 다시 읽는» 것을 그만뒀다** — `chain_ingestion_worker._followup_builtin_rules()` :1935(@`dc877746` — 구 표기 :1682) 가 «리로드당 한 번» 싣고 `_FOLLOWUP_BUILTIN_RULES` :1932 에 든다. 🔴 **무효화는 «다른 워커 캐시와 같은 자리»다**(`reload_worker_process_cache` :1947) — **리셋 없는 캐시는 「리로드가 아무 뜻도 없게 되는」 이유**이고, 이 프로세스에는 그 자리가 이미 하나 있다.

### 🆕㉘ `server/admin/schema_drift.py` (**864줄**, S-193 `98eee71f`) — 기동이 「없는 표」라 부르던 것 중 열이 «뷰»였다
...
🆕㉚ **[S-199-d `75a04c3e`] `QUIET_SEVERITIES = ("SELF-HEALING", "INFO")` 가 «모듈 상수»다** — 배너와 시험이 «한 이름»을 읽는다. 🔴 종전처럼 양쪽이 각자 「무엇이 조용한가」를 적으면 **시험이 조용하다고 보는 것과 배너가 조용히 넘기는 것이 갈릴 수 있고**, 갈린 쪽이 조용하다 — 이 파일이 존재하는 이유가 바로 그 부류(상설 오류 줄)다.

### 🆕㉚ `server/tests/conftest.py` — 스위트 전체가 «지나는» 자리 (2026-09-12 S-191·S-199-b·S-200)

> 📐 **줄 번호를 안 적는다** — 이 파일은 09-12 하루에 세 번 자랐고 `retire_dynamic_model` 이 :75 → :147 로
> 밀렸다(이 문서가 하루 전 :75 를 적었다). 이름으로 grep 한다.

| 심볼 | 무엇인가 |
|---|---|
| `_the_model_registries_come_back()` (autouse, S-200 `47401217`) | **시험마다 «건네받은» 레지스트리를 돌려받는다** — 앞 시험이 무엇을 남겼든. 🔴 **파일 «예순다섯»이 각자 기억할 수는 없다**(제 실측 2026-09-12: `init_dynamic_models` 를 부르는 시험 파일 **74** — 독스트링과 일치. 되빼는 쪽은 `git grep -l "retire_dynamic_model\|DYNAMIC_TABLES.pop" -- server/tests` 가 **10** 이고 그중 하나가 `conftest.py` 자신이라, 독스트링의 «아홉»은 「시험 파일」 모집단에서 맞다) — 그래서 `DYNAMIC_TABLES` 와 `Base.metadata` 가 «런 내내» 자라고, 대가는 «그것을 세는 파일»에 떨어진다(샌 쪽도 누수도 이름 대지 않고). 🔵 실물: `test_the_shipped_catalogue_builds` 가 46 표 카탈로그에 대해 **63** 을 읽었고 **혼자 돌리면 초록**이었다. ⛔ **더해진 것은 «지우고», 빼진 것은 «되돌리지 않는다»** — 기준선에서 이름을 은퇴시킨 시험은 «그러려고» 한 것이라, 여기서 되살리면 «옳은 teardown»을 무르는 것이다. 새는 것은 «더해진 것»이다. ⚠️ 다만 **클래스가 «바뀐» 이름은 되돌린다** — 대역이 프로세스 전역 레지스트리에 남으면 그 뒤 «모든 파일»에서 진짜 모델을 대신 답한다 |
| `requires_live(shape)` · `LIVE_TESTS_ENV = "ASSY_TEST_LIVE"` (S-199-b `8aade672`, 판정 309) | **판정의 «주어»가 gitignore 된 라이브 파일인 시험은 «이름 대어» skip 한다.** 🔴 그 시험도 라이브 파일도 «틀린 게 아니다» — 답이 «한 기계에 대한 사실»이라 여기의 빨강은 운영에 대해 아무 말도 안 한다. 기본으로 돌리면 「스위트가 초록이다」가 «누구의 체크아웃인가»에 달린다. ⛔ 사유가 «요구되는 모양»을 든다 — 「스킵됐다」만 말하는 스킵은 독자에게 그것을 무시하라고 가르친다. 📎 [HARNESS_DISCIPLINE_GUIDE §5-ter](../guide/HARNESS_DISCIPLINE_GUIDE.md) |
| `retire_dynamic_model(name)` (S-191 `1f730cc9`, 판정 307) | 동적 표를 «두 싱글턴»에서 같이 뺀다. 🔴 pop 만 하면 `Table`·`Index` 가 살아남아 **세 파일 건너**에서 「index … already exists」로 터진다(실측 1,008 오류). ⛔ 「저장했다 되돌리기」 세 좌석은 «예외»이고 소스가 이름 대어 적는다. 📎 위 가이드 §5-bis |

🔵 **이 셋이 «짝»이다**: `retire_dynamic_model` 은 시험이 «일부러» 부르는 것, autouse 픽스처는
«아무튼» 일어나는 것 — 같은 두 싱글턴, 같은 짝. 그리고 그 효과가 수로 나왔다:
**전 스위트 18분 → 12분**(S-200 커밋 본문, 이 박스).
📌 게이트가 «집합 형태»다 — `test_the_shipped_catalogue_builds` 가 «수»가 아니라 카탈로그와의 «집합»을 견준다.
수는 63 과 46 처럼 «어디서 왔는지 모르는 차»를 내고, 집합은 «어느 이름이 남았나»를 낸다.

### 🆕㉚ `server/notebooks/` — 워크벤치 노트북 둘 (2026-09-12 S-197 ⓑ·ⓒ·ⓓ)

| 파일 | 무엇인가 |
|---|---|
| `mapper_workbench.ipynb` (셀 13) · `parser_workbench.ipynb` (셀 15) | 고치는 셀은 «하나». 부품을 새로 만들지 않고 `dev_bench` 를 부른다 — `server/tests/test_the_workbench_notebooks_call_production.py` 가 그것을 채점한다 |
| `README.md` | 그 둘의 입구 |
| ⚰️ ~~`server/parsers/notebooks/parser_workbench.ipynb`~~ | **[S-197-d `4519a2df`] 은퇴했다 — 디렉터리째 없다.** 🔴 **그 전신은 «미추적이 아니라 추적»돼 있었다**(`2a6adb50`, 08-19) — 09-12 에 총괄과 제가 그 한 줄을 «두 번» 뒤집었고, 갈랐던 것은 도구다: `git check-ignore` 는 「ignore 규칙에 걸리나」를 답하지 「추적되나」를 «답하지 않는다»(추적 파일도 패턴에 걸린다). 추적 여부의 철자는 `git ls-files --error-unmatch` 다 |
...
| `reset_cache()` / `_verified_by_left_table(db)` / `rules_for(db, left_table)` · `rules_for_right(db, right_table)` | 캐시 무효화(호출자 `main.reload_local_process_cache`) / TTL 캐시 + `left_table`·`right_table` 버킷팅 — **예외는 빈 dict로 삼킨다**(선언을 못 읽으면 조인이 없는 것이지 요청이 죽는 것이 아니다) / 해당 테이블 규칙 사본.<br>🆕 🔴 **[`e88cb2be`, 2026-09-15 장애 다섯째] `_verified_by_left_table` 은 «읽는 쪽의 세션»으로 검증하지 않는다** — 캐시 미스마다 «모든» 선언의 카탈로그 SQL(그리고 어제부터 인덱스 프로브)이 «캐시를 놓친 사람»의 세션에서 돌았고, 표 B 의 선언 하나가 던지면 표 A 를 읽던 트랜잭션이 abort 된 채 «아무도 롤백하지 않았다»(「내가 건드리던 테이블과 완전 다른 건데 왜 튀어나와서 막았던 거야」). 이제 `from database.database import SessionLocal; own = SessionLocal()` 로 «자기 세션»을 만들어 `vjc.load_verified_rules(own, …)` 에 넘기고 `finally` 에서 `own.rollback(); own.close()`. **인자 `db` 는 이 함수 안에서 «더는 DB 에 안 닿는다»** — 읽는 쪽은 «메모리만» 읽는다. ⚠️ 시그니처는 그대로(`db` 를 여전히 받는다) — 호출자 `rules_for`/`rules_for_right` 가 넘기는 것을 «안 쓴다» | ~~~91/97/126~~ 🆕 **:109 / :116 / :162 · :167** @`474f1aa9` |
...
### `server/admin/retroactive.py` (🆕 **1,609줄** @`dc877746` — 🆕⑳ 1,502 @`328a5c20` — 🆕⑱ 1,441 @`6345ef3d` — 🆕⑰ 1,156 @`64b562b6` — 🆕⑯ 1,139 @`9eb30691` — 🆕⑨ 545 @`5359fdd`, 구 표기 697) — 소급 적용의 **레지스트리**

> 🟢 🆕⑯ **[2026-08-31 전량 재측정 — `9eb30691` 커밋 blob]** 최상위 심볼 전건 + `OPERATIONS` 항목 전건 + 실행 행 계열 전량. 🔴 **라인 번호는 싣지 않는다** — 위치는 `git grep -n "<심볼>" -- <경로>`로 확정하라.

> 🟢 🆕⑱ **[2026-09-05 최상위 심볼 전건 재측정 — `6345ef3d`] «큐 뷰 + 실행 신원» 절이 통째로 들어왔다**(+285줄). 아래 심볼들은 🆕⑯ 의 목록에 «하나도 없던» 것이다:
> ```
> 큐 뷰      queue_view(db, now=None)                    · PICKER_INTERVAL_SECONDS = 5
> 움직임     MOVING_PROGRESSING · MOVING_STALLED · MOVING_UNREPORTED
> 취소 가능성  CANCEL_AT_NEXT_BATCH · CANCEL_UNKNOWN · CANCEL_NEVER
> 비행 중     IN_FLIGHT_STATES = (RUN_RUNNING, RUN_CANCEL_REQUESTED)
>            in_flight(db, now=None, stall_after=None)
> 신원       runner_identity() -> str   ·   _runner_state(runner) -> "owned"|"orphaned"|"unknown"
> ```
> 🔴 **`runner_identity()` 가 돌려주는 것은 «세 조각»이다 — `name/host/pid`.** ⚠️ **소스 docstring 의 요약 줄은 「as `host/pid`」라 적어 «둘»로 말한다 — 그 줄이 자기 본문과 어긋난다**(본문은 `"%s/%s/%d" % (name, gethostname(), getpid())`). 짝인 `_runner_state` 가 `len(parts) != 3` 으로 가르므로 **셋이 맞다.** 코드 소관이라 여기서 못 고친다 — **docstring 을 형식의 출처로 쓰지 마라.**
> 🔴 **`name` 은 «하트비트 이름»이고 `heartbeat.own_name()` 에서 온다**([§5 `heartbeat.py`](#5-소형-서버-모듈)). 🔴 **그것이 «호스트가 더는 중요하지 않게 된» 이유다** — `host/pid` 만 물으면 다른 기계에서 찍힌 실행은 여기서 영원히 `unknown` 이지만, 이름은 «하트비트를 읽을 수 있는 아무 데서나» 답해진다.
> 🔴 **«import 시점이 아니라 실행이 시작될 때» 읽는다** — fork 하거나 제자리 재실행된 프로세스가 「이 모듈을 처음 import 한 것」의 신원을 찍으면, **상속될 수 있는 신원은 없느니만 못하다**(구체적으로 보이기 때문에).
> ⛔ **아무것도 이 값으로 «회수»하지 않는다.** `_runner_state` 는 `owned`/`orphaned`/`unknown` 을 «말할» 뿐이고, 이 호스트가 아닌 pid 는 여기서 죽었다고 «부를 수 없다» — 그렇게 부르는 것이 「영원히 안 끝난다」를 「둘이 동시에 끝난다」로 바꾸는 길이다. 도장이 아예 없는 행은 컬럼 이전이라 **`unknown` 이고 `orphaned` 가 아니다.**
> **소비자**: `main.py` 의 어드민 큐 라우트가 `retroactive.in_flight(db)`(→ `scheduler_bucket["blocked_by"]`) 와 `retroactive.queue_view(db)`(→ `scheduler_bucket["queue"]`) 를 부른다.
> ⚠️ **이 패스는 위 심볼들의 «존재와 시그니처»만 쟀다** — `queue_view`·`in_flight` 의 본체와 `OPERATIONS` 표는 열지 않았다. 아래 🆕⑯ 서술은 그 기준 그대로다.

> 🔴 🆕⑯ **[정정] `graph_orphans` 는 «없는 op 다».** `server/` 전건 grep **0건**. 종전 이 절은 그것을 다섯 중 하나로, `_count_graph_orphans`/`_run_graph_orphans` 를 함수로 싣고 있었다 — **앵커로 쓰지 마라.** 🪦 그 op 의 모듈(`server/graph_orphans.py`)은 2026-08-16 에 이미 삭제됐고([§0](#0-묘비-목록--소스에-존재하지-않는-이름)) 등록부 항목은 그보다 늦게 따라 나갔다. ⚠️ **이름은 클라 쪽에 «두 자리» 남아 있다** — `client2/src/retroactive_view.js` 의 산문 주석 한 줄(「five buttons … `graph_orphans` deletes」)과 `client2/tests/retroactive_view_harness.mjs` 의 **픽스처**다. 하니스 것은 「서버가 어떤 op 를 보내든 렌더한다」를 재는 합성 입력이라 정당하고, **산문 쪽이 낡았다**(코드 소관이라 여기서 못 고친다).

> 🔴 **이 파일은 아무것도 계산하지 않는다 — 이미 있는 «여섯» 개의 조작에 공통 표면을 씌운다.** 각 op는 `count`(드라이런 미리보기)와 `run`(실행)의 **함수 쌍**이고, 둘 다 기존 모듈을 부른다. **여기에 일곱 번째 집계 구현이 생기면 그것이 이 파일의 실패다.** ⚠️ 🆕⑯ **모듈 docstring 은 아직 「four existing operations」이고 넷만 나열한다** — 실재는 여섯이다. 코드 소관이라 여기서 못 고친다. **docstring 을 op 목록의 출처로 쓰지 마라.**

- 상수: `RUN_EVENT_TYPE = event_constants.EVENT_RETROACTIVE_RUN` · 🆕⑳ **`RUN_EVENT_TABLE = event_constants.RETROACTIVE_RUN_TABLE`**(값은 `"__retroactive__"` 그대로이고 바뀐 것은 «누가 소유하나»다 — 읽는 쪽이 「이 이름은 표가 아니다」를 물을 곳이 생겼으므로 리터럴이 참조가 됐다. [§6 `event_constants.py`](#6-기타-서버-모듈-한줄-요약)) · `COUNT_EXACT`/`COUNT_SAMPLE`/`COUNT_UPPER_BOUND` · `DEFAULT_SCAN_LIMIT=200`/`MAX_SCAN_LIMIT=2000` · `class RetroactiveRefused(Exception)`.
...
| `enrichment_backfill` | — | number | ✅ | ✅ | 소스 청크마다 |
| `enrichment_confirm` | — | number | ❌ | ✅ | 쓰기 청크마다 |

  - 🆕⑯ 🔴 **`ledger_rescope` 의 `cancellable: False` 와 「커밋 둘」은 같은 사실이다** — 회수와 재작성 사이에 죽으면 원자는 빠지고 아직 안 쓰인 상태로 남는다. **같은 범위를 다시 돌리면 끝난다**(소스가 2026-08-31 에 실제로 그렇게 됐다고 적어 둔다).
  - 🆕⑯ **`ledger_backfill` 이 소유자가 이름 댄 그것이다** — 「백필 돌리다 서버 렉먹는데 백필만 못꺼서 서버 재기동」. 페이지마다 커밋하고 커서에서 이어받으므로 **페이지 «사이»에서 멈추는 것이 공짜**다.
- 🔴 **`count_kind`가 세 값인 것이 이 표면의 정직성 계약이다**: `exact` / `sample`(스캔 상한까지만 봤다) / `upper_bound`(집계 두 개로 답했고 행을 걷지 않았다). `_count_withdraw`는 `scan_limit`을 **의도적으로 무시**하고 `scanned=None`·`scan_limit=None`을 돌려준다 — 걷지 않은 것을 "N행 스캔"이라고 말하지 않기 위해서다.
- 카운터 **6종**: `_count_chain_replay`(🆕 **[`bd0a3db7` S-242] `builtin:` 종류면 «행»을 센다** — `s["builtin_kind"]` 가 참이면 `affected = s["mapper_items"]` · `affected_label = "다시 계산할 행"`, 아니면 `cells_proposed` · `"덮어쓸 셀"`. builtin 은 «자기가 쓰므로» 제안 셀이 0 이라 종전 동의 화면이 «모든 대상 행을 다시 쓰는» 소급 앞에서 「0 셀」이라 말했다 — 행을 「셀」이라 부르지 않고 «자기 단위·자기 라벨»로) · `_count_withdraw` · `_count_enrichment_backfill` · `_count_enrichment_confirm`(**노브가 꺼져 있어도 측정은 되게** `ignore_knob=True`로 드라이런하고 `blocked_reason="auto_confirm_off"`를 세운다) · 🆕⑯ `_count_ledger_backfill`(**`backfill.rows_past_cursor` 를 부른다** — `extra` 에 `{source, rows_past_cursor, is_all}`) · 🆕⑯ `_count_ledger_rescope`(도우미 `_rescope_absence(withdraw, remake, rows)` 가 «회수/재작성» 두 수에서 부재 낱말을 고른다).
- 실행기 **6종**: `_run_chain_replay` · `_run_withdraw` · `_run_enrichment_backfill` · `_run_enrichment_confirm`(**여기서는 노브가 동의 게이트라 `ignore_knob=False`**) · 🆕⑯ `_run_ledger_backfill` · 🆕⑯ `_run_ledger_rescope`. 🆕⑯ 실행기 넷은 이제 `control=None` **넷째 인자**를 받는다.
  - ⚠️ **`_run_chain_replay`는 `limit`을 넘기지 않는다** — 카운트는 표본이고 실행은 무제한이다. 미리보기의 수가 실행의 수와 같다고 읽으면 안 된다.
  - 🆕⑯ 도우미 둘: `_checkpoint(control)`(→ 연산에 넘길 «멈출까요» 콜백. `control` 이 없으면 `None`) · `_final_progress(control, rows)`.
- 🆕⑯ **실행 «행» 계열 — 이 라운드의 신설 절반.** 상태 여섯 `RUN_QUEUED`/`RUN_RUNNING`/`RUN_DONE`/`RUN_CANCEL_REQUESTED`/`RUN_CANCELLED`/`RUN_FAILED`. 🔴 **`cancel_requested` 는 «요청»이지 결과가 아니다** — 세워진 시점에 연산은 아직 돌고 있고, 배치 사이에서 실제로 멈춘 뒤에야 `cancelled` 가 된다. **둘을 접으면 아직 쓰는 중인 실행이 끝난 것처럼 보인다.**
  - **`class RunControl(run_id, session_factory=None)`** — `stop_requested()`(🔴 **끈적하다(sticky)**: 한 번 True 면 다시 묻지 않는다. 여러 자리에서 확인하는 연산이 True 뒤에 False 를 받고 계속하는 일이 없게) · `progress(processed=None, total=None)`(`total=None` 은 **NULL 로 남는다 — 모름이지 0 이 아니다**). 🔴 **자기 세션을 «일부러» 쓴다** — 취소 플래그는 «다른 프로세스»의 웹 요청이 세우는데 연산은 자기 긴 트랜잭션 안에 있어서, 연산 세션으로 읽으면 그 스냅샷이라 **플래그를 영원히 못 본다**. 진행 기록을 연산 세션에 쓰는 것은 그 거울상 실패다(배치 롤백이 「어디까지 갔나」를 지운다). **묻기가 실패하면 「멈추라」로 읽지 않는다**(False 를 답한다).
  - **`request_cancel(db, run_id) -> dict`** — 값만 세운다. **끝난 실행은 이름으로 거절**(`RetroactiveRefused`).
  - **`runs(db, limit=50) -> list`** — 최신순, `limit` 은 1..500 으로 클램프. ⚠️ **파일 인제션은 여기 «없고 옮기지 않는다»** — `file_ingestion_checkpoints` 가 이미 파일마다 total/processed/chunk 를 들고 있다.
  - **`_mark_run(run_id, *, state, started=False, finished=False, result=None, error=None)`** — 🔴 **자기 세션.** 「실행이 실패했다」가 바로 그 행이 연산과 함께 롤백되면 안 되는 순간이다. `error` 는 2000자로 자른다. 🆕⑱ **`started=True` 인 경로에서 `values["runner"] = runner_identity()` 를 «같이» 쓴다** — 시작 시각과 신원이 한 갱신에 들어간다.
  - 저장은 **`models.RetroactiveRun`**(테이블 `retroactive_runs` — [§5 소형 서버 모듈](#5-소형-서버-모듈)의 `server/database/models.py` 절) — 🔴 **연산마다 표를 만들지 않는다**(등록부가 연산을 «값»으로 다루므로 실행 기록도 값이어야 한다. 표를 나누면 새 op 가 «항목 하나»가 아니라 «마이그레이션»이 된다).
- 표면: `operation(op)` · **`inventory()`(순수 config 투영, DB 접근 0 — 그래서 요청 경로에서 안전하다. 🆕⑯ 각 param 은 `_resolved_choices` 를 거쳐 나간다)** · `validate(op, params)`(미선언 param 이름을 **먼저** 거부 → 🆕⑯ **`choices` 밖의 값을 이름으로 거부**(편의다 — 안전 속성은 연산 자신의 재거절 쪽) → `withdraw`의 `source`가 `chain_replay.PROTECTED_SOURCES`면 거부. 🔴 **리터럴을 베끼지 않고 그 상수를 읽는다**) · `clamp_scan_limit(limit)` · **`count(db, op, params, scan_limit)`(`finally: db.rollback()` · `blocked_reason`/`absence`/`scan_limit` 세 칸을 `setdefault` 로 «항상» 채운다 — 「고르지 않은 것」과 「해당 없음」이 갈리게)** · **`publish(db, op, params, requested_by=None)`** · **`execute(payload, log)`**.
- 🔴 **`publish`와 `execute`가 갈린 것이 토폴로지 계약이다**: 웹 프로세스는 outbox 행 하나를 쓰고 `NOTIFY`만 한다(**아무것도 실행하지 않는다**), 실제 실행은 스케줄러 프로세스가 자기 데몬 스레드에서 한다([§6 `run_auto_update.py`](#6-기타-서버-모듈-한줄-요약)). `execute`는 **어떤 예외도 raise하지 않고** `status="error"`로 접는다 — 소급 실행 하나가 스케줄러 데몬을 죽이면 안 된다.
  - 🆕⑯ 🔴 **`publish` 는 outbox 행과 `RetroactiveRun` 행을 «같은 커밋»에 쓴다.** 이벤트만 있으면 아무도 볼 수도 취소할 수도 없는 일이 되고, 행만 있으면 영원히 `queued` 로 앉는다 — **반쪽이 둘 다 없는 것보다 나쁘다**(소스 주석).
  - 🆕⑯ 🔴 **`execute` 에서 「멈춤」과 「완료」는 다른 결과다.** `control.stopped` 면 `status="cancelled"` + `RUN_CANCELLED` 이고 `done` 이 아니다 — 취소된 실행은 쓴 것을 전부 커밋했고 **남은 일이 있다.**
- **호출자**: 🆕⑯ `main.py`의 라우트 **5종** · `run_auto_update.MultiDiscoveryScheduler.start_retroactive_run`. ⚠️ **`{op}/run` 만 `require_admin_token_strict` 이고 나머지 넷(`operations`·`{op}/count`·`runs`·`runs/{run_id}/cancel`)은 평범한 `require_admin_token` 이다.**
- **채점자**: 🆕⑯ `server/tests/test_retroactive_admin.py` **1,359줄 / `def test_` 50**.

### `server/enrichment/backfill.py` (**458줄**(🆕⑨ `5359fdd` 실측 — 구 표기 411), 신설) — 룰 도입 **이전** 소스 행의 파생 행 생성
...
> 🪦 **[2026-08-16] 둘 중 `graph_stale_edges`는 삭제됐습니다** — 이 절은 이제 **한 종**만 서술합니다(제목의 「2종」은 신설 당시의 수입니다). 아래 🪦 절 참조.

🆕㉚ 🔴 **[S-199-c `e77fab84`, 판정 309-b] 삼중이 «둘»이고 역할도 «둘»이다** — `dt_job/dt_x/dt_y` 는 «온톨로지 키»(원장 샘플의 `dt_transfer` 가 바인딩하는 것)이고, `dt_job_id/b_wx/b_wy` 는 «인제션 정체»다. 그래서 픽스처의 `dt_log` 가 **양쪽을 다 싣는다**. ⚠️ **총괄의 「샘플을 고쳐라」가 코드 안의 판정 213 과 모순이었고 코드가 그것을 잡았다**(판정 309-b) — 카탈로그가 `dt_inventory.business_key = dt_job_id` 를 `[dt_job]` 에서 조성하므로 «한 값, 두 이름»이다. 📌 부류: **판정을 고치기 전에 「이미 코드 안에 판정이 있나」를 본다** — 이 저장소의 상설 관문 그대로다.

### 🆕 `server/notation_norm.py` (🆕 **910줄** @`b1db471a` — 구 표기 808) — WF/lot/slot 표기 정규화 — 🔴 **읽기 시점의 접기이지 저장되는 파생 컬럼이 아니다** — 🆕 그리고 **키 식의 «한 저자»**(S-245)

> 🟢 **심볼 실측 완료** — 이 절의 심볼은 **`5609ff0`의 커밋된 blob에 존재함이 확인됐고, 그 뒤 HEAD가 움직인 다음 재대조에서도 동일했다**(워킹트리 아님). 🆕 **[2026-09-15] 아래 표의 S-245 행(키 식 다섯)은 `b1db471a` 커밋 blob 실측이다.** 🔴 **라인 번호는 싣지 않는다** — 위치는 `git grep -n "<심볼>" -- <경로>`로 확정하라. 숫자가 남아 있는 곳은 **파일 줄 수**이거나, 이름이 없는 덩어리를 가리키며 측정 sha가 함께 적힌 자리뿐이다.
...
| 🆕 🔴 **키 식 «한 저자, 두 철자»**(`ddd5b3ba` S-245) — `is_text_type(sa_type) -> bool` · `_install_text_cast_construct()` → `_TextCast`(PostgreSQL `x::text` / 그 밖 `CAST(x AS TEXT)`) · **`key_expression_sql(column, rules=None)`**(SQLAlchemy 요소 — 조인이 «비교하는» 것) · **`key_expression_text(inner_sql, rules=None, text_column=True) -> str`**(PostgreSQL 텍스트 — 인덱스가 «세워지는» 것) | 🔴 **접기는 키의 «반쪽»이다.** 조인이 비교하고 · 유일 인덱스가 서고 · 중복 프로브가 묶는 것은 접기가 아니라 `coalesce(fold(col), '')`, 그리고 09-15 부터 비텍스트 컬럼이면 `coalesce(fold(col::text), '')` 다. 그 세 조각이 «네 자리»(`config.index_key_expression`(DDL 과 그 위의 모든 프로브) · `executor.join_onclause`(읽기 시점 ON) · `join_into._folded`(쓰기 시점 조인) · 그리고 각자의 타입 판단)에 «따로» 철자돼 있었고, 그래서 한 `number` 키가 한 자리에선 타입 오류를 내고 다른 자리는 «이미» 캐스트했다. ⛔ **어긋남은 실패가 아니라 «조용함»이다** — 표현식 인덱스는 질의 식이 «일치»할 때만 쓰이므로 둘째 철자는 순차 스캔이고 시험은 초록이다(S-181). 그래서 «호출자당»이 아니라 «렌더링당» 하나다. 캐스트가 «먼저», «컬럼»에(접기 구성물은 String 을 선언하므로 뒤에 물으면 늘 「텍스트」). 텍스트 컬럼은 «바이트 동일» — 어제 선 인덱스가 오늘도 맞는 이유이고, 캐스트가 «조건부»인 이유의 전부. `::text` 철자인 이유: `config.normalize_index_expression` 이 `::text` 를 PostgreSQL 이 덧붙이는 잡음으로 «벗기므로» 캐스트 있는 식과 없는 식이 같은 정규형이 된다 — `CAST(... AS TEXT)` 는 안 벗겨져 접힌 조인 «전부»가 미승인이 됐을 것. SQLite(= 스위트)엔 `::` 가 없어 접기 구성물과 «같은 모양»(요소 하나, 방언 둘 — 스위트는 «배선»을, `contracts/` 는 실제 PostgreSQL 에서 «철자»를 채점). `key_expression_text` 의 `text_column=True` 기본값은 «옛 답» — 컬럼 타입을 못 푸는 호출자는 S-245 이전 식을 «그대로» 받는다(숫자 케이스가 그 자리에서 «안 고쳐진 채» 남는 쪽이, 텍스트 케이스가 모든 자리에서 «조용히» 모양을 바꾸는 쪽보다 낫다 — 후자는 장애, 전자는 현상 유지). 호출자(`git grep … HEAD`, 시험 제외): `chain/join_into._folded` :130(`_sql`) · `virtual_join/executor.join_onclause` :219(`_sql`) · `virtual_join/config.index_key_expression` :272(`_text` — 표를 든 «유일한» 자리, `column_is_text` :220 가 타입을 답한다, §5-C). ⚠️ `fold_notation_sql` 을 «직접» 부르는 자리가 둘 남아 있다 — `ledger/source_preparation.py` :259 · `mappers/core_usage_mapper.py.sample` :158 — 둘 다 «키 비교»가 아니라 값 접기라 이 규칙 밖이다. 시험 `test_one_key_expression_at_every_seat_that_compares_a_key.py`(225줄 / `def test_` 11) |
...
>
>
> 📌 **`server/config/sample/notation_rules.json.sample`(**48줄**)의 구조만 기술한다** — `server/config/*`는 gitignored라 라이브 값은 문서화 대상이 아니다. 최상위 키: `__comment` · `__how_to_enable` · **`rules`**(파일 수준 기본 규칙 집합) · **`tables`**(테이블별 override + 컬럼 선언).
...
- **진단 로그**: `_diag_index_block(per_candidate, out, ruling, source_indices, cell_owner, …)`가 순번 축 전용 블록을 쓴다(`_diag_scoring_block`과 나란히). 파일은 데이터 루트의 `align.log`(테스트는 **OS 임시 디렉터리의 `assy_align_test_<pid>.log`** — 2026-08-19. 종전엔 `server/align_test.log`였고, 회전 핸들러가 파일을 열어 둔 채 롤오버 때 이름을 바꾸므로 pytest 프로세스 둘이 같은 이름을 쓰면 `PermissionError: [WinError 32]`로 충돌했다), 회전 상한 `_DIAG_MAX_BYTES` · 백업 `_DIAG_BACKUPS` · 셀 표본 `_DIAG_INDEX_CELLS`. 빌드 신원은 `build_identity()`(`_git_sha`·`_feature_tokens`).
...
### `server/dt_map_derivation.py` (**940줄**(🆕⑨ `5359fdd` 실측 — 구 표기 849)) — 프레임 **어휘**와 DT 맵 유도
...
| 🆕㉗ **`routing_keys()`** :360 (S-188 ⓐ `a64f888f`) | **이 규칙 문법이 최상위에서 «받는» 이름 전부** — `RULE_ROUTING_REQUIRED` + `RULE_ROUTING_OPTIONAL`. 🔴 **함수로 내는 이유는 「한 목록」이 두 벌이 되지 않게 하려는 것이다** — 필수와 선택을 각자 import 해 합치는 자리가 둘이면 그 둘이 갈릴 수 있고, **갈린 쪽이 «조용히» 덜 거절한다**. 📌 그리고 `chain_skeleton.json` 이 이 함수에서 «생성»된다(위 🆕㉗ 절) |
| 🆕㉗ **`MAPPER_KEY = "mapper"`** :238 · **`PARAMS_KEY = "params"`** :239 · `params_of(rule)` :285 (S-188 ⓓ `87be0737`) | **칸 «하나»가 매퍼를 이름 댄다** — 두 칸 철자(`mapper_module` + `mapper_function`)는 읽히는 채로 남는다. 매퍼 «고유»의 값은 `params` 안에 평평하게 놓이고, 로더는 최상위에 흘러나온 그 이름들을 «이름 대어» 거절하며 **어느 칸을 옮기라고 말한다**(:275 부근). ⚠️ 합성 규칙이 `params` 안에 두는 이름(`decision_key`·`reference_views`·`aggregations`·`alignment`)은 이 목록에 «없다» — 인리치 규칙의 칸이거나 `params` 의 내용이고, 최상위에서 읽히지 않는다 |
...
> 🪦 **[2026-09-13, S-209] 이 파일은 «이 저장소에 없습니다»** — `authoring/examples/lot_slot_wafer_mapper.py.sample` 로 옮겨졌습니다(판정 348·349). 아래 절의 «심볼·계약»은 그대로 참이고, «경로»만 바깥입니다.
...
> 🆕⑥ **[2026-08-13 신설 등재]** `server/ledger/` 패키지(11파일) + 읽기 측(`ledger/trace.py`·`ledger/trace_router.py`) + 클라 3종 + 하니스 + 마이그레이션. 설계는 [`docs/architecture/CANONICAL_LEDGER_DESIGN.md`](./CANONICAL_LEDGER_DESIGN.md), 슬라이스 범위는 `docs/process/LEDGER_SLICE_1_BRIEF.md`, 판정은 `docs/process/LEDGER_RULINGS.md`. **이 지도는 그 문서들을 요약하지 않는다 — 소스에 실재하는 심볼만 싣는다.**
>
...
**무엇인가.** `lot_event` 같은 기존 소스 테이블을 **읽어** 봉투 7필드짜리 원자(atom)로 번역해 `ledger_events`에 **추가만** 하는 계보 원장. 쓰기 경로(`crud.apply_batch_updates`·`cell_sources`·체인·그래프)는 **한 줄도 바뀌지 않았다** — 원장은 기존 시스템의 **소비자**로 태어난다. ⚰️ 🆕⑬ **[2026-08-29] 그 콘솔은 없다.** **데이터**에 답하는 것은 여전히 **`GET /api/ledger/subgraph` 하나**이고 그 옆에 선언 카탈로그 `GET /api/ledger/declaration` 이 서 있다. 🆕⑯ **[2026-08-31] 라우트 전건 계수는 이제 «3»이다**(`9eb30691` 실측 — `3b77fbc3` 에서는 2) — 셋째는 `GET /api/ledger/gaps` 이고 🔴 **키를 받는 데이터 라우트가 «아니다»**: 인자 없으면 «선언만» 읽어 질문 «이름»을 내고, `name=` 이면 그 질문 «하나»를 센다. 즉 개정 6 이 없앤 그 부류가 아니다. ~~🆕⑦ 읽기 측은 이제 라우트 하나가 아니라 **원장 콘솔**이다~~ — 계보 추적(`/trace`) 옆에 관측 종류 카탈로그(`/kinds`) · 형제 대조(`/siblings`) · 온톨로지 구조(`/structure`) · 랏 격자(`/lots`) · 축별 맵 투영(`/lot_map`) · 커버리지(`/coverage`)가 섰다.

**결합(실측 · 🆕⑦ `a729a7f` 재측정).** `server/main.py`의 등록은 여전히 두 줄이다(`import ledger_trace_router` + `app.include_router(...)`, SPA catch-all보다 위 — 등록 순서가 계약이다. 아래로 가면 index.html이 200으로 서빙된다). 🆕⑫ **정정 — `ledger_trace_router`가 import하는 것은 7모듈이 아니라 «15»다**(`bd0c192e` 실측: 위 일곱 + `enrichment_actions`·`ledger_catalog`·`ledger_composition`·`ledger_explorer`·`ledger_journey`·`ledger_selection`·`ledger_subgraph`·`ledger_trends`), 그리고 `ledger_walk_contrast`가 `mechanism_gate`를 쓴다. 🔴 **그 15가 이 라우터를 파일 통째로 얼릴 수 없는 이유다** — 은퇴 울타리는 라우트가 «부르는 심볼»로 갈린다(아래 `ledger/trace_router.py` 절의 🆕⑫ 표, 8 동결 / 8 열림). 🔴 🆕⑦ **정정 — 「`ledger/trace.py`는 `server/ledger/` 패키지를 import하지 않는다」는 이제 글자 그대로는 거짓이다.** `ledger_trace._vocabulary()`와 `ledger_structure._vocabulary()`가 **`from ledger import vocabulary`를 지연 import** 한다(walk가 따라갈 술어를 선언에서 읽기 위해 — 두 번째 어휘 사본을 만들지 않으려는 선택). **부팅 경로 무접촉이라는 의도된 뜻은 여전히 참이다** — import가 함수 안에 있어 웹 서버 기동은 `server/ledger/`를 열지 않는다. 쓰기 측 자체를 상시 프로세스가 import하지 않는다는 사실도 그대로다(번역기는 CLI 백필로만 돈다).

### `server/ledger/` — 쓰기 측

🆕⑦ **[2026-08-14] 11 → 13파일.** 🆕⑧ **[2026-08-16] 13 → 19파일.**
...
🆕⑬ **[2026-08-29 파일 목록만 재측정 — `3b77fbc3` 커밋 blob] 34 → «27»이다.** `git ls-tree -r --name-only <rev> -- server/ledger` 전건 대조 — **빠진 것 일곱 · 신설 0**. 🔴 **그중 넷은 `server/_archive/` 로 갔고 셋은 삭제됐다** — `chain_mapper.py` · `profile_chain_mapper.py` · `profile_lookup_adapters.py` · `examples/__init__.py` 는 **`server/_archive/` 아래**에 있고, `vocabulary.py` · `legacy_import.py` · `shadow_parity.py` 는 **없다**. 🔴 **`vocabulary.py` 의 자리를 물려받은 것은 `ledger.config.load()["vocabulary"]` 다** — 라우터의 `_followable_predicates()` 가 그렇게 읽는다(§5-H 「`server/ledger/trace_router.py`」 절). 🆕⑭ **여기 함께 적혀 있던 `_continuing_predicates()` 는 «없는 이름»이다**(`2a4e51cf`) — 그 자리는 `_static_types()`·`_static_step_predicates()` 가 물려받았고 둘은 `vocabulary` 가 아니라 `entities` 를(그리고 `_static_step_predicates` 는 둘 다를) 읽는다. ⚠️ **이 패스는 파일 목록만 재섴고 줄 수도 심볼 표도 열지 않았다** — 아래 표의 줄 수는 여전히 `b3e1ec83` 기준이고, 사라진 일곱이 있던 구간을 지난 만큼 **밀렸다고 가정하라.**

🪦 🆕⑬ **[2026-08-29] 그래서 이 절이 «33파일 + `examples/__init__.py`»라 적는 것은 거짓이다** — `server/ledger/examples/` 는 디렉터리 자체가 없다(사본은 `server/_archive/examples/__init__.py`). 아래 두 문단은 그것이 참이었을 때의 서술이다.
...
🆕⑯ **[2026-08-31 파일 전건 + 줄 수 전건 재측정 — `9eb30691` 커밋 blob] 27 → «29»이고, «표에 없는 파일 다섯»이 행으로 살아 있었다.** `git ls-tree -r --name-only HEAD -- server/ledger` 전건 대조.
> 🔴 **① 없는 파일이 표에 있었다 — 다섯.** `chain_mapper.py` · `profile_chain_mapper.py` · `profile_lookup_adapters.py` · `legacy_import.py` · `shadow_parity.py`. 🆕⑬ 이 **산문으로는 「넷은 `_archive/` 로, 셋은 삭제」라 적어 놓고 표는 안 고쳤다** — 그래서 표를 읽는 독자에게는 다섯이 살아 있었고 그중 `shadow_parity.py` 는 「레거시 ↔ v2 결정적 의미 그림자 대조」라는 «역할 서술»까지 달고 있었다. 아래에서 묘비 한 행으로 접었다. 실측: `_archive/` 에 셋(`chain_mapper`·`profile_chain_mapper`·`profile_lookup_adapters`), **삭제 둘**(`legacy_import`·`shadow_parity` — `_archive/` 에도 없다).
> 🔴 **② 신설 둘.** `gaps.py`(346) · `gap_names.json`(72) — 결측 검출기와 그 이름표. 아래 별항.
> 🔴 **③ 줄 수 «여덟»이 어긋나 있었다**(전건 `git show HEAD:<path> | wc -l` — 나머지 열아홉은 정확했다): `backfill` 748→**1,238** · `runtime_v2` 385→**528** · `store` 503→**532** · `schema` 430→**470** · `source_contract` 319→**361** · `setup` 365→**392** · `envelope` 329→**330** · `gate` 581→**580**. ⚠️ **심볼 표를 이 패스가 다시 연 것은 `backfill`·`runtime_v2`·`store` 셋뿐이다** — `schema`·`source_contract`·`setup`·`envelope`·`gate` 는 **줄 수만** 고쳤으니 그 절들의 심볼 표에 신설이 빠져 있다고 가정하라.

🆕⑱ **[2026-09-05 파일 전건 + 줄 수 전건 재측정 — `6345ef3d` 커밋 blob] 파일 목록은 «29 그대로»**(신설 0 · 삭제 0), **줄 수는 «아홉»이 어긋나 있었다.** 전건 `git show HEAD:<path> | wc -l`:
> 🔴 `backfill` 1,238→**1,289** · `config` 1,154→**1,182** · `config_drafts` 828→**850** · `config_explorer_service` 946→**989** · `envelope` 330→**359** · `gate` 580→**601** · `implementations` 172→**228** · `source_preparation` 1,060→**1,070** · `ledger_skeleton.json` 710→**797**.
> 🔴 **`ledger_skeleton.json` 은 «드리프트가 아니라 자기모순»이었다** — 🆕⑮ 가 [§5-H-bis](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재) 에 **797** 이라 적으면서 이 표는 안 고쳐서, **한 문서가 같은 파일에 두 수를 들고 있었다.** 두 자리가 같은 값을 말해야 하는 곳은 그 둘이 «따로» 재지는 자리다.
> ⚠️ **심볼 표를 이 패스가 다시 연 것은 `envelope`·`implementations` 둘뿐이다** — 나머지 일곱은 **줄 수만** 고쳤으니 그 절들의 심볼 표에 신설이 빠져 있다고 가정하라. 나머지 스무 파일의 줄 수는 정확했다.
> 🔴 🆕⑱ **`envelope.check_envelope` 의 «반환 모양»이 바뀌었다 — 문자열 리스트가 아니라 `{code, path, message}` 리스트다**(`1936c356`). 코드 여섯이 이 파일에 상수로 산다: `ENVELOPE_OCCURRED_AT_MISSING` · `..._OCCURRED_AT_NAIVE` · `ENVELOPE_SOURCE_WHO_EMPTY` · `ENVELOPE_TRANSLATOR_VER_EMPTY` · `ENVELOPE_RAW_REF_EMPTY` · `ENVELOPE_PAYLOAD_NOT_PRESERVABLE`. 🔴 **그리고 «게이트가 그 코드로 갈래를 튼다»** — 종전엔 거절 사유를 **메시지 문자열에서 `"raw_ref"`·`"occurred_at"` 을 찾아» 정했고, 그래서 **문장을 다듬으면 원자가 받는 거절이 조용히 바뀌었다**(payload 갈래는 «다른 모듈의 예외 텍스트» 세 구절을 대조하고 있었다). **문장은 안 바뀌었다 — 새로 생긴 것은 «어느 필드인가»다.**

| 파일 | 줄 | 한 줄 |
|---|---|---|
| `__init__.py` | **27** | 코드 0줄, docstring만. ⚠️ 🆕⑩ **그 docstring의 읽는 순서가 이미 낡았다** — `lot_event_translator.py`를 여섯째로 지목하는데 **그 파일이 없다**. 소스 소관이라 여기서 못 고치고, **이 지도가 그 순서를 복사하지 않는다**(종전 이 행이 복사하고 있었다) |
| `envelope.py` | 🆕⑱ **359** @`6345ef3d`(🆕⑯ 330 · 구 표기 329 — `1936c356` 이 거절 여섯에 «코드와 주소»를 달았다) | 봉투 7필드의 파이썬 표현 + 타입 보존 |
| `uuid7.py` | **123** | 단조 UUIDv7 — `id`이자 워터마크이자 기록 시각 (🆕⑩ 무변동) |
| `gate.py` | 🆕⑳ **636** @`328a5c20`(🆕⑱ 601 · 🆕⑯ 580 · 구 표기 581) | 문 앞에서 거절하고 **센다**. 단위는 행이 아니라 **분자(molecule)**. 🆕⑳ **거절이 이제 «주소»를 나른다** — 아래 `gate.py` 별항 |
| 🔒 `config.py` | 🆕⑱ **1,182** @`6345ef3d`(🆕⑭ 1,154 · 구 표기 1,073) | **[동결 — 은퇴 울타리 A군]** `ledger_config.json` 로더/검증 + 번역기 버전 해시. ⚠️ **v2 정본은 이 파일이 아니라 `setup_bundle.py`+`setup.py`다**(`SETUP_VERSION`·`CONFIG_FILENAME` 소유가 그쪽으로 갔다). 🆕⑪ **새 작업의 출발점으로 읽지 마라** — 🔴 🆕⑭ **그러나 「동결」을 「안 돈다」로 읽지 마라**: 읽기 라우터의 답 넷(`follow` 후보 · static 타입 · static 술어 · `GET /declaration`)이 전부 이 파일의 **`load()`** 위에 선다. 🔴 🆕⑭ **그리고 `load()` 는 더 이상 `validate()` 로 «직행하지 않는다»** — 신설 `_validate_for_version(raw, origin)` 이 `setup_version` 을 보고 갈래를 튼다: **< 5 면 이 파일의 v3 `validate()`, ≥ 5 면 `setup_bundle.validate_bundle_errors(raw, catalog=setup.live_physical_catalog())`**. 이 박스의 tracked 선언 둘은 **둘 다 `setup_version: 5`** 이므로 **아래 `config.py` 심볼 표의 `validate()` 서술은 오늘 «안 타는 갈래»를 설명한다.** 소스가 그 사고를 적어 둔다 — 고치기 전엔 v5 선언의 모든 load 가 raise 했고, 화면이 답한 유일한 이유는 `ledger_subgraph` 가 같은 파일을 **평범한 `json.load`** 로 따로 열었기 때문이다. ⚠️ **카탈로그 없이 부르면 «카탈로그를 달라»는 불평 한 건이 나오고, 그것을 「선언이 틀렸다」로 읽는 것이 이 문단이 막으려는 함정이다**(소스 주석) |
| `schema.py` | 🆕⑯ **470**(구 표기 430) | 물리 DDL — **첫날부터 시간 파티션**. 유일한 철자. ⚠️ **줄 수만 재측정** — 아래 심볼 표는 `aeddac8` 기준이다 |
| `store.py` | 🆕⑯ **532**(구 표기 503) | 원자 append와 커서 전진을 **한 트랜잭션**으로. 🆕⑯ **`write_batch` 에 `advance_cursor=True` 키워드가 붙었다** — 아래 `store.py` 별항 |
| `observability.py` | 🆕⑩ **428** | 하트비트 note + **2계층 lag 보고** |
| `backfill.py` | 🆕⑳ **1,305** @`328a5c20`(🆕⑱ 1,289 · 🆕⑯ 1,238 · 구 표기 748/983/895/1,574 — **또 자랐다**) | 커서 루프 + CLI. 🆕⑯ **이 구간의 증분은 「범위 재번역」과 「페이스」다** — 아래 `backfill.py` 별항. **분자를 반으로 자르지 않는다.** 🔴 **드라이버는 이제 «하나»다** — `_run_lineage`/`_run_observation`/`_run_transfer`/`_run_declared` 넷과 그 헬퍼(`_flush`·`_refusal_totals`·`_refusal_delta`·`_watermark_json`·`_cursor_json`)가 번역기 넷과 함께 삭제됐고, 남은 실행 경로는 `run()` → **`_run_v2_lineage`**뿐이다. 🆕⑪ 🪦 **이 구간에 «고아 열»이 실제로 삭제됐다**(`d239bdf8`+`7e2a7ca7`, −235줄): `fetch_observation_page`·`fetch_runs`·`fetch_declared_page`·`fetch_transfer_page`·`fetch_transfer_group`·`fetch_containers`·`_group_transfer_rows`·`_plain`·`_forget_registers`·`_transfer_select`. 🆕⑩ 신설: `PREVIEW_FETCH_ROWS = 200` · **`preview_first_batch(engine, setup, source, fetch_rows=PREVIEW_FETCH_ROWS)`**(쓰기 0·커서 무이동, [§5-H-bis](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재)의 test-run이 유일한 소비자) · `v2_base_select_columns` · `prepare_v2_cursor_batch` · `_fetch_v2_lineage_{page,group,rows}` · `_v2_frame` · `_v2_registration_subjects` · `_no_join_reader`. **아래 「소스가 이 절을 반박하는 자리」에 이 파일의 실측 반박 1건이 있다** |
| `dry_run.py` | 🆕㉝ **58** (구 표기 196/179) | ⚰️ **미리보기는 내려가 있다.** `preview()`는 `DryRunUnavailable`을 던지고 `main.py`(`post_ledger_dry_run`)가 그것을 `declaration_rejected`로 렌더한다. 남은 실물은 `begin_read_only(connection)`·`envelope_of(atom)`·`_existing_registrations(...)`. 🔴 **v2의 쓰기 없는 미리보기는 이 파일이 아니라 `backfill.preview_first_batch` / `setup.preview_selected_cursor_batch`이고, 이제 HTTP 라우트가 «있다**」(`POST /admin/ontology-explorer/test-run`)<br>⚰️ 🆕㉝ **[S-203 `8d344e1d`, 판정 319] 그 문이 «은퇴했다» — 「거절만 하던」 문이었다.** `preview()` 와 `DryRunUnavailable` 이 나갔고, `main.py` 의 `post_ledger_dry_run` 도 같이 나갔다. 🔴 **부류: 「실패하는 것이 무해한 것은 아니다」** — 언제나 거절하는 문은 안 돌아서 무해해 보이지만, 있는 동안 «이 길이 있다»고 말한다. 판정 319 가 그것을 「막힌 저장이 아니라 «죽은 문»」으로 갈랐다. 🔵 남은 실물은 `begin_read_only(connection)` :40 «하나»이고, 그것은 «쓰는 사람이 있는» 헬퍼다 — 위 문장의 나머지(「v2 의 쓰기 없는 미리보기는 `backfill.preview_first_batch` / `setup.preview_selected_cursor_batch`」)는 오늘도 참이다 |
| `source_contract.py` | 🆕⑯ **361**(구 표기 319 — ⚠️ **줄 수만 재측정**) | 소스 선언·번역 프로필·가능 Claim 전수·live vocabulary를 한 작성 계약으로 컴파일 |
| `source_profile_builtins.py` | 🆕⑮ **150** @`5a73021a`(구 표기 146) | `lot-lineage@1`, `transfer@1` Pack/Claim/Role 및 `source_position` symbolic constant 등록 데이터. 🔴 🆕⑮ **[2026-08-30 `50be7eb6`] 내장 템플릿 둘의 `entity_types` 철자가 «선언 쪽으로» 맞춰졌다** — `("Lot",)`→**`("lot",)`** · `("Wafer",)`→**`("wafer",)`**. **선언이 2026-08-27에 엔티티 타입의 정본이 됐고 거기서는 맨 소문자다**(`5a73021a` 실측: 102·117행). 🔴 **드리프트가 «보이지 않았던» 이유가 등재할 값이다** — 템플릿의 `entity_types` 를 타입 레지스트리와 대조하는 곳이 «없어서» 프로필을 파싱하기 전까지 아무도 몰랐다. 증상은 `_parse_entity` 가 타입을 «잘 풀고» 나서 거절하는 것이었다(`entity type 'lot' is not supported by template 'lot_lineage'`). ⚠️ **아직 강제되지 않는다** — 대조하는 자리가 생긴 것이 아니라 철자만 맞췄으므로, 같은 드리프트가 다시 나면 다시 안 보인다. 🔴 **앵커로 `"Wafer"`/`"Lot"` 을 쓰지 마라** |
| 🆕⑩ `setup.py` | 🆕⑯ **392**(구 표기 365/251 — ⚠️ **줄 수만 재측정**) | **셋업 경계 — config root 하나를 읽어 스냅샷 하나로 컴파일한다.** 구 `cutover_v2.py`의 개명(`b4c5870`)이고 진입 함수는 **`load_setup(root)`**. `DEFAULT_ONTOLOGY_ROOT`(= `server/config/ontology/`) 소유. 최상위: `LedgerSetupError` · `LedgerSetup` · `load_setup` · **`setup_from_document`** · **`live_physical_catalog`** · **`physical_catalog_path`** · `preview_selected_cursor_batch` · `execute_selected_cursor_batch` · `dry_run_report` · `_require_declared_source` · `_authoring_issues` · `_resolve_cli_root` · `main(argv=None)`. CLI `--root <디렉터리>` |
| 🆕⑩ `setup_bundle.py` | 🆕㉖ **2,224** @`67416e45`(2026-09-08 오후 · 카탈로그 `kind` · `e32c0ce1`) · ~~🆕㉕ 2,203~~ [2026-09-08 오후 낡음] ·  @`d8ec410b`(2026-09-08 — `attributes` 문법이 들어온 날, `4b3ac8ac`) · ~~🆕⑳ 2,004~~ [2026-09-08 낡음] · 구 표기 2,004 @`328a5c20`(🆕⑮ 1,995, 🆕⑭ 2,004, 구 표기 1,883/1,854/1,610 — **줄어든 것은 아래 ③ 의 `continues` 수용 삭제다**) | DB/runtime import 없는 **순수 authoring schema + strict loader**. 🔴 🆕⑭ **이제 «걷기 정책»의 문법도 여기 산다 — 엔티티의 `class` 와 `references` 둘 다 이 파일이 정본이다.** `_validate_entities` 의 exact 키 집합: `required=("keys",)` · `optional=("key_types", "allow_null", "references", "class")`. ① **`class`** 는 `"static"`\|`"dynamic"` 만 받고 그 밖은 `invalid_entity_ref`. **선택이고, 없으면 `dynamic` 이다** — 소스 주석이 그 «없음»을 결정으로 적는다(안 쓰는 선언에는 쓰지 않는다. 없을 때 기본으로 읽히는 필드는 계속 없어야 「선언된 dynamic」과 「한 번도 분류 안 함」이 갈린다). 이것이 `ledger_trace_router._static_types()` 가 읽는 그 필드다. ② **`references`** 는 `_validate_references(value, path, own_keys, section, problems)` 가 전건 검사한다 — 리스트가 비면 거절, 낱말은 **`from`/`edge`/`to`** 이고(매핑의 `subject`/`predicate`/`target` 과 **일부러 다르다** — 참조는 원자가 없는 «합성» 엣지라 같은 낱말을 쓰면 「그 술어의 원자를 찾을 수 있다」는 거짓 약속이 된다), 모르는 필드와 이 엔티티에 없는 키 이름은 **거절**한다(오타는 조용히 엣지 0을 내고 화면에서 정직한 부재와 구별되지 않는다). `to.keys` 가 복수인 것은 컨테이너 이름에 키가 여럿 필요할 수 있어서다(lot slot = (lot, slot)). ⚠️ **문법은 살아 있는데 «읽는 쪽»이 없다** — 투영의 `_link_containers` 가 삭제됐고 `follow` 도 `vocabulary` 만 본다(위 §5-H 라우터 절의 🆕⑭ 문단). ③ 🔴 ✅ **[🆕⑮ 2026-08-30 `1d17c34a`] `continues` 가 그 튜플에서 «나갔다» — 이제 «거절»이다.** `_validate_vocabulary` 의 `problems.exact(...)` 인자가 **`required=("status", "subjects", "object")` 뿐이고 `optional=` 이 아예 없다**(`5a73021a` 실측). 🔴 **예고한 순서가 «그대로» 지켜졌다** — 🆕⑭ 가 「문을 먼저 열고 선언을 나중에 청소한다; 그 줄은 청소와 함께 나간다」로 적어 둔 그 청소가 `36802e42`(선언에서 여섯 제거)로 먼저 착지했고, 이 커밋이 문을 닫았다. 소스 주석이 ⏳ 에서 **🗄️ 묘비**로 바뀌면서 그 순서를 적는다. 🔬 **재측정 `5a73021a`: tracked 선언 «둘 다» `"continues"` 계수 0**(`config/sample/ledger_config.json.sample` · `config/sample/ontology/transfer_explorer/ledger_config.json` — 같은 프로브로 `"references"` 도 둘 다 0) · `server/` 전체에서 `continues` 라는 «낱말»이 남은 자리는 전부 **산문**이다(이 묘비 주석 · 같은 파일 1047행의 대조 주석 · `ledger_subgraph.py` 의 역사 주석 넷 · 그 밖은 영어 단어 「계속된다」). 🔴 **「선언에 아직 있으니 문을 못 닫는다」가 더 이상 참이 아니고, 닫지 않으면 은퇴한 낱말이 «도는 규칙»으로 조용히 읽힌다** — 그것이 이 커밋의 명시된 사유다. **아래는 `2a4e51cf` 기준의 기록이다:** ~~`_validate_vocabulary` 는 `optional=("continues",)` 를 «시한부로» 통과시킨다(소스에 ⏳ 표시) — 읽는 코드는 0인데 라이브 선언이 아직 여섯을 들고 있어, 거절하면 서버가 선언을 못 읽는다~~. 🔴 **`SETUP_VERSION = 5`**(구 4 — 이주 스크립트가 `scripts/migrate_ledger_config_to_v{4,5}.py` 둘. 🆕⑪ **셋째 이주기 `migrate_ledger_config_drop_vocabulary_layer.py`는 버전을 «안 올린다»**) · `CONFIG_FILENAME = "ledger_config.json"` · `PHYSICAL_CATALOG_FILENAME = "table_config.json"` · `LOGICAL_SECTIONS`/`OPTIONAL_SECTIONS`(`virtual_joins`)/`ALL_SECTIONS` · `_FORBIDDEN_DECLARATION_KEYS`/`_FORBIDDEN_EXECUTABLE_KEYS`(**선언은 이름을 대지 경로·표현식을 대지 않는다** — `implementations.py`가 그 규칙의 나머지 반쪽) · 🆕⑪ **`_RETIRED_BINDING_FIELDS = ("approval_status", "binding_origin", "suggestion_reason")`**(🔴 **거절이 아니라 «삼킴»이 이 상수의 존재 이유다** — 위 은퇴 문단) + `_RETIRED_FIELD_HELP` · 역할 어휘 `SUBJECT_ROLE`/`OCCURRED_AT_ROLE`/`TARGET_ROLE`/`VALUE_ROLE` + 🆕⑪ `_ROLE_KINDS` **7종**(`entity`·**`time`**·`quantity`·`identity`·`order`·`attribute`·`symbolic`)/`_SCALAR_ROLE_KINDS` · `validate_bundle`/`validate_bundle_errors`/**`bundle_readiness_errors`**/`require_ready_bundle` · `load_setup_bundle`/`setup_bundle_errors`/`load_physical_catalog` · `public_bundle_schema()` · `predicate_claim()` · `role_binding_kinds()`.<br>🔴 🆕⑳ **[2026-09-05 `06bd33a9`] `public_bundle_schema()` 가 «키 하나»를 더 낸다 — `identity_binding_kinds = ["column", "constant"]`.** 그래서 이 함수의 키는 **8**이고, 그것이 [`config_authoring.closed_lists`](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재) 의 23키 중 늘어난 하나다. 🔴 **두 번째 규칙이 아니라 «첫 번째 규칙을 화면이 읽을 수 있는 자리에» 적은 것이다** — `_validate_binding` 이 엔티티 «식별키»에 쓰인 `entity` 바인딩을 이미 `invalid_binding` 으로 거절하므로(「entity identity keys allow only column or constant bindings」), 그 칸에서 셋을 다 내미는 폼은 **거절밖에 못 받는 값을 권한다**. ⚠️ **기존 `binding_kinds`(셋, `entity` 포함)는 그대로다** — 좁힌 것은 «식별키 칸» 하나다. 채점자 `server/tests/test_identity_keys_are_not_offered_an_entity.py` 는 이 리터럴을 믿지 않고 **모든 kind 를 검증기에 먹여** 둘을 서로 대조한다. 🆕⑪ 🔴 **`bundle_readiness_errors`는 이제 «규칙이 0개»이고 언제나 빈 튜플을 낸다** — 유일한 규칙이 `approval_status == "approved"`였다. **단계를 남긴 이유는 자리다**: 호출자 셋(`setup.load` · `setup_registry.snapshot_compile_errors` · `config_drafts.compile_draft_preview`)이 구조 검증과 컴파일 **사이**에 놓으므로 「이게 돌아도 되는가」류의 다음 규칙이 앉을 곳이 여기다. **「검증을 통과했다」의 증거로 인용하지 마라**<br>🆕㉗ **[S-187 `8d4f2ceb`] `catalog_kind(entry)` :217 — 기본값이 «한 자리»에서 정해진다.** 🔴 **네 자리가 각자 `str(x.get("kind") or "table")` 를 적고 있었다** — 여기 검증기 · 동적 모델 빌더 · 그리드의 쓰기 문 · 스크립트 하나. 한 기본값의 네 철자는 하나가 갈릴 네 번의 기회이고, **그 어긋남은 보이지 않는다** — 뷰여야 할 관계를 표로 읽는 것은 `_validate_catalog` 가 오타를 거절해서 막으려는 바로 그 실패다. ⚠️ **이 함수는 «검증하지 않는다»** — 모르는 낱말은 «로드 때 경로를 대고» 거절되고, 여기서 그 질문에 다시 답하면 첫 관문과 «어긋날 수 있는» 둘째 관문이 된다<br>🆕㉙ **[S-196 `6692ae9f`·`aa0f1217`, 판정 306-b 되돌림] `required_mapper_input_columns(mapper, profile, profile_path)` :2535 — 「`input_columns` 가 담아야 할 것」의 «열거 하나, 호출자 둘».** 검증기는 빠진 것을 거절하고 작성 폼은 «같은 목록»을 유도하므로, 둘이 «한 질문»에 답해야 한다. 🔴 그런데 그렇지 않았다 — 유도가 「준비된 프레임 «빼기» `read` 가 이미 읽는 것」이었고, 그것은 **그룹이나 바인딩이 필요로 하는 컬럼을 정확히 빼 버린다**. 그렇게 다시 지은 번들을 «자기 검증기»가 거절했다. ⛔ **그리고 수리가 «손으로 적은 목록»이면 안 됐다** — 첫 수리는 그룹 컬럼만 더했고 다음 소스가 «바인딩» 컬럼에서 넘어졌다. 손으로 늘린 목록은 «다음 선언이 쓰는 것»을 놓친다. 이 함수는 «종류»를 열거하므로 새 종류는 여기 하나에 더하고 «두 호출자가 같이» 배운다. ⚠️ **경로가 같이 실리는 이유**: 거절문이 「어디서 요구하는지」를 이름 대야 한다 — 「Profile column 'base_id' at …bind.subject.keys.x.column is missing」이 운영자를 «그것을 원하는 선언»으로 보내고, 「없다」만으로는 못 보낸다. 🔵 그리고 **행 단위는 그룹 컬럼을 «요구하지 않는다»**(판정 306) — `unit.kind == "group_by"` 일 때만<br>🆕㉜ **[S-144 `86fff683`, 판정 327] `attribute_cardinality` — 속성의 «복수성»이 «선언된 한 칸»이다** (검증 :1318·:1375 · 스켈레톤 `ledger/ledger_skeleton.json` :193, 그래서 «폼이 그 칸을 그린다»). 🔵 판별식대로다: 「이 값을 «사용자가 적을 수» 있나」 → 있으면 선언이고, 코드가 갈래를 트는 자리가 아니다<br>🆕㉝ **[S-147-a `05255959`] 술어의 optional 키가 «둘»이 됐다 — `("cardinality", "absence_confirmed_by")` :1223** (검증 :1249~). 🔴 **빠진 것은 «칸»이 아니라 «하위 축»이었다** — 표 A 의 A1(노드 키)은 「부재의 뜻」을 `allow_null` 로 들고 있었는데 A2(술어)에는 «짝이 아예 없었다». 🔴 **그리고 값이 «다른 술어의 이름»인 것은 고른 게 아니라 «강제»다** — 「안 봤다」와 「보았고 없었다」는 «다른 원자»의 존재로만 갈리고, `absence: none\|unknown` 류 플래그로는 표현할 수 없다. ⚠️ 어느 술어가 «검사»인가는 «도메인 사실»이라 «선언»한다 — 코드가 그 이름을 알면 그것이 «코드에 박힌 도메인 낱말»이다. 문법이 «exact» 라서 이름 없는 칸은 «무시가 아니라 거절»이다 |
...
| 🆕⑩ `source_preparation.py` | 🆕⑳ **1,086** @`328a5c20`(🆕⑱ 1,070 · 🆕⑪ 1,060 · 구 표기 1,022/968) | v2 Stage 5 **pandas 소스 준비 경계**. 상세는 [§5-H-bis](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재) |
| 🆕⑩ `runtime_v2.py` | 🆕⑳ **537** @`328a5c20`(🆕⑯ 528 · 구 표기 385/365 — 🆕⑳ 증분은 «주석»이다: `_screened_atoms` 가 `gate.screen_compiled_molecule` 의 리포트를 «버리는» 이유를 적었다. 🔴 그 호출은 `gate.building_molecule` 안이라 거절이면 **`MoleculeRefused` 가 던져지고 반환하지 않는다** — 즉 이 줄이 묶는 쌍은 언제나 «수용» 쪽이고, 소비해도 거절에 대해 아무것도 못 얻는다. 거절의 코드·주소는 [`gate.refuse(..., addresses=)` → 표본 → `backfill` 의 run result](#5-h-정본-원장-canonical-ledger) 로 간다) | v2 Stage 6 — 기존 gate/store 트랜잭션 **위의** 실행 어댑터. `LedgerV2RuntimeError` · **`CursorBatchPreview`**/**`CursorBatchExecutionResult`** · `preview_cursor_batch`/`execute_cursor_batch` · 🆕⑯ **`execute_scoped_batch`**(아래 별항) · 🆕⑯ `_require_scope` · 🆕⑯ `_screened_atoms` · `_filtered_event_atoms`(🔴 `register`를 낼 소스가 조회 방법을 선언 안 했으면 **`registration_context_required`로 거절** — `known_registrations`가 `None`일 때만 발화한다) · `_stamp_occurred_at_basis` · `_known_registrations` · 🆕⑯ `_source_plan` · `_cursor_value` · 🆕⑯ `_json_scalar`/`_semantic_atom`/`_canonical` |
| 🆕⑩ `implementations.py` | 🆕⑳ **261** @`328a5c20`(🆕⑱ 228 · 구 표기 172 — 🆕⑱ `implementation_choices` 신설 · 🆕⑳ `preparer_output_columns` 신설) | 🔴 **손으로 관리하던 목록 «넷»을 대체한다.** 상세는 [§5-H-bis](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재) |
| 🆕⑩ `column_stats.py` | **287**(신설) | 컬럼 선택을 위한 물리 통계 프로브. 상세는 [§5-H-bis](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재) |
| 🆕⑩ `config_authoring.py` | 🆕⑳ **2,003** @`328a5c20`(🆕⑪ 1,937 · 구 표기 1,548) | 작성 패널의 엔진 — 「무엇이 강제이고 무엇이 여전히 질문인가」. 상세는 [§5-H-bis](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재) |
...
| 🆕⑩ `config_drafts.py` | 🆕⑱ **850** @`6345ef3d`(🆕⑪ 828 · 구 표기 806/548) | 탐색기의 **파일시스템 작업 초안**(`OntologyDraftStore`) — 🔴 **작성 경로에서 «쓰는» 유일한 자리**(`config_authoring`은 한 줄도 안 쓴다)<br>🆕㉘ **[S-194 `15fa6006`, 판정 303] 줄 수 «937»(구 표기 850), 그리고 수명주기가 «문서 어댑터»를 통해 묻는다.** `class DraftContext` :304 · `class LedgerDocument` :319 · `class OntologyDraftStore(root, document=None)` :354. 🔴 **`LedgerDocument` 의 모든 메서드가 «오늘의 코드를 옮긴 것»이다** — 판정 303 이 요구한 관문이 「원장 경로가 «바이트 동일»하게 나온다」라, 이 클래스는 아무것도 «다르게 정하면 안 된다». 이름만 준다. 🔴 **기본값이 `LedgerDocument` 다** — 기존 호출자가 «한 줄도» 안 바뀌고, 바이트 동일 관문이 «대조할 것»을 갖는다. 📎 둘째 문서는 `chain_bindings.ChainRuleDocument`(아래)<br>🆕㉜ **[S-143 `3238bff6`, 판정 319·320·323] 초안 검토 응답이 `redo` 칸을 든다** — `DraftPreview.redo` :155 · `_redo_for(active_setup, node, db)` :168, 그리고 «두 문서 어댑터가 공유»한다. 🔴 **커밋 제목이 그 결함을 이름 댄다 — 「계산됐는데 «떨어뜨려지고» 있었다」**: 비용이 이미 계산되던 자리가 있었고 전선에 «안 실렸다». 「착지는 배선이 아니다」의 실물이고, 정정이 «계산»이 아니라 «배선»이었다. ⚠️ **`redo` 가 `None` 이면 「그 질문을 «안 물었다»」이지 「비용이 0」이 아니다**(소스 :828) — 0 으로 접으면 운영자가 «안전하다»로 읽는다<br>⚰️ 🆕㉞ **[S-208 `df108854` · S-205 `6cfd921a`] 「문서 어댑터」 층이 «되접혔다» — 남은 것은 `OntologyDraftStore` :365 «하나»다.** `LedgerDocument` · `ChainRuleDocument` · `ChainRuleIndex` · `DraftContext` 전부 «없다»(`git grep class` 로 확인). 🔴 **둘째 문서가 «끝내 안 왔다»** — 판정 325 가 체인 규칙의 문을 «체인 탭의 raw 라우트»로 두면서 `ChainRuleDocument` 는 «제품 호출자를 한 번도 못 가진 채» S-205 에서 은퇴했다. 남은 것은 «구현이 하나뿐인 층»이고, 그것은 상설이 금하는 것이다 — **층은 «둘일 때» 짓지 «둘이 될 것 같을 때» 짓지 않는다**. 🔵 판정 303 에서 서 있는 절반은 「합칠 사람이 없다 — 수명주기는 하나」이고, 그 문장이 이제 「수명주기 하나, 문서는 원장 하나 — 둘째가 오면 그때 이음새」로 고쳐졌다. 📐 접기 전에 «셌다»: `self._document` 를 지나는 호출 열여섯 · 제품 호출자 «하나»(문서를 안 넘긴다) · 문서를 넘기는 시험 «하나». ⚠️ **[2026-09-13 자기 정정] 제가 D-14 에 이 층을 「수명주기 하나 · 문서 둘」로 적고 «좋게» 기록했다 — 그때 문서는 «하나»였고 둘째는 «예정»이었다. 즉 제가 «여럿이 될 거라는 베팅»을 「이미 여럿」으로 옮겨 적은 것이다**(상설: 「무한 케이스인가, 여럿이 될 거라는 베팅인가」) |
...
| 🆕㉑ **`followup.py`** | 🆕 **659** @`dc877746`(645 @`474f1aa9` · 신설 `1d4bb79d` 때 234) | 🆕 **[`b1e89e36` S-249 ⓒ] `enqueue(table_name, row_ids, event_type, transaction_id=None, chain_depth=None)` :338** — 홉이 «같이 탄다»(큐 튜플 여섯째 칸). 없으면 없는 채로 — 체인의 것이 아닌 변경에 0 을 지어내면 «모든 보통 편집»이 연쇄의 첫 걸음으로 보인다. **`drain_once(engine, setup)` :523 의 `done` 이 `transaction_id`·`chain_depth` 를 «든다»** :551~:552 — 종전엔 큐에서 꺼내고 «여기서 버렸다»; 후속 종류가 「이 변경에 이미 답했나」를 물으려면 변경을 «이름 댈» 수 있어야 하고 그 이름이 원인 tx 다(§4 `_run_builtin_followups`). 「원장이 «자기가 읽는 표»를 따라간다」 — 체인/사람이 그 표의 행을 고치면 그 분자만 다시 번역한다(S-54). 🔴 **큐일 뿐이고 번역은 안 한다** — 체인 워커가 `(표, row_ids, EDIT)` 를 «메모리 deque»에 넣고 제 길을 가고, 옆에서 도는 페이싱된 asyncio 작업이 `backfill.rescope` 를 부른다. 범위 컬럼은 «15 소스 전부» `backfill._page_key`(갈래 «없음» — 판정 131). `enqueue`/`row_ids_of`/`queue_depth`/`note`/`reset`/`sources_for_table`/`scope_column`/`drain_once` · `FOLLOWED_EVENT_TYPES = ("EDIT",)` · `FOLLOWUP_JOB = "chain_followup"` · `MAX_QUEUED_EVENTS = 10000`. ⚠️ **큐는 메모리이고 유실이 «사실의 유실이 아니다»** — 소급 실행이 정본 메움이고 이쪽은 «빠른» 쪽이다. 채점자 `server/tests/test_the_ledger_follows_the_table_it_reads.py`(**350줄** / `def test_` 16, 수집 19) |
...
| `@dataclasses.dataclass class Atom` | 한 주장. 의미 봉투 필드 + 원천 사건 상관 `source_event_id`/`source_event_state`. **컬럼이 되지 않는 셋**은 `molecule_ref`(writer가 불투명 Event ID로 접고 버림) · `derivation`(선언된 규칙 이름) · `id`(쓰기 직전 채번) |
| `Atom.ensure_id()` | 워터마크를 찍는다. 🔴 **`__post_init__`이 아닌 것이 의도** — 구성 시각에 채번하면 「번역기가 만든 순서」로 정렬되고 거절된 분자가 id를 태운다. **쓰기 직전에** 한 번 |
| `Atom.ensure_source_event_identity()` | 같은 source/time/molecule 또는 source record를 결정적 UUIDv5로 접는다. 해결 우선순위가 아니라 Evidence Graph의 Event→Claim 경계만 서빙한다. 과거 호환은 원자 1개=`legacy_atom` 하나이며 유사 시각 regroup 금지 |
...
| `ROW_COLUMNS` | 물리 insert의 컬럼 순서 **13개**(`id`…`supersedes`, `source_event_id`, `source_event_state`) 한 튜플. writer와 DDL이 같은 철자를 쓰므로 한쪽에만 컬럼을 더하면 문법 오류가 나고 조용히 밀리지 않는다 |
...
| 🆕⑳ `refuse(source, reason, detail, atoms=0, rows=1, addresses=())` | 원자가 **아예 안 생긴** 것의 거절. 가짜 원자를 지어 거절받게 하는 것보다 진입점 둘이 낫다는 판단. 🆕⑳ **인자 `addresses` 가 붙었다**(`75d3d59f`·`71b0de61`) — `envelope.check_envelope` 가 이미 내던 `{code, path, message}` 목록이 «여기까지» 온다 |
| 🆕⑳ **`class MoleculeRefused(source, reason, detail, addresses=())`** | 🔴 **`self.addresses` · `self.code` · `self.path` 를 «갖는다».** 사유가 지도에 남을 값이다 — `config_explorer_service._test_run_refusal(source)` 이 `getattr(source, "code", "")`/`getattr(source, "path", "")` 를 읽고 `code` 가 비면 **`type(source).__name__` 으로 폴백**하므로, 게이트 거절이 test-run 화면에 닿으면 `code="MoleculeRefused"` · `path=""` 로 보고됐다. 「어느 칸을 고치나」가 존재 이유인 화면에 **예외 클래스 이름**을 답하고 있었다. `code`/`path` 는 **첫 주소**이고 나머지는 `addresses` 가 든다(`_envelope_reason` 이 같은 목록에서 첫 항목을 고르는 것과 «같은» 선택이라, 코드와 주소가 «같은» 위반을 가리킨다) |
| 🆕⑳ `_record(source, reason, atoms, detail, rows=1, addresses=())` | 🔴 **주소는 «표본»에 실리지 «계수»에 실리지 않는다.** `_refusals` 는 `(source, reason) -> count` 이고 그 불변식은 **분해가 거절 수와 «합»이 같다**는 것이다 — 여기에 원자별 주소를 달면 그 등식이 «맞아 보이면서» 깨진다. `_samples` 는 거절 하나당 한 항목이고 이미 상한이 있으며 어떤 합에도 안 들어가므로, **주소를 들 수 있는 유일한 운반체**다. 표본 항목에 `addresses: [{code, path}]` 가 붙는다(문장은 `detail` 이 이미 들고 있어 다시 싣지 않는다) |
...
> ⚠️ 🆕⑭ **[2026-08-29] 이 표는 «v3 갈래»의 표다.** `load()` 가 `_validate_for_version` 을 거쳐 `setup_version >= 5` 면 `setup_bundle.validate_bundle_errors` 로 가고, 이 박스의 tracked 선언 둘은 **둘 다 v5** 다. 그러므로 **아래 `validate(...)` 행이 서술하는 거절들은 오늘 «안 타는 갈래»**이고, 실제로 도는 문법은 `setup_bundle.py` 쪽이다. 이 패스가 잰 것은 **최상위 심볼 이름 전건 + `load`/`_validate_for_version` 본체**이고, 아래 행들의 «산문 서술»은 재검증하지 않았다.
>
> 🔴 🆕⑭ **「`class`」라는 낱말이 이 저장소에 «둘» 있고 서로 다른 것이다 — 섞으면 엉뚱한 파일을 연다.**
> ```
...
> ```
> (같은 파일의 `OBSERVATION_OPTIONAL_COLUMNS` 에도 `"class"` 가 있는데 그건 **소스 표의 컬럼 이름**이고 셋째 뜻이다.)
>
> ⚠️ 🆕⑭ **아래 표에 «없는» 최상위 심볼이 여럿이다**(이 패스가 이름만 셌다): `SOURCE_KIND_*` 넷/`SOURCE_KINDS` · `EMIT_CLASS_OBSERVATION`/`EMIT_CLASS_INFERENCE`/`EMIT_CLASSES` · `LINEAGE_REQUIRED_COLUMNS`/`OBSERVATION_REQUIRED_COLUMNS`/`OBSERVATION_OPTIONAL_COLUMNS`/`TRANSFER_REQUIRED_COLUMNS`/`TRANSFER_OPTIONAL_COLUMNS`/`DECLARED_REQUIRED_COLUMNS` · `DERIVATION_*` · `OCCURRED_AT_BASES` · `WHEN_OPERATORS` · `COLUMN_REF_PREFIX` · `PLACE_*` · `TRANSFER_PREDICATE`/`OBSERVATION_PREDICATE` · `CONFIG_SUBDIR = "ontology"` · `sample_path` · `_validate_for_version` · `_validate_{observation,transfer,declared}_source` · `_validate_chain_mapper_selection` · `_validate_emit_rule` · `_declared_entity_types` · `_identity_keys` · `source_kind` · `selected_profile` · `declared_subject_types` · `declared_inference_derivations`. **이 표를 「이 파일의 전부」로 읽지 마라.**
...
| 🆕⑭ `config_path(filename=CONFIG_FILENAME)` / `_config_dir()` / `CONFIG_SUBDIR = "ontology"` / `sample_path(path=None)` | `_config_dir()`는 `paths.CONFIG_DIR` 경유(import 실패 시 파일 기준 폴백). 🔴 **정본 경로는 그 밑의 «`ontology/`»다** — `config_path()` = `<CONFIG_DIR>/ontology/ledger_config.json`(2026-08-24 판정: 읽는 쪽 여섯을 세어 보니 다섯은 «존재하지 않는» `config/ledger_config.json` 을 거쳐 v3 샘플로 떨어졌고, 걷기 하나만 실재하는 `config/ontology/…` 를 직접 열고 있었다. 사본을 만드는 대신 경로를 옮겼다). 🔴 **`sample_path` 는 «config 루트»에 걸려 있지 `config_path()` 의 디렉터리에 걸려 있지 않다** — 아니면 `config/ontology/sample/…` 를 찾아 신규 체크아웃에서 조용히 「샘플 없음」이라 답한다. 결과 경로는 `<CONFIG_DIR>/sample/ledger_config.json.sample` |
| `load(path=None) -> dict` | 라이브 파일이 없으면 `<name>.sample`로 폴백(운영자 config의 프로젝트 관례). `__origin__`을 심어 거절 문장이 어느 파일을 말하는지 밝힌다. 🔴 🆕⑭ **검증은 `_validate_for_version` 이 «버전을 보고» 고른다**(위 문단) — 아래 `validate` 로 직행하지 않는다 |
...
| `CREATE_LEDGER` | 11컬럼 + CHECK 🆕㉖ **7종**(구 표기 「5종」 — `ck_ledger_source_event_state` 와 `ck_ledger_occurred_at_basis` 가 «세어지지 않고» 있었다. 이 패스가 `28b298d6` 에서 셌다): `ck_ledger_object_kind` · `ck_ledger_register_has_no_object`(양방향) · 🆕㉖ **`ck_ledger_objectless_carries_only_qualifiers`**(`28b298d6`, 구 `ck_ledger_objectless_has_no_payload` — 🔴 **이름이 «뜻과 함께» 바뀌었다**: 목적어 없는 원자가 `object_payload.qualifiers` «만» 들 수 있게 넓혔다. S-52 의 등록 원자가 속성을 그 자리에 나르는데 옛 규칙이 «payload 자체»를 금해 출하 선언이 INSERT 되지 못했다. 옛 이름은 상수 `RETIRED_OBJECTLESS_CONSTRAINT` 로 남아 마이그레이션이 찾는다. 🔴 **몸과 이름의 철자는 «하나»** — `OBJECTLESS_PAYLOAD_CHECK`/`OBJECTLESS_PAYLOAD_CONSTRAINT` 를 이 DDL 과 `ensure_objectless_payload_constraint` 의 ALTER 가 «같이» 읽는다(`uq_ledger_atom` 이 이름만 보고 두 박스에 서로 다른 인덱스를 만든 그 사고의 방지). 🔬 **부모에서 `ADD ... NOT VALID` 가 파티션 전부로 재귀하고 `VALIDATE` 도 부모에서 된다**(PG 18.3 실측, 스크래치 파티션 표). 새 규칙은 옛 규칙 OR 한 항이라 «엄격히 약해» 기존 행은 위반 불가) · `ck_ledger_subject_keys_is_object` · `ck_ledger_no_self_supersede` · `ck_ledger_source_event_state` · `ck_ledger_occurred_at_basis`) + `PRIMARY KEY (id, occurred_at)` + **`PARTITION BY RANGE (occurred_at)`** |
...
| `INDEXES` | **3종, 그리고 소비자 이름이 있는 것만 넣는다는 것이 admission rule이다.** `uq_ledger_atom`(멱등성 — `store.insert_atoms`의 `ON CONFLICT DO NOTHING`) · `idx_ledger_subject_lot`(`(subject_keys->>'lot'), predicate` — **`server/ledger/trace.py`의 walk**. 🔴 부모에 선언해 아직 없는 파티션까지 cascade) · `idx_ledger_register`(PARTIAL, `WHERE predicate='register'` — `store.existing_registrations`). 🔴 **소스 주석이 기각된 후보 3종을 가격과 함께 남긴다**(`idx_ledger_type_pred_time` · `idx_ledger_subject_gin` · `idx_ledger_id`) — 되살리는 것이 새 추측이 아니라 숫자 붙은 결정이 되도록 |
...
| 🆕㉖ **`LedgerStore.write_batch(source, translator_ver, atoms, cursor_value, molecules, refused=0, incomplete=0, *, reasons, enforce_translator_version=False, advance_cursor=True, withdraw_refs=None)`** (🆕⑯ 은 `withdraw_refs` 가 없었다) | 🔴 🆕㉖ **`withdraw_refs` 는 «이 배치가 대체하는 세대»이고, 그 DELETE 는 새 원자를 쓰는 «그 커밋 안»에서 돈다**(`ddf3fb10`, S-60 — 등급 1). 종전에 `backfill.rescope` 가 그 삭제를 «자기 트랜잭션»에 커밋하고 나서 번역했기 때문에, remake 가 실패하면 «철회만 남아» 원자가 사라졌다 — 실제로 2026-09-08 에 `dt_job` 하나의 원자 둘이 그렇게 없어졌다. 🔴 **재정렬은 답이 아니다**: remake 를 먼저 하면 `uq_ledger_atom` 에 부딪혀 전부 `deduped` 되고 그다음 철회가 전부 가져간다 — 같은 손실인데 반환값이 초록이다. 반환에 `withdrawn` 이 는다. 넘기는 자리는 `runtime_v2.execute_scoped_batch` «하나»이고(전진 스캔은 안 넘긴다 — 철회할 것이 없는 호출자는 «세대를 더하는» 것이 맞다), 받지 못하는 store 는 `advance_cursor` 와 «같은 모양»으로 `unsupported_store_contract` 로 거절된다 | 🔴 **원자 단위.** 원자 append + 커서 전진 + **commit 한 번**, 아니면 전무. 카운터는 **SET이 아니라 누적**이다(SET은 이 시스템에서 이미 결함이었다 — QA D-1). 🔴 **`reasons` 는 기본값이 «없고» 키워드 전용이다**(판정 R-2026-08-13-H-bis 2) — 기본값이 있으면 「이 컬럼이 생기기 전에 센 것」(배포 이력)과 「지금 장부에 구멍이 있다」가 **한 수**가 된다. 하류 `ledger_trace` 가 `molecules_refused - sum(breakdown)` 의 **부호**로 그 둘을 가르기 때문이다. 명시 `None` 도 거절이고, 유일한 합법 빈 값은 **아무것도 안 거절한 실행의 `{}`** 다. 🔴 🆕⑯ **`advance_cursor=False` 가 «범위 재작성»이고, 그것이 «같은 문»이다** — 커서 문 «하나»만 건너뛰고 위쪽(게이트·번역·원자 표·한 커밋)은 전부 그대로다. 그래서 위치가 앞으로도 **뒤로도** 안 가고, 두 트랜잭션 사이에 값이 틀려 있을 창이 없다. **카운터도 안 움직이는데 그건 별개 결정이 아니라 같은 사실이다**(카운터가 커서 행의 컬럼이고 「전진 스캔이 어디까지 갔나」를 뜻하므로). 🔴 **이 플래그를 넘기는 자리는 `runtime_v2.execute_scoped_batch` «하나»이고, 그것도 배치가 지목된 범위 «안»임을 증명한 뒤다** — 증명 없이는 「소스 전체를 쓰고 커서에 흔적을 안 남긴다」가 되고 그게 상설이 금하는 «두 번째 문»이다 |
...
#### 🪦 `lot_event_translator.py` — 파일이 없습니다 (2026-08-21 실측)

> 🆕⑩ 🔴 **여기 있던 심볼 표 10행을 삭제했다.** `SOURCE` · `class Molecule` · `molecule_key` · `group_molecules` · `class LotEventTranslator` · `.translate` · `._positional_pairs` · `._slot_map` · `raw_ref` — **전부 없는 이름이다.** 이 표는 「이 파일은 감사됐다」는 인상을 주면서 다른 에이전트를 없는 파일로 보내고 있었다.
>
> 검사(히트 0건이어야 한다):
> ```bash
> git grep -nE "class (LotEvent|Observation|Transfer)Translator|def group_molecules|def molecule_key" -- server/
> ```
>
> **대체된 것.** 「소스 행 → 원자」는 이제 번역기 모듈이 아니라 **선언 + 두 컴파일러**다: `source_preparation.prepare_source_batch`(EventFrame) → `roleframe.map_event_frame`/`compile_role_frame`(RoleFrame → LedgerFrame) → `runtime_v2.execute_cursor_batch`(게이트·store). 소스마다 파이썬 파일을 쓰던 자리는 `mappers/ledger_v2_*.py` **구현 등록**(→ `implementations.py`)으로 옮겨 갔다.
>
> ⚠️ **살아남은 두 계약은 이 지도의 다른 자리에 있다** — ① `source_translator_ver`의 **`#<derivation>` 접미**(관습에 기댄 원자를 질의 가능하게 만드는 자리, 읽기 측 `ledger_trace.claim_basis`가 읽는다)는 §5-H 읽기 측 표에 그대로 있다. ② 「한 소스 사건은 한 배치에 통째로 착지한다」는 규율은 `backfill._run_v2_lineage`의 **쪼갬 가드**로 옮겨 갔다(아래 `backfill.py` 표).
...
| 🆕⑩ `preview_first_batch(engine, setup, source, fetch_rows=PREVIEW_FETCH_ROWS)` | 🔴 **쓰기 0, 커서 무이동, 그리고 «첫 페이지»이지 커서가 아니다.** 작성 중인 소스엔 커서 행이 없고, 커서가 있는 소스는 그 뒤부터 읽으면 **이미 돈 소스에서만 빈 페이지가 답이 된다** — 「내 선언이 되나」라는 질문에 정확히 거꾸로 답한다. 🔴 **`known_registrations`가 «빈 스냅샷»인 것이 결정이다**: 라이브 등록 집합을 넘기면 이미 백필한 소스의 `register` 문장이 전부 0을 보고하고(**소스 docstring이 대는 수치이고 이 패스가 재지 않았다** — `lot_event`에서 라이브 1,173 대 빈 것 1,323, 차이 150은 원장이 이미 든 등록), **일 없는 문장과 침묵하는 문장이 같은 모양이 된다**. 🔴 **등록 프로브를 «선언 안 한» 소스에는 `None`을 넘긴다** — 빈 집합으로 바꿔치우면 `runtime_v2._filtered_event_atoms`의 `registration_context_required` 거절을 삼킨다. ⚰️ **[2026-09-08] 반환과 읽기 폭이 «둘 다» 바뀌었다.** 종전 이 자리는 「반환 `(rows_read, preview_or_None)` · `None` 은 「0행 읽음」이라는 답」이었다. 🆕 🔴 **`TestRunReading(rows_read, pages, preview, refusals, excluded_rows)`**(`de8808b4`) — 그 쌍으로는 「0행 읽음」과 「1000행 읽고 하나도 못 만듦」을 «구별할 수 없어서»이고, 둘은 «반대 행동»을 부른다. 🔴 **«첫 페이지»가 아니라 «분자가 나올 때까지 이어 읽는다»** — 그리드가 만든 키 NULL 행과 초기 이력 행이 커서 순서의 «앞»에 앉으므로 머리 200행은 운영자 데이터가 «가장 없을» 창이다(소유자 09-07). 상수 둘(`PREVIEW_MIN_MOLECULES`·`PREVIEW_MAX_PAGES`)이 멈춤과 예산이고 «선언 축이 아니다». 🔴 **응답이 «값으로» 더 낸다**: `refused {count, reasons, samples}`(`4f1001ac`) · `excluded {rows}`(`b38065fc`, 표지 «없는» 소스에는 «키 자체가 없다** — `0` 은 「재 봤고 없다」라는 «다른 주장»이다). 제외됨 ≠ 거절됨 ≠ 없음 |
...
| 🆕⑯ **`resolve_pace(name, paces=None)` / `load_paces(path=None)`** | **표는 여기 «없다»** — 둘 다 [`server/pacing.py`](#5-j-serverpacingpy--pacingjson--긴-작업이-얼마나-미는가-표-하나-49988247-신설) 로 위임하는 얇은 껍질이다. 🔴 **여기 남은 것은 «거절의 모양»뿐**: `pacing.UnknownPace` 를 `LedgerSetupError("unknown_pace", "pace", …)` 로 갈아 낸다 — 이 경로의 다른 거절이 전부 그것이고, 「선언 안 된 것을 달라 했다」에 예외 타입이 둘이면 호출자가 결국 하나만 잡는다. ⚠️ **`pacing.py` 가 원본이라는 사실을 지우지 마라** — 두 이름이 여기 있어서 「원장이 자기 표를 든다」로 읽히기 쉽다 |
| 🆕⑯ 페이싱 루프(`_run_v2_lineage` 안) | `pages_per_cycle, rest_seconds = resolve_pace(pace)` 를 🔴 **첫 페이지 «전»에** 푼다 — 오타를 「쓰다가 중간에」가 아니라 「쓰기 전에」 거절한다. 쉬는 자리는 **페이지 사이**이고, 🔴 **그건 멈춤(`checkpoint`)이 쓰는 «그 경계»다** — 직전 페이지의 원자와 커서가 한 커밋으로 내려갔고 다음 페이지는 아직 안 읽은 지점이라, 여기서만 «멈춤이 공짜이고 재개가 정확»하다. 조건은 `pages_per_cycle and result["batches"] and rest_seconds and (batches % pages_per_cycle == 0)` |
| 🆕⑯ `checkpoint` 훅(`_run_v2_lineage` 안) | 페이지 루프 «머리»에서 `checkpoint(result["rows_read"])` 를 부르고 참이면 **`result["stopped"] = True`** 후 break. 🔴 **취소는 이 훅 «하나»로 들어온다** — [§5-D](#5-d-2026-08-04-신설-서버-모듈) 의 `retroactive._checkpoint(control)` 가 `RunControl.stop_requested` 를 감싸 넘긴다. 백필 자신은 `RetroactiveRun` 도 세션도 모른다 |
| 🆕⑯ **`rows_past_cursor(engine, setup, source, limit=DEFAULT_FETCH_ROWS) -> (rows, complete)`** | **읽기 전용, 커서 무이동.** 소급 등록부의 `ledger_backfill` 카운트가 부르는 그것. 🔴 **`preview_first_batch` 로 이 질문에 답하면 «틀린다»** — 그쪽은 관계의 «첫 페이지»를 컴파일하므로 오래전에 번역된 행을 「남은 일」로 센다. 소스가 수를 적어 둔다(2026-08-31, `dt_transfer`): 첫 페이지 **199행** 대 커서 뒤 실제 **0행**. 그 수를 냈으면 운영자는 할 일이 있다고 읽고 눌렀다가 아무 일도 안 일어나는 것을 보고 **버튼이 고장 났다**고 판단한다. 🔴 **`complete` 는 페이지가 «짧게» 돌아왔을 때만 True** — 그때는 그 페이지가 곧 나머지라 수가 정확하고, 꽉 찼으면 호출자가 `sample` 이라고 말해야 한다(「12건 남음」과 「최소 200건 남음」은 다른 문장이다) |
| 🆕⑯ `preview_rescope(engine, setup, source, scope_column, scope_values)` · **`rescope(engine, setup, source, scope_column, scope_values, apply=False)`** | 「고친 입력이 원장에 닿지 못한 «그 범위만» 다시 번역」. 🔴 **회수 → 재작성 «순서»이고 커밋이 «둘»이다** — 회수는 `source_who` 가 삭제 술어에 들어가므로 **다른 소스가 같은 다이에 쓴 원자에는 여기서 닿을 수 없다.** 재작성은 `runtime_v2.execute_scoped_batch`(= `store.write_batch(advance_cursor=False)`)로 간다. ⚠️ **둘 사이에서 죽으면 원자는 빠지고 아직 안 쓰인 상태로 남고, 같은 범위를 다시 돌리면 끝난다** — 그래서 소급 등록부가 이 op 를 `cancellable: False` / `restartable: True` 로 선언한다 |
| 🆕⑯ `_scope_predicate(plan, scope)` · `count_orphan_atoms(engine, source, scan_limit=ORPHAN_SCAN_LIMIT)` · `_ref_row_keys(ref)` · `ORPHAN_SCAN_LIMIT = 4000` · `ORPHAN_CHUNK = 1000` | 범위 술어 조립과 「소스 행이 사라졌는데 원자만 남은 것」 계수 |
| 🆕⑳ `_run_v2_lineage` 의 **run result 신설 키 «셋»** (`71b0de61`) | **`refused_total`**(= `sum(gate.refusals().values())`) · **`refused_samples`**(= `gate.samples()`) · **`refused_samples_capped`**(= `refused_total > len(refused_samples)`). 🔴 **주소가 «읽히는» 자리가 여기다** — 게이트는 `check_envelope` 착지 이래 `(code, path)` 를 만들고 있었고 운반체가 셋이었는데 **읽는 쪽이 없었다**(리포트의 유일한 운영 호출자가 그것을 버렸다). ⚠️ **상한을 «수»로 말한다** — `refused_samples` 는 `gate.MAX_REFUSAL_SAMPLES`(20)에서 멈추므로 「400 거절 · 20 주소」가 「거절 20건」으로 읽히면 그것이 **절단을 부재로 읽는** 그 실패다. 그래서 문장이 아니라 **수 둘 + 불리언**이다. ⚠️ **`gate` 는 함수 «안»에서 import 한다** — 이 파일의 지연 import 규약 그대로 |

#### 🆕⑯ `runtime_v2.execute_scoped_batch` — 커서를 **건드리지 않는** 쓰기 (`9eb30691` 실측)

`execute_scoped_batch(snapshot, source_id, base_rows, scope, join_reader, preparers, mappers, store, *, known_registrations=None, withdraw_refs=None) -> CursorBatchExecutionResult`  🆕㉖ (`withdraw_refs` 는 `ddf3fb10` — 위 `write_batch` 행)

- 🔴 **「한 소스의 «지목된 부분»만 다시 한다. 같은 게이트, 같은 번역, 커서는 손대지 않는다.」** 커서를 배치의 마지막 행으로 쓰면 워터마크가 「여기까지 읽었다」를 **더 읽은 소스에 대해** 말하게 되고, 범위가 «현재 위치보다 뒤»에 있으면(교정은 대개 그렇다) 워터마크를 **거꾸로 끌어당긴다** — 그 사이 행이 전부 다시 읽히거나 다음 교정 때 건너뛰어진다. **썼다가 되돌리는 것**은 그 위험에 더해 「크래시가 틀린 값을 커밋된 채로 남기는 창」까지 얹는다. **그래서 커서 문은 «고치는» 것이 아니라 «내지 않는다».**
- 🔴 **`_require_scope(base_rows, scope)` 가 장식이 아니다.** 커서 문이 없으면 이 함수는 「이 행들에 원자를 쓰고 흔적을 남기지 않는다」이고, 그것이 **소스 전체에 열려 있으면 상설이 금하는 «두 번째 문»**이다. 그래서 「범위를 넘겼나」가 아니라 **「배치의 «모든 행»이 호출자가 선언한 값을 대는가」**를 증명한다 — 소스 전체 배치는 자기가 뭐라 주장하든 통과하지 못한다.
- ⚠️ `unwritten_cursor` 를 **계산은 한다**(미리보기 계약이 「배치의 물리적 한 행」인 커서 튜플을 요구한다) — 그리고 안 쓴다. 저장된 커서를 «읽어» 넘기지 않는 것도 결정이다(읽고-쓰기 경쟁이 되어 같은 되감기를 긴 길로 낸다).
- **원자 회수는 여기 «없다»** — `backfill.rescope` 가 `source_raw_ref` 로 «먼저» 지운다. 이 함수만 부르면 세대를 **교체하는 게 아니라 더한다.**
...
| 심볼 | 무엇인가 |
|---|---|
| `SPEC_EXCLUDED = ("observed@1",)` | 🔴 **이 파일에서 이름이 대지는 유일한 술어**이고, 선언에서 유도할 수 없어 스펙 §1 배제 ③ 을 «인용»한 것이다 — **부재가 좋은 결과**인 술어(결함 없는 다이는 좋은 다이). 여기서 «정하지» 않는다 |
| `NAMES_PATH` · `load_names(path=None)` · `_bare(name)` | `gap_names.json` 경로/로더 · `of_kind@1 → of_kind`(표는 맨 철자, 선언은 버전째 — 버전은 선언의 것이지 이름의 것이 아니다) |
| `class GapTableMismatch(ValueError)` | 선언과 표가 어긋남. 🔴 **추측하지 않고 거절한다** — 스스로 이름을 채운 검출기는 «자신 있게 틀린» 답을 내고, 틀린 답은 맞은 답과 똑같이 생겼다 |
| **`vacuous_types(declaration) -> {(predicate, type)}`** | 🔴 **0 이 «아니고» 행을 지우지도 «않는다».** 노드는 저장되지 않고 원자의 키에서 유도되므로, 어떤 술어가 그 타입의 **어휘 안 «유일한» 등장**이면 그 술어 없는 그 타입의 노드는 **애초에 존재할 수 없다** — 「봤는데 없더라」가 아니라 **구성상 공집합**이다. 0 을 내면 전자로 읽히고, 행을 빼면 다음 사람이 같은 질문을 다시 한다. **한 규칙이 주어·목적어 양쪽을 덮으므로** 새 술어가 와도 여기 손댈 일이 없다 |
| **`questions(declaration, names=None) -> list`** | 🔴 **순수 — DB 를 «한 줄도» 안 탄다.** 표의 `pairs`/`subject_sides`/`object_sides` 를 선언에 대고 풀어 `{form, type, present, absent, name, action|meaning, vacuous}` 목록을 낸다. `pairs` 한 행은 **질문 «둘»**(`a_only`·`b_only`)이 된다. 배제 ① (목적이 노드가 아닌 술어)은 «삼키지 않고» `"해당 없음"` 행으로 **보여 준다** |
| `_refuse_unnamed(declaration, live, named)` | 위의 «다른 방향». 배제 ② 도 여기 산다 — 🔴 **주어 타입과 목적어 타입이 «같은» 술어는 사슬**이고, 사슬은 머리와 꼬리가 늘 있어서 세면 화면이 「빠진 일」이 아니라 「사슬의 모양」으로 찬다. 선언에서 기계적으로 나오고 **양쪽 다** 적용된다 |
| `NODE_SCAN_LIMIT = 200` · `SAMPLE_NOT_AGE_ORDERED` | 🔴 **요청 경로라서 작다** — 소스가 적은 수치(2026-08-31): 상한 없는 안티조인 «하나»가 **26.20초**. 🔴 **표본은 «가장 오래된 N» 이 아니고 그렇게 «말한다»** — 나이순으로 고르려면 그 타입 전체를 먼저 훑어야 하고 그게 이 예산이 피하는 비용이다. 돌아온 것들끼리만 오래된 순이다 |
| `_nodes_of_type_sql()` | 🔴 **주어 «와» 목적어 양쪽에서 모은다.** 노드는 저장되지 않으므로 «어느 쪽이든» 원자가 이름 대는 순간 존재하기 시작한다. 소스가 적은 실측: `defect_kind@1`·`recipe@1` 은 자기 원자가 **0**이고 목적어로만 나오므로, 주어만 세면 「나이가 없다」가 아니라 **「멤버가 아예 없다」**로 보고된다 |
| `_has_predicate_sql()` | 🔴 **방향으로 «분기하지 않는다»** — 나갔어야 할 술어와 들어왔어야 할 술어는 «같은 노드에 대한 같은 질문»이고, 표현을 둘로 쓰면 질문마다 어느 쪽인지 정해야 하며 그 둘이 갈라질 자리가 생긴다 |
| `class GapQuestionUnknown(ValueError)` | `only` 가 표에 없는 이름. 🔴 **빈 결과로 답하지 않는다** — 오타의 빈 결과는 「이 결측은 없다」라는 «좋은 소식»과 똑같이 읽히고, 운영자는 그걸 보고 넘어간다 |
| **`measure(engine, declaration, names=None, scan_limit=NODE_SCAN_LIMIT, only=None) -> list`** | **읽기 전용**(`finally: rollback + close`). 🔴 **`only` 는 화면 예산 때문에 있다** — 스물을 한꺼번에 세면 «30초»이고 화면의 판별식은 「열고 3초 안에」다. `questions()` 가 공짜이므로 이름 스물을 즉시 주고 «펼 때» 값을 낸다(배치의 약 1/20). 🔴 **수마다 «어떤 수인지»가 붙는다** — `count_kind` 는 스캔이 그 타입 «전부»를 봤을 때만 `exact` 이고 아니면 `sample`, 성립하지 않는 질문은 `absence="not_applicable"` 에 **`count=None`**(0 이 아니다) |

`gap_names.json` 구조(값은 스펙 소관 — 여기서는 «모양»만): `pairs[]` = `{type, side_a[], side_b[], a_only, a_only_meaning, b_only, b_only_meaning}` · `subject_sides[]`/`object_sides[]` = `{predicate, type, name, action}`. 🔴 **`register` 는 «한 이름»을 세 주어가 나눠 쓴다** — 운영자가 하는 일이 셋 다 같기 때문이고(등록 기록을 가져온다), 그래서 네 번째 주어가 와도 새 이름이 필요 없다.

**소비자**: `ledger_trace_router.ledger_gap_catalogue`(`GET /api/ledger/gaps`) «하나». **채점자**: `server/tests/test_ledger_gaps.py` — 🆕⑯ **263줄 / `def test_` 8**.
...
### 🔒 `server/ledger/trace.py` (🆕⑭ **1,700줄** @`2a4e51cf` — 🆕⑬ 1,688 @`3b77fbc3`, 구 표기 2,039줄(🆕⑦/🆕⑩ `5359fdd` 실측)이고 그 사이에 **장사 다섯이 떠났다**. 구: 2,039줄(🆕⑨ `5359fdd` 실측 — 구 표기 1,937) @`a729a7f`) — 읽기 측 **[동결]**

> 🔒 🆕⑪ **[2026-08-23 은퇴 울타리 A군] 이 절(`ledger/trace.py`)은 «동결»이다** — 지도에서 지우지 않되 **살아 있는 표면으로 갱신하지 않았고, 새 작업의 출발점으로 읽지 마라.** 줄 수는 🆕⑫가 `bd0c192e`에서 재확인했다: **2,039**(무변동).
>
> ⚰️ 🆕⑬ 🔴 **[2026-08-29] 아래 «살아 있는 읽기 측 여섯» 은 이제 «하나»다** — `ledger_subgraph` 만 남았고 `ledger_catalog`·`ledger_composition`·`ledger_selection`·`ledger_siblings`·`ledger_trends` 다섯은 **삭제됐다**(`3b77fbc3` 실측). 아래는 그 다섯이 살아 있던 때의 서술이다.
> 🆕⑫ 🔴 **[2026-08-23 후속 정정] 「이 절과 아래 `ledger/trace_router.py` 절은 동결」이라 적혀 있었다 — 라우터는 «더 이상 파일 통째로 동결이 아니다».** 그 파일은 살아 있는 읽기 측 여섯(`ledger_subgraph`·`ledger_catalog`·`ledger_composition`·`ledger_selection`·`ledger_siblings`·`ledger_trends`)의 **유일한 HTTP 표면**이라 통째 동결이 그들의 문을 잠갔다. 라우트 16개가 **8 동결 / 8 열림**으로 갈렸다 — 아래 절의 🆕⑫ 표.
>
> 🆕⑫ 🔴 **이 «동결» 모듈이 얹고 있는 공용 배관은 아무것도 얼리지 않는다.** `relation_exists`(카탈로그 탐침) · `_fetch`(DBAPI/SQLAlchemy 두 모양을 받는 실행 어댑터) · `ResolverConfigError` · `REASON_RELATION_ABSENT` · `rollup_subject_types` · `SqlClaimLookup`. 🔬 **`_fetch`가 그 부류라는 것은 실측이 강제한다** — 열린 여덟 모듈이 **8/8 전부** `from ledger_trace import _fetch, relation_exists`로 시작하므로, `_fetch`를 동결 계산으로 세면 라우터의 16라우트가 **전부** 동결이 된다. 이 절에서 **얼리는 것은 계산 심볼 `trace`와 `coverage`뿐이다.**
...
| 🆕⑭ `DEFAULT_RESOLVER_CONFIG` | 계급표는 `if` 사다리가 아니라 **선언 데이터**. 키는 **아홉**이다(구 표기가 여덟이라 `confirmed_derivations` 를 빠뜨리고 있었다) — `pin_predicates`(기본 `["pin"]`) · `confirmed_predicates`(기본 `["frame_confirmed"]`) · `confirmed_sources` · `confirmed_payload_flag` · **`confirmed_derivations`** · `inference_sources` · `inference_payload_flag` · **`inference_derivations`**(기본 `["slot_preserving"]`) · **`display_timezone`**(기본 `"Asia/Seoul"`). 🔴 **`confirmed_derivations` 는 2026-08-29 에 «비워졌다»**(소유자 판정) — `job_run_to_confirmed_container` 는 «문법»으로 살아 있지만 이 박스의 어느 선언도 그것을 낼 수 없어서(전 소스가 `kind: null`), 아무도 설 수 없는 계단으로 순위를 매기고 있었다. 🔴 **되돌아오는 조건이 «강제되는 자리»에 있다** — `test_ledger_trace_contract.py::test_every_declared_derivation_is_explicitly_classified` 가 「config 가 찍을 수 «있는»데 여기 어느 목록도 분류하지 않는」 derivation 에서 빨개진다. 확인하는 transfer 소스를 선언하는 날 그 시험이 이름을 되돌리라고 «말한다» |
...
| `class ClaimLookup` | 🔴 **본체가 비어 있습니다** — 프리미티브 둘(`reachable_lots`·`claims_for_lots`)과 `neighbourhood` 는 계보 walk 은퇴(2026-08-27)와 함께 나갔고, 독스트링만 그것들을 설명한 채 남아 있습니다. 하위 `InMemoryClaimLookup`·`SqlClaimLookup` 도 같은 상태입니다 |
...
| 페이로드 리더 | 🆕⑬ **[2026-08-29] 여덟 중 «셋이 없다»** — 살아 있는 것: `_object_key(claim, name)`(정체성) · `_object_qualifier(claim, name)`(서술) · `_payload_lot` · `_slot_text(value)` · `_as_text`. 🪦 없는 이름: ~~`_payload_wafer`~~ · ~~`_payload_slot`~~ · ~~`_slot_map_pair(claim)`~~. 🔴 **`keys`/`qualifiers` 분리가 읽기 측에서도 살아남아야 한다** — 평평한 페이로드는 `slot`을 웨이퍼 정체성으로 읽는다. 평평한 철자도 **받아 주는** 것은 관용이 아니라 **룩업 교체 가능성의 조건**이다(머티리얼라이즈 프로젝션·손픽스처가 자연히 그 모양을 낸다). `_slot_text`는 `3`/`"3"`/`"03"` 세 출처를 선행 0을 떼고 텍스트로 비교한다 |
...
| 🆕⑬ `_hop(frm, to, resolution, predicate, zone, config=None)`(구 표기에 `config` 인자가 빠져 있었다) | 핀된 홉 모양 — `from`·`to`·**`predicate`**·`state`·`rank`·`n`·`reason`·`occurred_at`·`event_id`. `predicate`는 핀된 dict에 **더해진** 한 필드다(개명도 삭제도 아니다) |
| 🪦 🆕⑬ ~~`trace(lot, slot=None, lookup=None, config=None, max_depth=DEFAULT_MAX_DEPTH)`~~ — **[2026-08-29] 이 함수는 없다**(`^def trace` 히트 0). 계보 물음은 `GET /api/ledger/subgraph` 에 `follow` 를 주어 답한다. 다음은 살아 있던 때의 서술이다 | 🔴 **`hops`가 빈 리스트인 것은 구성상 불가능하고 그것이 기능 전부다.** 빈 원장에도 원자 0인 lot을 지목하는 `unresolvable` 홉 하나 + `terminal_reason`을 낸다(끝에서 `assert hops`). `lookup=None`은 `ValueError` — 해결과 룩업은 일부러 분리돼 있다. 홉은 **질문**의 열이다: `has_wafer(lot, slot)` → `derived_from(lot)` → `slot_map(lot→parent, slot)`. 반환 `{hops, terminal_reason, generated_at}`이고 **`generated_at`도 같은 선언 zone**이다(다른 시계면 눈으로 9시간을 빼게 된다). terminal 태그 — `[unknown_subject]` · `[root]` · `[dead_end]` · `[broken]` · `[cycle]` · `[depth_cap]` |
| 🪦 🆕⑬ ~~`_map_slot(index, cur_lot, parent, cur_slot, cfg)`~~ — **[2026-08-29] 없다** (`trace` 와 같은 구간에 갔다). 아래는 그때의 서술 | 한 계보 홉을 건너 슬롯을 나른다. **양방향을 다 찾는다** — §4.2가 `from`이 어느 쪽인지 핀하지 않으므로 **어느 랏이 주어인가**라는 원자의 사실로 방향을 정한다. 둘 다 안 맞는 원자는 **읽지 않는다** — 틀린 슬롯이 화면에 가느니 정직한 `unresolvable`(`[no_slot_map]`) |

### `server/ledger/trace_router.py` — 🆕⑯ **라우트는 «셋»이다** (🆕⑬ 「둘」에서 `GET /gaps` 가 붙었다)
...
>
> 🆕⑬ 🔴 **[2026-08-29] 위 🆕⑫의 「16라우트 · 8 동결 / 8 열림」 판정은 폐기됐다 — 얼릴 것도 열 것도 «파일이 없다».** 개정 6이 키를 받는 데이터 라우트를 통째로 지웠고(`/trace`·`/coverage`·`/explore`·`/explore_entity`·`/entities`·`/journey`·`/structure`·`/lots`·`/lot_map`·`/siblings`·`/trends`·`/composition`·`POST /selection/resolve`·`/subgraph/table`), 그 몸이던 모듈도 같이 갔다(아래 묘비 표). **남은 라우트는 둘이고 울타리를 그을 짝이 없다.**
>
> 🆕⑬ **import는 15가 아니라 «둘»이다**(🆕⑭ `2a4e51cf` 재확인, 표준·프레임워크·`database.database.get_db` 제외): `from ledger_api import ledger_subgraph` · `import ledger_trace`. `main.py`의 등록은 여전히 두 줄이고(`import ledger_trace_router` + `app.include_router(...)`, SPA catch-all보다 **위** — 등록 순서가 계약이다) 그 사실만 무변동이다.
>
> 🆕⑭ 🔴 **그러나 이 파일은 «선언»을 세 함수에서 지연 import 한다** — `_followable_predicates`·`_static_types`·`_static_step_predicates` 가 각각 함수 «안»에서 `from ledger import config as _config` 를 하고 `/declaration` 핸들러도 같다. 즉 **읽기 라우터의 답 넷이 `ledger.config.load()` 위에 서 있고**, 그 함수는 §5-H 쓰기 측 표에서 «동결」 딱지가 붙은 `server/ledger/config.py` 의 것이다. 「새 작업의 출발점으로 읽지 마라」는 그 딱지와 **여기가 어긋난다** — 동결은 «고치지 마라»이지 «안 돈다»가 아니고, 이 네 답은 그것이 돌아야 나온다.
>
> 🔴 🆕⑬ **모듈 docstring이 자기 파일을 반박한다.** 첫 줄이 「the ledger read routes — ten of them」이라 적고 `subgraph`·`subgraph/table`·`siblings`·`trends`·`composition`·`selection/resolve`·`kinds`·`declaration`·`structure`·`lot_map` 열을 나열하는데, **파일에 있는 `@router.` 는 둘뿐이다.** 코드 소관이라 여기서 고치지 않았다 — **docstring을 라우트 목록의 출처로 쓰지 마라.** (🆕⑭ `2a4e51cf` 재확인 — **여전히 그렇다**.)

**🆕⑯ 라우트 전건 (`9eb30691` 실측 — `grep -c '^@router\.'` = **3**. 🆕⑭ 는 `2a4e51cf` 에서 **2** 였다)**

| 라우트 | 핸들러 | 부르는 계산 |
|---|---|---|
| `GET /subgraph` | `evidence_subgraph` | `_signed_start` → `_evidence_graph` → `ledger_subgraph.subgraph` + `ledger_subgraph.SqlEvidenceLookup`. `ledger_trace` 접촉은 `relation_exists`·`_fetch`(배관)뿐 |
| 🆕⑯ **`GET /gaps`** | **`ledger_gap_catalogue(name: str = Query(None))`** | **[2026-08-31 신설]** `ledger.config.load()` → `ledger.gaps`([§5-H](#5-h-정본-원장-canonical-ledger) 의 `gaps.py` 별항). 🔴 **라우트는 «하나»이고 인자가 둘로 가른다 — 새 라우트가 아니라 «같은 질문의 두 배율»이다**: 인자 없으면 `gaps.questions(declared)` 로 **질문 «이름»만**(선언만 읽으므로 DB 무접촉·즉시, `{mode:"names", count, gaps}`), `name=…` 이면 `gaps.measure(engine, declared, only=name)` 로 **그 질문 하나**(~1초, `{mode:"measured", count:1, gaps}`). 화면의 판별식이 「열고 3초 안에 끊을지 정한다」인데 스물을 한꺼번에 세면 30초다. 🔴 **비-200 이 셋이고 서로 «다른 사실»이다**: 선언을 못 읽으면 **503 `declaration_unreadable`** · 없는 이름이면 **404**(`GapQuestionUnknown` — 빈 목록으로 답하면 「이 결측 없음」이라는 좋은 소식과 똑같이 읽힌다) · 선언과 이름표가 어긋나면 **503 `gap_table_mismatch`**(🔴 **500 이 아니다** — 둘 중 하나를 사람이 고쳐야 하는 «배포 사실»이지 이 요청의 실패가 아니다) |
| `GET /declaration` | `ledger_declaration_catalog` | **원장을 한 줄도 읽지 않는다** — `ledger.config.load()`의 `entities`/`vocabulary`를 읽어 그대로 낸다. **DB 세션 인자 자체가 없다.** 🆕⑯ **`sources[]` 절이 붙었다** — 아래 별항 |
| 🆕㉔ **`GET /key-values`** | **`ledger_key_values(type, key=None, limit=50)`** | **[2026-09-06 신설]** 「씨앗 칸에 «무엇을» 적나」 — 원장에 **오늘 있는** 주어를 센다. 타입·키를 선언에 대고 검증하고(`_collectable_types()` · `_declared_keys()`), 선언에 없으면 422(`node_type_not_declared` / `key_not_declared`). `key` 를 **안 주면** 그 타입이 선언한 키 **전부**로 묶어 «주어»를 답한다 — 축별로 따로 물으면 화면이 «교차곱»을 만든다. 🔴 **절단이 «둘»이고 «따로» 보고한다** — `scan_truncated`(행을 다 못 봤다) 와 `values_truncated`(값이 더 있는데 안 실었다). 한 표지로 접으면 「이 키에는 값이 이만큼뿐」과 「이만큼까지만 봤다」가 **같은 답**이 된다. 사유는 소스의 docstring 이 적는다 |

| 심볼 | 무엇인가 (🆕⑭ `2a4e51cf` 실측) |
|---|---|
| `router = APIRouter(prefix="/api/ledger", tags=["ledger"])` | 자족적 라우터 — `main.py` 등록이 두 줄 |
| `LEDGER_RELATION = "ledger_events"` · `LEDGER_CURSOR_RELATION = "ledger_translator_cursor"` | 인라인이 아니라 이름 붙인 이유가 **이음매**라서. ⚠️ **`LEDGER_CURSOR_RELATION`의 docstring은 「`GET /coverage`가 `sources`로 보고하는 것」이라 적는데 그 라우트는 없다** — 상수는 살아 있고 설명만 낡았다 |
| 🆕⑭ `@router.get("/subgraph") evidence_subgraph(node_id=Query(..., alias="id"), hops=12, direction="both", node_limit=400, edge_limit=1200, positive=None, negative=None, follow=None, backbone_hops=ledger_subgraph.DEFAULT_BACKBONE_HOPS, db)` | **인자 아홉**(수는 그대로, **아홉째의 이름이 바뀌었다**). `hops` `ge=1 le=40` · `direction` `^(outgoing\|incoming\|both)$` · `node_limit` `ge=10 le=1000` · `edge_limit` **`ge=20, le=ledger_subgraph.MAX_EDGE_LIMIT`**(= **6000**, 구 표기 3000 — 라우터가 수를 옮겨 적는 대신 **상수를 참조**하게 바뀌었다) · `backbone_hops` `ge=0 le=40`. `positive`/`negative`/`follow`는 **반복 쿼리 파라미터**다. 🔴 **`continues_hops` 는 없다** — 2026-08-29 저녁에 `backbone_hops` 로 개명됐고 **옛 철자의 별칭을 받지 않는다**(소스 주석: 소비자가 둘 다 우리 것이라 호환층은 걷을 사람이 없어진다). 🔴 **`enrich_actions`·`observations`·`include_values`·`shape`·`property_limit`은 없다**<br>🆕㉓ 🔵 **[2026-09-06] `collect` 는 «돌아왔다» — 다만 «다른 축»이다.** 구 `collect` 는 «배관 종류»(entity·point·claim·collection)를 골랐고 2026-08-28 에 은퇴했다. 새 `collect: list[str]|None` 은 «도메인 노드 타입»(die·defect…)을 고르고, 값은 `/declaration` 의 entities 에서 오며 버전 접미를 «양쪽에서 벗긴다». 안 주면 «전부»(무회귀). 🔴 **걷기는 안 바뀐다 — 필터는 «응답 조립» 자리다**(`follow` 가 «길», `collect` 가 «짐»). 선언에 없는 이름은 «미지/선언 집합»을 본문에 담아 422. 검증은 `_collectable_types()`. 📌 축 셋의 정본은 [`WALK.md`](WALK.md) |
| 🆕⑬ `_followable_predicates()` | `follow`에 쓸 수 있는 이름 — **선언에서만 읽는다**(`ledger.config.load()["vocabulary"]`의 키를 `@` 앞에서 자른다). 🔴 **선언에 없는 술어는 «빈 그래프»가 아니라 422다**(`reason: "predicate_not_declared"` + `unknown` + `declared`) — 영원히 참일 수 없는 필터는 진짜 부재와 구분되지 않는다. 읽지 못하는 선언은 **빈 집합**을 답해 전부 거절한다 |
| 🪦 🆕⑭ ~~`_continuing_predicates()`~~ | **함수가 없다**(`2a4e51cf` 실측). 술어별 `continues: true` 플래그가 **엔티티의 `class`** 로 대체됐다 — 아래 둘이 그 자리를 물려받았다. 🔴 **앵커로 쓰지 마라** |
| 🆕⑭ `_static_types()` | 선언이 `class: "static"` 이라 적은 **엔티티 타입**들 — 사건이 아니라 «이름». `ledger.config.load()["entities"]` 를 읽어 `@` 앞에서 자른 맨 이름의 집합. 🔴 **못 읽으면 «빈 집합»이고 그것이 오늘의 걷기 그대로다** — 어느 타입이 허브인지 추측하지 않는다. ✅ **재측정 `3a8494dc`: 샘플이 `class: "static"` 을 «셋» 든다**(`quantity@1`·`defect_kind@1`·`recipe@1`) — `0fe0ec09` 에서 라이브와 같은 모양으로 옮겨졌다. 이 집합이 비는 것은 `sample/ontology/transfer_explorer/` 쪽뿐이고, 그 선언에는 정적일 엔티티 자체가 없다 |
| 🆕⑭ `_static_step_predicates()` | 주어와 목적어가 **둘 다** static 인 술어만 — static 노드를 펼칠 때 쓰는 좁은 `follow`. 같은 모양, 같은 이유(못 읽으면 빈 집합 = static 노드를 아예 안 펼침). 🔴 **정책이 «한 층 늦게» 걸려 있던 것을 앞당긴 자리다** — `s -> d` 원자를 투영에서 버리던 시절엔 이미 질의가 가져와 claim 예산을 다 쓴 뒤였다. 소스가 수로 적어 둔다(defect 씨앗 하나 `hops=4`): `of_kind` 포함 → claim **6,000**(천장)·노드 13·hop 2 에서 정지 / 제외 → claim **371**·노드 315·hop 4 도달 |
| 🆕⑬ `_signed_start(node_id, positive, negative)` | 부호 있는 seed의 **유일한 조립 지점**. 🔴 **둘 다 비면 `node_id`를 «그대로» 돌려준다** — 아무것도 안 찍은 요청은 예전과 **같은 인자**로 `subgraph()`에 닿는다. `id`는 항상 positive다 |
| 🆕⑭ `_evidence_graph(connection, *, node_id, hops, direction, node_limit, edge_limit, follow=None, backbone_hops=ledger_subgraph.DEFAULT_BACKBONE_HOPS)` | 관계 존재 확인(`ledger_trace.relation_exists`) → `_subgraph_contract_state` → `ledger_subgraph.subgraph(...)`. 🔴 **선언에서 읽은 것 «둘»을 여기서 넘긴다** — `static_types=_static_types()` · `static_follow=_static_step_predicates()`. (`continuing=` 은 없다) |
| `_subgraph_contract_state(connection)` | 느린 스캔 «전»에 배포 결손을 이름으로 답한다 — `source_event_id`·`source_event_state` 컬럼과 `idx_ledger_source_event`·`idx_ledger_object_entity` 인덱스 넷. 빠지면 503 `source_event_projection_not_deployed` |
| `SQLSTATE_UNDEFINED_TABLE = "42P01"` · `_is_undefined_table(exc)` | 관계 부재 판정이 문자열 매칭이 아니라 SQLSTATE — `exc.orig`와 `exc` 양쪽의 `pgcode`. 모든 로캘·모든 드라이버에서 같은 다섯 글자 |
| `_relation_absent() -> HTTPException` | 부재-관계 거절의 **유일한 철자** — `{reason: ledger_trace.REASON_RELATION_ABSENT, state: "absent", relation, message}`. 클라는 `detail.reason`으로 분기하고 운영자는 `detail.message`를 읽는다 |
| 🆕⑬ `@router.get("/declaration") ledger_declaration_catalog()` | 「무엇을 물을 수 있나」 — `{state, entities[{type, keys}], predicates[{name, subjects, object, origin}]}`. 🔴 **목록을 코드에 다시 적지 않는다**: 선언에서 술어를 하나 지우면 이 답도 하나 준다. 🔴 **`subjects`는 버전째로**(`die@1`) 나간다 — 화면이 주어로 옵션을 거르는데 맨 철자면 아무것도 안 맞는다. 🔴 **빈 목록은 답이지 오류가 아니다**(목적어로만 나오는 타입은 나가는 술어가 없다). 유일한 비-200은 선언을 못 읽을 때의 503 `declaration_unreadable`. 🔴 🆕⑭ **`entities[]` 는 `{type, keys}` «둘»뿐이고 `class` 를 싣지 않는다**(`2a4e51cf` 실측) — 즉 **화면은 어느 타입이 static 인지 이 라우트로 알 수 없다.** 서버는 `_static_types()` 로 그것을 쓰고 있으므로, 걷기 결과가 왜 그 방향으로 안 갔는지를 클라가 설명할 재료가 «없다». 코드 소관이라 여기서 고치지 않고 사실만 적는다 |
| 🆕⑯ 같은 라우트의 **`sources[]` 절** (`9eb30691` 신설) | `[{source, relation, emits[], scope_columns[]}]`. `ledger.setup.load_setup().snapshot.source_plans` 를 돌며 만든다 — 🔴 **`scope_columns` 는 `source_preparation.base_select_columns(plan)` «그것»이고 「비슷한 두 번째 목록」이 아니다.** 범위 읽기가 «거절»에 쓰는 바로 그 목록이라, 화면이 다른 데서 옵션을 뽑으면 서버가 곧 거절할 컬럼을 내놓게 되고 운영자는 **맞는 거절을 고장 난 버튼으로** 읽는다. 그래서 **원시 선언이 아니라 «컴파일된» 계획**에서 온다(`emits` 는 컴파일 형태가 없어 선언의 `bind.mappings` 에서 읽는다 — 이 라우트의 평소 자세 그대로). 🔴 **셋업이 컴파일 안 되면 키를 «비우지 않고 뺀다»** — 이 값이 먹이는 라벨은 「원장 소스다」·「소스가 아니다」·「알아내지 못했다」 «셋»을 말해야 하는데, `[]` 로 답하면 뒤의 둘이 **똑같이 그려지고** 화면이 「이 표는 소스가 아니다」라는 «거짓»을 말한다. **키가 있으면 목록이 정본, 없으면 모름.** 나머지 카탈로그(`entities`·`predicates`)는 그래도 답한다. 🔴 **소비자는 클라 셋이다** — [§7](#7-client2src--웹-클라이언트) 의 `grid_source_label.js`·`grid_rescope_menu.js`(둘 다 `main.js` 의 `loadLedgerDeclaration` 하나를 «주입»받는다) |
| 🆕㉔ `@router.get("/key-values") ledger_key_values(type, key=None, limit=Query(KEY_VALUE_DEFAULT_LIMIT, ge=1, le=KEY_VALUE_MAX_LIMIT), db)` · `KEY_VALUE_SCAN_ROWS = 20000` · `KEY_VALUE_DEFAULT_LIMIT = 50` · `KEY_VALUE_MAX_LIMIT = 500` · `_declared_keys(bare_type)` | 🔴 **자르는 것은 «읽는 행 수»이지 «그룹»이 아니다** — 한 타입 전체에 `GROUP BY` 를 걸면 그 타입의 «모든» 행을 방문한다. 창을 «먼저» 뜨고 그 위에서 센다(같은 규율이 `enrichment_candidates.execute_candidate_probe` 에 이미 있다). 키 «이름»은 보간이 아니라 **바인드**한다 — 선언에서 온 이름이라도 그렇다. 응답의 `covers_declared_keys` 가 「이 묶음이 선언한 키를 다 덮나」를 말한다(안 덮으면 그 값은 «단독으로 씨앗이 안 된다»). 정렬은 «빈도 내림차순, 동률은 값 오름차순»이고 응답이 `order` 로 적는다 — 적지 않으면 질의가 정한다. `_declared_keys` 는 `/declaration` 과 **같은 선언**을 읽는다 — 키 목록을 여기에 **다시 적지 않는다** |
| 🆕㉖ `_declared_entities()` :409 · 같은 `/subgraph` 라우트의 **`format`** 가지 :141·:211·:216 | **새 라우트가 «없다» — 자란 것은 «인자»다**(S-183). `format` 은 `json`·`rows` 둘뿐이고 그 밖은 거절이며, `rows` 면 `PlainTextResponse` 로 나간다. ⚠️ **선언된 컬럼을 채우려면 엔터티 카탈로그가 필요하고** 그것을 이 함수가 준다 — 라이브 거절 둘이 그것을 가르쳐 줬다(`aae64301` 라우트 가지에 시험이 없어 `NameError` 출하 · `f493eb0d` 픽스처가 제품 모양이 아니라 선언 컬럼 0)<br>🆕㉜ **[S-206 `df7de5c6`] 발효가 걷기로 하여금 선언을 «다시 읽게» 한다 — 재기동이 아니라.** 🔴 그 캐시가 «리로드 좌석에 안 앉아» 있었고, 부류는 D-15 에 적힌 그대로다: **리셋 없는 캐시는 「리로드가 아무 뜻도 없게 되는」 이유**다. 이 프로세스엔 그 좌석이 이미 있었다<br>🆕㉝ **[S-149 `46498f25`, 판정 338·339] `node["absence"]` — 참·거짓 답이 «세 값»이고 `false` 는 «얻어야» 한다.** `VERDICT_TRUE`/`FALSE`/`UNKNOWN` :854~:856 · `WHY_NOT_DECLARED` :867 · `WHY_NOT_EXAMINED` :868, 판정은 `predicates[]` «옆»에 앉는다. 🔴 **모집단은 «선언»이다**(`_absence_confirmers` :741, 소스 :738 이 그렇게 적는다) — 데이터가 아니다. 즉 「검사 술어가 선언에 없으면」이 곧 「`false` 를 얻을 수 없다」다. 🔵 **그리고 `why` 가 «절단»을 든다**(:905~) — 잘린 걷기는 검사에 «닿지 못했을» 수 있으므로, 예산이 있었으면 그것이 정직한 사유다. ⚠️ 이것이 없으면 「안 봤다」가 「없다」로 읽힌다 — 상설 「미상은 0이 아니다」의 술어 판이다 |

> 🔴 🆕⑬ **소스가 소스를 반박한다 — `follow`의 참조 엣지 절반은 코드에 없다.** `_followable_predicates`의 docstring이 「`entities.<type>.references[].edge`(오늘: `in_container`)도 이름댈 수 있게 넓혔다」고 적지만, **본체는 `vocabulary`만 읽는다**(`references`라는 낱말이 함수 본체에 없다). `/declaration`의 `predicates[]`도 `"origin": "vocabulary"`를 상수로 박아 낸다. 같은 밤에 `ledger_api/declared_entities.py`가 「선언에서 `references`가 빠져 합성 엣지가 0이 됐고 `reference_edges`·`reference_edge_names`·`targets_for`는 호출자가 0이라 지웠다」고 적고 있으므로 **docstring 쪽이 낡은 것으로 보이지만, 어느 쪽이 의도인지는 소스로 판정할 수 없어 사실로 등재하지 않는다.**
>
> 🆕⑭ **`2a4e51cf` 재측정 — 둘 다 그대로이고, «세 번째 사실»이 붙는다.** `_followable_predicates` 본체는 여전히 `vocabulary` 만 읽고, 그런데 **`references` 의 «문법»은 선언 검증기에 살아 있다** — `setup_bundle._validate_entities` 가 `references` 를 optional 키로 받고 `_validate_references` 가 `from`/`edge`/`to` 를 전건 검사한다(§5-H 쓰기 측 `setup_bundle.py` 행). 즉 **「선언할 수는 있는데 아무도 안 읽는다」**가 오늘의 상태다: 문법 살아 있음 · 투영의 합성 엣지 없음(`_link_containers` 삭제) · `follow` 로 이름 댈 수 없음. 🔴 **그리고 이 박스의 tracked 선언 둘(`config/sample/ledger_config.json.sample` · `config/sample/ontology/transfer_explorer/ledger_config.json`)은 `references` 를 «하나도» 선언하지 않는다**(키 존재 계수 0/0). 그래서 「돌아가는지」를 샘플로 시험하면 세 층 다 조용히 0을 낸다.
>
> ✅ 🆕⑮ **[2026-08-30 `12a94097`] 위 «자기모순»은 해소됐다 — 이제 반박하는 것은 소스가 아니라 이 두 문단이다.** `_followable_predicates` 의 docstring 이 다시 쓰였고, 새 문장이 본체와 맞는다: 「THE BRIDGE CAME BACK AS A PREDICATE, so this list is the vocabulary and nothing else.」 **본체는 무변동**이고(`vocabulary` 만 읽는다) **바뀐 것은 docstring 하나**다. 🔴 **그러므로 🆕⑬·🆕⑭ 가 두 번 적어 둔 「docstring 이 `references` 로 넓혔다고 적는다」는 «앵커로 쓰지 마라»** — `12a94097` 이후 그 문장은 파일에 없다.
>
> 🔬 🆕⑮ **`5a73021a` 실측이 새 docstring 을 받친다**(tracked 샘플 `config/sample/ledger_config.json.sample` 구조 프로브 — 실값은 옮기지 않는다):
> ```
> in_container@1 ∈ vocabulary        True     ← 술어로 «선언»됐다. 합성 엣지가 아니다
> references 를 가진 엔티티          0 / 9    ← 낱말 자체가 선언에 없다("references" 문자열 0건)
> in_container@1 을 emit 하는 매핑    2       ← 둘 다 sources/bonded_from/bind/mappings 아래
> ```
> 🔴 **그래서 「넓히기」는 «틀린» 것이 아니라 «주어가 없어진» 것이다** — 참조 엣지가 다시 선언되는 날에만 다시 문제가 된다. 문법 세 층 중 **둘이 그대로이고 하나가 늘었다**: 검증기 문법 살아 있음(`setup_bundle._validate_references`) · `follow` 로 이름 댈 수 없음(무변동) · 🔴 **작성 폼이 이제 그것을 «그린다»** — `ledger/ledger_skeleton.json` 이 `719 → 797줄`(`5a73021a`)로 자랐고 증분 전부가 엔티티 밑 **`references` 노드 하나**다(`kind: "map"`, `keyed_by: "index"`, 레코드 필드 `edge`(필수) · `from.when`(선택) · `to.{entity,keys}`(필수), `to.entity` 는 `hint: "ref"` + `section: "entities"`). ⚠️ **`config_authoring.py` 는 이 구간에 «안 바뀌었다»** — 폼 생성기가 아니라 **선언 데이터**가 자란 것이고, 그것이 그 파일의 설계다.

### ⚰️ 🆕⑬ `server/enrichment_actions.py` — **`server/_archive/`로 이동했다 (2026-08-28)**

🪦 **`server/enrichment_actions.py`도 `server/tests/test_enrichment_actions.py`도 없다** — 둘 다 `server/_archive/` 아래에 있다(`server/_archive/enrichment_actions.py` · `server/_archive/tests/test_enrichment_actions.py`). 여기 있던 심볼 표 6행(`EnrichAction` · `enrich_action_node_id` · `decode_enrich_action_id` · `action_node` · `EnrichmentActionLookup` · `SqlEnrichmentActionLookup`)을 **묘비로 대체했다** — 운영 소스에 없는 이름이고, `/subgraph`는 `enrich_actions` 인자도 `action_lookup` 배선도 더 이상 넘기지 않는다(`ledger_trace_router._evidence_graph` 실측). `ledger_subgraph.subgraph`의 `action_lookup=None` 키워드 «자체»는 살아 있으나 **넘기는 호출자가 레포에 없다.**

⚠️ 계약 문서 [CLAIM_REQUIREMENT_WORKLIST_SPEC](../spec/CLAIM_REQUIREMENT_WORKLIST_SPEC.md)·[LEDGER_EVIDENCE_SUBGRAPH_SPEC](../spec/LEDGER_EVIDENCE_SUBGRAPH_SPEC.md)는 **이 패스가 열지 않았다** — 문서 소관(doc-keeper)이다.

### ⚰️ 🆕⑬ 읽기 측 모듈군 — **열둘이 사라졌다 (2026-08-29 실측)**

🔴 **여기 있던 심볼 표는 «없는 파일»의 표였다.** `git ls-tree -r --name-only HEAD -- server` 전건 대조로, 이 절과 아래 전용 절들이 앵커를 달고 싣던 파일이 HEAD에 하나도 없다. 이것이 §5-H가 `lot_event_translator.py`에서 이미 한 번 겪은 실패 모양이라 **표를 지우고 묘비로 대체한다.**

| 🪦 없는 이름 | 있던 곳 | 지금 |
|---|---|---|
| `ledger_kinds.py` · `ledger_siblings.py` · `ledger_structure.py` · `ledger_lots.py` · `ledger_trends.py` · `ledger_composition.py` · `ledger_selection.py` · `ledger_identity.py` · `ledger_catalog.py` · `ledger_journey.py` · `ledger_walk_contrast.py` | `server/` · `server/ledger_api/` | **삭제.** 그들이 떠받치던 라우트가 함께 은퇴했다 — 같은 질문은 `GET /api/ledger/subgraph`를 `follow`로 좁혀 답한다 |
| `finding_kinds.py` | `server/ledger_api/` | **`server/scripts/support/finding_kinds.py`로 이사.** 운영 읽기 표면이 아니라 스크립트 지원물이 됐다 — `DEFAULT_KIND = "void"` 같은 도메인 낱말이 라우트 갈래를 트는 자리였다 |
| `mechanism_gate.py` · `enrichment_actions.py` · `audit_changeset.py` | `server/ledger_api/` · `server/` | **`server/_archive/` 아래로.** 운영 소비자 «0» |
| `server/ledger/vocabulary.py` | `server/ledger/` | **삭제.** 어휘는 `ledger.config.load()["vocabulary"]`로 읽는다(🆕⑭ `2a4e51cf` 실측 — 읽는 자리는 `ledger_trace_router._followable_predicates`·`_static_step_predicates`·`ledger_declaration_catalog`. 🪦 여기 적혀 있던 `_continuing_predicates` 는 **없는 이름**이다) |
| `server/config/sample/ledger_vocabulary.json.sample` | `server/config/sample/` | **삭제** |

⚠️ **여기 있던 `ledger_walk_contrast`(1,337줄, 심볼 20여 행)와 `mechanism_gate`(370줄, 심볼 10행) 전용 절도 같이 지웠다** — 3관문·`WALK_DEFAULTS`·`REASON_*` 17종·`verdict(kind, candidate_key, graph=None)`은 **전부 없는 파일의 이름**이다. 기전 그래프는 이제 원장의 선언된 엔티티(`quantity@1`)와 술어(`leads_to@1`)라 **walk이 걸어서 닿는다**. 아카이브된 사본을 읽어야 하면 `server/_archive/ledger_api/mechanism_gate.py`이고, **거기 있는 것은 기록이지 배선이 아니다.**

⚠️ **`server/ledger/explorer.py`는 살아 있고 «줄었다»** — 445 → **175줄** (`3b77fbc3` 실측). 남은 것은 **id 코덱과 그 이웃뿐**이다: `MAX_HOPS` · `DEFAULT_NODE_LIMIT = 400` · `DEFAULT_EDGE_LIMIT = 1200` · `_canonical` · **`entity_id(entity_type, keys)`** · **`decode_entity_id(value)`** · `_entity` · `_target_of` · `_instant` · `_edge_rows` · `_depths`. 🔴 **`explore`도 `explore_entity`도 없다** — 그 둘이 이 파일의 「동결 대상」이었고, 동결 대신 삭제됐다. `ledger_subgraph`가 import하는 것은 **id 코덱**이다.

⚠️ **`server/ledger/trace.py`도 살아 있고 줄었다** — 2,039 → **1,688줄**(`^def `/`^class ` 전건 **48**). 살아 있는 것: `Resolution`/`resolve`/`claim_class`/`claim_rank_key`/`hop_basis`/`SqlClaimLookup`/`coverage`/`relation_exists`/`_fetch`/`resolve_display_zone`. 🔴 **그러나 이 지도가 이 파일에 대해 주장하는 심볼 148개를 전건 대조하니 «다섯이 없는 이름»이었다** — **`trace`** · **`_map_slot`** · `_payload_wafer` · `_payload_slot` · `_slot_map_pair`. 앞의 둘은 그 절의 «주인»이었고(계보 walk 자체), 지금 그 질문은 `GET /api/ledger/subgraph` 가 `follow` 로 답한다. 위 §5-H 의 해당 행들은 묘비로 대체했다. 🔴 `coverage()`는 있으나 **HTTP 소비자가 «0»**이다(`/coverage` 라우트가 없다) — 라우터가 이 모듈에서 실제로 부르는 것은 `relation_exists`·`_fetch`·`REASON_RELATION_ABSENT` 셋뿐이다. ⚠️ **이 패스는 그 절의 산문 서술(거절 어휘·응답 모양)은 열지 않았고 «이름의 존재»만 재섬다** — 나머지는 `a729a7f` 기준이다.

### ⚰️ 🆕⑬ 클라 — 원장 콘솔은 **삭제됐다 (2026-08-24, 소유자 판정 「ㅇㅇ 버려」)**

🪦 **페이지 셋과 그 모듈 전부가 없다.** `ledger.html` · `ledger-graph.html` · `rnd-console.html`(+ `dist/` 사본) · `ledger_trace.js`/`_core`/`_view` · `ledger_setup*.js` · `ontology_structure*.js` · `case_control_*.js` · `contrast_*.js` · `journey_*.js` · `lot_reference_*.js` · `surprise_*.js`(4모듈) · `ledger_map_panel.js` · `ledger_graph/*` · `rnd_console/*` · 그 하니스 아홉. 🔴 **여기 있던 `ledger_trace_core.js` export 25종 · `ledger_trace_view.js` 3종 · `ledger_trace.js` 뷰 다섯 · `ledger_trace_harness.mjs`의 P1–P3 서술을 전부 지웠다** — 파일이 없으므로 심볼도 없다.

➡️ **후계는 `client2/rnd-board.html` + `client2/src/rnd_board/`이고 [§7-B](#7-b-rd-보드-rnd_board-2026-08-29-신설-등재)에 등재했다.** 콘솔이 뷰 다섯을 한 URL 공간에서 갈아 끼우던 자리를, 보드는 **격자에 앉은 부품 여럿 + 바깥의 마킹 저장소**로 바꿨다.

### 채점자 (`server/tests/`)

> 🆕⑯ **[2026-08-31 재측정 — `9eb30691`]** 같은 철자(줄 수 `git show HEAD:<path> | wc -l` · 개수 `grep -c "def test_"`). 🆕⑭ 가 잰 아홉 중 **넷이 어긋나 있었고 그중 하나는 «줄었다»**(`l1_unit` +217/+7 · `subgraph` +64/+2 · `transfer_unit` **+10줄인데 시험은 −1** — 🔴 **줄 수만 보면 「자랐으니 괜찮다」로 읽히는 자리다** · `finding_kinds` 무변동). **신설 하나**: `test_ledger_gaps.py`.

| 파일 | 줄 | `def test_` | 구 표기 |
|---|---|---|---|
| `test_ledger_l1_unit.py` | 🆕⑯ **1,096** | **41** | ~~879 / 34~~ @`2a4e51cf` · ~~869~~ / ~~1,530 / 61~~ |
| `test_ledger_l1_pg.py` | **1,606** | **32** | ~~1,126 / 22~~ (🆕⑯ 무변동) |
| `test_ledger_trace.py` | **431** | **11** | ~~898 / 38~~ (🆕⑯ 무변동) |
| `test_ledger_trace_contract.py` | **422** | **8** | ~~403~~ / ~~476 / 14~~ (🆕⑯ 무변동) |
| `test_ledger_trace_pg.py` | **1,630** | **47** | ~~1,658 / 48~~ (🆕⑯ 무변동) |
...
`test_ledger_trace_contract.py::test_every_declared_derivation_is_explicitly_classified`는 **여전히 있다**(🆕⑭ `2a4e51cf` 재확인) — 번역기 config가 낼 수 있는 모든 derivation을 열거해 해결기가 명시적으로 분류하지 않은 것에서 실패한다. 🔴 **이 시험이 「`confirmed_derivations` 를 비운 판정」의 «강제되는 자리»다** — 확인 관계를 선언하는 transfer 소스가 하나 생기면 이 시험이 빨개져서 그 이름을 되돌리라고 말한다. 판정이 산문에만 있으면 한 달 뒤 없는 규칙이 되지만, 이건 시험 안에 산다.

⚠️ **이 표의 `server/tests/` 에는 원장 계열이 이 아홉 말고도 «스물한 개» 더 있다**(`test_ledger_setup_bundle` 1,643/71 · `test_ledger_setup_registry` 985/39 · `test_ledger_roleframe` 498/20 …). 이 절은 **읽기 측 채점자만** 싣던 표라 그대로 두었고, 쓰기·설정 계열은 [§5-H 쓰기 측](#5-h-정본-원장-canonical-ledger)·[§5-H-bis](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재) 소관이다.
...
- 🔴 **`ledger/trace.py`의 `DISPLAY_TIMEZONE_RULING = __doc__`과 `CONVENTION_DERIVATIONS_RULE = __doc__`은 둘 다 *모듈 docstring*에 바인딩된다** — 모듈 스코프의 `__doc__`이 그것이기 때문이다. 즉 **두 상수는 같은 문자열이고, 이름이 가리키는 판정문(바로 위 `#:` 주석)은 어느 쪽에도 들어 있지 않다.** 판정문 자체는 주석으로만 존재한다(파이썬 런타임에 없다). 의도가 「이름 붙은 참조점」인지 「그 텍스트를 담는 상수」인지는 소스에서 판정할 수 없어 **어느 쪽도 사실로 적지 않는다.**
- `server/config/sample/ledger_config.json.sample`이 **`partitioning` 블록을 선언하는데 저장소 어디에도 읽는 코드가 없다**(전건 grep 0건 — `server/ledger/`도 마이그레이션도). `sources.<name>.columns.equipment`도 마찬가지로 **선언만 되고 읽히지 않는다**(`config.validate`의 필수 7종에 없고 `backfill.fetch_page`의 SELECT에도 없다). 🆕⑦ **`a729a7f`에서 재확인 — 둘 다 여전히 참이다**(`partitioning`의 grep 히트는 sample 자신·마이그레이션 docstring 산문·테스트 1곳뿐이고, `equipment`를 컬럼으로 읽는 코드는 신설 번역기 2종에도 없다). ⚠️ 🆕⑩ **이 항은 `5359fdd` 기준이고 재측정하지 않았다** — 그 사이 `.sample`이 워킹트리에서 수정 중이었다(다른 레인).
- 🆕⑩ 🔴 **`backfill.py`의 docstring이 「`dry_run.py`가 `fetch_*` 아홉을 import한다」고 적지만 `dry_run.py`는 하나도 import하지 않는다.** 그 아홉의 호출자는 레포 전체에서 0건이다 — 위 `backfill.py` 표의 마지막 행에 실측을 적었다. 살려 두는 근거가 명시돼 있고 그 근거가 거짓인 형태라, **다음 독자가 근거만 읽고 통과시킬 것**이므로 여기 겹쳐 적는다.
- 🆕⑩ 🔴 **`server/ledger/__init__.py`의 「읽는 순서」가 없는 파일을 여섯째로 지목한다** — `lot_event_translator.py`. 위 항목이 「신설 번역기 2종을 아직 모른다」고 적었는데, 이제는 **그 목록이 가리키는 파일 자체가 삭제됐다.** 소스 소관이라 여기서 못 고친다.
- ⚰️ 🆕⑭ **[해소됨 — 절반만] 「라우터가 `edge_limit` 상한을 3000 에 묶어 둔다」는 «더 이상 참이 아니다».** `2a4e51cf` 실측 — 라우트가 `edge_limit: int = Query(1200, ge=20, le=ledger_subgraph.MAX_EDGE_LIMIT)` 으로 **상수를 참조**한다. 수를 옮겨 적는 대신 이름을 부르므로 두 자리가 갈라질 수 없다. 🔴 **그러나 «기본값»의 갈라짐은 그대로다** — 모듈 `DEFAULT_EDGE_LIMIT = 6000` 대 라우트 기본값 **1200**. 모듈을 직접 부르는 시험과 HTTP 로 오는 화면이 여전히 다른 기본 예산으로 돈다. 어느 쪽이 의도인지는 소스로 판정할 수 없어 사실만 적는다.
- ⚰️ 🆕⑭ **[해소됨 — 이름이 사라졌다] `NODE_TABLE_COLUMNS`·`EDGE_TABLE_COLUMNS`·`PROPERTY_TABLE_COLUMNS` 는 «없는 이름»이다.** 🆕⑬ 이 「호출자 0」이라 적고 삭제 판정을 서버 도메인에 넘겼고, `2a4e51cf` 에서 셋 다 삭제됐다(`TABLE_COLUMNS` 전건 grep 히트 **0**). 🔴 **이 문서의 그 행이 그대로 작업 지시가 됐다** — 다음 패스는 자기가 적은 「안 쓰인다」 항목이 착지했는지 확인한다.
- 🆕⑭ 🔴 **`ledger_api/ledger_subgraph.py` 의 «모듈 docstring» 은 은퇴한 그래프를 그린다.** 첫머리가 `Event --asserts--> Claim --subject--> Entity` 도식과 Finding Collection · Finding Point · Quantity · `mechanism_models.json` 을 설명하는데, **그 노드 종류는 전부 2026-08-25~28 에 은퇴했고**(claim 은 엣지, finding 은 노드가 아님, 기전 로드는 walk 에서 나갔다) 이 파일이 오늘 내는 노드는 **선언된 엔티티 하나**뿐이다. 코드 소관이라 여기서 못 고친다 — **이 파일의 docstring 을 응답 모양의 출처로 쓰지 마라.** 같은 파일 `_declared_key_order` 의 docstring도 `ledger/vocabulary.py` 의 `ENTITY_TYPES` 를 지목하는데 **그 파일은 없다**(§5-H 묘비).
- ✅✅ 🆕⑮ **[2026-08-30] 이 항목은 «닫혔다» — 선언도 0, 검증기 수용도 0이다.** `1d17c34a` 가 `_validate_vocabulary` 의 `optional=("continues",)` 를 지웠으므로 이제 그 키를 든 선언은 **거절**된다(§5-H 쓰기 측 `setup_bundle.py` 행 ③). 🔴 **「별건이라 남겨 둔다」던 그 별건이 착지했고, 그것이 이 항목을 «소스가 반박하는 자리» 목록에서 내리는 조건이었다.** 실측 `5a73021a` — 코드에서 `continues` 를 읽는 자리 **0** · tracked 샘플의 `"continues"` 계수 **0** · 남은 것은 산문 여섯 줄(전부 은퇴를 «설명하는» 주석). **아래는 그 전 두 단계의 기록이고 앵커로 쓰지 마라:** 🆕⑭ ✅ ~~`continues` 는 «치워졌다» — 재측정 `3a8494dc`: 선언 «0건».~~ `36802e42` 가 라이브와 샘플에서 여섯을 지웠다(`server/config/backup/**` 스냅샷에만 남아 있다). ~~`setup_bundle._validate_vocabulary` 는 아직 `optional=("continues",)` 로 받는데, 그 수용을 지우면 아직 그 키를 든 선언을 «거절»하게 되므로 별건이다.~~ **아래는 `2a4e51cf` 기준의 기록이다:** 🔴 **`continues` 는 «읽는 코드가 0»인데 선언에는 여섯 개 남아 있고, 검증기가 «일부러» 통과시킨다.** 실측 `2a4e51cf` — `get("continues")`/`["continues"]` 전건 grep이 `server`·`client2` 에서 **0건**(읽던 `_continuing_predicates` 가 삭제됐다). 그런데 `setup_bundle._validate_vocabulary` 는 `optional=("continues",)` 로 여전히 받고, **소스 주석이 그 이유를 시한부로 적는다** — 「라이브 선언이 아직 여섯을 들고 있어서 거절하면 서버가 선언을 못 읽는다. 문을 먼저 열고 선언을 나중에 청소한다; 이 줄은 그 청소와 함께 간다」(⏳ 표시). tracked 샘플 `config/sample/ledger_config.json.sample` 도 `"continues": true` 를 **여섯** 들고 있다. 🔴 **읽는 쪽이 없으므로 이 여섯은 오늘 아무것도 «안 한다»** — 「선언에 있으니 도는 규칙」으로 읽지 마라.
- 🆕⑭ ✅ **재측정 `3a8494dc`: 그 구멍은 «메워졌다».** tracked 샘플이 `class: "static"` 을 셋 들고(`0fe0ec09`), 라이브도 셋이다. 걷기 가드 둘은 이제 샘플로 도는 박스에서도 «발화한다». **아래는 `2a4e51cf` 기준의 기록이고, 그것이 이 구멍을 찾아낸 문장이다:** 🔴 **그 후계인 `class` 는 반대로 «읽는 코드는 있는데 선언이 없다».** `_static_types()`·`_static_step_predicates()` 가 `entities.<id>.class == "static"` 을 읽는데, 이 저장소의 tracked 선언 **둘 다** `"class"` 키를 **하나도** 갖고 있지 않다(`config/sample/ledger_config.json.sample` 0건 · `config/sample/ontology/transfer_explorer/ledger_config.json` 0건 — 라이브 `config/ontology/ledger_config.json` 은 gitignored 라 이 패스가 열지 않았다). 🔴 **그래서 샘플로 도는 박스에서는 두 집합이 «빈 집합»이고, 이 밤에 들어온 걷기 가드 둘(`s -> d` 거절 · static 전용 좁은 `follow`)이 «한 번도 발화하지 않는다».** 시험이 초록인 것과 가드가 도는 것은 다른 사실이다 — 가드를 실제로 태우려면 선언이 `class: "static"` 을 «말해야» 한다.

---

## 5-H-bis. 온톨로지 config 작성·탐색 표면 (2026-08-21 신설 등재)

> 🔬 🆕⑳ **[2026-09-05] 이 절에서 «`328a5c20` 기준으로 재측정된 것»은 다섯이다** — `config_authoring`(`closed_lists` 전량 · `_implementation_fields` diff · 줄 수) · `implementations`(전량) · `source_preparation`(`declared_output_columns` 만) · `ledger_skeleton.json`(줄 수 · `$defs` diff) · 클라 부품 표(줄 수 · `^export` 전건). **나머지는 아래 `8d1e6c4c` 기준 그대로다.**
>
> 🔬 **측정 기준: `8d1e6c4c`(HEAD)의 커밋된 blob**(`git show 8d1e6c4c:<path>` — 워킹트리 아님. 측정 시각에 이 절의 대상 파일은 전부 blob과 동일했다). 🔴 **라인 번호는 이 절에 하나도 없다.** 위치는 `git grep -n "<심볼>" -- <경로>`로 확정하라.
>
> ⚠️ **이 절이 «주장하지 않는» 것**: 아래 각 파일의 **전체 심볼 표가 아니다.** 이 패스가 본체까지 읽은 것은 `config_explorer_service.test_run` 계열 · `config_authoring`의 `Ground`/`Field`/`authoring_plan` 머리 · `source_preparation._assemble_prepared_frame` · `backfill.preview_first_batch`/`_run_v2_lineage` · 라우터 전량이고, 나머지는 **최상위 심볼 이름과 docstring 머리만** 실측했다.

**무엇인가.** 원장 v2는 소스마다 파이썬 번역기를 쓰던 자리를 **선언 하나 + 등록된 구현**으로 바꿨다([§5-H](#5-h-정본-원장-canonical-ledger) 묘비 참조). 그러자 **그 선언을 사람이 쓸 수 있어야 한다**는 문제가 남고, 이 표면이 그것을 담당한다 — 「무엇을 입력해야 하고 어떻게 입력하는지를 UI로」. 서버 절반은 `server/ledger/config_*.py` + 라우터 하나, 클라 절반은 `client2/src/ontology_explorer*.js`다.

**config root의 모양이 바뀌었다(구조만 — 실값은 여기 옮기지 않는다).** 🔴 **정본은 `server/config/ontology/`(= `setup.DEFAULT_ONTOLOGY_ROOT`)의 «단일 파일» `ledger_config.json`이다.** 종전 다중 파일 레이아웃(`manifest.json` + `catalog/tables.json` + `catalog/virtual_joins.json` + `dataflows/chains.json` + `dataflows/enrichments.json`)은 이 구간에 **접혔고**(이주기 `server/scripts/convert_ontology_to_single_file.py`), 샘플도 `server/config/sample/ontology/transfer_explorer/ledger_config.json` 하나로 남았다. 🔴 **라이브 `server/config/ontology/ledger_config.json`은 이 구간에 tracked에서 «빠졌다»** — `.gitignore:61`이 **`server/config/ontology/*`**(디렉터리 통째)를 무시하므로 tracked로 남은 것은 `README.md` 하나뿐이고, **디렉터리 훑기로는 sample만 뜬다.** 파일을 이름으로 지목할 것. 검사: `git check-ignore -v server/config/ontology/ledger_config.json`이 **그 61행을 지목해야** 한다(안 하면 무시 규칙이 바뀐 것이다). 물리 카탈로그는 같은 root의 `table_config.json`(`setup_bundle.PHYSICAL_CATALOG_FILENAME`)이고 `setup.live_physical_catalog()`가 읽는다.

### `server/ledger_api/ontology_config_explorer_router.py` (🆕⑱ **264줄** @`6345ef3d` — 무변동 · 🆕⑩ 264, 구 표기 129)

> 🟢 🆕⑱ **[2026-09-05 재계수 · `6345ef3d`] 라우트 «15» · 비-strict 게이트 «6» · strict 「10」 — 셋 다 그대로다.** ⚠️ **strict 의 10 은 «줄» 계수이지 라우트 계수가 아니다**(`require_admin_token_strict` 가 import 1줄 + 여러 줄 데코레이터에 걸쳐 나온다). **라우트 단위로 세려면 `^@router\.` 를 세고 그 데코레이터 블록 안을 봐라.** ⚠️ **이 패스는 라우트 «이름과 계수»만 쟀고 핸들러 본체는 열지 않았다.**
...
| 🆕⑩ POST `/bootstrap` | `require_admin_token_strict` | 빈 root에서 config를 세운다 |
| 🆕⑩ POST `/drafts/new` | `require_admin_token_strict` | `kind`+`canonical_id`로 **새 선언** 초안 |
| PUT `/drafts/{draft_id}` | `require_admin_token_strict` | |
| POST `/drafts/{draft_id}/review` · `/revise` · `/activate` | `require_admin_token_strict` | ⚠️ **한 행이지만 라우트 «셋»이다** — 이 행을 하나로 세면 총계가 13이 되고, 실측은 15다 |
...
### 🆕⑩ POST `/admin/ontology-explorer/test-run` → `OntologyExplorerService.test_run(engine, *, source_id)`

🔴 **「선언이 도는가」에 화면이 «측정»으로 답하는 자리.** 종전엔 백필을 손으로 돌려 봐야 알 수 있었다.

| 술어 | 실측 |
|---|---|
| 무엇을 실행하나 | `backfill.preview_first_batch(engine, setup, source_id)` — **첫 페이지 200행**(`PREVIEW_FETCH_ROWS`), 커서 아님 |
| 무엇을 쓰나 | 🔴 **원자 0 · `ledger_cursor` 무접촉.** 게이트 «한 걸음 앞»에서 멈춘다. ⚠️ **그러나 「아무것도 안 쓴다」는 아니다** — `status == "passed"`면 `_record_test_run`이 **`draft_store.root/test_runs.json`에 파일을 쓴다**. 원장이 아니라 초안 옆이고, `ledger_config.json` 안이 아닌 이유는 **테스트 결과가 선언의 일부가 아니기** 때문(넣으면 매 실행이 누군가 편집 중인 파일의 재작성이 된다) |
| 왜 POST인가 | 🔴 **바꾸기 때문이 아니라 «비싸기» 때문이다**(페이지 읽기 + 전체 컴파일 1회) |
| 거절을 어떻게 내나 | 🔴 **선언 문제는 raise하지 않는다 — 거절이 이 엔드포인트가 «내려는 답»이다.** 400 한 문장으로 던지면 폼이 가리킬 칸이 없어진다. `ValueError`(400)로 나가는 것은 **아무것도 이름 대지 않는 `source_id`뿐**(`unknown_source`) |
| 컴파일에서 떨어진 선언 | 🔴 **그 자체가 답이다.** 스냅샷에 없으면 `_invalid[key]["reasons"][0]`을 렌더하고 `status="refused"` — 「unknown source」로 답하면 운영자가 **없는 이름을 찾으러** 가고 진짜 거절은 `invalid`에 남는다 |
| 예외 포획 범위 | 🔴 **클래스 목록이 아니라 `except Exception`이다.** 오늘 아홉 종류가 이 호출에 닿고 열째는 다음 문법과 함께 온다; 없는 테이블의 psycopg 오류도 **답이지 500이 아니다** |
| 상태 어휘 | `passed` · `refused` · `empty`(0행 읽음 — **통과가 아니다**) · `unverified`(`OntologyExplorerService.UNVERIFIED`) |
| 폼 좌표 | `_test_run_refusal(source)` → `{code, path, message, form_path}`. 🔴 **`form_path`는 «매핑»이지 «발명»이 아니다** — `bundle.`로 시작하거나 첫 토막이 `AUTHORABLE_SECTION_NAMES`에 있을 때만 채우고, 나머지(`cursor_value`·`known_registrations`·`role_frame.rows[0].roles.subject`)는 **비운 채 원문 그대로** 보인다. 「추측한 칸은 자리 못 잡은 문장보다 나쁘다」 |
| 문장별 원자 수 | `_atoms_per_sentence(preview, plan)` — 🔴 **`derivation`으로 묶는다**(= sentence. `roleframe.compile_role_frame`이 `derivation = row["sentence"]`를 찍으므로 귀속이 컴파일러 자신의 것이다). 🔴 **선언된 문장은 «아무 말도 안 한 것까지» 전부 나열한다** — Profile에서 0으로 씨를 뿌린 뒤 카운트를 덮는다. **`0`은 행이지 부재가 아니다**(빠뜨리면 그 구멍이 다시 안 보인다). 집계 시점은 `_filtered_event_atoms` **뒤**라 배치가 스스로 중복 제거한 `register`가 두 번 세어지지 않는다 |
| 기록 키 | `_record_test_run`은 **`definition_hash`로 건다** — 소스의 한 필드만 고쳐도 해시가 어긋나 기록이 **그 편집과 함께 죽는다**. 「이 선언이 돌았다」와 「이 이름을 가진 무언가가 한 번 돌았다」의 차이 |

### `server/ledger/config_explorer_service.py` (🆕⑱ **989줄** @`6345ef3d` — 🆕⑩ 946, 구 표기 305)

`class OntologyExplorerService` 하나. 생성자는 **전부 키워드**이고 주입 지점이 넷이다: `config_root=DEFAULT_ONTOLOGY_ROOT` · `draft_root` · `setup_loader=load_setup` · `catalog_loader=live_physical_catalog` · `convergence_probe`. 소유 상태 — `draft_store`(`OntologyDraftStore`) · `test_run_path`(`draft_store.root/test_runs.json`) · `_lock`(`RLock`) · `_stamp`/`_setup`/`_index`/`_compiled_at`/`_invalid`/`_config_level`.

메서드(이름 실측): `invalidate` · `active(force=False)` · `view` · `_resolution` · `_knocked_out_reasons` · `delete_declaration` · `deletion_preview` · `_definition_change` · `column_picker` · **`test_run`** · `_test_run_refusal` · `_test_runs` · `_record_test_run` · `_verification_view` · `authoring_schema` · `authoring_prefix` · `authoring` · `bootstrap_config` · `create_draft` · `create_declaration_draft` · `save_draft` · `review_draft` · `revise_draft` · `discard_draft` · `activate_draft` · `_file_stamp` · `_assert_context`. 클래스 상수 `UNVERIFIED = "unverified"`.
...
### `server/ledger/config_authoring.py` (🆕⑳ **2,003줄** @`328a5c20` — 🆕⑪ 1,937, 구 표기 1,548) — 작성 패널의 엔진

🔴 **한 줄도 쓰지 않는다.** bundle(유효하든 반쯤 쓰였든)과 물리 카탈로그 위의 **읽기**이고, 실제로 적용하는 쪽은 `config_drafts`다.

| 심볼 | 무엇인가 |
|---|---|
| **4상태** | `derived`(다른 선언이 정한다 — **자유도 0**, 화면이 채우고 묻지 않는다) · `missing`(강제인데 없다 — 할 일) · `unanswered`(진짜 자유, 아직 안 골랐다 — 후보와 함께) · `answered`. `Field.__post_init__`이 이 넷 밖을 `ValueError`로 거부한다 |
...
| `authoring_plan(bundle, catalog, *, selection_prefix=None)` | 표면 전체. 🔴 **`bundle`이 유효하지 않아도 돈다** — 반쯤 쓰인 것이 정상 작성 상태이고, 컴파일된 bundle에서만 도는 계획은 **필요한 바로 그때 쓸모없다**. 필드 출처 다섯: `_entities_fields` · `_vocabulary_fields` · `_implementation_fields` · `_profile_fields` · `_source_fields` |
| `skeleton()` / `SKELETON_PATH` | `ledger/ledger_skeleton.json`을 `lru_cache`로 편다(`_deref`가 `$defs` 참조를 푼다) |
| `empty_value(node, defs, …)` / `empty_declaration(section)` | 🔴 **값이 없으면 «빈 폼으로 하위 구조가»** — 스켈레톤에서 빈 목적 객체를 만든다 |
| 🆕⑳ `closed_lists(sources=None)` / `versioned_sections()` | 폼이 고를 수 있는 닫힌 목록 · `@N` 버전을 다는 절. 🔴 **인자는 «하나»이고 선택이다** — 아래 [`implementation_choices`](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재) 블록의 사유 그대로. ⚠️ 이 지도가 종전에 이 행을 `closed_lists()` 로 싣고 있었다(인자가 붙은 것은 `b72843cc`) |
| 🆕⑳ `closed_lists` 가 «발행하는 키 수» | **23** @`328a5c20`(직전 22). 철자: `setup_bundle.public_bundle_schema()` 가 **8** + 이 함수가 직접 쓰는 **15**. 🔴 **늘어난 하나는 `identity_binding_kinds` 이고 «이 함수»가 아니라 `setup_bundle.public_bundle_schema()` 에서 온다** — 이 함수 본문만 세면 못 본다. ⚠️ **키를 여기 전건 열거하지 않는다** — 열거하면 두 번째 철자가 되고, 세는 것은 위 두 grep 으로 언제든 다시 된다 |
| `ground_node_key(path)` · `is_remaining(row)` · `relation_columns` · `prepared_columns` · `profile_binding_columns` | 조회·판정 헬퍼 |
| 🆕㉔ `config_explorer.ISOLATION_ROOTS` :125 · `setup_registry.SourcePlan.planned` :458 / `.refusal` :459 / `.is_runnable()` :472 | **깨진 선언이 «혼자» 떨어진다**(S-177 ②③, 판정 280·281). `planned=False` + `refusal{code,path,message}` 이고 로더가 «어느 것이» 떨어졌는지 말한다. 교차 검사는 «성한 부분집합»에서 다시 돌고(고정점), 그 고정점이 «가상 조인 규칙까지 닿지 않는다»를 시험이 못 박아(`f9fa9c24`) 뿌리가 «자기 상수»를 갖는다. ⚠️ **[2026-09-12 정정] 그 상수는 `config_authoring` 이 아니라 `config_explorer` 에 산다** — 이 문서가 한때 `ledger/config_authoring.ISOLATION_ROOTS` 라 적었고(D-8), `config_authoring` 은 그것을 :62 에서 «import 할 뿐»이다. 그리고 `is_runnable()` 은 「여섯 호출 지점이 각자 `and plan.planned` 를 적는 것」을 막는 한 철자다 |
...
| 🆕⑳ `_implementation_fields` 의 **`output_columns` 갈래 둘** (`172efda6`) | 🔴 **구현에게 «먼저» 묻고, 대개는 답하지 못한다.** `preparer_output_columns(implementation_id, implementation_version)` 가 `None` 이 아니면 그 행이 **`state="derived"` · `tier=TIER_STRUCTURAL` · `disposition="grammar_requires_it"`** 로 서고 `ground.rule = "preparer_output_columns_from_implementation"` 이다. `None` 이면 **종전 행 그대로**(`answered`\|`unanswered` · `TIER_CONSTRAINED` · `preparer_output_collision_from_relation`). 🔴 **`value` 가 이름 목록이 아니라 «매핑»이다** — `filled_declaration` 이 이 값을 파일에 앉히는데 `_column_types` 가 `{column: type}` 아닌 것을 거절하므로, 이름 목록을 넣으면 「채워졌다」고 말한 그 칸을 검증기가 거절한다. ⚠️ **`None` 은 「아무것도 안 만든다」가 아니라 「말하지 않았다」**이고, 빈 매핑과 다르다 |
...
### 🆕⑩ `server/ledger/ledger_skeleton.json` (🆕⑳ **761줄** @`328a5c20` — 🆕⑮ 797 @`5a73021a`, 🆕⑪ 710, 구 표기 763) — config가 «무엇으로 이루어지는가»
...
> 🔬 🆕⑳ **[2026-09-05 `06bd33a9`] `$defs` 에 «레코드 하나»가 늘었다 — `identity_binding`.** 엔티티 식별키 맵의 `of.use` 가 `"binding"` → **`"identity_binding"`** 으로 바뀌었고, 새 레코드는 `binding` 과 **같은 모양에서 `entity` 만 빠진 것**이다(`kind` 잎의 `list` 가 `identity_binding_kinds`, 그 뒤 `column`/`value` 는 `when: {field:"kind", is: …}` 로 갈린다). 🔴 **여전히 «한 층»이고 여전히 바인딩 레코드다** — 새 개념을 만든 것이 아니라 검증기가 이미 거절하는 선택지를 폼에서 뺀 것이다(위 §5-H `setup_bundle.py` 행). ⚠️ **`config_authoring.py` 는 이 변경으로 «폼 문법»을 배우지 않았다** — 스켈레톤이 `list` 이름을 대고 `closed_lists` 가 그 이름을 발행할 뿐이라 클라도 무변경이다. 그게 이 파일의 설계다.
>
> 🔬 🆕⑮ **[2026-08-30] `719 → 797` (+78) 이고 «그 증분이 이 파일의 설계를 증명한다».** 늘어난 것 전부가 엔티티 밑 **`references` 노드 하나**이고, **`config_authoring.py` 는 이 구간에 한 줄도 안 바뀌었다** — 즉 폼에 새 구획이 생기는 데 **코드 0줄**이 들었다. 위 DoD 문장의 실측 사례로 인용할 수 있는 자리다. ⚠️ **예외적으로 이 한 필드의 «구조»만 적는다** — [§5-H 라우터 절](#5-h-정본-원장-canonical-ledger)의 「`references` 는 문법만 살아 있고 읽는 쪽이 없다」가 이 커밋으로 «한 층 바뀌었기» 때문이고, 그 판정을 다음 패스가 재유도하려면 어디를 볼지 알아야 한다. **값이 아니라 어느 층이 그것을 아는가가 요점이다**: 검증기 문법 있음(`setup_bundle._validate_references`) · 작성 폼 그림(**여기, 신설**) · `follow` 로 이름 못 댐 · 투영의 합성 엣지 없음 · **tracked 선언 둘의 실제 선언 수 0**. 🔴 **「폼이 묻는다」를 「돌아간다」로 읽지 마라** — 다섯 층 중 셋이 아직 0이다.

### 🆕⑩ `server/ledger/implementations.py` (🆕⑳ **261줄** @`328a5c20` — 🆕⑱ 228, 구 표기 172) — 실행 가능한 구현의 **유일한 발견자**

🔴 **손으로 관리하던 목록 «넷»을 대체한다**(trusted 카탈로그 · preparer 레지스트리 · mapper 레지스트리 · transfer 샘플 테스트 지원 안의 네 번째 리터럴). 실측 사고: **이미 쓰였고 이미 맞았던 구현 둘**(`roleframe.DeclarativeRoleMapper` · `source_preparation.DirectJoinSourcePreparer`)이 **아무 목록에도 없어서 config에서 닿지 못했다** — 증상은 컴파일 시점 `untrusted_implementation`이고 클래스가 존재한다는 힌트는 없었다.

**신뢰 조건 넷(각각 검사된다).** ① `BaseSourcePreparer` 또는 `BaseLedgerMapper`의 하위 ② `implementation_id`(비공백) + `implementation_version`(양수)를 **자기 자신에** 선언(상속만으로는 안 된다 — 중간 베이스가 정체성을 모든 하위에 빌려주면 안 되므로) ③ 이 파일이 import하는 모듈 안에 있을 것 — `ledger.roleframe` · `ledger.source_preparation` · **`mappers/ledger_v2_*.py`**(🔴 import 집합은 **저장소 레이아웃**의 성질이지 config의 성질이 아니다. 그것이 선언에게 「무엇을 import할지」를 못 고르게 하는 자리) ④ 레지스트리 자신의 구조 관문(`map()`/`prepare_batch()`가 final이고 인자 없이 생성 가능).
...
반환 모양: `{"prepare": {...}, "map": {...}}`, 각 블록이 **`{"options": [{"id","version"},…], "counts": {id: n}, "default": id|None}`**.

| 성질 | 무엇이고 왜 그런가 |
|---|---|
| **목록은 «레지스트리»에서 온다 — 리터럴에서 절대 오지 않는다** | 운영자에게 맨바닥에서 `declarative-role` 을 타이핑하라는 것은 «도메인 질문»이 아니라 **답할 수 없는 질문**이다. 그렇다고 그 문자열을 코드나 폼에 적는 것은 **더 나쁘다** — 코드에 든 도메인 낱말이다. 🔴 **`_ids` 는 id 마다 «최신 버전»만 남긴다**(`best[name] = max(...)`) |
| 🔴 **기본값은 «선언»이 아니라 «세어서» 나온다** | **이 배포의 소스들이 «이미 제일 많이 쓰는 것»** 이다. 다른 소스 집합을 주면 다른 답을 낸다 — **그 성질이 이 값을 코드 밖에 붙들어 둔다.** `counts` 가 «함께» 나가므로 독자는 「default」라는 낱말을 믿는 대신 **무엇을 세어서 나온 값인지**를 본다 |
...
#### 🆕⑳ `preparer_output_columns(identifier, version=None) -> dict | None` — **준비기가 «스스로 밝힌» 산출 컬럼** (`172efda6`)

반환: `{컬럼: 타입}` 매핑, **또는 `None`**. `version` 을 생략하면 그 이름으로 등록된 **최신 버전**이 이긴다(= `implementation_choices` 의 `_ids` 가 고르는 것과 같은 것이라, 폼이 고른 것과 여기가 읽는 것이 갈리지 않는다).

| 성질 | 무엇이고 왜 그런가 |
|---|---|
| 🔴 **`__dict__` 로 읽는다, `getattr` 이 아니다** | `_self_declared_identity` 와 **같은 사유**다 — 중간 베이스가 답하면 그 답을 모든 하위에 빌려주고, 컬럼 하나를 더 만드는 하위 클래스가 **조용히 하나 짧은** 선언을 받아 채워진다 |
| 🔴 **`None` 은 「말하지 않았다」이지 「아무것도 안 만든다」가 아니다** | 빈 매핑은 「나는 컬럼을 안 더한다」는 «선언»이다. 둘을 접으면, 한 번도 말한 적 없는 클래스를 대신해 `{}` 로 칸을 채우는 것이 되고 그것이 이 파일이 없애려고 존재하는 그 실패다 |
| ⚠️ **오늘 «말하는» 것은 하나다** | `mappers/ledger_v2_lot_event_role_mapper.LiveLotEventSourcePreparer` 의 `declared_output_columns` **7키**(`lot`·`row_identity`·`slots`·`wafers`·`EVENT_GROUP_COLUMN`·`SOURCE_EVENT_INCOMPLETE_COLUMN`·`SOURCE_ROW_EXCLUDED_COLUMN`). 🔴 **소스가 「내포로 만들지 않고 손으로 적는다」를 사유와 함께 적는다** — 컬럼이 하나 늘 때 타입을 «여기서» 정하게 하려는 것이고, 루프로 만들면 전부 `"string"` 을 물려받는다. `DirectJoinSourcePreparer` 는 «말하지 않는다»(측정 `None`) |
| 소비자 | `config_authoring._implementation_fields`(위 표의 `output_columns` 갈래 둘) · 채점자 `server/tests/test_a_preparer_can_state_its_own_outputs.py` |

> 🔴 **선언 쪽 기본값은 `BaseSourcePreparer.declared_output_columns = None` 이고, 그것이 기본값인 채로 있어야 한다**(§5-H-bis `source_preparation.py` 표의 같은 행). 「둘 다 마침 안다」에서 「준비기는 안다」를 유도하면 그것은 규칙이 아니라 **그 둘의 묘사**다.
> ⛔ **런타임은 이 값을 «읽지 않는다».** `_assemble_prepared_frame` 은 산출 컬럼을 **선언**에 대고 채점한다 — 여기에 대고 채점하면 클래스를 자기 자신과 비교하는 것이고, 그 칸을 자유도 0으로 만드는 검사가 «공허»해진다. 이것은 폼이 **선언에 써 넣는** 값이지 비교의 반대편이 아니다.

### 🆕⑩ `server/ledger/column_stats.py` (**287줄**, 신설) — 컬럼을 «고르기 위한» 물리 프로브

`GET /admin/ontology-explorer/columns`의 몸. `MAX_MEASURED_COLUMNS = 64` · `ColumnStatsError` · `declared_unique_keys(table)` · `physical_columns(db, relation)` · `estimated_rows(db, relation)` · `population(db, relation, columns=None)` · **`combination_uniqueness(db, relation, columns)`** · **`ordering_candidates(db, relation, table)`** · `_checked`(관계·컬럼을 물리 카탈로그에 대고 검증 — SQL 보간 전) · `_quote`.
...
| 🆕⑳ **`BaseSourcePreparer.declared_output_columns: dict[str,str] \| None = None`** (`172efda6`) | 🔴 **준비기가 「내가 무엇을 더하는가」를 «스스로» 말하는 자리.** 값의 모양은 선언의 `output_columns` 와 **같은 객체**다. 🔴 **기본값이 `None` 이고 «기본값으로 남아야 한다»** — 오늘 출하되는 준비기 둘이 마침 둘 다 자기 산출을 아는데, 산출 «이름»이 행에서 나오는 준비기는 못 안다. 「이 둘이 안다」를 「준비기는 안다」로 올리면 규칙이 아니라 그 둘의 묘사다. ⛔ **런타임이 이것을 읽는 것은 금지다** — 읽는 곳은 작성 폼([`implementations.preparer_output_columns`](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재))뿐이고, `_assemble_prepared_frame` 은 여전히 «선언»에 대고 채점한다 |
| 🆕⑩ `_assemble_prepared_frame(context, base, outputs)` | 아래 순서 표 |

**`_assemble_prepared_frame`의 검사 순서 — 이 순서가 계약이다.**

1. `outputs`가 Mapping인가 → 아니면 `invalid_source_preparer_output`
2. 출력 컬럼 집합이 **선언과 정확히 일치**하는가(`source_plan.driver.preparation.preparer.output_columns`) — 부분집합도 초과집합도 거절
3. 컬럼마다 **base 행 수와 정확히 같은 개수**의 값인가(문자열·Mapping은 Sequence로 안 쳐 준다)
4. **base의 물리 값을 바꾸지 않았는가** → 아니면 `source_preparation_output_collision`
5. 🔴 **여기서 «비로소» `SOURCE_ROW_EXCLUDED_COLUMN`의 행이 나간다.** 값이 전부 `bool`이어야 하고, `True`인 행을 떨군 뒤 인덱스를 다시 세운다
6. `_required_entity_columns(source_plan)`의 «컬럼»이 있는가 → 아니면 `source_preparation_incomplete`(경로 `event_frame.columns.<col>`) — «선언» 결함이라 페이지 거절 그대로. 🆕 🔴 **[2026-09-08 · `2b72f339`] «값»이 빈 것은 이제 페이지가 아니라 «그 행의 분자»를 거절한다** — 문지기의 이름 그대로(`no_identity`·`missing_occurred_at`), 나머지 분자는 «들어간다**. 단위는 «분자»다(판정 116): 이벤트의 순간은 그 그룹의 «모든 행»에서 한 번에 읽히므로 한 행이 비면 이벤트가 «작아지는» 게 아니라 «성립하지 않는다». 🔴 **거절은 «값»이다** — `source_preparation` 은 이름 붙여 세기만 하고 프로세스 카운터를 «안 건드린다**; 실행 두 문이 기록하고 미리보기는 보고한다(그 한 줄이 시험 실행과 실행의 차이 전부). `order_by`·`cursor_columns` 는 «페이지가 어디인가»를 정하므로 «오늘과 동일**하게 페이지 거절이다

🔴 **왜 5가 4 «뒤»인가 — 그것이 이 변경을 「가드를 좁힌 것」으로 만들고 「내린 것」이 아니게 한다.** 배제 표지도 **base 행마다 값 하나**를 내야 하고 base 값을 바꾸면 안 된다는 두 계약을 **먼저 채점받는다.** 살아남은 행은 여전히 전부 정체성을 요구받고, 6의 거절문은 한 글자도 안 바뀌었으며, **이 컬럼을 선언하지 않은 소스는 5를 아예 지나지 않는다**(소스 주석은 그 수를 25라고 적지만 **이 패스가 세지 않았다** — 개수를 인용하려면 선언을 직접 세어라).

⚠️ **전량 배제된 페이지는 «거절이 아니라 빈 프레임 + 원자 0」이다.** 페이지는 세대가 아니라 **커서**로 잘리므로 옛 행만 든 페이지는 정상이고, 거절하면 백필이 거기서 영원히 선다. 🔴 **커서는 «살아남은 것»이 아니라 «base 페이지»에서 전진한다** — 실측: `backfill._run_v2_lineage`가 `_v2_frame(complete)`의 `frame`에서 `next_cursor`를 뽑고 그 뒤에 `execute_selected_cursor_batch`를 부른다. ⚠️ **소스 주석은 그 자리를 `backfill._backfill_source`라고 적지만 그런 함수는 없다**(레포 전체 grep 1건 = 그 주석 자신). 감싸는 실물은 **`_run_v2_lineage`**다.

### 🆕⑩ 클라 — 온톨로지 탐색기 (`8d1e6c4c` 실측, 워킹트리 동일)
...
| `client2/src/ontology_explorer_view.js` | 🆕㉑ **2,515**(구 표기 2,277) | `renderOntologyExplorer(root, state)` **하나**(🆕⑪ 재확인 — export는 여전히 이것뿐이다) |<br>🆕㈢ **[C-96 `69e05ca0`] 저장 버튼 옆에 «다시 도는 비용»이 선다** — `redo_cost.js` 를 읽고, 그 칸은 서버가 전부터 실어 보내던 `redo` 다(S-143 `3238bff6`). ⚠️ `count` 가 `None` 이면 「안 물었다」이지 「0」이 아니고, 그 구별을 `absent.js` 계열이 글자로 가른다
...
> 🔴 🆕⑳ **[2026-09-05 재측정 · `328a5c20`] 이 절의 제목이 «일곱»이었는데 표에 열넷이 실려 있었다.** 「일곱」은 첫 배치의 수이고 그 뒤 일곱이 더 붙었는데 제목만 안 따라왔다 — 아래 마지막 문단은 이미 「이 열넷이」라고 적고 있었으므로 **한 절이 두 수를 들고 있었다.** 🔴 **그리고 뒤 여섯의 표 행이 «각각 빈 줄로 끊겨» 있어 마크다운이 표로 렌더하지 않았다**(머리 없는 파이프 문단 여섯). 한 표로 이어 붙였다.
>
> 🔴 🆕⑳ **줄 수 «넷»이 틀려 있었고 그중 셋은 이 절이 처음 실릴 때부터 틀렸다** — `5cdb92de` 와 HEAD 에서 **바이트 동일**인 파일들이다(워킹트리를 쟀거나 편집 «전»에 쟀다는 뜻이다):
> ```
> join_verification.js   82 -> 83      gap_catalogue.js   76 -> 74
> form_demand.js         55 -> 51      pickup_state.js    84 -> 80
> ```
> 나머지 열은 실측값과 일치했다.
...
| `client2/src/raw_registry_panel.js` | 🆕㉜ **403** (구 표기 208) | `registryView` · `RawRegistryPanel` | 「선언 하나를 앱 안에서 등록」의 근원 템플릿 |
| `client2/src/table_config_panel.js` | 54 | `TABLE_REGISTRY` · `tableConfigView` · `TableConfigPanel` | 위 템플릿의 «선언» ① (표 등록) |
| `client2/src/chain_rule_panel.js` | 🆕㉜ **68** (구 표기 59) | `CHAIN_RULE_REGISTRY` · `chainRuleView` · `ChainRulePanel` | 위 템플릿의 «선언» ② (체인 규칙)<br>🆕㉜ **[C-86 `910c18fb`] chain 탭이 «없던 규칙을 더할 수» 있다** — `RawRegistryPanel` 템플릿의 `addLabel` 로 [규칙 추가] 가 서고, 폼은 «탐색기의 `ontology_explorer_view.renderSkeletonForm`»(:1664)을 그대로 쓴다(모양은 서버의 `skeleton`, §1 라우트 표). 🔴 **값을 경로에 쓰는 자리는 `ontology_path.writeShapeAtPath`(:75) «하나»** — 두 벌이면 「어느 쪽이 초안을 바꿨나」가 두 답이 된다. 📌 상설 「근원 템플릿 요소 개발 후 데이터 갈아끼우기」 그대로다: 템플릿은 `raw_registry_panel`, 선언은 표 등록·체인 규칙 «둘» |
...
| `client2/src/uniqueness.js` | 96 | `UNIQUENESS_UNREAD` · `uniquenessVerdict` · `orderingVerdicts` | 선언된 유니크 키가 «측정»을 견딜는가 — `GET /columns` 의 첫 소비자 |
| `client2/src/join_verification.js` | 🆕⑳ **83**(구 표기 82) | `JOIN_UNREAD` · `joinVerificationView` | 조인 선언이 «승인»됐나 + «만들 DDL» — `GET /admin/config/virtual-join/verify` 의 첫 소비자 |
| `client2/src/gap_catalogue.js` | 🆕⑳ **74**(구 표기 76) | `GAPS_UNREAD` · `SIDE_LABELS` · `gapCatalogueView` | 선언이 「있어야 한다」 한 자리 중 «비어 있는» 곳 — «공허»(있을 수 없음)를 «없음»과 가른다. `GET /api/ledger/gaps` 의 첫 소비자 |
| `client2/src/form_demand.js` | 🆕⑳ **51**(구 표기 55) | `PLAN_UNREAD` · `SHAPE_MISSING` · `demandState` | «빈 선언»의 칸을 누가 설명하나 — 계획이 비면 «스켈레톤»이 받는다 (「문」) |
| `client2/src/pickup_state.js` | 🆕⑳ **80**(구 표기 84) | `PICKUP_UNREAD` · `ageText` · `pickupState` | 「집는 이가 살아 있나」 — 대기는 «두 봉우리»라 줄 길이로는 안 갈린다. `/admin/chain/queue` 의 `queue` 버킷(안 읽히던 것)이 재료 |
...
## 5-I. 미등재 모듈 레지스트리 2026-08-18 실측

> 🔴 **이 절은 「맵이 무엇을 «주장»하는가」를 감사해서는 절대 보이지 않는 드리프트를 담는다 — 맵에 «절이 아예 없는» 파일이다.** 같은 날 앞선 두 감사(파일 존재 53건 · 심볼 749건)는 둘 다 **맵이 이미 적어 둔 것**만 대조했으므로, 새로 들어온 코드는 구조적으로 시야 밖이었다. 아래는 `git ls-files` ∖ (이 문서에 이름이 나오는 파일)의 차집합 중 **운영 코드만** 골라낸 것이다.
>
> ⚠️ **여기 있는 것은 «존재와 크기와 한 줄»뿐이고, 심볼 표는 없다.** 있다고 주장하지 않으려고 일부러 그렇게 했다 — 이 패스는 각 파일의 docstring 머리와 줄 수만 실측했고 **본체를 읽지 않았다.** 여기 오른 모듈을 쓰기 전에 소스를 열어라. 절이 필요할 만큼 중요해지면 §5-x로 승격하고 이 목록에서 지운다.
>
> 🔬 **측정 기준: `5359fdd`(HEAD)의 커밋된 blob.** 줄 수는 `git show HEAD:<path> | wc -l`.
>
> 🚫 **이 목록이 받지 않는 것**: `server/tests/**` · `server/scripts/**`(진단·씨앗·마이그레이션 러너) · `server/scratch/**` · `server/setup/**` · `server/migrations/*.py`. 이 지도는 애초에 그 계열을 파일 단위로 등재하지 않으므로 **「미등재」가 드리프트가 아니다.** 예외로 아래 「마이그레이션·맵퍼」 항만 이름을 남긴다(다른 절이 이름으로 참조할 수 있어서).

### 서버 — 원장 읽기 측 (라우터 `ledger_trace_router`의 소비 모듈)

🆕⑬ **[2026-08-29 전면 재작성 — `3b77fbc3`] 이 표는 «여섯 행 중 넷이 없는 파일»이었다.** 개정 6이 키를 받는 데이터 라우트를 지우면서 그 몸도 같이 갔다. 남은 읽기 측은 **셋**이고, 그중 라우터가 부르는 것은 `ledger_subgraph` 하나 + `ledger_trace`의 배관 셋(`relation_exists`·`_fetch`·`REASON_RELATION_ABSENT`)뿐이다.

| 파일 | 줄 | 무엇인가 (🆕⑬ 실측) |
|---|---|---|
| `server/ledger_api/ledger_subgraph.py` | 🆕⑭ **1,188**(🆕⑬ 1,030 @`3b77fbc3` — **+158**) | `GET /subgraph`의 몸. **↓ 아래 전용 항목** |
| `server/ledger/explorer.py` | 🆕⑬ **175**(구 표기 445 — **−270**) | 🔴 **이제 «id 코덱»이다.** `entity_id(entity_type, keys)` · `decode_entity_id(value)` · `_entity` · `_edge_rows` · `_depths` · `_target_of`. **`explore`·`explore_entity`는 없다** — 「동결」 딱지가 붙어 있던 그 둘이 동결이 아니라 삭제로 끝났다 |
| `server/ledger/trace.py` | 🆕⑭ **1,700**(🆕⑬ 1,688, 구 표기 2,039) | 해결기·계보 walk. 🔴 **`trace()` 자체가 없다** — `^def trace` 전건 히트 0 (`_map_slot` 도 같이 갔다). `coverage()` 는 살아 있으나 **HTTP 소비자가 «0»이다**(`/trace`·`/coverage` 둘 다 라우트가 없다). 라우터가 실제로 쓰는 것은 배관 셋뿐이고, **심볼 표는 §5-H에 있으며 `a729a7f` 기준이라 재검증되지 않았다** |
| 🆕⑬ `server/ledger_api/declared_entities.py` | **93**(신설) | 선언된 «개체 타입»과 «신원 키»의 유일한 캐시 리더 — `load(force_reload=False)` · `declared_types()` · `identity_keys(entity_type)` · `_bare("wafer@1") -> "wafer"`. 🔴 **절대 예외를 올리지 않는다** — 선언이 없거나 깨지면 목록이 비는 것이고, 그것이 답이다. docstring이 「`reference_edges`·`reference_edge_names`·`targets_for`는 호출자가 «0»이라 2026-08-28에 나갔다」고 적는다 |
| 🪦 `ledger_trends.py` · `ledger_composition.py` · `ledger_selection.py` · `ledger_identity.py` · `ledger_catalog.py` · `ledger_journey.py` | — | **삭제.** 구 표기 511 / 301 / 1,423 / 62 / (catalog·journey는 §5-H 표) — **앵커로 쓰지 마라** |
...
### 🆕⑱ 서버 — 미등재 신설 둘 (2026-09-05 실측 · `6345ef3d`)

> 🔬 이 항목도 위 규율 그대로다 — **존재·줄 수·최상위 심볼·소비자 계수뿐이고 본체를 읽지 않았다.**

| 파일 | 줄 | 무엇인가 (🆕⑱ 실측) |
|---|---|---|
| **`server/chain/activity.py`** | 🆕 **264** @`b1db471a`(🆕⑳ 120 @`328a5c20` · 🆕⑱ 82 · 신설 `2574e251`) | 「지금 어느 맵퍼 «안»에 무엇이 들어 있나」의 프로세스 내 레지스트리. `class ChainActivityRegistry` :30 — `attach()` · 🆕⑳ **`note_reload()`** · `note_outbox_purge(deleted, capped)` :70 · `attached`(property) · `start(rule, mapper, target_table, rows_in)` :94 · `record_outcome(rule, outcome, reason=None)` :105 · `seed_rules(names)` :125 · `outcomes()` :140 · `finish(token)` :149 · `snapshot()` · 🆕⑳ **`ages()`** · `clear()`, 그리고 싱글턴 **`registry = ChainActivityRegistry()`** :205.<br>🆕 **[`38b5d8e1` S-246] 등록이 «컨텍스트 매니저» 하나가 됐다** — `NO_ROWS_REASON = "the mapper produced no rows"` :211 · `class _Run` :214(`produced(rows_out)` :220 — 안에서 말할 수 있는 «하나», 낸 행 수) · **`@contextlib.contextmanager running(rule, mapper, target_table, rows_in, no_rows_reason=NO_ROWS_REASON)`** :225. `registry.start` → `yield entry` → 던지면 `record_outcome(RULE_OUTCOME_FAILED, "<Type>: <msg>")` 후 re-raise · 아니면 `entry.rows_out` 이 «None 이 아닐 때만» `RAN_CHANGED`/`RAN_UNCHANGED`(+`no_rows_reason`) · `finally` 에서 `registry.finish(token)`(던진 런이 «영원히 도는 중»으로 남는 바로 그 경우). ⚠️ **행 수 «부재»는 0 이 아니다** — `produced` 를 안 부른 호출자는 결과를 «그대로 둔다»(「안 셌다」와 「0 이었다」는 다른 사실이고 이 레지스트리는 그 둘이 헷갈려서 생겼다). ⛔ 자기 때문에 «절대» 던지지 않는다. 🔴 **문 «둘»이 같은 등록을 지난다** — `chain/mapper_call.execute_custom_mapper` :130(파일 맵퍼) · `chain/builtins.run_builtin` :302(`builtin:` 종류 — 종전엔 등록이 «파일 맵퍼의 문 안에만» 있어 `join_into`·`builtin:join`·`auto_confirm` 이 큐 화면에 «안 뜨고» 돌았고, 로더가 모든 규칙을 `never_evaluated` 로 씨앗하므로 천 번 돈 builtin 이 「아직 평가 안 됨」이었다. 소유자 2026-09-15 「체인 대기열에서 안 뜨고 돌고 있었네」). 시험 `test_a_rule_that_runs_is_in_the_queue_view_whichever_door_ran_it.py`(215줄 / `def test_` 12). ⚰️ ~~**소비자 «2»**~~ → 🆕 **«넷»**(`git grep` 전건, 시험 제외): `chain/ingestion_worker.py`(채우는 쪽 — `attach()` :3497 · `start`/`finish` 는 이제 «안 부른다» · 🆕⑳ `note_reload()` :3596 · `seed_rules` :934 · `note_outbox_purge` :310 · `record_outcome` :1183) · `chain/mapper_call.py`(`running`) · `chain/builtins.py`(`running`) · `main.py`(어드민 큐 라우트 :4542~:4582 가 `snapshot()`·`outcomes()`·`.attached`·🆕⑳ `ages()` 를 읽는다). 🔴 **`attached` 가 이 모듈의 요점이다** — 「아무것도 안 돈다」와 「이 프로세스에는 루프가 없어서 «볼 수 없다»」는 다른 사실이고, 레지스트리가 프로세스 안에 사는 한 웹 프로세스는 후자다.<br>🔴 🆕⑳ **`ages() -> {"loop_uptime_seconds", "mapper_reload_age_seconds"}`(`f8bbdf47`) — 「재기동하면 풀리나」에 답하는 두 수.** 상태는 `_attached_at`(=`attach()` 시각) · `_reloaded_at`(=`note_reload()` 시각)이고 `clear()` 가 둘 다 `None` 으로 되돌린다. ⚠️ **한 번도 없었으면 `None` 이고 `0` 이 아니다** — `0` 은 「방금」이라는 «반대» 사실이고, 루프가 «시작»한 것은 재적재가 아니다. **시각이 아니라 나이로 내는 이유**: 읽는 쪽이 이 둘을 서로 그리고 `oldest_waiting_seconds` 와 비교하므로, 시각 문자열이면 자기 시계와 먼저 화해시켜야 한다.<br>🔴 **재적재 순간은 «이미 기록돼 있었고 나가지 못했다»** — `QueueHeadWatch.note_reload` 가 정체 감시 착지 이래 들고 있었지만 나가는 길이 **루프가 찍는 로그 «문장 속 글자»** 뿐이었고, 그것도 큐 머리가 1분 넘게 막힌 «뒤»에만 찍혔다. 즉 「상태가 데이터가 아니라 이 프로세스에 있나」라는 질문을 **이미 멈춘 시스템의 로그를 읽는 사람만** 물을 수 있었다. 채점자 `server/tests/test_the_queue_says_when_the_mappers_reloaded.py` |
| 🔴 **`server/mapper_sdk.py`** | 🆕 **294** @`9b498ec9`(구 표기 266 · 신설 `f74f09c9`) | 체인 맵퍼 저자가 `df -> df` 만 쓰고 비즈니스 키는 프레임워크가 정하게 하는 SDK. `_python_scalar` · `class MapperContractError` · `payloads_to_df(payloads)` · `class BaseMapper` · `sql(db, query, params=None)` · `df_to_updates(df, table_name, *, source_name, updated_by)` · `mapper(target_table=None, *, source_name="chain_ingestion", …)` 데코레이터.<br>⚰️ ~~운영 소비자 «0»~~ — 🆕㈡ **[2026-09-13 S-215 `c3015b8a`] 그 문장은 낡았다.** 오늘 이 모듈이 내놓는 것은 «세 가지»다 — `BaseMapper` :77 · `payloads_to_df` :53 같은 자기 함수, 그리고 🔴 **`MAPPER_SURFACE` :422 — 운영자의 라이브 맵퍼가 실제로 import 하는 이름 «41» 표**(제 실측: 항목 41 · 원천 모듈 9 · 사설 둘 `_cells_of`·`_load_metas`). 🔵 **표가 곳 선언이다** — `__getattr__` :473 에 그 밖의 출처가 «없어서», 「선언된 표면에서만」이 습관이 아니라 «코드의 성질»이다. 그리고 돌려주는 것은 «같은 객체»이지 사본이 아니다(판정 368 ①) — 감싸면 구현이 둘이 되고 갈라진다. ⚠️ 이름은 «고른 것이 아니라 잼 것»이라 줄이는 순간 이 저장소가 볼 수 없는 파일이 깨진다. ⚠️ 게으른 풀기인 이유도 적혀 있다 — 그 아홉 모듈이 13,406줄이고 이 파일은 체인 워커의 «부팅 경로»에서 읽힌다. ⚰️ 아래 낡은 문장을 남긴다 — 「그때 무엇을 재고 그렇게 적었나」가 기록이다. 종전 문장: `server/` 전건에서 이 모듈을 import 하는 것은 `server/tests/test_mapper_sdk.py` 뿐이었다(나머지 히트는 `docs/guide/chain_ingestion_guide.md` 와 `task/**` 의 산문). **가이드가 이미 그것을 가르치는데 도는 맵퍼는 아직 하나도 안 쓴다** (2026-09-07 재확인: 추적 견본 일곱 · 라이브 아홉 «전부» 데코레이터를 안 쓰고, `df_to_updates` 를 부르는 자리는 `mapper()` «하나»다). 🆕 🔴 **[2026-09-08 · `9b498ec9`] `df_to_updates` 가 「선언을 못 읽음」과 「선언에 키가 없음」을 «같은 `{}`»로 접고 «둘 다에서» 봉투를 그냥 내보내고 있었다** — `TABLE_CONFIG.get(name) or {}` 에 composite/plain 두 갈래뿐이고 `else` 가 없어서, 그 표를 이 프로세스가 못 보면 `business_key_val` «없이» 착지했다. 유일 인덱스는 NULL 을 서로 다른 것으로 치므로 «오류 없이» 실행마다 사본이 하나씩 늘었다(소유자 신고 09-07). 이제 «둘 다 거절»하고 «문장이 갈린다** — 못 읽음은 «설정» 사실, 키 없음은 «선언» 사실이라 운영자의 다음 행동이 다르다 ⚠️ 🆕⑳ **종전 이 줄이 같은 부류의 «형제»로 들던 둘은 «둘 다 그 부류를 떠났다»** — `health.roster_states` 는 [🪦 지워졌고](#5-소형-서버-모듈) `implementations.implementation_choices` 는 [배선됐다](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재)(`b72843cc`). 🔴 **그래서 이 항목은 이제 그 부류의 «유일한» 구성원이고, 그것이 이 행을 더 중요하게 만든다** — 옆에 같은 것이 둘 더 있어서 안 이상해 보이던 것이 아니게 됐다<br>🆕㉗ **[S-188 ⓑ `9c7d850c`] 줄 수 «395»(구 표기 294), 그리고 데코레이터가 «말해지던 그 레지스트리»가 됐다** — `MAPPER_REGISTRY` :253 · `class MapperNameClaimedTwice(MapperContractError)` :261 · `reset_registry()` :268 · `_origin(fn)` :281 · `register(name, fn, params=())` :293 · `discover(package="mappers")` :311 · `mapper(...)` :339 이 `params=` 와 `name=` 를 받는다. 🔴 **한 이름을 둘이 주장하면 «이름을 대고» 던진다** — 조용히 덮으면 어느 매퍼가 도는지 «아무도 모른다». 🔵 그리고 그 레지스트리를 «처음 쓴 것»이 벤치다(위 🆕㉗ `dev_bench.try_mapper` — 「추측할 이름이 없다」) |

#### 🆕⑰ `server/ledger_api/ledger_subgraph.py` (**1,253줄** @`64b562b6` — 🆕⑭ 1,188 @`2a4e51cf`) — **노드 종류가 «하나»고, 걷기의 가드가 «셋»이 됐다**

> ✅ 🆕⑱ **[2026-09-05] 이 절은 재측정할 것이 «없다» — 파일이 `64b562b6` 과 «blob 동일»이다**(양쪽 다 `1ab00a96`). 🔴 **`server/ledger_api/` 전체가 이 구간에 한 바이트도 안 바뀌었다**(`git diff 64b562b6..HEAD -- server/ledger_api/` 무출력). 🆕⑰ 가 잰 가드 셋·시그니처·줄 수는 **글자 그대로 그대로다.**
...
🆕⑭ **[2026-08-29 심야] 걷기 규칙 둘이 들어오면서 이 표의 시그니처 «넷»이 바뀌었다** — `_reach` · `_propagation` · `subgraph` · 그리고 `DEFAULT_CONTINUES_HOPS` 라는 상수 이름 자체. 아래 행에 실측으로 갈아 끼웠다.

> ✅ 🆕⑮ **[2026-08-30] 아래 표는 재측정할 것이 없다 — 파일이 `2a4e51cf` 와 «blob 동일»이다**(둘 다 `5a93d837`). 🆕⑭ 가 잰 심볼·시그니처는 `5a73021a` 에서 **글자 그대로** 그대로다.
>
> 🔴 🆕⑮ **그런데 «모듈 docstring» 은 은퇴한 그래프를 그린다 — 그리고 그것이 이 파일에서 가장 먼저 읽히는 것이다.** 27줄짜리 머리말이 오늘 존재하지 않는 노드 종류와 엣지로 도식 «셋»을 그린다:
> ```
> docstring 이 그리는 것                          오늘의 실측 (5a73021a)
> Event --asserts--> Claim --subject--> Entity    노드 종류가 «하나»다. decode_node_id 는
>                                                 ledger-entity:v1: 만 풀고 그 밖은 ValueError
> Finding Collection / terminal Finding Point     🪦 은퇴 (2026-08-28). 아래 표의 묘비 행
> Value --binding--> Quantity                     🪦 binding 은 투영이 지어낸 엣지였다
> Finding Collection --finding--> Quantity        🪦 finding 도 마찬가지
> Quantity --mechanism--> Quantity                🪦 mechanism 도. 「선언된 mechanism graph 를
> (synthesized from `mechanism_models.json`)      mechanism_models.json 에서 합성」하는 적재가
>                                                 2026-08-28 에 나갔고 그 파일은 _archive/ledger_api/
> ```
> 🔴 **소스 «자신»이 본체에서 그 docstring 을 반박한다.** `subgraph` 안에 남은 주석이 「THE MECHANISM LOAD LEFT 2026-08-28. It ran on EVERY request to build `models_by_name`, and the only thing that read it was a quantity seed branch that `decode_node_id` cannot produce」라고 적는다. **머리말은 안 고쳐졌고 본체 주석만 고쳐졌다** — 그래서 파일을 위에서부터 읽는 사람이 «가장 낡은 문장»을 «가장 먼저» 만난다. `mechanism_models` 라는 낱말은 이 파일에 **딱 한 번**, docstring 16행에만 나온다.
>
> 🔴 🆕⑮ **그리고 `_seed_node` 의 갈래 «다섯»이 도달 불가다 — 이것은 docstring 이 아니라 «코드»다.** `seed_ref` 는 `subgraph` 의 `seed_refs = {item: decode_node_id(item) for item in seed_signs}` 에서만 오고, `decode_node_id` 는 `{"kind": "entity"}` 를 내거나 raise 한다. 그러므로 `_seed_node` 의 `entity` 갈래만 돌고 **`event`·`claim`·`point`·`collection`·`action` 다섯은 영원히 안 돈다.** 🔴 **그 죽은 갈래들이 은퇴한 도메인 낱말을 문자열로 들고 있다** — `"type": "Finding Collection"` · `"type": "Finding Point"` · `"schema_kind": "terminal_finding_point_projection"` · `seed_ref["finding_kind"]`. **grep 으로 「Finding Collection 이 아직 있다」를 재면 여기서 걸리고, 그것은 «도는 코드»가 아니다.** 같은 이유로 `subgraph` 끝의 `any(ref["kind"] == "action" for ref in seed_refs.values())` 는 **영원히 거짓**이다— 「영원히 거짓인 필터는 거짓이 정답인 동안 숨는다」와 같은 모양이라, **`action` 씨앗이 다시 생기는 날 조용히 틀린다.** ⚠️ **코드 소관이라 여기서 고치지 않는다. 사실만 등재한다.**
>
> ⚠️ **[§5-H](#5-h-정본-원장-canonical-ledger) 의 `ledger/trace_router.py` 절이 이미 같은 모양을 적어 두었다** — 그 파일의 모듈 docstring도 「the ledger read routes — ten of them」이라며 열을 나열하는데 `@router.` 는 둘뿐이다. **같은 폴더의 파일 둘이 같은 방식으로 낡았다: 「`docstring` 을 목록의 출처로 쓰지 마라」는 이 폴더의 규칙이지 한 파일의 사고가 아니다.**

| 심볼 | 실측 (🆕⑭ `2a4e51cf`, 표기 없으면 `3b77fbc3`) |
|---|---|
...
| 🆕⑬ `_signed_seeds(start)` | 첫 인자가 id 하나 **또는** `{"positive": [...], "negative": [...]}`. 🔴 **상태가 셋이고 셋인 채로 둔다** — `+` 관측 / `−` 찾아봤는데 없음(대조군) / **미등재 = 한 번도 안 봄**. 목록에 없는 주체를 대조군으로 승격하지 않는다. 같은 id가 양쪽에 있으면 이름을 대고 거절 |
| 🆕⑬ `_declared_key_order(entity_type)` / `_entity_node(entity_type, keys)` | **라벨의 키 순서를 «선언»에서 읽는다** — `ledger_config.json`의 `entities.<type>.keys` 순서(캐시, 예외 없음). 🔴 **`ledger_explorer._entity`의 순서는 v1 `ENTITY_TYPES` 기반이라 나중에 선언된 타입이 «부재»이고**, 그때 라벨이 페이로드 JSON의 우연한 순서를 쓰게 된다 — `die`에서 `x`/`y`가 앞에 서고 자재를 이름 짓는 유일한 키 `mat_id`가 두 값짜리 라벨 밖으로 밀린다. 못 읽는 선언은 **오늘의 라벨 그대로** |
| 🆕⑭ `_reach(nodes, edges, seed_signs, static_types=())` → **`(reach, parents, kinds)`** | 🔴 **더 이상 «나누지 않는다»**(소유자 판정 2026-08-29: 「그냥 많이 재면 신호 약해지겠구나」). 씨앗 하나가 노드에 닿으면 **1**, 그래서 `reach` 는 `노드 -> [양성 씨앗 수, 음성 씨앗 수]` 의 **정수쌍**이고 거리도 감쇠도 안 센다. 나누면 «더 많이 재어 본» 주어가 벌을 받는다(소스 실측: 발견 3개를 든 다이는 각 0.333, 12개를 든 다이는 각 0.083). 실측이 그 문장을 받친다 — 나누면 후보 996 중 **981이 소수**이고 최상층이 서로 지배 못 하는 둘이었는데, 안 나누면 값이 **다섯 개 정수**이고 최상층이 **한 노드**다. 동률이 «정확»해져 반올림 허용오차를 지어낼 필요도 없다. 🔴 **반환이 셋인 것이 이 개정의 요점이다** — 셋째 `kinds` 는 `씨앗 -> 그 씨앗이 닿은 «선언된 타입» 집합`, 즉 순위가 필요로 하는 **분모**다(reach 0 은 자기에 대해 아무 말도 못 한다). 🔴 **자기 가드 둘을 «다시» 적어 갖는다** — ① 이름은 다른 이름으로 갈 수는 있어도 세상으로 돌아 나오지 못한다 ② 방금 올라온 술어로 되내려가지 않는다(단 이름↔이름 쌍은 면제). 소스가 그 중복을 «비용»이라 이름 대고 적어 둔다 — 대안은 씨앗마다 fetch 한 번이라 질의가 네 배다. 안 하면 나누기를 멈춘 순간 후보 996이 **전부 [2,2]** 로 나온다(실측) |
| 🆕⑭ `_propagation(nodes, edges, seed_signs, complete, static_types=())` | 순위 블록. `_reach` 에 `static_types` 를 그대로 흘린다. 🔴 **`ranked[]` 항목에 `reachable` 이 «`reach` 옆에» 붙었다** — 중첩 헬퍼 `_reachable(node_type)` 이 「이 후보의 «타입»에 한 번이라도 닿은 씨앗 수」를 양/음으로 세어, 각 축이 **`reach[i]` / `reachable[i]` 쌍**으로 읽힌다. 없으면 「길이 있었는데 이 인자를 안 실었다」와 「그 타입에 길 자체가 없었다」가 **둘 다 0** 으로 같아 보인다. 소스가 세 픽스처의 수를 적는다 — 건전한 대조군에서 `SYN-R-CMP-01` 이 2/2 대 0/2, 다리가 끊긴 대조군에서는 같은 레시피가 **0/0** 이라 거짓 답이 «자신 있어» 보이는 대신 «보인다». ⚠️ **분모는 «타입»이지 «항목»이 아니다** — 열여섯 물리량을 잰 대조군은 `quantity` 에 닿으므로, 그중 안 잰 항목도 0/2 로 보인다. 소스가 그 한계를 스스로 적는다<br>🆕㉞ **[S-147-b `21440c63`] «물어볼 수 없었던» 대조는 분모에서 나간다.** 종전 분모는 «타입»이라, `quantity` 에 «닿기만» 한 대조가 «한 번도 재지 않은» 항목에 대해 분모에 들어갔고 **0/2 가 진짜 차이처럼 보였다**. 🔴 가르는 것은 「그 대조에게 이 질문이 «판정 가능»했나」이고, 그것이 S-149 가 이미 노드에 접어 둔 verdict 다 — `unknown` 이 분모를 떠나므로 그 자리는 **0/0** 이 되고, **0/0 은 차이가 아니다**. 📌 부류: 「모집단이 틀리면 수가 거짓말한다」 — 이 파일의 주석이 그 결함을 «스스로» 적고 있었다 |
| `_rank_layers(items)` · `_evidence(nodes, parents, seed_signs, node_id)` · 🆕⑬ `_seed_node(seed_id, seed_ref, action_lookup)` | 순위·근거·씨앗. 🆕⑬ **`_seed_node`의 `models_by_name` 인자가 빠졌다**(기전 모델 로드가 walk에서 나갔다). **예산이 끊은 후보는 「없음」이 아니라 「미검사」로 답한다** — 응답의 `truncated`가 무엇이 끊었는지 말한다(`depth`/`nodes`/`edges`/`claims`/`actions`) |
| `SqlEvidenceLookup(connection, relation="ledger_events")` · `InMemoryEvidenceLookup` · `EvidenceAtom` · `ATOM_COLUMNS`/`EVIDENCE_COLUMNS` | 🔴 **`follow`는 SQL 안에 있다**(`claims_for_entities(..., follow=None)`) — 여기서 걸러진 술어는 **애초에 안 가져오므로 예산을 안 쓴다**. 투영에서 거르면 걷기는 같은 벽에서 멈추고 무엇을 모았는지만 감춘다. `relation`은 맨 식별자만 받는다(`_IDENTIFIER`) |
| 🆕㉝ `seed_type` 축 :1666·:1701~ · `DEFAULT_SEED_LIMIT = 200` :284 · `limits.seeds`/`truncated.seeds` (:2269·:2318) | **씨앗 집합을 «저장하거나 열거»하지 않고 «타입으로 기술»한다**(S-148-a `3644b8dc`+`9b3b20a9`+`e29f7ab6`, 판정 337) — 대조군이 «요청 하나»에 들어간다. ⛔ 선언에 없는 타입은 `seed_type_not_declared` 로 «아는 집합과 함께» 거절한다(:1711). 🔴 **예산이 «이미 있는 쌍» 위에 붙었다** — `limits`/`truncated` 에 `seeds` 가 한 칸 늘었을 뿐이고, 별도 예산을 만들면 「잘렸나」의 답이 «둘»이 된다(판정 331 의 「예산은 하나」 그대로). ⚠️ **그리고 이 기능은 «세 번째 좌석»에서야 닫혔다** — ① 함수는 초록이었고 ② 라우트가 `id` 를 «필수»로 요구해 «핸들러에 닿기 전» 거절했고(`9b3b20a9`) ③ 질의가 SQLAlchemy 커넥션에 «DBAPI 커서»를 써서 박스에서 500 이었다(`e29f7ab6`). 🔴 **앞 두 겹은 «함수를 직접 부르는» 시험이 영원히 못 본다** — 그래서 처방이 「라우트를 태우는 시험」이다<br>🆕㈢ **[C-97 `0ce286b9`, 판정 365] 그 축을 «읽는 화면»이 생겼다** — 걷기 페이지가 `seed_type=<type>` 을 보낸다. 🔴 종전엔 타입만 고르고 키를 비우면 **422**(`entity id must contain [type, structured keys]`) 였고, 「이 타입이 무엇에 닿나」는 **주어를 모를 때 묻는 질문**이라 그 끝이 막힌 것이 결함이었다. ⚠️ 라우트는 S-148-a 부터 받고 있었고 클라가 «제로» 읽지 않았다(`git grep seed_type -- client2/src` = 0) — C-96 과 같은 문장이다 |
...
| 🆕㉜ `AGGREGATE_MEASURES` :59 (`count`·`distinct`·`sum`·`mean`·`min`·`max`·`median`) · `NUMERIC_MEASURES` :63 · `_fold(measure, values)` :95 · `group_by` 가 이름 댈 수 있는 것 :65 | **걷기가 «자기 모집단»을 접는다**(S-146 `a4d80302`, 판정 331) — 그래서 **수가 예산을 «살아남는다»**. 🔴 그것이 이 인자의 존재 이유다: 클라가 세려면 걷기가 «전부» 실어 와야 하고, 큰 규모에서 그것은 「세려다 잘린다」가 된다. 접는 자리를 옮기면 «잘린 표»가 아니라 «옳은 수»가 온다. 🔵 측정자는 «일곱이고 그 밖은 없다»(소스가 그렇게 적는다) — 열린 목록이면 화면마다 다른 것을 세기 시작한다. ⛔ **못 받는 이름은 «이름 대어» 거절한다** — 철자 틀린 measure 가 조용히 떨어지면 그 수는 «다른 수»이고 아무도 모른다. 숫자를 접는 측정자에 숫자 아닌 값이 오면 `measure_needs_numbers` 로 이름 댄다. 📎 계약 `contracts/walk_aggregate/`<br>🆕㉝ **[S-146-c `826464ee`, 판정 336] 한 그룹이 «측정자 여럿»에 답하고 «언제인지»도 말한다** — `groups[]` 의 value 가 **«항상 맵»**(측정자 하나여도), 행에 `at` :256, 그리고 `VALUE_SOURCES` :130 = `attributes`·`qualifiers`·**`predicates`** — 즉 «술어의 개수»도 값의 출처다. 🔴 **value 를 «항상 맵»으로 둔 것이 설계다** — 측정자가 하나일 때만 스칼라면 읽는 쪽이 «두 모양»을 다뤄야 하고, 그 갈래가 곧 「측정자를 하나 더 물었더니 화면이 깨진다」다. 🔵 그리고 **엣지가 접기의 «입력»이다** — 노드만 접으면 「이 술어로 몇 번 닿았나」를 세지 못한다. 📎 응답이 `value_sources` 를 «같이» 낸다(:2310) — 화면이 「무엇으로 묶을 수 있나」를 «추측»하지 않게 |
| 🪦 `tabular_projection` · `NODE_KINDS` · `RETIRED_NODE_KINDS` · `FOLDED_KINDS` · ~~`collect`~~(🆕㉓ **이름이 «돌아왔다»** — 배관 종류를 고르던 그 축은 죽었고, «도메인 노드 타입»을 고르는 새 인자가 같은 이름을 쓴다. 위 라우트 행 참조) · `observation_mode` · `include_values` · `quantity_node_id` · `_quantity_node` · `_bound_quantities` · 🆕⑭ **`NODE_TABLE_COLUMNS`·`EDGE_TABLE_COLUMNS`·`PROPERTY_TABLE_COLUMNS`** · 🆕⑭ `_link_containers` · 🆕⑭ `DEFAULT_CONTINUES_HOPS` · 🆕⑭ `continuing` | **전부 없다**(`2a4e51cf` 전건 grep). 🔴 **세 `*_TABLE_COLUMNS` 는 🆕⑬ 이 「호출자 0」이라 적어 둔 바로 그 셋이고, 그 행이 삭제 근거가 됐다.** 살아 있는 상한 상수는 `MAX_EDGE_LIMIT = 6000` · `MAX_CLAIM_SCAN = 6000` 이고 소스가 정착 실측을 곁들여 적는다(`edge_limit 3000` 은 아직 엣지에서 끊기고 **6000 에서 5,079 엣지로 정착** · claim scan **6000 에서 1,805노드/720엔티티**). ⚠️ `DEFAULT_PROPERTY_LIMIT`/`MAX_PROPERTY_LIMIT` 은 «선언만» 남아 있다 — 이 파일 안에 읽는 자리가 없다 🆕㉝ **[S-150 `3953f695`] 엣지의 상수 칸 «셋»이 은퇴했다 — «아무도 안 읽었기» 때문이다.** 🔵 그것을 판정한 방식이 요점이다: 「소비자 0」을 세고, 그 0 이 «데코레이터·시험·설정 문자열·명령줄» 네 갈래 중 하나가 아님을 확인했다(상설의 그 네 갈래). 시험이 그 부재를 못 박는다 (`test_an_edge_carries_no_constant_nobody_reads.py`) — 다시 붙는 날 빨개진다 📐 남은 것은 `_edge(edge_type, source, target, *, original_predicate=None, cardinality=None)` :1006 이고, `cardinality` 는 «선언이 말한 것»이다(S-133 ④) — `one` 이면 화면이 «그린 것에서 추측»하지 않고 그렇게 말할 수 있다. ⚠️ **`None` 은 「선언이 «안 말했다»」이지 「하나가 아니다」가 아니다** — 소스가 그 구별을 적어 둔다 |
...
`main.py` :846·:950·:2724·:2785 · `ledger_api/ledger_subgraph.py` :1066 · `admin/dev_bench.py` :94.
⚠️ **지시서는 「자리 다섯」이라 적었다** — 제품 자리가 다섯이고 여섯째는 «벤치»(`dev_bench`)다.
벤치가 같은 함수를 지나는 것이 요점이라(아래) 이 줄은 여섯으로 세고 갈래를 적는다.

#### `server/admin/dev_bench.py` (🆕㉙ **824줄** — S-197 에서 385→824) + `server/scripts/try_core.py` (**59줄**) — 샘플 하나로 «돌려 보는» 자리 (S-192 `803a709c`·`12985006`)
...
🆕㉙ **[S-197 ⓐ `bcde6af3`] 벤치가 «제품이 주는 입력»을 그대로 건네고, «못 읽음»이 값이 됐다** — `input_for_mapper(table_or_sample, *, rows=None, row_ids=None, where=None)` :328(`DEFAULT_INPUT_ROWS = 200` :309 · `_ensure_dynamic_models` :312) · `raw_for_parser(file_path, *, lines=20, head_bytes=65536)` :394 · `claimers(file_path, *, scripts_path=None)` :436 · `run_stages(parser_cls, file_path, …)` :469. 🔴 **`UNREADABLE = "DataFrame 으로 못 읽음"` :386 이 «값»이다** — 파서 작업은 「이 파일이 무엇인가」를 모르는 데서 시작하므로 «못 읽었다»가 예외가 아니라 답이어야 한다. 🔵 인코딩은 사다리로 시도한다(`ENCODING_LADDER` :391 — `utf-8-sig` → `utf-8` → `cp949` → `latin-1`). 🔵 `claimers` 는 「이 파일을 «무는» 파서가 누구인가」를 제품과 «같은 방식»(각 파서의 `match()`)으로 묻는다.
🆕㉙ **[S-197 ⓑ `8ec6bf4a`] 발행은 «스스로를 증명»한다** — `publish_mapper(...)` :607 · `publish_parser(...)` :692 · `cell_body(fn)` :549 · `class PublishRefused` :496 · 템플릿 둘 :588·:657 · `_import_published` :523 · `_discard` :532. 🔴 **요점은 파일이 아니라 «대조»다** — `inspect.getsource` 도 복붙도 「마지막으로 «실행된» 셀」을 내보내지 「화면 위의 셀」을 내보내지 않는다. 그래서 발행된 파일을 «다시 import 해서» 노트북이 돌던 «그 프레임»에 돌리고, 같은 행이 나올 때만 살아남는다. 어긋나면 «지우고 이름 대어 거절»한다. ⚠️ **프레임을 «넘겨받지 다시 유도하지 않는다»** — 다시 유도하면 창(`rows=`/`where=`)이 다시 돌아 «다른 창 = 다른 답»이 되고, 그러면 대조가 «본문과 무관한 이유»로 실패한다.
...
>
> 🆕⑩ 🔴 **`source_preparation.py`의 `_event_frames` 서술 한 줄은 살려서 옮긴다**(2026-08-19 판정): `occurred_at.basis`가 있으면 그룹 사건의 시각을 그룹의 **`min`**으로 접고(적재 시각은 한 사건 안에서 여럿일 수 있다), `column`이면 종전대로 **정확히 하나**를 요구하며 거절한다(세계 시각이 둘이면 그룹이 틀린 것이라 어떤 집계도 못 고친다). ⚠️ **이 패스는 `_event_frames` 본체를 다시 열지 않았다** — 위 문장은 `5359fdd` 기준이고 심볼 존재만 `8d1e6c4c`에서 확인했다.

⚰️ 🆕⑩ **[2026-08-18 워킹트리 경고 철회 — 그 레인이 착지했다]** 이 자리에 「번역기 다섯이 워킹트리에서 삭제된 상태이고 HEAD 기준으로는 살아 있다」는 경고가 있었다. **`8d1e6c4c` 실측: 다섯 전부 HEAD에서도 없다**(+`cutover_v2.py`·`examples/grouped_translator_template.py`·`mappers/ledger_lot_event_mapper.py`). 예고된 묘비는 [§5-H](#5-h-정본-원장-canonical-ledger)에 섰고 `lot_event_translator.py`의 심볼 표도 거기서 대체됐다. 🔴 **그 경고가 «다음 패스에 무엇을 하라」를 적어 둔 덕에 이 패스가 삭제부터 찾을 수 있었다** — 미래형 경고는 값이 있다.

### 서버 — 그 밖의 운영 모듈

| 파일 | 줄 | docstring 첫 줄 (실측) |
|---|---|---|
| ⚰️ `server/ledger_api/ontology_config_explorer_router.py` | ~~129~~ → 🆕⑩ **264** | **[§5-H-bis](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재)로 승격했다.** 🔴 여기 적혀 있던 **「라우트 7종」은 2026-08-21 실측 «16»이다** — 그 사이 `/columns`·`/authoring/schema`·`/authoring/plan`·`/deletion-preview`·`/test-run`·`/bootstrap`·`/drafts/new`·`DELETE /declarations/{key:path}`가 섰다. 게이트 서술도 정정됐다(읽기 `/view`만 보통 토큰 → **여섯이 보통 토큰**) |
| 🔒 `server/ledger/admin.py` | 🆕 **1,284** @`dc877746`(🆕⑪ 848, 구 표기 844) | **[동결 — A군]** admin으로 소스를 원장에 잇고 어휘를 늘린다 — **문법 검증과 저장(1단·3단)**. 판정 정본은 `docs/process/LEDGER_RULINGS.md` **R-2026-08-15-M**. 🆕 **[`00da7e91` S-244 · `2504fbde` 판정 402] `save_chain_rule_raw(name, declaration, base)` :613 — 체인 규칙 «날것» 저장 관문이 «로더가 세울 것»을 심판한다**: ① `rule_shape.expand_declaration(entry, crud.TABLE_CONFIG)` :691 — 거절이면 `declaration_refused` :694 ② 세워진 후보 «각각»을 `chain_bindings.rule_refusals(candidate, …, mapper_resolvable=ingestion_worker._resolvable_mapper, …)` :702 로 — `MAPPER_REGISTRY.get` 은 `builtin:` 종류를 몰라 `builtin:join_into` 가 «여기서 거절, 저기서 실행»이었다 ③ 저장될 «집합 전체»를 확장해 `ingestion_worker._validate_chain_cascade_graph(expanded_set)` :723 — ⚰️ **결과로 거절하지 «않는다»**(구 `chain_cycle` 거절은 «도는 선언»을 운영자의 «유일한 문»에서 막았다). 옛 평면 규칙은 `expand_declaration` 이 «그대로» 돌려주므로 그 경로는 바이트 그대로 |
| ⚰️ 🆕⑬ `server/audit_changeset.py` — **[2026-08-28] `server/_archive/audit_changeset.py` 로 이동** (자기 테스트도 `server/_archive/tests/test_audit_changeset.py` 로 같이 갔다) | ~~416~~ | 🅿️ **파킹 — 배선 안 됨, 운영 호출자 0개.** 자기 테스트 파일 말고는 import하는 곳이 **레포 전체에 없다**(`crud.py` 쓰기·`main.py` 읽기 둘 다 **리팩터 이전 인코딩 그대로**). 레인은 **2026-08-12에 의도적으로 중단**됐고 `crud.py`/`main.py`/`schemas.py` hunk는 되돌려졌다 — 이 파일이 추적 대상이 된 건 6일 뒤 `1aeda91`의 **경로 없는 `git add`** 때문이다. `server/tests/test_audit_changeset.py`는 **23건 전부 skip**. docstring이 「감사 행의 CHANGESET 모양 — WRITE 1건당 1행」이라 **착지한 것처럼** 적혀 있으나 **제안일 뿐**이니 라이브 감사 모양으로 읽지 말 것. 서술된 결함 5종은 **여전히 열려 있다**(감사 «이력»만 손실, 셀 값은 정상). 판정: `docs/process/PROJECT_STATUS.md:1560-1568` |
| `server/chain/key_gate.py` | **232** | 체인은 키 컬럼이 안 채워진 행을 낼 수 없다 — **관문 «하나», 일곱 개가 아니라** |
...
| `server/runtime/launcher_args.py` | **212** | `run_decoupled_app.py`의 **거절 절반**(명령행 파싱) |
| `server/parsers/void_sat_format.py` | **680** | SAT void 포맷 + **키를 못 만드는 행을 거절하는 관문**. docstring이 「왜 이 모듈은 추적되고 플러그인은 안 되는가」를 적는다 |
| `server/parsers/voids_json_format.py` | **424** | 외부 `WAFERID/WORK_DATETIME/voids.json` 피드 파서 — 🔴 **디렉터리 이름이 소스 데이터다** |
| `server/parsers/pipeline_base.py` | **109** | docstring 없음(pandas/numpy 임포트로 시작) — 파서 파이프라인 기반. ⚠️ **한 줄도 실측하지 못했다** |
| `server/utils/payload_helper.py` | **38** | docstring 없음 — JSON 페이로드 헬퍼. ⚠️ **〃** |


### 서버 — 마이그레이션·맵퍼 (이름만)

이 지도는 마이그레이션을 파일 단위로 등재하지 않지만, 아래는 **다른 절이 이름으로 참조할 수 있어** 존재만 남긴다. **내용은 열지 않았다.**

`migrations/drop_graph_storage.py`(+`_reverse.sql` — `2ec78b9`, 2026-08-14에 그래프 표를 DROP한 그 마이그레이션) · `add_ledger_source_events.py` · `add_ledger_refusal_reasons.py` · `add_ledger_entity_catalog_indexes.py` · `align_indexes_to_declarations.py` · `normalize_schema.py` · `migrate_jsonb_numeric.py` · 🆕⑩ `add_ledger_occurred_at_basis.py`(신설) · 🆕 `add_vjoin_null_safe_indexes.py`(**156줄** @`b1db471a` — 🔴 이 하나는 «열었다», S-245 의 호출자라서: `_duplicate_census(db, table, columns, folds)` :47 가 `vjc.index_key_expression(c, f, table)` :57 으로 인덱스 DDL 과 «같은 식»을 GROUP BY 한다 — 조사와 DDL 이 «다른 식»이면 조사가 「중복 없음」이라 하고 DDL 이 23505 를 낸다. `_rules(db)` :40 · `_existing(db, name)` :75 · `main(argv=None)` :81) — 그리고 맵퍼 **둘**: 🆕⑩ `mappers/ledger_v2_lot_event_role_mapper.py`(**v2 Role 맵퍼** — 준비된 `lot_event` 소스 이벤트 하나) · 🆕⑩ `mappers/ledger_v2_dt_job_mapper.py`(**신설**). 🪦 🆕⑩ **`mappers/ledger_lot_event_mapper.py`는 삭제됐다**(구 체인 계열 「순수 등록 맵퍼」 — 앵커로 쓰지 마라). ⚠️ 살아 있는 둘은 `.py.sample`이 아니라 **추적되는 `.py`**라 [§5-G](#5-g--dtcore-프레임-유도-체인-2026-08-11-신설-등재)의 「맵퍼는 `.py.sample`」 규율과 다른 계급이고, 🔴 **`ledger_v2_` 접두어가 계급 표지 자체다** — [`implementations.py`](#5-h-bis-온톨로지-config-작성탐색-표면-2026-08-21-신설-등재)가 그 접두어로만 import한다.

### 클라 — 미등재 모듈 13종 + 페이지 2종

⚰️ 🆕⑬ **[2026-08-29] 그 둘은 사라졌다.** `rnd-console.html`·`ledger-graph.html`·`ledger.html` 세 페이지와 `src/rnd_console/**`·`src/ledger_graph/**` 모듈 전부가 HEAD 에 없고(`3b77fbc3` 실측), vite 엔트리는 **7**이며 그중 신설은 `rnd_board`다 — [§7-B](#7-b-rd-보드-rnd_board-2026-08-29-신설-등재) 에 등재했다. 아래 문단은 그 둘이 살아 있던 때의 서술이다: ~~🔴 **vite 엔트리 둘(`rnd_console` · `ledger_graph`)이 이 지도에 «화면도 모듈도» 없었다~~.** [§7](#7-client2src--웹-클라이언트) 도입부의 엔트리 개수도 함께 정정했다(7 → **9**).
...
> 🔴 **값이 상수가 아니라 «파일»에 있는 것이 요점 전부다.** 체인 쪽의 같은 것들(`chain/ingestion_worker.py` 의 `OUTBOX_PURGE_MAX_CHUNKS`·`SWEEP_INTERVAL`)은 **상수**라, 새벽 두 시에 화면이 기어가는 것을 본 운영자가 «코드를 고치고 서버를 재기동»해야 한다 — **그 재기동이 바로 이 손잡이가 없애려는 것**이다.

**페이스는 «두 수»이고 그 이상이 아니다**: 얼마나 일하고 나서 양보하는가(`units_per_cycle`), 얼마나 쉬는가(`rest_seconds`). 🔴 **「단위」가 무엇인지는 «호출자»의 것이다** — 원장은 «페이지», 인제션은 «청크». 그 경계가 각자의 일이 «이미 커밋된» 자리이고 재개가 정확한 자리라서다. **표는 그걸 알 필요가 없다.**

| 심볼 (`server/pacing.py`) | 무엇인가 |
|---|---|
| `PACING_PATH` | `Path(__file__).with_name("pacing.json")` |
| `DEFAULT_PACE = "fast"` | 🔴 **「이 손잡이가 생기기 «전»과 정확히 같다」는 뜻**(상한 없음·쉼 0). **어느 것이 기본인가는 다른 결정이고 소유자 소관**이라고 소스가 적어 둔다 |
| `class UnknownPace(ValueError)` | 🔴 **폴백이 «여기서는» 위험한 답이다.** 서비스가 이미 힘들어서 `slow` 를 고른 사람인데 오타가 조용히 `fast` 를 뜻하면, 막으려던 바로 그것을 보면서 **「손잡이가 안 듣는다」와 「손잡이가 도움이 안 됐다」를 구별할 수 없고** 다시 눌러도 아무 일이 없다 |
| `load_paces(path=None)` | JSON 의 `["paces"]` 를 그대로 |
| **`resolve(name, paces=None) -> (units_per_cycle, rest_seconds)`** | `name=None` 이면 `DEFAULT_PACE`. **미선언 이름은 `UnknownPace`** 이고 메시지가 선언된 이름들과 파일 경로를 댄다. `units_per_cycle` 이 `None` 이면 「양보하지 않는다」. ⚠️ **`units_per_cycle` 이 없으면 `pages_per_cycle` 을 본다** — 원장 시절 철자의 호환 자리 |
...
| **`server/run_auto_update.py`** (🆕⑨ **840줄** @`5359fdd` — 구 표기 882, **줄었다**) | 스케줄 기반 사용자 스크립트 자동 실행.<br>🆕 🔴 **[신설] 소급 실행의 실행 절반이 여기다** — `MultiDiscoveryScheduler.__init__`의 필드 2종 `_retroactive_thread`/`_retroactive_last`(🆕⑯ **407–409** @`9eb30691` — 구 표기 ~412/413) + **`retroactive_busy()`(🆕⑯ **668** — 구 표기 ~707)** / **`start_retroactive_run(payload)`(🆕⑯ **672** — 구 표기 ~711)**. 🔴 **데몬 스레드로 도는 것이 계약이다**: 인라인 실행은 60초 하트비트를 멈추고 그러면 `/health`가 이 데몬을 **WEDGED로 보고한다**. outbox 폴 블록이 가장 오래된 미처리 `RETROACTIVE_RUN` 행 하나를 집어 **스레드가 끝나기 전에** `processed_chain=True`로 표시하고(at-most-once), 이미 실행 중이면 행을 **큐에 남긴다**.<br>🆕 **[`4aae627`] `_apply_proxy_policy()`(**~25**) — 모듈 스코프에서 즉시 1회 실행된다(**~80**).** 수집 스크립트는 사용자가 쓰고 `exec`로 도는 코드라 각자 프록시를 다룰 수 없고, 그것들이 치는 곳은 **사내 인트라넷**이라 프록시를 **우회**해야 한다(2026-07-30 실측: 경유하면 403). 🔴 **개별 변수를 지우는 것으로는 반대 결과가 난다** — `urllib.request.getproxies()`는 `getproxies_environment() or getproxies_registry()`이고, 환경 dict가 **비면** 뒤의 **레지스트리**로 넘어가 운영 머신의 사내 프록시가 되살아난다. 그래서 `*_proxy` 계열을 전부 걷어낸 뒤 **`no_proxy="*"` 하나를 남겨 dict를 비지 않게** 만든다(`requests`도 같은 함수를 타므로 두 라이브러리에 동시에 듣는다). 끄는 스위치는 `auto_update_control.json`의 **`bypass_proxy`**(기본 **true = 직결**, boolean 아니면 경고 후 기본값). 제거한 변수는 **이름만** 로그한다(프록시 URL에 자격증명이 실릴 수 있다). ⚠️ **전역 `NO_PROXY` 환경변수와 혼동하지 말 것** — 그것을 프로세스 밖에 세우는 것은 [§1.7](#17-serverinternal_event_clientpy--내부-http-호출의-단일-소유자-23a346d-신설)이 **명시적으로 금지**하는 조치다.<br>🔴 **이 삽입이 파일 머리에 있어서 83행 이후 앵커가 전부 균일 +59다** — 아래 값은 전부 `c520012` 재측정이다.<br>🪦 🆕⑯ **[2026-08-31 실측] `MultiDiscoveryScheduler.maybe_sweep_graph_orphans` 는 «없는 이름»이다** — `server/` 전건 grep 히트가 `tests/test_graph_branch_retired.py` **한 건**(은퇴를 «채점하는» 시험)뿐이었다. 🔴 🆕⑱ **[2026-09-05 재측정] 이제 «0건»이다 — 그 시험 파일도 `8ffe23d7` 에서 삭제됐다**([§0](#0-묘비-목록--소스에-존재하지-않는-이름) 게이트 ㉕-c 의 기대값이 1 → 0 으로 바뀐 것이 이것이다). 🔴 **그리고 이 절이 그 함수에 달아 둔 앵커 `~668` 은 지금 `retroactive_busy` 를 가리킨다** — 실재하는 다른 함수라 도착지가 멀쩡해 보이는 그 함정 그대로다. **이 이름과 아래 서술을 앵커로 쓰지 마라**(그 모듈 `server/graph_orphans.py` 도 2026-08-16 에 삭제됐다). 아래는 그것이 살아 있던 때의 서술이다: ~~**[`530fdfd` 신설] `maybe_sweep_graph_orphans(self, now=None)`(~668)**~~ — ⚠️ **모듈 함수가 아니라 메서드다.** `run()`의 5초 틱에서 `maybe_backup_configs()`(3단계) 바로 뒤 **4단계**로 호출(**~792**). 상태는 `__init__`의 `_last_orphan_check`/`_last_orphan_sweep = 0.0`("첫 틱에 실행" 규약 — 장애 후 올라온 프로세스가 놓친 유지보수를 부팅 때 한다, `_last_backup_check`와 같은 관례). **JSON config 키가 없다** — 주기는 전부 `graph_orphans`에서 온다(스로틀 `CHECK_INTERVAL_SEC` 1800초, 실제 주기는 `due()`가 `SWEEP_INTERVAL_SEC` 86400초로 판정). 끄는 스위치는 **환경변수 `GRAPH_ORPHAN_SWEEP_ENABLED=false`**. 가드 순서: 스로틀 미달이면 `None` → `_last_orphan_check` 스탬프 → `due()` 아니면 `None` → **`_last_orphan_sweep`을 실행 전에 스탬프**(raise하는 스윕이 재시도 루프를 못 돌게) → `try/except`로 감싸 실패는 로그만(**스윕 실패가 수집기를 절대 멈추지 않는다**). ⚠️ 사소한 두 가지: `now` 인자를 받지만 **쓰지 않는다**(`maybe_backup_configs`는 전달한다), 그리고 `_last_orphan_check`는 wall-clock(`time.time()`)인데 `_last_orphan_sweep`은 monotonic이다(`due()`의 계약상 의도적이나 한 메서드에 두 시계가 섞여 있다). docstring이 "수집기가 아니라 유지보수 작업"임을 근거로 든다 — 테이블이 없고 인제션돼선 안 되며, 지금 아무것도 생산하지 않는 수집기는 설계상 FAIL을 보고한다. 매 틱 제어 파일(`auto_update_control.json`)을 읽어 disabled 수집기는 실행 스킵+`last_status="SKIPPED"`+next_run 전진(핫 반영, 재활성화 시 백로그 폭주 없음). run-now는 active 무관 실행. 함수 앵커: `class GenericScriptRunnerCollector`(**~138**)·**`execute()`(**~165**)**·`parse_script_comments`(**~362**)·`class MultiDiscoveryScheduler`(**~391**)·`discover_and_load_collectors`(**~411**)·`_load_collector_from_script`(**~455**)·`_collector_key`(**~516**)·`_write_status_file`(**~521**)·`run_collector_on_demand`(**~553**)·`execute_collector`(**~567**)·`check_and_run_schedules`(**~594**)·**`maybe_backup_configs()`(**~637** — [`b35bc9f` C3] 틱마다 30분 게이트(`config_backup.CHECK_INTERVAL_SEC`)로 `config_backup.run_scheduled()` 호출. cron식이 아니라 `due()`가 디스크의 최신 스냅샷 나이로 판정 — 놓친 주가 다음 틱에 자가 치유. 백업 실패는 절대 수집기 실행을 멈추지 않는다)**·**`run()`(**~703**, 루프 안 `heartbeat.beat("scheduler")` **~734**)**.<br>🔴 **[2026-07-30 기록] 직전 패스가 이 파일의 앵커 셋을 막 고쳐 놓았는데 다음 라운드가 그것을 다시 밀었다** — `maybe_backup_configs` 578→**637** · `run()` 644→**703** · beat 675→**734**. 직전 밀림(39줄)은 `530fdfd`의 `maybe_sweep_graph_orphans`가, 이번 밀림(59줄)은 `4aae627`의 `_apply_proxy_policy`가 만들었다. **두 번 다 "파일에 함수 하나가 들어왔다"이고 두 번 다 그 옆의 앵커는 아무도 안 쟀다.**<br>**[`512dca7` 재작성 — `execute()`의 실패 계약] "확인할 수 없었다"를 "이상 없다"로 보고하지 않는다.** 판정은 세 로컬(`out_data`·**`out_declared`**(**~208**)·**`exec_error`**(**~209**))의 조합이고 **함수 밖에 헬퍼가 없다** — 4상태: ① `out` 미정의 + 무예외 → stdout 수집기이며 폴백이 **정상 경로**(INFO) ② `out = None` **대입** → 스크립트가 줄 게 없다고 선언한 것이므로 **즉시 FAIL, stdout 재실행 없음**(**~260**, 이런 스크립트는 네트워크 페치의 에러 핸들러라 재실행하면 외부 호출만 반복한다) ③ `out`은 있는데 비었음 → 이번 주기에 수집할 게 없다, SUCCESS ④ 실행이 **raise** → ERROR + 트레이스백, 폴백은 시도하되 **그것도 비면 raise해서 FAIL**(**~344**). ①과 ④가 종전엔 같은 WARNING 한 줄로 뭉개져 **크래시해서 0행을 모은 실행이 깨끗한 성공으로 보고됐다**. ②가 ①과 구분 불가였던 것은 `.get("out")`이 "None 대입"과 "미정의"를 구분하지 못하기 때문이다.<br>**`SystemExit` 처리(**~237**)**: `sys.exit(0)`으로 끝나는 수집기는 정상 완료이므로 `out`을 존중한다. `SystemExit`는 `BaseException`이라 잡지 않으면 `execute_collector`·`check_and_run_schedules`(둘 다 `Exception`만 잡는다)를 **관통해 스케줄러 데몬을 종료시킨다**. 0/None이 아닌 코드는 `exec_error`로 강등 후 폴백 시도.<br>**`exec(code_content, script_ns)` — globals/locals에 같은 dict 하나(**~231**)**: 서로 다른 두 dict를 넘기면 클래스 바디 스코핑이 돼 모듈 레벨 `def`/`import`가 locals에만 바인딩되고 **함수 본문은 `LOAD_GLOBAL`이라 그것을 못 본다** → 다른 함수에서 호출된 헬퍼가 `NameError`로 죽고 수집기는 조용히 아무것도 못 모은다(모듈 레벨 호출은 `LOAD_NAME`이라 locals를 보므로 **일부 수집기만 고장 나 보였다**). 테스트: `tests/test_auto_update_script_exec.py`(21건 — `TestSingleNamespace`·`TestStdoutFallback`·`TestFailuresAreLoud`·`TestOutAssignedNone`·`TestOutFormattingFailure`·`TestSystemExitContainment`) |
| 🆕㉓ **`server/maps/alignment_batch_counts.py`** (**191줄**, `f6371646` 기준) | **체인 그룹 한 번의 «초»를 층별로 모으는 자리 — 세 dict.** `phase(name)`(뷰 빌드 «안») · `stage(name)`(그룹 «전체»의 기계류) · 🆕 `write_step(name)`(`write:*` «안», 셋째 dict라 «두 번 세지 않는다»). 정렬을 한 번도 안 만진 그룹은 줄을 «안 찍는다**(0 으로 채운 줄이 진짜를 묻는다) | (S-94·S-151, 판정 241·261) |
| `server/event_constants.py` (🆕㉒ **400줄** @`df483dcd` — 🆕⑳ 302 @`328a5c20`, 🆕⑱ 286 @`6345ef3d`, 🆕 223 @`34d2518`, 종전 86) | 🔴 🆕㉒ **[2026-09-06 신설 — 체인이 «자기 홉을 센다»] `CHAIN_DEPTH_KEY = "chain_depth"` · `DEFAULT_MAX_CHAIN_DEPTH = 8` · `chain_depth_of(payload)` · `max_chain_depth(rules)`.** 종전엔 「체인이 체인을 못 깨운다」와 「옵트인하면 무제한」 사이에 «아무것도 없었다» — `source_name == "chain_ingestion"` 은 **누가 썼나**에만 답하고 **몇 홉이 만들었나**에는 답할 수 없어서, `allow_chain_trigger` 는 규칙을 들여보낸 뒤 더 할 말이 없었다. 유일한 제동은 로더의 **정적** 고리 검사였고 그것은 «길기만 한» 폭포를 못 본다.<br>🔴 **상태가 «둘이 아니라 셋»이다** — 키가 «없으면» 체인이 쓴 것이 아니고(그런 이벤트는 깊이로 «거절되면 안 된다»), 키가 있으면 체인이 쓴 것이다 — **값이 0 이어도** 그렇다. `chain_depth_of` 는 그래서 «부재»에 `None` 을 주고, 정수가 아닌 것에도 `None` 을 준다(⚠️ `True` 는 파이썬에서 `int` 라 그냥 통과시키면 «깊이 1» 로 읽혀 폭포를 한 홉 일찍 끊는다). `max_chain_depth` 는 선언(`chain_rules.max_chain_depth`)을 읽고, 양의 정수가 아니면 기본값으로 «떨어진다» — 이 키가 생기기 전에 쓰인 config 가 그 자리다.<br>🔴 **찍는 자리가 «하나»다** — 맵퍼 열이 자기가 돌려주는 «행»에 `source_name` 을 철자하므로 깊이를 그 옆에 찍으면 맞춰야 할 자리가 열이 된다. outbox 봉투는 `database._outbox_envelope` «한 곳»에서 지어지므로, 워커가 컨텍스트 변수 `database.context.request_chain_depth` 를 **한 번** 세우고(`incoming_depth + 1`, 끝나면 `reset`) 스테이저 둘이 그것을 읽는다. ⚠️ **거절은 `_rule_accepts_event` «안»이 아니다** — 그 술어는 이벤트당 여러 번 불리므로 거기서 거절하면 같은 말을 여러 번 하거나 «아무 말도 안 하게» 된다. 워커 루프에서 «한 번» 거절하고 `[Chain Depth]` 로 자기 이름을 대며, 그 행은 «끝난 것으로» 표시한다(안 그러면 매 틱 다시 읽혀 뒤를 영원히 막는다 — 독 든 행 결함을 새 기능이 다시 들여오는 모양). ⛔ **정적 고리 검사는 «안 풀었다»** — 깊이는 «긴» 폭포를, 그 검사는 «도는» 폭포를 막는다. 깊이가 무는 것을 보이기 전에 두 번째를 풀면 진짜 무한 루프가 열린다. 시험: **`server/tests/test_the_chain_counts_its_hops.py`**<br>🔴 🆕⑳ **[신설 — `CONTROL_EVENT_TYPES` 바로 «아래»] 「그 이름은 표가 아니다」 절**: **`RETROACTIVE_RUN_TABLE = "__retroactive__"`** · **`PLACEHOLDER_TABLE_NAMES = frozenset({RETROACTIVE_RUN_TABLE})`**. 🔴 **이 «선언»이 곧 그 문장이다** — `database_outbox.table_name` 은 관례상 비지 않아서 표가 없는 행이 이름을 지어내고, 그 컬럼을 운영자에게 보이는 읽는 쪽은 이 집합을 «먼저» 묻는다(아무도 안 만든 이름은 «부재»를 이름으로 말한 것이라 운영자의 헛걸음이 된다). ⛔ **「통제 이벤트인가」가 아니다** — 그 부류는 한 명 넓어서 되물려졌다(`SCHEDULER_RUN_NOW` 는 호출자가 준 진짜 표를 싣는다). 소비자 둘: `main.get_chain_queue_depth`(거르는 쪽) · `retroactive.RUN_EVENT_TABLE`(쓰는 쪽 — 🆕⑳ 리터럴이 이 상수 참조로 바뀌었다).<br>🔴 🆕⑱ **[2026-09-05 신설 — 파일 «말미»] 미배달 마커의 철자가 여기로 «모였다»**(`443c1181`): **`UNDELIVERED_MARKER_STATUS = "SUCCESS"`** · **`UNDELIVERED_MARKER_PROCESSED_CHAIN = True`** · **`UNDELIVERED_MARKER_TAG = "undelivered_notification"`**(`payload["marker"]`, 귀속용). 종전엔 `internal_event_client.record_undelivered_notification`(쓰는 쪽)과 `chain_ingestion_worker.sweep_undelivered_broadcasts`(걷는 쪽)가 **같은 세 값을 각자 손으로 철자**했고, 한쪽만 바뀌면 **오류도 예외도 없이 마커가 영원히 안 걷혔다.** ⚠️ **`broadcast_at` 은 «일부러» 여기 없다** — NULL 은 쓰는 쪽이 세우는 값이 아니라 걷는 쪽이 찾는 «부재»이고, 이름을 주면 누군가 그걸 쓰게 된다. 상세는 [§1.7](#17-serverinternal_event_clientpy--내부-http-호출의-단일-소유자-23a346d-신설).<br>🔴 🆕⑱ **[신설] 「대기 중인 행을 «누가» 비우나」 절** — **`SCHEDULER_OWNED_EVENT_TYPES`**(= `{SCHEDULER_RUN_NOW, RETROACTIVE_RUN}`) · **`CHAIN_OWNED_EVENT_TYPES`**(= `{"CREATE","EDIT","DELETE"}`) · **`OUTBOX_OWNER_SCHEDULER`/`_CHAIN`/`_UNKNOWN`** · **`outbox_owner(event_type)`**. 🔴 **`processed_chain = false` 는 「체인 워커가 밀렸다」를 «뜻하지 않는다»** — 이 표를 비우는 데몬이 «둘»이고, 둘의 행을 더해 보고하는 계기는 스케줄러의 행을 두고 체인이 밀렸다고 말한다(소스가 2026-09-04 사례를 적는다). **판정이 여기 사는 이유는 «집이 하나여야» 해서다.**<br><br>🆕 🔴 **[`528dfcb`] 이 모듈에 outbox *접기(collapse)* 절이 통째로 생겼다 — outbox 이벤트가 스냅샷에서 *포인터*로 바뀐 자리다.** 종전엔 인제션이 데이터 행마다 outbox 행 하나에 컬럼 전량을 실었다(실측 2,113.5 B/행 → 1천만 행 파일에 19.7 GiB). 이제 이벤트는 **트리거 row_id들과 테이블 이름**을 나르고 파생 쪽이 행을 **다시 읽는다**(20,000행 → 20 이벤트, 36.0 B/행).<br>**선언 — 전건 열거**: `OUTBOX_MODE_PER_ROW`(**기본값** — 옵트인 안 한 호출자는 종전 그대로. ⚰️ **[S-82, 2026-09-09] 「사람/교정 경로는 여기 머물러야 한다」는 되물렸다** — 실측: 교정 고리가 이 표를 «안 지난다». 제품 문의 화면은 `crud.apply_batch_updates` 의 «반환값»으로 짓고, 복구 스윕(`sweep_undelivered_broadcasts`)은 `table_name` 으로 표 단위 새로고침만 쏜다. payload 의 «컬럼»을 읽는 소비자는 체인 워커 «하나»이고 그것은 «펴서» 읽는다) · `OUTBOX_MODE_COLLAPSED`(**부피를 나르는 쓰기 «셋»이 명시 옵트인** — 인제션(`directory_watcher`) · 체인 워커 · 🆕 **제품 문 `main.apply_batch_updates_endpoint`**(S-82). ⚠️ **`request_source`로 추론하지 않고**(그건 인제션 경로에서 *파일명*이지 채널이 아니다) **행 수로도 추론하지 않는다** — 추론은 «넷째» 호출자가 아무도 정하지 않은 채 접히는 길이다) · `OUTBOX_COLLAPSE_CHUNK_ROWS = 1000`(이벤트 하나가 나르는 row_id 상한) · `OUTBOX_GROUP_MAX_ROWS = 20000`(🔴 **새 손잡이가 아니라 구 `LIMIT 20000`이 뜻을 지킨 것** — 접힌 뒤에도 이벤트로 세면 한 배치가 2천만 행을 끌어와 **1,000배 증폭**된다. 그래서 예산은 **행**으로 매긴다) · `OUTBOX_PAYLOAD_EXCLUDED_COLUMNS`(frozenset — 생산자가 안 쓰는 컬럼과 **정확히** 같아야 확장기가 같은 페이로드를 다시 짓는다).<br>**함수**: `is_collapsed_payload(payload)`(🔴 **`event_type`이 아니라 판별 키(`row_ids`) 멤버십으로 판정한다** — 접기가 `event_type`을 CREATE/EDIT로 **일부러 유지**하므로 「테이블 T의 데이터 변경인가」만 묻는 소비자는 전부 무접촉이고, `payload['data']`를 실제로 **읽는** 소비자만 여기서 분기한다) · `payload_row_count(payload)`(per-row는 1) · `trim_events_to_row_budget(events, budget, payload_of=None)`(🔴 **필터가 아니라 id 순 *접두사*다** — 꼬리는 `processed_chain=False`로 남아 다음 반복이 **같은 순서로** 집는다. **항상 최소 하나는 돌려준다**, 예산보다 큰 청크 하나가 배수를 영구히 막지 않도록).<br>⚠️ **DELETE는 접히지 않는다** — 지워진 행은 다시 읽을 수 없다(`database.auto_stage_database_outbox`).<br>기존 절 — 프로세스 간 내부 이벤트(`/internal/events/*`) 공용 상수.<br>🆕 **[신설] outbox 제어 이벤트 절**: `EVENT_SCHEDULER_RUN_NOW`(**21**) · **`EVENT_RETROACTIVE_RUN`(25)** · **`CONTROL_EVENT_TYPES` frozenset(🔴 **40** @`34d2518` — 구 `~28`은 낡았다)**. 🔴 **체인 워커가 제어 행을 리터럴 `"SCHEDULER_RUN_NOW"` 비교가 아니라 이 집합의 멤버십으로 거른다** — 제어 이벤트가 둘이 되는 순간 리터럴 비교는 새 종류를 **데이터 이벤트로 오인해 체인에 먹인다**.<br>`MAX_NOTIFY_CREATED_LOGS=500`(🔴 **49** @`34d2518`, 구 `~36`) · [P2] `MAX_AUDIT_VALUE_CHARS=4096`(🔴 **69**, 구 `~44`)과 `truncate_audit_value(value, max_chars)`(🔴 **197**, 구 `~47` — 반환 `(값, 절단여부)`, str은 `…[truncated: 총 N자]` 마커, dict/list는 타입·길이 플레이스홀더)를 `crud.create_audit_log`가 소비.<br>🔴 **소비처 앵커는 매 패스 다시 잰다 — 한 패스에서 4개 중 3개가 틀린 적이 있다.** 🆕 **실측(`34d2518`, 전건 grep — 이번엔 넷 중 셋이 또 밀렸고 하나는 *늘었다*)** — 🔴 **`directory_watcher:1917`**(구 1813, **+104**) · 🔴 **`chain_ingestion_worker:594`**(구 540) · 🔴 **`main.py`는 소비처가 둘이 아니라 셋이다**: **2714–2715 · 5790 · 5838**(구 `5165/5213`은 낡았고 개수도 틀렸다) · ✅ **`server/database/crud.py` `create_audit_log` 안 1117–1118 — 이번엔 밀리지 않았다**(구 지도의 `613–614`가 낡았던 그 자리). `CONTROL_EVENT_TYPES` 멤버십 소비는 🔴 **`chain_ingestion_worker:1122`**(구 1005). ⚠️ **`admin/retroactive.py:124` · `run_auto_update.py:836`는 이번 패스에서 재측정하지 않았다 — 낡았다고 가정하라.** **"재측정했다"는 표지가 붙은 수를 다시 재는 것이 이 행의 존재 이유다** — 표지는 측정이 아니다 |
| 🆕㉔ **`_split_collapsed_event(...)`** + `MAX_REEXPANSION_DEPTH = 12` | **격리 재전개가 «이분»이다**(S-173) — 실패한 청크를 반으로 가르고 «각 반쪽이 자기 트랜잭션 그룹»이다. 독 든 행 하나가 사건 «21 개»(종전 1,000). 자식이 `reexpanded_from.depth` 와 «자기 경로 접두»(`a`·`ab`·`abb`)를 들어, 한 번의 접두 검색으로 계보가 나오고 깊이가 «자식의 페이로드 안»에서 읽힌다 | |
| 🆕 **`server/outbox_expand.py`** (🆕㉓ **393줄** @`a223dcac` — 구 표기 302) 🆕㉓ **`_report_unreadable(db, model, table_name, unique_ids, chunk_size)`** — 이름 댄 행 집합이 «하나도» 안 읽히는 «그 순간»에 DB 가 «무엇이라 답했는지»를 한 줄로 남긴다(S-158, 판정 266). 🔴 설명 «셋»이 측정으로 죽은 뒤에 생긴 계기다(세션 공유 아님 · read committed · 같은 순간 행이 보임). ⚠️ 문장은 «제품 자신의 질의»에서 렌더링한다 — 손으로 베낀 닮은꼴은 «그 닮은꼴»에 대해 답한다 | **접힌 outbox 이벤트를 다시 행으로 펴는 유일한 자리 — 소비자 쪽 절반.** 🔴 **세 번째 페이로드 모양을 만들지 않는다**: `chain_replay`가 이미 하던 재구성을 **재사용**하고, 그래서 라이브 트리거 경로도 「페이로드를 먹는 유일한 소비자」이기를 그만둔다. 함수 — `_data_columns(model)` · `_synthesize_payload(row, columns, envelope)` · `load_rows_by_ids(db, table_name, row_ids, chunk_size=OUTBOX_COLLAPSE_CHUNK_ROWS)` · `event_key(event)` · **`expand_events(db, events) -> dict`**(체인 워커의 소비 지점) · **`reexpand_collapsed_event(db, event, payload, error_reason=None) -> int`**. 🔴 **실패한 접힌 청크는 per-row로 다시 펴지되 *per-row transaction id* 아래로 간다** — 워커의 실패 단위가 이벤트가 아니라 **그룹**이라, 원래 id로 다시 펴면 그 행들이 **다시 함께 실패하도록** 묶인다 |
...
| 🆕 **`server/scripts/count_absent_null_layers.py`** (**135줄**, `478fcf32` 신설 2026-09-15, S-243-b) | **읽기 전용 «세기»** — 판정 405 «이전»에 파일 소스의 빈 칸이 세운 NULL 층(`cell_sources`)이 몇이고 그중 «값을 가리는» 것이 몇인지. 🔴 **NULL 층 = JSON 리터럴 `null`, SQL NULL 이 아니다** — `cell_sources.value` 는 JSON 컬럼이라 진짜 문으로 들어온 빈 칸이 네 글자 `null` 로 앉는다(소스 주석: 2026-09-15 `apply_batch_updates` 실측, `user`·`chain_ingestion`·파일 셋 다). 지시서의 `value IS NULL` 은 그 행들 위에서 «영원히 0»을 냈을 것이고 그 거짓 0 은 「치울 것이 없다」로 읽힌다 — 필터는 `s.value IS NULL OR CAST(s.value AS TEXT) = 'null'` 둘 다. **두 수를 따로**: `layers`(NULL 층) · `hiding`(같은 (표,행,컬럼)에 값 있는 «다른 층»이 하나라도 있는 것 — 서열은 안 따진다: 이 수는 「지울 후보」이지 「지금 보이는 것」이 아니다). 🔴 **술어는 import** — `crud.can_mean_emptied(source_name)` :86([§2](#2-serverdatabasecrudpy--레이어링-코어))로 사람·체인의 층을 «뺀다», 다시 철자하지 않는다. `MAX_ROWS_SHOWN = 40` :35(«나열»만 접고 «합계»는 안 자른다) · `count(db) -> list[(table, column, source, layers, hiding)]` :38(가림 내림차순) · `render(found) -> str` :94(「없음」 / 가림 0 / 가림 N — 세 문장, 각각 「→ 다음:」을 든다) · `main(argv=None)` :120. ⛔ **`--apply` 는 «없다» — 정책이 아니라 «부재»로**(있는 플래그는 쳐진다; 지우기는 S-243-c, 내보내기 먼저). 🔴 「없음」은 «조인이 보인다»는 뜻이 아니다 — 잔해 «한 종류»만 센다. import 소비자 «0»이 정상(명령줄 도구 — 「소비자 0」 넷째 예외). 명령은 `RUN.md` §1-bis. 시험 `test_the_stored_null_layers_are_counted_before_anyone_erases.py`(164줄 / `def test_` 10 — 술어를 «소스 텍스트»가 아니라 `count.__code__.co_names` 로 묻는다) |
...
| 🆕㉔ **`server/scripts/tune_layer_tables.py`** · **`server/scripts/outbox_triage.py`** | **운영자 «명령» 둘 — 소유자가 SQL 을 못 내기 때문에 생겼다**(판정 276). 전자는 층 표의 배큠 상태를 «읽고 아무것도 정하지 않는다»(dry-run 기본). 후자는 터진 재전개 홍수를 «건너뛰고 접힌 채로» 다시 쏜다(`--count` 로 먼저 센다) |
...
제자리(`chain/ingestion_worker.py` · `chain/replay.py` · `scripts/chain_replay_cli.py`)에 그대로 있다.
...
> 🔴 🆕⑳ **[2026-09-05 전건 재계수 · `328a5c20`] 「6계약」은 낡았다 — 그리고 «수 하나»로는 이 디렉터리를 셀 수 없다.** `git ls-tree -r --name-only HEAD -- contracts` 전건:
> ```
> 계약 «디렉터리»          9    band_arithmetic · blank_predicate · config_resolve_report · doe_band_rules
>                            legend_map_scope · map2_seam · map_seam · notation_fold · 🆕⑳ temporal_text
> `client_harness.mjs`     7    위에서 notation_fold · temporal_text 를 «뺀» 일곱
>                            -> `check_contracts.mjs` 가 발견하는 수가 이것이다 (아래 「발견되는 것은 6개」는 낡았다)
> `test_*.py` (파이썬 절반) 6    blank_predicate · config_resolve_report · map2_seam · map_seam
>                            notation_fold · 🆕⑳ temporal_text
> ```
...
> client_harness.mjs         11   (구 표기 7)   walk_node_shape 하나 + 🆕 C-69 의 얇은 러너 «셋»
...
> ```
>
> ✅ **[C-69] `legend_map_scope` 와 `map_seam` 이 이고 있던 `import vm from 'node:vm'` 둘을 «지웠다»** — 프로브로 전환하고 남은 죽은 import 였고, 두 파일 다 `vm.` 을 한 번도 부르지 않는다(전건 grep). 그래서 위 표의 「vm 으로 잘라 돌린다 2」는 이제 «import 까지» 둘이다.
...
>
#### 🆕⑳ `client2/tests/lib/appended_module.mjs` (**71줄**, `545e0e42` 신설) — **잘라쓰기를 대신하는 «다리»**

> 🔴 **소유자 상설(2026-09-02)이 「잘라쓰기 하니스 절대 금지」다** — 대상 파일을 텍스트로 읽어 함수 본문을 잘라 `node:vm` 에 넣는 방식. 사유는 그것이 «동작»이 아니라 «글자 모양»을 재기 때문이고(import 를 하나 더하면 조각이 던지고, `const` 는 vm 컨텍스트의 속성이 안 되고, 헬퍼가 늘면 「뽑을 목록」에 없어 던진다), **반대도 참이다 — 틀렸는데 초록일 수 있다.**

| 심볼 | 무엇인가 |
|---|---|
| **`importWithAccessors(sourceUrl, accessors) -> Promise<namespace>`** | 원본 «전문» + 뒤에 붙인 접근자 소스로 **사본 파일**을 만들어 `import` 하고, `finally` 에서 지운다. 🔴 **사본은 원본 «옆»에 놓인다**(`.appended-<name>-<pid>-<seq>.mjs`) — 상대 import(`./config.js` …)가 그대로 풀려야 하고, 다른 곳에 두면 그것들이 죽어 **우리가 고치려던 병을 다시 만든다** |
| 🔴 **퇴화 방지 단언 — 헬퍼 «안»에 있다** | `body.startsWith(original)` 이 아니면 던진다. 잘라내는 양이 «0» 이라 금지의 사유가 하나도 안 남는 것이 이 다리의 근거이므로, 그 전제가 깨지면 조용히 잘라쓰기로 돌아간다. ⚠️ **밖에 두면 24군데가 각자 만들고 병이 이름만 바뀐다** |
| `startsWithOriginal(original, appended)` | 그 규칙 «자체»를 하니스가 재 볼 수 있게 노출한 순수 함수 |

> 🔴 **왜 그냥 `import` 하면 안 되나 — 이 문단의 «수»가 낡았다 (`106ed25b`, 2026-09-07):** `import('../src/map_editor.js')` 는 성공하고(옛 사유 `import './tokens.css'` 는 사라졌다), **`exported names` 는 이제 0 이 아니라 «91»** 이다 — 판정 94 의 Ⓐ 가 하니스 열이 잘라 쓰던 이름 91 개 앞에 `export` 를 붙였다(순수 접두, 번들 바이트 «동일»). 그러므로 「밖에서 잡을 이름이 없다」는 **오늘 거짓이다.**
> ⚠️ **그래도 접근자가 남는 이유는 «다른 것»이다** — ESM 네임스페이스는 봉인돼 있고 `export let` 은 밖에서 «읽기 전용»이라, 하니스가 모듈 «상태»를 심어야 하는 자리(재대입되는 `let`, 갈아 끼울 협력자)는 여전히 붙이기가 유일한 길이다. 즉 `export` 는 «읽는» 이름을 열었고 «쓰는» 이름은 안 열었다.
> ⚠️ **도착지가 아니다.** 도착지는 「재려는 로직을 import 되는 모듈로 뺀다」이고 이건 그 대공사를 오늘 안 하기 위한 다리다 — **다리를 영구 건물로 읽지 마라**(소스가 그 문장을 직접 적는다).
> 🧪 채점자 `client2/tests/appended_module_harness.mjs`, 게이트 바닥값 **17**(`check_harnesses.mjs` 의 `FLOORS`). 같은 커밋이 `failure_copy_harness.mjs` 의 바닥값을 **5 → 6** 으로 올렸다.
...
| 파일 | 책임 |
|---|---|
| 🆕⑳ **`contracts/temporal_text/vectors.json`** (**103줄**, `"version": 1`, `ebde9b9d` 신설) | **[`crud._TemporalText` 두 팔의 정본 명세 — 파이썬 렌더와 SQL 렌더가 «기록된 한 답»에 채점된다.]** 최상위 키: `$comment` · `version` · **`seam`**(3) · **`format`**(2) · **`compiled_sql`**(3) · **`cases`(9)** · **`passthrough`(3)**. 🔴 **엔진 대 엔진이 «아니다»** — 둘이 «같은 방향으로» 틀리면 서로 대조는 초록이고, 이 둘은 상수에서 조립되므로 같이 미끄러지는 것이 **쉬운 쪽**이다. 그래서 양쪽이 전부 이 파일의 `expected` 에 채점된다 |
| 🆕⑳ **`contracts/temporal_text/test_temporal_text_contract.py`** (**284줄, 17건**, `grep -c "^def test_"` = 17) | **축이 «셋»이고 그중 하나는 시끄럽게 스킵된다.** ① 파이썬 렌더 ② 각 팔이 **컴파일되는 SQL 텍스트**(리터럴 바인드라 포맷 문자열이 자리표시자가 아니라 «보인다») ③ 🔴 **PostgreSQL 이 그 철자를 «같은 바이트로 계산하나»** — 이것만 `ASSY_CONTRACT_PG_URL` 이 있어야 돌고, 없으면 스킵이다. 🔴 **그 스킵이 이 계약에서 제일 중요한 부분이다** — 기본 스위트 dialect(SQLite)에서 `_default` 팔은 **평범한 CAST** 라 결함이 있던 적이 없는 쪽이고, 즉 «하중을 지는 절반»이 바로 그 스킵이다. 그래서 `-rs` 가 명령의 일부다.<br>🔴 **왜 생겼나 — `notation_fold` 의 «쌍둥이»인데 계약이 없었다.** 실측 2026-09-05: `contracts/` 어디에도 `temporal_text` 가 없었고, 이름을 대는 유일한 시험(`test_virtual_join_types.py`)은 `db_session`(SQLite)에서 돌아 **postgres 팔을 채점하는 것이 0** 이었다. 그 팔은 장식이 아니다 — PostgreSQL 의 `CAST(timestamptz AS varchar)` 는 **세션 타임존**으로 렌더하고 소수부가 0이면 생략하므로, 같은 행을 든 서버 둘이 다른 텍스트를 비교하고 해석된 시각 컬럼의 `lessThan` 이 «연대순이기를 그만둔다».<br>⛔ **판정하지 않는다** — 두 팔이 어긋나면 «보고»하고 해소하지 않는다 |
| 🆕⑳ **`server/tests/test_temporal_text_contract.py`** (**79줄**) | 기본 스위트 편입 **심(shim)**. `map_seam`·`blank_predicate` 와 **같은 형태·같은 사유**(커맨드라인에 경로를 주면 pytest 가 `testpaths` 를 무시한다). 🔴 **계약 파일이 없으면 skip 이 아니라 `RuntimeError`** — 조용히 아무것도 안 덮는 심은 심이 없는 것보다 나쁘다 |
...
> 🆕㈥ **[2026-09-13 D-33 대조]** 클라 셋(C-101 · C-102 `30cdbb48` · C-103 `803f451b`)이 건드린 `client2/src` 파일 «여덟»이 전부 이 절에 **행을 가진다**(`admin.js` · `chain_rule_panel.js` · `closed_list.js` · `ontology_explorer_view.js` · `raw_registry_panel.js` · `api.js` · `ui.js` · `enrichment_reference_view.js`). ⚠️ 서술의 정본은 `frontend.md` 이고 클라 레인이 그쪽에 썼다 — 이 절은 «자리»의 지도다.
> ⚠️ **그리고 대조하다 만난 함정 하나를 적어 둔다** — 지시가 준 C-101 해시 `d3c97b75` 는 «병합 커밋»이라(부모 둘) `git show --name-only` 가 **빈 결과**를 낸다. 🔴 그대로 읽으면 「C-101 은 소스를 안 건드렸다」가 되는데 거짓이다 — `-m` 을 붙이거나 착지 쪽 부모(`70a4c115`)를 봐야 파일이 보인다. 📌 부류: **빈 결과는 「없다」가 아니라 「이 질의가 아무것도 못 짚었다」일 수 있다**

Vite + Vanilla ESM + AG-Grid. 상태는 `state.js` 싱글턴(리액티브 아님 — 변조 후 명시적 리프레셔 호출).

> 🆕⑥ 🔴 **[2026-08-13 `vite.config.js` 실측 정정] 멀티페이지 엔트리는 6이 아니라 7이고, 종전 목록은 없는 것 하나를 담고 있고 있는 것 둘을 빠뜨렸다.** 실측(`aeddac8`의 `build.rollupOptions.input`) — `main`(index.html) · `admin` · `map_editor` · **`map_editor2`** · `graph` · `trace` · **`ledger`**. ~~`enrichment`~~는 `ab36fab`이 페이지와 함께 걷어 갔다(위 표의 해당 행). 🔴 **`trace`와 `ledger`는 서로 다른 화면이다** — 앞은 `/graph/trace`(G2 지식그래프 보고서, [§7 `trace.js`](#7-client2src--웹-클라이언트)), 뒤는 `/api/ledger/trace`(⚰️ **[2026-08-28] 이쪽도 은퇴했다.** 정본 원장 계보 walk 이었고, [§5-H](#5-h-정본-원장-canonical-ledger)). 소스 주석이 합치지 않은 이유를 적고 있다: 「어느 주장이 왜 이겼는가」를 보여 주는 화면을 「무엇이 연결돼 있는가」를 보여 주는 화면 뒤에 두게 된다.

> ⚰️ 🆕⑬ 🔴 **[2026-08-29 재측정] 엔트리는 «7»이고 구성이 바뀜다.** 실측(`3b77fbc3` 의 `build.rollupOptions.input`) — `main`(index.html) · `admin` · `map_editor` · `map_editor2` · `graph` · `trace` · 🆕 **`rnd_board`**(rnd-board.html — [§7-B](#7-b-rd-보드-rnd_board-2026-08-29-신설-등재)). 🔴 **`ledger`·`rnd_console`·`ledger_graph` 셋은 삭제됐다**(은퇴가 아니라 삭제 — `vite.config.js` 주석이 소유자 판정 「ㅇㅇ 버려」를 적고 있다). 아래는 그것들이 살아 있던 때의 서술이다: ~~🆕⑨ **[2026-08-18 재측정] 엔트리는 7이 아니라 «9»다**~~. 실측(`5359fdd`의 `build.rollupOptions.input`) — `main`(index.html) · `admin` · `map_editor` · `map_editor2` · `graph` · `trace` · `ledger` · 🆕 **`rnd_console`**(rnd-console.html) · 🆕 **`ledger_graph`**(ledger-graph.html). 🔴 **뒤의 둘은 이 지도에 «화면도 모듈도» 등재된 적이 없다** — 소비 모듈은 [§5-I](#5-i-미등재-모듈-레지스트리-2026-08-18-실측)에 존재·크기만 올렸다. ⚠️ **`graph`와 `trace` 엔트리는 살아 있으나 그 서버 라우트는 410으로 은퇴했다**([§1.5](#15-그래프-조회-구간은-410으로-은퇴했다--그-일곱은-이제-없다)) — **이 패스는 두 화면이 의도적 잔존인지 잔재인지 판정하지 않았다**(클라 도메인 소관).
...
### `main.js` (🆕 **2,144줄** @`c904b389` — ⚠️ 🆕⑰ 의 2,333 은 이 범위(`b1db471a` 2,149) «전»에 이미 낡아 있었다, 그 구간은 이 패스가 열지 않았다 — 🆕⑰ 2,333 @`64b562b6` — 🆕⑯ 2,267 @`9eb30691`, 🆕⑪ 2,112 @`b3e1ec83`, 구 표기 2,048) — index 페이지 오케스트레이터

> 🆕 **[2026-09-15 C-109 `c87080e3`] 배너가 «이 표»를 알게 되는 자리가 «하나»가 됐다** — **`redoBannerFollows(table)`** :247(@`c904b389`): `redoBanner.setRelation(table)` · `setBusinessKey(state.currentBusinessKey)` · `loadReplayableRules(table).then((rules) => redoBanner.setRules(rules))` :254. 부팅(`init()` :101 의 :176)과 표 바꿈(`setupEventListeners()` :301 의 table select :478)이 «같은 줄»을 지난다 — 갈라지면 한쪽 경로에서만 목록이 낡는다(기준 ④ 「둘이 갈라질 수 있나」). 그 부품의 import 은 «넷»이 됐다: 🆕 `{ loadReplayableRules } from './replayable_rules.js'` :32. ⚰️ **`loadChainRuleNames()` 는 «없다»**(§0 검사 — 후계는 아래 `replayable_rules.js` 절). `initRedoBanner()` :262 는 이제 규칙 목록을 «안 부른다» — 표가 정해질 때 온다. ⚠️ `client2/tests/startup_socket_gate_harness.mjs` 는 `redoBannerFollows` 를 «noop 으로 심는다» :270 — 그 하니스의 앵커도 이 이름을 «찾는다» :544.
>
...
> 🆕🆕🆕 **[2026-08-11 후속 · `dab9152`] 행/셀 이력이 무한정 배열에서 서버 봉투(`{logs, truncated, next_cursor}`) 기반 페이징으로 바뀌었다** — 서버 짝은 [`admin/audit_history.py`](#6-기타-서버-모듈-한줄-요약)의 `fetch_page`. **전역(global) 이력 탭은 이번에 손대지 않았다** — 그쪽은 여전히 `/audit_logs/recent`의 맨 배열을 그대로 순회한다(그 라우트가 헤더로만 truncation을 흘리는 이유이기도 하다, §`admin/audit_cache.py`).
...
### `map_editor2.js` (🆕🆕🆕🆕 **496줄**(🆕⑨ `5359fdd` 실측 — 구 표기 470) @`3d43a6c`, 412에서 **+58**) — 페이지 엔트리
...
### 🆕 `retroactive_view.js` (🆕⑯ **610줄** @`9eb30691` — 구 표기 446) — 소급 적용 화면의 **뷰 모델**(DOM 없음)
...
> 🆕⑯ **[2026-08-31] export 가 «11 → 15»가 됐다**(전건 계수) — 신설 «넷»: `elapsedMinutes` · `buildProgressCell` · `buildRunsView` · `buildConfirmActions`. 그 밖에 module-private 상수 둘(`RUN_FINISHED` · `RECENT_FINISHED_SHOWN`)이 생겼고 **export 가 아니다**. 실행 «목록»과 취소 버튼의 판정이 전부 여기로 들어왔다 — `admin.js` 는 「그린 것」만 한다(그 파일 주석이 그렇게 못 박는다).
> - **`elapsedMinutes(startedAt, now)`** · **`buildProgressCell(processed, total, minutes)`** — 🔴 **`total` 이 `null` 이면 퍼센트를 «만들지 않는다»**. 모름을 0 으로 접으면 화면이 「할 일 없음」과 0% 를 그린다.
> - **`buildRunsView(payload, now, cancellable)`** — 🔴 **`payload` 가 «두 출처»다**: `{runs, ingestions}`. 서버의 `/admin/retroactive/runs` 는 파일 인제션을 «안 싣고»(그 판정은 [§5-D](#5-d-2026-08-04-신설-서버-모듈) 의 `runs()`), **합치는 것은 여기 화면 쪽**이다 — `admin.js.refreshRunning` 이 `/admin/retroactive/runs` 와 `/admin/file-ingestion/active` 를 **`Promise.all` 로 같이** 읽고 🔴 **한쪽 실패를 «빈 배열로 접지 않고» `failedSources` 로 말한다**(접으면 화면이 「도는 작업 없음」이라는 거짓을 말한다). 🔴 **셋째 인자는 «어느 op 가 멈출 수 있나»의 맵**이고 `/operations` 에서 온다 — 그게 «늦게» 오므로 도착 뒤 다시 그린다. 🔴 **모르는 상태 낱말은 «도는 것»으로 본다**(반대로 두면 서버가 상태를 하나 늘리는 날 도는 작업이 조용히 사라진다) — 대신 × 는 안 단다. 🔴 **끝난 실행의 시계는 «멈춘다»** — `finished_at` 으로 재지 않으면 어제 끝난 것이 「1,000분째」가 되고, 이 화면에서 시간은 곧 판단이다. `RUN_FINISHED = ['done','cancelled','failed']` 로 갈라 최근 **`RECENT_FINISHED_SHOWN = 3`** 개만 «아래»에 붙이고, **`liveCount` 는 «도는 것»만 센다**. 인제션 줄은 `cancel: false`(이 라우트가 취소를 안 받는다) · `moving` 은 `status === 'PROCESSING'` 일 때만(`QUEUED` 는 heavy 레인 정상 대기다).
> - **`buildConfirmActions()`** — 확인창의 버튼 «둘». `buildActionsView(operation, record)` 와 짝이다.
> - ⚠️ 🔴 **파일 머리 주석이 「five buttons … `graph_orphans` deletes」라 적는데 `graph_orphans` 는 «없는 op» 이고 오늘 op 는 «여섯»이다**([§5-D](#5-d-2026-08-04-신설-서버-모듈)). 코드 소관이라 여기서 못 고친다 — **이 주석을 op 목록의 출처로 쓰지 마라.** 같은 이름이 `client2/tests/retroactive_view_harness.mjs` 의 픽스처에도 있지만 그쪽은 「서버가 무엇을 보내든 렌더한다」를 재는 합성 입력이라 **정당하다.**
...
> 🔴 **조립식** — `constructor(host, deps)` 가 자기 host 와 `{doc, loadDeclaration}` 을 받고 **모듈 수준 상태 0**. 라우트도 `apiBase` 도 모르고 **함수 하나**를 주입받는다. `loadDeclaration` 이 주입되지 않으면 `'refused'` 다(읽은 적이 없으므로 「아님」으로 그리면 거짓말).
...
> 🔴 **URL 질의에 «안 싣는다»**(총괄 지시) — 범위 값은 길고 주소창은 데이터를 두는 자리가 아니다(이력·서버 로그·어깨 너머로 샌다).
> 🔴 **한 번 쓰고 «한 번 먹는다»** — `takeRescopeHandoff` 가 읽으면서 지운다. 남겨 두면 다음에 어드민을 열었을 때 «지금 고른 적 없는» 범위가 채워져 있고 운영자는 그걸 자기가 고른 것으로 읽는다.
...
### 🆕⑰ `redo_banner.js` (🆕 **375줄** @`c904b389` — 350 @신설 `0a888db4`·`0e48f79f`·`6e060fd7`; `c87080e3` C-109 +25) — 「고른 행을 «어떤 그룹으로 묶어» 다시 돌릴 것인가」
...
> 🔴 **두 버튼의 «줄»을 서로 다른 것이 정한다**: 원장은 `scope_column` **값**마다(선언이 준다 → 여기서 묶는다) · 체인은 **규칙**마다(🆕 **C-109 `c87080e3`: 화면이 넣어 주는 것은 «이름 목록»이 아니라 `{name, trigger_table, target_table, kind}` «객체 목록»이다**, `setRules(rules)` :100 — 줄이 `name · trigger → target — <크기>` 를 말하고(가른이는 «기호» — 공백 둘은 HTML 이 하나로 접어 이름과 표가 붙었다, 소스 주석) `kind` 는 배지 `span.redo-panel__kind`(:268, CSS `style.css` :3073)로 «칸에» 단다. 서버가 `kind` 를 안 말했으면 배지도 «없다» — 지어내지 않는다. 이름 없는 규칙은 `params: null` — 누르는 줄로 두지 않는다).
> 🔴 **`null`(못 읽음)과 `[]`(선언에 없음)을 «다르게» 그린다** — `this.sources` 도 `this.rules` 도(🆕 C-109: 규칙 쪽 `[]` 의 뜻이 «좁아졌다» — 「서버에 규칙이 없다」가 아니라 「**이 표를 트리거로 하는** 규칙이 없다」, 문장 `'이 표를 트리거로 하는 규칙 없음'`). 합치면 **403 과 빈 설정이 같은 픽셀**이 된다.
...
### 🆕 `replayable_rules.js` (**40줄**, `c87080e3` 신설 2026-09-15, C-109) — 「이 표를 트리거로 하는, 다시 돌릴 수 있는 규칙」의 «목록»을 묻는 한 함수

> 🔴 **왜 자기 파일인가** — `main.js::loadChainRuleNames` 의 후계인데 `main.js` 는 node 가 import 못 한다(최상단 ag-grid·CSS import + 마지막 줄 `init()`). 그 안에 두면 재는 길이 «잘라쓰기»뿐이고 그것은 금지다(2026-09-02). `source_rows.js`(C-14) · `write_guard.js`(C-107) 가 같은 사유로 나온 자리이고 이 파일이 셋째(파일 머리글).
> 🔴 **답이 «세 상태»** — 객체 배열(읽었다) · `[]`(이 표를 트리거로 하는 규칙이 없다) · `null`(못 읽었다: `table` 없음 · 토큰 없음 · `!res.ok` · 모양이 다름 · throw). 셋을 둘로 접으면 403 과 빈 목록이 같은 픽셀이 된다 — 부품(`redo_banner`)의 두 문장이 이 세 상태를 읽는다.
> ⚠️ **서버가 준 객체를 «그대로»** 돌려준다 — 여기서 고르면 칸이 하나 느는 날 화면이 선언보다 «덜» 말한다. 무엇을 그릴지는 부품이 정한다. 거르는 것도 «서버»다(S-250, [§1.4](#14-api-라우트-표--어드민운영그래프맵인리치먼트)) — 화면은 묻기만 한다.

- **`loadReplayableRules(table) -> Promise<Array<object>|null>`** :24(@`c904b389`) — `GET ${API_BASE}/admin/chain/rules/replayable?table=${encodeURIComponent(table)}` :31, 헤더 `[ADMIN_TOKEN_HEADER]: readAdminToken()`. import 둘: `API_BASE`(`config.js`) · `{ADMIN_TOKEN_HEADER, readAdminToken}`(`admin_token.js`). `^export` 1.
- **소비자**: `main.js::redoBannerFollows(table)` :254 «하나». **채점자**: `client2/tests/replay_rules_harness.mjs`(**329줄**, import 기반 — `client2/scripts/check_harnesses.mjs` :1465 에 28 건으로 등록) — 세 상태 · 배너 줄의 `trigger → target` · `kind` 배지(`join`/`mapper`) · 배지 없음(서버가 안 말함). `redo_banner_harness.mjs`(811줄, 51 건)도 객체 목록으로 갈아탔다(`c87080e3`).
...
### 🆕 `config_resolve_view.js` (🆕㉜ **373줄** — 구 표기 324) — config 해석 보고서의 **뷰 모델**(DOM 없음)

> 🆕㉜ **[C-87 `1fa76cd9`] 도메인을 «서버가 말한 셋업 순서»로 세운다** — 걸음 순서를 «화면이 계산하지 않고» `SETUP_STEPS` 를 읽는다(§`config_resolve_report`). 🔴 그것이 판정 312 의 「라우트만 · 둘째 판정기 없음」이고, 화면이 자기 차단 규칙을 계산했으면 서버가 「돈다」고 보는 걸음을 화면이 「막혔다」로 그릴 수 있었다 — **둘 다 오류를 안 낸다**. 📎 하니스는 S-202
...
### `graph_viewer.js` (**1,274줄**(🆕⑨ `5359fdd` 실측 — 구 표기 1,254)) — 지식그래프 서브그래프 뷰어 (graph.html 엔트리, 무라이브러리)
...
| `tokens.css` (🆕㉖ **351** — 구 표기 308) | 디자인 토큰(색·타이포·간격) — 듀얼 테마 CSS 변수의 SSOT. 2026-07-25 다크 세트 심화(Ground L* 9.2, WCAG AA 유지). **[`a98dc72`] `#toast-container`가 하단 중앙 배너로 전환** — 우하단 고정이 맵 에디터 자재 목록을 가리던 문제와 그 회피 장치(`--toast-inset-right` 변수)를 함께 제거, 모션은 opacity 전용<br>🆕㉖ **[C-80 `f1497dbb`] 여백·크기 토큰 «한 벌»이 여기 들어왔다** — `--space-1..6` :167~172(3.4 그리드) · `--fs-label` :178 · `--graph-max-height` :182. 🔴 값의 출처는 «캐논»(`client2/src/ontology_explorer.css`, 1,286줄)이고 스킬 `.claude/skills/ui-design-system/SKILL.md` 가 「어느 규칙이 있나」를 말한다 — 둘이 어긋나면 «파일이 이긴다». ⚠️ **오늘 `--space-5`·`--space-6` 은 읽는 곳이 «0» 이고, `var(--space*)` 를 쓰는 `.css` 파일도 «0» 이다**(참조 스물하나가 전부 `client2/admin.html` 의 인라인 `<style>`) — 소비자 전수는 [frontend.md §3.4](./frontend.md) 가 센다 |
...
| `utils.js` (**347**, 337 → **+10**) | `getLocalTimeString`(~2) / **전역 토스트 재작성**(~29–174) / 🆕 **[`c3a5239`] `dismissToasts(dedupeKey)`(**~54** export — 지시형 토스트("Ctrl+V를 누르세요")는 그 지시가 참이 아니게 되는 순간 사라져야 한다. 안 그러면 **이미 실행된 동작을 반복하라고 말하고 있는 것**이다. 소비: `main.js`의 스마트 페이스트 해제·소비 경로 3곳)** / `getCleanFilename`(**~176**) / **[`269b39e`] 인제션 진행 카드 상한** — `MAX_VISIBLE_PROGRESS_CARDS=3`(**~197**)·`collapseProgressOverflow`(**~204**, 초과분은 숨기고 "…N건" 한 줄로 집계 — 숨은 카드도 계속 갱신되며 앞 카드가 끝나면 표면화)·`dismissProgressCard`(**~229**, 카드 제거 로직 단일화) / 진행 토스트 `showIngestionProgress`(**~247**)·`finishIngestionProgress`(**~308**). **토스트 규율(전 페이지 영향)**: 만료는 **벽시계 `expireAt`** 기준(`sweepToasts`가 `now >= expireAt` 비교 — 백그라운드 탭 `setTimeout` 스로틀링으로 무한 누적되던 원인 제거, 타이머는 스윕을 깨우는 힌트일 뿐) · 상한 `TOAST_MAX_VISIBLE=4`(~29)이고 퇴거는 **비-에러 오래된 것 우선**, 방금 삽입분은 `keep` 인자로 면제 · TTL `{info:5s, success:5s, warning:9s, error:15s}`(~30 — **에러 15초는 성공 알림에 밀려나지 않게 하는 의도적 예외**) · 스윕 트리거는 타이머 + `visibilitychange` + `window.focus` + 삽입 전후(~101–105) · `dedupeKey` 합치기는 **에러 제외**(건별 원인이 중요), 같은 키+타입이면 `count+=1`·만료 연장·`… · N건` 표기 · 본문은 `textContent`(HTML 해석 금지). 🆕 **`escapeHtml`(~17, export) — 이스케이프의 «정본 하나»**: `& < > " '` 다섯 자를 감싸고 `null`/`undefined`는 **빈 문자열**이다(「null」을 화면에 찍는 것은 값이 아니라 사고다). 소비 넷 — `admin_rows.js` · `map_editor.js:11108 escapeHtmlAttr` · `transfer_plan.js:101 esc` · `map2/excel_io.js:757 esc`. 🔴 **뒤 셋은 «지역 별칭»이고 본문이 위임 한 줄**이다. 그 셋이 «함수 선언»인 것은 호출자가 정의보다 위에 있어서다 — `const`로 바꾸면 **TDZ로 죽는다**(사유가 `map_editor.js:11106`에 적혀 있다). 🔴 **이 이름이 사본 셋이던 동안 셋이 서로 «달랐다»**: `excel_io`가 따옴표를 안 감싸 속성 «안»에서만 안전하지 않았고, `null` 처리도 갈렸다 — 이름이 같아서 같은 것으로 보였다. 채점: `tests/escaping_harness.mjs`(행동으로 잰다 — 「불렀나」가 아니라 「값이 속성 밖으로 나오나」) |
| 🆕 `admin_rows.js` (**150**) | `admin.js`의 목록 행 다섯을 **문자열로 짓는 순수 함수 다섯** — `fileLogRowHtml` · `activeIngestionRowHtml` · `workspaceRowHtml` · `mapperRowHtml` · `autoUpdateRowHtml`. 유일한 소비자는 `admin.js`. 🔴 **자기 모듈인 이유는 «하니스가 import 해서 채점»하기 위해서다** — `admin.js`는 `tokens.css`를 import 해 node 가 못 읽고, 잘라쓰기는 금지다. 배지(`statusBadge`·`configBadge` …)도 같이 들어왔다: 그것들이 `log.status`·`ws.config_file` 같은 **서버 문자열을 안에 품어서**, 호출자에 두면 이스케이프 결정이 두 파일로 갈린다 |
| 🆕㉖ `chain_graph.js` (**419**, C-78/C-79 `0236eb78`·`128c30a5`) | **네 파일에 나눠 적힌 한 흐름을 «한 그림»으로** 그리는 클라 절반(서버 절반은 §5 `chain/graph.py`). export **아홉** — `EDGE_KINDS`(mapper·enrich·vjoin·ledger) · `edgeClass` · `originOf` · `layersOf` · `layoutFits` · `countLine` · `contestedLine` · `chainGraphView` · `class ChainGraphPanel`. 🔴 **`ledger_error` 는 상태가 «셋»이고 좋은 날엔 키가 «없다»**(없음 / 문장 / 빈 문자열) — 그 줄이 없으면 원장 엣지 0 이 「원장이 아무것도 안 깨운다」로 읽히는데 진실은 「그 절반이 안 실렸다」다. **같은 픽셀, 다른 사실.** ⚠️ `counts` 는 «수 여덟에 단위가 없어» 그림 «위 한 줄»이고 순서는 응답의 것(`Object.entries`)이라 아홉째가 와도 여기를 안 고친다. 수가 아예 없으면 «줄을 안 그린다»(0 여덟이 아니라). 소비자 — `admin.js` + `client2/tests/chain_graph_harness.mjs` |
| 🆕㉖ `runtime_panel.js` (**167**, C-76 계열 `b018a9b1`) | `GET /runtime` 의 고리들을 «값으로» 세우는 패널. export **아홉** — `ALIVE`(●/○/`ABSENT`) · `TONE` · `COLUMNS` · `visibleColumns` · `secondsText` · `aliveText` · `rowOf` · `runtimeView` · `class RuntimePanel`. 🔴 **컬럼은 «값»을 따른다** — 어느 행도 값을 안 가진 컬럼은 «안 그린다», 한 행이라도 있으면 남긴다. 그리고 생사는 «값»에서 색이 오지 «컬럼 이름»에서 오지 않는다 — **모르는 것에는 색이 없다**. ⚠️ 없는 칸은 «생략»이지 0 이 아니다(「lap 을 낸 적 없다」 ≠ 「마지막 lap 이 0 초」). 소비자 — `admin.js` + `client2/tests/runtime_panel_harness.mjs` |
| 🆕㉖ `server_time.js` (**81**, C-77 `a7a98595`) | **서버 시각을 «UTC 순간»으로 읽는 한 자리.** export **다섯** — `NO_TIME`(`-`) · `parseServerInstant` · `localStamp` · `localMinute` · `localShort`. 🔵 이 모듈의 모양이 「로더 목록」이 아니라 «기제»인 것이 C-77 의 값이다 — 화면마다 자기 파싱을 적으면 naive 시각이 화면 수만큼 다른 답을 갖는다(서버 쪽 짝은 `utils/time_format.py`, S-182 ⓐ). 소비자 «일곱» — `admin.js` · `rnd_board/main_trend_panel.js` · `rnd_board/reach_panel.js` · 하니스 넷(`board_modules.mjs` · `rnd_board_control_trend_harness` · `rnd_board_reach_harness` · `server_time_harness`) |
| `dom.js` (~57) | DOM 참조 일원화 — `elements` 게터 객체(+`traceBtn`/`menuTrace`) |
| `config.js` (~5) | `API_BASE`/`CURRENT_USER`/`pageLimit` |
| 🆕 `walk/derive.js` (🆕㉝ **268** — 🆕㉑ 231 · 구 표기 102) | 걷기 화면(`walk.html` → `src/walk/main.js`)의 **순수 판정 🆕㉑ 열하나**(구 표기 여섯) — `bareName` · `followFromRoute` · `followChoices` · `tableColumns` · 🆕 `cellSource` · `cutBudgets` · `staticTypes` · `keepWalkableRoutes` · 🆕 **[C-70] `COLUMNS` · `sectionsByType` · `sectionHeading`**. 🆕㉑ **[2026-09-10] 신설 둘이 짝이다** — `tableColumns` 가 컬럼«마다» 「어디서 읽나」를 말하고 `cellSource` 가 그대로 읽는다: 종전에는 렌더러가 산술(`slice`)로 되쪼개 컬럼 무리가 «셋»이 되는 날 조용히 어긋났다(ruling 130-C ㉡). 🆕㉑ 제품 소비자는 «셋»입니다 — `client2/src/walk/main.js:35`(import 문이 여러 줄이라 그 문장이 «끝나는» 줄이 여기다) · 🆕 **[C-70] `client2/src/rnd_board/walk_box_panel.js:42`** · 🆕 **[C-72] `client2/src/walk/table_view.js:16`**. 🔴 **둘째가 C-70 의 값이다** — 걷기 표가 둘인데 컬럼을 정하는 쪽이 하나가 됐다(기준 ④). 🔴 **셋째는 C-72 의 값이다** — 걷기 «페이지»의 표 절반이 모듈이 되면서 `tableColumns`·`cellSource`·`sectionsByType`·`sectionHeading` 넷의 호출자가 `main.js` 에서 그리로 옮겨 갔다. 하니스 둘도 이 파일을 «import» 한다 — `client2/tests/walk_route_fill_harness.mjs` · `contracts/walk_node_shape/client_harness.mjs`. 그게 이 모듈이 따로 있는 이유다. 🔴 **약속: 도메인 낱말도 키 이름도 이 파일에 없다** — 표의 컬럼은 선언의 그 타입 `keys` + 응답이 실어 온 qualifier 이름에서 오고(`tableColumns`), 정적 타입은 `class === 'static'` **하나**로 갈린다(서버 `_static_types()`와 «같은 술어»). 🔴 거르는 것은 「정적 타입을 지나는 경로」가 아니라 **`static → not static` 걸음**이다 — static→static 은 기제 연쇄라 walk 이 허용한다. 🔴 `bareName`이 있는 이유: 선언은 `wafer@1`로 판을 붙이고 타입 그래프와 전선은 안 붙인다 — 두 철자를 «직접» 비교하는 것이 이 파일이 닫은 결함이고, 그 결함은 **조용했다**(아무것도 안 켜져서 클릭을 무시한 것처럼 보였다). 자기 모듈인 이유는 `main.js`의 클로저였으면 채점에 DOM 을 세워야 해서다 — 하니스 `walk_route_fill_harness.mjs`가 **같은 함수를 import** 한다<br>🆕㉝ **[C-89 `149e792e`] `pluralAttributes(answer)` :154 — 선언이 `many` 라 부른 속성을 «리스트 그대로» 읽는다.** 🔴 **독자가 «하나»다** — `walk/table_view.js` :85 와 `rnd_board/walk_box_panel.js` :571 «둘»이 이 함수를 부른다(각자 펴지 않는다). 두 벌이면 「이 속성이 여럿인가」가 두 화면에서 갈린다. 📎 서버 쪽 짝은 S-144 `attribute_cardinality`(§5-H `setup_bundle`) |
| 🆕㉞ `rnd_board/api.js` 의 «좌석 선언» — `mapModel(answer, grid, axis, plan)` · `waferFactsFromWalk(answer, kind, plan)` (C-92 `17819603`, 판정 343) | **맵·웨이퍼 사실 파생이 술어·타입을 «이름 대지 않는다» — 좌석이 선언한다.** 종전 `api.js` 의 «여덟 줄»이 `inspected`·`observed`·`wafer`·`defect_kind`(·`die`)를 들고 있었다 — C-90 이 트렌드 경로에서 걷어낸 «같은 부류»가 다른 파생 «둘»에 앉아 있었던 것이다. 🔴 **선언이 «없는» 좌석은 셀을 «하나도» 안 그린다** — 오늘의 이름이 «숨은 기본값»이 아니라는 뜻이다. 📌 상설 「코드에 도메인 낱말이 «없다»」의 실물이고, 그 낱말이 사라진 자리는 «선언»이다 |
| 🆕㉞ `client2/tests/lib/board_modules.mjs` 의 `moduleUrl(src)` — `outward()` 가 «그 안»에 산다 (C-93 `d30e0a1d`, 판정 345) | 🔴 **C-92 가 낱말 하나를 남겼고 그 벽을 제가 「이 파일들은 import 가 안 된다」로 이름 댔는데, 재 보니 벽이 «더 작고 더 나빴다»** — `board_modules.mjs` 는 바깥 지정자를 다시 쓰는 «일반 규칙»을 «이미» 들고 있었고(자기 주석이 이유를 적는다: 손으로 적은 목록이 하니스를 «두 번» 넘어뜨렸다 — C-70·C-77), `api.js` 의 줄이 그것을 «안 지났고», «사본 여섯»이 각자 같은 세 줄을 같은 누락과 함께 들고 있었다. ⚠️ **그래서 `api.js` 가 첫 import 를 얻는 날 보드 하니스 여덟이 «아무것도 안 재게» 됐다.** 🔵 수리는 «문 하나»다 — 모든 모듈 URL 을 `moduleUrl(src)` 이 짓고 `outward()` 가 그 «안»에 있으므로 **호출 자리가 «잊을 수 없다»**(일곱 자리가 전부 잊고 있었다) |
| ⚰️ ~~`rnd_board/api.js` 의 `AGGREGATE` export~~ | **[C-90 ② `deebe3da`] 다리가 «걷혔다».** 트렌드가 «서버가 접은» `groups` 를 읽고, 클라의 집계와 그 계약 클라 절반이 같이 은퇴했다. 🔵 **⏳ 로 적어 둔 대로 됐다** — D-19 에 이 줄을 「다리다, C-90 ② 에서 은퇴 예정」으로 적었고 하루 만에 그대로 걷혔다. 다리를 놓을 때 «언제 걷을지»를 같이 적으면 영구 건물이 되지 않는다. 📌 그리고 잘린 걷기는 «자기 수를 그린다» — 수가 예산을 살아남는다(S-146 계열) |
| 🆕㉑ `walk/table_view.js` (**102**, C-72 신설) | 걷기 «페이지»의 표 절반 — **DOM 이 한 조각도 없다**. export **여섯** — `ROW_CAP`(200) · `valueText(value)` · `isNumericText(text)` · `qualifiersByNode(edges)` · `qualifierNamesOf(nodes, byNode)` · **`walkTableView(result, entities, cap)`**. 소비자 `client2/src/walk/main.js:38`(`walkTableView`). 🔴 **여기서 «정하지» 않는다** — 구획·머리·컬럼·셀 값은 전부 `derive.js` 의 것이고, 이 파일이 더하는 것은 「그 넷을 어떤 순서로 엮나」와 「값이 어떤 «글자»가 되나」뿐이다. 🔴 **왜 모듈인가**: 그 결정들이 `boot()` 안 클로저인 동안은 「이 화면이 그 함수를 부르나」를 «거동으로» 잴 자리가 없었다. 채점 `client2/tests/walk_table_harness.mjs`(바닥값 15) — `boot` 에 «가짜 doc» 을 넣어 화면을 세우고 그려진 머리·컬럼·셀을 이 함수의 답과 «대조»한다. 🔴 **그 코퍼스의 M1 이 「사본을 다시 넣기」이고 행 수도 구획 수도 «안 바꾼다»** |
...
| 🆕 `truncation.js` (**123** — 76에서, `187ab50d`) | 「잘렸나」의 **정본 하나**. export **다섯** — `fetchLimitFor(cap)` · `isTruncated(rows, cap)` · `withinCap(rows, cap)` · `saysTruncated(said)` · `suggestTruncatedNote(shown)`(값 제안이 잘렸을 때 «두 화면이 같이» 쓰는 한 문장). 소비: **일곱** — `map_editor.js` · `timeline.js` · `value_suggest.js` · `map2/decode.js` · `map2/api.js` · `map2/main.js`(`187ab50d`) · `ontology_explorer_store.js`(`187ab50d`) · `saysTruncated` 호출 **여덟**(timeline 이 둘). 🔴 **한 파일에 «두 질문»이 있고 그게 의도다**: 앞의 셋은 「돌아온 행이 상한을 넘었나」를 «클라가» 판정하고, `saysTruncated` 는 「«서버가» 자기 응답을 잘렸다고 말하나」다. 운영자에게 「잘렸다」는 «하나»라서 답도 한 자리에 있어야 한다. 🔴 **`total` 을 받지 않는다** — 받으면 전수 count 를 다시 치르고(첫 화면 비용의 66~95%), 상한+1 로 청해 「한 행 더 왔나」를 보면 «같은 정확도»다. 🔴 **판별식은 `>` 이지 `>=` 가 아니다** — 상한만큼 «딱 맞게» 온 정상 응답을 절단으로 읽으면 온전한 맵이 「불완전」으로 강등된다. 🔴 `saysTruncated` 는 **`true`·`false`·`null`(모름) 셋**을 낸다 — 「잘렸다」의 «진릿값»이 아니라 「그렇게 «말했나»」이고, 전선의 `truncated` 가 실측 «여섯 모양»이라 있음·진릿값만 보면 틀린다. 🔴 **여섯째가 정본 축-지도 `{<축>: {cut, omitted, reason}}` 이고, 이 독자는 그것을 «모름»이 아니라 «틀리게» 답하고 있었다**(`187ab50d`, 판정 99) — 그 모양에는 최상위 `reason` 이 없어 잘린 응답이 `false` 로 돌아왔다. 판별은 `cut` 이지 `reason` 이 아니다(서버가 사유 없이 자르는 자리가 실재한다). ⚠️ 이 파일의 값어치는 「코드 재사용」이 아니라 **「두 독자가 «같은 답»을 받는다」**다 — `value_suggest` 와 `map_editor` 는 «같은 라우트의 같은 칸»을 각자 읽어 다른 문장을 만들고, 갈라져도 **오류가 안 난다**. ✅ **그 후보 둘이 들어왔다**(S-5, 2026-09-07) — `map2/decode.js` · `map2/api.js` 가 이제 부른다. 이 파일이 `client2/src/` 에 사는 이유가 그것이었고, 소속 판별식이 서 있었으므로 «확장»이지 누락 수리가 아니다 |
...
## 7-B. R&D 보드 rnd_board (2026-08-29 신설 등재)

> 🔬 **측정 기준: `3b77fbc3`(HEAD)의 커밋된 blob**(`git show 3b77fbc3:<path>`). 🔴 **라인 번호는 이 절에 하나도 없다** — 위치는 `git grep -n "<심볼>" -- client2/src/rnd_board`로 확정하라.
>
> **무엇인가.** 삭제된 원장 콘솔([§5-H 클라 묘비](#5-h-정본-원장-canonical-ledger))의 후계. 콘솔이 **뷰 다섯을 한 URL 공간에서 갈아 끼우던** 화면이었다면, 보드는 **격자에 앉은 부품 여럿 + 바깥의 «이름 붙은 마킹» 저장소**다. vite 엔트리 `rnd_board`(`rnd-board.html`)이고 **`ledger` 엔트리의 자리를 물려받은 것이 아니라 따로 선다**(`vite.config.js` 주석 실측).
>
> ⚠️ **이 패스가 «본체를 읽은» 것은 `api.js`·`main.js`·`marking_store.js`·`walk_box_panel.js`·`panel.js`·`grid_shell.js`·`table_part.js` 일곱이다.** 나머지 부품은 **존재·줄 수·클래스 이름만** 실측했고 심볼 표가 없다 — 있다고 주장하지 않으려고 그렇게 뒀다.
>
> ⚠️ 🆕⑭ **[2026-08-29 심야] 이 절은 «재검증하지 않았다». 단 한 가지만 다시 쟀다** — `api.js` 는 **1,567 → 1,733줄**(`2a4e51cf`)이고, 그 사이에 서버의 개명이 클라에도 착지했다: 쿼리 파라미터가 **`continues_hops` → `backbone_hops`**(소스 주석이 그 이사를 날짜와 함께 적는다), 인자 이름은 `backboneHops`. 🔴 **아래 표의 나머지 수치·심볼은 여전히 `3b77fbc3` 기준이고 한 줄도 다시 열지 않았다** — 이 절을 쓰기 전에 소스를 열어라.

**파일 20개 (js 19 = 7,148줄 · css 1 = 623줄)**

| 파일 | 줄 | 무엇인가 |
|---|---|---|
| `api.js` | **1,567** | 경계 전부 — `ROUTES` · `COLLECTS` · `createWalk` · `createWalkBoxWalk` · 모델 변환기. **↓ 아래 전용 항목** |
| `main.js` | **812** | 합성 루트 — 좌석(seat) 선언 배열 + `apiBase`/`fetchImpl` 을 **한 번** 묶는다. 🔴 부품에 `apiBase`가 다시 나타나면 그것이 결함이다 |
...
| `walk_box_panel.js` | **544** | **↓ 아래 전용 항목** |
| `map_panel.js` | **1,071** | 웨이퍼 맵 좌석 |
...
#### 🆕⑬ `marking_store.js` (**159줄**) — 마킹은 «바깥»에 살고 «여럿»이다

| 심볼 | 실측 |
|---|---|
| `export const SIGN = Object.freeze({ ABSENT: 0, CASE: 1, CONTROL: -1 })` | 🔴 **상태가 셋이고 셋인 채로 있다** — 없음 / 관측 / 대조군. 서버 `ledger_subgraph._signed_seeds`의 `positive`/`negative`와 같은 삼분이다 |
| `export class MarkingStore` | `signOf(name, nodeId)` · `count(name)` · `entries(name)` · `names()` · `set(name, nodeId, sign)` · `toggle(name, nodeId, sign)` · 🆕⑬ **`replace(name, entries)`** · `clear(name)` · `subscribe(name, listener)` · private `_emit(name)`. 🔴 **첫 인자가 «이름»인 것이 이 클래스의 전부다** — 마킹1(맵이 읽는 것)과 마킹2(후보 선택)가 한 저장소 안에 «따로» 산다. 부품은 읽을 이름과 쓸 이름을 각각 선언한다 |

#### 🆕⑬ `walk_box_panel.js` (**544줄**) — 걷기 검색창
...
| 심볼 | 실측 |
|---|---|
| `export const ROUTES` | `lotMap: '/api/ledger/lot_map'` · `composition: '/api/ledger/composition'` · `subgraph: '/api/ledger/subgraph'` · `trends: '/api/ledger/trends'` · `siblings: '/api/ledger/siblings'` · `mapGrid: '/tables/wafer_map_metadata/data'`. 🔴 **서버가 여는 원장 라우트는 `subgraph`와 `declaration` 둘뿐이므로 `lot_map`·`composition`·`trends`·`siblings` 넷은 404다**(서버 실측 `3b77fbc3`). `mapGrid`는 원장 라우트가 아니라 **선언된 관계의 범용 리더**이고 소스 주석이 「`/api/tables/...`는 404, 이쪽이 200」이라 실측을 적어 둔다 |
| `export const LEGACY_ROUTES` (🆕 구 이름 `COLLECTS`) | 🔴 **개명됐다 — 좌석 선언 키도 `spec.collect` → `spec.legacyRoute`다**(`rnd_board/api.js:1454`·`1540`). 사유: `collect`는 이제 **전선의 낱말**(walk 이 무엇을 실어 올지)이고, 이 표는 **아직 «라우트 이름»을 대는 좌석**만 담는다 — 뜻이 둘이던 한 이름을 갈랐다. 🔴 **옛 키는 무시되지 않고 «거절»된다**(`api.js:1549` — 조용히 다른 질문을 보내는 것보다 왕복 한 번이 싸다). ⏭ 그 거절은 개명의 것이지 기능의 것이 아니라, 좌석이 도메인 타입을 선언하게 되는 라운드에 **같은 커밋에서 제거**돼야 한다고 소스가 적어 둔다. 🔵 `follow`를 선언한 좌석은 이 표에 **행이 필요 없다** — 그래서 이 표가 «비어 가는 것»이 그 라운드의 자기 계측이다. 일곱 — `trend_y` · `candidate` · `wafer_process` · `map` · `basis` · `peer` · `reach`. 🔴 **그중 `/subgraph`로 가는 것은 «둘»(`candidate`·`reach`)이고 나머지 다섯은 죽은 라우트로 간다**: `trend_y`→`fetchTrends` · `wafer_process`→`fetchComposition` · `map`→`fetchLotMap` · `basis`→`fetchComposition` · `peer`→`fetchSiblings`. 🔴 **`control-bar` 좌석이 `collect: 'trend_y'`를 선언하고 있고**(`main.js`), `control_bar_panel.js`·`main_trend_panel.js` 둘 다 `options.collect \|\| 'trend_y'`로 폴백한다 — `main_trend_panel.js`의 주석이 그 폴백을 **위험으로 스스로 적는다**(「기본값이 살아나면 좌석이 이름을 뗐는데도 죽은 trends 라우트를 부른다」) |
| `export function createWalk(deps)` | 부품이 부르는 **한 함수**. `apiBase`/`fetchImpl`을 합성 루트에서 한 번 묶고, 🆕 **`spec.legacyRoute`**(구 `spec.collect`)가 없으면 `WALK`(= `fetchSubgraph`)로 간다. **같은 key의 요청은 `inflight` Map으로 합류**한다. 선언 안 된 `legacyRoute`는 «빈 답»이 아니라 **reject** — 물어본 적 없는 질문에 「없음」을 그리지 않기 위해 |
| `export function createWalkBoxWalk(deps)` · `export async function fetchDeclaration(params)` | 걷기 검색창 전용. 성공도 실패도 `{ok}`를 다는 **한 모양**이다 |
| `export function entitySeedId(type, keys)` · `typeGraph(declaration)` · `pathsBetween(declaration, from, to)` | 선언에서 씨앗 id와 타입 그래프를 만든다 |
| 모델 변환기 | `projectionModel` · `compositionModel` · `subgraphModel` · `trendsModel` · `reachModel` · `mapModel` · `basisCountsFromComposition` · `slotPagesFromLotMap` · `peerCountFromSiblings` · `waferFactsFromLotMap` · 🆕 walk 판(`compositionFromWalk` · `waferFactsFromWalk` · `peerCountFromWalk` · `trendFromWalk`) · `isStampedNodeId` · `measuredFromHops__untilServerServesIt`. 🔴 **이름에 `__untilServerServesIt`가 박힌 것은 «임시라는 선언»이다** — 소스 주석이 「`/subgraph`가 그 구분을 직접 실으면 이 함수는 사라지고 어느 부품도 안 바뀐다」고 적는다 |
| ⚠️ `main.js`의 죽은 import | `fetchLotMap`·`fetchComposition`·`fetchTrends`·`fetchSiblings`·`fetchSubgraph` 다섯은 **`main.js`가 import만 하고 한 번도 부르지 않는다**(전건 grep: 각각 `main.js`에 히트 1줄 = import 문). 함수 자체는 `api.js` 안 `COLLECTS`에서 살아 있으므로 **고아가 아니라 «죽은 import»**다 |
...
3. **체인 인제션**: `apply_batch_updates`의 outbox 발화 → NOTIFY → `start_chain_ingestion_worker` 루프 → `process_pending_groups` → `process_chain_transaction_group`(맵퍼 실행, 예: `map_enrichment_dedup`) → 파생 테이블 `apply_batch_updates`(source=chain_ingestion, 순환 차단) → ⚰️ (**[M3] `MapMetaCollector` 가 여기 있었다. S-38 에서 은퇴**) → `_dispatch_broadcasts` → `/internal/events/broadcast`(created_logs 500건 절단 + `total_log_count` 실건수) → WS.
...
7. **맵 에디터**: (부트 시 **[`280ebf0`] `restoreLastOpenMap`**이 `map_editor_last_open` localStorage를 읽어 **수동 LOAD 경로 그대로** 마지막 맵을 재오픈) → `loadExistingMap`(**[`6db517d` H1] 로드 시점에 초안을 덮어쓰지 않는다** — 초안 우선순위 판정 후 1회만 저장) → GET `/tables/{t}/data`(REST) → 편집 → `pushMapData`(**[gate4 `deed6d2`] 적재 대상이 로그형(맵 계약 밖 데이터 컬럼 보유)이면 모든 다이얼로그보다 앞에서 거부** — `/schema`의 site 선언 `map_push_ok: true`가 있을 때만 1회 손실 인지 confirm으로 강등. **[`6db517d` H2] 프레임이 화면의 비어 있지 않은 셀을 전부 못 담으면 confirm 전에 거부** — replace_map 절단 방지) → PUT `/data/updates`(**[gate4] 서버는 purge 스코프 유도 불가 시 400으로 거부하고, 응답 `scope: {filters, deleted, inserted}`로 실제 지운 범위를 되비춘다** — 침묵 noop 폐지). 🆕 **[`019140c`] 📐 표준 로드는 이제 데이터의 원점을 프레임에 **선언**한다**(`startX = minX`) — 셀에서 `minX`를 빼던 종전 코드는 삭제됐다. 두 줄은 한 양이라 화면은 하나도 안 움직이고, 바뀌는 것은 **화면이 말하는 좌표 = Push가 쓰는 좌표 = 저장된 좌표**가 됐다는 것뿐이다(실측: 메타 없는 맵 4개, 1,923셀 중 451셀이 밀린 좌표로 Push까지 갔다). 🆕 **[`02a72c6`] 프리셋은 규격만 기입하고 회전·면은 건드리지 않는다** — 저장된 프리셋이 전부 rot 0/front를 선언하므로 종전엔 **아무 프리셋 적용이나** 회전·뒷면 맵의 좌표를 통째로 재번호했고(표본: 187 중 173) 대비 관문은 격자·원을 떠난 셀이 0이라 아무 말도 하지 않았다. 지금은 선언을 **읽고 적용하지 않으며 info 토스트로 그 사실을 말한다**. 프리셋은 `/map-presets` CRUD. 페인트 잠금은 기동 시 GET `/api/maps/paint-rules` → `applyPaintLockConfig` → 전 편집 경로가 `isProtectedFCell` 단일 관문 통과 — **[U6 `95bf072`] 같은 응답이 `value_column_candidates`·`default_legend`를, [F1 `17f65bd`] `binding`(서버 해석 좌표 바인딩)을 나른다**(`overlayContract`·`servedBindingCache` 캐시 → 드롭다운 preselect·빈 맵 시드·자동 추가 값 색. 클라 사본 0). (WS 미사용)
   ...
   - **[zone 모델 `b35bc9f`] 전사 계획(계획 = 그 맵 자체, DOE = legend 행)**: 맵 로드(`loadExistingMap`) → `readRegistryScope`가 GET `/tables/map_split_registry/data`를 **map_key 필터로** 읽고(`REGISTRY_SCOPES=['map']` — 테이블 전체 어휘 시드는 `269b39e` 결함으로 삭제, 행이 없으면 `seedEmptyDoe` — **[U6] 서빙된 `default_legend` \| 빈 DOE 1행**) `applyRegistryRowsToLegend` + `legendReplaceScope{table, mapKey, fingerprint}` 확립 → `notifyMapContext` → `transfer_plan.js`가 `stage_of_table` 역인덱스로 stage 유도 → GET `/api/transfer-plan/{stages,source-summary}` → 패널에서 STACK·1H/MID/TOP·자재 편집 → `commitRow` → `controller.updateLegendRow` → **`scheduleCellDraft`(로컬 초안만 — 자동 서버 저장 없음)** → 사용자가 **⚡ Push**(`pushMapData`) → `saveLegendToServer` → **PUT `/tables/map_split_registry/data/updates` with `replace_map: true`**(맵 하나의 **값 전체 집합 1회 쓰기** — 지운 값·구역·자재는 집합에 없다는 것만으로 삭제된다. 별도 삭제 단계 없음).
     ...
     - ⚰️ **[S-38 · 2026-09-07] ②가 사라져 출처가 다시 «사람 쪽»이다** — ① 맵 에디터 ⚡ Push · 확정 기록. `b697d34`~`S-38` 사이에는 ② 인제션의 `MapMetaCollector`가 부재분만 합성 등록했고(`source="auto_map_meta"`, 서열 99), **그때 쓰인 행은 그대로 남아 있다**. 오버레이·가용량의 정렬 근거가 여전히 **메타 하나**라는 계약은 바뀌지 않았다 — 바뀐 것은 **메타가 비어 있는 맵의 수**다([§5 `map_meta_registrar.py`](#5-소형-서버-모듈)).
