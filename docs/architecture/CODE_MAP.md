...
> 🔴 **왜 그냥 `import` 하면 안 되나 (2026-09-05 실측, 소스가 적는다):** `import('../src/map_editor.js')` 는 이제 **성공한다** — 옛 사유(`import './tokens.css'`)는 사라졌다. 그런데 **`exported names = 0`** 이라 밖에서 «잡을 이름»이 없다. ESM 네임스페이스는 봉인돼 있고 `export let` 도 밖에선 읽기 전용이라, 접근자를 «붙이는» 것 말고 모듈 내부를 재는 길이 없다.
