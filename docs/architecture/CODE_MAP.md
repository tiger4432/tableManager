...
> **Status:** 🟢 Living | **Last-verified:** 2026-07-28 (**HEAD `55ddffd`의 커밋된 blob 실측** — 워킹트리 아님. [§0 측정 기준](#0-묘비-목록--소스에-존재하지-않는-이름)) | **Owner:** 전 에이전트 공용 | **Source-of-truth:** 각 표의 코드 경로
...
| **`scheduleLegendServerSave`** (legend 디바운스 자동 저장) | `client2/src/map_editor.js` | **자동 저장 자체가 삭제**됐다(사용자 지시 2026-07-28) — 서버 쓰기는 **⚡ Push(`pushMapData` → `saveLegendToServer`) 하나**뿐이고, 편집 경로는 전부 `scheduleCellDraft`(로컬 초안)로만 흐른다. 묘비 주석이 ~2976에 있다 |
...
| `server/main.py` | ~4,232 | [§1](#1-servermainpy--api--ws-허브) |
| **`server/admin_auth.py`** (어드민 토큰 게이트) | ~219 | [§1.6](#16-serveradmin_authpy--어드민내부-토큰-게이트-90e284f-신설) |
| `server/database/crud.py` | ~2,118 | [§2](#2-serverdatabasecrudpy--레이어링-코어) |
| `server/parsers/directory_watcher.py` | ~2,086 | [§3](#3-serverparsersdirectory_watcherpy--파일-인제션) |
| `server/chain_ingestion_worker.py` | ~977 | [§4](#4-serverchain_ingestion_workerpy--체인-워커) |
| 소형 서버 모듈 (models/std_parser/enrichment_*/ingestion_activity/ingestion_checkpoint/bonding_plan/**map_overlay**/**transfer_plan**) + 그래프 트랙(graph_sync_worker/graph_materializer/ontology_config) + **운영 6종**(paths/process_supervisor/health/heartbeat/product_tables/**config_backup**) | ~10,055 | [§5](#5-소형-서버-모듈) |
| 기타 서버 모듈 (한줄 요약) + 설치·개발환경 스크립트 + **교차 구현 계약 `contracts/`** | — | [§6](#6-기타-서버-모듈-한줄-요약) |
| `client2/src/*` | ~22,000 (js 19,136 + css 2,825) | [§7](#7-client2src--웹-클라이언트) |
...
| `get_column_filter_condition(table_model, col_name, f_info)` | 컬럼 필터 → SQLAlchemy 조건 변환(타입별) | ~994 |
| `reload_local_process_cache()` | 웹서버 config 핫리로드 — `models.refresh_dynamic_models(engine)` 위임(싱글턴·ORM·신규 테이블 물리 CREATE, 이슈 #7) + `crud._ontology_cache` 무효화 | ~2943 |
| `load_maps_config() / save_maps_config(data)` | 맵 프리셋 JSON 파일 IO (`MAPS_CONFIG_PATH = paths.config_path("maps.json")` ~3026) | ~3028/3037 |

### 1.1-bis 헬스 블록 (`8117456` 신설 — 파일 상단 ~95–182)

**등록 위치가 계약이다.** FastAPI는 등록 순서로 매칭하므로 이 블록은 파일 맨 아래 SPA catch-all `@app.get("/{file_name:path}")`(~4186)보다 **위에** 있어야 한다. 이 라우트가 없던 시절 `/health`는 catch-all로 떨어져 **HTML을 200으로** 반환했다 — 외부 모니터가 죽은 서버를 살아 있다고 불렀다. `tests/test_health_endpoint.py`가 양쪽(‌`/health`는 JSON · 엉뚱한 경로는 여전히 HTML)을 단언하므로, 재배치로 라우트가 다시 가려지면 조용히 죽지 않고 테스트가 깨진다.
...
| GET `/tables/{t}/data` | `get_table_data` | **메인 조회** — 페이지네이션+필터+정렬+메타 병합 | ~1098 |
| GET `/tables/{t}/schema` | `get_table_schema` | 스키마 계약(`table_config.json` 기반). **[gate4 `deed6d2`] `map_push_ok` 필드 동봉**(~1708 — `config.get("map_push_ok") is True` **엄격 판정**: 문자열 `"true"`·1 등 오타는 잠금 유지, JSON boolean true만 유효. 클라 gate 4의 site 선언 서빙 — [§7 map_editor.js](#7-client2src--웹-클라이언트). 테스트: `tests/test_schema_map_push_ok.py` 3건) | ~1669 |
| GET `/tables/{t}/{row_id}` | `get_row_data` | 단일 행 조회 | ~1714 |
| GET `/tables/{t}/export` | `export_table_csv` | CSV 스트리밍 export | ~1474 |
| POST `/tables/{t}/rows` | `create_row` | 빈 행 N개 생성(+WS 통지) | ~1802 |
| PUT `/tables/{t}/data/updates` | `apply_batch_updates_endpoint` | **메인 편집** — crud.apply_batch_updates 호출 후 병합·브로드캐스트. 배치의 `replace_map:true`(schemas.py ~87)는 **동일 맵 기존 행 클린 삭제 후 재기록** — 맵 Push와 **[M2.6] legend/DOE 저장(`map_split_registry`)**이 이 연산을 쓴다([§7 map_editor.js](#7-client2src--웹-클라이언트)). **[gate4 `deed6d2` 정직한 스코프]** `replace_report` out-param(~1877)으로 crud가 채운 **실제 purge 필터·삭제 행수**를 받아 응답 `scope: {filters, deleted, inserted}`(~1981)로 되비춘다(비-replace_map은 null) — 스코프 유도 불가는 crud `ValueError` → **400**(종전의 "아무것도 안 지운 200" 폐지). **순수 스코프 wipe(deleted>0, upsert 0)도 count 캐시를 무효화**(~1895). 테스트: `tests/test_replace_map.py`(1→**7건**) | ~1864 |
| DELETE `/tables/{t}/rows/{row_id}` | `delete_row` | 단일 삭제 | ~1308 |
| POST `/tables/{t}/rows/batch_delete` | `delete_rows_batch_endpoint` | 일괄 삭제(+WS) | ~1331 |
| POST `/tables/{t}/row_ids/target` | `get_target_row_ids` | 필터 조건 → row_id 목록(범위 작업용) | ~1386 |
| POST `/tables/{t}/upload` | `upload_file` | 파일 업로드 → 워크스페이스 투입(`paths.workspace_path(table,"raws")` ~2404) | ~2397 |
...
| GET `/dashboard/summary` | `get_dashboard_summary` | 대시보드 통계 (+`_get_recorrection_stat` ~916 — [`ec75d4c` 핵심가치 #1 계측] 재수정률 통계) | ~947 |
| GET `/tables/{t}/rows/{r}/history` | `get_row_history` | 행 이력 | ~1749 |
| GET `/tables/{t}/rows/{r}/cells/{c}/history` | `get_cell_history` | 셀 이력 (⚠️ ~2552에 동일 경로 중복 정의 — 선등록인 ~1775이 유효) | ~1775 |
| GET `/tables/{t}/{r}/{c}/sources` | `get_cell_sources` | 셀의 레이어(소스) 목록 | ~2423 |
| DELETE `/tables/{t}/{r}/{c}/sources/{s}` | `delete_cell_source` | 단일 소스 삭제(+재계산·WS) | ~2467 |
| PUT `/tables/{t}/{r}/{c}/priority` | `set_cell_priority` | 단일 셀 수동 우선순위(Pin) | ~2500 |
| PUT `/tables/{t}/cells/priority/batch` | `set_cell_priority_batch_endpoint` | Pin 일괄 | ~2564 |
| POST `/tables/{t}/cells/sources/delete/batch` | `delete_cell_source_batch_endpoint` | 소스 삭제 일괄 | ~2635 |
| POST `/tables/{t}/cells/sources/query` | `query_cells_sources` | 셀 범위 소스 일괄 조회 | ~2696 |
...
| 메서드 경로 | 핸들러 | 역할 | 라인 |
|---|---|---|---|
| POST `/api/graph/sync` | `manual_graph_sync` | 그래프 **백필/복구** 트리거(:8090 프록시 — 주 경로는 materializer). ⚠️ `/admin` 접두어가 아니라 **게이트 대상이 아니다** | ~1986 |
| 🔒 POST `/admin/outbox/retry-failed` | `retry_failed_outbox_events` | outbox 실패 재시도 | ~2780 |
| 🔒 GET `/admin/outbox/failed` | `get_failed_outbox_events` | outbox 실패 목록(페이징) | ~2819 |
| 🔒 GET `/admin/file-ingestion/logs` · `/failed` | `get_file_ingestion_logs` 등 | 파일 인제션 로그/실패 목록 | ~2889/2924 |
| 🔒 GET `/admin/file-ingestion/active` | `get_active_file_ingestions` | **[P1]** 진행 중 인제션 스냅샷(레지스트리 `snapshot()` — 인메모리, TTL 퇴거 포함) — admin File 탭/헬스 스트립 소비 | ~2930 |
| 🔒 POST `/admin/file-ingestion/retry-failed` | `retry_failed_file_ingestion` | 아카이브 파일 재처리(동기 콜백 배선 포함, 내부 `sync_refresh_callback` ~3583) — 워크스페이스는 `resolve_workspace_root` 역조회(별칭 대응) | ~3551 |
| 🔒 GET `/admin/file-ingestion/workspaces` | `get_ingestion_workspaces` | 워크스페이스 현황 — 표시 table_name에 글로벌 별칭(`find_workspace_alias`) 우선 적용 | ~3330 |
| 🔒 POST `/admin/reload-configs` | `reload_system_configs` | config 핫리로드 — 동기 CREATE(1차 DDL 소유자)가 outbox 발화보다 선행 (+SYSTEM_RELOAD outbox 발화) | ~2978 |
| 🔒 GET `/admin/chain/rules` · `/admin/mappers/list` | `get_chain_rules` / `get_mappers` | 체인 룰·맵퍼 목록 | ~3410/3431 |
| 🔒 GET `/admin/auto-update/status` | `get_auto_update_status` | 스케줄러 상태 — 항목별 `active` 부가(제어 파일 실시간 계산) | ~3660 |
| 🔒 POST `/admin/auto-update/toggle` | `toggle_auto_update_script` | 수집기 active 토글 — `config/auto_update_control.json` 갱신(핫 반영, 404/400 명시) | ~3693 |
| **🔒! POST `/admin/auto-update/run-now`** | `trigger_auto_update_run_now` | 즉시 실행(**active 무관** — 수동 실행은 명시적 의도). **strict인 이유: 스케줄러에게 임의 파이썬 파일을 실행시킨다**(아래 `scripts/code`와 짝) | ~3728 |
| 🔒 GET `/admin/scripts/list` · GET `/admin/scripts/code` | `list_admin_scripts` / `get_admin_script_code` | Monaco 에디터용 스크립트 조회 (경로 검사 `_resolve_admin_script_path` ~3977 — 격리 서버가 라이브 트리에 쓰려 하면 **403**. ⚠️ 이 403은 **게이트가 낸 것이 아니라 핸들러가 낸 것**이라 `WWW-Authenticate`가 없다. 클라가 둘을 구분하는 근거 → [§7 `admin.js` `isGateRejection`](#7-client2src--웹-클라이언트)) | ~3918/4021 |
| **🔒! POST `/admin/scripts/code`** | `save_admin_script_code` | 스크립트 저장. **strict인 이유: `mappers/`·`ingestion_workspace/`에 임의 파이썬 파일을 쓴다** | ~4058 |
| GET/POST/DELETE `/map-presets` (+`/api/` 별칭) | `_save_map_preset_impl` 등 | 맵 프리셋 CRUD | ~3059–3123 |
| GET `/api/bonding-plan/core-summary` | `get_bonding_plan_core_summary` | **[본딩 M1]** 코어(lot,slot) 역할별 집계 — `bonding_plan.get_core_summary` 위임([§5](#5-소형-서버-모듈)), `region` 파라미터(rects — 현 클라 미사용), 잘못된 region 400 | ~3128 |
| GET `/api/maps/overlay` | `get_map_overlay(target_table, target_key, sources, eqp=None, limit=None)` | **[M2 신설 · 맵 인프라]** 임의의 맵들을 타깃 맵 프레임 좌표로 정렬해 `overlays[]` 반환. `sources`는 `table` 또는 `table:key`의 CSV(키 생략 시 target_key 승계, 최대 8종). `map_overlay.get_overlay` 위임([§5](#5-소형-서버-모듈)), `parse_sources` ValueError → 400, 셀 상한 `MAX_OVERLAY_CELLS=20,000`(초과 시 `truncated:true`). ⚠️ **`eqp` 쿼리 파라미터는 no-op으로 존치** — `map_overlay.get_overlay`의 `eqp` 인자는 `by_eqp` 분기와 함께 삭제됐다(축소는 총괄 승인 사항). **맵 에디터 클라는 이 엔드포인트를 호출하지 않는다** | ~3161 |
| GET `/api/maps/paint-rules` | `get_map_paint_rules(table=None)` | **[M2 신설]** 페인트 잠금 선언 정본(**기존엔 클라 하드코딩 `'F'`**) — `map_overlay.get_paint_rules`. **[U6 `95bf072`] 맵 기본값 2종을 같은 응답에 동봉해 클라가 사본을 갖지 않게 한다**: `value_column_candidates`(해석 **완료** 목록 — 항상 존재, `resolve_value_column_candidates`) · `default_legend`(선언 그대로 \| **null** = 정직한 부재, `get_default_legend`). **[F1 `17f65bd`] `binding` 동봉** — `table` 지정 시 그 테이블의 **해석 완료 좌표 바인딩** `{x, y, val, key_columns[], source: "declared"\|"derived"\|"fallback_guess"}` \| null(`map_overlay.resolve_binding_info` — 클라 재유도 금지의 단일 원천, [F2] `fallback_guess`는 데이터 경로가 거부하는 추측이라 클라가 **경고해야** 한다). 응답 `{table, rules{…}, binding, default_legend, value_column_candidates}` | ~3199 |
| GET `/api/transfer-plan/stages` | `get_transfer_plan_stages` | **[M2 신설]** 선언된 전사 stage 목록 + 역할 연결 상태(config 해석만 — 행 조회 없음). `transfer_plan.list_stages` | ~3231 |
...
| GET `/api/transfer-plan/validate` | `validate_transfer_plan(ref_table, map_key)` | 계획 검증 — **계획 정체성 = 지금 열어 편집 중인 맵**(`plan_id` 폐기). stage는 `stages.*.target_map.table` 역인덱스로 유도, 미선언 맵은 404가 아니라 `stage_unknown` 경고 + `status:"unverified"`. **`plan_store.registry` 미구성만 404**. **[`b35bc9f` zone 모델] validate는 이제 zone 컬럼(`stack`/`mat_*`)을 읽고 `bands`는 레거시 읽기 전용** — [§5 transfer_plan.py](#5-소형-서버-모듈) | ~3302 |
| GET `/enrichment/rules` · `.../references/{index}` | `get_enrichment_rules` / `get_enrichment_reference` | 인리치먼트 규칙 공개본·참조 뷰 조회 | ~3483/3494 |
| WS `/ws` | `websocket_endpoint` | WS 접속(ConnectionManager). ⚠️ **WS 라우트는 게이트 대상이 아니다** — `Depends`가 HTTP 라우트에만 걸리므로 `test_admin_auth.py`도 WS와 mount는 건너뛴다 | ~2385 |
| 🔒 POST `/internal/events/batch-refresh` · `/broadcast` · `/file-processed` | `internal_event_*` | **워커/워처 → 웹서버 브로드캐스트 위임 (경계 계약)** — 수신부는 `total_log_count`(실건수) 우선 + `MAX_NOTIFY_CREATED_LOGS` 방어 절단(인시던트 `cc57b64`). [P1] batch-refresh는 msg 재구성 시 `total_log_count` 동봉(~3808 — 체인 passthrough 경로와 대칭화), broadcast는 `file_ingestion_progress`를 레지스트리에 인터셉트(~3831), file-processed는 레지스트리 제거 인터셉트(~3862 핸들러). **[`90e284f`] 게이트 추가 — `/internal`이 `/admin`과 같이 묶인 이유는 `broadcast`가 임의 dict를 전 WS 클라이언트에 중계하고 audit_cache에 주입하기 때문**(읽기 전용 admin은 잠그고 이건 열어 두는 것이 거꾸로였다) | ~3783–3902 |
| 🔒 POST `/internal/events/ingestion-state` | `internal_event_ingestion_state` | **[P1]** watcher → 진행 스냅샷 push(QUEUED/PROCESSING/FINISHED — heavy 파일만 명시 통지). **WS 브로드캐스트 없음** — 레지스트리 전용 내부 이벤트 | ~3904 |
| GET `/admin`·`/admin.html` | `serve_admin_page` | **어드민 HTML 자체는 게이트 없이 서빙된다** — 페이지가 떠야 토큰을 물어볼 수 있다. `test_admin_auth.PUBLIC_ADMIN_PATHS`가 **이 둘만** 면제로 허용한다(이름으로 고정) | ~4138 |
| GET `/map-editor`·`/enrichment` | `serve_map_editor_page` / `serve_enrichment_page` | 정적 페이지 서빙 | ~4155/4172 |
| GET `/{file_name:path}` | **`serve_static_or_index`** | SPA catch-all. **[`90e284f`] 격리(containment) 경계** — 아래 §1.4-bis. **catch-all이 파일 최하단인 것이 계약** — `/health`가 이보다 위에 등록돼야 한다(§1.1-bis) | ~4186/4188 |
...
| 구간 | 라인 | 내용 |
|---|---|---|
| 접두어 목록 (`tables`/`ws`/`audit_logs`/`dashboard`/`admin`/`map-editor`/`map_editor`/`map-presets`/`enrichment/`/`api`) | ~4193–4203 | **API 섀도잉 방지장치이지 보안 경계가 아니다** — 소스 주석이 그렇게 명시한다. 경로 **시작**만 보므로 `../../server/config/table_config.json`은 `admin`과 조금도 닮지 않아 그대로 통과한다 |
| **격리 검사** | **~4219–4223** | `dist_base = os.path.abspath(client2_dist_path)` → `target_path = os.path.abspath(os.path.join(dist_base, file_name))` → **`target_path`가 `dist_base` 자신이거나 `dist_base + os.sep`로 시작하지 않으면 거부.** `_resolve_admin_script_path`와 **같은 모양**이다 |
| 서빙/폴백 | ~4225–4231 | 통과한 실파일만 `FileResponse`, 그 외 `index.html` |
...
- **거부는 403이 아니라 404다**(~4150 주석) — 정적 라우트가 "그 탈출 경로는 파싱됐다"고 확인해 주면 안 된다.
...
| 메서드 경로 | 핸들러 | 역할 | 라인 |
|---|---|---|---|
| (상수) | `GRAPH_NEIGHBOR_NODE_CAP=500` / `GRAPH_LABEL_LIST_LIMIT_CAP=200` / `GRAPH_TRACE_NODE_CAP=1000` / `GRAPH_TRACE_DEPTH_CAP=3` 등 | 하드캡(C-7 무제한 로드 금지) | ~2035–2040 |
| (헬퍼) | `_escape_like_term(term)` | LIKE 메타문자 이스케이프 | ~2044 |
| (헬퍼) | `_expand_graph_subgraph(db, seed_nodes, depth, node_cap, edge_types=None, time_from=None, time_to=None)` | 뷰어/추적 **공용 BFS 코어** — 방향별 (from,type)/(to,type) 인덱스 2쿼리, 홉·방향당 엣지 페치 캡 2000, 노드 500청크 IN, 캡 절단 시 dangling 엣지 제외 | ~2049 |
| (헬퍼) | `_serialize_graph_nodes(nodes)` | 노드 `{id,label,identity_key,props}` 직렬화 | ~2142 |
| GET `/graph/stats` | `get_graph_stats` | label/edge_type GROUP BY 카운트 + last_sync | ~2151 |
| GET `/graph/neighbors` | `get_graph_neighbors` | k-hop(1\|2) 서브그래프 — `_expand_graph_subgraph([center])` 위임, truncated | ~2176 |
| GET `/graph/nodes/search` | `search_graph_nodes` | identity 시작일치 ILIKE 자동완성(limit 캡 50) + **빈 q + label = 라벨 전체 리스팅**(identity 오름차순, limit/offset, 캡 200. 전 테이블 덤프 금지 유지) | ~2211 |
| (헬퍼) | `_parse_trace_time(value, field)` | ISO 8601 파싱(`Z` 허용), 실패 시 400 | ~2271 |
| POST `/graph/trace` | `post_graph_trace(req: GraphTraceRequest, db)` | **[G2]** 멀티 시드 BFS 합집합 — 시드 순서보존 dedup→(label,identity) 인덱스 조회→missing_seeds 분리→공용 BFS. depth 1..3, 시간·타입 필터, 의미 검증 400 | ~2284 |
| GET `/graph/mapping-summary` | `get_graph_mapping_summary` | `ontology_config.load_ontology_mappings(known_tables=crud.TABLE_CONFIG)` — materializer와 동일 신호원, 요청 시 디스크 로드 | ~2361 |
...
| `bulk_upsert_cell_sources(db, mappings)` / `bulk_upsert_cell_overwrites(db, mappings)` | 메타 테이블 벌크 업서트(ON CONFLICT) | ~443/475 |
| `bulk_delete_cell_overwrites(db, delete_keys)` | overwrite 벌크 삭제 | ~508 |
| `_get_or_create_row(db, table_model, update_item, row_cache, table_name) -> (row, is_new)` | row_id/비즈니스키로 행 확보(캐시 활용) | ~524 |
| `_update_row_business_key(row, key_col, update_item, row_cache)` | 비즈니스 키 갱신 | ~560 |
| `_load_metadata_row_cell(...) -> (sources_list, overwrite)` | 셀 메타 로드(캐시·업서트 큐 연동) | ~580 |
| `apply_row_update_internal(db, table_name, update_item, row_cache, sources_cache, overwrites_cache, transaction_id, logs_to_cache, cell_sources_to_upsert, cell_overwrites_to_upsert, cell_overwrites_to_delete, deleted_row_ids) -> (row, is_new, changed_cols)` | **[통합 코어]** 단일 행 업데이트 + 레이어링 재계산. 모든 쓰기 경로가 여기로 수렴. 미선언 컬럼 드롭 지점 ~698–702 | ~644 |
| **`derive_replace_map_scope(table_name, batch) -> dict\|None`** | **[gate4 `deed6d2` 신설] replace_map purge 스코프의 단일 원천(순수 함수)** — DELETE 필터와 응답 echo가 **같은 이 함수**를 불러 어긋날 수 없다. 해석 순서: ① `batch.scope`(명시 — **엄격 검증**: 미선언 컬럼·map-key 계약 밖·물리 부재·빈 값은 전부 `ValueError`. 필터 하나가 조용히 빠지면 DELETE가 **넓어진다**) ② `updates[0]` 유도(`map_key_columns` 선언 우선, 없으면 레거시 폴백 — 비좌표 컬럼 전부). **None 반환은 거부로 취급해야 한다**(빈 필터 = 전 테이블 삭제 또는 역사적 결함인 "아무것도 안 지운 200") | ~1127 |
| `apply_batch_updates(db, table_name, batch, replace_report=None)` | **배치 진입점** — tx 컨텍스트, 캐시 프리로드, 행별 코어 호출, 벌크 flush, outbox 발화. 반환 `(results, changed_cells, created_logs, deleted_row_ids)`(4-튜플 불변 — 워커/파서/테스트 호출부 보호). [P2] 워처가 이 함수의 commit에 오프셋 갱신을 동승시킨다. **`batch.replace_map`(purge 블록 ~1222–1273)** — `derive_replace_map_scope`가 정한 스코프의 기존 행을 클린 삭제 후 재기록(**차집합 계산 없는 집합 교체** 연산. 소비자: 맵 Push·DOE 저장). **[gate4] 스코프 유도 불가면 `ValueError`로 거부**(침묵 noop 폐지), **`replace_report` out-param**(~1201)에 `{filters, deleted}`를 채워 API 계층이 정직하게 echo. **명시 `scope` + 빈 `updates` = 그 스코프의 의도적 전량 삭제**(erase-all) | ~1200 |
| `create_empty_row(s)_batch(db, table_name, count, user_name)` | 빈 행 생성 | ~1393/1398 |
| `delete_row(db,...)` / `delete_rows_batch(db, table_name, row_ids, user_name)` | 행 삭제(+감사·메타 정리) | ~1442/1446 |
| `delete_cell_source_batch(db, table_name, cells, source_name)` | 소스 레이어 일괄 삭제 + 표시값 재계산 | ~1510 |
| `delete_cell_source(db, ...)` | 단일 소스 삭제(배치 위임) | ~1666 |
| `set_cell_manual_priority_batch(db, table_name, updates, source_name, updated_by)` | 수동 Pin 일괄(§크고 복잡 — 표시값 재계산·감사 포함) | ~1671 |
| `set_cell_manual_priority(db, ...)` | 단일 Pin(배치 위임) | ~2030 |
| `get_ontology_mapping()` / `check_needs_rollback(table_name, modified_cols)` | 그래프 보조 — v2 검증+enrichment 승격 적용 결과 캐시 / v2 매핑 인식 rollback 신호(v1 폴백) | ~2039/2081 |
...
>
> ⚠️ **앵커 재측정 (2026-07-28, HEAD `0c6ac1a`)** — [Flatten] 도입으로 파일 1,764 → **2,086줄**, `_classify_lane` 이후 앵커 전부 **+312** 이동(그 앞은 +1~+36). 이전 재측정(2026-07-27, `90e284f`)의 교훈은 유효하다: 앞판은 `be58210` 실측이었고 그 뒤 이 절이 밀린 채 방치됐었다(+53) — 커밋 diff를 따라가는 갱신 방식으로는 영원히 안 잡히는 종류의 드리프트다.
>
> 🫀 **진척 비트가 "루프"가 아니라 "작업 단위"에 걸린다** — `process_with_retry`(~1051)와 `process_archived_file_sync`(~1317)는 **얇은 래퍼**이고, 각각 `heartbeat.work_claim(HEARTBEAT_NAME, …)`(~1062/~1329)로 감싼 뒤 실제 본체 `_process_with_retry`(~1066)·`_process_archived_file_sync`(~1334)에 위임한다. 파일 1건의 인제션 전체가 하나의 claim이고, 그 안에서 찍히는 비트가 claim의 진척을 갱신한다([§5 `heartbeat.py`](#5-소형-서버-모듈)의 `DEFAULT_STALL_AFTER_SEC`). **`_` 접두 본체를 직접 부르면 claim 없이 돌아 `/health`가 그 인제션을 보지 못한다.**
...
- 테스트: `tests/test_workspace_config_deprecation.py`(21개) · `tests/test_heavy_lane.py`(27개, `hvy_test_*`) · `tests/test_ingestion_checkpoint.py` · **`tests/test_flatten_nested_dirs.py`(15개, `0c6ac1a` 신설 — 결함 주입 포함)**.

| 시그니처 | 역할 | 라인 |
|---|---|---|
| `load_global_table_config() -> dict` | table_config.json 로드 (`paths.config_path` ~80) | ~75 |
| `warn_legacy_workspace_config(config_path)` | 레거시 config.json 발견 시 경로당 1회 deprecation WARNING | ~93 |
| `_log_alias_conflict_once` / `warn_invalid_std_parse_once` | 별칭 충돌·std_parse 비-bool 경고 dedup(키별 1회 — QA D5/D6) | ~117/125 |
| `DEFAULT_HEAVY_FILE_MB=10` / `INGESTION_SETTINGS_PATH` | [P1] heavy 임계 기본값·설정 파일 경로 — `paths.config_path("ingestion_settings.json")`(`.sample` tracked) | ~143/145 |
| `load_ingestion_settings()` / `warn_invalid_heavy_threshold_once` / `get_heavy_threshold_bytes()` | [P1] 임계 로더 — **파일 이벤트(라우팅 결정)당 1회 디스크 읽기**(파일 경계 핫리로드), 양수만 유효·그 외 기본 10MB+1회 경고 | ~148/162/174 |
| `DEFAULT_DEDUP_BY_SIGNATURE=True` / `DEFAULT_RESUME_FROM_CHECKPOINT=True` / `_bool_setting(key, default)` | [P2] dedup·재개 기본값과 설정 판독기(같은 `ingestion_settings.json`) | ~188/189/192 |
| `dedup_by_signature_enabled()` / `resume_from_checkpoint_enabled()` | [P2] 게이트 — `dedup_by_signature: false`가 **전역 강제 재처리 스위치**(파일명 `__force__`와 관리자 재시도가 나머지 2경로) | ~207/215 |
| **`DEFAULT_FLATTEN_NESTED_DIRS=True` / `FLATTEN_SEP="~"` / `FLATTEN_STABILITY_INTERVAL_SECONDS=1.0` / `FLATTEN_STABILITY_MAX_WAIT_SECONDS=600` / `FLATTEN_DISCARD_NAMES`** | **[Flatten `0c6ac1a`]** 상수군 — 구분자가 `__`가 아니라 `~`인 이유는 `__force__` 토큰 조작 차단(커밋 메시지·주석 명시). 정온 폴 1s·최대 대기 600s(초과 시 존치+스윕 재시도). 잡파일 3종+`._*` | ~227/233/239/242/246 |
| **`flatten_nested_dirs_enabled()`** | [Flatten] 토글 판독(기본 true) — 트리거(디렉토리 이벤트/스윕)당 1회 읽기, 핫리로드는 "다음 폴더부터" | ~249 |
| `get_workspace_serial_lock(workspace_path) -> Lock` | [P1] **워크스페이스 직렬화 락 — 모듈 레벨 경로 키 레지스트리**(핸들러 복수여도 공유). heavy 워커/인라인/run_watcher 재처리 폴러가 공용 | ~264 |
| `class HeavyIngestionLane` — `submit/_ensure_running/_worker_loop/stop` | [P1] FIFO `queue.Queue` + 데몬 워커 스레드 `watcher-heavy-lane` **1개**(첫 제출 시 지연 기동). WorkspaceWatcher가 1개 생성해 전 핸들러 주입. heavy끼리는 직렬(escalation §6-3) | ~274–326 |
| `find_workspace_alias(folder_name, table_config) -> str\|None` | 폴더명↔`workspace_name` 명시 별칭 매칭 — 섀도잉·중복 선언 별칭은 무효+ERROR 1회(QA D3) | ~328 |
| `resolve_workspace_root(base_dir, table_name, table_config) -> str` | 테이블→워크스페이스 루트 **역조회 공용 함수**(별칭 포함) — 결과 기반 경로 검사(base 직속 자식만, 드라이브 상대경로 탈출 차단, QA D2). main.py `retry-failed`·run_watcher 폴러가 사용 | ~369 |
| `resolve_workspace_table(folder_name, table_config) -> str\|None` | 폴더→테이블 해석: 별칭 > 폴더명 규약 | ~402 |
| `_register_legacy_import_shim()` | 구식 사용자 파이프라인 스크립트의 import 호환 shim | ~416 |
| `class IngestionHandler(FileSystemEventHandler)` | **워크스페이스 1개 담당 핸들러** — 생성자(~494) 말단 kwargs `on_ingestion_state_callback`/`heavy_lane`(기본 None=종전 인라인 경로, 하위호환). **[Flatten] `_flattening_dirs` 집합(~523, `_processing_lock` 보호)** — 같은 트리에 이벤트+스윕이 겹쳐도 flatten 1회 | ~490 |
| ├ `_load_legacy_config()` | [deprecated] 레거시 워크스페이스 config.json 파싱(이것만 캐시) | ~525 |
| ├ `_resolve_table_name(global_cfg)` | 테이블명 해석: 글로벌 `workspace_name` 별칭 > 레거시 `table_name` > 폴더명 규약 | ~547 |
| ├ `_snapshot_table_context() -> (t_name, table_info)` | **파일당 1회 config 스냅샷**(QA D1) | ~562 |
| ├ `_std_parse_enabled_for(t_name, table_info) -> bool` | std_parse 게이트: 글로벌(JSON bool만 유효) > 레거시 폴백 > 기본 true | ~573 |
| ├ `table_name` / `std_parse_enabled` / `errors_path` (property) | 즉석 해석 래퍼 — **글로벌 조회 비캐시**(핫리로드 반영) | ~593–606 |
| ├ `on_created/on_moved` | 이벤트 수신 — **[Flatten] `is_directory`면 `request_flatten`으로 분기**(observer `recursive=False`라 raws/ 직속 자식만 발화), 파일이면 `_handle_event` | ~608/616 |
| ├ `_handle_event(file_path)` | 파일 처리 진입(processing_files check-then-add 락 원자화) → [P1] `_route_and_process` 위임 | ~624 |
| ├ **`raws_path` (property) / `request_flatten(dir_path)`** | **[Flatten] 진입점** — raws/ 직속 자식만(결과 기반 경로 검사), 토글 게이트, `_flattening_dirs` 멱등 가드 후 **전용 단명 데몬 스레드**로 정온 대기(observer 디스패치 스레드 비차단 — P1 HOL 규율과 동일) | ~655/658 |
| ├ `_flatten_worker(abs_dir, key)` | 예외 격리 래퍼 — 실패 시 폴더 존치 + ERROR(주기 스윕이 재시도), finally에서 가드 해제 | ~693 |
| ├ `_snapshot_tree(abs_dir)` (static) / `_wait_tree_quiescent(abs_dir)` | 트리 스냅샷 `{(kind, relpath): (size, mtime)}` — 스윕의 (mtime,size) 시그니처를 트리로 일반화, stat 불가 파일은 never-equal 마커 / **1s 간격 동일 스냅샷 2회 = 정온**(복사 중 폴더를 반쯤 평탄화하지 않는 근거), 최대 600s 초과·소멸 시 False | ~707/732 |
| ├ `_is_discardable_system_file(name)` / `_sanitize_flatten_component(comp)` | OS 잡파일 판정(Thumbs.db/desktop.ini/.DS_Store/`._*`) / 접두 성분 정화 — 경로 적대 문자 치환 + **`__force__` 토큰 중화 루프**(치환 결과가 주변 `_`와 재조합되는 것까지 반복 제거) | ~759/766 |
| ├ `_build_collision_name(comps, basename)` / `_resolve_flatten_dest(raws_root, comps, basename, claimed)` | 충돌명 = 상대경로 접두 `~` 조인 — `user(<name>)` 업로더 토큰은 앞으로 재게양(`_extract_user_from_filename`의 startswith 계약 보존) / **무덮어쓰기 목적지 선정**: 원명 > 접두명 > `~2..~999`, `claimed`는 같은 배치에서 먼저 승격·아카이브된 동명 파일과의 재충돌 방지 | ~780/796 |
| ├ **`_flatten_directory(abs_dir)`** | **[Flatten] 본체** — 정온 대기 → mtime 오름차순 수집(스윕과 같은 순서 규칙) → `os.rename` 승격(`os.replace`/`shutil.move` 아님 — Windows에서도 무덮어쓰기) → 잡파일 폐기 → 빈 폴더 bottom-up **`os.rmdir`만**(비어 있지 않으면 실패하는 것이 곧 "내용물 있는 폴더는 절대 삭제 안 됨" 보증) → 승격 파일을 `_handle_event`로 명시 디스패치(watchdog 중복 이벤트는 processing_files 가드로 무해) | ~819 |
| ├ `_classify_lane(abs_path)` / `_heavy_backlog_nonzero()` | [P1] 이벤트 시점 `os.stat` 1회 크기 분류 / 워크스페이스 heavy backlog 잔여 확인 | ~920/934 |
| ├ `_route_and_process(abs_path, uploader) -> bool` | [P1] **레인 라우팅 본체** — heavy(크기)·backlog(>0이면 크기 무관 큐 후미=FIFO 보존)·인라인은 직렬화 락 **논블로킹 try-acquire**(실패 시 큐 후미 재라우팅 — HOL 방지+순서 보존 동시 만족) | ~938 |
| ├ `_submit_to_heavy_lane(abs_path, uploader, lane, size_bytes)` | [P1] 큐 제출 — QUEUED 통지를 **submit 이전 선발신**(드릴 결함1: 즉시 픽업 역전 경합 제거), submit 실패 시 FINISHED 정리 통지 후 인라인 폴백. `lane`은 분류 실값(재라우팅 소형은 "normal" — QA F4) | ~972 |
| ├ `_run_lane_job(...)` / `_notify_ingestion_state(state)` | [P1] heavy 워커 잡 본체(직렬화 락 획득→`process_with_retry`→finally 정리) / 상태 push 콜백 래퍼 | ~1010/1040 |
| ├ **`process_with_retry(file_path, uploader, retries=3, delay=1.0)`** | **`heartbeat.work_claim`(~1062) 래퍼일 뿐** — 실제 처리는 `_process_with_retry`(~1066) | ~1051 |
| ├ `_process_with_retry(...)` | 처리 본체 — 스냅샷→파싱→[P2] 시그니처 계산(~1084)→dedup skip→`_plan_checkpoint`(~1106)→`_send_to_upsert`→`_finalize_checkpoint`→아카이브/에러 이동, 재시도 | ~1066 |
| ├ `_compose_detail(skipped_no_key, plan)` (staticmethod) | [P2] 완료 통지 `detail` 조립 — 키 결측 스킵 수 + 재개/재시작 사유 | ~1161 |
| ├ `_try_dedup_skip(file_path, basename, t_name, signature) -> bool` | [P2] 동일 시그니처 `DONE`이면 skip — **무음 skip 금지**: WARNING + archive + `FileIngestionLog(status="SKIPPED")` + 콜백 status는 `"SUCCESS"`(수신부가 비-SUCCESS를 실패로 렌더링하므로 오표기 방지) + 사유 detail | ~1170 |
| ├ `_plan_checkpoint(...)` / `_finalize_checkpoint(plan, processed_rows)` | [P2] `ingestion_checkpoint.plan_ingestion` 게이트 래퍼(실패 시 `CheckpointPlan.disabled(note=...)`) / `mark_done` — 실패 시 "dedup will not apply" 경고 | ~1224/1250 |
| ├ `_log_ingestion_record(...)` / `_log_ingestion_failure/success(..., t_name=None)` | FileIngestionLog 기록(직접 DB, 스냅샷 테이블명). `error_message`는 SUCCESS/SKIPPED에서 **detail 슬롯**으로 겸용 | ~1263/1288/1293 |
| ├ `_retry_should_restart(t_name, signature) -> bool` | [P2] 재시도 시 완료 체크포인트가 있으면 처음부터 재시작 판정 | ~1301 |
| ├ **`process_archived_file_sync(log_entry, db, uploader)`** | 어드민 재처리 경로. **이것도 `work_claim`(~1329) 래퍼**이고 본체는 `_process_archived_file_sync`(~1334) — 스냅샷 진입점, 내부에서 락 안 잡음. [P2] 체크포인트는 태우되 **dedup skip은 미적용**(재시도는 명시적 의도) | ~1317 |
| ├ `_move_to_err_folder` / `_archive_file` | 파일 이동 | ~1377/1405 |
| ├ `_discover_and_execute_pipeline(file_path, meta=None) -> list[dict]\|None` | 사용자 파이프라인 스크립트(pipeline_*.py) 탐색·실행 | ~1432 |
| ├ `_resolve_rows(file_path, t_name=None, table_info=None, ...)` | **파서 라우팅** — 파이프라인 우선, 없으면 std parser 폴백(스냅샷 인자 전파). `source_kind`(`"std"` / `"pipeline:<Class>"`)의 산출처 | ~1526 |
| ├ `_try_std_parse(file_path, t_name, table_info)` / `_extract_user_from_filename(filename)` | std_parser 호출 래퍼(게이트·에러 처리) / 파일명에서 업로더 유도 | ~1564/1594 |
| └ `_send_to_upsert(rows, uploader, filename, total_rows, t_name=None, table_info=None, checkpoint=None)` | list 또는 스트리밍 이터레이터 → 청킹 → `crud.apply_batch_updates` 직접 호출 + 진행률 콜백. [P2] `checkpoint`로 `resume_from` 스킵(~1661)·오프셋 초과 경고(~1668)·**청크마다 `record_chunk_progress`(~1727, 같은 트랜잭션)**, created_logs는 `MAX_NOTIFY_CREATED_LOGS` 잔여분만 누적(~1739) | ~1605 |
| `class WorkspaceWatcher` | 전체 워크스페이스 관리자 — [P1] `HeavyIngestionLane` 1개 생성(~1780)·전 핸들러 주입 + `on_ingestion_state_callback` 배선 | ~1772 |
| ├ `_provision_workspaces()` | 폴더 스캐폴딩 — **config.json 신설 중단**(폴더만 보충), `workspace_name` 별칭 폴더명 지원(unsafe 별칭 무시) | ~1800 |
| ├ `_register_workspace(raws_root, table_config)` | 핸들러 등록(+`handlers_by_raw_path` 레지스트리, `heavy_lane` 주입) — 레거시 config 발견 시 1회 경고(QA D4) | ~1829 |
| ├ `discover_and_watch()` / `sync_new_workspaces()` | 기동 스캔·신규 워크스페이스 동기화(신규 raws는 등록 직후 스윕) | ~1888/1904 |
| ├ `sweep_existing_files(raw_paths)` / `_sweep_safely` / `sweep_existing_files_async(...)` | **[Startup Sweep]** raws/ 직속 기존 파일을 mtime 오름차순으로 `_handle_event` 경로 재사용 처리 — [P1] 스윕도 자동으로 heavy 라우팅을 탐. (mtime,size) 시그니처로 무한 재시도 차단, err/ 형제 폴더 제외. **[Flatten] raws/ 직속 디렉토리는 스윕 후보가 아니라 flatten 트리거** — `handler.request_flatten(fp)`(~1975)로 넘긴다(다운타임 중 떨어진 폴더·유실 이벤트의 안전망) | ~1934/2005/2011 |
| ├ `_periodic_sweep_loop()` / `_ensure_periodic_sweep_running()` | 이벤트 유실 안전망 — 300s 주기 잔류 재스캔 데몬(**flatten 재시도 경로이기도 하다** — 잠긴 파일·미정온 트리) | ~2020/2024 |
| └ `_ensure_observer_running()` / `stop()` / `start(blocking)` | watchdog Observer 수명 관리 — start()가 observer 기동 후 기동 스윕+주기 스윕 킥, stop()이 heavy 레인도 정지 | ~2033/2049/2055 |
...
### `server/bonding_plan.py` (~499줄, **`1fefd12`로 +57**) — [본딩 M1] 역할 바인딩 config 로더 + 집계 코어
...
| `get_core_summary(db, lot, slot, rects=None, config=None) -> dict` | **집계 진입점** — 역할별 카운트(맵 모드 fail_values 필터, used_chips distinct), `remaining = total − defect − eds_fail − used`(음수 가능 — 과도기), history 50건+warnings, region 교차(좌표 하드캡 100k, 응답 미포함). 좌표 정렬은 `map_overlay.resolve_map_transform`(~340) + `map_overlay.align_status_label`(~353) 위임. **[`1fefd12`] `fail_values` 선언 + `val` 미해석이면 필터 없는 카운트를 거부**하고 0 + 강등 status(~361 — 전 행을 fail로 세는 반대 방향 오염 차단, align_unavailable과 같은 규율) | ~274 |
...
### `server/map_overlay.py` (~784줄, **U6 `95bf072`로 +38 · F1/F2 `17f65bd`로 +51**) — [M2 신규] 범용 맵 오버레이 (계획 전용 아님 — 맵 인프라)
...
| `_align_summary(rotation, flip)` / `align_status_label(align)` | 표시용 요약 dict(변환에는 안 쓰인다) / 상태 문자열 마커 `aligned:180` 등 — **`bonding_plan`에서 이관**(변환 소유 모듈이 마커도 소유). 소비자: `bonding_plan.get_core_summary` ~353 · `transfer_plan._canonical_fail_set` ~1082 | ~324/335 |
...
### `server/transfer_plan.py` (~3,170줄, **`1fefd12`로 +56 · `deed6d2`로 +34**) — [M2] Universal Transfer Plan 엔진 (v2 = 계획 정체성이 곧 맵 정체성, **`b35bc9f` zone 모델 · `2baf9ff` U9 marker · `1fefd12` count_only/column_unresolved 강등 · `deed6d2` self-frame fail count_only**)
...
>
> **수량은 저장되지 않고 유도된다** — `zone_layers`(1H=1층·TOP=1층·MID=`stack−이웃 수`) × `painted(값)` = `zone_demand`, 자재당 = `ceil(total / n)`(**배분이 아니라 충분성 검사**). 그래서 페인팅 분포 읽기(`_painted_values`)가 **load-bearing**이고, 실패 시 모든 required가 0이 되어 부족이 영원히 발화하지 않는다 → `painted_reliable`(~2829)이 유도 전체를 게이트한다.
>
> **[`1fefd12`] 유령 잔여(phantom remaining) 수정 — 강등 어휘 2종 추가**: ① **`connected(count_only)`** — `transfer_log`가 x/y 없이 바인딩되면 카운트는 실값이되 칩 정체 미상이라 집합 감산이 불가능하다(used_set이 비어 `remaining`이 과대). 강등 엔진이 remaining을 null + 상한(upper bound)으로 내리고, `by_core`의 used/remaining도 **log·area_map 양 경로 모두 null**(가짜 0 금지 — `used_count_only` 플래그 ~1188/~1211). ② **`connected(column_unresolved:<roles>)`** — 선언-미해석 컬럼(config 오타)의 침묵 제거, 기계장치는 `bonding_plan._ResolvedColumns` 공유(`_demote_unresolved`/`_unresolved_of` ~271/279가 위임 래퍼). `fail_values` 선언 + `val` 미해석은 필터 없는 카운트를 **거부**(0 + 강등 — 감산항 과대는 상한 불변식 위반).
>
> **[`deed6d2`] 유령류(phantom class) 마감 — `used_count_only`의 형제 `fail_count_only`(~1250)**: origin_rows(집합 감산) 경로에서 **self-frame fail 원천이 x/y 없이** 바인딩되면 `fail_breakdown`엔 카운트가 실리되 `fail_union`엔 아무것도 안 실려 감산이 그 칩들을 조용히 놓친다(remaining 과대 — 같은 유령류). 판정 지점 ~1321: `connected(count_only)`로 강등해 remaining을 null + 상한으로 내린다. **cnt를 대신 빼지 않는 것이 의도** — 칩 정체 미상이라 used_set·타 fail 원천과 겹칠 수 있고, 과감산은 상한을 **아래로** 깨뜨린다(놓친 점은 union을 줄일 뿐이라 `total − |union|`은 진짜 잔여의 상한으로 유지된다). 폴백 경로(origin_rows 없음 — 카운트 감산)는 좌표 없이도 올바르므로 **강등하지 않는다**(~1325 주석). `by_core` used/remaining null은 ~1432–1433/~1479–1480. 테스트: `tests/test_transfer_plan.py` count_only §에 self-frame 4건 추가(~2702–2760).
>
...
>
> **좌표 변환 사본 없음** — 이 모듈은 `map_overlay.resolve_map_transform` **하나만** 쓴다(~1076). **zone 산술·규칙은 클라 `doe_bands.js`와 공유 벡터로 고정** — [§6-2 `contracts/doe_band_rules/`](#6-2-교차-구현-계약-contracts). 레거시 `bands` 산술(`to` 3상태, prevTo 걷기)은 종전대로 [`contracts/band_arithmetic/`](#6-2-교차-구현-계약-contracts).
...
| `_resolve(src_cfg, required)` / **`_demote_unresolved(status, cols)` / `_unresolved_of(cols)`** / `_binding_status(...)` / `_stage_role_statuses(stage_cfg)` / `_plan_store_statuses(cfg)` | 바인딩 → (model, 컬럼맵 — `bonding_plan._ResolvedColumns`) / **[`1fefd12`] `column_unresolved` 강등 위임 래퍼 2종**(기계장치는 bonding_plan) / `connected`\|`missing`(+미해석 마커 합성) / stage 역할별·plan_store 역할별 상태(`registry` + `material_identity` 고정 + `source_region` 선언 시에만) | ~263/271/279/289/296/327 |
| **`stage_of_table(cfg, ref_table)`** | **[v2 핵심] `stages.*.target_map.table` 역인덱스** — 열린 테이블에서 stage를 유도한다 | ~356 |
| `list_stages(cfg)` | `GET /api/transfer-plan/stages` 응답 `{stages[], plan_store}` | ~373 |
| `_status_is_degraded` / `_degradation_effect(role, fail_roles)` / `assess_degradation(statuses, fail_roles)` | **[QA F1 1층]** 역할 강등 탐지 → `(경고 리스트, remaining_reliable, total_reliable)`. **[`1fefd12`] `connected(count_only)`·`connected(column_unresolved:…)`도 강등으로 판정** | ~394/410/424 |
| `build_chips_block(...)` | **[QA F1 3층]** chips 블록 조립 + **음수 remaining 불변식**. 신뢰불가면 `remaining: null`(오표시 구조 차단), `total_reliable ∧ remaining≥0`일 때만 `remaining_upper_bound` | ~463 |
| `load_source_region(...)` / `_region_block(...)` / `_core_region_counts(...)` | 소스 사용 영역 로드(**현재 휴면**) / 영역 내 집계 / core-kind 어댑터 | ~504/538/564 |
| **[BIN 축 `269b39e`]** `parse_bin_request(raw)` / `_bin_axis_binding` / `_bins_unavailable` / `_bin_universe` / `_bin_cell_sets` / **`_bins_block(...)`** / `_merge_bins_over_slots` / `_lot_slots` | `bins=` 파라미터 파싱 / stage의 BIN 축 바인딩 / 불가 블록(`bin_axis_unavailable`) / 맵의 distinct BIN 열거(캡 200, ORDER BY로 재현성) / BIN별 좌표 집합 / **BIN별 total·fail·used·remaining 분해**(requested BIN은 전부 답을 받는다 — 부재는 `bin_absent`, 절대 0 아님) / `scope=lot` 슬롯 합산 / 로트→슬롯 전개(대장 조회, 캡 50) | ~646/668/685/695/730/757/841/920 |
| `_reshape_m1_summary(m1, stage_name, stage_cfg)` | M1 `bonding_plan.get_core_summary` 응답을 M2 공통 형태로 재성형 | ~954 |
| `_fetch_pairs(...)` / `_origin_map_id(...)` | 좌표쌍 페치(캡 적용) / origin map_id 조립 | ~990/1006 |
| `_canonical_origin_meta(...)` / `_canonical_fail_set(...)` | origin-frame 원천의 canonical 맵 메타(첫 원천이 기준, 메타 없으면 None — 뒤로 넘어가지 않는다) / fail 좌표를 `map_overlay.resolve_map_transform`(~1076)으로 canonical 프레임에 사상 | ~1012/1049 |
| `_collect_history(db, source_cfg, lot, slot)` | process_history 최근 N건 + result fail 경고 | ~1091 |
| **`_summarize_inline(db, stage_name, stage_cfg, lot, slot, region=None, bins=None)`** | **가용 엔진 정본(tape-kind)** — `origin_log` 연결 시 `remaining = total − \|fail_union ∪ used_set\|`, 미해석 시 M1식 감산 폴백. `by_core` 7키 + `by_core_origin` 마커 `"log"`\|`"area_map"`(`fail=None`으로 0 위장 금지). `bins` 요청 시 `_bins_block` 동봉. **[`1fefd12`] `transfer_log`가 x/y 없이 바인딩되면 `connected(count_only)` 강등**(`used_count_only` ~1188/~1211) — `transferred` 카운트는 유지하되 remaining은 null+상한, `by_core` used/remaining은 **양 경로 모두 null**(가짜 0 금지). **[`deed6d2`] self-frame fail 원천의 x/y 부재도 같은 강등**(`fail_count_only` ~1250/~1321 — 위 유령류 마감 문단). fail 소스의 `val` 미해석은 카운트 거부(0 + 강등) | ~1139 |
| `_bin_warnings(bins)` / `get_stage_source_summary(db, cfg, stage_name, lot, slot, bp_config=None, ref_table=None, map_key=None, bins=None)` / **`get_lot_bin_summary(db, cfg, stage_name, lot, bins=None)`** | BIN 블록 경고 승격 / **핸들러 진입점(scope=slot)** — 미선언 stage `KeyError`(→404) / **[`269b39e`] scope=lot 진입점** — 슬롯 전개 후 BIN 합산, **`chips` 없음**(로트 단위 헤드라인 잔여를 지어내지 않는다) | ~1582/1612/1665 |
| `_plan_store_binding(cfg, role, required)` | plan_store 역할 바인딩 (⚠️ `_num`은 삭제된 채 유지 — 저장 수치 파싱 없음) | ~1776 |
| **`_parse_bands(raw) -> (밴드[], 읽었는가, 거부된_원소_수)`** | **[레거시 `bands` 경로]** 3-튜플(호출부 `bands, readable, dropped = …` ~2740). **"못 읽음"과 "구간 없음"을 절대 합치지 않는다.** 객체가 아닌 원소는 거부하되 조용히 버리지 않는다(세어서 `layer_range_invalid reason:"not_a_band"`로 표면화). 크기 검사가 `json.loads`보다 **먼저** | ~1782 |
| `_band_seq(raw)` / `_band_materials(band)` / `_assign_band_seqs(bands)` | [레거시] 선언 `seq` 판독(정수값 float 허용 — `JSON.parse`가 먼저 접으므로 클라는 물리적으로 거부 불가) / 자재 목록 정규화(문자열 아님 거부) / 계획 내 `seq` 유일화 | ~1819/1851/1880 |
| `BAND_TO_OK/BLANK/INVALID` · **`STACK_MARKER`** · **`_int_state(raw)`** · `_band_to(band)` · **`_bin_of(raw)`** · `_prev_to(bands, i)` | 판정 어휘 3종(~1909–1911) + **[`2baf9ff` U9] `STACK_MARKER="marker"`(~1915 — STACK 전용 제4상태.** `_int_state`에서 절대 나오지 않고 **`stack_state`만 명시적 0을 승격**한다 — band `to`·BIN은 여전히 0 거부. 클라 `stackState`의 `'marker'` 미러) · **[`b35bc9f` 신설] 단일 정수 판독기** — `stack`과 `to`가 **같은 판독기**를 쓴다(클라 `bandToState` 미러. `'7.5'`→invalid — 컬럼 타입이 아니라 이것이 가독성을 결정) · `to` 판정(_int_state 위임) · BIN 라벨 판독 · prevTo 걷기(blank·invalid 동일 스킵, invalid는 보고) | ~1909–1915/1920/1962/1972/1991 |
| **[zone 코어 `b35bc9f`·`2baf9ff` — `doe_bands.js` 미러, 정본은 `contracts/doe_band_rules`]** `parse_material_list(raw)` / `_zone_tokens` / `stack_state(row)` / `mid_zone(row)` / `zone_layers(row, zone)` / `zone_demand(row, zone, painted)` / `parse_material_token(raw)` / **`material_pool_key(tok)`** / `validate_zone_plan(rows)` / `material_rollup_rows(rows, painted_of)` / `remaining_state(availability, used)` / **`bands_to_zones(bands)`** | mat_* JSON 배열 판독 / 행별 토큰 수집(비문자열 거부) / **`stack` 4상태(ok·blank·invalid·marker — 명시적 0만 marker, 음수는 invalid로 값 보존)** / MID 구간 유도(marker는 `{size:0, known:true}` — E행의 0층과 같은 "진짜 0") / 구역 층수(1H=1·TOP=1·MID=stack−이웃, **marker는 전 구역 `[]`**) / 구역 수요 = painted×layers / 토큰 → `{lot, slot?, bin?, scope}` 문법 / **풀 정체 키 — 분리자 조인이 아니라 튜플**(U+001F 조인이 디스크에서 분리자를 잃고 두 풀을 합산한 사고의 재발 방지) / **V1–V6 차단 규칙**(+W-DUP-MAT advisory — **marker 행은 V6 하나에만 답한다**: V6 블록 ~2315, V3 풀 스캔 제외 ~2396) / MAT×BIN 롤업 행(**marker 행은 부재** ~2453 — "사용 0" 행이 아니라 아예 없음) / 잔여 판정(신뢰 불가 가용은 잔여를 억제) / **레거시 band 계획 → zone 사상**(불가하면 `not_convertible`로 거부 — 손실 접기 금지) | ~2013/2039/2089/2113/2137/2160/2180/2233/2268/2435/2488/2512 |
| **`_material_identity_rule(cfg)` / `_split_material(material_id, rule)`** | 자재 ID → 소스 `(lot, slot)`. 규칙은 **`plan_store.material_identity` 선언으로만** 성립. 분해는 뒤에서부터(클라 `splitMaterialId`와 방향 동일, `map_overlay.build_key_filters`는 반대). **분리자가 없으면 거부** | ~2576/2595 |
| `_painted_values(db, ref_table, map_key, overlay_cfg)` | 대상 맵 자신의 셀 값 분포 group-by(`ORDER BY` — 절단 재현성). 반환 `({값: 셀수}, 상태, 절단여부)` — 세 번째 값이 [B2]의 핵심 | ~2628 |
| `validate_plan(db, cfg, ref_table, map_key, overlay_cfg=None)` | **핸들러 진입점** — **`plan_store.registry` 미구성은 `LookupError`(→404)**. 레지스트리 1회 조회(`ORDER BY value` ~2721 — 절단 재현성). **[zone] 행별로 `stack`/`mat_*`를 먼저 읽고**(내부 `_reg_get` ~2729, zone_row 조립 ~2747), zone이 비어 있으면 레거시 `bands`를 `bands_to_zones`로 사상(~2774 — `REGISTRY_LEGACY_ROLE` 미선언이면 그마저 없음). **`validate_zone_plan` 호출 ~2855가 V1–V6을 채점**하고 위반은 `zone_rule_violation`으로. marker 행은 수요 유도에서도 자연 소멸(`zone_layers`가 `[]` — 자재 해석·조회·수요 0, 주석 ~2971). **[B2] `painted_reliable`(~2829)이 유도 전체를 게이트.** `remaining_reliable=False`면 부족·fail 판정 전부 생략 + `availability_unreliable`만. 캡 절단은 `result_truncated`를 역할·캡별로 각각 발행. **`structural_refusal`(~2738, 세트 ~2756/2780/2812)이 `availability_checked`에 AND로 물린다**(~3131). 최종 `status`는 `ok`/`warnings`/**`unverified`**(~3153) 3값 — "검사 안 함"과 "이상 없음"을 같은 값으로 내지 않는다 | ~2678 |
...
| `server/database/schemas.py` (169줄) | Pydantic — `GeneralUpdateItem`(~76)/`GeneralUpdateBatch`(~83) 등 API·배치 계약. `GeneralUpdateBatch.replace_map`(~87) + **[gate4 `deed6d2`] `scope: Optional[Dict]`(~92)** — 명시 purge 스코프 `{column: value}`. 지정 시 유도 대신 이것을 쓰며(엄격 검증 — [§2 `derive_replace_map_scope`](#2-serverdatabasecrudpy--레이어링-코어)), **명시 scope + 빈 `updates`는 그 스코프의 의도적 전량 삭제** |
...
| `server/event_constants.py` (51줄) | 프로세스 간 내부 이벤트(`/internal/events/*`) 공용 상수 — `MAX_NOTIFY_CREATED_LOGS=500`(~14, 발신측 created_logs 절단 상한: 워처 `directory_watcher:1739` · 체인 워커 `chain_ingestion_worker:474` · 수신 `main.py:3802/3850` 공유 — 2026-07-28 gate4 패스 재측정) · [P2] `MAX_AUDIT_VALUE_CHARS=4096`(~22)과 `truncate_audit_value(value, max_chars)`(~25 — 반환 `(값, 절단여부)`, str은 `…[truncated: 총 N자]` 마커, dict/list는 타입·길이 플레이스홀더)를 `crud.create_audit_log`가 소비 |
...
### `ui.js` (~409줄) — 그리드 밖 UI 갱신
- export: `setupBeforeUnloadWarning`(~8) `updateSelectedCellUI`(~18) `updateTxModeUI`(~33) `setTransactionFilter`(~79) `applyValueToSelectedRange(newValue)`(~106, 범위 일괄 적용→배치 PUT) `updatePageCacheOnUpsert`(~260) `updateEnrichmentBadge`(~331 — **[`1fefd12`] 배지 카운트 필터도 `rule.queue_filters` 우선**(~350): 워크리스트·어드민과 세 수치가 어긋날 수 없다) `notifyEnrichmentTableEvent`(~382) `updatePageCacheOnDelete`(~392).
...
### `map_editor.js` (~5,830줄, **F1/F3 `17f65bd`로 +92 · gate4 `deed6d2`로 +73**) — 웨이퍼 맵 에디터 (단일 페이지 스크립트, export 없음 — 단 `import`는 있다: `transfer_plan.js`의 `bandToState`·**[U6] `stageTargetTables`**, **`doe_bands.js`의 `parseMaterialList`·`bandsToZones`**)
- **[U6 `95bf072`] 서빙된 맵 기본값 — 클라 사본 0**: `overlayContract`(~91 — paint-rules 응답에 실려 오는 `{valueColumnCandidates, defaultLegend}` 캐시, 첫 성공 fetch까지 null. **"미조회/불가"는 "자동선택 없음·빈 시드 1행"으로 동작**하고 절대 내장 목록이 아니다) · `EMPTY_DOE_SEED`(~232 — "선언 없음" arm의 빈 DOE 1행, 구 ~~`DEFAULT_LEGEND`~~) · `defaultLegendRows()`(~236 — 선언 > 시드) · `declaredLegendRow(value)`(~245 — 값 자동 추가 시 선언 행의 color/desc 우선) · **`LEGEND_PALETTE`(~2416 — 12색 사본 3곳 통합)** · `pickUnusedColor`(~2419) · **`autoAddLegendValue(value, fallbackDesc)`(~2428 — 값이 legend에 자동 추가되는 **유일 경로**: 패널 [+ 값]·E1/E2 autopaint·미지 값)**. `switchTable`(~1169)이 스키마 fetch와 병렬로 paint-rules 왕복을 **await**한다.
- **[F1/F3 `17f65bd`] 좌표 바인딩도 서빙받는다 — 클라 매칭기 0**: `servedBindingCache`(~102, 테이블별 Map) · `normalizeServedBinding`(~106) · `fetchServedBinding(table)`(~122 — 오버레이 소스용 단독 조회, 같은 paint-rules 엔드포인트. 실패는 throw·무캐시, "서버가 null이라 함"은 캐시). 컬럼 매칭은 **서버에서 1회**(`resolve_binding_info` — 선언 table_bindings > table_config 유도): 구 클라 측 대소문자 무시 x/y 매칭기·값 후보 매칭기(~~fix D의 정확 일치 포함~~)·~~`deriveMapBinding`~~은 **전부 삭제** — 같은 질문에 대한 2·3번째 매칭기가 서버와 어긋나는 것이 F3 결함이었고, 선언 바인딩(tx/ty·대문자 컬럼)은 어떤 클라 관례 매칭기도 자동선택 못 한다. `fillColumnDropdowns`(~1300)의 드롭다운 3종 preselect가 소비 지점(~1319–1334 — 서빙 바인딩 없으면 자동선택 없음, 드롭다운이 수동 탈출구). **[F2] `source:'fallback_guess'`는 경고**(~1339–1344 — 드롭다운 title + 토스트: 데이터 경로가 거부하는 추측을 사용자도 조용히 믿으면 안 된다).
- 좌표 변환 코어: `getPhysicalCoords`(~1388) `getCellFromPhysicalCoords`(~1438) `getCellFromVisualCoords`(~1478) `getVisualCoords`(~1547) `getTransformedPhysicalConfig`(~1562) `getWaferBoundingBox`(~1495) `getScreenShift`(~1597) `isCellInsideWaferFast`/`isCellInsideWafer`(~1623/1661) — 회전/면반전 불변식은 [MAP_EDITOR_SPEC](../spec/MAP_EDITOR_SPEC.md).
  - **[7d931dc] 프레임 창(frame window)** — `physFrameOverride`(~1361) + `physNum`(~1364)/`gridDimNum`(~1373)/`withPhysFrame(frame, fn)`(~1382). 변환 함수가 규격을 DOM에서 읽는 지점을 잠깐 갈아끼우는 장치로, **주입 지점은 `getTransformedPhysicalConfig`·`getWaferBoundingBox` 두 곳뿐**이다. `withPhysFrame`은 **동기 전용**(내부 `await` 금지 — `try/finally` 복원이 새면 조용한 오답). 기존 `parseFloat(v) || dflt` 규약(0 → 기본값) 보존.
- 캔버스 렌더: `renderGridCanvas`(~2013, 본체 — 오버레이 마커 호출 ~2175) `scheduleRenderGridCanvas`(~1976) `fitGridToWorkspace`(~1998) `updateNotchPosition`(~2357).
- 데이터 IO(REST — WS 아님): `loadExistingMap(opts={})`(~3500 — **[`280ebf0`] `{quiet}` 옵션**은 last-open 부트 복원용. **[`0052d76` fix G] 사용자 취소(맵 키 공란·선택 모달 dismiss)는 `{count:0, cancelled:true}` 반환**(~3522/~3664) — 호출자가 "미구축 맵"과 구분한다. **[`6db517d` H1] 로드 경로는 legend 채택 시점에 초안을 저장하지 않는다** — 구 `saveLegendToStorage()` 즉시 호출이 방금 로드한 **서버 상태**를 초안 위에 덮어써 리로드마다 칠한 셀 초안을 전멸시켰다(사유 주석 ~3838–3843). 저장은 초안 우선순위 판정이 끝난 뒤 registry 블록 안에서 **1회**만 — **단 [`0052d76` fix A] 낡은 초안(stale draft)을 보존 중이면 그 1회도 생략**: `staleDraftKept`(~3896 선언, ~3917 판정) 상태에서 persist하면 "초안은 지우지 않았습니다" 토스트가 방금 한 약속을 그 초안을 덮어쓰며 깬다 → `if (!staleDraftKept) saveLegendToStorage()`(~3932). **[`0052d76` fix B] 로드 성공은 `legendDirty = false`로 리셋**(~3860 — 맵 A의 미저장 칩이 맵 B로 새던 결함. 초안 복구가 이 리셋 **뒤에** 다시 세우므로 복구 초안은 여전히 미저장으로 표시 — 읽기 실패 분기도 초안 내용이 화면을 바꿨으면 ~3949에서 재마킹). 복구된 초안은 여전히 미저장 편집이므로 `legendDirty = true` ~3907. **[U6-1 `95bf072`] 0셀 로드는 registry 적용 전에 `seedEmptyDoe()`로 리셋**(~3881, `read.ok` 안에서만): `uniqueVals.size > 0` 가드가 페인트 유도 legend 재구성을 건너뛰어 **같은 테이블의 직전 맵 legend가 새 빈 맵에 상속**되던 결함 — 읽기 실패 시엔 화면 유지(미상 서버 상태는 "빈 것"이 아니다), 초안 우선순위 블록은 종전대로 그 뒤에 이긴다. **[`0052d76` fix C] 메타 없는 맵의 기본 선택지 = 데이터 전체 bbox + "마스크 없음"**(~3676–3690): 종전엔 좌패널 물리 규격(웨이퍼 원)이 기본 프레임 밑에 살아 있어 모서리 셀이 `inside:false`로 찍히고 H2 관문이 모든 Push를 거부했다(1293→379). inside 술어에 off 스위치가 없으므로 "마스크 없음"을 그 어휘로 표현한다 — chip 1×1·offset 0 + 격자 반(半)대각선을 외접하는 웨이퍼 지름을 `applyPresetObject`(기존 프리미티브)로 기입. 선택 모달 버튼 라벨도 정직화(~3619)) `pushMapData`(~4075, 저장 본체 — **[`b35bc9f`] legend/DOE 저장의 유일한 트리거이기도 하다**: 내부에서 `saveLegendToServer` 호출 ~4364. **[`6db517d` H2] 대비 관문(contrast guard) ~4225–4250**: 직렬화된 페이로드의 비어 있지 않은 셀 수 < 화면의 비어 있지 않은 셀 수면 **confirm 이전에 적재 거부** — `replace_map`은 맵 행 전체를 지우고 다시 쓰므로 프레임이 덮지 못한 셀은 누락이 아니라 **삭제**된다(QA 실측 1293→379, 메타 없는 맵 + 기본 프레임 — fix C가 1차 방어, 이 관문은 2선 유지). 계산은 루프와 같은 공백 술어라 사용자가 지운 셀은 양쪽에서 상쇄) `fetchGridMetaFor(table, mapId)`(~3467) 프리셋 `fetchAndRenderPresets`/`saveCustomPreset`/`deleteCustomPreset`(~1718/1824/1874) + `applyPresetObject`(~1774, `loadSelectedPreset` ~1807에서 추출한 공용 함수 — fix C의 규격 기입자).
  - **[gate4 `deed6d2`] 데이터 보호 관문 4호 — 로그형 적재 대상**: zone 컬럼 부재·레거시 판독 불가·대비 관문(H2)에 이은 **넷째**(같은 패턴 — 직렬화하지 않은 것을 지우는 쓰기를 막는다). 근접 사고(2026-07-28): `dt_log`를 맵으로 열고 ⚡ Push했다면 replace_map이 스코프 내 **실제 로그 행**을 에디터 조작본(키·좌표·값)으로 대체해 `dt_id`·`eventtime`·코어 lot/slot·`cx/cy`·`dt_eqp`가 전부 소실됐다. 장치: `PUSH_SYSTEM_COLUMNS`(~4028 — 서버 관리 컬럼 목록, 검색 컬럼 필터 ~1259와 **한 목록 한 답**) · `getUnprotectedPushColumns(schema, xCol, yCol, valCol)`(~4050 — 맵 계약(맵 키+X/Y/값+시스템) 밖 데이터 컬럼 열거) · `logShapedPushDecision`(~4069 — `{mode: 'clean'\|'confirm'\|'block', extras}`. **`schema.map_push_ok === true` 엄격 판정**: 서버 `/schema`의 site 선언([§1.2](#12-api-라우트-표--데이터-조회편집))이 있을 때만 hard block이 **1회 손실 인지 confirm**으로 강등 — R&D 수기 계측 덮어쓰기 같은 선언된 흐름용, 선언 제거는 재잠금) · 관문 본체 ~4076–4121(**모든 다이얼로그보다 앞** — 허용될 수 없는 Push에서 사용자가 질문 하나도 받으면 안 된다). 테스트: **`client2/tests/push_gate_harness.mjs`(147줄, `node client2/tests/push_gate_harness.mjs` — contracts 하니스와 같은 vm 추출 방식, 실서빙 스키마 fixture로 15 check: dt_id 축 명시 단언·부분 키·`"true"` 문자열 오타는 block 유지)** + 서버 측 `tests/test_schema_map_push_ok.py`(3건).
...
- 레전드/브러시: `renderLegendTable`(~3110) `selectBrush`(~3298 — **[`280ebf0` 입력 랙 수정] 선택은 DOE 리스트 재구성·전 그리드 카운트 스캔을 유발하지 않는다**(의도 주석 ~3312): mousedown이 innerHTML 재구성을 유발해 커서 밑 input이 분리되던 결함) + `saveLegendToStorage`(~2459). `getCurrentMapKey`(~2669)는 **로드된 맵이 아니라 현재 메타 입력 필드**를 읽는다(오버레이 관문 F2의 근원). 서버 레지스트리 IO는 아래 전용 블록 참조.
- **Split Registry = DOE의 유일한 기록자**(~260–560 선언 · ~2701–3108 IO). **[`269b39e`+`b35bc9f` ZONE 모델]** legend 행 = DOE 1건 = `{value, desc, color, knobs, stack, mat_1h[], mat_mid[], mat_top[]}` — zone 계약 정본 주석 ~291–311(`bands`는 **폐기됐지만 읽기 전용으로 생존**: 서버에 band 계획이 남아 있고 저장이 `replace_map`이라, 안 읽으면 다음 키 입력이 그 계획을 빈 집합으로 지운다). `SPLIT_REGISTRY_TABLE='map_split_registry'`(~260) · **`SPLIT_KEY_SEP='\|'`(~263)** · `buildSplitKey`(~288).
  - 정규화 순수 함수: `parseJsonCol`(~312) **`normalizeBands`(~318 — 하니스 검증 대상, 레거시 읽기 전용. `to` 해석은 `transfer_plan.js`의 `bandToState` **하나만** 쓴다. 읽을 수 없는 값은 **원문 그대로 보존**)** `normalizeKnobs`(~351) `knobsToObject`(~363) `serializeKnobs`(~372) `normalizeLegendItem`(~403 — zone 필드 포함) **`cloneLegend`(~424 — 깊은 복사. 배열 필드를 얕게 넘기면 프레임 스냅샷과 화면이 같은 배열을 공유한다)**. (~~`serializeBands`~~는 band 편집기와 함께 삭제)
  - 동시성 장치: **`legendReplaceScope`(~276 — `{table, mapKey, fingerprint}`. `replace_map` **권한**이자 동시성 검사의 **기준선**)** · **`legendVocabularySeed`(~280 — [`269b39e` 신설] 플레이스홀더 시드 시점의 행 서명 Map. `reconcileVocabClaims`가 "사용자가 이 행을 바꿨다"를 **유도**하게 한다)** · **`legendConflict`(~284 — 다른 세션이 바꿨다. 리로드 전까지 그 맵의 registry 쓰기 전량 차단. upsert 강등 금지)** · `legendSaveState`(~286). 지문 계산 `LEGEND_PAYLOAD_COLUMNS`(~449)/`canonRegistryRow`(~470)/`FP_UNIT`·`FP_ROW`(~477–478)/`registryFingerprint`(~479) — **`contracts/legend_map_scope` 하니스가 이 상수·함수들을 이름으로 추출**한다.
  - 페이로드 순수 함수: **`buildLegendRegistryUpdates(refTable, mapKey, legendArr, user, nowStr)`(~494)** / **`parseLegendRegistryRows(result, dedupeByValue)`(~543)**.
  - 서버 IO: `REGISTRY_SCOPES=['map']`(~2701 — **테이블 전체 어휘 시드는 삭제됐다**, `269b39e` 결함의 원인) `fetchRegistryRows`(~2702, `limit=500` — **부분 읽기는 읽은 게 아니라 throw**) `readRegistryScope`(~2727) `legendRowSignature`(~2738) `reconcileVocabClaims`(~2759) `applyRegistryRowsToLegend`(~2784 — 빈 legend 폴백도 **[U6] `defaultLegendRows()`** 경유) **`saveLegendToServer(mapKeyOverride)`(~2863 — 쓰기 단위가 **한 맵의 값 전체 집합**이라 `replace_map:true`. 지운 값·구간·자재는 집합에 없다는 것만으로 서버에서 삭제된다. 쓰기 **전에** ① `probeZoneColumns` 게이트(~2981, `ZONE_COLUMNS` ~2980 — 물리 zone 컬럼이 없으면 **거부**: crud가 페이로드에서 zone을 떨어뜨린 채 replace_map이 나가면 계획 전체가 층 구조 없이 대체된다) ② 재조회+지문 비교(불일치면 `legendConflict`로 거부). **참조는 정확히 2곳 — 정의 + `pushMapData`(~4364)**)** `LEGEND_SAVE_MESSAGE`(~3009) `applyLegendSaveResult`(~3022) `getPlanSaveState`(~3049) `persistLegend`(~3062) **`scheduleCellDraft`(~3084 — 400ms 디바운스로 `saveDoeDraft`+`notifyLegendChanged`. 페인팅·드래그·fill·paste·legend 개명까지 **호출 10곳**(1099/2340/3275/3404/3454/4015/4546/4573/4595/5709 — 2026-07-28 gate4 패스 재계수) — 모든 편집 경로가 새로고침을 견딘다. **[fix E] `persistLegend`(~3064)·`scheduleCellDraft`(~3086) 두 영속화 관문이 `frameTouched = true`도 세운다** — 뒤로가기 경고의 "실제 편집" 판정 근거)** `renderLegendMetaOnly`(~3093). ⚠️ **~~`scheduleLegendServerSave`~~(자동 저장)는 삭제** — 묘비 주석 ~3036, 서버 쓰기는 ⚡ Push 하나뿐([§0](#0-묘비-목록--소스에-존재하지-않는-이름)).
  - 로컬 초안(**[`b35bc9f`] 셀까지 나른다**): `seedEmptyDoe`(~2445 — 행 없는 맵의 시드, `vocab:true` — **[U6] 시드 원천은 `defaultLegendRows()`**: 서빙된 `default_legend` \| `EMPTY_DOE_SEED` 1행) `doeDraftKey`(~2467, `map_doe_draft::<table>::<mapKey>`) `DRAFT_VERSION=3`(~2489) `cellsDigest`(~2493, FNV-1a) `draftBase`(~2505 — 열었을 때 서버가 갖고 있던 것의 지문) **`saveDoeDraft`(~2507 — DOE(zone·knob 있는 행만)+`cells`+기반 지문을 함께 저장 — 초안 지문 우선순위의 근거)** `readDoeDraft`(~2544, v3 미만은 "기반 미상"으로 수용) `clearDoeDraft`(~2557) `applyDoeDraftRecord`(~2616 — **우선순위 판정은 호출부**, 이 함수는 적용만. **[`6db517d` H1] "applied"의 의미가 "내용이 있었다"→"화면을 바꿨다"로 교체**됐다(~2619–2645, before/after JSON 비교): 초안은 registry 저장 직후에도 재저장되므로 Push+새로고침 뒤 초안 = 서버 행인데, 그것을 "복구"로 보고하면 계획 있는 맵을 리로드할 때마다 유령 미저장 칩·토스트가 부활한다) `applyDraftCells`(~2653).
  - **[`280ebf0`] last-open 복원**: `LAST_OPEN_KEY='map_editor_last_open'`(~2565) `recordLastOpenMap`(~2567 — **depth-0 정체성만**: 자재 프레임은 여정이지 집이 아니다. 기록 지점은 로드 성공 ~3963·Push 성공 ~4349) `restoreLastOpenMap`(~2586 — 부트 1회 호출 ~690. **수동 경로 그대로**(switchTable+메타 입력+`loadExistingMap({quiet})`)를 걷는다 — 초안 우선순위·missing-key 동작이 별도 복원 경로가 아니라 같은 코드. 테이블이 사라졌으면 조용히 초기 화면).
  - 패널 관문(컨트롤러가 부르는 쓰기 3종): `addLegendRowForPanel`(~3333 — **[U6] 색 선정은 `autoAddLegendValue` 경유**) **`updateLegendRowForPanel(value, patch)`(~3344 — DOE 변조의 유일 관문. 배열 필드는 **새 배열로 통째 교체**, 제자리 수정 금지)** `deleteLegendRowForPanel`(~3389). + `fetchMapKeyColumns`(~3410) `probeMapExists`(~3428 — **존재를 추측하지 않는다**) `remapGridValues`(~3445).
- 편집 도구: `fillGrid`(~3991) `getEdgeClassification`(~4404) `selectEdgeCells`(~4484) `autoPaintE1E2`(~4511 — **[U6] E1/E2 고정 색 삭제**: `autoAddLegendValue` 경유, 선언 행 > 팔레트 규칙) `copyGridToExcel`(~4598).
- 프레임 스택: **[`0052d76` fix E] `frameTouched`(~4776 선언)** — "이 프레임을 연 뒤 실제로 편집했는가". `framePushed`만으로는 `!framePushed && cells>0`이 **보기만 한** 자재 맵에서도 뒤로가기 confirm을 띄웠다. 세우는 곳은 영속화 관문 2곳(`persistLegend`/`scheduleCellDraft`), 리셋은 `setLoadedIdentity`(~5005), **초안 복구는 의도적으로 안 세운다**(복구 내용은 초안 슬롯에 살아 있어 되돌아가도 잃는 게 없다). `snapshotEditorState`(~4778 — **[fix B/E] `legendDirty`·`frameTouched`도 스냅샷**에 포함 ~4804–4805: 자재 맵 왕복이 부모 프레임의 미저장 칩·백가드 기준선을 지우던 결함) `restoreEditorState`(~4839, 복원 ~4889–4890) `openMapFrame`(~5071 — `allowEmpty`로 빈 그리드 오픈 허용, 키는 Push에서 생성. **[fix G] `r.cancelled`면 프레임 롤백 + info 토스트 1회**(~5103–5113) — 종전엔 "맵이 아직 없습니다" 거짓 토스트와 함께 빈 격자 프레임이 열리거나 무피드백 롤백) `popMapFrame`(~5137 — **[fix E] dirty 판정에 `frameTouched` AND**(~5141): 편집 없는 프레임은 조용히 복귀). **복원 대상에 `overlayLayers`·캔버스 스크롤이 포함된다**(~4760 주석).
- **[M2] 페인트 잠금**(~41–230, 서버 선언 소비 — 구 `'F'` 하드코딩 대체): `NO_PAINT_LOCK`(~41) `isLockedValue`(~45) `isOverlayLocked`(~55) **`isProtectedFCell`(~67 — 편집 불가 판정의 단일 관문, 전 편집 경로가 여기로 수렴)** `applyPaintLockConfig`(~72) `fetchPaintRules`(~142, GET `/api/maps/paint-rules` — **[U6] 같은 응답에서 `overlayContract`도 갱신**: 후보 목록을 실은 응답만 캐시를 바꾼다, 구버전 서버는 기존 값 유지. **[F1] `binding`도 이 응답에서 `servedBindingCache`로 인터셉트**(~178)) `updatePaintLockIndicator`(~191) `recomputeLockedCells`(~208). 404/405만 "선언 없음"(해제)이고 네트워크·5xx는 **직전 잠금 유지** + `source:'stale'` + 툴바 칩. ⚠️ **[QA C4 미해소] 콜드 스타트는 여전히 fail-open** — "직전 값"이 페이지 로드 직후엔 기본값 `NO_PAINT_LOCK{enabled:false}`라 첫 조회가 실패하면 강제 지점이 열린 채 시작한다(칩은 뜨므로 **조용한** fail-open은 아님). 테이블 전환 시 실패하면 **이전 테이블의 잠금 값**을 새 테이블에 계속 적용한다(fail-closed 방향이라 안전하나 의미상 부정확).
- **[7d931dc] 오버레이 레이어**(~5182–5830) — **변환은 클라 단일 구현**이다. 계약은 [MAP_EDITOR_SPEC §5](../spec/MAP_EDITOR_SPEC.md). ⚠️ **[`280ebf0`] `.overlay-box`/`.ov-*` CSS는 `b35bc9f`의 transfer_plan.css 재작성이 떨어뜨렸다가 원문 그대로 복원**됐다(마크업은 계속 그 클래스를 쓰고 있었다):
  - 상태 `OVERLAY_COLORS`(~5182) / `overlayLayers`(~5183) / `activeOverlayLayers`(~5184) / `overlaySeq`(~5185) / `recomputeActiveOverlays`(~5187, 렌더 루프 내 재계산 금지) / `drawOverlayMarkers`(~5192, 렌더 호출 ~2175).
  - **프레임 계산**: `frameFromMeta(meta)`(~5216, `grid_metadata` JSON → 프레임 기술자. **없는 물리 항목은 undefined로 남겨** 현재 화면 값 폴백) / `currentFrame()`(~5243) / `resolveFrame(frame)`(~5256, 축 전부를 실값으로 확정) / `frameAxesKey(rf)`(~5273, 회전·면·y반전·START·치수·물리 6종 = identity/derived 판정의 유일한 근거).
  - **`projectCellsToPhys(cells, frame)`(~5284)** — `getCellFromVisualCoords` → `getPhysicalCoords`를 **소스 프레임을 씌운 채** 호출한다. `loadExistingMap` 셀 루프와 **같은 함수·같은 인자 순서** — **오버레이 전용 기하식은 0줄**이다.
  - `pushFailedOverlay`(~5303) — 실패도 목록 행으로 남긴다(같은 소스 중복은 갱신).
  - 소스 읽기: `OVERLAY_CELL_LIMIT=2000`(~5328, 메인 로드와 동일 상한) **[F1 `17f65bd`] 바인딩은 서빙받는다** — `fetchServedBinding(sourceTable)`(~122, paint-rules `binding`)이 ~~`fetchTableSchemaCached`~~·~~`OVERLAY_SYSTEM_COLS`~~·~~`deriveMapBinding`~~(클라 로컬 유도 일습)을 **대체**했다(사유 주석 ~5325 — 클라 사본 0, 선언 table_bindings > table_config 유도는 서버 1곳). `buildKeyFilters(keyColumns, mapKey)`(~5331, 서버 `build_key_filters`와 동일 — 마지막 컬럼이 나머지 흡수).
  - `addOverlayLayer(sourceTable, sourceKey, targetOverride)`(~5351) — **메인 로드와 코드 경로 완전 분리**. 흐름: ① 바인딩 **서빙**(`fetchServedBinding` ~5383 — null이면 `binding_unavailable` + 선언 안내 토스트) → ②③ `Promise.allSettled`로 셀 + 소스/타깃 메타 병렬 조회 → ④ 프레임 확정 → ⑤ `cols×rows` 호환성 관문 → ⑥ 정렬 요약 + 격자 밖 셀 카운트. 명명된 실패 status **4종**: `binding_unavailable` `meta_unavailable` `no_data` `align_unavailable` (+ IO 실패는 일반 `error`). **이 함수는 `/api/maps/overlay`를 호출하지 않는다**.
  - `removeOverlayLayer`(~5561) `toggleOverlayLayer`(~5568) `clearOverlayLayers`(~5577).
  - `overlayGeomSig`(~5590) / `currentGeomSignature`(~5592) / `syncOverlayGeometry`(~5609, 서명 변경 시 `rawCells`+`o.frame`에서 재투영. 훅 2곳 — 렌더 ~2052 · 프레임 복원 ~4881). ✅ **[QA C7 해소]** 서명이 화면 7축 + **물리 6종**을 담는다(소스 메타 완비 시 재투영은 항등 — 6종이 실제로 일하는 곳은 물리 규격 미등록 폴백 경로뿐).
  - `overlayAlignChip(o)`(~5632) — 정렬 상태 칩. 판정은 **`align.origin`으로만**.
  - `importOverlayToGrid(id)`(~5663) — 유일한 의도적 교차: 오버레이 셀을 `gridData`로만(**서버 쓰기 없음**, `isProtectedFCell` 존중, 웨이퍼 밖 셀 스킵). `ensureLegendValues`(~5721)는 **로컬 legend 캐시만** 갱신.
  - `renderOverlayList`(~5730) `handleAddOverlayClick`(~5784) `CORE_CANONICAL_TABLE='core_defect_map'`(~5813) `addOverlayForSource(sourceTable, lot, slot)`(~5815) `listOverlayLayers`(~5824) — 뒤 둘은 `transfer_plan.js`에 넘기는 컨트롤러 표면.
  - **오버레이 해제 지점은 2곳이다** — 테이블 전환 `switchTable`(호출 ~1209, 토스트) · **맵 로드 `loadExistingMap`(호출 ~3550, 토스트)**. 둘 다 `overlayLayers.length > 0`일 때만 실행한다. ⚠️ **`openMapFrame`은 오버레이를 해제하지 않는다** — 프레임 스택은 반대로 오버레이를 **보존**한다(~4760 주석). 툴바 버튼(`#btn-clear-overlays`) 배선은 ~846.
- **전사 계획 배선**: `initTransferPlan({...})`(~650, import ~6 — `bandToState`·**[U6] `stageTargetTables`**도 여기서 들여온다. **[U6] `loadTablesList`(~1105)의 초기 테이블 선택이 `stageTargetTables()`를 소비** — 구 `bonding_map` 하드코딩 대체: 선언된 stage 타깃 맵 우선, 없으면 첫 맵 테이블) + `notifyMapContext`(1216/3962/4348/5111/5125/5132/5151 = **7곳** — 2026-07-28 gate4 패스 재계수, 마지막은 [fix G] 취소 롤백 지점) `notifyLegendChanged`(3033/3088/3094/3111 = **4곳** — ⚠️ `selectBrush`엔 의도적으로 없다, 주석 ~3312) `notifyPaintCounts`(~1928). rect 영역 선택 모드는 **전면 폐기**(값 페인팅이 정본 — 코드 부재).
...
### `transfer_plan.js` (~1,397줄) — 「2. Legend & DOE」 패널 (map_editor.html에서 소비, **`b35bc9f` zone 편집기로 재작성 · `2baf9ff` U9 marker/U8 피드백 · `95bf072` U6 stage 선언 단일화 · `0052d76` fix G**)
...
- stage 유도(**[U6] 선언의 단일 원천 = 서버**): `normalizeStage`(~112) `stageOfTable`(~129, 서버 `stage_of_table`의 클라 미러) `sourceTableOf`(~142) `sourceTableOfStage`(~158) `stagesPromise`(~164 — 진행 중 fetch 1개 공유) `fetchStages`(~166, GET `/api/transfer-plan/stages` — **부재 2종을 가른다**: 404/405·빈 선언 = "선언 없음"(확정, `stages=[]`) vs 그 외 실패 = "확인 불가"(**마지막으로 안 선언 유지** + 다음 맵 전환 시 재시도 ~1247. 내장 목록은 절대 아니다) `ensureStages`(~188) **`stageTargetTables()`(~196 export — 선언된 stage 타깃 테이블 목록, 선언 순. `map_editor.loadTablesList`의 초기 테이블 선택이 소비. 불가면 `[]` = 계획 테이블 선호 없음)**. 강등은 화면이 말한다 — `renderPlanHead`의 「일반 맵」 배지에 조회 실패 툴팁(~479).
- 소스 요약(**풀 단위**): `poolCacheId`(~338) `summaryKeyFor`(~342) `getPoolSummary(pool, force)`(~347, GET `/api/transfer-plan/source-summary` — BIN·scope 파라미터 포함) `availabilityOfPool`(~392) `isPlainNotFound`(~432) `materialMetaValues`(~439) `probeMaterialMap`(~458) `refreshMaterials(force=false)`(~1151 — **[`2baf9ff` U8] `force=true`는 [↻ 가용] 버튼 단 하나의 경로**이고, 그때만 완료 토스트를 낸다: 전 풀 정상이면 info, 미상이 있으면 **최빈 사유 1줄** warning — 종전엔 서버가 BIN 축을 거부해 전부 미상이면 같은 미상 셀만 다시 칠해져 버튼이 죽은 것으로 보였다. 셀별 사유는 종전대로 미상 툴팁, 클라측 폴백 계산 없음) `rewardAfterReturn`(~1124 — 복귀 시 그 자재만 재조회).
- 렌더(zone 그리드): `renderPlanHead`(~472) `zoneCellHtml`(~561) `ZONE_PLACEHOLDER`(~593) `zoneIsInapplicable`(~605 — **[`2baf9ff` U9] marker 행(STACK 0)은 전 구역 `inapplicable`**: `해당 없음` 렌더 + fix 문구 `'STACK 0 = 상태 표시 값 (층 없음)'` — 구조적 부재 셀과 같은 취급) `materialChipHtml`(~633) `planOf`(~647) `renderDoeList`(~651 — STACK 입력의 `bad` 클래스는 `ok`·`marker` 둘 다 아님일 때만, 푸터 노트에 marker 안내 1줄) `refreshRowZones`(~736 — **행 단위 갱신**: 전체 리스트 재구성이 입력 랙의 원인이었다, `280ebf0`) `focusedRowValue`/`focusedColumnId`(~766/772) `bindDoeList`(~781 — keystroke 배지도 `marker`를 오류로 칠하지 않는다) `rollupRows`(~862) `renderMaterialPane`(~874) `knobChipsFor`(~983) `renderAll`(~1189) `buildWorkspace`(~1195). **[`280ebf0`] DOE 입력·라벨 한 단계 확대**(.68→.82rem — Excel 붙여넣기 계약은 index 기반이라 무영향).
- **Excel 클립보드(6열 TSV 계약 — `tsv.js` + `doe_bands.js` 소비)**: `planClipboardActive`(~1008) `onPlanPaste`(~1015 — `parseTsv`→`mapPastedGrid`→행별 `commitRow`) `onPlanCopy`(~1073 — `planToGrid`→`serializeTsv`). 왕복 동일성은 `contracts/doe_band_rules`의 `roundtrip_cases`가 고정한다.
- 이동 허브: `openMaterial(id)`(~1090) — **맵 간 이동의 유일 지점**(프레임 스택). **[`280ebf0` LOAD-parity]** `(lot, slot)`으로 분해되지 않는 id도 **라우팅한다**(~1097–1110): 첫 키 컬럼 필터 `{firstKeyColumn: rawId}`로 폴백 — 「1. Map Search & Load」에서 그 필드 하나만 치는 것과 같은 경로. 행 없는 키는 빈 그리드로 열리고 ⚡ Push에서 생성된다. **`probeMaterialMap`은 이런 id에 계속 null(미상)을 반환한다** — 내비게이션 추측은 되지만 존재 주장은 안 된다. **[`0052d76` fix G] `r.cancelled`는 실패 토스트를 내지 않는다**(~1115–1117) — 에디터가 이미 취소 토스트를 냈으므로 "열기 실패"를 겹쳐 부르면 취소를 오류라 부르는 셈.
- 진입/통지 export: `notifyMapContext(info={})`(~1233 — 여기서 서버 조회를 하지 않는다: legend 로드·채택·가드는 전부 `map_editor`의 registry 경로. **[U6] `stagesStatus === 'error'`면 여기서 fetchStages 재시도** ~1248) `notifyLegendChanged`(~1256) `notifyPaintCounts(counts)`(~1268, **`textContent`만 패치**) `initTransferPlan(paintController)`(~1294 — **[U6] `ensureStages()` 호출**: `stageTargetTables()`가 먼저 불렀어도 fetch는 1회). (+ 순수 함수 `bandToState`·`prevTo` export ~256, `stageTargetTables` export ~196)
- ⚠️ **`__held_*` 함수군(~1316–1397)은 명시적 보류 구역** — 호출자 없음. 검증/경고 UI는 사용자 지시로 미구현. ⚠️ `__HELD_WARN_SEVERITY`(~1366)에 **`layer_coverage_gap` 키가 남아 있으나 서버에서 삭제된 경고 타입이라 사문**이다.
...
- Enrichment 탭: `renderEnrichmentTable`(~1369) `fetchEnrichmentStatus`(~2626, 15s TTL 캐시 — 스트립·탭·Overview 3소비처 공용. **[`1fefd12`] 결손 카운트 필터는 `rule.queue_filters` 우선**(~2641) — 워크리스트·배지와 동일 수치 보장, 폴백만 target-blank 수제).
...
### `enrichment.js` (~758줄) — 인리치먼트 컨베이어 페이지 (export 없음)
- 규칙: `loadRules`(~69) `selectRule`(~116) `rebuildGrid`(~151). 워크리스트: **`buildBlankFilters`(~184 — [`1fefd12`] 큐 술어는 `rule.queue_filters` 서버 조성 우선**(판단키 notBlank AND target blank), 구버전 서버 폴백만 target-blank 수제) `fetchWorklist`(~194) `fetchTotalAll`(~245) `refillIfNeeded`(~261).
- 입력 흐름: `renderDetail`(~314) `onInputKeydown`(~388) `moveSelection`(~406) `saveCurrent`(~431, PUT `/data/updates`).
- 참조 패널: `initReferencePanel`(~521) `loadActiveReference`(~584) `renderRefTable`(~641).
...
| `tokens.css` (~290) | 디자인 토큰(색·타이포·간격) — 듀얼 테마 CSS 변수의 SSOT. 2026-07-25 다크 세트 심화(Ground L* 9.2, WCAG AA 유지). **[`a98dc72`] `#toast-container`가 하단 중앙 배너로 전환** — 우하단 고정이 맵 에디터 자재 목록을 가리던 문제와 그 회피 장치(`--toast-inset-right` 변수)를 함께 제거, 모션은 opacity 전용 |
