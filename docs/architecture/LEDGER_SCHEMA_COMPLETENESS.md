...
| 축 | 상태 | 자리 |
|---|---|---|
| B7 as-of · 구간 · 세대 선택 | 🔴 **③ 이되 «셋»이다** (D-13 이 가름, 09-09 20:1x) | 라우트에 시각 인자가 «없다»(히트 0)는 그대로. 🔵 그러나 셋의 «크기가 다르다»: **구간**(occurred_at ∈ [t1,t2]) = ② — 자리가 이미 있다(`occurred_at` 이 원자 SELECT 의 고정 컬럼이자 걷기의 «정렬 축»), 인자 넷이면 된다 · **as-of**(「그때 원장은 무엇을 알았나」) = 🔴 ③ **기제 없음** — 철회가 `DELETE FROM ledger_events` 라 「그때는 알았지만 지금은 아닌 것」이 남지 않는다. 소속의 «유효 구간»도 같은 이유로 등록·철회에서 읽을 수 없다 · **세대**(supersedes) = ③, 원자엔 칸이 있고 걷기가 안 읽는다. 🔴 「walk 은 멱등」은 구간을 넣어도 «선언이 안 바뀌었을 때»만 참이다 — 재번역이 그 구간의 원자를 지우고 다시 쓴다. 자세히 D-13 |
...
| **② 「유효 구간」을 두 사실(등록·철회)로 읽기** | 🔴 **③ — 오늘 «불가능»합니다** | 철회가 «사실이 아니라 삭제»입니다: `store._withdraw_refs` = `DELETE FROM ledger_events WHERE source_who = %s AND source_raw_ref = ANY(%s)`. 「t2 에 철회됐다」를 적은 행이 «없습니다». 그래서 「t1~t2 사이엔 소속이었다」를 읽을 재료가 없습니다 |
...
갈라 보면
  ...
  as-of  「t 시점의 원장은 무엇을 알고 있었나」              <- 🔴 ② 때문에 «안 됩니다».
         철회가 DELETE 라 「그때는 알았지만 지금은 아닌 것」이 «남아 있지 않습니다»
