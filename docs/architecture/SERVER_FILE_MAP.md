...
| 🆕㈤ | `chain/rule_order` | 5,183 | 2026-09-13 | The order rules run in, DERIVED from the declaration rather than declared (S-156 `c7bc59db`). The worker reads it; `chain/replay` delegates to it. The self-edge exception is a PROPERTY of the pair, never a name (S-232 `c70dc340`, 판정 387 · S-232-b `684ffb4d`, 판정 388). 🆕 [2026-09-15 `ea8f91d2`] a disabled or `follow_up` producer orders nothing — an edge that cannot fire cannot close a cycle. 🆕 [2026-09-15 판정 402 `2504fbde`] `order_rules` never raises: a cycle across tables is reported through `on_cycle` (`say_cycle_once`, once per trail per process) and those rules keep the DECLARATION's order; `RuleCycleRefused` is deleted. |
| 🆕 | `chain/rule_shape` | 15,608 | 2026-09-14 | The unified declaration (`derive: {kind: join \| decide}` + `on` + `into`) translated to today's rule dict, and back. `is_switched_off`: `enabled: false` stands NO rule and touches no database (판정 399). `decide_rules` reaches `enrichment.config.chain_rules_for` — one expander, two front doors (S-239 `dc29056a`). 🆕 [2026-09-15 S-244 `00da7e91`] `expand_declaration(entry, table_config)` → (rules it stands, refusal, notes) is the ONE author the loader AND the admin raw-save route call — the save gate judges what the loader would stand, and the file keeps the operator's text. |
| 🆕 | `chain/join_into` | 13,477 | 2026-09-15 | `builtin:join_into` — the unified `join` kind that WRITES the right side into a real column (S-237 `7f3c3634`). Borrows only `notation_norm`'s fold, never the read-time executor. A left row with two right answers is written by neither, named once per batch (`593aac50`); it builds no unique index and nobody reads `key.unique` yet (S-240). 🆕 [2026-09-15] a non-text key is cast to TEXT on both sides before the NULL fold (`fd53b87b`); its writes carry layer `chain_ingestion` with the rule name in `updated_by` (`1aa50d3d` — the rule name in the LAYER read as a person's edit to the wake filter and ping-ponged); R1 backfills it via `run_builtin` (`bd0a3db7`). |
...
| ⚰️📦 | ~~`_archive/ledger/chain_mapper`~~ | 16,279 | 2026-08-18 | Trusted Chain-mapper calls for the existing Ledger-owned execution loop. |
...
| ⚰️📦 | ~~`_archive/ledger/examples`~~ | 82 | 2026-08-16 | Copyable ledger translator examples; no module here is runtime-registered. |
...
| ⚰️📦 | ~~`_archive/ledger/profile_chain_mapper`~~ | 19,620 | 2026-08-17 | Canonical Profile evaluation inside the registered Chain-mapper boundary. |
| ⚰️📦 | ~~`_archive/ledger/profile_lookup_adapters`~~ | 4,650 | 2026-08-17 | Registered read-only lookup capabilities for canonical Profile execution. |
...
| 🪦 | ~~`parsers/custom_parser_template`~~ | — | 2026-09-13 | **이동됨 → `authoring/examples/custom_parser_template.py`** (S-209, 판정 348). 그 파일의 계약은 `parse_file(file_path) -> list[dict]` 함수 하나이고, **이 저장소 코드에는 그 이름을 부르는 자리가 없다**(실측: `directory_watcher` :1157 은 `BasePipelineParser` 하위 «클래스»만 집는다). |
