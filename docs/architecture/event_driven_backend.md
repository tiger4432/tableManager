...
>
> **Status:** 🟠 부분 최신 (§1~§3 Living · §4 🗄️ 은퇴) | **Last-verified:** 2026-09-16 (§3 머리 — 판정 406·S-252 «착지»: 루프는 자기 행만 집고 처리 0 이면 기다린다(`pending_chain_events`·`idle_wait`), 런처는 API 자식에 `ASSY_CHAIN_WORKER=0` 을 넘기고 단일 프로세스는 «경고하며» 안에서 돈다) · 직전 2026-09-15 «후속» (§3 머리 — 판정 406·S-252 지시 · §3.4 ② 고리 줄도 `operator_line`, S-247-b) · 직전 2026-09-15 (§3.4 — 고리는 «거절»이 아니라 «보고»다(판정 402) · 뒤따르기 랩도 ③의 홉이다(S-249)) · 직전 2026-09-05 (**§3 미전달 마커의 «모양»이 한 곳으로** — `event_constants.UNDELIVERED_MARKER_STATUS`/`_PROCESSED_CHAIN`/`_TAG`. 종전에는 쓰는 쪽과 줍는 쪽이 세 값을 각자 철자했고 묶는 것이 «아무것도 없었다» — 한쪽이 바뀌면 마커가 오류 없이 안 주워지고, **스윕 시험 다섯이 자기 행을 손으로 만들어** 그것을 볼 수 없었다. `broadcast_at` 은 «일부러» 그 묶음 밖) · 직전 2026-08-29 (§4 후계 라우트 정정 · **§2.2에 「`request_source`는 채널 라벨이지 레이어 소스명이 아니다」 + 「컨텍스트를 안 세우는 자동 경로는 사람의 편집으로 나간다」 신설** — `ffb23d6`+`53f9187`. 실제 사고: 소급 수리 패스 **둘**(R3·R2)의 아웃박스 이벤트가 `user`/`system`+이벤트마다 uuid4로 나가 하류 매퍼 전원을 깨우고 tx id가 안 묶여 N개 직렬 그룹이 됐다. **R1은 라벨이 구성상 옳은데 tx id는 페이지당 하나로 남아 있다 — 두 필드는 서로를 함의하지 않는다.** 🔴 **라벨이 쓰기/삭제 대상을 바꾸지 않는다는 것은 경로마다 따로 확인한다**(R2는 층을 지우므로 R3와 질문이 달랐고, 판정은 스냅샷 해시였다). 직전 2026-08-06 **[`2fc4f00`] §3.6에 「삭제 id 목록도 `BROADCAST_ITEM_LIMIT`을 받고, 잘렸다는 사실이 같이 나간다」 신설** — `batch_refresh_required`의 `deleted_row_ids_omitted`. 이 팔에는 상한이 아예 없었는데 `deleted_row_ids`가 collision-merge 껍데기 몇 건만 나르던 동안은 도달 불가능한 결함이었다. 직전 [`25de4ae`] **§2.3에 「NOTIFY는 트랜잭션당 1회」**(그 레인이 직접 기입). 직전 2026-08-04 **[`2aab7e2`] 브로드캐스트 복구 두 갈래** — ① **스윕의 재발사 집합이 넓어졌다**: `refresh_targets = affected_targets | source_tables`. 종전에는 체인 규칙에 걸리지 않는 미전달 행(규칙 비활성·트리거 아님·워처 마커)이 **아무것도 발사되지 않은 채 확정만** 찍혀 조용히 소각됐다 — 「무한 스윕을 막는 것은 발사 안 함이 아니라 확정을 찍는 것」. ② **워처의 실패한 통지가 durable 마커를 남긴다**(`record_undelivered_notification` → `EVENT_BROADCAST_RECOVERY`): 예전엔 로그하고 버려서 허브가 몇 초 깜박이는 것만으로 통지가 영구 유실됐다. **새 큐가 아니라 기존 스윕이 이미 줍는 모양**으로 쓴다. 함께: `/health` 도달성 판정이 상태코드에서 **응답 BODY**로 바뀌었다(`own_health_payload`). 직전 2026-07-30: **[F8] 통지 전송 계층 항목 신설** — loopback 통지가 사내 프록시로 나가 403으로 거절된 운영 장애의 근본 수리. `internal_event_client.internal_event_session()`(`trust_env=False`) 단일 팩토리, 발신자 자체 클라이언트 생성 금지를 테스트로 강제, 실패 줄에 `admin-gate=yes|no` 판정 + 조치 명시. 직전: 2026-07-25) | **Owner:** Backend / Sync | **Source-of-truth:** `server/database/database.py`, `chain/ingestion_worker.py`, **`server/internal_event_client.py`** (⚰️ 종전 이 목록에 있던 `graph_sync_worker.py`·`graph_materializer.py`·`ontology_config.py`는 **2026-08-16 트리에서 제거**됐습니다 — §4의 정본이 사라진 것이지 §1~§3에는 영향이 없습니다)
...
> ⚰️ **[2026-08-14] 이 그림에서 「Graph Sync Worker materializer → graph_nodes/graph_edges」 갈래가 삭제됐습니다**(§4 참조). outbox의 나머지 계약은 그대로입니다.
...
* **ContextVars 정의**: `server/database/context.py`
  ...
  * `request_source`: 호출 근원 (예: `"user"`, `"pipeline_parser"`, `"chain_ingestion"`)
    * 🔴 **이것은 *채널 라벨*이지 *레이어 소스명*이 아니다 (2026-08-11 `ffb23d6`).** 두 필드가 같은 이름을 쓰지만 출처도 효과도 다르다: **레이어**를 만드는 것은 `update_item.source_name`(항목별 pydantic 필드 → `cell_sources` 행)이고, **라벨**은 이 컨텍스트 변수다 — **서버 트리 전체에서 이 변수를 읽는 곳은 `database._outbox_envelope` 하나**다. 둘이 만나는 지점은 배치 쓰기 경로가 배치의 소스명을 이 변수로 복사하는 **한 줄뿐**이고, 그 줄을 지나지 않는 쓰기 경로(예: `chain_replay.recompute_display_values`의 맨 `setattr`)에서는 **라벨을 세워도 레이어가 생기지 않는다.** 실측으로 고정돼 있다(`test_recompute_creates_no_cell_sources_layer`).
    * 🔴 **컨텍스트를 세우지 않는 자동 경로는 「사람의 편집」으로 나간다.** 아무도 안 세우면 `_outbox_envelope`가 기본값을 읽어 `source_name="user"`·`updated_by="system"`·**이벤트마다 새 uuid4**를 싣는다. 앞의 둘은 하류의 순환 필터(`chain_ingestion_worker._rule_accepts_event`)를 통과시켜 **사람이 깨우는 체인을 전부 깨우고**, 셋째는 워커가 transaction id로 묶기 때문에 **N개 행을 N개의 직렬 그룹**으로 만든다. 🔴 **ORM을 통해 행을 고치는 배치·수리 패스는 반드시 `crud.transaction_context(user, tx_id, source)`로 셋을 함께 선언한다** — 실행 하나에 tx id 하나가 기본값이고, 이벤트마다 새 id는 결함이다.
    * **실제 사례 셋, 그리고 두 필드가 서로를 함의하지 않는다는 것**(2026-08-11): `chain_replay`의 세 연산이 각각 다르게 걸렸다 — **R3**(`ffb23d6`)와 **R2**(`53f9187`)는 컨텍스트를 안 세워 **라벨과 tx id 둘 다** 틀렸고, **R1**은 배치 경로를 지나므로 `apply_batch_updates`가 항목의 `source_name`을 컨텍스트로 복사해 **라벨은 구성상 옳은데 tx id가 여전히 *페이지당* 하나**다. 🔴 **라벨이 맞다고 그룹핑도 맞은 것이 아니다** — 같은 엔벨로프에 실릴 뿐이다. 절차 정본은 [chain_ingestion_guide §5.6](../guide/chain_ingestion_guide.md).
    * ⚠️ **라벨을 세우는 것이 *무엇을 쓰거나 지우는지*를 바꾸지 않는다는 것은, 그 경로마다 따로 확인한다.** R3는 층을 안 만들고 R2는 층을 **지우므로** 물어야 할 질문이 달랐다 — R2에서는 삭제 술어(`_claimed_filter`와 DELETE)가 **`source_name` 파라미터**로 짜이고 라벨은 컨텍스트 변수라 **한쪽에서 다른 쪽으로 가는 경로가 없다**. 판정은 읽기가 아니라 **컨텍스트를 no-op 시킨 실행과의 스냅샷 해시 비교**다.
...
* **레거시 인덱스 정리**: 비부분 인덱스 4종(`ix_database_outbox_id`(pkey 중복)·`ix_database_outbox_event_uuid`(미사용)·`ix_database_outbox_status`(부분 인덱스로 대체)·`ix_database_outbox_processed_chain`(부분 인덱스로 대체), 실측 합계 429MB)은 models.py 선언에서 제거되었고 기존 운영 DB는 `scripts/ops_setup_db_performance.py`의 멱등 DROP으로 정리한다.
* **기존 백로그 정리**: 라이브 프로세스가 건드리지 않는 수동 스크립트 `scripts/ops_purge_outbox_backlog.py`(dry-run 지원, 청킹 DELETE) — 실행 순서는 스크립트 docstring 참조.
...
* 🆕 **[S-140, 2026-09-10] 페이로드가 «바뀐 컬럼 이름»을 싣는다** — `columns: [이름…]`(중복 제거·정렬).
  «이름만이고 값은 절대 안 실린다»: 규칙이 묻는 것은 「내 컬럼이 바뀌었나」이지 「무엇으로」가 아니고,
  값을 실으면 아웃박스에 사용자 데이터가 들어간다. 체인 규칙이 `trigger_columns` 를 선언하면 그 집합과
  «교집합이 있을 때만» 돈다([chain_ingestion_guide §4](../guide/chain_ingestion_guide.md)).
  
  🔴 **키가 «없으면» 「모른다」이고 그때는 «전부 돈다».** 그래서 쓰는 쪽은 빈 집합일 때 `[]` 를 찍지 않고
  «키를 뺀다» — 이 키가 생기기 전에 적재된 이벤트와 «행 단위(비접힘)» 이벤트가 그 가지로 들어간다.
  빈 배열로 읽었다면 아무도 다시 적재하지 않은 그 이벤트들에서 컬럼 규칙이 «조용히» 멈췄을 것이다.
  ⚠️ 그러므로 «행 단위 모드에서는 이 기능이 작동하지 않는다**(전부 돈다) — 안전한 방향이지만 사실이다.
...
* 🔴 **스냅샷 → 현재 상태로 의미가 바뀐다. 이건 발견이 아니라 선언이다.**
  ...
  * 🔴 **[2026-09-11 S-158 · 판정 265] 그런데 「WARNING 한 줄」은 «조용히 넘기지 않는다»에 못 미쳤다.**
    행을 «하나도» 못 읽으면 맵퍼가 «빈 페이로드»를 받아 아무것도 안 하고 그룹이 **SUCCESS 로 끝났다** —
    사건은 처리됨으로 찍히고 그 행들은 파생 표에 영영 안 닿았다. 09-11 에 1,000 행짜리 사건 넷이
    그렇게 지나갔다. 🔴 **그리고 저 WARNING 의 «설명»(「지워졌다」)이 틀렸다 — 그 행들은 전부 살아 있었다.**
    지금: 「전부 없음」은 «거절»이고(265), 「아직 안 보임」은 «미룸»이다(267 ⓑ, 아래).
  * 🔵 **「전부 없음」과 「아직 안 보임」을 가르는 이름**: `ROWS_NOT_VISIBLE`(`chain_ingestion_worker`).
    내는 쪽과 알아보는 쪽이 «같은 상수»를 읽는다 — 두 철자가 갈리면 한쪽이 조용히 다른 답을 낸다.
    미룸은 «시도가 아니다»(`retry_count` 와 별도) — 시도로 세면 `max_group_attempts`(기본 1) 때문에
    멀쩡한 청크가 «첫 번에» 격리되고, 격리는 청크를 행 단위로 다시 편다(실측: 쓰기 여섯 → 사건 ≈1,700).
    상한은 `max_rows_not_visible_defers`(기본 30 ≈ 1분)이고, 넘으면 «이름 대어» 거절한다.
...
### 2.5 ⚰️ 사건과 그 행의 커밋 이음새 — **닫힘** (S-160, 2026-09-11 13:37 실측)

```
관측    접힌 사건이 «보이는데» 그 사건이 이름 댄 행이 «아직 안 보이는» 창이 있다(<100 ms).
       프로브: 사건이 1,000 행을 이름 댐 → 세션이 «0» 읽음 → «같은 세션»이 100 ms 뒤 1,000 «전부» 봄
       (새 세션도 같음 · id 는 String 컬럼에 str · 격리 read committed)
첫 전제  「사건을 행보다 «먼저» 커밋하는 이음새가 있다」(판정 267 ⓐ)
⚰️ 반증  사건과 그 행 1,000 이 «같은 xmin»(판정 268, 09-11 10:56). 문 하나 · 세션 하나 ·
       `before_flush` 의 문장이 오늘도 참이다. 즉 «한 스냅샷 안»에서는 「보이는데 안 보인다」가 «불가능»하다
🔴 그래서 남은 것  ㉠ 커밋 «전»에 읽었다 = 단순 경합   ㉡ 무언가가 그 행을 «가리고» 있었다
                 — 오늘은 «어느 쪽인지 모른다». 이 줄은 그 «모름»을 적어 두는 자리다
```
🔵 **[09-11 13:37] 원인이 나왔습니다 — 갈래 «둘 다» 아니었습니다** (S-167 `fa46e798`):
```
LISTEN 은 autocommit 이 필요해 `engine.raw_connection()` 으로 받은 «풀 커넥션»의
격리 수준을 바꾼다(`set_isolation_level(0)`). 그 프록시를 닫으면 커넥션은 «autocommit 인 채로»
풀에 돌아간다 — 실측: 바로 다음 대여가 «같은 커넥션 · autocommit=True»
=> 그 커넥션을 쥔 세션은 «트랜잭션을 시작하지 않는다».
   그래서 ① 그 위의 모든 `begin_nested()` 가 25P01 로 터지고(S-166),
       ② 행과 사건이 «서로 다른 커밋»에 쓰인다 = 이 절의 그 창
고침   LISTEN 커넥션이 «풀을 떠난다». 그러면 돌려줄 것이 없다
확인   S-167 뒤 1,000 행 × 6 연속 — LOST 0 · `rows_not_visible` 0 · 계기 0
```
🔴 **그리고 앞선 측정 둘이 «둘 다 옳았습니다»** — 판정 268 의 「사건과 행이 같은 xmin」은
«멀쩡한 커넥션»을 잰 것이라 참이고, 프로브의 「0 을 읽었다」는 «오염된 커넥션»이 쓴 순간이라 참입니다.
한쪽이 틀린 것이 아니라, 둘이 «다른 커넥션»을 보고 있었습니다.
📌 그래서 이 절이 「미지」로 남아 있던 것이 옳았습니다 — 그때 있던 갈래 둘(경합 / 가려짐) 중
   어느 쪽으로도 닫았으면 «틀린 원인»을 문서에 박았을 것입니다.
⚠️ **미룸 가드는 그대로 둡니다**(판정 267 ⓑ). 원인이 사라졌어도 그 가드는 「입력이 아직 없다」를
   «조용한 성공»으로 만들지 않는 두 겹 중 하나이고, 다른 원인이 같은 창을 열 수 있습니다.
📎 부류: 「부재는 «말 안 함»이지 «없음»이 아니다」 — 여기서는 「안 보임」이 「없음」으로 읽혔고,
   그 오독의 비용이 행 «4,000» 이었습니다.
...
> 🔴 **[2026-09-15 22:3x 박스 장애 → 판정 406 · S-252 — 착지 `07143bbe`·`86016d10`]** 통합 모드(uvicorn 단독)에서 이 루프는 API 와 «같은 이벤트 루프»에 삽니다. 그날 루프가 자기가 «소비할 수 없는» 제어 행(`RETROACTIVE_RUN` — 스케줄러의 것)을 틱마다 집어 건너뛰고, «대기열이 비지 않았다»는 이유로 대기를 안 타서 **await 없는 고리**가 됐고 API 이벤트 루프가 굶었습니다(리플레이 안 돎 · audit log 새로고침 굳음 · DB 막힘 0 — py-spy 로 잰 것). 오늘은 둘이 막습니다.
> **S-252 — 루프는 «자기 행만» 집고, «처리 0 이면» 기다립니다.** 대기 중 행을 긷는 질의가 `pending_chain_events(db)` 하나이고 거기서 `CONTROL_EVENT_TYPES` 를 «SQL 로» 뺍니다(루프 안에서 «이미» 건너뛰던 그 집합을 질의로 옮긴 것 — 새 판정이 아닙니다. `event_type` 은 NOT NULL 이라 `NOT IN` 이 행을 삼키지 않습니다. 플랜은 그대로 `idx_outbox_unprocessed` 인덱스 스캔 + 필터). 그리고 «아무것도 안 한 틱»의 두 출구(빈 대기열 · 집었는데 전부 걸러짐)가 «한 자리» `idle_wait()`(`listener.wait(2.0)`)를 지납니다 — 「행을 집었다」와 「할 일이 있다」는 다른 사실이고, 셋째 출구가 생겨도 대기를 «물려받습니다».
> **판정 406 — 체인 루프의 자리는 «자기 프로세스»입니다.** 런처(`run_decoupled_app.py`)는 API 자식에 `DECOUPLED=True`(기동부가 워처·체인 인라인 기동 «전»에 반환)와 **`ASSY_CHAIN_WORKER=0`** 을 «둘 다» 넘기고 체인은 `run_chain_worker.py` 가 돕니다 — 두 번째 줄이 그 사실을 «명시»합니다. uvicorn 을 «손으로» 띄우면 루프는 여전히 안에서 돌고, 그때 기동 로그가 **경고** `[Chain Worker] running INSIDE the API process - a slow tick blocks every HTTP request …` 를 찍습니다(개발·박스용). 꺼진 쪽(`ASSY_CHAIN_WORKER=0`)은 `info` 「NOT started in this process」 — 런처 아래서는 «정상 상태»라 경고가 아닙니다. ⚰️ 그 꺼진 가지는 종전에 `None` 태스크에 콜백을 달아 `AttributeError` 로 «한 번도 돈 적이 없었고», 「Startup step failed (watcher/chain worker)」로 보고됐습니다 — 같은 커밋에서 고쳐졌습니다(가드는 도달 가능해지는 날 틀린다). ⛔ 스레드풀(executor)로 틱을 옮기지 «않습니다» — 체인을 도는 길이 둘이 됩니다(S-252-b 는 여기서 닫힘). 겉은 [RUNTIME_MAP §0](./RUNTIME_MAP.md).
...
### 3.4 순환 루프 방지 장치 — **문이 «셋»이고, 셋이 «다른 것»을 막습니다**

> 🔴 **[2026-09-06] 이 절이 「두 장치」라고 적고 있었고, 그날 «셋째»가 착지했습니다**(`639655d7`).
> 그리고 이 문단은 2026-09-04 에도 «거짓»으로 한 번 정정된 자리입니다 — 두 번째입니다.
> ⚠️ 「몇 개인가」를 적은 문장은 «장치가 늘 때마다» 거짓이 됩니다. 그래서 아래는 «수»가 아니라
> **「각각이 «무엇을» 막는가」**로 적습니다.

무한 재트리거(A 테이블 ➡️ B 테이블 ➡️ A 테이블...)를 막는 것은 **셋**이고, **서로 다른 것을 막습니다.**

| # | 장치 | 언제 | 막는 것 | 🔴 «안» 막는 것 |
|---|---|---|---|---|
| ① | 런타임 옵트인 (`_rule_accepts_event`) | 이벤트마다 | 체인이 쓴 것이 «남의 규칙»을 깨우는 것 | `allow_chain_trigger` 를 «선언한» 규칙은 «받습니다» — 억제가 아니라 «옵트인»입니다 |
| ② | 로드 시점 그래프 검증 (`_validate_chain_cascade_graph`) | 워커 «뜨기 전» | ⚰️ **[2026-09-15 판정 402] 아무것도 «막지 않습니다»** — 옵트인 엣지만으로 된 고리를 «찾아서» trail 을 돌려주고(어드민 그래프) 프로세스당 한 번 말합니다 — 그 줄도 다른 거절 줄과 같이 `operator_line` 을 지납니다(S-247-b `d469bacf`: 손으로 철자하던 것을 저자 하나로). 고리는 «모양»이고 유한성은 ③이 답합니다 | 종전 「길다」와 「무한」을 구별 못 해 «도는» 선언을 거절하고 드레인이 배치마다 그 거절을 «스크롤»시켰습니다 — 그래서 거절이 «사라졌습니다» |
| ③ | 🆕 실행 시점 깊이 (`chain_depth` · `max_chain_depth`) | 이벤트마다 | «몇 홉»을 넘는 연쇄 — 이름을 대고 거절하고 그 행을 «끝냅니다»(FAILED). 🆕 **[2026-09-15 S-249] 뒤따르기 랩(`_run_builtin_followups`)도 이 홉을 «씁니다»**(`incoming+1`) — 종전엔 그 스레드에 홉이 «안 찍혀» 랩을 지나는 고리는 «무한»이었습니다 | 🔴 «새» 연쇄를 안 막습니다. 깊이는 «무한»을 막지 «시작»을 막지 않습니다 |

🔴 **①과 ② 사이가 «비어 있었습니다».** ①은 옵트인(전면 금지가 너무 빡빡해서 풀린 것)이고
②는 «그때» 「고리면 설정 «전체» 거절」이라, 「선언은 했는데 얼마나 깊어지나」에 답하는 것이 «없었습니다».
③이 생긴 뒤 ②의 거절은 «남는 이유가 없어» 판정 402 로 사라졌습니다 — 오늘 고리를 «셈»하는 자리는 ②, «막는» 자리는 ③ «하나»입니다.
③이 그 자리입니다 — 소유자 원문: 「원래는 체인이 체인 유발 아예 막았는데 너무 빡빡해서
풀었더니 이래 된 거네, **깊이 넣으면 될 듯**」.

#### ③ 의 성질 — 「막았다」가 «조용하지» 않습니다
```
한계        `chain_rules.json` 의 `max_chain_depth` — «선언»입니다 (상수 아님)
            선언이 없으면 `event_constants.DEFAULT_MAX_CHAIN_DEPTH` 가 답합니다
표지        아웃박스 payload 의 `chain_depth`.  🔴 «세 상태»입니다 — 키 없음(체인 밖) · 0 · N
            「없음」을 0 으로 «접지 않습니다» — 그 구분이 «결정하는 자리»에서 사라집니다
찍는 자리   «하나» — `database._outbox_envelope` 가 컨텍스트 변수를 읽습니다.
            (맵퍼들이 각자 적는 `source_name` 은 «행의 소스 컬럼»이고 «봉투»가 아닙니다)
거절        `_rule_accepts_event` «밖»에서 — 그 함수는 이벤트당 «다섯 번» 불리는 순수 술어라
            거기서 거절하면 다섯 번 로그하거나 «침묵»합니다
🔴 끝냄      거절한 행은 `FAILED` 로 «끝내고 커밋»합니다. 안 그러면 그 행이 매 틱 다시 읽히며
            «뒤를 영원히 막습니다» — `92d1c1ff` 로 고친 그 결함을 한 파일 옆에서 되살리지 않기 위해
```

#### ⚠️ 그리고 «넷째»가 아니라 ①의 일부인 것 — 이벤트 «타입»
```
DELETE 는 체인을 «못 깨웁니다» — 워커가 트리거를 «CREATE/EDIT» 로 거르는 자리가 «셋» 있습니다
=> 그래서 벌크 삭제에 collapsed DELETE 이벤트를 실어도(2026-09-06 `d95936a4`) «새 연쇄가 안 생깁니다»
⚠️ 이걸 「깊이가 막는다」로 읽지 마십시오 — 깊이는 «길이»를 막고, 타입은 «시작»을 막습니다
```
...
**② 로드 시점 그래프 검증** (`load_chain_rules` → `_validate_chain_cascade_graph`). ①이 옵트인이라 **옵트인 엣지끼리 고리를 만들 수 있고**, 그래서 워커가 뜨기 전에 그 엣지만으로 그래프를 세워 순환이면 ⚰️ ~~**거절**합니다~~ → **[2026-09-15 판정 402 `2504fbde`] trail 을 «한 번 말하고» 목록으로 «돌려줍니다» — 거절하지 않습니다.** `dt_log → dt_inventory`(맵퍼)와 되돌아오는 join 은 «의도된» 고리이고, 유한하게 만드는 것은 ③의 `max_chain_depth` 입니다. 저장 라우트(`save_chain_rule_raw`)도 같은 이유로 고리로 거절하지 않고, 순서 유도(`rule_order`)는 고리면 «선언 순»을 그대로 씁니다.
```
꼭짓점  표
...
* **🔴 [`2aab7e2` 2026-08-04] 통지가 실패하면 워처도 durable 마커를 남긴다**: `run_watcher.post_event`는 예전에 실패를 **로그하고 버렸습니다** — 마커도 재시도도 없어서, 허브가 닿지 않는 동안 워처가 낸 `batch-refresh`·`file-processed`·`ingestion-state` 통지가 **영구 유실**됐습니다(행은 DB에 앉고 화면은 끝내 모릅니다). 재시작만의 이야기가 아니라 **허브가 몇 초 깜박여도** 그렇게 됩니다. 수리는 새 큐가 아니라 **이미 있는 모양**입니다(체인 워커의 미확정 브로드캐스트 스윕이 정확히 이것을 합니다 — [PRIMITIVES §6](./PRIMITIVES.md)): `internal_event_client.record_undelivered_notification()`이 `database_outbox`에 스윕이 이미 줍는 모양으로 한 줄을 씁니다 — `processed_chain=True`(데이터 트랜잭션으로 재실행되지 않음, 매퍼 없음) · `status='SUCCESS'`(**데이터는 성공했고 통지만 실패했다**) · `broadcast_at=NULL`(마커 본체) · `event_type=EVENT_BROADCAST_RECOVERY`(`"BROADCAST_RECOVERY"`, `CONTROL_EVENT_TYPES` 소속이라 체인 큐도 그래프 승격도 건너뜁니다).
  * 🔴 **[2026-09-05] 그 모양의 «철자»가 이제 한 곳에 있습니다 — `event_constants.UNDELIVERED_MARKER_STATUS` · `UNDELIVERED_MARKER_PROCESSED_CHAIN` · `UNDELIVERED_MARKER_TAG`.** 종전에는 **쓰는 쪽**(`internal_event_client.record_undelivered_notification`)과 **줍는 쪽**(`chain_ingestion_worker.sweep_undelivered_broadcasts` 의 필터)이 같은 세 값을 **각자 손으로 적고 있었고, 둘을 묶는 것이 아무것도 없었습니다.** 한쪽만 바뀌면 마커가 스위퍼가 안 줍는 모양으로 쓰이는데 — **예외도 오류도 안 납니다.** 행은 DB 에 영원히 앉고 화면은 끝내 모릅니다. 정확히 이 기제가 막으려던 사고입니다.
  * 🔴 **그리고 시험이 그것을 볼 수 없었습니다** — 스윕을 덮는 시험들이 **자기 행을 손으로 만들기** 때문에, 쓰는 쪽이 무엇을 하든 초록입니다. 철자를 공유하는 것이 **둘을 같이 빨개지게** 만드는 유일한 장치입니다(그물: `server/tests/test_undelivered_marker_seam.py`).
  * ⚠️ **`broadcast_at` 은 «일부러» 그 상수 묶음에 없습니다.** 그것은 쓰는 쪽이 세우는 값이 아니라 줍는 쪽이 찾는 **부재**입니다 — 이름을 주면 누군가 그것을 «쓰게» 됩니다.
...
* **Outbox 폴링 부분 인덱스**: `database_outbox` 스캔 비용을 상수화하기 위해 부분/표현식 인덱스를 사용합니다(`models.py` `DatabaseOutbox.__table_args__`, 기존 운영 DB는 `scripts/ops_setup_db_performance.py`로 멱등 반영).
...
* **LISTEN 전용 커넥션 상시 유지 (`OutboxListener`)**: 빈 폴링 후 `LISTEN/NOTIFY` 대기 시, 예전에는 **대기마다 새 커넥션으로 `LISTEN outbox_event`를 재등록**하여 빈 폴링과 등록 사이 발행된 NOTIFY가 유실(최대 2초 tail latency)됐습니다. 이제 워커 시작 시 LISTEN 커넥션을 **1회만** 등록·재사용합니다(`chain/ingestion_worker.py`, `OutboxListener`). LISTEN이 항상 폴링보다 선행 등록되므로 폴링 이후 NOTIFY는 소켓에 버퍼링되고, 대기 진입 직후 버퍼된 통지를 먼저 소비(drain)해 즉시 재폴링을 유도합니다. 커넥션 끊김/예외 시 안전 재생성(누수 방지), blocking `select`는 `asyncio.to_thread`로 오프로딩, SYSTEM_RELOAD 통지도 같은 채널로 공존합니다.
...
> * **순서 보존은 "동일 `target_table`" 정적 추정 기반**입니다(§3.5 HOL 가드 + §3.6 F2). ⚰️ **[2026-09-10 정정]** 아래 「교차 테이블 read 의존은 보호하지 못한다」는 «낡았습니다» — `process_pending_groups` 는 이제 `_group_target_tables ∪ _group_read_tables`(쓰기 ∪ «읽기»)로 판정하므로, 앞선 실패 그룹이 쓴 표를 «읽는» 후속 그룹도 같은 배치에서 보류됩니다. 읽기 집합도 규칙 선언에서 정적으로 뽑습니다. (원문 보존:) 즉 **매퍼가 자신의 `target_table`이 아닌 다른 테이블을 read하여 계산하는 교차 테이블 의존(cross-table read dependency)** 은 이 가드가 보호하지 못합니다. 선행 그룹이 테이블 X를 쓰고 후행 그룹의 매퍼가 X를 읽어 테이블 Y를 쓰는 경우, 두 그룹의 `target_table`(Y vs X)이 달라 보류 대상이 아니므로 **순서가 보장되지 않습니다**. 이런 의존이 있는 규칙은 현재 선언적으로 표현할 수 없으며, 필요 시 규칙 스키마에 `depends_on`(교차 테이블 선행 조건) 도입을 검토해야 합니다.
...
## 4. ⚰️ 온톨로지 그래프 승격 (Graph Materializer) — **은퇴** (2026-08-14 `2ec78b9` · R-2026-08-14-H)

🔴 **이 절 전체는 더 이상 현행 서술이 아닙니다.** 아래 흐름은 **아무도 실행하지 않습니다** — `graph_sync_worker`가 프로세스 스택에서 빠졌고, 저장소 `graph_nodes`·`graph_edges`·`graph_sync_state`는 **DROP**됐으며(약 841 MB), 진입 라우트 일곱은 **410**을 답합니다. 은퇴 계약의 정본은 [backend §2](./backend.md)의 은퇴 블록입니다.

- **은퇴 근거는 실측입니다** — 이 워커에 `ledger` 참조 **0건**, `server/ledger/`에 그래프 결합 **0건**. **두 갈래가 같은 outbox·같은 소스 표를 각자 읽으면서 서로를 몰랐고**, 원장(`ledger_events`)이 개체 층인데 이쪽이 사본을 하나 더 들고 있었습니다.
- **outbox 자체는 그대로입니다** — 소비자가 셋에서 **둘**(체인 워커 · 스케줄러)이 됐을 뿐입니다. §1~§3의 outbox 계약은 이 은퇴에 **한 줄도 영향받지 않습니다.**
- 🔴 **저장소를 지우는 것만으로는 은퇴가 아니었습니다** — 표를 **다시 만들** 경로가 셋 있었고(부팅 `create_all` · 핫리로드 `ensure_graph_tables` · **스케줄러의 고아 스윕**) 셋 다 봉인됐습니다. 그러지 않았다면 재기동이 빈 표를 돌려주고 뷰어가 「그래프가 아직 비어 있습니다」라 말했을 것입니다 — **은퇴가 「아직 안 채워짐」의 옷을 입는 것.**
- **후계**: 개체 층은 원장([LEDGER_GUIDE](../guide/LEDGER_GUIDE.md)), 걷기는 `GET /api/ledger/subgraph` **하나**, 유형 층은 `GET /api/ledger/declaration`. 🔴 **[2026-08-28] 종전 이 줄이 대던 `GET /api/ledger/trace` 와 `GET /api/ledger/structure` 는 둘 다 «없다».**

<details>
<summary>⚪ 이하 §4.1~§4.5는 <b>역사 기록</b>입니다 — 「무엇이 있었나」를 읽을 때만 펼치십시오. 여기 적힌 어떤 흐름도 오늘 돌지 않습니다.</summary>

인제션·교정되는 모든 로우를 매핑 config에 따라 PG 엣지 스토어(`graph_nodes`/`graph_edges`)의 속성 그래프로 자동 승격하던 데몬입니다(`graph_sync_worker.py` + `graph_materializer.py`, :8090). 설계 배경은 [ONTOLOGY_GRAPH_SPEC](../spec/ONTOLOGY_GRAPH_SPEC.md)(🗄️ 부분 대체).
...
읽기는 워커가 아니라 웹서버(main.py)의 read-only API가 담당했습니다 — ⚰️ **[2026-08-14] 은퇴 · [2026-09-05 정정] 종전 「지금 전부 410입니다」는 거짓입니다**: 거절 스텁까지 `8ffe23d7` 에서 삭제돼 `/graph/*` 는 SPA catch-all 이 **index.html + 200** 으로 답하고 `/api/graph/sync` 만 404 입니다([backend §2](./backend.md)의 은퇴 블록).
