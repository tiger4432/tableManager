...
> 🔴 **[2026-09-15 22:3x 박스 장애 → 판정 406 · S-252 — «지시됨, 아직 착지하지 않음»]** 통합 모드에서 이 루프는 API 와 «같은 이벤트 루프»에 삽니다. 그날 루프가 자기가 «소비할 수 없는» 제어 행(`RETROACTIVE_RUN` — 스케줄러의 것)을 틱마다 집어 건너뛰고, «대기열이 비지 않았다»는 이유로 대기를 안 타서 **await 없는 고리**가 됐고 API 이벤트 루프가 굶었습니다(리플레이 안 돎 · audit log 새로고침 굳음 · DB 막힘 0 — py-spy 로 잰 것). 판정 406: **API 프로세스는 체인 루프를 «안 돌리게 될 것»입니다 — 런처(기동 스크립트)가 uvicorn 에 `ASSY_CHAIN_WORKER=0` 을 넘기고 체인은 `run_chain_worker` 로.** 오늘 그 변수는 킬스위치일 뿐이고 기본은 «켬»입니다(`main.py` 기동부). S-252: 루프는 «자기 소유 행만» 집고 «처리 0 이면 대기»하게 «될 것»입니다. 둘 다 **지시서에 올라 있고 착지하지 않았습니다** — 착지하면 이 문단과 [RUNTIME_MAP §0](./RUNTIME_MAP.md) 의 「통합 모드」 상자를 같이 고칩니다.
