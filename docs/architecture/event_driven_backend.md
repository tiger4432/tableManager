...
| ② | 로드 시점 그래프 검증 (`_validate_chain_cascade_graph`) | 워커 «뜨기 전» | 옵트인 엣지만으로 «고리»가 생기는 설정 | 🔴 「길다」와 「무한」을 **구별 못 합니다** — 유한한 고리도 «같은 거절»을 받습니다 |
| ③ | 🆕 실행 시점 깊이 (`chain_depth` · `max_chain_depth`) | 이벤트마다 | «몇 홉»을 넘는 연쇄 — 이름을 대고 거절하고 그 행을 «끝냅니다»(FAILED) | 🔴 «새» 연쇄를 안 막습니다. 깊이는 «무한»을 막지 «시작»을 막지 않습니다 |

🔴 **①과 ② 사이가 «비어 있었습니다».** ①은 옵트인(전면 금지가 너무 빡빡해서 풀린 것)이고
②는 「고리면 설정 «전체» 거절」이라, 「선언은 했는데 얼마나 깊어지나」에 답하는 것이 «없었습니다».
