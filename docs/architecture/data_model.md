...
| **컬럼 추가의 순서 위험** | 🔴 **양방향으로 방어돼 있어 순서에 의존하지 않습니다.** 쓰기 쪽은 `schema.ensure_schema`가 같은 추가 문장을 **모든 백필의 첫 단계**에 적용합니다(번역기가 못 쓰는 표를 만날 수 없습니다). 🪦 **읽기 쪽의 정본이던 `ledger_trace.coverage` 는 2026-09-16 에 «없어졌습니다»**(S-261 `8868d1a0`) — 오늘 그 자리를 지키는 것은 **둘이고 «부재의 모양»이 서로 다릅니다**: ① walk 라우트(`trace_router._subgraph_contract_state`)가 `pg_attribute` 로 **먼저 묻고**, 없으면 그 필드 없이 답하는 대신 **503 `source_event_projection_not_deployed` 로 빠진 컬럼·인덱스를 «이름 대어» 거절**합니다 ② 커서를 읽는 `ledger.admin.ingestion_view` 는 고정 컬럼 목록을 SELECT 하고 **실패를 잡아** `unavailable` 에 사유를 실어 답합니다(500 이 아닙니다). 카탈로그만 읽는 컬럼 프로브의 공용 자리는 `ledger.admin.relation_columns` 입니다. 이 프로젝트가 `add_frame_confirmation.py`에서 이미 값을 치른 위험입니다 |
...
| **어휘·개체 타입** | **컬럼이 아니라 선언**입니다(`server/config/ontology/ledger_config.json`의 `vocabulary`·`entities`) — 코드가 낱말을 따로 들고 있던 층은 2026-08-27에 사라졌습니다 — 저장 계층은 술어를 **문자열**로 받고 CHECK 둘(`register`의 ∅, `subject_keys`가 객체)만 압니다. 2026-08-14에 술어가 **일곱 → 아홉**(`processed_with`·`has_param`), 개체 타입에 **`Recipe`**(키 `["recipe","rev"]`)가 들어왔고, **3차에 `observed`까지 «열하나»**가 됐습니다(`transferred` 포함). 🔴 **스키마 변경은 매번 0줄입니다** — 어휘 확장이 DDL을 건드리지 않는 것이 이 설계가 사려던 것이고, 그래서 **마이그레이션도 재배포도 필요 없습니다.** 계약은 [spec §3.7](../spec/LEDGER_TECHNICAL_SPEC.md) |
...
#### 1.2-bis.1 🔴 결함 종류 레지스트리 — 종류는 **분기가 아니라 조회**입니다 (`server/ledger_api/finding_kinds.py`)
...
| `server/ledger_api/ledger_siblings.py`의 `runs`/`scanned`/`found` 조립 | 케이스-컨트롤 화면 경로 | **런을 시간 창으로 좁혀야** 하고, `found`를 **그 좁혀진 런을 «통해»** 정의해야 합니다 — 그래야 「창 안에서는 깨끗하고 창 밖에서 발견된」 패키지가 여기서 found로 세어지지 않고, `found ⊆ scanned`가 유지됩니다(그것이 깨지면 `clean = scanned − found` 자체가 틀립니다) |

🔴 **그러므로 「철자가 하나다」라고 쓰지 마십시오 — 지금은 둘이고, 둘이 갈라지는 날을 «탐지하는 것이 없습니다».** 종류를 더하거나 창 규칙을 손볼 때는 **양쪽을 다** 보십시오.
⚠️ **같은 수를 말하는 자리가 셋이고 값이 다릅니다** — `server/ledger_api/finding_kinds.py`의 docstring 두 곳은 **277,500**, `server/ledger_api/ledger_siblings.py`는 **280,000**, 위 실측은 **280,001**입니다. **크기의 논증은 어느 쪽이든 같지만 인용할 때 출처를 붙이십시오**(총괄 판정 전까지 여기서는 **나중 측정**을 싣습니다).
...
### 2.1-quater 빈 칸은 층이 아니다 — 「비웠다」를 뜻할 수 있는 저자만 NULL 층을 세운다 · 2026-09-15 판정 405 (S-243 `fac454af` · S-243-b `478fcf32`)

소유자: 「빈 층 고의 입력은 진짜 빈 것, 그냥 없던 것은 아직 입력하지 않은 것」. 둘은 종전에 **어디서도 구별되지 않았습니다** — 파일의 빈 셀이 NULL 로 캐스팅돼 «그 소스의 층»으로 저장됐고, 사람이 손으로 지운 것과 같은 모양이었습니다. 파일 소스(`pipeline_parser` 2 · 미등재 99)는 체인(4)보다 서열이 높으므로 그 NULL 층이 **조인이 옳게 쓴 값을 영구히 가렸습니다** — 조인은 계속 돌고 계속 옳은데 화면은 빈칸이었습니다.
...
> 3. **호출자** — `crud.apply_batch_updates(..., drop_report={})`(2026-08-11 신설). **1·2번이 답하지 못하는 질문이 「내 쓰기가 착지했나」입니다** — 프로세스 수명 계수기에는 배치도 행도 트랜잭션도 없기 때문입니다. `replace_report`와 **같은 out-parameter 계약**(4-튜플 반환 시그니처가 ~10개 운영 호출 지점에서 언패킹되므로 반환값에 실을 수 없습니다). 사유는 이름으로 옵니다 — `undeclared_column`(config 편집) · `system_column`(발신자 버그) · `rows_refused`(버전 게이트) · 🆕 `absent_not_written`(빈 값을 층으로 «안 쓴» 것 — 판정 405, §2.1-quater. 실패가 아니라 계수). 상세는 캡이 있고 **건수는 캡이 없으며** 보고서가 스스로 얼마나 감췄는지 말합니다.
>
