...
| **어휘·개체 타입** | **컬럼이 아니라 코드의 선언**입니다(`server/ledger/vocabulary.py`) — 저장 계층은 술어를 **문자열**로 받고 CHECK 둘(`register`의 ∅, `subject_keys`가 객체)만 압니다. 2026-08-14에 술어가 **일곱 → 아홉**(`processed_with`·`has_param`), 개체 타입에 **`Recipe`**(키 `["recipe","rev"]`)가 들어왔습니다. 🔴 **스키마 변경은 0줄입니다** — 어휘 확장이 DDL을 건드리지 않는 것이 이 설계가 사려던 것이고, 그래서 **마이그레이션도 재배포도 필요 없습니다.** 계약은 [spec §3.7](../spec/LEDGER_TECHNICAL_SPEC.md) |
