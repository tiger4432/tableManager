...
| | |
|---|---|
| **테이블** | `ledger_events` + `ledger_translator_cursor`. **`models.py`에 없습니다** — DDL의 유일한 철자는 `server/ledger/schema.py`이고 `server/migrations/add_ledger_events.py`가 그것을 부릅니다(사본이 없으니 드리프트할 자리도 없습니다). 부팅 시 `server/ledger`를 import하는 프로세스는 **없습니다** — 마이그레이션을 안 돌려도 아무것도 안 깨집니다 |
| **컬럼** | 열하나 — 7필드 봉투를 편 것입니다: `id`(uuid7) · `subject_type` · `subject_keys`(JSONB) · `predicate` · `object_kind` · `object_payload`(JSONB) · `occurred_at` · `source_who` · `source_translator_ver` · `source_raw_ref` · `supersedes` |
...
### 1.2-bis void(보이드) 스키마 — 분모가 자기 행을 갖는다 (2026-08-13 `346aa88`)

SAT 보이드 관측을 담는 **동적 테이블 둘**(`table_config.json` 선언, `product_tables.py` 아님). 여덟 컬럼짜리 소스가 말하는 것은 **어디**(패키지·스택 층·다이 안 위치)와 **얼마나 큰가**(타원 크기) 둘뿐이고, 수율이 필요로 하는 셋째를 말하지 못해 테이블이 둘입니다.

| 테이블 | 무엇 | 업무 키 |
|---|---|---|
| `inspection_run` | **분모** — 스캔이 «있었다»는 사실 하나당 한 행 | `run_uid = method\|base_wafer_id\|base_x\|base_y\|stack_gate\|observed_at` |
| `void_obs` | **관측** — 한 스캔이 본 보이드 하나당 한 행 | `void_uid = run_uid\|inchip_x\|inchip_y` |
