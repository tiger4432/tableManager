...
> - **`/admin/*` 라우트 수 재실측** — 종전 「16개」는 낡았습니다. 🔴 **[2026-08-06] 그 재실측(**22개**)도 이미 낡았고, 그것이 이 항목의 교훈입니다 — 라우트 수는 라운드마다 바뀌므로 산문에 적으면 매번 낡습니다.** 세지 말고 실행하십시오: `grep -cE '^@app\.(get|post|put|delete|patch)\("/admin' server/main.py`(2026-08-06 실측 **26**). 커버리지의 정본은 수가 아니라 **라우트 테이블 열거**입니다.
> 
