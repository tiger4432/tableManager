...
- 채점: `server/tests/test_schema_drift_startup.py`(24). 부팅 경로를 **재기동 없이** 검증하는 3층 ― ⓐ `run_at_startup`을 일부러 좁힌 SQLite에 직접 먹인다 ⓑ 두 호출 자리의 **순서는 소스로 단언**(포트 가드와 같은 기법) ⓒ `--preflight-only`로 **진짜 런처가 진짜 DB에 진짜 점검을 돌리되 아무것도 안 띄운다**.
...
2-ter. **집합 기반 쓰기 경로 (P3, 2026-08-07)** — 대용량 파일 인제션(신규 삽입)에서 **행마다 나가던 3문장을 청크마다 나가는 문장으로** 바꿨다. 실측(격리 `assy_qa`, 100,000행 맵 파일, **이 워크스테이션 — 운영 아님**): 문장 **301,100 → 1,200**(행당 3.011 → 0.012), 벽시계 **796.2s → 375.8s(2.12×, 7.96 → 3.76 ms/행)**. 쓰인 행 수는 전부 동일하다(데이터 100,000 / `cell_sources` 700,000 / `audit_logs` 100,000 / `database_outbox` 100,000).
  ...
  * **`bulk_upsert_cell_sources`/`_overwrites`는 청크마다 VALUES 절을 새로 짓지 않는다.** `_is_executemany_safe`가 게이트다(키 집합이 균일하고 값에 `ClauseElement`가 없을 때만). 🔴 **`BULK_CHUNK_SIZE` 청킹은 그대로다** — 바뀐 것은 **어떻게 보내는가**이지 **한 번에 몇 행인가**가 아니다. 충돌 병합이 이 dict에 넣던 `func.now()` 3곳은 실제 datetime으로 바꿨다(같은 함수의 다른 생산자들은 원래 그랬다).
    ...
    * **수리(`_pg_multirow_upsert`): 청크마다 진짜 다중 행 `VALUES` 문장 하나를 보낸다.** 같은 20,000 매핑이 **문장 20개**가 되고, 벽시계 **5.65s → 1.19s(−79%)**, 3라운드 교차 측정 전부 같은 방향. 함수 단위 A/B(격리 `assy_qa`, 100,000 매핑 = 6,250행 × 16컬럼, 3라운드 교차): `bulk_upsert_cell_sources` **31.2s → 7.8s(−75.0%)**, `bulk_upsert_cell_overwrites` **29.2s → 6.9s(−76.3%)**. 저장된 행은 7개 컬럼 전부 **바이트 동일**(같은 매핑을 양쪽 경로로 써서 대조 — `value::text` 포함).
    * 🔴 **`exec_driver_sql`이지 원시 커서가 아니고, 그 차이가 계약이다.** `psycopg2.extras.execute_values`도 같은 속도(1.21s)였으나 **거절했다** — 원시 커서는 `psycopg2.Error`를 던지지 `sqlalchemy.exc.IntegrityError`를 던지지 않으므로, 그 클래스를 잡는 2-quater의 업무 키 회복이 분류해야 할 예외를 **못 보게 된다**(그리고 Session이 자기 트랜잭션이 깨진 것을 모른다). 주입해서 확인함: `NOT NULL` 위반이 새 경로에서 `sqlalchemy.exc.IntegrityError`로 올라오고 `orig`는 psycopg2 `NotNullViolation` 그대로다.
    * **거절(=구 경로로 폴백)하는 조건 셋**: PostgreSQL+psycopg2가 아니다 · 매핑 키가 테이블 컬럼이 아니다 · **파이썬측 `default`를 가진 컬럼을 매핑이 빠뜨렸다**(`CellOverwrite.is_overwrite`가 `default=True`다 — SQLAlchemy는 채워 보내고 생 SQL은 NULL을 넣는다). 셋 다 주입해서 실제로 발화하는 것을 확인했고, SQLite는 첫 조건에 걸려 **스위트는 이 경로를 한 번도 타지 않는다**(그래서 증거는 PostgreSQL 실측뿐이다).
