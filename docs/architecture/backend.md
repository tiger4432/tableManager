...
> 
> ### 이번 라운드 (2026-07-31)
> - **`GET /admin/config/resolve` 행의 ⏳ 해제** (`93610cb`) — 클라 렌더러가 착지했고 **INV-F9-4가 `PENDING`에서 실행 채점으로** 바뀌었습니다(하네스가 `config_resolve_view.js`를 임포트). 🔴 뷰 모델이 DOM 빌더 안으로 되돌아가면 그 절반이 다시 `PENDING`이 됩니다.
> - **읽기 실패의 분류는 상태코드가 아니라 헤더입니다** (`1dc761b`) — 401이 우리 게이트라는 보장은 상태코드에 없고, 판정은 **`WWW-Authenticate: X-Admin-Token`**입니다. 앞단 프록시가 같은 포트에 자기 `Basic realm=…`으로 답한 인시던트가 실제로 있었습니다.
> - **§1의 CORS 문단에 `expose_headers` 규율 신설** (`cde3398`) — 노출하지 않은 헤더는 **브라우저가 교차 출처에서 지우므로**, vite dev 오리진에서만 게이트 거부가 「앞단이 답했다」로 잘못 표시됐습니다. 값은 헤더 **이름**뿐이라 비밀이 없습니다.
> 
