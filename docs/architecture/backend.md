...
2-ter. **집합 기반 쓰기 경로 (P3, 2026-08-07)** — 대용량 파일 인제션(신규 삽입)에서 **행마다 나가던 3문장을 청크마다 나가는 문장으로** 바꿨다. 실측(격리 `assy_qa`, 100,000행 맵 파일, **이 워크스테이션 — 운영 아님**): 문장 **301,100 → 1,200**(행당 3.011 → 0.012), 벽시계 **796.2s → 375.8s(2.12×, 7.96 → 3.76 ms/행)**. 쓰인 행 수는 전부 동일하다(데이터 100,000 / `cell_sources` 700,000 / `audit_logs` 100,000 / `database_outbox` 100,000).
  ...
  * **`bulk_upsert_cell_sources`/`_overwrites`는 청크마다 VALUES 절을 새로 짓지 않는다.** 문장을 한 번 컴파일하고 파라미터 목록을 넘긴다(`_is_executemany_safe`가 게이트: 키 집합이 균일하고 값에 `ClauseElement`가 없을 때만). 🔴 **`BULK_CHUNK_SIZE` 청킹은 그대로다** — 그것은 튜닝 손잡이가 아니라 int16 파라미터 한계이고, 바뀐 것은 **어떻게 보내는가**이지 **한 번에 몇 행인가**가 아니다. 충돌 병합이 이 dict에 넣던 `func.now()` 3곳은 실제 datetime으로 바꿨다(같은 함수의 다른 생산자들은 원래 그랬다).
