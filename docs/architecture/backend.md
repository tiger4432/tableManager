...
> **Status:** 🟢 Living | **Last-verified:** 2026-07-30 (**[F9] 후속 정정 2건** — ① `/admin/config/resolve` 행에 ⏳ **클라 렌더러 미착지** 표기(실측 `client2/src`에 소비자 0건 — 종전 「클라는 그것을 그대로 렌더하고」는 **구현의 서술처럼 읽혔으나 계약이다**). 계약 하네스가 금지 절반(INV-F9-7)만 채점하고 긍정 절반(INV-F9-4)은 `PENDING`으로 보고한다는 사실과, **INV-F9-8**(`detail`에 Python repr·리터럴 마크다운 금지 — 클라가 문장을 못 지으므로 **하류에 고칠 자리가 없다**, `f9289f6`)을 명기. ② `GET /enrichment/rules` 행의 참조뷰 노출을 **「`label` + `candidate_for` 둘만」**으로 못박고 설정 관점 정본을 링크 — [guide/config/enrichment_rules](../guide/config/enrichment_rules.md)의 필드 표가 같은 커밋 안에서 「클라엔 `label`만」과 모순돼 있었다. 직전 **[F9] §2 표에 config 해석 라우트 2건 신설 + enrichment 2행 갱신** — ① `GET /admin/config/resolve`(선언 → effective/ineffective/rejected, 닫힌 사유 어휘는 **런타임 열화 어휘 재사용**, DB 질의 0건) ② `GET /admin/enrichment/auto-confirm/dry-run`(읽기 전용 sweep 노출, **HTTP에 `apply` 없음**) ③ `GET /enrichment/rules`에 **가산 필드 `candidate_for`**(총괄 승인 — 경계 계약 변경) ④ 참조뷰 실행이 `execute_reference_view` 단일 정의로 수렴(라우트의 인라인 사본 제거). 직전 **[F8] 어드민 게이트 2행 갱신** — ① 토큰 **지문**(sha256 앞 8자리)이 웹서버·데몬 양쪽 기동 배너에 남는다: 유출 규율의 유일한 예외이고, 그 예외가 없어서 "양쪽이 같은 토큰인가"를 제품이 답하지 못했다. ② `/internal/events/*` 전송 계층을 `internal_event_client`가 독점(`trust_env=False`) — loopback 통지가 사내 프록시로 나가 403을 받은 운영 장애의 근본 수리. 직전 **Directory Watcher 행에 제자리 적재 반영**(`600b49d`) — 폴더 드롭이 **평탄화가 아니라 제자리 디스패치**가 됐고 상대 POSIX 경로가 `self.rel_path`로 파서에 전달된다. 접두 승격 기계장치 삭제 · 아카이브는 `is_managed_source`로 조건부 · `_unique_dest` 충돌 방어 · 토글 키는 뜻이 바뀐 채 `flatten_nested_dirs` 유지. 상세·`filename_rules` 선언 규격은 [INGESTION_GUIDE §1.9/§1.9-bis](../guide/INGESTION_GUIDE.md)가 정본. 직전 **그래프 조회 표에 문서 없던 경계 계약 2건 등재** — `server/main.py` 실측(`get_chip_trace`·`_chip_trace_leg`·`_chip_trace_declaration`·`get_graph_mapping_summary`, `GRAPH_CHIP_TRACE_*` 상수). ① **`GET /graph/chip-trace` 신설 행**(`8670e3b`+`ae2811c`): 파라미터 `identity` 하나·**depth 없음**·고정 형상 3다리·**다리별 닫힌 어휘 5종 + `scope_unresolved`**(`mapping_unavailable`과 `not_reached`가 왜 별개인지 포함)·절단은 상태가 아니라 플래그·응답 `declaration{}`. 이 라우트는 `/graph/*` 전 라우트가 등재된 이 표에서 **빠져 있었다**. ② **`/graph/mapping-summary` 행 갱신**(`530fdfd`): `rejected[]`·`rejected_count`·`source{path, exists}` — **파일 부재는 거부가 아니라 `source.exists`**. 직전 **[F5] 로드 시 프리셋 라우팅 신설** — §2 표에 `GET /api/maps/preset-routing` + 새 모듈 `server/map_preset_routing.py`. 선언은 `map_overlay_config.preset_routing`, 프리셋 본문은 `maps.json`. **저장된 메타 > 라우팅 > 패널**을 서버가 강제하고, 조회 테이블 부재·miss는 정상으로 취급해 조용히 패턴 규칙으로 떨어진다. 직전 **F3 검수 라운드 반영** — §2.1의 실측 수치 정정(`leg` distinct **342**·`base` **397,602**·인덱스 **418MB**·loose scan **~33ms**로 세 문서 단일화; 종전 6/11,058/370MB/19~41ms는 오기였고 첨부 실행계획이 이미 반증하고 있었다), 그리고 규율 3줄 신설 — **대소문자 무시의 기준은 DB의 `lower()`**(`db_fold`로 접두를 같은 함수로 접는다), **술어는 정확한 범위**(상한 자리올림 — 재검사 없는 `/graph/nodes/search`가 두 번째 소비자다), **무효 인덱스는 없는 것보다 나쁘다**(`indisvalid AND indisready`). 직전 **F3 고유값 조회 신설** — §2 표에 `GET /tables/{t}/columns/{c}/values` + **§2.1 신설**(loose index scan·바이트 순서 접두 인덱스·`truncated` 계약·강등 규율). 같은 술어를 `/graph/nodes/search`가 재사용하도록 `ILIKE 'q%'` → 범위 비교로 교체(의미론 불변, 인덱스 사용). 직전 **부팅 경로 조용한 실패 수리(#13/#16ⓐ)** — §1에 ⓐ DDL이 **import 시점 → `bootstrap_database_schema()` 기동 단계**로 이동(테스트 수집이 운영 DB에 DDL을 내던 경로 차단, 신규 설치 자동 생성은 유지), ⓑ 손상 `table_config.json`에 대한 **기동 fail-fast** 등재. 직전 **정본 계기 수리 라운드** — `PUT /data/updates` 응답에 `effort_recorded`/`effort_error` 신설, 잘못된 `effort`는 **교정을 막지 않고** 폐기·보고, 최상위 오타 키 보고, `unavailable_reason` 실제 원인 지목. 직전: **정본 계기 서버 구현 착지** — `PUT /data/updates`의 선택 필드 `effort` + `GET /api/effort/config` + `/dashboard/summary` → `effort` 신설, 재교정률은 **보조 계기**로 정정. 직전 2026-07-27: C1 공유 토큰 게이트 + `/internal` 게이트 + 정적 폴백 containment · `source-summary` BIN 축 신설) | **Owner:** Backend / Sync
...
| `GET /admin/config/resolve?domain=` | **[F9 2026-07-30]** config **선언의 효과**. `{domains:[{domain, title, sources[], settings[], effective[], ineffective[], rejected[], counts}], vocabulary}`. 🔴 세 모집단을 **이름으로** 반환하고 사람이 읽을 문장(`detail`)을 **서버가 만든다** — 클라는 그것을 **그대로 렌더해야 하고** 「효과 없음」을 스스로 판정해서는 안 된다(하드코딩 사본 계급 U6). ⏳ **클라 렌더러는 아직 없다**(2026-07-30 실측: `client2/src`에 이 두 라우트의 소비자 0건 — 어드민 화면이 아니라 `curl`/토큰으로 읽는다). 그래서 위 문장은 **구현의 서술이 아니라 계약**이고, `contracts/config_resolve_report/client_harness.mjs`가 그렇게 적어 두고 있다: 금지 절반(**INV-F9-7** — 사유 4단어가 `client2/src`에 **소스 리터럴로** 있으면 divergence)은 렌더러 없이도 채점되고, 긍정 절반(**INV-F9-4** — 렌더된 문장 == 서버 `detail`)은 **`PENDING`으로 이름 붙여** 보고된다(통과로 세지 않는다). 🔴 **INV-F9-8 — `detail`은 운영자가 읽는 최종 문자열이다**(`f9289f6`): Python repr(`['slot']을(를)`)도 리터럴 마크다운(`**…**`)도 실어 보내지 않는다. 클라가 자기 문장을 짓는 것이 금지돼 있으므로 **하류에 고칠 자리가 없다** — 운영자에게 되비추는 값은 그가 편집한 파일의 문법인 **JSON으로** 적는다(JSON `true`는 `'true'`가 아니다). 사유는 닫힌 어휘 4종이고 **전부 런타임 열화 어휘 재사용**(`not_declared`·`mapping_unavailable`·`scope_unresolved`·`not_reached` — `main.CHIP_TRACE_*`의 부분집합임을 계약 테스트가 검사). `settings`는 실효값 + **그 값이 온 파일**을 말한다(파일 부재로 기본값인 경우 포함). **DB 질의 0건**(config만) — 그래서 요청 경로에 앉을 수 있다. 구현 `server/config_resolve_report.py`(`_RESOLVERS`에 도메인당 등록기 1개, 2026-07-30 현재 `enrichment` 1개), 계약 벡터 `contracts/config_resolve_report/` |
