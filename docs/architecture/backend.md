...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-04 | **Owner:** Backend/Sync
> 
...
- 🔴 **`_declared()`는 `TESTING`을 남기지 않는다.** 바 스크립트에서만 세우고 되돌린다. 런처는 `os.environ.copy()`를 자식 다섯에 물려주므로, 여기서 새면 웹서버가 자기 부팅 DDL을 거부하고 워커가 전부 일을 건너뛴다.
