...
>
> ### 이번 라운드 (2026-08-06 · `2fc4f00` — 업서트 레인이 자기 변경만 적고 간 자리)
...
2. **replace_map 모드** — 맵 저장 시 `map_key_columns` 기준으로 기존 행·`CellSource`·`CellOverwrite`를 bulk purge 후 신규 활성 칩만 재적재(유령 셀 0%). purge 범위는 `derive_replace_map_scope()` 단일 리졸버가 결정한다(요청의 명시적 `scope` 필드 우선, 없으면 `updates[0]`에서 파생). **범위를 못 잡으면 `ValueError` → 라우터 400** — 과거의 "아무것도 안 지우고 200" 무음 no-op은 폐기됐다(2026-07-28 U6). 명시적 `scope` + 빈 `updates`는 합법적 전량 소거이며, 응답 `scope`로 실제 사용된 필터와 건수를 정직하게 알린다(라우터가 같은 리졸버로 echo).
  * **응답 `scope`의 필드**(`main.py`가 조립 — **개수를 적지 않는다. 필드는 라운드마다 붙는다**): `filters`(실제 purge 필터) · `deleted`(지운 행 수) · `inserted`(이 페이로드가 새로 만든 행 수) · **[2026-08-06 `2fc4f00`] `adopted`**(선언한 스코프 **밖**에서 데려온 행 수) · **`mode`**(현재 항상 `"purge"`) · **`reason`**(어느 리졸버가 `filters`를 만들었나 — `declared_map_key_columns` \| `legacy_column_derivation`) · **`delete_ids_omitted`**(`batch_row_delete` 브로드캐스트가 `BROADCAST_ITEM_LIMIT` 초과로 **보류한** id 개수. 정상 경로 0, **조용히 생략되는 일은 없다**). 🔴 **종전 이 자리는 `{filters, deleted, inserted}`라고 적고 있었고 그것은 이제 불완전하다.**
  * 🔴 **[`2fc4f00`] 맵 A로의 Push가 맵 B의 행을 가져올 수 있고, 지금은 그 사실을 보고한다.** 행 조회는 **업무 키만으로** 하고 스코프 필터가 없다(`_get_or_create_row` → `get_row_by_business_key`). 맵 키가 업무 키 **안에** 있는 테이블에서는 일어날 수 없지만(키가 자기 맵을 지목한다), `dt_log`는 **일부러** 그렇지 않다 — 키는 물리 목적지 셀 `(dt_job, dt_x, dt_y)`이고 `(dt_lot, dt_slot)`은 **추론 대상이라 신원 밖에 둔다**(「추측이 신원 안에 앉으면 안 된다」). ⚠️ **신원을 스코프 지역으로 바꾸는 것이 수리가 아니다** — 그러면 물리 테이프 셀 하나에 행이 둘 생겨 config가 선언하고 데이터가 지키는 유일성이 깨진다(실측 8,995행 / 8,995 고유 키 / 중복 0). 결함은 쓰기가 아니라 **쓰기가 안 보였다는 것**이었다: `deleted: 0`, delete 이벤트 없음, 맵 B를 보던 화면은 셀이 떠난 것을 끝내 몰랐다. 이제 그 행들은 `deleted_row_ids`에 실려 **떠남으로 통지**되고, 동시에 `replace_report["adopted_row_ids"]`에 **따로** 남는다 — 한 리스트로 합치면 「사라졌다」와 「옮겨 갔다」를 되살릴 수 없기 때문이다. 판정은 `is_new`가 아니라 **스코프 행 id와의 차집합**이다(오늘은 purge가 이미 스코프를 비워 둘이 우연히 일치하지만, 스코프 diff 라운드가 착지해 in-scope 행이 살아남으면 갈린다). 그물 `server/tests/test_replace_map_cross_scope.py`
