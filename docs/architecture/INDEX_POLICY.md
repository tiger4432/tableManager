...
- `ix_<표>_is_graph_synced` / `ix_<표>_needs_graph_rollback` — 읽힙니다(`assy_qa`의 `dt_inventory`
  354회). ⚠️ **[2026-08-14 `2ec78b9`] 그 읽는 함수의 «집»이 은퇴했습니다** — `graph_sync_worker`가 프로세스 스택에서 빠졌으므로 이 인덱스들은 이제 **읽는 사람이 0**입니다(다음 라운드에서 지울 근거가 이번에 생겼습니다). 종전 서술: 다만 **boolean 2값**이라 부분 인덱스가 맞고, 읽는 함수가 `graph_sync_worker`에서
