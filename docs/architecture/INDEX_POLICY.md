...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-31 (**「은퇴 안 하는 것」의 그래프 인덱스 두 행만** — 「다음 라운드에서 지운다」가 왜 안 왔는지가 판정으로 적혔습니다: 컬럼 DROP 은 되돌릴 수 없어 «1단»만 진행) · 직전 2026-08-14 (신설 — F6 라운드. `models.init_dynamic_models`가
...
>
> 🔴 **컬럼 «이름»은 오늘의 «바인드»이지 계약이 아닙니다.** 아래 표가 드는 이름은 그 역할이 이 설치에서 지금 무엇에 묶여 있나이고, **계약은 §0 의 «역할»**(`map_key_columns` → `composite_key_source` → `business_key`)입니다. 이름을 계약으로 읽고 그 컬럼에 인덱스를 파면, 역할이 다른 컬럼에 묶인 설치에서 **틀린 인덱스**가 됩니다 — 이 문서가 실제로 한 번 그렇게 틀렸습니다(아래 `lot_event` ①).
>
...
| **dt_log**<br>13,789행 | ① DT 잡 **하나의 셀 전부를 연다** ② 설비·시각으로 잡을 되짚는다(귀속 참조 뷰) | ① `(dt_job)` — `build_key_filters` · `core_usage_mapper` · `core_alignment_mapper` · `dt_inventory_metadata_mapper` · `dt_standard_map_mapper` · `enrichment_rules.json`의 참조 뷰 `WHERE dt_job = :dt_job`<br>② **(그 참조 뷰가 조인에 쓰는 두 컬럼)** — 이름을 여기 적을 수 없습니다: 술어가 `enrichment_rules.json` 의 **SQL 문자열**이라 컬럼을 그 설치의 뷰 «본문»이 정합니다 | ① ✅ `map_key_columns`<br>② ❌ **자동화 불가 — 손으로.** 이유: **조인 술어가 config JSON 안의 SQL 문자열**에 있습니다. 선언 어휘에 「이 두 컬럼으로 조인한다」를 적는 자리가 없습니다 → `add_dt_log_trigger_indexes.sql`(헤더가 **NOT RUN**이라 밝히고 있고 여전히 안 돌았습니다) |
...
| **lot_event**<br>44행 | ① 원장 백필이 **시각 순서로 걸어 나간다** ② 참조 뷰가 `track_in` 사건을 설비·시각으로 찾는다 | ① 🔴 **이 칸은 컬럼 이름을 적을 수 없습니다 — 둘 다 «선언에서» 옵니다**: **(그 소스의 `occurred_at.column`, 그 소스가 `row_identity` 에 바인드한 컬럼)**. `ledger/backfill.fetch_page` 가 `ORDER BY {time_column}, {identity}` 를 조립하고 `identity = columns["row_identity"]` 입니다 · `fetch_group` · `ledger/observability.probe_source_head`.<br>⚠️ **종전 이 칸은 `(event_time, business_key_val)` 이라 적고 있었고, 둘째 컬럼이 선언에도 코드에도 없습니다** — 커밋된 `config/sample/ledger_config.json.sample` 의 `sources.lot_event.read.cursor.columns` 는 `["event_time", "row_id"]` 이고, `row_identity` 는 `ledger/config.py` 의 `DECLARED_REQUIRED_COLUMNS` 가 요구하는 **역할 이름**이라 그것이 어느 물리 컬럼인지는 **설치마다 다릅니다**.<br>🔴 **그래서 수리는 `business_key_val` 을 `row_id` 로 갈아끼우는 것이 «아닙니다»** — 틀린 리터럴을 다른 리터럴로 바꾸면 설치가 달라지는 날 «같은 방식으로» 다시 틀립니다. 이 칸이 들 수 있는 것은 «술어»입니다<br>② **(그 참조 뷰가 `track_in` 을 찾는 데 쓰는 컬럼들)** — 위와 같은 이유로 이름을 적을 수 없습니다: `enrichment_rules.json` 의 SQL 문자열이 정합니다 | 🔴 **선언 인덱스 `(lot, event_type, event_time)`은 오늘 읽는 사람이 없습니다.** 선두 `lot`은 어디에서도 술어가 아니고 **SELECT만 됩니다.**<br>① ❌ **자동화 불가 — 손으로.** 백필의 키셋 커서는 `ledger_config.json`이 정하고 `table_config`가 아닙니다<br>규칙은 그래도 붙입니다(44행이라 비용 0에 가깝고, 규칙에 표별 예외를 파는 순간 규칙이 아니게 됩니다). **원장 백필이 규모로 돌기 시작하면 ① 인덱스가 필요해집니다 — 그때 손으로.** |
...
- `ix_<표>_is_graph_synced` / `ix_<표>_needs_graph_rollback` — ⚠️ **[2026-08-14 `2ec78b9`] 읽는 함수의 «집»이 은퇴했습니다** — `graph_sync_worker`가 프로세스 스택에서 빠졌으므로 이 인덱스들은 **읽는 사람이 0**입니다.
  🔴 **[2026-08-31 재대조 — 「다음 라운드에서 지운다」가 두 주 넘게 안 왔고, 그 이유가 판정입니다.]** 컬럼 `is_graph_synced`·`needs_graph_rollback`·`graph_synced_at`은 **이미 있는 표에 그대로 있고**, 서버는 스키마 응답과 행 페이로드에 그 셋을 **계속 실어 보냅니다**(사라진 것은 클라가 «그리는 자리»입니다 — [backend §2 은퇴 블록](./backend.md)·[data_model §1.2](./data_model.md)).
  **정리가 «되돌릴 수 있는 선»에서 갈립니다** — **1단**은 «새로» 만들어지는 표가 그 셋(과 인덱스 둘)을 안 받게 하는 것이고 **빌더만** 바뀝니다. 🔴 **여기서 「빌더만 고쳐서는 기존 테이블이 한 개도 안 바뀐다」는 «결함이 아니라 설계»입니다** — `create_all` 이 기존 표에 인덱스를 더하지 않는 그 성질이, 이 은퇴를 되돌릴 수 있게 만듭니다. **2단**(44개 표에서 컬럼·인덱스 DROP)은 **되돌릴 수 없어 판정되지 않았습니다**(총괄 판정 2026-08-31 · 보드 소유).
  🔴 **그러므로 이 두 행을 「곧 없어질 것」으로 읽지 마십시오** — 새 표에서는 안 생겨도 **기존 표에서는 계속 유지비를 냅니다.** 그 유지비를 멈추는 것이 2단이고, 결정은 이 문서가 아니라 보드에 있습니다.
  종전 서술: 읽힙니다(`assy_qa`의 `dt_inventory` 354회). 다만 **boolean 2값**이라 부분 인덱스가 맞고, 읽는 함수가 `graph_sync_worker`에서 **`[DEPRECATED — C-7]`** 표시입니다.
...
| **`lot_event` 백필의 키셋 커서** — `(그 소스의 occurred_at.column, row_identity 에 바인드된 컬럼)` | 아직 없음 | 백필의 키셋 커서는 **`ledger_config.json`**이 정합니다. `table_config`가 아닙니다.<br>🔴 **컬럼 «이름»을 적지 않습니다** — 둘째 값은 «역할»(`row_identity`)이고 어느 물리 컬럼인지는 설치마다 다릅니다. 종전 이 행은 `(event_time, business_key_val)` 이라 적고 있었고 그 이름은 선언에도 코드에도 없었습니다(§1 `lot_event` ① 참조) |
