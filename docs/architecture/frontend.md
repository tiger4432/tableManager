...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-13 | **Owner:** UI / Excel Interaction
> **Source-of-truth:** `client2/src/*`, `client2/vite.config.js`, `client/desktop_wrapper.py`
>
> ### 이번 라운드 (2026-08-13 2차 · `0c3ea65` + `d9b98ab`)
...
- **모듈 셋**: `ledger_trace_core.js`(순수 — 입력 정규화, 홉 해석, `basis` 파싱) · `ledger_trace_view.js`(렌더) · `ledger_trace.js`(조립·요청). 하네스 `client2/tests/ledger_trace_harness.mjs` + 픽스처 둘(`ledger_trace_live.json`·`ledger_trace_probe.json`).
...
- ⏳ **그 파스는 임시 정본입니다.** 홉에 구조화 `basis: {kind, name}`가 붙으면 통째로 사라집니다 — L2가 `predicate`를 필드로 올린 것과 **같은 논거**이고, 온톨로지 소유자가 [R-2026-08-13-C](../process/LEDGER_RULINGS.md)로 승인했습니다(시기는 2주차 enum 확장과 같은 커밋 — 라우트 계약을 두 번 흔들지 않습니다). 🔴 **상설 원칙으로 승격됐습니다: `reason`은 사람을 위한 산문이고, 화면이 분기해야 하는 사실은 반드시 구조화 필드로 나갑니다.**
