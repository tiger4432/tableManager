...
- **게이트는 계약 하네스만이 아닙니다**(2026-07-30 정정). 종전 이 줄은 *"4계약 전부 통과"*로 끝났는데, 그것은 **`check:contracts`의 상태일 뿐 게이트 전체의 상태가 아닙니다.** 지금 세 채점자가 있습니다:
  ...
  - `check:harnesses` — **`client2/tests/*.mjs` 발견식 스캔**(2026-07-30 밤 신설, `scripts/check_harnesses.mjs`). 계약 러너와 같은 모양이고 **빈 스캔은 실패**입니다. 실측 **커밋 트리 `77d27d3` 기준 18개 중 13개 강제 · 5개는 스크립트 안에 사유가 적힌 부채 목록**이며, 부채 항목이 초록으로 돌아오면 「목록에서 빼라」는 줄을 출력합니다. ⚠️ **수는 `client2/tests/*.mjs` − `KNOWN_RED` 하나로 나옵니다.** 그 디렉터리에는 **`seam_7b_oracle.py`도 있는데 파이썬이라 스캔 대상이 아닙니다** — 파일 19개 ≠ 하네스 19개입니다. 🔴 **직전에 적혀 있던 「16개 중 11」은 적힌 그 시점에 이미 틀린 수였습니다** — `map_key_datalist_harness.mjs`(`dde342c`)가 아무도 세지 않은 채 이미 착지해 실제로는 17이었고, `virtual_column_render_harness.mjs`(`4b50135`)가 18로 만들었습니다. **이 수는 하네스가 추가되는 커밋마다 낡습니다** — 인용할 일이 있으면 세지 말고 **러너를 돌려 그 출력을 읽으십시오.** 🔴 **이 게이트가 생긴 이유**: 직전까지 15개 중 **14개를 아무도 부르지 않았고**, 그 조건이 `split_registry_harness.mjs`를 몇 주 죽어 있게 뒀으며 `da8f390`이 두 개를 더 죽인 채 푸시되게 했습니다. **조용히 skip하면 게이트가 없는 것과 같으므로** 부채는 목록으로 드러냅니다. 🔴 **2026-08-04 (리팩토링 라운드 0, H1)**: 종료 코드만으로는 「0개 단언으로 빨강」(단언 전에 죽은 하네스)과 「N개 단언으로 빨강」을 구분하지 못해 죽은 하네스 3개가 부채로 위장했습니다 — 이제 **모든 하네스가 자기 요약 지점에서 `ASSERTIONS <ran> <failed>` 한 줄을 찍고 러너가 그것을 읽습니다**(러너는 세지 않고 읽기만 합니다 — 하네스의 자기 집계가 유일한 채점자). 초록인데 줄이 없거나 `ran=0`이면, 그리고 `KNOWN_RED` 항목의 기록된 기대쌍(ran은 바닥, failed는 천장)을 벗어나면 **부채 여부와 무관하게 BLOCKING**입니다. 단언 없는 순수 측정 프로브(`reposition_regime_probe.mjs`)는 `ASSERTIONS 0 0`을 찍으므로 초록으로 돌아오는 순간 러너가 「단언을 달라」고 거절합니다 — 의도된 강제입니다.
...
> **줄 수는 `wc -l` 실측입니다**(2026-07-30 · `state`/`api`/`grid`/`clipboard`/`ui` 다섯은 **2026-07-31 재실측** — `4b50135`이 다섯 모두를 건드렸습니다). 정확성이 아니라 **상대 규모**를 읽는 열입니다 — 어느 모듈이 이 화면의 무게중심인지 보기 위한 것이므로, 소수점 몇 줄이 어긋나는 것보다 **행이 아예 없는 모듈**이 훨씬 나쁩니다(실제로 `value_suggest.js`·`doe_bands.js`·`tsv.js` 셋이 행 없이 굴러갔습니다).
...
| `admin.js` | 3155 | 어드민 5탭(§5) |
| `config_resolve_view.js` | 235 | **config 해석 보고서의 뷰 모델(§5, F9)** — DOM 없는 순수 모듈. `GET /admin/config/resolve` 응답을 렌더 트리로 바꾸면서 **모든 문자열에 출처를 태그**한다(`server`=페이로드 원문 · `value`=페이로드 값의 JSON 철자 · `chrome`=고정된 클라 라벨표 · `count`=클라가 센 정수). DOM 빌더 안에 있으면 node에서 채점할 수 없어서 분리한 것이고, `contracts/config_resolve_report/client_harness.mjs`가 **이 모듈을 임포트해** INV-F9-4를 실행 채점한다 |
| `map_editor.js` | 8564 | 맵 에디터 + 페인트 잠금 + **오버레이 레이어**(§4) + 유효 다이 참조([MAP_EDITOR_SPEC §5.7/§5.7-bis](../spec/MAP_EDITOR_SPEC.md)). **이 저장소에서 가장 큰 클라 모듈**입니다. ⚠️ **「프레임 채택·저장 좌표 재배치」는 이 행에서 삭제됐습니다**(F8 `61440e6`+`94b9baa`로 심볼 8종이 소스에서 사라졌습니다 — 찾지 마십시오). 줄 수도 재실측입니다(종전 4209 → 8650 → **8564**) |
