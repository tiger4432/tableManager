...
> **Status:** 🟢 Living | **Last-verified:** 2026-09-17 (§3 표에 **행 둘 신설** — `disabled_reason.js`(C-124 `dbdd3443`) · `admin_token.js`(C-122 `c66a7332`) · §3.1 배너의 «사유» 정정(그리드에서 직접 돈다) · §3.1-bis `applyViewWriteGuard` → `write_guard.applyWriteGuards` · §3.3 `absent.js` 소비자 7 → 19 · 맵2 파일 수 표시) · 직전 2026-09-16 «후속» (§3 표 — 부팅 «순서»는 `startup.js` 에 산다, C-110 · §3.3 끝 — 택1의 빈 값은 `{}`, 내려가기의 저자는 `childOf` 하나, C-115) · 직전 2026-09-16 (**§3.1 배너 — 체인 쪽 신원은 `row_id` 다**(C-112 · 서버 S-254) · row_id 없는 행은 이름을 달고 서고 그 줄이 행을 «보여 준다»(C-114) · 큰 선택은 크기를 «말하고 막지 않는다»(C-113) · 배너 문구 한 벌 한국어 · **§3.3 끝 — 「택1」 노드 `oneOf`**(C-111 · 서버 S-241/S-241-b)) · 직전 2026-09-05 (**§3.3 신설 — 부재가 «일급»이 됐다**(`absent.js`·`count_with_absence.js`·`closed_list.js`) · §3 표에 **어드민 패널 여섯 · 진행 카드 · 부재 어휘** 행 · 🔴 **「묘비」 행이 거짓이 되어 묘비 주석으로 교체** — `graph_viewer.js`·`trace*.js` 넷과 `graph.html`·`trace.html` 은 **트리에서 제거**됐다(`45d8b66f`), 안내 페이지도 남기지 않는다) · 직전 2026-09-02 (**§3.2 신설 — 「몇 건인가」가 세 상태로**(개수가 늦게 온다) · §3.1 의 **`grid_rescope_menu.js` → `redo_banner.js`** 정정(우클릭 메뉴 → 헤더 배너) · `dropdown.js` 의 닫힘 한 벌) · 직전 2026-08-31 (**§3.1 신설** — 그리드가 원장을 아는 세 자리 + 은퇴한 그래프 컬럼 셋이 그리드에서 빠진 사실 · §6 overview 탭에 소급 블록·실행 목록·취소) · 직전 2026-08-29 (개정 6 — §4 의 `collect` 서술 정정) | **Owner:** Client
...
| 층 | 모듈 | 책임 |
|---|---|---|
| 진입/상태 | `main.js` · 🆕 **`startup.js`** · `state.js` · `config.js` · `theme.js` | 부팅(순서는 `startup.js::startup({prepare, tableChosen})` — 소켓 «첫 줄» → `prepare` → health → 표 목록 → `tableChosen`; `main.js::init` 은 그 둘만 넘긴다. 모듈 최상단이 DOM·CSS 를 안 만져 node 가 import 하고, 하니스가 «자르지 않고» 잰다 — C-110 `c96527d9`), 전역 상태, API 주소, 테마 |
...
| 맵2 | `map_editor2.js` + `src/map2/*` (⚠️ **[2026-09-17] 「18 파일」은 낡았다 — HEAD 는 `.js` 21. 수를 다시 적지 말고 `git ls-tree -r HEAD client2/src/map2` 로 셀 것**. 줄 수 10,437 은 2026-08-27 실측 기록) | 정렬 화면. 층 경계로 읽습니다 — `view_model` 은 DOM 없이 채점됩니다 |
...
| 🆕 **꺼진 컨트롤** | 🆕 **`disabled_reason.js`** | **「왜 못 누르나」를 «그 컨트롤 자기 자리»(`title`)에 두는 좌석 하나** — `setDisabledReason(el, why)`, `why === ''` 면 켠다. 마크업이 이미 단 `title` 은 `data-title-was` 에 기억했다 되돌린다. 소비자 다섯(`write_guard`·`redo_banner`·`rnd_board/walk_box_panel`·`walk/main`·`ontology_explorer_view`). 🔴 사유의 **철자는 부르는 쪽 상수**이지 여기서 짓지 않는다. ⚠️ **「끄기」와 「숨기기」는 다른 답** — 셸이 «제공할 수 없는» 기능은 `hidden` 이다(`map2/main.js`), 이 좌석이 아니다. 새로 `el.disabled = …` 를 쓰면 `tests/disabled_reason_harness.mjs` 의 전수가 그 자리를 «미분류»로 빨갛게 만든다(빚 상한 0) |
| 🆕 **어드민 전송** | 🆕 **`admin_token.js`** | **`/admin/**` 로 가는 전송과 「이 거절이 «우리 게이트»의 것인가」의 판정이 한 자리에** — `adminFetch`(토큰 헤더 · 503 본문 세우기 · 날아가는 중 토큰이 바뀌면 조용히 재시도 · 게이트 거절이면 «한 번만» 다시 묻기) + `isGateRejection`. 페이지가 정하는 것은 `deps` 둘(`askForToken` · `onServiceUnavailable`)뿐 — 안 넘기면 «안 한다». 🔴 **401/403 을 상태코드로 판정하지 말 것**: 판정은 `WWW-Authenticate: X-Admin-Token` 이다(앞단 프록시의 `Basic realm=…` 을 인증 실패로 읽으면 «멀쩡한 저장 토큰을 덮어쓴다»). 🔴 토큰을 새로 저장하면 `bumpTokenGeneration()` 을 «같이» 부를 것. ⚠️ **「유일한 전송」은 오늘 참이 아니다** — `replayable_rules.js` 가 맨 `fetch` + 손수 붙인 헤더로 한 라우트를 부르고, 게이트(`refusal_seat_harness` S1)는 `/admin/` 과 `fetch(` 가 «한 줄»에 있을 때만 잡아 그 자리를 못 본다 |
...
- **「고른 행을 어느 컬럼으로 묶어 다시 번역할 것인가」** (**`redo_banner.js`** — ⚠️ **[2026-09-02] 종전 철자 `grid_rescope_menu.js` 는 없다**: 우클릭 메뉴에서 **헤더의 배너**로 옮겼다(`6e060fd7`). 소유자 지시가 「컨텍스트 메뉴에 두지 마라」였고, **선택이 그리드에서 일어나므로** 버튼이 선택을 지켜보다 같이 살아나는 것이 맞는 모양이다. `scopeValuesFor` 는 그대로 옮겨 왔다) — 🔴 **고를 수 있는 컬럼은 «서버가 준 목록»(`scope_columns`)뿐이다.** 서버가 거절할 컬럼은 **화면에 아예 없어서** 고른 뒤 400 을 받는 길이 없다. 「없음」도 셋으로 갈린다(선언 안 된 표 = 줄이 하나도 없음 / 행 미선택 = 버튼이 죽어 있음 / 고른 행에 그 컬럼 값이 없음 = **그 줄을 지우지 않고 이름을 댄다** — 빈 범위는 어드민에서 400 이라 운영자가 «왜 없는지»를 봐야 한다). 🔴 **이 부품에 `/admin/` 도 fetch 도 토큰도 «없다»** — 돌리는 것은 «주입된 한 함수」(`run(op, params)`)이고 토큰은 «있는지만» 묻는다(`hasToken()`). 값을 안 들고 있으므로 하니스가 «진짜 토큰 없이» 채점할 수 있고, 그 사실은 «출력»이 아니라 «파일»에 대고 단언한다(렌더 결과로는 못 보이므로). ⚠️ **[2026-09-17 정정] 그 경계의 «사유»는 더 이상 「고르는 곳은 그리드, 실행하는 곳은 어드민」이 아니다** — 2026-09-01 소유자 요구로 **그리드에서 직접 돌리게** 바뀌었고(2026-08-31 판정 ⓐ를 무름, `redo_banner.js:7-9`), 실제 전송은 `main.js::runRetroactive` → `admin_token.js::adminFetch` 다. 경계는 «남았지만» 이유가 바뀌었다: 이제는 **부품이 토큰을 안 들기 위해서**다.
- **「다시 돌릴 수 있는 규칙」은 «이 표»의 것이다** (2026-09-15 C-109, 소유자 「리플레이 버튼 누를 시 목록은 해당 테이블이 트리거인 체인 규칙만」·「조인도 달아줘」) — 목록을 긷는 자리는 `replayable_rules.js::loadReplayableRules(table)` «하나»이고, 거르는 것은 «서버»다(`GET /admin/chain/rules/replayable?table=`, S-250). 🔴 **답이 세 상태다**: 객체 배열 · `[]`(이 표를 트리거로 하는 규칙 없음) · `null`(못 읽음). 종전에는 이름 «문자열» 목록이라 ⓐ 어느 표의 규칙인지 화면이 몰랐고 ⓑ 같은 이름의 조인과 합성이 «같아 보였»고 ⓒ 표를 바꿔도 목록이 안 바뀌었다. 🔴 **줄은 「어떤 규칙이 어디서 어디로」를 말한다** — `name · trigger → target — M rows` + 종류 배지 한 낱말(join·decide·mapper) (⚰️ 종전 「N keys from M rows」 — C-112 뒤 신원이 `row_id` 라 값의 수가 곧 행의 수다). ⚠️ 이름 사이를 «공백 둘»로 띄우면 HTML 이 하나로 접어 이름과 표가 한 낱말처럼 붙는다(실측) — 그래서 기호(`·`)다. ⚠️ 서버가 안 말한 칸은 «그리지 않는다»(종류 없으면 배지 없음 · 표를 모르면 화살표 없음 · 이름 없으면 «누르는 줄이 아니다»). 🔴 **크기는 줄에 남는다** — 확인 창이 없고 «그 줄을 누르는 것»이 확인이기 때문이다.
- **표를 알려 주는 자리도 «하나»다** (`main.js::redoBannerFollows(table)`) — 부팅과 표 바꿈이 같은 줄을 지난다(`setRelation` + 목록 다시 긷기 — ⚰️ C-112 전에는 `setBusinessKey` 도 지났다; 배너가 보내는 신원이 `row_id` 가 되어 표마다 알려 줄 것이 없어졌다). 갈라지면 한쪽 경로에서만 목록이 낡는데, 그 고장은 오류를 안 낸다. ⚠️ 그 «배선»은 하니스가 못 잰다 — `main.js` 는 node 가 import 할 수 없고(ag-grid·CSS·`init()`), 잘라쓰기는 금지다(부팅 «순서»는 C-110 으로 `startup.js` 에 나와 import 된다 — 남은 것은 `main.js` 안의 배선뿐이다). 그래서 목록을 긷는 «함수»를 자기 모듈로 뺐고(`source_rows.js`·`write_guard.js` 와 같은 사유), 남은 배선은 실제 페이지에서 본다.
- **체인 쪽은 «자기가 든 신원» `row_id` 를 넘긴다** (2026-09-16 C-112 `1295fab4` · 서버 S-254 `fe2d0c6c`) — 종전엔 화면에 «보이는» 업무 키를 보냈는데, `composite_key_source` 표의 저장 키는 «조립된 문자열»이라 어느 컬럼에도 없고, 그래서 `rows_scanned 0` 에 «오류 없음»이었다(박스 실측 2026-09-15). `row_id` 는 그리드가 «모든 표»에서 드는 하나의 신원이다(`scopeValuesFor(rows, 'row_id')`, 읽기는 이미 주입된 `readValue` 의 `row[col]` 폴백 — 새 배관 없음). 🔴 **누르는 길과 넘기는 길이 «같은 신원»이다** — 줄은 `params.row_ids`, 「어드민에서 열기」는 `{op, params: {row_ids}}`(`adoptRescopeHandoff` 가 `params` 를 그대로 앉힌다). 서버는 `row_ids` 와 `business_keys` 를 같이 받으면 «거절»한다(한 물음에 두 답) — 그래서 평키 표만 옛 길을 타게 두지 않았다(그 표에서만 나는 고장이 된다). ⚰️ 종전 이 줄의 「룰 목록이 어드민 토큰 뒤라 화면이 안 묶는다」는 C-109 뒤 거짓 — 목록은 이 표의 것으로 서버가 거른다(위 줄). 🔴 **row_id 없는 행은 조용히 빠지지 않는다**(판정 407 ②) — 「row_id 없는 행 N — 다시 돌릴 수 없음」 한 줄로 규칙 줄 «위»에 서고(그 줄이 왜 3이지 4가 아닌지 말한다) 누를 수 없다; 다만 그 줄을 누르면 «첫 행»을 그리드에서 보여 준다(C-114 `3bd3b829` — 페이지가 넣는 함수 하나 `reveal`, `main.js` 가 `forEachNode` 로 «객체 동일성»(`node.data === row`) 으로 찾아 `ensureNodeVisible`; 키로 못 찾는 것이 그 줄의 정의이므로 키 조회는 «될 수 없다»). 🔴 **큰 선택은 «막지 않고 말한다»**(C-113 `7156ee62`) — 운영은 한 트랜잭션에 수천 행이라 큰 선택은 실수가 아니다. `warnAbove`(부품 옵션, 기본 1000 — 부품에 수를 «박지» 않는다; 오늘 `main.js` 는 안 넘겨 기본값) 를 넘으면 `.redo-panel__warn` 한 줄 「선택 N행 · 권장 K행 이하」가 줄들 «위»에, 누를지는 사람이 정한다. 배너 문구는 한국어 명사형 한 벌(「관리자 토큰 없음 · 어드민 한 번 열기」 · 「어드민에서 열기」 · 「규칙 목록 못 읽음 · 어드민에서 선택」 · 「원장 소스 아님」 · 「범위 컬럼 선언 없음」 · 「선택 행에 row_id 없음」); 크기 문자열(`3 rows` · `2 groups from 3 rows`)은 짧은 영어 그대로다.
...
### 3.1-bis 「뷰에는 못 쓴다」는 «한 규칙»이다 (2026-09-13 C-102 · C-107)

- **규칙이 한 파일이다** (`write_guard.js`, C-107) — `writeRefusal()` 이 「거절되나, 왜」에 답하고, 깔때기 넷은 `if (refuseWrite()) return;` 한 줄이며, `applyWriteGuards()` 가 «쓰는 컨트롤 여섯»(행 추가·행 삭제·트랜잭션 적용·스마트 붙여넣기·파일/폴더 업로드 — 실측: 여섯 다 `state.currentTable` 에 쓴다, 뒤 셋은 `POST /tables/<현재 표>/upload`)과 머리의 배지를 «같은 답»에서 맞춘다. 🔴 판정은 «원래» 하나였고(`tableIsView`), 손으로 복사되던 것은 «그 판정을 묻는 일»이었다 — 그래서 일곱째 좌석이 빠졌다(C-102). 오늘 `tableIsView` 의 소비자는 이 파일 «하나»다.
- **표가 «자기 종류»를 말한다** — 머리의 배지. 종전에는 뷰라는 사실이 화면에 «없었고» 쓰려고 해야 알았다. 글자는 `VIEW_READ_ONLY_NOTE` 그대로 — 같은 사실에 두 문구를 쓰면 언젠가 갈라진다.


- **판정은 하나, 좌석은 «깔때기»다** — `tableIsView()`(카탈로그의 `kind`, S-187) 를 묻는 곳이 붙여넣기 · 지우기 · 일괄 채우기 · **행 추가** 넷이고, 각 깔때기가 «요청을 안 만들고» 사유를 한 줄 적는다(`VIEW_READ_ONLY_NOTE`, 철자 한 곳). 🔴 **컨트롤이 아니라 깔때기에 두는 이유**: 버튼에만 두면 그 버튼을 지나지 않는 호출자에게 답이 없다.
- **보이는 절반은 «한 자리»다** (🆕 **`write_guard.js::applyWriteGuards()`** — ⚠️ **[2026-09-17] 종전 철자 `ui.js::applyViewWriteGuard` 는 HEAD 의 `client2/src` 에 «없다»**(grep 0). 부르는 곳은 `api.js:185`) — 표의 종류를 아는 첫 줄(`switchTable` 의 `loadSchema` 다음)에서 쓰기 컨트롤의 상태를 맞춘다. 🔴 C-84 가 좌석 여섯을 «하나씩» 덮었고 일곱째(행 추가)가 빠져서 소유자가 뷰에서 그것을 눌렀다 — 자리를 하나로 두는 것이 그 재발을 막는 유일한 모양이다.
- ⚠️ **비활성이 «보여야» 비활성이다** — `.glass-btn:disabled` 가 «없었다»(있던 것은 `.glass-page-btn`·`.timeline-more-btn`). 잠긴 버튼이 멀쩡해 보이면 운영자는 누르고 «아무 일도 안 일어나는» 것을 본다. 그리고 마크업이 «이미 적은» `title` 은 지워지지 않는다 — 기억했다 되돌린다. 🆕 **[2026-09-17] 그 기억(`data-title-was`)의 «집»은 가드가 아니라 좌석이다** — `disabled_reason.js::setDisabledReason(el, why)`(C-124 `dbdd3443`). 가드는 그것을 «부를» 뿐이고, 컨트롤을 끄는 모든 자리가 같은 좌석을 지난다(아래 §3.1-ter).
- **거절은 «읽기»에도 온다** (C-105) — `fetchData` 가 `res.ok` 를 «안 보고» 거절의 본문을 행 목록으로 읽어 «던졌다»(`result.data.length`). 실경로에서 422 의 사유(「전순서가 없어 페이지를 읽을 수 없다 — business_key 를 선언하라」)가 콘솔로만 갔다. 🔴 이제 **거절은 답이다**: 사유를 «맨 먼저» 적고(뒤가 실패해도 문장이 남는다 — 실측으로 그 순서가 필요했다) · 격자를 비우고 · 개수는 «0 이 아니라 모름»이고 · 토스트는 «자기 실패를 자기가» 먹는다. ⚠️ 같은 읽기 셋의 오늘: `/data` 고침 · `/data/count` 는 조용히 반환(개수가 「…」로 남아 «맞다») · `/schema` 는 아직 던지고 「Schema load error」만 낸다(같은 부류, 범위 밖).
- **사유를 만드는 자리는 «하나»다** (`refusalText`) — 쓰기(`addRows`)와 읽기(`fetchData`)가 같은 함수를 지난다. 둘이 각자 적으면 한쪽이 「[object Object]」를 그리기 시작하는 날 다른 쪽은 멀쩡하다.
- **거절의 사유는 «서버의 문장»이다** — `addRows` 가 `detail` 을 그린다(셀 수정이 이미 그렇게 하고 있었다: `api.js` :531). 🔴 문장이 «문자열일 때만» 그것이 답이다 — FastAPI 의 기본 422 는 `detail` 이 목록이고, 그대로 그리면 화면이 「[object Object]」를 말한다. ⚠️ 화면 쓰기는 `catch` «밖»이다(그 파일 머리글: catch 안에서 DOM 을 쓰면 처리된 장애가 처리되지 않은 거절이 된다). 🔴 **아직 접는 자리 하나**: `api.js` :619 일괄 삭제.

### 3.1-ter 참조뷰는 «전부 탭»이다 (2026-09-13 C-103 · C-108)

- **탭이 자기 «행 수»를 든다** (C-108) — 글자는 `rowCountText` «한 벌»이고 탭의 배지와 표 위의 띠(C-60)가 그것을 «같이» 씁니다. 🔴 「0행」은 «잰» 것이고 빈 칸은 「셀 것이 안 왔다」(거절·무응답)입니다 — 종전 판정이 `rowCount ? … : ''` 라 둘이 화면에서 «같은 모양»이었고, 바로 위 주석은 그 둘을 구별한다고 적고 있었습니다(깔끔 ①: 이 줄이 참인가). 그래서 판정을 `absent.js::isCount` 로 옮겼습니다.
- **표마다 «마지막 탭»을 기억한다** — `localStorage['assy.refview.tab.<표>']` 한 칸. 🔴 기억하는 것은 «이름»이지 «번호»가 아닙니다: 선언이 뷰를 하나 끼워 넣거나 순서를 바꾸면 번호는 조용히 «다른 표»를 가리키고 그 고장은 오류를 안 냅니다. 이름이 오늘 목록에 없으면 «첫 탭»입니다. ⚠️ «그리기»는 기억하지 않고 «고르기»만 기억합니다(누르기와 ←→ 가 같은 한 줄 `chooseView` 를 지납니다) — 안 그러면 한 번 그린 것이 곧 선택이 되어, 아무도 안 고른 표에 선택이 생깁니다.
- **←→ 는 «탭 줄 위»에서만 듣는다** — 표 안에서도 들으면 범위를 읽던 손이 표를 바꾸고, `Shift+←→` 는 이미 «선택»의 몫입니다(`installSelectionKeys`). 끝에서는 «안 돕니다»(도는 목록은 「지금 어디쯤인가」를 잃습니다). 그리고 키는 거기서 «끝납니다»(`preventDefault`) — 안 그러면 탭이 바뀌는 «동시에» 탭 줄이 가로로 밀립니다(`overflow-x: auto`).
- 🔴 **고른 탭은 «보이는» 것까지가 고른 것이다** — 실측(실제 브라우저, 09-13): 기억해 둔 다섯째 탭이 `offsetLeft 412` · 보이는 폭 `394` · `scrollLeft 0` 이라 화면은 「첫 탭을 고른 것처럼」 보였고, `focus()` 는 이 줄을 «안 밀었습니다». 그래서 미는 자리는 `selectView` «하나»입니다 — 세 경로(누르기 · ←→ · 그리기)가 각자 밀면 그중 하나가 빠집니다.


- **갈래가 하나다** (`enrichment_reference_view.js::render`) — 뷰마다 탭 하나 · 패널 하나, 순서는 응답(`results`) 그대로. 패널이 하나면 탭 줄은 숨는다(그때 이름은 패널의 «띠»가 단다 — C-60).
- 🔴 **번호가 «하나»인 것이 요점이다** — 갈래가 있던 동안 탭 번호와 뷰 번호가 따로 셌고, 선택·복사가 «어느 표의 것인가»를 정하는 것이 `td.dataset.view` 다. 두 번호가 어긋나면 「탭은 이 표를 보여 주는데 복사는 저 표에서 나온다」가 되고, 그건 오류를 «안 낸다».
- ⚠️ **「채울 수 있는 뷰」는 배치가 아니라 «규칙»의 성질이다** — `candidate_for` 는 붙여넣기 계획·컬럼 순서·①② 표시를 정하고(`fillPlan`), 그 자리는 이 라운드가 한 글자도 안 건드렸다. 지워진 것은 «어느 상자에 그릴지»를 고르던 규칙뿐이다(목업 2b 의 「근거는 격자 아래」 — 소유자가 2026-09-13 에 바꿨다).
...
- **철자는 «한 곳»이다** (`absent.js`) — 결측은 `—` 이고 그 글자는 여기서만 정해진다. `isCount(v)` · `countText(v)`(좁은 배지용, 천단위 없음) · `localeCountText(v)`(문장 안). 🔴 **0 으로 대체하지 않는다** — 「없음」은 값이 아니고 0 은 값이다. 오늘 소비자 **19**(🔴 **[2026-09-17] 종전 「일곱」은 낡았다 — 수를 여기 다시 적지 말고 `git grep -l "absent.js" -- client2/src` 로 셀 것**. 종전 목록: `admin.js`·`timeline.js`·`chain_queue_panel`·`ledger_sources_panel`·`raw_registry_panel`·`progress_card`·`count_with_absence`).
...
- **한 «멤버»가 묶음과 보일 이름을 들 수 있다** (같은 파일) — 문자열이면 값과 이름이 같고 묶음이 없다(오늘 두 소비자가 전부 그렇다). 객체면 `{value, label, group}` 이고 묶음마다 `<optgroup>` 하나가 «만나는 순서»로 선다. 🔴 **값과 보이는 이름이 갈라질 수 있으므로 고른 것을 읽는 쪽은 «언제나» `value` 다.** ⚠️ 목록이 모르는 «현재 값»은 묶음 «밖» 맨 앞에 남는다 — 그건 「고를 수 있는 것」의 분류가 아니라 「지금 참인 것」이다.
  ```
...
- **등록 폼은 «자기 것»을 셋 든다** (`raw_registry_panel.js`, C-101) — `open`(무엇이 열려 있나) · `draft`(저장 안 된 글자) · `draftOf`(그 글자가 «어느 문서»의 것인가). 🔴 그 위에 규칙 하나: **«배경 갱신»은 목록이고, 열려 있는 문서를 갈아 끼우지 않는다.** 페이지의 30초 읽기는 이름을 «안 대고»(`refreshChainRule()`), 그 응답에는 문서가 없어서 그리면 편집기가 «사라진다». 페이지는 「이건 내 주기 갱신이다」만 말하고(`if (!name) opts.background = true`, 등록부 둘 다) 무엇을 할지는 부품이 정한다 — 판정을 페이지에 두면 등록부마다 한 벌씩 생긴다.
  - ⚠️ 「편집 중」만 지키는 것이 아니다 — «고른 것이 풀리는 것»도 초기화다. 그리고 초안은 자기 문서만큼 산다: 저장이 답했거나 다른 이름이 열리면 끝난다(보관은 제안 사항이다).
  - ⚠️ 부품이 «자기를» 다시 그리는 것은 배경 갱신이 «아니다»(`_again()` 이 그 깃발을 벗긴다). 안 그러면 탭이 열린 뒤의 접기 버튼이 아무 일도 안 한다.
- **참조 칸은 카탈로그에서 고른다** (같은 파일 + `chain_rule_panel.js`, C-101 ②) — 등록부가 «목록 이름»을 대면(`refList: 'tables'`) 맥락의 `declared` 가 그 목록을 낸다. 스켈레톤이 리프에 `section` 을 대면 그 이름이 이긴다. 🔴 컨트롤은 새로 만들지 않았다 — 탐색기가 이미 `datalist` 로 그리고, 그것이 「검색 가능 + 손타이핑 허용 + 목록에 없어도 표시 없음」이다. 없던 것은 «목록»이지 컨트롤이 아니었다(실측: 체인 스켈레톤의 `hint: 'ref'` 리프 «일곱»이 전부 `section` 을 안 댄다).
- **한 고르개가 «두 철자»를 쓴다** (`chain_rule_panel.js`, C-101 ③) — 맵퍼는 `mapper`(등록된 이름) «또는» `mapper_module` + `mapper_function` 이고, 서버는 앞의 것을 «등록부로만» 푼다(`chain_bindings.mapper_resolvable`). 그래서 파일 함수는 두 칸이거나 아무것도 아니다. 등록부가 네 함수를 든다: `mapperChoices`(응답의 두 칸 → 후보 하나, 묶음 둘) · `splitMapper`(고른 것이 «어느 칸들»에, `null` 은 지움) · `joinMapper`(문서가 든 것 → 고르개의 값) · `mapperNotes`(「여기 있지만 고를 수 없는 것」 한 줄씩). 템플릿은 그 넷을 부르고 낱말을 «하나도» 짓지 않는다.
  - ⚠️ `candidates`(S-223, 판정 379)가 오면 그것이 이긴다. 그 전에는 `registered` + `data` 로 «같은 모양»을 만드는 다리이고, 다리는 블록 하나라 그때 «지우면» 된다.
  - 🔴 그리고 목록이 읽히면 «고르개»가 첫 화면에 선다 — 그 컨트롤이 두 철자를 다 쓰므로, 문서가 무엇을 들고 있는지로 고르개를 접으면 「고를 수 있는 유일한 자리」가 「고급」 뒤로 간다. 값을 «든» 칸은 그래도 첫 화면이다.
- **「택1」은 값이 아니라 «모양»을 고른다** (`ontology_explorer_view.js::renderSkeletonOneOf` + `raw_registry_panel.js` `edit-shape-branch` + `chain_rule_panel.js`, 2026-09-16 C-111 `321e03c3` · 서버 S-241 `0dda6f08`/S-241-b `7c81c6d5`) — 체인 규칙 스켈레톤이 문법 «둘»을 든다(`skeleton.root` = 평면, `skeleton.unified_root` = `on`/`derive`/`into`/`key`/`limits`), 어느 쪽을 그릴지는 서버 `GET /admin/chain/rules/raw?name=` 의 **`grammar: flat|unified`** 칸이 정한다(`CHAIN_RULE_REGISTRY.formRoot` · `grammarOf`; 화면이 `derive` 유무로 «다시 유도하지» 않고, 서버가 안 말했으면 낱말도 안 그린다 — 「모름」을 「평면」으로 적으면 거짓). `derive`(join·decide·mapper)·`into`(table·read)는 새 노드 종류 **`oneOf`** 다(어휘의 저자는 `config_authoring.SKELETON_NODE_KINDS` = record·map·leaf·oneOf — 종전엔 원장 스켈레톤을 «걸어서» 유도해 마지막 사용을 지우면 어휘가 조용히 줄었다). 고르개는 이미 있던 닫힌 목록 컨트롤이고 선택지는 `branches` 의 «키»다 — 서버가 `list` 를 안 낸다(체인 쪽엔 닫힌 목록을 내는 곳이 없고, 아무도 안 내는 목록 이름은 「폼은 그리는데 읽는 쪽이 없다」). «고른 가지만» 그리고 안 고른 자리는 «아무것도» 없다(「안 골랐다」와 「비었다」는 다른 사실); 가지의 칸은 고르개 «바로 밑»에(상자에 넣으면 깊이 규칙에 접혀 실측에서 칸이 안 보였다). 가지 노드는 «가지 키 밑»에 산다 — `join`·`decide` 는 레코드, `mapper`·`table`·`read` 는 잎(판정 411: `into: {"table": "dt_x"}` 그대로, 첫 판이 셋을 레코드로 감쌌던 것을 클라가 커밋된 선언 픽스처에 대고 재서 잡았다). 고르면 «짐을 갈아 끼운다»: 고른 가지 하나만 남고(둘을 남기면 로더가 «화면이 안 보여 준» 판정을 한다), 적혀 있던 `kind` 는 지우지도 짓지도 않고 «맞추며», 새 가지의 빈 값은 `emptyOf`(잎 `''` · 레코드 `{}` · 🆕 택1 `{}` — C-115 `dd4cd689`: 「안 골랐다」는 가지 키 없는 매핑이다. 종전엔 필수 택1이 잎 꼬리로 떨어져 `''` 였고 출하 새-규칙 씨앗이 `{"derive":"","into":""}` 였다 — 그리는 쪽과 읽는 쪽이 둘 다 느슨해 «같이 보였을» 뿐). 내려가기의 저자는 `ontology_skeleton.js::childOf(node, step, defs)` «하나»(record 는 키 · map 은 member · oneOf 는 «가지 키»)라 택1 밑의 택1도 `shapeAt`·`emptyOf` 가 같이 본다 — 종전엔 둘이 각자 record·map 만 내려가 그 자리가 둘 다에 안 보였다. ⚠️ `CHAIN_RULE_REGISTRY.oneOf`(평면 폼의 맵퍼 두 철자 접기)는 «남는다» — 평면 문법과 «같이» 은퇴한다(판정 412).
