...
- **보이는 절반은 «한 자리»다** (`ui.js::applyViewWriteGuard`) — 표의 종류를 아는 첫 줄(`switchTable` 의 `loadSchema` 다음)에서 쓰기 컨트롤의 상태를 맞춘다. 🔴 C-84 가 좌석 여섯을 «하나씩» 덮었고 일곱째(행 추가)가 빠져서 소유자가 뷰에서 그것을 눌렀다 — 자리를 하나로 두는 것이 그 재발을 막는 유일한 모양이다.
- ⚠️ **비활성이 «보여야» 비활성이다** — `.glass-btn:disabled` 가 «없었다»(있던 것은 `.glass-page-btn`·`.timeline-more-btn`). 잠긴 버튼이 멀쩡해 보이면 운영자는 누르고 «아무 일도 안 일어나는» 것을 본다. 그리고 가드는 마크업이 «이미 적은» `title` 을 지우지 않는다 — 기억했다 되돌린다.
