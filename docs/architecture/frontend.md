...
> - 🔴 **[§6.4 신설] 넷째 질문(`?view=surprise` — 놀라움 장치·트렌드)이 이 문서에 «행이 없는 채로» 출하돼 있었습니다** — `src/surprise_*.js` 다섯 + `contrast_*.js` 둘이 서버 라우트 셋(`/lots`·`/lot_map`·`/siblings`)을 소비하는 화면 전체가 §6의 어느 절에도 없었습니다. 핵심만: **표는 가로로 돕니다**(소유자 지시 — 랏이 열, 트렌드는 좌→우) · **열 구성 = URL**(질문이 곧 링크) · **마킹 = 체크박스(`data-mark-lot`)이고 마킹 집합이 대조 패널의 scope가 됩니다**(마킹 5랏이 요청 130개를 만들던 것이 5개로) · **2장 마킹 = 우측 쌍 대조 레일**.
> - 🔴 **[§6.4] `wafer` URL 파라미터**(`f21a916`) — 좌석(슬롯·프레임 키)이 아니라 **웨이퍼 이름**이고, 화이트리스트 한 항목이라 마킹 토글·열 편집(href를 다시 짓는 두 경로)에서 조용히 떨어질 뻔한 자리가 닫혔습니다.
> - ✅ **[§6.3] 「소비 라우트가 아직 없다」가 낡았습니다** — `GET /api/ledger/structure`는 착지했고([backend §2](./backend.md)) 기전 층까지 렌더됩니다.
> - ⏳ **P0-2-1(마킹 단위 승격)·P0-3(여정 대조)·P0-4(서브그래프 탐색기)는 «설계 확정, 미구현»** — 정본은 [SCENARIO_CONSOLE_BRIEF](../process/SCENARIO_CONSOLE_BRIEF.md)이고 이 문서는 그것들을 기능으로 서술하지 않습니다.
>
> ### 직전 라운드 (2026-08-14 · E1 — 클라를 서버 `5bacdfc` 계약에 배선)
> - ✅ **[§6.1] 아래 라운드의 ⏳가 닫혔습니다 — `reason` 산문 파스가 «지워졌고» 홉 상태가 넷이 됐습니다.** 🔴 **`basis: {kind, name}`는 편의 필드가 아니라 «유도 불가능»을 닫습니다**: 아무도 다투지 않는 관례 기반 홉은 `resolved`이고 그것은 완전 실측 홉이 받는 **바로 그 단어**라, 「가정인가」를 `state`로 판정하는 코드는 어느 방향으로도 옳을 수 없습니다. 🔴 **그리고 문장을 읽으면 «뒤집힙니다»** — 문장은 **패자들**의 근거를 나열하고 필드는 **승자**를 서술합니다. ⚠️ **실제 출력에서는 문장과 필드가 언제나 일치하므로 어떤 캡처도 무엇을 읽는지 강제하지 못합니다** — 그래서 하네스는 둘이 **어긋나는** 홉을 한 방향씩 먹여 판별합니다(C5c/C5d).
> - 🔴 **`contested`는 `확정`도 `이견`도 아닙니다** — 계급이 승자를 **선언**했고 하위 계급이 반대합니다(`확정 · 반대 N종`, 칩도 별개). **둘 다면 `candidate`**(약한 단어가 이깁니다).
> - 🔴 **그 확장은 라벨 밖까지 닿았습니다 — 이 라운드의 «수리»는 거기입니다.** `hasLineageStep`이 `state === 'resolved'`를 보고 있어서 **유일한 혈통 걸음이 다투어진 사슬**이 「등재됐으나 혈통 주장 없음」으로 발표됐습니다(세 세대를 걸은 답에 「부모 없음」을 씌우는 그 결함이 종단 태그가 아니라 **상태 어휘**를 통해 재입장). ⚠️ **`candidate`에서는 이 라운드 이전부터 틀려 있었습니다.** 지금은 상태 단어가 아니라 **`to`가 있는가**로 판정합니다.
> - 픽스처: 캡처 셋에 `basis` 역산 backfill(문장 접미와 필드는 «같은 두 호출»의 두 보고라 정확) + 신규 `ledger_trace_contested.json` — **DB 없이** 선언된 원자를 `InMemoryClaimLookup`에 물려 **출하 중인 `trace()`**가 만든 진짜 출력입니다(자연 원장에 `contested`가 278홉 중 **0건**이라 캡처가 불가능합니다). 하네스 319 → **380 단언**, 변이 44 → **55종**.
>
> ### 직전 라운드 (2026-08-13 2차 · `0c3ea65` + `d9b98ab`)
> - 🔴 **[§4.2] 정렬 룰을 고르면 테이블·바인딩·목록이 따라옵니다 — 서버 변경 0줄입니다**(`0c3ea65`). 🔴 **`selection.map_tables`는 라우트가 생긴 이래 모든 응답에 실려 정규화까지 되고 클라 헤더가 「부트스트랩 3단계」라고 «적어 두기»까지 했는데 소비자가 0개였습니다** — 그 자리를 부트스트랩 발견(같은 아홉 테이블)이 채우고 있었습니다. 🔴 **고를 수 없는 테이블을 채택하는 것이 「기능 고장」의 모양이었습니다**: 라우트는 거절하지 않고 200에 전 단위 `unscorable / map_keys_unavailable`을 실어, 「당신 데이터가 나쁘다」로 읽힙니다(실측 150/150, **그중 데이터에 대한 사실 0건**). 🔴 **첫 컨트롤이 자기가 눌린 것을 부정**하고 있었고(제안 플래그를 다시 표시), 그 아래는 정확히 채워져 **기능이 돌면서 안 돈 것처럼 보였습니다.** 그 밖에 셋 — 룰 전환 시 **고르는 순간** 행 비우기(재구축이 실패하면 옛 행이 «영구히» 남습니다) · `derived`가 `fallback_guess`로 접히지 않음(**부정이 아니라 구성원으로 철자** — 부정은 `none`을 삼킵니다) · 손 덮어쓰기가 선언과 구별됨. ⚠️ **이 데이터에서 「룰에 따라 목록이 달라진다」는 시연 불가**(두 룰의 단위 목록이 동일). 어휘 정본 [MAP_ALIGNMENT_SPEC §9.13](../spec/MAP_ALIGNMENT_SPEC.md).
> - 🔴 **[§2 · §6.1 신설] 원장 혈통 추적 화면 `ledger.html`**(`d9b98ab`). 🔴 **결함은 `candidate` 홉을 «실제로 만들어 봐야만» 나타났습니다** — 관례/실측 구분이 홉 dict에 없고 `reason` 산문에 살며, `_with_basis`가 **승자**의 basis를 접미로 붙이는 동안 `candidate`의 reason은 **패자들**의 basis를 문장 안에 나열합니다. 그래서 소박한 `includes('convention:')`은 판정을 **뒤집습니다**(가정이 실측에 밀린 홉을 「가정」이라 부릅니다). ⏳ 구조화 `basis` 필드로 이 파스를 지우는 것이 승인됐습니다([R-2026-08-13-C](../process/LEDGER_RULINGS.md)) — **상설 원칙: `reason`은 사람 몫, 화면이 분기할 사실은 필드.** **신호 둘은 직교**(색=state, 무늬=basis), **복잡도 예산 +1 컨트롤이 단언으로 고정**, **끊긴 사슬은 내용으로 그려집니다.**
>
> ### 직전 라운드 (2026-08-11 3차 · `42d7600`)
...
**② 정렬 띠는 제거됐다 (2026-08-22).** △소유자 「일치든 거절이든 없애 어차피 사람들이 알아서 함」. 세 번에 걸쳐 사라졌다 — 어긓난다는 경고(2026-08-21) → 초록 「일치」 배지 → 「불가」까지 전부. 🔴 띠가 사라지면 `alignmentVerdict` 는 소비자가 0이 된다. 그래서 **함수와 그것을 재던 하네스 절반을 같은 커밋에서** 지웠다(FLOORS 22 → 13) — 아무도 부르지 않는 순수 함수를 하네스가 계속 초록으로 재면 죽은 코드 위의 게이트가 된다. 남은 것은 서버의 진짜 거절이다 — 가상 조인 컬럼에 쓰면 HTTP 400이고 **배치 전체가 죽는다**(2026-08-22 실측: 같은 요청의 정상 컬럼도 남지 않았다). 그 사실은 이제 미리 말하지 않고 붙여넣기 뒤에 나타난다.
...
### 3.7 화면 폭은 이름에 양보하고, 감사 탭은 표다 (2026-08-22 이주 2b 마감 · 2c)

**① 목업의 열 폭은 «최솟값»이지 확정값이 아니다.** `applyMockupLayout` 은
`width = max(목업 폭, 헤더 라벨이 안 잘리는 폭)` 을 쓴다. 목업은 자기 라벨(`Slot`)로 폭을 재고
소유자는 이름을 라이브 스키마(`DT_SLOT`)로 쓰라고 판정했으니 둘이 동시에 참일 수 없고,
상설 규칙(「작은 글씨는 없느니만 못하다」)이 어느 쪽이 양보할지를 정한다 — **잘린 컬럼 이름은
«좁은 것»이 아니라 «없는 것»이다.** `HEADER_CHROME_PX = 32` 는 감이 아니라 실측이다(두 컬럼이
정확히 32px 부족이었다). 낱개 특례는 금지 — 먼저 눈에 띄었던 `DT_SLOT` 옆에 `DT_EQP` 가 같이 잘려
있었다. 같은 규칙이 2c 의 Tx 칸에도 적용된다(목업 84px → 120px: 그 칸은 목업과 달리
필터 🔍 와 펼치기 ▶ 를 들고 있고, 84px 에선 ▶ 가 칸 밖으로 잘려 **클릭이 안 됐다**).

**② 한 값을 보는 자리가 둘인 것은 이 저장소의 관례다.** `Copy Header` 는 Options 드롭다운과
참조 패널 하단에 둘 다 있고, 둘 다 `localStorage['copyHeader']` 하나에 붙는다 — `map_editor.js` 가
`map-copy-header-toggle` 로 이미 하던 것과 같은 모양이다. 새 저장 키도 이벤트 브리지도 없다.
🔴 하나만 두면 안 되는 이유: 참조 패널은 규칙 없는 표에서 `display:none` 이고(실측: `lot_event`
— 탭 숨김 · 패널 숨김 · rect 0×0), 이 토글은 **메인 그리드의** 복사를 결정한다(`clipboard.js:256`, `:655`).

**③ Global 감사 탭은 표이고, Row 탭은 카드다.** 분기는 `loadHistory` 한 자리에서 컨테이너에
`.audit-table` 를 붙이는 것으로 난다(탭 핸들러 넷이 아니라). 항목이 수백이고 주어가 수백개인
화면에서 카드는 훑어보기를 스크롤로 바꿀 뿐이고, Row 탭은 반대라 그대로 둔다.
🔴 **종류 알약은 새 분류가 아니라 이미 색을 고르던 그 분기다**(`auditKind`). 목업은 7종을
적었으나 감사 행이 실제로 지닌 것은 `column_name` · `source_name` · `total_count` 이고, 그것이 이
파일이 줄곴 그려 온 6갈래다. 나머지 7종을 그리면 **아무것도 계산하지 않는 분류**가 화면에 올라간다.
⚠ 목업의 상단 필터 줄과 「더 보기」는 **구현하지 않았다** — 감사 필터링은 존재하지 않고,
`historyUrl` 은 cell/row URL 만 만들어 Global 라우트엔 페이징 경로가 없다.

**④ 근거 뷰는 탭이 아니라 그리드 «아래»에 쌓는다.** `candidate_for` 를 선언하는 뷰는 «붙여넣기
출처» 그리드이고, 선언하지 않는 뷰는 근거다. 아무것도 선언하지 않는 규칙은 종전대로 첫 뷰가 그리드다.
🔴 그랬기 때문에 복사 핸들러와 `selectedColumnNames` 는 더 이상
`querySelectorAll('.reference-view-table')[viewIndex]` 로 **위치를 키로 쓰지 않는다** — 근거 표가 그리드
아래로 가면 문서 순서와 `results` 순서가 다른 목록이 되고, 인덱스는 그에도 표를 하나 돌려준다.
틀린 표를, 오류 없이. 표가 `data-view` 를 들고 그 이름으로 찾는다.
...
| 서버 왕복 | GET `/api/transfer-plan/{stages,source-summary,validate}` + PUT `/tables/map_split_registry/data/updates`(`replace_map`). ~~`map_doe`/`map_doe_source`~~는 M2.6에서 폐기, **2026-08-13 `c0fb735`로 선언·설치기에서 은퇴**(물리 DROP은 [OPERATOR_RUNBOOK §5](../process/OPERATOR_RUNBOOK.md)) |
...
- 🔴 **룰을 고르면 테이블·바인딩·목록이 «따라옵니다» — 그리고 그중 하나는 서버가 줄곧 답하고 있던 것입니다**(`0c3ea65` · 2026-08-13). 서버 변경 0줄입니다. 판정·사유 어휘의 정본은 [MAP_ALIGNMENT_SPEC §9.13](../spec/MAP_ALIGNMENT_SPEC.md)이고 여기는 **모듈 경계와 결함의 모양**만 적습니다.
  - 🔴 **`selection.map_tables`는 라우트가 생긴 이래 «모든» 워크리스트 응답에 실려 왔고 `normaliseWorklist`가 세션에 정규화까지 하고 있었는데, 소비자가 «0개»였습니다.** 그 자리를 채우던 것은 부트스트랩 발견(모든 `map_key_columns` 선언 테이블 = 어느 룰에서든 같은 아홉)이었습니다. **클라 자신의 헤더가 그것을 「부트스트랩 3단계」라고 적고 있었습니다** — 서빙되고, 정규화되고, 문서화된 필드가 **아무도 안 읽는** 상태로 살아 있었던 것이 이 결함의 모양입니다.
  - **거절은 이 화면이 좁히는 것이 아니라 «서빙받는 사실»입니다.** 테이블 레코드의 `selectable`·`reason`은 서버의 것이고, **`selectable` 부재는 거절이 아니라 선택 가능**입니다(부트스트랩 카탈로그는 어떤 워크리스트도 답하기 «전»에 만들어집니다 — 부재를 거절로 읽으면 첫 화면이 통째로 잠깁니다).
  - 🔴 **고를 수 없는 테이블은 선택지가 아니고, 그것을 채택하는 것이 「기능이 고장 난 것처럼」 보이던 이유였습니다.** 라우트는 **거절하지 않습니다** — 200에 전 단위 `unscorable / map_keys_unavailable`을 실어 보내고, 그 화면은 「당신 데이터가 나쁘다」로 읽힙니다. 실측(`assy_qa`): `core_frame_review`(소스 `dt_core_view`)에 `dt_map`을 고르면 150/150이 그 사유이고 **그중 데이터에 대한 사실은 0건**입니다.
  - 🔴 **첫 컨트롤이 자기가 눌린 것을 부정하고 있었습니다** — 룰을 고르는 순간 룰 선택기가 「규칙 선택」 자리로 되돌아갔습니다(핸들러가 `options: null`을 넘기고 **선택이 아니라 제안 플래그**를 다시 표시했으며, 가능한 룰이 둘이면 제안이 플레이스홀더 선택 상태로 조립됩니다). 테이블·컬럼·목록은 그 아래에서 **정확히 채워지고 있었고**, 기능은 **돌면서 안 돈 것처럼 보였습니다.**
  - **룰이 바뀌면 행은 «즉시» 사라지고 시퀀스가 움직입니다.** 룰은 단위가 «무엇인가»를 선언하므로, 옛 룰로 가져온 페이지는 이 답의 낡은 사본이 아니라 **다른 질문에 대한 답**이 새 룰의 라벨 아래 앉아 있는 것입니다. 지우는 시점이 **답이 아니라 «고르는 순간»**인 이유는 사이의 카탈로그 재구축이 여러 왕복이고 **실패할 수 있기 때문**입니다 — 실패하면 재요청이 영영 안 나가 옛 행이 **영구히** 남습니다. ⚠️ **검색어는 살아남습니다**(조작자의 것이지 룰의 것이 아닙니다).
  - **`derived`는 `fallback_guess`로 접히지 않고 자기 이름으로 삽니다.** 접는 동안 라우트가 애써 그리는 구분 하나를 잃고 있었습니다 — `fallback_guess`는 **값 컬럼이 발명된 것**이라 데이터 경로가 아예 거절하고, `derived`는 `table_config`에서의 평범한 상속입니다. **둘 다 아무도 동의하지 않은 쌍**이라 제안 표시와 쓰기 고지는 «집합 소속»으로 걸리고, 「추측」이라는 낱말만 `fallback_guess` 하나에 걸립니다. 🔴 **부정(`!== declared`)이 아니라 «구성원»으로 철자합니다** — 부정은 `none`(아무 쌍도 안 고름)을 조용히 삼켜 빈 select 둘 옆에 컬럼 확인 컨트롤을 세웁니다.
  - **선언을 손으로 덮어쓴 것이 덮어쓴 것으로 보입니다.** 컬럼을 손으로 지목하면 출처가 `declared`로 올라가는데(쓰기에 대해서는 옳습니다 — 조작자의 선택이 config를 이깁니다) 그 결과 **선언 자신을 보여주는 화면과 구별이 안 됐습니다.**
  - ⚠️ **이 데이터에서 「룰에 따라 목록이 달라진다」는 시연될 수 없습니다** — 두 룰이 **동일한 단위 목록**을 냅니다(둘 다 `derived_table: dt_inventory` · `decision_key: ['dt_job']`). 바뀌는 것은 테이블 제안·바인딩·단위별 사유 코드·와이어의 `?rule=`이고, 실측된 것도 그것뿐입니다.
...
원장 슬라이스 1의 **L3**. 서버 계약은 [backend §2 `GET /api/ledger/trace`](./backend.md), 판정 대장은 [process/LEDGER_RULINGS](../process/LEDGER_RULINGS.md).

- **모듈 셋**: `ledger_trace_core.js`(순수 — 입력 정규화, 홉 해석, **`basis` 필드 판독**) · `ledger_trace_view.js`(렌더) · `ledger_trace.js`(조립·요청). 하네스 `client2/tests/ledger_trace_harness.mjs` + 픽스처 **넷**(`ledger_trace_live.json`·`_probe.json`·`_nothings.json`·**`_contested.json`**).
- 🔴 **이 레인이 찾은 결함은 «`candidate` 홉을 실제로 만들어 봐야만» 나타났습니다.** 온톨로지 소유자가 판정한 관례/실측 구분은 **홉 dict 어디에도 없고 `reason` 산문 안에 삽니다.** 그리고 `_with_basis`는 **승자의** basis를 접미로 붙이는데, `candidate`의 `reason`은 **패자들의** basis를 문장 안에 나열합니다. 그래서 `reason.includes('convention:')`은 구분을 놓치는 정도가 아니라 **뒤집습니다** — 가정이 실측에 **밀린 바로 그 홉**을 「가정」이라 이름 붙입니다. 프로브의 경합 홉 둘 다 이 함정을 밟았습니다. 지금은 **한 함수 안의 앵커드 정규식 하나**이고 양방향이 핀돼 있으며 변이가 그 핀이 실패할 수 있음을 증명합니다.
- ✅ **그 파스는 «지워졌습니다»** (2026-08-14 · 서버 `5bacdfc` + 클라 E1). 홉이 `basis: {kind, name}`를 싣고 화면은 그것을 **소비합니다**(`kind`는 `convention`\|`measured` — 서버의 `BASIS_KINDS` 그대로). L2가 `predicate`를 필드로 올린 것과 **같은 논거**이고 [R-2026-08-13-C](../process/LEDGER_RULINGS.md)가 승인한 그대로입니다. 🔴 **상설 원칙: `reason`은 사람을 위한 산문이고, 화면이 분기해야 하는 사실은 반드시 구조화 필드로 나갑니다.**
  - 🔴 **그리고 이 필드는 «편의»가 아니라 «불가능»을 닫습니다 — `basis`는 `state`에서 유도되지 않습니다.** 아무도 다투지 않는 관례 기반 홉은 `resolved`이고, 그것은 완전 실측 홉이 받는 **바로 그 단어**입니다. 그래서 「가정인가」를 상태 문자열로 판정하는 코드는 **어느 방향으로도** 옳을 수 없습니다. 하네스 C3c/C3d·N4가 그 두 홉 쌍을 채점합니다.
  - ⚠️ **C5c/C5d가 이 라운드의 «판별식»입니다.** 실제 서버 출력에서는 문장과 필드가 **언제나 일치**하므로 어떤 캡처도 둘 중 무엇을 읽는지 강제하지 못합니다. 그 둘은 문장과 필드가 **서로 어긋나는** 홉을 한 방향씩 먹여, 아직 산문을 읽는 판독기가 **둘 다 거꾸로** 답하게 만듭니다.
  - 접미사 판독은 **`basis` 키가 «아예 없는» 응답에서만** 살아 있습니다(5bacdfc 이전 서버). `basis: null`은 서버가 「선언된 근거 없음」이라 **말한 것**이라 그대로 받습니다 — 거기서 문장으로 되돌아가면 방금 지운 판독을 되살립니다.
- 🔴 **홉 상태가 셋에서 «넷»이 됐고, 새 단어는 두 이웃 중 어느 쪽의 동의어도 아닙니다** (서버 `5bacdfc`). `contested` = 계급이 승자를 **선언**했고 **하위** 계급이 여전히 반대한다(순위는 확실, 모순은 살아 있다) · `candidate` = 최상위 계급 자체가 갈렸다(**아무도 선언하지 않았고** 동점처리만 갈랐다). **둘 다면 `candidate`** — 약한 단어가 이깁니다. 그래서 화면은 `확정`(동의를 주장) 도 `이견`(승자 없음을 주장) 도 쓰지 못하고 **`확정 · 반대 N종`**을 씁니다(`N = n - 1`, 서버 문장의 `하위 계급 반대 N종`과 같은 수). 요약 칩도 **`반대`와 `이견`이 각각**이고 `확정`에 접히지 않습니다.
- 🔴 **그 확장은 라벨 밖까지 닿았고, 그 자리가 이 라운드의 «수리»입니다.** `trace()`는 `res.answer`가 있으면 부모로 **이동**하므로 `contested`·`candidate` 홉도 워크를 옮깁니다. 그런데 `hasLineageStep`이 `state === 'resolved'`를 보고 있어서, **유일한 혈통 걸음이 다투어진 사슬**은 「등재됐으나 혈통 주장 없음」으로 발표됐습니다 — 세 세대를 걸은 답에 「부모 없음」을 씌우는 그 결함(`no-lineage-claimed-on-a-real-root`)이 **종단 태그가 아니라 상태 어휘를 통해** 다시 들어온 것입니다. ⚠️ **`candidate`에서는 이 라운드 «이전부터» 틀려 있었습니다.** 지금은 상태 단어가 아니라 **`to`가 있는가**(워크 자신의 이동 기록)로 판정하므로 다음 확장에서 다시 낡지 않습니다.
- **신호 둘을 «직교»로 씁니다 — 색 = `state`, 무늬 = `basis`.** 관례 홉은 점선 레일·점선 테두리·점선 칩, 실측 홉은 실선 레일과 근거 칩. 조작자가 **둘 중 어느 것인지를 문장을 읽지 않고** 볼 수 있습니다. ⚠️ **칩의 글자는 여기 옮기지 않습니다 — 마크업이 정본입니다.**
- 🔴 **복잡도 예산이 단언으로 고정돼 있습니다**: 앱 전체에서 폼 컨트롤 **+1**. 그 한 칸이 `Enter`로 확정하며 **URL을 다시 씁니다 — 그래서 답이 곧 링크**이고, 그것이 두 번째 입력도 제출 버튼도 히스토리 컨트롤도 없는 이유입니다. 단언 둘이 그것을 붙들어 미래의 필터 바가 **빨개지며 자기를 변호하게** 만듭니다.
- 🔴 **끊긴 사슬은 «내용»으로 그려집니다.** 모르는 랏은 빈 화면도 토스트도 아니고 **간극 홉 하나 + 종료 사유**를 가진 추적입니다(서버가 빈 `hops`를 구조적으로 못 내는 것과 같은 규율).
- 🔴 **「없음」은 넷이고 화면은 그 넷을 다르게 말합니다** (`92b3b47`. ⚠️ 이 항목은 그 라운드가 이 문서에 옮겨지지 않아 2026-08-13 뒤늦게 등재됐습니다). 사용자가 `ledger_events` 없는 DB에 물었더니 **빈 화면**이 나왔고, 서로 무관한 넷이 그 하나로 보였습니다 — 마이그레이션 미실행(**배포**) · 백필 미실행(**운영**) · 없는 랏(**데이터 경계**) · 등재됐으나 혈통 주장 없음(**내용**). 🔴 **그중 둘은 추적만으로는 구분이 «불가능»합니다**: 원자가 0이면 워크가 모든 랏에 대해 `[unknown_subject] … 원장에 원자 0`을 답하므로 **가득 찬 원장의 없는 랏과 바이트 단위로 같습니다.** 가르는 유일한 사실은 `GET /api/ledger/coverage`의 `state`(`absent`/`empty`/`ready`)이고, 그래서 그것은 **추론이 아니라 인자**입니다. 이 라우트가 없는 서버에서는 `unknown`으로 **강등**되어 예전 화면 그대로가 됩니다 — 재보지 않은 박스에 대한 주장은 하지 않습니다.
- 🔴 **판정은 `[tag]`로 하고 문장으로 하지 않습니다.** `[root]`의 문장에는 「없음」이, `[unknown_subject]`의 문장에는 「원자 0」이 이미 들어 있어 부분 문자열 판독은 **두 방향으로** 틀릴 수 있습니다. 위 `convention:` 함정과 **같은 계급의 결함**입니다.
- 🔴 **없는 랏에는 «다음 걸음»이 붙습니다** (2026-08-13). 「원장에 없음」은 맞는 **문장**이었지만 **화면은 막다른 길**이었습니다 — 랏을 잘못 친 조작자가 옳은 답을 읽고 갈 곳이 없었습니다. 지금은 종단 블록 **아래**에 `물을 수 있는 랏` + 표본 링크가 붙습니다(⚠️ **위가 아닙니다** — 사슬 위는 답의 «뜻»을 바꾸는 자리이고, 진단보다 먼저 나온 탈출구는 진단 «대신» 읽힙니다). 데이터는 페이지가 이미 받아 둔 coverage 본문이라 **요청이 늘지 않고**, 표본은 빈 상태가 쓰는 **같은 함수**로 철자되어 여기서 권한 랏과 저기서 권한 랏이 **다르게 답할 수 없습니다**. 🔴 **이 하나에만 붙습니다**: 빈/부재 원장은 가리킬 것이 없고(없는 랏 목록은 그것들이 «있다»는 주장이 됩니다), 혈통 주장 없는 랏은 **이미 찾힌** 랏이라 다른 데로 보낼 이유가 없습니다. coverage가 없으면 아무것도 지어내지 않습니다. **컨트롤은 0개 늘었습니다 — 앵커는 컨트롤이 아닙니다.**

### 6.2 케이스-컨트롤 조사 콘솔 (`ledger.html` 같은 페이지 · `src/case_control_*.js`) — 2026-08-14
...
- 🔴 **분모 없는 숫자가 «구조적으로» 나갈 수 없습니다.** `rateReading(n, d)`는 분모 없이 백분율을 **반환하지 못하고 거절을 반환**하며, 뷰의 유일한 백분율 경로인 `renderRate`가 그 거절을 「분모 없음」으로 그립니다. 분수는 백분율 **옆에** 항상 렌더됩니다(호버·툴팁 금지) — 6건 위의 83%와 600건 위의 83%는 다른 주장이고, 독자는 양쪽이 보여야만 구분합니다.
- 🔴 **모집단은 «셋»이고 미스캔은 분모 밖입니다.** `분모 = 발견 + 깨끗-스캔됨`, 미스캔은 자기 칸·자기 수·점선 테두리. 미스캔을 깨끗에 접으면 **검사하지 않은 만큼 모든 율이 작아집니다** — 커버리지 간극이 품질 개선으로 그려집니다(픽스처에서 2.4% → 0.44%). 패널이 그 산술을 문장으로 인쇄해 독자가 **확인**할 수 있게 합니다.
- 🔴 **void는 기본값이지 분기가 아닙니다.** `DEFAULT_FINDING_KIND`가 그 문자열의 **유일한** 등장이고 대입입니다. 종류는 ① 조작자가 물은 것 ② 카탈로그의 `default` ③ 카탈로그 첫 행 ④ 상수 순으로 정해집니다. 하네스 픽스처는 카탈로그 기본값을 **`scratch`**로 두어 「카탈로그를 읽었다」와 「상수로 갔다」가 **다른 답**을 내게 만듭니다 — 둘이 같은 답을 내는 표본은 아무것도 판별하지 못합니다.
- 🔴 **class 축도 같은 규율입니다** (`51f3119` · [MI_LEDGER_SCHEMA_PROPOSAL §6-quater](./MI_LEDGER_SCHEMA_PROPOSAL.md)). 불량의 class는 `observed` payload 안에 **같은 발화로** 실려 오고(별도 조회 없음), 값 집합은 **finding_kind별로 닫혀** 어휘 시그니처가 선언합니다. 그래서 클라에 class 목록을 적으면 `finding_kind === 'void'`와 **같은 결함**입니다 — 종류를 바꾸면 class 목록이 따라 바뀌어야 하고, 하네스는 class 집합이 **서로 겹치지 않는** 두 종류로 그것을 채점합니다. class는 현황판의 **첫** 슬라이스 축이자 대조 축입니다. ⚠️ **낱말은 「클래스」이고 그보다 센 말을 쓰지 않습니다 — class는 분류이지 판정이 아니고(「합격인가」는 여전히 저장 금지) 화면 문구가 판정처럼 읽히면 그 경계를 화면이 깹니다.**
- 🔴 **「분모 없음 — 대조 불가」는 오류 상태가 아니라 «내용»입니다.** `observed_by` 미선언 종류(임의 OM 관측)에는 나눌 `inspection_run` 모집단이 없습니다. 빈 패널은 「차이 없음」으로 읽혀 **진실의 반대**가 되므로, 패널이 그 사유를 자기 본문으로 그립니다 — §6.1의 「없음 넷」과 같은 규율.
- 🔴 **팩트 칩 렌더러는 «하나»입니다** — `measured`·`observed`·`processed_with`. 어느 필드가 이름이고 어느 것이 값인지는 core의 **표**(`FACT_SPEC`)이지 뷰의 분기가 아니라, 뷰는 `quantity`가 뭔지 모릅니다. 표에 없는 술어도 자기 이름으로 렌더됩니다(어휘가 자라서 칩이 사라지는 것이 더 나쁜 결함). 모든 칩에 **발화자 배지**(`source.who`)와 **근거 ref**(`raw_ref`/`event_id`)가 붙고, 없으면 「출처 미상」·「근거 ref 없음」이 **내용으로** 나갑니다. `note`는 관찰자의 낱말 그대로이고 **파싱하지 않습니다**(R-C).
...
- **모듈**: `surprise_core.js`(순수 — bare node, 하네스 `client2/tests/surprise_harness.mjs`) · `surprise_view.js`(렌더) · `surprise_axis.js`(**맵 데이터로 네트워크를 만지는 유일한 파일** — 등록 프레임·유효 다이 플로어를 맵 에디터가 늘 쓰던 범용 테이블 라우트로 읽는다) · `surprise_map_core.js`/`surprise_map_view.js`(맵 스트립) · `contrast_core.js`(순수)/`contrast_view.js`(마킹 대조 패널 + 쌍 레일). 조립·요청은 `ledger_trace.js`가 같은 페이지의 다른 절과 함께 든다. ⚠️ `contrast_core.js` 헤더가 인용하는 `tests/contrast_harness.mjs`는 **실존하지 않는다**(총괄 보고 대상).
- 🔴 **표는 «가로로» 돈다** (소유자 지시 2026-08-14: 「상단 트렌드 표는 세로 말고 가로로 리스팅해」) — 랏이 **생산 순서로 열**, 항목이 행이고, 트렌드는 눈이 읽는 방향(좌→우)으로 눕는다. 조건부서식은 항목 축을 따라 **자기 기저 대비**이고 `level`·`ratio_to_baseline`은 서버가 낸다(기저 제외 규칙을 클라가 알 수 없으므로 — [backend §2 `/lots`](./backend.md)). 🔴 **「미검사」는 0이 아니다** — 모든 수치 읽기는 `numOrNull` 하나를 통과하고 셀은 서버의 `state` 낱말로 그린다(`Number(null) === 0`이 「안 셌다」를 「세었더니 0」으로 뒤집는 그 함정은 §6.2에서 이미 값을 치렀다).
- 🔴 **열 구성 = URL이고, 편집 «전에» 해소된 집합을 URL에 쓴다** (`66e2925`). 무엇이 열이 될 수 있는가는 서버의 지표 선언에서 오고(내일 선언된 지표는 여기 한 줄 안 바뀌고 나타난다), 어느 열이 떠 있는가는 URL(`cols=`)이다 — **열 집합이 질문 그 자체라 붙여넣으면 같은 표다.** 열 «빼기»는 집합이 서버 기본값에서 왔을 때 무동작이었다(질문이 열을 이름 대지 않아 편집이 같은 URL을 낳는다) — 지금은 해소된 집합을 먼저 URL에 쓰고 편집한다. 열 «더하기»는 나머지 셋을 떨어뜨리는 거울 결함이었고 같은 수리를 받았다. 열 메뉴가 비어 있던 죽은 버튼도 같은 커밋이다: 서버는 집계를 `name`으로 싣는데 독자가 `aggregate`→`id`를 물어 여섯 전부가 걸러졌다 — 그리고 그 상태에서 완결 문장은 「선언된 지표 전부 떠 있음」을 주장했다(12 중 1이 떠 있을 때). 독자는 `name`을 받고, 완결 문장은 수가 맞을 때만 나온다.
- 🔴 **마킹은 랏 열 머리의 체크박스이고(`data-mark-lot`), 마킹 «집합»이 대조의 scope다** (`66e2925`). 여럿을 마킹하는 것은 「이들이 어떻게 **다른가**」를 묻는 것이고 그림 더미는 그 답이 아니다 — 종전에는 5랏 마킹이 맵 125장을 12,283px로 쌓았고 그 뒤에 요청 폭풍(5 + 5×25 = **130 응답**)이 숨어 있었다. 지금은 **5개**다. 대조 패널은 `/siblings` **한 엔드포인트**에 `mode`·`finding`을 파라미터로 묻고 scope가 마킹 집합을 따라가므로 **마킹을 바꾸면 다시 묻는 것이 전부다.** 관문 셋(실재·상류·기전)은 응답에서 **읽지** 조립하지 않는다 — 넷째 관문은 넷째 칩이고 바뀌는 줄이 없다. ⚠️ 패널 문구의 자연어화·조건부서식 페인트 수리(P0-2-3/-4)는 이 절 범위 밖의 후속 커밋(`a729a7f`)이라 이 절은 문구를 인용하지 않는다.
- 🔴 **2장 마킹 = 우측 고정 레일**에 쌍을 지표별로 나란히 — 화면에 이미 있는 표 데이터에서 그리므로 둘째 체크 순간 뜬다. **3장 이상은 조용히 딴 일을 하지 않고 그렇게 말하며**, 그룹 랭킹은 아래 그대로 남아 쌍 뷰 거절이 아무것도 숨기지 않는다. ⚠️ **화면을 봐서 잡은 결함 둘**: 465 대 430인 카운트 쌍이 「대조 불가」로 붙었고(비교가능성 검사가 분모를 요구했다 — **분모 없는 카운트도 카운트다**), 레일 브레이크포인트가 1280px인데 창이 1256px라 레일이 **말없이** 정적 블록이 됐다. 둘 다 수리됨. ⏳ **이 레일은 «여정 대조»로 재설계가 확정돼 있다**(P0-3 — 소유자 승인 시안. 표가 아니라 두 웨이퍼가 걸은 길이 축, n=2에 집단 통계 금지) — 정본은 [SCENARIO_CONSOLE_BRIEF P0-3](../process/SCENARIO_CONSOLE_BRIEF.md). ⚠️ **[2026-08-14 밤] 서버 절반이 착지했다 — `GET /api/ledger/journey`([backend §2](./backend.md) · 의미론 [spec §4.9](../spec/LEDGER_TECHNICAL_SPEC.md)). 🔴 «화면은 여전히 없다»**: 이 레일은 아직 표 데이터로 그리는 옛 모양이고 그 라우트를 부르는 클라 코드가 **0줄**이다. **이 절의 어떤 문장도 여정 축이 화면에 있다는 뜻이 아니다.**
- 🔴 **조용히 잘리는 것이 없다.** 잘림 안내는 실수(20/37, 미표시 17)를 싣고 **`shown < scored`에서도** 발화한다 — 짧게 주고 말 안 하는 서버도 부분 목록을 완전한 것으로 통과시킬 수 없다. 다랏 맵 요약의 보류 프레임 수는 추정이 아니라 플랜 자신의 목록에서 찍는다. 귀속 커버리지·대조군 표본 비율·「규칙 미적용」과 「제외 0건」을 가르는 제외 버킷 둘이 전부 화면에 있다.
- 🔴 **맵의 단위는 웨이퍼이고, 선이 답 못 하는 자리에서 정확히 거절한다.** 포커스 웨이퍼의 DT 소스는 랏 하나(이름) + 슬롯 25개로, 코어 소스는 랏 둘 + 슬롯 25개로 보고하되 **쌍짓기를 거절한다** — 응답은 「무엇이 나타나는가」를 말하지 「무엇이 짝인가」를 말하지 않으므로 곱은 절대 화면에 안 나온다. 신원 없는 웨이퍼는 죽은 링크가 아니라 텍스트로 렌더되고 이유를 말한다.
- 🔴 **`wafer` URL 파라미터 — 좌석이 아니라 «앉은 웨이퍼»** (`f21a916`). 서버가 `projections[].frame.wafer`로 주는 base WF id 그대로이고 슬롯 번호도 프레임 키도 아니다(그 둘은 좌석 이름이고 좌석엔 시간에 따라 다른 웨이퍼가 앉는다). `parseSurpriseQuery`/`surpriseQuery` 화이트리스트 한 항목 — **`lotsQuery`에는 안 실린다**(`/lots`는 어느 웨이퍼가 포커스인지에 의견이 없다). 새 탭에 차게 붙여넣어도 바이트 동일로 왕복하고, **마킹 토글·열 편집**(href를 다시 짓는 두 경로 — 화이트리스트에 없는 파라미터가 조용히 떨어지는 자리)을 살아남는다.
