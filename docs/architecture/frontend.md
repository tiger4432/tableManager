...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-14 | **Owner:** UI / Excel Interaction
...
- ⏳ **설계 확정 · 미구현 셋** (정본 [SCENARIO_CONSOLE_BRIEF P0-2/P0-3/P0-4](../process/SCENARIO_CONSOLE_BRIEF.md) — **기능으로 읽지 말 것**): ① P0-2-1 마킹 단위 승격 — 웨이퍼 클릭=웨이퍼 마킹, 비교 모집단은 마킹 집합 그대로(서버 절반은 `scope=wafer:<id>`로 오늘 표현 가능 — [backend §2 `/siblings`](./backend.md)) ② P0-3 여정 대조(위 레일 항목) ③ P0-4 대치 사다리 + 스텝 서브그래프 탐색기(소유자: «내일», 공동 설계 후 착수).
