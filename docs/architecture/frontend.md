...
- **게이트는 계약 하네스만이 아닙니다**(2026-07-30 정정). 종전 이 줄은 *"4계약 전부 통과"*로 끝났는데, 그것은 **`check:contracts`의 상태일 뿐 게이트 전체의 상태가 아닙니다.** 지금 세 채점자가 있습니다:
  ...
  - `check:harnesses` — **`client2/tests/*.mjs` 발견식 스캔**(2026-07-30 밤 신설, `scripts/check_harnesses.mjs`). 계약 러너와 같은 모양이고 **빈 스캔은 실패**입니다. 실측 **커밋 트리 `77d27d3` 기준 18개 중 13개 강제 · 5개는 스크립트 안에 사유가 적힌 부채 목록**이며, 부채 항목이 초록으로 돌아오면 「목록에서 빼라」는 줄을 출력합니다. ⚠️ **수는 `client2/tests/*.mjs` − `KNOWN_RED` 하나로 나옵니다.** 그 디렉터리에는 **`seam_7b_oracle.py`도 있는데 파이썬이라 스캔 대상이 아닙니다** — 파일 19개 ≠ 하네스 19개입니다. 🔴 **직전에 적혀 있던 「16개 중 11」은 적힌 그 시점에 이미 틀린 수였습니다** — `map_key_datalist_harness.mjs`(`dde342c`)가 아무도 세지 않은 채 이미 착지해 실제로는 17이었고, `virtual_column_render_harness.mjs`(`4b50135`)가 18로 만들었습니다. **이 수는 하네스가 추가되는 커밋마다 낡습니다** — 인용할 일이 있으면 세지 말고 **러너를 돌려 그 출력을 읽으십시오.** 🔴 **이 게이트가 생긴 이유**: 직전까지 15개 중 **14개를 아무도 부르지 않았고**, 그 조건이 `split_registry_harness.mjs`를 몇 주 죽어 있게 뒀으며 `da8f390`이 두 개를 더 죽인 채 푸시되게 했습니다. **조용히 skip하면 게이트가 없는 것과 같으므로** 부채는 목록으로 드러냅니다.
