...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-06 | **Owner:** UI / Excel Interaction
> **Source-of-truth:** `client2/src/*`, `client2/vite.config.js`, `client/desktop_wrapper.py`
>
> ### 이번 라운드 (2026-08-06 · 프레임 바인딩 삭제 `62520b9` + 한 동작 확정 `02416d4`)
...
- 🔴 **확정은 한 동작입니다**(`02416d4`, 제품 소유자 판정 2026-08-06) — 무장(arm) 단계도 두 번째 확인창도 없고, 확정 버튼 클릭 또는 `Enter` 한 번이 곧 쓰기입니다. `Enter`는 **확정 버튼에 포커스가 있거나 아무 컨트롤에도 포커스가 없을 때만** 그 경로를 탑니다. ⚠️ **HEAD 시점에 마크업(`map_editor2.html`)의 버튼 라벨·힌트는 아직 두 단계 문구를 들고 있고**, 그것을 코드와 맞추는 편집은 **워킹트리에만** 있습니다 — **동작은 착지했고 문구만 안 착지했습니다.** 문구가 커밋되기 전까지 이 절을 「화면이 그렇게 말한다」로 읽지 마십시오.
