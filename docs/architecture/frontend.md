...
> **Status:** 🟢 Living | **Last-verified:** 2026-08-08 | **Owner:** UI / Excel Interaction
> **Source-of-truth:** `client2/src/*`, `client2/vite.config.js`, `client/desktop_wrapper.py`
>
> ### 이번 라운드 (2026-08-08 · 리빙 동기화 ― `e943e46`·`21209d7`·`c959368`·`15a2b39`)
> - ✅ **§4.2의 「마크업 문구는 아직 두 단계」가 해소됐습니다** — HEAD 실측: `map_editor2.html`이 「쓰기는 한 동작(클릭 또는 Enter)」·`Enter 확정`을 들고 있고 `data-armed`는 없습니다.
> - 🔴 **§4.2에 셋을 더했습니다.** ① **확정 버튼의 유일한 관문은 「무언가 골랐는가」**(`21209d7`) — `not_scorable`과 「추측 위에 선 답」은 **막는 것이 아니라 알리는 것**이 됐습니다(채점기가 못 채점한 상태가 **사람이 유일한 답인 바로 그 상태**였습니다). ② **후보 문구는 `앞면/뒷면`이 아니라 `좌상단/우상단 시작`**(`c959368`) — 순번 축에서 **거울 반쪽이 곧 우상단 시작 반쪽**이고, 정본은 [MAP_ALIGNMENT_SPEC §2.4](../spec/MAP_ALIGNMENT_SPEC.md)입니다(여기서 다시 유도하지 마십시오). ③ **워크리스트는 테이블마다 묻는다**(`e943e46`) — 안 다시 물으면 실패는 「낡은 목록」이 아니라 **「한 화면에 두 테이블」**이고, 교체는 `AbortController` 하나이며 **취소는 실패가 아닙니다.**
...
| `map_editor2.js` + **`src/map2/*` (18 파일)** | 412 + 9,877 | **Map Editor 2 — 맵 정렬 화면**(§4.2, 개발 중). 🔴 **파일별 행을 여기 만들지 않습니다** — 이 디렉터리는 라운드마다 모듈이 갈라지고 합쳐지는 중이라 목록이 낡는 속도가 이 표의 유지 주기보다 빠릅니다. **읽어야 할 것은 파일 이름이 아니라 층 경계**이고 그 정본은 [MAP_ALIGNMENT_SPEC §0.2](../spec/MAP_ALIGNMENT_SPEC.md)입니다. 🔴 **레거시와 달리 이 층들은 모듈 상태를 갖지 않고, 그래서 하네스가 소스를 자르지 않고 `import`합니다** — 그 성질이 깨지면 이 디렉터리를 만든 이유가 없어집니다 |
...
- 🔴 **후보 문구는 `앞면/뒷면`이 아니라 `좌상단/우상단 시작`입니다**(`c959368`). 순번 축에서 `rotθ_back`과 `rotθ_front`는 **serpentine이 어느 모서리에서 시작하는가**로만 갈리므로, **거울 반쪽이 곧 우상단 시작 반쪽**입니다([MAP_ALIGNMENT_SPEC §2.4](../spec/MAP_ALIGNMENT_SPEC.md)가 정본이고 여기서 다시 유도하지 마십시오). `뒷면`이라고 적으면 조작자에게 **물리 웨이퍼 면이 선택됐다**고 말하는 것인데, 한 클릭 옆 레거시 에디터에는 **진짜 물리 뒷면이 있고 이 화면은 그것을 결정하지 않습니다.** ⚠️ **저장된 철자(`rot270_back`)는 그 옆에 mono로 그대로 남습니다** — 이 화면의 규칙은 「DB가 든 것을 **감추지 않는다**」이지 「뜻을 말하면 안 된다」가 아닙니다.
