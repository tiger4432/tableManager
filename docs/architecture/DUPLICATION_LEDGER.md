...
> **Status:** 🟢 Living | **Last-verified:** 2026-09-05 — **D-6 이 «삭제»로 해소돼 묘비 + 생존자 줄로 교체**(사본이 살던 파일 셋이 트리에 없다. 합쳐진 것이 아니라 **양쪽이 지워졌다**) · 직전 2026-08-06 — **D-1 재실측 + 라인 앵커 전면 삭제** | **Owner:** 전 에이전트 공용 · **유지: doc-keeper** | **Source-of-truth:** 각 항목의 「몇 곳」 — 전부 grep 실측(**심볼 기준**)
...
- **어느 것이 정본인가**: `utils/time_format.to_local_str`. 소비자는 `main.py`(재수출 포함)와 `chain/ingestion_worker.py`.
- **왜 아직 안 합쳤나**: **이유 없음 — 그냥 안 했다.** `utils/time_format.py`가 분리될 때 이 사본이 함께 걷히지 않았다. ⚰️ **[2026-08-14 `2ec78b9`] 사본의 «집»이 은퇴했다** — `graph_sync_worker.py`는 프로세스 스택에서 빠졌고 모듈은 판정 ④의 코드 제거 라운드에서 파일째 사라진다. **이 D-5 항목은 그때 자동으로 닫힌다**(중복이 합쳐져서가 아니라 사본이 없어져서 — 두 결말은 대장에 다르게 적어야 한다).
- **합칠 때의 함정**:
...
### 🗄️ D-6. ~~`labelToken` + `PALETTE_TOKENS` — 바이트 동일 사본 둘~~ — **[2026-09-04 `45d8b66f`] «삭제»로 해소됨**

🗄️ **믿지 말 것**: 사본이 살던 파일 셋(`graph_viewer.js` · `trace.js` · `trace_core.js`)이 **트리에 없다.** 「합칠 때의 함정」으로 적혀 있던 것도 합칠 대상이 없어 **행동 지침이 아니다.** 합쳐서 해소된 것이 아니라 **양쪽이 함께 지워져서** 해소됐다.

✅ **살아남는 생각 둘** — 다음에 라벨에 색을 배정하는 사람이 읽어야 하는 것:
- 🔴 **첫 목격 순서로 인덱스를 배정하면**(`팔레트[본 순서 % 길이]`) **같은 라벨이 화면마다 다른 색이 된다** — 그리고 그 어긋남은 **합칠 때 사용자에게 「색이 왜 바뀌었나」로 나타난다.**
- 🔴 **진짜 수리는 사본을 합치는 것이 아니라 «순서 의존»을 없애는 것이다** — 라벨 문자열의 해시 → 토큰. 그러면 화면들이 **구조적으로** 같은 색을 갖고, 새 화면이 셋째 사본을 만들 유인도 사라진다. 이 저장소의 「근원 템플릿 요소 개발 후 데이터 갈아끼우기」와 같은 형태다.
...
| **`server/scripts/reapply_chain.py`** — `chain/replay.py`의 R1과 같은 일을 하면서 `source_name="reapply_chain"`을 썼는데, 그 이름이 `SOURCE_PRIORITY`에 **미등재라 99로 떨어졌다**(등록된 모든 소스보다 아래). 옳은 값을 쓰고도 아무것에나 지고, R2로 철회하려면 그 이름을 알아야 했다 | `8f8be4b` (2026-07-31) | 소급 진입점의 정본은 [guide/BACKFILL_GUIDE](../guide/BACKFILL_GUIDE.md)의 결정표 하나다 |
