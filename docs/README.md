...
| 🟢 [config_reference/](./guide/config_reference/README.md) | **이 환경의 실 config 사본**(2026-08-02 신설) — `.sample`이 *형태*라면 이쪽은 **실제로 돌아가고 있는 선언 전체**다(자격증명·호스트·절대경로는 복사 전 제외). 🔴 **배포물이 아니다** — 현장마다 이름이 다르므로 복사해 덮으면 그쪽 선언이 사라진다. **절차는 CONFIG_ROLLOUT_GUIDE, 선언은 여기** |
