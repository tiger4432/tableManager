...
| 🟢 [backend.md](./architecture/backend.md) | 5-프로세스 토폴로지, API 엔드포인트, outbox 패턴, **프로세스 감시·`/health`·진행 박동(§1.3)** · **§1.2에 `server/scripts` 한 방향 문**(2026-07-31 — 런타임이 그쪽을 import하면 **운영에서만** 죽고 트리거는 **200 `queued` 뒤 워커 스레드에서** 죽는다. 「import되는가」는 공유 인터프리터에서 테스트 불가라 판정은 별도 프로세스) · **§2.2 `/schema`의 `virtual_columns`**(가상 컬럼을 **`columns`에 합치지 않고 별도 키로** 알린다 — 그 키를 무시하는 클라는 키가 없던 때와 같게 동작한다. 🔴 읽기 전용을 지키는 것은 여전히 쓰기 깔때기) · **소급 적용 어드민 3라우트**(건수는 `count_kind`를 함께 답한다 · 실행은 아웃박스 한 줄 + 즉시 반환) · **그래프 조회 표에 `GET /graph/chip-trace` 등재**(2026-07-30 — depth 없는 고정 형상 · 다리별 닫힌 어휘 5종 + `scope_unresolved` · 절단은 상태가 아니라 플래그) |
| 🟢 [frontend.md](./architecture/frontend.md) | client2 웹(AG-Grid) + QtWebEngine 데스크톱 셸 · **§2.1 빌드 게이트**(`prebuild`의 **세 채점자** — 클립보드 관례 + `contracts/*` **5계약** 발견식 스캔 + `client2/tests/*.mjs` **18개 중 13 강제·5 부채**. **빈 스캔은 실패**이고 목록의 정본은 `package.json` — 🔴 **이 수는 인용하지 말고 러너를 돌려 읽을 것**, 직전 「16」은 적힌 시점에 이미 틀렸다) · **§3.4 가상 조인 컬럼 렌더**(2026-07-31 — 🔴 **그리는 순간 그 컬럼은 쓰기 대상이다**: 붙여넣기는 컬럼 목록이 아니라 **그리드 컬럼 id**로 배치를 만든다. 쓰기 문 넷에 가드, **복사는 반대로** 받아들인다. ⏳ CSV 추출 누락 · `미상` 행 검색 불가) · **§3.3 값 제안 셀 에디터**(`Enter` 한 번이 채택이고 확정 · `Escape`는 시계가 아니라 `suggestionsEngaged`가 판정) · **§5 config 해석 한 줄**(F9 — 문장은 서버가 만들고 클라는 그대로 렌더 · **읽기 실패 다섯 갈래는 상태코드가 아니라 `WWW-Authenticate` 헤더로 갈린다**) |
