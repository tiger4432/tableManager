...
| 🟢 [frontend.md](./architecture/frontend.md) | client2 웹(AG-Grid) + QtWebEngine 데스크톱 셸 · **§2.1 빌드 게이트**(`prebuild`의 **세 채점자** — 클립보드 관례 + 계약 하네스 4종 발견식 스캔(**빈 스캔은 실패**) + 값 제안 키보드 계약. 목록의 정본은 `package.json`) · **§3.3 값 제안 셀 에디터**(`Enter` 한 번이 채택이고 확정 · `Escape`는 시계가 아니라 `suggestionsEngaged`가 판정) |
...
| 웨이퍼 맵 에디터 | 🟢 [map_editor/](./map_editor/README.md) · [spec/MAP_EDITOR_SPEC.md](./spec/MAP_EDITOR_SPEC.md) — 🔴 **§1의 0) 좌표 규약이 먼저다**(화면이 기준 · 표시 = 오리진 + DB 값 · `start_x/y` = 유효 다이 영역의 최소 열·행이자 **운영자의 선언** · 오리진 = start가 (0,0)으로 읽히는 칸 · **저장 좌표 = 오리진 기준 칸수, mm 아님** — 피치를 곱해 읽으면 없는 결함이 만들어진다). §1~§4 격자 에디터 · **§4-ter 회사 양식 왕복**(COPY HEADER MODE ↔ Ctrl+V · 거부 5갈래 · ⚠️ 선언 맵 179개 중 27개만 프레임 지문을 가져 **나머지에서는 붙여넣기 거부가 정상**) · **§5 범용 맵 오버레이**(`wafer_map_metadata`가 정렬의 유일한 기준 · 변환은 클라 단일 구현) · **§5.7-bis 참조가 정렬되지 않을 때**(아무것도 채택하지 않고 **셀은 자기 저장 좌표가 가리키는 칸으로 다시 앉는다** · 알람 축은 치수가 아니라 **원점**) · **§6 전사 계획**(층 구조는 zone 모델 · 저장을 막는 데이터 보호 게이트 4종은 §6.0-ter). 🔴 **§5.7-bis는 기본적으로 불신하라** — 2026-07-30 하루에 네 번 뒤집혔고 제품에 없는 문구를 인용한 코드 블록이 두 번 남았다. **인용 전 소스 grep.** |
...
| 유효 다이 맵 가이드(사용자) | 🟢 [guide/VALID_DIE_MAP_GUIDE.md](./guide/VALID_DIE_MAP_GUIDE.md) — **유효 다이도 맵이다**: 어느 셀이 유효한지를 원 기하가 아니라 **다른 맵 하나**가 정하게 한다. 프리셋의 「🧩 템플릿 만들기」로 저작 → ⚡ Push → 쓸 맵의 「🎯 유효 다이 맵」 칸에 키 지정 → ⚡ Push. 참조는 1단계 · 키를 비우면 원 복귀. **§4-bis.0 좌표 규약 5줄이 먼저다**(spec §1의 0)과 같은 내용 — 🔴 **저장 좌표 = 오리진 기준 칸수, mm 아님**). **§4-bis.1~.2**: 지정은 어떤 축도 채택하지 않지만(격자·회전·면·물리 규격·START 전부 그대로) **칠한 셀은 화면에서 움직인다** — 자기 저장 좌표가 가리키는 칸으로 다시 앉는 것이고 **안 움직이면 오히려 좌표가 깨진 것**이다. 알람은 크기가 아니라 **원점**을 보므로 **크기가 같아도 뜬다**. `⚡ Push`가 쓰는 x/y는 지정 전과 동일. **§4-bis.3 `[유효다이]` 콘솔 7줄**로 눈으로 확인 · **§4-bis.4** 거절은 참조 메타의 격자 치수가 `1~100` 정수 밖일 때 하나뿐 · **§4-bis.5** 정렬을 맞추려면 **START를 고친다**. ⚠️ 이 절은 하루에 세 번 뒤집혔다 — 종전의 *"지정은 화면을 한 글자도 안 바꾼다"*와 그것이 인용한 토스트 블록은 `da8f390`으로 거짓이 됐다. |
