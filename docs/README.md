...
| 🟢 [frontend.md](./architecture/frontend.md) | client2 웹(AG-Grid) + QtWebEngine 데스크톱 셸 · **§2.1 빌드 게이트**(`prebuild`의 **세 채점자** — 클립보드 관례 + `contracts/*` 발견식 스캔 + `client2/tests/*.mjs` 발견식 스캔. **빈 스캔은 실패**이고 목록의 정본은 `package.json` — 🔴 **수를 여기 적지 않는다. 러너가 매 실행마다 `N harnesses ― M gated, K known-red`를 찍으므로 그것을 읽을 것.** 「16」도 「18개 중 13 강제·5 부채」도 적힌 뒤 곧 틀렸고, 두 번째 것은 **「이 수는 인용하지 마라」는 문장 바로 앞에** 적혀 있었다. **단언 플로어(바닥)와 천장(`CEILINGS` — 현재 `map_editor.js`의 모듈 레벨 가변 바인딩 상한)까지가 이 게이트다**) · **§3.4 가상 조인 컬럼 렌더**(2026-07-31 — 🔴 **그리는 순간 그 컬럼은 쓰기 대상이다**: 붙여넣기는 컬럼 목록이 아니라 **그리드 컬럼 id**로 배치를 만든다. 쓰기 문 넷에 가드, **복사는 반대로** 받아들인다. ⏳ CSV 추출 누락 · `미상` 행 검색 불가) · **§3.3 값 제안 셀 에디터**(`Enter` 한 번이 채택이고 확정 · `Escape`는 시계가 아니라 `suggestionsEngaged`가 판정) · **§5 config 해석 한 줄**(F9 — 문장은 서버가 만들고 클라는 그대로 렌더 · **읽기 실패 다섯 갈래는 상태코드가 아니라 `WWW-Authenticate` 헤더로 갈린다**) |
