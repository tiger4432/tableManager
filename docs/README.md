...
| 문서 | 내용 |
|---|---|
| 🟢 [qa/FEATURE_CHECKLIST.md](./qa/FEATURE_CHECKLIST.md) | **기능 인벤토리 + QA 수동 점검 체크리스트** — 서브시스템별 기능 지도·진입 경로·릴리스 전 회귀 점검 절차(SLO·멱등성 포함). **§2.0 자동 게이트**(pytest + `npm run build` — 손으로 점검하기 전에 통과시킬 것) · **§1.11/§2.15 운영 감시**(감시자·`/health`·격리 환경) 포함. **2026-07-30 신설 6행** — §1.1 **값 제안 셀 에디터**(+ §2.1에 F3 점검 11건 · ⚠️ `switchTable`이 `txModeActive`를 되켠다는 절차 경고) · §1.7 유효 다이 맵(M4) · §1.9 **칩 추적**·재동기화 알림·**고아 스윕** · §1.10 **셸의 서버 주소 해석**. 새 기능 병합 시 doc-keeper가 갱신 |
