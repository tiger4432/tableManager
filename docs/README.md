...
| 유효 다이 맵 가이드(사용자) | 🟢 [guide/VALID_DIE_MAP_GUIDE.md](./guide/VALID_DIE_MAP_GUIDE.md) — **유효 다이도 맵이다** · 원으로 표현 못 하는 테이프 모양 · 프리셋 목록의 「🧩 템플릿 만들기」로 저작 → ⚡ Push → 다른 맵의 「🎯 유효 다이 맵」 칸에 키 지정 → ⚡ Push · 참조는 1단계 · 키 비우면 원 복귀 · **§4-bis 격자 크기가 다를 때**(2026-07-30 개정 — 격자는 **템플릿 크기로 열리고 칠해진 셀은 보존·재배치**된다: 저장 좌표는 그대로 있고 화면에서 앉은 칸만 바뀌며 그 개수를 알림이 말한다. 🔴 **저장 좌표는 오리진 기준 칸수이고 mm가 아니다** — 피치를 곱해 읽으면 없는 결함이 만들어진다. **새 격자가 그 좌표를 표현할 수 없을 때만 거절**하고, 그때 복구는 **격자 맞춤 → 📂 Load → Push** 3단계) · §8 = **Fill All 필터는 착지**, 오염된 초안 정리는 진행 중 |
