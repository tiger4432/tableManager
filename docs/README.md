...
| 유효 다이 맵 가이드(사용자) | 🟢 [guide/VALID_DIE_MAP_GUIDE.md](./guide/VALID_DIE_MAP_GUIDE.md) — **유효 다이도 맵이다** · 원으로 표현 못 하는 테이프 모양 · 프리셋 목록의 「🧩 템플릿 만들기」로 저작 → ⚡ Push → 다른 맵의 「🎯 유효 다이 맵」 칸에 키 지정 → ⚡ Push · 참조는 1단계 · 키 비우면 원 복귀 · **§4-bis 격자 크기가 다를 때**(2026-07-30 **재개정** — 🔴 **아무것도 채택하지 않는다**: 지정은 격자·회전·물리 규격을 **하나도 가져오지 않고**, 참조 격자가 다른 크기면 **마스크가 화면에서 밀려 보이는 것이 정상**이며 그 사실을 **info 토스트 1회**로만 말한다. **`⚡ Push`가 쓰는 x/y는 지정 전과 완전히 같다** — 셀에 좌표가 붙어 있지 않고 Push 시점에 캔버스 칸에서 유도되므로, "좌표 보존"의 유일한 구현이 **칸을 안 건드리는 것**이다. **거절은 참조 메타의 격자 치수가 `1~100` 정수 밖일 때 하나뿐**이고, 크기 차이로는 거절하지 않으므로 **복구 절차도 없다**. 🔴 **저장 좌표는 오리진 기준 칸수이고 mm가 아니다** — 피치를 곱해 읽으면 없는 결함이 만들어진다. ⚠️ 종전 이 자리의 *"격자가 템플릿 크기로 열리고 셀이 재배치된다 · 표현 불가일 때 거절 · 격자 맞춤→📂 Load→Push 복구"* 서술은 같은 날 오후 `61440e6`+`94b9baa`로 **전부 거짓이 됐다**) · §8 = **Fill All 필터는 착지**, 오염된 초안 정리는 진행 중 |
