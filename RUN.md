...
```
🔴 **알려진 함정 (2026-09-17 현재, 고치는 중):** «기존 규칙을 한 번 열어 본 뒤» 「+ 규칙 추가」를
   누르면 «옛 폼»이 나옵니다. 「추가」가 서버를 다시 안 부르고 앞서 받은 것을 다시 그리기 때문입니다.
   -> 지금은 «화면을 새로고침한 뒤» 바로 「+ 규칙 추가」를 누르십시오. 그러면 새 폼이 나옵니다.
