...
### ⑧ 🆕 개발 벤치에 `builtin:…` 을 넣으면 이제 «이름을 대어 거절»합니다 (09-17)

```
전  builtin:auto_confirm → {"rows": 0, "refusal": null}   <- 「행이 없다」와 같은 모양. 조인 종류는 그냥 터졌습니다
후  builtin:auto_confirm → refusal: "…는 등록된 종류이지 파일 맵퍼가 아닙니다 … 소급 화면으로"
```
샘플 파일을 입력으로 받는 것은 «파일 맵퍼»뿐입니다. `builtin:…` 은 스스로
데이터베이스에서 행을 찾으므로 벤치가 줄 것이 없습니다 — 그런 규칙은 «소급 화면»에서 돌리십시오.
⚠️ `모듈:함수` 로 직접 적는 방식은 «그대로»입니다.
