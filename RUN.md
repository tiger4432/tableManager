...
### ③ 🔴 「읽기 시점 조인」 선언은 이제 «거절»됩니다
```
사유   소유자 판정(09-16): 운영은 조인 컬럼을 «표에 써서» 씁니다 (into.table)
       그래서 읽기 시점 조인(into.read)은 «은퇴»했습니다
보이는 것   그런 선언이 남아 있으면 「읽기 시점 조인(into.read)은 은퇴했습니다 — …」가 «이름 대어» 뜹니다
할 일      그 선언을 `into: { table: … }` 로 바꾸십시오. «조용히 무시되지 않습니다»
