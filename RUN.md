...
> 🔵 **04:40 갱신.** 아래 ⓞ 가 «제일 먼저»입니다. 나머지는 그다음입니다.
...
③ 「0.」 절의 순서 셋
```
📌 아래 ①~⑨ 는 «할 일»이 아니라 **「화면·로그에서 «본 것»의 뜻」** 입니다. 무언가 이상해 보일 때 그 번호를 찾으십시오.
...
🔴 ⚠️ 위 ㉡ 의 수리는 «`virtual_join_rules.json` 에 사는 선언»에만 됩니다 (2026-09-17 추가)
   조인이 «원장 setup 번들 안»에 선언돼 있으면 오늘 **그 수리를 못 하십니다** — 번들 검증기가
   `materialize` 필드 «자체»를 거절합니다(「field is not allowed」). 즉 빼면 로더가 거절하고
   넣으면 번들이 거절합니다. 작성 폼에도 그 칸이 «아직 없습니다»
   -> 그 경우 **고치려 하지 마시고 그 줄만 알려 주십시오.** 제품 수정이 필요한 자리이고 진행 중입니다
   -> 어느 쪽인지 가르는 법: 그 조인 이름이 `virtual_join_rules.json` 에 있으면 위 ㉡ 그대로 하시면 됩니다
```
