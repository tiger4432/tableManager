...
```
[builtin:join] <규칙> | 왼쪽 행 N 개가 오른쪽에서 «둘 이상»과 맞아 «쓰지 않았습니다» … k=<값> → 오른쪽 2행 … → 다음: …
```
🔴 **이 줄이 뜨면 그 칸은 «비어 있습니다». 그리고 수리가 «둘 중 하나»입니다:**
```
키가 «좁다»   -> 선언을 고칩니다 (조인 키에 컬럼을 더해 오른쪽이 «하나»가 되게)
진짜 «중복»   -> 데이터를 고칩니다 (오른쪽 표의 중복 행을 치웁니다)
⚠️ 뒤집으면 사실이 사라집니다. 줄에 실린 «키 값»과 «오른쪽 행 수»가 그 둘을 가릅니다
```
종전에는 이 경우에 «둘 중 아무 값이나» 조용히 써 넣었습니다 — 그래서 화면에는 값이 있었고,
그 값이 «둘 중 어느 쪽인지»는 아무도 알 수 없었습니다.
