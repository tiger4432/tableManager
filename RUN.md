...
> 🔵 **01:00 갱신.** 아래 ⓞ 가 «제일 먼저»입니다. 나머지는 그다음입니다.
...
### ③ 🔴 「읽기 시점 조인」 은퇴 — 지금은 «통합 선언 파일»만 거절합니다
```
사유   소유자 판정(09-16): 운영은 조인 컬럼을 «표에 써서» 씁니다 (into.table)
       그래서 읽기 시점 조인은 «은퇴»합니다
지금 거절되는 것   chain_rules.json 의 `into: { read: true }` 선언
       -> 「읽기 시점 조인(into.read)은 은퇴했습니다 — …」가 «이름 대어» 뜹니다
       할 일: 그 선언을 `into: { table: … }` 로 바꾸십시오
⚠️ 아직 거절 «안» 되는 것   virtual_join_rules.json 의 선언은 «그대로 돕니다».
       그쪽 은퇴는 다음 라운드입니다 — 지금 옮기실 필요 없습니다
       (`materialize: true` 인 선언은 «쓰기 조인»이라 은퇴 대상이 «아닙니다»)
⚠️ 하나도 없으면 아무것도 안 보입니다. 그게 정상입니다
```
...
```
git revert 21abf176 7ecc6a1f     <- 🔴 «둘을 같이». 하나만 되돌리면 트리가 깨집니다
                                    (인덱스 좌석 이동이 두 커밋에 나뉘어 착지했습니다)
```
