...
🔵 ⚰️ **[해소됨] 위 ㉡ 의 수리가 «원장 setup 번들» 안에서도 됩니다** (판정 481·482 착지)
   ...
   빼면 로더가 거절하고 넣으면 번들이 «필드 자체»를 거절했습니다. 이제 아닙니다:
     ...
     · ⚠️ **작성 폼에 그 칸이 «생겼습니다». 다만 브라우저에서 만들면 아직 «꺼진 채» 태어납니다**
       (판정 484 는 «서버» 씨앗을 고쳤고, 브라우저는 서버를 안 거치고 «클라»가 씨앗을 짓습니다.
        그 절반이 아직 안 고쳐졌습니다 — 진행 중입니다. 제 앞선 줄이 «이미 켜진 채»라 적었는데 «거짓»이었습니다)
       🔴 **그래서 폼으로 조인을 만드시면 그 칸을 «직접 켜 주십시오»** — 안 켜면 거절됩니다.
          거절되면 그 문장이 「materialize: true 로 두라」고 그 자리에서 말합니다
       합법값이 «하나»라 «묻지 않습니다» — 폼으로 만든 조인은 그 칸 때문에 거절되지 않습니다
       「max_rewrite_rows」(선택)은 비어서 태어납니다 — 상한은 운영자가 정하는 값입니다
