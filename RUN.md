...
```
**확인 — 화면에서**: 인리치 규칙 목록에 통합 문법으로 적은 규칙이 «뜨면» 맞습니다.
그 규칙의 참조뷰를 누르면 표가 나와야 합니다.
```
🔴 안 뜨면    그 선언이 «거절»됐거나 `enabled: false` 입니다 — 부팅 로그의 그 이름 줄을 보십시오
⚠️ 뜨는데 표가 비면   참조뷰의 바인드(`:컬럼명`)가 판단키 컬럼이 아닙니다. 거절 문구가 이름을 댑니다
```
**남아 있는 것 (이번에 «안» 건드렸습니다)**: 같은 목록을 읽는 자리가 **15**인데 이번에 넓힌 것은
참조뷰 라우트 **둘**뿐입니다. 나머지 13(대시보드 요약 · 정렬 확정/작업목록 · 후보 탐침 · 그래프 ·
설정 해석 보고 · 모델 · 소급 · 정렬 뷰 · 스크립트 둘 등)은 여전히 통합 문법 선언을 «못 봅니다».

## 🆕 19:40 — 선언 «종류별»로 도는지 한 번에 재는 시험
...
```
```
6 passed 면 다섯 종류가 «행을 남기며» 돕니다
  join · decide(파생행) · decide(오토컨펌+참조뷰) · mapper(운영자 파일) · virtual_join
