# 🔓 [클라 V] 추가 게이트 ④⑤⑥ — **⑥ 이 빠져 있었고 채웠습니다.** 그리고 «흉내내지 않았습니다»

## ④ 자기 고리 술어가 «경로에 나타납니다» — 실측
```
wafer → defect_kind 다섯 경로 중 «셋»이 자기 고리를 지납니다
   2홉 [measures, leads_to]                              loops 0
   3홉 [inspected, observed, of_kind]                    loops 0
   4홉 [inspected, «bonded_from», observed, of_kind]     loops 1   wafer → die → die → defect → …
   4홉 [inspected, «transfer», observed, of_kind]        loops 1
   5홉 [inspected, bonded_from, transfer, observed, …]   loops 2   die → die → die
=> 지시가 요구한 둘(transfer · bonded_from) «둘 다» 나타납니다
```

## ⑤ 홉 수가 보이고 «따라옵니다»
```
목록      「경로 A · 2홉 · wafer → quantity → defect_kind」  — 홉이 라벨에 있습니다
고르면    this.hops = route.hops  ->  spec.hops 로 «실려 나갑니다» (앞 커밋에서 배선)
실측      모든 경로가 자기 hops 를 «들고» 있습니다 (true)
```

## ⑥ 🔴 «빠져 있었습니다» — 반복 횟수를 말 안 하고 있었습니다
```
전   자기 고리 경로도 그냥 「4홉」이라고만 했습니다
     -> 「한 대 위까지만 봤다」가 화면에서 «「그게 전부다」»로 읽힙니다
후   자기 고리가 있는 경로에 «한 줄» 답니다:
     「경로 C · 4홉 · wafer → die → die → defect → defect_kind · 같은 타입 되짚기 «1회»가 기본값입니다」
=> 「묻거나, 기본값이면 기본값이라고 말한다」 중 «뒤엣것»입니다. 오늘의 부재 규율 그대로입니다
```

## ⛔ 그리고 «흉내내지 않았습니다» — 지시대로 올립니다
```
지시   「자기 고리 술어면 hops 를 넉넉히」로 화면에서 흉내낼 수 있지만 «그건 임시»이고,
       제대로 하려면 walk 의 깊이 계산이 «자기 고리를 0으로» 세야 한다 — «서버 변경»
제 상태  hops 는 «구조적 홉 수» 그대로 보냅니다 (자기 고리도 한 홉으로 셉니다).
         넉넉히 얹지 «않았습니다» — 얹으면 그 수가 «무엇을 뜻하는지»가 화면과 서버에서 달라집니다
⏭ 그래서 이 줄을 보고로 올립니다: 자기 고리 경로의 hops 는 «서버가 0으로 세 주면» 하나 줄어듭니다.
   그때까지 화면은 «구조적 수»를 말하고, 되짚기가 기본 1회임을 «문장»으로 답니다
```
하니스: walk_box 48/0 · board 170/0
...
## ① 절단 문장 — «그려집니다»
```
코드    walk_box_panel:_resultBox  ->  truncated.reason 이 있으면 is-cut 문장
⚠️ 제가 「모델이 배열로 주니 이 검사는 항상 거짓」이라고 «의심»했는데 틀렸습니다:
   이 부품은 subgraphModel 이 아니라 createWalkBoxWalk 을 씁니다 -> 서버의 «원본 truncated» 를
   그대로 받습니다. .reason 이 «맞는 필드»입니다
실측 (서버)   node_limit=400   truncated.reason = "nodes"
              node_limit=1000  truncated.reason = "nodes, edges, claims"
실측 (부품)   그 값으로 렌더 -> 「예산에서 끊겼습니다 — nodes, claims. 이게 전부가 아닙니다」 «1개»
```
📌 오늘 여러 번 「이 검사는 도달 불가」를 잡았기 때문에 같은 눈으로 봤는데, 이번엔 «아니었습니다».
   고치기 전에 «어느 함수가 답을 만드는지»를 본 것이 갈랐습니다.

## ② 행 클릭 = 이동 — «이미 이력에 남습니다»
```
경로    행 클릭 -> mark(id, CASE, replace) -> 저장소에 씀 -> «제 구독»이 밖의 변화로 보고 push
실측    클릭 전 이력 0 -> 클릭 후 «1» · 저장소 [["A",1]]
=> onRowClick 을 moveTo 로 «바꿀 필요가 없었습니다». 밖에서 쓴 것을 자식으로 붙이는 그 줄이
   부품 자신의 쓰기에도 «그대로» 성립합니다 — 쓰는 곳이 하나라는 원리의 부수 효과입니다
```

## 라운드 W 부품 넷 — 상태
```
① 시작(타입·키)   있던 것       ② 걷기(follow)   있던 것
③ 결과 + 절단문장 «있던 것 · 재서 확인»       ④ 클릭/찍기 + 이력 나무   이번 라운드
게이트 ①②③④⑤⑦ 전부 실측 통과 (앞 두 보고에 수치)
```
⏭ 지시서 범위는 여기까지입니다. 코드 변경 «0» 이라 빌드도 안 했습니다 — dist 는 `ee9272eb` 그대로입니다.

---

# 🔓 [클라 W] **이력 나무 표시 착지** — 게이트 ⑦ 을 «화면에서 진짜 찍어» 통과했습니다

## 게이트 ⑦ — 빈 상태가 아니라 «채운 상태»로
```
⚠️ 첫 시도는 «0 걸음»이었습니다 — 본딩 맵을 찍었는데 그건 marking:1 이고 탐색기는 marking:2 를 씁니다.
   그 자체가 게이트 ①(이름이 다르면 안 섞인다)의 실물 확인이라 그대로 적습니다.
실제 시험   코어 맵(writes marking:2)을 «두 번» 찍었습니다
   찍기 1회  ->  이력 «1»
   찍기 2회  ->  이력 «3»
   머리      「탐색 이력 3 · marking:2」
   나무      「1개」 · 「· 0개」 · 「· · 1개 ←」   (들여쓰기 = 깊이 · ← 는 지금 선 칸)
```
📌 가운데 «0개»가 토글 해제입니다 — 같은 다이를 다시 눌러 «뺀» 걸음. 이력이 «뺀 것도» 남깁니다.
   되감으면 그 상태로 돌아갑니다 — 그게 「자르지 않는 나무」의 뜻입니다.

## 표시 자체 (스텁 위 실측)
```
그린 칸 수 = 이력 크기 «4 = 4»
현재 표시  «1개»만 is-here
아무 칸이나 눌러 이동   -> 저장소가 그 값으로 «따라옵니다» ([["n1",1]])
   그리고 이력이 «안 자랍니다» (4 -> 4) — goto 는 칸을 «만들지 않습니다»
쓸 이름이 없는 인스턴스는 이력을 «아예 안 그립니다» — 빈 상자는 「했는데 없다」로 읽히므로
```

## 남은 것
```
지시서 부품 넷 중 ①시작 ②걷기 ③결과 는 «이미 있던 것»이고 ④(클릭/찍기)가 이번에 붙었습니다
⚠️ 결과의 truncated 를 «문장»으로 내는 것은 아직입니다 — 지시서 ③의 그 줄입니다
```
하니스: walk_box 48/0 · board 170/0

---

# 🔓 [클라 W] **탐색기 상태 기계 착지** — 지시서 다섯 줄 그대로. 이력 «표시»는 다음 조각

## 게이트 — 실측 (DOM 스텁 위에서 «두 인스턴스»로)
```
① 조립식      탐색기 «둘» · 다른 쓰기 이름(marking:1 · marking:3)
              A 이력 3 · B 이력 1 · marking:3 = [["z1",1]] · B 가 marking:1 을 «안 건드림» ✅
② replace     한 번 emit (앞 커밋에서 실측 · 이 커밋은 그걸 «유일한 쓰기»로 씁니다) ✅
③ 루프 없음   goto 두 번 -> 이력 증가 «0». 재진입 플래그 «없음» ✅
              (goto 직후 저장소 값 == 현재 값 -> 「밖에서 바뀜」이 값 비교로 자동 거짓)
④ 안 자름     뒤로 간 뒤 «다른 갈래»로 이동 -> 먼저 갈래가 «살아 있음» (형제 1) ✅
⑤ 서버 «0» · 새 라우트 «0» ✅
```

## 두 동작이 «갈립니다» (소유자 ②)
```
클릭 = 이동   moveTo(id)   -> push([[id, CASE]])        마킹이 «그 노드 하나»가 됩니다
              실측: 값 [["n1",1]] · 저장소도 같은 값
찍기 = 수집   collect(id, sign) -> push(현재 ± (id, sign))   집합을 «쌓습니다»
              실측: 이동 뒤 찍기 -> 2 노드
```

## 밖에서 바뀌면 «자식»으로 붙습니다
```
실측   markings.replace(밖에서) -> 이력 «+1» · 현재 값이 그 밖의 값
뜻     다른 부품이 찍은 것도 «탐색의 한 걸음»입니다. 그래야 「어디서 왔는지」가 이력에 남습니다
```

## 쓰는 곳은 `goto` «하나»
```
moveTo · collect · 뒤로 · 다른 갈래 · 밖에서 바뀜 — 전부 push/goto 를 지나갑니다
=> 「무엇이 마킹을 바꿨나」의 답이 «언제나 한 줄»입니다
쓸 이름은 좌석의 `writes` 입니다 — 마킹3·4 가 «선언 한 줄»이 되는 것이 소유자 ③ 그대로입니다
   (실측에서 B 인스턴스가 marking:3 으로 섰습니다)
```
⏭ 다음: 이력 «나무 표시»(아무 칸이나 눌러 goto)와 게이트 ⑦(화면에서 진짜 찍어 재기).
하니스: walk_box 48/0 · board 170/0 · walk 32/0

---

# 🔒 [클라 B] **잡습니다 (라운드 W)** — 그리고 저장소 primitive 를 «먼저» 착지시킵니다

잡는 파일: `walk_box_panel.js` · `marking_store.js` · `api.js` · `main.js` (지명받은 넷)

## 지시대로 «제가 다시 쟀습니다» — 총괄 수와 «조금 다릅니다»
```
총괄   「clear() + set()×N 이라 emit 이 N+1 번」
제 실측
   이름이 «비어 있지 않았을 때»  N+1 «맞습니다» (3노드 복원 -> emit 4)
   이름이 «비어 있었을 때»       N 입니다      (emit 3) — clear 가 빈 것엔 emit 을 «안 합니다»
```
### 🔴 그런데 더 나쁜 것은 «횟수»가 아니라 «보이는 값»이었습니다
```
구독자가 본 크기   [0, 1, 2, 3]     <- «0 을 봅니다»
그 0 이 문제입니다: 부품이 그걸 「아직 안 골랐습니다」로 읽고 «다시 걷습니다».
즉 이력에서 한 걸음 뒤로 갈 때마다 화면이 노드 수만큼 «다시 그리고 다시 묻습니다»
```

## `replace(name, entries)` — 착지
```
3노드 복원(비어 있지 않은 이름 위)   emit «1» · 구독자가 본 크기 [3]      (전: 4 · [0,1,2,3])
같은 값으로 replace                 emit «0»   <- set() 의 기존 가드를 «집합 전체»로 연장
빈 값으로 replace                   emit 1 · count 0
빈 값 위에 빈 값                    emit «0»
다른 이름                           안 건드림 · 부호 보존 (CONTROL -1 그대로)
```
📌 「같은 값이면 emit 안 함」이 탐색기의 «밖에서 바뀌었나»를 «값 비교»로 만들어 줍니다 —
   goto 직후엔 저장소 값 = 현재 값이라 그 검사가 자동으로 거짓이고, 재진입 플래그가 필요 없습니다.

⏭ 다음: 탐색기(나무 · 이동/수집 · 이력 표시)를 `walk_box_panel.js` 에.
...
## 게이트 ① — 404
```
전   요청 8 · 404 «2»   (trends?window=180d · trends?kinds=void&grain=…)
후   요청 7 · 404 «1»   (trends?window=180d «만»)
남은 것   🔴 좌석 3 «control-bar» 입니다 — 좌석 4·6 이 아닙니다
          그 좌석은 Y축 «고르는 목록»이고 마킹을 안 읽습니다(reads: axis:y · start 없음).
          ⓐ 를 그대로 적용하면 «마킹이 비면 고를 것도 없는» 화면이 됩니다 — 그건 제품 결정이라
          안 했습니다. 지시가 좌석 4·6 이었던 것과도 맞습니다
📌 참고로 그 좌석의 «Y축 목록»을 만들던 두 번째 trend_y 호출은 제거했습니다 —
   종류는 응답의 defect_kind 노드가 이미 말하므로 «같은 걷기»에서 나옵니다
```

## 게이트 ② — 마킹 «비었을 때»와 «찼을 때» 둘 다
```
비었을 때   메인 트렌드   「marking:1 이 이 차트의 주어입니다 · marking:1 이 비었습니다 — 고르면 그립니다」
            후보 트렌드   「marking:2 이 … 비었습니다 — 후보를 고르면 그립니다」
찼을 때     맵에서 다이 하나를 «실제로 찍고» 재측정:
            맵 머리가 「마킹 1 · 표시 1」로 바뀌고 트렌드가 다시 걷습니다
            walk 실측  nodes 400 · edges 518 · complete «false» · truncated ["nodes","claims"]
            limits {"nodes":400,"edges":1200,"claims":2400,"actions":1200,"max_hops":40}
            -> 모델이 «수 대신» 「이 걷기는 예산에서 끊겼습니다 — 여기까지 봤습니다」
```
### 🔴 그리고 그 자리에서 화면이 «틀린 문장»을 쓰고 있었습니다
```
모델은   state "truncated" + 위 문장을 «싣고 있었는데»
화면은   점 0 이면 무조건 「이 창에 점이 없습니다」를 그렸습니다
=> 예산에서 «못 본 것»을 «없는 것»으로 바꿔 말하는 자리입니다. 오늘 unscanned 에서 세운 규칙과
   같은 부류라 화면이 모델의 문장을 쓰게 고쳤습니다
```

## 어떻게 했나 — 서버 «0» · 새 라우트 «0»
```
좌석 4·6   collect: trend_y  ->  start = 읽을 마킹 · follow = [inspected, observed, of_kind]
trendFromWalk   웨이퍼별로 점 하나. at = 그 웨이퍼 엣지의 최신 occurred_at ·
                found = observed · denominator = inspected · rate = 둘의 비
                kinds = 응답의 defect_kind 노드 (코드에 종류 이름 «없음»)
절단            complete:false / truncated 면 «점을 안 만듭니다». 수를 내면 거짓이 됩니다
부재            마킹이 비면 「아직 안 골랐습니다」 — 다른 좌석들과 같은 문장 규율
```
하니스: board 170/0 · control_trend 36/0 · composition 40/0 · walk 32/0

---

# 🔓 [클라 B] 남긴 422 — **재서 잡았습니다.** 한 줄이었고, 제 어제 판단은 «틀렸습니다»

## 어제 제가 적은 것과 실제
```
어제   「그 좌석은 이번 로드에 요청을 «아예 안 냅니다». 배선이 아니라 상태 표시입니다」
실제   요청은 «나가고 있었습니다» —  GET /api/ledger/subgraph?id=«SYN-CX-CHIP-001»&follow=bonded_from
왜 못 봤나  walk 이 같은 spec 을 «합칩니다». 세 좌석(구성·머리·펼친 층)이 «같은 요청 하나»를
            나눠 쓰고 있어서, 목록에 한 줄뿐이라 제가 «다른 좌석 것»으로 읽었습니다
```
🔴 「요청 목록에 없다」로 「이 부품은 안 묻는다」를 결론지은 것이 틀렸습니다. 합쳐진 요청은
   «누가 냈는지»를 안 적습니다.

## 원인 — `this.start` 를 «이 부품만» 안 받고 있었습니다
```
panel.js        startFor() 가 this.start 를 «읽기만» 합니다. 대입은 «안 합니다»
composition_panel · head_summary_panel   각자 `this.start = options.start` 를 «들고 있습니다»
expanded_layer_panel                     «없었습니다»  <- 여기
=> 좌석이 마킹을 선언해도 this.start 가 undefined -> subjectStart() 가 박힌 칩으로 떨어짐
   -> id 에 칩 이름이 그대로 나가 422
```
...
칩 씨앗 요청   «0»        (전: 1)
요청 수        «8»        (전: 9)
화면           「층을 찍으면 여기에 펼칩니다」   (전: 「서버가 거절했습니다 (HTTP 422)」)
404            trends «2» — 소유자 판정 대기 그대로
```
하니스: board 170/0 · composition 40/0 · walk 32/0

---

# 🔓 [클라 B] 라운드 Z-3 마무리 — **404 «4 → 2»**. 남은 둘은 소유자 판정 대기

## 게이트 ① — 요청과 상태
```
요청 «9»  (13 -> 9)
  200 «7»  declaration 1 · subgraph 4 · wafer_map_metadata 2
  404 «2»  trends 2   <- 좌석 4·6. 소유자 판정(모집단) 대기라 «안 건드렸습니다»
사라진 것  composition «2 -> 0» · lot_map «0» · siblings «0»
```

## 판정 ① — 네 번째 부재 문장
```
설비 또래   units === 0 이면 「이 씨앗에는 측정이 없습니다」
            (「축이 없다」와 «다른 문장»입니다 — 축은 있고 이 씨앗에 값이 없습니다)
이제 넷이 화면에서 «갈립니다»: 축이 없다 · 이 씨앗에 없다 · 걷기가 잘렸다 · 아직 안 불렀다
```

## 판정 ② — 기준 알약 «삭제»
```
뺀 것   bases 셋(bond_layer · dt_slot · wafer_grid) + basisChipId   좌석 8·12 «둘 다»
근거    선언된 엔티티 타입 아홉에 그 셋이 «없습니다». 매핑하면 방금 지운 가운데 표를 다시 만듭니다
효과    composition 호출자 «0». 알약 자리는 사라졌고, 필요하면 «엔티티 선언»으로 올라옵니다
```

## 그리고 죽은 기본값을 하나 더 찾았습니다
```
세 부품이 `this.collect = options.collect || «wafer_process»` 를 들고 있었습니다.
좌석이 이름을 지워도 이 기본값이 살아나 «죽은 라우트를 계속 불렀습니다» —
composition 404 가 그래서 한 자리 남아 있었습니다. 이제 좌석이 이름을 안 대면
«합성 루트가 묶어 준 걷기»를 씁니다.
```

## ⚠️ 남긴 것 하나 — 「펼친 층」이 «문장을 틀리게» 말합니다
```
화면    「서버가 거절했습니다 (HTTP 422)」
실측    이번 로드에서 그 좌석이 낸 «요청은 0» 입니다 (요청 9 = declaration 1 · subgraph 4 ·
        metadata 2 · trends 2. 그 좌석 몫이 없습니다)
=> 요청은 «안 나가는데» 옛 거절 상태가 화면에 남아 있습니다. 즉 배선이 아니라 «상태 표시»
   문제이고, 손을 더 대기 전에 적습니다 — 오늘 이 자리에서 두 번 고쳤고 두 번 다 다른
   경로였습니다(무조건 mount · 빈 주어 load). 세 번째는 재고 나서 갑니다.
```
하니스: board 170/0 · composition 40/0 · control_trend 36/0 · walk 32/0
...
```
🔴 **제 실수 하나 적습니다:** 편집 중 구분자를 `NUL` 로 넣어 api.js 에 **NUL 3바이트**가
