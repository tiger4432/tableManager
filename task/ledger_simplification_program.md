...
- **소유자 판정 (2026-08-18): 「레거시 없애」 — 전면 제거.** `--legacy` CLI 플래그,
  `ledger/config.py`, legacy 소스 문법 분기(`observation`/`transfer`/`declared`/`lineage`)와
  그 진입점을 모두 없앤다. 남는 실행 경로는 v2 하나다.
  - 진입점을 먼저 막고 사체는 그다음이다. 한 커밋에 다 넣지 말고, 무엇을 왜 지웠는지
    라운드 보고에 적는다.
  - `chains`의 `mode`가 함께 무의미해진다 — 단일 파일 전환(1라운드)과 같은 묶음으로 처리.

### 수집 오류 개수 — 8과 10은 서로 다른 것을 센 것이다

구현자가 10건이라 보고했고 재측정했다. **둘 다 맞다. 세는 대상이 다르다.**

- `server/`에서 `pytest tests` → **8건.** 전부 삭제된 번역기 import가 원인이다.
- 저장소 루트에서 `pytest` → server/tests 기준 **10건**(전체 36건). 늘어난 두 개는
  `test_map2_seam_contract` · `test_notation_fold_contract`인데, **두 파일만 직접 지목해
  돌리면 정상 수집된다.** 삭제 여파가 아니라 rootdir/conftest 수집 경로 문제다.

따라서 이번 정리로 고쳐야 할 대상은 **8건**이고, 나머지는 별개 항목이다. 「10건을 다
고쳤다」로 보고하면 두 종류의 문제를 한 덩어리로 만든다.
...
**소유자 지시 갱신 (2026-08-18): 「UI 개선안도 추가해서 착수」 — 3라운드는 감사를 기다리지
않고 착수한다.** 감사 결과는 도착하는 즉시 진행 중인 작업에 병합한다.

소유자가 **다른 레인에 explorer UI 감사를 지시**해 두었다(2026-08-18). 그 결과가 도착하면
지시서에 병합한다. 이미 이쪽에서 확인한 결함(생성 불가, 백지 500,
