...
> ## ⚠️ 2026-08-18 정정 — 아래 「벽 셋」은 **이 개발 박스**의 표를 보고 쓴 것이다
>
> **운영의 `void`는 다른 표다.** 소유자 확인(2026-08-18): 운영 키는
> **`x` · `y` · `waferid` · `tkouttime` · `void_index`**이고, **운영은 문제없이 돌고 있다.**
>
> 그래서 아래 ①②는 운영에 해당하지 않는다:
>
> - **시각이 있다.** `tkouttime`이 세계 시각이므로
>   `"occurred_at": { "column": "tkouttime", "timezone": "Asia/Seoul" }`로 정직하게 선언된다.
>   「시각 없음 선언」은 이 소스의 차단 사유가 아니다 — 지금 코드로 이미 된다.
> - **재스캔이 덮어쓰지 않는다.** 키에 `tkouttime`이 있어 같은 위치를 다시 검사해도 다른 행이다.
>   `void_obs`가 `run_uid`로 풀었던 문제를 이 표는 시각으로 푼다.
> - **`void_index`가 있다.** 같은 다이·같은 시각에 보이드가 여럿이어도 구분된다.
>
> 이 박스의 `void`는 컬럼 9개(`base_wafer_id`·`base_x`·`base_y`·`layer`·`inchip_x`·
> `inchip_y`·`radius_x`·`radius_y`·`classname`)에 키도 시각도 없는 **다른 물건**이다.
> 아래 서술은 그 표에 대한 것으로만 읽을 것.
>
> 🔴 **교훈:** 이 박스의 `table_config.json`은 운영의 스키마가 아니다. 소스 표의 «모양»을
> 근거로 판정하려면 소유자에게 운영의 선언을 받아야 한다. 2026-08-18 하루에 같은 실수를
> 세 번 했다 — DB 접속, 빠진 config, 그리고 이 표.
>
> ## (아래는 이 개발 박스 기준) 남은 벽 셋
>
> 오늘 코드 쪽은 두 가지가 열렸다: 폴더 이름의 `{workid}_`를 받게 됐고(`31cd5fc`),
> **외부 경로도 그 표의 워크스페이스 플러그인으로 들어가게 됐다**(`401dc72` — `parser`를
> 적지 않으면 `<표>/scripts/`에서 찾는다. 관리 `raws/`와 같은 입구다).
>
> 그런데 `void` 표로 켜려면 **세 가지가 아직 없다.** 순서대로 걸린다:
>
> **① `table_config.json`의 `void`에 유일키가 없다.** 지금 컬럼 9개만 있고
> `business_key`도 `composite_key_source`도 없다(스키마 시트에 키가 없었다). 감시 폴더의
> 파일은 다시 배달될 수 있어 키 없는 표는 워처가 **등록 자체를 거절**한다. 가장 먼저 걸린다.
> — 정할 것: 위치 6컬럼(`base_wafer_id`·`base_x`·`base_y`·`layer`·`inchip_x`·`inchip_y`)으로
> 충분한가, 아니면 스캔 식별자가 필요한가. `void`에는 스캔을 가리키는 값이 없어서, 같은
> 웨이퍼를 두 번 검사하면 **두 번째가 첫 번째를 덮어쓴다**. `void_obs`가 `run_uid`로 풀었던
> 문제다. 빈 표(0행)라 실측으로 확인할 수 없고 도메인 지식으로 선언해야 한다.
>
> **② `ingestion_workspace/void/scripts/`에 플러그인 «클래스»가 없다.** 그 폴더의 파일은
> `BasePipelineParser` 하위 클래스가 아니라 내장 모듈의 사본(`parse_voids_json` 함수)이다.
> `parser`를 비우고 선언하면 거절되며, 거절문이 그 폴더를 이름으로 댄다.
> — 필요한 것: `match()`(이 파일 받겠다)와 `parse()`를 가진 클래스 하나.
>
> **③ `options.filename` 기본값이 `voids.json`이다.** 파일 이름이 다르면 **거절도 없이
> 조용히 무시**된다. 다르면 반드시 `options`에 적을 것.
>
> 확인 명령(읽기만 함): `python scripts/check_external_sources.py` — 항목별로 받아들여졌는지,
> 거절이면 이유가 무엇인지 말한다. **0/0으로 뜨면 「거절」이 아니라 `enabled:false`이거나
> 선언 자체가 없는 것이다.**
...
```text
<외부 루트>/<WAFERID>/<WORKID>_<YYYYMMDD>_<HHMMSS>/voids.json
```

- 첫 폴더 = **웨이퍼 식별자**(`base_wafer_id`)
- 둘째 폴더 = **작업 식별자 + 시각**. 시각이 이름 **끝**에 붙어 있으면 되고, 그 앞의 것은
  무엇이든 `work_id` 컬럼으로 그대로 기록됩니다. 시각은 `observed_at`이 되고
  **`inspection_run`의 키 재료**입니다
...
| 폴더 이름 | work_id | 비고 |
|---|---|---|
| `JOB1234_20260818_143000` | `JOB1234` | 운영 형태 |
| `WORK_20260818_143000` | `WORK` | 종전 형태, 그대로 동작 |
| `20260818_143000` | 없음(`None`) | 종전 형태, 그대로 동작 |

> `work_id`는 **기록만 되고 키가 아닙니다.** 같은 웨이퍼·같은 층을 같은 초에 두 번 스캔하면
> 두 run이 한 키로 합쳐집니다. 그걸 막으려면 `work_id`를 `composite_key_source`에 넣어야
> 하는데, 그때는 `void_sat_format.compose_run_uid`도 **같은 커밋에서** 함께 바꿔야 합니다
> (한쪽만 바꾸면 두 경로가 다른 키를 만듭니다). 코드에 그 경고가 달려 있습니다.
