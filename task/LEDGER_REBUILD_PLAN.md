...
```
⚰️ ① 「버리면 화면 표본이 빈다」 — 🔴 «틀렸다». 총괄이 안 재고 쓴 문장이다 (2026-08-26 정정)
      구현자가 「재적재가 117,824 를 지우는데 게이트 ③이 그걸 못 견딘다」고 멈춰 세워서 쟀다:
      ```
      processed_with 중 목적어가 «recipe»   선언 3,022 · 스크립트 «0»
                                          (스크립트 25,132 는 전부 «값» 목적어라 recipe 에 «안 닿는다»)
      스크립트 117,824 중 «엔티티 목적어»    1,016  (0.9%)   <- walk 이 보는 것은 이것뿐
      그 1,016 조차  has_wafer 738 · slot_map 217 · derived_from 61
                    = «정확히 2단계가 lot_slot 결로 다시 만드는 그것»
      보드 씨앗 SYN-BW-101-16 이 잃는 것    11개 (processed_with 9 · register 2), «둘 다 값»
      ```
      -> 소유자 08-24 체인의 recipe 홉은 «100% 선언»이다. 게이트 ③은 안 깨진다.
      -> 판정이 필요 없어졌으므로 「3단계 판정이 4단계보다 뒤에 온다」는 순서 오류도 소멸한다.
      🔴 교훈: «술어 총수»를 세고 «홉»을 말했다. 그 둘은 다른 수다.
         (`processed_with` 28,154 중 walk 이 recipe 로 가는 데 쓰는 것은 3,022 뿐)

② dt_lot_slot 의 키   mat_id 를 「dt_lot|slot」 한 문자열로 붙이나, lot_slot@1 을 따로 쓰나
                    -> 붙이면 die@1 하나로 끝나고, 나누면 자리 어휘가 둘이 된다

③ 파티션 (2026-08-26 추가·해결)  `ledger_events` 는 파티션 «부모»이고 자식이 «여덟»이다.
   부모만 개명하면 새 표의 `ensure_partition` 이 `ledger_events_2026_09` 를 만들 때 «충돌»한다.
   -> 부모 «와» 자식 여덟을 같이 개명한다. (구현자 발견, 총괄 승인)
```
