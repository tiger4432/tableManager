...
## 🔴 [09-16 21:22 총괄 정정] §B 의 헤드라인은 «과장»입니다 — 그 넷은 «부르지 않습니다»

이 문서의 커밋 제목이 「five board seats **call** four routes that are gone」인데, **제가 열어서 재 보니 안 부릅니다.**

### 제가 잰 것
```
① 브라우저로 rnd-board.html 을 «열었습니다»
   나간 요청: /api/effort/config · /api/ledger/declaration · /api/ledger/subgraph  — «전부 200»
   은퇴 라우트 호출 «0». 보드는 오늘 walk(subgraph)로 삽니다
② 라우트는 확실히 죽었습니다        lot_map 404 · composition 404 · trends 404 · siblings 404
③ 🔴 그런데 «부르는 자리»가 없습니다
   git grep 'fetchLotMap(' 등 — 정의부 제외, client2/src 전체에서 «0»
   main.js 에는 이름이 «정확히 한 번»씩, 그것도 import 줄뿐입니다
   client2/tests 에도 «0»
```

### 그러므로 분류가 바뀝니다
```
❌ 「기능이 없는 UI」(=깨진 화면)   부르지 않으므로 사용자가 404 를 만날 «길이 없습니다»
✅ 「잔해」(상설 ④ 가 아니라 ③)     함수 넷 + 그 라우트 상수 넷이 «아무도 안 타는 갈래»입니다
                                그리고 import 가 그것들을 «번들에 살려 둡니다»
```
🔵 **이 구분이 중요한 이유:** 「깨졌다」로 읽으면 레인이 «화면을 고치러» 갑니다. 실제 일은 «지우는 것»이고, 그건 다른 라운드·다른 크기·다른 위험입니다.

### 남는 진짜 질문 (지우기 «전»에)
```
「이 넷이 왜 아직 있나」 — api.js 가 주석으로 «옛 계약»을 자세히 적어 두고 있습니다(:7 · :35 · :846).
   그 주석이 «기록»이면 함수는 지우고 주석은 docs/history 로 옮깁니다
   그 주석이 「곧 되살릴 것」이면 그 판정을 찾아야 합니다 — 오늘 밤 제가 상태 표시줄에서 그 함정을 밟았습니다
      (호출자 0 인 함수와 «되돌릴 수 있게 남겨 둔 것»은 코드 모양이 «같습니다»)
⛔ 그러므로 «지우기 전에» 그 판정부터 찾으십시오. 제가 판정합니다
```
⚠️ 이 문서의 나머지(§A UI 없는 기능 9 · §C 두 경로 6 · §D 문서/코드 불일치 6)는 제가 «아직 안 쟀습니다». 그 항목들은 그대로 두되, **§B 의 다섯 줄은 위 정정을 달고 읽으십시오.**
