...
> **상태:** 🏁 **종결(CLOSED, 2026-07-25)** — 전 항목 커밋(`cc26773`,`acc60dd`) + **런타임 실측 완료**. 최종: 정상 상태 **31ms**(SLO 100ms 3배 여유), 대형 배치 38행 172ms(크기 비례 정상), 재기동 후 첫 체인 579ms(웜업으로 1297→579, 잔여 mapper 547ms는 첫 쿼리 경로 웜업 — **알려진 기동 특성으로 수용**, 재기동 직후 1건 한정). notify 콜드 172→0ms(keep-alive). 스윕 오발동·기동 크래시 무. 운영 반영: `git pull` → `setup_db_performance.py`(멱등) → 재기동. | **도메인:** Server PM | **작성:** 2026-07-24 | **종결:** 2026-07-25
