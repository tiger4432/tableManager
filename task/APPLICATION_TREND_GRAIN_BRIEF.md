...
> 🔴 **정정 (소유자 판정 20:3x) — 이 절의 「`WaferLeg` 을 선언한다」는 «틀렸습니다».**
> 소유자: 「wafer leg 제거」 · 「bonding job 하위 leg 노드가 있고 거기에 개별 칩이 걸리는 게 낫지 않을까」
>
> 총괄이 `bonding_map` 을 재서 확인했습니다 — **leg 이 base 웨이퍼를 «가로지릅니다»**
> (leg 이름 셋이 base 여섯에 걸쳐 나타나고, base 여섯이 leg 을 «둘씩» 가집니다).
> 그러므로 `WaferLeg` 은 개체가 아니라 **«가지 × 운반체» 의 곱**입니다.
>
> 🔴 **재정정 (소유자 20:4x) — 위 문장을 제가 처음엔 「`leg` 과 `chip` 을 노드로」라고 썼는데
> 그것도 틀렸습니다.** 소유자: 「wafer 니 leg 니 이런 스키마에 종속되면 안 됨.
> 운영에서 언제 바꿀지 모름」. `WaferLeg` 을 지우고 `Leg` 를 박으면
> **같은 실수를 이름만 바꿔** 다시 하는 것입니다.
>
> 이름을 빼고 «역할»로 적으면 이렇습니다:
> ```
> 단위(unit)      한 행이 하나          — 이 박스에서는 pkg_id 1,181
> 운반체(carrier)  그 단위가 앉은 것      — 이 박스에서는 base 7
> 가지(arm)       그 단위가 배정된 조건   — 이 박스에서는 leg 3, 운반체를 «가로지름»
> ```
> **`wafer`·`leg`·`base` 는 이 셋의 «값»이지 종류가 아닙니다.**
> 다른 팹에서는 같은 자리에 다른 낱말이 앉고, 구조가 같으면 그대로 돌아야 합니다.
>
> 🔴 **최종 (소유자 20:5x) — 위 셋을 «선언 축»으로 만들지 마십시오. 이미 있는 것이 그 자리입니다.**
> 소유자: 「운반체도 결국 노드 타입으로 지정하고 그 노드 타입에서 걸어서 닿는 요소를
> trend, map 그리면 됨」 · **「내가 이래서 walk collect 패턴을 만든 거」**
> ```
> 운반체를 «노드 타입»으로 지목  -> 씨앗
> 거기서 걸어서 닿는 것          -> subgraph
> 그중 그릴 종류만 거둔다        -> collect
> ```
> 위의 「단위·운반체·가지」는 **데이터 모양을 이름 없이 «서술»한 것**이지
> 새로 선언할 축이 아닙니다. 그렇게 읽지 마십시오.
>
> ```
> 아래 §2 에서 그대로인 것   marking 필드가 은퇴 자리라는 것 · grain 이 «개체»여야 한다는 것
> 바뀌는 것                  그 개체가 WaferLeg 이 «아니라» leg · chip 입니다
> ```
>
> ⚠️ **제가 「WaferLeg 원자 42건」을 근거로 삼은 것이 잘못이었습니다** —
> 원자가 있다는 것은 «번역기가 그렇게 썼다»는 뜻이지 «모델이 맞다»는 뜻이 아닙니다.
> (「존재는 확정이 아니다」의 또 한 판입니다.)
>
> 📎 그리고 이 정정이 §4 를 «강화»합니다 — 같은 base 웨이퍼에 leg 이 둘이라
> 「웨이퍼 자신이 컨트롤인 A/B」가 «이제야 제대로 섭니다». 제가 지키자고 한 자체 대조가
> 없어지는 게 아니라 **제 모양으로 서는** 것입니다.
