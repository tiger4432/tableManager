...
으로 떠 있었습니다. 파고 보니 «남의 것»이 아니라 **제 것이 안 들어간 것**이었습니다.

## 무슨 일이 났나
```
31d34ea0 이 실제로 담은 것   rename «둘» 뿐  (git show --stat 로 확인: 2 files changed, 0 insertions)
담겼어야 했는데 빠진 것      ① tests/test_ledger_subgraph.py 의 죽은 import 삭제
                            ② ledger_subgraph.py:697 의 「is untouched」 주석 정정
원인   그 커밋의 `git add` 에 «이미 옮겨진 경로 둘»을 같이 적었습니다.
       git 은 없는 pathspec 하나만 있어도 «명령 전체»를 거절하는데,
       제가 `2>/dev/null` 로 stderr 를 죽이고 `;` 로 이어서 «실패가 안 보였습니다».
       그래서 rename 만(이미 `git mv` 로 스테이지돼 있던) 커밋됐습니다
```

## 🔴 그 결과 — 지시서가 «막으려던 바로 그 상태»가 HEAD 에 있었습니다
```
HEAD    tests/test_ledger_subgraph.py 가 `from ledger_api import mechanism_gate` 를 들고 있고
        mechanism_gate.py 는 `_archive/` 에 있습니다
실측    from ledger_api import mechanism_gate
        -> ImportError: cannot import name 'mechanism_gate' from 'ledger_api'
=> 그 파일은 31d34ea0 이후 «수집에서 에러»였습니다
```
🔴 **제 게이트가 초록이었던 이유가 이것입니다** — 4,257 을 «작업 트리»에서 쟀고, 그 트리엔
수정이 «이미» 들어 있었습니다. 커밋된 것과 잰 것이 달랐습니다.
총괄 검증(PID 51156)이 같은 트리에서 돌았다면 같은 이유로 초록이었을 것입니다 — **그 초록은
「4,257 이 맞다」가 아니라 「내가 안 잰 것을 쟀다」였습니다.**

## 지금 상태
```
8a79f27f  두 수정 착지
수집       «4,256»   (문서 라운드 직전 수와 같습니다 — traversable 라운드에서 하나 줄었던 그 값)
트리       server/ · docs/ 에 미커밋 변경 «0»
```

## 제가 바꾸는 습관 — 「경로를 적었으니 됐다」가 아니라 «status 로 확인»
```
① `git add` 에 «옮긴 경로»를 적지 않습니다 (rename 은 이미 스테이지돼 있습니다)
② `git add` 뒤에 «항상» `git status --porcelain <경로>` 를 찍어 «스테이지 표시»를 눈으로 봅니다
③ stderr 를 `/dev/null` 로 보내지 않습니다 — 오늘 그 한 글자가 라운드를 반만 착지시켰습니다
④ 게이트는 «커밋한 것»에 대고 재야 합니다. 작업 트리에서 잰 초록은 커밋의 초록이 아닙니다
```
📌 이건 「빌드했다고 로드된 건 아니다」와 같은 부류입니다 — 이번엔 «커밋했다고 담긴 건 아니다» 입니다.
...
## 🔴 먼저 정정 — 「형태는 `ledger_trace.py` 와 그 시험 안에 생존」의 «앞쪽»이 안 맞습니다
지시문 문장: 「`server/ledger_trace.py` «살아 있습니다» … 그러니 **「라우트가 은퇴했고 형태는
모듈 안에 남아 있다」**로 적어야 참입니다」. **모듈은 살아 있는 게 맞고, 형태는 그 안에 «없습니다».**
```
def trace                      server/ 전체에 «0»  (시험 헬퍼 `trace_on` 과 무관한 fixture 클래스 하나뿐)
hops·terminal_reason·generated_at 를 «만드는» 자리
                               server/ 비-시험 전체에 «0»
                               `ledger_trace.py:118` 의 «주석» 한 줄이 terminal_reason 을 언급하는 전부
그 세 키를 «이름 대어» 든 곳     server/tests/test_ledger_trace_pg.py:637 «하나»
                               그리고 그 시험은 은퇴한 `/api/ledger/trace` 를 부르고
                               `ASSY_PG_TEST_DATABASE_URL` 없이 «시끄럽게 건너뜁니다»
```
그래서 §4.6 도장에는 **「라우트 은퇴 · 이 형태를 오늘 만드는 코드는 «없다» · 이름을 아직 든 곳은
그 시험 한 줄이고 그것도 은퇴한 라우트를 부른다」**로 적었습니다. 지시문대로 적으면 문서가
「모듈 안에 형태가 있다」고 말하는데 찾으러 간 사람이 못 찾습니다 — 오늘 걷어낸 것과 같은 부류라
그 자리를 새 거짓으로 채우지 않았습니다. **다르게 판정하시면 그대로 고치겠습니다.**

## 게이트
```
① §4.6 이 둘 다 말하나       ✅ 「라우트는 은퇴했다」 + 「형태가 어디 남았는지」를 한 문단에 적었습니다
                              (위 정정 반영된 형태로)
② 삭제된 줄 «0»             ⚠️ «6» 입니다. 내역을 그대로 적습니다:
                              2 = §4.5·§4.6 «제목 줄» (같은 제목 + 도장으로 다시 적었습니다)
                              4 = 검사식 ①이 짚은 «그 문단» — 0 이 되려면 이 넷이 바뀌어야 했습니다
                              🔴 본문은 «한 줄도» 안 지웠습니다. 순증 +18 / −6
③ 검사식 ① 이 파일에서 «0»   ✅ 0 (전: 2)
④ 다른 절을 고치고 싶어지면 멈춤  ✅ 멈췄습니다 — 아래
```

## ④ 보고만 하고 «안 고친» 것
```
§4.4 홉 상태 기계 (resolved·candidate·unresolvable + `[no_slot_map]` 등 하위 태그)
     -> 이것도 `/trace` 의 계약입니다. `STATE_UNRESOLVABLE` 은 `ledger_trace.py` 에 «정의만» 있고
        내보내는 라우트가 없습니다. 지시가 §4.5·§4.6 이라 «손대지 않았습니다»
§4.7 /structure · §4.8 /kinds · §4.9 journey  -> 지시대로 손대지 않았습니다
```
그리고 §4.1~4.3 은 «삽니다» — `ledger_explorer.py:125-126` 이 `claim_class`·`hop_basis` 를
프로덕션에서 부릅니다. 도장 대상이 아닙니다.

## 참고 — `ledger_trace.py` 는 «살아 있는데 안이 갈렸습니다»
```
살아 있는 것   `_fetch` (walk 이 씁니다) · `claim_class` · `claim_rank_key` · `hop_basis` ·
              `resolve` · 세 `ClaimLookup` · `Claim` · `rollup_subject_types`
없는 것       `trace` · `lineage_predicates` · `traversal_predicate` · 모듈 `__getattr__`
남은 산문     60~80줄 주석이 그 없는 넷을 아직 «설명»합니다 (코드가 아니라 주석입니다 — 보고만 합니다)
```

---
# ⚖️ [A 구현자] **문서 정비 2차 — 셋 착지, 넷째는 «pin» 때문에 판정 요청** (2026-08-28 18:0x)

## 세 수 (총괄이 요구하신 부류 셋)
```
고친 현재형   «대량» — 아래 §다시 쓴 것
남긴 기록     «0»    — 이 문서엔 「그때 그랬다」로 말하는 문장이 없었습니다. 전부 현재형이었습니다
바꾼 예시     «0»    — 규칙만 참이고 예시가 죽은 자리는 없었습니다. 규칙 자체가 옛 그래프였습니다
```

## 배분 넷의 검사식 ①
```
docs/architecture/backend.md                 0
docs/architecture/data_model.md              0
docs/spec/LEDGER_EVIDENCE_SUBGRAPH_SPEC.md   0   ← 오늘 다시 씀
docs/spec/LEDGER_TECHNICAL_SPEC.md           2   ← 🔴 아래 판정 요청
```

## ✅ `LEDGER_EVIDENCE_SUBGRAPH_SPEC.md` — 다시 썼습니다 (커밋 `4debd8bf`)
```
바이트   32,648 -> 13,495  (−59%)
검사식 ①  v1 낱말 0
검사식 ②  이름 댄 파일·심볼 «열넷» 표본 -> 열넷 다 실재 (파일 6 · 함수 3 · 인덱스/제약 3 · SQL 1 · 라우트 1)
검사식 ③  줄었습니다
검사식 ④  두 기둥과 어긋나는 문장 «0»
```
🔴 **이 문서는 「왜 세 노드인가」가 제목이었습니다.** 종류가 하나가 된 날 §1~§4 의 «틀»이 통째로
거짓이 됐습니다 — 그래서 낱말을 고른 게 아니라 절을 다시 썼습니다.
```
지운 것   노드 종류 여섯(Entity·Source Event·Claim·Finding Collection/Point·Enrich Action·Quantity)
         id 접두 «일곱 중 여섯» · 요청 인자 여섯(include_values·observations·enrich_actions·
         shape·property_limit·collect) · Spotfire 3장표 절 · Viewer 절의 죽은 경로
         「기존 API」 표의 네 행 — 실측 `/api/ledger` 아래 «둘»뿐입니다 (`/subgraph`·`/declaration`)
새로 적은 것  ① 먼 쪽이 전진한다 (오늘 수리 + 그것을 드러낸 실측)
             ② `propagation` 은 오늘 «항상» not_requested — 깨울 인자가 없습니다
             ③ `/ledger-graph.html` 이 `client2/` 에 «없습니다»
바로잡은 수  Claim scan 상한이 `min(5000,…)` 로 적혀 있었는데 코드는 `MAX_CLAIM_SCAN=6000` 입니다
§검증기준     단언 스무 줄을 옮겨 적던 것을 «시험 파일 이름»으로 바꿨습니다 — 계약이 두 벌이면 어긋납니다
```

## 🛑 `LEDGER_TECHNICAL_SPEC.md` — 안 열었습니다. 문서 자신이 「에스컬레이션」이라 적고 있습니다
남은 v1 낱말 «둘»은 §4.5 안에 있고, 그 §4.5 는 «계보 walk» 의 알고리즘 절입니다.
그런데 바로 옆 §4.6 이 자기 안에 이렇게 적고 있습니다:
```
### 4.6 응답 형태 — 동결
🔴 총괄이 pin했고 클라 레인이 이것에 대고 지어졌다. 바꾸는 것은 편집이 아니라 에스컬레이션이다.
```
그래서 멈추고 올립니다. **실측부터 적습니다 — 이 셋이 판정의 재료입니다.**
```
라우트     `/api/ledger/trace` 는 «없습니다». 라우터 스스로 3줄째에 「retired」라고 적고 있고,
           `/api/ledger` 아래 살아 있는 것은 `/subgraph` 와 `/declaration` «둘»입니다
함수       `ledger_trace.py` 에 `def trace` 가 «없습니다». `lineage_predicates` ·
           `traversal_predicate` · `LINEAGE_PREDICATES` · 모듈 `__getattr__` 도 전부 없습니다
           (60~80줄 «주석»만 그 넷을 아직 설명하고 있습니다 — 코드가 아니라 산문입니다)
선언       `traversable` 은 v5 선언에 «0» 회 나옵니다 (sample 대조).
           즉 「인출 집합은 어휘의 traversable 선언에서 파생된다」는 문장이 오늘 «거짓»입니다
상태값     `STATE_UNRESOLVABLE` 등은 `ledger_trace.py` 에 «정의만» 남아 있고 내보내는 라우트가 없습니다
```
그리고 «살아 있는 것»도 갈라서 적습니다 — 여기가 중요합니다:
```
🟢 §4.1~4.3 (네 계급 · 계급 안 순서 · 전순서) 는 «삽니다»
   `ledger_explorer.py:125-126` 이 `claim_class` · `hop_basis` 를 프로덕션에서 부릅니다
🔴 §4.4 (홉 상태 기계) · §4.5 (보행 알고리즘) · §4.6 (응답 형태) 는 `/trace` 의 계약입니다
```

### 세 갈래 중 어느 것인지 주시면 바로 갑니다
```
ⓐ 지운다        정비 규칙 ①③ 그대로 — 「계보 walk 은 «없는 것»으로 쓴다」. §4.4~4.6 삭제,
                §4.1~4.3 은 그대로. 문서가 −N KB
ⓑ 은퇴 도장     §4.9 가 이미 쓰고 있는 그 형식(⚰️ 「은퇴했다. 아래는 «기록»이다」)을 §4.4~4.6 에
                답니다. 본문은 안 건드립니다 — 부류 ② 취급
ⓒ 그대로 둔다   pin 이 살아 있다고 보시면 §4.5 의 두 문장만 「거짓」인 채로 남습니다
```
👉 저는 **ⓑ** 를 권합니다. §4.6 은 클라 레인이 대고 지은 «동결 계약»이라 지우면 그 레인의 근거가
   사라지는데, 도장을 달면 「지금은 없다」와 「그때 이랬다」가 «둘 다» 참인 채로 남습니다.
   다만 pin 을 만드신 분이 총괄이라 제가 정할 자리가 아닙니다.
...
## 실측 — 검사는 루프 «머리»에 있습니다
```
454 |        for complete, next_after, _last_page in pages:
455 |            if max_batches is not None and result["batches"] >= max_batches:
456 |                break
457 |            frame = _v2_frame(complete)
```
`result["batches"]` 는 0 에서 시작합니다. `max_batches=0` 이면 «첫 반복의 첫 문장»에서 `0 >= 0` 이
참이라 **한 배치도 돌기 전에 break** 합니다. 0 은 이미 0 을 뜻합니다.

## 그래서 제 위반 보고가 «틀렸습니다»
```
제가 적은 것   「검사가 루프 «끝»에 있어 max_batches=0 이 한 배치를 돈다」  <- 거짓
제가 실제로 쓴 것  door3.py 의 gate ③ «한 번», max_batches=«1», written 3,998
                 나머지 프로브는 전부 max_batches=0 -> «0 배치»
그러면 +8,103 은?  총괄 replay 입니다 — 그때 09:46~09:57 로 돌고 있었고
                 제가 그 진행 중인 수를 «제 것으로» 읽었습니다
```
🔴 **저는 안 한 일을 자백했고, 총괄은 그 자백을 근거로 없는 버그의 수정을 지시하셨습니다.**
「내 것인가」를 «인자의 의미를 읽지 않고» 단정한 것이 첫 오류이고, 그 오류를 지시서가 물려받았습니다.
제가 수를 대며 보고한 것이 좋았다고 하셨는데, **그 수의 «귀속»이 틀렸습니다.**

## ⛔ 그래서 ①은 «하지 않았습니다»
지금 코드대로 0 은 0 입니다. 고치면 «있지도 않은 함정»에 대한 방어가 코드에 남습니다.
👉 다만 «음수»는 아직 이름 대어 거절하지 않습니다 — `max_batches=-1` 이면 `0 >= -1` 이라
   역시 0 배치입니다(무해). 이름 대어 거절할지는 지시 주시면 하겠습니다.

## ② 「안 쓰고 확인하는 법」 — `preview_first_batch` 가 «맞습니다». 재서 확인했습니다
```
server/ledger/backfill.py:534  preview_first_batch(engine, setup, source, fetch_rows=…)
   docstring    「WRITES NOTHING, MOVES NO CURSOR」
   본문 실측    write_batch «없음» · execute_selected_cursor_batch «없음» · DELETE «없음»
               · _advance_cursor «없음» · commit «없음»
   반환         (rows_read, preview_or_None) — 0 행이면 «답»이지 실패가 아님
   커서         읽지도 쓰지도 않고 «첫 페이지»부터 읽습니다
```
👉 지시서 한 줄: **「배치를 돌리지 않고 확인할 때는 `backfill.preview_first_batch` 를 쓴다 —
   쓰지도, 커서를 움직이지도 않는다. `run(max_batches=0)` 도 0 배치이지만 그건 «실행 경로»라
   승인·가드를 전부 지난다.」**

⛔ 총괄 replay 가 도는 동안 표에 아무것도 안 썼고, 이 확인은 «코드를 읽어서» 했습니다.
---

# ✅ [A 구현자] **찾았습니다 — «백필이 성공했습니다». 커서 표가 처음부터 적고 있었습니다** (2026-08-28 09:4x)

착수 전 물어보신 「무엇이 그 원자를 썼습니까」의 답입니다. 추측 아니고 **기록**입니다.

## 답 — `ledger_translator_cursor` 가 들고 있었습니다
```
source              started_at        updated_at        atoms_written  deduped
void_observation    08-27 22:52:05    08-27 23:07:57         103,841        0
die_inspection      08-26 12:31:20    08-26 12:45:54         117,662        0
transfer_event      08-26 12:31:03    08-26 12:31:03           1,405        0
… (나머지 여섯도 전부 08-26)
```
🔴 **`void_observation` 만 «08-27 22:52~23:07» 에 «혼자» 돌았습니다.** 15분, 103,841건, 중복 0.
즉 **정상 백필이 «성공»했습니다.** watcher 도 기동 훅도 아니고, 그냥 «돈 것»입니다.

## 교차 검증 — 원자 자신에게도 물었습니다
```
uuid7 은 앞 48비트가 «쓴 시각»입니다. 원자에서 직접 디코드:
   observed   103,841   08-27 22:52 ~ 23:07     <- 커서와 «분 단위로 일치»
   inspected  117,662   08-26 12:31 ~ 12:45
   transfer   401,206   08-26 12:31 ~ 17:15
```
서로 «다른 두 출처»(커서 표 · 원자 id)가 같은 창을 가리킵니다.

## 그래서 ③의 기전은 «우연이 아닙니다»
```
선언을 고침 -> 백필을 «돌림» -> 원장이 따라옴
```
다만 **누가 그 백필을 돌렸는지는 못 찾았습니다.** 커서는 「돌았다」를 적지 「누가」를 안 적습니다.
그 시각 저는 walk 축소 작업 중이었고 백필을 «돌린 적 없습니다». 총괄 아니면 다른 레인입니다.

## ⚠️ 그리고 제가 «틀린 결론으로 갈 뻔한» 자리 — 적어 둡니다
```
처음에 본 것   원자의 source_translator_ver  5042c15f…
              커서의 translator_ver          cb09db70…
              -> 「다르다! 재번역이 안 됐다」고 결론 낼 뻔했습니다
🔴 그런데      그 둘은 «설계상 다른 지문»입니다 — scripts/ledger_restamp_cursor.py 의
              머리말이 정확히 그걸 설명합니다(커서는 per-source fingerprint, 원자는 translator ver)
              «그 스크립트가 존재하는 이유»가 제 오독의 반증이었습니다
```
숫자가 문장을 죽이기 전에 그 숫자가 «무엇을 세는지» 봐야 한다는 그 자리였습니다.

📌 이제 지시하신 «승인의 문»으로 갑니다 — 재번역은 «이미 한 번 된 일»이고, 우리가 만드는 것은
   그것을 «반복 가능하고 기록되게» 만드는 문입니다.

---

# 🔒 [A 구현자] **잡습니다: `server/ledger/backfill.py`** (09:2x)

원장 리팩토링 첫 지시 — 재번역 승인의 «문». 다만 지시대로 «착수 전 측정»부터 합니다:
어젯밤 defect@1 원자를 «무엇이» 썼는지.
...
## 수
```
65 failed · 4,059 passed · 190 skipped · 1 xfailed   (971초)
```
⚠️ 이 수는 «오늘 밤 레인 셋이 동시에 지운» 트리에서 잰 것입니다. 그 자체로는 누구 것인지 안 말해 줍니다.

## 실패의 «모양» — 파일별
```
test_syn_complex_composite     12      test_ledger_registration_probe   9
test_source_ontology_profile    7      test_ontology_config_explorer    6
test_ledger_setup_boundary      4      test_ledger_l1_unit              3
… 나머지는 dt 매퍼 · dual_stack · entrypoint 격리 등 오늘 밤 walk 작업과 무관한 영역
```

## 🔴 제 파일은 «단독으로 전부 초록»입니다
```
test_ledger_subgraph + test_enrichment_actions   14 passed · 1 skipped
test_syn_complex_composite «단독»                26 passed        <- 스위트에선 12 실패
```
즉 composite 의 12는 «순서 의존»입니다 — 제가 몇 시간 전 「특정 못 했다」고 올린 그 조건이고,
지금은 12건으로 커져 있습니다.

## 폴루터 후보 «셋»을 배제했습니다 (모듈 캐시를 만지는 시험들)
```
test_finding_kinds  + composite -> 33 passed
test_ledger_trace   + composite -> 37 passed
test_mechanism_gate + composite -> 37 passed
```
🔴 제가 오늘 `entity_references` 에 캐시 둘(`_keys`·`_edges`)을 «추가»했기 때문에 **제 것부터**
   의심했는데, 위 셋으로는 재현이 안 됩니다. **제 캐시가 원인이라는 증거는 못 찾았고,
   아니라는 증거도 아닙니다** — 폴루터를 아직 특정 못 했습니다.

## 그래서 제가 «주장하지 않는» 것
```
⛔ 「65는 제 것이 아니다」    — 파일별 기준선이 없어 그렇게 말할 근거가 없습니다
⛔ 「스위트가 초록이다」      — 아닙니다
✅ 말할 수 있는 것          제 파일은 단독 초록 · composite 12는 순서 의존 ·
                          나머지는 오늘 밤 여러 레인이 동시에 만진 영역에 퍼져 있습니다
```
👉 이등분으로 폴루터를 잡으려면 실행 한 번이 «16분»입니다. 지시 주시면 잡겠습니다.
...
동률 규칙(첫 일치 -> 동률이면 None).
③(trends 죽은 CTE + 스펙 1124)은 B가 그 스펙을 놓았는지 확인한 «뒤» 따로 잡겠습니다.

📌 그리고 `ledger_trends.py` 는 «놓았습니다» — 한 줄도 안 고쳤습니다(제 22:5x 보고 참조).

---

# 🛑 [A 구현자] **놓습니다 — 고치지 않았습니다. 그 결함이 «지금 코드에 없습니다»** (2026-08-27 22:5x)

착수 직전 「지금 무엇이 나오나」를 먼저 봤고, **재현이 안 됩니다.** 그래서 멈추고 올립니다.

## 실측 — 지금 «세 경로» 전부 같은 답
```
① 현재 코드 (TestClient, 서버 없이)      SYN-CX-BW-001
     leg HBM-B_LOW-P   found «17» · denom 64
     leg LOGIC-A_REF   found «11» · denom 64
② 돌고 있는 서버 (PID 4284, 8080)        «완전히 같은 값» 17 / 11
③ 서버 기본 grain (grain 파라미터 «없이»)  «역시 같은 값» 17 / 11
전체 72점 중 found > 0 인 점 «67» (「전부 scanned_clean」이 아닙니다)
```

## 🔴 그리고 그 수는 «이미 맵과 같습니다» — 레그로 «쪼개져» 있을 뿐입니다
```
트렌드   17 + 11 = «28»       맵 found    «28»   ✅
트렌드   64 + 64 = «128»      맵 scanned  «128»  ✅
```
같은 curl 로 맵도 방금 확인했습니다(found 28 · scanned 128). **분자도 분모도 이미 맞습니다.**
트렌드는 «레그당» 한 점이고 맵은 «웨이퍼 합계»라, 둘을 나란히 놓으면 28 對 17 로 보입니다 —
그게 「0」으로 보였던 것과는 다른 이야기입니다.

## 그래서 왜 총괄은 0 을 보셨나 — 프로세스가 «다릅니다»
```
총괄 측정   「서버 PID 58296」
지금 8080   PID «4284» · 시작 시각 «21:38»
```
🔴 그 PID 는 지금 없습니다. 이 저장소에 이미 적혀 있는 부류입니다 —
**「빌드했다고 로드된 건 아니다 · 서버는 «프로세스 시작 시각»」.**
제가 「그때 코드가 이랬다」를 증명할 수는 없습니다. 증명할 수 있는 것은 **지금 셋 다 28 이라는 것**뿐입니다.

## ⛔ 지시받은 수정을 «하지 않았습니다» — 하면 «맞는 것을 틀리게» 만듭니다
```
지시    분자를 «구별되는 다이 수»로 -> 이미 그렇습니다 (count(DISTINCT die), die = (mat_id,x,y))
       분모를 «구별되는 die 참조»로 -> 지금 64/leg = 128/wafer 로 이미 맵과 같습니다
지금 손대면   28 을 다른 수로 바꾸게 됩니다. 오늘 밤 세 번 막은 그 모양입니다
```

👉 **총괄께: 재측정 부탁드립니다.** 같은 curl 을 지금 서버(4284)에 태우시면 17/11 이 나옵니다.
   그래도 0 이 나오면 그때는 «요청이 다른 것»이니 그 요청 원문을 주십시오 — 바로 잡겠습니다.
   **파일은 놓았습니다. `ledger_trends.py` 한 줄도 안 고쳤습니다.**

---

# 🔒 [A 구현자] **잡습니다: `server/ledger_api/ledger_trends.py`** (22:5x)

트렌드 0 라운드 — 분자·분모 «둘 다» 구별되는 다이 수로. 총괄 인수 블록(cd7d7988) 그대로.
목표는 «맵과 같은 수» 28/128 입니다.

⛔ 안 건드림  보드의 grain 선언(main.js:246·306) · mat_type 필터(사유만 주석으로)

---

# ✅ [A 구현자] **멈춤 조건 «해제» — 28 도 128 도 원장에서 «정확히» 나옵니다** (2026-08-27 22:4x)

## 실측 — 맵의 두 수를 원장만으로 재현
```
분자 후보   observed 원자 (SYN-CX-BW-001)                 121  <- 원자 «수»
           구별되는 (mat_id, x, y)                        «28»  ✅ 맵의 발견 28 과 «일치»
분모 후보   inspected 원자 (subject_keys.wafer = 그 웨이퍼) 256
           object_payload 의 구별되는 die 참조             «128» ✅ 맵의 검사 128 과 «일치»
```
🔴 **28 / 128 = 21.9%** — 맵과 «같은 수»입니다. 판정하신 판별식을 원장이 만족합니다.

📌 곁가지: inspected 원자가 256 = 128 × 2 입니다. 다이당 «둘»이라 원자 수를 그대로 분모로 쓰면
   256 이 되고, 그것도 「0 이 아닌 아무 수」의 한 종류입니다. **분모도 «구별되는 다이»여야 합니다.**
   분자(121→28)와 «같은 실수»가 분모 쪽에도 있습니다.

## 그래서 이 라운드의 모양이 확정됩니다
```
분자   observed 의 «구별되는 (mat_id,x,y)»           = 28
분모   inspected 목적어의 «구별되는 die 참조»          = 128
둘 다  「원자 수」가 아니라 «구별되는 다이 수»입니다 — 이게 이번 결함의 한 문장입니다
```
🟢 「원장으로 답한다」 기둥에 안 걸립니다 — 분모가 «원장 안»에 있습니다(`inspected@1`).

## ⛔ 아직 «구현하지 않았습니다» — 제 컨텍스트가 다 됐습니다
정직하게 적습니다: 여기서 시작하면 반쯤 하다 끊깁니다. **측정까지만 올리고 멈춥니다.**
다음 세션이 위 네 줄로 바로 착수할 수 있습니다. 잡고 있는 파일 «없습니다».

⚠️ 그리고 지시하신 「mat_type 필터는 만들지 말되 이유를 코드 옆에 적어라」도 «아직 안 적었습니다».
   구현할 때 같이 들어가야 합니다 — 오늘 103,841/103,841 이 Wafer 라 맞는 것이지
   선언이 보장하는 것이 아니라는 그 문장입니다.
...
## ① 대조군 — 계측기는 «삽니다»
```
observed 원자 전체        103,841   전부 finding_kind = void
                                    -> 「분자가 0 아닌 것을 낼 수 있나」에 «예»
```

## ② 이 웨이퍼의 관측은 «있습니다» — 121건
```
subject_keys->>'mat_id' = 'SYN-CX-BW-001'   «정확히» 121건
접두 일치도 «같은 121» -> 접미사(-01 류) 불일치 가설은 «죽었습니다»
```
🔴 즉 「원장에 없다」가 아닙니다. **있는데 안 세어집니다.**

## ③ 원인 — 총괄 가설 그대로입니다. 「웨이퍼」라는 키가 «그 원자에 없습니다»
```
observed 원자의 subject_keys 에 'wafer' 키가 있는 것      «0 / 103,841»
observed 원자의 subject_keys 에 'mat_id' 키가 있는 것      «103,841»
   실제 모양   {x, y, mat_id, mat_type}    mat_id 가 웨이퍼 id 를 «들고» 있습니다

대조군 — 'wafer' 키를 «가진» 원자는 있습니다               120,684
   inspected 117,662 · processed_with 3,022
```
🔴 **선언대로입니다** — `observed@1` 의 주어는 `die@1` 이고 die 의 키는 `{mat_id, x, y, mat_type}`
입니다. 웨이퍼는 die 의 «키 하나(mat_id)»이지 별도 필드가 아닙니다.
그런데 트렌드의 grain 은 `identity_fields: ["wafer"]` 이고, `wafer` 라는 이름의 키는
**observed 원자에 «단 하나도» 없습니다.** 그래서 분자가 «구조적으로» 0 입니다.

## 왜 「비었다」가 아니라 「깨끗하다」로 나오나
```
분자   wafer 키로 묶으니 «0 행»  -> found_chip_count 0
분모   다른 경로(scanned)에서 옴 -> 64 가 그대로 남음
결과   0/64 = 「scanned_clean」 «긍정 주장»
```
🔴 분모 어긋남(맵 128 vs 트렌드 64)도 같은 뿌리로 보입니다 — 분자와 분모가 «서로 다른 주어»를
세고 있습니다(die 對 wafer). 다만 분모 쪽은 아직 안 쟀습니다.

## ⛔ 고치지 않았습니다
고치는 방향은 «둘 중 하나»인데 그건 판정입니다:
```
ⓐ 트렌드가 die 원자를 mat_id 로 묶는다 (읽는 쪽이 선언을 따라간다)
ⓑ 선언이 die 에 wafer 참조를 싣는다   (in_container 가 이미 die->wafer 입니다)
```
📎 ⓑ 는 오늘 넓힌 `follow` 의 참조 엣지와 «같은 자리»입니다.
...
## 실측 (라이브 원장 645,203 원자)
```
predicate = 'transferred' 인 원자                                    «0»
object_payload 에 'to' 키가 있는 원자                                 «0»
   ...그중 to.type = 'bond_layer'      (selection:272 · trends:486)  «0»
object_payload 에 'component' 키가 있는 원자  (selection:286 · trends:500)  «0»
to.keys.base_wafer_id 가 있는 원자           (trends:486 의 조인 키)   «0»

대조군  전체 원자 «645,203» · 서로 다른 술어 «8»   <- 계측기가 산다는 증거
```
🔴 **0 이 다섯 줄인데 그게 「질의가 고장」이 아니라는 근거가 대조군입니다.** 같은 연결·같은 표에서
645,203 과 8 이 나옵니다.

## 실재하는 술어 여덟 — `transferred` 는 그중에 «없습니다»
```
...
```
📌 **`transfer`(있음, 40만)와 `transferred`(없음, 0)** — 한 글자 차이입니다. 다만 이게 오타인지
«다른 술어»인지는 제가 판정할 일이 아닙니다. 네 갈래가 요구하는 «페이로드 모양»도 같이 0 이기
때문입니다(`to.type='bond_layer'` · `component.component_id` · `to.keys.base_wafer_id`).
즉 이름만 바꿔서는 살아나지 않습니다 — **그 갈래는 «없는 문법»을 통째로 기다리고 있습니다.**

## 그래서 이 넷은 지금 무엇을 하나
```
네 갈래 모두   조인/필터가 «0 행»을 내므로 결과에 아무 기여도 안 합니다
증상          오류가 아니라 «조용한 빈 값». 사용자에겐 「데이터가 없다」로 보입니다
```
⛔ 지시대로 «수리하지 않았습니다». 총괄이 「⑦(composition 복구)의 같은 뿌리」로 묶어 두신 것과
   같은 자리로 보이고, 그 판정 때 «한 번에» 처리하는 것이 맞다고 봅니다.
...
```
ledger_structure.py:634   editable 갈래는 `sig.get("origin") == "config"` 일 때만 탑니다
총괄 실측                  PREDICATES 13 의 origin 분포 -> {«없음»: 13}
                         즉 origin == "config" 인 술어 «0»
이유                     origin 이 "config" 가 되려면 v1 확장 파일에서 온 술어여야 하는데
                         이 박스엔 그 파일이 «없습니다»
```
🔴 **그 갈래에 무엇을 쓰든 오늘은 화면에 «안 나옵니다».**
그러니 표지를 고친 뒤 「화면이 그대로다」가 나와도 그건 «통과»가 아니라 «원래 안 나오던 것»입니다.

## 그래서 이 라운드를 집는 사람에게 — 게이트를 이렇게 두십시오
```
① origin=="config" 인 술어를 «만들어» 그 갈래를 «태우고» route 가 비었는지 확인 -> 되돌린다
   ⚠️ 라이브 config/ledger_vocabulary.json 을 «만들지 마십시오» — 만드는 순간 어휘가 바뀝니다
      임시 경로 + 로더 인자, 또는 시험 안에서
② 그게 어려우면 «못 쟀다»고 적으십시오. 「안 나온다」를 「됐다」로 적는 것보다 낫습니다
```

## 적어만 두는 것 — 지금 답하지 않습니다 (총괄 지시대로)
```
v5 로 선언된 술어는 운영자가 고칠 수 있습니다(drafts -> activate).
그러면 editable 갈래는 «죽는» 게 아니라 «주어가 바뀌는» 것일 수 있습니다
   origin=="config" 의 뜻이  「v1 확장에서 왔다」 -> 「ledger_config.json 에서 왔다」
🛑 v1 을 걷어낸 «뒤»에 화면이 무엇을 말해야 하는가의 문제입니다. 지금 정하면 아직 없는
   상태에 대해 정하는 것입니다
```

📌 이 세션은 컨텍스트 한계로 여기까지입니다. 표지 수정·라우트 은퇴는 «시작하지 않았습니다».
   4단계 순서와 심볼 게이트, 커밋 분리 규칙은 바로 앞 보고에 있습니다.

---

# 📌 **①의 판단은 «이미 잰 것»이 답합니다 — route 를 비우고 사유를 답니다** (구현자 11:3x)

「지어내지 마십시오 — v5 의 편집 단위가 「술어 하나」가 아닐 수 있다」고 하신 그 자리,
**제 직전 측정이 그 답을 들고 있습니다.**

```
v5 쓰기 경로   PUT /drafts/{id}   payload.«raw» = «문서 전체»   (router:187-193)
             -> 편집 단위는 «선언 문서»이지 «술어 하나»가 아닙니다
             -> 「이 술어를 이 라우트로 고쳐라」는 v5 에서 «참이 아닙니다»
```
그러므로 표지는 **route 를 v5 로 바꿔 다는 것이 아니라 «비우고 사유를 적는» 쪽**입니다 —
「닿을 수 없는 곳을 가리키는 표지는 없는 것보다 나쁘다」는 그 규율 그대로입니다.
```
제안하는 모양 (구현은 다음 컨텍스트)
   editable: false
   reason:   「술어는 선언 문서 단위로 고칩니다 — 온톨로지 편집기의 초안에서」
   route:    «없음»   (있으면 누군가 그 주소를 부릅니다)
```
⚠️ 다만 `_EDIT_TARGETS` 에는 «translator» 항목도 있습니다. 그것이 v1 라우트를 쓰는지는
   «안 셌습니다» — 표지를 고칠 때 그 행부터 세야 합니다. 술어 행만 보고 표 전체를 고치면
   오늘 여러 번 나온 그 부류(하나를 보고 부류를 판정)를 반복합니다.

## 🔴 이 세션은 여기서 끝냅니다 — 컨텍스트 한계
①의 코드 변경(표지 수정)과 ②(라우트 둘 은퇴)는 «시작하지 않습니다». 반쯤 하면 화면이
없는 문을 가리킨 채로 남습니다. 다음 컨텍스트가 집을 것을 정리해 둡니다:
```
1단계   ledger_structure.py:632 · :838 · :854 의 표지  -> editable:false + 사유, route 제거
        ⚠️ _EDIT_TARGETS 의 «translator» 행을 «먼저» 세십시오
        그다음 /admin/ledger/save · /vocabulary/retire 은퇴 + check_predicate_declaration
2단계   PROJECTION_ONLY_WORDS · SIGNATURE_FIELDS · LAYER_* · EDITABLE_LAYER 사망
3단계   PREDICATES · ENTITY_TYPES 독자들을 선언으로 (첫 자리 ledger_catalog.entity_types)
4단계   vocabulary.py 삭제 — 게이트: import grep 0 · «서버가 뜨는가» · 보드 16/14/오류0
        · POST /admin/ledger/dry-run 을 «태워» 볼 것
심볼 게이트  ① 저장소 전체 grep -> 코드 0 (편집한 파일 «안»까지 · 0 아닌 것도 낼 수 있는
           grep 인지 먼저 확인)   ② 그 다음 «불러» 본다
커밋       트리 업로드 / 이사 / 표지 수정을 «각각 따로»
```

---

# ✅ **1단계 선행 계수 — v5 authoring 은 술어 추가를 «오늘 할 수 있습니다». 멈춤 조건 «해당 없음»** (구현자 11:1x)

지시대로 «세는 것»부터 했습니다. 결론: **운영자 기능에 구멍이 없습니다.** 배선만 옮기면 됩니다.

## 쓰기 경로가 이미 있습니다
```
POST /drafts/new                 -> 초안 생성
PUT  /drafts/{id}   payload.raw  -> 🔴 «문서 전체»를 받습니다  (router:187-193)
POST /drafts/{id}/review · /revise · /activate
전부 require_admin_token_strict 뒤 — v1 의 save 와 «같은 등급»의 통로입니다
```
`raw` 가 문서 전체라 `vocabulary` 항목 추가가 그 통로로 «표현 가능»합니다.

## 그리고 검증기가 «받습니다» — 사본에 실제로 먹여 봤습니다
```
새 술어 scrapped@1 을 추가
   {status, subjects, object:{kind:"none"}}                       -> 거절
      bundle.vocabulary.scrapped@1.object.qualifiers: field is required
   {…, object:{kind:"none", qualifiers:{required:[],optional:[]}}} -> «ACCEPTED» ✅
```
🔴 **거절문이 빠진 칸의 «이름»을 말해 줍니다.** 그래서 이건 「못 한다」가 아니라
「모양을 검증기에서 뽑아 쓰면 된다」입니다 — 이 저장소 규율(「형식은 검증기에서 뽑는다」) 그대로고,
운영자에게도 같은 방식으로 보입니다.

## 그래서 1단계는 «구멍 메우기»가 아니라 «배선 이전»입니다
```
남는 일   어드민 화면의 「술어 추가」가 v1 save 대신 v5 drafts 경로를 부르게 하는 것
안 남는 일 v5 쪽에 없는 기능을 «만드는» 것 — 없는 게 없습니다
🛑 멈춤 조건(「v5 가 술어 추가를 못 하면 멈춰라」) -> «해당 없음»
```
⚠️ 제가 «안 잰» 것: 그 화면이 실제로 어느 버튼에서 v1 save 를 부르는지(어드민 UI 쪽 배선)와
   `POST /admin/ledger/save` 의 다른 소비자. 배선 이전을 시작할 때 그 둘을 먼저 세겠습니다.

## 🔴 이 세션은 여기서 넘깁니다
컨텍스트가 한계입니다. 1단계의 «판정 재료»가 위에 다 있고, 다음 컨텍스트가 배선 이전부터
집으면 됩니다. 남은 순서(2 심볼 사망 · 3 PREDICATES/ENTITY_TYPES 를 선언으로 · 4 파일 삭제)와
심볼마다의 게이트 두 줄은 앞 보고들에 적혀 있습니다.

---

# 📌 **충돌 판정 ⓐ 접수 — 「좁은 구멍」을 지시대로 «한 줄»로 남깁니다** (구현자 10:5x)

```
🔴 raws/ 에 «아직 처리 안 된» 파일이 앉아 있는데 같은 경로가 덮이면,
   앞 파일의 내용은 «한 번도 안 읽히고» 사라집니다.
   -> 업로드가 «만든» 구멍이 아닙니다. 기계에서 같은 경로에 다시 떨어뜨려도 OS 가 똑같이
      덮으므로, «두 문이 공유하는 성질»입니다. 이번 라운드에서 고치지 않습니다.
```
그리고 사용자에게 보이는 뜻은 총괄이 소유자께 전하시는 그 문장 그대로입니다 —
**「같은 (웨이퍼, 작업시각) 폴더를 다시 올리면 «수정»입니다」.**

## 제 쪽 남은 것 — 게이트 ④의 «서버 절반»은 이미 서 있습니다
```
서버   relative_path 를 받고, 있으면 트리로 · 없으면 오늘 그대로   ✅ (f08cced0)
클라   브라우저 webkitRelativePath · 랩퍼 relpath 를 «싣는» 쪽 -> 클라 레인
       -> 붙으면 같은 트리가 두 문에서 «같은 경로»로 앉는지 제가 재겠습니다
```

## 🔴 이 세션은 여기까지입니다 — 다음 컨텍스트에 남기는 것
```
바로 집을 수 있는 것   DECL_REFUSALS -> ledger_admin  (경계 밖에 온전히 선 «하나». 집도 판정됨)
판정 대기            v1 확장 파일의 생사 (그 하나에 심볼 여섯이 걸려 있습니다)
                    게이트 ④ (클라가 경로를 싣는 뒤)
규율                심볼마다 ① 저장소 전체 grep -> 코드 0 (편집한 파일 «안»까지,
                    그리고 «0 아닌 것도 낼 수 있는 grep 인지» 먼저 확인)  ② 그 다음 «불러» 본다
                    커밋은 트리 업로드와 이사를 «따로»
```

---

# ✅ **게이트 ① 열렸습니다 — 트리면 잎 이름 «그대로», 아니면 오늘 그대로** (구현자 10:4x)

## 실측 — 실제 라우트 함수를 태워서
```
트리 (게이트 ①)     SYN-BW-1/WORK_20260101_000000/voids.json   <- 파서가 요구하는 «정확한» 모양
단일 파일 (게이트 ③) user(kim)_voids_f43f60ec.json              <- 오늘과 «동일»
탈출 시도           user(kim)_x_251d5552.csv (raws/ 바로 아래)  <- 가드 그대로
```
🔴 잎 이름을 지킨 것은 «가드를 푼 것»이 아니라 **같은 가드가 다른 상황을 읽은 것**입니다.
uuid 접미는 «평평한 디렉터리»를 지키는 장치이고, 트리에서는 «트리 자체»가 유일성입니다 —
`WAFERID/WORK_DATETIME/` 이 바로 둘을 가르는 것이고, 생산자가 폴더를 쓰는 이유가 그것입니다.
그리고 거기서는 접미가 불필요할 뿐 아니라 «치명적»입니다(파서가 이름을 정확히 봅니다).

## 🔴 남은 것 하나 — 같은 트리를 «두 번» 올리면 «덮어씁니다»
지금 동작이 그렇습니다. 이것은 제가 정하지 않았습니다:
```
ⓐ 덮는다      같은 (웨이퍼, 시각)이면 «같은 관측»이라는 전제. 재업로드가 «수정»이 됩니다
ⓑ 거절한다    이미 있으면 409. 「덮었는지 몰랐다」가 안 생깁니다
ⓒ 옆에 둔다   WORK_DATETIME 뒤에 무언가 붙이는 것 -> 🔴 실측으로 «기각»
              (정규식이 YYYYMMDD_HHMMSS 로 끝나야 합니다)
```
📌 이 박스에 void 데이터가 «없어» 실제 재업로드 빈도를 못 잽니다. 운영에서 같은 트리가
   다시 오는 일이 있는지가 판정의 재료입니다.

## 이사 쪽 — 지시 접수
`LAYER_*` 는 v5 검증기가 그 칸을 «적극 거절»하므로 옮기지 않습니다(총괄 실측 확인).
경계 밖에 온전히 선 것은 `DECL_REFUSALS` «하나»뿐이고, 그것도 `ledger_admin` 이 자기
`REFUSAL_CODES` 와 합집합으로 쓰니 그 파일이 집이라는 판정 그대로입니다.
⚠️ 이 세션의 컨텍스트가 한계라 그 하나는 «시작하지 않았습니다» — 반쯤 옮기는 것보다 낫습니다.
   다음 컨텍스트가 집을 수 있게 계수·게이트·커밋 분리 규칙을 앞 보고에 적어 뒀습니다.
...
```
relative_path 를 «받습니다» (없으면 오늘 동작 그대로)
성분마다 소독 · 빈 성분과 점 하나·둘은 «버림» · 깊이 상한 «10»
결과 검증을 「직접 자식」 -> «commonpath 로 아래인가»  (뜻은 그대로, 철자만 넓힘)
저장 직전 makedirs
```

## 게이트 ② — **전부 raws/ «안»입니다** (실제 라우트 함수를 태워서)
```
평범한 트리         IN  depth 2
상대 경로 없음       IN  depth 0   <- 오늘 동작 «그대로» (게이트 ③)
상위참조 두 칸       IN  depth 0
역슬래시 상위참조     IN  depth 0
a 뒤 상위참조 둘     IN  depth 1
빈 성분 a//b///c    IN  depth 3
아주 깊은 트리(40)   IN  depth «10»   <- 상한이 먹습니다
점만 있는 성분들     IN  depth 0
```
🛑 멈춤 조건(하나라도 밖)에 «해당 없음».

## 🔴 게이트 ① — 막힙니다. 막는 것은 «트리»가 아니라 «파일 이름»입니다
```python
voids_json_format.py:138
   if filename.lower() != expected_filename.lower():
       raise ValueError("source filename must be 'voids.json', got ...")
```
```
업로드가 만드는 이름   user(kim)_voids_<uuid8>.json
파서가 요구하는 이름   voids.json   «정확히»
-> 성분 수는 3으로 맞는데 «이름이 달라» 여전히 거절됩니다
```
🔴 즉 지시서 가드 ④(「uuid 접미 유지」)와 파서 규칙이 **정면으로 충돌합니다.**
접미를 유지하면 트리를 보존해도 «게이트 ①이 참이 될 수 없습니다».

## 판정 청합니다 — 제가 안 골랐습니다
```
ⓐ 상대 경로가 «있을 때만» 접미를 안 붙인다
   근거   그때는 «트리 자체»가 유일성입니다 — 폴더가 있는 이유가 그것입니다.
          접미는 «평평한 디렉터리»를 지키던 장치입니다
   남는 것 같은 (웨이퍼, 시각) 트리를 두 번 올릴 때 «덮나 거절하나»
ⓑ 접미 유지 + 파서 규칙을 넓힘
   ⛔ 파서는 제 소관 밖이고, 그 규칙이 「이 파일이 그 파일인가」를 지킵니다
ⓒ 접미를 «디렉터리»에 붙임
   -> 실측으로 «기각»: WORK_DATETIME 이 YYYYMMDD_HHMMSS 로 끝나야 해서 정규식이 깨집니다
```
제 관측으로는 **ⓐ**뿐이고, 그 안에서 「두 번째 업로드」를 어떻게 할지가 남습니다.

⚠️ 클라 쪽(브라우저 webkitRelativePath · 랩퍼 relpath 를 «보내는» 것)은 이 커밋에 «없습니다» —
서버가 받을 준비만 됐고, 안 보내면 오늘과 동일합니다. 게이트 ④는 그것이 붙은 뒤에 잽니다.
⚠️ 이 박스에 void 데이터가 없어 ①은 «만든 트리»로만 쟀습니다. 운영과 섞지 않았습니다.
...
## 한 것
```
setup_bundle.py   _OBJECT_KINDS -> «OBJECT_KINDS» (밑줄 뗌 — 밖에서 셋이 읽습니다)
                  왜 이 값이 참인지 주석으로 (선언이 none 을 씁니다 · register@1 원자 396)
vocabulary.py     짧은 사본 «제거» + 은퇴 주석 (어디로 갔는지)
config.py:927     setup_bundle 쪽을 봅니다
ledger_admin.py   `_grammar_object_kinds()` 한 함수로 검증기에서 읽습니다
config_authoring · 시험 둘   이름만 따라갑니다
```

## 게이트 넷
```
③ 심볼 «하나»      정의부 grep -> setup_bundle.py:130 «1개» ✅
④ 라이브 재검증    «()» ✅ (register@1 이 살아 있습니다)
② 어드민 카탈로그   ['entity_ref','event_ref',«'none'»,'value'] ✅
                  <- 이 이동의 «효과»입니다. 전엔 선언이 쓰는 값을 화면에서 «고를 수 없었습니다»
① 거절/허용       라이브 선언이 그대로 통과하고, `tally` 는 거절되며 거절문이 이제 네 값을 «부릅니다»
```
⚠️ ①을 「네 값 × 두 경로」로 더 잘게 재려던 프로브는 «제가 못 만들었습니다» — 술어의 object 를
   손으로 갈아끼우면 kind 말고 «주변 칸»이 먼저 거절돼서, 그 수는 kind 에 대해 아무 말도 안 합니다.
   그래서 위 넷만 적습니다. 못 잰 것을 잰 것처럼 적지 않겠습니다.

## 🔴 그리고 «제가 제 작업을 지웠습니다» — 적어 둡니다
「이 여섯 실패가 제 변경 전에도 나나」를 확인하려고 `git checkout HEAD -- <7파일>` 을 했는데,
**그 앞의 백업 `cp` 가 조용히 실패해 있었습니다** (Windows 경로를 큰따옴표로 감싸서
`$(basename …)` 이 «문자 그대로» 들어갔습니다). 되돌리기는 성공했고 제 편집이 통째로 날아갔습니다.
```
얻은 것   여섯 실패는 HEAD 에서도 «같은 여섯» — 제 변경과 무관함이 확인됨
잃은 것   그 편집 전부. 기계적이라 다시 했습니다
규칙      되돌리기 «전»에 백업이 «실재하는지» 확인한다. cp 의 성공은 «파일이 있는 것»으로 센다
```
📌 여섯 실패는 `test_ontology_config_explorer` 계열이고 **HEAD 에서도 여섯**입니다.

## 📌 총괄께 — 분류 나머지는 «이미 보냈습니다»
`DECL_REFUSALS`(① 문법 -> setup_bundle) · `ISSUED_TYPES`(③ 선언, 소비자 1곳 먼저 확인)는
직전 보고(`6cb1ebc5`)에 적혀 있습니다. 못 보셨으면 그 커밋을 봐 주십시오.
`EDITABLE_LAYER` 가 별칭이면 «옮기지 말고 지우라»는 지적 받았습니다 — 다음 커밋에 그렇게 합니다.
...
`server/scripts/shrink_uq_ledger_atom.py` (dry-run 기본 · 게이트 후 커밋 · 머리에 롤백 SQL).

## 게이트 — 넷 다 통과
```
① 크기      «파티션 합»으로  1,123.6 MB -> «159.7 MB»  (14%)
② 유일성    옛 축의 중복 그룹 «0 -> 0» · 행 «645,203 -> 645,203»
③ 계획      walk (subject_type, subject_keys)  -> Index Only Scan  «그대로»
            새 키의 유일성 조회                 -> Index Scan
④ ON CONFLICT  아래에 «재서» 적습니다
```

## 🔴 ④ — 「조용한 병합이 구조적으로 불가」는 **제가 틀렸습니다**. 재니 반쪽만 맞습니다
```
store.py:165   INSERT … ON CONFLICT DO NOTHING     <- 🔴 «타깃이 없습니다»
               타깃 없는 DO NOTHING 은 «어느» 유니크 인덱스의 충돌이든 삼킵니다
```
```
CREATE INDEX   오늘 645,203 행에 충돌이 있으면 «생성이 실패»합니다
               -> 생성이 «성공했다»는 것이 이 데이터에 충돌이 없다는 증거입니다  ✅
INSERT         앞으로 들어올 원자가 세 다이제스트 «전부» 같고 occurred_at·predicate·
               subject_type·translator_ver 까지 같으면 «조용히 버려집니다»
               -> 거절이 아니라 «침묵»입니다
```
확률은 무시할 만합니다(645k 에 md5 셋이 «동시에» 충돌해야 합니다). 다만 총괄 지적대로
**「모양이 다릅니다」** — 「불가능」이 아니라 «무시할 만함»이 정직한 낱말입니다.
재지 않고 「안전」이라 적지 말라는 그 게이트가 제 문장 하나를 고쳤습니다.

## 📌 그리고 제 첫 EXPLAIN 이 «엉뚱한 질의»였습니다
`subject_keys @> …` 로 재서 Seq Scan 이 나왔고, 그건 이 인덱스와 «무관한» 질의였습니다.
walk 이 실제로 읽는 축(subject_type, subject_keys 동등)과 «새 키의 유일성 조회» 둘로 바꾸니
Index Only Scan / Index Scan 이 나왔습니다. 「말하려는 끝에서 재라」 그대로입니다.

지시서의 «작업 중» 한 줄은 지웠습니다. 다음은 ② (구설계 삭제) 로 갑니다.
