...
> 최초 측정 `pid 66708 · 08:06:59` 기동본 = 경로 113 · 오퍼레이션 117.
> 🔴 **그 뒤 구 그래프 분기가 «삭제·재기동»됐습니다** (`8ffe23d7` + `45d8b66f`) —
> 지금은 **오퍼레이션 110**, `/graph/**` 여섯과 `/api/graph/sync` 가 «없습니다».
> 아래 표는 «삭제 전» 사진이고, 그 일곱 줄만 빼고 읽으십시오.
...
| ~~`/graph/**`~~ | ~~6~~ | ⚰️ **삭제됨** (2026-09-04). 지웠더니 200-HTML 이 되는 것이 아래 ①의 증거입니다 |
...
주어·술어·목적어로 원자를 «찾는» 자리
   ...
   그걸 Query 로 받는 라우트가 «0». walk 은 씨앗(`id`)이 필수라 우회 불가
...
- 🔴 **walk 이 「키 부분이 같은 다른 노드」를 표현하지 못합니다** (구현자 실측 2026-09-04).
  은퇴한 `/api/ledger/siblings` 가 `scope=<axis>:<value>` 로 그걸 답했으므로,
  «있다가 없어진» 능력입니다. 소유자 판정 대기.
