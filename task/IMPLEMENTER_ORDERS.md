...
```
사고 이력   대상이 «설비·챔버»이고 «시간 구간»을 가집니다 (시점 아님 — 구간이어야 겹침이 생깁니다)
            종류·심각도·짧은 서술. 10~20건이면 충분합니다
...
table_config      새 표를 카탈로그에 등록하는 것은 «필요»합니다 (그건 소유자 파일이므로
                  «무엇을 넣어야 하는지 보고»만 하고 총괄이 넣겠습니다)
