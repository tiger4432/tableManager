...
```
① 가르는 측정   선언된 기전 그래프에 «순환»이 있는가,
                또는 한 씨앗에서 한 노드에 «양방향»으로 닿는가
                  없다 -> 나눗수를 선언 dir 에서 뽑으면 끝 (총괄 읽기 성립)
                  있다 -> 그 노드에서 「걷는 방향」이 둘 -> 구현자 반론이 살아남
② 수리          체인 링크에서 안 나눈다. 비준 규칙 3조(감쇠 상수 없음)로 되돌리는 것
③ 한 착지       수리 + 수락 B «재채점» 이 «같은 커밋». 반으로 나누면 B 가 무슨 뜻인지 모르게 됨
④ 수락          「선언 엔진과 숫자가 맞는다」가 «아니라»,
                체인 길이만 다르고 «분기 수는 같은» 후보 쌍에서 순위가 뒤집히는가
                (그 쌍은 지금 픽스처에 «없음» — 이 라운드가 만든다)
```
