...
def assume_grid_from(meta: dict | None, basis_meta: dict | None,
                     basis: dict = None) -> dict | None:
    """소스 메타 + **바닥에서 빌린 격자 치수** → 계산용 사본. 빌릴 것이 없으면 None.

    🔴 **`assume_phys_from`과 같은 계급이고 같은 불변식이다** — 아무것도 쓰지 않고, 표지를
       달고, 출처를 싣는다. 다른 것은 **어느 필드를 덮는가** 하나뿐이다.

    ═══ [D10] **치수는 빌리고 원점은 빌리지 않는다** (총괄 판정 2026-08-06) ═══════════════
    🔴 종전에는 `grid_start_x/y`도 함께 덮었고, 근거는 [D5]였다: 「하나만 빌리면 평행이동이
       남고, 그 오차는 **시프트 풀이(±3)를 넘어서면** 조용한 오답이 된다」. 그 문장은 쓰인
       시점에 참이었고 지금은 **없는 기계장치에 대한 규칙**이다 — 앵커(§map_alignment
       `_anchor_shift`)가 평행이동을 탐색하지 않고 **결정한다**. 창을 안 쓰므로 창을 넘을 수
       없다. 이유가 만료된 규칙은 존중해서 보존하는 것이 아니라 **은퇴시킨다**.

    🔴 그리고 갈라짐이 **원리적이다**: 치수는 **제품의 다이 배치**에 대한 사실이라 그 제품의
       모든 맵이 공유하고, 그래서 빌리는 것이 맞다([D8]은 그대로 선다). **원점은 이 맵의
       사실**이다 — 이 맵이 고른 것이고, 확정이 알아내려는 것이 정확히 그것이다. 남의 원점을
       끼워 넣는 것은 **사칭**이고, 이 파일의 출처 어휘 전체가 막으려는 그 행위를 토큰이
       아니라 산술로 저지르는 것이다.

    🔴 실측이 그 대가를 준다(2026-08-06). 원점까지 빌리면 같은 항이 **네 곳**에서 어긋난다:
       ① 확정이 바닥의 원점을 맵에 적어 반대 방향으로 한 시프트 민다 ② 확정이 배치를 아예
       안 적는다 ③ payload는 맵의 **선언된** 메타를 싣는데 `shift`는 **빌린** 공간에서 풀려
       화면이 원점 차이만큼 어긋난다(실측: 선언 공간 231/266 대 빌린 공간 266/266)
       ④ `grid_needs_basis`가 원점까지 비교해 **확정된 원점이 다음 읽기에서 지워진다**.
       ④ 하나만으로도 결론이 난다 — 결정한 사실을 다음 채점 패스가 덮어쓰는 시스템은
       아무것도 결정하지 않은 것이다.

    ⚠️ **`grid_start_*`는 `orientation_declaration`이 채점하는 축이기도 하다.** 이제 이 함수는
       그 축을 **건드리지 않으므로** 빌린 시작이 `declared`로 보고되는 문제 자체가 사라진다.
       표지(`GRID_ASSUMED_KEY`)는 **치수에 대한** 주장으로 남는다.

    None을 돌려주는 경우 — **둘은 실패가 아니라 「할 일 없음」이다**:
      · 바닥에 읽을 격자가 없다 — 빌릴 것이 없다(호출자가 이름을 대고 거절한다).
      · 소스의 **치수**가 바닥과 이미 같다 — 덮어도 값이 안 바뀐다. 그때 표지를 달면
        일어나지 않은 빌림을 기록하게 되고, 「무엇 위에 섰나」가 과잉 주장이 된다.
        (원점이 다른 것은 더 이상 빌릴 사유가 아니다 — 그것이 [D10]이다.)
    ...
    if g is None:
        return None
    # [D10] 비교도 **치수만** 한다. 원점이 다르다는 것은 빌릴 사유가 아니다.
    if grid_dims(meta) == (g["cols"], g["rows"]):
        return None
    out = dict(meta)
    out["grid_cols"], out["grid_rows"] = g["cols"], g["rows"]
    # ═══ [D10-b] 원점은 **선언일 때만** 지킨다 ═══════════════════════════════════════════
    # [D10]의 논거는 「원점은 이 맵이 **고른** 것」이다. 그 문장이 거짓인 행이 있다:
    # 등록기가 자동 생성한 행의 `grid_start_*`는 **그 작업이 건드린 다이들의 bbox 최솟값**
    # 이고, 스펙 §9.5가 「하한이지 격자가 아니다」라고 못 박은 값이다([D8]이 치수에 대해
    # 말한 것과 **같은 문장**이 원점에도 그대로 적용된다). 아무도 고르지 않은 값이므로
    # 지킬 선택이 없고, 사칭도 성립하지 않는다.
    #
    # 🔴 **어휘를 새로 만들지 않는다.** 이 프로젝트에는 이미 그 술어의 이름이 있다 —
    #    `auto_registered` = `GEOMETRY_AUTO_REGISTERED`(「값은 있지만 선언이 아니다」).
    #    네 번째 규칙을 지어내는 대신 그 표지를 읽는다.
    #
    # 🔴 그리고 [D5]의 전제는 **이 모집단에서 만료되지 않았다**: 등록기 행·미등록 맵은
    #    순번 컬럼이 없는 경우가 많아 평행이동을 앵커가 아니라 ±3 탐색이 푼다. 합성 원점을
    #    그대로 두면 맵이 통째로 밀리고 그 오차는 창을 넘는다 — 실측 **467/467 오답**,
    #    [D5]가 적어 둔 바로 그 수이고 [D10]을 이 갈래까지 밀었을 때 스위트가 낸 수다.
    if meta.get(AUTO_REGISTERED_KEY):
