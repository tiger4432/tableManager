...
async def process_chain_transaction_group(tx_id, events, db, rules):
    ...
    #    (reads `source_name` off the EVENT payload, which a collapsed event carries
    #     at top level exactly as a per-row one does - no expansion needed to decide
    #     whether an event is ours.)
