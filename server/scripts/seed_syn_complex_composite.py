...
def verify_live():
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT predicate, count(id) FROM ledger_events "
                "WHERE source_who = %s GROUP BY predicate ORDER BY predicate", (SOURCE,))
            predicates = dict(cursor.fetchall())
            cursor.execute(
                "SELECT atoms_written, atoms_deduped, molecules_done "
                "FROM ledger_translator_cursor WHERE source = %s", (SOURCE,))
            cursor_row = cursor.fetchone()
            cursor.execute(
                "SELECT target_table, count(*) FROM wafer_map_metadata "
                "WHERE map_id LIKE 'SYN-CX-%' GROUP BY target_table ORDER BY target_table")
            meta_counts = dict(cursor.fetchall())
            cursor.execute("SELECT count(*) FROM valid_die_ref WHERE product = %s",
                           (MAP_PRODUCT,))
            valid_count = cursor.fetchone()[0]
            cursor.execute(
                "SELECT count(*) FROM bonding_log WHERE bond_lot LIKE 'SYN-CX-BOND-%'")
            bond_count = cursor.fetchone()[0]
            cursor.execute(
                "SELECT l.bond_lot, m.leg, count(*), "
                "count(*) FILTER (WHERE l.dt_lot IS NOT NULL AND l.dt_slot IS NOT NULL), "
                "count(*) FILTER (WHERE l.core_lot IS NOT NULL AND l.core_slot IS NOT NULL) "
                "FROM bonding_log l JOIN bonding_map m "
                "ON m.base=l.base_id AND m.x=l.bx AND m.y=l.by "
                "WHERE l.bond_lot LIKE 'SYN-CX-BOND-%' "
                "GROUP BY l.bond_lot, m.leg ORDER BY l.bond_lot")
            map_leg_rows = cursor.fetchall()
            cursor.execute(
                "SELECT r.base_wafer_id, b.leg, r.method, count(*) "
                "FROM inspection_run r JOIN bonding_map b "
                "ON b.base=r.base_wafer_id AND b.x=r.base_x AND b.y=r.base_y "
                "WHERE r.base_wafer_id LIKE 'SYN-CX-BW-%' "
                "GROUP BY r.base_wafer_id, b.leg, r.method "
                "ORDER BY r.base_wafer_id, b.leg, r.method")
            scan_denominators = [
                {"wafer": wafer, "bonding_leg": leg,
                 "method": method, "count": count}
                for wafer, leg, method, count in cursor.fetchall()]
            cursor.execute(
                "SELECT subject_keys->>'wafer', object_payload->>'bonding_leg' "
                "FROM ledger_events "
                "WHERE source_who=%s AND predicate='observed' "
                "AND object_payload->>'finding_kind'='void' "
                "AND object_payload->>'method'='SYN_CX_INSPECTION' "
                "GROUP BY subject_keys->>'wafer', object_payload->>'bonding_leg' "
                "ORDER BY subject_keys->>'wafer', object_payload->>'bonding_leg'",
                (SOURCE,))
            found_aggregation_units = [tuple(row) for row in cursor.fetchall()]
            cursor.execute(
                "SELECT count(*) FILTER (WHERE object_payload::text LIKE %s), "
                "count(*) FILTER (WHERE object_payload::text LIKE %s), "
                "count(*) FILTER (WHERE object_payload::text LIKE %s) "
                "FROM ledger_events WHERE source_who = %s",
                ("%answer_population%", "%root_cause_signal%", "%cause_regroup%",
                 SOURCE))
            forbidden_counts = dict(zip(
                ("answer_population", "root_cause_signal", "cause_regroup"),
                cursor.fetchone()))
        body = ledger_composition.composition(
            connection, UI_PRESETS["hero_defect"], window="90d",
            now=ACCEPTANCE_NOW)
        map_body = ledger_lots.lot_map(
            connection, row=BOND_SPATIAL_MAPS[0]["bond_lot"], kind="void",
            by="bond_lot", slot=BOND_SPATIAL_MAPS[0]["bond_slot"], window="90d",
            now=ACCEPTANCE_NOW)
        trend_body = ledger_trends.trends(
            connection, kinds="void", window="90d", limit=200,
            max_points=300, now=ACCEPTANCE_NOW)
        selection_body = ledger_selection.resolve(connection, {
            "schemaVersion": 5, "finding_kind": "void", "window": "365d",
            "selection": [
                {"id": "defect", "groupId": "A", "kind": "entity_set",
                 "identity": experiment_identity(UI_PRESETS["hero_defect"])},
                {"id": "reference", "groupId": "B", "kind": "entity_set",
                 "identity": experiment_identity(UI_PRESETS["hero_reference"])},
            ],
        }, now=ACCEPTANCE_NOW)
        coverage_body = ledger_selection.resolve(connection, {
            "schemaVersion": 5, "finding_kind": "void", "window": "365d",
            "selection": [
                {"id": f"unit-{index:02d}",
                 "groupId": "A" if chip in DEFECT_CHIPS else "B",
                 "kind": "entity_set",
                 "identity": experiment_identity(chip)}
                for index, chip in enumerate(FINAL_CHIPS)
            ],
        }, now=ACCEPTANCE_NOW)
    finally:
        connection.close()
    syn_trend_rows = [
        row for row in trend_body["table"]["rows"]
        if row["identity"]["keys"]["wafer"].startswith("SYN-CX-BW-")]
    trend_states = [next(metric["state"] for metric in row["metrics"]
                         if metric["kind"] == "void" and metric["subtype"] is None)
                    for row in syn_trend_rows]
    if (len(syn_trend_rows), trend_states.count("found"),
            trend_states.count("scanned_clean")) != (12, 6, 6):
        raise RuntimeError("experiment-unit trend is not exactly 12 = 6 found + 6 clean")
    if (sum(row["traceability"]["core"]["state"] == "ready"
            for row in syn_trend_rows) != 12):
        raise RuntimeError("not every experiment aggregation unit has ready Core trace")
    if [row["found"] for row in map_body["projections"]] != [9, 9, 9]:
        raise RuntimeError("hero VOID overlay did not survive Bond/DT/Core projection")
    selection_states = [row["state"] for row in selection_body["selections"]]
    if selection_states != ["resolved", "resolved"]:
        raise RuntimeError(f"selection A/B did not resolve: {selection_states}")
    comparison = selection_body["comparison"]
    process_facets = comparison["facets"]["process"]
    sequence_clusters = comparison["sequence"]["clusters"]
    min_sequence_steps = min((len(row["tokens"]) for row in sequence_clusters),
                             default=0)
    sequence_steps = sorted({token["step"] for row in sequence_clusters
                             for token in row["tokens"]})
    required_prefixes = {"ETCH", "CMP", "PHOTO", "CLN"}
    found_prefixes = {prefix for prefix in required_prefixes
                      if any(step.startswith(prefix + "_") for step in sequence_steps)}
    if min_sequence_steps < 20 or found_prefixes != required_prefixes:
        raise RuntimeError("selection did not retain the 20+ Core step sequence")
    void_signal = next((row for row in process_facets
                        ...
                        and row["signature"].get("recipe") ==
                            process_recipe("HBM", "B", "ETCH_PATTERN_02")), None)
    if not void_signal:
        raise RuntimeError("HBM-B ETCH recipe is absent from selection facets")
    signal_groups = {row["group_id"]: row for row in void_signal["groups"]}
    if not (signal_groups["A"]["frequency"] > 0
            and signal_groups["B"]["frequency"] == 0
            and void_signal["evidence_ids"]):
        raise RuntimeError("HBM-B ETCH recipe lacks A/B frequency or evidence")
    common_facets = sum(all(group["frequency"] and group["frequency"] > 0
                            for group in row["groups"])
                        for row in process_facets)
    if not common_facets:
        raise RuntimeError("selection has no shared process facets to collapse")
    measurement_facets = coverage_body["comparison"]["facets"]["measurement"]
    if not measurement_facets or measurement_facets[0].get("state") == "absent":
        raise RuntimeError("6v6 selection has no canonical measured candidates")
    measurement_excursion = next((row for row in measurement_facets
                                  if row.get("signature", {}).get("metric") == "etched_cd"
                                  and row.get("signature", {}).get("recipe") ==
                                      process_recipe("HBM", "B", "ETCH_PATTERN_02")), None)
    if not measurement_excursion:
        raise RuntimeError("HBM-B etched-CD measurement candidate is absent")
    measurement_groups = {row["group_id"]: row
                          for row in measurement_excursion["groups"]}
    if (not any(item["value"] == 48.8
                for item in measurement_groups["A"]["values"])
            or measurement_groups["B"]["values"]
            or not measurement_excursion["evidence_ids"]
            or len(measurement_groups["A"]["wafer_mark_keys"]) != 6
            or len(measurement_groups["B"]["wafer_mark_keys"]) != 6):
        raise RuntimeError("measurement excursion lacks 6v6 values/evidence/marks")
    shared_measurement = next((row for row in measurement_facets
                               if all(group["values"] for group in row["groups"])
                               and set(item["value"] for item in row["groups"][0]["values"])
                               & set(item["value"] for item in row["groups"][1]["values"])), None)
    if not shared_measurement:
        raise RuntimeError("measurement candidates contain no common A/B value")
    measurement_states = {
        state for row in measurement_facets for group in row["groups"]
        for state, count in group["state_counts"].items() if count}
    if not {"missing", "not_performed", "unknown"} <= measurement_states:
        raise RuntimeError(f"measurement absence states lost: {measurement_states}")
    coverage_states = [row["state"] for row in coverage_body["selections"]]
    if coverage_states != ["resolved"] * len(FINAL_CHIPS):
        raise RuntimeError(f"12-unit selection coverage failed: {coverage_states}")
    stage_payload_counts = defaultdict(int)
    stage_unique_maps = defaultdict(set)
    unit_map_coverage = []
    for answer in coverage_body["selections"]:
        identity = answer["aggregation_units"][0]
        unit = {"wafer": identity["keys"]["wafer"],
                "bonding_leg": identity["context"]["bonding_leg"]}
        scoped = [row for row in answer["maps"]
                  if row.get("subject_wafer") == unit["wafer"]
                  and row.get("subject_leg") == unit["bonding_leg"]]
        by_stage = defaultdict(set)
        for row in scoped:
            map_id = row["frame"]["mapId"]
            by_stage[row["stage"]].add(map_id)
            stage_payload_counts[row["stage"]] += 1
            stage_unique_maps[row["stage"]].add(map_id)
            if set(row["layers"]) != {
                    "valid_die", "process_area", "used_area",
                    "supply_material", "defect"}:
                raise RuntimeError(f"incomplete map payload layers: {row['id']}")
            for layer in ("valid_die", "process_area", "used_area",
                          "supply_material"):
                if row["layers"][layer]["state"] != "ready" or not row["layers"][layer]["cells"]:
                    raise RuntimeError(f"empty {layer} payload: {row['id']}")
        core_families = {
            family for family in ("LOGIC", "HBM")
            if any(f"SYN-CX-{family}-MRG_" in map_id
                   for map_id in by_stage["core"])}
        if (len(by_stage["bond"]) < 1 or len(by_stage["dt"]) < 2
                or len(by_stage["core"]) < 2
                or core_families != {"LOGIC", "HBM"}):
            raise RuntimeError(f"stage/family map coverage missing for {unit}: {by_stage}")
        unit_map_coverage.append({
            "wafer": unit["wafer"], "bonding_leg": unit["bonding_leg"],
            "stage_map_counts": {stage: len(by_stage[stage])
                                 for stage in ("bond", "dt", "core")},
            "core_families": sorted(core_families)})
    map_spec_by_lot = {spec["bond_lot"]: spec for spec in BOND_SPATIAL_MAPS}
    map_leg_bridges = []
    for lot, leg, cells, dt_cells, core_cells in map_leg_rows:
        spec = map_spec_by_lot[lot]
        if leg != spec["bonding_leg"] or (cells, dt_cells, core_cells) != (64, 64, 64):
            raise RuntimeError(f"map->leg->DT/Core bridge broken for {lot}")
        map_leg_bridges.append({
            "bond_lot": lot, "wafer": final_wafer(spec["chip"]),
            "bonding_leg": leg, "final_chip_id": spec["chip"],
            "cells": cells, "dt_resolved": dt_cells, "core_resolved": core_cells})
    def measurement_summary(row):
        return {
            "signature": row["signature"], "state": row["state"],
            "groups": [{
                "group_id": group["group_id"], "state": group["state"],
                "state_counts": group["state_counts"], "count": group["count"],
                "total": group["total"],
                "values": [{"value": value["value"], "count": value["count"]}
                           for value in group["values"]],
                "wafer_mark_count": len(group["wafer_mark_keys"]),
                "evidence_count": len(group["evidence_ids"])}
                for group in row["groups"]],
            "wafer_mark_count": len(row["wafer_mark_keys"]),
            "evidence_count": len(row["evidence_ids"]),
            "evidence_ids": row["evidence_ids"][:5],
        }
    return {
        "predicates": predicates,
        "persisted_answer_shortcuts": forbidden_counts,
        "cursor": {"atoms_written": cursor_row[0], "atoms_deduped": cursor_row[1],
                   "molecules_done": cursor_row[2]} if cursor_row else None,
        "composition": {
            "state": body["state"],
            "component_count": (body.get("summary") or {}).get("component_count"),
            "dt_collection_count": (body.get("summary") or {}).get("dt_collection_count"),
            "core_types": (body.get("summary") or {}).get("core_types"),
            "components_with_process": sum(
                bool(row["upstream_process"]["evidence_ids"])
                for row in body.get("components") or [])},
        "spatial": {
            "metadata_by_table": meta_counts, "valid_die_cells": valid_count,
            "bonding_cells": bond_count,
            "lot_map_state": map_body["state"],
            "projections": [{"axis": p["axis"], "state": p["state"],
                             "cells": len(p["cells"]), "found": p["found"],
                             "valid_die_map_id": ((p.get("frame") or {})
                                                  .get("valid_die_ref") or {}).get("map_id")}
                            for p in map_body["projections"]]},
        "trend_population": {
            "scan_denominators": scan_denominators,
            "found_aggregation_units": [
                {"wafer": wafer, "bonding_leg": leg}
                for wafer, leg in found_aggregation_units],
            "scanned_clean_aggregation_units": [
                {"wafer": wafer, "bonding_leg": leg}
                for wafer, leg in sorted(
                    {(row["wafer"], row["bonding_leg"])
                     for row in scan_denominators if row["method"] == "sat"}
                    - set(found_aggregation_units))]},
        "trend_route": {
            "state": trend_body["state"], "syn_rows": len(syn_trend_rows),
            "found": trend_states.count("found"),
            "scanned_clean": trend_states.count("scanned_clean"),
            "core_ready": sum(row["traceability"]["core"]["state"] == "ready"
                              for row in syn_trend_rows),
            "dt_partial": sum(row["traceability"]["dt"]["state"] == "partial"
                              for row in syn_trend_rows),
            "example_identity": syn_trend_rows[0]["identity"]},
        "selection_route": {
            "state": selection_body["state"], "selection_states": selection_states,
            "process_facets": len(process_facets),
            "shared_process_facets": common_facets,
            "measurement_facet_count": len(comparison["facets"]["measurement"]),
            "sequence_clusters": len(sequence_clusters),
            "minimum_sequence_steps": min_sequence_steps,
            "step_prefixes": sorted(found_prefixes),
            "void_signal": {
                "signature": void_signal["signature"],
                "groups": void_signal["groups"],
                "evidence_ids": void_signal["evidence_ids"],
                "surprise": void_signal["surprise"]}},
        "population_measurement": {
            "groups": {row["group_id"]: {
                "component_count": row["component_count"],
                "aggregation_unit_count": row["aggregation_unit_count"],
                "measurement": row["measurement"]}
                for row in coverage_body["comparison"]["groups"]},
            "facet_count": len(measurement_facets),
            "excursion": measurement_summary(measurement_excursion),
            "shared": measurement_summary(shared_measurement),
            "states": sorted(measurement_states)},
        "all_aggregation_unit_map_coverage": {
            "resolved_units": len(unit_map_coverage),
            "stage_payload_counts": dict(sorted(stage_payload_counts.items())),
            "stage_unique_map_counts": {
                stage: len(map_ids)
                for stage, map_ids in sorted(stage_unique_maps.items())},
            "units": unit_map_coverage},
        "map_leg_bridge": map_leg_bridges,
    }
...
def main():
    ...
    if args.rollback:
        ...
        return
    if args.verify_live:
        print(json.dumps(verify_live(), ensure_ascii=False, indent=2, default=str))
        return
    atoms = build_atoms()
    key = validate(atoms)
    if args.answer_key:
