"""TEMPORAL TEXT contract -- the shim that puts it in the DEFAULT SUITE.

The contract itself is `contracts/temporal_text/test_temporal_text_contract.py`, scored
against `contracts/temporal_text/vectors.json`. This file owns no seam assertions: it
...
🔴 `-rs` IS PART OF THE COMMAND, and here it matters more than usual. The postgres half
CANNOT run on the suite dialect -- SQLite compiles `temporal_text_sql` to a plain
`CAST(ts AS VARCHAR)`, the arm that never had the defect -- so on a bare `server/tests/`
run the load-bearing half of this contract is SKIPPED, and a bare `-q` reports that as an
anonymous "N skipped". To score it:

    ASSY_CONTRACT_PG_URL=postgresql://... conda run -n assy_manager python -m pytest \
        server/tests/test_temporal_text_contract.py -q -rs
...
_CONTRACT_DIR = pathlib.Path(__file__).resolve().parents[2] / "contracts" / "temporal_text"
_CONTRACT_FILE = _CONTRACT_DIR / "test_temporal_text_contract.py"
...
if not _CONTRACT_FILE.exists():
    raise RuntimeError(
        f"the temporal text contract is not where this shim expects it: {_CONTRACT_FILE}\n"
...
pytest.register_assert_rewrite("test_temporal_text_contract")

import importlib.util as _ilu  # noqa: E402

# Loaded by PATH, not by name: this shim and the contract share a module basename, so a
# plain `import test_temporal_text_contract` resolves to whichever `sys.path` entry wins -
# and under `pytest server/tests/` that is this file, which would silently re-export
# itself and score nothing.
_spec = _ilu.spec_from_file_location("_temporal_text_contract_impl", _CONTRACT_FILE)
_contract = _ilu.module_from_spec(_spec)
sys.modules["_temporal_text_contract_impl"] = _contract
...
def test_shim_reexports_every_contract_test():
    ...
    assert here == _CONTRACT_TESTS, (
        ...
        "Seam assertions belong in contracts/temporal_text/, never here.")
