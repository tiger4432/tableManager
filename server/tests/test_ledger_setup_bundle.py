...
# ---------------------------------------------------------------- frames in the grammar
def framed(entity_extra=None, frame=None):
    """The plant, with a coordinate on the input entity and a frame on its source."""
    value = logical_bundle()
    entity = value["entities"]["InputEntity@1"]
    # `{}` means "declare no coordinate" and must not fall through to the default -
    # `or` would take the default and V4 would never get its precondition.
    entity.update({"frame_keys": ["x", "y"]} if entity_extra is None else entity_extra)
    if frame is not None:
        value["sources"][next(iter(value["sources"]))]["frame"] = frame
    return value


LEGAL_FRAME = {"InputEntity@1": {"x": {"from": "y", "sign": -1, "offset": 7},
                                 "y": {"from": "x", "sign": 1, "offset": 0}}}


def test_a_declared_frame_is_accepted_and_its_absence_is_still_the_normal_case():
    """Both fields are optional; a plant that declares neither is untouched."""
    assert not [item for item in validate_bundle_errors(logical_bundle())
                if item.code in ("invalid_frame", "unknown_field")]
    assert not [item for item in validate_bundle_errors(framed(frame=LEGAL_FRAME))
                if item.code in ("invalid_frame", "unknown_field")]


def test_each_way_a_frame_can_silently_do_nothing_is_refused_by_name():
    """🔴 EVERY ONE OF THESE IS OTHERWISE SILENT, which is why each is a named refusal.

    A frame that names a key the entity does not have transforms nothing; one that is not a
    permutation cannot be inverted, and the walk's descent inverts it; a float offset turns
    a stored `1` into `1.0` and matches no row. None of them raise, and each renders exactly
    like "no frame declared yet".
    """
    # V1 - the out-keys are the coordinate, exactly
    assert issue(framed(frame={"InputEntity@1": {
        "z": {"from": "y", "sign": 1, "offset": 0},
        "y": {"from": "x", "sign": 1, "offset": 0}}}), "invalid_frame")
    # V2 - invertibility
    assert issue(framed(frame={"InputEntity@1": {
        "x": {"from": "x", "sign": 1, "offset": 0},
        "y": {"from": "x", "sign": 1, "offset": 0}}}), "invalid_frame")
    # V3 - a float offset, and a sign that is neither +1 nor -1
    assert issue(framed(frame={"InputEntity@1": {
        "x": {"from": "y", "sign": -1, "offset": 7.0},
        "y": {"from": "x", "sign": 1, "offset": 0}}}), "invalid_frame")
    assert issue(framed(frame={"InputEntity@1": {
        "x": {"from": "y", "sign": 0, "offset": 7},
        "y": {"from": "x", "sign": 1, "offset": 0}}}), "invalid_frame")
    # V4 - a frame on a type that declares no coordinate can never fire
    assert issue(framed(entity_extra={}, frame=LEGAL_FRAME), "invalid_frame")
