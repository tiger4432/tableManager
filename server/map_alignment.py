...
def confirmed_meta_for(meta: dict | None, basis_meta: dict | None, basis: dict,
    ...
    🔴 종전 이 갈래는 격자를 손대지 않았고, 근거는 「격자는 확정된 사실이 아니다」였다. 그
       ...
       있고**(은퇴한 자동 등록기가 쓴 행들이 그렇다), 그것은 스펙 §9.5가 「하한이지 격자가 아니다」라고 못 박은
    ...
    # 🔴 [2026-08-07 · 시도했다가 되돌림] **`side`를 `front`로 내리는 것은 안 된다.**
    #    이 축의 `back`이 **거울**(`x → −x`)이고 물리 뒷면이 아니라는 것은 맞다(사용자 확인).
    #    그래서 「채점만 거울로 하고 기록은 front」로 바꿔 봤고, 근거는 **거울이 이미
    #    `grid_start_*`에 흡수돼 있다**는 것이었다 — 원점을 정렬이 알아냈고 그 정렬이 거울
    #    프레임 아래서 돌았으니까. **그 근거가 거짓이다.**
    #
    #    `test_the_written_start_is_where_the_editor_redraws_it`가 값으로 답했다:
    #        114 of 120 cells open in the editor on a different die (dx=-4, dy=0)
    #        written start=(8,1), the source actually ran at (8,1)
    #    **start는 같은데 셀이 x로 4칸 밀린다.** 즉 거울은 원점이 아니라 **`side` 그 자체가
    #    나른다.** 지우면 읽는 쪽이 다른 다이에 앉힌다.
    #
    #    ⚠️ 저장된 행 하나의 메타를 손으로 `front`로 바꿔 화면이 멀쩡했던 것은 반증이 아니다
    #    — 그 맵의 `phys_offset_*`가 0이고 회전이 상쇄 자리에 앉으면 어긋남이 안 보인다.
    #    이 함수가 만드는 것은 **모든 프레임**에 대한 값이다.
    #
    #    📌 남는 문제는 실재한다: 이 칸이 **「거울 좌표계」와 「물리 뒷면」 두 뜻을 겸한다.**
    #    맵 에디터엔 진짜 뒷면이 있으므로 혼동도 실재한다. 다만 답은 이 대입을 바꾸는 것이
    #    아니라 **거울을 담을 자리를 따로 만드는 것**이고, 그건 읽는 쪽(`_side_of` ·
    #    `_frame_phys_params` · 편집기 · 오버레이) 전부가 걸리는 별도 라운드다.
    #    화면 라벨(`우상단 시작`)은 이미 그 뜻을 사람에게 말한다.
