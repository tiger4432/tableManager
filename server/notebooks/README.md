...
`server/parsers/notebooks/` 에 파서 워크벤치가 하나 있었다. **2026-09-12 에 지워졌다** —
같은 기능에 두 경로가 있으면 갈라지고, 갈라진 쪽은 오류를 안 낸다.

그 노트북이 «더 잘 하던 것 둘»은 지우기 «전»에 벤치 함수로 올렸다:
`frame_delta`(가공이 무엇을 바꿨나 — 모양이 숨기는 것) · `value_types`(인서터가 받는 값의
타입, 평범하지 않은 것 경고). 셀 구조가 **다시 쓰인** 이유는 소유자가 요구한 자유폼 넷
(raw → read_df → process → publish)이고, 옛 것은 초안 파서를 «문자열»로 쓰는 모양이었다.
